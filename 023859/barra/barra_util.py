...
def calc_profit(trading_days, strategy_list, root_dict, ZZ1000_buy_amt_daily, deviation, vote_num):
    ...
    # yesterday_buy_info_by_industry = pd.DataFrame({
    #     'sw_industry_code_1': ZZ1000_groupby_industry.loc[pd.to_datetime(start_date_)].index,
    #     'total_buy': 0
    # })
    # yesterday_sell_info_by_industry = pd.DataFrame({
    #     'sw_industry_code_1': ZZ1000_groupby_industry.loc[pd.to_datetime(start_date_)].index,
    #     'total_sell': 0
    ...
    yesterday_buy_info_by_ticker = pd.DataFrame({
        'act_buy_amt': 0,
        'beta': df_strategy_sign_sw1.loc[pd.to_datetime(start_date_),'CNLTS_BETA'],
        'sw_industry_code_1_t-1': df_strategy_sign_sw1.loc[pd.to_datetime(start_date_),'sw_industry_code_1']
    })
    yesterday_sell_info_by_ticker = pd.DataFrame({
        'act_sell_amt': 0,
        'beta': ZZ1000_sw_weight_and_price.loc[pd.to_datetime(start_date_),'CNLTS_BETA'],
        'sw_industry_code_1_t-1': ZZ1000_sw_weight_and_price.loc[pd.to_datetime(start_date_)]['sw_industry_code_1']
    })
    ...
    for date in trading_days:
        ...
        if date not in df_strategy_sign_sw1.index.get_level_values(0).unique():
            ...
            yesterday_buy_info_by_industry = yesterday_buy_info_by_industry.rename(columns={'total_buy': 'yesterday_total_buy'})
            industry_weight_info = pd.merge(yesterday_buy_info_by_industry, ZZ1000_groupby_industry.loc[date, 'weight'].reset_index(), on='sw_industry_code_1', how='right')
            ...
            # yesterday_buy_info_by_industry = pd.DataFrame({
            #     'sw_industry_code_1': ZZ1000_groupby_industry.loc[pd.to_datetime(date)].index,
            #     'total_buy': 0
            # })
            # yesterday_sell_info_by_industry = pd.DataFrame({
            #     'sw_industry_code_1': ZZ1000_groupby_industry.loc[pd.to_datetime(date)].index,
            ...
            yesterday_buy_info_by_ticker = pd.DataFrame({
                'act_buy_amt': 0,
                'beta': df_strategy_sign_sw1.loc[pd.to_datetime(start_date_), 'CNLTS_BETA'],
                'sw_industry_code_1_t-1': df_strategy_sign_sw1.loc[pd.to_datetime(date)]['sw_industry_code_1']
            })
            yesterday_sell_info_by_ticker = pd.DataFrame({
                'act_sell_amt': 0,
                'beta': ZZ1000_sw_weight_and_price.loc[pd.to_datetime(date), 'CNLTS_BETA'],
                'sw_industry_code_1_t-1': ZZ1000_sw_weight_and_price.loc[pd.to_datetime(date)]['sw_industry_code_1']
            })
            continue
        # 组合的行业每日规模限制
        ...
        sell_info_date = pd.concat([zz1000_sw_weight_and_price_date[['buy_amt','sw_industry_code_1']], yesterday_sell_info_by_ticker[['act_sell_amt','sw_industry_code_1_t-1']]], axis=1)
        sell_info_date[['buy_amt','act_sell_amt']] = sell_info_date[['buy_amt','act_sell_amt']].fillna(0)
        sell_info_date['sell_capacity'] = (sell_info_date['buy_amt'] - sell_info_date['act_sell_amt']).clip_lower(0)
        sell_capacity_dict = sell_info_date['sell_capacity'].to_dict()
        industry_sell_capacity = sell_info_date.groupby('sw_industry_code_1')['sell_capacity'].sum()
        industry_sell_capacity_dict = industry_sell_capacity.to_dict()

        df_strategy_sign_sw1_date = df_strategy_sign_sw1.loc[date]

        portfolio_date = pd.concat([df_strategy_sign_sw1_date[['vote_sum_pred','sw_industry_code_1']],zz1000_sw_weight_and_price_date[['vote_sum_pred','sw_industry_code_1']]])
        ...
        for j in industries:
            yesterday_buy_industry = yesterday_buy_info_by_ticker[yesterday_buy_info_by_ticker['sw_industry_code_1_t-1'] == j]['act_buy_amt'].sum()
        ...
        # beta风格因子限制 高beta股票不要买太多
        prob += pulp.lpSum([portfolio_date.loc[i,'CNLTS_BETA']*buy_vars[i]+yesterday_buy_info_by_ticker.loc[i,'act_buy_amt']*yesterday_buy_info_by_ticker.loc[i,'CNLTS_BETA'] for i in portfolio_date['var']]) <= 1.5*ZZ1000_buy_amt_daily,'Max_Beta'

        # 每日总买入额度限制
        prob += pulp.lpSum([buy_vars[i] for i in df_strategy_sign_sw1_date['var']]) <= (ZZ1000_buy_amt_daily - yesterday_buy_info_by_ticker['act_buy_amt'].sum()), 'Max_Daily_Buy'
        prob.solve()
        if pulp.LpStatus[prob.status] != 'Optimal':
            print(f'求解状态：{pulp.LpStatus[prob.status]}')
            raise Exception('线性规划问题未能找到最优解')
        # 确定买入端
        df_strategy_sign_sw1_date['act_buy_amt'] = [pulp.value(buy_vars[i]) for i in ZZ1000_sw_weight_and_price_date.index]
        df_strategy_sign_sw1_date = pd.merge(df_strategy_sign_sw1_date.reset_index().drop('act_buy_amt', axis=1),df_strategy_sign_sw1_date.groupby(['vote_sum_pred', 'sw_industry_code_1'])['act_buy_amt'].mean().reset_index(),on=['sw_industry_code_1', 'vote_sum_pred'])
        ...
        ZZ1000_sw_weight_and_price_date.loc[date] = [pulp.value(buy_vars[i]) for i in df_strategy_sign_sw1_date.index]
        ...
        #         if date == pd.to_datetime('20211213'):
        #             import pdb
        #             pdb.set_trace()
