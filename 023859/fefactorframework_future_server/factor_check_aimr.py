...
def format_check(factor_name, factor_type, check_date, before_submit_name_list):
    ...
    try:
        modname = 'factor_lib.factor_%d.factor_%s' % (check_date, factor_name)
...
def run_func_test(func,factor_name, factor_type, basic_file_path,factor_data_path):
    ...
    run_start_date,run_end_date=20220801,20250430
...
def pre_check(factor_name, factor_type, check_date, check_res_path, before_submit_name_list,basic_file_path,factor_data_path,filter_df,filter_df_2225):
    check_dic = {'代码格式检查': '', '因子值一致性检查': '', '函数运行检查': '', '因子测试' : '','预检测':'not pass'}

    check_dic['代码格式检查'], func = format_check(factor_name, factor_type, check_date, before_submit_name_list)
...
check_res_path = '/data/user/023859/factor_zooF/all_factor_check/' # qyh:这里修改
if not os.path.exists(check_res_path): os.makedirs(check_res_path)
basic_file_path = '/data/user/023859/factor_zooF/factor_lib/Basic_future_20220801_20250430.h5' # '/data/group/800463/project/project1_prod/left_v2212/Basic_zt_test/Basic_zt_001.h5'
label_file_path = '/data/user/023859/factor_zooF/factor_lib/sft_basic_formal_20220801_20250430.h5' # /data/group/800463/project/project1_prod/left_v2212/Label_zt_test/Label_zt_001.h5
factor_data_path = '/data/user/023859/factor_zooF/all_factor/' # /data/user/018107/factor_zoo1/europa_factor_ezt/，qyh: 这里要改
data_path_dic= {
...
for index, inf in factor_inf.iterrows():
    ...
    factor_name, factor_type, check_date, factor_owner = inf['factor_name'], inf['factor_type'], inf['提交时间'], inf['factor_owner']
    if factor_type not in ['T-1_factor', 'TTickabAll', 'TTransaction', 'TOrder',
                           ...
                           'TEmotion', 'T-1_Emotion','other','TTickabAll']:
    ...
    print('%s,%s,%s'%( factor_name,factor_type,check_date))
    before_submit_name_list = all_factor_inf.loc[:index]['factor_name'].to_list()[:-1]
    check_ser = pre_check(factor_name, factor_type, check_date, check_res_path, before_submit_name_list,basic_file_path,factor_data_path,all_df,filter_df_2225) # qyh:倒数第二个变量如果有filter_df要改
    print(check_ser, '\n')
