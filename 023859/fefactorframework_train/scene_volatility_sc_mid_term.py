...
update_xlsx = False
scene_factor_name = 'tsq_newneptune_sc_scene_volatility_mid_term' # 分场景因子名称
...
minutes = ['m955','m1055','m1325','m1425']
period_dict = {
    'ret_t_1330_t_1430':['m1325','m1425'],
    'ret_t_1100_t_1330':['m1055','m1325'],
    'ret_t_1000_t_1100':['m955','m1055'],
    'ret_t-1_1430_t_1000':['last_m1425','m955'],
}
...
for i, period in enumerate(period_dict):
    ...
    basic_file[f'factor_ret_{i + 1}'] = md[period].unstack().shift((i+1) // 4).stack()
    basic_file[f'factor_ret_{i + 5}'] = md[period].unstack().shift((i+1) // 4 + 1).stack()
    basic_file[f'factor_ret_{i + 9}'] = md[period].unstack().shift((i+1) // 4 + 2).stack()
    basic_file[f'factor_ret_{i + 13}'] = md[period].unstack().shift((i+1) // 4 + 3).stack()
    basic_file[f'factor_ret_{i + 17}'] = md[period].unstack().shift((i+1) // 4 + 4).stack()
    basic_file[f'factor_ret_{i + 21}'] = md[period].unstack().shift((i+1) // 4 + 5).stack()
