...
class factor_xbc_md_20250407_16(BaseFactor):
    strategy_name = "neptune"
    factor_name = "xbc_md_20250407_16"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "xbc"  # 开发人员姓名
    factor_explain = "close的最大值与最小值" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            md_data['max'] = md_data['close'].unstack().rolling(80, 1).max().stack()
            md_data['near'] = md_data['close'].unstack().rolling(1, 1).mean().stack()
            md_data[factor_name] = (md_data['max'] - md_data['near'])/(md_data['max'] + md_data['near'])
            # -------------------------------------------------------------------------------------------------------------------
