...
class factor_xbc_md_20250421_6(BaseFactor):
    strategy_name = "neptune"
    factor_name = "xbc_md_20250421_6"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "xbc"  # 开发人员姓名
    factor_explain = "high减Low的滚动平均" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            def to_discrete(x, sort_fatocr):
                new_x = x.sort_values(sort_fatocr, ascending=True, inplace=False)
                num_not_nan = new_x.dropna().size
                new_x['socre'] = list(range(num_not_nan)) + [None] * (new_x.shape[0] - num_not_nan)
                new_x['socre'] = new_x['socre'] / new_x['socre'].shape[0] * 100
                new_x = new_x.reset_index().set_index('Ticker')
                return new_x
            ...
            md_data['stat'] = md_data['high'] - md_data['low']
            md_data2 = md_data[['stat']].groupby(level=[0]).apply(lambda x: to_discrete(x, 'stat'))
            factor_df[factor_name] = (md_data2['stat']).unstack().rolling(2, min_periods=1).mean().stack()
            md_data[factor_name] = factor_df[factor_name] / md_data['open']
            # -------------------------------------------------------------------------------------------------------------------
