...
class factor_xbc_md_20250514_3(BaseFactor):
    strategy_name = "neptune"
    factor_name = "xbc_md_20250514_3"
    fill_na_value = 1
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['s1'] = (df_ori['high'] - df_ori['close'])/ df_ori['pre_close'] / df_ori['turn']
            df_ori['s2'] = (df_ori['open'] - df_ori['low'])/ df_ori['pre_close'] / df_ori['turn']
            df_ori['factor'] = df_ori['s1'] - df_ori['s2']
            df_ori[self.factor_name] = df_ori['factor'].unstack().rolling(2,1).apply(f_calc_std).stack() - \
                                       df_ori['factor'].unstack().rolling(6,1).apply(f_calc_std).stack().apply(lambda x : round_(x,5)).replace(0,np.nan)
