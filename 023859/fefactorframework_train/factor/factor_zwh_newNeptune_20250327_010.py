...
class factor_zwh_newNeptune_20250327_010(BaseFactor):
    owner = 'zwh'
    strategy_name = "neptune"
    factor_name = "zwh_newNeptune_20250327_010"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    factor_explain = "价格波动" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        high = (df['open'] - df['close']) / (df['high'] - df['low'] + EPS)

        N = 28
        res = high.unstack().rolling(N, min_periods=N).std().stack()
