...
class factor_tsq_newneptune_20250320_3(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250320_3"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "MACD" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['factor'] = df_ori['adjclose'].unstack().rolling(10,1).mean().stack() - df_ori['adjclose'].unstack().rolling(20,1).mean().stack()
