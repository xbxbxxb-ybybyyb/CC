...
class factor_tsq_newneptune_20250403_21(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250403_21"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "收盘价差排序" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['delta_close'] = df_ori['close'] - df_ori['close'].unstack().shift(1).stack()
            df_ori['delta_close_10_min'] = df_ori['delta_close'].unstack().rolling(10,1).min().stack()
            df_ori['delta_close_10_max'] = df_ori['delta_close'].unstack().rolling(10,1).max().stack()
            df_ori['factor'] = -df_ori['delta_close']
            df_ori.loc[df_ori['delta_close_10_min']>0,'factor'] = df_ori.loc[df_ori['delta_close_10_min']>0,'delta_close']
            df_ori.loc[df_ori['delta_close_10_max']<0,'factor'] = df_ori.loc[df_ori['delta_close_10_max']<0,'delta_close']
