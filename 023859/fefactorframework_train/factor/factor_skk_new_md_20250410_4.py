...
class factor_skk_new_md_20250410_4(BaseFactor):
    strategy_name = "neptune"
    factor_name = "skk_new_md_20250410_4"  # 以这个来保存文件名
    fill_na_value = 216.0
    need_pre_calculate_T_N = True
    owner = "skk"  # 开发人员姓名
    factor_explain = "收益率30日逆序数" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        def count_nixushu(X):
            # print(1111111111111111111111, X)
            X = list(X)
            nixushu = 0
            for i in range(len(X) - 1):
                ...
                for xx in y:
                    if x > xx:
                        nixushu += 1
                # print(111111111, x, y)
            # print(22222222222222222222222, nixushu)
            return nixushu
        ...
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            #####################
            df_ori[self.factor_name] = df_ori['pct_chg'].unstack().rolling(30, min_periods=1).apply(lambda x: count_nixushu(x)).stack()  #
            #############
