...
class factor_skk_new_md_20250410_3(BaseFactor):
    strategy_name = "neptune"
    factor_name = "skk_new_md_20250410_3"  # 以这个来保存文件名
    fill_na_value = 115.0
    need_pre_calculate_T_N = True
    owner = "skk"  # 开发人员姓名
    factor_explain = "收益率22日逆序数" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            #####################
            df_ori[self.factor_name] = df_ori['pct_chg'].unstack().rolling(22, min_periods=1).apply(lambda x: count_nixushu(x)).stack()  #
            #############
