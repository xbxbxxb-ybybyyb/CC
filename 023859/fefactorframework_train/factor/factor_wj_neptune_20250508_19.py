...
class factor_wj_neptune_20250508_19(BaseFactor):
    strategy_name = "neptune"
    factor_name = "wj_neptune_20250508_19"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "wj"  # 开发人员姓名
    factor_explain = "中户量差(仅主动)(手)，5日最大值" # 因子逻辑解释
    ...
    t_1_factor_data = [
        {'name': 'AShareMoneyFlow',  # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 80,  # 注意为正数
         'column': ['VOLUME_DIFF_MED_TRADER_ACT','BUY_VOLUME_EXLARGE_ORDER','SELL_VOLUME_EXLARGE_ORDER']
         }]  # T-N factor数据，格式如上
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True:  # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['long'] = (df_ori['VOLUME_DIFF_MED_TRADER_ACT']).unstack().rolling(5,
                                                                                                            1).apply(
