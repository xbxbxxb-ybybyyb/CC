...
class factor_tsq_newneptune_20250403_44(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250403_44"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "收益率排序与调整后成交量排序之积" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['close5'] = df_ori['close'].unstack().rolling(5,1).mean().stack()
            df_ori['factor1'] = (df_ori['high'] - df_ori['low'])/df_ori['close5'].replace(0,np.nan)
            df_ori['factor1'] = df_ori['factor1'] - df_ori['factor1'].unstack().shift(1).stack().fillna(0)
            ...
            df_ori['factor3'] = (df_ori['high'] - df_ori['low']) * (df_ori['vwap'] - df_ori['close'])/df_ori['close5'].replace(0,np.nan)
            df_ori['factor2'] = df_ori['factor2']/df_ori['factor3'].replace(0,np.nan)
