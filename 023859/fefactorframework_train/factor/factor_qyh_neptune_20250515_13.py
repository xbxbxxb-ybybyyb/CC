...
class factor_qyh_neptune_20250515_13(BaseFactor):
    strategy_name = "neptune"
    factor_name = "qyh_neptune_20250515_13"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "qyh"  # 开发人员姓名
    factor_explain = "财务-ACCT_RCV 单季度" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            res = df_balancesheet.groupby(['dt','Ticker'])['ACCT_RCV_diff'].apply(lambda x : f_calc_cct(x.tail(4))).to_frame(name = self.factor_name)
