...
class factor_xly_neptune_t_1_b5(BaseFactor):
    strategy_name = "neptune"
    factor_name = "xly_neptune_t_1_b5"
    fill_na_value = 0
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True:
            ...
            try:
                tick_df_period1 = tick_df[(tick_df['MDTime'] >= 93000000)&(tick_df['MDTime'] < 100000000)]
                tick_df_period2 = tick_df[(tick_df['MDTime'] >= 143000000)&(tick_df['MDTime'] < 145700000)]
                res = (tick_df_period1['Buy1Price'].diff()>0).astype(int).mean() - (tick_df_period2['Buy1Price'].diff()>0).astype(int).mean()
                if np.isinf(res):
                    res = 1
            except:
