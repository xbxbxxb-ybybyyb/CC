...
class factor_xly_neptune_t_1_b6(BaseFactor):
    strategy_name = "neptune"
    factor_name = "xly_neptune_t_1_b6"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "xly"  # 开发人员姓名
    factor_explain = "价格变动" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True:
            ...
            try:
                if is_zcz:
                    tick_df['Buy1Price'] = tick_df['Buy1Price']/2
                tick_df_period1 = tick_df[(tick_df['MDTime'] >= 93000000) & (tick_df['MDTime'] <= 113000000)]
                tick_df_period2 = tick_df[(tick_df['MDTime'] >= 130000000) & (tick_df['MDTime'] < 145700000)]
                res = (tick_df_period1['Buy1Price'].mean()-tick_df_period2['Buy1Price'].mean())/pre_close
                if np.isinf(res):
                    res = 1
            except:
