...
class factor_wj_neptune_20250424_3(BaseFactor):
    strategy_name = "neptune"
    factor_name = "wj_neptune_20250424_3"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "wj"  # 开发人员姓名
    factor_explain = "大单尾盘资金流入率，60日变异系数" # 因子逻辑解释
    ...
    t_1_factor_data = [
        {'name': 'AShareMoneyFlow',  # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 80,  # 注意为正数
         'column': ['CLOSE_NET_INFLOW_RATE_VOLUME_L']
         }]  # T-N factor数据，格式如上
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True:  # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['long'] = (df_ori['CLOSE_NET_INFLOW_RATE_VOLUME_L']).unstack().rolling(60, 3).apply(lambda x: np.mean(x)/(1e-3+np.std(x))).stack()
