...
class factor_xbc_md_20250407_3(BaseFactor):
    strategy_name = "neptune"
    factor_name = "xbc_md_20250407_3"
    fill_na_value = 0
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            md_data['stat'] =  (md_data['close'] - md_data['vwap'])*100000/md_data['pre_close']
            market_mean = md_data['stat'].unstack().mean(axis=1)
            factor_df = pd.DataFrame()
            factor_df[factor_name] =  (md_data['stat'] -(market_mean)).unstack().rolling(3).mean().stack()
            condition = (((factor_df.reset_index()["dt"] >= '20200824') & (factor_df.reset_index()["Ticker"].apply(lambda x:x[0:2]=='30')))|((factor_df.reset_index()["Ticker"].apply(lambda x:x[0:2]=='68')))).values
