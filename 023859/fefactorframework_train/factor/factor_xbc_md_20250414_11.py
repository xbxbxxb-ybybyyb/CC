...
class factor_xbc_md_20250414_11(BaseFactor):
    strategy_name = "neptune"
    factor_name = "xbc_md_20250414_11"
    fill_na_value = 0
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            md_data[factor_name] = md_data['stat'].unstack().rolling(2, min_periods=1).agg(lambda rows: rows[0]).stack()
            # -------------------------------------------------------------------------------------------------------------------
