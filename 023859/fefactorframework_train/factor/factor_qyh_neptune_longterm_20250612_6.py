...
class factor_qyh_neptune_longterm_20250612_6(BaseFactor):
    strategy_name = "neptune"
    factor_name = "qyh_neptune_longterm_20250612_6"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "qyh"  # 开发人员姓名
    factor_explain = "vwap absdiff" # 因子逻辑解释
    zcz_adjusted = "是" # 是否针对注册制调整：是/否
    logic_type = "长周期-价格形态" # 逻辑类别
    low_cost = "是" # 是否低耗时
    #
    t_day_data = []
    t_1_factor_data = [
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            # -------------------------------------------------------------------------------------------------------------------
            def f_calc_absdiffsum(factor_series):
                factor_series_notnan = factor_series[~np.isnan(factor_series)]
                factor_series_notnan_s = np.insert(factor_series_notnan[:-1], 0, 0)
                diff = factor_series_notnan - factor_series_notnan_s
                res = abs(diff[~np.isnan(diff)][1:]).sum()
                return res
            df_ori['factor'] = df_ori['vwap'] / df_ori['pre_close'] * df_ori['amt']
            df_ori[self.factor_name] = df_ori['factor'].unstack().rolling(10,1).apply(f_calc_absdiffsum).stack() / df_ori['amt'].unstack().rolling(10,1).apply(f_calc_absdiffsum).stack()
            df_ori[self.factor_name] = df_ori[self.factor_name].replace(np.inf,np.nan).replace(-np.inf,np.nan)
            # -------------------------------------------------------------------------------------------------------------------
