...
class factor_tsq_newneptune_20250410_30(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250410_30"
    fill_na_value = 0.25
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            # -------------------------------------------------------------------------------------------------------------------
            df_ori['vwap_mean'] = (df_ori['vwap'].unstack().rolling(10,1).mean().stack()).apply(lambda x : round_(x,5))
            df_ori['factor1'] = df_ori['open'] - df_ori['vwap_mean']
            df_ori['factor1'] = df_ori['factor1'].unstack().rank(axis=1,pct=True).stack()
            df_ori['factor2'] = df_ori['close'] - df_ori['vwap']
            df_ori['factor2'] = df_ori['factor2'].unstack().rank(axis=1,pct=True).stack()
