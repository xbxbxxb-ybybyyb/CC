...
class factor_tsq_newneptune_20250522_7(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250522_7"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "财务-应收/应付账款及票据 max/min" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            res = (df_balancesheet['ACCOUNTS_RECEIVABLE_BILL'] / (1e3+df_balancesheet['ACCOUNTS_PAYABLE'])).groupby(['dt','Ticker']).apply(lambda x : x.tail(16).max()/(1e-3+x.tail(16).min())).to_frame(name = self.factor_name)
