...
class factor_tsq_newneptune_20250327_10(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250327_10"
    fill_na_value = 1
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "20日最大异常成交量" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['vol_mean'] = df_ori['volume'].unstack().rolling(20,1).mean().stack()
            df_ori['vol_to_vol_mean'] = df_ori['volume']/df_ori['vol_mean'].replace(0,np.nan)
            df_ori['factor'] = df_ori['vol_to_vol_mean'].unstack().rolling(20,1).max().stack()
