...
class factor_tsq_newneptune_20250403_38(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250403_38"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "收盘价时序排序的截面排序乘价差排序" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['close_ts_rank'] = df_ori['close'].unstack().apply(lambda col: bn.move_rank(col.values, window=10)).stack().apply(lambda x: round_(x, 5))
            df_ori['close_open'] = df_ori['close']/df_ori['close']
            df_ori['factor1'] = (df_ori['close_ts_rank'].unstack().rank(axis=1) / (~ df_ori['close_ts_rank'].unstack().isnull()).values.sum(axis=1).reshape(-1, 1)).stack()
            df_ori['factor2'] = (df_ori['close_open'].unstack().rank(axis=1) / (~ df_ori['close_open'].unstack().isnull()).values.sum(axis=1).reshape(-1, 1)).stack()
