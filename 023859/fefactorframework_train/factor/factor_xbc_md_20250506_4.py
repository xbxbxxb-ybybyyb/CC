...
class factor_xbc_md_20250506_4(BaseFactor):
    strategy_name = "neptune"
    factor_name = "xbc_md_20250506_4"
    fill_na_value = 1
    need_pre_calculate_T_N = True
    owner = "xbc"  # 开发人员姓名
    factor_explain = "high low open 的滚动平均" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['s1'] = (df_ori['high'] - df_ori['open']) / df_ori['pre_close']
            df_ori['s2'] = (df_ori['close'] - df_ori['low']) / df_ori['pre_close']
            df_ori['factor'] = df_ori['s1'] - df_ori['s2']
            df_ori[self.factor_name] = df_ori['factor'].unstack().rolling(8,1).apply(f_calc_avg).stack() + \
                                       df_ori['factor'].unstack().rolling(25,1).apply(f_calc_avg).stack().apply(lambda x : round_(x,5)).replace(0,np.nan)
