...
class factor_tsq_newneptune_20250320_4(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250320_4"
    fill_na_value = 50
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "RSI" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['RS_up'] = df_ori['pct_chg'].unstack().rolling(20).apply(lambda x: x[x>0].mean()).stack().apply(lambda x: round_(x, 2)).fillna(0)
            df_ori['RS_down'] = df_ori['pct_chg'].unstack().rolling(20).apply(lambda x: x[x<0].mean()).stack().apply(lambda x: round_(x, 2)).fillna(0)
            df_ori['RS'] = df_ori['RS_up'] / (1e-6 + df_ori['RS_down'])
            df_ori['factor'] = 100-(100/(1+df_ori['RS']))
