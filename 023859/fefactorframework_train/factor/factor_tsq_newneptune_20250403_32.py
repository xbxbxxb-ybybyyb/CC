...
class factor_tsq_newneptune_20250403_32(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250403_32"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "价格倒数排序与价差排序的关系" # 因子逻辑解释
    ...
    t_1_factor_data = [
        {'name': 'MD_CHINA_STOCK_DAILY_WIND',  # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 260,  # 注意为正数
         'column': ['pre_close','adjfactor','open','close','high','vwap','volume']
         }
    ]  # T-N factor数据，格式如上
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['high5'] = df_ori['high'].unstack().rolling(5, 1).mean().stack()
            df_ori['vwap_'] = df_ori['vwap'] - df_ori['vwap'].unstack().shift(5).stack()
            df_ori['factor1'] = 1 / df_ori['close'].replace(0,np.nan)
            df_ori['factor1'] = (df_ori['factor1'].unstack().rank(axis=1) / (~ df_ori['factor1'].unstack().isnull()).values.sum(axis=1).reshape(-1, 1)).stack()
            df_ori['factor2'] = df_ori['high'] - df_ori['close']
            df_ori['factor2'] = (df_ori['factor2'].unstack().rank(axis=1) / (~ df_ori['factor2'].unstack().isnull()).values.sum(axis=1).reshape(-1, 1)).stack()
            df_ori['factor3'] = (df_ori['vwap_'].unstack().rank(axis=1) / (~ df_ori['vwap_'].unstack().isnull()).values.sum(axis=1).reshape(-1, 1)).stack()
            df_ori['factor'] = df_ori['factor1']*df_ori['factor2']*df_ori['volume']/df_ori['volume20']*df_ori['high']/df_ori['high5'].replace(0,np.nan) - df_ori['factor3']
