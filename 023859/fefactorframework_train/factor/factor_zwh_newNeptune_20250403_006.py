...
class factor_zwh_newNeptune_20250403_006(BaseFactor):
    owner = 'zwh'
    strategy_name = "neptune"
    factor_name = "zwh_newNeptune_20250403_006"
    fill_na_value = 0
    ...
    t_1_factor_data = [
        {'name': 'MD_CHINA_STOCK_DAILY_WIND',  # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 100,  # 注意为正数
         'column': ['pct_chg','turn', 'high', 'close', 'vwap', 'pre_close', 'adjfactor']
         }]
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        amt_s = df['pct_chg'].unstack().rolling(40, min_periods=5).mean().stack()
        amt_s *= df['turn'].unstack().rolling(10, min_periods=5).mean().stack()
        res = amt_s
