...
class factor_xbc_md_20250522_1(BaseFactor):
    strategy_name = "neptune"
    factor_name = "xbc_md_20250522_1"
    fill_na_value = 1
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['high'] = df_ori['high'].unstack().rolling(6,1).apply(f_calc_avg).stack()
            ...
            df_ori['open'] = df_ori['open'].unstack().rolling(6,1).apply(f_calc_avg).stack()
            ...
            df_ori[self.factor_name] = df_ori['factor'].unstack().rolling(2,1).apply(f_calc_avg).stack()+df_ori['factor'].unstack().rolling(1,1).apply(f_calc_avg).stack()
