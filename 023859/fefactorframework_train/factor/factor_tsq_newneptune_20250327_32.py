...
class factor_tsq_newneptune_20250327_32(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250327_32"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "尾盘博弈强度" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['last_range'] = ((df_ori['close'] - df_ori['low'])/(df_ori['high'] - df_ori['low']).replace(0,np.nan)).apply(lambda x: round_(x, 5))
            df_ori['volume_10'] = df_ori['volume'].unstack().rolling(10,1).mean().stack().apply(lambda x: round_(x, 5))
            df_ori['factor'] = (df_ori['last_range'] * np.log(df_ori['volume']/df_ori['volume_10']) * np.sign(df_ori['close'] - df_ori['open'])).apply(lambda x: round_(x, 5))
            df_ori['factor'] = df_ori['factor'].unstack().rolling(10,1).mean().stack()
