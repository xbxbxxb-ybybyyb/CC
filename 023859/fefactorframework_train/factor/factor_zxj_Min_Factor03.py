...
class factor_zxj_Min_Factor03(BaseFactor):
    strategy_name = "neptune"
    factor_name = "zxj_Min_Factor03"
    fill_na_value = 0
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        def calculate_dzej_for_stock(stock_day_df: pd.DataFrame) -> float:
            trading_data = stock_day_df[stock_day_df['datetime'].dt.time >= time(9, 46)].copy()
        ...
        daily_data = database['xdb_tick1m']
        if daily_data.empty:
            database['pre_T_N'] = pd.DataFrame(columns=[self.factor_name])
            return database
