...
class factor_tsq_newneptune_20250327_21(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250327_21"
    fill_na_value = 0
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['factor'] = df_ori['TRADES_COUNT'].unstack().rolling(60,1).std().stack() / df_ori['TRADES_COUNT'].unstack().rolling(60,1).mean().stack().replace(0,np.nan)
