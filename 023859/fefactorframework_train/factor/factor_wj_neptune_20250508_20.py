...
class factor_wj_neptune_20250508_20(BaseFactor):
    strategy_name = "neptune"
    factor_name = "wj_neptune_20250508_20"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "wj"  # 开发人员姓名
    factor_explain = "净流入量(手)，20日标准差" # 因子逻辑解释
    ...
    t_1_factor_data = [
        {'name': 'AShareMoneyFlow',  # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 80,  # 注意为正数
         'column': ['S_MFD_INFLOWVOLUME','BUY_VOLUME_EXLARGE_ORDER','SELL_VOLUME_EXLARGE_ORDER']
         }]  # T-N factor数据，格式如上
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True:  # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['long'] = (df_ori['S_MFD_INFLOWVOLUME'].diff()).unstack().rolling(20,
                                                                                                            1).apply(
