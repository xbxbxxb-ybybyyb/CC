...
class factor_wj_neptune_20250424_2(BaseFactor):
    strategy_name = "neptune"
    factor_name = "wj_neptune_20250424_2"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "wj"  # 开发人员姓名
    factor_explain = "大单尾盘资金流入率，10天最大值" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True:  # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['long'] = (df_ori['CLOSE_NET_INFLOW_RATE_VOLUME_L']).unstack().rolling(10, 3).apply(lambda x: np.max(x)).stack()
