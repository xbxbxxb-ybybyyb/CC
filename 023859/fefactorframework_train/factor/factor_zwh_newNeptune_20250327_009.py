...
class factor_zwh_newNeptune_20250327_009(BaseFactor):
    owner = 'zwh'
    strategy_name = "neptune"
    factor_name = "zwh_newNeptune_20250327_009"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    factor_explain = "价格形态" # 因子逻辑解释
    ...
    t_1_factor_data = [
        {'name': 'MD_CHINA_STOCK_DAILY_WIND',  # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 100,  # 注意为正数
         'column': ['close', 'high', 'turn', 'vwap', 'pre_close', 'adjfactor']
         }]
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        high = (df['high'] - df['close']) / (EPS+ df['vwap'] )
        N = 40
        nn = 5
        res = high.unstack().rolling(N, min_periods=N).median().stack() + high.unstack().rolling(nn, min_periods=nn).median().stack()
