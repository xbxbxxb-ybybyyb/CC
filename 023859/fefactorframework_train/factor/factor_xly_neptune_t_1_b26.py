...
class factor_xly_neptune_t_1_b26(BaseFactor):
    strategy_name = "neptune"
    factor_name = "xly_neptune_t_1_b26"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "xly"  # 开发人员姓名
    factor_explain = "买卖相关性" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True:
            ...
            try:
                tick_df_period = tick_df[(tick_df['MDTime'] >= 143000000) & (tick_df['MDTime'] < 145700000)]
                res = tick_df_period['Buy1OrderQty'].diff().fillna(0).corr(tick_df_period['Sell1OrderQty'].diff().fillna(0))
                if np.isinf(res):
                    res = 1
            except:
