...
update_xlsx = True
scene_factor_name = 'tsq_newneptune_sa_scene_volatility_short_term' # 分场景因子名称
...
minutes = ['m1425','m1325','m1055','m955']
period_dict = {
    'ret_t-1_1100_t_1000':['last_m1055','m955'],
    'ret_t-1_1330_t_1100':['last_m1325','m1055'],
    'ret_t-1_1430_t_1330':['last_m1425','m1325'],
    'ret_t_1000_t_1430':['m955','m1425'],
}
...
md = md[md['amt']>0]
close_px = IO.read_data([20160101,end_date],columns=minutes, alt='/data/group/800463/data/generalStrong/minute5/close.h5')
md = md.join(close_px)
...
for i, period in enumerate(period_dict):
    ...
    basic_file[f'factor_ret_{i + 1}'] = md[period].unstack().shift(i).stack()
    basic_file[f'factor_ret_{i + 5}'] = md[period].unstack().shift(i+3).stack()
    basic_file[f'factor_ret_{i + 9}'] = md[period].unstack().shift(i+6).stack()
    basic_file[f'factor_ret_{i + 13}'] = md[period].unstack().shift(i+9).stack()
    basic_file[f'factor_ret_{i + 17}'] = md[period].unstack().shift(i+12).stack()
    basic_file[f'factor_ret_{i + 21}'] = md[period].unstack().shift(i+15).stack()
...
if update_xlsx:
    scene_factor_bank_inf_sa = pd.read_excel('/data/user/023859/factor_zooZZ/scene_factor_inf_sa.xlsx')
