...
factor_version_date = 20250627 # 策略迭代版本
start_date, end_date = 20170110, 20211231 # 数据起始日期
strategy_path = os.path.join('/dfs/user/023859/neptune',str(factor_version_date)) # 策略数据存储路径
basic_file_path = f'/dfs/user/023859/neptune/{factor_version_date}/basic_file_zz1000_sa_20170110_20241231.pkl' # zz1000 sa时点基础样本
factor_bank_inf_path = os.path.join(strategy_path,'factor_bank_inf_sa.xlsx') # 可用因子列表
...
t_factor_path = '/dfs/user/018107/share_file/for_tsq/20250626_zz1000_sa_factor_value_20170101_20211231.pkl'
emotion_factor_path = f'/dfs/user/023859/neptune/{factor_version_date}/index_emotion_factors/20170110_20241231' # 情绪因子路径
...
# 读取分场景因子
# factor_list.append(pd.read_hdf(os.path.join(scene_volatility_factor_path,'tsq_newneptune_sc_index_scene_volatility.h5')).loc[pd.Timestamp(str(start_date)):pd.Timestamp(str(end_date))])
# factor_list.append(pd.read_hdf(os.path.join(scene_volatility_factor_path,'tsq_newneptune_sc_scene_volatility_long_term.h5')).loc[pd.Timestamp(str(start_date)):pd.Timestamp(str(end_date))])
# factor_list.append(pd.read_hdf(os.path.join(scene_volatility_factor_path,'tsq_newneptune_sc_scene_volatility_mid_term.h5')).loc[pd.Timestamp(str(start_date)):pd.Timestamp(str(end_date))])
factor_list.append(pd.read_hdf(os.path.join(scene_volatility_factor_path,'tsq_newneptune_sa_scene_volatility_short_term.h5')).loc[pd.Timestamp(str(start_date)):pd.Timestamp(str(end_date))])
# factor_list.append(pd.read_hdf(os.path.join(scene_swing_factor_path,'tsq_newneptune_sc_index_scene_swing.h5')).loc[pd.Timestamp(str(start_date)):pd.Timestamp(str(end_date))])
factor_list.append(pd.read_hdf(os.path.join(scene_swing_factor_path,'tsq_newneptune_sa_scene_swing_short_term.h5')).loc[pd.Timestamp(str(start_date)):pd.Timestamp(str(end_date))])
...
# 原始因子值
all_factor_df.to_pickle(os.path.join(strategy_path, f'factor_df_sa_{start_date}_{end_date}.pkl'))