...
md['swing_t-3'] = md['swing'].groupby('Ticker').shift(3)

basic_file['swing_t-1'] = md['swing_t-1']
basic_file['swing_t-2'] = md['swing_t-2']
basic_file['swing_t-3'] = md['swing_t-3']

basic_file['tsq_newneptune_s1_scene_swing'] = basic_file[['swing_t-1','swing_t-2','swing_t-3']].mean(axis=1)
basic_file['tsq_newneptune_s1_scene_swing'] = basic_file['tsq_newneptune_s1_scene_swing'].fillna(0)

if update_xlsx:
    scene_factor_bank_inf_s1 = pd.read_excel('/data/user/023859/factor_zooZZ/scene_factor_inf_s1.xlsx')
    scene_factor_bank_inf_append = pd.DataFrame({'factor_name':['tsq_newneptune_s1_scene_swing'],'factor_type':["['T-1_Factor']"],'factor_owner':['tsq'],'提交时间':['20250527'],'emotion':[""], 't':['T-1']})
...
IO.pd_hdf5_writer(basic_file[['tsq_newneptune_s1_scene_swing']], hdf5=os.path.join(res_path,'tsq_newneptune_s1_scene_swing.h5'), dataset='neptune')
