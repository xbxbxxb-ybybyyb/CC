...
factor_data_path='/dfs/user/023859/factor_zooZZ/all_factor'
factor_test_path='/dfs/user/023859/factor_zooZZ/all_factor_test_backup/all_scene'


result_path = '%s/%s/%d_%d/'%(factor_test_path,s_xx, start_date, end_date)
if not os.path.exists(result_path):
    os.makedirs(result_path)


sft = FactorTest(start_date, end_date, style_list=['free_turn', 'T_o2pre'],
                 test_sample='bt', cal_mi=True,
                 buy_type='0' + s_xx if len(s_xx) == 3 else s_xx,
                 sell_type='v',lzt_pattern=[3,4])
