...
def generate_neptune_basic_stocks_pool(start_date, end_date, md, t='s1'):
...
price_931_path = f'/dfs/user/023859/neptune/931_price'
price_1430_path = f'/dfs/user/023859/neptune/1430_price'
# 读取更长区间的全市场数据
...
index_available_dict_s1 = generate_neptune_basic_stocks_pool(start_date, end_date, md_,t='s1')
mi_tuples = []
for date, stock_list in index_available_dict_s1.items():
...
neptune_basic_file_s1 = md_filtered[['list_len','Circu_Mkt']]

price_931 = []
filenames = os.listdir(price_931_path)
for file in filenames:
    if file.endswith('.pkl'):
        price_931.append(pd.read_pickle(os.path.join(price_931_path,file)))
df_price_931 = pd.concat(price_931)
neptune_basic_file_s1 = neptune_basic_file_s1.join(df_price_931)
neptune_basic_file_s1['931_is_zdt'] = ((neptune_basic_file_s1['price_931']==neptune_basic_file_s1['ul_price']) | (neptune_basic_file_s1['price_931']==neptune_basic_file_s1['dl_price']))
neptune_basic_file_s1 = neptune_basic_file_s1[neptune_basic_file_s1['931_is_zdt']==0]
neptune_basic_file_s1[['list_len','Circu_Mkt']].to_pickle(md_path+f'basic_file_zz1000_s1_{start_date}_{end_date}.pkl')

index_available_dict_sc = generate_neptune_basic_stocks_pool(start_date, end_date, md_,t='sc')
mi_tuples = []
for date, stock_list in index_available_dict_sc.items():
    for stock in stock_list:
        mi_tuples.append((pd.to_datetime(date), stock))
filter_idx = pd.MultiIndex.from_tuples(mi_tuples, names=md_.index.names)
md_filtered = md_.loc[md_.index.intersection(filter_idx)]
md_filtered = md_filtered.sort_index()
neptune_basic_file_sc = md_filtered[['list_len','Circu_Mkt']]
price_1430 = []
filenames = os.listdir(price_1430_path)
for file in filenames:
    if file.endswith('.pkl'):
        price_1430.append(pd.read_pickle(os.path.join(price_1430_path,file)))
df_price_1430 = pd.concat(price_1430)
neptune_basic_file_sc = neptune_basic_file_sc.join(df_price_1430)
neptune_basic_file_sc['1430_is_zdt'] = ((neptune_basic_file_sc['1430_price']==neptune_basic_file_sc['ul_price']) | (neptune_basic_file_sc['1430_price']==neptune_basic_file_sc['dl_price']))
neptune_basic_file_sc = neptune_basic_file_sc[neptune_basic_file_sc['1430_is_zdt']==0]
neptune_basic_file_sc[['list_len','Circu_Mkt']].to_pickle(md_path+f'basic_file_zz1000_sc_{start_date}_{end_date}.pkl')