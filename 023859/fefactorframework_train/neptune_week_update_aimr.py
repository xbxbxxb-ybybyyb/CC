...
print(param)
param_list=param.split(';')
start_date=int(param_list[0])
end_date=int(param_list[1])
strategy=param_list[2]
dock_pool_num=int(param_list[3])
factor_name_list=param_list[4].split('-')

output_dir = f'/dfs/user/023859/neptune/20250428/{start_date}_{end_date}/'
os.makedirs(output_dir,exist_ok=True)
df = pd.read_excel('/dfs/user/023859/neptune/20250428/factor_bank_inf.xlsx')
# filtered_df = df[df['factor_name']==factor_name]
filtered_df = df[df['factor_name'].isin(factor_name_list)]
filtered_df = filtered_df.sort_values(by='factor_type')
