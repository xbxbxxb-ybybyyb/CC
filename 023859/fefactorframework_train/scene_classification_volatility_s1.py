...
md['t-2_pre_close_open'] = md.groupby('Ticker')['pre_close_open'].shift(2)

volatility_factor_df = basic_file.join(md[periods_t_1+periods_t_2+periods_t_3+['t_pre_close_open','t-1_pre_close_open','t-2_pre_close_open']])
volatility_factor_df['tsq_newneptune_s1_scene_volatility'] = volatility_factor_df[periods_t_1+periods_t_2+periods_t_3+['t_pre_close_open','t-1_pre_close_open','t-2_pre_close_open']].std(axis=1)
volatility_factor_df['tsq_newneptune_s1_scene_volatility'] = volatility_factor_df['tsq_newneptune_s1_scene_volatility'].fillna(0)

if update_xlsx:
    scene_factor_bank_inf_s1 = pd.read_excel('/data/user/023859/factor_zooZZ/scene_factor_inf_s1.xlsx')
    scene_factor_bank_inf_append = pd.DataFrame({'factor_name':['tsq_newneptune_s1_scene_volatility'],'factor_type':"['MarketLast3Tick']",'factor_owner':['tsq'],'提交时间':['20250527'],'emotion':[""], 't':['T-1']})
...
IO.pd_hdf5_writer(volatility_factor_df[['tsq_newneptune_s1_scene_volatility']], hdf5=f'/dfs/user/023859/neptune/{strategy_version}/scene_factors_volatility/{start_date}_{end_date}/tsq_newneptune_s1_scene_volatility.h5', dataset='neptune')
