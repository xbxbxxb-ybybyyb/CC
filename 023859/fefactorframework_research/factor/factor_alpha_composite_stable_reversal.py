...
class factor_alpha_composite_stable_reversal(BaseFactor): # TODO py文件名称要和类名称相同
    strategy_name = "neptune"
    factor_name = "alpha_composite_stable_reversal"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "zxj"  # 开发人员姓名  TODO 填你的名字缩写，zxj
    factor_explain = "" # TODO 简要填写因子逻辑，如20日收益率排名与10日成交量标准差排名加权和
    ...
    def pre_calculate_T_N_data(self, database):
        if database.get("skip", False):
            ...
            def f_calc_std(factor_series):
                return np.std(factor_series[~np.isnan(factor_series)], ddof=1) # TODO 标准差计算可使用该函数
            # --- 参数定义 ---
            window_088 = 20
            window_097 = 10
            w_reversal = 0.5
            ...
            factor_reversal = -1 * ((close/close.shift(window_088)-1) * 100) # TODO 收益率计算可使用这种写法
            factor_stability = -1 * (volume.rolling(window=window_097).apply(f_calc_std)) # TODO 标准差计算函数
