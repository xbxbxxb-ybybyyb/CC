...
class factor_tsq_newneptune_20250417_3(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250417_3"
    fill_na_value = 0
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['factor'] = ((df_minute5_high[columns] - df_minute5_low[columns]).div(df_minute5_high['pre_close'].replace(0,np.nan),axis=0) - 1).std(axis=1).apply(lambda x : round_(x,5))
            df_ori['factor'] = df_ori['factor'].unstack().rolling(20,1).apply(f_calc_cv).stack()
