...
class factor_tsq_newsaturn_20250320_1(BaseFactor):
    # 以下为因子基本信息
    strategy_name = "saturn"
    factor_name = "tsq_newsaturn_20250320_1"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "卖单撤单时间差偏度" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True:
            return database
        data = database['xdb_cancel']
        data = filter_930(data)
        data_all = []
        for date in data.index.get_level_values(0).unique():
            data_ = data.loc[date]
            data_ = generate_time_delta_930(data_)
            data_sell = data_[data_['OrderBSFlag'] == 2]
            data_sell['time_delta'] = data_sell['MDTime_delta'].diff()
            data_all.append(data_sell[['time_delta']])
        data_all = pd.concat(data_all)
        res = data_all['time_delta'].skew()
