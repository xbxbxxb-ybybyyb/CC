...
class factor_skew_ret_v1(BaseFactor):
    ...
    def skew_ret(self,df):
        ...
        if len(filtered) > 0: # TODO
            ...
            daily_result = pd.Series(np.nan,index=data.index.get_level_values(1)) # TODO 返回索引层级不一致
        return daily_result
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        daily_result = daily_result.stack() if isinstance(daily_result, pd.DataFrame) else daily_result
        res = daily_result.groupby('Ticker',group_keys=False).mean()
        res = res.to_frame(name=self.factor_name) # TODO
