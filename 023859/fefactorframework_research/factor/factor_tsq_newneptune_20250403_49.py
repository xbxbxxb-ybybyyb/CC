...
class factor_tsq_newneptune_20250403_50(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250403_50"
    fill_na_value = 0
    need_pre_calculate_T_N = False
    owner = "tsq"  # 开发人员姓名
    factor_explain = "vwap" # 因子逻辑解释
    ...
    def calculate(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            vwap = (data['LastPx'] * data['vol']).sum() / data['vol'].sum()
            res = vwap / data['pre_close'].mean() - 1
