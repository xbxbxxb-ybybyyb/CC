...
class factor_tsq_newneptune_shortterm_20250703_2(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_shortterm_20250703_2"
    fill_na_value = 0
    need_pre_calculate_T_N = False
    owner = "tsq"  # 开发人员姓名
    factor_explain = "中大单主动买入成交占比" # 因子逻辑解释
    ...
    def calculate(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            res = data[(data['TradeMoney'] < 50000)&(data['TradeBSFlag'] == 2)]['TradeQty'].sum() / (1e-6+data[data['TradeBSFlag'] == 2]['TradeQty'].sum())
