...
class factor_tsq_newneptune_20250327_51(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250327_51"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "日内卖单挂单量标准差" # 因子逻辑解释
    ...
    t_day_data = []
    t_1_factor_data = [
        {'name': 'minute5_TotalOfferQty',  # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori = database['minute5_TotalOfferQty']
            # -------------------------------------------------------------------------------------------------------------------
