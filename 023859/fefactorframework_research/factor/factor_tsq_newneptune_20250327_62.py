...
class factor_tsq_newneptune_20250327_62(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250327_62"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "量价收敛扩散指标" # 因子逻辑解释
    ...
    t_1_factor_data = [
        {'name': 'MD_CHINA_STOCK_DAILY_WIND', # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 80, #注意为正数
         'column': ['adjfactor','high', 'low', 'close','open','pre_close','volume']
    }]  # T-N factor数据，格式如上
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['price_range'] = ((df_ori['high'] - df_ori['low']) / df_ori['pre_close'].replace(0,np.nan)).apply(lambda x: round_(x, 5))
            df_ori['volume_20'] = df_ori['volume'].unstack().rolling(20,1).mean().stack().apply(lambda x: round_(x, 5))
            df_ori['volume_range'] = (df_ori['volume'] / df_ori['volume_20'].replace(0,np.nan)).apply(lambda x: round_(x, 5))
            df_ori['price_range_std'] = df_ori['price_range'].unstack().rolling(20,5).std().stack().apply(lambda x: round_(x, 5))
            df_ori['volume_range_std'] = df_ori['volume_range'].unstack().rolling(20,5).std().stack().apply(lambda x: round_(x, 5))
            df_ori['factor'] = df_ori['price_range_std'] * df_ori['volume_range_std']
            df_ori[self.factor_name] = df_ori['factor'].apply(lambda x: round_(x, 8))
            # -------------------------------------------------------------------------------------------------------------------
