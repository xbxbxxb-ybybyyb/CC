...
class factor_tsq_newneptune_20250522_13(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250522_13"
    fill_na_value = 0
    ...
    t_1_factor_data = [
        {'name': 'MD_CHINA_STOCK_DAILY_WIND', # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 80, #注意为正数
         'column': ['open','pre_close','amt','volume','pct_chg','adjfactor']
    }]  # T-N factor数据，格式如上
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['vol2ret'] = (df_ori['volume'] / abs(df_ori['pct_chg']).replace(0,np.nan)).apply(lambda x : round_(x,5))
            df_ori['factor'] =  df_ori['vol2ret'].unstack().rolling(20,1).apply(f_calc_avg).stack() / df_ori['vol2ret'].unstack().rolling(20,1).apply(f_calc_median).stack().replace(0,np.nan)
