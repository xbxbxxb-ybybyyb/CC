...
# 卖方预测 cs
class factor_tsq_newneptune_longterm_20250619_6(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_longterm_20250619_6"
    fill_na_value = 0
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            # -------------------------------------------------------------------------------------------------------------------
            res = df_mfyc.groupby(['dt', 'Ticker']).apply(lambda x : x.tail(5)['NPADJUSTRATE'].mean()).to_frame(name=self.factor_name) #
            # -------------------------------------------------------------------------------------------------------------------
