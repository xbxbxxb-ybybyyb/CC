...
class factor_tsq_newneptune_20250327_1(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250327_1"
    fill_na_value = 1
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "KDJ指标" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            # -------------------------------------------------------------------------------------------------------------------
            df_ori['low_10'] = df_ori['low'].unstack().rolling(9,1).min().stack().apply(lambda x : round_(x,5))
            df_ori['high_10'] = df_ori['high'].unstack().rolling(9, 1).max().stack().apply(lambda x: round_(x, 5))
            df_ori['RSV'] = 100*(df_ori['close'] - df_ori['low_10'])/(df_ori['high_10'] - df_ori['low_10']).replace(0,np.nan).apply(lambda x : round_(x,5))
            df_ori['K'] = df_ori['RSV'].unstack().rolling(9,1).mean().stack().apply(lambda x : round_(x,5))
            df_ori['D'] = df_ori['K'].unstack().rolling(9,1).mean().stack().apply(lambda x: round_(x, 5))
            df_ori['J'] = 3*df_ori['K'] - 2*df_ori['D']
            df_ori['factor'] = df_ori['J'].apply(lambda x : round_(x,5))
