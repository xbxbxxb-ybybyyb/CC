...
class factor_tsq_newneptune_20250403_40(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250403_40"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "最高价与最低价平方根较vwap的涨跌幅" # 因子逻辑解释
    ...
    t_1_factor_data = [
        {'name': 'MD_CHINA_STOCK_DAILY_WIND',  # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 20,  # 注意为正数
         'column': ['pre_close','adjfactor','high','low','vwap','volume']
         }
    ]  # T-N factor数据，格式如上
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['factor'] = (np.sqrt(df_ori['high'] * df_ori['low']) - df_ori['vwap']) / df_ori['pre_close'].replace(0,np.nan)
