...
class factor_tsq_newneptune_shortterm_20250619_1(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_shortterm_20250619_1"
    fill_na_value = 0
    need_pre_calculate_T_N = False
    owner = "tsq"  # 开发人员姓名
    factor_explain = "日内动量反转" # 因子逻辑解释
    ...
    def calculate(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            if len(data):
                ...
                prices = sorted(data['LastPx'].unique())
                if len(prices) >= 2:
                    res = (prices[-1] - prices[-2])/(1e-6+prices[-1])
                else:
                    res = 0
            else:
                res = np.nan
