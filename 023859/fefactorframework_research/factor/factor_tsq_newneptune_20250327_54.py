...
class factor_tsq_newneptune_20250327_54(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250327_54"
    fill_na_value = 3
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "日内买单挂单量峰度" # 因子逻辑解释
    ...
    t_day_data = []
    t_1_factor_data = [
        {'name': 'minute5_TotalBidQty',  # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori = database['minute5_TotalBidQty']
            # -------------------------------------------------------------------------------------------------------------------
