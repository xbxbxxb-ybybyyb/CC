...
class factor_tsq_newneptune_20250327_3(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250327_3"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "价格波动与流动性冲击" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['shortcut'] = (2*(df_ori['high'] - df_ori['low'])-abs(df_ori['close']-df_ori['open'])).apply(lambda x : round_(x,2))
            df_ori['factor'] = (df_ori['shortcut'] / df_ori['volume']).unstack().rolling(60,5).std().stack()
