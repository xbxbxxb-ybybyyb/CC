...
class factor_tsq_newneptune_20250327_68(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250327_68"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "隔夜缺口修复动量" # 因子逻辑解释
    ...
    t_1_factor_data = [
        {'name': 'MD_CHINA_STOCK_DAILY_WIND', # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 20, #注意为正数
         'column': ['adjfactor','high', 'low', 'close','open','pre_close','volume','vwap']
    },
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            # -------------------------------------------------------------------------------------------------------------------
            df_volume = database['minute5_volume']
            df_ori['morning_volume'] = df_volume[['m930', 'm935', 'm940', 'm945', 'm950', 'm955', 'm1000','m1005', 'm1010', 'm1015', 'm1020', 'm1025', 'm1030', 'm1035', 'm1040','m1045', 'm1050', 'm1055', 'm1100', 'm1105', 'm1110', 'm1115', 'm1120','m1125']].sum(axis=1)
            df_ori['afternoon_volume'] = df_volume[['m1300', 'm1305', 'm1310', 'm1315', 'm1320', 'm1325', 'm1330','m1335', 'm1340', 'm1345', 'm1350', 'm1355', 'm1400', 'm1405', 'm1410','m1415', 'm1420', 'm1425', 'm1430', 'm1435', 'm1440', 'm1445', 'm1450','m1455']].sum(axis=1)
            df_ori['factor'] = ((df_ori['afternoon_volume'] - df_ori['morning_volume']) / df_ori['morning_volume'].replace(0,np.nan)).apply(lambda x: round_(x, 5))
            df_ori['factor'] = df_ori['factor']*df_ori['close'] / df_ori['vwap'].replace(0,np.nan)
