...
class factor_tsq_newneptune_shortterm_20250619_6(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_shortterm_20250619_6"
    fill_na_value = 0
    need_pre_calculate_T_N = False
    owner = "tsq"  # 开发人员姓名
    factor_explain = "主动买卖成交笔数不平衡" # 因子逻辑解释
    ...
    def calculate(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            res = (len(data[data['TradeBSFlag']==1]) - len(data[data['TradeBSFlag']==2]))/(1e-6+len(data))
