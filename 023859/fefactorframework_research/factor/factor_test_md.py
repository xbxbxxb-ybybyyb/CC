...
class factor_test_md(BaseFactor):
    ...
    t_1_factor_data = [
        {'name': 'MD_CHINA_STOCK_DAILY_WIND', # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 80, #注意为正数
         'column': ['pre_close','close','adjfactor','amt']
    }]  # T-N factor数据，格式如上
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            factor = df_ori['adjclose'].unstack().rolling(10, min_periods=3).corr(df_ori['pre_close'].unstack())
            factor = pd.DataFrame(factor.stack())
