...
class factor_tsq_newneptune_20250327_56(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250327_56"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "流动性加权ATR" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['TR1'] = (df_ori['high'] - df_ori['low']).apply(lambda x: round_(x, 5))
            df_ori['TR2'] = abs(df_ori['high'] - df_ori['pre_close']).apply(lambda x: round_(x, 5))
            df_ori['TR3'] = abs(df_ori['low'] - df_ori['pre_close']).apply(lambda x: round_(x, 5))
            df_ori['TR'] = df_ori[['TR1', 'TR2', 'TR3']].max(axis=1)
            df_ori['ATR'] = df_ori['TR'].unstack().rolling(5,1).mean().stack().apply(lambda x: round_(x, 5))
            df_ori['volume'] = df_ori['volume'].unstack().rolling(5, 1).mean().stack().apply(lambda x: round_(x, 5))
            df_ori['factor'] = (df_ori['ATR']/df_ori['volume'].replace(0,np.nan))*(df_ori['close']/df_ori['open'].replace(0,np.nan))
