...
class factor_zxj_MD_Factor05(BaseFactor):
    ...
    t_1_factor_data = [
        {'name': 'MD_CHINA_STOCK_DAILY_WIND',
         ...
         'lag': 200,
         'column': ['high', 'low', 'close','pre_close', 'pct_chg', 'amt']
    }]
    ...
    def pre_calculate_T_N_data(self, database):
        if database.get("skip", False):
            ...
            # --- 3. 计算风险调整因子 Risk_t ---
            def f_calc_mean(factor_series):
                return factor_series[~np.isnan(factor_series)].mean()

            r_rolling_mean = r_pivot.rolling(window=d, min_periods=d).apply(f_calc_mean)
            ...
            half_life = m / 2
            def rolling_ewm_mean(s, roll_window=10, ewm_span=3):
                return s.rolling(window=roll_window).apply(
                    lambda x: x.ewm(span=ewm_span).mean().iloc[-1],raw=False
                )
            composite_alpha = product_df.apply(rolling_ewm_mean, roll_window=10, ewm_span=3)
