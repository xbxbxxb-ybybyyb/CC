...
class factor_tsq_newneptune_20250327_63(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250327_63"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "非对称订单流强度" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['buy_pressure'] = (((df_ori['close']-df_ori['low'])/(df_ori['high']-df_ori['low']).replace(0,np.nan))*df_ori['volume']).apply(lambda x: round_(x, 5))
            df_ori['sell_pressure'] = (((df_ori['high']-df_ori['close'])/(df_ori['high']-df_ori['low']).replace(0,np.nan))*df_ori['volume']).apply(lambda x: round_(x, 5))
            df_ori['factor'] = (df_ori['buy_pressure'].unstack().rolling(5,1).sum().stack() - df_ori['sell_pressure'].unstack().rolling(5,1).sum().stack()) / df_ori['volume'].unstack().rolling(5,1).sum().stack().replace(0,np.nan)
