...
# 财务+MD
class factor_tsq_newneptune_caiwu_test1(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_caiwu_test1"
    fill_na_value = 0
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_balancesheet[self.factor_name] = (df_balancesheet['FIX_ASSETS'] + df_balancesheet['MONETARY_CAP']).unstack().rolling(8,1).sum().stack()
