...
class factor_tsq_newneptune_20250417_44(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250417_44"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "买单下单额与成交额占比"  # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        data = data.join(database['xdb_tick1m']['TotalValueTrade'])
        data['ValueTrade'] = data['TotalValueTrade'] - data['TotalValueTrade'].shift(1).fillna(0)
        data = filter_930(data)
        res = (data['OrderAmt_buy'] / data['ValueTrade'].replace(0,np.nan)).mean()
