...
# 一致预期
class factor_tsq_newneptune_longterm_20250612_2(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_longterm_20250612_2"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "近60日预测每股收益均值" # 因子逻辑解释
    ...
    t_1_factor_data = [
        {'name': 'DWD_EXP_FORECASTSECUDERIVED', # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 80,
         'column':['FORECASTDATE','FORECASTEPS']
    }]  # T-N factor数据，格式如上
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            # -------------------------------------------------------------------------------------------------------------------
            df_ori[self.factor_name] = df_ori['FORECASTEPS'].unstack().rolling(60,1).mean().stack().apply(lambda x: round_(x, 5))
            # -------------------------------------------------------------------------------------------------------------------
