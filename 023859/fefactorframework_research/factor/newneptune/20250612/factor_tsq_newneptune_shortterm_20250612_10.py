...
class factor_tsq_newneptune_shortterm_20250612_10(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_shortterm_20250612_10"
    fill_na_value = 0
    need_pre_calculate_T_N = False
    owner = "tsq"  # 开发人员姓名
    factor_explain = "买卖挂单量波动" # 因子逻辑解释
    ...
    def calculate(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            if len(data):
                res = (data['Buy1OrderQty'] - data['Sell1OrderQty']).std()
            else:
                res = np.nan
