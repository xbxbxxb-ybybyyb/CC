...
class factor_tsq_newneptune_shortterm_20250612_4(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_shortterm_20250612_4"
    fill_na_value = 0
    ...
    def prepare_T_data(self, database):
        tick_df = database['T1mOrder']
        database['T1mOrder'] = tick_df
    ...
    def calculate(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            Buy_Num = len(data[((data['timestamp'].diff()) > '1ms') & (data['OrderBSFlag'] == 1)])
            Sell_Num = len(data[((data['timestamp'].diff()) > '1ms') & (data['OrderBSFlag'] == 2)])
