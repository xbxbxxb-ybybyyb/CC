...
class factor_tsq_newneptune_20250327_45(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250327_45"
    fill_na_value = 100
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "CR指标" # 因子逻辑解释
    ...
    t_1_factor_data = [
        {'name': 'MD_CHINA_STOCK_DAILY_WIND', # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 260, #注意为正数
         'column': ['adjfactor','high', 'low', 'close','pre_close','amt']
    }]  # T-N factor数据，格式如上
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['HLCC'] = ((df_ori['high']+df_ori['low']+df_ori['close'])/3).apply(lambda x : round_(x,5))
            df_ori['HLCC_1'] = df_ori['HLCC'].unstack().shift(1).stack()
            df_ori['CR1'] = (df_ori['high'] - df_ori['HLCC_1']).apply(lambda x: x if x>0 else 0)
            df_ori['CR1'] = df_ori['CR1'].unstack().rolling(240,1).mean().stack().apply(lambda x : round_(x,5))
            df_ori['CR2'] = (df_ori['HLCC_1'] - df_ori['low']).apply(lambda x: x if x > 0 else 0)
            df_ori['CR2'] = df_ori['CR2'].unstack().rolling(240, 1).mean().stack().apply(lambda x: round_(x,5))
            df_ori['factor'] = 100*(df_ori['CR1'] / df_ori['CR2'].replace(0,np.nan))
