...
class factor_tsq_newneptune_20250417_1(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250417_1"
    fill_na_value = 0
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            # -------------------------------------------------------------------------------------------------------------------
            df_ori['factor'] = (df_minute5_high.div(df_minute5_high['pre_close'],axis=0)[columns] - df_minute5_low.div(df_minute5_low['pre_close'],axis=0)[columns]).std(axis=1)
            df_ori['factor'] = df_ori['factor'].unstack().rolling(60,1).max().stack()
