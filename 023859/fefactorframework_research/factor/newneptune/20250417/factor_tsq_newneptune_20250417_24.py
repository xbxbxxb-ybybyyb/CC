...
class factor_tsq_newneptune_20250417_24(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250417_24"
    fill_na_value = 0.5
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "成交量平滑"  # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        for date in data.index.get_level_values(0).unique():
            ...
            data_['Expected_Volume'] = data_['VolumeTrade'].rolling(20, min_periods=1).mean()
            data_all.append(data_)
        data_all = pd.concat(data_all)
        res = (data_all['VolumeTrade'] / data_all['Expected_Volume'].replace(0,np.nan)).mean()
