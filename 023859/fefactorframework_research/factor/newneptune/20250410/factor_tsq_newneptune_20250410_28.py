...
class factor_tsq_newneptune_20250410_28(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250410_28"
    fill_na_value = 0
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['volume_90'] = df_ori['volume'].unstack().rolling(90,1).mean().stack().apply(lambda x: round_(x, 5))
            df_ori['volume_range'] = (df_ori['volume'] / df_ori['volume_90'].replace(0,np.nan)).apply(lambda x: round_(x, 5))
            df_ori['price_range_std'] = df_ori['price_range'].unstack().rolling(90,5).std().stack().apply(lambda x: round_(x, 5))
            df_ori['volume_range_std'] = df_ori['volume_range'].unstack().rolling(90,5).std().stack().apply(lambda x: round_(x, 5))
