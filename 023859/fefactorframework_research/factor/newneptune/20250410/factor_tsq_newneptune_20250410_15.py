...
class factor_tsq_newneptune_20250410_15(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250410_15"
    fill_na_value = 0
    ...
    def calculate(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            data_mid = data[(data['TradeMoney'] >= 50000)&(data['TradeMoney'] < 200000)]
            if data_mid['TradeQty'].sum():
                res = len(data_mid[(data_mid['TradeBSFlag'] == 1)]) / len(data_mid['TradeQty'])
            else:
                res = np.nan
