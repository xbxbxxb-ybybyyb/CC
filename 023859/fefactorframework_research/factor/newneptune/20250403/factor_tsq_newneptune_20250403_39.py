...
class factor_tsq_newneptune_20250403_39(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250403_39"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "最高价标准差排序乘最高价和成交量的相关系数" # 因子逻辑解释
    ...
    t_1_factor_data = [
        {'name': 'MD_CHINA_STOCK_DAILY_WIND',  # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 20,  # 注意为正数
         'column': ['pre_close','adjfactor','high','volume']
         }
    ]  # T-N factor数据，格式如上
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['high_std'] = df_ori['high'].unstack().rolling(10,1).std().stack()
            df_ori['factor1'] = (df_ori['high_std'].unstack().rank(axis=1) / (~ df_ori['high_std'].unstack().isnull()).values.sum(axis=1).reshape(-1, 1)).stack()
            df_ori['exy'] = (df_ori['high'] * df_ori['volume']).unstack().rolling(10, 1).mean().stack().apply(lambda x: round_(x, 5))
            df_ori['ex'] = (df_ori['high']).unstack().rolling(10, 1).mean().stack().apply(lambda x: round_(x, 5))
            df_ori['ey'] = (df_ori['volume']).unstack().rolling(10, 1).mean().stack().apply(lambda x: round_(x, 5))
            df_ori['stdx'] = (df_ori['high']).unstack().rolling(10, 1).std().stack().apply(lambda x: round_(x, 5))
            df_ori['stdy'] = (df_ori['volume']).unstack().rolling(10, 1).std().stack().apply(lambda x: round_(x, 5))
