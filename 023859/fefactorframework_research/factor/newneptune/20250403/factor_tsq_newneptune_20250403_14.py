...
class factor_tsq_newneptune_20250403_14(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250403_14"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "日内收益率变异系数" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            # -------------------------------------------------------------------------------------------------------------------
            df_ori['factor'] = ((df_minute5_high[columns] - df_minute5_low[columns]).div(df_minute5_high['pre_close'].replace(0,np.nan),axis=0) - 1).std(axis=1)
            df_ori['factor'] = df_ori['factor'].unstack().rolling(20,1).std().stack() / df_ori['factor'].unstack().rolling(20,1).mean().stack().replace(0,np.nan)
