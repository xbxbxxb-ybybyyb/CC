...
class factor_amt_coverage(BaseFactor):
    strategy_name = "neptune"
    factor_name = "amt_coverage"
    fill_na_value = -1e5
    need_pre_calculate_T_N = True
    owner = "wenj"  # 开发人员姓名
    factor_explain = "成交额均线收敛因子" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            for window in windows:
                ...
                rolling_means[col_name] = df_ori.groupby('Ticker')['amt'].transform(lambda x: x.rolling(window).mean())
