...
class factor_tsq_newneptune_20250327_53(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250327_53"
    fill_na_value = 0
    ...
    t_1_factor_data = [
        {'name': 'AShareMoneyFlow', # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 80, #注意为正数
         'column': ['BUY_VALUE_SMALL_ORDER_ACT','BUY_VALUE_SMALL_ORDER']
    }]  # T-N factor数据，格式如上
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['factor'] = (df_ori['BUY_VALUE_SMALL_ORDER_ACT']/df_ori['BUY_VALUE_SMALL_ORDER'].replace(0,np.nan)).unstack().rolling(20,5).mean().stack()
