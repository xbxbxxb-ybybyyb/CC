...
class factor_sheep_corr(BaseFactor):
    strategy_name = "neptune"
    factor_name = "sheep_corr"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "wenj"  # 开发人员姓名
    factor_explain = "每日峰值成交量和后续的跟随峰值成交量的相关系数" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        sheep_corr = data[data['flag']==1][['VolumeTrade','volume_sheep']].groupby('Ticker').apply(lambda x: x.corr().iloc[0,1])


        res = sheep_corr.to_frame(self.factor_name)
        dt = data.index[0][0]
