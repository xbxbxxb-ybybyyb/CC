...
class factor_tsq_newneptune_20250327_60(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250327_60"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "机构与散户金额差波动" # 因子逻辑解释
    ...
    t_1_factor_data = [
        {'name': 'AShareMoneyFlow', # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 80, #注意为正数
         'column': ['VALUE_DIFF_INSTITUTE_ACT','VALUE_DIFF_SMALL_TRADER_ACT']
    }]  # T-N factor数据，格式如上
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['factor'] = (df_ori['VALUE_DIFF_INSTITUTE_ACT']-df_ori['VALUE_DIFF_SMALL_TRADER_ACT']).unstack().rolling(40,5).std().stack()
