...
class factor_tsq_newneptune_20250327_58(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250327_58"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "行为金融认知偏误" # 因子逻辑解释
    ...
    t_1_factor_data = [
        {'name': 'MD_CHINA_STOCK_DAILY_WIND', # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 80, #注意为正数
         'column': ['adjfactor','high', 'low', 'close','open','pre_close','volume','turn']
    }]  # T-N factor数据，格式如上
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['last_high'] = df_ori['high'].unstack().shift(1).stack()
            df_ori['last_swing'] = ((df_ori['high'] - df_ori['low']) / df_ori['pre_close'].replace(0,np.nan)).unstack().shift(1).stack().apply(lambda x: round_(x, 5))
            df_ori['overnight_ret'] = (df_ori['open']/df_ori['pre_close'] - 1).apply(lambda x: round_(x, 5))
            df_ori['factor'] = df_ori['overnight_ret']*(df_ori['last_high'] - df_ori['close'])/df_ori['last_swing'].replace(0,np.nan)
