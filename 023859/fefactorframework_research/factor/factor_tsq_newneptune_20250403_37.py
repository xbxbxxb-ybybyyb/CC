...
class factor_tsq_newneptune_20250403_37(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250403_37"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "价差截面排序的时序排序次方" # 因子逻辑解释
    ...
    t_1_factor_data = [
        {'name': 'MD_CHINA_STOCK_DAILY_WIND',  # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 40,  # 注意为正数
         'column': ['pre_close','adjfactor','high','low','open','close','vwap','volume']
         }
    ]  # T-N factor数据，格式如上
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['vwap_delta'] = df_ori['vwap'] - df_ori['vwap'].unstack().shift(3).stack().fillna(0)
            df_ori['factor1'] = (df_ori['low'] - df_ori['vwap'])/(df_ori['open'] - df_ori['midpx']).replace(0,np.nan)
            df_ori['factor1'] = df_ori['factor1'].unstack().apply(lambda col: bn.move_rank(col.values, window=10)).stack().apply(lambda x : round_(x,5))
            df_ori['factor2'] = (df_ori['vwap_delta'].unstack().rank(axis=1) / (~ df_ori['vwap_delta'].unstack().isnull()).values.sum(axis=1).reshape(-1, 1)).stack()
            df_ori['factor'] = df_ori['factor2']**df_ori['factor1']
