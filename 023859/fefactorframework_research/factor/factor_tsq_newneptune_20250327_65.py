...
class factor_tsq_newneptune_20250327_65(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250327_65"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "路径依赖效率比" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['act_distance'] = np.log(df_ori['close']).unstack().diff().abs().stack().apply(lambda x: round_(x, 5))
            df_ori['act_distance'] = df_ori['act_distance'].unstack().rolling(5,1).sum().stack().apply(lambda x: round_(x, 5))
            df_ori['straight_distance'] = np.log(df_ori['close']).unstack().diff(5).abs().stack().apply(lambda x: round_(x, 5))
            df_ori['volume_10'] = df_ori['volume'].unstack().rolling(10,1).mean().stack().apply(lambda x: round_(x, 5))
            df_ori['volume_5'] = df_ori['volume'].unstack().rolling(5, 1).mean().stack().apply(lambda x: round_(x, 5))
            df_ori['factor'] = (df_ori['act_distance'] - df_ori['straight_distance'])*df_ori['volume_10']/df_ori['volume_5']
