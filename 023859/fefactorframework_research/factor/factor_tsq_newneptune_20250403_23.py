...
class factor_tsq_newneptune_20250403_23(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250403_23"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "价格排序与成交量排序相关系数" # 因子逻辑解释
    ...
    t_1_factor_data = [
        {'name': 'MD_CHINA_STOCK_DAILY_WIND',  # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 20,  # 注意为正数
         'column': ['close','pre_close','adjfactor','volume']
         }
    ]  # T-N factor数据，格式如上
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['close_rank'] = (df_ori['close'].unstack().rank(axis=1) / (~ df_ori['close'].unstack().isnull()).values.sum(axis=1).reshape(-1, 1)).stack()
            df_ori['volume_rank'] = (df_ori['volume'].unstack().rank(axis=1) / (~ df_ori['volume'].unstack().isnull()).values.sum(axis=1).reshape(-1, 1)).stack()
            df_ori['exy'] = (df_ori['close_rank']*df_ori['volume_rank']).unstack().rolling(5, 1).mean().stack().apply(lambda x : round_(x,5))
            df_ori['ex'] = (df_ori['close_rank']).unstack().rolling(5, 1).mean().stack().apply(lambda x : round_(x,5))
            df_ori['ey'] = (df_ori['volume_rank']).unstack().rolling(5, 1).mean().stack().apply(lambda x : round_(x,5))
            df_ori['stdx'] = (df_ori['close_rank']).unstack().rolling(5, 1).std().stack().apply(lambda x : round_(x,5))
            df_ori['stdy'] = (df_ori['volume_rank']).unstack().rolling(5, 1).std().stack().apply(lambda x : round_(x,5))
