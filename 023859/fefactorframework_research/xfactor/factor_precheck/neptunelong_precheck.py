...
def value_same_check(kls, result_path, filter_df):
    ...
    try:
        if os.path.exists(
            ...
            if not db:
                db = precheck_utils.prepare_database_for_value_same_check(kls, 'neptunelong', str(long_interval[0]),
                                                                          str(long_interval[1]))
            ...
            long_df = FactorUtil.fill_factor_na_values(long_df, kls, neptunelong_basic)
            if not os.path.exists(result_path):
                os.makedirs(result_path)
            long_df.to_pickle(
                result_path + "/{}_{}_{}_{}.pkl".format(s_xx, factor_name, long_interval[0], long_interval[1]))
    except Exception as e:
        return '函数运行出错: factor_name={}, strategy={}, 测试区间:{}-{}-{}'.format(factor_name, "neptunelong", long_interval[0],
                                                                           long_interval[1], e)
    ...
    try:
        ...
        for short_date in short_interval_list:
            if os.path.exists(result_path + "/{}_{}_{}_{}.pkl".format(s_xx, factor_name, short_date, short_date)):
                ...
                neptunelong_basic = pd.read_hdf(settings.path_dict['neptunelong']['Basic'])
                short_df = FactorUtil.fill_factor_na_values(short_df, kls, neptunelong_basic)
        ...
        return '函数运行出错: factor_name={}, strategy={}, 测试区间:{}-{}-{}'.format(factor_name, "neptunelong", short_date,
                                                                           short_date, e)
    return "pass"
...
def score_check(factor_name, factor_value_df, filter_df):
    try:
        ...
        if factor_value_df.reindex(filter_df.index)[factor_name].value_counts(normalize=True).max() > 0.2:
            return '相同值比例较高: factor_name={}, strategy={},'.format(factor_name, "neptunelong") + ' %.2f' % (
                factor_value_df.reindex(filter_df.index)[
                    factor_name].value_counts(normalize=True).max())
...
def neptunelong_precheck_aftercalc(kls, error_factors, start_date, end_date, factor_value, factor_type, result_path):
    ...
    pkl_path = result_path + '/precheck/neptunelong/result/'
    ...
    if factor_type == precheck_utils.FactorUtil.FactorType.T_1_FACTOR:
        if str(start_date) == settings.precheck_path_dict["neptunelong"]["long_interval"][0] and str(end_date) == \
                settings.precheck_path_dict["neptunelong"]["long_interval"][1]:
            check_dic['因子值一致性检查'] = value_same_check(kls, result_path + '/precheck/neptunelong/same_test/', filter_df)
        else:
            logger.warning("起止区间与settings中设置不一致，跳过长短区间检测！ factor={}, strategy=neptunelong".format(kls.factor_name))
