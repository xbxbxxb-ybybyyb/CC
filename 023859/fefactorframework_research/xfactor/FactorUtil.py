...
# 202508 新增判断是否为纯粹财务因子
def is_pure_finance_factor(factor_class):
    '''
    没有T日数据，没有T_1数据，且xdb数据只有财务，认为是纯财务因子，否则不是
    '''
    t_data_list = factor_class.t_day_data
    t_1_factor_data_list = factor_class.t_1_factor_data
    xdb_data_list = [x['name'] for x in factor_class.xdb_data]
    if len(t_data_list) == 0 and len(t_1_factor_data_list) == 0 \
        and set(xdb_data_list) <= {'xdb_balancesheet', 'xdb_income', 'xdb_cashflow', 'xdb_balancesheet_cs', 'xdb_income_cs', 'xdb_cashflow_cs'}:
        is_pure_finance_factor = 1
    else:
        is_pure_finance_factor = 0
    return is_pure_finance_factor
