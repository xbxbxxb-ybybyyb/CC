...
def find_target_contract_delta_tolerance(data, df, delta_ratio, mkt_ratio, contract_month, fee=0.0025, tol_range=['虚值二档', '虚值一档', '平值', '实值一档', '实值二档']):
    ...
    for option_type in ['平值', '虚值一档', '虚值二档', '实值一档', '实值二档']:
        ...
        for dt in data.index:
            option_type_dt = df.loc[(dt,contract),'option_type'] if option in list(df.xs(dt, level=0)['Option']) else None# 更新档位
            if option_type_dt not in tol_range: # 超出容忍度范围，进行合约替换
                ...
                option = df.loc[(dt, contract), 'Option']
                ...
                target_buy_vol = min(np.floor(target_amt * 1e4 / data.loc[dt,'index_pre_close'] / 10000), 5000)
                buy_amt = min(target_buy_vol * 10000 * trade_price / 10000, mkt_ratio * mkt_amt)
                buy_vol = np.floor(buy_amt * 1e4 / trade_price / 10000)
                buy_amt = (buy_vol * 10000 * trade_price / 10000)

                cost = 1.6 * (hold_vol+buy_vol) / 10000 + (hold_amt+buy_amt)*fee# 万元
            ...
            # 在容忍度内，不进行调仓，但考虑加减仓
                ...
                mkt_amt = df.loc[(dt, contract), 'amt']
                trade_price = df.loc[(dt, contract), 'twap']
                target_hold_vol = np.floor(target_amt * 1e4 / data.loc[dt, 'index_pre_close'] / 10000)
                if target_hold_vol > hold_vol: # 继续买
                    target_buy_vol = min(target_hold_vol,5000) - hold_vol
                    trade_amt = min(target_buy_vol * 10000 * trade_price / 10000, mkt_ratio * mkt_amt)
                    trade_vol = min(np.floor(trade_amt * 1e4 / trade_price / 10000), 5000)
                    trade_amt = (trade_vol * 10000 * trade_price / 10000)
                    ...
                    target_sell_vol = hold_vol - target_hold_vol
                    trade_amt = min(target_sell_vol * 10000 * trade_price / 10000, mkt_ratio * mkt_amt)
                    trade_vol = min(np.floor(trade_amt * 1e4 / trade_price / 10000), 5000)
                    trade_amt = (trade_vol * 10000 * trade_price / 10000)
                ...
                hold_amt = hold_vol * trade_price
                pct = df.loc[(dt,contract),'pct']
                cost = 1.6*trade_vol/10000 + trade_amt*fee
                data.at[dt, f'{contract_month}{option_type}持有合约手数'] = {**data.at[dt, f'{contract_month}{option_type}持有合约手数'], contract: data.at[dt, f'{contract_month}{option_type}持有合约手数'].get(contract, 0) + hold_vol}
                # data.loc[dt, f'{contract_month}{option_type}买入合约市场成交额'] = mkt_amt
...
def find_target_contract_delta(data, df, delta_ratio, mkt_ratio, contract_month):
    ...
    for option_type in ['平值','虚值一档','虚值二档','实值一档','实值二档']:
        ...
        pct = df_filtered['pct'].droplevel('Ticker') - 0.005
        ...
        target_buy_vol = np.floor(target_amt * 1e4 / data['index_pre_close'] / 100)
        target_buy_amt = (target_buy_vol * 100 * buy_price / 10000).combine(mkt_ratio*mkt_amt, min)
...
def fix_hedging(data, df, contract_month, amt_tot, mkt_ratio, spread_cost):
    ...
    for option_type in ['平值', '虚值一档', '虚值二档', '实值一档', '实值二档']:
        ...
        for i in range(len(extention_dates)):
            ...
            data.loc[dt_start:dt_end, f'{contract_month}{option_type}持有合约手数'] = data.loc[dt_start:dt_end, f'{contract_month}{option_type}持有合约手数'].apply(lambda dct: {**dct, contract:dct.get(contract,0)+buy_vol})
            ...
            data.loc[dt_start:dt_end, f'profit_{contract_month}{option_type}'] = (buy_amt * ((1 + pct_).cumprod() - (1 + pct_).cumprod().shift(1).fillna(1)) - cost).values

        data[f'profit_{contract_month}{option_type}'] = data[f'profit_{contract_month}{option_type}'].shift(1).fillna(0)
    return data

def find_target_contract_fix_tolerance(data, df, contract_month, k=5, amt_tot=20000, mkt_ratio=0.1, spread_cost=0.005, tol_range=['虚值二档','虚值一档','平值','实值一档','实值二档']):
    df = df.copy()
    ...
    for option_type in ['平值', '虚值一档', '虚值二档', '实值一档', '实值二档']:
        ...
        for i in range(len(extention_dates)):
            ...
            for j in range(k):  # 分k天建仓、平仓
                ...
                dt_end_ = pd.Timestamp(fd.tradingday(dt_end.strftime('%Y%m%d'), j + 1)[-1]) if i < len(extention_dates) - 1 else dt_end
                _dt_end_ = pd.Timestamp(fd.tradingday(dt_end_.strftime('%Y%m%d'), -2)[0]) if i < len(extention_dates) - 1 else pd.Timestamp(fd.tradingday(dt_end.strftime('%Y%m%d'), -2)[0])
                ...
                target_buy_vol = min(np.floor(target_amt * 1e4 / data.loc[_dt_start_, 'index_pre_close'] / 100), vol_limit_dict[_dt_start_])
                target_buy_amt = min((target_buy_vol * 100 * buy_price / 10000), mkt_ratio * mkt_amt)
                ...
                for dt_tmp in fd.tradingday(_dt_start_.strftime('%Y%m%d'), _dt_end_.strftime('%Y%m%d')):
                    ...
                    if len(tol_range) and df.loc[(dt_end_tmp_,contract),'option_type'] not in tol_range:
                        ...
                        data.at[_dt_start_, f'{contract_month}{option_type}买入合约市场成交额'] = {**data.at[_dt_start_, f'{contract_month}{option_type}买入合约市场成交额'], contract: mkt_amt}
                        data.loc[_dt_start_:_dt_end_tmp_, f'{contract_month}{option_type}持有合约手数'] = data.loc[_dt_start_:_dt_end_tmp_, f'{contract_month}{option_type}持有合约手数'].apply(lambda dct: {**dct, contract: dct.get(contract, 0) + act_buy_vol})
                        pct_ = pct - spread_cost
                        data.at[_dt_start_, f'buy_amt_{contract_month}{option_type}'] = {**data.at[_dt_start_, f'buy_amt_{contract_month}{option_type}'], contract: data.loc[_dt_start_, f'buy_amt_{contract_month}{option_type}'].get(contract, 0) + act_buy_amt}
                        data.at[_dt_start_, f'buy_vol_{contract_month}{option_type}'] = {**data.at[_dt_start_, f'buy_vol_{contract_month}{option_type}'], contract: data.loc[_dt_start_, f'buy_vol_{contract_month}{option_type}'].get(contract, 0) + act_buy_vol}
                        data.loc[dt_start_:dt_end_tmp_, f'profit_{contract_month}{option_type}'] += (act_buy_amt * ((1 + pct_).cumprod() - (1 + pct_).cumprod().shift(1).fillna(1)) - cost).values
                        # 换仓
                        _dt_start_ = dt_end_tmp_
                        data.at[_dt_start_, f'sell_vol_{contract_month}{option_type}'] = {**data.at[_dt_start_, f'sell_vol_{contract_month}{option_type}'], contract: data.loc[_dt_start_, f'sell_vol_{contract_month}{option_type}'].get(contract,0) + act_buy_vol}
                        dt_start_ = pd.Timestamp(fd.tradingday(_dt_start_.strftime('%Y%m%d'), 2)[-1])
                        df_tmp = df.xs(_dt_start_, level='dt').query(f"option_type == '{option_type}'")
                        contract = df_tmp.index[df_tmp.index.str[:6] == contract[:6]][0] if _dt_start_ < dt_end else df_tmp.index[1]
                        buy_price = df.loc[(_dt_start_, contract), 'twap']
                        mkt_amt = df.loc[(_dt_start_, contract), 'amt']
                        target_buy_vol = min(np.floor(target_amt * 1e4 / data.loc[_dt_start_, 'index_pre_close'] / 100), vol_limit_dict[_dt_start_])
                        target_buy_amt = min((target_buy_vol * 100 * buy_price / 10000), mkt_ratio * mkt_amt)
                ...
                if len(pct) == 0 or act_buy_vol == 0:
                    continue
                data.at[_dt_start_, f'{contract_month}{option_type}买入合约市场成交额'] = {**data.at[_dt_start_, f'{contract_month}{option_type}买入合约市场成交额'], contract: mkt_amt}
                data.loc[_dt_start_:_dt_end_contract, f'{contract_month}{option_type}持有合约手数'] = data.loc[_dt_start_:_dt_end_contract,f'{contract_month}{option_type}持有合约手数'].apply(lambda dct: {**dct, contract: dct.get(contract, 0) + act_buy_vol})
                pct_ = pct - spread_cost
                data.at[_dt_start_, f'buy_amt_{contract_month}{option_type}'] = {**data.at[_dt_start_, f'buy_amt_{contract_month}{option_type}'],contract: data.loc[_dt_start_, f'buy_amt_{contract_month}{option_type}'].get(contract,0) + act_buy_amt}
                data.at[_dt_start_, f'buy_vol_{contract_month}{option_type}'] = {**data.at[_dt_start_, f'buy_vol_{contract_month}{option_type}'],contract: data.loc[_dt_start_, f'buy_vol_{contract_month}{option_type}'].get(contract,0) + act_buy_vol}
                data.loc[dt_start_:dt_end_, f'profit_{contract_month}{option_type}'] += (act_buy_amt * ((1 + pct_).cumprod() - (1 + pct_).cumprod().shift(1).fillna(1)) - cost).values
                data.at[dt_end_, f'sell_vol_{contract_month}{option_type}'] = {**data.at[dt_end_, f'sell_vol_{contract_month}{option_type}'],contract: data.loc[dt_end_, f'sell_vol_{contract_month}{option_type}'].get(contract,0) + act_buy_vol}
...
def classify_moneyness_top5(df, index_col='index_pre_close', strike_col='Strike'):
    """
    对于每个日期 dt，标记最多5个合约：平值、实值一/二档、虚值一/二档，其余为空字符串''
    """
...
'''
basic_info = pd.read_pickle('/dfs/group/800463/data/option_data/000852/basicinfo/map_option_basicinfo_date.pkl')
...
df = s.get_factor_value('WIND_ChinaOptionEODPrices', factors=['S_INFO_WINDCODE','TRADE_DT','S_DQ_PRESETTLE','S_DQ_SETTLE','S_DQ_OI','S_DQ_AMOUNT'],TRADE_DT='<=20250630',S_INFO_WINDCODE="like 'MO%P%CFE'")
...
df['Strike'] = df['Ticker'].apply(lambda x: int(x[9:13]))
df = df.set_index(['dt','Ticker']).sort_index()
df['LastTradingDate'] = basic_info['LastTradingDate']
for idx in df.index:
    dt = idx[0]
    ...
    df.loc[(dt,Ticker),'twap'] = df_tick['LastPx'].mean()
...
df_valuation = s.get_factor_value('WIND_ChinaOptionValuation', factors=['S_INFO_WINDCODE','TRADE_DT','W_ANAL_DELTA'], TRADE_DT='<=20250630', S_INFO_WINDCODE="like 'MO%P%CFE'")
...
# 分五个交易日建仓
k = 5
date_to_extend = {date: pd.Timestamp(fd.tradingday(date.strftime('%Y%m%d'), -2-k)[0]) for date in unique_last_trading_dates}
df['ExtentionDate'] = df['LastTradingDate'].map(date_to_extend)
extention_dates = set(df[df['ExtentionDate'] <= data.index.get_level_values('dt')[-1]]['ExtentionDate'])
extention_dates.add(data.index.get_level_values('dt')[0])
extention_dates = sorted(list(extention_dates))

# date_to_extend = {date: pd.Timestamp(fd.tradingday(date.strftime('%Y%m%d'), -3)[0]) for date in unique_last_trading_dates}
# df['ExtentionDate'] = df['LastTradingDate'].map(date_to_extend)
# df = df[df.index.get_level_values('dt') < df['ExtentionDate']]
# extention_dates = set(df[df['ExtentionDate'] <= data.index.get_level_values('dt')[-1]]['ExtentionDate'])
# extention_dates.add(data.index.get_level_values('dt')[0])
# extention_dates = sorted(list(extention_dates))
...
df_filtered_this_month = classify_moneyness_top5(df_filtered_this_month)
df_filtered_next_month = df[df['tag'] == 2]
df_filtered_next_month = classify_moneyness_top5(df_filtered_next_month)
df_filtered_next_2_month = df[df['tag'] == 3]
df_filtered_next_2_month = classify_moneyness_top5(df_filtered_next_2_month)
...
# data = find_target_contract_fix_tolerance(data, df_this_month, contract_month='当月', k=k, amt_tot=20000, mkt_ratio=0.1, spread_cost=0.002, tol_range=['虚值二档','虚值一档','平值','实值一档'])
# data = find_target_contract_fix_tolerance(data, df_next_month, contract_month='次月', k=k, amt_tot=20000, mkt_ratio=0.1, spread_cost=0.002, tol_range=['虚值二档','虚值一档','平值','实值一档'])

# data = find_target_contract_fix_tolerance(data, df_this_month, contract_month='当月', k=5, amt_tot=20000, mkt_ratio=0.1, spread_cost=0.002, tol_range=['虚值二档','虚值一档','平值','实值一档'])
# data = find_target_contract_fix_tolerance(data, df_next_month, contract_month='次月', k=5, amt_tot=20000, mkt_ratio=0.1, spread_cost=0.002, tol_range=['虚值二档','虚值一档','平值','实值一档'])
...
# with pd.ExcelWriter(f'/dfs/user/023859/options/期权固定对冲回测结果_20000_20220722_20250630.xlsx', engine='xlsxwriter') as writer:
...
data['当月平值持有合约信息'] = data[['当月平值持有合约手数','index_close']].apply(lambda row: merge_vol_pct_amt_delta(row, df), axis=1)
data['当月平值持有合约总规模'] = data['当月平值持有合约信息'].apply(lambda dct: sum(sub['amt'] for sub in dct.values()))
data['当月平值delta敞口'] = data['当月平值持有合约信息'].apply(lambda dct: sum(sub['vol']*sub['delta_exposure'] for sub in dct.values()))
data['当月平值敞口'] = data['当月平值持有合约信息'].apply(lambda dct: sum(sub['vol']*sub['exposure'] for sub in dct.values()))

data['事件型策略日度最大回撤'] = (data['策略日度盈利'].cumsum().cummax() - data['策略日度盈利'].cumsum())
data['期权+事件型策略日度最大回撤'] = ((data['策略日度盈利']+data['profit_当月平值']).cumsum().cummax()-(data['策略日度盈利']+data['profit_当月平值']).cumsum())
...
for period in half_year_dict:
    ...
    res_half_year[(period,'期权+事件型策略')] = sta_profit(data.loc[start_date:end_date,'策略日度盈利']+data.loc[start_date:end_date,'profit_当月平值'])

res_df_half_year = pd.DataFrame(res_half_year)
res_df_half_year = res_df_half_year.sort_index(axis=1,level=0)

data_best = data[['策略股票持仓','策略日度盈利','事件型策略日度最大回撤','index_close','index_pre_close','当月平值持有合约信息','当月平值持有合约总规模','当月平值敞口','当月平值delta敞口',\
                  'profit_当月平值','期权+事件型策略日度最大回撤','当月平值买入合约市场成交额','buy_amt_当月平值','buy_vol_当月平值','sell_vol_当月平值',\
                  '策略累计盈利','当月平值累计盈利','当月平值期权端累计盈利']].rename(columns={'当月平值持有合约信息':'持有合约信息','当月平值持有合约总规模':'持有合约总市值',\
                                                                                    '当月平值敞口':'持有合约名义市值','当月平值delta敞口':'delta敞口',\
                                                                                    'profit_当月平值':'profit','当月平值买入合约市场成交额':'买入合约市场成交额',\
                                                                                    'buy_amt_当月平值':'buy_amt','buy_vol_当月平值':'buy_vol','sell_vol_当月平值':'sell_vol',\
                                                                                    '当月平值累计盈利':'对冲后策略累计盈利','当月平值期权端累计盈利':'策略期权端累计盈利'})

# with pd.ExcelWriter(f'/dfs/user/023859/options/期权固定对冲回测结果_20000_当月平值_20220722_20250630.xlsx', engine='xlsxwriter') as writer:
#     data_best.to_excel(writer, sheet_name='20220722~20250630当月平值数据')
#     res_df_half_year.to_excel(writer, sheet_name='期权当月平值+事件型策略收益统计')
#     writer.save()
