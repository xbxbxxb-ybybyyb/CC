...
'''
财务因子（Finance Factor）测试框架：
1、财务因子取财报获取后的第一个交易日的label作为评测依据，非财报相邻日期被剔除
2、财务因子的中性化并非截面中性化
...
factor_df_ori：全部样本
factor_df：财报邻近日样本
1）计算和sft文件中其他因子的相关性，使用的是全部样本，从而和入库端重新计算（而非调用框架结果）的逻辑保持一致
2）因子分布，因子得分，因子IC等，都是用邻近日的样本
3）重复值、极端值都用的全部样本
'''

class FactorTest:
    def __init__(self, start_date, end_date, segment_number=20,
                 style_list=['Circu_Mkt'],  # 风格因子，原先用于中性化，现保留
                 cal_mi=False,
                 buy_type='s1',
        ...
        if self.strategy_name == 'neptune':
            self.return_type = 'label_s1_short'
    ...
    def load_data(self):
        ...
        if self.strategy_name == 'neptune':
            # sft_basic_path = '/dfs/group/800463/public/projectZZ_public/factor_lib/sft_update_931_20160101_20201231.pkl'
            sft_basic_path = '/dfs/user/023859/neptune/sft_finance.pkl'
            # sft_basic_path = '/data/user/015585/01-因子挖掘/20250811_因子框架改造/sft_test.pkl' # TODO: 改回来！
            df = pd.read_pickle(sft_basic_path).loc[pd.Timestamp(str(self.start_date)):pd.Timestamp(str(self.end_date))]
        else:
            logger.error('strategy_name error, must in [neptune]')
            raise TypeError
        # 202507 新增筛选
        df = df[(df['list_len'] >= 120) & (df['STPT'] == False)]
        df = df[~df[ret_type].isna()]
        #
        ...
        # 202508 保留没有用财务日期筛选的结果，用于和其他因子的相关性计算
        self.basic_df_ori = df[['value', 'class', 'month', 'year'] + self.style_list]
        all_factors_available = ['label_s1_short','label_s1_mid','label_s1_long','Circu_Mkt','STPT','list_len','first_after_financedate']
        ...
        # 202508 finance factor只保留财报临近日
        df = df[df['first_after_financedate'] == 1]
        #
        self.basic_df = df[['value', 'class', 'month', 'year'] + self.style_list]

    def _neutralize_group(self, group):
        group = group.droplevel('dt')
        target = group['factor']
        style_factors = group[self.style_list]

        valid_mask = (style_factors.notnull().all(axis=1)) & (target.notnull())
        X = style_factors[valid_mask]
        y = target[valid_mask]
        ...
        fitted = model.predict(X_with_const)
        residuals = y - fitted
        res = pd.Series(index=group.index, dtype=float)
        res.loc[valid_mask] = residuals.values
        return res
    ...
    def extreme_value_sta(self):
        med = self.basic_df_ori['factor'].median()
        mad = (self.basic_df_ori['factor'] - med).abs().median()
        sta = pd.Series()
        sta['up_3MAD_rate'] = (self.basic_df_ori['factor'] > (med + 3 * mad)).mean()
        sta['down_3MAD_rate'] = (self.basic_df_ori['factor'] < (med - 3 * mad)).mean()
        return sta

    def factor_test(self, factor_df, cost_list, result_path, factor_corr_test=False, generate_pdf=False):
        if len(factor_df.columns) > 1:
            print('factor_df must be only one column')
            raise Exception
        self.load_data()
        if factor_corr_test:
            check_res = pd.read_excel(
                '/dfs/group/800463/public/projectZZ_public/factor_lib/check_res_tot_neptune.xlsx')
        ...
        self.basic_df['factor'] = self.basic_df.groupby('dt').apply(lambda x: self._neutralize_group(x)) # basic_df的做中性化，其他不做
        #
        self.factor_df_ori = pd.DataFrame(index=self.basic_df_ori.index)  # 全部样本
        self.factor_df_ori['factor'] = factor_df[self.factor_name]
        self.basic_df_ori['factor'] = self.factor_df_ori['factor']
        # self.basic_df['factor'] = self.basic_df.groupby('dt').apply(lambda x: self._neutralize_group(x))

        self.result_dic['factor_information'] = self.get_factor_information()

        self.result_dic['corr_group'] = self.get_corr_group()

        self.result_dic['corr_sta'] = self.get_corr_sta(self.result_dic['corr_group'])
        ...
        self.result_dic['factor_group_sort'] = self.get_factor_sort()

        self.result_dic['check_score_res'] = self.check_score(self.result_dic['factor_group_sort'].copy(),
                                                              self.result_dic['corr_group'].copy(),
    ...
    def check_score(self, factor_group_df, group_corr_df, dir, label_test=['value', 'class']):
        ...
        diff_dic = {'value': {'reverse': [0.6, 0.5], 'diff': [0.25, 0.35]},
                    'class': {'reverse': [0.6, 0.5], 'diff': [0.08, 0.13]}}

        stability_dic = {'value': [0.05, 0.1],
                         'class': [0.05, 0.1]}
        s_s = 20 / len(group_corr_df)
        stability_score = pd.Series([s_s, s_s / 2, 0, -s_s / 2, -s_s],
                                    index=['high', 'mid', 'low', 'reverse_mid', 'reverse_high'])

        d_s = 30 / len(factor_group_df.index.get_level_values(0).unique())
        diff_score = pd.Series([d_s, d_s / 2, 0], index=['high', 'mid', 'low'])

        for label in label_test:
            group_corr_df[label] = group_corr_df[label] * dir[label]

        stability_df = pd.DataFrame(index=group_corr_df.index)
        diff_res_df = pd.DataFrame()

        for label in label_test:
            diff, reverse = [], []
            for group in factor_group_df.index.get_level_values(0).unique():
                rate_arr = factor_group_df[label].unstack().loc[group].values
            ...
            diff_res_df['%s_high_reverse' % (label)], diff_res_df['%s_mid_reverse' % (label)] = diff_res_df[
                                                                                                    '%s_diff' % (
                                                                                                        label)] * \
                                                                                                diff_dic[label][
                                                                                                    'reverse'][1], \
            ...
            diff_res_df.loc[((diff_res_df['%s_diff' % (label)] > diff_dic[label]['diff'][0]) & (
                    ...
                    '%s_mid_reverse' % (label)])), '%s_standard' % (label)] = 2
            ...
            stability_df.loc[group_corr_df[label] >= stability_dic[label][1], '%s_standard' % (label)] = 'high'
            stability_df.loc[(group_corr_df[label] >= stability_dic[label][0]) & (
                        group_corr_df[label] < stability_dic[label][1]), '%s_standard' % (label)] = 'mid'
            stability_df.loc[(group_corr_df[label] > -stability_dic[label][0]) & (
                        group_corr_df[label] < stability_dic[label][0]), '%s_standard' % (label)] = 'low'
            stability_df.loc[(group_corr_df[label] <= -stability_dic[label][0]) & (
                        group_corr_df[label] > -stability_dic[label][1]), '%s_standard' % (label)] = 'reverse_mid'
            stability_df.loc[group_corr_df[label] <= -stability_dic[label][1], '%s_standard' % (label)] = 'reverse_high'
    ...
    def get_factor_sort(self, data=None, segment_number = 8, factor_group_num=5): # segment_number 为分N组，factor_group_num为每组内再分M组
        segment_number = self.segment_number if segment_number is None else segment_number
        sorted_factor_data = self.basic_df.sort_values(['dt']).copy()
        ...
        #
        data = sorted_factor_data.copy() if (data is None) else data
        after_data = pd.DataFrame()

        for group in data['group'].unique():
            group_factor_data = data[data['group'] == group].copy()
            ...
            date_factor_data = group_factor_data.sort_values(by='factor')
            date_factor_data['factor_group'] = group_list
            after_data = after_data.append(date_factor_data)

        tmp_result = pd.DataFrame()
        for group in after_data['group'].unique():
            group_after_data = after_data[after_data['group'] == group]
            for j in range(factor_group_num):
                tmp_result[(group, j)] = group_after_data[group_after_data['factor_group'] == j].mean()[self.label_list]
        tmp_result = tmp_result.T.reset_index()
        tmp_result['dt'] = tmp_result['index'].apply(lambda x: x[0])
        tmp_result['factor_group'] = tmp_result['index'].apply(lambda x: x[1])
        tmp_result = tmp_result.set_index(['dt', 'factor_group']).drop(['index'], axis=1)
        return tmp_result

    def get_corr_sta(self, corr_group):
        ...
        sta_df['corr_tot'] = [self.basic_df['factor'].corr(self.basic_df[label], method='spearman') for label in self.label_list]  # 使用普通IC而非截面IC
        sta_df['corr_group_mean'] = [corr_group[label].mean() for label in self.label_list]
        sta_df['corr_group_std'] = [corr_group[label].std() for label in self.label_list]
        sta_df['corr_group_mean_std'] = sta_df['corr_group_mean'] / sta_df['corr_group_std']
        return sta_df.T

    def get_max_one_ratio(self):
        import collections
        sample_number = len(self.factor_df_ori)
        factor_value_distribution = np.array(list(collections.Counter(self.factor_df_ori['factor']).values()))
    ...
    def get_corr_group(self, data=None, segment_number=None,):
        segment_number = self.segment_number if segment_number is None else segment_number
        data = self.basic_df if data is None else data

        sorted_factor_data = data.sort_values(['dt']).copy()
        num_each_group = int(len(sorted_factor_data) / segment_number)
        group_index_list = []
        for i in range(segment_number):
            group_index_list = (group_index_list + [i] * num_each_group) if i != (segment_number - 1) \
        ...
        groupby_group_data = sorted_factor_data.groupby('group')
        corr_group_df['count'] = groupby_group_data.count()['factor']
        for label in self.label_list:
            corr_group_df[label] = groupby_group_data.apply(lambda x : x['factor'].corr(x[label], method='spearman'))
        return corr_group_df
    ...
    def get_corr_with_all_factor(self, group_num=5):
        ...
        corr_res['factor_corr'] = self.all_factor.corrwith(self.factor_df_ori['factor'], method='spearman')
