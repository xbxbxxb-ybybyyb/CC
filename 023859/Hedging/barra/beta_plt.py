...
root_dict = {'index_info':'/data/user/023859/Hedging/index_price_20210701_20240630.pkl',\
             ...
             'strategy_info':'/data/user/023859/Hedging/ceres+mimas+p4/df_strategy_basic_sw1_beta_20210701_20221231.pkl',\
...
for vote_num in [4,3,2]:
    day_df_strategy, _,_,_ = util.calc_profit(trading_days,strategy_list,root_dict,2e8,1.5,vote_num)
    ...
    fig.autofmt_xdate()
    plt.tight_layout()
    plt.savefig(f'/dfs/user/023859/Hedging/{strategy_join}/neutral_vote{vote_num}.png')  # 保存为PNG格式
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(f'/dfs/user/023859/Hedging/{strategy_join}/neutral_vote{vote_num}_period.png')  # 保存为PNG格式
    plt.show()