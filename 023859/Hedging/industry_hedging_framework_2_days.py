...
strategy_list = ['mimas']
# zz1000_amt_dict = {'3亿现货':3e8}
zz1000_amt_dict = {'1亿现货':1e8,'2亿现货':2e8,'3亿现货':3e8}
# deviation_dict = {'行业偏离度1':1,'行业偏离度1.5':1.5}
# deviation_dict = {'固定权重偏离0.02':0.02,'固定权重偏离0.05':0.05,'固定权重偏离0.1':0.1}
deviation_dict = {'偏离度max{2%,1.5倍}':[1.5,0.02]}
...
def calc_res(trading_days, strategy_list, root_dict, zz1000_amt_dict, deviation_dict, vote_num):
    ...
    for ZZ1000_buy_amt in zz1000_amt_dict.keys():
        for deviation in deviation_dict.keys():
            ...
            day_df_buy_amt[strategy_join+'行业完全对冲信号端买入规模'] = day_df_strategy[strategy_buy_amt_dict[f'{strategy_join}行业完全对冲信号端']]
            day_df_buy_amt[strategy_join+'行业完全对冲信号端收益'] = day_df_strategy[f'{strategy_join}_act_profit']
            day_df_buy_amt[strategy_join+'原信号买入规模'] = day_df_strategy[f'{strategy_join}信号样本买入规模']
            day_df_buy_amt[strategy_join+'原信号收益'] = day_df_strategy[f'{strategy_join}原信号']

            if len(strategy_list) > 1:
                for strategy in strategy_list:
                    day_df_buy_amt[strategy + '行业完全对冲信号端买入规模'] = day_df_strategy[f'{strategy}行业完全对冲信号样本买入规模']
                    day_df_buy_amt[strategy + '行业完全对冲信号端收益'] = day_df_strategy[f'{strategy}行业完全对冲信号样本收益']
                    day_df_buy_amt[strategy + '原信号买入规模'] = day_df_strategy[f'{strategy}信号样本买入规模']
                    day_df_buy_amt[strategy + '原信号收益'] = day_df_strategy[f'{strategy}原信号']
    ...
    return sta_df_deviation, sta_period, sta_period1, day_df_buy_amt

# result = calc_res(trading_days, strategy_list, root_dict, zz1000_amt_dict, deviation_dict, 0)

with Pool(processes=5) as pool:
    results = pool.starmap(calc_res,[(trading_days, strategy_list, root_dict, zz1000_amt_dict, deviation_dict, vote_num) for vote_num in vote_nums])

excel_writer1 = pd.ExcelWriter(f'/dfs/user/023859/industry_hedging/{strategy_join}_行业中性对冲回测结果_{start_date}_{end_date}_固定+浮动权重偏离_0.001_单票200_无信号_votes.xlsx')
excel_writer2 = pd.ExcelWriter(f'/dfs/user/023859/industry_hedging/{strategy_join}_行业中性对冲信号端每日规模变动及收益_{start_date}_{end_date}_固定+浮动权重偏离_单票200_无信号_votes.xlsx')

# sta_df_deviation = result[0]
# sta_period = result[1]
# sta_period1 = result[2]
# sta_day_buy_amt = result[3]
# sta_df_deviation.to_excel(excel_writer1, sheet_name=f'单样本200万')
# sta_period1.to_excel(excel_writer1, sheet_name=f'单样本200万', startrow=30, startcol=0)
# sta_period.to_excel(excel_writer1, sheet_name=f'单样本200万', startrow=0, startcol=11)
# sta_day_buy_amt.to_excel(excel_writer2, sheet_name=f'单样本200万')
# excel_writer1.save()
# excel_writer2.save()

for i in range(len(vote_nums)):
    vote_num = vote_nums[i]
    sta_df_deviation = results[i][0]
    sta_period = results[i][1]
    sta_period1 = results[i][2]
    sta_day_buy_amt = results[i][3]
    sta_df_deviation.to_excel(excel_writer1, sheet_name=f'vote>={vote_num}')
    sta_period1.to_excel(excel_writer1, sheet_name=f'vote>={vote_num}', startrow=30, startcol=0)
    sta_period.to_excel(excel_writer1, sheet_name=f'vote>={vote_num}', startrow=0, startcol=11)
    sta_day_buy_amt.to_excel(excel_writer2, sheet_name=f'vote>={vote_num}')
excel_writer1.save()
excel_writer2.save()
