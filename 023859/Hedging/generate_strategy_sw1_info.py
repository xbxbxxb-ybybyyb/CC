...
cols = ['vote_sum_pred','label_buy_amt','label_pct',\
        'label_buy_amt_100','label_pct_100','label_buy_amt_200','label_pct_200','label_buy_amt_300','label_pct_300','label_buy_amt_400','label_pct_400',\
        'label_buy_amt_100_p2','label_pct_100_p2','label_buy_amt_200_p2','label_pct_200_p2','label_buy_amt_300_p2','label_pct_300_p2','label_buy_amt_400_p2','label_pct_400_p2',\
        'label_buy_amt_100_p3','label_pct_100_p3','label_buy_amt_200_p3','label_pct_200_p3','label_buy_amt_300_p3','label_pct_300_p3','label_buy_amt_400_p3','label_pct_400_p3']
...
#
# df_saturn_1 = pd.read_csv('/data/user/013550/For_TSQ/signal/saturn/fac_20241129/集成信号_20210701_20220630.csv')
# df_saturn_2 = pd.read_csv('/data/user/021012/团队分享/for_tsq/saturn/fac_20241129_s1/集成信号_20220701_20230630_all.csv')
# df_saturn  = pd.concat([df_saturn_1,df_saturn_2],axis=0)
...
df_saturn = pd.read_excel('/dfs/user/003371/share_files/for_tsq/low_frequency_strategy202107_202306_update.xlsx',sheet_name='saturn').reset_index()
df_saturn['dt'] = pd.to_datetime(df_saturn['datelist'].astype(str))
df_saturn = df_saturn.set_index(['dt','Ticker'])
df_saturn = df_saturn.loc[pd.Timestamp(str(start_date)):pd.Timestamp(str(end_date))]
df_saturn = df_saturn[cols]#.rename(columns={'label_pct':'pct','label_buy_amt':'buy_amt'})
df_saturn['strategy'] = 'saturn'

df_ceres = pd.read_excel('/dfs/user/003371/share_files/for_tsq/low_frequency_strategy202107_202306_update.xlsx',sheet_name='ceres').reset_index()
df_ceres['dt'] = pd.to_datetime(df_ceres['datelist'].astype(str))
df_ceres = df_ceres.set_index(['dt','Ticker'])
df_ceres = df_ceres.loc[pd.Timestamp(str(start_date)):pd.Timestamp(str(end_date))]
df_ceres = df_ceres[cols]#.rename(columns={'label_pct':'pct','label_buy_amt':'buy_amt'})
...
#
df_mimas = pd.read_excel('/dfs/user/003371/share_files/for_tsq/low_frequency_strategy202107_202306_update.xlsx',sheet_name='mimas').reset_index()
df_mimas['dt'] = pd.to_datetime(df_mimas['datelist'].astype(str))
df_mimas = df_mimas.set_index(['dt','Ticker'])
df_mimas = df_mimas.loc[pd.Timestamp(str(start_date)):pd.Timestamp(str(end_date))]
df_mimas = df_mimas[cols]#.rename(columns={'label_pct':'pct','label_buy_amt':'buy_amt'})
...
# df_p4_1_2 = pd.read_csv('/data/user/013550/For_TSQ/signal/p4/fac_20240807/集成信号_20210701_20220630.csv')
# df_p4_3 = pd.read_csv('/data/user/013550/For_TSQ/signal/p4/fac_20241225/集成信号_20220701_20221231.csv')
# df_p4 = pd.concat([df_p4_1_2,df_p4_3])
# df_p4['dt'] = pd.to_datetime(df_p4['dt'])
# df_p4 = df_p4.set_index(['dt','Ticker'])
# df_p4 = df_p4.loc[pd.to_datetime(str(start_date)):pd.to_datetime(str(end_date))]
# df_p4 = df_p4[['label_pct','label_buy_amt','vote_sum_pred']].rename(columns={'label_pct':'pct','label_buy_amt':'buy_amt'})
# df_p4['strategy'] = 'p4'

df_p4 = pd.read_excel('/dfs/user/003371/share_files/for_tsq/low_frequency_strategy202107_202306_update.xlsx',sheet_name='p4').reset_index()
df_p4['dt'] = pd.to_datetime(df_p4['datelist'].astype(str))
df_p4 = df_p4.set_index(['dt','Ticker'])
df_p4 = df_p4.loc[pd.Timestamp(str(start_date)):pd.Timestamp(str(end_date))]
df_p4 = df_p4[cols]#.rename(columns={'label_pct':'pct','label_buy_amt':'buy_amt'})
df_p4['strategy'] = 'p4'
...
#
df_strategy_basic = pd.concat([df_saturn,df_ceres,df_mimas,df_p4],axis=0).sort_index() # saturn、ceres、mimas策略样本一起考虑
# df_strategy_basic = df_p4
# df_strategy_sign = pd.concat([df_ceres_sign,df_mimas_sign,df_saturn_sign],axis=0).sort_index() # saturn、ceres、mimas策略样本一起考虑
...
for dt in tqdm(dt_strategy_basic_stock_list.index):
    ...
    sw2 = s.hsi(dt_strategy_basic_stock_list.loc[dt], date, flag, 2).set_index('stock')
...
df_strategy_basic_sw1 = df_strategy_basic.reset_index().merge(strategy_basic_sw1_info.reset_index(),on=['dt','Ticker'],how='left').set_index(['dt','Ticker']).sort_index() #策略信号样本收益信号及所属申万一级行业

df_strategy_basic_sw1['sw_industry_code'] = df_strategy_basic_sw1.apply(lambda row: row['sw_industry_code_2'] if row['sw_industry_name_1'] == '非银金融' else row['sw_industry_code_1'],axis=1)
df_strategy_basic_sw1['sw_industry_name'] = df_strategy_basic_sw1.apply(lambda row: row['sw_industry_name_2'] if row['sw_industry_name_1'] == '非银金融' else row['sw_industry_name_1'],axis=1)

df_strategy_basic_sw1.to_pickle('/dfs/user/023859/industry_hedging/saturn+ceres+mimas+p4/df_strategy_basic_sw1_%s_%s.pkl'%(start_date, end_date))