...
class BaseFactor(object):
    ...
    # {
    #    name: '', # xdb_order, xdb_trade, xdb_cancel, xdb_tickfull, xdb_tick1s
    ...
    '''
    pre_calculate_T_N_data
    概述：根据T-N相关数据，计算出相关中间变量。研究端和实盘端的盘前数据准备保持一致
    返回值: database
    注意1：
        case1：t_1_factor_data
              与旧框架一致，返回全时间段全市场的df，index为dt和ticker，无需shift
        case2：t_1_factor_data + T日高频
              此时输入并不会给未来时间段的h5文件，输出为全市场标的最后一个交易日的df，index为dt和ticker（即需要stack），无需shift
        case3：xdb + T日高频（+t_1_factor_data）
              输出为单标的（从xdb数据中获取）的1行df，默认的[0,1,2,3...]index
    注意2：返回的数据统一保存至database["pre_T_N"]，不得为其他key
    '''
    @abstractmethod
    ...
    '''
    prepare_T_data
    概述：根据T-N相关中间变量，及T日相关数据需求，针对database中T日数据进行预处理。可以直接赋值给database，如 database["zcz"] = True
    注意：仅涉及T日数据的因子才可覆写此方法，否则直接return database即可
    '''
    @abstractmethod
    def prepare_T_data(self, database):
        ...
        if database["skip"] == True:
            ...
            df = database['TTickab']
            para1 = df['pre_close'].values[0]
            database['pre_close'] = para1
        return database
