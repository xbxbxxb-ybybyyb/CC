    ...
    try:
        if os.path.exists('{}{}_{}_{}.pkl'.format(result_path, factor_name, long_interval[0], long_interval[1])):
            ...
            if not db:
                ...
                db = precheck_utils.prepare_database_for_value_same_check(kls, "jupiter", str(long_interval[0]),
                                                                          str(long_interval[1]))
    ...
    try:
        for short_date in short_interval_list:
            ...
            if np.nanmax((short_df - tmp_long_df).abs().values) > 1e-8:
                logger.error('因子值不一致1: factor_name={}, strategy={}, 计算区间:{}-{}和{}-{}'.format(
                    factor_name, "jupiter", long_interval[0], long_interval[1], short_date, short_date))
                logger.error('symbol={}, max_abs={}'.format((short_df - tmp_long_df).abs().idxmax().values[0],
                                                            np.nanmax((short_df - tmp_long_df).abs().values)))

                return '因子值不一致1: factor_name={}, strategy={}, 计算区间:{}-{}和{}-{}'.format(
                    factor_name, "jupiter", long_interval[0], long_interval[1], short_date, short_date)
            if np.nanmax((short_df.reindex(basic_index).fillna(fill_na_value) - tmp_long_df.reindex(basic_index).fillna(
                ...
                return '因子值不一致2: factor_name={}, strategy={}, 计算区间:{}-{}和{}-{}'.format(
                    factor_name, "jupiter", long_interval[0], long_interval[1], short_date, short_date)
    except Exception as e:
        return '函数测试出错: factor_name={}, strategy={}, 测试区间:{}-{}-{}'.format(
            factor_name, "jupiter", short_date, short_date, e)
    return 'pass'
...
def score_check(factor_name, factor_value_df, filter_df):
    try:
        ...
        if same_rate > 0.8:
            return '相同值比例较高: factor_name={}, strategy={},'.format(factor_name, "jupiter") + ' %.2f%%' % (
                    same_rate * 100)
        ...
        if (factor_mean < 0.001) and (factor_std < 0.001):
            return '因子波动小: factor_name={}, strategy={}, std:%.8f,mean:%.8f'.format(
                factor_name, "jupiter", factor_mean, factor_std)
...
def jupiter_precheck_aftercalc(kls, error_factors, start_date, end_date, factor_value, factor_type, result_path):
    ...
    pkl_path = result_path + '/precheck/jupiter/result/'
    ...
    if factor_type == precheck_utils.FactorUtil.FactorType.T_1_FACTOR:
        if str(start_date) == settings.precheck_path_dict["jupiter"]["long_interval"][0] and str(end_date) == \
                settings.precheck_path_dict["jupiter"]["long_interval"][1]:
            check_dic['因子值一致性检查'] = value_same_check(kls, result_path + '/precheck/jupiter/same_test/', filter_df)

        else:
            logger.warning("起止区间与settings中设置不一致，跳过长短区间检测！ factor={}, strategy=jupiter".format(kls.factor_name))
