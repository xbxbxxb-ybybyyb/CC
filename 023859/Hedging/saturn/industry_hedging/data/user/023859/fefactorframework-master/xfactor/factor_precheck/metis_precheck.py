...
def value_same_check(kls, result_path, filter_df):
    ...
    logger.info('因子一致检查: factor_name={}, strategy={}'.format(factor_name, "metis"))
    ...
    try:
        if os.path.exists('{}{}_{}_{}.pkl'.format(result_path, factor_name, long_interval[0], long_interval[1])):
            ...
            if not db:
                logger.info("start loading data for value same check! factor={}".format(factor_name))
                db = precheck_utils.prepare_database_for_value_same_check(kls, "metis", str(long_interval[0]),
                                                                          str(long_interval[1]))
            logger.info("finish loading data for value same check! factor={}".format(factor_name))
    ...
    try:
        for short_date in short_interval_list:
            ...
            if np.nanmax((short_df - tmp_long_df).abs().values) > 1e-8:
                logger.error(
                    '因子值不一致1: factor={}, strategy={}, 计算区间:{}-{}和{}-{}'.format(factor_name, "metis", long_interval[0],
                ...
                return '因子值不一致1: factor={}, strategy={}, 计算区间:{}-{}和{}-{}'.format(factor_name, "metis",
                                                                                  long_interval[0], long_interval[1],
            ...
            if np.nanmax((short_df.reindex(basic_index).fillna(fill_na_value) - tmp_long_df.reindex(basic_index).fillna(
                ...
                return '因子值不一致2: factor={}, strategy={}, 计算区间:{}-{}和{}-{}'.format(factor_name, "metis",
                                                                                  long_interval[0], long_interval[1],
                                                                                  short_date, short_date)
    except Exception as e:
        logger.error(
...
def score_check(factor_name, factor_value_df, filter_df):
    try:
        ...
        if (factor_mean < 0.001) and (factor_std < 0.001):
            return '因子波动小: factor_name={}, strategy={},'.format(factor_name, "metis") + ' std:%.8f,mean:%.8f' % (
...
def metis_precheck_aftercalc(kls, error_factors, start_date, end_date, factor_value, factor_type, result_path):
    ...
    pkl_path = result_path + '/precheck/metis/result/'
    ...
    if factor_type == precheck_utils.FactorUtil.FactorType.T_1_FACTOR:
        if str(start_date) == settings.precheck_path_dict["metis"]["long_interval"][0] and str(end_date) == \
                settings.precheck_path_dict["metis"]["long_interval"][1]:
            check_dic['因子值一致性检查'] = value_same_check(kls, result_path + '/precheck/metis/same_test/', filter_df)

        else:
            logger.warning("起止区间与settings中设置不一致，跳过长短区间检测！ factor={}, strategy=metis".format(kls.factor_name))
