...
def value_same_check(kls, result_path, filter_df):
    ...
    logger.info('因子一致检查: factor_name={}, strategy={}'.format(factor_name, "mimas"))
    ...
    try:
        if os.path.exists(
            ...
            if not db:
                db = precheck_utils.prepare_database_for_value_same_check(kls, 'mimas', str(long_interval[0]),
                                                                          str(long_interval[1]))
        ...
        return '函数运行出错: factor_name={}, strategy={}, 测试区间:{}-{}-{}'.format(
            factor_name, "mimas", long_interval[0], long_interval[1], e)
    ...
    try:
        ...
        if len(diff_short_date) > 0:
            return '区间测试未通过: factor_name={}, strategy={}, 不一致日期:{}'.format(factor_name, "mimas", str(diff_short_date))
    except Exception as e:
        return '函数运行出错: factor_name={}, strategy={}, 测试区间:{}-{}-{}'.format(factor_name, "mimas", short_date, short_date,
                                                                           e)
    return "pass"
...
def score_check(factor_name, factor_value_df, filter_df):
    try:
        ...
        if factor_value_df.reindex(filter_df.index)[factor_name].value_counts(normalize=True).max() > 0.2:
            logger.error('相同值比例较高: factor_name={}, strategy={}'.format(factor_name, "mimas"))
        ...
        if factor_std < 1e-15:
            logger.error('因子值波动小: factor_name={}, strategy={}'.format(factor_name, "mimas"))
            return '因子值波动小'
        ...
        #     return '相同值比例较高: factor_name={}, strategy={}, %.2f'.format(
        #         factor_name, "mimas", factor_value_df[factor_name].value_counts(normalize=True).max())
...
def mimas_precheck_aftercalc(kls, error_factors, start_date, end_date, factor_value, factor_type, result_path):
    ...
    pkl_path = result_path + '/precheck/mimas/result/'
    ...
    if factor_type == precheck_utils.FactorUtil.FactorType.T_1_FACTOR:
        if str(start_date) == settings.precheck_path_dict["mimas"]["long_interval"][0] and str(end_date) == settings.precheck_path_dict["mimas"]["long_interval"][1]:
            check_dic['因子值一致性检查'] = value_same_check(kls, result_path + '/precheck/mimas/same_test/', filter_df)
        else:
            logger.warning("起止区间与settings中设置不一致，跳过长短区间检测！ factor={}, strategy=mimas".format(kls.factor_name))
