...
def value_same_check(kls, result_path, filter_df):
    ...
    logger.info('因子一致检查: factor_name={}, strategy={}'.format(factor_name, "mercury"))
    if not os.path.exists(result_path):
        os.system("mkdir -p " + result_path)
    long_interval = settings.precheck_path_dict["mercury"]["long_interval"]
    short_interval_list = settings.precheck_path_dict["mercury"]["short_interval_list"]

    db = {}
    factor_instance = precheck_utils.FactorUtil.create_factor_instance(kls)
    fill_na_value = factor_instance.fill_na_value
    s_xx = settings.mercury_s_xx
    ...
    try:
        if os.path.exists(
            ...
            if not db:
                db = precheck_utils.prepare_database_for_value_same_check(kls, 'mercury', str(long_interval[0]),
                                                                          str(long_interval[1]))
        ...
        return '函数运行出错: factor_name={}, strategy={}, 测试区间:{}-{}-{}'.format(factor_name, "mercury", long_interval[0],
                                                                           long_interval[1], e)
    ...
    try:
        ...
        if len(diff_short_date) > 0:
            return '区间测试未通过: factor_name={}, strategy={}, 不一致日期:{}'.format(factor_name, "mercury", str(diff_short_date))
    except Exception as e:
        return '函数运行出错: factor_name={}, strategy={}, 测试区间:{}-{}-{}'.format(factor_name, "mercury", short_date,short_date, e)
    return "pass"
...
def factor_value_test(factor_df, factor_name):
    ...
    if int(np.isinf(factor_df).sum()) > 0:
        logger.error('因子值存在inf: factor_name={}, strategy={}'.format(factor_name, "mercury"))
        res.append('因子值存在inf')
    if factor_df[factor_df.columns[0]].std() == 0:
        logger.error('因子值全部值为常数: factor_name={}, strategy={}'.format(factor_name, "mercury"))
        res.append('因子值全部值为常数')
    if factor_df.columns[0] != factor_name:
        logger.error('列名与因子名不符: factor_name={}, strategy={}'.format(factor_name, "mercury"))
        res.append('列名与因子名不符')
...
def score_check(factor_name, factor_value_df, filter_df):
    try:
        ...
        if factor_value_df.reindex(filter_df.index)[factor_name].std() < 1e-15:
            return '因子值波动小: factor_name={}, strategy={}'.format(factor_name, "mercury")

        if factor_value_df.reindex(filter_df.index)[factor_name].value_counts(normalize=True).max() > 0.2:
            return '相同值比例较高: factor_name={}, strategy={},'.format(factor_name, "mercury") + ' %.2f' % (
...
def mercury_precheck_aftercalc(kls, error_factors, start_date, end_date, factor_value, factor_type, result_path):
    logger.info("Start mercury factor precheck (after calc):  factor_name={}, factor_owner={}"
                .format(kls.factor_name, kls.owner))
    check_dic = {'代码格式检查': 'pass', '因子值一致性检查': '', '函数运行检查': '', '因子测试': '', '预检测': 'not pass'}
    pkl_path = result_path + '/precheck/mercury/result/'
    ...
    if kls.factor_name in error_factors:
        ...
        return
    filter_df = pd.read_hdf(settings.precheck_path_dict["mercury"]["precheck_basic_path"])
    ...
    if factor_type == precheck_utils.FactorUtil.FactorType.T_1_FACTOR:
        if str(start_date) == settings.precheck_path_dict["mercury"]["long_interval"][0] and str(end_date) == \
                settings.precheck_path_dict["mercury"]["long_interval"][1]:
            check_dic['因子值一致性检查'] = value_same_check(kls, result_path + '/precheck/mercury/same_test/', filter_df)
        else:
            logger.warning("起止区间与settings中设置不一致，跳过长短区间检测！ factor={}, strategy=mercury".format(kls.factor_name))
    ...
    if check_dic['函数运行检查'] != 'pass':
        logger.error("函数运行检查未通过！ factor_name={}, strategy={}".format(kls.factor_name, "mercury"))

    check_dic['因子测试'] = score_check(kls.factor_name, factor_value, filter_df_cpy)

    if check_dic['因子测试'] != 'pass':
        logger.error("因子测试未通过！ factor_name={}, strategy={}".format(kls.factor_name, "mercury"))

    if check_dic["函数运行检查"] == 'pass' and check_dic["代码格式检查"] == 'pass' and check_dic["因子值一致性检查"] == "pass" and \
            check_dic["因子测试"] == "pass":
        check_dic["预检测"] = 'pass'
    else:
        logger.error("预检测未通过！ factor_name={}, strategy={}".format(kls.factor_name, "mercury"))

    pre_check_res = pd.Series(check_dic)
    pre_check_res.to_pickle(pkl_path + '{}.pkl'.format(kls.factor_name))
    logger.info("预检测完成！ factor_name={}, strategy={}".format(kls.factor_name, "mercury"))
    return
