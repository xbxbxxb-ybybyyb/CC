...
class factor_tsq_neweuropa_20250123_1(BaseFactor):
    # 以下为因子基本信息
    strategy_name = "jupiter/europa"
    factor_name = "tsq_neweuropa_20250123_1"
    fill_na_value = 0
    need_pre_calculate_T_N = False
    owner = "tsq"  # 开发人员姓名
    factor_explain = "触发前30秒价格最大回撤偏度" # 因子逻辑解释
    ...
    def calculate(self, database):
        if database["skip"] == True:
            ...
            if len(data):
                ...
                data = data[(data['trigger_time'] - data['MDTime_delta']) <= 30 * 1000]
                res = (data['HighPx'] - data['LowPx']).skew()
            else:
                res = np.nan
