...
class factor_tsq_neweuropa_20250123_2(BaseFactor):
    # 以下为因子基本信息
    strategy_name = "jupiter/europa"
    factor_name = "tsq_neweuropa_20250123_2"
    fill_na_value = 0
    need_pre_calculate_T_N = False
    owner = "tsq"  # 开发人员姓名
    factor_explain = "触发前五分钟十档买卖挂单量不平衡" # 因子逻辑解释
            ...
            if len(data):
                ...
                for i in levels:
                    data[f'imbalance_level_{i}'] = (
                        data[f'Buy{i}OrderQty'] - data[f'Sell{i}OrderQty']
                    ) / (data[f'Buy{i}OrderQty'] + data[f'Sell{i}OrderQty'] + 1e-6)
