...
class factor_tsq_neweuropa_20241212_1(BaseFactor):
    # 以下为因子基本信息
    strategy_name = "jupiter/europa"
    factor_name = "tsq_neweuropa_20241212_1"
    fill_na_value = 0
    ...
    def calculate(self, database):
        if database["skip"] == True:
            ...
            data = data[data['TradeBSFlag'] == 2]
            data = generate_time_delta_930(data, 'MDTime_delta_trans')
            res = (data['MDTime_delta_trans'].diff()*data['TradeMoney']).sum()/data['MDTime_delta_trans'].diff().sum()
