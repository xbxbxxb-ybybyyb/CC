...
class factor_tsq_neweuropa_20241212_2(BaseFactor):
    # 以下为因子基本信息
    strategy_name = "jupiter/europa"
    factor_name = "tsq_neweuropa_20241212_2"
    fill_na_value = 0
    ...
    low_cost = "是" # 是否低耗时
    # 以下均为数据准备信息
    t_day_data = ['TTransaction','TOrder']
    xdb_data = []
    t_1_factor_data = []
    ...
    def calculate(self, database):
        if database["skip"] == True:
            ...
            data = data[data['TradePrice']/data['pre_close'] >= 1.095]
            df_buy = data.groupby('TradeBuyNo')['TradeMoney'].sum()
            BigBuyOrder = df_buy[df_buy < 50000].index
            df_bigbuy = data[data['TradeBuyNo'].isin(BigBuyOrder)]
            res = (df_bigbuy.groupby('TradeBuyNo')['TradeBSFlag'].mean()).mean()
