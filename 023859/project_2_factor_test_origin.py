...
class pj2FactorTest:
    def __init__(self, start_date, end_date, segment_number=20, style_list=['free_turn', 'T_o2pre'], cal_mi = True,
                 ...
                 lzt_pattern = [3,4], high_open = None # lzt_pattern = [3,4], high_open = 0.8
                 ):
    ...
    def load_data(self):
        ...
        sft_basic_path = '/dfs/group/800463/data/project2_public/factor_lib/sft_update.pkl'
        df = pd.read_pickle(sft_basic_path).loc[pd.to_datetime(str(int(self.start_date))):pd.to_datetime(str(int(self.end_date)))]
        ...
        if buy_type == '0940':
            all_filter = all_filter & ((df['T_day_first_ZT_Time'] <= 94000000) == False)& ((df['T_day_first_DT_Time'] <= 94000000) == False)
    ...
    def get_in_out_LS_performance(self):
        in_out_LS_performance = pd.DataFrame(index = self.label_list) #['value','class']
...
if __name__ == '__main__':
    ...
    df0 = pd.read_hdf('/data/user/023859/tmp/tsq_20241128_3.h5')
    result_path = '/data/user/023859/tmp/'
