...
class factor_skk_new_md_20250320_4(BaseFactor):
    strategy_name = "neptune"
    factor_name = "skk_new_md_20250320_4"  # 以这个来保存文件名
    fill_na_value = 2.014959
    need_pre_calculate_T_N = True
    owner = "skk"  # 开发人员姓名
    factor_explain = "涨跌幅的10日标准差" # 因子逻辑解释
    ...
    t_1_factor_data = [
        {'name': 'MD_CHINA_STOCK_DAILY_WIND', # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 80, #注意为正数
         'column': ['pct_chg', 'turn', 'close', 'pre_close', 'high', 'low', 'amt', 'vwap']
    }]  # T-N factor数据，格式如上
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            #####################
            # df_ori[self.factor_name] = df_ori['factor'].unstack().rolling(5, 1).mean().stack()
            ##############
            df_ori['tmp_score'] = df_ori['pct_chg'].unstack().rolling(10, min_periods=1).std().stack()
            df_ori[self.factor_name] = df_ori['tmp_score']
            #############
            # df_ori[self.factor_name] = df_ori['pct_chg']
            #############
            # df_ori[self.factor_name] = df_ori[self.factor_name].apply(lambda x: round_(x, 8))
            #####################
            # -------------------------------------------------------------------------------------------------------------------
