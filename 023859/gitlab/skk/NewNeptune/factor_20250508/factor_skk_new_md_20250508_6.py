...
class factor_skk_new_md_20250508_6(BaseFactor):
    strategy_name = "neptune"
    factor_name = "skk_new_md_20250508_6"
    fill_na_value = -0.615385
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori[self.factor_name] = df_ori['aaa'].unstack().rolling(4, 1).min().stack()
            #####################
            # -------------------------------------------------------------------------------------------------------------------
