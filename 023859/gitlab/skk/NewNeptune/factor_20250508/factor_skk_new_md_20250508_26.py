...
class factor_skk_new_md_20250508_26(BaseFactor):
    strategy_name = "neptune"
    factor_name = "skk_new_md_20250508_26"
    fill_na_value = -0.769737
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            #####################
            df_ori['aaa'] = (2 * df_ori['close']-df_ori['high']-df_ori['low'])/(df_ori['high']-df_ori['low']+1e-6)

            df_ori[self.factor_name] = df_ori['aaa'].unstack().rolling(5, 1).min().stack()
