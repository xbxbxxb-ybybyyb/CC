...
class factor_wj_neptune_20250410_6(BaseFactor):
    strategy_name = "neptune"
    factor_name = "wj_neptune_20250410_6"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "wj"  # 开发人员姓名
    factor_explain = "过去60日换手率变化/换手率均值" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True:  # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori[self.factor_name] = df_ori['factor'].unstack().rolling(60, 5).apply(lambda x: (x[-1]-x[0])/(1e-5+np.mean(x))).stack()
