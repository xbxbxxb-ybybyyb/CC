...
# 单独财务数据，非CS
class factor_wj_neptune_20250508_5(BaseFactor):
    strategy_name = "neptune"
    factor_name = "wj_neptune_20250508_5"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "wj"  # 开发人员姓名
    factor_explain = "近一年，预付款项+负债合计" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            #print(df_balancesheet.columns.tolist())
            #df_balancesheet = df_balancesheet[~df_balancesheet['ANN_DT'].isna()]

            df_balancesheet = df_balancesheet[df_balancesheet['ANN_DT'].apply(int) >= df_balancesheet['S_INFO_LISTDATE'].apply(int)]
            res = (df_balancesheet['PREPAY'] + df_balancesheet['TOT_LIAB']).tail(4).sum()
