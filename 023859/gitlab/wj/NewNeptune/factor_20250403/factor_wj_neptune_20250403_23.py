...
class factor_wj_neptune_20250403_23(BaseFactor):
    strategy_name = "neptune"
    factor_name = "wj_neptune_20250403_23"
    fill_na_value = 0
    need_pre_calculate_T_N = False
    owner = "wj"  # 开发人员姓名
    factor_explain = "委买量/委卖量/涨跌幅，求最大值" # 因子逻辑解释
    ...
    def calculate(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            if len(df) > 1:
                ...
                res3 = ((df['BidQty'])/(1e-3+df['OfferQty'])/(1e-3+df['pct_chg'])).max()# - df['OfferQty']
            else:
                res3 = 0
