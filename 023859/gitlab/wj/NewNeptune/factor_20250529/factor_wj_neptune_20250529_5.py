...
# 单独财务数据，非CS
class factor_wj_neptune_20250529_5(BaseFactor):
    strategy_name = "neptune"
    factor_name = "wj_neptune_20250529_5"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "wj"  # 开发人员姓名
    factor_explain = "" # 因子逻辑解释
    zcz_adjusted = "否" # 是否针对注册制调整：是/否
    logic_type = "少数股东权益合计，20期最大值/(均值-中位值)" # 逻辑类别
    low_cost = "是" # 是否低耗时
    xdb_data = [{
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True:  # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            res = ( df_balancesheet['factor_name']).groupby(['dt', 'Ticker']).apply(
                lambda x: (x.tail(20).max() )/(1e-3+x.tail(20).mean()- x.tail(20).median())).to_frame(name=self.factor_name)
