...
class factor_wj_neptune_20250417_8(BaseFactor):
    strategy_name = "neptune"
    factor_name = "wj_neptune_20250417_8"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "wj"  # 开发人员姓名
    factor_explain = "过去60日涨跌幅之和-过去30日涨跌幅之和" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True:  # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['long'] = df_ori['pct_chg'].unstack().rolling(60, 3).apply(lambda x: np.sum(x)).stack()
            df_ori['short'] = df_ori['pct_chg'].unstack().rolling(30, 3).apply(lambda x: np.sum(x)).stack()
