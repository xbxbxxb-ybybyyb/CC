...
class factor_wj_neptune_20250424_12(BaseFactor):
    strategy_name = "neptune"
    factor_name = "wj_neptune_20250424_12"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "wj"  # 开发人员姓名
    factor_explain = "机构买入卖出总量之差，10日变异系数" # 因子逻辑解释
    ...
    t_1_factor_data = [
        {'name': 'AShareMoneyFlow',  # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 80,  # 注意为正数
         'column': ['S_MFD_INFLOW_CLOSE','BUY_VOLUME_EXLARGE_ORDER','SELL_VOLUME_EXLARGE_ORDER']
         }]  # T-N factor数据，格式如上
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True:  # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['long'] = (df_ori['BUY_VOLUME_EXLARGE_ORDER']-df_ori['SELL_VOLUME_EXLARGE_ORDER']).unstack().rolling(10,
                                                                                                            1).apply(
                lambda x: np.mean(x)/(1e-3+np.std(x))).stack()
