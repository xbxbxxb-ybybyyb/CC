...
class factor_qyh_neptune_20250417_33(BaseFactor):
    owner = 'qyh'
    strategy_name = "neptune"
    factor_name = 'qyh_neptune_20250417_33'
    fill_na_value = 0
    need_pre_calculate_T_N = True
    factor_explain = "5分钟volume的5日std" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True:
            ...
            res = (df_volume).max(axis=1)
            # .divide(df_amt.max(axis=1).replace(0,np.nan))
            res = res.unstack().rolling(5).apply(f_calc_std).stack().to_frame()
