...
class factor_qyh_neptune_20250515_10(BaseFactor):
    owner = 'qyh'
    strategy_name = "neptune"
    factor_name = 'qyh_neptune_20250515_10'
    fill_na_value = 0
    need_pre_calculate_T_N = True
    factor_explain = "high 5 min" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True:
            ...
            res = (df).mean(axis=1)
            res = res.unstack().rolling(5,1).min().stack().to_frame()
