...
class factor_qyh_neptunelong_longterm_20250911_zjtx2zzc4(BaseFactor):
    strategy_name = "neptunelong"
    factor_name = "qyh_neptunelong_longterm_20250911_zjtx2zzc4"
    fill_na_value = ('industry_median',0)
    need_pre_calculate_T_N = True
    owner = "qyh"  # 开发人员姓名
    factor_explain = "财务：折旧与摊销占总资产比例" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            res1 = (df_balancesheet['单季度平均资产总额']).groupby(['dt','Ticker'])\
                .apply(lambda x : x.tail(4).mean()).to_frame(name='单季度平均资产总额')
            res2 = (df_cf[f'折旧摊销总额']).groupby(['dt','Ticker'])\
                .apply(lambda x : x.tail(4).mean()).to_frame(name='折旧摊销总额')
            res = pd.concat([res1,res2],axis=1)
            res[self.factor_name] = res['折旧摊销总额'] / res['单季度平均资产总额'].replace(0,np.nan)
