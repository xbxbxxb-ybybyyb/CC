...
class factor_xly_neptune_t_1_a22(BaseFactor):
    strategy_name = "neptune"
    factor_name = "xly_neptune_t_1_a22"
    fill_na_value = 0
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True:
            ...
            ind_df = df_ori.groupby(['date', 'Ind'])[['factor']].max()-df_ori.groupby(['date', 'Ind'])[['factor']].median()
            ind_df.columns = ['factor_ind']
            df_ori2 = pd.merge(df_ori[['date', 'Ind', 'factor']], ind_df, left_on=['date', 'Ind'], right_index=True, how='left')
            df_ori2[self.factor_name] = df_ori2['factor_ind'].unstack().rolling(3, min_periods=1).mean().stack()
