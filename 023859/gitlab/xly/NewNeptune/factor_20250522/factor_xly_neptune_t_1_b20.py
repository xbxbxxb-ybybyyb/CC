...
class factor_xly_neptune_t_1_b20(BaseFactor):
    strategy_name = "neptune"
    factor_name = "xly_neptune_t_1_b20"
    fill_na_value = 0
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True:
            ...
            try:
                tick_df_period1 = tick_df[(tick_df['MDTime'] >= 143000000) & (tick_df['MDTime'] < 145700000)]
                res = tick_df_period1['Sell1Price'].corr(tick_df_period1['Sell1OrderQty'])
                if np.isinf(res):
                    res = 1
            except:
