...
class factor_fc_n20250417_35(BaseFactor):
    owner = 'fc'
    strategy_name = "neptune"
    factor_name = sys._getframe().f_code.co_name[7:]
    fill_na_value = 1e8
    need_pre_calculate_T_N = True
    factor_explain = "过去两日散户买入总量标准差" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True:
            ...
            res = df_ori["BUY_VOLUME_SMALL_ORDER"].unstack().rolling(2, 1).apply(std_).stack()
