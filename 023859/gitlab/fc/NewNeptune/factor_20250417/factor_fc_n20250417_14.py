...
class factor_fc_n20250417_14(BaseFactor):
    owner = 'fc'
    strategy_name = "neptune"
    factor_name = sys._getframe().f_code.co_name[7:]
    fill_na_value = 1e8
    need_pre_calculate_T_N = True
    factor_explain = "过去一年尾盘资金流入率均值" # 因子逻辑解释
    ...
    t_1_factor_data = [
        {'name': 'AShareMoneyFlow', # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 360,
         'column': ["CLOSE_NET_INFLOW_RATE_VOLUME"]
    }]
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True:
            ...
            res = df_ori["CLOSE_NET_INFLOW_RATE_VOLUME"].unstack().rolling(120, 1).apply(mean_).stack()
