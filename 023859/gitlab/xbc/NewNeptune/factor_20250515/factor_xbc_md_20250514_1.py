...
class factor_xbc_md_20250514_1(BaseFactor):
    strategy_name = "neptune"
    factor_name = "xbc_md_20250514_1"
    fill_na_value = 1
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori[self.factor_name] = df_ori['factor'].unstack().rolling(9,1).apply(f_calc_avg).stack() + \
                                       df_ori['factor'].unstack().rolling(20,1).apply(f_calc_std).stack().apply(lambda x : round_(x,5)).replace(0,np.nan)
