...
class factor_xbc_md_20250421_3(BaseFactor):
    strategy_name = "neptune"
    factor_name = "xbc_md_20250421_3"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "xbc"  # 开发人员姓名
    factor_explain = "WeightedAvgBidPx滚动方差" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            md_data[factor_name] = (md_data['stat'] - stock_std).unstack().rolling(6, min_periods=1).mean().stack().round(5)
            # -------------------------------------------------------------------------------------------------------------------
