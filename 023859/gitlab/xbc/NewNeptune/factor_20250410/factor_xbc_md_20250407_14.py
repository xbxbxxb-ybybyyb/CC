...
class factor_xbc_md_20250407_14(BaseFactor):
    strategy_name = "neptune"
    factor_name = "xbc_md_20250407_14"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "xbc"  # 开发人员姓名
    factor_explain = "close的中位数和均值" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            md_data['stat'] = md_data['close'] / md_data['pre_close']
            md_data.loc[md_data['stat'] > 1.1, 'stat'] = 1.1  # 截断
            md_data.loc[md_data['stat'] < 0.9, 'stat'] = 0.9  # 截断

            md_data['stat2'] = md_data['stat'].unstack().rolling(90, 1).median().stack()
            md_data['stat3'] = md_data['stat'].unstack().rolling(3, 1).mean().stack()
            md_data['stat3'] = md_data['stat3'].apply(lambda x: np.nan if abs(x) <= 0.00001 else x)
            md_data[factor_name] = md_data['stat2']/ md_data['stat3']
            # -------------------------------------------------------------------------------------------------------------------
