...
class factor_xbc_md_20250414_12(BaseFactor):
    strategy_name = "neptune"
    factor_name = "xbc_md_20250414_12"
    fill_na_value = 0
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            def calculate_stat(x):
                x = np.array(x)
                result = np.max((np.maximum.accumulate(x) - x) / np.maximum.accumulate(x))
                return result

            md_data['stat'] = md_data.apply(lambda x: calculate_stat(x), axis=1)
            market_mean = md_data['stat'].unstack().mean(axis=1)
            md_data[factor_name] = (md_data['stat']-market_mean).unstack().rolling(3, min_periods=1).mean().stack() #46
            # -------------------------------------------------------------------------------------------------------------------
