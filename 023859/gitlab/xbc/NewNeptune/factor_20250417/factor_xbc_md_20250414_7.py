...
class factor_xbc_md_20250414_7(BaseFactor):
    strategy_name = "neptune"
    factor_name = "xbc_md_20250414_7"
    fill_na_value = 0
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            md_data[factor_name] = md_data[factor_name].unstack().rolling(1, min_periods=1).mean().stack()
            # -------------------------------------------------------------------------------------------------------------------
