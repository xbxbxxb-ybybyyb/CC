...
class factor_xbc_md_20250522_4(BaseFactor):
    strategy_name = "neptune"
    factor_name = "xbc_md_20250522_4"
    fill_na_value = 1
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['high'] = df_ori['high'].unstack().rolling(3,1).apply(f_calc_avg).stack()
            df_ori['close'] = df_ori['close'].unstack().rolling(3,1).apply(f_calc_avg).stack()
            df_ori['low'] = df_ori['low'].unstack().rolling(3,1).apply(f_calc_avg).stack()
            df_ori['vwap'] = df_ori['vwap'].unstack().rolling(3,1).apply(f_calc_avg).stack()
            df_ori['turn'] = df_ori['turn'].unstack().rolling(3,1).apply(f_calc_avg).stack()
            df_ori['pre_close'] = df_ori['pre_close'].unstack().rolling(3,1).apply(f_calc_avg).stack()
            df_ori['open'] = df_ori['open'].unstack().rolling(3,1).apply(f_calc_avg).stack()
            df_ori['amt'] = df_ori['amt'].unstack().rolling(3,1).apply(f_calc_avg).stack()
            df_ori['pct_chg'] = df_ori['pct_chg'].unstack().rolling(3,1).apply(f_calc_avg).stack()
            ...
            df_ori[self.factor_name] = df_ori['factor'].unstack().rolling(2,1).apply(f_calc_avg).stack() + \
                                       df_ori['factor'].unstack().rolling(4,1).apply(f_calc_std).stack().apply(lambda x : round_(x,5)).replace(0,np.nan)
