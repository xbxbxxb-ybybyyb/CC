...
class factor_xbc_md_20250527_5(BaseFactor):
    strategy_name = "neptune"
    factor_name = "xbc_md_20250527_5"
    fill_na_value = 1
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            def f_calc_median(factor_series):
                return factor_series[~np.isnan(factor_series)].median()
            df_ori['high'] = df_ori['high'].unstack().rolling(5,1).apply(f_calc_max).stack()
            df_ori['close'] = df_ori['close'].unstack().rolling(5,1).apply(f_calc_median).stack()
            df_ori['low'] = df_ori['low'].unstack().rolling(5,1).apply(f_calc_min).stack()
            df_ori['vwap'] = df_ori['vwap'].unstack().rolling(5,1).apply(f_calc_avg).stack()
            df_ori['turn'] = df_ori['turn'].unstack().rolling(2,1).apply(f_calc_avg).stack()
            df_ori['pre_close'] = df_ori['pre_close'].unstack().rolling(5,1).apply(f_calc_avg).stack()
            df_ori['open'] = df_ori['open'].unstack().rolling(5,1).apply(f_calc_median).stack()

            df_ori['s1'] = (df_ori['high'] - df_ori['close']) / df_ori['pre_close']/(df_ori['turn']+1)
            df_ori['s2'] = (df_ori['open'] - df_ori['low']) / df_ori['pre_close']/(df_ori['turn']+1)
            df_ori['factor'] = df_ori['s1'] - df_ori['s2']
            df_ori[self.factor_name] = df_ori['factor'].unstack().rolling(7,1).apply(f_calc_avg).stack()-df_ori['factor'].unstack().rolling(7,1).apply(f_calc_median).stack()
