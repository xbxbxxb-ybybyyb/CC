...
class factor_zwh_newNeptune_20250327_001(BaseFactor):
    owner = 'zwh'
    strategy_name = "neptune"
    factor_name = "zwh_newNeptune_20250327_001"
    fill_na_value = 0
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        pre_vwap = vwap.unstack().shift().stack().fillna(0.0)
        high = (df['high'] - df['close']) / pre_vwap / (1e-8 + np.log(1 + amt))
        high = high * 10
        N = 40
        nn = 2
        res = high.unstack().rolling(N, min_periods=N).mean().stack() - high.unstack().rolling(nn,min_periods=nn).mean().stack() #
