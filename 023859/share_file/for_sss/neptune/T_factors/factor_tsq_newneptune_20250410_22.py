...
class factor_tsq_newneptune_20250410_22(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250410_22"
    fill_na_value = 0
    need_pre_calculate_T_N = False
    owner = "tsq"  # 开发人员姓名
    factor_explain = "最大回撤波动" # 因子逻辑解释
    ...
    def calculate(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            if len(data) > 0:
                data['hl_pct'] = (data['HighPx'] - data['LastPx']) / data['pre_close']
                res = data['hl_pct'].std()
            else:
                res = np.nan
