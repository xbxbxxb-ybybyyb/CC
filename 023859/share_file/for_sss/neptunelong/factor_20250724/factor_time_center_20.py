...
class factor_time_center_20(BaseFactor):
    strategy_name = "neptunelong"
    factor_name = "time_center_20"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "wenj"  # 开发人员姓名
    factor_explain = "20日时间中心度因子" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            tmp = (df_ori['adj_pct_chg'].groupby('dt',group_keys=False).apply(lambda x: (x-x.mean())/x.std()) ** 2)
            factors =  1 / tmp.groupby('Ticker',group_keys=False).rolling(20).mean()
