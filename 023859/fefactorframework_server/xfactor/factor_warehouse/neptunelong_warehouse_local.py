...
s_xx = settings.neptunelong_s_xx
in_interval = settings.warehouse_settings_dict["neptunelong"]["in_interval"]
out_interval = settings.warehouse_settings_dict["neptunelong"]["out_interval"]
max_corr = settings.warehouse_settings_dict["neptunelong"]["max_corr"]

factor_data_path = settings.path_dict["neptunelong"]["factor_value_path"]
pre_check_path = settings.path_dict["neptunelong"]["factor_precheck_path"]
factor_test_path = settings.path_dict["neptunelong"]["factor_test_path"]

res_path = settings.warehouse_settings_dict["neptunelong"]["res_path"]
res_public_path = settings.warehouse_settings_dict["neptunelong"]["res_public_path"]
sft_basic_path = settings.warehouse_settings_dict["neptunelong"]["sft_basic_path"]
basic_factor_path = settings.warehouse_settings_dict["neptunelong"]["basic_factor_path"]

# 读取因子列表
test_factor_inf = pd.read_excel(settings.warehouse_settings_dict["neptunelong"]["all_factors_list"])
test_factor_inf = test_factor_inf[test_factor_inf['factor_type'] != 'other']
test_factor_inf = test_factor_inf[~test_factor_inf['factor_type'].str.contains('xdb_tickex|xdb_order|xdb_trade')] # neptunelong不使用T-1高频数据
...
for test_date in test_date_list[start_index:]:
    logger.info("-" * 20 + "neptunelong warehouse: test_date={}".format(test_date) + "-" * 20)
    ...
    if last_test_date is None:
        ...
        check_res_inf = pd.read_excel(res_path + 'check_res/check_res_tot_neptunelong_{}.xlsx'.format(last_test_date))[check_res_cols]
    ...
    for index, inf in day_factor_inf.iterrows():
        ...
        if '预检测' in pre_check.index:
            ...
            logger.error("入库失败！预检测结果格式异常！factor_name={}, strategy=neptunelong".format(factor_name))
            continue
        ...
        in_pkl = pd.read_pickle('{}/{}_{}/neptunelong/{}.pkl'.format(factor_test_path, start_date_new, in_interval[1], factor_name)) #TODO 确认路径
        out_pkl = pd.read_pickle('{}/{}_{}/neptunelong/{}.pkl'.format(factor_test_path, out_interval[0], out_interval[1], factor_name)) #TODO 确认路径
        ...
        corr_factor_ser = all_factor_df1.astype(float).rank().corrwith(test_factor_df1[factor_name].astype(float).rank()).abs()  # 比直接使用spearman更快
        ...
        if factor_res['bank_type'] != 'linear':
            ...
            if len(high_corr_factor_list) == 0:
                ...
                if len(high_corr_other_basic) > 0:
                    ...
                    elif (factor_res['bank_type'] == 'nonlinear') and (corr_factor_bank_type == 'nonlinear'): # 目前neptunelong无互信息
                        # 都为非线性，比较互信息
                        corr_max_mic = \
                            check_res_inf[check_res_inf['factor_name'].apply(lambda x: x in high_corr_factor_list)][
                                'Mutual_Info'].max()
    ...
    check_res_inf_.to_excel(res_path + 'check_res/check_res_tot_neptunelong_{}.xlsx'.format(test_date), index=False)
    ...
    check_res_inf_sta['factor_owner'] = check_res_inf_sta['factor_owner'].replace(
        {'fc': '冯炽', 'qyh': '秦雨豪', 'skk': '孙康康', 'sss': '孙少森', 'wj': '王敬', 'xbc': '徐碧村', 'xly': '谢璐遥',
         'zwh': '张文虎', 'tsq':'唐松乔', 'wenj':'文晶', 'zxj':'赵行健'})
    check_res_inf_sta.to_excel(res_path + 'check_res/check_res_tot_neptunelong_sta_{}.xlsx'.format(test_date), index=False) #统计截图

    # 输出结果
    update_file = res_path + 'sft_update_931_20160101_20211231.pkl'
    all_factor_df.to_pickle(update_file)

    # 公共地址输出结果
    check_res_inf_.to_excel(res_public_path + 'check_res_tot_neptunelong.xlsx', index=False)

    update_public_file = res_public_path + 'sft_update_931_20160101_20201231.pkl'
    all_factor_df.loc[pd.to_datetime(str(in_interval[0])):pd.to_datetime(str(in_interval[1]))].to_pickle(update_public_file)
