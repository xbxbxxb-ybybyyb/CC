...
def neptune_warehouse():
    ...
    for test_date in test_date_list[start_index:]:
        ...
        if last_test_date is None:
            ...
            check_res_inf = pd.read_excel(res_path + 'check_res/check_res_tot_neptune_{}.xlsx'.format(last_test_date))[
                check_res_cols]
        ...
        check_res_inf_sta['factor_owner'] = check_res_inf_sta['factor_owner'].replace(
            {'fc': '冯炽', 'qyh': '秦雨豪', 'skk': '孙康康', 'sss': '孙少森', 'wj': '王敬', 'xbc': '徐碧村', 'xly': '谢璐遥',
             'zwh': '张文虎'})
        check_res_inf_sta.to_excel(res_path + 'check_res/check_res_tot_neptune_sta.xlsx'.format(test_date), index=False)
