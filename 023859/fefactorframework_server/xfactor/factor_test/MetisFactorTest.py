...
class strongFactorTest:
    def __init__(self, start_date, end_date, segment_number=20,
                 ...
                 filter_name='',
                 strategy_name=''):
        self.start_date, self.end_date = int(start_date), int(end_date)
    ...
    def load_data(self):
        df = IO.read_data([self.start_date, self.end_date],alt='/dfs/user/018107/factor_zoo1/factor_lib_metis/sft_update_metis_filter_20160101_20201231.h5')

        # 样本筛选
        #
        df['label_T_is_touch'] = (df['label_T_is_touch'] ==1).astype(int) # 2024-01
        df = df.rename(columns = {'label_T_is_touch':'class', 'label_pct_graded':'value'})
        ...
        df['Touch_Time'] = df['trigger_time_ms']
    ...
    def factor_test(self, factor_df, cost_list, result_path, factor_corr_test=False, generate_pdf=True, tag=''):
        ...
        if factor_corr_test:
            check_res=pd.read_excel('/dfs/group/800463/data/project1_public/factor_lib_metis/check_res_tot_metis.xlsx')
        ...
        if 'Touch_Time' in self.result_dic['double_group_sort']:
            self.result_dic['check_diff_res'], self.result_dic['check_score_res'] = self.check_score(self.result_dic['double_group_sort']['Touch_Time'].copy(), self.result_dic['corr_month'].copy(), np.sign(self.result_dic['corr_sta'].loc['corr_tot']))
    ...
    def get_factor_information(self):
        ...
        factor_information[' '] = ['Segment Number', 'Style List', '', '', '', '', '', '']
        factor_information['Settings'] = [self.segment_number, str(self.style_list), '', '', '', '', '', '']
