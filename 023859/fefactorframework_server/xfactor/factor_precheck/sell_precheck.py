...
def value_same_check(kls, result_path, filter_df, preload_database):
    ...
    full_factor_value_path = os.path.join(settings.path_dict["sell"]["factor_precheck_path"] + "/same_test/",
    ...
    try:
        ...
        return '函数运行出错: factor_name={}, strategy={}, 测试区间:{}-{}-{}'.format(factor_name, "sell", long_interval[0], long_interval[1], e)
    ...
    try:
        ...
        for short_date in short_interval_list:
            if os.path.exists(result_path + "/{}_{}_{}.pkl".format(factor_name, short_date, short_date)):
                ...
                if not db:
                    logger.error("sell precheck - preload database empty!")
                short_db = {"skip": False}
                for data_dict in kls.t_1_factor_data:
                    break_flag = True
                    counter = 0
            ...
            if (long_ser.shape != short_ser.shape) or ((long_ser - short_ser).abs().max() > 1e-6):
                logger.info(
                    '因子检查不一致: factor_name={}, date={}, strategy=sell, long_shape={}, short_shape={}, diff={}, diff_stock={}'.format(
        ...
        return '函数运行出错: factor_name={}, strategy={}, 测试区间:{}-{}-{}'.format(factor_name, "sell", short_date, short_date, e)
    return "pass"
...
def score_check(factor_name, factor_value_df, filter_df):
    ...
    if factor_value_df.reindex(filter_df.index)[factor_name].value_counts(normalize=True).max()>0.2:
        return '相同值比例较高: factor_name={}, strategy={}'.format(factor_name, "sell") + ', %.2f' % (factor_value_df.reindex(filter_df.index)[factor_name].value_counts(normalize=True).max())

    return 'pass'


def sell_precheck_aftercalc(kls, pre_calc_check_res, error_factors, start_date, end_date, factor_type, result_path, preload_database):
    ...
    if kls.factor_name in error_factors:
        ...
        pre_check_res.to_pickle(settings.path_dict["sell"]["factor_precheck_path"] + '{}.pkl'.format(kls.factor_name))
        return

    factor_value_path = os.path.join(settings.path_dict["sell"]["factor_value_path"], kls.factor_name, kls.factor_name + ".h5")
    if os.path.exists(factor_value_path):
        factor_value = pd.read_hdf(factor_value_path)
    else:
        logger.error("未找到相关因子值文件！ factor_name={}, strategy=sell, value_path={}".format(kls.factor_name, factor_value_path))
