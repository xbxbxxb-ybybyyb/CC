...
# 单独财务数据，非CS
class factor_wj_neptune_longterm_20250626_1(BaseFactor):
    strategy_name = "neptune"
    factor_name = "wj_neptune_longterm_20250626_1"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "wj"  # 开发人员姓名
    factor_explain = "少数股东权益合计，20期均值与中位值之差/末期值" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True:  # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            res = (df_balancesheet['factor_name']).groupby(['dt', 'Ticker']).apply(
                lambda x: ( x.tail(20).mean()-x.tail(20).median()) / (1e-3 + x.iloc[-1])).to_frame(
