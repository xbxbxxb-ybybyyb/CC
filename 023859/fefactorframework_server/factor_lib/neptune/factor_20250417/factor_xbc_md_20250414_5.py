...
class factor_xbc_md_20250414_5(BaseFactor):
    strategy_name = "neptune"
    factor_name = "xbc_md_20250414_5"
    fill_na_value = 0
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            md_data['score1'] = (md_data['vwap']/md_data['pre_close'])/(md_data['close']/md_data['pre_close'])
            md_data['score2'] = (md_data['high']/md_data['pre_close'])/(md_data['low']/md_data['pre_close'])
            ########
            md_data['score'] = md_data['score1'] * md_data['score2']
            md_data[factor_name] = md_data['score'].unstack().rolling(25, min_periods=1).median().stack() * md_data['score'].unstack().rolling(25, min_periods=1).min().stack()
            # -------------------------------------------------------------------------------------------------------------------
