...
class factor_tsq_newneptune_20250417_9(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250417_9"
    fill_na_value = 0
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['factor'] = (1e6 * (df_minute5_high[columns].div(df_minute5_high['pre_close'].replace(0,np.nan),axis=0) - 1) / df_minute5_amt[columns].replace(0,np.nan)).std(axis=1).apply(lambda x : round_(x,5))
            df_ori['factor'] = df_ori['factor'].unstack().rolling(20,1).apply(f_calc_std).stack()
