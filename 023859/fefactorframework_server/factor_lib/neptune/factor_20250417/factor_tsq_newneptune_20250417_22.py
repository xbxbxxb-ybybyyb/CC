...
class factor_tsq_newneptune_20250417_22(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250417_22"
    fill_na_value = 0
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        for date in data.index.get_level_values(0).unique():
            ...
            data_['BidAdd'] = data_['TotalBidQty'] - data_['TotalBidQty'].shift(1).fillna(0)
            data_['AskAdd'] = data_['TotalOfferQty'] - data_['TotalOfferQty'].shift(1).fillna(0)
            data_ = filter_930(data_)
            data_all.append(data_)
        data_all = pd.concat(data_all)
        if data_all['ValueTrade'].sum():
            res = 1e4 * (data_all['BidAdd'] - data_all['AskAdd']).mean() / data_all['ValueTrade'].sum()
        else:
            res = np.nan
