...
class factor_wj_neptune_20250417_2(BaseFactor):
    strategy_name = "neptune"
    factor_name = "wj_neptune_20250417_2"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "wj"  # 开发人员姓名
    factor_explain = "涨跌幅/成交量，过去60日均值/标准差" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True:  # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['short'] = df_ori['turn'].unstack().rolling(60, 3).apply(lambda x: np.mean(x)/(1e-4+np.std(x))).stack()
