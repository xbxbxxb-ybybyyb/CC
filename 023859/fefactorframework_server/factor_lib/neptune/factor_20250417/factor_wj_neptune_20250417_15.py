...
class factor_wj_neptune_20250417_15(BaseFactor):
    strategy_name = "neptune"
    factor_name = "wj_neptune_20250417_15"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "wj"  # 开发人员姓名
    factor_explain = "过去30日，成交额，（均值-中位值）/振幅" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True:  # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['short'] = df_ori['amt'].unstack().rolling(30, 3).apply(lambda x: (np.mean(x)-np.median(x))/(1e-3 + np.max(x)- np.min(x))).stack()
