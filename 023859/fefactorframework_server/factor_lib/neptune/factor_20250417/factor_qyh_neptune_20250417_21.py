...
class factor_qyh_neptune_20250417_21(BaseFactor):
    strategy_name = "neptune"
    factor_name = "qyh_neptune_20250417_21"
    fill_na_value = 1
    need_pre_calculate_T_N = True
    owner = "qyh"  # 开发人员姓名
    factor_explain = "vwap 360 加权变化率" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['factor'] = (df_ori['vwap']) * df_ori['amt']
            df_ori[self.factor_name] = df_ori['factor'].unstack().rolling(360,1).apply(f_calc_change).stack() \
                                       / df_ori['amt'].unstack().rolling(360,1).apply(f_calc_change).stack().apply(lambda x: round_(x, 2)).replace(0, np.nan)
