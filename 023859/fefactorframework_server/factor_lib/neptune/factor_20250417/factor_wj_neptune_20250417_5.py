...
class factor_wj_neptune_20250417_5(BaseFactor):
    strategy_name = "neptune"
    factor_name = "wj_neptune_20250417_5"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "wj"  # 开发人员姓名
    factor_explain = "（当日-均值）/标准差，过去60日-过去10日" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True:  # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['long'] = df_ori['amt'].unstack().rolling(60, 5).apply(lambda x: (x[-1]-np.mean(x))/(1e-5+np.std(x))).stack()
            df_ori['short'] = df_ori['amt'].unstack().rolling(10, 5).apply(lambda x: (x[-1] - np.mean(x)) / (1e-5 + np.std(x))).stack()
