...
class factor_tsq_newneptune_20250417_18(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250417_18"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "成交量加权平均收益率" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        data_all['AvgPx'] = data_all['ValueTrade'] / data_all['VolumeTrade']
        data_all['ret'] = 100 * data_all['AvgPx'].pct_change()
        res = np.sum(data_all['ret'] * data_all['VolumeTrade']) / np.sum(data_all['VolumeTrade'])
