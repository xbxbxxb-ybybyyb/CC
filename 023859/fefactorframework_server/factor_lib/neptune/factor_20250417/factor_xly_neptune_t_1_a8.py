...
class factor_xly_neptune_t_1_a8(BaseFactor):
    strategy_name = "neptune"
    factor_name = "xly_neptune_t_1_a8"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "xly"  # 开发人员姓名
    factor_explain = "涨跌幅方差" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            print(df_ori.head())
            df_ori[self.factor_name] = df_ori['factor'].unstack().rolling(5,min_periods=1).std().stack()
