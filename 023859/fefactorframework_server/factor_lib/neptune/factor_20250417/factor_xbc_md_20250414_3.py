...
class factor_xbc_md_20250414_3(BaseFactor):
    strategy_name = "neptune"
    factor_name = "xbc_md_20250414_3"
    fill_na_value = 0
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            for i in range(len(m_cols)-1):
                md_data2[m_cols[i]] = md_data2[m_cols[i+1]]-md_data2[m_cols[i]]
            m_cols = m_cols[:-1]
            ...
            while start_ind+window<=len(m_cols):
                md_data2['sum'] += md_data2[m_cols[start_ind:start_ind+window]].mean(axis=1)
                start_ind += step

            factor_df = pd.DataFrame()
            factor_df[factor_name] = md_data2['sum']
            # -------------------------------------------------------------------------------------------------------------------
