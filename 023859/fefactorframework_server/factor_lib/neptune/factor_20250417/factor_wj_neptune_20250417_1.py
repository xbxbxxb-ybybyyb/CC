...
class factor_wj_neptune_20250417_1(BaseFactor):
    strategy_name = "neptune"
    factor_name = "wj_neptune_20250417_1"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "wj"  # 开发人员姓名
    factor_explain = "涨跌幅/成交量，过去50日最大值-中位值" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True:  # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['factor'] = 1e6*df_ori['pct_chg']/(1e-4+df_ori['volume'])
            df_ori['long'] = df_ori['factor'].unstack().rolling(50, 3).apply(lambda x: np.max(x)-np.median(x)).stack()
            df_ori[self.factor_name] = (df_ori['long']).apply(
