...
class factor_tsq_newneptune_20250417_30(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250417_30"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "K线指标"  # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        for date in data.index.get_level_values(0).unique():
            ...
            data_['High'] = data_['LastPx'].cummax()
            data_['Low'] = data_['LastPx'].cummin()
            data_all.append(data_)
        data_all = pd.concat(data_all)
        res = np.sum(data_all['VolumeTrade'] * (2 * data_all['LastPx'] - data_all['High'] - data_all['Low']) / (data_all['High'] + data_all['Low']))
