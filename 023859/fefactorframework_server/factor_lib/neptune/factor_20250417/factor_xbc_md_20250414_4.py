...
class factor_xbc_md_20250414_4(BaseFactor):
    strategy_name = "neptune"
    factor_name = "xbc_md_20250414_4"
    fill_na_value = 0
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            def max_drawdown(x):
                ...
                for i in range(len(x)):
                    if len(profit_list) == 0:
                        profit_list.append(x[i])
                    else:
                        profit_list.append(profit_list[-1] + x[i])
                end = np.argmax(np.maximum.accumulate(profit_list) - profit_list)
                if end == 0:
                    return 0
                start = np.argmax(profit_list[:end])
                risk = profit_list[start] - profit_list[end]
                return risk
            ...
            def standard(x):
                ...
                return x
            ##########################################
            md_data['stat'] = md_data['pct_chg'].unstack().rolling(4, min_periods=1).apply(lambda x: max_drawdown(x)).stack()
            md_data['stat'] = md_data['stat'].groupby(level=[0]).apply(lambda x: standard(x))
            md_data[factor_name] = md_data['stat'].unstack().rolling(5, min_periods=1).mean().stack() + md_data['stat'].unstack().rolling(10, min_periods=1).mean().stack()
            # -------------------------------------------------------------------------------------------------------------------
