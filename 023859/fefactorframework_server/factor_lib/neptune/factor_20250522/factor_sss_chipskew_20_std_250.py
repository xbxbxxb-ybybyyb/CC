...
class factor_sss_chipskew_20_std_250(BaseFactor):
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            for i in range(1, chip_period + 1):
                ...
                md['atr_sum'] += md['atr%s' % i]
                md['value_sum'] += md['atr%s' % i] * md['rc%s' % i]
            md['mean'] = md['value_sum'] / md['atr_sum']

            md['vvalue_sum'] = 0
            md['vvalue_sum'] += md['atr0'] * ((md['rc0'] - md['mean']) ** 2)
            for i in range(1, chip_period + 1):
                md['vvalue_sum'] += md['atr%s' % i] * ((md['rc%s' % i] - md['mean']) ** 2)
            md['var'] = md['vvalue_sum'] / md['atr_sum']

            md['vvvalue_sum'] = 0
            md['vvvalue_sum'] += md['atr0'] * ((md['rc0'] - md['mean']) ** 3)
            for i in range(1, chip_period + 1):
                md['vvvalue_sum'] += md['atr%s' % i] * ((md['rc%s' % i] - md['mean']) ** 3)
            md['value'] = md['vvvalue_sum'] / md['atr_sum'] / (md['var'] ** 1.5)
