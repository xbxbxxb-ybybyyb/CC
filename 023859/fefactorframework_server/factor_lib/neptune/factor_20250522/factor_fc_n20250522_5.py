...
class factor_fc_n20250522_5(BaseFactor):
    owner = 'fc'
    strategy_name = "neptune"
    factor_name = sys._getframe().f_code.co_name[7:]
    fill_na_value = 1e8
    need_pre_calculate_T_N = True
    factor_explain = "近1个月中单流出单数的偏度" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True:
            ...
            res = df_ori["SELL_TRADES_MED_ORDER"].unstack().rolling(20, 3).apply(skew_).stack()
