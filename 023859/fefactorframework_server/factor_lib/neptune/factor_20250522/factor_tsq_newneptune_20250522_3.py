...
class factor_tsq_newneptune_20250522_3(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250522_3"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "财务-营业总收入-营业总成本 每年均值" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            res = (df_income['TOT_OPER_REV'] - df_income['TOT_OPER_COST']).groupby(['dt','Ticker']).apply(lambda x : x.tail(4).mean()).to_frame(name = self.factor_name)
