...
class factor_qyh_neptune_20250522_1(BaseFactor):
    owner = 'qyh'
    strategy_name = "neptune"
    factor_name = 'qyh_neptune_20250522_1'
    fill_na_value = 0
    need_pre_calculate_T_N = True
    factor_explain = "vol 5m max" # 因子逻辑解释
    ...
    t_1_factor_data_types = ['minute5','MD']
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True:
            ...
            #
            df = df_volume.divide(md_data['adjfactor'], axis=0)
            res = (df).mean(axis=1)
            # .divide(df_amt.max(axis=1).replace(0,np.nan))
            res = res.unstack().rolling(5,1).max().stack().to_frame()
            # res = res.to_frame()
