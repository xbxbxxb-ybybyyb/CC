...
class factor_tsq_newneptune_20250522_1(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250522_1"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "财务-未分配利润偏度" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            res = df_balancesheet.groupby(['dt','Ticker'])['UNDISTRIBUTED_PROFIT'].apply(lambda x : x.tail(12).skew()).to_frame(name = self.factor_name)
