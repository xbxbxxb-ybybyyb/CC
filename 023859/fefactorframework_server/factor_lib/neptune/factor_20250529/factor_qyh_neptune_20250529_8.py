...
class factor_qyh_neptune_20250529_8(BaseFactor):
    strategy_name = "neptune"
    factor_name = "qyh_neptune_20250529_8"
    fill_na_value = 1
    need_pre_calculate_T_N = True
    owner = "qyh"  # 开发人员姓名
    factor_explain = "20 abspct" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori[self.factor_name] = df_ori['factor'].unstack().rolling(20,1).max().stack() / df_ori['amt'].unstack().rolling(20,1).max().stack().replace(0,np.nan)
            # -------------------------------------------------------------------------------------------------------------------
