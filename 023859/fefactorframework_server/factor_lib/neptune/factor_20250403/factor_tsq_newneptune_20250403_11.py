...
class factor_tsq_newneptune_20250403_11(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250403_11"
    fill_na_value = 1
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "长短期波动成交比" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['ATR_short'] = df_ori['TR'].unstack().rolling(5,1).mean().stack().apply(lambda x: round_(x, 5))
            df_ori['ATR_long'] = df_ori['TR'].unstack().rolling(20,1).mean().stack().apply(lambda x: round_(x, 5))
            df_ori['factor'] = df_ori['ATR_short']/df_ori['ATR_long'].replace(0,np.nan)
