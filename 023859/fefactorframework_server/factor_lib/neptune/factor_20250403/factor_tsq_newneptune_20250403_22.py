...
class factor_tsq_newneptune_20250403_22(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250403_22"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "价差与成交量排序乘积" # 因子逻辑解释
    ...
    t_1_factor_data = [
        {'name': 'MD_CHINA_STOCK_DAILY_WIND',  # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 20,  # 注意为正数
         'column': ['vwap','close','pre_close','adjfactor','volume']
         }
    ]  # T-N factor数据，格式如上
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['vwap_close'] = df_ori['vwap'] - df_ori['close']
            df_ori['vwap_close_min'] = df_ori['vwap_close'].unstack().rolling(3,1).min().stack()
            df_ori['vwap_close_max'] = df_ori['vwap_close'].unstack().rolling(3,1).max().stack()
            df_ori['delta_vol'] = df_ori['volume'] - df_ori['volume'].unstack().shift(3).stack().fillna(0)
            df_ori['factor1'] = (df_ori['vwap_close_min'].unstack().rank(axis=1) / (~ df_ori['vwap_close_min'].unstack().isnull()).values.sum(axis=1).reshape(-1, 1)).stack()
            df_ori['factor2'] = (df_ori['vwap_close_max'].unstack().rank(axis=1) / (~ df_ori['vwap_close_max'].unstack().isnull()).values.sum(axis=1).reshape(-1, 1)).stack()
            df_ori['factor3'] = (df_ori['delta_vol'].unstack().rank(axis=1) / (~ df_ori['delta_vol'].unstack().isnull()).values.sum(axis=1).reshape(-1, 1)).stack()
            df_ori['factor'] = (df_ori['factor1']+df_ori['factor2'])*df_ori['factor3']
