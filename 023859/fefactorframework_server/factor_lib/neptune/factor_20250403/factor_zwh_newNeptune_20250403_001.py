...
class factor_zwh_newNeptune_20250403_001(BaseFactor):
    owner = 'zwh'
    strategy_name = "neptune"
    factor_name = "zwh_newNeptune_20250403_001"
    fill_na_value = 0
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        zt_price = np.floor(df['pre_close'] * 100 * 1.1 + 0.5) / 100
        N = 30 ## 40 32.6
        v_max = vwap.unstack().rolling(N, min_periods=N).max().stack()  # ?
        v_min = vwap.unstack().rolling(N, min_periods=N).min().stack()  ##?
        res = zt_price/(EPS+v_min)+  v_max/(EPS+zt_price)
