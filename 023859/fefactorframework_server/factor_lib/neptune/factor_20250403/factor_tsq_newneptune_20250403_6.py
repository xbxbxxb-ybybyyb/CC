...
class factor_tsq_newneptune_20250403_6(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250403_6"
    fill_na_value = 0
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['ATR'] = df_ori['TR'].unstack().rolling(60,1).mean().stack().apply(lambda x: round_(x, 5))
            df_ori['volume'] = df_ori['volume'].unstack().rolling(60, 1).std().stack().apply(lambda x: round_(x, 5))
