...
class factor_tsq_newneptune_20250403_28(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250403_28"
    fill_na_value = 1
    ...
    t_1_factor_data = [
        {'name': 'MD_CHINA_STOCK_DAILY_WIND',  # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 40,  # 注意为正数
         'column': ['pre_close','adjfactor','high','close','vwap','volume']
         }
    ]  # T-N factor数据，格式如上
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['factor1'] = (df_ori['vwap'] - df_ori['close']) / df_ori['pre_close']
            df_ori['factor2'] = (df_ori['vwap'] + df_ori['close']) / df_ori['pre_close']
            ...
            df_ori['factor'] = df_ori['factor'].unstack().rolling(20,1).mean().stack()
