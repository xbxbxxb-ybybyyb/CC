...
class factor_tsq_newneptune_20250403_5(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250403_5"
    fill_na_value = 1
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['factor1'] = df_ori['high_close_1'].unstack().rolling(120,1).std().stack().apply(lambda x : round_(x,5))
            df_ori['factor2'] = df_ori['close_1_low'].unstack().rolling(120,1).std().stack().apply(lambda x: round_(x, 5))
