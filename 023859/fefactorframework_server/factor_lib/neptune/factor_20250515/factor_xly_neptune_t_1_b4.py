...
class factor_xly_neptune_t_1_b4(BaseFactor):
    strategy_name = "neptune"
    factor_name = "xly_neptune_t_1_b4"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "xly"  # 开发人员姓名
    factor_explain = "总委买量占比" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True:
            ...
            try:
                ...
                res = tick_df[tick_df['MDTime'] >= 143000000]['TotalBidQty'].sum()/tick_df['TotalBidQty'].sum()
                if np.isinf(res):
                    res = 1
            except:
