...
class factor_turn_coverage(BaseFactor):
    strategy_name = "neptune"
    factor_name = "turn_coverage"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "wenj"  # 开发人员姓名 TODO 用你的拼音，本来应该是wj，但组里有同事名字缩写也是wj，这里给你改成wenj了
    factor_explain = "换手率均线收敛因子"  # 因子逻辑解释 TODO 请简要填写因子逻辑
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True:  # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            factor = -np.log(1 + rolling_means.std(axis=1))
            df_ori[self.factor_name] = factor.groupby('dt',group_keys=False).apply(lambda x: (x-x.mean())/x.std())
