...
class factor_tsq_newneptune_20250327_6(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250327_6"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "成交额异动因子" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['last_amt'] = df_ori['amt'].unstack().shift(1).stack()
            df_ori['amt_pct_chg'] = df_ori['amt']/df_ori['last_amt'] - 1
            df_ori['factor'] = df_ori['amt_pct_chg'].unstack().rolling(20,5).std().stack()
