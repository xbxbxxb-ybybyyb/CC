...
class factor_qyh_neptune_20250327_34(BaseFactor):
    strategy_name = "neptune"
    factor_name = "qyh_neptune_20250327_34"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "qyh"  # 开发人员姓名
    factor_explain = "量价比例/20偏度" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori[self.factor_name] = df_ori['factor'] / \
                                       df_ori['factor'].unstack().rolling(20,1).apply(f_calc_skew).stack()
