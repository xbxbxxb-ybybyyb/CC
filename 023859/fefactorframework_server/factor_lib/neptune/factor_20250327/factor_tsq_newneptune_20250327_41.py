...
class factor_tsq_newneptune_20250327_41(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250327_41"
    fill_na_value = 1
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "尾盘资金流入强度" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['factor'] = (df_ori['S_MFD_INFLOW_CLOSE'] / (df_ori['S_MFD_INFLOW_CLOSE'].unstack().rolling(60,1).max().stack()).replace(0,np.nan))
