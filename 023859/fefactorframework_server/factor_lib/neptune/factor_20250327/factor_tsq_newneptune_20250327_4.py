...
class factor_tsq_newneptune_20250327_4(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250327_4"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "RSI类指标" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['pct_chg_up'] = df_ori['pct_chg'].apply(lambda x : x if x>0 else 0)
            df_ori['pct_chg_down'] = df_ori['pct_chg'].apply(lambda x : x if x<0 else 0)

            df_ori['RV_up'] =   ((df_ori['pct_chg_up'])**2).unstack().rolling(40,1).sum().stack().apply(lambda x : round_(x,5))
            df_ori['RV_down'] =   ((df_ori['pct_chg_down'])**2).unstack().rolling(40,1).sum().stack().apply(lambda x : round_(x,5))
            df_ori['RV'] =   ((df_ori['pct_chg'])**2).unstack().rolling(40,1).sum().stack().apply(lambda x : round_(x,5))
            df_ori['factor'] = (df_ori['RV_up'] - df_ori['RV_down']) / df_ori['RV'].replace(0,np.nan)
