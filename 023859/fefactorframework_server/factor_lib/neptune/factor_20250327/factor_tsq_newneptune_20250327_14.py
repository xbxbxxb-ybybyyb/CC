...
class factor_tsq_newneptune_20250327_14(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250327_14"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "成交量加权涨跌" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['volume_40_mean'] = df_ori['volume'].unstack().rolling(40,1).mean().stack()
            df_ori['factor'] = (df_ori['volume'] / df_ori['volume_40_mean'].replace(0,np.nan)).apply(lambda x : round_(x,5))
            df_ori['factor'] = (df_ori['factor']*df_ori['pct_chg']).apply(lambda x : round_(x,5))
            df_ori['factor'] = df_ori['factor'].unstack().rolling(40,1).mean().stack()
