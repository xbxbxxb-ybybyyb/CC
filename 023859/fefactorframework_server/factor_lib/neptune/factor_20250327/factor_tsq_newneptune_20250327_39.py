...
class factor_tsq_newneptune_20250327_39(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250327_39"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "收盘价加权振幅" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            # -------------------------------------------------------------------------------------------------------------------
            df_ori['swing'] = ((df_ori['high']-df_ori['low'])/df_ori['pre_close']).apply(lambda x: round_(x, 5))
            df_ori['factor'] = (df_ori['close']*df_ori['swing']).unstack().rolling(60,1).sum().stack().apply(lambda x: round_(x, 5))
            df_ori['factor'] = df_ori['factor'] / df_ori['close'].unstack().rolling(60,1).sum().stack().replace(0,np.nan)
