...
class factor_qyh_neptune_20250327_43(BaseFactor):
    owner = 'qyh'
    strategy_name = "neptune"
    factor_name = 'qyh_neptune_20250327_43'
    fill_na_value = 0
    need_pre_calculate_T_N = True
    factor_explain = "最高价的5日加权平均变式" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True:
            ...
            df_amt = database['amt']
            res = (df_high * df_amt).median(axis=1).divide(df_amt.max(axis=1).replace(0,np.nan))
            # res = df_amt.max(axis=1)
            res = res.unstack().rolling(5).min().stack().to_frame()
