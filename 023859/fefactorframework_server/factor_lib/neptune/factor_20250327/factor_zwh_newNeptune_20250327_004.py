...
class factor_zwh_newNeptune_20250327_004(BaseFactor):
    owner = 'zwh'
    strategy_name = "neptune"
    factor_name = "zwh_newNeptune_20250327_004"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    factor_explain = "zt日买卖力量对比统计" # 因子逻辑解释
    ...
    t_1_factor_data = [
        {'name': 'MD_CHINA_STOCK_DAILY_WIND',  # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 100,  # 注意为正数
         'column': ['pct_chg', 'turn', 'close', 'open', 'pre_close', 'adjfactor']
         }]
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        amt_rank = (turn.unstack().rank(axis=axis_) / (~ turn.unstack().isnull()).values.sum(axis=axis_).reshape(-1,1)).stack()
        pct_rank = (pct.unstack().rank(axis=axis_) / (~ pct.unstack().isnull()).values.sum(axis=axis_).reshape(-1,1)).stack()
        pct_rank = pct_rank.unstack().rolling(5, min_periods=5).mean().stack()
        amt_rank = amt_rank.unstack().rolling(5, min_periods=5).mean().stack()

        res = pct_rank / (0.5 + amt_rank)
        # N = 40
        # res = result.unstack().rolling(N, min_periods=N).mean().stack() - result
