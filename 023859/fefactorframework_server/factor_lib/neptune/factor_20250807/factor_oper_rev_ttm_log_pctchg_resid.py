...
class factor_oper_rev_ttm_log_pctchg_resid(BaseFactor):
    strategy_name = "neptune"
    factor_name = "oper_rev_ttm_log_pctchg_resid"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "wenj"  # 开发人员姓名
    factor_explain = "使用TTM对数营业收入用回归取残差法计算的增长率" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            res = daily_result[['OPER_REV_ttm_log_pctchg_resid']]
            res = res.rename(columns={'OPER_REV_ttm_log_pctchg_resid':'oper_rev_ttm_log_pctchg_resid'})
            res['dt'] = date
