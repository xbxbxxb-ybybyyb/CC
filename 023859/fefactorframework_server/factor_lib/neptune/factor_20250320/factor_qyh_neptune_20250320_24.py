...
class factor_qyh_neptune_20250320_24(BaseFactor):
    strategy_name = "neptune"
    factor_name = "qyh_neptune_20250320_24"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "qyh"  # 开发人员姓名
    factor_explain = "close除以20日变异系数" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['factor'] = df_ori['factor'] / df_ori['factor'].unstack().rolling(20,1).apply(f_calc_cv).stack().apply(lambda x : round_(x,6)).replace(0,np.nan)
