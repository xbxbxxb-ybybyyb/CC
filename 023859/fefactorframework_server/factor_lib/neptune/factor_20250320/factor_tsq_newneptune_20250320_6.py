...
class factor_tsq_newneptune_20250320_6(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250320_6"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "流动性" # 因子逻辑解释
    ...
    t_1_factor_data = [
        ...
        {'name': 'EODDerivativeIndicator',  # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         'path': '/data/group/800080/warehouse/prod/DATABASE/WIND/AShareEODDerivativeIndicator/AShareEODDerivativeIndicator.h5',
         'lag': 80,  # 注意为正数
         'column': ['FLOAT_A_SHR_TODAY']
         }
    ]  # T-N factor数据，格式如上
    t_1_factor_data_types = ['MD','AShareEODDerivativeIndicator'] # T-1的h5文件类型列表
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            return database
        else:
            df_ori = database['MD_CHINA_STOCK_DAILY_WIND'] # 和上面t-1_factor_data的name一致
            df_eod = database['EODDerivativeIndicator']
            df_ori['float_shares'] = df_eod['FLOAT_A_SHR_TODAY']
            # -------------------------------------------------------------------------------------------------------------------
            df_ori['factor'] = df_ori['volume'] / df_ori['float_shares'].replace(0,np.nan)
