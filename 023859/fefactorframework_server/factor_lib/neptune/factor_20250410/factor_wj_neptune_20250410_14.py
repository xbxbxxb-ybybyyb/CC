...
class factor_wj_neptune_20250410_14(BaseFactor):
    strategy_name = "neptune"
    factor_name = "wj_neptune_20250410_14"
    fill_na_value = 0
    need_pre_calculate_T_N = False
    owner = "wj"  # 开发人员姓名
    factor_explain = "委买委卖差值之差最大值/成交量差值最大值" # 因子逻辑解释
    ...
    def calculate(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            if len(df) > 1:
                ...
                res3 = ((df['BidQty'].diff()) - (df['OfferQty'].diff())).max()/(1e-3+((df['VolumeTrade'].diff() + 1e-3)).max())
            else:
                res3 = 0
