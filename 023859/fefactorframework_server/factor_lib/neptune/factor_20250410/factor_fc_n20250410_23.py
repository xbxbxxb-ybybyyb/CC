...
class factor_fc_n20250410_23(BaseFactor):
    owner = 'fc'
    strategy_name = "neptune"
    factor_name = sys._getframe().f_code.co_name[7:]
    fill_na_value = 1e8
    need_pre_calculate_T_N = True
    factor_explain = "最近3个月超大单净主动买入量，当日超大单主动买量-超大单主动卖量(手)均值" # 因子逻辑解释
    ...
    t_1_factor_data = [
        {'name': 'AShareMoneyFlow', # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 360,
         'column': ["VOLUME_DIFF_INSTITUTE_ACT"]
    }]
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True:
            ...
            res = df_ori["VOLUME_DIFF_INSTITUTE_ACT"].unstack().rolling(90, 1).apply(mean_).stack()
