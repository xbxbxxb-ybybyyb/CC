...
class factor_xbc_md_20250407_19(BaseFactor):
    strategy_name = "neptune"
    factor_name = "xbc_md_20250407_19"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "xbc"  # 开发人员姓名
    factor_explain = "high与max的关系" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            md_data['max'] = md_data[['open', 'close']].max(axis=1)
            md_data['min'] = md_data[['open', 'close']].min(axis=1)
            md_data['stat1'] = (md_data['high'] - md_data['max']) / md_data['pre_close']
            md_data['stat2'] = (md_data['min'] - md_data['low']) / md_data['pre_close']
            md_data['stat'] = (md_data['stat1'] - md_data['stat2']) * md_data['amt']
            md_data['stat'] = md_data['stat'].unstack().rolling(5, 1).max().stack()
            md_data['stat3'] = md_data['amt'].unstack().rolling(15, 1).max().stack()
            md_data[factor_name] = md_data['stat'] / (md_data['stat3'] + 1)
            # -------------------------------------------------------------------------------------------------------------------
