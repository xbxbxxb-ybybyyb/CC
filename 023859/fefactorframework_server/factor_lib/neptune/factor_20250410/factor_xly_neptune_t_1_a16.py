...
class factor_xly_neptune_t_1_a16(BaseFactor):
    strategy_name = "neptune"
    factor_name = "xly_neptune_t_1_a16"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "xly"  # 开发人员姓名
    factor_explain = "行业取中位值" # 因子逻辑解释
    ...
    t_1_factor_data = [
        {'name': 'MD_CHINA_STOCK_DAILY_WIND',
         ...
         'lag': 30,
         'column': ['high','open','close','pre_close']},
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True:
            ...
            df_ori['factor'] = (df_ori['high'] - df_ori[['open','close']].max(axis=1))/df_ori['pre_close']
            df_ori.loc[df_ori['factor'] > 0.2, 'factor'] = 0.2
            df_ori.loc[df_ori['factor'] < -0.2, 'factor'] = -0.2
            ...
            ind_df = df_ori.groupby(['date', 'Ind'])[['factor']].median()
            ind_df.columns = ['factor_ind']
            df_ori2 = pd.merge(df_ori[['date', 'Ind', 'factor']], ind_df, left_on=['date', 'Ind'], right_index=True, how='left')
            df_ori2[self.factor_name] = df_ori2['factor_ind'].unstack().rolling(5, min_periods=1).median().stack()
