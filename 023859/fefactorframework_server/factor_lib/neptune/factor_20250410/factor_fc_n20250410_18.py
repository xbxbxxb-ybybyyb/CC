...
class factor_fc_n20250410_18(BaseFactor):
    owner = 'fc'
    strategy_name = "neptune"
    factor_name = sys._getframe().f_code.co_name[7:]
    fill_na_value = 1e8
    need_pre_calculate_T_N = True
    factor_explain = "近3日大单净流入量，当日大单买量-大单卖量(手)" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True:
            ...
            res = df_ori["VOLUME_DIFF_LARGE_TRADER"].unstack().rolling(3, 1).apply(mean_).stack()
