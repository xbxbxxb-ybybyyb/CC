...
class factor_tsq_newneptune_20250410_26(BaseFactor):
    strategy_name = "neptune"
    factor_name = "tsq_newneptune_20250410_26"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "tsq"  # 开发人员姓名
    factor_explain = "beta60日均值排序" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            # -------------------------------------------------------------------------------------------------------------------
            df_ori['factor'] = df_ori['Beta'].unstack().rolling(60,1).mean().stack().apply(lambda x : round_(x,5))
