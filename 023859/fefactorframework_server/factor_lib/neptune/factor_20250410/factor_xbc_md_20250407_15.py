...
class factor_xbc_md_20250407_15(BaseFactor):
    strategy_name = "neptune"
    factor_name = "xbc_md_20250407_15"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "xbc"  # 开发人员姓名
    factor_explain = "close的中位数和最小值" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            md_data['stat3'] = md_data['stat'].unstack().rolling(2, 1).min().stack()
            md_data['stat3'] = md_data['stat3'].apply(lambda x: np.nan if abs(x) <= 0.0001 else x)
