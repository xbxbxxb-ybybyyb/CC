...
class factor_wj_neptune_20250410_3(BaseFactor):
    strategy_name = "neptune"
    factor_name = "wj_neptune_20250410_3"
    fill_na_value = 0
    need_pre_calculate_T_N = False
    owner = "wj"  # 开发人员姓名
    factor_explain = "委卖金额差值振幅" # 因子逻辑解释
    zcz_adjusted = "否" # 是否针对注册制调整：是/否
    logic_type = "卖单强度" # 逻辑类别
    low_cost = "是" # 是否低耗时
    ...
    def calculate(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            if len(df) > 1:
                ...
                res3 = ((df['OfferQty']*df['WeightedAvgOfferPx']).diff()).max()-((df['OfferQty']*df['WeightedAvgOfferPx']).diff()).mean()
            else:
                res3 = 0
