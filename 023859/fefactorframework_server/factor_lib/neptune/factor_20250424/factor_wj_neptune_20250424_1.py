...
class factor_wj_neptune_20250424_1(BaseFactor):
    strategy_name = "neptune"
    factor_name = "wj_neptune_20250424_1"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "wj"  # 开发人员姓名
    factor_explain = "（high-close）/amt，求mean/std" # 因子逻辑解释
    ...
    t_1_factor_data = [
        {'name': 'MD_CHINA_STOCK_DAILY_WIND',  # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 80,  # 注意为正数
         'column': ['pct_chg', 'turn', 'close', 'high','low','open','amt', 'vwap', 'volume','adjfactor']
         }]  # T-N factor数据，格式如上
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True:  # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['price2_adj'] = df_ori['close'] * df_ori['adjfactor']
            df_ori['factor'] = (df_ori['price_adj'] - df_ori['price2_adj'])/(df_ori['amt'])
            df_ori['long'] = (df_ori['factor']).unstack().rolling(60, 3).apply(lambda x: np.mean(x)/(1e-3+np.std(x))).stack()
