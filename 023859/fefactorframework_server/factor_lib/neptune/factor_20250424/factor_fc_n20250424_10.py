...
class factor_fc_n20250424_10(BaseFactor):
    owner = 'fc'
    strategy_name = "neptune"
    factor_name = sys._getframe().f_code.co_name[7:]
    fill_na_value = 1e8
    need_pre_calculate_T_N = True
    factor_explain = "近三个月开盘资金流入率的标准差" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True:
            ...
            res = df_ori["OPEN_NET_INFLOW_RATE_VALUE"].unstack().rolling(60, 1).apply(std_).stack()
