...
class factor_qyh_neptunelong_longterm_20250918_mc_2(BaseFactor):
    strategy_name = "neptunelong"
    factor_name = "qyh_neptunelong_longterm_20250918_mc_2"
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            res = (df_balancesheet['MONETARY_CAP']).groupby(['dt','Ticker'])\
                .apply(lambda x : x.tail(4).sum()).to_frame(name='货币资金统计量')
