...
class factor_uncover_np_pctchg(BaseFactor):
    strategy_name = "neptunelong"
    factor_name = "uncover_np_pctchg"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "wenj"  # 开发人员姓名
    factor_explain = "找到未被过度炒作的净利润增长率高的股票" # 因子逻辑解释
    ...
    def quarter_stat_free(self,data,labels):
        ...
        for f in data.drop(columns=['MDDate']).columns:
            data[f'{f}_pctchg'] = data[f].groupby('Ticker').diff() / abs(data[f].groupby('Ticker').shift(1))
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        res_income = daily_result[['NET_PROFIT_EXCL_MIN_INT_INC_ttm_log_pctchg']]
        res_income['dt'] = date
        res_income = res_income.reset_index().set_index(['dt','Ticker']) #DataFrame

        res_md = md_data['pct_chg'].groupby('Ticker').transform(lambda x: x.rolling(20).sum()).unstack().iloc[[-1]].stack().reset_index()
        res_md['dt'] = date
        res_md = res_md.set_index(['dt', 'Ticker'])  #DataFrame

        res_income['mmt_20'] = -res_md
        factor = self.rho_theta_factor(res_income['mmt_20'],res_income['NET_PROFIT_EXCL_MIN_INT_INC_ttm_log_pctchg'])
