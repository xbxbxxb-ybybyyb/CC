...
class factor_oper_rev_ttm_log_pctchg_min2(BaseFactor):
    strategy_name = "neptunelong"
    factor_name = "oper_rev_ttm_log_pctchg_min2"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "wenj"  # 开发人员姓名
    factor_explain = "使用TTM对数营业收入计算的增长率,对负数的情况进行了稳健化处理" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            res = daily_result[['OPER_REV_ttm_log_pctchg_min2']]
            res = res.rename(columns={'OPER_REV_ttm_log_pctchg_min2':'oper_rev_ttm_log_pctchg_min2'})
            res['dt'] = date
