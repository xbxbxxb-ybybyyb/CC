...
#初始化参数
parallel_num=10
path='/data/user/023859/Data_Week/'
...
#读取指数成分股数据
member0=s.get_factor_value('WIND_AIndexMembersWIND', S_CON_INDATE = '<20160101', F_INFO_WINDCODE="like'884%'")#进出记录需要取全部时间区间，数量超过上限分为两部分。
member1=s.get_factor_value('WIND_AIndexMembersWIND', S_CON_INDATE=['>=20160101','<20200101'], F_INFO_WINDCODE="like'884%'")
member2=s.get_factor_value('WIND_AIndexMembersWIND', S_CON_INDATE=['>=20200101', '<20220101'], F_INFO_WINDCODE="like'884%'")
member3=s.get_factor_value('WIND_AIndexMembersWIND', S_CON_INDATE=['>=20220101', '<20230101'], F_INFO_WINDCODE="like'884%'")
member4=s.get_factor_value('WIND_AIndexMembersWIND', S_CON_INDATE='>=20230101', F_INFO_WINDCODE="like'884%'")
wind_member=pd.concat([member0,member1,member2,member3,member4],ignore_index=True)
...
ccpt_daily_data_1 = s.get_factor_value('WIND_AIndexWindIndustriesEOD',TRADE_DT=['>='+begin_date,'<'+'20230101'],S_INFO_WINDCODE="like'884%'")
ccpt_daily_data_2 = s.get_factor_value('WIND_AIndexWindIndustriesEOD',TRADE_DT=['>='+'20230101','<='+now_date],S_INFO_WINDCODE="like'884%'")
ccpt_daily_data = pd.concat([ccpt_daily_data_1,ccpt_daily_data_2],ignore_index=True)
...
pool.map_async(run_block_data_parallel, wind_member['block'].unique())
# factor_parallel = pool.map_async(run_block_data_parallel, wind_member['block'].unique())
pool.close()