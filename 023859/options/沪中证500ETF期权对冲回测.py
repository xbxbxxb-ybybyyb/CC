    ...
    for option_type in ['平值', '虚值一档', '虚值二档', '实值一档', '实值二档']:
        ...
        for dt in data.index:
            ...
            if option_type_dt not in tol_range: # 超出容忍度范围，进行合约替换
                ...
                cost = 0 * (hold_vol+buy_vol) / 10000 + (hold_amt+buy_amt)*fee# 万元
            ...
            # 在容忍度内，不进行调仓，但考虑加减仓
                ...
                mkt_amt = df.loc[(dt, contract), 'amt']
                trade_price = df.loc[(dt, contract), 'twap']
                target_hold_vol = min(np.floor(target_amt * 1e4 / data.loc[dt, 'index_pre_close'] / 10000), 5000)
                if target_hold_vol > hold_vol: # 继续买
                    target_buy_vol = target_hold_vol - hold_vol
                    trade_amt = min(target_buy_vol * 10000 * trade_price / 10000, mkt_ratio * mkt_amt)
                    trade_vol = np.floor(trade_amt * 1e4 / trade_price / 10000)
                ...
                hold_amt = hold_vol * 10000 * trade_price / 10000
                pct = df.loc[(dt,contract),'pct']
                cost = 0*trade_vol/10000 + trade_amt*fee
                data.at[dt, f'{contract_month}{option_type}持有合约手数'] = {**data.at[dt, f'{contract_month}{option_type}持有合约手数'], contract: data.at[dt, f'{contract_month}{option_type}持有合约手数'].get(contract, 0) + hold_vol}
                # data.loc[dt, f'{contract_month}{option_type}买入合约市场成交额'] = mkt_amt
...
def classify_moneyness_top7(df, index_col='index_pre_close', strike_col='StrikePrice'):
    """
...
data_2022 = pd.DataFrame(index=[pd.Timestamp(date) for date in fd.tradingday(20220919, 20221231)],
                         columns=['策略股票持仓', '策略日度盈利'])
...
fdd = FundData()
data_index = fdd.get_fund_factor_value(['510500.SH'],trading_days,['pre_close','close']).droplevel(1)
...
df = pd.read_pickle('/dfs/user/023859/options/df_510500_20220919_20250630.pkl')
df['pct_settle'] = df['Next_SettlePrice_adj']/df['SettlePrice_adj'] - 1
df = df[df['CallOrPut'] == False]
df = df[df['AdjustSign'] == 'M']

unique_last_trading_dates = set(df['LastTradingDate'])
# # 分五个交易日建仓
# k = 5
# date_to_extend = {date: pd.Timestamp(fd.tradingday(date.strftime('%Y%m%d'), -2 - k)[0]) for date in
#                   unique_last_trading_dates}
...
# data= find_target_contract_delta_tolerance(data, df_filtered_next_month, ratio, mkt_ratio, '次月', tol_range=['虚值二档', '虚值一档', '平值', '实值一档'])
...
with pd.ExcelWriter(f'/dfs/user/023859/options/期权低比例delta对冲回测结果_0.1_510500_twap_20230101_20250630.xlsx', engine='xlsxwriter') as writer:
...
def merge_vol_pct_amt_delta(row, df):
    dt = row.name
    vol_dict = row['当月虚值二档持有合约手数']
    index_close = row['index_close']
    merged = {}
    for contract, vol in vol_dict.items():
        pct = df.loc[(dt, contract), 'pct_chg']
        amt = df.loc[(dt, contract), 'SettlePrice'] * vol * 10000 / 10000
        delta_exposure = df.loc[(dt, contract), 'delta'] * index_close * 10000 / 10000
        exposure = index_close * 10000 / 10000
        merged[contract] = {'vol': vol, 'pct': pct, 'amt': amt, 'delta_exposure': delta_exposure, 'exposure': exposure}
    return merged

data['当月虚值二档持有合约信息'] = data[['当月虚值二档持有合约手数', 'index_close']].apply(lambda row: merge_vol_pct_amt_delta(row, df), axis=1)
data['当月虚值二档持有合约总规模'] = data['当月虚值二档持有合约信息'].apply(lambda dct: sum(sub['amt'] for sub in dct.values()))
data['当月虚值二档delta敞口'] = data['当月虚值二档持有合约信息'].apply(lambda dct: sum(sub['vol'] * sub['delta_exposure'] for sub in dct.values()))
data['当月虚值二档敞口'] = data['当月虚值二档持有合约信息'].apply(lambda dct: sum(sub['vol'] * sub['exposure'] for sub in dct.values()))

data['事件型策略日度最大回撤'] = (data['策略日度盈利'].cumsum().cummax() - data['策略日度盈利'].cumsum())
data['期权+事件型策略日度最大回撤'] = ((data['策略日度盈利'] + data['profit_当月虚值二档']).cumsum().cummax() - (data['策略日度盈利'] + data['profit_当月虚值二档']).cumsum())
...
for period in half_year_dict:
    ...
    res_half_year[(period, '期权+事件型策略')] = sta_profit(data.loc[start_date:end_date, '策略日度盈利'] + data.loc[start_date:end_date, 'profit_当月虚值二档'])

res_df_half_year = pd.DataFrame(res_half_year)
res_df_half_year = res_df_half_year.sort_index(axis=1, level=0)

data_best = data[
    ['策略股票持仓', '策略日度盈利', '事件型策略日度最大回撤', 'index_close', 'index_pre_close', '当月虚值二档持有合约信息',
     '当月虚值二档持有合约总规模', '当月虚值二档敞口', '当月虚值二档delta敞口', \
     'profit_当月虚值二档', '期权+事件型策略日度最大回撤', 'buy_amt_当月虚值二档',
     'buy_vol_当月虚值二档', 'sell_vol_当月虚值二档', 'sell_amt_当月虚值二档', \
     '策略累计盈利', '当月虚值二档累计盈利', '当月虚值二档期权端累计盈利']].rename(
    columns={'当月虚值二档持有合约信息': '持有合约信息', '当月虚值二档持有合约总规模': '持有合约总规模', \
             '当月虚值二档敞口': '持有合约名义市值', '当月虚值二档delta敞口': 'delta敞口', \
             'profit_当月虚值二档': 'profit', \
             'buy_amt_当月虚值二档': 'buy_amt', 'buy_vol_当月虚值二档': 'buy_vol', 'sell_vol_当月虚值二档': 'sell_vol', 'sell_amt_当月虚值二档': 'sell_amt', \
             '当月虚值二档累计盈利': '对冲后策略累计盈利', '当月虚值二档期权端累计盈利': '策略期权端累计盈利'})

with pd.ExcelWriter(f'/dfs/user/023859/options/期权低比例delta对冲回测结果_0.1_510500_twap_当月虚值二档_20230101_20250630.xlsx',
                    engine='xlsxwriter') as writer:
    data_best.to_excel(writer, sheet_name='20230101~20250630当月虚值二档数据')
