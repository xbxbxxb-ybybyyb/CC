...
def find_target_contract_delta(data, df, delta_ratio, mkt_ratio, contract_month):
    ...
    for option_type in ['平值','虚值一档','虚值二档','实值一档','实值二档']:
        ...
        buy_price = df_filtered['SettlePrice'].droplevel('Ticker')
...
def find_target_contract_fix_tolerance(data, df, contract_month, k=5, amt_tot=20000, mkt_ratio=0.1, spread_cost=0.002, tol_range=['虚值二档','虚值一档','平值','实值一档','实值二档']):
    df = df.copy()
