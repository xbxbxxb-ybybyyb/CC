    ...
    if task["factor_type"] == FactorType.T_1_FACTOR:
        ...
        if task["task_data_info"]["xdb_data_info"]:
            for data_dict in task["task_data_info"]["xdb_data_info"].values():
                if (data_dict["name"] == "xdb_tickfull" or data_dict["name"] == "xdb_tick1s") and (calc_date < "20170110"):
    ...
    for data_dict in factor.xdb_data:
        ...
        if symbol_data_df.empty:
            symbol_data_df = groupby.apply(lambda x: get_symbol_xdb_datadict(
                factor.factor_name, x.name[1], cur_df, valid_df, start_date, task["strategy"], data_dict, {}))
        else:
            symbol_data_df = groupby.apply(lambda x: get_symbol_xdb_datadict(
                factor.factor_name, x.name[1], cur_df, valid_df, start_date, task["strategy"], data_dict,
...
def get_symbol_xdb_datadict_test(factor_name, symbol, cur_df, calc_date, strategy, data_dict, result):
    if not "skip" in result:
        result["skip"] = False
    if not "Ticker" in result:
        result["Ticker"] = symbol
    ...
    if cur_df.empty:
        ...
        days = sorted(cur_df["MDDate"].unique())
        df = cur_df[(cur_df["MDDate"] >= days[-min(int(data_dict["lag"]), len(days))])
                                      & (cur_df["MDDate"] <= days[-1])].copy()
        if df.empty:
            cur_dict["skip"] = True
            logger.warning(
                "因子数据准备异常：存在empty dataframe！factor={}, date={}, strategy={}, symbol={}, empty_data_name={}".format(
