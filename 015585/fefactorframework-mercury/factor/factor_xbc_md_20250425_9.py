...
class factor_xbc_md_20250425_9(BaseFactor):
    strategy_name = "europa"
    factor_name = "xbc_md_20250425_9"
    fill_na_value = 0.915
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            md['last_high_hot_day'] = 101
            for i in range(20,0,-1):
                md['tmp_ratio'] = 0
                md['tmp_ratio'] = md.groupby('Ticker')['high_ratio'].shift(i)
                md.loc[md['tmp_ratio']>=1.04,'last_high_hot_day'] = i

            factor_df = pd.DataFrame()
            factor_df[factor_name] = md['last_high_hot_day']
            # -------------------------------------------------------------------------------------------------------------------
