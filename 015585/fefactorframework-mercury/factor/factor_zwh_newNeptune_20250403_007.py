...
class factor_zwh_newNeptune_20250403_007(BaseFactor):
    owner = 'zwh'
    strategy_name = "europa"
    factor_name = "zwh_newNeptune_20250403_007"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    factor_explain = "价格波动变化" # 因子逻辑解释
    ...
    t_1_factor_data = [
        {'name': 'MD_CHINA_STOCK_DAILY_WIND',  # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 100,  # 注意为正数
         'column': ['turn', 'high', 'close', 'amt', 'pre_close', 'adjfactor']
         }]
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        pctv = (df['high'] - df['pre_close']) / np.log(1.1 + df['amt'])
        vd5 = pctv.unstack().rolling(60, min_periods=60).max().stack()
        vd20 = pctv.unstack().rolling(60, min_periods=60).min().stack()

        res =  pctv / (EPS + vd5) - pctv / (EPS + vd20)
