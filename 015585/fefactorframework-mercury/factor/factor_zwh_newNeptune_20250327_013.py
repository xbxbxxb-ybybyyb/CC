...
class factor_zwh_newNeptune_20250327_013(BaseFactor):
    owner = 'zwh'
    strategy_name = "europa"
    factor_name = "zwh_newNeptune_20250327_013"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    factor_explain = "openclose波动比" # 因子逻辑解释
    ...
    t_1_factor_data = [
        {'name': 'MD_CHINA_STOCK_DAILY_WIND',  # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 100,  # 注意为正数
         'column': ['close', 'high', 'low', 'vwap', 'turn','open', 'pct_chg', 'pre_close', 'adjfactor']
         }]
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        open_pct = (df['open'] - df['pre_close']) / df['pre_close'] - 1
        close_pct = (df['close'] - df['pre_close']) / df['pre_close'] - 1
        pct = df['pct_chg']
        N = 14

        high = close_pct.unstack().rolling(N, min_periods=N).sum().stack() - close_pct.unstack().rolling(60,
                                                                                                         min_periods=60).sum().stack()
        wait = pct.unstack().rolling(N, min_periods=N).min().stack()
        wait0 = open_pct.unstack().rolling(N, min_periods=N).min().stack()
        res = high*(wait+wait0)
