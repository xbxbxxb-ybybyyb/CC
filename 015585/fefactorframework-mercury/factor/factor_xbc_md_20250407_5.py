...
class factor_xbc_md_20250407_5(BaseFactor):
    strategy_name = "europa"
    factor_name = "xbc_md_20250407_5"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "xbc"  # 开发人员姓名
    factor_explain = "换手率与金额的关系" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            md_data['stat'] =  (md_data['close'] - md_data['vwap'])/(md_data['pre_close']+0.01)
            md_data['amt'] = md_data['amt']/10000

            factor_df = pd.DataFrame()
            factor_df[factor_name] =  (md_data['stat']*md_data['turn']*md_data['amt'] ).unstack().rolling(3, min_periods=1).mean().stack().round(4)+(md_data['stat']*md_data['turn']*md_data['amt'] ).unstack().rolling(4, min_periods=1).mean().stack().round(4)
