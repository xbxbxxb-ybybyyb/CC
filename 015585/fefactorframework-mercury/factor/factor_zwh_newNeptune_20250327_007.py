...
class factor_zwh_newNeptune_20250327_007(BaseFactor):
    owner = 'zwh'
    strategy_name = "europa"
    factor_name = "zwh_newNeptune_20250327_007"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    factor_explain = "close波动排名" # 因子逻辑解释
    ...
    t_1_factor_data = [
        {'name': 'MD_CHINA_STOCK_DAILY_WIND',  # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 100,  # 注意为正数
         'column': ['volume', 'close', 'vwap', 'open', 'pre_close', 'adjfactor']
         }]
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        xx = close / (1e-9 + pre_close) - 1
        yy = pre_close / (1e-9 + volume) - 1
