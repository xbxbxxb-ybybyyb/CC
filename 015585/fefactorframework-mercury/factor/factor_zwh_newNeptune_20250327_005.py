...
class factor_zwh_newNeptune_20250327_005(BaseFactor):
    owner = 'zwh'
    strategy_name = "europa"
    factor_name = "zwh_newNeptune_20250327_005"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    factor_explain = "closevwap振幅排名" # 因子逻辑解释
    ...
    t_1_factor_data = [
        {'name': 'MD_CHINA_STOCK_DAILY_WIND',  # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 100,  # 注意为正数
         'column': ['vwap', 'high', 'close', 'open', 'pre_close', 'adjfactor']
         }]
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        close_v = df['close'] / df['pre_close'] - 1
        vv = vwap / df['pre_close'] - 1
        amt_rank = (close_v.unstack().rank(axis=axis_) / (~ close_v.unstack().isnull()).values.sum(axis=axis_).reshape( -1, 1)).stack()
        pct_rank = (vv.unstack().rank(axis=axis_) / (~ vv.unstack().isnull()).values.sum(axis=axis_).reshape(-1,  1)).stack()
        result = amt_rank / (0.5 + pct_rank)
        N = 20
        res = result.unstack().rolling(N, min_periods=N).mean().stack()  # /(1e-9+result)
