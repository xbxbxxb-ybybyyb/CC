...
class factor_emo_20250909_6(BaseFactor):
    strategy_name = "europa"
    factor_name = "emo_20250909_6"
    fill_na_value = 20
    need_pre_calculate_T_N = True
    owner = "qyh"  # 开发人员姓名
    factor_explain = "近20交易日内涨停板数量，计算当日最高板标的，剔除次新（类似连板高度）" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['is_zt_20'] = df_ori['is_zt'].unstack().rolling(20,1).sum().stack()
            zt_20_max = df_ori.groupby('dt')['is_zt_20'].max()
            df_ori['zt_20_max'] = df_ori.index.get_level_values('dt').map(zt_20_max)
            df_ori[self.factor_name] = df_ori['zt_20_max']
