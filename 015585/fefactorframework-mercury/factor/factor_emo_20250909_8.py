...
class factor_emo_20250909_8(BaseFactor):
    strategy_name = "europa"
    factor_name = "emo_20250909_8"
    fill_na_value = 20
    need_pre_calculate_T_N = True
    owner = "qyh"  # 开发人员姓名
    factor_explain = "全市场涨幅>0标的占比,近5日" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['is_up'] = (df_ori['pct_chg'] > 0).apply(int)
            up_num = df_ori.groupby('dt')['is_up'].sum()
            df_ori['up_num'] = df_ori.index.get_level_values('dt').map(up_num)
            all_num = df_ori.groupby('dt')['is_up'].count()
            df_ori['all_num'] = df_ori.index.get_level_values('dt').map(all_num)
            df_ori[self.factor_name] = (df_ori['up_num'] / df_ori['all_num']).unstack().rolling(5,1).mean().stack()
