...
class factor_xbc_md_20250425_10(BaseFactor):
    strategy_name = "europa"
    factor_name = "xbc_md_20250425_10"
    fill_na_value = 0.
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            high_days = 60
            md['before_n_close_high_adj'] = 1e10
            for i in range(1,1+high_days):
                md['before_high_adj_tmp'] = 0
                md['before_high_adj_tmp'] = md.groupby('Ticker')['close_adj'].shift(i)
                md['before_n_close_high_adj'] = md[['before_high_adj_tmp','before_n_close_high_adj']].min(axis=1)
