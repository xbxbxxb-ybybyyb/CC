...
class factor_xbc_md_20250407_9(BaseFactor):
    strategy_name = "europa"
    factor_name = "xbc_md_20250407_9"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "xbc"  # 开发人员姓名
    factor_explain = "pct_chg的均值与方差" # 因子逻辑解释
        ...
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            def max_drawdown(capital_line):
                ...
                for i in range(len(capital_line)):
                    temp_profit = capital_line[i]
                    if len(profit_list) == 0:
                        profit_list.append(temp_profit)
                    else:
                        profit_list.append(profit_list[-1] + temp_profit)
                profit_list = np.array(profit_list)
                drawdown = np.max(np.maximum.accumulate(profit_list) - profit_list)
                return profit_list[-1], drawdown
            def calculate_stat(x):
                profit, drawdown = max_drawdown(np.array(x))
            ...
            md_data['stat'] = md_data['pct_chg'].unstack().rolling(5, min_periods=3).apply(lambda x: calculate_stat(x)).stack()

            market_mean = md_data['stat'].unstack().mean(axis=1)
            factor_df[factor_name] = (md_data['stat']-market_mean).unstack().rolling(5, min_periods=1).min().stack()
            # -------------------------------------------------------------------------------------------------------------------
