...
class factor_zwh_newNeptune_20250327_006(BaseFactor):
    owner = 'zwh'
    strategy_name = "europa"
    factor_name = "zwh_newNeptune_20250327_006"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    factor_explain = "量价对比排名" # 因子逻辑解释
    ...
    t_1_factor_data = [
        {'name': 'MD_CHINA_STOCK_DAILY_WIND',  # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 100,  # 注意为正数
         'column': ['volume', 'high', 'vwap', 'open', 'pre_close', 'adjfactor']
         }]
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        xx = pre_close / (1e-9 + volume) - 1
        # yy =  df['pre_close']/(1e-9+volume) -1  21.9 0.8
        yy = high / (1e-9 + vwap) - 1

        def rank_(data_):
            data_r = (data_.unstack().rank(axis=1) / (~data_.unstack().isnull()).values.sum(axis=1).reshape(
                -1, 1)).stack()
            return data_r
        ...
        N = 20
        nn = 3
        res = result.unstack().rolling(N, min_periods=N).max().stack() * result.unstack().rolling(nn,min_periods=nn).max().stack()
