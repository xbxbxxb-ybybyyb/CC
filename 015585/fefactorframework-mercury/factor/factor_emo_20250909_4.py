...
class factor_emo_20250909_4(BaseFactor):
    strategy_name = "europa"
    factor_name = "emo_20250909_4"
    fill_na_value = 20
    need_pre_calculate_T_N = True
    owner = "qyh"  # 开发人员姓名
    factor_explain = "前5日破板数量" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori[self.factor_name] = df_ori['destroy_num'].unstack().rolling(5,1).mean().stack()
            # -------------------------------------------------------------------------------------------------------------------
