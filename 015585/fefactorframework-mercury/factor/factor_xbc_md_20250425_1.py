...
class factor_xbc_md_20250425_1(BaseFactor):
    strategy_name = "europa"
    factor_name = "xbc_md_20250425_1"
    fill_na_value = 2.3
            ...
            def count_reverse_number(X):
                ...
                for i in range(len(X) - 1):
                    ...
                    for xx in y:
                        if x > xx:
                            reverse_number += 1
                return reverse_number/len(X)

            md_data.loc[md_data['pct_chg'] > 10, 'pct_chg'] = 10  # 截断
            md_data.loc[md_data['pct_chg'] < -10, 'pct_chg'] = -10  # 截断
            md_data['pct_chg'] = md_data['pct_chg']*md_data['turn']
            factor_df = pd.DataFrame()
            factor_df[factor_name] = md_data['pct_chg'].unstack().rolling(10, min_periods=1).apply(lambda x: count_reverse_number(x)).stack()
            # -------------------------------------------------------------------------------------------------------------------
