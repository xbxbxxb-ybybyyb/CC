...
class StrongFactorTest:
    ...
    def load_data(self):
        if self.strategy_name == 'hotspot':
            df = pd.read_pickle('/dfs/user/015585/00_hotspot/basic_files/factor_files/sft_init_20160101_20231231.pkl').loc[pd.Timestamp(str(self.start_date)):
                                                                                                                           pd.Timestamp(str(self.end_date))]
        else:
            logger.error('strategy_name error,must in [hotspot]')
            raise TypeError
        # 样本筛选
        ...
        df['class'] = df['value'].apply(lambda x : 1 if x > 0 else 0)
        df['mixed'] = df['class']
