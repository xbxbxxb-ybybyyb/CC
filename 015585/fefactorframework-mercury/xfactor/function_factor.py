...
def round_(x, n=0):
    ...
    return res
# 因子属性函数
def f_pro_b1diff(tick_df):
    tick_df['factor'] = (tick_df['Buy1OrderQty']).diff()
    return tick_df['factor']
def f_pro_b1diff2mv(tick_df):
    tick_df['factor'] = (tick_df['Buy1OrderQty']).diff() / tick_df['ff_shares']
    return tick_df['factor']
def f_pro_b22s2diff(tick_df):
    tick_df['factor'] = (tick_df['Buy2OrderQty'] - tick_df['Sell2OrderQty']).diff()
    return tick_df['factor']
def f_pro_b22s2diff2mv(tick_df):
    tick_df['factor'] = ((tick_df['Buy2OrderQty'] - tick_df['Sell2OrderQty']).diff()) / tick_df['ff_shares']
    return tick_df['factor']
def f_pro_b22s22mv(tick_df): #买2-卖2的量 除以ffshares
    tick_df['factor'] = (tick_df['Buy2OrderQty'] - tick_df['Sell2OrderQty']) / tick_df['ff_shares']
    return tick_df['factor']
def f_pro_b12mv(tick_df):
    tick_df['factor'] = (tick_df['Buy1OrderQty']) / tick_df['ff_shares']
    return tick_df['factor']
...
def generate_factor(tick_df,
    ...
    # 筛选tick价格
    p = dic_tick_kind3[tick_kind3_i](tick_df_t) # 注意是用筛选过时间的tick
    if p > 0:
...
'''
'''
def generate_factor_addcompare(tick_df,property_i,time_kind_i,time_type_i,
                               tick_kind1_i,tick_kind2_i,tick_kind3_i,tick_type3_i,len_type_i,
                               std_i,calc_i,compare_i,
                               dic_time_kind,dic_tick_kind1,dic_tick_kind2,dic_tick_kind3,dic_len_type,dic_property,
                               dic_std,dic_calc):
    if compare_i == 'nocompare':
        value = generate_factor(tick_df,property_i,time_kind_i,time_type_i,
                                tick_kind1_i,tick_kind2_i,tick_kind3_i,tick_type3_i,len_type_i,std_i,calc_i,
                                dic_time_kind,dic_tick_kind1,dic_tick_kind2,dic_tick_kind3,
                                dic_len_type,dic_property,dic_std,dic_calc)
    elif (compare_i == 'compare_t') & (time_type_i == 'before'):
        value_1 = generate_factor(tick_df,property_i,time_kind_i,'before',
                                  tick_kind1_i,tick_kind2_i,tick_kind3_i,tick_type3_i,
                                  len_type_i,std_i,calc_i,
                                  dic_time_kind,dic_tick_kind1,dic_tick_kind2,dic_tick_kind3,dic_len_type,dic_property,
                                  dic_std,dic_calc)
