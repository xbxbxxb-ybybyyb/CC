...
def value_same_check(kls, result_path, filter_df):
    logger.info('因子一致检查: factor_name={}, strategy={}'.format(kls.factor_name, "hotspot"))
    if not os.path.exists(result_path):
        os.system("mkdir -p " + result_path)

    factor_name = kls.factor_name
    long_interval = settings.precheck_path_dict["hotspot"]["long_interval"]
    short_interval_list = settings.precheck_path_dict["hotspot"]["short_interval_list"]
    ...
    try:
        if os.path.exists('{}{}_{}_{}.pkl'.format(result_path, factor_name, long_interval[0], long_interval[1])):
            ...
            if not db:
                db = precheck_utils.prepare_database_for_value_same_check(kls, 'hotspot', long_interval[0],
                                                                          long_interval[1])
            db_cpy = copy.copy(db)
        ...
        logger.error('函数测试出错: factor={}, strategy={}, 测试区间:{}-{}-{}'.format(factor_name, "hotspot", long_interval[0],
                                                                            long_interval[1], e))
        return '函数测试出错: factor={}, strategy={}, 测试区间:{}-{}-{}'.format(factor_name, "hotspot", long_interval[0],
                                                                      long_interval[1], e)
    ...
    try:
        for short_date in short_interval_list:
            if os.path.exists('{}{}_{}_{}.pkl'.format(result_path, factor_name, short_date, short_date)):
                ...
                if not db:
                    db = precheck_utils.prepare_database_for_value_same_check(kls, 'hotspot', str(long_interval[0]),
                                                                              str(long_interval[1]))
                short_db = {"skip": False}
            ...
            if np.nanmax((short_df - tmp_long_df).abs().values) > 1e-8:
                logger.error(
                    '因子值不一致1: factor={}, strategy={}, 计算区间:{}-{}和{}-{}'.format(factor_name, "hotspot", long_interval[0],
                ...
                return '因子值不一致1: factor={}, strategy={}, 计算区间:{}-{}和{}-{}'.format(
                    factor_name, "hotspot", long_interval[0], long_interval[1], short_date, short_date)
        ...
        logger.error(
            '函数测试出错: factor={}, strategy={}, 测试区间:{}-{}-{}'.format(factor_name, "hotspot", short_date, short_date, e))
        return '函数测试出错: factor={}, strategy={}, 测试区间:{}-{}-{}'.format(factor_name, "hotspot", short_date, short_date, e)
    return 'pass'
...
def factor_value_test(factor_df, factor_name):
    ...
    if int(np.isinf(factor_df).sum()) > 0:
        logger.error('因子值存在inf: factor_name={}, strategy={}'.format(factor_name, "hotspot"))
        res.append('因子值存在inf')
    if factor_df[factor_df.columns[0]].std() == 0:
        logger.error('因子值全部值为常数: factor_name={}, strategy={}'.format(factor_name, "hotspot"))
        res.append('因子值全部值为常数')
    if factor_df.columns[0] != factor_name:
        logger.error('列名与因子名不符: factor_name={}, strategy={}'.format(factor_name, "hotspot"))
        res.append('列名与因子名不符')
...
def score_check(factor_name, factor_value_df, filter_df):
    try:
        ...
        if same_rate > 0.8:
            return '相同值比例较高: factor_name={}, strategy={},'.format(factor_name, "hotspot") + ' -%.2f%%' % (
                    same_rate * 100)
        if ('yzhan' in factor_name) and (same_rate > 0.2):
            return '相同值比例较高: factor_name={}, strategy={},'.format(factor_name, "hotspot") + ' %.2f%%' % (same_rate * 100)
        ...
        if (factor_mean < 0.001) and (factor_std < 0.001):
            return '因子波动小: factor_name={}, strategy={}, std:%.8f,mean:%.8f'.format(
                factor_name, "hotspot", factor_mean, factor_std)

        if factor_lsd < 0.01:
            logger.warning('离散度太低: factor_name={}, strategy={}, 暂未为筛选指标！！！！！！！！！！！'.format(factor_name, "hotspot"))
...
def hotspot_precheck_aftercalc(kls, error_factors, start_date, end_date, factor_value, factor_type, result_path):
    logger.info("Start hotspot factor precheck (after calc):  factor_name={}, factor_owner={}"
                .format(kls.factor_name, kls.owner))
    check_dic = {'代码格式检查': 'pass', '因子值一致性检查': '', '函数运行检查': '', '因子测试': '', '预检测': 'not pass'}
    pkl_path = result_path + '/precheck/hotspot/result/'
    ...
    if kls.factor_name in error_factors:
        ...
        return
    filter_df = pd.read_pickle(settings.precheck_path_dict["hotspot"]["precheck_basic_path"])
    ...
    if factor_type == precheck_utils.FactorUtil.FactorType.T_1_FACTOR:
        if str(start_date) == settings.precheck_path_dict["hotspot"]["long_interval"][0] and str(end_date) == \
                settings.precheck_path_dict["hotspot"]["long_interval"][1]:
            check_dic['因子值一致性检查'] = value_same_check(kls, result_path + '/precheck/hotspot/same_test/', filter_df)
        else:
            logger.warning("起止区间与settings中设置不一致，跳过长短区间检测！ factor={}, strategy=hotspot".format(kls.factor_name))
            check_dic['因子值一致性检查'] = 'pass'
    else:
        check_dic['因子值一致性检查'] = 'pass'
    if check_dic['因子值一致性检查'] != 'pass':
        logger.error("因子值一致性检查未通过！ factor_name={}, strategy={}".format(kls.factor_name, "hotspot"))

    check_dic['函数运行检查'] = factor_value_test(factor_value, kls.factor_name)
    if check_dic['函数运行检查'] != 'pass':
        logger.error("函数运行检查未通过！ factor_name={}, strategy={}".format(kls.factor_name, "hotspot"))

    check_dic['因子测试'] = score_check(kls.factor_name, factor_value, filter_df_cpy)

    if check_dic['因子测试'] != 'pass':
        logger.error("因子测试未通过！ factor_name={}, strategy={}".format(kls.factor_name, "hotspot"))

    if check_dic["函数运行检查"] == 'pass' and check_dic["代码格式检查"] == 'pass' and check_dic["因子值一致性检查"] == "pass" and \
            check_dic["因子测试"] == "pass":
        check_dic["预检测"] = 'pass'
    else:
        logger.error("预检测未通过！ factor_name={}, strategy={}".format(kls.factor_name, "hotspot"))

    pre_check_res = pd.Series(check_dic)
    pre_check_res.to_pickle(pkl_path + '{}.pkl'.format(kls.factor_name))
    logger.info("预检测完成！ factor_name={}, strategy={}".format(kls.factor_name, "hotspot"))
    return
