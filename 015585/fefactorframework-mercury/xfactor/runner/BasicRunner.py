...
def run(start_date, end_date, strategy, output_dir, options=None, data_filter = 'no_filter', class_list_out=[]):
    calc_num_cpus = 1
    ...
    result, check_result = __calc_factor(strategy, start_date, end_date, mode, calc_num_cpus,
                                         precheck, run_factor_test, report, output_dir, local_evaluator, class_list_out, data_filter)

    return result, check_result


def __calc_factor(strategy, start_date, end_date, mode, calc_num_cpus, precheck, run_factor_test,
                  report, output_dir, local_evaluator, class_list_out, data_filter):
    result = {}
    cost_result = {}
    checker_result = {}
    task_manager = TaskManager(start_date, end_date, strategy, class_list_out,data_filter)
...
def __factor_check(strategy, start_date, end_date, saved_factor_dirs, factor_class_map, cost_result, output_dir,
    ...
    if calc_num_cpus == 1:
        logger.info("计算开始：当前任务种类=factor_test, 任务数量={}, 进程数={}".format(len(saved_factor_dirs), 1))
        ...
        logger.info("计算开始：当前任务种类=factor_test, 任务数量={}, 进程数={}".format(len(saved_factor_dirs), min(calc_num_cpus, len(saved_factor_dirs))))
        for path_dict in saved_factor_dirs:
            checker = tester.get_tester(strategy, factor_class_map[path_dict["name"]], start_date, end_date,
                                        local_evaluator_path)
