...
class FactorTest:
    def __init__(self, start_date, end_date, df_sft, segment_number=20,
                 style_list=['Circu_Mkt'],  # 风格因子用来中性化
                 cal_mi=False,
                 buy_type='s1',
        ...
        self.style_list = style_list  # 风格因子用来做市值中性化
        self.cal_mi = cal_mi
        self.buy_type = buy_type
        self.strategy_name = strategy_name
        if self.strategy_name == 'neptune':
            self.return_type = 'label_t2o30d1'
    ...
    def load_data(self):
        ...
        all_factors_available = ['label_t2o30d1', 'label_t4o30d1', 'label_t6o30d1', 'Circu_Mkt']
    ...
    def factor_test(self, factor_df, cost_list, result_path, factor_corr_test=False, generate_pdf=False):
        ...
        if factor_corr_test:
            check_res = pd.read_excel(
                '/data/group/800463/data/projectZZ_public/factor_lib/check_res_tot_neptune.xlsx')
    ...
    def check_score_list(self, double_group_df_dic, month_df_dic, dir, label_test=['value']):
        ...
        for key in keys:
            ...
            score_df = self.check_score(month_df, dir)
            res_df[key] = score_df.loc['score']
        return res_df.T.sort_index()

    def check_score(self, factor_group_df, month_df, dir, label_test=['value', 'class']):
        ...
        for label in label_test:
            diff, reverse = [], []
            for date in factor_group_df.index.get_level_values(0).unique():
                rate_arr = factor_group_df[label].unstack().loc[date].values
            ...
            stability_df.loc[(month_df[label] >= stability_dic[label][0]) & (
                        month_df[label] < stability_dic[label][1]), '%s_standard' % (label)] = 'mid'
            stability_df.loc[(month_df[label] > -stability_dic[label][0]) & (
                        month_df[label] < stability_dic[label][0]), '%s_standard' % (label)] = 'low'
            stability_df.loc[(month_df[label] <= -stability_dic[label][0]) & (
                        month_df[label] > -stability_dic[label][1]), '%s_standard' % (label)] = 'reverse_mid'
    ...
    def get_corr_sta(self, corr_month):
        ...
        sta_df['corr_tot'] = [
            self.basic_df.groupby('dt').apply(lambda x: x['factor'].corr(x[label], method='spearman')).mean() for label
            in self.label_list]  # TODO 截面IC
    ...
    def get_corr_month(self):
        ...
        groupby_month_dt_data = self.basic_df.groupby(['month', 'dt'])
        groupby_month_data = self.basic_df.groupby('month')

        corr_month_df['count'] = groupby_month_data.count()['factor']
        for label in self.label_list:
            corr_month_df[label] = groupby_month_dt_data.apply(
                lambda x: x['factor'].corr(x[label], method='spearman')).groupby('month').mean()
...
sft_basic_path = '/data/group/800463/data/projectZZ_public/factor_lib/sft_basic_formal_931_20160101_20191231.h5'  # 这个文件里有label和所有因子
df_sft =  df = IO.read_data([20160101, 20191231], alt=sft_basic_path)
def generate_factor_test(h5_name, save_path):
    if not os.path.exists(f'{save_path}{h5_name}.pkl'):
        df = pd.read_hdf(f'/dfs/user/015585/01_factor_develop_store/fast_factor_newframe/neptune/20250516_xdb_balancesheet_cs/factor_value/neptune/{h5_name}.h5')
        factor_test = FactorTest(20160101, 20191231, df_sft, cal_mi=False)
        for col in df.columns:
            print(col)
            # print(time.localtime())
            factor_test.factor_test(df[[col]], [np.nan], save_path,
        ...
        print(f'{h5_name} 已进行factor_test')
    return
...
pool = Pool(10)
task_list = []
save_path = '/dfs/user/015585/01_factor_develop_store/fast_factor_newframe/neptune/20250516_xdb_balancesheet_cs/factor_test/neptune/'
h5_name_list = os.listdir('/dfs/user/015585/01_factor_develop_store/fast_factor_newframe/neptune/20250516_xdb_balancesheet_cs/factor_value/neptune/')
h5_name_list = [x.replace('.h5','') for x in h5_name_list if '.h5' in x]
h5_name_list.sort()
for h5_name in h5_name_list:
    task_list.append(pool.apply_async(generate_factor_test,args=(h5_name,save_path)))
