...
from fast_factor.code.neptunelong.bs.function_factor_xdb_bs import *
...
# 计算
list_del = []
for file_name in os.listdir('/dfs/user/015585/01_factor_develop_store/fast_factor_newframe/neptunelong/20250918_xdb_balancesheet_cs/factor_value/neptunelong/'):
...
class factor_{}(BaseFactor):
    ...
    if len(list_class) > 0:
        res, check_res = Runner.run(start_date=20170110, end_date=20201231, strategy=strategy,
                         output_dir=f"/dfs/user/015585/01_factor_develop_store/fast_factor_newframe/{strategy}/20250924_xdb_balancesheet_cs/",
                         options={
                             "calc.num_cpus": 30,
                             "local_evaluator": "",
                             'precheck': False,
