...
def f_calc_cct(tick_series): # (tick_df['factor']**2).sum() / (tick_df['factor'].sum())**2 if abs(tick_df['factor'].sum()) > 1e-3 else np.nan
...
def f_calc_m2m(tick_series): # tick_df['factor'].max() / tick_df['factor'].mean() if tick_df['factor'].mean()>0 else np.nan
    if tick_series.empty:
        return np.nan
    else:
