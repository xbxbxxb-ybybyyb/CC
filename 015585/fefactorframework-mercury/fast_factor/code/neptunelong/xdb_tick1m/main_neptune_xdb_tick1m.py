...
from fast_factor.code.neptune.xdb_tick1m.function_factor_xdb_tick1m import *
...
# 计算
list_del = []
for file_name in os.listdir('/dfs/user/015585/01_factor_develop_store/fast_factor_newframe/neptune/20250820_xdbtick1m/factor_value/neptune/'):
    list_del.append(file_name.replace('.h5',''))
for file_name in os.listdir('/dfs/user/015585/01_factor_develop_store/fast_factor_newframe/neptune/20250825_xdbtick1m/factor_value/neptune/'):
    list_del.append(file_name.replace('.h5', ''))
...
class factor_{}(BaseFactor):
    ...
    if len(list_class) > 0:
        res, check_res = Runner.run(start_date=20170110, end_date=20201231, strategy=strategy,
                         output_dir="/dfs/user/015585/01_factor_develop_store/fast_factor_newframe/neptune/20250901_xdbtick1m/",
                         options={
                             "calc.num_cpus": 24,
                             "local_evaluator": "",
                             'precheck': False,
...
#            print('IC:',check_res[i + '_' + strategy].result_dic['corr_sta'].loc['corr_tot', 'value'])
        res, check_res = Runner.run(start_date=20210101, end_date=20211231, strategy=strategy,
                         output_dir="/dfs/user/015585/01_factor_develop_store/fast_factor_newframe/neptune/20250901_xdbtick1m_2/",
                         options={
                             "calc.num_cpus": 24,
                             "local_evaluator": "",
                             'precheck': False,
