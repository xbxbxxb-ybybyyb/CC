...
def f_pro_s2ttran(tick_df):#委卖金额/该阶段总成交额,series（开盘后）
    tick_df['sell_amt'] = tick_df['TotalOfferQty'] * tick_df['WeightedAvgOfferPx']
    tick_df['factor'] = (tick_df['sell_amt'])/(tick_df['ValueTrade'].sum()+1)
    return tick_df['factor']
...
def f_pro_xyx1(tick_df):# 相对下影线：last - low / high - low，series
    ...
    tick_df['amp'] = tick_df['pcummax'] - tick_df['pcummin']
    tick_df['amp'] = tick_df['amp'].apply(lambda x: np.nan if abs(x)<0.0001 else x)
    tick_df['xyx1'] = (tick_df['LastPx'] - tick_df['pcummin'])\
                      / tick_df['amp']
    return tick_df['xyx1']
...
def f_len_all(tick_df):
    return tick_df
def f_len_h15(tick_df):
    if len(tick_df)>15:
        return tick_df.head(15)
    else:
        return tick_df
def f_len_t15(tick_df):
    if len(tick_df)>15:
        return tick_df.tail(15)
    else:
        return tick_df
...
def f_calc_cv(tick_series): # tick_df['factor'].std() / tick_df['factor'].mean() if abs(tick_df['factor'].mean()) > 1e-5 else np.nan
    if tick_series.empty:
        return np.nan
    else:
...
def f_calc_cct(tick_series): # res = (tick_df['factor']**2).sum() / (tick_df['factor'].sum())**2 if abs(tick_df['factor'].sum()) > 1e-3 else np.nan
