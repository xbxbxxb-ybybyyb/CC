...
def f_pro_orderp2bp(tick_df):# 订单价格 - 买均对应的收益率
    ...
    return tick_df['factor'].replace(np.inf,np.nan).replace(-np.inf,np.nan)
...
def f_len_t20(tick_df):
    if len(tick_df)>20:
        ...
        return tick_df
def f_len_t100(tick_df):
    if len(tick_df)>100:
        return tick_df.tail(100)
    else:
        return tick_df
def f_len_t1min(tick_df):
    def inttime2deltamls(itime):
        mls = int(str(int(itime))[-3:])
        s = int(str(int(itime))[-5:-3])
        m = int(str(int(itime))[-7:-5])
        h = int(str(int(itime))[:-7])
        time_mls = h * 3600 * 1000 + m * 60 * 1000 + s * 1000 + mls
        time_mls_930 = 9 * 3600 * 1000 + 30 * 60 * 1000
        if int(itime) > 120000000:
            time_delta = time_mls - time_mls_930 - 5400000
        else:
            time_delta = time_mls - time_mls_930
    ...
    return tick_df[tick_df['MDTime_delta'] >= (tick_df['MDTime_delta'].max() - 60*1000)]
