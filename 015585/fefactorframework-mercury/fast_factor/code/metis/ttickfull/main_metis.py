...
from fast_factor.code.metis.ttickfull.function_factor import *
...
class factor_{}(BaseFactor):
    ...
    low_cost = "是"  # 是否低耗时
        ...
        t_day_data = ['TTickfulladdorder_MetisAll']
        xdb_data = []
        ...
        def prepare_T_data(self, database):
            if database["skip"] == True:
                ...
                tick_df = database['TTickfulladdorder_MetisAll']
                tick_df['VolumeTrade'] = tick_df['TotalVolumeTrade'] - tick_df['TotalVolumeTrade'].shift(1).fillna(0)
                database['TTickfulladdorder_MetisAll'] = tick_df
                return database
        if calc_i == 'nocalc':
    ...
    if len(list_class) > 0:
        res, check_res = Runner.run(start_date=20170101, end_date=20191231, strategy=strategy,
                         output_dir="/dfs/user/015585/01_factor_develop_store/fast_factor_newframe/metis/20240830ttickfull/",
                         options={
                             "calc.num_cpus": 24,
                             "local_evaluator": "",
                             'precheck': False,
