...
from fast_factor.code.europa.xdb_cancel.function_factor_xdb_cancel import *
...
# 计算
list_del = []
# for file_name in os.listdir('/dfs/user/015585/01_factor_develop_store/fast_factor_newframe/europa/20241206tcancel/factor_value/europa'):
...
class factor_{}(BaseFactor):
       ...
       'lag': 1 # 回看日期，N为往前回看1~N天
        ...
        def pre_calculate_T_N_data(self, database):
            if database["skip"] == True:
                ...
                # 筛选买卖单
                tick1m_df_t_1 = dic_cancel_kind1[cancel_kind1_i](tick1m_df_t)
                # 筛选大小单
                tick1m_df_t_2 = dic_cancel_kind2[cancel_kind2_i](tick1m_df_t_1)
                # 筛选cancel价格
                p = dic_cancel_kind3[cancel_kind3_i](tick1m_df_t)
                if p > 0:
                    tick1m_df_t_3 = get_f_p_filter(tick1m_df_t_2, cancel_type3_i, p)
                else:
                ...
                # 如果是amt,尝试标准化
                if (property_i == 'amt'):
                    factor_origin = dic_std[std_i](tick1m_df, factor_origin)
                # 计算最终结果
    ...
    if len(list_class) > 0:
        res, check_res = Runner.run(start_date=20170101, end_date=20191231, strategy=strategy,
                         output_dir="/dfs/user/015585/01_factor_develop_store/fast_factor_newframe/europa/20241223xdb_cancel/",
                         options={
                             "calc.num_cpus": 24,
                             "local_evaluator": "",
                             'precheck': False,
