...
class factor_xbc_md_20250425_3(BaseFactor):
    strategy_name = "hotspot"
    factor_name = "xbc_md_20250425_3"
    fill_na_value = 0
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            # consective up
            md['consective_up_days'] = 0
            md['consective_up_days_tmp'] = 0
            md['consective_up_flag'] = True
            md['consective_up_ratio'] = 1
            md['consective_up_ratio_tmp'] = 1
            for i in range(1,11):
                flag = md['consective_up_flag'] & (md['close_ratio'+str(i)]<=0.99)
                md.loc[flag, 'consective_up_flag'] = False
                flag = md['consective_up_flag']
                md.loc[flag, 'consective_up_ratio_tmp'] *= md.loc[flag,'close_ratio'+str(i)]
                md.loc[flag, 'consective_up_days_tmp'] +=1
                flag = md['consective_up_flag'] & (md['consective_up_ratio_tmp']>1)
                md.loc[flag, 'consective_up_ratio'] *= md.loc[flag,'consective_up_ratio_tmp']
                md.loc[flag, 'consective_up_days'] += md.loc[flag,'consective_up_days_tmp']
                md.loc[flag, 'consective_up_ratio_tmp'] = 1
                md.loc[flag, 'consective_up_days_tmp'] = 0
            factor_df = pd.DataFrame()
            factor_df[factor_name] = md['consective_up_ratio']
            # -------------------------------------------------------------------------------------------------------------------
