...
class factor_xbc_md_20250425_2(BaseFactor):
    strategy_name = "hotspot"
    factor_name = "xbc_md_20250425_2"
    fill_na_value = 0
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            # consective down
            md['consective_down_days'] = 0
            md['consective_down_days_tmp'] = 0
            md['consective_down_flag'] = True
            md['consective_down_ratio'] = 1
            md['consective_down_ratio_tmp'] = 1
            for i in range(1,10+1):
                flag = md['consective_down_flag'] & (md['close_ratio'+str(i)]>=1.01)
                md.loc[flag,'consective_down_flag'] = False
                flag = md['consective_down_flag']
                md.loc[flag, 'consective_down_ratio_tmp'] *= md.loc[flag,'close_ratio'+str(i)]
                md.loc[flag, 'consective_down_days_tmp'] +=1
                flag = md['consective_down_flag'] & (md['consective_down_ratio_tmp']<1)
                md.loc[flag, 'consective_down_ratio'] *= md.loc[flag,'consective_down_ratio_tmp']
                md.loc[flag, 'consective_down_days'] += md.loc[flag,'consective_down_days_tmp']
                md.loc[flag, 'consective_down_ratio_tmp'] = 1
                md.loc[flag, 'consective_down_days_tmp'] = 0

            factor_df = pd.DataFrame()
            factor_df[factor_name] = md['consective_down_ratio']
            # -------------------------------------------------------------------------------------------------------------------
