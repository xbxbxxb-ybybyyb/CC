...
class factor_zwh_20240530_005(BaseFactor):
    owner = 'zwh'
    strategy_name = "hotspot"
    factor_name = "zwh_20240530_005"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    factor_explain = "交易量换手波动" # 因子逻辑解释
    zcz_adjusted = "是" # 是否针对注册制调整：是/否（本因子需要调整，为简单起见未加入注册制部分）
    logic_type = "交易量换手波动" # 逻辑类别
    low_cost = "是" # 是否低耗时
    #
    t_day_data = []
    xdb_data = []
    t_1_factor_data = [
        {'name': 'MD_CHINA_STOCK_DAILY_WIND',  # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 50,  # 注意为正数
         'column': ['turn',  'pre_close', 'amt',  'adjfactor']
         }]
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        x1 = x.unstack().rolling(14, min_periods=7).std().stack()
