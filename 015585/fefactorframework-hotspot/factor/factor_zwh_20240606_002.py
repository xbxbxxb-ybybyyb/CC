...
class factor_zwh_20240606_002(BaseFactor):
    owner = 'zwh'
    strategy_name = "hotspot"
    factor_name = "zwh_20240606_002"
    fill_na_value = 0
    ...
    t_1_factor_data = [
        {'name': 'MD_CHINA_STOCK_DAILY_WIND',  # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 50,  # 注意为正数
         'column': ['high',  'amt', 'vwap', 'adjfactor', 'pre_close']
         }]
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        x = md_data['high'] / (EPS + md_data['vwap']) / (EPS + md_data['amt'])
        x1 = x.unstack().rolling(20, min_periods=20).max().stack()
        x2 = x.unstack().rolling(3, min_periods=3).min().stack()
        result = x1 / (EPS + x2)
