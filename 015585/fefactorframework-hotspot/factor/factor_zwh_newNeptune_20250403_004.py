...
class factor_zwh_newNeptune_20250403_004(BaseFactor):
    owner = 'zwh'
    strategy_name = "hotspot"
    factor_name = "zwh_newNeptune_20250403_004"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    factor_explain = "量价波动变化" # 因子逻辑解释
    ...
    t_1_factor_data = [
        {'name': 'MD_CHINA_STOCK_DAILY_WIND',  # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 100,  # 注意为正数
         'column': ['turn', 'high', 'close', 'vwap', 'pre_close', 'adjfactor']
         }]
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        high_pct = (high-df['pre_close']) /df['pre_close'] -1
        vv = high_pct / np.log(df['turn'] + 1)  # * df['pct_chg']
        N = 5 # 5-32
        # res = vv.unstack().rolling(N, min_periods=N).min().stack()
        res = vv.unstack().rolling(N, min_periods=N).max().stack()  - vv.unstack().rolling(N, min_periods=N).min().stack()
