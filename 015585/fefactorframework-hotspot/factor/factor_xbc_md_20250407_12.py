...
class factor_xbc_md_20250407_12(BaseFactor):
    strategy_name = "hotspot"
    factor_name = "xbc_md_20250407_12"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "xbc"  # 开发人员姓名
    factor_explain = "high与low的关系" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            factor_df[factor_name] =  (md_data['stat']-0.5*market_mean).unstack().rolling(20, min_periods=1).mean().stack()
            # -------------------------------------------------------------------------------------------------------------------
