...
class factor_zwh_newNeptune_20250327_002(BaseFactor):
    owner = 'zwh'
    strategy_name = "hotspot"
    factor_name = "zwh_newNeptune_20250327_002"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    factor_explain = "价格波动率趋势" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        high = (df['open'] - df['close']) / (df['high'] - df['low'] + 0.000001)
        high_rank = (high.unstack().rank(axis=1) / (~ high.unstack().isnull()).values.sum(axis=1).reshape(-1, 1)).stack()

        N = 14
        nn = 2
        res = high.unstack().rolling(N, min_periods=N).mean().stack() - high.unstack().rolling(nn,min_periods=nn).mean().stack()  # 49.9
