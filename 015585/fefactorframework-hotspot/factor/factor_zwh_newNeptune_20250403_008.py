...
class factor_zwh_newNeptune_20250403_008(BaseFactor):
    owner = 'zwh'
    strategy_name = "hotspot"
    factor_name = "zwh_newNeptune_20250403_008"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    factor_explain = "close短期趋势" # 因子逻辑解释
    ...
    t_1_factor_data = [
        {'name': 'MD_CHINA_STOCK_DAILY_WIND',  # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 100,  # 注意为正数
         'column': ['turn', 'high', 'close', 'low', 'pre_close', 'adjfactor']
         }]
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        vv = (abs(high_pct) + abs(low_pct)) * (df['turn'])
        vv = vv.round(5)
