...
class factor_zwh_20240606_004(BaseFactor):
    owner = 'zwh'
    strategy_name = "hotspot"
    factor_name = "zwh_20240606_004"
    fill_na_value = 0
    ...
    t_1_factor_data = [
        {'name': 'MD_CHINA_STOCK_DAILY_WIND',  # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 50,  # 注意为正数
         'column': ['pct_chg', 'volume',  'amt', 'vwap', 'adjfactor', 'pre_close']
         }]
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        x = md_data['pct_chg'] * md_data['volume'] * md_data['amt']
        def rank_(data_):
