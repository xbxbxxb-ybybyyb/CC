...
class factor_zwh_hotspot_20250311_003(BaseFactor):
    strategy_name = "hotspot"
    factor_name = "zwh_hotspot_20250311_003"
    fill_na_value = 0
    ...
    t_1_factor_data = [
        {'name': 'MD_CHINA_STOCK_DAILY_WIND',  # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 30,  # 注意为正数
         'column': ['amt','pct_chg','close', 'open', 'pre_close', 'adjfactor']
         }]
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        ma = amt.unstack().rolling(20, min_periods=10).std().stack()
        mi = pct.unstack().rolling(20, min_periods=10).std().stack()

        res = pct * (ma - mi) / (0.0001 + mi)
