...
class factor_xbc_md_20250407_17(BaseFactor):
    strategy_name = "hotspot"
    factor_name = "xbc_md_20250407_17"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "xbc"  # 开发人员姓名
    factor_explain = "high与close的关系" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            md_data['stat'] = (md_data['high'] - md_data['close']) / (md_data['high'] - md_data['low'])
            stock_std = md_data['stat'].unstack().rolling(100, min_periods=3).max().stack()
            md_data[factor_name] = (md_data['stat']/stock_std).unstack().rolling(6,1).mean().stack()
            # -------------------------------------------------------------------------------------------------------------------
