...
class factor_zwh_20240606_003(BaseFactor):
    owner = 'zwh'
    strategy_name = "hotspot"
    factor_name = "zwh_20240606_003"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    factor_explain = "涨幅换手 max*min/skew" # 因子逻辑解释
    zcz_adjusted = "是" # 是否针对注册制调整：是/否（本因子需要调整，为简单起见未加入注册制部分）
    logic_type = "涨幅换手关系" # 逻辑类别
    low_cost = "是" # 是否低耗时
    #
    t_day_data = []
    xdb_data = []
    t_1_factor_data = [
        {'name': 'MD_CHINA_STOCK_DAILY_WIND',  # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 50,  # 注意为正数
         'column': ['pct_chg', 'open',  'amt', 'vwap', 'adjfactor', 'pre_close']
         }]
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        x = md_data['pct_chg'] * md_data['amt']
        def rank_(data_):
            data_ = data_.apply(lambda x: round_(x, 4))
            data_r = data_.unstack().rank(axis=1, method='average', pct=True, na_option='keep').stack()
            return data_r
        x = rank_(x).apply(lambda x: round_(x, 6))

        x1 = x.unstack().rolling(30, min_periods=30).median().stack()
        x2 = x.unstack().rolling(5, min_periods=5).median().stack()

        result = x1 / (EPS + x2)
        md_data[self.factor_name] = result.apply(lambda x: round_(x, 5))
