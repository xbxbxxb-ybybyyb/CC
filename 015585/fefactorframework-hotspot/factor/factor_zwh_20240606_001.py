...
class factor_zwh_20240606_001(BaseFactor):
    owner = 'zwh'
    strategy_name = "hotspot"
    factor_name = "zwh_20240606_001"
    fill_na_value = 0
    ...
    t_1_factor_data = [
        {'name': 'MD_CHINA_STOCK_DAILY_WIND',  # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 50,  # 注意为正数
         'column': ['pct_chg', 'high', 'amt', 'vwap', 'adjfactor', 'pre_close']
         }]
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        x = md_data['pct_chg'] * md_data['high'] - md_data['amt']
        def rank_(data_):
        ...
        x1 = x.unstack().rolling(14, min_periods=14).std().stack()
