...
class factor_qyh_hotspot_pct20high(BaseFactor):
    strategy_name = "hotspot"
    factor_name = "qyh_hotspot_pct20high"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "qyh"  # 开发人员姓名
    factor_explain = "20日最高价（复权）对应最新close的涨跌幅" # 因子逻辑解释
    ...
    t_1_factor_data = [
        {'name': 'MD_CHINA_STOCK_DAILY_WIND', # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 80, #注意为正数
         'column': ['pct_chg', 'turn', 'pre_close', 'amt', 'close', 'adjfactor','high']
    }]  # T-N factor数据，格式如上
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['factor'] = df_ori['high'].unstack().rolling(20,1).max().stack() / df_ori['close'] - 1
