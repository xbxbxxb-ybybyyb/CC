...
def convert_tick(df, date, output_path): # tick1s, tickfull转为二进制文件
    ...
    with open(output_path + "/" + date, 'wb') as file:
        ...
        for symbol in symbol_list:
            ...
            int_selected = cur_df[[
                "MDTime", "NumTrades", "TotalVolumeTrade",
...
def prepare_cancel_data_new(df, base_date):
    try:
        ...
        df['dl_price'] = np.floor(df['pre_close'] * 100 * 0.9 + 0.5 + 1e-8) / 100
        df['dl_price'][df['zcz']] = np.floor(df['pre_close'] * 100 * 0.8 + 0.5 + 1e-8) / 100
        df = df[['MDDate', 'MDTime', 'OrderIndex', 'OrderPrice', 'OrderQty', 'OrderBSFlag',
                 'ff_shares', 'pattern', 'industry', 'after_not_ul_len', 'pre_close','ul_price','dl_price']]
        for col in ['OrderPrice','pre_close','ul_price','dl_price']:
            df[col] = df[col].apply(lambda x : round_(x,2))
        for col in ['ff_shares']:
...
def prepare_data(trading_days, data_types, base_path, cpus=16, lag=10, back_up_lag=20, strategy=''):
    ...
    europa_basic = pd.read_hdf('/data/group/800463/data/project1_public/factor_lib_v3/Basic_zt_001_20150901_20191231.h5')
...
if __name__ == "__main__":
    ...
    # base_path_dict = {'europa_jupiter': "/dfs/user/015585/test/europa_jupiter/",
    #                   'saturn_sell': "/dfs/user/015585/test/saturn_sell/",
    #                   'mimas': "/dfs/user/015585/test/mimas/",
    #                   'metis': "/dfs/user/015585/test/metis/"}
    start_date = "20180101"
    end_date = "20180131"
    cpus = 16
    lag = 3
    data_types = ["xdb_order",'xdb_trade',"xdb_tick1s", "xdb_tickfull",'xdb_tickex','xdb_cancel']
