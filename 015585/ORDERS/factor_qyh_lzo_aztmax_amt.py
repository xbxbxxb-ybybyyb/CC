...
import pandas as pd
# NO
# 逻辑：t-1日涨停挂单峰值以后，挂单金额
# score:-0.02,2.5
# wu
factor_name = 'qyh_lzo_aztmax_amt'#
def factor_qyh_lzo_aztmax_amt(order_df, return_fillna_dic=False):
    if return_fillna_dic:
        # 返回因子为nan时的填充值
        return {factor_name: 214091091}
    # 0930以后
    ...
    #
    amt = (order_df[order_df['min'] > t]['OrderQty'] * order_df[order_df['min'] > t]['OrderPrice']).sum()
