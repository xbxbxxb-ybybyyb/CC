...
import numpy as np
import pandas as pd
# 逻辑：t-1日涨停挂单峰值以后，新增挂买均价
# score:0,0
# wu
factor_name = 'qyh_lzo_aztmax_p'#
def factor_qyh_lzo_aztmax_p(order_df, return_fillna_dic=False):
    ...
    p = (order_df_after['OrderQty'] * order_df_after['OrderPrice']).sum() / order_df_after['OrderQty'].sum()
    ratio = (p_zt - p) / pre_close
