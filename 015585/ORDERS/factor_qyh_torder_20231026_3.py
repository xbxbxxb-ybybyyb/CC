import numpy as np
import pandas as pd
factor_name = 'qyh_torder_20231026_3'#
def factor_qyh_torder_20231026_3(order_df, return_fillna_dic=False):
    ...
    #
    order_df = order_df.head(int(len(order_df)/2)) if len(order_df)>10 else order_df
    #
    if order_df['MDTime_delta'].empty:
        res = np.nan
    else:
        res = order_df['MDTime_delta'].std() / order_df['MDTime_delta'].mean() if abs(order_df['MDTime_delta'].mean()) > 0 else np.nan
    #
