...
import pandas as pd
# zcz,dtj
# 非涨停且时间靠前的订单，买卖单的下单时间差异(标准化)
# 35，-0.07
#
factor_name = 'qyh_torder_20231109_1'#
def factor_qyh_torder_20231109_1(order_df, return_fillna_dic=False):
    ...
    #
    res1 = order_df1['MDTime_delta'].sum() / (order_df1['MDTime_delta'].std() ** 0.8+1)
    res2 = order_df2['MDTime_delta'].sum() / (order_df2['MDTime_delta'].std() ** 0.8+1)
    res = res1/res2 if res2 > 0 else np.nan
    res = 1000 if res > 1000 else res
