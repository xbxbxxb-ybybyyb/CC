        ...
        if data_name == "xdb_order":
            for symbol in symbols:
                ...
                for date in date_li:
                    ...
                    if (daily_float_a_share_df.empty) | (daily_md_df.empty):
                        logger.error(
                            "Empty md or float_a_share dataframe! date={}, symbol={}, type={}".format(date, symbol,
                                                                                                      data_name))
                        raise RuntimeError(
            ...
            for symbol in symbols:
                ...
                for date in date_li:
                    ...
                    tick_df['VolumeTrade'] = (
                                tick_df['TotalVolumeTrade'] - tick_df['TotalVolumeTrade'].shift(1).fillna(0)).apply(
                        lambda x: round_(x, 2))
...
def prepare_data(trading_days, data_types, base_path, cpus=16, lag=10, back_up_lag=20, strategy=''):
    ...
    neptune_basic = neptune_basic.query('Ticker == "600624.SH"')
