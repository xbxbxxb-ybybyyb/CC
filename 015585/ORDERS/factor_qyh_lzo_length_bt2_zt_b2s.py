...
import pandas as pd
# dtj
# 逻辑：t-1日,封板前挂涨停的买卖单数量比
# 0.07，26
# zcz
#
factor_name = 'qyh_lzo_length_bt2_zt_b2s'#
def factor_qyh_lzo_length_bt2_zt_b2s(order_df, return_fillna_dic=False):
    ...
    length1 = len(order_df[order_df['OrderBSFlag'] == 1])
    length2 = len(order_df[order_df['OrderBSFlag'] == 2])
    factor_dict = {factor_name: length1/length2 if length2 > 0 else length1}
