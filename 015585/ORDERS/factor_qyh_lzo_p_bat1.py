...
import pandas as pd
#
# 逻辑：t-1日涨停挂单峰值前后的挂单均价对应的涨跌幅的比例
# 0.06,12
# wu
factor_name = 'qyh_lzo_p_bat1'#
def factor_qyh_lzo_p_bat1(order_df, return_fillna_dic=False):
    if return_fillna_dic:
        # 返回因子为nan时的填充值
        return {factor_name: 0.53}
    # 0930以后
    ...
    #
    order_df_1 = order_df[order_df['min'] < t]
    order_df_2 = order_df[order_df['min'] > t]
    p_1 = (order_df_1['OrderQty'] * order_df_1['OrderPrice']).sum() / (order_df_1['OrderQty'].sum())
    p_2 = (order_df_2['OrderQty'] * order_df_2['OrderPrice']).sum() / (order_df_2['OrderQty'].sum())
    if pre > 0.1:
        ratio_1 = p_1/pre - 1
        ratio_2 = p_2/pre - 1
        if abs(ratio_2) >= 0.001:
