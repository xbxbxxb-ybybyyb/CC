...
import pandas as pd
# dtj
# 逻辑：t-1日挂单量稳定以后，挂买金额 / 流动市值
# score:-0.05,9
# wu
factor_name = 'qyh_lzo_a95_amt_b2mv'#
def factor_qyh_lzo_a95_amt_b2mv(order_df, return_fillna_dic=False):
    if return_fillna_dic:
        # 返回因子为nan时的填充值
        return {factor_name: 63}
    # 3mad
    ...
    amt = order_df[order_df['OrderBSFlag'] == 1]['OrderAmt'].sum()
    if amt < 10:
        amt = np.nan
    #
