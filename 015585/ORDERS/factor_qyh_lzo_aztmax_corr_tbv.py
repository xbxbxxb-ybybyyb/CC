...
import pandas as pd
# 待提交
# 逻辑：t-1日涨停挂单峰值以后，新增挂买的数量与时间的corr
# score:0.01
# group test对称分布
factor_name = 'qyh_lzo_aztmax_corr_tbv'#
def factor_qyh_lzo_aztmax_corr_tbv(order_df, return_fillna_dic=False):
    ...
    # 峰值时间
    order_df_zt = order_df[(order_df['OrderPrice'] >= p_zt) & (order_df['OrderBSFlag'] == 1)]
    ...
    corr = order_df_zt.loc[t:].reset_index().corr(method = 'spearman').iloc[0,1]
