...
def get_use_data(py_code):
    ...
    data_dic = {'/data/group/800080/warehouse/prod/MD/CHINA_STOCK/DAILY/WIND/MD_CHINA_STOCK_DAILY_WIND.h5': 'MD',
                ...
                '/data/group/800463/data/generalStrong/concept_h5_except_300/':'concept'#其他地方没改
                }
    data_use_list = []
...
def format_check(factor_name, factor_type, check_date, before_submit_name_list):
    ...
    if factor_type not in ['T-1_factor', 'other', 'TTickab', 'TTransaction', 'TOrder',
    ...
    try:
        ...
        tick_df = pd.read_pickle('/data/group/800463/data/project1_prod/tickab_test_001/20160104.pkl')
...
def value_same_check(func, factor_name, result_path,filter_df):
    ...
    long_interval = [20150901, 20190930]
    short_interval_list = s.tradingday(20150901, 20150907)
...
def score_check(factor_name,filter_df_1619,factor_data_path):
    print('因子测试', dt.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    factor_file_path = factor_data_path + '%s/' % (factor_name)
    try:
        all_sample_df = pd.read_hdf(factor_file_path + '/%s.h5' % (factor_name))

        same_rate = (all_sample_df.reindex(filter_df_1619.index)[factor_name].value_counts().max() / len(all_sample_df.reindex(filter_df_1619.index)))
        if same_rate > 0.8:
            return '相同值比例较高-%.2f%%' % (same_rate * 100)
        if ('yzhan' in factor_name) and (same_rate > 0.2):
            return '相同值比例较高-%.2f%%' % (same_rate * 100)

        factor_mean = abs(all_sample_df.reindex(filter_df_1619.index)[factor_name].mean())
        factor_std = all_sample_df.reindex(filter_df_1619.index)[factor_name].std()
...
def pre_check(factor_name, factor_type, factor_date,check_res_path, before_submit_name_list,basic_file_path,factor_data_path,filter_df,filter_df_1619):
    ...
    check_dic['因子测试'] = score_check(factor_name,filter_df_1619,factor_data_path)

    if check_dic['因子测试']=='pass':
        check_dic['预检测'] = 'pass'
    return pd.Series(check_dic)

check_res_path = '/data/user/018107/factor_zoo1/europa_precheck/'
...
factor_data_path = '/data/user/018107/factor_zoo1/europa_factor_ezt/'
...
filter_df_1619=filter_df.loc[pd.Timestamp('20160101'):pd.Timestamp('20190930')]
...
for index, inf in factor_inf.iterrows():
    ...
    if factor_type not in ['T-1_factor', 'TTickab', 'TTransaction', 'TOrder',
                           'TTransaction_TTickab', 'TTransaction_TOrder', 'TOrder_TTickab',
                           'LastTouchTTick','MarketTTick','Market1TTick','MarketIndTTick',
    ...
    check_ser = pre_check(factor_name, factor_type, factor_date,check_res_path, before_submit_name_list,basic_file_path,factor_data_path,filter_df,filter_df_1619)
    print(check_ser, '\n')
