...
def prepare_data(trading_days, data_types, base_path, cpus=16, lag=10, back_up_lag=20):
    ...
    lowfrequency_basic = pd.read_hdf('/data/user/015585/share_file/for_wj/basic_20150101_20200720.h5')
...
if __name__ == "__main__":
    ...
    cpus = 1
    lag = 3
    data_types = ['xdb_tickex']# xdb_trade,xdb_tickex,xdb_order
    base_path = "/dfs/group/800463/data/xdb_data_lag3/mercury/"
    print(start_date,end_date)
    print(base_path)
    xquant_factor_data = FactorData()
    start_date = xquant_factor_data.tradingday(start_date, 1)[0]
    real_start_date = xquant_factor_data.tradingday(start_date, -(lag + 500 + 1))[0]
    trading_days = xquant_factor_data.tradingday(real_start_date, end_date)

    prepare_data(trading_days, data_types, base_path, cpus, lag, back_up_lag = 500)
