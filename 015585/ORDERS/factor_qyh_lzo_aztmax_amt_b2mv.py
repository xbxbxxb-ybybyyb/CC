...
import pandas as pd
#
# 逻辑：t-1日涨停挂单峰值以后，挂买额 / 流动市值
# score:-0.03,7
# wu
factor_name = 'qyh_lzo_aztmax_amt_b2mv'#
def factor_qyh_lzo_aztmax_amt_b2mv(order_df, return_fillna_dic=False):
    if return_fillna_dic:
        # 返回因子为nan时的填充值
        return {factor_name: 380}
    # 0930以后
    ...
    #
    order_df_after = order_df[(order_df['min'] > t) & (order_df['OrderBSFlag'] == 1)]
    amt = (order_df_after['OrderQty'] * order_df_after['OrderPrice']).sum()
    mv = order_df_after['ff_shares'].max() * order_df_after['pre_close'].max()
    factor_dict = {factor_name: amt/mv}
    # ---------------------------------------------------------------------------------------------------------------
