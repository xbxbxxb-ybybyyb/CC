...
import pandas as pd
#
# 逻辑：t-1日,早盘激进订单中，小单挂单金额/mv
# zcz
# 14,0.041
factor_name = 'qyh_lzo_morn9_amts_s2mv'#
def factor_qyh_lzo_morn9_amts_s2mv(order_df, return_fillna_dic=False):
    ...
    if mv > 10:
        ratio = order_df[order_df['OrderAmt'] <= 50000]['OrderAmt'].sum() /mv
    else:
        ratio = np.nan
