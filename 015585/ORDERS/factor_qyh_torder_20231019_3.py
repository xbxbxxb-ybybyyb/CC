...
factor_name = 'qyh_torder_20231019_3'#
def factor_qyh_torder_20231019_3(order_df, return_fillna_dic=False):
    if return_fillna_dic:
        # 返回因子为nan时的填充值
        return {factor_name: 0.073}
    ...
    order_df['MDTime_delta'] = (order_df['MDTime_delta']/10000).apply(lambda x :int(x))
    res_series = order_df.groupby('MDTime_delta').sum()['OrderAmt']
    factor_dict = {factor_name: (res_series**2).sum()/res_series.sum()**2}
    # ---------------------------------------------------------------------------------------------------------------
