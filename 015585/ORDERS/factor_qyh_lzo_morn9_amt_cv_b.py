...
import pandas as pd
#
# 逻辑：t-1日,早盘激进订单中，买单金额的变异系数
# zcz
# 25,0.06
factor_name = 'qyh_lzo_morn9_amt_cv_b'#
def factor_qyh_lzo_morn9_amt_cv_b(order_df, return_fillna_dic=False):
    if return_fillna_dic:
        # 返回因子为nan时的填充值
        return {factor_name: 1.46}
    # zcz
    ...
    # cv2 = order_df[order_df['OrderBSFlag'] == 2].std() / order_df[order_df['OrderBSFlag'] == 2].mean()
    factor_dict = {factor_name: cv1}
    # ---------------------------------------------------------------------------------------------------------------
