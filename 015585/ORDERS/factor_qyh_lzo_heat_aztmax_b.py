...
import pandas as pd
# dtj
# 逻辑：t-1日涨停挂单峰值以后，挂买单数目
# score:-0.01,2:最右侧负值很高
# wu
factor_name = 'qyh_lzo_heat_aztmax_b'#
def factor_qyh_lzo_heat_aztmax_b(order_df, return_fillna_dic=False):
    if return_fillna_dic:
        # 返回因子为nan时的填充值
        return {factor_name: 1344.5}
    # 0930以后
    ...
    #
    length = len(order_df[(order_df['min'] > t) & (order_df['OrderBSFlag'] == 1)])
