...
import pandas as pd
#
# 逻辑：t-1日,早盘挂单中，涨停与非涨停的集中度差异
#
#
# zcz
factor_name = 'qyh_lzo_10_amtcct_zt2nzt'#
def factor_qyh_lzo_10_amtcct_zt2nzt(order_df, return_fillna_dic=False):
    ...
    cct1 = (order_df1['OrderAmt'] ** 2).sum() / (order_df1['OrderAmt'].sum()**2) \
        if order_df1['OrderAmt'].sum()>0 else np.nan
    cct2 = (order_df2['OrderAmt'] ** 2).sum() / (order_df2['OrderAmt'].sum() ** 2) \
        if order_df2['OrderAmt'].sum() > 0 else np.nan
    cct = cct1-cct2
