...
def prepare_order_data_new(df, base_date):
    try:
        ...
        df.columns = ['symbol', 'appl_seq_num', 'OrderIndex', 'OrderPrice', 'OrderQty', "MDTime", "order_index",
                      'local_index', "OrderBSFlag", "OrderType", "timestamp", "MDDate",
        ...
        # ed = time.time()
        # logger.info("add index cost={}".format(ed - st))

        # st = time.time()
        df = df[['MDDate', 'MDTime', 'OrderIndex', 'OrderPrice', 'OrderQty', 'OrderBSFlag', "OrderType", 'HTSCSecurityID',
                 'ff_shares', 'pattern', 'industry', 'after_not_ul_len', 'pre_close']]
        # ed = time.time()
        # logger.info("select cost={}".format(ed - st))
...
def prepare_trade_data_new(df, base_date):
    try:
        ...
        df.columns = ['symbol', 'appl_seq_num', 'TradeBuyNo', 'TradeSellNo', 'TradePrice', "TradeQty", "MDTime", "TradeIndex",
                      'local_index', "TradeBSFlag", "timestamp", "MDDate",
                     'ff_shares', 'pre_close', 'industry', 'after_not_ul_len', 'HTSCSecurityID', 'Ticker', 'pattern', 'dt']
        # ed = time.time()
        # logger.info("rename cost={}".format(ed - st))
        # st = time.time()
        df["TradeMoney"] = df["TradeQty"] * df["TradePrice"]
        df["TradeType"] = 0
        df["dt"] = pd.to_datetime(df["dt"])
        df.set_index(["dt", "Ticker"], inplace=True)
        # ed = time.time()
...
def prepare_tick_data_new(df, base_date):
    try:
        # st = time.time()
        ...
        df = df[[
            ...
            'ff_shares', 'pattern', 'industry', 'after_not_ul_len', "OpenPx",
            "TotalBidQty",
            ...
            "Buy6OrderQty", "Buy7OrderQty", "Buy8OrderQty", "Buy9OrderQty", "Buy10OrderQty",
            "Buy1NumOrders", "Buy2NumOrders", "Buy3NumOrders", "Buy4NumOrders",
            "Buy5NumOrders",
            "Buy6NumOrders", "Buy7NumOrders", "Buy8NumOrders", "Buy9NumOrders", "Buy10NumOrders",
            ...
            "Sell10OrderQty", "Sell1NumOrders", "Sell2NumOrders", "Sell3NumOrders", "Sell4NumOrders",
            "Sell5NumOrders", "Sell6NumOrders", "Sell7NumOrders", "Sell8NumOrders", "Sell9NumOrders",
            "Sell10NumOrders", ]]
        # ed = time.time()
        # logger.info("select cost={}".format(ed - st))
    except Exception as e:
        ...
        if data_name == "xdb_order":
            for symbol in symbols:
                ...
                for date in date_li:
                    ...
                    order_df["ff_shares"] = daily_df["free_float_shares"].values[0]
            ...
            for symbol in symbols:
                ...
                for date in date_li:
                    ...
                    if daily_df.empty:
                        logger.error("Empty daily dataframe! date={}, symbol={}, type={}".format(date, symbol, data_name))
                        raise RuntimeError("Empty daily dataframe! date={}, symbol={}, type={}".format(date, symbol, data_name))
                    trade_df["ff_shares"] = daily_df["free_float_shares"].values[0]
            ...
            #             continue
            #         daily_df = xdb_datasource.get_dailydata(date, symbol)
            #         cancel_df["ff_shares"] = daily_df["free_float_shares"].values[0]
            ...
            for symbol in symbols:
                ...
                for date in date_li:
                    ...
                    tickex_df["ff_shares"] = daily_df["free_float_shares"].values[0]
                ...
                tmp_df["dt"] = dates[-1]
                tmp_df = prepare_tick_data_new(tmp_df, dates[-1])
                result[symbol] = tmp_df
            ...
            for symbol in symbols:
                ...
                for date in date_li:
                    ...
                    tick1s_df["ff_shares"] = daily_df["free_float_shares"].values[0]
            ...
            for symbol in symbols:
                ...
                for date in date_li:
                    ...
                    if daily_df.empty:
                        logger.error("Empty daily dataframe! date={}, symbol={}, type={}".format(date, symbol, data_name))
                        raise RuntimeError("Empty daily dataframe! date={}, symbol={}, type={}".format(date, symbol, data_name))
                    tickfull_df["ff_shares"] = daily_df["free_float_shares"].values[0]
...
def prepare_data(trading_days, data_types, base_path, cpus=16, lag=10, back_up_lag=20):
    ...
    for i in data_types:
        ...
        while idx <= len(trading_days):
            dates = trading_days[idx-lag-back_up_lag:idx]
...
if __name__ == "__main__":
    ...
    # data_types = ["xdb_trade"]
    base_path = "/dfs/group/800463/data/xdb_data_lag3/jupiter_europa/"
