...
'''
1、下载xquant-tick3s、order、trade的2016年数据
2、逻辑&格式与xdb基础数据相同，即按不同策略basic确认标的，过去3日（追溯20交易日）的高频数据
3、优先完成tick3s和trade，order需要等sss进行合成再下载
'''
import pandas as pd
import os
import numpy as np
...
def prepare_order_data_new_xquant(df, base_date):
    try:
        ...
        df = df.rename(columns={'OrderNo': 'OrderIndex'}) # 请sss确认到底要不要这一步
        df = df[df['OrderPrice'] > 0]
            ...
            for symbol in symbols:
                ...
                for date in date_li:
                    ...
                    if symbol == "000043.SZ" and date < str(20191216):
                        daily_float_a_share_df = float_a_share_df.loc[idx[date:date, "001914.SZ":"001914.SZ"], :]
                        daily_md_df = md_df.loc[idx[date:date, "001914.SZ":"001914.SZ"], :]
                    elif symbol == "000022.SZ" and date < str(20181226):
                        daily_float_a_share_df = float_a_share_df.loc[idx[date:date, "001872.SZ":"001872.SZ"], :]
                        daily_md_df = md_df.loc[idx[date:date, "001872.SZ":"001872.SZ"], :]
                    elif symbol == "200022.SZ" and date < str(20181226):
                        daily_float_a_share_df = float_a_share_df.loc[idx[date:date, "201872.SZ":"201872.SZ"], :]
                        daily_md_df = md_df.loc[idx[date:date, "201872.SZ":"201872.SZ"], :]
...
if __name__ == "__main__":
    ...
    cpus = 15
    lag = 3
    # data_types = ["xdb_order",'xdb_trade','xdb_tickex']# xdb_trade,xdb_tickex,xdb_order
    data_types = ['xdb_trade','xdb_tickex']
    base_path = "/dfs/group/800463/data/xdb_data_lag3/mimas/"
