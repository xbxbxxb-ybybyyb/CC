...
'''
1、新增addorder的全息盘口
2、order, cancel, trade, tickex, tick1s, tickfull新增appl_seq_num字段
 for zbw: 按昨日沟通，涉及的数据都增加appl_seq_num
 1）convert_tick也要修改，增加appl_seq_num部分，注意之前appl_seq_num错误但因为未使用所以忽略的问题
'''
import pandas as pd
import os
import numpy as np
...
def prepare_data(trading_days, data_types, base_path, cpus=16, lag=10, back_up_lag=20, strategy=''):
    if strategy not in ['europa_jupiter', 'saturn_sell', 'mimas', 'metis']:
        logger.error("策略名称不在枚举中")
    if cpus == 1:
        logger.error("cpu=1太慢了！多设一点")
        # return
    logger.info("start prepareing data!")
    jupiter_basic = pd.read_hdf('/data/user/018107/factor_zoo1/left_v2310/Basic_zt/Basic_zt.h5')
    saturn_basic = pd.read_hdf('/data/group/800463/project/project2_prod/daily_data/Basic/Basic_closed_hf_finish.h5')
    sell_basic = pd.read_hdf('/data/group/800463/project/project2_prod/daily_data/Basic/Basic_closed_hf_finish.h5')
    europa_basic = pd.read_hdf('/data/user/018107/factor_zoo1/left_v2310/Basic_zt_test/Basic_zt_001.h5')
    metis_basic = pd.read_hdf('/data/group/800463/project/project1_prod/left_v2212/Basic_metis/Basic_metis.h5')
    mimas_basic = pd.read_hdf('/data/user/018107/factor_zoo2/history/20160101_20230625/Basic_next_hf_finish_20160101_20230625.h5')
    ...
    if strategy == 'europa_jupiter':
        basic_dict = {key: basic_dict_all[key] for key in ['jupiter', 'europa', 'industry', 'md_df']}
    elif strategy == 'saturn_sell':
        basic_dict = {key: basic_dict_all[key] for key in ['saturn', 'sell', 'industry', 'md_df']}
    elif strategy == 'metis':
        basic_dict = {key: basic_dict_all[key] for key in ['metis', 'industry', 'md_df']}
    elif strategy == 'mimas':
        basic_dict = {key: basic_dict_all[key] for key in ['mimas', 'industry', 'md_df']}
    else:
        raise TypeError
    tasks = []
    for i in data_types:
...
if __name__ == "__main__":
    base_path_dict = {'europa_jupiter': "/dfs/group/800463/data/xdb_data_lag3_test/europa_jupiter/",
                      'saturn_sell': "/dfs/group/800463/data/xdb_data_lag3_test/saturn_sell/",
                      'mimas': "/dfs/group/800463/data/xdb_data_lag3_test/mimas/",
                      'metis': "/dfs/group/800463/data/xdb_data_lag3_test/metis/"
                      }
    start_date = "20170110"
    end_date = "20211231"
    cpus = 24
    ...
    data_types = ["xdb_order", 'xdb_trade', "xdb_tick1s", "xdb_tickfull", 'xdb_tickfulladdorder',
                  'xdb_cancel'] # 可以不跑xdb_tickex，因为交易所tick没有appl_seq_num
    # data_types = ['xdb_tickfulladdorder',]
