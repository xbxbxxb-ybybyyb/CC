import numpy as np
import pandas as pd
factor_name = 'qyh_torder_20231130_3'#
def factor_qyh_torder_20231130_3(order_df, return_fillna_dic=False):
    ...
    order_df = order_df.tail(int(len(order_df)/4))
    ...
    mean_f = (order_df['factor'] * order_df['OrderAmt']).sum() / order_df['OrderAmt'].sum()
    std_f = ((((order_df['factor'] - mean_f)**2) * order_df['OrderAmt']).sum() / order_df['OrderAmt'].sum())**0.5
    res = ((((order_df['factor'] - mean_f) / std_f)**3)* order_df['OrderAmt']).sum() / order_df['OrderAmt'].sum()
