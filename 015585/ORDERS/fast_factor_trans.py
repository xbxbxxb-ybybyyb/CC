...
def f_t_kind_930(order_df):
    return 93000000
def f_t_kind_1000(order_df):
    return 100000000
def f_t_kind_t1(order_df):
...
def f_t_kind_t2(order_df):
    ...
    return time_min * 100000
def f_t_kind_1430(order_df):
    return 143000000
def f_t_kind_1450(order_df):
    return 145000000
...
dic_time_kind = {
                 '930':f_t_kind_930,
                 # '1000':f_t_kind_1000,
                 # 't1':f_t_kind_t1,# 涨停挂单峰值时间
                 # 't2':f_t_kind_t2,# 挂单稳定时间
                 # '1430':f_t_kind_1430,
                 # '1450':f_t_kind_1450
                }
...
def write_excel(result_df):
    ...
    result_df.loc[factor_name,'repeat'] = factor_test.result_dic['max_same_ratio'].loc['ratio','first']
                            ...
                            for property_i in dic_property:
                                ...
                                for std_i in dic_std:
                                    ...
                                    for calc_i in dic_calc:
                                        if (property_i not in ['amt']) & (calc_i != 'nocalc'):
                                            continue#目前只有金额类因子为series可以使用calc
                                        if (property_i in ['amt']) & (calc_i == 'nocalc'):
                                            continue#金额类因子为series，必须calc
                                        for compare_i in dic_compare:
                                            ...
                                            factor_name = time_kind_i + '_' + time_type_i + '_'\
                                                          ...
                                                          + calc_i + '_'\
                                                          + compare_i
                                            if factor_name in list(pd.read_csv('/data/user/015585/01-因子挖掘/04-Sell/因子快速开发/汇总excel/'
                                                                + '快速开发因子_第一次测试' + '.csv')['factor_name']):
                                                print(factor_name)
                                                continue
                                            ...
                                            print(factor_name)
                                            factor_path = '/data/user/015585/01-因子挖掘/04-Sell/因子快速开发/h5/'
                                            ...
                                            df = pd.read_hdf(factor_path + factor_name + '_20160101_20181231.h5')
                                            result_path = '/data/user/015585/01-因子挖掘/04-Sell/因子快速开发/回测报告/'
...
for i in dic_time_kind.keys():
    str_time += (i+'_')
result_df.to_csv('/data/user/015585/01-因子挖掘/04-Sell/因子快速开发/汇总excel/'
                 + '快速开发因子_' + str_property + '_' + str_time + '_' + time_now + '.csv')