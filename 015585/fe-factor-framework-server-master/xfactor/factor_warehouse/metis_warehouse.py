...
def metis_warehouse():
    in_interval = settings.warehouse_settings_dict["metis"]["in_interval"]
    out_interval = settings.warehouse_settings_dict["metis"]["out_interval"]
    max_corr = settings.warehouse_settings_dict["metis"]["max_corr"]
    factor_data_path = settings.path_dict["metis"]["factor_value_path"]
    pre_check_path = settings.path_dict["metis"]["factor_precheck_path"]
    factor_test_path = settings.path_dict["metis"]["factor_test_path"]  # 因子报告
    res_path = settings.warehouse_settings_dict["metis"]["res_path"]
    res_public_path = settings.warehouse_settings_dict["metis"]["res_public_path"]  # 公共结果地址
    sft_basic_path = settings.warehouse_settings_dict["metis"]["sft_basic_path"]  # 初始地址
    jupiter_cutdate = settings.warehouse_settings_dict["metis"]["jupiter_cutdate"]
    all_factor_for_jupiter = pd.read_pickle(settings.warehouse_settings_dict["metis"]["all_factor_for_left"]).loc[
                             pd.Timestamp(str(in_interval[0])):pd.Timestamp(str(out_interval[1]))]

    # 读取jupiter因子列表
    df_dic = pd.read_excel(settings.warehouse_settings_dict["metis"]["jupiter_all_factors_list"], sheet_name=None)
    ...
    test_factor_inf = test_factor_inf[test_factor_inf['factor_type'].isin(
        ['T-1_factor', 'TTransaction', 'TEmotion', 'T-1_Emotion', 'TTickab', 'TOrder',
         'LastTouchTTick', 'TTransaction_TTickab', 'TTransaction_TOrder', 'TOrder_TTickab'])]
    test_factor_inf = test_factor_inf[test_factor_inf['factor_date'] < jupiter_cutdate]
    # 读取metis因子列表
    metis_inf = pd.read_excel(settings.warehouse_settings_dict["metis"]["all_factors_list"])
    test_factor_inf = pd.concat([test_factor_inf, metis_inf], sort=False)
    ...
    for test_date in test_date_list[start_index:]:
        logger.info("-" * 20 + "metis warehouse: test_date={}".format(test_date) + "-" * 20)
        ...
        if last_test_date is None:
            ...
            check_res_inf = pd.read_excel(res_path + 'check_res/check_res_tot_metis_{}.xlsx'.format(last_test_date))[
                check_res_cols]
        ...
        for index, inf in day_factor_inf.iterrows():
            ...
            # 预检测
            if int(test_date) < jupiter_cutdate:
                if factor_name in all_factor_for_jupiter.columns:
                ...
                if '预检测' in pre_check.index:
                    ...
                    logger.error("入库失败！预检测结果格式异常！factor_name={}, strategy=metis".format(factor_name))
                    continue
            ...
            # 因子值
            if int(test_date) < jupiter_cutdate:
                test_factor_df = all_factor_for_jupiter[[factor_name]]
            else:
                if factor_type not in ['T-1_Emotion', 'TEmotion']:
                    ...
                    test_factor_df = pd.read_pickle('{}/{}/{}.pkl'.format(factor_data_path, factor_name, factor_name)).loc[
                                     pd.Timestamp(start_date_new):pd.Timestamp(str(out_interval[1]))]
        ...
        check_res_inf_.to_excel(res_path + 'check_res/check_res_tot_metis_{}.xlsx'.format(test_date), index=False)
        last_test_date = test_date

        # 因子入库
        check_res_inf_test = check_res_inf_[(check_res_inf_['factor_date'] == test_date)]
        ...
        check_res_inf_sta['factor_owner'] = check_res_inf_sta['factor_owner'].replace(
            {'fc': '冯炽', 'qyh': '秦雨豪', 'skk': '孙康康', 'sss': '孙少森', 'wj': '王敬', 'xbc': '徐碧村', 'xly': '谢璐遥',
             'zwh': '张文虎'})
        check_res_inf_sta.to_excel(res_path + 'check_res/check_res_tot_metis_sta.xlsx'.format(test_date), index=False)

        # 输出结果
        update_file = res_path + 'sft_update_metis.h5'
        ...
        # 公共地址输出结果
        check_res_inf_.to_excel(res_public_path + 'check_res_tot_metis.xlsx', index=False)
        update_public_file = res_public_path + 'sft_update_metis.h5'
