...
def saturn_warehouse():
    s_xx = settings.saturn_s_xx
    in_interval = settings.warehouse_settings_dict["saturn"]["in_interval"]
    out_interval = settings.warehouse_settings_dict["saturn"]["out_interval"]
    max_corr = settings.warehouse_settings_dict["saturn"]["max_corr"]

    factor_data_path = settings.path_dict["saturn"]["factor_value_path"]
    pre_check_path = settings.path_dict["saturn"]["factor_precheck_path"]
    factor_test_path = settings.path_dict["saturn"]["factor_test_path"]

    res_path = settings.warehouse_settings_dict["saturn"]["res_path"]
    res_public_path = settings.warehouse_settings_dict["saturn"]["res_public_path"]
    sft_basic_path = settings.warehouse_settings_dict["saturn"]["sft_basic_path"]
    basic_factor_path = settings.warehouse_settings_dict["saturn"]["basic_factor_path"]

    # 读取因子列表
    test_factor_inf = pd.read_excel(settings.warehouse_settings_dict["saturn"]["all_factors_list"])
    ...
    for test_date in test_date_list[start_index:]:
        logger.info("-" * 20 + "saturn warehouse: test_date={}".format(test_date) + "-" * 20)
        ...
        if last_test_date is None:
            ...
            check_res_inf = pd.read_excel(res_path + 'check_res/check_res_tot_saturn_{}.xlsx'.format(last_test_date))[
                check_res_cols]
        ...
        for index, inf in day_factor_inf.iterrows():
            ...
            if '预检测' in pre_check.index:
                ...
                logger.error("入库失败！预检测结果格式异常！factor_name={}, strategy=saturn".format(factor_name))
                continue
        ...
        check_res_inf_.to_excel(res_path + 'check_res/check_res_tot_saturn_{}.xlsx'.format(test_date), index=False)
        ...
        check_res_inf_sta['factor_owner'] = check_res_inf_sta['factor_owner'].replace(
            {'fc': '冯炽', 'qyh': '秦雨豪', 'skk': '孙康康', 'sss': '孙少森', 'wj': '王敬', 'xbc': '徐碧村', 'xly': '谢璐遥',
             'zwh': '张文虎'})
        check_res_inf_sta.to_excel(res_path + 'check_res/check_res_tot_saturn_sta.xlsx'.format(test_date), index=False)
