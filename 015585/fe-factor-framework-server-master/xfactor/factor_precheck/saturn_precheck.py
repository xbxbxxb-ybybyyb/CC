...
def value_same_check(kls, result_path, filter_df, preload_database):
    ...
    logger.info('因子一致检查: factor_name={}, strategy={}'.format(factor_name, "saturn"))
    long_interval = settings.precheck_path_dict["saturn"]["long_interval"]
    short_interval_list = settings.precheck_path_dict["saturn"]["short_interval_list"]

    full_factor_value_path = os.path.join(settings.path_dict["saturn"]["factor_precheck_path"] + "/same_test/",
    ...
    try:
        ...
        return '函数运行出错: factor_name={}, strategy={}, 测试区间:{}-{}-{}'.format(
            factor_name, "saturn", long_interval[0], long_interval[1], e)
    ...
    try:
        ...
        for short_date in short_interval_list:
            if os.path.exists(result_path + "/{}_{}_{}.pkl".format(factor_name, short_date, short_date)):
                ...
                if not db:
                    logger.error("saturn precheck - preload database empty!")
                short_db = {"skip": False}
                for data_dict in kls.t_1_factor_data:
                    break_flag = True
                    counter = 0
            ...
            if (long_ser.shape != short_ser.shape) or ((long_ser - short_ser).abs().max() > 1e-6):
                logger.info(
                    '因子检查不一致: factor_name={}, date={}, strategy=saturn, long_shape={}, short_shape={}, diff={}, diff_stock={}'.format(
        ...
        if len(diff_short_date) > 0:
            return '区间测试未通过: factor_name={}, strategy={}, 不一致日期:{}'.format(factor_name, "saturn", str(diff_short_date))
    except Exception as e:
        return '函数运行出错: factor_name={}, strategy={}, 测试区间:{}-{}-{}'.format(factor_name, "saturn", short_date, short_date, e)
    return "pass"
...
def factor_value_test(factor_df, factor_name):
    ...
    if int(np.isinf(factor_df).sum()) > 0:
        logger.error('因子值存在inf: factor_name={}, strategy={}'.format(factor_name, "saturn"))
        res.append('因子值存在inf')
    if factor_df[factor_df.columns[0]].std() == 0:
        logger.error('因子值全部值为常数: factor_name={}, strategy={}'.format(factor_name, "saturn"))
        res.append('因子值全部值为常数')
    if factor_df.columns[0] != factor_name:
        logger.error('列名与因子名不符: factor_name={}, strategy={}'.format(factor_name, "saturn"))
        res.append('列名与因子名不符')
...
def score_check(factor_name, factor_value_df, filter_df):
    ...
    if factor_value_df.reindex(filter_df.index)[factor_name].std() < 1e-15:
        return '因子值波动小: factor_name={}, strategy={}'.format(factor_name, "saturn")

    if factor_value_df.reindex(filter_df.index)[factor_name].value_counts(normalize=True).max() > 0.2:
        return '相同值比例较高: factor_name={}, strategy={}'.format(factor_name, "saturn") + ', %.2f' % (factor_value_df.reindex(filter_df.index)[factor_name].value_counts(normalize=True).max())

    return 'pass'


def saturn_precheck_aftercalc(kls, pre_calc_check_res, error_factors, start_date, end_date, factor_type, result_path, preload_database):
    logger.info("Start saturn factor precheck (after calc):  factor_name={}, factor_owner={}"
    ...
    if kls.factor_name in error_factors:
        ...
        pre_check_res.to_pickle(settings.path_dict["saturn"]["factor_precheck_path"] + '{}.pkl'.format(kls.factor_name))
        return

    factor_value_path = os.path.join(settings.path_dict["saturn"]["factor_value_path"], kls.factor_name, kls.factor_name + ".h5")
    if os.path.exists(factor_value_path):
        factor_value = pd.read_hdf(factor_value_path)
    else:
        logger.error("未找到相关因子值文件！ factor_name={}, strategy=saturn, value_path={}".format(kls.factor_name, factor_value_path))
        check_dic["函数运行检查"] = "未找到相关因子值文件"
        pre_check_res = pd.Series(check_dic)
        pre_check_res.to_pickle(settings.path_dict["saturn"]["factor_precheck_path"] + '{}.pkl'.format(kls.factor_name))
        return

    filter_df = pd.read_hdf(settings.precheck_path_dict["saturn"]["precheck_basic_path"])
    ...
    if check_dic['函数运行检查'] != 'pass':
        logger.error("函数运行检查未通过！ factor_name={}, strategy={}".format(kls.factor_name, "saturn"))

    check_dic['因子测试'] = score_check(kls.factor_name, factor_value, filter_df_cpy)

    if check_dic['因子测试'] != 'pass':
        logger.error("因子测试未通过！ factor_name={}, strategy={}".format(kls.factor_name, "saturn"))

    if check_dic["函数运行检查"] == 'pass' and check_dic["代码格式检查"] == 'pass' and check_dic["因子值一致性检查"] == "pass" and check_dic["因子测试"] == "pass":
        check_dic["预检测"] = 'pass'
    else:
        logger.error("预检测未通过！ factor_name={}, strategy={}".format(kls.factor_name, "saturn"))
    pre_check_res = pd.Series(check_dic)
    pre_check_res.to_pickle(settings.path_dict["saturn"]["factor_precheck_path"] + '{}.pkl'.format(kls.factor_name))
    logger.info("预检测完成！ factor_name={}, strategy={}".format(kls.factor_name, "saturn"))
    return
