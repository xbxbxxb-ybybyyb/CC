...
from xfactor.factor_precheck import jupiter_precheck, europa_precheck, sell_precheck, saturn_precheck, mimas_precheck, metis_precheck, neptune_precheck, neptunelong_precheck
...
def pre_calc_check(factor_kls_list, prev_factor_list):
    pre_calc_check_res_dict = {}
    for factor_dict in factor_kls_list:
        factor = factor_dict["factor_class"]
        module_path = factor_dict["module_path"]
        res = format_check(factor, module_path, prev_factor_list)
        pre_calc_check_res_dict[factor.factor_name] = res
    return pre_calc_check_res_dict

def run_precheck(strategy, pre_calc_check_res, error_factors, kls, start_date, end_date, factor_type, output_dir, preload_database):

    if strategy in func_map:
        errors = []
        if strategy in error_factors:
            errors = error_factors[strategy]
        func_map[strategy](kls, pre_calc_check_res, errors, start_date, end_date, factor_type,
                                                    output_dir, preload_database)
    else:
        logger.error("run precheck failed, startegy not known! input_strategy=" + strategy)
    return


def format_check(kls, module_path, before_submit_name_list):
    ...
    try:
        module_li = module_path.split("/")
        module_li.append('factor_%s' % (factor_name))
        modname = '.'.join(module_li)
    ...
    try:
        ...
        f = open(os.getcwd() + '/' + module_path + '/factor_%s.py' % (factor_name))
        py_code = f.readlines()
        ...
        if (len(set(dic_use_data) - set(real_use_data)) > 0) or (len(set(real_use_data) - set(dic_use_data)) > 0):
            ...
            return 't_1_factor_data_types列举错误'

    except Exception as e:
        return '检查t_1_factor_data_types列举时出现异常'
