...
def value_same_check(kls, result_path, filter_df, preload_database):
    ...
    full_factor_value_path = os.path.join(settings.path_dict["europa"]["factor_precheck_path"] + "/same_test/",
    ...
    try:
        if os.path.exists(full_factor_value_path):
            ...
            for data_dict in kls.t_1_factor_data:
                ...
                while break_flag:
                    ...
                    try:
                        start_date_new = \
                        precheck_utils.FactorUtil.factor_data.tradingday(long_interval[0], -int(data_dict["lag"]))[0]
                        break_flag = False
                    except Exception as e:
                        logger.warning("tradingday接口调用失败！重试...")
                        counter += 1
                long_db[data_dict["name"]] = db[data_dict["name"]].loc[
                                              pd.Timestamp(str(start_date_new)):pd.Timestamp(str(long_interval[1]))].copy()
        ...
        logger.error('函数测试出错: factor={}, strategy={}, 测试区间:{}-{}-{}'.format(factor_name, "europa", long_interval[0], long_interval[1], e))
        return '函数测试出错: factor={}, strategy={}, 测试区间:{}-{}-{}'.format(factor_name, "europa", long_interval[0], long_interval[1], e)
    ...
    try:
        for short_date in short_interval_list:
            if os.path.exists('{}{}_{}_{}.pkl'.format(result_path, factor_name, short_date, short_date)):
                ...
                if not db:
                    logger.error("europa precheck - preload database empty!")
                short_db = {"skip": False}
                for data_dict in kls.t_1_factor_data:
                    break_flag = True
                    counter = 0
                    ...
                    while break_flag:
                        ...
                        try:
                            start_date_new = precheck_utils.FactorUtil.factor_data.tradingday(short_date, -int(data_dict["lag"]))[0]
                            break_flag = False
                        except Exception as e:
                            logger.warning("tradingday接口调用失败！重试...")
                            counter += 1
            ...
            if np.nanmax((short_result - tmp_long_df).abs().values) > 1e-6:
                logger.error('因子值不一致1: factor={}, strategy={}, 计算区间:{}-{}和{}-{}'.format(factor_name, "europa", long_interval[0], long_interval[1], short_date, short_date))
                ...
                return '因子值不一致1: factor={}, strategy={}, 计算区间:{}-{}和{}-{}'.format(factor_name, "europa", long_interval[0], long_interval[1], short_date, short_date)
            if np.nanmax((short_result.reindex(basic_index).fillna(fill_na_value) - tmp_long_df.reindex(basic_index).fillna(
                ...
                return '因子值不一致2: factor={}, strategy={}, 计算区间:{}-{}和{}-{}'.format(factor_name, "europa", long_interval[0], long_interval[1], short_date, short_date)
    except Exception as e:
        logger.error('函数测试出错: factor={}, strategy={}, 测试区间:{}-{}-{}'.format(factor_name, "europa", short_date, short_date, e))
...
def score_check(factor_name, factor_value_df, filter_df):
    ...
    try:
        ...
        if (factor_mean < 0.001) and (factor_std < 0.001):
            return '因子波动小: factor_name={}, strategy={}'.format(factor_name, "europa") + ', std:%.8f,mean:%.8f' % (factor_mean, factor_std)
...
def europa_precheck_aftercalc(kls, pre_calc_check_res, error_factors, start_date, end_date, factor_type, result_path, preload_database):
    ...
    if kls.factor_name in error_factors:
        ...
        pre_check_res.to_pickle(settings.path_dict["europa"]["factor_precheck_path"] + '{}.pkl'.format(kls.factor_name))
        return

    factor_value_path = os.path.join(settings.path_dict["europa"]["factor_value_path"], kls.factor_name, kls.factor_name + ".h5")
    if os.path.exists(factor_value_path):
        factor_value = pd.read_hdf(factor_value_path)
    else:
        logger.error("未找到相关因子值文件！ factor_name={}, strategy=europa, value_path={}".format(kls.factor_name, factor_value_path))
