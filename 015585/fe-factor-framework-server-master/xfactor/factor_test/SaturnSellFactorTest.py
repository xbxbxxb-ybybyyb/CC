...
class FactorTest:
    def __init__(self, start_date, end_date, segment_number=20,
        ...
        elif self.strategy_name == 'saturn':
            self.return_type = 'label_v2o10d1' # TODO 待少森确认
    ...
    def load_data(self):
        ...
        if self.strategy_name == 'sell':
            sft_basic_path = '/data/user/018107/factor_zooS/factor_lib_v2/931/sft_update_filter_20160101_20201231.h5'
            df = IO.read_data([self.start_date, self.end_date], alt=sft_basic_path)
        elif self.strategy_name == 'saturn':
            sft_basic_path = '/dfs/user/018107/factor_zoo2/factor_lib/sft_update_filter_20160101_20201231.pkl'
            df = pd.read_pickle(sft_basic_path).loc[pd.to_datetime(str(int(self.start_date))):pd.to_datetime(str(int(self.end_date)))]
        else:
            logger.error('strategy_name error, must in [sell,saturn]')
            raise TypeError
