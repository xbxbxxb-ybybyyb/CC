...
class FactorTest:
    def __init__(self, start_date, end_date,
        ...
        # self.t_day_data = t_day_data
        if self.strategy_name == 'neptune':
            # if not self.t_day_data:
            self.return_type = 'label_s1_short' # 短周期评价标签
            # else:
            #     self.return_type = 'label_t2o9d1_pos' # 长周期评价标签
    ...
    def load_data(self):
        ...
        if self.strategy_name == 'neptune':
            sft_basic_path = '/data/user/023859/factor_zooZZ/factor_lib/sft_update_931_20160101_20211231.pkl'  # 这个文件里有label和所有因子
        ...
        all_factors_available = ['label_s1_short','label_s1_mid','label_s1_long','Circu_Mkt','STPT','list_len']
    ...
    def factor_test(self, factor_df, cost_list, result_path, factor_corr_test=False, generate_pdf=False):
        ...
        if factor_corr_test:
            if self.strategy_name == 'neptune':
                ...
                # pass  # 2025-03:暂没有neptune的历史因子分数
    ...
    def get_corr_with_all_factor(self, group_num=5):
        ...
        corr_res['factor_corr'] = self.all_factor.rank().corrwith(self.factor_df['factor'].rank())
    ...
    # def get_corr_with_all_factor(self, group_num=5):
    #     corr_res = pd.DataFrame()
    #     other_factors = self.all_factor.columns
    #     factor_join = self.all_factor.join(self.factor_df)
    #     corr_res['factor_corr'] = (
    #         factor_join.groupby('dt').apply(lambda x: x[other_factors].rank().corrwith(x['factor'].rank()))).mean()
    #     corr_res['factor_corr_abs'] = corr_res['factor_corr'].abs()
    #     corr_res = corr_res.sort_values(by='factor_corr_abs', ascending=False)
    #     return corr_res[['factor_corr']].abs()