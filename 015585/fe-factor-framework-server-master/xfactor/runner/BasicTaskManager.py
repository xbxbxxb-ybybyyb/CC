...
class BasicTaskManager(object):
    ...
    def __init__(self, full_factor_name_list, start_date, end_date, strategy):
        self.full_class_list = FactorUtil.get_factor_class_list(full_factor_name_list)
    ...
    def generate_task(self, filtered_factor_class_list):
        factor_type_groups = FactorUtil.split_calc_factor_into_group(self.strategy, filtered_factor_class_list)
        ...
        for data_dict in task_data_info_all["t_1_factor_data_info"].values():
            if data_dict["column"] is None or len(data_dict["column"]) == 0:
                col = None
            else:
                col = list(data_dict["column"])
            cur_data = {
                "name": data_dict["name"],
                "path": data_dict["path"],
                "lag": data_dict["lag"],
        ...
        if 'MarketIndTTick' in task_data_info_all['t_day_data_info']:
            max_xdb_lag = FactorUtil.get_max_xdb_lag(filtered_factor_class_list)
        ...
        if factor_type_groups["t_day_factor"] or factor_type_groups["combined_t_1_factor"]:
            factor_class_li = []
            cmb = factor_type_groups["t_day_factor"] + factor_type_groups["combined_t_1_factor"]
            for item in factor_type_groups["xdb_tick"] + factor_type_groups["non_xdb_tick"]:
                if item["factor"] in cmb:
                    factor_class_li.append(item["factor"])

            for date in self.calc_days:
                task = {
                    "strategy": "",
                    "factor_class_list": factor_class_li,
            ...
            for data_dict in factor.t_1_factor_data:
                ...
                if data_name in t_1_factor_data_dict:
                    t_1_factor_data_dict[data_name]["lag"] = max(t_1_factor_data_dict[data_name]["lag"],
                                                                 data_dict["lag"])
                    if t_1_factor_data_dict[data_name]["column"] is not None:
                        if data_dict["column"] is None or len(data_dict["column"]) == 0:
                            t_1_factor_data_dict[data_name]["column"] = None
                        else:
