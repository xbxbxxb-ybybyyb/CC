...
import numpy as np
import pandas as pd
# 逻辑：T日09:31数据，主动买入的大单金额占比(总体选择主买部分)
# score:15，7，0.09
# wd_t1_big_bid_vol_pct
factor_name = 'qyh_T1mtra_ratio_buy_big2small_1'
def factor_qyh_T1mtra_ratio_buy_big2small_1(transaction_df, return_fillna_dic=False):
    ...
    total_amt = buy_amt.sum()
    # ratio
    if abs(total_amt) >= 0.001:
        ratio = (buy_amt[buy_amt >= 200000].sum() - buy_amt[buy_amt <= 50000].sum()) / total_amt
    else:
        ratio = np.nan
