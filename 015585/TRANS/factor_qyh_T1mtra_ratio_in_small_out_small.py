...
import numpy as np
import pandas as pd
# 逻辑：T日09:31数据 + 集合竞价，(流入部分小单 且 流出部分小单)金额占比
# score:GG
factor_name = 'qyh_T1mtra_ratio_in_small_out_small'#del big out
def factor_qyh_T1mtra_ratio_in_small_out_small(transaction_df, return_fillna_dic=False):
    ...
    sell_no_small = sell_no[sell_no < 50000]
    buy_no_big_ob = transaction_df[(transaction_df['TradeBuyNo'].isin(buy_no_small.index)) &
                                    (transaction_df['TradeSellNo'].isin(sell_no_small.index))]['TradeMoney'].sum()
    # ratio
