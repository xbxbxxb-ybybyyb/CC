...
import numpy as np
import pandas as pd
# 逻辑：T日09:31数据(包括集合竞价）,买单的每单标准差（除以均值） - 卖单平均每单（非每笔）金额的标准差（除以均值），相当于综合买方和卖方的复杂度
# score:6,3,-0.01
factor_name = 'qyh_T1mtra_amtpertra_inout_s'
def factor_qyh_T1mtra_amtpertra_inout_s(transaction_df, return_fillna_dic=False):
