...
md_data_dual = IO.read_data([start_date_, end_date], columns=['open', 'high','low','close','adjfactor']
                       , alt='/data/group/800080/warehouse/prod/MD/CHINA_STOCK/DAILY/WIND/MD_CHINA_STOCK_DAILY_WIND.h5')
md_data_dual['hh'] = (md_data_dual['high'] * md_data_dual['adjfactor']).unstack().rolling(N,1).max().shift(1).stack() / md_data_dual['adjfactor']
md_data_dual['lc'] = (md_data_dual['close'] * md_data_dual['adjfactor']).unstack().rolling(N,1).min().shift(1).stack() / md_data_dual['adjfactor']
md_data_dual['hc'] = (md_data_dual['close'] * md_data_dual['adjfactor']).unstack().rolling(N,1).max().shift(1).stack() / md_data_dual['adjfactor']
md_data_dual['ll'] = (md_data_dual['low'] * md_data_dual['adjfactor']).unstack().rolling(N,1).min().shift(1).stack() / md_data_dual['adjfactor']
md_data_dual['range1'] = (md_data_dual['hh'] - md_data_dual['lc'])
md_data_dual['range2'] = (md_data_dual['hc'] - md_data_dual['ll'])
md_data_dual['range'] = md_data_dual[['range1', 'range2']].max(axis=1)
# md_data_dual['buyline'] = md_data_dual['open'] + K1 * md_data_dual['range']
...
def get_trigger(index):
    ...
    try:
        ...
        #Todo:计算触发时间
        range = md_data_dual.loc[index,'range']
        lowpx = tick_df['LastPx'].cummin()
        if not zcz:
            tick_df['trigger1'] = (tick_df['LastPx'] > lowpx + K1 * range) & (tick_df['LastPx'] > pre_close * 0.97)
        else:
            tick_df['trigger1'] = (tick_df['LastPx'] > lowpx + K1 * range) & (tick_df['LastPx'] > pre_close * 0.94)
