...
for index in sample.index:
    ...
    ## trigger 1 新低 + MACD背离
    '''
	对每个9：40之后的时刻T，T时刻股价小于等于930-T时刻的最低价 且 T时刻股价低于T时刻1分钟前的股价跌去0.1%，则观察T时刻的MACD，如果高于T时刻之前的MACD最小值+MACD的0.5倍标准差，作为买点
    '''
    tick_df['new_min1'] = (tick_df['LastPx'] <= tick_df['LastPx'].cummin())
    tick_df['new_min2'] = (tick_df['LastPx'] <= tick_df['LastPx'].shift(20) * 0.999)
    tick_df['macd_real_min'] = tick_df['macd_real'].cummin()
    tick_df['macd_real_depart'] = (tick_df['macd_real'] > ( tick_df['macd_real_min'] + tick_df['macd_real_std'] * 0.5))
    tick_df['trigger1'] = (tick_df['new_min1']) & (tick_df['new_min2']) & (tick_df['macd_real_depart'])
    # tmp = tick_df[(tick_df['MDTime'] >= 94000000)][['MDTime', 'LastPx', 'diff_real', 'dea_real', 'macd_real','macd_real_min','macd_real_std','new_min1','new_min2','trigger1']]
    trigger_time1 = tick_df[(tick_df['trigger1']==True) & (tick_df['MDTime'] >= 94000000)]['MDTime'].min()
    ...
    trigger_time2 = tick_df[(tick_df['trigger2']==True) & (tick_df['MDTime'] >= 94000000)]['MDTime'].min()
    ## trigger 3 :新低 + DIF背离
    '''
    同理，用DEA代替MACD的效用
    '''
    tick_df['new_min1'] = (tick_df['LastPx'] <= tick_df['LastPx'].cummin())
    tick_df['new_min2'] = (tick_df['LastPx'] <= tick_df['LastPx'].shift(20) * 0.999)
    tick_df['diff_real_min'] = tick_df['diff_real'].cummin()
    tick_df['diff_real_depart'] = (tick_df['diff_real'] > ( tick_df['diff_real_min'] + tick_df['diff_real_std'] * 0.5))
    tick_df['trigger3'] = (tick_df['new_min1']) & (tick_df['new_min2']) & (tick_df['diff_real_depart'])
    # tmp = tick_df[(tick_df['MDTime'] >= 94000000)][['MDTime', 'LastPx', 'diff_real', 'dea_real', 'macd_real','macd_real_min','macd_real_std','new_min1','new_min2','trigger1']]
    trigger_time3 = tick_df[(tick_df['trigger3']==True) & (tick_df['MDTime'] >= 94000000)]['MDTime'].min()
    ## trigger 4：平开后下跌3% + 瞬时反弹 + 940以前
    '''
    平开（-3%到+3%）：931以后，距离开盘价跌幅到3%，且15S涨跌幅>=0，且在940之前。如果没有出现该情况则不新增
    '''
    tick_df['pct'] = tick_df['LastPx'] / tick_df['PreClosePx']-1
    ratio1 = 0.03
    ratio1= 2 * ratio1 if zcz else ratio1 # 平开范围
    ratio2 = 0.03
    ratio2 = 2 * ratio2 if zcz else ratio2 # 下杀幅度
    if abs(tick_df['pct'].iloc[0]) <= ratio1:
        limit = tick_df['pct'].iloc[0] - ratio2
        tick_df['pct_short'] = (tick_df['LastPx'] - tick_df['LastPx'].shift(5)) / tick_df['PreClosePx']
        tick_df['trigger4'] = (tick_df['pct'] <= limit) & (tick_df['MDTime'] <= 94000000) & (tick_df['pct_short'] > 0)
        trigger_time4 = tick_df[(tick_df['trigger4']==True)]['MDTime'].min()
    else:
        trigger_time4 = np.nan
    #计算未来十分钟twap
    sample.loc[index, 'trigger_time'] = trigger_time
    ...
    if len(tick_buy) > 0:
        sample.loc[index, 'T_trigger_10_twap_before_ZT'] = tick_buy['LastPx'].mean()
    ...
    tick_buy = tick_buy[tick_buy['LastPx'].cummin() > dl_price]
    if len(tick_buy) > 0:
        sample.loc[index, 'T_s1_10_twap_before_ZT'] = tick_buy['LastPx'].mean()
