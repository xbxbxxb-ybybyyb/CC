...
'''
1、931判断，最新挂买-挂卖 / （挂买 + 挂卖）>=-0.17（约66%分位数）， 且 最新涨跌幅 >= -3%，则931买入
2、否则，对价格序列进行小波变换，剔除噪音后，在RSI(6)背离时、MACD背离时买入，取两者的首个大于931的时间点
背离的窗口改为1h以内，叠加买入价格低于931
'''
s = FactorData()
...
df = pd.read_pickle('/data/user/018107/share_file/for_qyh/basic_p4_20160101_20191231.pkl')
...
def get_trigger(index):
    ...
    try:
        ...
        label_T_first_ul_time = sample.loc[index,'label_T_first_ul_time']
        label_T_first_dl_time = sample.loc[index,'label_T_first_dl_time']
        ...
        tick_df = pd.read_pickle(f'/dfs/user/015585/20241210-P4基础数据/tick_allday/{date}.pkl').reset_index()
        tick_df = tick_df[tick_df['Ticker'] == Ticker]
        ...
        # transaction_df = mdp.get_data_by_date("Transaction", Ticker, date) # qyh:暂时用不到trade
        #
        # min_price = tick_df[tick_df['MDTime'] <= 93100000]['LastPx'].min()
        price931 = tick_df[tick_df['MDTime'] <= 93100000]['LastPx'].tail(1).mean() if len(tick_df[tick_df['MDTime'] <= 93100000]) > 0 else np.nan # 931最新价
        buy_amt931 = tick_df[tick_df['MDTime'] <= 93100000]['TotalBidQty'].tail(1).mean() * \
                     tick_df[tick_df['MDTime'] <= 93100000]['WeightedAvgBidPx'].tail(1).mean()
        sell_amt931 = tick_df[tick_df['MDTime'] <= 93100000]['TotalOfferQty'].tail(1).mean() * \
                     tick_df[tick_df['MDTime'] <= 93100000]['WeightedAvgOfferPx'].tail(1).mean()
        rcleanb = (buy_amt931 - sell_amt931) / (buy_amt931 + sell_amt931 + 1e-5)
        if rcleanb >= -0.17 and price931 > (pre_close * 0.94) and buy_amt931 > 1 and sell_amt931 > 1 and zcz:
            trigger_time = 93100000
        elif rcleanb >= -0.17 and price931 > (pre_close * 0.97) and buy_amt931 > 1 and sell_amt931 > 1 and (not zcz):
            trigger_time = 93100000
        else:
            def cumstd(series):
            ...
            def get_pywt(noisy_signal):
                coeffs = pywt.wavedec(noisy_signal, wavelet, level=level, mode='symmetric')
            ...
            price_931_new = tick_df[tick_df['MDTime'] <= 93100000]['LastPx_new'].tail(1).mean() if len(tick_df[tick_df['MDTime'] <= 93100000]) > 0 else np.nan # 931小波分析后的最新价
            # RSI背离
            ...
            tick_df_1min['diff'] = tick_df_1min['LastPx_new'].shift(1).diff().fillna(0) # 前一分钟的DIFF，避免未来信息
            ...
            for j in tick_df.index:
                ...
                tick_df_before = tick_df[tick_df['MDTime_min'] < MDTime_min] # MDTime_min代表当前所属的分钟，“小于”保障了不会用到未来信息
            ...
            tick_df['LastPx_new_cummin'] = tick_df['LastPx_new'].rolling(1200,1).min()
            tick_df['new_min1'] = (tick_df['LastPx_new'] <= tick_df['LastPx_new_cummin']) & (tick_df['LastPx_new'] <= price_931_new)# portfolio6 只取最近1h价格作为背离条件之一
            tick_df['new_min2'] = (tick_df['LastPx_new'] <= tick_df['LastPx_new'].shift(20) * 0.999)
            tick_df['rsi_real_min'] = tick_df['rsi_real'].rolling(1200,1).min()
            ...
            if zcz:
                trigger_time1 = tick_df[(tick_df['trigger1']==True) & (tick_df['MDTime'] > 93100000) & (tick_df['LastPx_new'] >= pre_close*0.94)]['MDTime'].min()
            else:
                trigger_time1 = tick_df[(tick_df['trigger1'] == True) & (tick_df['MDTime'] > 93100000) & (
            ...
            tick_df['new_min1'] = (tick_df['LastPx_new'] <= tick_df['LastPx_new_cummin']) & (tick_df['LastPx_new'] <= price_931_new)# portfolio6 只取最近1h价格作为背离条件之一
            tick_df['new_min2'] = (tick_df['LastPx_new'] <= tick_df['LastPx_new'].shift(20) * 0.999)
            tick_df['macd_real_min'] = tick_df['macd_real'].rolling(1200,1).min()
            ...
            if zcz:
                trigger_time2 = tick_df[(tick_df['trigger1'] == True) & (tick_df['MDTime'] > 93100000) &
        ...
        if not np.isnan(trigger_time):
            ...
            if not np.isnan(label_T_first_ul_time):
                tick_buy = tick_buy[tick_buy['MDTime'] < label_T_first_ul_time]
            if not np.isnan(label_T_first_dl_time):
                tick_buy = tick_buy[tick_buy['MDTime'] < label_T_first_dl_time]
            if len(tick_buy) > 0:
        ...
        return
factor_df_list = Parallel(n_jobs=1)(delayed(get_trigger)(index) for index in sample.index)
...
sample['label_TN_v2o10d1']=sample['next_vwap']*sample['next_adjfactor']/sample['T_s1_10_twap_before_ZT']/sample['adjfactor']-1
...
zt_dt_filter1 = (sample['label_T_first_dl_time'].fillna(150000000)>93100000) & (sample['label_T_first_ul_time'].fillna(150000000)>93100000)
zt_dt_filter2 = (sample['label_T_first_dl_time'].fillna(150000000)>sample['trigger_time']) & (sample['label_T_first_ul_time'].fillna(150000000)>sample['trigger_time'])
# can_buy_filter = sample['T_first_trans_ZT'] != 1
# base_filter = st_filter & open_filter & after_not_ul_len_filter & can_buy_filter

sample_filter931 = sample[base_filter & zt_dt_filter1 & (~sample['label_TN_v2o10d1'].isna())]
sample_filter_t = sample[base_filter & zt_dt_filter2 & (~sample['label_v2t10'].isna())]
print('筛选样本：',len(sample_filter931),len(sample_filter_t))
print('基准label:', sample_filter931['label_TN_v2o10d1'].mean())
print('新时点label:', sample_filter_t['label_v2t10'].mean())
print(error_list)
sample.to_pickle('/data/user/015585/01-因子挖掘/20240812-Ceres新增买入时点/file_p4/test_trigger_p4_portfolio6_7.pkl')
