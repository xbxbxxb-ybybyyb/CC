...
def get_trigger(index):
    ...
    try:
        ...
        tick_df['trigger1'] = (tick_df['rsi_real'] > 0.2) \
                              * (tick_df['rsi_real'].shift(20) < 0.2) \
                              * (tick_df['LastPx'] > tick_df['LastPx'].rolling(5,1).mean())
        trigger_time = tick_df[(tick_df['trigger1']==True) & (tick_df['MDTime'] >= 94000000)]['MDTime'].min()
...
sample.to_pickle('/data/user/015585/01-因子挖掘/20240812-Ceres新增买入时点/file/test_trigger5_2_4.pkl')
