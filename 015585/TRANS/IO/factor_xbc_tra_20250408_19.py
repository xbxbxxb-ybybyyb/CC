...
class factor_xbc_tra_20250408_19(BaseFactor):
    strategy_name = "hotspot"
    factor_name = "xbc_tra_20250408_19"
    fill_na_value =  75
    need_pre_calculate_T_N = False
    owner = "xbc"  # 开发人员姓名
    factor_explain = "factor_xbc_20230921_14.py" # 因子逻辑解释
    ...
    def calculate(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            if transaction_df.shape[0]>0:
                transaction_df = transaction_df[transaction_df['MDTime'] >= fun_get_time(int(transaction_df.iloc[-1]['MDTime']), -10)]

                if transaction_df.shape[0]>2:
                    big_money = round_(np.percentile(transaction_df['TradeMoney'],50),6)
                    small_money = round_(np.percentile(transaction_df['TradeMoney'],50),6)
                else:
                    big_money=30000
                    small_money=30000
                ...
                score = (transaction_df_S1['TradeMoney'].sum()/(transaction_df_S2['TradeMoney'].sum()+1) *transaction_df_B1['TradeMoney'].sum())/(transaction_df_B2['TradeMoney'].sum()+1)
                if score > 1000:
                    score = 1000
            else:
                score = np.nan
