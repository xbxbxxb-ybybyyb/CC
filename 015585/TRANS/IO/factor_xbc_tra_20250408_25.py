...
class factor_xbc_tra_20250408_25(BaseFactor):
    strategy_name = "hotspot"
    factor_name = "xbc_tra_20250408_25"
    fill_na_value =  22
    need_pre_calculate_T_N = False
    owner = "xbc"  # 开发人员姓名
    factor_explain = "factor_xbc_20231019_6.py" # 因子逻辑解释
    ...
    def calculate(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            if transaction_df.shape[0] > 10:
                ...
                bar_value = np.percentile(transaction_df_near['dir_money'].abs(),95)
                value = transaction_df_near['dir_money'].sum()/bar_value
                if bar_value==0:
                    value=nan_value
            else:
                value = nan_value
