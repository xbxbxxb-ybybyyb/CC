...
def get_trigger(index):
    ...
    try:
        ...
        # max_price = tick_df[tick_df['MDTime'] <= 93100000]['LastPx'].max()
        ...
        if new_price > open_price and new_price > (pre_close * 0.98):
            trigger_time = 93100000
        else:
            tick_df['min_price'] = tick_df['LastPx'].cummin()
            tick_df['trigger1'] = tick_df['min_price'] <= open_price * 0.97
            tick_df['trigger2'] = tick_df['LastPx'] > open_price
            trigger_time = tick_df[(tick_df['trigger1'] == True) & (tick_df['trigger2'] == True) & (tick_df['MDTime'] >= 93100000)]['MDTime'].min()
        #计算未来十分钟twap
...
sample.to_pickle('/data/user/015585/01-因子挖掘/20240812-Ceres新增买入时点/file_CTA/test_trigger_calculate_3.pkl')
