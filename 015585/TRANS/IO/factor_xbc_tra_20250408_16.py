...
class factor_xbc_tra_20250408_16(BaseFactor):
    strategy_name = "hotspot"
    factor_name = "xbc_tra_20250408_16"
    fill_na_value =  0.05
    need_pre_calculate_T_N = False
    owner = "xbc"  # 开发人员姓名
    factor_explain = "factor_xbc_up_down_ratio_money_diff.py" # 因子逻辑解释
    ...
    def calculate(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            if transaction_df.shape[0] > 130:
                ...
                value_near = np.sum(transaction_df_near['up_down_ratio']*transaction_df_near['TradeMoney'])/transaction_df_near['TradeMoney'].sum()
                value_far = np.sum(transaction_df_far['up_down_ratio']*transaction_df_far['TradeMoney'])/transaction_df_far['TradeMoney'].sum()
                value = value_near-value_far
            else:
                value = nan_value
