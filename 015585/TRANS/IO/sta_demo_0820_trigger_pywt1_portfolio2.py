        ...
        if new_price > open_price and new_price > (pre_close * 0.96) and zcz:
            ...
            if zcz:
                trigger_time2 = tick_df[(tick_df['trigger1'] == True) & (tick_df['MDTime'] >= 93200000) &
                                       (tick_df['LastPx_new'] >= pre_close * 0.94) & (tick_df['MDTime'] <= 143000000)]['MDTime'].min()
            else:
                trigger_time2 = tick_df[(tick_df['trigger1'] == True) & (tick_df['MDTime'] >= 93200000) &
                                       (tick_df['LastPx_new'] >= pre_close * 0.97) & (tick_df['MDTime'] <= 143000000)]['MDTime'].min()
            ...
            trigger_time = 143000000 if trigger_time == 150000000 else trigger_time # 强制1430买入
        #计算未来十分钟twap
...
sample.to_pickle('/data/user/015585/01-因子挖掘/20240812-Ceres新增买入时点/file/test_trigger_portfolio2.pkl')
