...
def get_trigger(index):
    ...
    try:
        ...
        tick_df['rsi_real_depart'] = (tick_df['rsi_real'] > ( tick_df['rsi_real_min'] + tick_df['rsi_real_std'] * 0.5))
        tick_df['trigger1'] = (tick_df['new_min1']) & (tick_df['new_min2']) & (tick_df['macd_real_depart'])
        if zcz:
            trigger_time = tick_df[(tick_df['trigger1']==True) & (tick_df['MDTime'] >= 94000000) & (tick_df['LastPx'] >= pre_close*0.94)]['MDTime'].min()
        else:
            trigger_time = tick_df[(tick_df['trigger1'] == True) & (tick_df['MDTime'] >= 94000000) & (
...
sample.to_pickle('/data/user/015585/01-因子挖掘/20240812-Ceres新增买入时点/file/test_trigger5_3_1.pkl')
