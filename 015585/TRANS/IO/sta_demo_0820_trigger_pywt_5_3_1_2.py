...
def get_trigger(index):
    ...
    try:
        ...
        ## 数据准备：计算RSI(6)
        period = 6
        tick_df['LastPx_new'] = tick_df['LastPx'].rolling(5,1).mean()
        ...
        if zcz:
            trigger_time = tick_df[(tick_df['trigger1']==True) & (tick_df['MDTime'] >= 93200000) & (tick_df['LastPx_new'] >= pre_close*0.94)]['MDTime'].min()
        else:
