...
md_data_rb['p1'] = ((md_data_rb['high'] + 5 * K * (md_data_rb['vwap'] - md_data_rb['low'])) * md_data_rb['adjfactor']).unstack().shift(1).stack() / md_data_rb['adjfactor']
md_data_rb['p2'] = ((md_data_rb['vwap'] - 5 * K * (md_data_rb['high'] - md_data_rb['vwap'])) * md_data_rb['adjfactor']).unstack().shift(1).stack() / md_data_rb['adjfactor']
md_data_rb['p3'] = (((1 + K) / 2 * (md_data_rb['high'] + md_data_rb['vwap']) - K * md_data_rb['vwap']) * md_data_rb['adjfactor']).unstack().shift(1).stack() / md_data_rb['adjfactor']
md_data_rb['p4'] = (((1 + K) / 2 * (md_data_rb['high'] + md_data_rb['vwap']) - K * md_data_rb['high']) * md_data_rb['adjfactor']).unstack().shift(1).stack() / md_data_rb['adjfactor']
md_data_rb['p5'] = md_data_rb['p1'] + 0.25 * (md_data_rb['p1'] - md_data_rb['p2'])
md_data_rb['p6'] = md_data_rb['p2'] - 0.25 * (md_data_rb['p1'] - md_data_rb['p2'])
...
def get_trigger(index):
    ...
    try:
        ...
        #Todo:计算触发时间
        p2 = md_data_rb.loc[index,'p2']
        p4 = md_data_rb.loc[index,'p4']
        tick_df['trigger1'] = (tick_df['LastPx'] > p4) & (tick_df['LastPx'].cummin() < p2) & (tick_df['LastPx'] > (pre_close * 0.97))
