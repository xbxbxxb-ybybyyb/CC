...
# RSI(6)背离 + 日内最高价相对于开盘涨幅<+5% + 放宽阈值 + 较5分钟均价下跌1%
s = FactorData()
...
def get_trigger(index):
    ...
    try:
        ...
        period = 6
        try:
            open_price = tick_df['LastPx'].iloc[0]
        except:
            open_price = np.nan
        tick_df['HighPx'] = tick_df['LastPx'].cummax()
        ...
        tick_df_1min['LastPx_1min'] = tick_df_1min['LastPx']
        tick_df_1min['diff'] = tick_df_1min['LastPx'].diff().fillna(0)
        ...
        for j in tick_df.index:
            ...
            tick_df.loc[j, 'diff_real'] = tick_df.loc[j, 'LastPx'] - LastPx_1min
        ...
        tick_df['new_min2'] = (tick_df['LastPx'] <= tick_df['LastPx'].rolling(100,1).mean() * 0.99)
        ...
        if zcz:
            trigger_time = tick_df[(tick_df['trigger1']==True) & (tick_df['MDTime'] >= 93200000) & (tick_df['HighPx'] <= open_price*1.1)]['MDTime'].min()
        else:
            trigger_time = tick_df[(tick_df['trigger1'] == True) & (tick_df['MDTime'] >= 93200000) & (tick_df['HighPx'] <= open_price*1.05)]['MDTime'].min()
        ...
        return
factor_df_list = Parallel(n_jobs=24)(delayed(get_trigger)(index) for index in sample.index)
...
sample.to_pickle('/data/user/015585/01-因子挖掘/20240812-Ceres新增买入时点/file/test_trigger5_3_1_3.pkl')
