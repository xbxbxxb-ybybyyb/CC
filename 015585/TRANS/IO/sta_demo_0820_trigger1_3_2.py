...
df = pd.read_pickle('/data/user/015585/01-因子挖掘/20240812-Ceres新增买入时点/'
                    'file/Basic_closed_hf_finish_20160101_20191231_saturn_update.pkl')
...
def get_trigger(index):
    ...
    tick_df['macd_real_depart'] = (tick_df['macd_real'] > ( tick_df['macd_real_min'] + tick_df['macd_real_std'] * 0.25))
    ...
    if zcz:
        trigger_time = tick_df[(tick_df['trigger1']==True) & (tick_df['MDTime'] >= 93200000) &
        (tick_df['LastPx'] >= pre_close*0.94)]['MDTime'].min()
    else:
        trigger_time = tick_df[(tick_df['trigger1']==True) & (tick_df['MDTime'] >= 93200000) &
        (tick_df['LastPx'] >= pre_close*0.97)]['MDTime'].min()
...
factor_df_list = Parallel(n_jobs=16)(delayed(get_trigger)(index) for index in sample.index)
...
sample.to_pickle('/data/user/015585/01-因子挖掘/20240812-Ceres新增买入时点/file_saturn/test_trigger1_3_2.pkl')
