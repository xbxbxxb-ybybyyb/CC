...
import numpy as np
import pandas as pd
# 逻辑：买单集中度：主动卖单的平方和 / 总平方和（>0)
# 9，6，0.02
factor_name = 'qyh_T1mtra_cct_sell'#concenteration
def factor_qyh_T1mtra_cct_sell(transaction_df, return_fillna_dic=False):
    ...
    if abs(total_buy) <= 1:
        ...
        buy_cct = (tm_sell ** 2).sum() / tm_sell_df['TradeMoney'].sum() ** 2
