...
#
factor_name = 'qyh_T1mtra_ratio_buy_big_1'
def factor_qyh_T1mtra_ratio_buy_big_1(transaction_df, return_fillna_dic=False):
    ...
    buy_big_amt = buy_amt[buy_amt >= 200000].sum()
    total_amt = buy_amt.sum()
    # ratio
    if abs(total_amt) >= 0.001:
        ratio = buy_big_amt / total_amt
    else:
        ratio = np.nan
