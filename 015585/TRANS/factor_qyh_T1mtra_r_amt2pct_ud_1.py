...
import pandas as pd
# 0216待提交
# 逻辑：(每个主动买单的金额 + 每个主动卖单的金额)/（abs(拉升幅度（基于上一单价格)) +abs（砸盘幅度））
# 9，4，-0.04
factor_name = 'qyh_T1mtra_r_amt2pct_ud_1'#
def factor_qyh_T1mtra_r_amt2pct_ud_1(transaction_df, return_fillna_dic=False):
    ...
    if abs(ret_1 + ret_2) > 0.001:
        factor_dict = {factor_name: (amt_1 + amt_2)/(ret_1 + ret_2)}
    else:
