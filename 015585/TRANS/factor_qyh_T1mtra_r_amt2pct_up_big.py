...
# 逻辑：大单中每个主动买单的金额/拉升幅度（基于单内价格）,说明单内价格的大单拉升幅度没用
# 8，4，-0.04
factor_name = 'qyh_T1mtra_r_amt2pct_up_big'#
def factor_qyh_T1mtra_r_amt2pct_up_big(transaction_df, return_fillna_dic=False):
    ...
    transaction_df_up_big = transaction_df_up_big[transaction_df_up_big > 200000]# 大单
    transaction_df_up = transaction_df_up[transaction_df_up['TradeBuyNo'].isin(transaction_df_up_big.index)]# 只要大单部分
    ...
    if transaction_df_up.empty:
        ...
        power_1 = transaction_df_up['TradeMoney'].sum() / ret.sum() / 100
