...
def factor_qyh_ttick_20231109_wj(transaction_df, return_fillna_dic=False):
    factor_name = 'qyh_ttick_20231109_wj'
    ...
    vol_med = float(transaction_df['ValueTrade'].quantile(0.8))
    transaction_df = transaction_df[(transaction_df['ValueTrade'] <= vol_med)]
    if len(transaction_df) > 0:
        ...
        ret_pct_b = (0.5*(sel_trans_before['HighPx']+sel_trans_before['LowPx'])
                     - sel_trans_before['LastPx'])\
                    /(np.log(sel_trans_before['ValueTrade'].mean()+1))
        ret = ret_pct_b.max()
    else:
        ret = 0
