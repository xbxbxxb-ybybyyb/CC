...
import numpy as np
import pandas as pd
# 逻辑：T日09:31数据 + 集合竞价，(流入部分大单 且 流出部分非大单)金额占比
# score:13,5,-0.03
factor_name = 'qyh_T1mtra_ratio_in_big_dbo_1'#del big out
def factor_qyh_T1mtra_ratio_in_big_dbo_1(transaction_df, return_fillna_dic=False):
    ...
    sell_no_nbig = sell_no[sell_no < 200000]
    buy_no_big_dbo = transaction_df[(transaction_df['TradeBuyNo'].isin(buy_no_big.index)) &
                                    (transaction_df['TradeSellNo'].isin(sell_no_nbig.index))]['TradeMoney'].sum()
    # ratio
    if abs(buy_no.sum()) <= 0.001:
        ratio = np.nan
    else:
        ratio = buy_no_big_dbo / buy_no.sum()
