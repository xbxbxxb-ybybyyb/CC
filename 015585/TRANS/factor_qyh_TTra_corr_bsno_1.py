...
# score:0.02,11
#
factor_name = 'qyh_TTra_corr_bsno_1'
def factor_qyh_TTra_corr_bsno_1(transaction_df, return_fillna_dic=False):
    ...
    n = int(len(transaction_df) / 2)
    if len(transaction_df) > n:
        corr = transaction_df.head(n)[['TradeBuyNo','TradeSellNo']].corr().iloc[0,1]
    else:
