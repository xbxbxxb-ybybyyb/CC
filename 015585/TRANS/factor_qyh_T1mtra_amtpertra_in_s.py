...
import numpy as np
import pandas as pd
# 逻辑：T日09:31数据(包括集合竞价）,买单平均每单（非每笔）金额的标准差(除以均值）,衡量投资者的复杂程度
# score:6,2,-0.02(除以均值) 9,4,-0.03（不除以均值）
factor_name = 'qyh_T1mtra_amtpertra_in_s'
def factor_qyh_T1mtra_amtpertra_in_s(transaction_df, return_fillna_dic=False):
