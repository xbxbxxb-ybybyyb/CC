...
class factor_qyh_newsat_20240321_1(BaseFactor):
    strategy_name = "saturn/sell"
    factor_name = "qyh_newsat_20240321_1"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "qyh"  # 开发人员姓名
    factor_explain = "价差在上涨和下跌时候的集中度差异" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        res1 = (tick_df1['factor'] ** 2).sum() / (tick_df1['factor'].sum()) ** 2 if abs(tick_df1['factor'].sum()) > 1e-3 else np.nan
        res2 = (tick_df2['factor'] ** 2).sum() / (tick_df2['factor'].sum()) ** 2 if abs(tick_df2['factor'].sum()) > 1e-3 else np.nan
