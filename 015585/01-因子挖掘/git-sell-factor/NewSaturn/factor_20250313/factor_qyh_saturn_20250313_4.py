...
class factor_qyh_saturn_20250313_4(BaseFactor):
    strategy_name = "saturn/sell"
    factor_name = "qyh_saturn_20250313_4"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "qyh"  # 开发人员姓名
    factor_explain = "小于7%的后一半撤单的价格离散程度" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            #
            if zcz:
                p = round_(pre_close * (1 + 0.07 * 2), 3)
            elif bj:
                p = round_(pre_close * (1 + 0.07 * 3), 3)
            else:
                p = round_(pre_close * 1.07, 3)
