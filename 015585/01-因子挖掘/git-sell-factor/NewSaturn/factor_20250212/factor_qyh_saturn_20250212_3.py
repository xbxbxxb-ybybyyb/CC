...
class factor_qyh_saturn_20250212_3(BaseFactor):
    strategy_name = "saturn/sell"
    factor_name = "qyh_saturn_20250212_3"
    fill_na_value = 0.36
    need_pre_calculate_T_N = True
    owner = "qyh"  # 开发人员姓名
    factor_explain = "前一半撤单成交额除以市值后的的标准差" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            mv = pre_close * ff_shares
            if mv > 10:
                cancel_df['factor'] = cancel_df['factor'].apply(lambda x : np.log(x+1)) / np.log(mv)
            else:
