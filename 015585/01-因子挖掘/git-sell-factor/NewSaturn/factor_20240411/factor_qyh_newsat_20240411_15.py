...
class factor_qyh_newsat_20240411_15(BaseFactor):
    strategy_name = "saturn/sell"
    factor_name = "qyh_newsat_20240411_15"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "qyh"  # 开发人员姓名
    factor_explain = "卖均对应涨跌幅在活跃/不活跃时候的标准差差异" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        tick_df1['factor'] = tick_df1['WeightedAvgOfferPx']/(tick_df1['pre_close'])
        tick_df2['factor'] = tick_df2['WeightedAvgOfferPx']/(tick_df2['pre_close'])
        if zcz:
            tick_df1['factor'] = (tick_df1['factor']-1) / 2 + 1
            tick_df2['factor'] = (tick_df2['factor']-1) / 2 + 1
