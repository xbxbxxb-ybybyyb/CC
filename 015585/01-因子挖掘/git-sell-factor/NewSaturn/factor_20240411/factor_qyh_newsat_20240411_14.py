...
class factor_qyh_newsat_20240411_14(BaseFactor):
    strategy_name = "saturn/sell"
    factor_name = "qyh_newsat_20240411_14"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "qyh"  # 开发人员姓名
    factor_explain = "涨幅*量在活跃/不活跃的首末变化值差异" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        tick_df1['factor'] = (tick_df1['LastPx']/tick_df1['pre_close']-1) * tick_df1['VolumeTrade']
        tick_df2['factor'] = (tick_df2['LastPx']/tick_df2['pre_close']-1) * tick_df2['VolumeTrade']
        if zcz:
            tick_df1['factor'] = tick_df1['factor'] / 2
            tick_df2['factor'] = tick_df2['factor'] / 2

        res = tick_df1['factor'].head(1).mean() - tick_df2['factor'].head(1).mean() - tick_df1['factor'].tail(1).mean() + tick_df2['factor'].tail(1).mean()
