...
class factor_qyh_newsat_20240516_2(BaseFactor):
    strategy_name = "saturn/sell"
    factor_name = "qyh_newsat_20240516_2"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "qyh"  # 开发人员姓名
    factor_explain = "前1分钟，委买金额/总成交额的离散程度" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        res = (tick_df['factor'].max() + tick_df['factor'].min()) / (tick_df['factor'].mean() + tick_df['factor'].min()) \
            if (tick_df['factor'].mean() + tick_df['factor'].min()) > 0 else np.nan
