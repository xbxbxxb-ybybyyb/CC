...
class factor_qyh_newsat_20240509_6(BaseFactor):
    strategy_name = "saturn/sell"
    factor_name = "qyh_newsat_20240509_6"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "qyh"  # 开发人员姓名
    factor_explain = "买1量离散程度在活跃/不活跃时候的差异" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        tick_df1['factor'] = tick_df1['Buy1OrderQty']
        tick_df2 = tick_df[tick_df['ValueTrade'] <= tick_df['ValueTrade'].quantile(0.25)]
        tick_df2['factor'] = tick_df2['Buy1OrderQty']
        # if zcz:
        res = (tick_df1['factor'].max() + tick_df1['factor'].min())/ (tick_df1['factor'].mean() + tick_df1['factor'].min()) - \
              (tick_df2['factor'].max() + tick_df2['factor'].min()) / (tick_df2['factor'].mean() + tick_df1['factor'].min())
