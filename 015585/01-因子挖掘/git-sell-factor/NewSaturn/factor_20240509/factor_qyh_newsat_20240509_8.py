...
class factor_qyh_newsat_20240509_8(BaseFactor):
    strategy_name = "saturn/sell"
    factor_name = "qyh_newsat_20240509_8"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "qyh"  # 开发人员姓名
    factor_explain = "买均-卖均的变化在活跃/不活跃时候的差异" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        tick_df1['factor'] = (tick_df1['WeightedAvgBidPx'] - tick_df1['WeightedAvgOfferPx'])/(tick_df1['pre_close'])
        tick_df2 = tick_df[tick_df['ValueTrade'] <= tick_df['ValueTrade'].quantile(0.25)]
        tick_df2['factor'] = (tick_df2['WeightedAvgBidPx'] - tick_df2['WeightedAvgOfferPx'])/(tick_df2['pre_close'])
        # if zcz:
        res = (tick_df1['factor'].head(1).mean() - tick_df1['factor'].tail(1).mean()) - \
              (tick_df2['factor'].head(1).mean() - tick_df2['factor'].tail(1).mean())
