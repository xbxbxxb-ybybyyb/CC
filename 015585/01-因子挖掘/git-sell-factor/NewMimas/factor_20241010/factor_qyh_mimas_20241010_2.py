...
class factor_qyh_mimas_20241010_2(BaseFactor):
    strategy_name = "mimas"
    factor_name = "qyh_mimas_20241010_2"
    fill_na_value = 0
    need_pre_calculate_T_N = False
    owner = "qyh"  # 开发人员姓名
    factor_explain = "订单价格大于市价*1.01部分，订单量/总挂卖量的末尾值" # 因子逻辑解释
    ...
    def calculate(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            zcz = database['zcz']
            if zcz:
                tick_df = tick_df[tick_df['OrderPrice'] >= (tick_df['LastPx'] * 1.02).apply(lambda x: round_(x, 5))]
            else:
                tick_df = tick_df[tick_df['OrderPrice'] >= (tick_df['LastPx'] * 1.01).apply(lambda x: round_(x, 5))]
            tick_df['factor'] = tick_df['OrderQty'] / (tick_df['TotalOfferQty'] +1)
