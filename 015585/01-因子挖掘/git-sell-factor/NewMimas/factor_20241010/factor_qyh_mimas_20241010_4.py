...
class factor_qyh_mimas_20241010_4(BaseFactor):
    strategy_name = "mimas"
    factor_name = "qyh_mimas_20241010_4"
    fill_na_value = 0
    need_pre_calculate_T_N = False
    owner = "qyh"  # 开发人员姓名
    factor_explain = "订单价格小于市价*0.99部分，订单量/总挂卖量的标准差" # 因子逻辑解释
    ...
    def calculate(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            zcz = database['zcz']
            if zcz:
                tick_df = tick_df[tick_df['OrderPrice'] <= (tick_df['LastPx'] * 0.98).apply(lambda x: round_(x, 5))]
            else:
