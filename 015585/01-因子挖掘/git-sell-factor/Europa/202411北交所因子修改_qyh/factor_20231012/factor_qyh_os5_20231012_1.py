...
s = FactorData()
factor_name = 'qyh_os5_20231012_1'
def factor_qyh_os5_20231012_1(start_date, end_date, IO, return_fillna_dic=False):
    if return_fillna_dic:
        # 返回因子为nan时的填充值
        return {factor_name: -0.078,'data':['ordersheet5_new','MD']}
    # -------------------------------------------------------------------------------------------------------------------
    ...
    df_pbid = IO.read_data([start_date,end_date],
                           alt = '/data/group/800463/data/generalStrong/ordersheet5_new/WeightedAvgBidPx.h5')
    df_poffer = IO.read_data([start_date, end_date],
                           alt='/data/group/800463/data/generalStrong/ordersheet5_new/WeightedAvgOfferPx.h5')
    ...
    res['bj'] = res.reset_index()['Ticker'].apply(lambda x: x[-2:] == 'BJ').values
    res.loc[res['zcz']==1,factor_name] = res.loc[res['zcz']==1,factor_name]/2
    res.loc[res['bj']==1,factor_name] = res.loc[res['bj']==1,factor_name]/3
    # -------------------------------------------------------------------------------------------------------------------
