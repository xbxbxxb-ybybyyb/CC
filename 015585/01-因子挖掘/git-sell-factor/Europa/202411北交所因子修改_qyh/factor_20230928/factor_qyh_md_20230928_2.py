...
s = FactorData()
factor_name = 'qyh_md_20230928_2'
def factor_qyh_md_20230928_2(start_date, end_date, IO, return_fillna_dic=False):
    if return_fillna_dic:
        # 返回因子为nan时的填充值
        return {factor_name: -4.42,'data':['MD']}
    # -------------------------------------------------------------------------------------------------------------------
    ...
    df_ori[factor_name] = df_ori['pct_chg'].unstack().rolling(12,1).min().stack()
    # -------------------------------------------------------------------------------------------------------------------
