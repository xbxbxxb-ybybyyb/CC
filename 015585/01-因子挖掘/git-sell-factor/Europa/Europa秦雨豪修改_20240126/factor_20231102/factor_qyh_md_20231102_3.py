...
s = FactorData()
factor_name = 'qyh_md_20231102_3'
def factor_qyh_md_20231102_3(start_date, end_date, IO, return_fillna_dic=False):
    ...
    df_ori['factor'] = df_ori['factor'].apply(lambda x :0 if x >=0 else x)
    df_ori[factor_name] = df_ori['factor'].unstack().rolling(5,1).sum().stack()
    # -------------------------------------------------------------------------------------------------------------------
