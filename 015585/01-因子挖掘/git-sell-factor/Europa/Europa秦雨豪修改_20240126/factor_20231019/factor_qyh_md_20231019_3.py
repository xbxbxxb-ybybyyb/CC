...
s = FactorData()
factor_name = 'qyh_md_20231019_3'
def factor_qyh_md_20231019_3(start_date, end_date, IO, return_fillna_dic=False):
    if return_fillna_dic:
        # 返回因子为nan时的填充值
        return {factor_name: 19.5,'data':['MD']}
    # -------------------------------------------------------------------------------------------------------------------
    ...
    df_ori = IO.read_data([start_date, end_date],
                          columns=['close', 'low', 'pre_close', 'amt'],
    ...
    df_ori[factor_name] = df_ori['factor'] / (df_ori['factor'].unstack().
                                              rolling(20,10).apply(lambda x:f_calc_cct(x)).stack())
    # -------------------------------------------------------------------------------------------------------------------
