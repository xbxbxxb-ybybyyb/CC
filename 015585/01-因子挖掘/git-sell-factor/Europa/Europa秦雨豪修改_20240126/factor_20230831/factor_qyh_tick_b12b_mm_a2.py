...
def factor_qyh_tick_b12b_mm_a2(tick_df, return_fillna_dic=False):
    ...
    res1 = tick_df1['b12b'].max() / tick_df1['b12b'].mean() if round_(tick_df1['b12b'].mean(),6) > 0 else np.nan
    res2 = tick_df2['b12b'].max() / tick_df2['b12b'].mean() if round_(tick_df2['b12b'].mean(),6) > 0 else np.nan
