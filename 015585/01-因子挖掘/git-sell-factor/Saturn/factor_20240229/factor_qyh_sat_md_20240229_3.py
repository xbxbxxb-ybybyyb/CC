...
s = FactorData()
factor_name = 'qyh_sat_md_20240229_3'
def factor_qyh_sat_md_20240229_3(start_date, end_date, IO, return_fillna_dic=False):
    ...
    df_ori[factor_name] = df_ori['factor'].unstack().rolling(5,1).apply(lambda x : f_calc_skew(x)).stack() \
                          / df_ori['factor'].unstack().rolling(20,1).apply(lambda x : f_calc_skew(x)).stack()
