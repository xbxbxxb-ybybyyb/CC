...
# 逻辑：计算60日里每日振幅的均值，对abs超过20%的部分截断
# score:25,0.06,波动大比较好，和std一样corr为正
import pandas as pd
import numpy as np
import os
from xquant.factordata import FactorData
s = FactorData()
factor_name = 'qyh_amp_60'
def factor_qyh_amp_60(start_date, end_date, IO, return_fillna_dic=False):
    ...
    f_data['amp'] = f_data['amp'].apply(lambda x: -0.22 if x<-0.22 else x)
    # 60日均值
    f_data[factor_name] = f_data['amp'].unstack().rolling(60,30).mean().stack()
