...
class factor_qyh_europa_comb_20241128_6(BaseFactor):
    strategy_name = "jupiter/europa"
    factor_name = "qyh_europa_comb_20241128_6"
    fill_na_value = 0
    need_pre_calculate_T_N = False
    owner = "qyh"  # 开发人员姓名
    factor_explain = "买均价差，总量强度的组合因子" # 因子逻辑解释
    ...
    def calculate(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            tick_df['factor3'] = abs(tick_df['LastPx']/tick_df['pre_close']-1) * tick_df['ValueTrade']
            tick_df['factor3'] = tick_df['factor3'].apply(lambda x : np.log(x + 1e-5))
            ...
            res3 = (tick_df['factor3']**2).sum() / (abs(tick_df['factor3']).sum())**2 if abs(tick_df['factor3'].sum()) > 1e-3 else 0
            #
            if zcz:
                res2 = res2/2
            if bj:
