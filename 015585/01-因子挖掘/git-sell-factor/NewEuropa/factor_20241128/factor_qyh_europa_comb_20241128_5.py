...
class factor_qyh_europa_comb_20241128_5(BaseFactor):
    strategy_name = "jupiter/europa"
    factor_name = "qyh_europa_comb_20241128_5"
    fill_na_value = 0
    ...
    def calculate(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            tick_df['factor1'] = (tick_df['LastPx'] / tick_df['pre_close'] - 1) * tick_df['VolumeTrade'] / tick_df['ff_shares']
            tick_df['factor2'] = (tick_df['Buy1Price'] / tick_df['WeightedAvgBidPx'])
            #
            res1 = (tick_df['factor1']**2).sum() / (tick_df['factor1'].sum())**2 if abs(tick_df['factor1'].sum()) > 1e-3 else 0
            res2 = tick_df['factor2'].max() / tick_df['factor2'].mean() if round_(tick_df['factor2'].mean(),5)>0 else 0
            res1 = 0 if np.isnan(res1) else res1
            res2 = 0 if np.isnan(res2) else res2
            #
            res = 1/(res1 + res2) if round_(abs(res1 + res2), 5) > 1e-3 else 0
