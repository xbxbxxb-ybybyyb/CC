...
class factor_qyh_europa_comb_20241128_1(BaseFactor):
    strategy_name = "jupiter/europa"
    factor_name = "qyh_europa_comb_20241128_1"
    fill_na_value = 0
    need_pre_calculate_T_N = False
    owner = "qyh"  # 开发人员姓名
    factor_explain = "买均和买均价差统计量的组合因子" # 因子逻辑解释
    ...
    def calculate(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            #
            res1 = 10 if np.isnan(res1) else res1
            res2 = 0.05 if np.isnan(res2) else res2
            res = res1 / res2 if round_(abs(res2),5) > 1e-3 else 1
            res = 1 / res if round_(abs(res),5) > 1e-3 else 0
