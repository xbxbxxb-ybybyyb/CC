...
class factor_qyh_europa_20240530_2(BaseFactor):
    strategy_name = "europa"
    factor_name = "qyh_europa_20240530_2"
    fill_na_value = 0
    need_pre_calculate_T_N = False
    owner = "qyh"  # 开发人员姓名
    factor_explain = "挂单价格-挂单时买均的变异系数" # 因子逻辑解释
    ...
    def calculate(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            res = tick_df['factor'].std() / tick_df['factor'].mean() if abs(round_(tick_df['factor'].mean(),5)) > 0 else np.nan
