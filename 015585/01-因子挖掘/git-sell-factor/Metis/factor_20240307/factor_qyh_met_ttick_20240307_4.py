import numpy as np
import pandas as pd
factor_name = 'qyh_met_ttick_20240307_4'#
def factor_qyh_met_ttick_20240307_4(tick_df, return_fillna_dic=False):
    ...
    t_fzt = tick_df[tick_df['HighPx'] >= round_(tick_df['HighPx'].max()-0.01,2)]['MDTime'].min() # 首次逼近涨停时间
    tick_df = tick_df[tick_df['MDTime'] >= t_fzt]
    #
    tick_df['buy_amt'] = tick_df['TotalBidQty'] * tick_df['WeightedAvgBidPx']
    tick_df['factor'] = (tick_df['buy_amt'])/(tick_df['ValueTrade'].sum()) if round_(tick_df['ValueTrade'].sum(),2) > 0 else np.nan
