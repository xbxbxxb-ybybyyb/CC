...
s = FactorData()
# dtj
# amt 5日上涨时的集中度rank - 20日上涨时集中度rank
# 19,0.046
#
factor_name = 'qyh_next_md_20231207_9'
def factor_qyh_next_md_20231207_9(start_date, end_date, IO, return_fillna_dic=False):
    ...
    df_ori['factor'] = df_ori['amt'] * df_ori['up']
    #
    df_ori['factor1'] = df_ori['factor'].unstack().rolling(5,1).apply(lambda x : f_calc_cct(x)).stack()
    df_ori['factor1'] = rank_(df_ori['factor1'])
    df_ori['factor2'] = df_ori['factor'].unstack().rolling(20,1).apply(lambda x : f_calc_cct(x)).stack()
    df_ori['factor2'] = rank_(df_ori['factor2'])
