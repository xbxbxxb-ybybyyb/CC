...
s = FactorData()
factor_name = 'qyh_m5_vol_h5_ratio_3'
def factor_qyh_m5_vol_h5_ratio_3(start_date, end_date, IO, return_fillna_dic=False):
    ...
    for i in vol_data_values:
        ...
        res.append(i[-5:].sum())
    vol_data['head5'] = res
    # vol_data['head5'] = vol_data.drop(['total'], axis=1).max(axis=1)
    vol_data['ratio'] = vol_data['head5'] / vol_data['total']
    vol_data[factor_name] = vol_data['ratio'].unstack().rolling(10,1).mean().stack()
