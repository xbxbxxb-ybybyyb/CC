...
s = FactorData()
factor_name = 'qyh_m5_vwap2oc_3'
def factor_qyh_m5_vwap2oc_3(start_date, end_date, IO, return_fillna_dic=False):
    if return_fillna_dic:
        # 返回因子为nan时的填充值
        return {factor_name: 0.489, 'data': ['minute5']}
    # -------------------------------------------------------------------------------------------------------------------
    ...
    res = res.mean(axis=1)
    res = pd.DataFrame(res.unstack().rolling(10, 2).mean().stack())
