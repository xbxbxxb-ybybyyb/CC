...
s = FactorData()
factor_name = 'qyh_m5_corr_apct_amt_2'
def factor_qyh_m5_corr_apct_amt_2(start_date, end_date, IO, return_fillna_dic=False):
    ...
    pct_data_array = abs(pct_data_array)
    res_array = []
    ...
    res = pd.DataFrame(res_array,columns = [factor_name],index = amt_data.index)\
        .unstack().rolling(2,2).mean().stack()
