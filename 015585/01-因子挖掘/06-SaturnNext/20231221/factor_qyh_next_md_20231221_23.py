...
# dtj
# 0.07,0.065,31
# abs(pct)在上涨时候的5日集中度/80日集中度
factor_name = 'qyh_next_md_20231221_23'
def factor_qyh_next_md_20231221_23(start_date, end_date, IO, return_fillna_dic=False):
    ...
    df_ori['factor'] = abs(df_ori['pct_chg']) * df_ori['up']
    df_ori[factor_name] = (df_ori['factor']).unstack().rolling(5,1).apply(lambda x :f_calc_cct(x)).stack() \
                          / (df_ori['factor']).unstack().rolling(80,1).apply(lambda x :f_calc_cct(x)).stack()
    # -------------------------------------------------------------------------------------------------------------------
