...
def factor_qyh_test_wj1(tick_df, return_fillna_dic=False):
    factor_name = 'qyh_test_wj1'
    ...
    compare_pc = np.min([pre_close, high_px])
    ...
    if len(tick_df) > 0:
        ...
        ret_pct_b = (sel_trans_before['LastPx'] - low_px) / close
        ret = ret_pct_b.mean()
    else:
        ret = 0
