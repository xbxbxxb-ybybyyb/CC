...
def f_pro_cleanb2ttran(tick_df):#净委买金额/该阶段总成交额,series（开盘后）
    tick_df['buy_amt'] = tick_df['TotalBidQty'] * tick_df['WeightedAvgBidPx']
    tick_df['sell_amt'] = tick_df['TotalOfferQty'] * tick_df['WeightedAvgOfferPx']
    cleanb2ttran = (tick_df['buy_amt'] - tick_df['sell_amt'])/(tick_df['ValueTrade'].sum() + 1e-3)
    return cleanb2ttran
...
def write_excel(result_df):
    ...
    result_df.loc[factor_name,'repeat'] = factor_test.result_dic['max_same_ratio'].iloc[0,0]
...
result_df = pd.DataFrame(columns = ['factor_name','IC','info',
                                    ...
                                    'compare'])
list_del = []
# for i in list(os.listdir('/data/user/015585/01-因子挖掘/06-SaturnNext/因子快速开发/h5/20230712talltick/')):
#     list_del.append(i[:-21])
for i in list(os.listdir('/data/user/015585/01-因子挖掘/06-SaturnNext/因子快速开发/h5/20230713talltick/')):
    list_del.append(i[:-21])
list_series = ['rcleanb',
                'cleanb2ttran',
                'cleanb2tran',
                                    ...
                                    for calc_i in dic_calc:
                                        ...
                                        for compare_i in dic_compare:
                                            ...
                                            if factor_name in list_del:
                                                print(factor_name)
                                                continue
                                            if factor_name != '930_after_all_all_0_bigger_h500_s2transtd_nostd_sum_nocompare':
                                                continue
                                            ...
                                            print(factor_name)
                                            factor_path = '/dfs/user/015585/01_factor_develop_store/fast_factor/saturnnext/h5' \
                                                          + '/20231201Next1mTickab/'
                                            run_factor(func=factor_func,
                                                       factor_name=factor_name,
                                                       factor_type='Next1mTickab',
                                                       start_date=20160101,
                                                       end_date=20191231,
                                            ...
                                            result_path = '/dfs/user/015585/01_factor_develop_store/fast_factor/saturnnext/factor_report' \
                                                          + '/20231201Next1mTickab/'
