...
s = FactorData()
factor_name = 'qyh_m5_vol_up_20'
def factor_qyh_m5_vol_up_20(start_date, end_date, IO, return_fillna_dic=False):
    ...
    res = pd.DataFrame(res,columns=[factor_name]).unstack().rolling(20,5).mean().stack()
    # -------------------------------------------------------------------------------------------------------------------
    return res