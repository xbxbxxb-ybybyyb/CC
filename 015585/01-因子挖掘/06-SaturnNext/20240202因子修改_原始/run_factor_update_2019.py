...
# excel
# res_excel = pd.read_excel('/data/user/015585/01-因子挖掘/06-SaturnNext/20240202因子修改_原始/因子列表_qyh.xlsx')
# basic_file_path = '/data/group/800463/data/project2_public/next_factor_lib/Basic_next_hf_finish_20160101_20191231.h5'
# #
# date_list = os.listdir('/data/user/015585/01-因子挖掘/06-SaturnNext/20240202因子修改_原始/')
# date_list = [x for x in date_list if 'factor_' in x and '.py' not in x]
# res = pd.DataFrame()
# for date in date_list:
#     factor_list_date = os.listdir('/data/user/015585/01-因子挖掘/06-SaturnNext/20240202因子修改_原始/' + date + '/')
#     factor_list_date = [x[:-3] for x in factor_list_date  if 'factor_' in x]
#     for factor in factor_list_date:
#         name = factor[7:]
#         if name == 'qyh_n1mtick_20231221_5':
#             continue
#         if len(res_excel[res_excel['factor_name'] == name]) == 0:
#             continue
#         print(factor)
#         path1 = date + '.' + factor
...
#                                 20160101, 20191231,
#                                 basic_file_path,
#                                 '/data/user/015585/01-因子挖掘/06-SaturnNext/20240202因子修改_原始/save_file/', interval_res=False)
#         for col in factor_df0:
#             print(col, abs(factor_df0[col]).max())
#         res[name] = factor_df0[name]
#         print(len(res.columns))
#
# res.to_pickle('/data/user/015585/01-因子挖掘/06-SaturnNext/20240202因子修改_原始/save_file/res_ori.pkl')

df1 = pd.read_pickle('/data/user/015585/01-因子挖掘/06-SaturnNext/20240202因子修改/save_file/res_ori.pkl') # 新
df2 = pd.read_pickle('/data/user/015585/01-因子挖掘/06-SaturnNext/20240202因子修改_原始/save_file/res_ori.pkl') # 旧
import project_2_factor_test_origin as sft
sample_sft = sft.pj2FactorTest(20191001, 20191231)
...
for col in df2.columns:
    ...
    ratio = len(df_col[abs(df_col[0] - df_col[1]) > 1e-6]) / len(df_col)
