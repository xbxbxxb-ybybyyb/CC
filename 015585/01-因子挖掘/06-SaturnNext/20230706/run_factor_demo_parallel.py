...
def run_factor(func, factor_name, factor_type, start_date, end_date, basic_file_path, result_path, append_next_tradingday=False, interval_res=True,
               ...
               data_path_dic={'NextTickabNextTickab':'/data/group/800463/data/project2_prod/everyday_Data/next_tick_cs/',
                              ...
                              'TallTrans_cs':'/data/group/800463/data/project2_prod/everyday_Data/all_transaction_cs/',}):
    # 因子代码执行函数
    ...
    if factor_type in  ['NextTickab','NextTickab_cs','NextTransaction','NextTransaction_cs',
        ...
        factor_df_list = Parallel(n_jobs=20)(delayed(cal_T_factors)(i) for i in range(len(last_zt_list)))
        # import multiprocessing
        # pool = multiprocessing.Pool(16)
        # factor_df_list = [pool.apply_async(cal_T_factors, args=(i,)).get() for i in range(len(last_zt_list))]
        # pool.close()
        # pool.join()
        # factor_df_list.append(pd.concat([tmp_df], axis=1))
        factor_df = pd.concat(factor_df_list, axis=0)
...
#     basic_file_path = '/data/group/800463/data/project2_public/factor_lib_next/Basic_next_hf_finish_20160101_20191231.h5'
#     result_path = '/data/user/022325/Factors/tmp/'
#     # print('................??????????????')
#
#     import my_factors.factor_test_last_factor as func_tmp
#     factor_df0 = run_factor(func_tmp.factor_test_last_factor, 'test_last_factor', 'Next_T-1_factor',
#                            start_date, end_date, basic_file_path, result_path,interval_res=False)
#
#     import my_factors.factor_test_trade_factor as func_tmp
#     factor_df1 = run_factor(func_tmp.factor_test_trade_factor, 'test_trade_factor', 'TallTrans',
#                            start_date, end_date, basic_file_path, result_path,interval_res=False)
