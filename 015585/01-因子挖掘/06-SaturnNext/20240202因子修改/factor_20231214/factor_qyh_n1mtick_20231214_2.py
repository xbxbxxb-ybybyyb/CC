...
def factor_qyh_n1mtick_20231214_2(tick_df, return_fillna_dic=False):
    ...
    tick_df['ValueTrade'] = tick_df['ValueTrade'].apply(lambda x : round_(x,6))
    ...
    #
    res = (tick_df['factor'].std())/tick_df['factor'].mean() if round_(abs(tick_df['factor'].mean()),6)>0 else np.nan
    #
    tmp = tick_df[tick_df['ValueTrade'] <= round_(tick_df['ValueTrade'].quantile(0.25),6)]
