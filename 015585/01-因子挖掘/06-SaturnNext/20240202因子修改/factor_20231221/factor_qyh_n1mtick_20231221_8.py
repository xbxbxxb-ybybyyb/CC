...
import pandas as pd
import numpy as np
def factor_qyh_n1mtick_20231221_8(tick_df, return_fillna_dic=False):
    factor_name = 'qyh_n1mtick_20231221_8'
    ...
    ##
    tick_df['factor'] = tick_df['vwap'] / tick_df['HighPx'] * tick_df['VolumeTrade']
    res = (tick_df['factor'].mean()+1e-5) / (tick_df['factor'].std()+1e-5)
    if round_(res1,5) < 6:
        res = res + 0.22*3
