...
# excel
res_excel = pd.read_excel('/data/user/015585/01-因子挖掘/06-SaturnNext/20240202因子修改/因子列表_qyh.xlsx')
basic_file_path = '/data/group/800463/data/project2_public/next_factor_lib/Basic_next_hf_finish_20160101_20191231.h5'
#
date_list = os.listdir('/data/user/015585/01-因子挖掘/06-SaturnNext/20240202因子修改/')
date_list = [x for x in date_list if 'factor_' in x and '.py' not in x]
res = pd.DataFrame()
for date in date_list:
    factor_list_date = os.listdir('/data/user/015585/01-因子挖掘/06-SaturnNext/20240202因子修改/' + date + '/')
    ...
    for factor in factor_list_date:
        ...
        if (type(res_excel[res_excel['factor_name'] == name]['修改原因'].iloc[0]) == str):
            ...
            factor_df0 = run_factor(func,
                                    ...
                                    20160101, 20191231,
                                    basic_file_path,
                                    '/data/user/015585/01-因子挖掘/06-SaturnNext/20240202因子修改/save_file/', interval_res=False)
            for col in factor_df0:
                print(col, abs(factor_df0[col]).max())
            res[name] = factor_df0[name]
            print(len(res.columns))

res.to_pickle('/data/user/015585/01-因子挖掘/06-SaturnNext/20240202因子修改/save_file/res_ori.pkl')
