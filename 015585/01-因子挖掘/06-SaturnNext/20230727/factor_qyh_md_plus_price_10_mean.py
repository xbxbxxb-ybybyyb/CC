...
# 逻辑：T-10日的PLUS价格：(2Ct - Ht - Lt)/Ct-1的均值
# -0.076,30
# skk_kmid_mean_a,41
import pandas as pd
import numpy as np
...
s = FactorData()
def factor_qyh_md_plus_price_10_mean(start_date, end_date, IO, return_fillna_dic=False):
    factor_name='qyh_md_plus_price_10_mean'
    ...
    f_data = f_data.unstack().rolling(10,1).mean().stack()
    # -------------------------------------------------------------------------------------------------------------------
    return f_data
