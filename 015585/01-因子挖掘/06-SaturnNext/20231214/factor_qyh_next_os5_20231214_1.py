...
s = FactorData()
factor_name = 'qyh_next_os5_20231214_1'
# dtj,zcz
# 过去3日，挂买挂卖差的std的均值
# 16，-0.039
def factor_qyh_next_os5_20231214_1(start_date, end_date, IO, return_fillna_dic=False):
    if return_fillna_dic:
        # 返回因子为nan时的填充值
        return {factor_name: 0.16,'data':['ordersheet5','MD']}
    # -------------------------------------------------------------------------------------------------------------------
    ...
    res[factor_name] = res[factor_name].unstack().rolling(3,1).mean().stack()
    # -------------------------------------------------------------------------------------------------------------------
