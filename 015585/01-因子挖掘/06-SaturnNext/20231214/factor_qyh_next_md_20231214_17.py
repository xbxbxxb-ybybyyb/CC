...
s = FactorData()
# dtj
# abs(pct*turn)的5日最大值/60日最大值
# 28，-0.066
#
factor_name = 'qyh_next_md_20231214_17'
def factor_qyh_next_md_20231214_17(start_date, end_date, IO, return_fillna_dic=False):
    ...
    df_ori = IO.read_data([start_date, end_date],
                          columns=['turn','close','pre_close','vwap','adjfactor','pct_chg'],
                          alt='/data/group/800080/warehouse/prod/MD/CHINA_STOCK/DAILY/WIND/MD_CHINA_STOCK_DAILY_WIND.h5')
    #
    df_ori['factor'] = abs(df_ori['pct_chg'] * df_ori['turn'])
    df_ori['factor1'] = df_ori['factor'].unstack().rolling(5,1).max().stack()
    df_ori['factor2'] = df_ori['factor'].unstack().rolling(60,1).max().stack()
