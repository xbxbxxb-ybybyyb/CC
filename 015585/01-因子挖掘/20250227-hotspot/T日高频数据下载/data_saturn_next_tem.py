...
def store_hf_data_for_one_day(date, Basic_next_hf_finish, cut_MDTime, result_path_tick, result_path_transaction):
    ...
    transaction_data_of_the_day_list = []
    for index, row in basic_data_in_the_day.reset_index().iterrows():
        # print(tradingday, index)
        stock = row['Ticker']
        pre_close = row['pre_close']
        ff_shares = row['ff_shares']
        ...
        try:
            ...
            used_cols = ['MDDate', 'MDTime', 'HTSCSecurityID', 'TradingPhaseCode', 'NumTrades',
                ...
                ['Buy%dPrice' % (i) for i in range(1, 6)] + ['Sell%dPrice' % (i) for i in range(1, 6)] + \
                ['Buy%dOrderQty' % (i) for i in range(1, 6)] + ['Sell%dOrderQty' % (i) for i in range(1, 6)] + \
                ['Buy%dNumOrders' % (i) for i in range(1, 6)] + ['Sell%dNumOrders' % (i) for i in range(1, 6)]
            ...
            transaction_data_of_the_day_list.append(available_transaction_data)
    ...
    transaction_data_of_the_day = pd.concat(transaction_data_of_the_day_list)
    ...
    tick_data_of_the_day.to_pickle(result_path_tick + str(tradingday) + '.pkl', compression = 'gzip')
    ...
    transaction_data_of_the_day.to_pickle(result_path_transaction + str(tradingday) + '.pkl', compression = 'gzip')
...
if __name__ == '__main__':
    ...
    result_path_tick='/dfs/user/015585/00_hotspot/tick_all_time35/'
    result_path_transaction='/dfs/user/015585/00_hotspot/trade_all_time35/'
    ...
    pool = Pool(20)
    task_list = []
    # all_Basic_next_hf_finish = pd.read_hdf('/dfs/user/020412/团队分享/for_qyh/hotspot/md2_20250414_20150901_20231231.h5').sort_index()
    all_Basic_next_hf_finish = pd.read_hdf('/dfs/user/020412/团队分享/for_hotspot/md2_20250512_20150901_20231231.h5').sort_index()

    print(all_Basic_next_hf_finish.shape)
    for tradingday in s.tradingday('20150901', '20231231'):
    # for tradingday in ['20230828', '20240206', '20240208', '20240417']:
