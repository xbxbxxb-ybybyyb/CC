...
def factor_qyh_ttick_risespeed_c(tick_df, return_fillna_dic=False):
    ...
    # tick_df['speed'] = tick_df['speed'].apply(lambda x : round_(x,6))
    if zcz:
    ...
    if len(tick_df) <= 61:
        ...
        rs1 = len(tick_df1[tick_df1['speed'] >= (0.005-1e-8)]) / len(tick_df1)
        tick_df2 = tick_df.tail(60)
        rs2 = len(tick_df2[tick_df2['speed'] >= (0.005-1e-8)]) / len(tick_df2)
