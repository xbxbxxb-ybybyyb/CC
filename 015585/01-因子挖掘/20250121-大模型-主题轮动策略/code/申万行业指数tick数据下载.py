...
def hf_preprocessing(data_type, md_df, btTime=None):
    if (data_type == 'Stock') or (data_type == 'StockAllDay') or (data_type == 'StockAllDayNoTradingPhaseCode'):  # TICK
        ...
        use_col = ['MDTime', 'PreClosePx', 'LastPx', 'OpenPx', 'HighPx', 'LowPx', 'HTSCSecurityID', 'TotalValueTrade', 'TotalVolumeTrade']
        md_df = md_df[use_col]
        md_df['MDTime'] = md_df['MDTime'].astype(int)
        md_df = md_df[md_df['MDTime'] <= btTime]  # 泛强势股的数据筛选
        return md_df
...
def store_hf_data_for_one_day(date, Basic_next_hf_finish, cut_MDTime, result_path_tick, result_path_transaction):
    ...
    for index, row in basic_data_in_the_day.reset_index().iterrows():
        ...
        try:
            tick_md_df = ma.getMDSecurityTickDataFrame(mdp_code, f"{tradingday}000000000",
                               f"{tradingday}235900000")

            tick_md_df['MDTime'] = tick_md_df['MDTime'].astype(int)
            tick_md_df = hf_preprocessing('Index', tick_md_df, btTime = 150000000)
            used_cols = ['MDTime', 'PreClosePx', 'LastPx', 'OpenPx', 'HighPx', 'LowPx', 'HTSCSecurityID', 'TotalValueTrade', 'TotalVolumeTrade']
            ...
            # print(tick_data_of_the_day.shape)
        except Exception as e:
            print(stock, tradingday, e)
            pass
    tick_data_of_the_day['MDDate'] = str(tradingday)
...
if __name__ == '__main__':
    ...
    result_path_tick='/dfs/user/015585/04_行业指数数据/申万2021/'
    result_path_transaction='/dfs/user/015585/04_行业指数数据/申万2021/'
    ...
    pool = Pool(30)
    task_list = []
    all_Basic_next_hf_finish = pd.read_pickle('/data/user/015585/01-因子挖掘/20250121-大模型-主题轮动策略/file/basic_file_sw_20160101_20241231.pkl').sort_index()
    print(all_Basic_next_hf_finish.shape)

    for tradingday in s.tradingday('20171201', '20241231'):
