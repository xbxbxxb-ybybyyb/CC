...
class factor_qyh_mercury_20240606_2(BaseFactor):
    strategy_name = "mercury"
    factor_name = "qyh_mercury_20240606_2"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "qyh"  # 开发人员姓名
    factor_explain = "用前日最低价对应涨跌幅/60日均值" # 因子逻辑解释
    ...
    t_1_factor_data = [
        {'name': 'MD_CHINA_STOCK_DAILY_WIND', # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 100, #注意为正数
         'column': ['low','amt','close','adjfactor','pre_close']
    }]  # T-N factor数据，格式如上S
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['low'] = df_ori['low'].unstack().fillna(method = 'ffill',limit=20).stack()
