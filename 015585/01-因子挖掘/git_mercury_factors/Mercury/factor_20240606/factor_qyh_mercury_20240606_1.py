...
class factor_qyh_mercury_20240606_1(BaseFactor):
    strategy_name = "mercury"
    factor_name = "qyh_mercury_20240606_1"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "qyh"  # 开发人员姓名
    factor_explain = "用前日换手率/20日最大值" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            # df_ori['factor'] = (df_ori['turn'] - df_ori['pre_close'])/(df_ori['vwap'])
            # df_ori.loc[df_ori['zcz'] == 1, 'factor'] = (df_ori.loc[df_ori['zcz'] == 1, 'factor'])/2
            df_ori[self.factor_name] = df_ori['turn'] / df_ori['turn'].unstack().rolling(20,1).max().stack()
            # -------------------------------------------------------------------------------------------------------------------
