...
class factor_qyh_mercury_20240530_9(BaseFactor):
    strategy_name = "mercury"
    factor_name = "qyh_mercury_20240530_9"
    fill_na_value = 1.1
    need_pre_calculate_T_N = True
    owner = "qyh"  # 开发人员姓名
    factor_explain = "最低价对应涨跌幅的10日均值" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori[self.factor_name] = df_ori['factor'] / df_ori['factor'].unstack().rolling(10,1).mean().stack()
            # -------------------------------------------------------------------------------------------------------------------
