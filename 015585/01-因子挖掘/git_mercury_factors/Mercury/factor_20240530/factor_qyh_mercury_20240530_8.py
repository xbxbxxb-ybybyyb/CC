...
class factor_qyh_mercury_20240530_8(BaseFactor):
    strategy_name = "mercury"
    factor_name = "qyh_mercury_20240530_8"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "qyh"  # 开发人员姓名
    factor_explain = "开盘价/成交量的T-1值除以10日集中度" # 因子逻辑解释
        ...
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            def f_calc_cct(factor_series):
                if abs(f_calc_sum(factor_series)) > 0:
                    ...
                    return np.nan
            df_ori[self.factor_name] = df_ori['factor'] / (df_ori['factor'].unstack().rolling(10,1).apply(lambda x : f_calc_cct(x)).stack()+1e-5)
            # -------------------------------------------------------------------------------------------------------------------
