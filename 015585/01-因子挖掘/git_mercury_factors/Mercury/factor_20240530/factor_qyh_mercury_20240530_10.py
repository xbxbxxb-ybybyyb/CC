...
class factor_qyh_mercury_20240530_10(BaseFactor):
    strategy_name = "mercury"
    factor_name = "qyh_mercury_20240530_10"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "qyh"  # 开发人员姓名
    factor_explain = "用前日vwap计算的涨跌幅的昨日值/20日集中度" # 因子逻辑解释
    ...
    t_1_factor_data = [
        {'name': 'MD_CHINA_STOCK_DAILY_WIND', # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 80, #注意为正数
         'column': ['low', 'pre_close','amt','close','vwap']
    }]  # T-N factor数据，格式如上S
