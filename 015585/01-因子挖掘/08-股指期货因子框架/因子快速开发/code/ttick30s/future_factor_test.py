...
class FactorTest:
    ...
    def load_data(self):
        ...
        if self.strategy_name == 'future':
            sft_basic_path = '/dfs/user/015585/00_股指期货策略/sft_basic_formal_20220801_20250430.h5'  # 这个文件里有label和所有因子
            self.end_date_ = (pd.Timestamp(str(self.end_date)) + pd.Timedelta(days=1)).strftime('%Y%m%d')  # 带时分秒之后，end_date要后延一天，否则最后一天取不到，因为默认是到end_date的0时0分0秒
            df = IO.read_data([self.start_date, self.end_date_], alt=sft_basic_path)
        else:
            logger.error('strategy_name error, must in [future]')
            raise TypeError
        ...
        self.basic_df = df[['value', 'class', 'month', 'year', 'tradingday']]

        all_factors_available = ['label']
    ...
    def factor_test(self, factor_df, result_path, factor_corr_test=False, generate_pdf=False):
        ...
        self.result_dic['factor_group_sort'] = self.get_factor_sort() # 按月分组，neptune是按天
    ...
    def check_score(self, factor_group_df, month_df, dir, label_test=['value', 'class']):
        ...
        diff_dic = {'value': {'reverse': [0.8, 0.6], 'diff': [0.07, 0.11]},
                    'class': {'reverse': [0.8, 0.6], 'diff': [0.09, 0.13]}}

        stability_dic = {'value': [0.02, 0.04],
                         'class': [0.015, 0.03]}
        s_s = 20 / len(month_df)
        ...
        for label in label_test:
            diff, reverse = [], []
            for date in factor_group_df.index.get_level_values(0).unique(): # 这里date代表month
            ...
            diff_res_df['%s_high_reverse' % (label)], diff_res_df['%s_mid_reverse' % (label)] = \
                diff_res_df['%s_diff' % (label)] * diff_dic[label]['reverse'][1], diff_res_df['%s_diff' % (label)] * diff_dic[label]['reverse'][0]
    ...
    def get_factor_sort(self, data=None, factor_group_num=10): # neptune分组为5，对应1700标的，这里按月有1万左右标的，分组改为10
        data = self.basic_df.copy() if (data is None) else data
        after_data = pd.DataFrame()
        for month in data['month'].unique():
            date_factor_data = data[data['month'] == month].copy()
        ...
        for month in after_data['month'].unique():
            month_after_data = after_data[after_data['month'] == month]
            for j in range(factor_group_num):
                tmp_result[(month, j)] = month_after_data[month_after_data['factor_group'] == j].mean()[self.label_list]
        tmp_result = tmp_result.T.reset_index()
        tmp_result['dt'] = tmp_result['index'].apply(lambda x: x[0]) # dt形如'22-08'
    ...
    def get_factor_information(self):
        ...
        factor_information['Factor Info'] = pd.Series({'Factor Name': self.factor_name,
                                                       ...
                                                       'Test Period': f'{self.start_date} - {self.end_date}',
