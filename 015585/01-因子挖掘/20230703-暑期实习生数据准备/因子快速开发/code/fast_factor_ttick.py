...
def run_factor(func,path,factor_name):
    file_list = os.listdir(path)
    file_list.sort()
    def calc(file):
        sys.stdout.write('\r' + file)
        sys.stdout.flush()
        df = pd.read_pickle('/data/user/015585/01-因子挖掘/20230703-暑期实习生数据准备/result/' + file)
        res = df.groupby(['dt', 'Ticker']).apply(lambda x: func(x))
        return res
    res_final = Parallel(n_jobs = 25)(delayed(calc)(file) for file in file_list)
    res_final = pd.concat(res_final,axis = 0)
    return pd.DataFrame(res_final,columns = [factor_name])
# 因子属性函数
...
def get_str_corr_factor(df):
    ...
    return str_
# 计算
result_df = pd.DataFrame(columns = ['factor_name','rank_IC'])
list_series = ['rcleanb',
                'cleanb2ttran',
                'cleanb2tran',
                'b2tran',
                                    ...
                                    for calc_i in dic_calc:
                                        ...
                                        for compare_i in dic_compare:
                                            ...
                                            factor_df0 = run_factor(func = factor_func,
                                                                    path = '/data/user/015585/01-因子挖掘/20230703-暑期实习生数据准备/result/',
                                                                    factor_name = factor_name,
                                                                    )
                                            res_label = pd.read_pickle('/data/user/015585/01-因子挖掘/20230703-暑期实习生数据准备/code/low_frequency.pkl')[['label']]
                                            res_label['factor'] = factor_df0[factor_name]
                                            corr = res_label.corr(method = 'spearman').iloc[0,1]
                                            dic_factor = pd.DataFrame()
                                            dic_factor.loc[factor_name,'rank_IC'] = corr
                                            print(corr)
                                            dic_factor.to_pickle('/data/user/015585/01-因子挖掘/20230703-暑期实习生数据准备/因子快速开发/回测结果/' +factor_name+'.pkl')
                                            result_df.loc[factor_name,'rank_IC'] = corr
                                            # print(result_df)