...
class strongFactorTest:
    def __init__(self, start_date, end_date, segment_number=20, style_list=['freeturn', 'ZT_Time'], cal_mi = ['value', 'class', 'mixed'],
                 ...
                 first_is_zt=False, open_is_zt=False, Flag_SH_SZ=None,
    ...
    def load_data(self):
        df = IO.read_data([self.start_date, self.end_date],alt='/dfs/group/800463/data/project1_public/factor_lib_metis/sft_update_metis.h5')

        # 样本筛选
        df['first_is_zt'] = (df['high_price'] >= (df['ul_price']))
        ...
        metis_time_filter=(df['trigger_time'] <= self.time_interval[1])

        all_filter = time_interval_filter & open_is_zt_filter&low_open_filter&after_not_ul_len_filter&low_price_filter\
                     & first_is_zt_filter& last_is_zt_filter& sh_sz_filter&metis_time_filter
        df = df[all_filter]
    ...
    def factor_test(self, factor_df, result_path, factor_corr_test=False, generate_pdf=True, tag=''):
        ...
        # 新增2019IC
        df_2019 = self.basic_df[['factor', 'value']].sort_values('factor')
        self.result_dic['2019IC'] = df_2019[df_2019.index.get_level_values(0) >pd.Timestamp('20190101')].corr(method='spearman').loc['factor','value']
