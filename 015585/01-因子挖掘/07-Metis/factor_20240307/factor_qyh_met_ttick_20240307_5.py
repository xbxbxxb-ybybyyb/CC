...
import pandas as pd
# zcz,dtj
# 首次逼近涨停价后，挂买均价对应涨跌幅在活跃/不活跃时候的变化值得差异
# 27，-0.045，-0.048
factor_name = 'qyh_met_ttick_20240307_5'#
def factor_qyh_met_ttick_20240307_5(tick_df, return_fillna_dic=False):
    ...
    res1 = tick_df1['factor'].head(1).mean() - tick_df1['factor'].tail(1).mean() if not tick_df1['factor'].empty else np.nan
    res2 = tick_df2['factor'].head(1).mean() - tick_df2['factor'].tail(1).mean() if not tick_df2['factor'].empty else np.nan
    res = res2 - res1
