...
sft_basic_path = '%s/sft_init_europa.h5'%(res_path) #初始地址
...
#读取因子列表
df_dic = pd.read_excel('/data/user/018107/factor_zoo1/alternate_factor_inf.xlsx', sheet_name=None)
...
for test_date in test_date_list[start_index:]:
    ...
    for index, inf in day_factor_inf.iterrows():
        ...
        all_factor_df1 = all_factor_df[(all_factor_df['ZT_Time'] >= 93000000) & (all_factor_df['ZT_Time'] <= 143000000) & (all_factor_df['open_is_zt'] == 0)
            ...
            & (all_factor_df['high_price'] < (all_factor_df['trigger_price'])) & (all_factor_df['last_is_zt'] == 0)&(all_factor_df['last_buy_rise'] <= 0.025)].copy()
