...
max_corr = 0.7
factor_data_path='/dfs/user/018107/factor_zoo1/all_factor/metis/'#因子数据
pre_check_path= '/dfs/user/018107/factor_zoo1/all_precheck/metis/'#预检测
factor_test_path='/dfs/user/018107/factor_zoo1/all_factortest/metis/all/'#因子报告
res_path = '/dfs/user/018107/factor_zoo1/factor_lib_metis/' #结果地址
res_public_path = '/dfs/group/800463/data/project1_public/factor_lib_metis/' #公共结果地址
sft_basic_path = '%s/sft_init_metis.h5'%(res_path) #初始地址
jupiter_cutdate=20231219
all_factor_for_jupiter=pd.read_pickle('/data/user/018107/factor_zoo1/factor_bank/jupiter/20231219/all_factor_df.pkl').loc[pd.Timestamp(str(in_interval[0])):pd.Timestamp(str(out_interval[1]))]

#读取jupiter因子列表
df_dic = pd.read_excel('/dfs/user/018107/factor_zoo1/alternate_factor_inf.xlsx', sheet_name=None)
...
#读取metis因子列表
metis_inf=pd.read_excel('/dfs/user/018107/factor_zoo1/metis_alternate_factor_inf.xlsx')
...
for test_date in test_date_list[start_index:]:
    ...
    if last_test_date is None:
        ...
        check_res_inf = pd.read_excel(res_path + 'check_res/check_res_tot_metis_%s.xlsx' % (last_test_date))[check_res_cols]
    ...
    for index, inf in day_factor_inf.iterrows():
        ...
        # 样本筛选
        all_factor_df1 = all_factor_df[(all_factor_df['ZT_Time'] >= 93000000) & (all_factor_df['ZT_Time'] <= 143000000) & (all_factor_df['open_is_zt'] == 0)
            & (all_factor_df['T_o2pre'] >= -0.05) & (all_factor_df['after_not_ul_len'] > 10) & (all_factor_df['pre_close'] >= 2)
            & (all_factor_df['high_price'] < (all_factor_df['ul_price'])) & (all_factor_df['last_is_zt'] == 0)&(all_factor_df['trigger_time']<= 143000000)].copy()
    ...
    check_res_inf_.to_excel(res_path + 'check_res/check_res_tot_metis_%d.xlsx' % (test_date),index=False)
    ...
    check_res_inf_sta.to_excel(res_path + 'check_res/check_res_tot_metis_sta.xlsx' % (test_date),index=False)
