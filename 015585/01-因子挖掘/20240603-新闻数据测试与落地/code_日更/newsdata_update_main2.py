...
'''
用于信息技术部新闻数据的日度更新
1、每天22：00运行，更新当日0点到20：45点的数据
2、次日每天3：00运行，更新前一日20：45点到23点59的数据
以上包括格式转换、正文正则化，不包括内容去重；默认只更新ITAINEWS
'''
...
def main_download_news_data(start_date,end_date,out_path1,out_path2,need_content = True):
    ...
    for date in date_list:
        ...
        #     continue
        try:
            data = nd.get_ai_news_data(start_date="{} 20:45:00".format(str(date).split(' ')[0]), end_date="{} 23:59:59".format(str(date).split(' ')[0]))
            print(date,len(data))
            ...
            data_ori = pd.read_pickle('{}{}.pkl'.format(out_path1,str(date).split(' ')[0].replace('-','')))
            data = pd.concat([data_ori,data])
...
if end_date == start_date:
    try:
        ...
        message_to_qyh_wys = '{}数据22:00~24:00新闻补充完成，新闻数量：{}，22：00以后新闻数量：{}，正文大于20的新闻数量：{}，新闻起始时间：{}，新闻截止时间：{}，df长度：{}'\
            .format(str(end_date).split( )[0],
                    len(set(df_check['id'])),
                    len(set(df_check[df_check['pubtime'] >= pd.Timestamp('{} 22:00:00'.format(str(end_date).split( )[0]))]['id'])),
    ...
    send_message(message_to_qyh_wys, users=['015585','003371','022325'])