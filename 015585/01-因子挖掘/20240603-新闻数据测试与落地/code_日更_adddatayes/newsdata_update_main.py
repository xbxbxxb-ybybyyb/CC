...
'''
用于信息技术部新闻数据的日度更新
1、每天20：00运行，更新当日0点到20：00点的数据
2、次日每天3：00运行，更新前一日20:00点到23点59的数据
以上包括格式转换、正文正则化，不包括内容去重；默认只更新ITAINEWS，2025年5月加入DATAYES
'''
cut_time = '19:59:59'
print(f'时间：{cut_time}前')
# ==================================================信息技术部财汇新闻===========================================================================================
...
def get_datayes_basicinfo(date):
    ...
    for i in [1,2,3]: # 3种新闻类型
        df_i = nd.get_datayes_news(f'{date_} 00:00:00', f'{date_} {cut_time}', i, date_field='pubdate')
        df_i['newstype'] = i
...
def get_datayes_stock(date):
    if '-' not in date:
        ...
        date_ = date
    df_stock = nd.get_datayes_company_score_news(start_date=f'{date_} 00:00:00', end_date=f'{date_} {cut_time}', )
...
def main_download_news_data(start_date,end_date,out_path,need_content = True):
    ...
    for date in date_list:
        try:
            data = nd.get_ai_news_data(start_date=f"{str(date).split(' ')[0]} 00:00:00", end_date= f"{str(date).split(' ')[0]} {cut_time}")
...
if end_date == start_date:
    try:
        ...
        message_to_qyh_wys = '{}数据下载完成(20:00前)，新闻数量：{}，正文大于20的新闻数量：{}，新闻起始时间：{}，新闻截止时间：{}，df长度：{}'\
