...
for year in [str(x) for x in range(2024,2024+1)]:
    ...
    md_data = IO.read_data([int(str(year + '0601')), int(str(year + '0930'))], columns=['amt', 'high','open','close','pre_close','vwap','adjfactor'],
    ...
    for i in os.listdir(path):
        if i.startswith(year):
            ...
            data_i['ticker'] = data_i['ticker'].apply(lambda x : [i + '.SH' if i.startswith('6') else i + '.SZ' for i in eval(x)] if type(x) == str else [])
            data_i = split_stock_tags(data_i)
            news_data = pd.concat([news_data,data_i[['newsID','effectiveTime','new_ticker','newsTitle']]])
    news_data = news_data.rename(columns = {'new_ticker':'ticker'})
    news_data['dt'] = news_data['effectiveTime'].apply(lambda x :pd.Timestamp(str(x).split(' ')[0]))
    #
    ...
    df_tl_num = pd.DataFrame(res.groupby(['dt_x', 'Ticker'])['is_value2'].sum()).query('is_value2 >= 1')
    ...
    #     df_IT_num.columns = ['IT_news']
    #     df_tl_num = pd.DataFrame(res.groupby(['dt_x', 'Ticker'])['is_value1'].sum()).query('is_value1 >= 1')
    #     df_tl_num = pd.merge(df_tl_num, df_IT_num, left_index=True, right_index=True, how='left')
    #     print('T日日均增量覆盖股票数：', len(df_tl_num[df_tl_num['IT_news'].isna()]) / len(set(md_data['dt'])))
    # 统计日均新闻数
    print(year,'统计日均新闻数')
    print(news_data[~news_data['newsID'].duplicated()].groupby('dt').count().mean())
    # 统计日均覆盖A股数 & 日均新闻数（A股）
    ...
    print(news_data_tmp.groupby('dt')['newsID'].apply(lambda x : len(set(x))).mean())
    ...
    # news_data_dt_stock.columns = ['stock_set_tl']
    ...
    # news_data_dt_stock['length_more_stock'] = (news_data_dt_stock['stock_set_tl'] - news_data_dt_stock['stock_set_IT'].apply(lambda x : eval(x))).apply(lambda x : len(x))
    ...
    #         df_delsimi_date = pd.read_pickle('/dfs/group/800463/data/news_data/datayes_basicinfo_delsimi/' + file)
    #         for newsID in df_delsimi_date[df_delsimi_date['max_similarity_ITnews'] >= 0.8]['newsID']:
    #             list_delsimi_id.append(newsID)
    # res = res[res['newsID'].isin(list_delsimi_id)]
