...
res_dic = {}
for year in [str(x) for x in range(2016,2022+1)]:
    ...
    path = '/dfs/group/800463/data/news_data/XNEWS_basicinfo/'
    news_data = pd.DataFrame()
    for i in os.listdir(path):
        if i.startswith(year):
            # print(i)
            news_data = pd.concat([news_data,pd.read_pickle(path + i)[['newscode','newstime','symbol','newstitle']]])
    news_data['dt'] = news_data['newstime'].apply(lambda x :pd.Timestamp(str(x).split(' ')[0]))
    #
    res = pd.merge(md_data, news_data, left_on='Ticker', right_on='symbol', how='left')
    ...
    pd.DataFrame(res.groupby(['dt_x', 'Ticker'])['is_value2'].sum()).query('is_value2 >= 1').to_pickle(
        '/data/user/015585/01-因子挖掘/20240603-新闻数据测试与落地/file/res_AINEWS_T_{}.pkl'.format(year))
    ...
    # # 统计日均新闻数
    # print(year,'统计日均新闻数')
    # print(news_data[~news_data['newscode'].duplicated()].groupby('dt').count().mean())
    # # 统计日均覆盖A股数 & 日均新闻数（A股）
    ...
    # stock_list.sort()
    # news_data_tmp = news_data[~news_data['symbol'].isna()]
    # news_data_tmp['is_A_stock'] = news_data_tmp['symbol'].isin(stock_list)
    # news_data_tmp = news_data_tmp[news_data_tmp['is_A_stock']]
    # print(news_data_tmp.groupby('dt')['symbol'].apply(lambda x : len(set(x))).mean())
    # print(news_data_tmp.groupby('dt')['newscode'].apply(lambda x : len(set(x))).mean())