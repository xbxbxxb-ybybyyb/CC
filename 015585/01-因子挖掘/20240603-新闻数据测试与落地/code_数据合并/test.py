...
def transfer_DATAYES(date):
    ...
    if os.path.exists(path + date + '.h5'):
...
def combo_main(date):
    ...
    df.to_pickle('/dfs/group/800463/data/news_data/' + date +'.pkl',compression='gzip')
    return
start_date = '20240628'
end_date = '20240628'
...
#
# factor_df_list = Parallel(n_jobs=1)(delayed(combo_main)(date) for date in date_list)
tmp = pd.read_pickle('/dfs/group/800463/data/news_data/20240628.pkl', compression = 'gzip')
print(tmp.groupby('resource').count())