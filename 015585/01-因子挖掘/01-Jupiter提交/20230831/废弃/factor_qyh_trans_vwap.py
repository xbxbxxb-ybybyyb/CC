...
# 价格凸性
#
factor_name = 'qyh_trans_vwap'#
def factor_qyh_trans_vwap(trans_df, return_fillna_dic=False):
    ...
    trans_df = trans_df[trans_df['MDTime'] <= trans_df['MDTime'].max() - 5000]
    trans_df = trans_df[trans_df['TradePrice'] > 0]
    trans_df = trans_df[trans_df['TradeBSFlag'] == 1]
    res = trans_df['TradeMoney'].sum() / trans_df['TradeQty'].sum()
    res = res/pre_close-1
