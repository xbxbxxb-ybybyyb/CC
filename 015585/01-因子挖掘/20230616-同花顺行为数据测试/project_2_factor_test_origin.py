...
class pj2FactorTest:
    ...
    def load_data(self):
        ...
        sft_basic_path = '/data/user/018107/factor_zoo2/warehouse/warehouse_931/sft_basic_formal_931_20160101_20181231.h5'
        ...
        if buy_type == 'o':
            if ret_type == 'label_od102o':
                all_filter = all_filter & ((df['T_day_first_ZT_Time'] <= 93000000) == False) & (df[ret_type].notnull()) & available_for_reaction_filter
            # all_filter = all_filter & (df['t2o'].notnull())
        if buy_type == 'od1':
            if ret_type == 'label_td12od1':
                all_filter = all_filter & ((df['T_day_first_ZT_Time'] <= 93100000) == False) & (df[ret_type].notnull()) & available_for_reaction_filter
        ...
        if buy_type == 'od30':
            if ret_type == 'label_od602od30':
                all_filter = all_filter & ((df['T_day_first_ZT_Time'] <= 100000000) == False) & (df[ret_type].notnull())  & available_for_reaction_filter
        if buy_type == 'od10':
            if ret_type == 'label_od302od10':
                all_filter = all_filter & ((df['T_day_first_ZT_Time'] <= 94000000) == False) & (df[ret_type].notnull())  & available_for_reaction_filter
        if buy_type == 'od180':
            if ret_type == 'label_od2402od180':
                all_filter = all_filter & ((df['T_day_first_ZT_Time'] <= 140000000) == False) & (df[ret_type].notnull())  & available_for_reaction_filter
        if buy_type == 'to':
