...
md = IO.read_data([s.tradingday(str(lastdate), -100)[0], lastdate], columns=['pre_close', 'close', 'amt', 'high']
...
md['bjs'] = (md.reset_index()['Ticker'].apply(lambda x: x[-2:] == 'BJ')).values
md['dl_price'] = np.floor(md['pre_close'] * 100 * 0.9 + 0.5) / 100
md['dl_price'][md['zcz']] = np.floor(md['pre_close'] * 100 * 0.8 + 0.5) / 100
md['dl_price'][md['bjs']] = np.ceil(md['pre_close'] * 100 * 0.7 - 1e-8) / 100

# md['close_is_dt'] = (md['close'] == md['dl_price']) & (md['high'] == md['dl_price'])
...
excel_saver({'Sheet1': pre_dt_df}, path_user + 'pre_dt_list_%s.xlsx' % date, index=False)
