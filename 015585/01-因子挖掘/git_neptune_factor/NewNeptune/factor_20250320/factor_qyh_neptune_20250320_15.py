...
class factor_qyh_neptune_20250320_15(BaseFactor):
    strategy_name = "neptune"
    factor_name = "qyh_neptune_20250320_15"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "qyh"  # 开发人员姓名
    factor_explain = "abspct除以10日偏度" # 因子逻辑解释
        ...
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            def f_calc_skew(factor_series):
                ...
                if n > 3:
                    ...
                    skew = np.nan
                return skew
            df_ori['factor'] = (abs(df_ori['pct_chg']) * df_ori['turn']).apply(lambda x : round_(x,4))
            df_ori = df_ori[df_ori['factor'] > 1e-8]
            df_ori['factor1'] = df_ori['factor'].unstack().rolling(10,5).apply(f_calc_skew).stack().apply(lambda x : round_(x,4)).replace(0,np.nan)
            df_ori['factor2'] = df_ori['factor'] / df_ori['factor1']
