...
class factor_qyh_neptune_20250320_30(BaseFactor):
    strategy_name = "neptune"
    factor_name = "qyh_neptune_20250320_30"
    fill_na_value = 1
    need_pre_calculate_T_N = True
    owner = "qyh"  # 开发人员姓名
    factor_explain = "h/v除以60日变异系数" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['factor'] = df_ori['factor'] / df_ori['factor'].unstack().rolling(60,5).apply(f_calc_cv).stack().apply(lambda x : round_(x,6)).replace(0,np.nan)
