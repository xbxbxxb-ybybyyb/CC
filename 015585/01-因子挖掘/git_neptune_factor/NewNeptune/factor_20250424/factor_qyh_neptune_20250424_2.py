...
class factor_qyh_neptune_20250424_2(BaseFactor):
    owner = 'qyh'
    strategy_name = "neptune"
    factor_name = 'qyh_neptune_20250424_2'
    fill_na_value = 0
    need_pre_calculate_T_N = True
    factor_explain = "amt的5m最大值的20日变化" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True:
            ...
            df = df_amt
            res = (df).max(axis=1)
            res = res.unstack().rolling(20,2).apply(f_calc_change).stack().to_frame()
