...
class factor_qyh_neptune_20250515_15(BaseFactor):
    strategy_name = "neptune"
    factor_name = "qyh_neptune_20250515_15"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "qyh"  # 开发人员姓名
    factor_explain = "财务-速动比率" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            #
            df_balancesheet['factor'] = (df_balancesheet['TOT_CUR_ASSETS'].fillna(0) - df_balancesheet['INVENTORIES'].fillna(0)) / df_balancesheet[
                'TOT_CUR_LIAB'].replace(0, np.nan)
            res = df_balancesheet.groupby(['dt','Ticker'])['factor'].apply(lambda x : x.tail(4).min()).to_frame(name = self.factor_name)
