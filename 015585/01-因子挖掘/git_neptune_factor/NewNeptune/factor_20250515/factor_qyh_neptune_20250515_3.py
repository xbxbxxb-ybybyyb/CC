...
class factor_qyh_neptune_20250515_3(BaseFactor):
    owner = 'qyh'
    strategy_name = "neptune"
    factor_name = 'qyh_neptune_20250515_3'
    fill_na_value = 0
    need_pre_calculate_T_N = True
    factor_explain = "lnpct max 20" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True:
            ...
            df = df_low / df_vwap
            res = (df).max(axis=1)
            res = res.unstack().rolling(20,1).sum().stack().to_frame()
