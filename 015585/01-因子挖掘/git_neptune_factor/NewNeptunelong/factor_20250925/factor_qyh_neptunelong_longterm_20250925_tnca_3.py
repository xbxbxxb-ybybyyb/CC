...
class factor_qyh_neptunelong_longterm_20250925_tnca_3(BaseFactor):
    strategy_name = "neptunelong"
    factor_name = "qyh_neptunelong_longterm_20250925_tnca_3"
    fill_na_value = ('industry_mean',0)
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            res = (df_balancesheet['TOT_NON_CUR_ASSETS']).groupby(['dt','Ticker'])\
                .apply(lambda x : x.tail(4).min()).to_frame(name='非流动资产统计量')
