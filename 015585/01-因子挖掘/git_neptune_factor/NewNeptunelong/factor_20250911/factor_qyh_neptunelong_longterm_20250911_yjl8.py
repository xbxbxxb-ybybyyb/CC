...
class factor_qyh_neptunelong_longterm_20250911_yjl8(BaseFactor):
    strategy_name = "neptunelong"
    factor_name = "qyh_neptunelong_longterm_20250911_yjl8"
    fill_na_value = ('industry_median',0)
    need_pre_calculate_T_N = True
    owner = "qyh"  # 开发人员姓名
    factor_explain = "财务：应计量 只取年报" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            res1 = (df_balancesheet['单季度平均资产总额']).groupby(['dt','Ticker'])\
                .apply(lambda x : x.tail(1).mean()).to_frame(name='单季度平均资产总额')
            res2 = (df_cf[f'折旧摊销总额']).groupby(['dt','Ticker'])\
                .apply(lambda x : x.tail(1).mean()).to_frame(name='折旧摊销总额')
            res3 = (df_balancesheet[f'流动负债diff'].fillna(0) + df_balancesheet[f'流动资产diff'].fillna(0)).groupby(['dt','Ticker'])\
                .apply(lambda x : x.tail(1).mean()).to_frame(name='流动资产与负债diff')
            res4 = (df_balancesheet['FIX_ASSETS']).groupby(['dt','Ticker'])\
                .apply(lambda x : x.tail(1).mean()).to_frame(name='固定资产')
            res5 = (df_income['OPER_REV_diff']).groupby(['dt','Ticker'])\
                .apply(lambda x : x.tail(1).mean()).to_frame(name='营业收入diff')
            res = pd.concat([res1,res2,res3,res4,res5],axis=1)
            res[self.factor_name] = (res['折旧摊销总额'].fillna(0) + res['流动资产与负债diff'].fillna(0)) / res['单季度平均资产总额'].replace(0,np.nan)
