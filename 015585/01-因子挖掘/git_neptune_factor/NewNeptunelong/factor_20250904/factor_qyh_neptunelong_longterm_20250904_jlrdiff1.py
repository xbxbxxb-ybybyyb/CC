...
class factor_qyh_neptunelong_longterm_20250904_jlrdiff1(BaseFactor):
    strategy_name = "neptunelong"
    factor_name = "qyh_neptunelong_longterm_20250904_jlrdiff1"
    fill_na_value = ('industry_median',0)
    need_pre_calculate_T_N = True
    owner = "qyh"  # 开发人员姓名
    factor_explain = "财务：净利润diff" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_income['净利润diff'] = df_income['NET_PROFIT_EXCL_MIN_INT_INC_diff'].groupby(['dt','Ticker']).diff()
            res1 = (df_income['净利润diff']).groupby(['dt','Ticker'])\
