...
class factor_qyh_neptunelong_longterm_20250918_mc_1(BaseFactor):
    strategy_name = "neptunelong"
    factor_name = "qyh_neptunelong_longterm_20250918_mc_1"
    fill_na_value = ('industry_median',0)
    need_pre_calculate_T_N = True
    owner = "qyh"  # 开发人员姓名
    factor_explain = "财务：货币资金过去1年统计量" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            res = (df_balancesheet['MONETARY_CAP']).groupby(['dt','Ticker'])\
                .apply(lambda x : f_calc_cv(x.tail(4))).to_frame(name='货币资金统计量')

            res[self.factor_name] = res['货币资金统计量']
