...
def _rank_data(x: Tensor, nan_mask: Tensor) -> Tensor:
    ...
    eq = x[:, None] == x[:, :, None]  # 处理 x 中的相等值              # [d, s, s]
...
def _batch_pearsonr_given_mask(
    ...
    x_mean, x_std, x = masked_mean_std(x, n, mask)
    y_mean, y_std, y = masked_mean_std(y, n, mask)
    cov = (x * y).nansum(dim=1) / n - x_mean * y_mean
    stdmul = x_std * y_std
    stdmul[(x_std < 1e-3) | (y_std < 1e-3)] = 1
    corrs = cov / stdmul
    if  torch.sum(~mask)< 2/3*x.shape[1]*x.shape[0]:  # 如果非nan的股票数不足2/3的话，也不能要
        return torch.zeros(corrs.shape).to(corrs.device)
    if corrs[0] == 0:
        print('mistake')
    return corrs
