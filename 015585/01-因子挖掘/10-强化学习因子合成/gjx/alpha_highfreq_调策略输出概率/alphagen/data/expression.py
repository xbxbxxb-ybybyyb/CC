...
class PairRollingOperator(Operator):
    ...
    def evaluate(self, data: StockData) -> Tensor:
        ...
        return self._apply(lhs[:,:,1:], rhs[:,:,1:])  # (L, S) 左开右闭
...
class Filter(FilterOperator):
    def _apply(self, operand: Tensor):
        ...
        if self._div_rule == '<mkt_mean':
            quan = torch.nanquantile(operand, 0.25, dim=1)[:,None,:]
            values = torch.where(operand < quan, operand, sub_tensor)
        if self._div_rule == '>mkt_mean':
            quan = torch.nanquantile(operand, 0.75, dim=1)[:, None, :]
            values = torch.where(operand > quan, operand, sub_tensor)
        if self._div_rule == '>ts_mean':
            quan = torch.nanquantile(operand, 0.25, dim=2)[..., None]
            values = torch.where(operand < quan, operand, sub_tensor)
        if self._div_rule == '<ts_mean':
            quan = torch.nanquantile(operand, 0.75, dim=2)[..., None]
            values = torch.where(operand > quan, operand, sub_tensor)
...
class AddDivSub(BinaryOperator):
    def _apply(self, lhs: Tensor, rhs: Tensor) -> Tensor:
        if len(lhs.shape) < len(rhs.shape):
            lhs = lhs[..., None]
        elif len(lhs.shape) > len(rhs.shape):
            rhs = rhs[..., None]
        epsilon = torch.tensor(1e-8, device=lhs.device)
        return (lhs - rhs)/ (rhs + lhs + epsilon)
