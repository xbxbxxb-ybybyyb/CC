...
class StockData:
    ...
    def _Dataloader(self, exprs, trade_date):
        ...
        return filter_df, target.loc[target.index.get_level_values(0).isin(trade_date)]
    ...
    def _get_data(self) -> Tuple[torch.Tensor, pd.Index, pd.Index]:
        ...
        df, target = self._load_exprs(features)
        dates = sorted((set(df.index.get_level_values(0))))  # 不能用下面这个，把切片前的也放进来了
        stock_ids = df.index.levels[1]
        self.n_stocks = 500
        values = df.values
        values = values.reshape((len(dates), self.n_stocks, -1,len(features)))  # type: ignore
        values = values.transpose(0,1,3,2)
        target = target.values
        target = target.reshape(len(dates), self.n_stocks)
        return (
            torch.tensor(values, dtype=torch.float, device=self.device),
            torch.tensor(target, dtype=torch.float, device=self.device),
            dates,
            stock_ids,
        )
        # 日期 时刻 特征数 股票数
