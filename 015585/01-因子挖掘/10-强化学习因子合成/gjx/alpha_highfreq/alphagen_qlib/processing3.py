...
def con_format(minute, second):
    # 将时分秒转化为字符串并补零保证两位数
    hour_str = str(9)  # 小时补零到2位
    minute_str = str(minute).zfill(2)  # 分钟补零到2位
    second_str = str(second).zfill(2)  # 秒钟补零到2位

    # 组合并返回格式为93000000样式
    return int(f"{hour_str}{minute_str}{second_str}000")


def replace_time(x):
    all_sec = (int(str(x)[1:3]) * 60 + int(str(x)[3:5]))  # 分情况讨论，要么整除，要么相差1以内
    if all_sec % 3 == 0:
        all_sec_deal = all_sec // 3 * 3
    elif all_sec % 3 == 1:
        all_sec_deal = all_sec // 3 * 3
    else:
        all_sec_deal = (all_sec // 3 + 1) * 3
    min = all_sec_deal // 60
    sec = all_sec_deal - min * 60
    return con_format(min, sec)
...
for pkl_file in tqdm(pkl_files):
    try:
        ...
        # 删除一些列
        df = df.drop(
            columns=['TradingPhaseCode', 'Buy3Price', 'Buy4Price', 'Buy5Price', 'Buy6Price', 'Buy7Price', 'Buy8Price',
                     'Buy9Price', 'Buy10Price', 'Sell3Price', 'Sell4Price', 'Sell5Price', 'Sell6Price',
                     'Sell7Price', 'Sell8Price', 'Sell9Price', 'Sell10Price', 'Buy3OrderQty',
                     'Buy4OrderQty', 'Buy5OrderQty', 'Buy6OrderQty', 'Buy7OrderQty',
                     'Buy8OrderQty', 'Buy9OrderQty', 'Buy10OrderQty', 'Sell3OrderQty', 'Sell4OrderQty',
                     'Sell5OrderQty', 'Sell6OrderQty', 'Sell7OrderQty', 'Sell8OrderQty',
                     'Sell9OrderQty', 'Sell10OrderQty', 'Buy3NumOrders', 'Buy4NumOrders', 'Buy5NumOrders',
                     'Buy6NumOrders', 'Buy7NumOrders', 'Buy8NumOrders',
                     'Buy9NumOrders', 'Buy10NumOrders', 'Sell3NumOrders', 'Sell4NumOrders', 'Sell5NumOrders',
                     'Sell6NumOrders', 'Sell7NumOrders', 'Sell8NumOrders', 'Sell9NumOrders',
                     'Sell10NumOrders'])

        df = df[df['MDTime']>=93000000]
        df = df[df['MDTime']<94000000]

        # print(df.loc[('2018-01-02', '603996.SH'), 'MDTime'].values)
        time_group = ('9:30', '9:40')
        interval = '3s'
        times = pd.date_range(start=time_group[0], end=time_group[1], freq=interval).time
        times = [time for time in times if time != pd.Timestamp('9:15').time()]
        ...
        # 对 "时刻" 列进行替换操作
        df['MDTime'] = df['MDTime'].apply(replace_time)
        times = sorted(list(set(df['MDTime'])))
        time_filter1 = times
        # print(time_filter1)

        dates = sorted(list(set(df.index.get_level_values(0))))
        # df = df.reset_index(level = 0) # 删除日期索引
        df = df.set_index('MDTime', append=True)
        stocks = sorted(list(set(df.index.get_level_values(1))))

        new_index = pd.MultiIndex.from_product([dates, stocks, time_filter1], names=('dt', 'Ticker', 'MDTime'))
        full_index_df = pd.DataFrame(index=new_index)
        df = full_index_df.merge(df, left_index=True, right_index=True, how='left')
        ...
        df['label'] = pd.cut(df['MDTime'], bins=time_filter, labels=time_filter[1:], right = True)
        groups = df.groupby(['dt', 'Ticker', 'label'], observed=False)
        df_piece = groups.apply(lambda x:x.iloc[-1])
        # 计算差分
        diff_cols = ['TotalVolumeTrade', 'TotalValueTrade', 'NumTrades']
        groupb1 = df_piece.groupby(['dt', 'Ticker'])
        first_values = groupb1[diff_cols].apply(lambda x:x.iloc[0])
        ...
        df_piece.loc[
            (df_piece.index.get_level_values('label') > 93000000) & (df_piece.index.get_level_values('label') <= 93100000), [
                'TotalVolumeTrade', 'TotalValueTrade', 'NumTrades']] /= 3
        df_piece.loc[
            df_piece.index.get_level_values('label') > 93900000, ['TotalVolumeTrade', 'TotalValueTrade', 'NumTrades']] /= 3
        df_piece.loc[
            (df_piece.index.get_level_values('label') > 93100000) & (df_piece.index.get_level_values('label') <= 93900000), [
        ...
        # df_piece = df_piece.reset_index(level='label')
        df_piece = df_piece.drop(columns=['label', 'MDTime'])
        if file_path[-12:] == '20181024.pkl':
            ddf = df_piece.loc[('2018-10-24', '603996.SH') :]
            df_piece = pd.concat((df_piece, ddf))  # 这一步做不了？？ 太大了？在细分里面做
        # df_piece.set_index('MDTime', append=True, inplace=True)
        data.append(df_piece)
        # print(df_piece.columns)
    except:
        continue
...
# label.to_pickle('./label1.pkl')
#
# data = pd.read_pickle('./high_data1.pkl')
