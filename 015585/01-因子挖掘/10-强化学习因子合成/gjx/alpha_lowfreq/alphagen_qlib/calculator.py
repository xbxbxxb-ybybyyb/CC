...
class QLibStockDataCalculator(AlphaCalculator):
    def __init__(self, data: StockData, target: Optional[Expression]):
        ...
        if target is None:  # Combination-only mode
            ...
            self.idx2_arr = self.data.idx[start1:stop1, :]
            self.idx1_arr = torch.arange(self.idx2_arr.shape[0]).unsqueeze(1).expand(-1, self.idx2_arr.shape[1])


    def _calc_alpha(self, expr: Expression) -> Tensor:
        # 只保留每天那500支股票的因子值
        alpha_value = expr.evaluate(self.data)
        return normalize_by_day(alpha_value[self.idx1_arr,self.idx2_arr])

    def _calc_alpha_nonorm(self, expr: Expression) -> Tensor:
        # 只保留每天那500支股票的因子值
        alpha_value = expr.evaluate(self.data)
        return alpha_value[self.idx1_arr,self.idx2_arr]

    def _calc_IC(self, value1: Tensor, value2: Tensor) -> float:
        return batch_pearsonr(value1, value2).mean().item()

    def _calc_rIC(self, value1: Tensor, value2: Tensor) -> float:
        if isinstance(batch_spearmanr(value1, value2),int):
            print('a')
        return batch_spearmanr(value1, value2).mean().item()

    def _calc_rICIR(self, value1: Tensor, value2: Tensor) -> float:
        if batch_spearmanr(value1, value2).std().item() == 0:
            return 0
    ...
    def calc_single_alpha_nonorm(self, exprs: List[Expression]) -> Tensor:
        n = len(exprs)
        factors: List[Tensor] = [self._calc_alpha_nonorm(exprs[i]) for i in range(n)]
        return factors  # type: ignore
