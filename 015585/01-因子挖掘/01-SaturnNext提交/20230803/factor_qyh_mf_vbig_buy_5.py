...
s = FactorData()
factor_name = 'qyh_mf_vbig_buy_5'
def factor_qyh_mf_vbig_buy_5(start_date, end_date, IO, return_fillna_dic=False):
    if return_fillna_dic:
        # 返回因子为nan时的填充值
        return {factor_name: 2851,'data':['AShareMoneyFlow']}
    # 可修改的因子编写部分
    ...
    f_data[factor_name] = f_data['BUY_VALUE_EXLARGE_ORDER'].unstack().rolling(5,1).mean().stack()
