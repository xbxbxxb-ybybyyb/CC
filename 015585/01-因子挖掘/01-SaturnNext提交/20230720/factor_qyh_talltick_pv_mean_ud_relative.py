...
def factor_qyh_talltick_pv_mean_ud_relative(tick_df, return_fillna_dic=False):
    factor_name = 'qyh_talltick_pv_mean_ud_relative'
    ...
    pv1 = pv1 / tick_df1['VolumeTrade'].sum()
    pv2 = ((tick_df2['LastPx'] / tick_df2['pre_close'].max() - 1) * tick_df2['VolumeTrade']).sum()
    pv2 = pv2 / tick_df2['VolumeTrade'].sum()
    if zcz:
        pv1 = pv1/2
        pv2 = pv2/2
    factor_dict = {factor_name: (pv1-pv2)}
    # ---------------------------------------------------------------------------------------------------------------
