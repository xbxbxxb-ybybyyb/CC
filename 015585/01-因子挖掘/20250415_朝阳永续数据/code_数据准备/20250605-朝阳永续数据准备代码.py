...
# basic_file = pd.read_pickle('/dfs/user/023859/share_file/for_qyh/basic_file_zz1800_20160101_20191231.pkl')
basic_file = pd.read_pickle('/dfs/user/023859/share_file/for_qyh/basic_file_neptune_all_20160101_20191231.pkl')
...
file_dic = dict(zip(table_list, [IO.read_data([20120101,20250430], alt = path_dic[x]) for x in table_list]))
...
for type in table_list:
    if not os.path.exists(f'{save_path}{strategy}/{type.replace("DWD_EXP","xdb").lower()}'):
        ...
        print(f'{save_path}{strategy}/{type.replace("DWD_EXP","xdb").lower()}','已新建该目录')
for tradingday in tradingday_list:
    for type in table_list:
