...
# 逻辑：拉抬订单的每单成交额
# score:0,11
# High_small_ratio:0.03,32
factor_name = 'qyh_TTra_up_peramt'#
def factor_qyh_TTra_up_peramt(trans_df, return_fillna_dic=False):
    ...
    buy = buy_max - buy_min
    amt = trans_df[trans_df['TradeBuyNo'].isin(list(buy[buy>0.001].index))].groupby('TradeBuyNo')['TradeMoney'].sum().mean()
