...
import numpy as np
import pandas as pd
factor_name = 'qyh_ttick_v2p_h2_cv'#
def factor_qyh_ttick_v2p_h2_cv(tick_df, return_fillna_dic=False):
    ...
    mean = (tick_df['vwap']/tick_df['LastPx']).mean()
    std = (tick_df['vwap']/tick_df['LastPx']).std()
    cv = std / mean if round_(mean,5) != 0 else std
