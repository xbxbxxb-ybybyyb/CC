...
# 逻辑：T-5日的PLUS价格：(2Ct - Ht - Lt)/Ct-1的均值
#
#
import pandas as pd
import numpy as np
...
s = FactorData()
def factor_qyh_md_plus_price_5_mean(start_date, end_date, IO, return_fillna_dic=False):
    factor_name='qyh_md_plus_price_5_mean'
    ...
    f_data[factor_name] = (f_data['plus']*2 +
                           f_data['plus'].unstack().shift(1).stack()*2 +
                           f_data['plus'].unstack().shift(2).stack()*2 +
                           f_data['plus'].unstack().shift(3).stack()*2 +
                           f_data['plus'].unstack().shift(4).stack()*2)/(10)
