...
s = FactorData()
factor_name = 'qyh_md_20231102_4'
# zcz,dtj
# pct*turn的60日上涨时的离散程度
# 35,-0.069
#
def factor_qyh_md_20231102_4(start_date, end_date, IO, return_fillna_dic=False):
    ...
    df_ori['factor'] = df_ori['factor'].apply(lambda x :x if x >0 else np.nan)
    df_ori[factor_name] = df_ori['factor'].unstack().rolling(60,1).max().stack() / \
        df_ori['factor'].unstack().rolling(60,1).mean().stack()
    # -------------------------------------------------------------------------------------------------------------------
