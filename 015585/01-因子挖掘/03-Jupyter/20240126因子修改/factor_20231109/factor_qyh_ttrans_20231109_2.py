...
import pandas as pd
# dtj
# 最后100笔/500笔聚合后离散程度差异
# 35，0.07
factor_name = 'qyh_ttrans_20231109_2'#
def factor_qyh_ttrans_20231109_2(trans_df, return_fillna_dic=False):
    ...
    trans_df2 = trans_df.tail(500)
    #
    res1 = (trans_df1.groupby('TradeBuyNo').sum()['TradeMoney'].mean())\
           /(trans_df1.groupby('TradeBuyNo').sum()['TradeMoney'].max()+1e-5)
    res2 = (trans_df2.groupby('TradeBuyNo').sum()['TradeMoney'].mean())\
           /(trans_df2.groupby('TradeBuyNo').sum()['TradeMoney'].max()+1e-5)
