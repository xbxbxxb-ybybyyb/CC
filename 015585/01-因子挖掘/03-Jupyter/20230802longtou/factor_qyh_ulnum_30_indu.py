...
s = FactorData()
factor_name = 'qyh_ulnum_30_indu'
def factor_qyh_ulnum_30_indu(start_date, end_date, IO, return_fillna_dic=False):
    ...
    # 分行业排名
    in_data = IO.read_data([start_date, end_date],
                          columns=['Industry']
                          , alt='/data/group/800080/warehouse/prod/RISK/CHINA_STOCK/DAILY/STYLEFACTOR/RISK_CHINA_STOCK_DAILY_STYLEFACTOR.h5')
    df_ori['industry'] = in_data['Industry']
    df_ori['ctn_num_calc_indu_rank'] = df_ori.groupby(['dt','industry'])['cnt_num_n_transfer'].rank(ascending = False,method = 'min')
    df_ori.loc[df_ori['cnt_num_n_transfer'] == 0,'ctn_num_calc_indu_rank'] = 999 # 未有连板的不视为龙头
    f_data = pd.DataFrame(df_ori['ctn_num_calc_indu_rank'])
