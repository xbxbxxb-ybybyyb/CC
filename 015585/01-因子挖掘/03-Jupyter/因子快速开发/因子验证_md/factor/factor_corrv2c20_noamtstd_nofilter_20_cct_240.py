...
factor_name = 'corrv2c20_noamtstd_nofilter_20_cct_240'#
def factor_corrv2c20_noamtstd_nofilter_20_cct_240(start_date, end_date, IO, return_fillna_dic=False):
    if return_fillna_dic:
        return {factor_name: 0}
    factor_explain = "corrv2c20_noamtstd_nofilter_20_cct_240"
    ...
    df_ori[factor_name] = df_ori['factor'].unstack().rolling(20,1).apply(lambda x : f_calc_cct(x)).stack()
    df_ori[factor_name] = df_ori[factor_name] / df_ori['factor'].unstack().rolling(240,1).apply(lambda x : f_calc_cct(x)).stack()
