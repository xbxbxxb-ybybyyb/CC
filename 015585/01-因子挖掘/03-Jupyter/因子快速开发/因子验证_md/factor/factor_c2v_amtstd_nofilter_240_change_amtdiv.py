...
factor_name = 'c2v_amtstd_nofilter_240_change_amtdiv'#
def factor_c2v_amtstd_nofilter_240_change_amtdiv(start_date, end_date, IO, return_fillna_dic=False):
    if return_fillna_dic:
        return {factor_name: 0}
    factor_explain = "c2v_amtstd_nofilter_240_change_amtdiv"
    ...
    df_ori[factor_name] = df_ori['factor'].unstack().rolling(240,1).apply(lambda x : f_calc_change(x)).stack()
    df_ori["amt"] = df_ori['amt'].unstack().rolling(240,1).apply(lambda x : f_calc_change(x)).stack()
