...
factor_name = 'corrv2c20_noamtstd_nofilter_60_sum_240'#
def factor_corrv2c20_noamtstd_nofilter_60_sum_240(start_date, end_date, IO, return_fillna_dic=False):
    if return_fillna_dic:
        return {factor_name: 0}
    factor_explain = "corrv2c20_noamtstd_nofilter_60_sum_240"
    ...
    df_ori[factor_name] = df_ori['factor'].unstack().rolling(60,1).apply(lambda x : f_calc_sum(x)).stack()
    df_ori[factor_name] = df_ori[factor_name] / df_ori['factor'].unstack().rolling(240,1).apply(lambda x : f_calc_sum(x)).stack()
