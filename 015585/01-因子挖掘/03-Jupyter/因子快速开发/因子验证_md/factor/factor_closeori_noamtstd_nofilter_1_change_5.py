...
factor_name = 'closeori_noamtstd_nofilter_1_change_5'#
def factor_closeori_noamtstd_nofilter_1_change_5(start_date, end_date, IO, return_fillna_dic=False):
    if return_fillna_dic:
        return {factor_name: 0}
    factor_explain = "closeori_noamtstd_nofilter_1_change_5"
    ...
    df_ori[factor_name] = df_ori['factor'].unstack().rolling(1,1).apply(lambda x : f_calc_change(x)).stack()
    df_ori[factor_name] = df_ori[factor_name] / df_ori['factor'].unstack().rolling(5,1).apply(lambda x : f_calc_change(x)).stack()
