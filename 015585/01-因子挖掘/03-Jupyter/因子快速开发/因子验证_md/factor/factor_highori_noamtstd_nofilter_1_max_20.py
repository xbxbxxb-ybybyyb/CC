...
factor_name = 'highori_noamtstd_nofilter_1_max_20'#
def factor_highori_noamtstd_nofilter_1_max_20(start_date, end_date, IO, return_fillna_dic=False):
    if return_fillna_dic:
        return {factor_name: 0}
    factor_explain = "highori_noamtstd_nofilter_1_max_20"
    ...
    df_ori[factor_name] = df_ori['factor'].unstack().rolling(1,1).apply(lambda x : f_calc_max(x)).stack()
    df_ori[factor_name] = df_ori[factor_name] / df_ori['factor'].unstack().rolling(20,1).apply(lambda x : f_calc_max(x)).stack()
