...
factor_name = 'pct_amtstd_nofilter_10_min_amtdiv'#
def factor_pct_amtstd_nofilter_10_min_amtdiv(start_date, end_date, IO, return_fillna_dic=False):
    if return_fillna_dic:
        return {factor_name: 0}
    factor_explain = "pct_amtstd_nofilter_10_min_amtdiv"
    ...
    df_ori[factor_name] = df_ori['factor'].unstack().rolling(10,1).apply(lambda x : f_calc_min(x)).stack()
    df_ori["amt"] = df_ori['amt'].unstack().rolling(10,1).apply(lambda x : f_calc_min(x)).stack()
