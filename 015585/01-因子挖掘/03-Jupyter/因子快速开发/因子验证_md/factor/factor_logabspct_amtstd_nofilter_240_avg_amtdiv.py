...
factor_name = 'logabspct_amtstd_nofilter_240_avg_amtdiv'#
def factor_logabspct_amtstd_nofilter_240_avg_amtdiv(start_date, end_date, IO, return_fillna_dic=False):
    if return_fillna_dic:
        return {factor_name: 0}
    factor_explain = "logabspct_amtstd_nofilter_240_avg_amtdiv"
    ...
    df_ori[factor_name] = df_ori['factor'].unstack().rolling(240,1).apply(lambda x : f_calc_avg(x)).stack()
    df_ori["amt"] = df_ori['amt'].unstack().rolling(240,1).apply(lambda x : f_calc_avg(x)).stack()
