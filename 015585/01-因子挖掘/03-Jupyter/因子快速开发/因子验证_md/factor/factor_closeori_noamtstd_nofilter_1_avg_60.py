...
factor_name = 'closeori_noamtstd_nofilter_1_avg_60'#
def factor_closeori_noamtstd_nofilter_1_avg_60(start_date, end_date, IO, return_fillna_dic=False):
    if return_fillna_dic:
        return {factor_name: 0}
    factor_explain = "closeori_noamtstd_nofilter_1_avg_60"
    ...
    df_ori[factor_name] = df_ori['factor'].unstack().rolling(1,1).apply(lambda x : f_calc_avg(x)).stack()
    df_ori[factor_name] = df_ori[factor_name] / df_ori['factor'].unstack().rolling(60,1).apply(lambda x : f_calc_avg(x)).stack()
