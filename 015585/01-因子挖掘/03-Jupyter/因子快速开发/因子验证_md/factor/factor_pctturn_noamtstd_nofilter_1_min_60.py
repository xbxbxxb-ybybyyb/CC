...
factor_name = 'pctturn_noamtstd_nofilter_1_min_60'#
def factor_pctturn_noamtstd_nofilter_1_min_60(start_date, end_date, IO, return_fillna_dic=False):
    if return_fillna_dic:
        return {factor_name: 0}
    factor_explain = "pctturn_noamtstd_nofilter_1_min_60"
    ...
    df_ori[factor_name] = df_ori['factor'].unstack().rolling(1,1).apply(lambda x : f_calc_min(x)).stack()
    df_ori[factor_name] = df_ori[factor_name] / df_ori['factor'].unstack().rolling(60,1).apply(lambda x : f_calc_min(x)).stack()
