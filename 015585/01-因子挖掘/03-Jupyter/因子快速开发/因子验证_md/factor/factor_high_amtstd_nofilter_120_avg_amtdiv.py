...
factor_name = 'high_amtstd_nofilter_120_avg_amtdiv'#
def factor_high_amtstd_nofilter_120_avg_amtdiv(start_date, end_date, IO, return_fillna_dic=False):
    if return_fillna_dic:
        return {factor_name: 0}
    factor_explain = "high_amtstd_nofilter_120_avg_amtdiv"
    ...
    df_ori[factor_name] = df_ori['factor'].unstack().rolling(120,1).apply(lambda x : f_calc_avg(x)).stack()
    df_ori["amt"] = df_ori['amt'].unstack().rolling(120,1).apply(lambda x : f_calc_avg(x)).stack()
