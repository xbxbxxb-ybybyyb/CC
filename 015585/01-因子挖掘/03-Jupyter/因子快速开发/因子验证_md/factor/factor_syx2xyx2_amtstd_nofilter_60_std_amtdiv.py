...
factor_name = 'syx2xyx2_amtstd_nofilter_60_std_amtdiv'#
def factor_syx2xyx2_amtstd_nofilter_60_std_amtdiv(start_date, end_date, IO, return_fillna_dic=False):
    if return_fillna_dic:
        return {factor_name: 0}
    factor_explain = "syx2xyx2_amtstd_nofilter_60_std_amtdiv"
    ...
    df_ori[factor_name] = df_ori['factor'].unstack().rolling(60,1).apply(lambda x : f_calc_std(x)).stack()
    df_ori["amt"] = df_ori['amt'].unstack().rolling(60,1).apply(lambda x : f_calc_std(x)).stack()
