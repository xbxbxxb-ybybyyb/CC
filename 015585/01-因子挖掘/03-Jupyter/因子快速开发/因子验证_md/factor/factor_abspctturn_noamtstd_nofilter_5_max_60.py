...
factor_name = 'abspctturn_noamtstd_nofilter_5_max_60'#
def factor_abspctturn_noamtstd_nofilter_5_max_60(start_date, end_date, IO, return_fillna_dic=False):
    if return_fillna_dic:
        return {factor_name: 0}
    factor_explain = "abspctturn_noamtstd_nofilter_5_max_60"
    ...
    df_ori[factor_name] = df_ori['factor'].unstack().rolling(5,1).apply(lambda x : f_calc_max(x)).stack()
    df_ori[factor_name] = df_ori[factor_name] / df_ori['factor'].unstack().rolling(60,1).apply(lambda x : f_calc_max(x)).stack()
