...
factor_name = 'low_noamtstd_nofilter_5_cv_60'#
def factor_low_noamtstd_nofilter_5_cv_60(start_date, end_date, IO, return_fillna_dic=False):
    if return_fillna_dic:
        return {factor_name: 0}
    factor_explain = "low_noamtstd_nofilter_5_cv_60"
    ...
    df_ori[factor_name] = df_ori['factor'].unstack().rolling(5,1).apply(lambda x : f_calc_cv(x)).stack()
    df_ori[factor_name] = df_ori[factor_name] / df_ori['factor'].unstack().rolling(60,1).apply(lambda x : f_calc_cv(x)).stack()
