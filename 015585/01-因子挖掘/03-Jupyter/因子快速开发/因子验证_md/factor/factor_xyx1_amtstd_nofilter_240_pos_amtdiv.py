...
factor_name = 'xyx1_amtstd_nofilter_240_pos_amtdiv'#
def factor_xyx1_amtstd_nofilter_240_pos_amtdiv(start_date, end_date, IO, return_fillna_dic=False):
    if return_fillna_dic:
        return {factor_name: 0}
    factor_explain = "xyx1_amtstd_nofilter_240_pos_amtdiv"
    ...
    df_ori[factor_name] = df_ori['factor'].unstack().rolling(240,1).apply(lambda x : f_calc_pos(x)).stack()
    df_ori["amt"] = df_ori['amt'].unstack().rolling(240,1).apply(lambda x : f_calc_pos(x)).stack()
