...
factor_name = 'turn_noamtstd_nofilter_20_pos_120'#
def factor_turn_noamtstd_nofilter_20_pos_120(start_date, end_date, IO, return_fillna_dic=False):
    if return_fillna_dic:
        return {factor_name: 0}
    factor_explain = "turn_noamtstd_nofilter_20_pos_120"
    ...
    df_ori[factor_name] = df_ori['factor'].unstack().rolling(20,1).apply(lambda x : f_calc_pos(x)).stack()
    df_ori[factor_name] = df_ori[factor_name] / df_ori['factor'].unstack().rolling(120,1).apply(lambda x : f_calc_pos(x)).stack()
