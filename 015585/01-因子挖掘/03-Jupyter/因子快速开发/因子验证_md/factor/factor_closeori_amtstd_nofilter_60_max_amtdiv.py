...
factor_name = 'closeori_amtstd_nofilter_60_max_amtdiv'#
def factor_closeori_amtstd_nofilter_60_max_amtdiv(start_date, end_date, IO, return_fillna_dic=False):
    if return_fillna_dic:
        return {factor_name: 0}
    factor_explain = "closeori_amtstd_nofilter_60_max_amtdiv"
    ...
    df_ori[factor_name] = df_ori['factor'].unstack().rolling(60,1).apply(lambda x : f_calc_max(x)).stack()
    df_ori["amt"] = df_ori['amt'].unstack().rolling(60,1).apply(lambda x : f_calc_max(x)).stack()
