...
factor_name = 'syx2xyx1_amtstd_nofilter_120_max_amtdiv'#
def factor_syx2xyx1_amtstd_nofilter_120_max_amtdiv(start_date, end_date, IO, return_fillna_dic=False):
    if return_fillna_dic:
        return {factor_name: 0}
    factor_explain = "syx2xyx1_amtstd_nofilter_120_max_amtdiv"
    ...
    df_ori[factor_name] = df_ori['factor'].unstack().rolling(120,1).apply(lambda x : f_calc_max(x)).stack()
    df_ori["amt"] = df_ori['amt'].unstack().rolling(120,1).apply(lambda x : f_calc_max(x)).stack()
