...
factor_name = 'syx2xyx1_noamtstd_nofilter_20_max_240'#
def factor_syx2xyx1_noamtstd_nofilter_20_max_240(start_date, end_date, IO, return_fillna_dic=False):
    if return_fillna_dic:
        return {factor_name: 0}
    factor_explain = "syx2xyx1_noamtstd_nofilter_20_max_240"
    ...
    df_ori[factor_name] = df_ori['factor'].unstack().rolling(20,1).apply(lambda x : f_calc_max(x)).stack()
    df_ori[factor_name] = df_ori[factor_name] / df_ori['factor'].unstack().rolling(240,1).apply(lambda x : f_calc_max(x)).stack()
