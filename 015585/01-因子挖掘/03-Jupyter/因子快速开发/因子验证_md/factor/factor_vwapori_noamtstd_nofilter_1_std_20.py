...
factor_name = 'vwapori_noamtstd_nofilter_1_std_20'#
def factor_vwapori_noamtstd_nofilter_1_std_20(start_date, end_date, IO, return_fillna_dic=False):
    if return_fillna_dic:
        return {factor_name: 0}
    factor_explain = "vwapori_noamtstd_nofilter_1_std_20"
    ...
    df_ori[factor_name] = df_ori['factor'].unstack().rolling(1,1).apply(lambda x : f_calc_std(x)).stack()
    df_ori[factor_name] = df_ori[factor_name] / df_ori['factor'].unstack().rolling(20,1).apply(lambda x : f_calc_std(x)).stack()
