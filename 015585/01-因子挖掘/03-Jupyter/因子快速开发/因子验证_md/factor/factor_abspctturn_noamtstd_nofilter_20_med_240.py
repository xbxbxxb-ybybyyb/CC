...
factor_name = 'abspctturn_noamtstd_nofilter_20_med_240'#
def factor_abspctturn_noamtstd_nofilter_20_med_240(start_date, end_date, IO, return_fillna_dic=False):
    if return_fillna_dic:
        return {factor_name: 0}
    factor_explain = "abspctturn_noamtstd_nofilter_20_med_240"
    ...
    df_ori[factor_name] = df_ori['factor'].unstack().rolling(20,1).apply(lambda x : f_calc_med(x)).stack()
    df_ori[factor_name] = df_ori[factor_name] / df_ori['factor'].unstack().rolling(240,1).apply(lambda x : f_calc_med(x)).stack()
