...
def f_pro_cleanb2ttran(tick_df):#净委买金额/该阶段总成交额,series（开盘后）
    tick_df['buy_amt'] = tick_df['TotalBidQty'] * tick_df['WeightedAvgBidPx']
    tick_df['sell_amt'] = tick_df['TotalOfferQty'] * tick_df['WeightedAvgOfferPx']
    cleanb2ttran = (tick_df['buy_amt'] - tick_df['sell_amt'])/(tick_df['ValueTrade'].sum()+1)
    return cleanb2ttran
...
def f_pro_b2tran(tick_df):#委买金额/tick成交额,series（开盘后）
    tick_df['buy_amt'] = tick_df['TotalBidQty'] * tick_df['WeightedAvgBidPx']
    b2tran = (tick_df['buy_amt'])/(tick_df['ValueTrade']+1)
    return b2tran
...
def f_pro_s2tran(tick_df):#委卖金额/tick成交额,series（开盘后）
    tick_df['sell_amt'] = tick_df['TotalOfferQty'] * tick_df['WeightedAvgOfferPx']
    s2tran = (tick_df['sell_amt'])/(tick_df['ValueTrade']+1)
    return s2tran
...
def f_pro_b12delb(tick_df):#买1/买均（开盘后），series
...
def f_pro_syx1(tick_df):# 相对上影线：high - last / high - low，series
    ...
    tick_df['syx1'] = (tick_df['pcummax'] - tick_df['LastPx'])\
                      / tick_df['amp']
    return tick_df['syx1']
...
def f_pro_bdiff(tick_df):#买均的一阶差分序列,series
    return (tick_df['WeightedAvgBidPx'] - tick_df['WeightedAvgBidPx'].shift(1))/tick_df['pre_close'].max()
...
def f_pro_pdiff(tick_df):#LastPx的一阶差分序列,series
    return (tick_df['LastPx'] - tick_df['LastPx'].shift(1))/tick_df['pre_close'].max()
...
def f_pro_pa(tick_df):#涨幅*金额,（开盘后）series
    return (tick_df['LastPx']/tick_df['pre_close'].max()-1) * tick_df['ValueTrade']
...
def f_pro_bias5(tick_df): # 和过去15S均价的差异
    ...
    return (tick_df['LastPx'] - tick_df['ma5'].shift(1))/tick_df['pre_close']
...
result_df = pd.DataFrame(columns = ['factor_name','IC','info',
                                    ...
                                    'compare'])
list_del = []
# for  i in list(os.listdir('/data/user/015585/01-因子挖掘/03-Jupyter/因子快速开发/h5/20230823TTick/')):
#     list_del.append(i[:-3])
# for i in list(os.listdir('/data/user/015585/01-因子挖掘/03-Jupyter/因子快速开发/h5/20230828TTick/')):
#     list_del.append(i[:-3])
# for i in list(os.listdir('/data/user/015585/01-因子挖掘/03-Jupyter/因子快速开发/h5/20231011TTick/')):
#     list_del.append(i[:-3])
# for i in list(os.listdir('/data/user/015585/01-因子挖掘/03-Jupyter/因子快速开发/h5/20231012TTick/')):
#     list_del.append(i[:-3])
# for i in list(os.listdir('/data/user/015585/01-因子挖掘/03-Jupyter/因子快速开发/h5/20231103TTick/')):
#     list_del.append(i[:-3])
list_in = []
...
sft = strongFactorTest(20160101, 20191231, filter_factor=None, cal_mi=None)
                                ...
                                for std_i in dic_std:
                                    ...
                                    for calc_i in dic_calc:
                                        ...
                                        for compare_i in dic_compare:
                                            ...
                                            factor_path = '/data/user/015585/01-因子挖掘/03-Jupyter/因子快速开发/h5/20241017TTick_combination/'
                                            ...
                                            result_path = '/data/user/015585/01-因子挖掘/03-Jupyter/因子快速开发/回测报告/20241017TTick_combination/'
                                            factor_test = strongFactorTest(20160101, 20191231,cal_mi=None)
