...
def f_pro_amt(tick_df):#成交额，series（开盘后）
    tick_df['factor'] = tick_df['ValueTrade']
    return tick_df['factor']
def f_pro_corrb2b1(tick_df):#挂买总量和买1的corr（开盘后）
    res = pd.concat([tick_df['TotalBidQty'],tick_df['Buy1OrderQty']],axis = 1).corr(method = 'spearman').iloc[0,1]
    return res
...
def f_pro_corrb12s1(tick_df):#挂买1和卖1的corr（开盘后）
    res = pd.concat([tick_df['Sell1OrderQty'],tick_df['Buy1OrderQty']],axis = 1).corr(method = 'spearman').iloc[0,1]
    return res
def f_pro_corrb2s(tick_df):#挂买1和卖1的corr（开盘后）
    res = pd.concat([tick_df['TotalBidQty'],tick_df['TotalOfferQty']],axis = 1).corr(method = 'spearman').iloc[0,1]
    return res
def f_pro_corrb2t(tick_df):#挂买和成交量的corr（开盘后）
    res = pd.concat([tick_df['TotalBidQty'],tick_df['VolumeTrade']],axis = 1).corr(method = 'spearman').iloc[0,1]
    return res
def f_pro_corrbp2bv(tick_df):#挂买均价和挂买量的corr（开盘后）
    res = pd.concat([tick_df['WeightedAvgBidPx'],tick_df['TotalBidQty']],axis = 1).corr(method = 'spearman').iloc[0,1]
    return res
def f_pro_corrbp2t(tick_df):#挂买均价和成交量的corr（开盘后）
    res = pd.concat([tick_df['WeightedAvgBidPx'],tick_df['ValueTrade']],axis = 1).corr(method = 'spearman').iloc[0,1]
    return res
def f_pro_corrb2tp(tick_df):#挂买价格和成交均价的corr（开盘后）
    res = pd.concat([tick_df['WeightedAvgBidPx'],tick_df['ValueTrade']/
...
def f_pro_sp(tick_df):#挂卖均价（开盘后），series
    tick_df['factor'] = tick_df['WeightedAvgOfferPx']/(tick_df['pre_close'].max())
    return tick_df['factor']
...
def f_pro_b12s1(tick_df):#买1-卖1（开盘后），series
    tick_df['factor'] = (tick_df['Buy1Price'] - tick_df['Sell1Price'])/(tick_df['pre_close'].max())
    return tick_df['factor']
def f_pro_b2s(tick_df):#买均-卖均（开盘后），series
    tick_df['factor'] = (tick_df['WeightedAvgBidPx'] - tick_df['WeightedAvgOfferPx']) / (tick_df['pre_close'].max())
    return tick_df['factor']
...
def f_pro_pb1(tick_df):#买1价，series
    tick_df['factor'] = tick_df['Buy1Price']/(tick_df['pre_close'].max())
    return tick_df['factor']
def f_pro_b(tick_df):#挂买总额(开盘后），后续除以市值，series
    tick_df['factor'] = tick_df['WeightedAvgBidPx'] * tick_df['TotalBidQty']*1000
    return tick_df['factor']
...
def f_pro_amt2newamt(tick_df):#amt/amt.tail(1)(开盘后),series
    tick_df['factor'] = tick_df['ValueTrade'] / (tick_df['ValueTrade'].tail(1))
    return tick_df['factor']
def f_pro_bdiff(tick_df):#买均的一阶差分序列,series
    tick_df['factor'] = (tick_df['WeightedAvgBidPx'] - tick_df['WeightedAvgBidPx'].shift(1)) / tick_df['pre_close'].max()
    return tick_df['factor']
def f_pro_sdiff(tick_df):#卖均的一阶差分序列,series
    tick_df['factor'] = (tick_df['WeightedAvgOfferPx'] - tick_df['WeightedAvgOfferPx'].shift(1))/tick_df['pre_close'].max()
    return tick_df['factor']
def f_pro_pdiff(tick_df):#LastPx的一阶差分序列,series
    tick_df['factor'] = (tick_df['LastPx'] - tick_df['LastPx'].shift(1)) / tick_df['pre_close'].max()
    return tick_df['factor']
def f_pro_pv(tick_df):#涨幅*量,（开盘后）series
    tick_df['factor'] = (tick_df['LastPx']/tick_df['pre_close'].max()-1) * tick_df['VolumeTrade']
    return tick_df['factor']
def f_pro_pa(tick_df):#涨幅*金额,（开盘后）series
    tick_df['factor'] = (tick_df['LastPx']/tick_df['pre_close'].max()-1) * tick_df['ValueTrade']
    return tick_df['factor']
...
def f_pro_lpcummax(tick_df):
    tick_df['factor'] = tick_df['LastPx'].cummax() / tick_df['pre_close']
    return tick_df['factor']
def f_pro_h2l(tick_df):
    tick_df['factor'] = (tick_df['HighPx'] - tick_df['LowPx']) / tick_df['pre_close']
    return tick_df['factor']
def f_pro_h2l2(tick_df):
    tick_df['factor'] = (tick_df['LastPx'].cummax() - tick_df['LastPx'].cummin()) / tick_df['pre_close']
    return tick_df['factor']
...
def f_len_half2(tick_df):
    if len(tick_df)>10:
        ...
        return tick_df
#
# rsync -av --exclude='**/2021*' --exclude='**/2022*' --exclude='**/2023*' --exclude='**/2024*' --exclude='**/2025*' /dfs/group/800463/data/xdb_data_lag3_new/neptune/  /dfs/group/800463/public/xdb_data_lag3_new/neptune/
# 后续处理
# 标准化处理
...
result_df = pd.DataFrame(columns = ['factor_name','IC','info',
                                    ...
                                    'compare'])
list_del = []
for  i in list(os.listdir('/data/user/015585/01-因子挖掘/03-Jupyter/因子快速开发/h5/20250530_zwhtick_filter/')):
    list_del.append(i[:-3])
print(f'已计算{len(list_del)}个因子')
list_in = [
'930_after_all_all_0_bigger_all_b12b_nostd_med_nocompare',
 '930_after_all_all_0_bigger_all_hlmid_nostd_m2m_nocompare',
 '930_after_all_all_0_bigger_t500_hp_nostd_cct_nocompare',
 '930_after_all_all_0_bigger_all_pb1_nostd_kurt_nocompare',
 '930_after_all_all_0_bigger_all_h2l2_nostd_m2m_nocompare',
 '930_after_all_all_0_bigger_t500_vwap2p_nostd_cv_nocompare',
 '930_after_all_all_0_bigger_all_pctturn_nostd_max_nocompare',
 '930_after_all_all_0_bigger_t500_numtradesdiff_nostd_cct_nocompare',
 '930_after_all_all_0_bigger_t500_tran2b_nostd_cv_nocompare',
 '930_after_all_all_0_bigger_all_amt2newamt_nostd_cct_nocompare',
 '930_after_all_all_0_bigger_all_hlmid2lp_nostd_avg_nocompare',
 '930_after_all_all_0_bigger_t500_b12s1_nostd_sum_nocompare',
 '930_after_all_all_0_bigger_t500_rcleanb_nostd_avg_nocompare',
 '930_after_all_all_0_bigger_all_h2l2_nostd_med_nocompare',
 '930_after_all_all_0_bigger_all_pa_nostd_cv_nocompare',
 '930_after_all_all_0_bigger_all_pa_nostd_avg_nocompare',
 '930_after_all_all_0_bigger_t500_s12s_nostd_cct_nocompare',
 '930_after_all_all_0_bigger_t500_s2tran_nostd_med_nocompare',
 '930_after_all_all_0_bigger_all_cleanb2ttran_nostd_change_nocompare',
 '930_after_all_all_0_bigger_all_pb1_nostd_cv_nocompare',
 '930_after_all_all_0_bigger_all_b1delb_nostd_min_nocompare',
 '930_after_all_all_0_bigger_all_b2s_nostd_change_nocompare',
 '930_after_all_all_0_bigger_t500_ratiob_nostd_skew_nocompare',
 '930_after_all_all_0_bigger_all_bp_nostd_cv_nocompare',
 '930_after_all_all_0_bigger_all_s2tran_nostd_skew_nocompare',
 '930_after_all_all_0_bigger_all_b2ttran_nostd_tail_nocompare',
 '930_after_all_all_0_bigger_all_s12s_nostd_avg_nocompare'
]
# for excel_name in ['20230823TTick', '20230828TTick', '20231011TTick', '20231012TTick', '20231103TTick']:
#     factor_done1 = pd.read_excel('/data/user/015585/01-因子挖掘/03-Jupyter/因子快速开发/res/{}.xlsx'.format(excel_name))
#     list_in = list_in + list(factor_done1[(abs(factor_done1['IC']) >= 0.04) & (factor_done1['score'] >= 20) & (factor_done1['same_ratio'] <= 0.2)]['factor_name'])
...
sft = strongFactorTest(20160101, 20191231, filter_factor=None)
                                ...
                                for std_i in dic_std:
                                    ...
                                    for calc_i in dic_calc:
                                        ...
                                        for compare_i in dic_compare:
                                            ...
                                            factor_path = '/data/user/015585/01-因子挖掘/03-Jupyter/因子快速开发/h5/20250530_zwhtick/'
                                            ...
                                            result_path = '/data/user/015585/01-因子挖掘/03-Jupyter/因子快速开发/回测报告/20250530_zwhtick/'
                                            factor_test = strongFactorTest(20160101, 20191231)
