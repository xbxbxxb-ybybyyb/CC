...
def generate_trade_factor(factor_name_logic):
    ...
    return res
### ============嵌入到模板============
def generate_ttrade_factor_file(factor_name_file, factor_name_logic):
    ...
    file_path = "/data/user/015585/fefactorframework-mercury/fast_factor/code/代码翻译/tmp/factor_ttrade_sample.py"
    code_content = read_py_file_to_string(file_path)
    res = generate_trade_factor(factor_name_logic)
    res_text = ''''''
    for i in res:
        if i != '':
            res_text += i
    ...
    code_content = code_content.replace('qyh_ttrade_sample', factor_name_logic)
    code_content = code_content.replace('ttrade_sample', factor_name_logic)
    return code_content
...
if __name__ == "__main__":
    ...
    df_factor_info = df_factor_info[df_factor_info['factor_type'] == 'trade']
    factor_list = list(df_factor_info['name'])
    for factor_name in factor_list:
        code_content = generate_ttrade_factor_file(factor_name, factor_name)
        save_factor_file(f'/data/user/015585/01-因子挖掘/03-Jupyter/因子快速开发/因子验证_trade/factor/factor_{factor_name}.py', code_content)
