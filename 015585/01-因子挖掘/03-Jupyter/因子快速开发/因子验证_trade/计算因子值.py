...
factor_name_list = os.listdir('/data/user/015585/01-因子挖掘/03-Jupyter/因子快速开发/因子验证_trade/factor/')
factor_name_list = [x.replace('.py','') for x in factor_name_list if '.py' in x and 'factor_' in x]
...
for factor_name in factor_name_list:
    ...
    result_path = '/data/user/015585/01-因子挖掘/03-Jupyter/因子快速开发/因子验证_trade/'
    if os.path.exists(result_path + factor_name + '.h5'):
        os.remove(result_path + factor_name + '.h5')
    factor_df0 = run_factor(func,
                            factor_name,
                            'TTransaction',## TTickab,TOrder,TTransaction,T-1_factor,MarketIndTTick,TOrder_TTickab
                            start_date, end_date,
                            basic_file_path,
