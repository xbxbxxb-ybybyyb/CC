...
factor_name = '930_allbs_allp_allamt_t300_bsdf_gbuy_amt2buypctdiff_tail_minus'#
def factor_930_allbs_allp_allamt_t300_bsdf_gbuy_amt2buypctdiff_tail_minus(trade_df, return_fillna_dic=False):
    if return_fillna_dic:
        return {factor_name: 0}
    factor_explain = "930_allbs_allp_allamt_t300_bsdf_gbuy_amt2buypctdiff_tail_minus"
    ...
    trade_df1 = trade_df1.groupby('TradeBuyNo')['factor'].sum().to_frame(name = 'factor')
    trade_df2 = trade_df2.groupby('TradeBuyNo')['factor'].sum().to_frame(name = 'factor')
    res1 = f_calc_tail(trade_df1['factor'])
    res2 = f_calc_tail(trade_df2['factor'])
    res = res1 - res2
