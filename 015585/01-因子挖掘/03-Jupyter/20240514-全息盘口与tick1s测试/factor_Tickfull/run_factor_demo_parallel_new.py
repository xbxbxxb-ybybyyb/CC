...
def run_factor(func, factor_name, factor_type, start_date, end_date, basic_file_path, result_path, append_next_tradingday=False, interval_res=True,
               ...
               data_path_dic= {'TTransaction': '/data/group/800463/data/project1_prod/transaction_europa/',
                               ...
                               'TTick1s':'/dfs/group/800463/data/xdb_data_europa/xdb_tick1s/',
                               'TTickfull':'/dfs/group/800463/data/xdb_data_europa/xdb_tickfull_new/'}):
    # 因子代码执行函数
    ...
    if factor_type in ['TTransaction', 'TTickab','TOrder',
        ...
        if factor_type in ['TTransaction', 'TTickab', 'TOrder']:
        ...
        elif factor_type in ['TTick1s', 'TTickfull']:
            #tradingday_list = s.tradingday(start_date, end_date)
            tradingday_list = list(map(lambda x: x.strftime('%Y%m%d'),basic_df.index.get_level_values(0).drop_duplicates()))
            ...
            def calc_tmp_df(tradingday, num):
                view_bar(num, len(tradingday_list), tradingday)
                data = pd.read_pickle('%s%s.pkl' % (data_path_dic[factor_type], tradingday), compression = 'gzip')
                tmp_df = data.groupby(level=[0, 1]).apply(lambda x: func(x))
                return tmp_df
            from joblib import Parallel, delayed
            factor_df_list = Parallel(n_jobs=28)(delayed(calc_tmp_df)(tradingday, num) for tradingday, num in
                                                 zip(tradingday_list, range(len(tradingday_list))))
            # factor_df_list.append(pd.concat([tmp_df], axis=1))
            factor_df = pd.concat(factor_df_list, axis=0)
            factor_df = factor_df.reindex(basic_df.index)
