...
from factor_qyh_ttick_lowcv_quickrise import factor_qyh_ttick_lowcv_quickrise as func
...
'''
date = '20230808'#周四日期
factor_name = 'factor_qyh_conceptnum'
print(factor_name)
...
#                         factor_name,
#                         'TTickab',## TTickab,TOrder
#                         start_date, end_date,
#                         basic_file_path,
#                         result_path, interval_res=False)
# TTransaction,LastTouchTTick
# df = pd.read_hdf(result_path + factor_name + '.h5')
df = pd.read_pickle('/data/user/015585/01-因子挖掘/20230816-强势股分析/factor/20230824/个股题材指数.pkl')
df.columns = ['factor_qyh_conceptnum']
df['factor_qyh_conceptnum'] = df['factor_qyh_conceptnum'].apply(lambda x:len(x))
res_df_concept_num = pd.read_pickle('/data/user/015585/01-因子挖掘/20230816-强势股分析/factor/20230824/每日题材总数.pkl')
print(df.head(5))
# 非快速拉升
print('全样本')
