...
import pandas as pd
# dtj,zcz
# vwap/lastpx在开盘2-3min和尾盘2-3min的变异系数
# 43,-0.1
# 56，-0.1
...
def factor_qyh_ttick_20231116_3(tick_df, return_fillna_dic=False):
    ...
    res1 = tick_df1['factor'].mean()/tick_df1['factor'].std() if abs(tick_df1['factor'].std()) > 0 else np.nan
    res2 = tick_df2['factor'].mean()/tick_df2['factor'].std() if abs(tick_df2['factor'].std()) > 0 else np.nan
    factor_dict = {factor_name: res1 / res2 if abs(res2) > 0 else np.nan}
    # ---------------------------------------------------------------------------------------------------------------
