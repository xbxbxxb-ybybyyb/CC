...
#
#
factor_name = 'qyh_ttick_20231026_wj2'#
def factor_qyh_ttick_20231026_wj2(tick_df, return_fillna_dic=False):
    ...
    vol_med = round_(tick_df['ValueTrade'].quantile(0.75),4)
    tick_df = tick_df[(tick_df['ValueTrade'] >= vol_med)]
    if len(tick_df)>0:
        ...
        if starttime<=93100000:
            sel_trans = tick_df.tail(int(len(tick_df)/2))
        else:
        ...
        if zcz_flag:
            ret_pct = ret_pct/2
        res = (ret_pct - ret_pct.max()).median()
    else:
        res = -0.07
