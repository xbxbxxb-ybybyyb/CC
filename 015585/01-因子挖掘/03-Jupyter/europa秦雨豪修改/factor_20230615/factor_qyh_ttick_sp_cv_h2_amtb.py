...
import numpy as np
import pandas as pd
factor_name = 'qyh_ttick_sp_cv_h2_amtb'#
def factor_qyh_ttick_sp_cv_h2_amtb(tick_df, return_fillna_dic=False):
    ...
    cv = tick_df['pct'].std() / abs(tick_df['pct'].mean()) if round_(tick_df['pct'].mean(),8) != 0 else 1
