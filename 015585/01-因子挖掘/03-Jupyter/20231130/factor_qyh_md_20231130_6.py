...
s = FactorData()
factor_name = 'qyh_md_20231130_6'
# zcz,dtj
# T-1振幅变形，使用high的5日均值代替high，基准使用high
# 60,-0.094
#
def factor_qyh_md_20231130_6(start_date, end_date, IO, return_fillna_dic=False):
    ...
    #
    df_ori['A'] = df_ori['low'].unstack().fillna(method='ffill',limit=20).stack()
    df_ori['B'] = df_ori['high'].unstack().rolling(5,1).mean().stack()
    df_ori['C'] = (df_ori['high']).unstack().fillna(method='ffill',limit=20).stack()
