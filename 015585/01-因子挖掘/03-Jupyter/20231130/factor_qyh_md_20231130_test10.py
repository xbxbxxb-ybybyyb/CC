...
s = FactorData()
# 北向资金：最近5日/20日持股市值变化占股票市值比例(大于0部分)，缺失值用行业均值填充
'''

'''
#
factor_name = 'qyh_md_20231130_test10'
def factor_qyh_md_20231130_test10(start_date, end_date, IO, return_fillna_dic=False):
    if return_fillna_dic:
        # 返回因子为nan时的填充值
        return {factor_name:26.4,'data':['MD']}
    # -------------------------------------------------------------------------------------------------------------------
    ...
    df_ori['qty_delta'] = df_ori['qty'] - df_ori['qty'].unstack().shift(1).stack()
    ind_data[factor_name] = df_ori['qty'] / ind_data['free_float_shares']
    para = 60
    ind_data[factor_name] = ind_data[factor_name].unstack().rolling(para,5).apply(lambda x : x[x>0].sum()).stack()
    #
