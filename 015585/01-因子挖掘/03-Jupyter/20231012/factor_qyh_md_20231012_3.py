...
s = FactorData()
factor_name = 'qyh_md_20231012_3'
# zcz,dtj
# 上影线的20日成交量加权
# 0.071,47
#
def factor_qyh_md_20231012_3(start_date, end_date, IO, return_fillna_dic=False):
    ...
    df_ori = IO.read_data([start_date, end_date],
                          columns=['close','high','pre_close','amt'],
    ...
    df_ori['factor'] = df_ori['factor'].unstack().rolling(20,1).sum().stack()
    df_ori[factor_name] = df_ori['factor'] / (1+df_ori['amt'].unstack().rolling(20,1).sum().stack())
    # -------------------------------------------------------------------------------------------------------------------
