...
s = FactorData()
factor_name = 'qyh_md_20231012_6'
# zcz,dtj
# 上影线-下影线按成交量加权的10日最大值
# 46，0.074
#
def factor_qyh_md_20231012_6(start_date, end_date, IO, return_fillna_dic=False):
    ...
    df_ori['factor'] = df_ori['factor'].unstack().rolling(10,1).max().stack()
    #
    df_ori['amt_20_max'] = df_ori['amt'].unstack().rolling(10,1).max().stack()
    df_ori[factor_name] = df_ori['factor'] / (df_ori['amt_20_max']+1)
    # -------------------------------------------------------------------------------------------------------------------
