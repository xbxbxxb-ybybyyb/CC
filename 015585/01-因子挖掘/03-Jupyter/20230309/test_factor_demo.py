...
class strongFactorTest:
    def __init__(self, start_date, end_date, segment_number=20, style_list=['freeturn', 'ZT_Time'], cal_mi = ['value', 'class', 'mixed'],
                 ...
                 first_is_zt=False, open_is_zt=False, Flag_SH_SZ=None, filter_factor=None,last_buy_rise=0.025):
    ...
    def load_data(self):
        if self.test_sample == 'zt':
            df = IO.read_data([self.start_date, self.end_date], alt='/data/group/800463/data/project1_public/factor_lib/sft_update_europa.h5')
        ...
        if self.test_sample == 'bt':
            ...
            all_filter = (time_interval_filter & last_is_zt_filter & open_is_zt_filter & first_is_zt_filter & sh_sz_filter&last_buy_rise_filter)
        #样本筛选
        df = df[all_filter]
    ...
    def factor_test(self, factor_df, result_path, factor_corr_test=False, generate_pdf=True, tag=''):
        ...
        if factor_corr_test:
            check_res=pd.read_excel('/data/group/800463/data/project1_public/factor_lib/check_res_tot_europa.xlsx')
            check_res=check_res.set_index('factor_name')[['in_score','in_IC_tot','Mutual_Info']]
