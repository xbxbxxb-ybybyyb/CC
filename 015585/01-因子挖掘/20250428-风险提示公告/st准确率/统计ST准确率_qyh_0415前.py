...
risk_list_20241231 = list(df_st[((df_st['REMOVE_DT'].isnull()) | (df_st['REMOVE_DT'] >= '20250101')) \
                       & (df_st['ENTRY_DT'] <= '20241231') & (df_st['S_TYPE_ST'] != 'R')]['S_INFO_WINDCODE']) # 20241231时候仍然有问题的股票
df_st_2025 = df_st[(df_st['ENTRY_DT'] >= '20250101') & (df_st['ENTRY_DT'] <= '20250415')]
df_st_2025_filter = df_st_2025[df_st_2025['S_TYPE_ST'] != 'R'] # 非“恢复上市”均考虑

# 获取pre_st的总和 df_pre_st
path_pre_st = '/data/user/015585/01-因子挖掘/20250428-风险提示公告/pre_st_qyh_规则4/'
'''
规则1234：/data/user/015585/01-因子挖掘/20250428-风险提示公告/pre_st_qyh/
规则123：/data/user/015585/01-因子挖掘/20250428-风险提示公告/st准确率/pre_st_file/
规则4：/data/user/015585/01-因子挖掘/20250428-风险提示公告/pre_st_qyh_规则4/
'''
...
# 往前取非停牌的前一个交易日
md_data = IO.read_data([20240101,20250414], columns=['amt'],
...
print('ST覆盖率，无风控：20250101-20250415:')
print(df1_entrydt_min.mean()['isin_pre_st'])
print('ST总数：',len(df1_entrydt_min))
...
print('每天的预测数量 20250101-20250430：')
print(res_tmp['Ticker1'].apply(len).mean())
print('每天的预测数量 20250101-20250415：')
print(res_tmp[res_tmp['dt'] <= '20250415']['Ticker1'].apply(len).mean())
print('每天的预测数量 20250416-20250430：')
print(res_tmp[res_tmp['dt'] > '20250415']['Ticker1'].apply(len).mean())
...
print('每天的预测数量（剔除名称中带ST的部分后）20250101-20250430：')
print(res_tmp_filter['Ticker1'].apply(len).mean())
print('每天的预测数量（剔除名称中带ST的部分后） 20250101-20250415：')
print(res_tmp_filter[res_tmp_filter['dt'] <= '20250415']['Ticker1'].apply(len).mean())
print('每天的预测数量（剔除名称中带ST的部分后） 20250416-20250430：')
print(res_tmp_filter[res_tmp_filter['dt'] > '20250415']['Ticker1'].apply(len).mean())
