...
def round_(x, n=0):
    ...
    return res
# 因子属性
def f_pro_high(df_amt, df_volume, df_low, df_close, df_high, df_open): #
    df = df_high
    return df
def f_pro_open(df_amt, df_volume, df_low, df_close, df_high, df_open): #
    df = df_open
    return df
def f_pro_close(df_amt, df_volume, df_low, df_close, df_high, df_open): #
    df = df_close
    return df
def f_pro_low(df_amt, df_volume, df_low, df_close, df_high, df_open): #
    df = df_low
    return df
def f_pro_amt(df_amt, df_volume, df_low, df_close, df_high, df_open): #
    df = df_amt
    return df
def f_pro_volume(df_amt, df_volume, df_low, df_close, df_high, df_open): #
    df = df_volume
    return df
def f_pro_h2p(df_amt, df_volume, df_low, df_close, df_high, df_open): #
    df_pre = df_close.T.ffill().iloc[-1].T.unstack().shift(1).stack()
    df = df_high.divide(df_pre, axis=0)
    return df
def f_pro_l2p(df_amt, df_volume, df_low, df_close, df_high, df_open): #
    df_pre = df_close.T.ffill().iloc[-1].T.unstack().shift(1).stack()
    df = df_low.divide(df_pre, axis=0)
    return df
def f_pro_c2p(df_amt, df_volume, df_low, df_close, df_high, df_open): #
    df_pre = df_close.T.ffill().iloc[-1].T.unstack().shift(1).stack()
    df = df_close.divide(df_pre, axis=0)
    return df
def f_pro_h2c(df_amt, df_volume, df_low, df_close, df_high, df_open): #
    df = df_high / df_close
    return df
def f_pro_l2c(df_amt, df_volume, df_low, df_close, df_high, df_open): #
    df = df_low / df_close
    return df
def f_pro_hl2p(df_amt, df_volume, df_low, df_close, df_high, df_open): #
    df_pre = df_close.T.ffill().iloc[-1].T.unstack().shift(1).stack()
    df = (df_high - df_low).divide(df_pre, axis=0)
    return df
def f_pro_vwap(df_amt, df_volume, df_low, df_close, df_high, df_open): #
    df_vwap = df_amt.cumsum(axis=1) / df_volume.cumsum(axis=1)
    return df_vwap
def f_pro_c2v(df_amt, df_volume, df_low, df_close, df_high, df_open): #
    df_vwap = df_amt.cumsum(axis=1) / df_volume.cumsum(axis=1)
    df = df_close / df_vwap
    return df
def f_pro_h2v(df_amt, df_volume, df_low, df_close, df_high, df_open): #
    df_vwap = df_amt.cumsum(axis=1) / df_volume.cumsum(axis=1)
    df = df_high / df_vwap
    return df
def f_pro_l2v(df_amt, df_volume, df_low, df_close, df_high, df_open): #
    df_vwap = df_amt.cumsum(axis=1) / df_volume.cumsum(axis=1)
    df = df_low / df_vwap
    return df
def f_pro_abspct(df_amt, df_volume, df_low, df_close, df_high, df_open): #
    df_pre = df_close.T.ffill().iloc[-1].T.unstack().shift(1).stack()
    df = abs(df_close.divide(df_pre, axis=0))
    return df
def f_pro_logabspct(df_amt, df_volume, df_low, df_close, df_high, df_open): #
    df_pre = df_close.T.ffill().iloc[-1].T.unstack().shift(1).stack()
    df = np.log(abs(df_close.divide(df_pre, axis=0)) + 1e-3)
    return df
def f_pro_abspctamt(df_amt, df_volume, df_low, df_close, df_high, df_open): #
    df_pre = df_close.T.ffill().iloc[-1].T.unstack().shift(1).stack()
    df = abs(df_close.divide(df_pre, axis=0)) * df_amt
    return df
def f_pro_syx1(df_amt, df_volume, df_low, df_close, df_high, df_open): #
    df_pre = df_close.T.ffill().iloc[-1].T.unstack().shift(1).stack()
    df = (df_high - df_close).divide(df_pre, axis=0)
    return df
def f_pro_syx2(df_amt, df_volume, df_low, df_close, df_high, df_open):  #
    df_pre = df_close.T.ffill().iloc[-1].T.unstack().shift(1).stack()
    df = (df_high - df_close.clip(lower=df_open)).divide(df_pre, axis=0)
    return df
def f_pro_xyx1(df_amt, df_volume, df_low, df_close, df_high, df_open):  #
    df_pre = df_close.T.ffill().iloc[-1].T.unstack().shift(1).stack()
    df = (df_close - df_low).divide(df_pre, axis=0)
    return df
def f_pro_xyx2(df_amt, df_volume, df_low, df_close, df_high, df_open):  #
    df_pre = df_close.T.ffill().iloc[-1].T.unstack().shift(1).stack()
    df = (df_close - df_close.clip(upper=df_open)).divide(df_pre, axis=0)
    return df
def f_pro_lengthk(df_amt, df_volume, df_low, df_close, df_high, df_open):  #
    df_pre = df_close.T.ffill().iloc[-1].T.unstack().shift(1).stack()
    df = abs(df_open - df_close).divide(df_pre, axis=0)
    return df
# 筛选
def f_filter_nofilter(df, df_amt, df_volume, df_low, df_close, df_high, df_open):
    return df
def f_filter_up(df, df_amt, df_volume, df_low, df_close, df_high, df_open):
    ...
    df = df * df_sign
    return df
def f_filter_down(df, df_amt, df_volume, df_low, df_close, df_high, df_open):
    df_close_delta = df_close.T.apply(lambda x : x - x.shift(1)).T.applymap(lambda x : round_(x,5))
    df_sign = df_close_delta.applymap(lambda x : 1 if x < 0 else np.nan)
    df = df * df_sign
    return df
# 横向加权变量
def f_day_div_no(df, df_amt, df_volume, df_low, df_close, df_high, df_open):
    return df
def f_day_div_amt(df, df_amt, df_volume, df_low, df_close, df_high, df_open):
    return df * df_amt
def f_day_div_volume(df, df_amt, df_volume, df_low, df_close, df_high, df_open):
    return df * df_volume
def f_day_div_close(df, df_amt, df_volume, df_low, df_close, df_high, df_open):
    return df * df_close
# 分母df
def get_fm_df(day_div, df_amt, df_volume, df_low, df_close, df_high, df_open):
    res = {
        'amt': df_amt,
        'volume': df_volume,
        'close':df_close
