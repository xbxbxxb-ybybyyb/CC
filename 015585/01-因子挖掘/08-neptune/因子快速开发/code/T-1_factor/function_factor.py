...
def f_pro_open(df_ori): # 开盘价：开/pre
    ...
    return df_ori
def f_pro_low(df_ori): # 最低价：开/pre
...
def f_pro_vwap(df_ori):#均价
    ...
    return df_ori
def f_pro_ho(df_ori): # 下影线1：(收-低)/pre
    df_ori['factor'] = (df_ori['high'] - df_ori['open']) / df_ori['pre_close']
    return df_ori
def f_pro_ol(df_ori): # 下影线1：(收-低)/pre
    df_ori['factor'] = (df_ori['open'] - df_ori['low']) / df_ori['pre_close']
    return df_ori
def f_pro_vl(df_ori): #
    df_ori['factor'] = (df_ori['vwap'] - df_ori['low']) / df_ori['pre_close']
    return df_ori
def f_pro_hv(df_ori): #
    df_ori['factor'] = (df_ori['high'] - df_ori['vwap']) / df_ori['pre_close']
    return df_ori
def f_pro_vo(df_ori): #
    df_ori['factor'] = (df_ori['vwap'] - df_ori['open']) / df_ori['pre_close']
    return df_ori
