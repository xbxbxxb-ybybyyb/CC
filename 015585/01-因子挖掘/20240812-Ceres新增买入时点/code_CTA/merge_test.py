...
# 读取样本集
df_s1=pd.read_pickle('/data/user/015585/01-因子挖掘/20240812-Ceres新增买入时点/file/20240826_sample_basic_20160101_20191231.pkl').loc[pd.to_datetime(str(start_date)):pd.to_datetime(str(end_date))]
name = '_portfolio1_trade' # _pywt1_1_3_2, _pywt1 1_3_2 and 5_3_1_2
df_1=pd.read_pickle('/data/user/015585/01-因子挖掘/20240812-Ceres新增买入时点/file_CTA/test_trigger{}.pkl'.format(name)).loc[pd.to_datetime(str(start_date)):pd.to_datetime(str(end_date))]
name = '_calculate_3' # # _pywt1_5_3_1_2, _calculate_3
df_2=pd.read_pickle('/data/user/015585/01-因子挖掘/20240812-Ceres新增买入时点/file_CTA/test_trigger{}.pkl'.format(name)).loc[pd.to_datetime(str(start_date)):pd.to_datetime(str(end_date))]
#
df_1['label_v2t10_2'] = df_2['label_v2t10']
df_1['trigger_time_2'] = df_2['trigger_time']
df_1['trigger_time_final'] = df_1[['trigger_time_2','trigger_time']].min(axis=1)
df_1['label_v2t10_final'] = df_1['label_v2t10']
df_1.loc[df_1['trigger_time_final'] == df_1['trigger_time_2'] ,'label_v2t10_final'] = df_1.loc[df_1['trigger_time_final'] == df_1['trigger_time_2'] ,'label_v2t10_2']

df_1['label_v2t10'] = df_1['label_v2t10_final']
df_1['trigger_time'] = df_1['trigger_time_final']
df_rehigh = df_1.copy()
name = '_pywt1 1_3_2 and 5_3_1_2 and s1_3'
df_rehigh.to_pickle('/data/user/015585/01-因子挖掘/20240812-Ceres新增买入时点/file_CTA/test_trigger{}.pkl'.format(name))
#
...
sta_df=sta_df.T
sta_df.to_excel('/data/user/015585/01-因子挖掘/20240812-Ceres新增买入时点/file_CTA/qyh{}.xlsx'.format(name))