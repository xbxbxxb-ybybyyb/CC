...
for name in ['_portfolio1_trade',]:
    ...
    df_rehigh=pd.read_pickle('/data/user/015585/01-因子挖掘/20240812-Ceres新增买入时点/file_CTA/test_trigger{}.pkl'.format(name)).loc[pd.to_datetime(str(start_date)):pd.to_datetime(str(end_date))]
    # 涨停情况
    ...
    def sta(df1):
        ...
        res_dic['重叠部分label_delta均值'] = (df1['label'] - df1['label_v2o10d1']).mean()
...
print('所有的931均值：')
print(df_rehigh['label_v2o10d1'].mean())
print('选出来在931买入的均值：')
print(df_rehigh[(df_rehigh['trigger_time'] == 93100000)]['label_v2o10d1'].mean())
...
print('进一步分析，931以后新条件触发的样本，它们的label_v2o10d1：')
print(df_rehigh[(df_rehigh['trigger_time'] > 93100000)]['label_v2o10d1'].mean())
