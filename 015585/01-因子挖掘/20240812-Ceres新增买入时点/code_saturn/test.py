...
for name in ['1_3_2']:
    ...
    df_s1 = IO.read_data([start_date, end_date], alt = '/data/group/800463/data/project2_public/factor_lib/sft_basic_formal_931_20160101_20191231.h5',)

    df_rehigh=pd.read_pickle('/data/user/015585/01-因子挖掘/20240812-Ceres新增买入时点/file_saturn/test_trigger{}.pkl'.format(name)).loc[pd.to_datetime(str(start_date)):pd.to_datetime(str(end_date))]
    # 涨停情况
    ...
    def filer_s1(sample):
        ...
        # qyh修正
        basic_file = pd.read_pickle('/data/user/015585/01-因子挖掘/20240812-Ceres新增买入时点/file/Basic_closed_hf_finish_20160101_20191231_saturn_update.pkl')
        sample.loc[sample['T_first_trans_ZT'] == -3,'T_first_trans_ZT'] = basic_file.loc[sample['T_first_trans_ZT'] == -3,'T_first_trans_ZT']
        sample['T_day_first_DT_Time'] = basic_file['T_day_first_DT_Time']
        sample = sample[sample['label_v2o10d1'] != -1]
        #
    ...
    sta_df=sta_df.T
    sta_df.to_excel('/data/user/015585/01-因子挖掘/20240812-Ceres新增买入时点/file_saturn/qyh{}.xlsx'.format(name))
