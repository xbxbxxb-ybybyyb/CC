...
if __name__ == '__main__':
    ...
    start_date = 20150930
    end_date = 20241031
    save_path = '/dfs/user/015585/20241107-数库产业链/20241107_样本距离_2016_2024_修正后/'
    little_ratio = 0.05
    little_punish = 4
    # 获取md
    def cal_ul_price(pre_close_dataframe, ratio=0.1):
        ...
        pre_close_dataframe['ul_price'] = np.floor(pre_close_dataframe['pre_close'] * 100 * (1 + ratio) + 0.5 + 1e-8) / 100
    ...
    md_data_ori = IO.read_data([start_date, end_date],
                      columns=['amt','pre_close','high'],
                      alt='/data/group/800080/warehouse/prod/MD/CHINA_STOCK/DAILY/WIND/MD_CHINA_STOCK_DAILY_WIND.h5')
    md_data_ori['ul_price'] = cal_ul_price(md_data_ori)
    md_data_ori['trigger_price'] = (md_data_ori['ul_price'] - 0.01).apply(lambda x : round_(x,2))
    md_data_ori = md_data_ori.query('high >= trigger_price') # 触板标的
    #
    from xquant.factordata import FactorData as FactorData2
    s2 = FactorData2()
    ...
    for tradingday in tradingday_list:
        md_data = md_data_ori.loc[pd.Timestamp(tradingday):pd.Timestamp(tradingday)] # test
        print(tradingday, len(md_data))
