...
if __name__ == '__main__':
    ...
    df_stock_product = df_stock_product[df_stock_product['report_date'] >= pd.Timestamp('20140101')]
    ...
    start_date = 20241122
    end_date = start_date
    save_path = f'/dfs/user/015585/20241107-数库产业链/20241125_全市场样本距离_2016_2024/{start_date}/'
    little_ratio = 0.05
    little_punish = 4
    # 获取md
    ...
    from xquant.factordata import FactorData as FactorData2
    s2 = FactorData2()
    tradingday_list = s2.tradingday(start_date, end_date)
    tradingday = tradingday_list[0] if len(tradingday_list) > 0 else -1
    if tradingday == -1:
        print('交易日不正确，请设置交易日期')
        raise ValueError
    md_data = md_data_ori.loc[pd.Timestamp(tradingday):pd.Timestamp(tradingday)]
    print(tradingday, f'MD样本总数{len(md_data)}')
    #
    graph = get_graph(tradingday, df_product_ori, df_chain_ori)
    report_judge_date = (pd.Timestamp(tradingday) - pd.Timedelta(days=120)).strftime('%Y%m%d')
    df_stock_product = df_stock_product_ori[df_stock_product_ori['report_date'] <= pd.Timestamp(report_judge_date)]
    ...
    stock_list1 = list(md_data.loc[pd.Timestamp(str(tradingday))].reset_index()['Ticker'])
    stock_list1 = [x for x in stock_list1 if x in df_stock_product_code_list.index]
    stock_list1.sort()
    stock_list2 = stock_list1
    stock_pair_list = itertools.product(stock_list1,stock_list2)
    stock_pair_list = [i for i in stock_pair_list if i[0]>=i[1]]

    for stock in stock_list1[5000:5100]:
        print(time.localtime())
        stock_pair_list_by_stock = [i for i in stock_pair_list if i[0] == stock]
        pool_num = 30
        pool_num=min(pool_num,len(stock_pair_list_by_stock))
        pool = Pool(pool_num)
        task_list = []
        print(f'开始计算，CPUS={pool_num}')
        print(tradingday, stock)
        for stock_pair in stock_pair_list_by_stock:
            task_list.append(pool.apply_async(parallel_get_stock_pair_path,args=(stock_pair[0],stock_pair[1],
                                                                                   graph,df_stock_product_code_list,little_punish)))
        pool.close()
        pool.join()
        matrix_distance = pd.concat([i.get() for i in task_list])
        if not os.path.exists(f'{save_path}{stock}.pkl'):
            matrix_distance.to_pickle(f'{save_path}{stock}.pkl')
        else:
            print(f'{save_path}{stock}.pkl 已存在')
        print(time.localtime())