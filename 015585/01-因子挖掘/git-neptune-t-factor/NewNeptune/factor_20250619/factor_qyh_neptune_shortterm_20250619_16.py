...
class factor_qyh_neptune_shortterm_20250619_16(BaseFactor):
    strategy_name = "neptune"
    factor_name = "qyh_neptune_shortterm_20250619_16"
    fill_na_value = 0
    ...
    def calculate(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            zcz = database['zcz']
            #
            tick_df['factor'] = ((tick_df['OrderQty'] * tick_df['OrderPrice']) / tick_df['ValueTrade'].sum()).replace(np.inf,np.nan).replace(-np.inf,np.nan)
