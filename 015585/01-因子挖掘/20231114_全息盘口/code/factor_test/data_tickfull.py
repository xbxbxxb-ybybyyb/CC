...
def update_data_tickab(start_date, end_date, basic_file_path, result_path='/data/group/800463/data/project1_prod/tickab_zt/'):
    ...
    def get_tickfull_data(tradingday_timestamp,df):
        tradingday = tradingday_timestamp.strftime('%Y%m%d')
        if not os.path.exists('%s%s.pkl' % (result_path, tradingday)):
            result_df_list = []
            from xdb.stockdata import StockData
            a = StockData()
            def get_tick_data(stock_code, tradingday):
                df = a.get_tickfull(tradingday, stock_code)
                rename_dic = {'md_time': 'MDTime', 'num_trades': 'NumTrades', 'total_volume': 'TotalVolumeTrade',
            ...
            for index, d in df.iterrows():
                ...
                try:
                    ...
                    tick_data = tick_data[tick_data['MDTime'] <= lag_time]  # 策略使用transaction触发, 由于order相较于transaction延迟，全息盘口截取时将时间戳提前一定时间
    ...
    factor_df_list = Parallel(n_jobs=15)(delayed(get_tickfull_data)(tradingday_timestamp,df) for tradingday_timestamp, df in basic_df.groupby(level=0))
...
if __name__ == '__main__':
    ...
    result_path = '/dfs/user/015585/20231115_tickfull_test/europa_data/'
    update_data_tickab(start_date, end_date, basic_file_path, result_path=result_path)