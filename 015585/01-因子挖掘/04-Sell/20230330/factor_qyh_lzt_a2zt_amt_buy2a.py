...
import pandas as pd
# dtj
# 逻辑：t-1日末次涨停后，成交额中主动买单金额 / 成交额
# score:5,-0.04
# 无
factor_name = 'qyh_lzt_a2zt_amt_buy2a'#
def factor_qyh_lzt_a2zt_amt_buy2a(trans_df, return_fillna_dic=False):
    ...
    #
    if trans_df['TradeMoney'].sum() > 0:
        ratio = trans_df[trans_df['TradeBSFlag'] == 1]['TradeMoney'].sum() / trans_df['TradeMoney'].sum()
    else:
        ratio = np.nan
