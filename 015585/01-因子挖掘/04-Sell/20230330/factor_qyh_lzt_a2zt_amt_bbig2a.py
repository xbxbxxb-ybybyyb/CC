...
import pandas as pd
# dtj
# 逻辑：t-1日末次涨停后，成交额中大买单金额 / 成交额
# score:14,0.07
# 无
factor_name = 'qyh_lzt_a2zt_amt_bbig2a'#
def factor_qyh_lzt_a2zt_amt_bbig2a(trans_df, return_fillna_dic=False):
    if return_fillna_dic:
        # 返回因子为nan时的填充值
        return {factor_name: 0.9}
    ...
    if trans_df_amt_a2zt.sum() > 0:
        ratio = trans_df_amt_a2zt[trans_df_amt_a2zt>200000].sum() / trans_df_amt_a2zt.sum()
    else:
        ratio = np.nan
