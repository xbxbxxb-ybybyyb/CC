...
import pandas as pd
#
# 逻辑：t-1日末次涨停后，买单每单成交额的波动率 / 均值
# score:0，1
# 无

factor_name = 'qyh_lzt_a2zt_amtpertra_b_s'#
def factor_qyh_lzt_a2zt_amtpertra_b_s(trans_df, return_fillna_dic=False):
    if return_fillna_dic:
        # 返回因子为nan时的填充值
        return {factor_name: 2.8}
    ...
    if trans_df_amt_a2zt.mean() > 1:
        ratio = trans_df_amt_a2zt.std() / trans_df_amt_a2zt.mean()
    else:
        ratio = np.nan
