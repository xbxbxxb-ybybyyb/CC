...
import pandas as pd
# no
# 逻辑：t-1日末次涨停后，(成交额中大买单金额 - 成交额中大卖单金额) / 当日总成交额
# score:0.137
# Lzt_pj2r_sell_buy_number_corr:-0.146

factor_name = 'qyh_lzt_a2zt_amt_bsbig2ttl'#
def factor_qyh_lzt_a2zt_amt_bsbig2ttl(trans_df, return_fillna_dic=False):
    ...
    if amt_ttl > 0:
        ratio_1 = trans_df_amt_a2zt[trans_df_amt_a2zt>200000].sum() / amt_ttl
    else:
        ratio_1 = np.nan
    #
    trans_df_amt_a2zt = trans_df.groupby('TradeBuyNo')['TradeMoney'].sum()
    if amt_ttl > 0:
        ratio_2 = trans_df_amt_a2zt[trans_df_amt_a2zt>200000].sum() / amt_ttl
    else:
        ratio_2 = np.nan
    factor_dict = {factor_name: ratio_2 - ratio_1}
    # ---------------------------------------------------------------------------------------------------------------
