...
import pandas as pd
#
# 逻辑：t-1日末次涨停前后，大买单的比值
# score:-0.131,21
# Lzt_pj2r_sell_buy_number_corr
factor_name = 'qyh_lzt_ab2zt_bbig'#
def factor_qyh_lzt_ab2zt_bbig(trans_df, return_fillna_dic=False):
    ...
    amt_1 = trans_df_amt_a1zt[trans_df_amt_a1zt > 200000].sum()

    #
    trans_df_2 = trans_df[trans_df['MDTime'] <= time]
    trans_df_amt_a2zt = trans_df_2.groupby('TradeBuyNo')['TradeMoney'].sum()
    amt_2 = trans_df_amt_a2zt[trans_df_amt_a2zt > 200000].sum()
    if amt_1 > 0:
        ratio = amt_2 / amt_1
    else:
        ratio = np.nan
