...
import pandas as pd
#
# 逻辑：t-1日末次涨停后，成交额中大卖单金额 / 当日总成交额,ln
# score:0.135,26
# 无

factor_name = 'qyh_lzt_a2zt_amt_sbig2ttl_1'#
def factor_qyh_lzt_a2zt_amt_sbig2ttl_1(trans_df, return_fillna_dic=False):
    if return_fillna_dic:
        # 返回因子为nan时的填充值
        return {factor_name: 0.83}
    ...
    if (amt_ttl > 0) & (trans_df_amt_a2zt[trans_df_amt_a2zt>200000].sum() > 0):
        ratio = np.log(trans_df_amt_a2zt[trans_df_amt_a2zt>200000].sum()) / np.log(amt_ttl)
    else:
        ratio = np.nan
