...
import pandas as pd
# dtj
# 逻辑：t-1日末次涨停后，成交额中中买单金额 / 当日总成交额
# score:0.07,11
# wu
factor_name = 'qyh_lzt_a2zt_amt_bmid2ttl'#
def factor_qyh_lzt_a2zt_amt_bmid2ttl(trans_df, return_fillna_dic=False):
    ...
    if amt_ttl > 0:
        ratio = trans_df_amt_a2zt[(trans_df_amt_a2zt<=200000) & (trans_df_amt_a2zt>=50000)].sum() / amt_ttl
    else:
        ratio = np.nan
