...
class FactorTest:
    ...
    def load_data(self):
        ...
        sft_basic_path = '/data/user/018107/factor_zooS/factor_lib_v2/931_public/sft_update_normals1_filter_20160101_20191231.h5'
    ...
    def get_transfer_factor(self,split_date = 20190101,segment_number = 10):
        split_date = pd.Timestamp(str(split_date))
        factor_in = self.basic_df.loc[:split_date]
        sorted_factor_data = factor_in.sort_values(by='factor').copy()#样本内按factor排序
        num_each_group = int(len(sorted_factor_data) / segment_number)
        group_index_list = []
        for i in range(segment_number):
            group_index_list = (group_index_list + [i] * num_each_group) if i != (segment_number - 1) \
        ...
        sorted_factor_data['group'] = group_index_list
        factor_group = sorted_factor_data.groupby('group').median()[['factor', 'value']]#得到样本内分组后每组的中位数、收益率
        # 线性插值函数
        def get_linear_itpl(x, factor_group):
            for i in factor_group.index:
                if i == 0:
                    continue
                elif i < (segment_number - 1):
                    if x < factor_group['factor'][i]:
                        x1 = factor_group['factor'][i - 1]
                        y1 = factor_group['value'][i - 1]
                        x2 = factor_group['factor'][i]
                        y2 = factor_group['value'][i]
                        y = (y2 - y1) / (x2 - x1) * x + (x2 * y1 - x1 * y2) / (x2 - x1)  # i,i-1
                        break
                else:
                    x1 = factor_group['factor'][i - 1]
                    y1 = factor_group['value'][i - 1]
            ...
            return y
        #
        factor_out_linear_itpl = pd.DataFrame()
        factor_in_linear_itpl = pd.DataFrame()
        factor_out = self.basic_df.loc[split_date:]
        # 对样本外线性插值
        factor_out_linear_itpl['factor'] = factor_out['factor'].apply(
            lambda x: get_linear_itpl(x, factor_group))
        # 对样本内线性插值
        factor_in_linear_itpl['factor'] = factor_in['factor'].apply(
            lambda x: get_linear_itpl(x, factor_group))
        return [factor_in_linear_itpl,factor_out_linear_itpl]
