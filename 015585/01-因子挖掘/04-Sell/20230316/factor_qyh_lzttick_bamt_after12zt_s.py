...
import pandas as pd
#
# 逻辑：首次封板到末次封板之间，每个tick挂买总额的波动率
# score:3,0.03
# 无
factor_name = 'qyh_lzttick_bamt_after12zt_s'#
def factor_qyh_lzttick_bamt_after12zt_s(tick_df, return_fillna_dic=False):
    ...
    # 末次涨停时间
    time_2 = tick_df[(tick_df['LastPx'] == p_zt)&(tick_df['LastPx_1'] != p_zt)]['MDTime'].max()
    tick_df = tick_df[tick_df['MDTime'] < time_2]
    #
