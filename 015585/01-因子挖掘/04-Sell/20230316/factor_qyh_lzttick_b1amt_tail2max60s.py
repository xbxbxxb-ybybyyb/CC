...
# score:
# 无
factor_name = 'qyh_lzttick_b1amt_tail2max60s'#
def factor_qyh_lzttick_b1amt_tail2max60s(tick_df, return_fillna_dic=False):
    ...
    amt_max60s = tick_df['amt'].rolling(20,1).sum().max()
    # 末次涨停时间
    ...
    n = 20
    if (len(tick_df['buyamt0']) > n):
        ratio = tick_df['buyamt0'].tail(n).mean() / amt_max60s
    else:
        ratio = tick_df['buyamt0'].mean() / amt_max60s
