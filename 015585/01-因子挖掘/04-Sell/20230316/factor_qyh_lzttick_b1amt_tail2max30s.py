...
# score:13,0.1
# 无
factor_name = 'qyh_lzttick_b1amt_tail2max30s'#
def factor_qyh_lzttick_b1amt_tail2max30s(tick_df, return_fillna_dic=False):
    if return_fillna_dic:
        # 返回因子为nan时的填充值
        return {factor_name: 1.27}
    ...
    if (len(tick_df['buyamt0']) > n):
        ratio = tick_df['buyamt0'].tail(n).mean() / amt_max30s
    else:
        ratio = tick_df['buyamt0'].mean() / amt_max30s
