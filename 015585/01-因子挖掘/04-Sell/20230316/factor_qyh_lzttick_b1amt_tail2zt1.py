...
# score:GG,无效涨停
#
factor_name = 'qyh_lzttick_b1amt_tail2zt1'#
def factor_qyh_lzttick_b1amt_tail2zt1(tick_df, return_fillna_dic=False):
    ...
    time_1 = tick_df[tick_df['LastPx'] == p_zt]['MDTime'].min()# 首次涨停时间
    ...
    n = 20
    tick_df_head = tick_df.head(n)
    buy1_zt1 = tick_df_head[tick_df_head['LastPx'] == p_zt]['buyamt0'].mean()
    if (len(tick_df['buyamt0']) > n) & (buy1_zt1 > 10):
        ratio = tick_df['buyamt0'].tail(n).mean() / buy1_zt1
    else:
        ratio = 1
