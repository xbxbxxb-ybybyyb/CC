...
def run_factor(func, factor_name, factor_type, start_date, end_date, basic_file_path, result_path, append_next_tradingday=False, interval_res=True,
               t=None, last_factor_dic = None, return_single_value=False,
               data_path_dic=None):
    # 因子代码执行函数
    ...
    if factor_type in  ['TTransaction', 'TTickab','T1mTransaction','T1mTickab','T10mTransaction','T10mTickab',
        ...
        for i in range(len(tradingday_list)):
            ...
            if factor_type[-2:] == 'cs':
                tmp_df = data.groupby(level=[0]).apply(lambda x: func(x, return_single_value=return_single_value))
            else:
        ...
        if return_single_value==False: #只返回截面因子中间变量时不进行fillna
        ...
        factor_df_list = []
        for tradingday in last_zt_list:
            if os.path.exists('%s%s.pkl' % (data_path_dic[factor_type], tradingday)) == False:
                ...
                sys.stdout.write('\r%s' % (tradingday))
                sys.stdout.flush()
#                 print(tradingday)
                data = pd.read_pickle('%s%s.pkl' % (data_path_dic[factor_type], tradingday))
            if factor_type[-2:] == 'cs':
                # T-1日利用截面数据的因子需要在因子文件中就输出Tickerz作为索引
