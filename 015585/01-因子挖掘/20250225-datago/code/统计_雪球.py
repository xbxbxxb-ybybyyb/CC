...
df_p_user = pd.read_csv(f'{root_path}{package_list[4]}/{package_list[4]}_20200101to20241231.csv')
...
# 全面性
## 统计数据df_nd_stat的全面性:标的覆盖率
'''
不分type：全市场覆盖率78.56%（2020-2024），策略样本覆盖率99.42%
'''
...
for type in [1,2,3,4]:
    res_md_data = pd.merge(md_data_all_market,df_nd_stat[df_nd_stat['relevant_type'] == type][['is_value']],left_index=True,right_index=True,how = 'left')
    print(f'type = {type}: {res_md_data["is_value"].sum()},{res_md_data.shape[0]},{res_md_data["is_value"].sum() / res_md_data.shape[0]}')
...
print(f'合并所有type: {res_md_data.shape[0]},{res_md_data.sum()},{res_md_data.sum() / res_md_data.shape[0]}')

md_data_strategy = md_data[(md_data['amt'] > 0) & (md_data['close'] == md_data['zt_price'])]
for type in [1,2,3,4]:
    res_md_data = pd.merge(md_data_strategy,df_nd_stat[df_nd_stat['relevant_type'] == type][['is_value']],left_index=True,right_index=True,how = 'left')
    print(f'限定在策略样本，type = {type}: {res_md_data["is_value"].sum()},{res_md_data.shape[0]},{res_md_data["is_value"].sum() / res_md_data.shape[0]}')
...
print(f'策略样本中，合并所有type: {res_md_data.shape[0]},{res_md_data.sum()},{res_md_data.sum() / res_md_data.shape[0]}')

## 统计数据df_nd_stat的全面性：分年度统计策略样本，不同帖子数量的分位数
md_data_strategy = md_data[(md_data['amt'] > 0) & (md_data['close'] == md_data['zt_price'])]
print(md_data_strategy.shape)
res1 = pd.DataFrame() # index = type year ; columns = 指标 分位数 策略内

tmp_df_nd_stat1 = df_nd_stat[['relevant_type','year','post_neg_sum','post_neu_sum','post_pos_sum']].reset_index().set_index(['dt','Ticker','relevant_type'])
...
res1 = res1.append(res_md_data.groupby(['relevant_type','year']).apply(lambda x : x.quantile([0.25,0.5,0.75]))[['post_neg_sum','post_neu_sum','post_pos_sum']])
res1.unstack().to_csv('策略标的帖子数目统计.csv')
...
'''
# 一致性：底层数据统计每日帖子数
df_p_info_stock = pd.merge(df_p_info, df_p_stock, left_on='post_id', right_on='post_id', how = 'left')
df_p_info_stock['dt'] = df_p_info_stock['post_time'].apply(lambda x : pd.Timestamp(x.split(' ')[0]))
stat_df_p_info_stock = df_p_info_stock.groupby(['dt','stock_id','post_type','relevant_level','sentiment'])['post_id'].count()
...
# post_type = 2, relevant_level = 1 >>>> 高相关长文 2
# post_type = 2, relevant_level = 0 >>>> 低相关长文
# post_type = 1, relevant_level = 1 >>>> 高相关短篇
# post_type = 1, relevant_level = 0 >>>> 低相关短篇
## 验证高相关长文
stat_df_p_info_stock_1 = pd.merge(stat_df_p_info_stock[(stat_df_p_info_stock['post_type'] == 2) & (stat_df_p_info_stock['relevant_level'] == 1)].set_index(['dt','Ticker']),
                                  df_nd_stat[df_nd_stat['relevant_type'] == 2][['post_neg_sum','post_neu_sum','post_pos_sum']],
                                  left_index=True, right_index=True,how='outer')
for col in [-1,0,1]:
    stat_df_p_info_stock_1[col] = stat_df_p_info_stock_1[col].fillna(0)
stat_df_p_info_stock_1['test1'] = stat_df_p_info_stock_1[-1] - stat_df_p_info_stock_1['post_neg_sum']
stat_df_p_info_stock_1['test2'] = stat_df_p_info_stock_1[0] - stat_df_p_info_stock_1['post_neu_sum']
stat_df_p_info_stock_1['test3'] = stat_df_p_info_stock_1[1] - stat_df_p_info_stock_1['post_pos_sum']
for col in ['test1','test2','test3']:
    print(col, '高相关长文中，底层和衍生不一致占比：', len(stat_df_p_info_stock_1[stat_df_p_info_stock_1[col] != 0]) / len(stat_df_p_info_stock_1))
## 验证提及的短贴
