...
# 因子计算
factor_columns = ['like_neg_sum', 'fav_neg_sum',
       'retweet_neg_sum', 'reply_neg_sum', 'post_neg_sum', 'uid_neg',
       'user_avg_barage_neg', 'like_neu_sum', 'fav_neu_sum', 'retweet_neu_sum',
       'reply_neu_sum', 'post_neu_sum', 'uid_neu', 'user_avg_barage_neu',
       'like_pos_sum', 'fav_pos_sum', 'retweet_pos_sum', 'reply_pos_sum',
       'post_pos_sum', 'uid_pos', 'user_avg_barage_pos', 'like_all_sum',
       'fav_all_sum', 'retweet_all_sum', 'reply_all_sum', 'post_all_sum',
       'uid_all', 'user_avg_barage_all', 'senti_score_div', 'senti_score_log',
       'senti_conform']
...
# ### 全市场
# for type in [1,2,3,4]:
#     md_data_calc = pd.merge(md_data, df_nd_stat[df_nd_stat['relevant_type'] == type][factor_columns], left_index=True, right_index=True, how='left')
...
#     md_data_calc.to_pickle(f'factor_type{type}_雪球_全市场_20250228.pkl')
#     corr_factor = md_data_calc.groupby('dt').apply(lambda x: x.rank().corr())
#     corr_factor.stack().reset_index().groupby(['level_1', 'level_2'])[0].mean().unstack().loc[factor_columns,columns_ori].to_csv(f'因子间相关性_type{type}_雪球_全市场.csv')
#     #
#     md_data_factor = md_data_calc.drop(columns_ori, axis=1)
#     corr_df = md_data_factor.groupby('dt').apply(lambda x: x.rank().corr()['label'])
#     corr_df.mean().to_csv(f'因子IC日均_type{type}_雪球_全市场_2020_2024.csv')
...
for type in [1,2,3,4]:
    md_data_calc = pd.merge(md_data_europa, df_nd_stat[df_nd_stat['relevant_type'] == type][factor_columns], left_index=True, right_index=True, how='left')
    for col in factor_columns:
        for func in func_dic:
            for rolling_day in rolling_param:
                name = f'{col}_{func}_{rolling_day}'
    ...
    md_data_calc.to_pickle(f'factor_type{type}_雪球_europa_20250228.pkl')
    corr_factor = md_data_calc.groupby('dt').apply(lambda x: x.rank().corr())
    corr_factor.stack().reset_index().groupby(['level_1', 'level_2'])[0].mean().unstack().loc[factor_columns,columns_ori].to_csv(f'因子间相关性_type{type}_雪球_europa.csv')
    #
    md_data_factor = md_data_calc.drop(columns_ori, axis=1)
    corr_df = md_data_factor.groupby('dt').apply(lambda x: x.rank().corr()['label'])
    corr_df.mean().to_csv(f'因子IC日均_type{type}_雪球_europa_2020_2024.csv')
