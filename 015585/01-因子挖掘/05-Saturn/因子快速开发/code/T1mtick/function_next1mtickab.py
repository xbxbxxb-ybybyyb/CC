...
def f_pro_l2hv(tick_df): # last /high * vol
    return tick_df['LastPx'] / tick_df['HighPx'] * tick_df['VolumeTrade']
def f_pro_l2h2v(tick_df): # last /high / vol
    return (tick_df['LastPx'] / tick_df['HighPx']+1e-5) / (tick_df['VolumeTrade']+1e-5)
def f_pro_vwap2hv(tick_df): # vwap /high * vol
    return tick_df['TotalValueTrade']/tick_df['TotalVolumeTrade'] / tick_df['HighPx'] * tick_df['VolumeTrade']
def f_pro_vwap2h2v(tick_df): # vwap /high / vol
    return (tick_df['TotalValueTrade']/tick_df['TotalVolumeTrade'] / tick_df['HighPx']+1e-5) / (tick_df['VolumeTrade']+1e-5)
# 时间筛选函数，返回时间点
