...
#
# 逻辑：0931，总挂卖除以总成交
# score:3.6,0.01:挂卖除以成交不说明问题，与挂买不同
#
factor_name = 'qyh_T1mtick_1m_samt2trade'#
def factor_qyh_T1mtick_1m_samt2trade(tick_df, return_fillna_dic=False):
    ...
    if tick_df['amt'].sum() > 10:
        ratio = (bstotal - buytotal).sum() / tick_df['amt'].sum()
    else:
        ratio = np.nan
