...
#
# 逻辑：0931，卖1除以总成交
# score:GG:卖单只能和挂买比较，不能和成交比较
#
factor_name = 'qyh_T1mtick_1m_s1amt2trade'#
def factor_qyh_T1mtick_1m_s1amt2trade(tick_df, return_fillna_dic=False):
    ...
    if tick_df['amt'].sum() > 10:
        ratio = (tick_df['sellamt10']).sum() / tick_df['amt'].sum()
    else:
        ratio = np.nan
