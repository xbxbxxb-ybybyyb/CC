...
import pandas as pd
# 待提交
# 逻辑：0931，买1波动率/成交额波动率
# score:23,14,0.06
# 无
factor_name = 'qyh_T1mtick_1m_b1amt_s2trade_s'#
def factor_qyh_T1mtick_1m_b1amt_s2trade_s(tick_df, return_fillna_dic=False):
    ...
    if tick_df['amt'].std() > 10:
        ratio = tick_df['buyamt0'].std() / tick_df['amt'].std()
    else:
        ratio = np.nan
