...
import pandas as pd
# 待提交
# 逻辑：0931，买1占总成交比例，总体计算
# score:25.6，20，0.08（但头部略次）
# 无
factor_name = 'qyh_T1mtick_1m_b1amt2trade'#
def factor_qyh_T1mtick_1m_b1amt2trade(tick_df, return_fillna_dic=False):
    ...
    if tick_df['amt'].sum() > 10:
        ratio = tick_df['buyamt0'].sum() / tick_df['amt'].sum()
    else:
        ratio = np.nan
