...
import pandas as pd
#
# 逻辑：0931，后30S - 前30S，卖1占买单挂单比例的中位数的差
# score:0.02,7.79
#
factor_name = 'qyh_T1mtick_1m_s1amt_ratio_buy_delta'#
def factor_qyh_T1mtick_1m_s1amt_ratio_buy_delta(tick_df, return_fillna_dic=False):
    ...
    length = int(len(tick_df['sellamt10'] / buytotal) /2)
    ratio_before = (tick_df['sellamt10'] / buytotal)[:length].median()
    if abs(ratio_before) > 5:
        ratio_before = 5
    ratio_later = (tick_df['sellamt10'] / buytotal)[length:].median()
