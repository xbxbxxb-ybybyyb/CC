...
import pandas as pd
# 待提交
# 逻辑：0931，成交额在前25%的tick里，买1占挂卖单比例
# score:18.2，6.9，0.05
# 无
factor_name = 'qyh_T1mtick_1m_b1amt_ratio_sell_bigtrade'#
def factor_qyh_T1mtick_1m_b1amt_ratio_sell_bigtrade(tick_df, return_fillna_dic=False):
    ...
    if len(tick_df) == 0:
        ...
        ratio = tick_df['buyamt'+str(0)].sum() / (bstotal - buytotal).sum()
