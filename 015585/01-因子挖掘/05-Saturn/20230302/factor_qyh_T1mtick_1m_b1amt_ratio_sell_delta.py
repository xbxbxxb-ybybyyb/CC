...
import pandas as pd
#
# 逻辑：0931，后30S - 前30S，买1占卖单挂单比例的中位数的差
# score:不太行
#
factor_name = 'qyh_T1mtick_1m_b1amt_ratio_sell_delta'#
def factor_qyh_T1mtick_1m_b1amt_ratio_sell_delta(tick_df, return_fillna_dic=False):
    ...
    length = int(len(tick_df['buyamt1'] / (bstotal - buytotal)) / 2)
    ratio_before = (tick_df['buyamt1'] / (bstotal - buytotal))[:length].median()
    if abs(ratio_before) > 5:
        ratio_before = 5
    ratio_later = (tick_df['buyamt1'] / (bstotal - buytotal))[length:].median()
    if abs(ratio_later) > 5:
        ratio_later = 5

    factor_dict = {factor_name: ratio_later - ratio_before}
    # ---------------------------------------------------------------------------------------------------------------
