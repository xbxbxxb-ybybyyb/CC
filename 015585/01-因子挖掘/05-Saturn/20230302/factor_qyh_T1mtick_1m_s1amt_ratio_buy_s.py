...
import numpy as np
import pandas as pd
# 待提交（factor_qyh_T1mtick_1m_bs1amt_ratio_sell_s）
# 逻辑：0931，每个tick的卖1总额占买单挂单比例的波动率
# score:24,18,-0.058（不过头部表现不如总体）：波动越小越好
# 无
factor_name = 'qyh_T1mtick_1m_s1amt_ratio_buy_s'#
def factor_qyh_T1mtick_1m_s1amt_ratio_buy_s(tick_df, return_fillna_dic=False):
    ...
    ratio = ((tick_df['sellamt'+str(10)])/ buytotal).std()
