...
import pandas as pd
#
# 逻辑：0931，后30S - 前30S，买单占比的中位数
# score:3.61,-0.01
#
factor_name = 'qyh_T1mtick_1m_bamt_ratio_delta'#
def factor_qyh_T1mtick_1m_bamt_ratio_delta(tick_df, return_fillna_dic=False):
    ...
    length = int(len(buytotal / bstotal) / 2)

    ratio_before = (buytotal / bstotal)[:length].median()
    if abs(ratio_before) > 5:
        ratio_before = 5
    ratio_later = (buytotal / bstotal)[length:].median()
