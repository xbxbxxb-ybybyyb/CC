...
# 待提交
# 逻辑：0931，买1总额/卖1总额
# score:25.8,14.8,0.06
# 无
factor_name = 'qyh_T1mtick_1m_b12s1amt'#
def factor_qyh_T1mtick_1m_b12s1amt(tick_df, return_fillna_dic=False):
    ...
    if (tick_df['sellamt10']).sum() > 10:
        ratio = (tick_df['buyamt0']).sum() / (tick_df['sellamt10']).sum()
    else:
        ratio = np.nan
