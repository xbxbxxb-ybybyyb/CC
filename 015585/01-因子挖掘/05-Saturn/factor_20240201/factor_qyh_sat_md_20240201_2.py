...
# 下影线的5日和10日变异系数比例
# 0.058，21，0.056
factor_name = 'qyh_sat_md_20240201_2'
def factor_qyh_sat_md_20240201_2(start_date, end_date, IO, return_fillna_dic=False):
    ...
    df_ori[factor_name] = df_ori['factor'].unstack().rolling(5,1).apply(lambda x : f_calc_cv(x)).stack() \
                          / df_ori['factor'].unstack().rolling(10,1).apply(lambda x : f_calc_cv(x)).stack()
    #
