...
class strongFactorTest:
    def __init__(self, start_date, end_date, segment_number=20, style_list=['freeturn', 'ZT_Time'],
    ...
    def load_data(self):
        if self.test_sample == 'zt':
            df = IO.read_data([self.start_date, self.end_date], alt='/data/user/018107/factor_zoo1/public/sft_base_data_zt_v2.h5')
    ...
    def factor_test(self, factor_df, result_path, factor_corr_test=False, generate_pdf=True, tag=''):
        ...
        if 'ZT_Time' in self.result_dic['double_group_sort']:
            self.result_dic['check_diff_res'], self.result_dic['check_score_res'] = \
                self.check_score(self.result_dic['double_group_sort']['ZT_Time'].copy(),
                                 self.result_dic['corr_month'].copy(),
                                 np.sign(self.result_dic['corr_sta'].loc['corr_tot']))
            self.result_dic['check_score_list'] = \
                self.check_score_list(self.result_dic['double_group_sort_dic'].copy(),
    ...
    def check_score_list(self, double_group_df_dic, month_df_dic, dir, label_test=['value', 'mixed']):
        ...
        for key in keys:
            ...
            x, score_df = self.check_score(double_group_df['ZT_Time'].copy(), month_df.copy(), dir)
