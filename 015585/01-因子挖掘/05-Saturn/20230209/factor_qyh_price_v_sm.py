...
# 已提交：0216
# 逻辑：聪明钱因子：取过去20日的5min数据，计算每个5min数据的smart指标：abs(涨跌幅)/ln(成交量)，
# 选取smart靠前的5min集合，计算集合上的均价（按成交量），比上总体的均价（按成交量）
# score:23.94，-0.07：聪明钱在低位吸筹
# 无高corr
import pandas as pd
import numpy as np
import os
...
s = FactorData()
factor_name = 'qyh_price_v_sm'
def factor_qyh_price_v_sm(start_date, end_date, IO, return_fillna_dic=False):
    if return_fillna_dic:
        # 返回因子为nan时的填充值
        return {factor_name: 0.6254}
    # 可修改的因子编写部分
