...
s = FactorData()
factor_name = 'qyh_kdj_j_change'
def factor_qyh_kdj_j_change(start_date, end_date, IO, return_fillna_dic=False):
    if return_fillna_dic:
        # 返回因子为nan时的填充值
        return {factor_name: 20}
    # 可修改的因子编写部分
    ...
    # f_data[factor_name] = (f_data['K-D'].unstack().rank(axis = 1) + f_data['K-D_deri'].unstack().rank(axis = 1)).stack()
    f_data['J'] = 3 * f_data['K'] - 2 * f_data['D']
    f_data[factor_name] = f_data['J'] - f_data['J'].unstack().rolling(20,1).min().stack()
