...
s = FactorData()
# dtj
# pct * turn的5日集中度/60日集中度
# 28，0.049，0.045
factor_name = 'qyh_sat_md_20240125_5'
def factor_qyh_sat_md_20240125_5(start_date, end_date, IO, return_fillna_dic=False):
    ...
    df_ori[factor_name] = df_ori['factor'].unstack().rolling(5,1).apply(lambda x : f_calc_cct(x)).stack() \
                          / df_ori['factor'].unstack().rolling(60,1).apply(lambda x : f_calc_cct(x)).stack()
    #
