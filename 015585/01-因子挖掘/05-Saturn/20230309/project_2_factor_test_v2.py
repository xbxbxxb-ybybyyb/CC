...
class pj2FactorTest:
    def __init__(self, start_date, end_date, segment_number=20, style_list=['free_turn', 'Lzt_ZT_Time', 'tradingday'],
                 cal_mi=True,
    ...
    def load_data(self):
        ...
        if high_open is not None:
            high_open_filter = df['T_o2pre'] <= high_open
        ...
        # 202302 涨停时间改为距离0930的毫秒数，剔除中午休市
        if 'Lzt_ZT_Time' in self.style_list:
            ...
            df['Lzt_ZT_Time'] = df['Lzt_ZT_Time'].apply(lambda x: inttime2deltamls(x))
        # 202302补充ic decay需要的label_decay
        if self.buy_type == '0931':
            label_decay = ['label_2v2o10d1', 'label_3v2o10d1', 'label_4v2o10d1', 'label_5v2o10d1']
            df[label_decay] = df[label_decay] * 100
            self.basic_df = df[['value', 'class', 'month', 'year'] + self.style_list + label_decay]
        else:
    ...
    def generate_pdf(self, result_dic, save_path):
        ...
        p_score_list_sta = generate_plot(result_dic['check_score_list'][
                                         ...
                                         plot_name='Interval Score', x_label='interval', y_label='score',
                                         color=['#525252', '#1f4068', '#900d0d', '#e23e57'], plot_type='bar',
                                         stacked=True)
        ...
        p_monthly_corr = generate_plot(result_dic['corr_month'][['value', 'class']],
                                       plot_name='Monthly Corr - Value&Class', x_label='month',
                                       y_label='corr of value&class', plot_type='bar', color=['#e23e57', '#1b7fbd'],
                                       rot=25)
    ...
    def check_score_list(self, double_group_df_dic, month_df_dic, dir, label_test=['value']):
        ...
        for key in keys:
            ...
            if 'tradingday' in self.style_list:
                x, score_df = self.check_score(double_group_df['tradingday'], month_df, dir)
                res_df[key] = score_df.loc['score']
                return res_df.T.sort_index()
            else:
                raise ValueError("tradingday必须在stylelist中")
    ...
    def get_decay_ic(self, decay_days=5):
        ...
        res = [factor_df.corr(self.basic_df['value'], method='spearman')]
        for i in range(2, decay_days + 1):
            label_df = self.basic_df['label_' + str(i) + 'v2o10d1']
            corr_decay_i = factor_df.corr(label_df, method='spearman')
            res.append(corr_decay_i)
        res = pd.DataFrame(res, index=[('T+' + str(i + 1)) for i in range(decay_days)], columns=['IC'])
        return res