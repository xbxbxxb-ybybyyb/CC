...
import pandas as pd
#
# 逻辑：0931，每个tick挂买和成交额的corr(>0) - 挂卖和成交额的corr(<0)
# score:4,2,0.03
# 无
factor_name = 'qyh_T1mtick_1m_corr_btst'#
def factor_qyh_T1mtick_1m_corr_btst(tick_df, return_fillna_dic=False):
    ...
    df_st = pd.concat([bstotal - buytotal,tick_df['amt']],axis = 1)
    corr = df_st.corr(method = 'spearman').iloc[0,1]
    df_bt = pd.concat([buytotal,tick_df['amt']],axis = 1)
    corr = df_bt.corr(method = 'spearman').iloc[0,1] - corr
