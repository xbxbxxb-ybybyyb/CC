...
# excel
# res_excel = pd.read_excel('/data/user/015585/01-因子挖掘/05-Saturn/20240202因子修改_原始/因子列表_qyh.xlsx')
# basic_file_path ='/data/group/800463/data/project2_public/factor_lib/Basic_closed_hf_finish_20160101_20191231.h5'
# #
# date_list = os.listdir('/data/user/015585/01-因子挖掘/05-Saturn/20240202因子修改_原始/')
# date_list = [x for x in date_list if 'factor_' in x and '.py' not in x]
# res = pd.DataFrame()
# for date in date_list:
#     factor_list_date = os.listdir('/data/user/015585/01-因子挖掘/05-Saturn/20240202因子修改_原始/' + date + '/')
#     factor_list_date = [x[:-3] for x in factor_list_date  if 'factor_' in x]
#     for factor in factor_list_date:
#         name = factor[7:]
#         print(factor)
#         if name in ['qyh_T1mtick_1m_amt_t2smin','qyh_T1mtick_1m_pmax_t_b1meanratio','qyh_T1mtick_1m_pmax_t_s1meanratio','qyh_T1mtra_r_amt2pct_down']:
#             continue
...
#                                 20160101, 20191231,
#                                 basic_file_path,
#                                 '/data/user/015585/01-因子挖掘/05-Saturn/20240202因子修改_原始/save_file/', interval_res=False)
#         for col in factor_df0:
#             print(col, abs(factor_df0[col]).max())
#         res[name] = factor_df0[name]
#         print(len(res.columns))
#
# res.to_pickle('/data/user/015585/01-因子挖掘/05-Saturn/20240202因子修改_原始/save_file/res_ori.pkl')
