...
class factor_qyh_neptune_test_3(BaseFactor):
    strategy_name = "neptune"
    factor_name = "qyh_neptune_test_3"
    fill_na_value = 0
    ...
    t_1_factor_data = [
        {'name': 'AShareEODDerivativeIndicator', # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         'path': '/data/group/800080/warehouse_event/prod/DATABASE/WIND/AShareEODDerivativeIndicator/AShareEODDerivativeIndicator.h5',
         'lag': 80, #注意为正数
         'column': ['S_VAL_PE_TTM']
    }]  # T-N factor数据，格式如上
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori[self.factor_name] = df_ori['S_VAL_PE_TTM'].unstack().rolling(5,1).mean().stack().apply(lambda x : round_(x,4))  # 矩阵大小不同时，Python精度差异，导致计算值不同
            # -------------------------------------------------------------------------------------------------------------------
