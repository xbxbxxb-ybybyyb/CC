...
class factor_qyh_neptune_20250424_3(BaseFactor):
    owner = 'qyh'
    strategy_name = "neptune"
    factor_name = 'qyh_neptune_20250424_3'
    fill_na_value = 0
    need_pre_calculate_T_N = True
    factor_explain = "close/pre的20日min" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True:
            ...
            res = (df).max(axis=1)
            res = res.unstack().rolling(20,2).min().stack().to_frame()
