...
class factor_qyh_neptunelong_longterm_20250904_cunhuo4(BaseFactor):
    strategy_name = "neptune"
    factor_name = "qyh_neptunelong_longterm_20250904_cunhuo4"
    fill_na_value = ('industry_median',0)
    need_pre_calculate_T_N = True
    owner = "qyh"  # 开发人员姓名
    factor_explain = "财务：存货diff" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_balancesheet['存货diff'] = df_balancesheet['INVENTORIES'].groupby(['dt','Ticker']).diff()
            res1 = (df_balancesheet['存货diff']).groupby(['dt','Ticker'])\
