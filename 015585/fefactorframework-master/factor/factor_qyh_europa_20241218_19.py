...
class factor_qyh_europa_20241218_19(BaseFactor):
    strategy_name = "jupiter/europa"
    factor_name = "qyh_europa_20241218_19"
    fill_na_value = 0
    need_pre_calculate_T_N = False
    owner = "qyh"  # 开发人员姓名
    factor_explain = "撤单数据中，价格大于9%的最后100单里，撤单价格/撤单vwap价格的累计和" # 因子逻辑解释
    ...
    def calculate(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            #
            if zcz:
                p = round_(pre_close * (1 + 0.09 * 2), 5)
            elif bj:
                p = round_(pre_close * (1 + 0.09 * 3), 5)
            else:
                p = round_(pre_close * 1.09, 5)
            ...
            if zcz:
                cancel_df['factor'] = (((cancel_df['OrderPrice']/pre_close)-1)/2 + 1) / (((cancel_df['vwap']/pre_close)-1)/2+1)
            if bj:
                cancel_df['factor'] = (((cancel_df['OrderPrice']/pre_close)-1)/3 + 1) / (((cancel_df['vwap']/pre_close)-1)/3+1)
            else:
