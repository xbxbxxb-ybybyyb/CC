...
class factor_qyh_neptune_20250417_29(BaseFactor):
    owner = 'qyh'
    strategy_name = "neptune"
    factor_name = 'qyh_neptune_20250417_29'
    fill_na_value = 0
    need_pre_calculate_T_N = True
    factor_explain = "5分钟最高价加权cv，5日min" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True:
            ...
            df_amt = database['amt']
            res = (df_high * df_amt).apply(f_calc_cv,axis=1).divide(df_amt.max(axis=1).replace(0,np.nan))
