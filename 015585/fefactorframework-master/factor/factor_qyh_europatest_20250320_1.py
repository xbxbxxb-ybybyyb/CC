...
class factor_qyh_europatest_20250320_1(BaseFactor):
    strategy_name = "europa"
    factor_name = "qyh_europatest_20250320_1"
    fill_na_value = 0.95
    need_pre_calculate_T_N = True
    owner = "qyh"  # 开发人员姓名
    factor_explain = "最高价/20日最高价" # 因子逻辑解释
    ...
    t_1_factor_data = [
        {'name': 'MD_CHINA_STOCK_DAILY_WIND', # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 80, #注意为正数
         'column': ['pct_chg', 'turn', 'pre_close', 'amt', 'vwap', 'high']
    }]  # T-N factor数据，格式如上
        ...
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            for col in ['high', 'low', 'open', 'vwap', 'close']:
                if col in df_ori.columns:
                    df_ori.loc[df_ori['zcz'] == 1, col] = ((df_ori.loc[df_ori['zcz'] == 1, col]/df_ori.loc[df_ori['zcz'] == 1, 'pre_close'] - 1) / 2 + 1) * \
                                                          df_ori.loc[df_ori['zcz'] == 1, 'pre_close']
                    df_ori.loc[df_ori['bj'] == 1, col] = ((df_ori.loc[df_ori['bj'] == 1, col]/df_ori.loc[df_ori['bj'] == 1, 'pre_close'] - 1) / 3 + 1) * \
                                                          df_ori.loc[df_ori['bj'] == 1, 'pre_close']
            # -------------------------------------------------------------------------------------------------------------------
            df_ori['factor'] = df_ori['high'] / df_ori['high'].unstack().rolling(20,1).max().stack()
            ...
            logger.info(df_ori[[self.factor_name]].tail())
            return database
