...
class factor_qyh_neptune_20250327_14(BaseFactor):
    strategy_name = "neptune"
    factor_name = "qyh_neptune_20250327_14"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "qyh"  # 开发人员姓名
    factor_explain = "最低价对应的涨跌幅，除以20日离群" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori[self.factor_name] = df_ori['factor'] / df_ori['factor'].unstack().rolling(20,1).apply(f_calc_m2m).stack()
            # -------------------------------------------------------------------------------------------------------------------
