...
class factor_qyh_neptune_20250403_11(BaseFactor):
    strategy_name = "neptune"
    factor_name = "qyh_neptune_20250403_11"
    fill_na_value = 1
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori[self.factor_name] = df_ori['factor'].unstack().rolling(5,1).max().stack() / \
                                       df_ori['factor'].unstack().rolling(60,1).min().stack().apply(lambda x : round_(x,5)).replace(0,np.nan)
