...
# 财务CS
class factor_qyh_finance_new_test3(BaseFactor):
    strategy_name = "neptune"
    factor_name = "qyh_finance_new_test3"
    fill_na_value = 0
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            res = (df_balancesheet['FIX_ASSETS'] + df_balancesheet['TOT_CUR_ASSETS'])\
                .groupby(['dt','Ticker']).apply(lambda x : x.tail(4).sum()).to_frame(name=self.factor_name)
