...
class factor_qyh_neptune_shortterm_20250717_8(BaseFactor):
    strategy_name = "neptune"
    factor_name = "qyh_neptune_shortterm_20250717_8"
    fill_na_value = 1
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            res = tick_df['factor'].max() / tick_df['factor'].mean() if abs(tick_df['factor'].mean()) > 1e-6 else np.nan
            # if zcz:
            #     res = res/2
            #
