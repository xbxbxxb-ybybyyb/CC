...
class factor_qyh_neptunelong_longterm_20250911_ldfz2zzc8(BaseFactor):
    strategy_name = "neptunelong"
    factor_name = "qyh_neptunelong_longterm_20250911_ldfz2zzc8"
    fill_na_value = ('industry_median',0)
    need_pre_calculate_T_N = True
    owner = "qyh"  # 开发人员姓名
    factor_explain = "财务：流动负债diff占比" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_balancesheet['流动负债diff_std'] = df_balancesheet['流动负债diff'] / df_balancesheet['单季度平均资产总额'].replace(0,np.nan)
            ##
            res1 = (df_balancesheet['流动负债diff_std']).groupby(['dt','Ticker'])\
