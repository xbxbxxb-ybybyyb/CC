...
class factor_qyh_neptune_shortterm_20250619_10(BaseFactor):
    strategy_name = "neptune"
    factor_name = "qyh_neptune_shortterm_20250619_10"
    fill_na_value = 0
    need_pre_calculate_T_N = False
    owner = "qyh"  # 开发人员姓名
    factor_explain = "ttickab1m 订单金额 统计量" # 因子逻辑解释
    ...
    def calculate(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            res = (tick_df['factor'].max() + tick_df['factor'].min()) / (tick_df['factor'].mean() + tick_df['factor'].min()) if tick_df['factor'].mean() > 0 else np.nan
            # if zcz:
            #     res = res/2
