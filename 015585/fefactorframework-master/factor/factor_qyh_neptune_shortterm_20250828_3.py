...
class factor_qyh_neptune_shortterm_20250828_3(BaseFactor):
    strategy_name = "neptune"
    factor_name = "qyh_neptune_shortterm_20250828_3"
    fill_na_value = ('median',0)
    need_pre_calculate_T_N = True
    owner = "qyh"  # 开发人员姓名
    factor_explain = "相对买均 统计量" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            #
            tick_df['factor'] = (tick_df['ValueTrade']/(tick_df['VolumeTrade'].replace(0,np.nan)) - tick_df['WeightedAvgBidPx'])/(tick_df['pre_close'])
            res = tick_df['factor'].max() / tick_df['factor'].mean() if tick_df['factor'].mean()>0 else np.nan
            if zcz:
                res = res/2
            #
