...
class factor_qyh_neptune_20250320_4(BaseFactor):
    strategy_name = "neptune"
    factor_name = "qyh_neptune_20250320_4"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "qyh"  # 开发人员姓名
    factor_explain = "涨跌影线相关性" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            df_ori['xy'] = (df_ori[x] * df_ori[y]).apply(lambda x : round_(x,5))
            ...
            df_ori['stdx'] = df_ori[x].unstack().rolling(20, 5).std().stack().apply(lambda x : round_(x,5)).replace(0,np.nan)
            df_ori['stdy'] = df_ori[y].unstack().rolling(20, 5).std().stack().apply(lambda x : round_(x,5)).replace(0,np.nan)
            df_ori['factor'] = (df_ori['exy'] - df_ori['ex'] * df_ori['ey']).apply(lambda x : round_(x,5)) \
            ...
            # df_ori[self.factor_name] = df_ori['factor'].unstack().rolling(5,1).mean().stack().apply(lambda x : round_(x,5))
