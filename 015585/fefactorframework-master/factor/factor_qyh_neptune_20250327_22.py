...
class factor_qyh_neptune_20250327_22(BaseFactor):
    strategy_name = "neptune"
    factor_name = "qyh_neptune_20250327_22"
    fill_na_value = 0
    need_pre_calculate_T_N = True
    owner = "qyh"  # 开发人员姓名
    factor_explain = "涨跌幅用amt加权的5日min" # 因子逻辑解释
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            # -------------------------------------------------------------------------------------------------------------------
            df_ori['factor'] = df_ori['amt'] * df_ori['pct_chg']
            df_ori[self.factor_name] = df_ori['factor'].unstack().rolling(5,1).min().stack() / df_ori['amt'].unstack().rolling(5,1).min().stack()
            df_ori[self.factor_name] = df_ori[self.factor_name].apply(lambda x : 1e8 if x > 1e8 else -1e8 if x < -1e8 else x)
            # -------------------------------------------------------------------------------------------------------------------
