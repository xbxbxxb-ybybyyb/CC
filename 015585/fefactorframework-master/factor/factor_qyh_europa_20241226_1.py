...
class factor_qyh_europa_20241226_1(BaseFactor):
    strategy_name = "jupiter/europa"
    factor_name = "qyh_europa_20241226_1"
    fill_na_value = 0
    need_pre_calculate_T_N = False
    owner = "qyh"  # 开发人员姓名
    factor_explain = "撤单数据中，价格大于9%的前1/2订单，订单价格的相对离散程度" # 因子逻辑解释
    ...
    def calculate(self, database):
        if database["skip"] == True: # 纯h5文件的T-1_Factor保留这一段，但不产生影响
            ...
            mean = (cancel_df['factor'] * cancel_df['OrderQty']).sum() / (cancel_df['OrderQty'].sum()+1)
            res = cancel_df['factor'].max() / mean if round_(mean, 5) > 0 else np.nan
