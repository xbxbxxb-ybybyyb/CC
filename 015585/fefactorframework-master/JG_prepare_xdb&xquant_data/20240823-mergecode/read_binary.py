...
import filecmp
for strategy in ['saturn_sell','europa_jupiter']:
    for data_type in ['xdb_tickex','xdb_trade','xdb_order']:
        path1 = '/dfs/user/015585/test/{}/{}/'.format(strategy, data_type)
        path2 = '/dfs/group/800463/data/xdb_data_lag3_new/{}/{}/'.format(strategy, data_type)
        for file in os.listdir(path1):
            print(strategy, data_type, file)
            df1 = pd.read_pickle(path1 + file)
            df2 = pd.read_pickle(path2 + file)
            if not df1.equals(df2):
                print(strategy, data_type, file, '不等')
# for strategy in ['saturn_sell','europa_jupiter']:
#     for data_type in ['xdb_tick1s','xdb_tickfull']:
#         path1 = '/dfs/user/015585/test/{}/{}/'.format(strategy, data_type)
#         path2 = '/dfs/group/800463/data/xdb_data_lag3_new/{}/{}/'.format(strategy, data_type)
#         for file in os.listdir(path1):
#             print(strategy, data_type, file)
#             if not filecmp.cmp(path1 + file, path2 + file):
#                 print(strategy, data_type, file, '不等')
# path = '/dfs/user/015585/test/saturn_sell/xdb_tickfull/'
# df = get_xdb_data_test(path,'20180131')