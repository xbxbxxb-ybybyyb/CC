...
1、财务因子不进行市值中性化
2、财务因子取财报获取后的第一个交易日的label作为评测依据，非财报相邻日期被剔除
...
class FactorTest:
    ...
    def check_score(self, factor_group_df, group_corr_df, dir, label_test=['value', 'class']):
        ...
        diff_dic = {'value': {'reverse': [0.6, 0.5], 'diff': [0.8, 1.2]},
                    'class': {'reverse': [0.6, 0.5], 'diff': [0.13, 0.18]}}

        stability_dic = {'value': [0.01, 0.02],
                         'class': [0.01, 0.02]}
    ...
    def get_factor_sort(self, data=None, factor_group_num=5):
        data = self.basic_df.copy() if (data is None) else data
        after_data = pd.DataFrame()
        for date in data.index.get_level_values(0).unique():
            date_factor_data = data[data.index.get_level_values(0) == date].copy()
            num_each_group = len(date_factor_data) // factor_group_num
            group_list = []
            for j in range(factor_group_num):
                if j == 0:
                    group_list += [j] * (len(date_factor_data) - (factor_group_num - 1) * num_each_group)
                else:
                    group_list += [j] * num_each_group
            date_factor_data = date_factor_data.sort_values(by='factor')
            date_factor_data['factor_group'] = group_list
            after_data = after_data.append(date_factor_data)
        tmp_result = pd.DataFrame()
        for date in after_data.index.get_level_values(0).unique():
            date_after_data = after_data.loc[date]
            for j in range(factor_group_num):
                tmp_result[(date, j)] = date_after_data[date_after_data['factor_group'] == j].mean()[self.label_list]
    ...
    def get_corr_group(self, data=None, segment_number=None,):
        ...
        sorted_factor_data = data.sort_values(['dt','Ticker']).copy()
