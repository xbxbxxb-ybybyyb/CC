...
class FactorTest:
    def __init__(self, start_date, end_date, segment_number=20,
        ...
        if self.strategy_name == 'neptunelong':
            self.return_type = 'label_t11o30d1'
    ...
    def load_data(self):
        ...
        if self.strategy_name == 'neptunelong':
            sft_basic_path = '/dfs/group/800463/public/projectZZmkt_public/factor_lib/sft_update_931_20160101_20201231.pkl'  # 这个文件里有label和所有因子
            df = pd.read_pickle(sft_basic_path).loc[pd.Timestamp(str(self.start_date)):pd.Timestamp(str(self.end_date))]
        else:
            logger.error('strategy_name error, must in [neptunelong]')
            raise TypeError
        # 202507 新增筛选
        ...
        all_factors_available = ['label_t2o30d1','label_t4o30d1','label_t6o30d1','label_t11o30d1','label_t2o9d1_pos', 'label_t2o9d1_neg','industry_code']
    ...
    def factor_test(self, factor_df, cost_list, result_path, factor_corr_test=False, generate_pdf=False):
        ...
        if factor_corr_test:
            check_res = pd.read_excel(
                '/dfs/group/800463/public/projectZZmkt_public/factor_lib/check_res_tot_neptunelong.xlsx')
