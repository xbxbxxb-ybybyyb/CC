...
class TD3Model(TrainBase):
    ...
    def train_model(self, start, end, n_cpu, model_kwargs=None):
        ...
        #     model = TD3.load(f'{self.base_dir}/model/test_model_{start}_{end}')
        if os.path.exists(f'{self.base_dir}/model/test_model_{start}_{end}.pkl'):
            with open(f'{self.base_dir}/model/test_model_{start}_{end}.pkl', 'wb+') as file:
                model = dill.load(file)
            # model = load_from_pkl(f'{self.base_dir}/model/test_model_{start}_{end}.pkl')
        ...
        model.learn(total_timesteps=200) # 10000
        model.save(f'{self.base_dir}/model/test_model_{start}_{end}')
        with open(f'{self.base_dir}/model/test_model_{start}_{end}.pkl', 'wb+') as file:
            dill.dump(model, file)
        # save_to_pkl(f'{self.base_dir}/model/test_model_{start}_{end}.pkl', model)
        pd.to_pickle(env.stk_list, f'{self.base_dir}/stk_list/{start}_{end}.pkl')
...
if __name__ == '__main__':
    TD3_PARAMS = {'batch_size': 128,
                  'buffer_size': 10000,
                  'learning_rate': 1e-3,
                  'gradient_steps': 10, # 测试时间
                  'verbose': 0}

    train_period = 40
    test_period = 20
    root_path = root_path + 'fengchi/'  # 区分路径
    path = f'{root_path}/TainingRes/TD3Test_train{train_period}_test{test_period}/'
    if not os.path.exists(path):
        os.makedirs(path)
    train_conn = TD3Model(20170101, 20201231, base_dir=path, model_kwd=TD3_PARAMS,
