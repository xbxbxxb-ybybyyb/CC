...
from R2D2.RLModel.conf import conf
...
class RLAgent:
    ...
    def train_A2C(self, model_name, model_params):
        """A2C model"""
        ...
        model = A2C('MlpPolicy', env_train,
                    ...
                    tensorboard_log=f"{conf.TENSORBOARD_LOG_DIR}/{model_name}"
                    )
        model.learn(total_timesteps=model_params['timesteps'], tb_log_name="A2C_run")
        end = time.time()

        model.save(f"{conf.TRAINED_MODEL_DIR}/{model_name}")
        print('Training time (A2C): ', (end - start) / 60, ' minutes' )
        return model

    def train_TD3(self, model_name, model_params=conf.TD3_PARAMS):
        """TD3 model"""
    ...
    def train_SAC(self, model_name, model_params=conf.SAC_PARAMS):
        """TD3 model"""
    ...
    def train_PPO(self, model_name, model_params=conf.PPO_PARAMS):
        """PPO model"""
