...
if __name__ == '__main__':
    ...
    train_samples, predict_samples = lr_model.get_dataset(train_start_date=20140101,
                                            ...
                                            sample_type = 'virga2consis_board')
    print('共使用因子%d个' % (train_samples.shape[1] - 1))
    print('股票池样本数共%d个' % train_samples.shape[0])
    factor_scaler_list = []
    ...
    # pred_res.to_pickle(pred_output_path + 'lr_clf_pred_signal.pkl')
    # print('已保存至%s' % (pred_output_path + 'lr_clf_pred_signal.pkl'))
