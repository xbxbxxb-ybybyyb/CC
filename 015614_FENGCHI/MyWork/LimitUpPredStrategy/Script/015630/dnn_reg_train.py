...
from LimitUpPredStrategy.model.ModelImpl.DNNModelReg import DNNModelReg
...
if __name__ == '__main__':
    lr_model = DNNModelReg(start_date=20140101, end_date=20191231, stock_pool_address=None)
    ...
    y_pred_median = y_pred.quantile(0.6)
    y_pred_max = y_pred.max()
    y_threshold = y_pred_median+0.05*(y_pred_max-y_pred_median)
    y_pred[y_pred > y_pred_median] = 1
    y_pred[y_pred <= y_pred_median] = 0
    y_pred.columns = ['y_pred']
    pred_res = pd.concat([y_test, y_pred], axis=1)
    pred_res.to_pickle(pred_output_path + 'dnn_reg_pred_signal1_7.pkl')
