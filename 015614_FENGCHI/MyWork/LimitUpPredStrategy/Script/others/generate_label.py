...
def calc_and_save_label(label_type='cls_当日收盘是否涨停'):
    ...
    elif label_type == 'reg_次日开盘溢价_vwap_30':
        limit_max = getData.get_daily_1factor('limit_max', date_list=date_list)
        adj_factor = getData.get_daily_1factor('adjfactor', date_list=date_list)
        limit_max_badj = limit_max * adj_factor
        limit_max_badj = limit_max_badj.apply(lambda x: round(x, 2))

        minute_amt = getData.get_minute_1factor('amt', start_datetime=start_date, end_datetime=end_date)
        minute_vol = getData.get_minute_1factor('vol', start_datetime=start_date, end_datetime=end_date)

        minute_amt = minute_amt.groupby('date').apply(lambda x: x[:31].sum())
        minute_vol = minute_vol.groupby('date').apply(lambda x: x[:31].sum())
        daily_vwap = minute_amt / minute_vol
        daily_vwap_badj = daily_vwap * adj_factor
        next_day_ret = daily_vwap_badj.shift(-1) / limit_max_badj - 1
...
if __name__ == '__main__':
    ...
    calc_and_save_label(label_type='reg_次日开盘溢价_vwap_30')