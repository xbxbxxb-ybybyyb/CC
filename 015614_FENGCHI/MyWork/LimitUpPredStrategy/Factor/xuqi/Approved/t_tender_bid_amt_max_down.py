...
class t_tender_bid_amt_max_down(object):
    ...
    def cal_tender_tick_factor(self, stk, date):
        ...
        return t_tender_bid_amt_max_down
    ...
    def mult_run(self, LimitPool,  j):
        ...
        for i in tqdm(range(k * j, min(k * (j + 1), len(LimitPool)))):
            ...
            try:
                ...
                stk = int(LimitPool.index[i][1])
                date = int(LimitPool.index[i][0])
    ...
    def calculate(self, factor_name, save_path=None):
        ...
        factor = factor.set_index(['date', 'code'])['t_tender_bid_amt_max_down']
        print(factor)
...
fc = t_tender_bid_amt_max_down(start_date=20210615, end_date=20210622)
test = fc.calculate('t_tender_bid_amt_max_down')
