...
class AvgPrem(object):
    ...
    def calculate(self, factor_name, save_path=None):
        ...
        date_list = get_date_range(get_pre_trade_date(self.start_date, 250), self.end_date)
        ...
        open_ret = open/preclose-1
        open_ret_mean = (zt.astype(int).shift(1).replace(0, np.nan) * open_ret).rolling(242, min_periods=1).apply(lambda x: np.nanmean(x))

        factor = open_ret_mean.shift(1)
...
fc = AvgPrem(start_date=20140102, end_date=20210715)
test = fc.calculate('AvgPrem', '/arch1/group/800442/800319/ZTfactors/Approved_2021/')
