...
class NetBuyCR(object):
    ...
    def get_large_trade(self, stock_stack, i):
        ...
        active_buy = filtered[(filtered['TradeBSFlag'] == 1)&(filtered['Price'] > 0)].groupby('TradeBuyNo')['Volume','Amount'].sum()
        buy_cr =(active_buy['Amount'] * active_buy['Amount']).sum()/((filtered['Amount'].sum())**2)*100
    ...
    def mult_run(self, stock_stack, j):
        ...
        for i in tqdm(range(k * j, k * (j + 1))):
            try:
                ...
                pass
        df = pd.DataFrame({'code': code, 'date': date, 'tick': tick, 'net_buy_cr': net_buy_cr})
    ...
    def calculate(self, factor_name, save_path=None):
        ...
        factor = factor.set_index(['date', 'code', 'tick'])['net_buy_cr']
...
fc = NetBuyCR(start_date=20140102, end_date=20210715)
test = fc.calculate('NetBuyCR', '/arch1/group/800442/800319/ZTfactors/Approved_2021/')