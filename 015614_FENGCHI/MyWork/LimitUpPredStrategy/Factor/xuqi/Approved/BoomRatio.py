...
class BoomRatio(object):
    ...
    def calculate(self, factor_name, save_path=None):
        ...
        limit_max = get_daily_1factor('limit_max', date_list)
        cb = (high == limit_max)
        boom_ratio = (cb.rolling(242).sum() - zt.rolling(242).sum()) / np.fmax(1, cb.rolling(242).sum())

        factor = boom_ratio.shift(1)
...
fc = BoomRatio(start_date=20140102, end_date=20210715)
test = fc.calculate('BoomRatio', '/arch1/group/800442/800319/ZTfactors/Approved_2021/')
