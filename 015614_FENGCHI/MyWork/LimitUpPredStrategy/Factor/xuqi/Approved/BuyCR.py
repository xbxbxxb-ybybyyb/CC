...
class BuyCR(object):
    ...
    def mult_run(self, stock_stack, j):
        ...
        for i in tqdm(range(k * j, k * (j + 1))):
            try:
                ...
                pass
        df = pd.DataFrame({'code': code, 'date': date, 'tick': tick, 'buy_cr': buy_cr})
    ...
    def calculate(self, factor_name, save_path=None):
        ...
        factor = factor.set_index(['date', 'code', 'tick'])['buy_cr']
...
fc = BuyCR(start_date=20140102, end_date=20210715)
test = fc.calculate('BuyCR', '/arch1/group/800442/800319/ZTfactors/Approved_2021/')