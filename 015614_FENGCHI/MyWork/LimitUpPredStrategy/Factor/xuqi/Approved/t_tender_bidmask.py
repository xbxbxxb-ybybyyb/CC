...
class t_tender_bidmask(object):
    ...
    def cal_tender_tick_factor(self, stk, date):
        ...
        if df['TotalBidQty'].iloc[-1] > 0:
            t_tender_bid = df['TotalBidQty'].iloc[-1]
        else:
            t_tender_bid = 0
            for i in range(1, 11):
                t_tender_bid += df['Buy%sOrderQty' % i].iloc[-1]
        ...
        t_tender_bidmask = t_tender_bid - t_tender_ask
        return t_tender_bidmask
    ...
    def mult_run(self, LimitPool,  j):
        ...
        for i in tqdm(range(k * j, min(k * (j + 1), len(LimitPool)))):
            try:
                ...
                t_tender_bidmask.append(a)
                code.append(stk)
                date_lst.append(date)
            except:
                pass
        df = pd.DataFrame({'date': date_lst, 'code': code, 't_tender_bidmask': t_tender_bidmask})
        return df
    ...
    def calculate(self, factor_name, save_path=None):
        ...
        factor = factor.set_index(['date', 'code'])['t_tender_bidmask']
...
fc = t_tender_bidmask(start_date=20210615, end_date=20210715)
test = fc.calculate('t_tender_bidmask')
