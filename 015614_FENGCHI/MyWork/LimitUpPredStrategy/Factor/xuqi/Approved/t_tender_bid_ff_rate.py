...
class t_tender_bid_ff_rate(object):
    ...
    def mult_run(self, LimitPool,  j):
        ...
        for i in tqdm(range(k * j, min(k * (j + 1), len(LimitPool)))):
            try:
                ...
                t_tender_bid.append(a)
                code.append(stk)
                date_lst.append(date)
            except:
                pass
        df = pd.DataFrame({'date': date_lst, 'code': code, 't_tender_bid': t_tender_bid})
        return df
    ...
    def calculate(self, factor_name, save_path=None):
        ...
        factor = factor.set_index(['date', 'code'])['t_tender_bid']
...
fc = t_tender_bid_ff_rate(start_date=20210615, end_date=20210715)
test = fc.calculate('t_tender_bid_ff_rate')
