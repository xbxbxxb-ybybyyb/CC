...
class WDownMean(object):
    ...
    def calculate(self, factor_name, save_path=None):
        ...
        w_down = (close - low) / (close - low).rolling(5).mean()
        w_down_mean = w_down.rolling(20).mean()
        factor = w_down_mean.shift(1)
...
fc = WDownMean(start_date=20210615, end_date=20210715)
test = fc.calculate('WDownMean')
