...
class Pct10d(object):
    ...
    def calculate(self, factor_name, save_path=None):
        ...
        pct_10d = adjclose.pct_change(10)

        factor = pct_10d.shift(1)
        ...
        factor = pd.DataFrame(
            np.repeat(factor.stack().loc[LimitPool.index].values, LimitPool.shape[1]).reshape(LimitPool.shape[0],
                                                                                              LimitPool.shape[1]),
...
fc = Pct10d(start_date=20140102, end_date=20210715)
test = fc.calculate('Pct10d', '/arch1/group/800442/800319/ZTfactors/Approved_2021/')