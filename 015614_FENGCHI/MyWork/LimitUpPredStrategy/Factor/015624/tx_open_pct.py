...
factor_name = 'open_pct'

open = getData.get_daily_1factor('open', date_list=LimitPool.index.levels[0].to_list(),code_list=LimitPool.index.levels[1].to_list())
open = open.stack().loc[LastPx.index]

factor = (LastPx.T / open).T
