...
def factor_tx_price_up_market(start_date,end_date):
    ...
    close = getData.get_daily_1factor('close', date_list=LimitPool.index.levels[0].to_list(),
                                      code_list=LimitPool.index.levels[1].to_list()).shift(1)

    close = close.stack().loc[LimitPool.index]
    factor = close.copy()
