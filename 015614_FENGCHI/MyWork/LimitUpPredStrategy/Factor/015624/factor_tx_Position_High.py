...
def factor_tx_Position_High(start_date,end_date):
    ...
    high_day = getData.get_daily_1factor('high', date_list=LimitPool.index.levels[0].to_list(),
                                         code_list=LimitPool.index.levels[1].to_list()).shift(1)
    high_day = high_day.rolling(10).max()
    high_day = high_day.stack().loc[LimitPool.index]
    factor = (LimitPool.T * high_day).T

    factor = LastPx / factor - 1
