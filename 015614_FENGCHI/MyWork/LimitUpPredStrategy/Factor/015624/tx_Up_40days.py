...
# 添加依赖因子
dependencies_factors = ['LimitPool']
# 导入因子
...
factor_name = 'Up_40days'
pct_chg = getData.get_daily_1factor('pct_chg', date_list=LimitPool.index.levels[0].to_list(),
                                    code_list=LimitPool.index.levels[1].to_list()).shift(1)
factor = (pct_chg > 0).rolling(40).sum() / 40
