...
factor_name = 'spearman_vol_close'

close = getData.get_daily_1factor('close', date_list=LimitPool.index.levels[0].to_list(),code_list=LimitPool.index.levels[1].to_list()).shift(1)
vol = getData.get_daily_1factor('volume', date_list=LimitPool.index.levels[0].to_list(),code_list=LimitPool.index.levels[1].to_list()).shift(1)
factor = close.rolling(20).corr(vol)

factor.replace(np.inf, 1, inplace=True)
factor.replace(-np.inf, -1, inplace=True)
