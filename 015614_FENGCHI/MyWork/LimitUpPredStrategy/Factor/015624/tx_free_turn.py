...
# 添加依赖因子
dependencies_factors = ['LimitPool','TotalVolumeTrade']
# 导入因子
...
factor = TotalVolumeTrade.loc[LimitPool.index].stack().loc[stock_pool_stack.index] / \
         free_shares.stack().loc[LimitPool.index].loc[stock_pool_stack.index]
