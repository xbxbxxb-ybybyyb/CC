...
factor_name = 'Turn_Bigger'
turn = getData.get_daily_1factor('turn', date_list=LimitPool.index.levels[0].to_list(),
                                 code_list=LimitPool.index.levels[1].to_list()).shift(1)
turn_bigger = (turn > 2).rolling(5).sum()

factor = turn_bigger.stack().loc[LimitPool.index]
