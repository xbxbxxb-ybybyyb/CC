...
def factor_tx_CorrPV(start_date,end_date):
    ...
    Price_sum = LastPx.T.expanding().mean()

    Volume_sum2 = (TickVolume ** 2).expanding().mean()
    Price_sum2 = (LastPx ** 2).T.expanding().mean()

    VolumePrice_sum = (LastPx.T * TickVolume).expanding().mean()
