...
volume_ratio = TotalVolumeTrade/volume_cumsum
volume_ratio = volume_ratio[LimitPool].stack()
volume_ratio = volume_ratio.reindex(stock_pool_stack.index)
threshold = 0.07
factor = volume_ratio*(pct>threshold)
#factor = factor[LimitPool].stack()
factor = factor.reindex(stock_pool_stack.index)
factor.to_pickle('/data/group/800442/800319/ZTfactors/Untested/zxf010.pkl')