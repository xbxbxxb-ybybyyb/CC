...
n = 0
factorlist = os.listdir('/data/group/800442/800319/zxf/ZTuntestedfactors')
for j in factorlist:
    if n<1:
        expression = pd.read_pickle('/data/group/800442/800319/zxf/ZTuntestedfactors/%s'%j)['expression']
        basic_factors = []
        ...
        flag = self.factor_test(factor_name,expression)
        if flag:
            print(expression)
            n = n+1
