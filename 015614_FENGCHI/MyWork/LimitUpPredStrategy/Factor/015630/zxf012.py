...
high = LastPx.cummax(axis=1)
high = high[LimitPool].stack()
high = high.reindex(stock_pool_stack.index)
LastPx = LastPx[LimitPool].stack()
LastPx = LastPx.reindex(stock_pool_stack.index)
pct = LastPx/close_tick-1
threshold = 0.07
condition = high==LastPx
factor = pct*(pct>threshold)*condition
#factor = factor[LimitPool].stack()
factor = factor.reindex(stock_pool_stack.index)
factor.to_pickle('/data/group/800442/800319/ZTfactors/Untested/zxf012.pkl')