...
from LimitUpPredStrategy.dataApi.indName import sw_level1, citics_level1
from LimitUpPredStrategy.dataApi.stockList import trans_windcode2int, trans_int2windcode
from LimitUpPredStrategy.dataApi.tradeDate import get_trade_date_interval, trans_datetime2int, get_minute_index, get_recent_trade_date, \
    get_pre_trade_date, get_date_range, get_desample_minute_panel
...
def get_daily_1factor(factor: object, date_list: object = None, code_list: object = None, type: object = 'stock', diy_address: object = None) -> object:
    if type == 'stock':
        address = base_address + 'daily'
        if code_list != None:
            code_list = [trans_windcode2int(x) for x in code_list]
...
def get_minute_1min(datetime, minute_interval=1, factor_list=None, code_list=None, type='stock', base_date=20130101,
                    diy_address=None):
...
def get_minute_1stock(code, start_datetime=None, end_datetime=None, factor_list=None,
    ...
    if end_datetime == None:
        ...
        stop_row = (get_trade_date_interval(end_datetime) + 1) * 242
    else:
    ...
    if desample_minute_period >= 2:
        methods = {'open': 'first', 'high': 'max', 'low': 'min', 'close': 'last', 'amt': 'sum', 'vol': 'sum',
                   'deal': 'sum'}
...
def _get_stock_short_tick(code, start_date, end_date, start_time=90000, end_time=160000):
    ...
    if len(df) == 0:
        ...
        df[['Buy1OrderDetail', 'Sell1OrderDetail']] = df[['Buy1OrderDetail', 'Sell1OrderDetail']].applymap(
            _trans_order_detail)
        return df
...
def get_tick_1stock(code, start_datetime=20150105090000, end_datetime=None, type='stock'):
    benchmarks = {'HS300': '000300.SH', 'ZZ500': '000905.SH', 'ZZ800': '000906.SH', 'SZZZ': '000001.SH',
                  'SZCZ': '399001.SZ',
    ...
    if type == 'stock':
        ...
        _end_date = int(
            dt.datetime.strftime(dt.datetime.strptime(str(start_date), '%Y%m%d') + dt.timedelta(60), '%Y%m%d'))
        df = pd.DataFrame()
...
def get_quarter_1factor(factor, code_list=None, date_list=None):
    ...
    df = fd.get_factor_value('Basic_factor', mddate=date_list, stock=code_list, factor_names=[factor]).iloc[:,
         0].unstack()
