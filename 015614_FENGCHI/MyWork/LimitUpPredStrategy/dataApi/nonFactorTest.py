...
class NonFactorTest(object):
    ...
    def __init__(self, start_date=20160104, end_date=20181228, stock_pool='ALL', future_days=10,
        ...
        start_date = get_pre_trade_date(start_date, -1) if get_pre_trade_date(start_date,
                                                                              0) != start_date else start_date
    ...
    def _calc_group_ret(self, groups=10):
        ...
        factor_group[self.mask] = np.nan

        future_group = np.c_['0,3', tuple(np.ma.array(self.future, mask=(factor_group != x))
                                          .mean(axis=2).data for x in range(1, groups + 1))]
        future_group = ((future_group.transpose(0, 2, 1) + 1) ** (1 / np.array(self.future_days))).transpose(2, 1,
                                                                                                             0) - 1
        ...
        turn_top = tuple(np.abs(top_pool[x, self.future_days[x]:] - top_pool[x, :-self.future_days[x]]).sum(axis=1) /
                         (top_pool[x, self.future_days[x]:].sum(axis=1) + top_pool[x, :-self.future_days[x]].sum(
                             axis=1))
    ...
    def test_factor(self, period='Y', IC=True, rank_IC=True, groups=10, group_IC=True, long_short_ret=True,
        ...
        if rank_IC:
            stats['rank_IC'], _, stats['rank_ICIR'], stats['rank_IC_pos'], _ = stats_ret(self.rank_ic, date_index)
            stats['rank_IC'] /= 244

        if group_IC:
            stats['group_IC'], _, stats['group_ICIR'], stats['group_IC_pos'], _ = stats_ret(self.group_ic, date_index)
            stats['group_IC'] /= 244
