...
class portfolioOptimization(object):
    ...
    def calc_opt_group(self, date):
        ...
        ##��ȡ��׼Ȩ�ص���ҵ�ֲ�Ȩ��##########
        _bench_ind = \
            pd.concat([_bench_weight.rename('wgt'), _ind.rename('ind')], axis=1).replace('nan',
                                                                                         np.nan).dropna().groupby(
    ...
    def calc_simulate_group(self, date):
        ...
        _simulate_gain = (self._last_group_exdiv.dot(_close - self._pre_close) + _vol_simulate.dot(_close - _twap) -
                          np.fmax(_vol_simulate, 0).dot(
                              _twap) * self.fee_buy + np.fmin(_vol_simulate, 0).dot(_twap) * self.fee_sell)
        ##��������ʱ�ֲֽ��=�������ֲ̳�*���ռ�+����ɽ�����
...
if __name__ == '__main__':
    config = dict(
        ...
        factor_address='/data/group/800442/800319/result_gp/model/',
        stock_pool_address='/data/group/800442/800319/result_gp/TX_middle_data',  ##�Ƚ���һ����Ʊ��
