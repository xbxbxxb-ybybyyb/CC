...
class FactorBackTest(object):
    ...
    def load_factor(self, factor, group=10):
        ...
        factor = factor.values.reshape(len(self.trade_dates), len(self.adj_minutes), len(self.stocks)).transpose(1, 0,
                                                                                                                 2)
        factor[:, ~ self.stock_list] = np.nan
        factor_rank = bottleneck.nanrankdata(factor, axis=2)
    ...
    def calc_group_ret(self, ret_type, win_ret=0.0012):
        ...
        ret = \
        {'twap': self.ret_twap, 'close': self.ret_close, 'excess': self.ret_excess, 'interval': self.ret_interval}[
            ret_type]
        win = ret > win_ret
        win[np.isnan(ret)] = np.nan
        mean_ret = pd.DataFrame(
            {x: np.nanmean(np.nanmean(np.ma.array(ret, mask=(factor_rank != x)), axis=2).data, axis=0)
             for x in np.arange(self.group)}, index=self.trade_dates)
        mean_win = pd.DataFrame(
            {x: np.nanmean(np.nanmean(np.ma.array(win, mask=(factor_rank != x)), axis=2).data, axis=0)
             for x in np.arange(self.group)}, index=self.trade_dates)
