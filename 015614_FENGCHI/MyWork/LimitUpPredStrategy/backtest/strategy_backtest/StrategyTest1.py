...
def del_sellVolume(start_date=20140101,end_date=20210228,save_path='/data/group/800442/800319/LimitStrategy_Test/'):
    dp = TickDataPrepare('/arch1/group/800442/800319/LimitTickData2/')  # 实例化类
    LimitPool = pd.read_pickle('/data/group/800442/800319/LimitUpStrategy/FilteredTick.pkl')
...
def get_volume(start_date,end_date,buy_weight=0.1,tick_delay=3,save_path='/data/group/800442/800319/LimitStrategy_Test/'):  #个股成交比例
...
def sell_DateTime(start_date=20140101,end_date=20210228,save_path='/data/group/800442/800319/LimitStrategy_Test/'):
    ########获取股票池##############
    LimitPool = pd.read_pickle('/data/group/800442/800319/LimitUpStrategy/FilteredTick.pkl')
    LimitPool_after1 = LimitPool.copy()
    LimitPool_after1['tick'] = LimitPool_after1['tick'].apply(lambda x: int((datetime.datetime.strptime(str(x), '%H%M%S') + datetime.timedelta(seconds=3)).strftime("%H%M%S")))
...
class StrategyTest(object):
    def __init__(self, start_date=20140101, end_date=20201231,buy_money=10000000,  #单只个股买入金额，单只个股成交比例
                 buy_weight=0.1,tick_delay=3,read_path='/data/group/800442/800319/LimitStrategy_Test/'):
        ...
        Limit_Result['Volume'] = pd.concat([(buy_money / Limit_Result['price'] / 100).astype(int) * 100,(Limit_Result['Volume'] / 100).astype(int) * 100], axis=1).min(axis=1)
        Limit_Result['money'] = Limit_Result['Volume']*Limit_Result['price']
    ...
    def get_strategy_result(self,factor,N=10,cost=0.001):
        ...
        Trade_Result=Trade_Result[Trade_Result['price'] > 2]
    ...
    def statistic_factor(self,save_path=None):
        ...
        ##1、收益率
        Result=pd.DataFrame(index=['胜率','平均收益率','盈亏比'],columns=['数值收益','实际收益'])
        Result.loc['胜率','数值收益']=len(self.Trade_Result['收益率'][self.Trade_Result['收益率']>0])/ len(self.Trade_Result)
        Result.loc['平均收益率', '数值收益']=self.Trade_Result['收益率'].mean()
        Result.loc['盈亏比', '数值收益'] = -self.Trade_Result['收益率'][self.Trade_Result['收益率']>0].mean()/self.Trade_Result['收益率'][self.Trade_Result['收益率']<0].mean()

        Result.loc['胜率', '实际收益'] = len(self.Trade_Result['实际收益率'][self.Trade_Result['实际收益率'] > 0]) / len(self.Trade_Result)
        Result.loc['平均收益率', '实际收益'] = self.Trade_Result['实际收益率'].mean()
        Result.loc['盈亏比', '实际收益'] = -self.Trade_Result['实际收益率'][self.Trade_Result['实际收益率'] > 0].mean() / \
                                    self.Trade_Result['实际收益率'][self.Trade_Result['实际收益率'] < 0].mean()
        ...
        for year in year_list:
            year_result = self.Trade_Result.loc[year*10000+101:year*10000+1231]
        ...
        print('以保存至%s' % save_path)

        return self.Trade_Result,Result,Net_Profit

if __name__ == '__main__':
    factor = pd.read_pickle('/data/group/800442/800319/Afengchi/LimitUpPredStrategy/predict_result/linear_reg/linear_reg_trainPeriod60_predictPeriod10_factorNum80_pctThreshold0.02_signal_r2s3.pkl')
    ...
    Trade_Result,Result,Net_Profit=self.statistic_factor(save_path='/data/user/015624/Result.xlsx')
