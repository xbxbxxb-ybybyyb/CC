...
from LimitUpPredStrategy.conf.path_conf import samples_path, label_path, factor_evaluation_bt_path
from sklearn import metrics
from LimitUpPredStrategy.conf.path_conf import proj_root_path
...
class RollingModelBaseReg:
    ...
    def set_dataset(self, train_start_date=20140101, train_end_date=20191231,
        ...
        train_samples = pd.read_pickle(samples_path + stock_pool_type + '.pkl')
    ...
    # 给滚动训练调用
    def get_dataset(self, train_idx, test_idx):
        factor_list = self.get_factor_evaluation(indicator='ret_tmr30_IC', early_date=train_idx[0])
    ...
    def get_factor_evaluation(self, stock_pool='all_board', indicator='ret_tmr30_IC', factor_num=80, early_date=None):
        ...
        factor_evaluation_bt_res = pd.read_pickle(factor_evaluation_bt_path + 'reg_factor_std60_%d.pkl' % last_month)
