...
class TurningEvaluation:
    def __init__(self, stk_id):
        # floder_path = '/data/user/015624/缠论拐点/858/'
    ...
    def calc_daily_indicator(self, prefix):
        ...
        for date in tqdm(self.date_list):
            ...
            for turning_enum in ['拐点类型', '第二类拐点类型', '第三类拐点类型']:
            # for turning_enum in ['拐点类型']:
                buy_df = df.query(f'{turning_enum}==1')
                sell_df = df.query(f'{turning_enum}==-1')
                ...
                daily_up_rec = [date, turning_enum, '向上', buy_num, buy_pct5m, buy_pct10m, buy_rel_close,
                                daily_profit, buy_quantile_allday, buy_quantile2close, buy_pct1]
                daily_down_rec = [date, turning_enum, '向下', sell_num, sell_pct5m, sell_pct10m, sell_rel_close,
        ...
        df.columns = ['日期', '拐点类型', '拐点方向', '拐点个数', '未来5分钟收益率', '未来10分钟收益率', '相对收盘价的收益率',
    ...
    def evaluate(self):
        ...
        for prefix in prefix_list:
            ...
            df['日期'] = df['日期'].map(str)
            rec_df = df.set_index(['日期', '拐点类型', '拐点方向'])
            rec_df['年度包含率'] = rec_df['当天在最值点分钟处比例'] > 0
            rec_df['买卖点日收益胜率'] = rec_df['买卖点日收益率'] > 0
            group_df = rec_df.groupby(['拐点类型', '拐点方向']).agg({'拐点个数': 'mean',
                                                             '未来5分钟收益率': 'mean',
                                                             '未来10分钟收益率': 'mean',
                                                             '相对收盘价的收益率': 'mean',
                                                             '买卖点日收益率': 'mean',
            ...
            rec_dict = dict()
            rec_dict['按日统计'] = rec_df
            rec_dict['按年统计'] = group_df
            filename = f'{self.stk_id}_{prefix}统计结果.xlsx'
