...
class TradePrepare:
    ...
    def trend_buy_prepare(self):
        tmp_df = pd.read_excel('/data/user/015614/MyWork/FaaMonitor/personal/fengchi/ftp_ssh/'
                               ...
                               sheet_name='交易准备')
        df = pd.DataFrame()
        buy_df = tmp_df[['买入股票代码', '买入股票名称', '买入所属主题']].dropna().drop_duplicates()
        ...
        for idx in range(len(buy_df)):
            ...
            adjfactor = self.adjfactor.loc[self.yes_date, stk_id]
            append_content = [stk_id, stk_name, ma, ma_boost, per_amt, pre_close, adjfactor, concept]
            df = df.append([append_content])
        df.columns = ['stk_id', 'stk_name', 'ma', 'ma_boost', 'per_amt', 'pre_close', 'adjfactor', 'concept']
