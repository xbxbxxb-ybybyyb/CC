...
from backtest.factor_test import SingleFactorTest
from backtest.utility import *
from backtest.report_generator import generate_pdf
from utility.normalization import Normalization2
from sklearn.linear_model import LinearRegression
from sklearn.model_selection import train_test_split

daily_store_path = '/data/user/015518/share/daily/'
...
class AlgoSingleFactorBacktest(SingleFactorTest):
    ...
    def compute_sampling_ret_stat(self, excess_return, in_sample=True ,random_state=0, bootstrap_steps=9, experiment_steps=10):
        """
        ...
        for i in range(experiment_steps):
            sample_bin_ret_mean.append(self.sample_random(excess_return, random_state=random_state, bootstrap_steps=bootstrap_steps) * 1e4)
            random_state += 1
        sample_bin_ret_mean = pd.Series(sample_bin_ret_mean, index=np.arange(1,experiment_steps+1))

        bins_ret_diff2ret = ( sample_bin_ret_mean.nlargest( int(experiment_steps/2) ).mean() - sample_bin_ret_mean.nsmallest( int(experiment_steps/2) ).mean() ) / sample_bin_ret_mean.mean()
        ...
        bin_ret_diff = pd.DataFrame(index=excess_return.index[::5],columns=[str(i) for i in np.arange(1,experiment_steps+1)]+['bins_ret_diff2ret', 'sample_std2ret'])
        ...
        for sdate, edate in zip(bin_ret_diff.index, bin_ret_diff.index[50:]):
            ...
            bin_ret_diff.loc[edate,'bins_ret_diff2ret'] = ( ret_list.nlargest( int(experiment_steps/2) ).mean() - ret_list.nsmallest( int(experiment_steps/2) ).mean() ) / ret_mean
...
if __name__=='__main__':
    ...
    instance = AlgoSingleFactorBacktest(start_date, end_date, universe='alpha_universe', holding_period=1, benchmark='alpha_universe',
                        transaction_cost=0.001, segment_number=10, seg_by_industry=False, interest_type='',
