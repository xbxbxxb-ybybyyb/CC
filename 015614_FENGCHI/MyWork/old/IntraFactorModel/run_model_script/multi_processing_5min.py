...
out_path = '/data/group/800319/junkData/IntraFactorModel/predictions/xgb_model_rise_down_zero_5min_2018all_mkt_origin_nodrop_factor_20200710/'
...
def wraper(stk):
    if os.path.exists(out_path + '%d.pkl' % stk):
        ...
        return 0
    try:
        lr = XGBModel(start_date=20170102, end_date=20191231, factor_path='/data/group/800319/junkData/IntraFactorModel/FactorByStock_from2017_whole_mkt/',
                      scare_list=scale_list, non_scare_list=non_scale_list)
        compare, metric = lr.training_methodology(stk, 'rise_down_zero_5min', best_param_clf_xgboost_rise_down_zero)
        pd.to_pickle([compare, metric], out_path + '%d.pkl' % stk)
    except:
        print('Wrong', stk)
        pd.to_pickle([], out_path + 'Wrong_%d.pkl' % stk)
