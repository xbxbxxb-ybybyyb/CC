...
from conf.feature_config import scale_list_5min, non_scale_list_5min
...
log_name = log_path + 'lr_rise_down_zero_5min_50factor_%s.log' % rq
logfile = log_name
...
def get_obj(model_type, metrics_type, stk_list, label):
    ...
    model = model_choice[model_type][0](start_date=20170103, end_date=20181231, factor_path='/data/group/800319/junkData/IntraFactorModel/FactorByStock_new/',
                                        scare_list=scale_list_5min, non_scare_list=non_scale_list_5min)
...
best = run('lr', 'acc', para_pool, 'rise_down_zero_5min')
logger.info('best:' + str(best))
pd.to_pickle(best, root_path + 'best_hyper_params/lr_param_rise_down_zero_5min_50factor.pkl')
