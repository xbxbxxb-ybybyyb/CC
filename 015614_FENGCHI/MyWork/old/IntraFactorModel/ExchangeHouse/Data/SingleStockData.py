...
class SingleStockData:
    ...
    @staticmethod
    def bi_find(time_stamp, time_stamp_list, mod):
        """
        ...
        for i in range(0, len(time_stamp_list) - 1):
            if mod == 'leftequal' and (time_stamp_list[i] <= time_stamp < time_stamp_list[i + 1]):
                return i
            elif mod == 'rightequal' and (time_stamp_list[i] <= time_stamp < time_stamp_list[i + 1]):
                return i + 1
        if time_stamp < time_stamp_list[0]:
            return -2
        elif time_stamp > time_stamp_list[1]:
            return len(time_stamp_list) - 1
        else:
            return -1
