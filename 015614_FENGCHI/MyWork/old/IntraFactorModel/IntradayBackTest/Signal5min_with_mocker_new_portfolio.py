...
def main():
    out_path = '/data/group/800319/junkData/IntraFactorModel/junkClassification/whole_mkt_version20200709/'
    flag = '_new_portfolio_20200709_v_whole_mkt_mlp'
    ...
    #    signal_ = pd.read_pickle('/data/group/800319/junkData/IntraFactorModel/junkClassification/predict_signal_lr_model_rise_down_zero_5min_from2017_origin_nodrop_v1800_factor_20200708.pkl')
    signal_ = pd.read_pickle('/data/group/800319/junkData/IntraFactorModel/junkClassification/predict_signal_mlp_model_rise_down_zero_5min_v_whole_mkt_20200709.pkl')
    print(signal_.shape)
    ...
    import time
    e = time.time()
    avg_price, deal_vol, record = SBTB.backtest(signal_.loc['20180101':'20181231'], target_vol.loc['20180101':'20181231'], kernal_num=12)
    print(time.time() - e)
    pd.to_pickle([avg_price, deal_vol],
                 out_path + 'backtest_res_rise_down_zero_5min_2018_all_factor_fillnapad%s.pkl' % flag)
    pd.to_pickle(record, out_path + 'record_rise_down_zero_5min_2018_all_factor_fillnapad%s.pkl' % flag)
    ...
    fulfill_percent, outperformance = SBTB.calc_performance(signal_, target_vol, avg_price, deal_vol)
    pd.to_pickle([fulfill_percent, outperformance], out_path + 'backtest_performance_rise_down_zero_5min_2018_all_factor_fillnapad%s.pkl' % flag)
    outperformance_all = (outperformance['buy'].sum(axis=1).shift(1) + outperformance['sell'].sum(axis=1)) / position
    ...
    pd.to_pickle(outperformance_all, out_path + 'improve_rise_down_zero_5min_2018_all_factor_fillnapad%s.pkl' % flag)
