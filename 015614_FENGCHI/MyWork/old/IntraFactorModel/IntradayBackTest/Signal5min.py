...
class Signal5min(SignalBackTestBase):
    ...
    def calc_stk_day(self, stk_id, date, vol, mk_data, signal):
        ...
        for idx, date_time in enumerate(signal.index[:-30]):
            ...
            record.append([date_time, dealed_vol * sign, mk_data.loc[date_time, 'close'], finished_vol * sign])
        if len(record) > 0:
            record = pd.DataFrame(record, columns=['datetime', 'vol', 'price', 'finished_vol'])
...
def main():
    out_path = '/data/group/800319/junkData/IntraFactorModel/junkClassification/temp_variable/'
    position = 200000000
    SBTB = Signal5min()
