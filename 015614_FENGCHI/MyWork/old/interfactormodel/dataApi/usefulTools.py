...
def ts_ewm(arr, l, ewa=0.9):
    ...
    weight = ewa ** np.arange(l).astype(float)
    weight /= weight.sum()
...
def ts_cumcorr(y, x, min_count=10):
    ...
    corr = (((np.arange(x.shape[0]) + 1) * cxy - cx * cy) / np.sqrt(
        ((np.arange(x.shape[0]) + 1) * cx2 - cx ** 2) * ((np.arange(y.shape[0]) + 1) * cy2 - cy ** 2))).transpose(2, 0, 1)
...
def ts_cumbeta(y, x, min_count=10):
    ...
    cx2 = ts_cumsum(x ** 2).transpose(1, 2, 0)
    cxy = ts_cumsum(x * y).transpose(1, 2, 0)
    beta = ((np.arange(x.shape[0]) + 1) * cxy - cx * cy) / ((np.arange(x.shape[0]) + 1) * cx2 - cx ** 2).transpose(2, 0, 1)
