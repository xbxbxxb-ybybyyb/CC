...
##2日内因子部分
#提取数据
pre_get_days = boll_window // 242 + 1
drop_minutes = pre_get_days * 242 - boll_window + 1
close = get_minute_1factor('close_badj',
                           get_pre_trade_date(start_date, pre_get_days + refer_buy_days + refer_sell_days), end_date)

#准备矩阵运算
index, columns = close.index[(pre_get_days + refer_buy_days + refer_sell_days) * 242:].map(
    lambda x: x[0] * 10000 + x[1]), close.columns
close = close.values[drop_minutes:]

#计算因子
cx = np.apply_along_axis(np.convolve, 0, close, np.ones(boll_window), 'valid')
cx2 = np.apply_along_axis(np.convolve, 0, close ** 2, np.ones(boll_window), 'valid')
cx2 -= cx ** 2 / boll_window
cx2 /= boll_window - 1
np.sqrt(cx2, out=cx2)
cx2 *= 2
cx /= boll_window
...
buy = boll_down.reshape(boll_down.shape[0] // 242, 242, boll_down.shape[1]).transpose(1, 0, 2)
sell = boll_up.reshape(boll_up.shape[0] // 242, 242, boll_up.shape[1]).transpose(1, 0, 2)
del boll_down, boll_up
...
ft2.evaluation(23)
ft2.result_output('hanxu_boll_close_distance', fileroot='/data/group/800319/factorScripts/')