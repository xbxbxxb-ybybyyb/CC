...
factor_name = 'gp_04_corr_alpha_amt_rank'
# 因子逻辑a2
# 利用alpha(from open)的截面分位数和amt_delta(amt_ma3 - amt_ma20)的截面分位数的相关系数构建指标
...
#信号参数
sign_percent_low = 0.02
sign_percent_high = 0.95
...
#计算日内放量比率
amt_delta = (np.apply_along_axis(np.convolve, 0, amt, np.ones(window_amt_short) / window_amt_short, 'valid'
)[window_amt_long - window_amt_short:
] - np.apply_along_axis(np.convolve, 0, amt, np.ones(window_amt_long) / window_amt_long, 'valid'))

#计算因子
x = alpha[window_amt_long - 1:].argsort().argsort().transpose(2, 1, 0)
y = amt_delta.argsort().argsort().transpose(2, 1, 0)

# #累计相关系数
# cx = np.nancumsum(x, axis=-1)
# cy = np.nancumsum(y, axis=-1)
# cxy = np.nancumsum(x * y, axis=-1)
# cx2 = np.nancumsum(x ** 2, axis=-1)
# cy2 = np.nancumsum(y ** 2, axis=-1)
# n = np.arange(1, x.shape[2] + 1)
# corr = (n * cxy - cx * cy) / np.sqrt((n * cx2 - cx ** 2) * (n * cy2 - cy ** 2))
# corr[(corr == np.inf) | (corr == - np.inf)] = np.nan

#滚动相关系数
n = 20
cx =  np.apply_along_axis(np.convolve, -1, x, np.ones(n), 'valid')
cy =  np.apply_along_axis(np.convolve, -1, y, np.ones(n), 'valid')
cxy =  np.apply_along_axis(np.convolve, -1, x * y, np.ones(n), 'valid')
cx2 =  np.apply_along_axis(np.convolve, -1, x ** 2, np.ones(n), 'valid')
cy2 =  np.apply_along_axis(np.convolve, -1, y ** 2, np.ones(n), 'valid')
corr = (n * cxy - cx * cy) / np.sqrt((n * cx2 - cx ** 2) * (n * cy2 - cy ** 2))
corr[(corr == np.inf) | (corr == - np.inf)] = np.nan

score = corr
score[np.isnan(score)] = 0
del alpha, amt, amt_bench, cx, cy, cxy, cx2, cy2
...
##3日内信号部分
#计算阈值
low_band = np.quantile(score, sign_percent_low, axis= 0)
high_band = np.quantile(score, sign_percent_high, axis= 0)

#计算信号
# score = score.transpose(2, 0, 1)
sign = np.zeros((score.shape[0], 242, score.shape[1]))

bool1 = (profit[2*window_amt_long - 2 :] < 1 + profit_buy_high).transpose(2, 0, 1)
bool2 = (profit[2*window_amt_long - 2 :] > 1 + profit_buy_low).transpose(2, 0, 1)
bool3 = (score < low_band).transpose(0,2,1)
bool_buy = bool1 & bool2 & bool3

bool1 = (profit[2*window_amt_long - 2 :] < 1 + profit_sell_high).transpose(2, 0, 1)
bool2 = (profit[2*window_amt_long - 2 :] > 1 + profit_sell_low).transpose(2, 0, 1)
bool3 = (score > high_band).transpose(0,2,1)
bool_sell = bool1 & bool2 & bool3

sign[:, 2*window_amt_long - 2 :, :][bool_buy] = 1
sign[:, 2*window_amt_long - 2 :, :][bool_sell] = -1
del low_band, high_band,  profit, bool1, bool2, bool3, bool_buy, bool_sell
