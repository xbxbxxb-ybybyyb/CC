...
import day_factor_backtest.utility.dt as tdt
from day_factor_backtest.backtest.report_generator import generate_pdf
from day_factor_backtest.backtest.utility import *
import day_factor_backtest.utility.common as ut
from collections import OrderedDict
from day_factor_backtest.backtest.naming_config import *
...
class SingleFactorTest:
    ...
    def __init__(self, start_date=20160101, end_date=20181231, holding_period=5, benchmark='zz500',
                 ...
                 segment_number=15, neutral_list=['Size', 'Industry'], fillna=False, easy_test=True,
    ...
    def load_data(self):
        ...
        if self.universe in universe_set:
            ...
            h5_filter = IO.read_data([self.start_date, self.end_date], columns='filter_stpt', ftype=FType.UNIV,
        ...
        if self.benchmark in universe_set and self.benchmark == self.universe:
            ...
            universe_ret = stk_ret[
                stock_filter.reindex(index=stk_ret.index, columns=stk_ret.columns).fillna(value=False)].mean(axis=1)
            benchmark_price = (universe_ret + 1).cumprod()
        else:
            index_price = IO.read_data([self.prev_start_date, self.end_date], ftype=FType.MD, dtype=DType.INDEX,
    ...
    def standard_process(self, boxskew=False):
        ...
        if self.fillna:
            ...
            factor_fillna_industry(self.data['factor_data'],
                                   stock_filter=self.data['stock_filter_' + str(self.universe)],
                                   stock_industry=self.data['neutral_dict']['Industry'])
    ...
    def summary_table(self):
        ...
        sft_setting2 = pd.DataFrame([['Easy Test', 'IC Type', 'Compare Style', 'Interest Type',
                                      'Ret Price', 'Ret Shift', ''],
                                     [self.easy_test, self.ic_type, self.compare_style, self.interest_type,
    ...
    def shoot(self, result_folder):
        ...
        self.avg_turnover = round(self.get_turnover(self.factor_data), 5)
        ...
        seg_return_30_stat_year = seg_return_30.loc[use_ind].groupby(seg_return_30.loc[use_ind].index.year).apply(
            calc_seg_measure)
