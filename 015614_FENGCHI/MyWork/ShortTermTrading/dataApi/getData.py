...
def get_minute_pickle(factor, date_list, code_list,
    ...
    month_list = sorted(get_date_range(start_date, end_date, 'M') + [end_date])
    short_month_list = sorted(list({x // 100 for x in month_list}))

    df_list = [load_pickle('%s/%s/%s_%s.pkl' % (address, factor, x, factor)) for x in short_month_list]

    start_keep = (get_trade_date_interval(month_list[0], start_date) + 2) * 242
    df_list[0] = df_list[0].iloc[-start_keep:]

    if len(df_list) > 1:
        end_keep = None if month_list[-1] == month_list[-2] else (get_trade_date_interval(
            month_list[-1], month_list[-2]) + 1) * 242
        df_list[-1] = df_list[-1].iloc[:end_keep]
