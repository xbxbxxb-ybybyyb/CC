...
if __name__ == '__main__':
    ...
    self.load_factor(compound, diy_address='/data/group/800442/800319/', neutral=True)
    import time
    t = time.time()
    time.time() - t
    self.calc_ic()
    ...
    aaa = pd.read_hdf('/data/group/800442/800319/error_data.h5', key='error_data')
    ...
    bench_percent = pd.DataFrame(np.r_['0,2', ((get_daily_1factor('SZ50_exdiv_weight', date_list, code_list).values > 0)
                                       ...
                                       tuple(((pd.read_hdf('/data/group/800442/800319/junkData/daily/common_stock_list.h5', x)
