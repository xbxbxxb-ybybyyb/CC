...
if __name__ == '__main__':
    now_date = get_today_date()
    print('今天日期为%d，分歧转一致策略开始新的一天！' % now_date)
    print('初始化...')
    t1 = time.time()
    dm = DailyMonitor(start_date=20200101, end_date=20210330)
    print('初始化完毕，耗时%ds' % (time.time() - t1))
    if not os.path.exists(daily_monitor_path + '分歧转一致追涨/%d/' % now_date + 'to_deal_list.pkl'):
        ...
        save_pickle(to_deal_list, daily_monitor_path + '分歧转一致追涨/%d/' % now_date, 'to_deal_list.pkl')
        print('得出有可能交易的个股，耗时%ds' % (time.time() - t1))

    print('开始判断日内')
    to_deal_list = load_pickle(daily_monitor_path + '分歧转一致追涨/%d/' % now_date + 'to_deal_list.pkl')
    ...
    dm.start_intra(to_deal_list, monitor_concept_df)