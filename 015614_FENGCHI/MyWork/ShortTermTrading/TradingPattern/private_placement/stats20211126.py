...
class PPStats:
    def __init__(self, start_date=20170101, end_date=20211121):
        fd = FactorData()
    ...
    def get_pp_data():
        ...
        pp_df['预案公告日'] = pp_df['预案公告日'].astype(int)
    ...
    def get_range_pct(self, stk_id, start_date, end_date, shift_flag=True):
    ...
    def get_alpha_pct(self, stk_id, start_date, end_date):
        ...
        if end_date > 20211121:
            return np.nan
        stk_pct = self.close_badj.loc[end, stk_id] / self.close_badj.loc[start, stk_id] - 1
...
if __name__ == '__main__':
    ...
    for shift_days_num in shift_days_nums:
        col_name = f'预案前{shift_days_num}个交易日'
        pp_df[col_name] = pp_df[preplan_date].\
            apply(lambda x: tradeDate.get_pre_trade_date(x, shift_days_num))
        pp_df[f'预案前{shift_days_num}日涨跌幅'] = pp_df[['股票代码', preplan_date, col_name]].\
            apply(lambda x: ppstats.get_range_pct(x['股票代码'], \
            x[col_name], tradeDate.get_pre_trade_date(x[preplan_date], 1), shift_flag=False), axis=1)

    for shift_days_num in [2, 3, 4] + shift_days_nums:
        col_name = f'预案后{shift_days_num}个交易日'
        pp_df[col_name] = pp_df[preplan_date]. \
            apply(lambda x: tradeDate.get_pre_trade_date(x, -shift_days_num))
        pp_df[f'预案后{shift_days_num}日涨跌幅'] = pp_df[['股票代码', preplan_date, col_name]]. \
            apply(lambda x: ppstats.get_range_pct(x['股票代码'], \
            tradeDate.get_pre_trade_date(x[preplan_date], -1), x[col_name], shift_flag=False), axis=1)
    ...
    # 统计预案日前后N日平均数、盈亏比等信息
    index_list = ['预案前%d日涨跌幅' % d for d in shift_days_nums] + ['预案后%d日涨跌幅' % d for d in [2,3,4] + shift_days_nums]
    col_name_list = ['收益率平均值', '中位数', '盈利个数', '亏损个数', '胜率', '盈亏比', '收益率方差']
    output_df12 = pd.DataFrame(index=col_name_list, columns=index_list)
    for shift_days_num in shift_days_nums:
        col_name = f'预案前{shift_days_num}日涨跌幅'
        output_df12.loc['收益率平均值', col_name] = pp_df[col_name].mean()
        output_df12.loc['中位数', col_name] = pp_df[col_name].median()
        output_df12.loc['盈利个数', col_name] = len(pp_df[pp_df[col_name] >= 0])
        output_df12.loc['亏损个数', col_name] = len(pp_df[pp_df[col_name] < 0])
        output_df12.loc['胜率', col_name] = len(pp_df[pp_df[col_name] > 0]) / len(pp_df)
        output_df12.loc['个数比', col_name] = len(pp_df[pp_df[col_name] > 0]) / len(pp_df[pp_df[col_name] < 0])
        output_df12.loc['盈亏比', col_name] = -pp_df[pp_df[col_name] > 0][col_name].mean() / \
                                          pp_df[pp_df[col_name] < 0][col_name].mean()
        output_df12.loc['收益率方差', col_name] = pp_df[col_name].std()
    for shift_days_num in [2, 3, 4] + shift_days_nums:
        col_name = f'预案后{shift_days_num}日涨跌幅'
        output_df12.loc['收益率平均值', col_name] = pp_df[col_name].mean()
        output_df12.loc['中位数', col_name] = pp_df[col_name].median()
        output_df12.loc['盈利个数', col_name] = len(pp_df[pp_df[col_name] >= 0])
        output_df12.loc['亏损个数', col_name] = len(pp_df[pp_df[col_name] < 0])
