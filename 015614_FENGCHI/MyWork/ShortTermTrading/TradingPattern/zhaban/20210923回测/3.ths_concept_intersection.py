...
from FaaMonitor.conf.path_conf import ths_path, new_ths_path
from ShortTermTrading.Util.tools import save_pickle
from ShortTermTrading.conf.path_conf import junk_path, ths_concept_rank_path
from ShortTermTrading.dataApi import tradeDate, stockList

start_date = 20210701
end_date = 20210917
zhaban_stock = pd.read_pickle(junk_path + 'zhaban_zt_time_15_20210923.pkl')

ths_dic = pd.read_json(ths_path + '概念板块同花顺%d.json' % 20210805, typ='dict')
ths_dic = pd.read_json(new_ths_path + '概念板块同花顺%d.json' % 20211025, typ='dict')
# ths_dic = pd.read_json(ths_concept_rank_path + '同花顺概念排名%d.json' % 20211018, typ='dict')
...
main_dict = {(20210701, 20210715): ['盐湖提锂', '第三代半导体', '锂电池', '光伏概念', '半导体及元件', '汽车整车', '稀土永磁'],
             ...
             (20210715, 20210809): ['光伏概念'],
             (20210819, 20210902): ['光伏概念'],
             (20210811, 20210909): ['磷化工'],
             (20210831, 20210909): ['煤炭开采加工', '电力', '云游戏', '天然气'],
             (20210909, 20210917): ['风电', '天然气', '煤炭开采加工', '磷化工', '氟化工']
             }
main_concept_df = generate_daily_active_stock(main_dict, main_concept_df)

ret_df = zhaban_stock & main_concept_df
save_pickle(ret_df, junk_path, '叠加板块zhaban_zt_time_15_20210923.pkl')
