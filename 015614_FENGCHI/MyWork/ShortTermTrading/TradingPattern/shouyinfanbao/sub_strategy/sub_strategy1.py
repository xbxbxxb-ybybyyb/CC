...
from ShortTermTrading.ConceptApi.ConceptApi import get_concept_values, get_basic_values
from ShortTermTrading.interface.ActiveConceptApi import get_active_stock_1concept
from ShortTermTrading.conf.path_conf import concept_read_path
...
class SignalGenerator:
    ...
    # 条件二：日间涨跌幅条件
    def add_dragon_cond(self):
        dragon_cond = get_basic_values('Dragon_Stock',start_date=self.shift_start_date, end_date=self.end_date)
        return dragon_cond.shift(1)
    ...
    def add_t1_cond(self):
        ...
        stk_daily_baodie_judge = stk_daily_baodie_judge.shift(1)
            ...
            for idx in range(len(trigger_stk_1concept_judge)):
                ...
                if concept_stk_list == 0:
                    ...
                    tmp_stk_pctchg = tmp_minute_close_badj.loc[date, time] / tmp_daily_pre_close_badj[
                        'pre_close_badj'] - 1
                    tmp_minute_close_speed = tmp_minute_close_badj.pct_change(2)
                    ...
                    # 所有个股开盘不低于-2%
                    tmp_flag = True
                    for stk_id in need_stk_list:
                        if start_stk_pctchg[stk_id] < -0.02:
                            tmp_flag = False
                            break
                    if not tmp_flag:
                        continue

                    # 至少存在一只个股的涨速同样大于1%/2min，且涨幅大于0
                    tmp_flag = False
                    for stk_id in need_stk_list:
                        if (tmp_minute_close_speed.at[(date, time), stk_id] > 0.01) & (tmp_stk_pctchg[stk_id] > 0):
                            tmp_flag = True
                            break
                    if not tmp_flag:
                        continue

                    if tmp_minute_close_speed.loc[date, time][need_stk_list].mean() > 0.01:
                        print(tmp_minute_close_speed.loc[date, time][need_stk_list].mean())
...
if __name__ == '__main__':
    print('初始化...')
    t1 = time.time()
    sg = SignalGenerator(start_date=20200101, end_date=20201231)
    ...
    pctchg_cond = sg.add_dragon_cond()
    t1_cond = sg.add_t1_cond()
    t2_cond = sg.add_t2_cond()
    amt_cond = sg.add_daily_amt_cond()
    turn_cond = sg.add_turn_cond()
    assert st_cond.shape == pctchg_cond.shape == t1_cond.shape == t2_cond.shape == amt_cond.shape == turn_cond.shape
