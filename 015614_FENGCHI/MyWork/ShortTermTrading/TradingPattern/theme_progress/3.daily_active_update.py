...
from ShortTermTrading.ConceptApi.ConceptApi import get_basic_values, get_concept_values, get_1factor_concept
...
class Save_Data(object):
    ...
    ###获取主流板块和活跃个股###
    def get_Active_concept(self, save_path='/data/user/fengchi/首阴反包/append_data/'):
        ###获取概念板块数量和日收益率###
                ...
                if ((Num >= 5) & (Num <= 10)):  ##如果是小版块##
                    ...
                    if Concept_Pct.loc[date, concept] >= 3.5:
                        if not self.judge_concept_pct(date, concept, Concept_Pct, small_list):
                            continue
                        ##再判断板块是不是有超额##
                        if Concept_Pct.loc[date, concept] - self.Index_pct.loc[date, 'SZZZ'] >= 1:
                            ##再判断当天个股涨幅是否达到要求##
                            concept_stock = get_concept_values('Concept_StockList', concept, date, date,\
                                                               read_path='/data/group/800319/Temporary_Data/RawData/').loc[date]
                    ...
                    if Concept_Pct.loc[date, concept] >= 2.5:
                        if not self.judge_concept_pct(date, concept, Concept_Pct, middle_list):
                            continue
                        ##再判断板块是不是有超额##
                        if Concept_Pct.loc[date, concept] - self.Index_pct.loc[date, 'SZZZ'] >= 0.5:
                            ##再判断当天个股涨幅是否达到要求##
                            concept_stock = get_concept_values('Concept_StockList', concept, date, date,\
                                                               read_path='/data/group/800319/Temporary_Data/RawData/').loc[date]
                            concept_stock = concept_stock[concept_stock == True].index.to_list()
                    ...
                    if Concept_Pct.loc[date, concept] > 1.5:
                        if not self.judge_concept_pct(date, concept, Concept_Pct, big_list):
                            continue
                        ##再判断板块是不是有超额##
                        if Concept_Pct.loc[date, concept] - self.Index_pct.loc[date, 'SZZZ'] >= 0:
                            ##再判断当天个股涨幅是否达到要求##
                            concept_stock = get_concept_values('Concept_StockList', concept, date, date, \
                                                               read_path='/data/group/800319/Temporary_Data/RawData/').loc[date]
                            concept_stock = concept_stock[concept_stock == True].index.to_list()
        ...
        Near_active = Active_concept.rolling(10).sum() >= 1
        Active_concept = Near_active
        ...
        self.Active_concept.to_hdf(save_path + 'Active_concept.h5', key='Active_concept')
        ...
        concept_pct = get_basic_values(factor='Concept_Pct', start_date=begin_date, end_date=20201201)
        for concept in tqdm(Active_concept.columns):
            ...
            concept_stock = get_concept_values('Concept_StockList', concept, begin_date, stop_date,\
                                               read_path='/data/group/800319/Temporary_Data/RawData/')
            ...
            res_concept_stock.to_hdf(save_path + 'Active_stock.h5', key=concept)
...
sd.get_Active_concept(save_path='/data/user/fengchi/首阴反包/append_data/')