...
start_date = 20210701
end_date = 20210917
trend_stock = pd.read_pickle(junk_path + 'trend_daily_stock_20210923.pkl')

ths_dic = pd.read_json(ths_path + '概念板块同花顺%d.json' % 20210805, typ='dict')
main_concept_df = pd.DataFrame().reindex_like(trend_stock)
...
def generate_daily_active_stock(main_dict, ret_df):
    for date_tuple, concepts in main_dict.items():
        ...
        stk_list = list(set(stk_list).intersection(set(trend_stock.columns.tolist())))
...
ret_df = trend_stock & main_concept_df
save_pickle(ret_df, junk_path, '叠加板块trend_daily_stock_20210923.pkl')
