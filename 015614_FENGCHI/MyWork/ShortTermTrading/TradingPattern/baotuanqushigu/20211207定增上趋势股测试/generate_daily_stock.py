...
class SignalGenerator:
    ...
    # 流通市值条件
    def add_mkt_cap_cond(self, cap=200):
    ...
    def add_multi_arrange_cond(self):
        data = pd.read_pickle(faamonitor_path + '中期趋势股20211206.pkl')
        data.columns = list(map(trans_windcode2int, data.columns.tolist()))
        data.index = list(map(int, data.index.tolist()))
        data = data.reindex(index=self.date_list, columns=self.stk_list).fillna(False)
    ...
    def calc_daily_stock_v2(self, multi_arrange_cond):
        # index_cond = self.add_index_cond()  # 使用大盘择时条件
        st_cond = self.add_st_cond()
        mkt_cap_cond = self.add_mkt_cap_cond(cap=0)
        t1_cond = self.add2_t1_cond()
        t5_cond = self.add2_t5_cond()
        multi_arrange_cond = multi_arrange_cond.shift(1).reindex(index=self.date_list, columns=self.stk_list).fillna(False)
...
def wrapper(start_date, end_date, trend_stock):
    print('start', start_date, end_date)
    sg = SignalGenerator(start_date=start_date, end_date=end_date)
    print('初始化完成')
    daily_cond = sg.calc_daily_stock_v2(multi_arrange_cond=trend_stock)
...
if __name__ == '__main__':
    ...
    save_pickle(res, junk_path, 'trend_daily_stock_20211208_mkt0.pkl')
