...
class StockStrategyDemo(StockStrategyBase):
    def __init__(self, stk, start_date, end_date, price_rolling_window=10, amt_per_signal=5000000, available_flag=None,
        ...
        pp_stk_list = pp_df['股票代码'].map(stockList.trans_windcode2int)
        pricing_date = pp_df['定价基准日'].tolist()
        pp_stk_dict = dict(zip(pp_stk_list, pricing_date))
    ...
    def bar_handler(self):
        ...
        if self.stk_id not in list(self.pp_stk_dict.keys()):
            return
        pricing_date = self.pp_stk_dict[self.stk_id]
        pricing_date = tradeDate.get_pre_trade_date(pricing_date, 1)
        shift_pricing_date = tradeDate.get_pre_trade_date(pricing_date, 60)
        if buy_signal == 1 and self.position['holding'] == 0:
            if shift_pricing_date < self.trading_day < pricing_date:
                self.buy()
        ...
        if tradeDate.get_trade_date_interval(self.trading_day, base_date=self.last_buy_time) == holding_day - 1:
            self.sell()
        if self.trading_day == pricing_date:
            self.sell()


def start_backtest(start_date, end_date, trend_stock, output_path, mode='multi'):
    ...
    strats.one_wave_run(stk_list, start_date, end_date, kernel=16, output_path=output_path, mode=mode)
...
if __name__ == '__main__':
    ...
    # trend_stock = pd.read_pickle(junk_path + 'trend_daily_stock_20211207_newVersion.pkl')
    # trend_stock = pd.read_pickle(junk_path + '叠加板块trend_daily_stock_20210923.pkl')
    trend_stock = pd.read_pickle(junk_path + 'trend_daily_stock_20211208_mkt0.pkl')
    bomb_stk_sum = trend_stock.sum() > 0
    stk_list = bomb_stk_sum[bomb_stk_sum].index.tolist()
