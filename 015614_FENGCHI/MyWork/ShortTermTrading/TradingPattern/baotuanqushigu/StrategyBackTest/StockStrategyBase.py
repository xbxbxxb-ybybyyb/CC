...
from dataApi.getData import get_date_range, get_minute_1stock, get_daily_1stock
...
class StockStrategyBase:
    ...
    def __init__(self, stk, start_date, end_date, price_rolling_window=2, amt_per_signal=5000000, available_flag=None,
                 isin_pool_flag=None):
        """
        :param stk:股票代码 int
    ...
    def __daily_update(self, date, i, pre_close_padj, pre_close, pre_low, pre_high, pre_open, pre_adj_ratio):
        ...
        self.record.append(
            [self.trading_day, 925, 'D', np.nan, np.nan, self.position['holding'], self.position['holding']])
    ...
    def backtest(self):
        ...
        for date, i, pre_close_padj, pre_close, pre_low, pre_high, pre_open, pre_adj_ratio in \
                zip(self.date_list, self.date_index, self.pre_dail_info['close_padj'],
                    self.pre_dail_info['close'], self.pre_dail_info['low'],
                    self.pre_dail_info['high'], self.pre_dail_info['open'],
                    self.pre_dail_info['adj_ratio']):
