...
class UniverseEvaluation:
    ...
    def __init__(self, Cls, buy_cost_ratio=0.0005, sell_cost_ratio=0.001, price_rolling_window=10, amt_per_signal=5000000, available_info=None, universe_info=None):
        """
        :param Cls:股票回测类的类名
    ...
    def multi_run(self, stk_list, start, end, kernel=10, append_para={}):
        ...
        for i in range(len(stk_list)):
            ...
            result[stk] = pool.apply_async(self.one_stk_wraper, (*(stk, start, end, append_para),),)
        pool.close()
        pool.join()
        # for key in result:
        #     try:
    ...
    def evaluate_daily(self, kernel=10):
        ...
        for i in range(len(stk_list)):
            ...
            #daily_result[stk] = pool.apply_async(self.evaluate_stk_by_day, (stk,),)
        pool.close()
        pool.join()
    ...
    def evaluate_by_signal(self, kernel=10):
        ...
        for stk in key_list:
            ...
            #pool_dict[stk] = pool.apply_async(self.evaluat_signal_by_stk, (stk,),)
        pool.close()
        pool.join()
