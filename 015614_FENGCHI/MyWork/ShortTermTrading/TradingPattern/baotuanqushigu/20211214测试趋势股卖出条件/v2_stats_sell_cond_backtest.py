...
sell_cond_dict = {
    ...
    '相比昨收跌幅': [-0.02, -0.01, 0],
    '触碰均线反弹力度': [0.005, 0.01],
    'MA5上方监测点参数': [0.05, 0.06, 0.07, 0.08],
    'MA5上方回撤参数': [-0.01, -0.02],
...
for param in sell_cond_param_prod:
    ...
    output_path = junk_path + f'sell_cond2_bt_result20210701_20210903/sell_cond_backtest_({param[0]})_({param[1]})_({param[2]})_({param[3]})_({param[4]}_({param[5]}).xlsx'
    ...
    record.append([param[0], param[1], param[2], param[3], param[4], param[5],
                   deal_times, per_profit, win_ratio, profit_and_coss_ratio])

ret = pd.DataFrame(record)
ret.columns = ['均线上方容错量', '相比昨收跌幅', '触碰均线反弹力度', 'MA5上方监测点参数', 'MA5上方回撤参数',
               'ma5下方止损点', '交易笔数', '单笔收益率',
               '胜率', '盈亏比']
ret.to_excel(junk_path + 'sell_cond2_bt_result_summary20210701_20210903.xlsx')