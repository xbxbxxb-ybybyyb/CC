...
from ShortTermTrading.conf.path_conf import ths_concept_rank_temp_path, ths_concept_rank_path
...
tmp = ret_df.groupby(['TRADINGCODE', 'SECUABBR']).apply(lambda x: get_top3_concept(x))
tmp = tmp.reset_index()
ret_dict = {}
...
file_object = open(ths_concept_rank_path + '同花顺概念排名%d.json' % today_date, 'w')
