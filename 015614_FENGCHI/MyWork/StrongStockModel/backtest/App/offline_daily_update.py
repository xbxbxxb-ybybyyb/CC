...
sys.path.append('/data/user/015836/HANXU/jithub/')

from backtest.App.online_conf import init_conf_path, holding_info_path, vol_info_path, code_list_path, \
    restrict_list_path, local_config_path, hyper_param_path
from xquant.factordata import FactorData
from xquant.compute.aimr import AIMR

from dataApi.tradeDate import trade_minutes, get_date_range, get_pre_trade_date
from dataApi.stockList import trans_int2windcode, trans_windcode2int, clean_stock_list
...
def get_code_list(target_day, restrict_list_path):
    ...
    isolate = sorted([x for x in isolate if x <= '隔离池%d.xls' % target_day])[-1]
    isolate = pd.read_excel(restrict_list_path + isolate)
    isolate = isolate.loc[isolate['交易市场'].isin(['深交所A', '深交所A']) &
                          (isolate['证券类别'] == '股票'), '证券代码'].to_list()

    blacklist = [x for x in os.listdir(restrict_list_path) if x[:3] == '黑名单']
    blacklist = sorted([x for x in blacklist if x <= '黑名单%d.xls' % target_day])[-1]
    ...
    code_list = sorted(list(set(stock_pool) - set(isolate) - set(blacklist)))
...
def daily_update(date, T_plus_1_date):
    ...
    :return:
    """
    per_signal_ratio = 0.005
    barly_max_buy = 200
    stk_min_amt = 200000
    bar_list = [1000, 1030, 1100, 1300, 1330, 1400, 1430]
    rolling_window = 5

    factor_list = pd.read_pickle(local_config_path + 'offline_factor_list.pkl')
    try:
        code_list = pd.read_pickle(code_list_path + '%d.pkl' % date)
    except:
    ...
    # pd.to_pickle(code_list, code_list_path + '%d.pkl' % date)
    # pd.to_pickle(vol_info, vol_info_path + '%d.pkl' % date)
    hyper_param_path = '/data/group/800319/strategy_local_path/factor_hyper_param3/'
...
#d_list = [20201014, 20201015, 20201016, 20201019, 20201020, 20201021, 20201022, 20201023, 20201026, 20201027, 20201028]
d_list = get_date_range(20210119, 20210127)

def _func(date_idx):
    date = d_list[date_idx]
    T_plus_1_date = get_pre_trade_date(date, -1)
    daily_update(date, T_plus_1_date)
    print(date)
