...
date = 20210105
path = '/data/group/800319/strategy_local_path3/validation/'
...
d_matrix = {x: {model: xgb.DMatrix(all_factor[x].fillna(0)[factor_map[model]]) if model.startswith('XGB') else all_factor[x].fillna(0)[factor_map[model]] for model in factor_map}
            for x in all_factor}
...
validate = pd.read_pickle(
    '/data/group/800319/Faamonitor/PL/all_mkt_ts_norm/FactorEvalOnlineTestVRobustOpt/XGBFactorEvalYearlyParam10_ic_half_d_train200_test10_factor_num400_norm_window_40.pkl').loc[
    date]
