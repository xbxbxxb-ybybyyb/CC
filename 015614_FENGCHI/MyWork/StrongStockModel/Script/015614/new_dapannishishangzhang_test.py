...
mkt_stmt = '/data/group/800319/市场情绪与概念板块/历史EMA市场情绪.h5'
    ...
    def bar_handler(self):
        ...
        for stk in sell_list:
            ...
            if self.stk_minute_limit_up_bool_prod.at[(date, time_point), stk] != 1 and \
                    self.stk_pctchg.loc[(date, time_point), stk] > 0.07 and \
                    self.available[stk] != 0:  # 止盈条件
                print(stk, date, time_point, '止盈卖出')
                logger.info(str(stk) + ',' + str(date) + ',' + str(time_point) + ',止盈卖出')
                self.sell_action(stk)
            # 低开幅度小于-4%，卖出
            if self.stk_pctchg.at[(date, time_point), stk] < -0.04 and \
                    self.available[stk] != 0:  # 止损条件
                print(stk, date, time_point, '低开或下跌卖出')
                logger.info(str(stk) + ',' + str(date) + ',' + str(time_point) + ',低开或下跌卖出')
                self.sell_action(stk)
            # 早盘高开5%以上，9:40分涨跌幅小于5%就卖出
            if time_point == 940 and (self.stk_pctchg.at[(date, 930), stk] > 0.05) and (
                    self.stk_pctchg.at[(date, 940), stk] < 0.05) and \
                    self.available[stk] != 0:  # 止损条件
                print(stk, date, time_point, '早盘高开回落卖出')
                logger.info(str(stk) + ',' + str(date) + ',' + str(time_point) + ',早盘高开回落卖出')
                self.sell_action(stk)
            # 9:50后如果个股当前涨跌幅小于0，且个股在均线下方的时间>60%，卖出
            if time_point > 1030 and self.stk_minute_vwap_pct.at[(date, time_point), stk] > 0.6 and \
                    self.available[stk] != 0:
                print(stk, date, time_point, '均线不满足卖出')
                logger.info(str(stk) + ',' + str(date) + ',' + str(time_point) + ',均线不满足卖出')
                self.sell_action(stk)
            # 9:50后个股相对于大盘的超额收益<0 且 时间占比超过50%，卖出
            if time_point > 1030 and self.alpha_pct.at[(date, time_point), stk] > 0.5 and \
                    self.available[stk] != 0:
                print(stk, date, time_point, '超额不满足卖出')
                logger.info(str(stk) + ',' + str(date) + ',' + str(time_point) + ',超额不满足卖出')
                self.sell_action(stk)
            if self.stk_minute_limit_up_bool_prod.at[(date, 930), stk] == 1 and \
                    self.stk_minute_limit_up_bool_prod.at[(date, time_point), stk] == 0 and \
                    self.available[stk] != 0:
                print(stk, date, time_point, '封板涨停卖出')
                logger.info(str(stk) + ',' + str(date) + ',' + str(time_point) + ',封板涨停卖出')
                self.sell_action(stk)
            if time_point == 1455 and \
                    getData.get_trade_date_interval(date, base_date=self.last_buy_time[stk][0]) == holding_day - 1 and \
                    self.available[stk] != 0 and \
                    self.stk_minute_limit_up_bool.at[(date, 1454), stk] != 1:
                print(stk, date, time_point, '强制平仓卖出')
                logger.info(str(stk) + ',' + str(date) + ',' + str(time_point) + ',强制平仓卖出')
                self.sell_action(stk)
