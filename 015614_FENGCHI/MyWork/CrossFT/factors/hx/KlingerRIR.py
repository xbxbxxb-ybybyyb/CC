...
class KlingerRIR(crossFactor):
    cross_group='sw1'
    cross_func='cross_mean'
    extend_days=62
    author='hx'
    logic='Klinger成交量摆动指标的个股排序与行业排序之和'
    article='广发证券-多因子Alpha系列报告之（四十二）：海量技术指标掘金Alpha因子-210730'
    freq='daily'
    basic_datas = {'daily': ['close_badj', 'high_badj', 'low_badj', 'turn'], '30mins': [], '5mins': [], '1min': []}
    ...
    def st_factor(self):
        ...
        pr = (close + high + low) / 3
        turn = pn_condition2(dt_delay(pr, 1), turn)
        turn = dt_mean(dt_mean(turn, 34) - dt_mean(turn, 55), 6)
        return turn
