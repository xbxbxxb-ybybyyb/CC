...
class MomUnderDealRIR(crossFactor):
    cross_group='sw1'
    ...
    def st_factor(self):
        ...
        rank = dt_rank(deal, 240)
        ret1 = np.nansum(np.where(rank < 0.2, ret, 0), axis=1, keepdims=True)
        ret2 = np.nansum(np.where(rank > 0.8, ret, 0), axis=1, keepdims=True)
        return ret1 - ret2
