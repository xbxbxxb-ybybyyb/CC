...
class Top10holderFreeRIR(crossFactor):
    cross_group='sw1'
    ...
    freq='daily'
    basic_datas = {'daily': ['float_a_shares']}
    ...
    def st_factor(self):
        ...
        df = fd.get_factor_value('Basic_factor', [trans_int2windcode(x) for x in self.code_list],
                                 ...
                                 stk_hold_list)
        df['holder_quantity'] -= df['holder_restrictedquantity']
        ...
        factor = df.values[:, None] / self.database['daily']['float_a_shares']
        return factor
