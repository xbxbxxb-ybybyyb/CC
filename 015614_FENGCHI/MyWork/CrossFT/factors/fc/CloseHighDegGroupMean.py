...
class CloseHighDegGroupMean(crossFactor):
    cross_group = 'sw1'
    cross_func = 'cross_mean'
    extend_days = 10
    author = 'fc'
    freq = 'daily'
    logic = '收盘价相对于日内最高价的回归系数'
    article = ''
    basic_datas = {'daily': ['high_badj', 'close_badj'], '30mins': [], '5mins': [], '1min': []}

    def st_factor(self):
        close = self.database['daily']['close_badj']
        high = self.database['daily']['high_badj']
        deg = dt_alpha2(close, high, 10)
        return deg
...
if __name__ == '__main__':
    # f = CloseHighDegGroupMean(start=20210401, end=20210501)
    # print(f.result())
