...
class LowPartPerAmtPct(crossFactor):
    cross_group = 'sw1'
    cross_func = 'cross_mean'
    extend_days = 10
    author = 'fc'
    logic = '日内低价区间内平均每笔成交量占全天平均每笔成交量的比例 行业内均值'
    ...
    def st_factor(self):
        ...
        trade_num_low = tradenum.copy()
        trade_num_low[flag.astype(bool)] = np.nan

        low_price_per_amt = np.nansum(amt_low, axis=1, keepdims=True) / \
                             np.nansum(trade_num_low, axis=1, keepdims=True)
        all_day_per_amt = np.nansum(amt, axis=1, keepdims=True) / \
                             np.nansum(tradenum, axis=1, keepdims=True)

        return low_price_per_amt / all_day_per_amt
...
if __name__ == '__main__':
    # f = LowPartPerAmtPct(start=20210401, end=20210501)
    # print(f.result())

    val = cal_factor('/data/user/015614/MyWork/CrossFT/factors/fc', 'LowPartPerAmtPct.py', {'daily': 6}, notrun=False)
