# coding: utf-8
# Author：fengchi863
# Date ：2021/12/23 19:26

from basic.crossFactor import crossFactor
from basic.crossOperators import *
from basic.crossUtils import *
from basic.operators import *


class VolAmountCorr_5m(crossFactor):
    cross_group = 'sw1'
    cross_func = 'cross_mean'
    extend_days = 3
    author = 'fc'
    freq = '5mins'
    logic = '过去4个小时的成交量与成交额的相关系数，分组求平均'
    article = ''
    basic_datas = {'daily': [], '30mins': [], '5mins': ['volume', 'amt'], '1min': []}

    def st_factor(self):
        vol = self.database['5mins']['volume']
        amt = self.database['5mins']['amt']
        corr = dt_corr2(vol, amt, 48)
        return corr

    def calc_groupst(self):
        indicator = self.st_factor()
        group = sameshape(indicator, self.group_factor())
        group_ret = st2groupst(indicator, group, cross_mean)
        ret = arr_match_index(group_ret, self.cal_date_range, self.date_range)
        return ret

    def result(self):
        return self.calc_groupst()


if __name__ == '__main__':
    # f = VolAmountCorr_5m(start=20210401, end=20210501)
    # print(f.result())

    cal_factor()
