...
class AvgMinCloseEwmSkew_5m(crossFactor):
    cross_group = 'sw1'
    cross_func = 'cross_mean'
    extend_days = 2
    author = 'fc'
    freq = '5mins'
    logic = '分钟均价指数加权移动平均值，计算最后十五分钟均值偏度。偏度越小，获取超额概率越大，分组求平均'
    article = ''
    basic_datas = {'daily': [], '30mins': [], '5mins': ['high', 'low', 'close'], '1min': []}

    def st_factor(self):
        close = self.database['5mins']['close']
        avg_close = dt_ewm(close, 42)
        ret = dt_skew(avg_close, 15)
        return ret
...
if __name__ == '__main__':
    # f = AvgMinCloseEwmSkew_5m(start=20210401, end=20210501)
    # print(f.result())
