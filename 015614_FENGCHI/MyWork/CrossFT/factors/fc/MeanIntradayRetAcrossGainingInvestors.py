...
class MeanIntradayRetAcrossGainingInvestors(crossFactor):
    cross_group = 'sw1'
    ...
    def calc_groupst(self):
        ...
        pct_1 = np.where(pct > 0, pct, 0)
        ret_1 = np.nansum(pct_1 * amt_weight, axis=1, keepdims=True)
        group = sameshape(ret_1, self.group_factor())
        group_ret = st2groupst(ret_1, group, self.group_func())
...
if __name__ == '__main__':
    # f = MeanIntradayRetAcrossLosingInvestors(start=20210401, end=20210501)
    # print(f.result())
