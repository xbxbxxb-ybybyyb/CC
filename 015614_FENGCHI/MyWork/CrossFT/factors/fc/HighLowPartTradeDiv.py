...
class HighLowPartTradeDiv(crossFactor):
    cross_group = 'sw1'
    cross_func = 'cross_mean'
    extend_days = 10
    author = 'fc'
    logic = '日内高价区间内成交笔数与低价区间内成交笔数差异 行业内均值'
    ...
    def st_factor(self):
        ...
        pct = np.nansum(trade_num_high, axis=1, keepdims=True) / \
...
if __name__ == '__main__':
    # f = HighLowPartTradeDiv(start=20210401, end=20210501)
    # print(f.result())

    val = cal_factor('/data/user/015614/MyWork/CrossFT/factors/fc', 'HighLowPartTradeDiv.py', {'daily': 6}, notrun=False)
