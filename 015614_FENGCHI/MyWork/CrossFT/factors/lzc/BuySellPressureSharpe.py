...
class BuySellPressureSharpe(crossFactor):
    window = 15
    cross_group = 'sw1'
    cross_func = '自定义'
    extend_days = 15
    author = 'lzc'
    logic = '个股买卖压日内均值/行业买卖压波动率'
    article = ''
    freq = 'daily'
    ...
    def cal_groupst(self):
        ...
        group_buyorderamt = st2groupst(buyorderamt, minute_group,  self.group_func())
        group_sellorderamt = st2groupst(sellorderamt, minute_group,  self.group_func())

        stk_pressure = buyorderamt / sellorderamt
        group_pressure = group_buyorderamt / group_sellorderamt

        stk_mean = np.nanmean(stk_pressure, axis=1)[:, None, :]
        group_std = np.nanstd(group_pressure, axis=1)[:, None, :]

        return arr_match_index(stk_mean / group_std, self.cal_date_range, self.date_range)
