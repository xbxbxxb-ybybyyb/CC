...
# @Author : Zhichen Lu
# @File : IndustryMCI_Active_daily.py
import sys
...
class IndustryMCI_Active_daily(crossFactor):
    window = 15
    cross_group = 'sw1'
    cross_func = ''
    extend_days = 15
    author = 'lzc'
    logic = '根据逻辑计算个股 主动订单部分(新增主动委买+新增主动委卖) 流动性 并 衍生分组主动订单流动性 相乘'
    article = '中信建投	20201023	因子深度研究系列	买卖报单流动性因子构建'
    freq = 'daily'
    basic_datas = {'daily': ['free_float_shares'], '30mins': [], '5mins': ['activebuyorderamt', 'activebuyordervol', 'activesellorderamt', 'activesellordervol', 'close'],
                   '1min': []}
    ...
    def cal_groupst(self):
        ...
        '''
        activebuyorderamt, activebuyordervol, activesellorderamt, activesellordervol, close = [self.database['5mins'][x] for x in
                                                                                               ['activebuyorderamt', 'activebuyordervol', 'activesellorderamt',
                                                                                                'activesellordervol', 'close']]
        ...
        minute_group = sameshape(activeordervol, self.group_factor())

        sell_vwap = activeorderamt / activeordervol
        sell_order_amt_group = st2groupst(activeorderamt, minute_group, cross_sum)
        industry_sell_vwap = sell_order_amt_group / st2groupst(activeordervol, minute_group, cross_sum)
        ...
        return arr_match_index(np.nanmean(MCIA_stk * MCIA_group, axis=1)[:, None, :], self.cal_date_range, self.date_range)
...
if __name__ == '__main__':
    ...
    send_message(['015664'], 'IndustryMCI_Active done ')
