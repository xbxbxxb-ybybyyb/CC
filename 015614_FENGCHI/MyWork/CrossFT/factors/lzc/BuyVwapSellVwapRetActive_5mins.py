...
# @Author : Zhichen Lu
# @File : BuyVwapSellVwapRetActive_5mins.py
import sys
...
class BuyVwapSellVwapRetActive_5mins(crossFactor):
    window = 15
    cross_group = 'sw1'
    cross_func = 'cross_sum'
    extend_days = 15
    author = 'lzc'
    logic = '卖单VWAP对买单VWAP收益率对分组收益的超额收益'
    article = ''
    freq = '5mins'
    ...
    def cal_groupst(self):
        ...
        group_ret = st2groupst(ret * free_float_shares, minute_group,  self.group_func()) / st2groupst(free_float_shares, daily_group, self.group_func())
        active = ret - group_ret

        # std = np.nanstd(active, axis=1)[:, None, :]

        return arr_match_index(active, self.cal_date_range, self.date_range)
...
if __name__ == '__main__':
    ...
    f = BuyVwapSellVwapRetActive_5mins()
    e = time.time()
    # f.result()
    f.save_result()
