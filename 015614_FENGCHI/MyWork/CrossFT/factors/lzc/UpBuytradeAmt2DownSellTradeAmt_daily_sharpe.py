...
# @Author : Zhichen Lu
# @File : UpBuytradeAmt2DownSellTradeAmt_daily_sharpe.py
import sys
...
class UpBuytradeAmt2DownSellTradeAmt_daily_sharpe(crossFactor):
    window = 15
    cross_group = 'sw1'
    cross_func = ''
    extend_days = 15
    author = 'lzc'
    logic = '截面上分组内上行主动买入额/截面上分组下行主动卖出额 * 个股主动买卖之比  日内求夏普'
    article = ''
    freq = 'daily'
    ...
    def cal_groupst(self):
        ...
        up_buy_trade_num = np.where(ret > 0, buytradeamt, selltradeamt)
        down_sell_trade_num = np.where(ret < 0, selltradeamt, buytradeamt)

        group_ratio = st2groupst(up_buy_trade_num, minute_group, cross_sum) / st2groupst(down_sell_trade_num, minute_group, cross_sum)

        factor = buytradeamt * group_ratio / selltradeamt
        daily_factor = np.nanmean(factor, axis=1) / np.nanstd(factor, axis=1)
...
if __name__ == '__main__':
    ...
    f = UpBuytradeAmt2DownSellTradeAmt_daily_sharpe()
    e = time.time()
    # f.result()
    f.save_result()
