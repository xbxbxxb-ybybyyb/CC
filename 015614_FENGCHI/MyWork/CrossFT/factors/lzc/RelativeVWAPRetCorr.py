...
class RelativeVWAPRetCorr(crossFactor):
    cross_group = 'sw1'
    cross_func = ''
    extend_days = 0
    author = 'lzc'
    logic = '日内上行阶段主动买入订单的VWAP 与 主动买入VWAP与收盘价的收益 的相关性 + 分组日内面板相关性 '
    article = ''
    freq = 'daily'
    basic_datas = {'daily': ['free_float_shares'], '30mins': [], '5mins': ['close', 'buytradeamt', 'buytradevol'], '1min': []}
    ...
    def st_factor(self):
        ...
        return [self.database['5mins'][x] for x in ['close', 'buytradeamt', 'buytradevol']]

    def cal_groupst(self):
        '''
        :return: np.array,个股值==》组值==》个股值 ，没有行业值在和个股值进行某些计算
        '''
        close, buytradeamt, buytradevol = self.st_factor()
        self.group = sameshape(self.database['daily']['free_float_shares'], self.group_factor())

        buytradevwap = buytradeamt / buytradevol
        ret = buytradevwap / close - 1
        net_lize_buytradevwap = buytradevwap / buytradevwap[:, [0], :]

        count_X = np.isfinite(net_lize_buytradevwap)
        count_y = np.isfinite(ret)
        count = count_X & count_y

        X = np.where(count, net_lize_buytradevwap, 0)
