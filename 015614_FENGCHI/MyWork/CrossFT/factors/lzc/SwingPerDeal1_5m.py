...
class SwingPerDeal1_5m(crossFactor):
    window = 15
    cross_group = 'sw1'
    cross_func = '自定义'
    extend_days = 15
    author = 'lzc'
    logic = '近240分钟振幅除以成交笔数 个股因子*分组因子'
    article = ''
    freq = '5mins'
    ...
    def cal_groupst(self):
        ...
        past_rolling_day_mean, past_rolling_day_std = intrad_past_day_rolling_mv(close_min * adjfactor)
        past_rolling_day_high = (dt_max(high * adjfactor, shape[1]) - past_rolling_day_mean) / past_rolling_day_std
        past_rolling_day_low = (dt_min(low * adjfactor, shape[1]) - past_rolling_day_mean) / past_rolling_day_std

        swing = past_rolling_day_high - past_rolling_day_low
        stk_swing_per_deal = swing / tradenum
        group_swing_per_deal = (st2groupst(past_rolling_day_high, minute_group, cross_max) - st2groupst(past_rolling_day_low, minute_group, cross_min)) / st2groupst(tradenum,
                                                                                                                                                                     minute_group,
                                                                                                                                                                     cross_sum)
