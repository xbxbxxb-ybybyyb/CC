...
# @Author : Zhichen Lu
# @File :RelativePriceAbsRetCorr.py
...
class RelativePriceAbsRetCorr(crossFactor):
    cross_group = 'sw1'
    cross_func = 'cross_sum'
    extend_days = 0
    author = 'lzc'
    logic = '单股票价格日内时序与收益率的相关性 + 分组日内面板相关性 '
    article = ''
    freq = 'daily'
    ...
    def cal_groupst(self):
        ...
        net_lize_close = close / close[:, [0], :]

        count_X = np.isfinite(net_lize_close)
        count_y = np.isfinite(ret)
        count = count_X & count_y

        X = np.where(count, net_lize_close, 0)
