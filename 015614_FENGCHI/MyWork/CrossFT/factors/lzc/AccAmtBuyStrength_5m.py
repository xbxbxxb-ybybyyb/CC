...
class AccAmtBuyStrength_5m(crossFactor):
    window = 15
    cross_group = 'sw1'
    cross_func = '自定义'
    extend_days = 15
    ...
    def cal_groupst(self):
        ...
        group_buyorderamt = st2groupst(activebuyorderamt, minute_group, cross_sum)
        group_sellorderamt = st2groupst(activesellorderamt, minute_group, cross_sum)
        ...
        stk_mean, stk_std = intrad_past_day_rolling_mv(stk_diff)  # [:, None, :]
        group_mean, group_std = intrad_past_day_rolling_mv(group_diff)
