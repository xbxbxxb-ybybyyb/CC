...
class MiddlePriceBias_daily(crossFactor):
    window = 15
    cross_group = 'sw1'
    cross_func = ''
    extend_days = 15
    ...
    def cal_groupst(self):
        ...
        daily_MPB = np.nanmean(MPB, axis=1) / np.nanstd(MPB, axis=1)
        daily_MPB_group = np.nanmean(MPB_group, axis=1) / np.nanstd(MPB_group, axis=1)
        return arr_match_index(daily_MPB[:, None, :] * daily_MPB_group[:, None, :], self.cal_date_range, self.date_range)
