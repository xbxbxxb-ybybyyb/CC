...
class OrderImbalace_daily(crossFactor):
    window = 15
    cross_group = 'sw1'
    cross_func = ''
    extend_days = 15
    author = 'lzc'
    logic = '计算根据流通股票本折算OFI子 再计算行业整体的OFI'
    ...
    def cal_groupst(self):
        ...
        OFI_stk = calc_ofi(adj_buytradevol, adj_selltradevol, vwap_buy, vwap_sell)
        ...
        OFI_group = calc_ofi(group_adj_buyvol, group_adj_sellvol, group_buyvwap, group_sell_vwap)

        OFI_stk_daily = np.nanmean(OFI_stk, axis=1)[:, None, :]
        OFI_group_daily = np.nanmean(OFI_group, axis=1)[:, None, :]

        return arr_match_index(OFI_stk_daily * OFI_group_daily, self.cal_date_range, self.date_range)
