...
class BuyVol_ActiveSell(crossFactor):
    ...
    article=''
    freq='daily'
    basic_datas = {'daily': [], '30mins': [], '5mins': ['buyorderamt', 'activesellorderamt'], '1min': []}
    ...
    def st_factor(self):
        ...
        '''
        # buyorderamt,activesellorderamt =
        return [self.database['5mins'][x] for x in ['buyorderamt', 'activesellorderamt']]
    ...
    def cal_groupst(self):
        ...
        buyorderamt, activesellorderamt = np.nansum(buyorderamt, axis=1)[:, None, :], np.nansum(activesellorderamt, axis=1)[:, None, :]
