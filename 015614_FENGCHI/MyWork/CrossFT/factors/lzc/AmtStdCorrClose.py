...
class AmtStdCorrClose(crossFactor):
    window = 15
    cross_group = 'sw1'
    cross_func = '自定义'
    extend_days = 15
    author = 'lzc'
    logic = '成交额日内除均值后求波动率，收盘价对日内价标准化，在截面求相关系数*成交额波动率'
    article = ''
    freq = 'daily'
    ...
    def cal_groupst(self):
        ...
        amt_std = np.nanstd(amt.swapaxes(0, 1) - np.nanmean(amt, axis=1), axis=0)[:, None, :]
        close_Z = ((close[:, 0, :] - np.nanmean(close_min, axis=1)) / np.nanstd(close_min, axis=1))[:, None, :]
        ...
        EX = st2groupst(amt_std, daily_group, cross_mean)
        gc.collect()
        print(2)
        EY = st2groupst(close_Z, daily_group, cross_mean)
        gc.collect()
        print(3)
        EXY = st2groupst(close_Z * amt_std, daily_group, cross_mean)
        gc.collect()
        print(1)
        ...
        STDX = st2groupst(amt_std, daily_group, cross_std)
        STDY = st2groupst(close_Z, daily_group, cross_std)
