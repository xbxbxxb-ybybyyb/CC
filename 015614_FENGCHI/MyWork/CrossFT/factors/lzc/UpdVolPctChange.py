...
class UpdVolPctChange(crossFactor):
    cross_group = 'sw1'
    cross_func = 'cross_mean'
    extend_days = 32
    author = 'lzc'
    logic = '上行成交额变化率 截面sharpe'
    article = ''
    freq = 'daily'
    ...
    def cal_groupst(self):
        ...
        up_amt = np.where(ret > 0, amt, np.nan)
        up_amt_daily = np.nansum(up_amt, axis=1)[:, None, :]
