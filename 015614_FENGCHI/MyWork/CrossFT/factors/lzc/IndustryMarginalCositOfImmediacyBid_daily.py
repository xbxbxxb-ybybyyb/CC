...
# @Author : Zhichen Lu
# @File : IndustryMarginalCositOfImmediacyBid_daily.py
import sys
...
class IndustryMarginalCositOfImmediacyBid_daily(crossFactor):
    window = 15
    cross_group = 'sw1'
    cross_func = ''
    extend_days = 15
    author = 'lzc'
    logic = '根据逻辑计算个股买单流动性 并 衍生分组买单流动性 相乘'
    ...
    def cal_groupst(self):
        ...
        return arr_match_index(np.nanmean(MCIB_stk, axis=1)[:, None, :] * np.nanmean(MCIB_group, axis=1)[:, None, :],
...
if __name__ == '__main__':
    ...
    f = IndustryMarginalCositOfImmediacyBid_daily()
    e = time.time()
    # f.result()
    f.save_result()
