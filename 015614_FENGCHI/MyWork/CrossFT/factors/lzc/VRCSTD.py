...
class VRCSTD(crossFactor):
    window = 15
    cross_group = 'sw1'
    cross_func = ''
    extend_days = 15
    author = 'lzc'
    logic = '个股筹码收益标准差 * 行业筹码收益标准差'
    artical = ''
    freq = 'daily'
    basic_datas = {'daily': ['a_mkt_cap'], '30mins': [], '5mins': ['close'],
                   '1min': []}
    ...
    def cal_groupst(self):
        ...
        VRC = close / close[:, 0, :][:, None, :]
        group_VRC = st2groupst(VRC * a_mkt_cap, minute_group,  self.group_func()) / st2groupst(VRC * a_mkt_cap, daily_group,  self.group_func())
        # np.nanstd(VRC)[:,None,:]*np.nanstd(group_VRC)[:,None,:]

        return arr_match_index(np.nanstd(VRC, axis=1)[:, None, :] * np.nanstd(group_VRC, axis=1)[:, None, :], self.cal_date_range, self.date_range)
