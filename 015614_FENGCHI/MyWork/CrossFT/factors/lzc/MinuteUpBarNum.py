...
class MinuteUpBarNum(crossFactor):
    window = 15
    cross_group = 'sw1'
    cross_func = 'cross_sum'
    extend_days = 15
    author = 'lzc'
    logic = '5分钟bar上超过或两倍标准差的bar的数量 * 分组内所有股票超过两倍标准差bar的数量占比的均值'
    ...
    def cal_groupst(self):
        ...
        over_2_std_count = np.nansum(np.where(abs(z_close) > 2, 1, 0), axis=0)[:, None, :]
        over_2_std_ratio = over_2_std_count / self.database['5mins']['close'].shape[1]
        group_2_std_ratio = st2groupst(over_2_std_ratio, daily_group, self.group_func())
