...
class GroupSelfCorr_5m(crossFactor):
    ...
    extend_days = 15
    author = 'lzc'
    logic = '个股收益在组内的zscore的自相关性 滚动240分钟计算'
    article = ''
    freq = '5mins'
    ...
    def cal_groupst(self):
        ...
        EXY = intrad_past_day_rolling_mean(grou_zscore_lag * grou_zscore)
        EXEY = intrad_past_day_rolling_mean(grou_zscore) * intrad_past_day_rolling_mean(grou_zscore_lag)
        STDXSTDY = intrad_past_day_rolling_std(grou_zscore) * intrad_past_day_rolling_std(grou_zscore_lag)
