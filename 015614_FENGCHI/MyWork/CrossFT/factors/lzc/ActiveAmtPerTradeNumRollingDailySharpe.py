...
# @Author : Zhichen Lu
# @File : ActiveAmtPerTradeNumRollingDailySharpe.py
...
class ActiveAmtPerTradeNumRollingDailySharpe(crossFactor):
    window = 15
    cross_group = 'sw1'
    cross_func = '自定义'
    extend_days = 15
    author = 'lzc'
    logic = '平均每笔成交带来的新增主动委托量滚动一日sharpe'
    article = ''
    freq = '5mins'
    ...
    def cal_groupst(self):
        ...
        stk_m, stk_v = intrad_past_day_rolling_mv(stk_factor)
        group_m, group_v = intrad_past_day_rolling_mv(group_factor)
        return arr_match_index((stk_m * group_m) / (stk_v * group_v), self.cal_date_range, self.date_range)
