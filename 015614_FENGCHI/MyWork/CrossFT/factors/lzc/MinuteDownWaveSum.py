...
class MinuteDownWaveSum(crossFactor):
    window = 15
    cross_group = 'sw1'
    cross_func = 'cross_sum'
    extend_days = 15
    author = 'lzc'
    logic = '5分钟bar上超过或两倍标准差的振幅之和/小于-2被标准差的振幅之和 * 组的该值'
    ...
    def cal_groupst(self):
        ...
        lower_neg2_std_wavecount = np.nansum(np.where(z_close < -2, z_close, 0), axis=0)[:, None, :] * -1
