...
class UpStdToDownStd(crossFactor):
    window = 15
    cross_group = 'sw1'
    cross_func = 'cross_sum'
    extend_days = 15
    author = 'lzc'
    logic = '个股上行波动率/下行波动率   *  分组收益均值上行波动率/下行波动率'
    article = ''
    freq = 'daily'
    ...
    def cal_groupst(self):
        ...
        down_std = np.nanstd(np.where(ret < 0, ret, np.nan), axis=1)[:, None, :]
        ...
        group_down_std = np.nanstd(np.where(group_mean_ret < 0, group_mean_ret, np.nan), axis=1)[:, None, :]
