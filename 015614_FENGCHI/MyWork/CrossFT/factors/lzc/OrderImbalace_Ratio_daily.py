...
# @Author : Zhichen Lu
# @File : OrderImbalace_Ratio_daily.py
import sys
...
class OrderImbalace_Ratio_daily(crossFactor):
    window = 15
    cross_group = 'sw1'
    cross_func = ''
    extend_days = 15
    author = 'lzc'
    logic = '计算根据流通股票本折算订单失衡率因子 再计算行业整体的订单失衡率   日内均值/标准差'
    artical = '中信建投	20210707	因子深度研究系列	多层次订单失衡及订单斜率因子'
    freq = 'daily'
    basic_datas = {'daily': ['free_float_shares'], '30mins': [], '5mins': ['buytradevol', 'selltradevol'], '1min': []}
    ...
    def cal_groupst(self):
        ...
        '''
        buytradevol, selltradevol = [self.database['5mins'][x] for x in ['buytradevol', 'selltradevol']]
        ...
        OIR_stk = (adj_buytradevol - adj_selltradevol) / (adj_buytradevol + adj_selltradevol)
        OIR_group = (group_adj_buyvol - group_adj_sellvol) / (group_adj_buyvol + group_adj_sellvol)

        OIR_stk = np.nanmean(OIR_stk, axis=1) / np.nanstd(OIR_stk, axis=1)
        OIR_group = np.nanmean(OIR_group, axis=1) / np.nanstd(OIR_group, axis=1)

        return arr_match_index(OIR_stk[:, None, :] * OIR_group[:, None, :], self.cal_date_range, self.date_range)
