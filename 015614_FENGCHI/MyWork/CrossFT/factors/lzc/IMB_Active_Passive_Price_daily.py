...
# @Author : Zhichen Lu
# @File : IMB_Active_Passive_Price_daily.py
import sys
...
class IMB_Active_Passive_Price_daily(crossFactor):
    window = 15
    cross_group = 'sw1'
    cross_func = ''
    extend_days = 15
    author = 'lzc'
    logic = '根据逻辑计算个股 主动新增买单、被动新增买单 不平衡因子 并 衍生分组 主动新增买单、被动新增买单 流动性不平衡因子 相乘 日频'
    ...
    def cal_groupst(self):
        ...
        passiveordervol, passiveorderamt, activebuyordervol, activeorderamt, activeordervol = [np.nansum(x, axis=1)[:, None, :] for x in
                                                                                               [passiveordervol, passiveorderamt, activebuyordervol, activeorderamt,
                                                                                                activeordervol]]
        ...
        IndutryPrice = st2groupst(close * free_float_shares, daily_group,  self.group_func())

        sell_vwap = activeorderamt / activeordervol
        sell_order_amt_group = st2groupst(activeorderamt, daily_group,  self.group_func())
        industry_sell_vwap = sell_order_amt_group / st2groupst(activeordervol, daily_group,  self.group_func())
        ...
        buy_order_amt_group = st2groupst(passiveorderamt, daily_group,  self.group_func())
        industry_buy_vwap = buy_order_amt_group / st2groupst(passiveordervol, daily_group,  self.group_func())
...
if __name__ == '__main__':
    ...
    f = IMB_Active_Passive_Price_daily()
    e = time.time()
    # f.result()
    f.result()
