...
class PassiveTurnOver_daily(crossFactor):
    window = 15
    cross_group = 'sw1'
    cross_func = ''
    extend_days = 15
    author = 'lzc'
    logic = '被动成交额计算的换手率 * 分组上的换手率'
    article = ''
    freq = 'daily'
    basic_datas = {'daily': ['a_mkt_cap'], '30mins': [], '5mins': ['passivebuyorderamt', 'passivebuyorderamt', 'close'], '1min': []}
    ...
    def cal_groupst(self):
        ...
        '''
        passivebuyorderamt, passivebuyorderamt, close = [self.database['5mins'][x] for x in ['passivebuyorderamt', 'passivebuyorderamt', 'close']]
        a_mkt_cap = self.database['daily']['a_mkt_cap']
        a_mkt_cap = delay(a_mkt_cap, 1)
        daily_group = sameshape(a_mkt_cap, self.group_factor())
        minute_group = sameshape(passivebuyorderamt, self.group_factor())
        ...
        group_active_amt = st2groupst(down_buy_tradeamt + up_sell_tradeamt, minute_group, cross_sum) / st2groupst(a_mkt_cap, daily_group, cross_sum)
        stk_active_amt = (down_buy_tradeamt + up_sell_tradeamt) / a_mkt_cap

        return arr_match_index(group_active_amt[:, [-1], :] * stk_active_amt[:, [-1], :], self.cal_date_range, self.date_range)
