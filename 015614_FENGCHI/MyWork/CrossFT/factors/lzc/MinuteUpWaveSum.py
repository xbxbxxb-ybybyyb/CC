...
class MinuteUpWaveSum(crossFactor):
    window = 15
    cross_group = 'sw1'
    cross_func = 'cross_sum'
    extend_days = 15
    author = 'lzc'
    logic = '5分钟bar上超过或两倍标准差的振幅之和 * 组的均值'
    ...
    def cal_groupst(self):
        ...
        over_2_std_wavecount = np.nansum(np.where(z_close > 2, z_close, 0), axis=0)[:, None, :]
        group_over_2_std_wavecount = st2groupst(over_2_std_wavecount, daily_group, cross_mean)

        return arr_match_index(over_2_std_wavecount * group_over_2_std_wavecount, self.cal_date_range, self.date_range)
...
if __name__ == '__main__':
    val1 = cal_factor(start=20210101,end=20210701,numd={},logic='haha,idiot')
    val2 = cal_factor(start=20210101,end=20210701,numd={'daily': 10},logic='haha,idiot')
    gap = abs(val1 - val2)
