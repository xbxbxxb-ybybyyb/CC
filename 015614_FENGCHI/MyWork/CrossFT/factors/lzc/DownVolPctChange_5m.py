...
class DownVolPctChange_5m(crossFactor):
    cross_group = 'sw1'
    cross_func = 'cross_mean'
    extend_days = 32
    author = 'lzc'
    logic = '下行成交额变化率 截面sharpe 前一日T时刻到当前交易日为一日'
    article = ''
    freq = '5mins'
    ...
    def cal_groupst(self):
        ...
        factor_up = st2groupst(up_amt_pct_change, daily_group, cross_mean) / st2groupst(up_amt_pct_change, daily_group, cross_std)

        down_amt_daily = intrad_past_day_rolling_sum(down_amt)
        down_amt_pct_change = down_amt_daily / delay(down_amt_daily, 1) - 1
        factor_down = st2groupst(down_amt_pct_change, daily_group, cross_mean) / st2groupst(down_amt_pct_change, daily_group, cross_std)

        return arr_match_index(factor_up / factor_down, self.cal_date_range, self.date_range)
