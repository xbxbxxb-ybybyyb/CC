...
# @Author : Zhichen Lu
# @File : HeavyActiveAmtRatio_daily.py
import sys
...
class HeavyActiveAmtRatio_daily(crossFactor):
    window = 15
    cross_group = 'sw1'
    cross_func = ''
    extend_days = 15
    author = 'lzc'
    logic = '放量过程中新增主动委托额占主动委托额的比例 滚动240分钟'
    article = ''
    freq = 'daily'
    basic_datas = {'daily': ['a_mkt_cap'], '30mins': [], '5mins': ['activesellorderamt', 'activebuyorderamt', 'amt'], '1min': []}
    ...
    def cal_groupst(self):
        ...
        '''
        activesellorderamt, activebuyorderamt, amt = [self.database['5mins'][x] for x in
                                                      ['activesellorderamt', 'activebuyorderamt', 'amt']]
        ...
        up = heavy_flag > 0
        down = heavy_flag < 0
        activeamt = activesellorderamt + activebuyorderamt
        heavy_activeamt = intrad_past_day_rolling_sum(activeamt, up)
