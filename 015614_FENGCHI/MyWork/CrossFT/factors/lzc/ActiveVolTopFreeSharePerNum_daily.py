...
class ActiveVolTopFreeSharePerNum_daily(crossFactor):
    window = 15
    cross_group = 'sw1'
    ...
    def cal_groupst(self):
        ...
        total_trade_vol = np.nansum(buytradevol + selltradevol, axis=1)[:, None, :]
        total_tradenum = np.nansum(tradenum, axis=1)[:, None, :]
        vol_percent_per_num = total_trade_vol / total_tradenum / free_float_shares

        group_vol_percent_per_num = st2groupst(total_trade_vol / free_float_shares, daily_group, cross_sum) / st2groupst(total_tradenum, daily_group, cross_sum)
...
if __name__ == '__main__':
    ...
    f = ActiveVolTopFreeSharePerNum_daily()
    e = time.time()
    # f.result()
    f.save_result()
