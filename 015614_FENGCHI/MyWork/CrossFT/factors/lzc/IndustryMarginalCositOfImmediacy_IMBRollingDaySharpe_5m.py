...
# @Author : Zhichen Lu
# @File : IndustryMarginalCositOfImmediacy_IMBRollingDaySharpe_5m.py
import sys
...
class IndustryMarginalCositOfImmediacy_IMBRollingDaySharpe_5m(crossFactor):
    window = 15
    cross_group = 'sw1'
    cross_func = ''
    extend_days = 15
    author = 'lzc'
    logic = '根据逻辑计算个股买卖报单流动性不平衡因子 并 衍生分组买卖报单流动性不平衡因子 相乘 滚动一日夏普'
    ...
    def cal_groupst(self):
        ...
        mean_stk, std_stk = intrad_past_day_rolling_mv(MCI_IMB_stk)
        mean_group, std_group = intrad_past_day_rolling_mv(MCI_IMB_group)

        stk_factor = (MCIB_stk - mean_stk) / std_stk
        group_factor = (MCIB_group - mean_group) / std_group
