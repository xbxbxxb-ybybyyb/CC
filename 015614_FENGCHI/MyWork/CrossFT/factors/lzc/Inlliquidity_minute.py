...
class Inlliquidity_minute(crossFactor):
    window = 15
    cross_group = 'sw1'
    ...
    article = ''
    freq = '5mins'
    basic_datas = {'daily': ['free_float_shares'], '30mins': [], '5mins': ['close_badj', 'amt'], '1min': []}
    ...
    def st_factor(self):
        ...
        return self.database['5mins']['close_badj'], self.database['daily']['free_float_shares'], self.database['5mins']['amt']

    def cal_groupst(self):
        '''
        :return: np.array,个股值==》组值==》个股值 ，没有行业值在和个股值进行某些计算
        '''
        close, free_float_shares, amt = self.st_factor()
        ...
        group_ret = ret * free_float_shares / st2groupst(free_float_shares, daily_group, cross_sum)
        group_turnover = st2groupst(amt, daily_group, cross_sum) / st2groupst(free_float_shares, daily_group, cross_sum)

        stk_illiquidity = ret / stk_turnover

        group_illiquidity = group_ret / group_turnover
