...
# @Author : Zhichen Lu
# @File : BuyVwapSellVwapRetActiveVolatility_5m.py
import sys
...
class BuyVwapSellVwapRetActiveVolatility_5m(crossFactor):
    window = 15
    cross_group = 'sw1'
    cross_func = 'cross_sum'
    extend_days = 15
    author = 'lzc'
    logic = '卖单VWAP对买单VWAP收益率对分组收益的超额收益的波动率 前日T时刻到该日T时刻为一日'
    article = ''
    freq = '5mins'
    ...
    def cal_groupst(self):
        ...
        std = intrad_past_day_rolling_std(active)

        return arr_match_index(std, self.cal_date_range, self.date_range)
