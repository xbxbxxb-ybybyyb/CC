...
# @Author : Zhichen Lu
# @File : UpActiveTurnOver_daily.py
import sys
...
class UpActiveTurnOver_daily(crossFactor):
    window = 15
    cross_group = 'sw1'
    cross_func = ''
    extend_days = 15
    author = 'lzc'
    logic = '上行主动成交额计算的换手率 * 分组上的换手率'
    article = ''
    freq = 'daily'
    basic_datas = {'daily': ['a_mkt_cap'], '30mins': [], '5mins': ['activebuyorderamt', 'activesellorderamt', 'close'], '1min': []}
    ...
    def cal_groupst(self):
        ...
        '''
        activebuyorderamt, activesellorderamt, close = [self.database['5mins'][x] for x in ['activebuyorderamt', 'activesellorderamt', 'close']]
