# @Time : 2021/8/16 15:31
# @Author : Zhichen Lu
# @File :CorrTrendRet_5m.py


from basic.crossFactor import crossFactor
from basic.crossUtils import *
from basic.crossConfig import *
from basic.crossOperators import *


def fill_nan(arr, l):
    return np.pad(arr, ((l, 0), (0, 0), (0, 0)), mode='constant', constant_values=np.nan)


def delay(arr, l=1):
    return fill_nan(arr[:-l], l)


class CorrTrendRet_5m(crossFactor):
    cross_group = 'sw1'
    cross_func = 'cross_mean'
    extend_days = 2
    author = 'lzc'
    logic = '价量趋势指标(振幅*换手率)与收益率相关性 昨日T时刻到当日T时刻为一天'
    article = ''
    freq = '5mins'
    basic_datas = {'daily': ['free_float_shares', 'volume', 'close'], '30mins': [], '5mins': ['close', 'vol', 'high', 'low'], '1min': []}

    window = 15

    def st_factor(self):
        '''
        :return: 1. np.array ,返回个股计算组值需要的个股因子
                2.  list(np.array)， 返回多个个股因子，有些为了计算组值，有些为了计算
        '''

        return [self.database['5mins'][x] for x in ['close', 'vol', 'high', 'low']]

    def cal_groupst(self):
        '''
        :return: np.array,个股值==》组值==》个股值 ，没有行业值在和个股值进行某些计算
        '''
        close, vol, high, low = self.st_factor()
        free_float_shares = delay(self.database['daily']['free_float_shares'], 1)
        turn = vol * close / delay(self.database['daily']['free_float_shares'] * self.database['daily']['close'], 1)
        swing = high / low - 1
        trend = turn * swing
        ret = close / delay(close.swapaxes(0, 1)).swapaxes(0, 1) - 1
        daily = sameshape(close, self.group_factor())
        # minute = sameshape(volume, self.group_factor())
        import bottleneck
        def intrad_past_day_rolling_sum(x_, finit=None):
            shape = x_.shape
            if finit is None:
                finit = np.isfinite(x_)
            x = np.where(finit, x_, 0)
            return bottleneck.move_sum(np.concatenate([delay(x, 1), x], axis=1), window=shape[1], axis=1)[:, -shape[1]:, :]

        Y = ret
        X = trend * ret

        count = np.isfinite(X) & np.isfinite(Y)
        X = np.where(count, X, 0)
        Y = np.where(count, Y, 0)

        SUMX2 = intrad_past_day_rolling_sum(X ** 2, count)
        SUMY2 = intrad_past_day_rolling_sum(Y ** 2, count)
        SUMXY = intrad_past_day_rolling_sum(X * Y, count)
        SUMY = intrad_past_day_rolling_sum(Y, count)
        SUMX = intrad_past_day_rolling_sum(X, count)
        COUNT = intrad_past_day_rolling_sum(count, count)

        stk_corr = (COUNT * SUMXY - SUMY * SUMX) / (COUNT * SUMX2 - SUMX ** 2) ** 0.5 / (COUNT * SUMY2 - SUMY ** 2) ** 0.5

        group_COUNT = st2groupst(COUNT, daily, cross_sum)
        group_SUMX2 = st2groupst(SUMX2, daily, cross_sum)
        group_SUMY2 = st2groupst(SUMY2, daily, cross_sum)
        group_SUMXY = st2groupst(SUMXY, daily, cross_sum)
        group_SUMY = st2groupst(SUMY, daily, cross_sum)
        group_SUMX = st2groupst(SUMX, daily, cross_sum)

        group_corr = (group_COUNT * group_SUMXY - group_SUMY * group_SUMX) / (group_COUNT * group_SUMX2 - group_SUMX ** 2) ** 0.5 / (
                group_COUNT * group_SUMY2 - group_SUMY ** 2) ** 0.5

        return arr_match_index(stk_corr * group_corr, self.cal_date_range, self.date_range)

    def result(self):
        '''
        :return: np.array,
                 如果只是个股计算组值进行平铺，则return cal_groupst()
                 如果是计算了分组然后又跟个股进行了计算，则 return cal_customst()
        '''
        return self.cal_groupst()


if __name__ == '__main__':
    cal_factor()
