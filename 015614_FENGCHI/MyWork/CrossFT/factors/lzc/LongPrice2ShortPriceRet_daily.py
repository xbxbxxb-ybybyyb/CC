...
# @Author : Zhichen Lu
# @File : LongPrice2ShortPriceRet_daily.py
import sys
...
class LongPrice2ShortPriceRet_daily(crossFactor):
    window = 15
    cross_group = 'sw1'
    cross_func = ''
    extend_days = 15
    ...
    def cal_groupst(self):
        ...
        passivebuyorderamt, activebuyorderamt, passivebuyordervol, activebuyordervol = [self.database['5mins'][x] for x in
                                                                                        ['passivesellorderamt', 'activesellorderamt', 'passivesellordervol', 'activebuyordervol',
        ...
        long_amt, long_vol, short_amt, short_vol = [np.nansum(x, axis=1)[:, None, :] for x in [long_amt, long_vol, short_amt, short_vol]]

        stk_pressure_net = (long_amt / long_vol) / (short_amt / short_vol)
        group_long_price = st2groupst(long_amt, daily_group, cross_sum) / st2groupst(long_vol, daily_group, cross_sum)
        group_short_price = st2groupst(short_amt, daily_group, cross_sum) / st2groupst(short_vol, daily_group, cross_sum)
...
if __name__ == '__main__':
    ...
    f = LongPrice2ShortPriceRet_daily()
    e = time.time()
    # f.result()
    f.result()
