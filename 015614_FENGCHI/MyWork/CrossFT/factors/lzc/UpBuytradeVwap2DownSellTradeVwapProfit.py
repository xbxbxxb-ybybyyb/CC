...
# @Author : Zhichen Lu
# @File : UpBuytradeVwap2DownSellTradeVwapProfit.py
import sys
...
class UpBuytradeVwap2DownSellTradeVwapProfit(crossFactor):
    window = 15
    cross_group = 'sw1'
    cross_func = ''
    extend_days = 15
    author = 'lzc'
    logic = '截面上分组内上行日内主动买入均价对截面上下行主动卖出均价的收益率 与 个股该值复利'
    article = ''
    freq = 'daily'
    ...
    def cal_groupst(self):
        ...
        up_buy_tradeamt = np.nansum(np.where(ret > 0, buytradeamt, 0), axis=1)[:, None, :]
        up_buy_tradevol = np.nansum(np.where(ret > 0, buytradevol, 0), axis=1)[:, None, :]
        down_sell_tradeamt = np.nansum(np.where(ret < 0, selltradeamt, 0), axis=1)[:, None, :]
        down_sell_tradevol = np.nansum(np.where(ret < 0, selltradevol, 0), axis=1)[:, None, :]

        group_up_buy_tradeamt = st2groupst(up_buy_tradeamt, daily_group, cross_sum)
        group_up_buy_tradevol = st2groupst(up_buy_tradevol, daily_group, cross_sum)
        group_down_sell_tradeamt = st2groupst(down_sell_tradeamt, daily_group, cross_sum)
        group_down_sell_tradevol = st2groupst(down_sell_tradevol, daily_group, cross_sum)
...
if __name__ == '__main__':
    ...
    f = UpBuytradeVwap2DownSellTradeVwapProfit()
    e = time.time()
    # f.result()
    f.result()
