...
# @Author : Zhichen Lu
# @File :ActiveBuyAmtRatioLogLossSharpe.py
...
class ActiveBuyAmtRatioLogLossSharpe(crossFactor):
    cross_group = 'sw1'
    cross_func = 'cross_sum'
    extend_days = 1
    author = 'lzc'
    logic = '日内主动买入成交额占比的交叉熵指数截面sharpe'
    article = ''
    freq = 'daily'
    ...
    def cal_groupst(self):
        ...
        H = np.nansum(pct * np.log(pct), axis=1)[:, None, :]
        factor = (H - st2groupst(H, daily_factor, cross_mean)) / st2groupst(H, daily_factor, cross_std)
