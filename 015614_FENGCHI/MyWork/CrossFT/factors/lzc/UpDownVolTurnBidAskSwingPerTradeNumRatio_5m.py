...
# @Author : Zhichen Lu
# @File : UpDownVolTurnBidAskSwingPerTradeNumRatio_5m.py
...
class UpDownVolTurnBidAskSwingPerTradeNumRatio_5m(crossFactor):
    window = 15
    cross_group = 'sw1'
    cross_func = '自定义'
    extend_days = 15
    author = 'lzc'
    logic = '缩量bar买单均价和买单均价价差总和除与成交笔数 个股/分组 前日T时刻到当日T时刻为一日'
    article = ''
    freq = '5mins'
    ...
    def cal_groupst(self):
        ...
        rolling_sum_diff = intrad_past_day_rolling_sum(np.where(amt_pct_change < 0, diff, np.nan))
        rolling_sum_trade_num = intrad_past_day_rolling_sum(np.where(amt_pct_change < 0, trade_num, 0))
        stk_factor_down = rolling_sum_diff / rolling_sum_trade_num
        group_factor_down = st2groupst(rolling_sum_diff, minute_factor, cross_sum) / st2groupst(rolling_sum_trade_num, minute_factor, cross_sum)

        rolling_sum_diff = intrad_past_day_rolling_sum(np.where(amt_pct_change > 0, diff, np.nan))
        rolling_sum_trade_num = intrad_past_day_rolling_sum(np.where(amt_pct_change > 0, trade_num, 0))
        stk_factor_up = rolling_sum_diff / rolling_sum_trade_num
        group_factor_up = st2groupst(rolling_sum_diff, minute_factor, cross_sum) / st2groupst(rolling_sum_trade_num, minute_factor, cross_sum)

        return arr_match_index((stk_factor_up * group_factor_up) / (stk_factor_down * group_factor_down), self.cal_date_range, self.date_range)
