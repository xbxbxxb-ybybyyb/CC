...
# @Author : Zhichen Lu
# @File : UpBuyTradeDownSellTradeRatio_minute.py
import sys
...
class UpBuyTradeDownSellTradeRatio_minute(crossFactor):
    window = 15
    cross_group = 'sw1'
    cross_func = 'cross_mean'
    extend_days = 15
    author = 'lzc'
    logic = '同向主动成交额占比(上行主动买入、下行主动卖出) * 分组的该值'
    article = ''
    freq = '5mins'
...
if __name__ == '__main__':
    ...
    f = UpBuyTradeDownSellTradeRatio_minute()
    e = time.time()
    # f.result()
    f.result()
