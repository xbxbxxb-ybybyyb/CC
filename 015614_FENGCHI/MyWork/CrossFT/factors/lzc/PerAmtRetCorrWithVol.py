...
class PerAmtRetCorrWithVol(crossFactor):
    cross_group = 'sw1'
    cross_func = ''
    extend_days = 0
    author = 'lzc'
    logic = '开盘2小时每一块成交额推动的收益与成交量的相关系数'
    article = ''
    freq = 'daily'
    basic_datas = {'daily': ['free_float_shares'], '30mins': [], '1min': [], '5mins': ['amt', 'close', 'vol']}
    ...
    def st_factor(self):
        ...
        return [self.database['5mins'][x] for x in ['amt', 'close', 'vol']]

    def cal_groupst(self):
        '''
        :return: np.array,个股值==》组值==》个股值 ，没有行业值在和个股值进行某些计算
        '''
        amt, close, vol = self.st_factor()
