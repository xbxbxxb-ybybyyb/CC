...
class UpStdToDownStd_5m(crossFactor):
    ...
    extend_days = 15
    author = 'lzc'
    logic = '个股上行波动率/下行波动率   *  分组收益均值上行波动率/下行波动率 前一日T时刻到当前交易日为一日'
    article = ''
    freq = '5mins'
    ...
    def cal_groupst(self):
        ...
        up_std = intrad_past_day_rolling_std(np.where(ret > 0, ret, np.nan))
        down_std = intrad_past_day_rolling_std(np.where(ret < 0, ret, np.nan))
