...
class activebuystrength_5m(crossFactor):
    cross_group = 'sw1'
    cross_func = 'cross_mean'
    extend_days = 1
    author = 'wyl'
    logic = '''日内主买强度 = mean（主动买入金额（成交量））/std（主动买入金额（成交量））'''
    article = '20200430-海通证券-行业轮动系列研究21：主动买入因子的行业有效性分析.pdf'
    freq = '5mins'
    basic_datas = {'1min': ['activebuyorderamt']}

    def st_factor(self):
        net = self.database['1min']['activebuyorderamt']
...
if __name__ == '__main__':
    ...
    val0 = np.load('/arch1/group/800442/800319/AAcross/factor_result/5mins/20140701_20210531/wyl/activebuystrength_5m.npy')
    val1 = cal_factor(cross_group = 'sw1',cross_func = 'cross_mean',wyl='wyl')
