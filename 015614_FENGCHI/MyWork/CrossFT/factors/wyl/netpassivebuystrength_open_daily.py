...
class netpassivebuystrength_open_daily(crossFactor):
    cross_group = 'sw1'
    cross_func = 'cross_mean'
    extend_days = 1
    author = 'wyl'
    logic = '''选取开盘时段(9:30-9:59)，净主买强度 = mean（被动买入金额（成交量）-被动卖出金额（成交量））/std（被动买入金额（成交量）-被动买入金额（成交量））'''
    article = '20200430-海通证券-行业轮动系列研究21：被动买入因子的行业有效性分析.pdf'
    freq = 'daily'
    basic_datas = {'1min': ['passivebuyorderamt','passivesellorderamt']}

    def st_factor(self):
        buy, sell = self.database['1min']['passivebuyorderamt'], self.database['1min']['passivesellorderamt']
        net = buy - sell
        strength = dt_mean(net,242)/dt_std(net,242)
        strength_open = np.nanmean(strength[:,1:31],axis=1,keepdims=True)
