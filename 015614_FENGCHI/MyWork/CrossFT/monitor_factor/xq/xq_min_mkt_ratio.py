...
from basic.operators import dt_pct
...
class xq_min_mkt_ratio(crossFactor):
    ...
    basic_datas = {'1min': ['close_badj']}
    def st_factor(self):
        '''
        :return: 1. np.array ,返回个股计算组值需要的个股因子
                2.  list(np.array)， 返回多个个股因子，有些为了计算组值，有些为了计算
        '''
        close = self.database['1min']['close_badj']#get_minute_1factor('close_badj', self.cal_date_range[0], self.cal_date_range[-1])
        pct = dt_pct(close,1)#.pct_change(1)
