...
class ConceptLimitTwiceWeight(crossFactor):
    def st_factor(self):
        '''
        ...
        IfLimit_min = pd.DataFrame(np.array(IfLimit.loc[stock_close.index.get_level_values('date')]),
                                   index=stock_close.index, columns=stock_close.columns)
...
if __name__=='__main__':
    f = ConceptLimitTwiceWeight(group='sw1', func='cross_sum',extend_days=5, author='tx', factor_name='ConceptLimitTwiceWeight',
                        logic='板块日内连板数量占比', freq='1min')
    f.save_result()
