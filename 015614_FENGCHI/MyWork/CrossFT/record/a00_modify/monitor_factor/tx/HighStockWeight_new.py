...
class HighStockWeight(crossFactor):
    def st_factor(self):
        '''
        ...
        HighStock = (Limit_stock.rolling(3).sum()==3) & (stock_pool==True)
        HighStock = HighStock.sum(axis=1)/stock_pool.sum(axis=1)
...
if __name__=='__main__':
    f = HighStockWeight(group='sw1', func='cross_sum',author='tx',extend_days=10,
                              factor_name='HighStockWeight',logic='3板及以上个股占比',freq='daily')
    f.save_result()
