...
class NewStockDailyLimitUpDown(crossFactor):
    def st_factor(self):
        '''
        ...
        limit_down = get_daily_1factor('limit_min', self.cal_date_range, self.code_list)
        LimitDown_stock = (close == limit_down)

        NewStockPct = Limit_stock[NewStock].sum(axis=1)/(LimitDown_stock[NewStock].sum(axis=1)+1)
...
if __name__=='__main__':
    f = NewStockDailyLimitUpDown(group='sw1', func=None,author='tx',extend_days=10,
                              factor_name='NewStockDailyLimitUpDown',logic='次新股涨跌停比',freq='daily')
    f.save_result()
