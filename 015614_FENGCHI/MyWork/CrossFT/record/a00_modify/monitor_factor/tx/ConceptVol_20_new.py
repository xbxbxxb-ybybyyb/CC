...
class ConceptVol_20(crossFactor):
    def st_factor(self):
        '''
    ...
    def result(self):
        ...
        factor = factor.rolling(10).mean()/factor.rolling(20).mean()
        factor = df_match_index_col(factor, self.code_list, self.cal_date_range)  # np.array

        return arr_match_index(factor,self.cal_date_range,self.date_range)

if __name__=='__main__':
    f = ConceptVol_20(group='sw1', func='cross_sum',extend_days=20,author='tx',factor_name='ConceptVol_20',logic='板块行业量比10/20',freq='daily')
    f.save_result()
