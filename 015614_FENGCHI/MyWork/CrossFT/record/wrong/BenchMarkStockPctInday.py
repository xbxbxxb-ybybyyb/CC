...
class BenchMarkStockPctInday(crossFactor):
    cross_group = 'sw1'
    ...
    def st_factor(self):
        ...
        NewStockPct = stock_pct[StockChoice_min].mean(axis=1)
        NewStockPct = np.array(NewStockPct)[:,np.newaxis].repeat(len(self.code_list), axis=1)
        result = pd.DataFrame(NewStockPct,index=stock_pct.index,columns=self.code_list)
        result = df_match_index_col(result, self.code_list, self.cal_date_range, '1min')  # np.array
