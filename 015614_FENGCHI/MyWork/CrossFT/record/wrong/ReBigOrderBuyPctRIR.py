...
class ReBigOrderBuyPctRIR(crossFactor):
    cross_group='sw1'
    cross_func='cross_mean'
    extend_days=6
    author='hx'
    logic='大单买入占比 行业排序与个股排序和'
    article='研究报告：海通证券-选股因子系列研究（七十二）：大单的精细化处理与大单因子重构-210120'
    freq='daily'
    basic_datas = {'1min': ['turn_order_active_buy']}

    def st_factor(self):
        buy = self.database['1min']['turn_order_active_buy']
        lb = np.log(1000 * buy)
        limit = dt_mean(lb, 1440) + dt_std(lb, 1440)
        factor = np.nansum(np.where(lb > limit, buy, 0), axis=1, keepdims=True)
        return factor
