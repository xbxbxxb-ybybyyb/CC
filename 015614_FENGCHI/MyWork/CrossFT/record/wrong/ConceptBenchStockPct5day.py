...
class ConceptBenchStockPct5day(crossFactor):
    cross_group='sw1'
    ...
    def st_factor(self):
        ...
        # 输出市场标杆股
        # 1、均线排列纯多头
        close_badj = get_daily_1factor('close_badj',self.date_range,code_list=self.code_list)
        ...
        # 3、涨停数量
        limit_max = get_daily_1factor('limit_max',date_list=self.date_range,code_list=self.code_list)
        close = get_daily_1factor('close',self.date_range,code_list=self.code_list)
        ...
        NewStockPct = stock_pct[StockChoice]
        NewStockPct = df_match_index_col(NewStockPct, self.code_list, self.cal_date_range)  # np.array
        return NewStockPct
