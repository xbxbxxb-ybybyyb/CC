...
class HighStockLimitDown(crossFactor):
    ...
    def st_factor(self):
        ...
        stock_pool = clean_stock_list(no_ST=True, least_live_days=10, no_pause=True, least_recover_days=0).loc[self.cal_start:self.end]
        ...
        HighStockPct = (stock_limit[self.code_list] & HighStock_min[self.code_list]).sum(axis=1)
        HighStockPct = np.array(HighStockPct)[:, np.newaxis].repeat(len(self.code_list), axis=1)

        return HighStockPct.reshape((len(self.cal_date_range),-1,len(self.code_list)))
...
if __name__=='__main__':
    val1 = np.load('/arch1/group/800442/800319/AAcross/factor_result/1min/20140701_20210531/tx/HighStockLimitDown.npy')
    val2 = cal_factor(numd={'1min': 20})
