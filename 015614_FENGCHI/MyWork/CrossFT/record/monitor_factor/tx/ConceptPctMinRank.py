...
class ConceptPctMinRank(crossFactor):
    def st_factor(self):
        '''
    ...
    def result(self):
        ...
        factor = np.argsort(np.argsort(factor_before))/factor_before.shape[-1]

        return factor

if __name__=='__main__':
    f = ConceptPctMinRank(group='sw1', func='cross_mean',extend_days=5, author='tx', factor_name='ConceptPctMinRank',
                        logic='板块日内涨跌幅排序', freq='1min')
    f.save_result()
