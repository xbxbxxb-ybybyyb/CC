...
class ConceptVol_1(crossFactor):
    def st_factor(self):
        '''
    ...
    def result(self):
        ...
        factor = factor/factor.rolling(5).mean()
        factor = df_match_index_col(factor, self.code_list, self.cal_date_range)  # np.array

        return arr_match_index(factor,self.cal_date_range,self.date_range)

if __name__=='__main__':
    f = ConceptVol_1(group='sw1', func='cross_sum',author='tx',extend_days=10,factor_name='ConceptVol_1',logic='板块行业量比1/5',freq='daily')
    f.save_result()
