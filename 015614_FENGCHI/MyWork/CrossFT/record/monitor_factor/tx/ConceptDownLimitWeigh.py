...
class ConceptDownLimitWeigh(crossFactor):
    def st_factor(self):
        '''
        ...
        limit_max = get_daily_1factor('limit_min',self.cal_date_range,code_list=self.code_list)
        ...
        limit_min_num = df_match_index_col(limit_min_num, self.code_list, self.cal_date_range, '1min')  # np.array

        stock_num = (stock_close>0)
        stock_num = df_match_index_col(stock_num, self.code_list, self.cal_date_range, '1min')  # np.array

        return limit_min_num,stock_num
...
if __name__=='__main__':
    f = ConceptDownLimitWeigh(group='sw1', func='cross_sum',extend_days=5, author='tx', factor_name='ConceptDownLimitWeigh',
                        logic='板块日内跌停占比', freq='1min')
    f.save_result()
