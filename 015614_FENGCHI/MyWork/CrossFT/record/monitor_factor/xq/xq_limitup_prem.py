...
class xq_limitup_prem(crossFactor):
    def st_factor(self):
        '''
    ...
    def cal_groupst(self):
        ...
        '''
        zt, pct = self.st_factor()
        zt_prem = np.where(zt == 1, pct, np.nan)
        self.group = sameshape(zt, self.group_factor())
        factor = st2groupst(zt_prem, self.group, self.group_func())
...
if __name__ == '__main__':
    ...
    f = xq_limitup_prem(group, func, 40, 20170101, 20210531, 'xq', 'xq_limitup_prem', '涨停溢价',
