...
class UpDownAmtSkewRatio_5m(crossFactor):
    cross_group = 'sw1'
    cross_func = ''
    extend_days = 5
    author = 'lzc'
    logic = '日内成交额按自由流通市值处理后 计算日内上行偏度和下行偏度之比 并计算 分组比例 前日T时刻到当日T时刻为一日'
    article = ''
    freq = '5mins'
    ...
    def cal_groupst(self):
        ...
        up_stk_skew, up_group_skew = get_group_std_and_stk_std(up_amt)
        down_stk_skew, down_group_skew = get_group_std_and_stk_std(down_amt)
