...
class UpStdToDownStd_5m(crossFactor):
    window = 15
    cross_group = 'sw1'
    cross_func = 'cross_sum'
    extend_days = 15
    author = 'lzc'
    logic = '个股上行波动率/下行波动率   *  分组收益均值上行波动率/下行波动率 前日T时刻到该日T时刻为1日'
    article = ''
    freq = '5mins'
    ...
    def cal_groupst(self):
        ...
        up_std = intrad_past_day_rolling_std(np.where(ret > 0, ret, np.nan))  # [:, None, :]
        down_std = intrad_past_day_rolling_std(np.where(ret < 0, ret, np.nan))  # [:, None, :]

        group_mean_ret = st2groupst(ret, minute_group, cross_mean)

        group_up_std = intrad_past_day_rolling_std(np.where(group_mean_ret > 0, group_mean_ret, np.nan))
        group_down_std = intrad_past_day_rolling_std(np.where(group_mean_ret < 0, group_mean_ret, np.nan))

        return arr_match_index((up_std * group_up_std) / (group_down_std * down_std), self.cal_date_range, self.date_range)
