...
# @Author : Zhichen Lu
# @File :HighFreqTurnoverRetCorr_5m.py
...
class HighFreqTurnoverRetCorr_5m(crossFactor):
    cross_group = 'sw1'
    cross_func = 'cross_sum'
    extend_days = 2
    author = 'lzc'
    logic = '日内换手率和收益的相关性 * 分组面板相关性  按昨日T时刻到今日T时刻为一日计算'
    article = ''
    freq = '5mins'
    ...
    def cal_groupst(self):
        ...
        free_float_cap = delay(self.database['daily']['free_float_shares'], 1) * self.database['daily']['close']
        ...
        SUM_X = intrad_past_day_rolling_sum(X)
        SUM_Y = intrad_past_day_rolling_sum(y)
        SUM_XY = intrad_past_day_rolling_sum(Xy)
        SUM_X2 = intrad_past_day_rolling_sum(X2)
        SUM_Y2 = intrad_past_day_rolling_sum(y2)
        SUM_COUNT = intrad_past_day_rolling_sum(count)
