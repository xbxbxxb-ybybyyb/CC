...
# @Author : Zhichen Lu
# @File :Vol5CorrWithNetPrice_5m.py
...
class Vol5CorrWithNetPrice_5m(crossFactor):
    cross_group = 'sw1'
    cross_func = 'cross_mean'
    extend_days = 0
    author = 'lzc'
    logic = '日内5分钟波动率与收盘净值组内相关性 * 个股波动率 前日T时刻到该日T时刻为1日'
    article = ''
    freq = '5mins'
    basic_datas = {'daily': ['free_float_shares'], '30mins': [], '5mins': ['close'], '1min': []}
    ...
    def cal_groupst(self):
        ...
        volatility = intrad_past_day_rolling_std(ret)
        net_close = close / delay(close, 1)

        EX = st2groupst(volatility, self.group, cross_mean)
        EY = st2groupst(net_close, self.group, cross_mean)
        EX2 = st2groupst(volatility ** 2, self.group, cross_mean)
        EY2 = st2groupst(net_close ** 2, self.group, cross_mean)
        EXY = st2groupst(net_close * volatility, self.group, cross_mean)
