...
# @Author : Zhichen Lu
# @File :ThirdMomentAcross5DayVolatility_5m.py
...
class ThirdMomentAcross5DayVolatility_5m(crossFactor):
    cross_group = 'sw1'
    cross_func = 'cross_mean'
    extend_days = 6
    author = 'lzc'
    logic = '日内累计收益率三阶矩剔除5日波动率  行业面板三阶矩剔除行业5日面板波动率 前日T时刻到该日T时刻为一日 '
    article = ''
    freq = '5mins'
    ...
    def cal_groupst(self):
        ...
        X3 = stk_cumret ** 3
        NX = intrad_past_day_rolling_sum(stk_cumret_finite, )
        SUMX3 = intrad_past_day_rolling_sum(X3, stk_cumret_finite)
        stk_3m = SUMX3 / NX
        group_3m = st2groupst(SUMX3, daily_minute, cross_sum) / st2groupst(NX, daily_minute, cross_sum)
