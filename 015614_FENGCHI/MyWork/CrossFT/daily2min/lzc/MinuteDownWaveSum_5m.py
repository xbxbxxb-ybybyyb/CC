...
class MinuteDownWaveSum_5m(crossFactor):
    window = 15
    cross_group = 'sw1'
    cross_func = 'cross_sum'
    extend_days = 15
    author = 'lzc'
    logic = '5分钟bar上超过或两倍标准差的振幅之和/小于-2被标准差的振幅之和 * 组的该值 前日T时刻到当日T时刻为1天'
    ...
    def cal_groupst(self):
        ...
        lower_neg2_std_wavecount = intrad_past_day_rolling_sum(z_close, z_close < -2) * -1

        group_neg2_std_wavecount = st2groupst(lower_neg2_std_wavecount, daily_group, cross_mean)

        return arr_match_index(group_neg2_std_wavecount * lower_neg2_std_wavecount, self.cal_date_range, self.date_range)
...
if __name__ == '__main__':
    ...
    try:
        ...
        send_message(['015664'], str(traceback.format_exc()))
