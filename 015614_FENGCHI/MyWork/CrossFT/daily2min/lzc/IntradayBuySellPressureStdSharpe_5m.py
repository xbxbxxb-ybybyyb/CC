...
# @Author : Zhichen Lu
# @File : IntradayBuySellPressureStdSharpe_5m.py
import sys
...
class IntradayBuySellPressureStdSharpe_5m(crossFactor):
    window = 15
    cross_group = 'sw1'
    cross_func = ''
    extend_days = 15
    author = 'lzc'
    logic = '上行主买成交额、下行主卖成交额买卖压截面sharpe 按昨日 T时刻到今日T时刻为一日计算'
    article = ''
    freq = '5mins'
    ...
    def cal_groupst(self):
        ...
        intra_std = intrad_past_day_rolling_std(buytradeamt / selltradeamt)
        factor = st2groupst(intra_std, minute_group, cross_mean) / st2groupst(intra_std, minute_group, cross_std)
