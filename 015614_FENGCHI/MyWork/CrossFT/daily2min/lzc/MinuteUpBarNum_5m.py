...
class MinuteUpBarNum_5m(crossFactor):
    window = 15
    cross_group = 'sw1'
    cross_func = 'cross_sum'
    extend_days = 15
    author = 'lzc'
    logic = '5分钟bar上超过或两倍标准差的bar的数量 * 分组内所有股票超过两倍标准差bar的数量占比的均值 前日T时刻到当日T时刻为1天'
    ...
    def cal_groupst(self):
        ...
        over_2_std_count = intrad_past_day_rolling_sum(z_close, z_close > 2) * -1
        over_2_std_ratio = over_2_std_count / self.database['5mins']['close'].shape[1]
        group_2_std_ratio = st2groupst(over_2_std_ratio, daily_group, cross_mean)

        return arr_match_index(group_2_std_ratio * over_2_std_count, self.cal_date_range, self.date_range)
...
if __name__ == '__main__':
    ...
    try:
        cal_factor()
        send_message(['015664'], 'OK')
    except:
        send_message(['015664'], 'not OK')
