...
# @Author : Zhichen Lu
# @File : UpBuytradeAmt2DownSellTradeAmtPressure_5m.py
import sys
...
class UpBuytradeAmt2DownSellTradeAmtPressure_5m(crossFactor):
    window = 15
    cross_group = 'sw1'
    cross_func = ''
    extend_days = 15
    author = 'lzc'
    logic = '上行主买成交额、下行主卖成交额买卖压截面sharpe 昨日T时刻到当日T时刻为一日'
    article = ''
    freq = '5mins'
    ...
    def cal_groupst(self):
        ...
        up_buy_trade_num = intrad_past_day_rolling_sum(np.where(ret > 0, buytradeamt, 0))
        down_sell_trade_num = intrad_past_day_rolling_sum(np.where(ret < 0, selltradeamt, 0))
        ratio = up_buy_trade_num / down_sell_trade_num
        factor = st2groupst(ratio, minute_group, cross_mean) / st2groupst(ratio, minute_group, cross_std)
