...
# @Author : Zhichen Lu
# @File : BuyVwapSellVwapRetCrossAmtWeightStd_5m.py
import sys
...
class BuyVwapSellVwapRetCrossAmtWeightStd_5m(crossFactor):
    window = 15
    cross_group = 'sw1'
    cross_func = ''
    extend_days = 15
    author = 'lzc'
    logic = '卖单VWAP对买单VWAP收益率在分组内用成交额面板上加权获得分组分钟面板加权收益 * 个股日内加权收益  按昨日 T时刻到今日T时刻为一日计算'
    article = ''
    freq = '5mins'
    ...
    def cal_groupst(self):
        ...
        amt_finit = np.isfinite(total_amt)
        both_finit = ret_finit & amt_finit
        ret = np.where(both_finit, ret, 0)
        total_amt = np.where(both_finit, total_amt, 0)
        adj_ret = ret * total_amt

        daily_adj_sum_ret = intrad_past_day_rolling_sum(adj_ret)
        amt_sum = intrad_past_day_rolling_sum(total_amt)

        daily_groupamt_sum = st2groupst(amt_sum, minute_group, cross_sum)
        daily_group_adj_sum_ret = st2groupst(daily_adj_sum_ret, minute_group, cross_sum)
