...
class UpBuytradeVwap2DownSellTradeVwapProfit_5m(crossFactor):
    ...
    extend_days = 15
    author = 'lzc'
    logic = '截面上分组内上行日内主动买入均价对截面上下行主动卖出均价的收益率 与 个股该值复利 前日T时刻到当日T时刻为一日'
    article = ''
    freq = '5mins'
    ...
    def cal_groupst(self):
        ...
        up_buy_tradeamt = intrad_past_day_rolling_sum(np.where(ret > 0, buytradeamt, 0))  # [:, None, :]
        up_buy_tradevol = intrad_past_day_rolling_sum(np.where(ret > 0, buytradevol, 0))
        down_sell_tradeamt = intrad_past_day_rolling_sum(np.where(ret < 0, selltradeamt, 0))
        down_sell_tradevol = intrad_past_day_rolling_sum(np.where(ret < 0, selltradevol, 0))
