...
class DownStdToStd_5m(crossFactor):
    window = 15
    cross_group = 'sw1'
    cross_func = 'cross_sum'
    extend_days = 15
    author = 'lzc'
    logic = '个股下行波动率/波动率   *  分组收益均值下行波动率/波动率 按昨日 T时刻到今日T时刻为一日计算'
    article = ''
    freq = '5mins'
    ...
    def cal_groupst(self):
        ...
        std = intrad_past_day_rolling_std(ret, ret_finit)  # [:, None, :]
        up_std = intrad_past_day_rolling_std(np.where(ret_down, ret, np.nan))  # [:, None, :]

        group_mean_ret = st2groupst(ret, minute_group, cross_mean)

        group_std = intrad_past_day_rolling_std(group_mean_ret)  # [:, None, :]
        group_up_std = intrad_past_day_rolling_std(np.where(group_mean_ret < 0, group_mean_ret, np.nan))  #[:, None, :]
