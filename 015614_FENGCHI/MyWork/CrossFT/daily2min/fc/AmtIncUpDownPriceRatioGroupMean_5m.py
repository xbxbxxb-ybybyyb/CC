...
class AmtIncUpDownPriceRatioGroupMean_5m(crossFactor):
    cross_group = 'sw1'
    ...
    article = ''
    basic_datas = {'daily': [], '30mins': [], '5mins': ['amt', 'close_badj']}
    ...
    def st_factor(self):
        ...
        ratio = np.divide(ts_cummean(pct1), ts_cummean(pct2),
                          where=abs(ts_cummean(pct2)) > 1e-8)
        return ratio
...
if __name__ == '__main__':
    # f = AmtIncUpDownPriceRatioGroupMean_5m(start=20210401, end=20210501)
    # print(f.result())
