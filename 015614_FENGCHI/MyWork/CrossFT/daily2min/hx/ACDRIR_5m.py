...
class ACDRIR_5m(crossFactor):
    cross_group='sw1'
    cross_func='cross_mean'
    extend_days=21
    author='hx'
    logic='ACD收集派发指标的个股排序与行业排序之和'
    ...
    def st_factor(self):
        ...
        acd = close - np.where(dt_delta(close, 1) > 0, min2(low, dt_delay(close, 1)), max2(high, dt_delay(close, 1)))
        acd = div2(acd, dt_delay(close, 1))
        acd = dt_mean(acd, 20)
        return acd
