...
class CRRIR_5m(crossFactor):
    cross_group='sw1'
    cross_func='cross_mean'
    extend_days=20
    author='hx'
    logic='CR能量指标 个股排序与行业排序之和'
    ...
    def st_factor(self):
        ...
        cr = div2(dt_mean(neg(relu(sub2(dt_delay(mid, 1), high))), 20),
                  dt_mean(neg(relu(sub2(low, dt_delay(mid, 1)))), 20))
        # cr = div2((neg(relu(sub2(dt_delay(mid, 1), high)))), (neg(relu(sub2(low, dt_delay(mid, 1))))))
        return cr
