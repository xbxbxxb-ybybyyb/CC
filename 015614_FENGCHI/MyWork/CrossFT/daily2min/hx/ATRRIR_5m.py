...
class ATRRIR_5m(crossFactor):
    cross_group='sw1'
    cross_func='cross_mean'
    extend_days=12
    author='hx'
    logic='ATR平均真实波幅 个股排序与行业排序和'
    article='广发证券-多因子Alpha系列报告之（四十二）：海量技术指标掘金Alpha因子-210730'
    freq='5mins'
    basic_datas = {'5mins': ['high_badj', 'low_badj', 'close_badj']}

    def st_factor(self):
        high = self.database['5mins']['high_badj']
        low = self.database['5mins']['low_badj']
        close = self.database['5mins']['close_badj']
        atr = max2(max2(high - low, abss(dt_delay(close, 1) - high)), abss(dt_delay(close, 1) - low))
        atr = dt_mean(atr, 12)
        return atr
