...
def interday_condition():
    ...
    pct_5d = close_badj.pct_change(5)
    pct_10d = close_badj.pct_change(10)
    zt = get_daily_factor('limit_up')
    zt_5d = zt.rolling(5).sum()

    f = (~((ma5 < ma20) & (ma20 < ma60))) & (vol_ratio > 0.8) & (zt_5d == 0) & (pct_5d < 0.1) & (pct_10d < 0.2)
...
high = get_daily_factor('high')
high_10d = high.rolling(20).max()


def concept_arbitrary(dragon_stk, stock_list, concept_name):
    ...
    limitmax = np.tile(np.array(limitmax_day.loc[limitmax_day.index, close.columns]), len(close)).reshape(close.shape[0], close.shape[1])
    zt = (cumhigh.shift(1) < limitmax) & (cumhigh == limitmax)

    buy_list = []
    # 1、判断板块条件: 板块实时涨跌幅>0.01,板块内无跌停股票，涨停股票数量<=2
    if concept_pct[concept_name].tolist()[-1] >= 0.01 and concept_min_num[concept_name].tolist()[-1] == 0 and concept_max_num[concept_name].tolist()[-1] <= 2:
        # 2、判断板块龙头股是否涨停
        if (close.loc[close.index[-1], dragon_stk].values == limitmax_day.loc[today, dragon_stk].values).sum() > 0:
            for stk in stock_list:
                # 3、判断是否满足日间条件
                if f.loc[str(pre_date), stk] == 0:
                    pass
                # 4、 判断是否平开且当前涨幅>2%
                elif abs(close[stk].tolist()[0]/preclose_day.loc[today, stk]-1) <= 0.03 and \
                        close[stk].tolist()[-1] /preclose_day.loc[today, stk] - 1 > 0.04 if int(stk[:6])//1000 == 300 else 0.02:
                    # 5、 判断是否突破日内高点
                    if cumhigh[stk].tolist()[-1] > high_10d.loc[str(pre_date), stk]:
                        # 6、判断是否满足买入条件
                        if (pct_1m[stk].tolist()[-1] >= 0.005 or pct_2m[stk].tolist()[-1] >= 0.01) and vol_ratio[stk].tolist()[-1] >= 1.5 \
                                and close[stk].tolist()[-1] > avgprice[stk].tolist()[-1] and (zt[dragon_stk].sum(axis=1).iloc[-10:] > 0).sum() > 0:
                            buy_list.append(stk)
...
while time.localtime()[3] < 15:
    ...
    concept_pct = get_concept_value('Pct_Change', concept_list)
    ...
    for con in concept_list:
        ...
        concept_arbitrary(dragon_stk, stock_list, concept_name)
