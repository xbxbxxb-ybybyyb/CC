...
def get_data_fromopen(date,concept_name,save_path='/arch1/group/800442/800319/RealTime_Data/'):
    ma = MarketData()
    #1、先获取最新时间段的文件名称
...
# 1、获取一个板块的指定因子数据
def get_oneconcept_alldata(concept_name,factor_list=None,read_path='/arch1/group/800442/800319/RealTime_Data/'):
...
# 2、获取多个板块的一个因子
def get_allconcept_onedata(factor,concept_list=None,read_path='/arch1/group/800442/800319/RealTime_Data/'):
...
def get_concept_value(factor,concept_list=None,read_path='/arch1/group/800442/800319/RealTime_Data/'):
    date = datetime.datetime.now().strftime('%Y%m%d')  # 获取今天日期
    ...
    # (4)板块非一字涨停数量
    elif factor == 'MaxNumFromOpen':
        close = get_allconcept_onedata('ClosePx', concept_list=concept_list, read_path=read_path)
        low = get_allconcept_onedata('LowPx', concept_list=concept_list, read_path=read_path)
        concept_result = pd.DataFrame(columns=list(close.keys()))

        all_data = all_data['max_price'].loc[date]
        for concept in concept_result.columns:
            calc_df = (close[concept].fillna(method='ffill') == all_data.loc[close[concept].columns]) & \
                      (low[concept].fillna(method='ffill').expanding().min() < all_data.loc[close[concept].columns])
            concept_result[concept] = calc_df.sum(axis=1)

        return concept_result

    # (5)板块非一字跌停数量
    elif factor == 'MinNumFromOpen':
        close = get_allconcept_onedata('ClosePx', concept_list=concept_list, read_path=read_path)
        high = get_allconcept_onedata('LowPx', concept_list=concept_list, read_path=read_path)
        concept_result = pd.DataFrame(columns=list(close.keys()))

        all_data = all_data['min_price'].loc[date]
        for concept in concept_result.columns:
            calc_df = (close[concept].fillna(method='ffill') == all_data.loc[close[concept].columns]) & \
                      (high[concept].fillna(method='ffill').expanding().max() > all_data.loc[close[concept].columns])
            concept_result[concept] = calc_df.sum(axis=1)
