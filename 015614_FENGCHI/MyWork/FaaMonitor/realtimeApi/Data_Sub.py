...
#security_list 为单一股票池
#save_name 为文件保存名称
#save_path 为文件保存路径
# 因子回调计算函数
def GetRealTimeData(security_list,factor_list,save_name,save_path):
    ...
    def calc_callback(self, df_dict,save_name=save_name):
        # 可选步骤：变更标的
        ...
        if int(now_time)<=150000:
            with open(save_path + save_name + now_time + '.pkl', 'wb') as f:
