...
class TickFactor_AmtPerTradeRatioDiff5trans(BaseFactor):
    #  定义因子参数
    ...
    def reform(self, temp_result):
        lag = self.reform_window - 1
        alpha_temp = temp_result - temp_result.rolling(lag).mean().shift(1)
        med = alpha_temp.median(axis=1)
        alpha = abs(alpha_temp.T.sub(med.values).T)
        return alpha
