...
class DivergenceStdRaw_5_15(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        mvolume_rpl = mvolume.resample(str(m) + 'T').sum().dropna(how='all', axis=0)  # 取出m分钟volume
        ...
        divergence = np.divide(np.multiply(divergence, mvolume_rpl), mvolume_rpl)  # 清除成交量为0的
        divergence = divergence[-237*5//m:, :]  # 取出n天

        alpha = self.agg(divergence, 'std')
