...
class MinEMVANorm(BaseFactor):    # 派生一个因子类
    factor_type = 'DAY'         
    # 声明因子类型为DAY
    ...
    def reform(self, temp_result):
        ans_df_mean = temp_result.rolling(self.reform_window,min_periods=10).mean()
        ans_df_std = temp_result.rolling(self.reform_window,min_periods=10).std()
        ans_df = ans_df_mean / ans_df_std
