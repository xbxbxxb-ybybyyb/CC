...
class SmallPlayersVwap(BaseFactor):
    ...
    def calc_single(self, database):
        ...
        moneyflow['smallvwap'] = (moneyflow['BUY_VALUE_SMALL_ORDER']+moneyflow['SELL_VALUE_SMALL_ORDER'])/(moneyflow['BUY_VOLUME_SMALL_ORDER']+moneyflow['SELL_VOLUME_SMALL_ORDER'])
       
        vwap.index = pd.to_datetime(vwap.index)
        moneyflow_small = moneyflow['smallvwap'].unstack().reindex(index = vwap.index, columns=vwap.columns)
        
        moneyflow_small_vwapratio = moneyflow_small/vwap-1

        return moneyflow_small_vwapratio.iloc[-1][valid]
