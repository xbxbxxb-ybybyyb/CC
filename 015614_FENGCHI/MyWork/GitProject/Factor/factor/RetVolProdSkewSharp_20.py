...
class RetVolProdSkewSharp_20(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        resultDF = pd.DataFrame(np.nan, index=[pd.Timestamp(date) for date in date_list], columns=minute_close.columns)

        n = 24
        for date in date_list:
            closedf = minute_close.loc[date]
            volumedf = minute_volume.loc[date]

            closedf = closedf.resample('5T').last()
            returndf = closedf.pct_change(periods=1)
            volumedf = volumedf.resample('5T').sum()

            volumeAm = volumedf.iloc[:n].sum()
            vol_last = (volumedf[-n:] / volumeAm * returndf[-n:]).skew()
            if len(vol_last.dropna()) != 0:
                resultDF.loc[date] = vol_last
            else:
                resultDF.loc[date] = 0.0
        ans = resultDF.iloc[-1, :]
        return -ans
