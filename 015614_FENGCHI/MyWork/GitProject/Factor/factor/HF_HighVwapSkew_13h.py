...
class HF_HighVwapSkew_13h(BaseFactor):
    """
    * 因子名：HF_HighVwapSkew_13h
    * 因子功能描述：High与Vwap之差的负偏度
    * 因子参数：MinuteHigh,MinuteVolume,MinuteTurnover
    * 作者：游加平
    * 因子创建日期： 2019.9.20
    """
    factor_type = "FIX"
    # 依赖的平台原始数据，包括FactorData和MarketData接口中的数据。 默认为空，必须设置
    depend_data = ["FactorData.Basic_factor.high_minute", "FactorData.Basic_factor.volume_minute", "FactorData.Basic_factor.amt_minute"]
    # 计算每个时点的因子所需要前移的数据窗口大小
    # 例如，为日频因子，lag=3表示计算某一日的因子值需要依赖前三个交易日和当日的数据，默认为0，可不设置
    lag = 0
    reform_window = 5

    def definition(self,MinuteHigh,MinuteVolume,MinuteTurnover):
        factor = self.minute_help(self.minute,'MinuteValidHelp',MinuteHigh,MinuteVolume,MinuteTurnover)
    ...
    def calc_single(self, database):
        ...
        volume[volume.values==0.] = np.nan
        vwap = amt.cumsum() / volume.cumsum()
        skew = (high - vwap).skew()
        return -skew

    def reform(self, factor):
        factor = - factor / self.rolling_min(factor,window=self.reform_window)
        return factor
