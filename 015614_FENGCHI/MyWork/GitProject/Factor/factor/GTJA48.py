...
class GTJA48(BaseFactor):
    ...
    def calc_single(self, database):
        ...
        close_delta = np.sign(close_delta).iloc[-3:,:].sum(axis=0)
        ret = day_volume.iloc[-5:,:].sum(axis=0) / day_volume.iloc[-20:,:].sum(axis=0)

        ans_df = close_delta * ret
        return ans_df
