...
class MinDailyCorrCloseVol_min5(BaseFactor):
    #  定义因子参数

    # 因子频率，默认为日频因子， 可不设置
    factor_type = "FIX"
    # 依赖的平台原始数据，包括FactorData和MarketData接口中的数据。 默认为空，必须设置
    depend_data = ["FactorData.Basic_factor.close_minute","FactorData.Basic_factor.volume_minute","FactorData.Basic_factor.volume_by_share","FactorData.Basic_factor.adjfactor","FactorData.Basic_factor.close","FactorData.Basic_factor.limit_status_minute"]
    ...
    def calc_single(self, database):
        ...
        the_lstdt = [i for i in min_close.index if i.date() == min_close.index[-1].date()]
        min_close = min_close.loc[the_lstdt]
        ...
        ans_df = array_coef(day_volume, day_close)
