...
class dretvolnew_kurtmean_20_10_daily(BaseFactor):
    #  定义因子参数
    ...
    lag=9
    reform_window=1
    batch_info={'sub_window':20,'intraday_stat':'kurt','daily_stat':'mean'}
    ...
    def calc_single(self, database):
        ...
        # 日间局部信号计算函数，此处的灵活性最高，可进行自定义的特征刻画
        ...
        # 提取数据
        fordate = deepcopy(database.depend_data["FactorData.Basic_factor.close_minute"])
        min_close = deepcopy(database.depend_data["FactorData.Basic_factor.close_minute"]);min_close = min_forward_adj(min_close)
        min_ret = pd.DataFrame(min_close.values/min_close.shift(1).values-1,index = min_close.index,columns = min_close.columns)
        min_ret = min_ret.fillna(0)
        intraday_stat = self.batch_info['intraday_stat'] # 要计算的单日统计特征
        daily_stat = self.batch_info['daily_stat'] # 要计算的日间统计特征
        sub_window = self.batch_info['sub_window'] # 日间局部信息的窗口
