...
class FactorMin53_mean_div_std(BaseFactor):
    #  定义因子参数
    # 因子频率，。默认为日频因子， 可不设置
    factor_type = "FIX"
    ...
    def calc_single(self, database):
        ...
        iliq=abs(data1_r)*(10**6)/data2
         
        
        ##放量
        data2_r=(data2-data2.shift(1))/abs(data2.shift(1))   
        
        iliq_up=iliq[(data2_r>0)]
        
        iliq_down=iliq[(data2_r<0)]
...
# 针对播放后的结果，进行相关的rolling等操作。所用的前序数据长度应为reform_window。默认不修改temp_result， 可不重写。
    def reform(self, temp_result):
           factor_values_std=temp_result.rolling(self.reform_window).std()
           factor_values_std[factor_values_std==0]=np.nan
           temp_result=temp_result.rolling(self.reform_window).mean()/factor_values_std
           temp_result=temp_result.convert_objects()
           return temp_result
