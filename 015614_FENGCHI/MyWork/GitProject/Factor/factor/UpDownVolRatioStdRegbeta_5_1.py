...
class UpDownVolRatioStdRegbeta_5_1(BaseFactor):
    #  定义因子参数
    ...
    depend_data = ["FactorData.Basic_factor.limit_status_minute", "FactorData.Basic_factor.close_minute",
                   "FactorData.Basic_factor.open_minute", "FactorData.Basic_factor.volume_minute"]
    ...
    def calc_single(self, database):
        ...
        mvolume_rpl = np.sqrt(mvolume.resample(str(m) + 'T').sum(min_count=1).reindex(index=mclose_rpl.index.to_list()))

        mvolume_long = np.ones(mvolume_rpl.shape) * np.nan
        mvolume_long[mclose_rpl.values > mopen_rpl.values] = mvolume_rpl.values[mclose_rpl.values > mopen_rpl.values]  # 多头成交量
        mvolume_short = np.ones(mvolume_rpl.shape) * np.nan
        mvolume_short[mclose_rpl.values < mopen_rpl.values] = mvolume_rpl.values[mclose_rpl.values < mopen_rpl.values]  # 空头成交量
        mvolume_long = mvolume_long[-237 * n // m:, :]  # 取出n天
        mvolume_short = mvolume_short[-237 * n // m:, :]  # 取出n天

        if aggtype == 'std':
            alpha = np.nanstd(mvolume_long, axis=0) / np.nanstd(mvolume_short, axis=0)
        elif aggtype == 'skew':
            alpha = skew(mvolume_long, axis=0, nan_policy='omit').__array__() / skew(mvolume_short, axis=0, nan_policy='omit').__array__()
        elif aggtype == 'kurt':
            alpha = kurtosis(mvolume_long, axis=0, nan_policy='omit').__array__() / kurtosis(mvolume_short, axis=0, nan_policy='omit').__array__()
        else:
