...
class FactorMin236_std(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        def quantile_index_std(x,flag_dir):
            x=x.dropna()
            if len(x)>0:
               ##所有的index列表
               ...
               xtemp=np.nanstd(index_where)

               return xtemp
            
   
        data1_index=data1.apply(quantile_index_std,flag_dir=False)
