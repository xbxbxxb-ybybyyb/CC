...
class RollingSignDownWick(BaseFactor):
    #  定义因子参数
    ptype = 'mean'
    # 因子频率，默认为日频因子， 可不设置
    factor_type = "FIX"
    ...
    depend_data = ["FactorData.Basic_factor.low_minute", "FactorData.Basic_factor.close_minute", "FactorData.Basic_factor.limit_status_minute"]
    ...
    def calc_single(self, database):
        ...
        min_low = Util.data_filter(low_minute, limit_status, method="minute")
        min_close = Util.data_filter(close_minute, limit_status, method="minute")
        low = min_low.resample("5T", label="right").min().dropna(axis=0, how='all').iloc[
                       -48 * (self.minute_lag):, :] 
        close = min_close.resample("5T", label="right").last().dropna(axis=0, how='all').iloc[
                       -48 * (self.minute_lag) - 1:, :]     
        delta = (close - close.shift(1)).iloc[1:,:]
        ans_df = (close - low) * np.sign(delta)     
        ans_df = ans_df.mean() * 48 / abs(ans_df).sum()      
        return ans_df
