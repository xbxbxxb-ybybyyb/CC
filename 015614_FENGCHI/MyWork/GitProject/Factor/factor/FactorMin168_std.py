...
class FactorMin168_std(BaseFactor):
    #  定义因子参数
    ...
    depend_data = ['FactorData.Basic_factor.limit_status_minute', "FactorData.Basic_factor.close_minute","FactorData.Basic_factor.high_minute","FactorData.Basic_factor.low_minute"]
    ...
    def calc_single(self, database):
        ...
        ##等分时间窗口 
        time_zone = [int(i) for i in np.linspace(0,data1.shape[0],9)]
        time_zonelist=list(zip(time_zone[:-1],time_zone[1:]))
         
        pricelist = pd.DataFrame(columns=data1.columns)
 
        ##每一个时间窗口选取最大值
        for i in range(0,8):
            time_zonelisttemp=time_zonelist[i]
            ##收盘
            data1temp=data1.iloc[time_zonelisttemp[0]:time_zonelisttemp[1],]
            ##最高
            data2temp=data2.iloc[time_zonelisttemp[0]:time_zonelisttemp[1],]
            ##最低
            data3temp=data3.iloc[time_zonelisttemp[0]:time_zonelisttemp[1],]

            
            ##信号
            datatemp=(data2temp.max()-data3temp.max())/abs(data1temp.iloc[-1,])
            
            pricelist=pricelist.append(datatemp,ignore_index=True)
