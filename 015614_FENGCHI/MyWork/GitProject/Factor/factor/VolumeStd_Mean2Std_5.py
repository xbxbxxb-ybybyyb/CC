...
class VolumeStd_Mean2Std_5(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        ans = -minute_volume.std()
        return ans

    def reform(self, temp_result):
        
        factor_values = temp_result  # 传入这里的函数每天都会调用一次播放数据计算中间量
        factor_values = factor_values.rolling(self.n, min_periods=1).mean() / factor_values.rolling(self.n).std()
