...
class FactorMin22_std(BaseFactor):
    #  定义因子参数
    # 因子频率，。默认为日频因子， 可不设置
    factor_type = "FIX"
    ...
    def calc_single(self, database):
        ...
        data2_temp=(data2.values-data2.shift(periods=1).values)/abs(data2.shift(periods=1).values)  

        arr = (data1_temp < 0) & (data2_temp < 0)
        data1_up = data1[pd.DataFrame(arr, columns=data1.columns, index=data1.index)]
        data2_up = data2[pd.DataFrame(arr, columns=data2.columns, index=data2.index)]     
    
    
        ans=Util.array_coef(data1_up,data2_up)
