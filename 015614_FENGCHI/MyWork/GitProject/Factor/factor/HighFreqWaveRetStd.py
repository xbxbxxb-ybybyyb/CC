...
import numpy as np
import pandas as pd
class HighFreqWaveRetStd(BaseFactor):
    # 因子频率，。默认为日频因子， 可不设置
    factor_type = "FIX"
    ...
    def reform(self,temp_result):
        factor = temp_result
        n = 5
        res = -(factor-factor.rolling(window=n,min_periods=1).mean())/factor.rolling(window=n,min_periods=1).mean()
        return res
    ...
    def minute_help(self, MinuteClose):
        ...
        ret_std = ret.rolling(window=5).std()
        ret_std_ref = ret.std()
        wave_ret_std = ret[ret_std.values<ret_std_ref.values].std()
