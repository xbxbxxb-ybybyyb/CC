...
class FactorMin103_std(BaseFactor):
    #  定义因子参数

    # 因子频率，。默认为日频因子， 可不设置
    factor_type = "FIX"
    ##fix_times = ["1100","1330","1400","1430"]
    ...
    def calc_single(self, database):
        ...
        ##(h-l)/c:多空力量
        data=(data3.values-data4.values)/data1.values 
        data=pd.DataFrame(data, columns=data1.columns, index=data1.index)
        
        
        data1_temp=(data2.values-data1.shift(periods=1).values)/abs(data1.shift(periods=1).values)   
        arr_up = data1_temp > 0
        arr_down = data1_temp < 0

        data_up = data[pd.DataFrame(arr_up, columns=data1.columns, index=data1.index)]
        data_down = data[pd.DataFrame(arr_down, columns=data1.columns, index=data1.index)]   
                                
        ans =data_up.mean()-data_down.mean()     
