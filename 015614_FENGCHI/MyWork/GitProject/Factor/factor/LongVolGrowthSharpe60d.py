...
*因子名 : LongVolGrowthSharpe60d
*因子描述 : 过去60天上涨成交量增长率的负夏普率
*因子逻辑 ： 放量逐渐加剧表明卖盘激增，未来相对看跌
*作者 : 沈天琦
*因子创建日期 : 2020.02.24

"""
class LongVolGrowthSharpe60d(BaseFactor):
    factor_type = 'DAY'
    ...
    def calc_single(self, database):
        ...
        df_high_rtn = pd.DataFrame(rtn_minute.values > 0, index=rtn_minute.index, columns=rtn_minute.columns)
        df_high_vol_growth = vol_growth_minute[df_high_rtn]

        result = -df_high_vol_growth.mean(axis=0) / df_high_vol_growth.std(axis=0)
