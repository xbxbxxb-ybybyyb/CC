...
class FactorMin66_mean(BaseFactor):
    #  定义因子参数
    # 因子频率，。默认为日频因子， 可不设置
    factor_type = "FIX"
    ##fix_times = ["1000","1100","1300","1330","1400","1430"]
    # 依赖的平台原始数据，包括FactorData和MarketData接口中的数据。 默认为空，必须设置
    depend_data = ['FactorData.Basic_factor.limit_status_minute',"FactorData.Basic_factor.close_minute", "FactorData.Basic_factor.high_minute","FactorData.Basic_factor.low_minute","FactorData.Basic_factor.volume_minute"]
    ...
    def calc_single(self, database):
        ...
        ##成交量   
        data4_r=(data4-data4.shift(1))/abs(data4.shift(1))
           
        ##真实高位
        data1_shift1=data1.shift(1)
        
        data2_realh=(data2+data1_shift1+(data2-data1_shift1).abs())/2
       
        ##真实低位
        data3_reall=(data3+data1_shift1-(data3-data1_shift1).abs())/2
        
        ##收盘-真实高位
        data12_r=(data1-data2_realh)/abs(data2_realh)   
        
        ##收盘-真实低位
        data13_r=(data1-data3_reall)/abs(data3_reall) 
              
        data12_r=data12_r[(data1_r<0) & (data4_r>0)]
                             
        data13_r=data13_r[(data1_r>0) & (data4_r>0)]
        
        ans=data12_r.sum()+data13_r.sum()
