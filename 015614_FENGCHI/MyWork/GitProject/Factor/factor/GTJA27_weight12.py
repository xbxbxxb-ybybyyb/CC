...
class GTJA27_weight12(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        min_close = min_forward_adj(min_close).iloc[-1,:]
        ...
        close_pre6 = close.iloc[0,:]
        close_pre3 = close.iloc[3,:]
        ans = (min_close / close_pre3 - 1 + min_close / close_pre6 - 1) * 100
        return ans

    def reform(self, temp_result):
        def weight_sum(df):
            n = len(df)
            weight = np.power(0.9, np.arange(n-1,-1,-1))
            return pd.Series(df*weight).sum(skipna=True)
        
        factor_values = temp_result  # 传入这里的函数每天都会调用一次播放数据计算中间量
        factor_values = factor_values.rolling(window=self.reform_window, min_periods=1).apply(func=weight_sum)
