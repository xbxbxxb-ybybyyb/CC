...
class CautionLongStd_5_5(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        mvolume_rpl = mvolume.resample(str(m)+'T').sum().reindex(index=mclose_rpl.index.to_list())  # 取出m分钟volume
        rtns_rpl = mclose_rpl.values[1:, :] / mclose_rpl.values[:-1, :] - 1  # 相对于前m分钟的涨跌幅
        candle = mclose_rpl.values / mopen_rpl.values - 1
        rtns_rpl = rtns_rpl[-237*n//m:, :]  # 取出n天
        candle = candle[-237*n//m:, :]  # 取出n天
        mvolume_rpl = mvolume_rpl.values[-237*n//m:, :]  # 取出n天

        ind = (candle < 0) & (rtns_rpl >= 0)  # 阴线收涨/平
        alpha = np.nansum(ind, axis=0).astype('float64')
        np.place(alpha, np.nansum(mvolume_rpl == 0, axis=0) > mvolume_rpl.shape[0] // 2, np.nan)  # 超过一半成交量为0的置为np.nan
