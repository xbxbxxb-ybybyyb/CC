...
class HighFreqHighMARetCorrSharp(BaseFactor):
    factor_type = "FIX"
    ...
    def calc_single(self, database):
        ...
        high_ma = high.rolling(window=5).mean().values[1:]
        ret = close.values[1:] / close[:-1] - 1.
        ret_ma = pd.DataFrame(ret, columns=close.columns).rolling(window=5).mean().values
        log_ret = np.log(ret_ma)
        ans = - self.array_coef(high_ma, log_ret)  
