...
class StdHmL_20(BaseFactor):
    ...
    def calc_single(self, database):
        ...
        day_low = minute_low.resample("1440T", label="right").min().dropna(axis=0, how='all').values[1:,:]
        minute_high = database.depend_data['FactorData.Basic_factor.high_minute']
        minute_high = min_forward_adj(minute_high)
        day_high = minute_high.resample("1440T", label="right").max().dropna(axis=0, how="all").values[1:,:]
        minute_close = database.depend_data['FactorData.Basic_factor.close_minute']
        minute_close = min_forward_adj(minute_close)
        day_close = minute_close.resample("1440T", label="right").last().dropna(axis=0, how="all").shift(1).values[1:,:]

        data_return_high = day_high / day_close - 1
        data_return_low = day_low / day_close - 1
        ans_df = data_return_high.std(axis=0) - data_return_low.std(axis=0)
