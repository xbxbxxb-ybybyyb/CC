...
class MinCapitalGainRH(BaseFactor):
    factor_type = "FIX"
    ...
    def calc_single(self, single_database):
        ...
        high_df = high_minute.loc[pre_date].iloc[i:end].append(high_minute.loc[compute_date])
        ...
        result = - array_coef(rel_price.rank(axis=0), high_df.shift(1).rank(axis=0))
        return result
