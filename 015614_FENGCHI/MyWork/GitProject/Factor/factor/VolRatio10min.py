...
class VolRatio10min(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        volume_sum_yesterday = volume.iloc[:237, :].sum(axis=0)
        volume_sum_yesterday[volume_sum_yesterday.values == 0] = np.nan
        volume_sum_part_today = volume.iloc[-30:, :].sum(axis=0)
        volume_sum_part_today[volume_sum_part_today.values == 0] = np.nan
        ans = volume_sum_part_today/volume_sum_yesterday
        return ans
