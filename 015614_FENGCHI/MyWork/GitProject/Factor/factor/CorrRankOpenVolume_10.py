...
class CorrRankOpenVolume_10(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        minute_open = data_filter(database.depend_data['FactorData.Basic_factor.open_minute'].iloc[-30:],limit_status,method='minute')
        ...
        rank_open = minute_open.rank(axis=1).values
        rank_volume = minute_volume.rank(axis=1).values
        ans_df = arraycoef(rank_open, rank_volume)

        current_date = minute_open.index[-1].strftime('%Y-%m-%d')
        df_factor = pd.DataFrame(index=[pd.Timestamp(current_date)], columns=minute_open.columns)
