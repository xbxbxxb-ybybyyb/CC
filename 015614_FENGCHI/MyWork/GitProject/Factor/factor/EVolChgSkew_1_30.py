...
class EVolChgSkew_1_30(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        mvolume = mvolume.iloc[-237:, ].resample('30T').sum(min_count=1)
        alpha = np.nanmax(mvolume, axis=0) - np.nanmin(mvolume, axis=0)
        ind = np.sum(mvolume == 0, axis=0) > (mvolume.shape[0] // 2)
        alpha[ind] = np.nan  # 控制0数量，超过一半是0认为这个值是无效的，置为np.nan
        alpha = pd.Series(alpha.flatten(), index=mvolume.columns.to_list())
