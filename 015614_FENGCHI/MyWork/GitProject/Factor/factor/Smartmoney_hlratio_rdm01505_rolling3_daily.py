...
class Smartmoney_hlratio_rdm01505_rolling3_daily(BaseFactor):
    #  定义因子参数

    # 因子频率，默认为日频因子， 可不设置
    # 依赖的平台原始数据，包括FactorData和MarketData接口中的数据。 默认为空，必须设置
    depend_data = ["FactorData.Basic_factor.close_minute","FactorData.Basic_factor.high_minute","FactorData.Basic_factor.low_minute","FactorData.Basic_factor.amt_minute","FactorData.Basic_factor.volume_minute"]
    # 计算每个时点的因子所需要前移的数据窗口大小
    # 例如，为日频因子，lag=3表示计算某一日的因子值需要依赖前三个交易日和当日的数据，默认为0，可不设置
    lag=2
    reform_window=5
    batch_info={'n':3,'ratio_beta':0.5,'filter_thrsh':0.15,'object_name':'hlratio','stat':'rdm'}
    ...
    def calc_single(self, database):
        ...
        filter_thrsh = self.batch_info['filter_thrsh']
        object_name = self.batch_info['object_name']  # 要使用的对象名
        stat_name = self.batch_info['stat']  # 要计算的统计特征
        ...
        Smart_money_part = pd.DataFrame(S_ratio_rank.values < filter_thrsh, index=S_ratio_rank.index,
                                                     columns=S_ratio_rank.columns)
        Smart_money_part = Smart_money_part[Smart_money_part]
        raw_data = switchcase_rawdata[object_name].fillna(0)
        Smart_money_raw_data = pd.DataFrame(raw_data.values * Smart_money_part.values, index=raw_data.index,
                                            columns=raw_data.columns)
        Smart_money_stat = statistic_choice.get(stat_name, raise_error)(Smart_money_raw_data)
        tot_stat = statistic_choice.get(stat_name, raise_error)(raw_data)
        tot_stat[pd.Series(tot_stat.values==0,index = tot_stat.index)] =0.000001
