...
class CloseOpenKurt2Mean5(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        alpha = kurtosis(mclose.values / mopen.values, axis=0, nan_policy='omit')
        alpha = pd.Series(alpha, index=mclose.columns.to_list())

        return alpha

    def reform(self, temp_result):
        factor_values = rolling_process(temp_result, 'mean', window=5, min_periods=5)
