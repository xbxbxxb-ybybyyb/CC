...
class GTJA20_std5(BaseFactor):
    #  定义因子参数

    # 因子频率，默认为日频因子， 可不设置
    factor_type = "FIX"
    # 依赖的平台原始数据，包括FactorData和MarketData接口中的数据。 默认为空，必须设置
    depend_data = ["FactorData.Basic_factor.close_minute",'FactorData.Basic_factor.adjfactor',"FactorData.Basic_factor.close","FactorData.Basic_factor.limit_status_minute"]
    ...
    def calc_single(self, database):
        ...
        day_close = database.depend_data['FactorData.Basic_factor.close']*adj_factor
        close_pre = day_close.iloc[0,:].values
        ans_df = 100 * (close/close_pre - 1)
