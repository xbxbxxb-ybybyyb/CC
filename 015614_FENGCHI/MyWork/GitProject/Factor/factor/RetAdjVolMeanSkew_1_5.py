...
class RetAdjVolMeanSkew_1_5(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        alpha = self.agg(mrev, 'mean')
        alpha = pd.Series(alpha, index=mclose_rpl.columns.to_list())

        return alpha

    def reform(self, temp_result):
        min_periods = self.decide_min_periods('skew', 5)
        factor_values = rolling_process(temp_result, 'skew', window=5, min_periods=min_periods)
