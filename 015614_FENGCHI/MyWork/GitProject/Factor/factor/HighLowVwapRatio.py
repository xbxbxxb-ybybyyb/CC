...
import numpy as np
import pandas as pd
class HighLowVwapRatio(BaseFactor):
    # 因子频率，。默认为日频因子， 可不设置
    factor_type = "FIX"
    # 依赖的平台原始数据，包括FactorData和MarketData接口中的数据。 默认为空，必须设置
    depend_data = ["FactorData.Basic_factor.amt_minute", "FactorData.Basic_factor.high_adj_minute",
    "FactorData.Basic_factor.volume_adj_minute","FactorData.Basic_factor.low_adj_minute"]
    ...
    def minute_help(self, MinuteHigh, MinuteLow, MinuteTurnover, MinuteVolume):
        ...
        vwap = turnover.sum() / volume.sum()
        mean_high = high.mean()
        mean_low = low.mean()
        ratio = (mean_high / vwap + mean_low / vwap) / 2
