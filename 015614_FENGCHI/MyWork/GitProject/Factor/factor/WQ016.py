...
class WQ016(BaseFactor):
    #  定义因子参数

    # 因子频率，默认为日频因子， 可不设置
    factor_type = "DAY"
    # 依赖的平台原始数据，包括FactorData和MarketData接口中的数据。 默认为空，必须设置
    depend_data = ["FactorData.Basic_factor.high", "FactorData.Basic_factor.volume_by_share", "FactorData.Basic_factor.adjfactor"]
    ...
    def calc_single(self, database):
        ...
        high_adj = high * adj
        cov = high_adj.rolling(5).cov(volume)
        ans = -cov.rank(axis=1, method='max')
        ans = ans.iloc[-1, :]
        return ans
