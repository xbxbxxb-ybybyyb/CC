...
class FactorMin94_mean(BaseFactor):
    #  定义因子参数
    # 因子频率，。默认为日频因子， 可不设置
    factor_type = "FIX"
    ...
    def calc_single(self, database):
        ...
        mopen = database.depend_data['FactorData.Basic_factor.high_minute'].copy()
        ...
        atr1=((data2.values-data3.values)+abs(data2.values-data1.shift().values)+abs((data2.values-data3.values)+np.abs(data2.values-data1.shift().values)))/2
        
        atr2=np.abs(data3.values-data1.shift().values)
        
        atr=pd.DataFrame((atr1+atr2+np.abs(atr1-atr2))/2,columns=data1.columns,index=data1.index)  
        
        data5_temp=(data5.values-data5.shift(periods=1).values)/abs(data5.shift(periods=1).values)   
        
        atr_up=atr[pd.DataFrame(data5_temp>0,columns=data5.columns,index=data5.index)]
        
        atr_down=atr[pd.DataFrame(data5_temp<0,columns=data5.columns,index=data5.index)]   
                                
        ans = atr_up.mean()/atr_down.mean()
