...
class HF_PVVCorrIndusRank(BaseFactor):

    """
    * 因子名：HF_PVVCorrIndusRank_13h
    * 因子功能描述：当日价格分钟线5分钟波动率与价格的相关性，相关性越低，说明异常炒作越少，具备较为稳定的超额能力。取行业超额比率作为因子值
    ...
    * 修改原因：尚未修改
    ...
    #     result = self.minute_help(self.minute, 'HF_PVVCorrIndusRank_13hHelp', MinuteClose,citicsX_industry_code)
    ...
    def industry_excess_div(self,data,indutry_code_df,date):
        ...
        for ind_i in industry_list:
            ...
            data.iloc[ind_idx_in_series] = data.iloc[ind_idx_in_series].div(data.iloc[ind_idx_in_series].mean(),axis=0)
        return data
