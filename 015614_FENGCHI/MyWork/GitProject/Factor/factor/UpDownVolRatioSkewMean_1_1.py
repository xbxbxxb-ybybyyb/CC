...
class UpDownVolRatioSkewMean_1_1(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        mvolume = np.sqrt(mvolume)
        mvolume_long = np.ones(mvolume.shape) * np.nan
        mvolume_long[mclose.values > mopen.values] = mvolume.values[mclose.values > mopen.values]  # 多头成交量
        mvolume_short = np.ones(mvolume.shape) * np.nan
        mvolume_short[mclose.values < mopen.values] = mvolume.values[mclose.values < mopen.values]  # 空头成交量
        mvolume_long = mvolume_long[-237:, :]  # 取出n天
        mvolume_short = mvolume_short[-237:, :]  # 取出n天
