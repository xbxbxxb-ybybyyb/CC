...
class FactorMin235_std(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        data1 = Util.data_filter(database.depend_data['FactorData.Basic_factor.volume_minute'],limit_status,method='minute')
        ...
        def quantile_index_mean(x,flag_dir):
            x=x.dropna()
            if len(x)>0:
               ##所有的index列表
               indexlist=x.index.tolist()
               ##排序
               x=x.sort_values(ascending=flag_dir)
               ##排名前列的index列表
               indexlist_top=x.index[0:round(x.shape[0]*0.25)].tolist()
               ##所有的index列表
               index_where=[]
            
               for i in indexlist_top:
                   index_where.append(indexlist.index(i))
                
               xtemp=np.nanmean(index_where)

               return xtemp
            
   
        data1_index=data1.apply(quantile_index_mean,flag_dir=False)
