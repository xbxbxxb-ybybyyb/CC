...
class Min1WeightedFlow_1(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        minute_close_value = minute_close.values
        ...
        minute_high_value = minute_high.values
        ...
        minute_low_value = minute_low.values
        minute_volume = data_filter(database.depend_data['FactorData.Basic_factor.volume_minute'],limit_status,method='minute')
        minute_volume_value = minute_volume.values

        ans_df = (minute_high_value+minute_close_value+minute_low_value) / 3 * minute_volume_value

        current_date = minute_close.index[-1].strftime('%Y-%m-%d')
        df_factor = pd.DataFrame(index=[pd.Timestamp(current_date)],columns = minute_close.columns,dtype = 'float64')
        df_factor.loc[current_date,:] = ans_df.max(axis=0)
