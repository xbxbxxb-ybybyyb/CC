...
class BigOrderNetInflowRate5d(BaseFactor):
    ...
    def calc_single(self, database):
        ...
        amtpertrade_rank =amtpertrade.rank(pct=True)
        bigminute = pd.DataFrame(amtpertrade_rank.values>0.8, index=amtpertrade_rank.index, columns=amtpertrade_rank.columns)
        
        bigorder_netflow = amt_minute[bigminute & f1].sum() - amt_minute[bigminute & f2].sum() 
        
         
        return  -bigorder_netflow/amt_minute[bigminute].sum()
