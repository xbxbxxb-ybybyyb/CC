...
class FactorMin124_diffdivstd(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        arr = data2_temp > 0
        data2_up = data2[pd.DataFrame(arr, columns=data1.columns, index=data1.index)]
        arr = data2_temp < 0
        data2_down = data2[pd.DataFrame(arr, columns=data2.columns, index=data2.index)]   
                  
        ans =data2_up.mean()/data2.mean()-data2_down.mean()/data2.mean()
