...
class OTC5std(BaseFactor):
    ...
    def calc_single(self, database):
        ...
        open_px = database.depend_data['FactorData.Basic_factor.open'].copy()*adjfactor
        ans = (open_px.iloc[-1, :] - close.iloc[-1, :]).rank()
        return -ans

    # 针对播放后的结果，进行相关的rolling等操作。所用的前序数据长度应为reform_window。默认不修改temp_result， 可不重写。
    def reform(self, temp_result):
         return temp_result.rolling(self.reform_window,min_periods=int(self.reform_window/2)).std()
