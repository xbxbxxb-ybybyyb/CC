...
class HF_RSRSZScore(BaseFactor):
    """
    *因子名 : HF_RSRSZScore_13h
    *因子功能描述 : 相对阻力支撑强度指标，High相对Low回归得到的beta作为因子值，值越大，买入越强度越大，次日收益越低,取其ZScore
    ...
    *因子创建日期 : 2019.6.21
    ...
    #     df = self.minute_help(self.minute, 'HF_RSRSZScore_13hHelp',MinuteHigh,MinuteLow)
    #     df = (df-df.rolling(window=10,min_periods=1).mean())/df.rolling(window=10,min_periods=1).std()
