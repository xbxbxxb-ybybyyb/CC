...
class GTJA176(BaseFactor):
    #  定义因子参数

    # 因子频率，默认为日频因子， 可不设置
    factor_type = "DAY"
    # 依赖的平台原始数据，包括FactorData和MarketData接口中的数据。 默认为空，必须设置
    depend_data = ["FactorData.Basic_factor.volume_by_share", "FactorData.Basic_factor.close",
                   "FactorData.Basic_factor.high", "FactorData.Basic_factor.low", "FactorData.Basic_factor.adjfactor"]
    ...
    def calc_single(self, database):
        ...
        close_percent = (close - low.rolling(12).min()) / (high.rolling(12).max() - low.rolling(12).min())
        close_percent_rank = close_percent.rank(axis=1, pct=True)
        volume_rank = volume.rank(axis=1, pct=True)
        ans = ut.array_coef(close_percent_rank.iloc[-6:], volume_rank.iloc[-6:])
        ans[np.isinf(ans)] = np.nan
        return -ans
