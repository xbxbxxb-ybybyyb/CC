...
class RetAdjVolStdRegrmse_3_5(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        mrtns = np.divide(np.diff(mclose_rpl.values, axis=0), mclose_rpl.values[:-1, :])  # m分钟收益率
        mvolume_rpl = mvolume.resample(str(m)+'T').sum()  # 取出m分钟volume
        mvolume_rpl = mvolume_rpl.reindex(index=mclose_rpl.index.to_list()[1:], columns=mclose_rpl.columns.to_list())
        volume_m3 = mvolume_rpl.values[-237*15//m:, :]  # 15天成交量
        volume_mean = np.nanmean(volume_m3, axis=0)
        volume_std = np.nanstd(volume_m3, axis=0)
        mvolume_rpl_zscore = np.divide(np.subtract(mvolume_rpl.values[-237*n//m:, :], volume_mean), volume_std)  # 做zscore
        mrev = np.multiply(mrtns[-237*n//m:, :], mvolume_rpl_zscore * 1000)  # "收益"
        mvolume_rpl = mvolume_rpl.values[-237*n//m:, :]
        mrev = np.divide(np.multiply(mrev, mvolume_rpl), mvolume_rpl)  # 成交量为0的置为nan

        alpha = self.agg(mrev, 'std')
        alpha = pd.Series(alpha, index=mclose_rpl.columns.to_list())

        return alpha

    def reform(self, temp_result):
        min_periods = self.decide_min_periods('regrmse', 5)
        factor_values = rolling_process(temp_result, 'regrmse', window=5, min_periods=min_periods)
