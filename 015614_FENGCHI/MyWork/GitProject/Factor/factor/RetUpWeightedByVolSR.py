...
class RetUpWeightedByVolSR(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        minute_vol = database.depend_data['FactorData.Basic_factor.volume_minute']
        minute_vol = data_filter(minute_vol, limit_status, method='minute')
        minute_ret = minute_close.pct_change(periods=5)
        minute_cum_vol = minute_vol.rolling(5).sum()
        minute_ret_up = minute_ret[minute_ret > 0]
        minute_vol_up = minute_vol[minute_ret > 0]
        minute_ret_up_weighted_by_vol = (minute_ret_up * minute_vol_up).sum() / minute_vol_up.sum()
        return minute_ret_up_weighted_by_vol
