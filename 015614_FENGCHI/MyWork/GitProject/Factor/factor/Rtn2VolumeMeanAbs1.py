...
class Rtn2VolumeMeanAbs1(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        mclose_v = mclose.values[-237-1:, :]
        mrtns = mclose_v[1:, :] / mclose_v[:-1, :] - 1
        mvolume = mvolume.values[-237:, :] / 1e9
        alpha = np.abs(np.nanmean(mrtns / mvolume, axis=0))
