...
class GTJA40(BaseFactor):
    #  定义因子参数

    # 因子频率，默认为日频因子， 可不设置
    factor_type = "FIX"
    # 依赖的平台原始数据，包括FactorData和MarketData接口中的数据。 默认为空，必须设置
    depend_data = ["FactorData.Basic_factor.volume_minute","FactorData.Basic_factor.close_minute",'FactorData.Basic_factor.adjfactor',"FactorData.Basic_factor.volume_by_share","FactorData.Basic_factor.close","FactorData.Basic_factor.limit_status_minute","FactorData.Basic_factor.Data_limit_pctg"]
    ...
    def calc_single(self, database):
        ...
        volume_increase = day_volume[close_delta>0]
        volume_decrease = day_volume[close_delta<=0]
        ans_df = volume_increase.iloc[-20:,:].sum() / volume_decrease.iloc[-20:,:].sum() * 100
        return ans_df
