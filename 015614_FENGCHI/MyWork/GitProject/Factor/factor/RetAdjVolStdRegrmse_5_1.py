...
class RetAdjVolStdRegrmse_5_1(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        volume_m3 = mvolume_rpl.values[-237*15:, :]  # 15天成交量
        volume_mean = np.nanmean(volume_m3, axis=0)
        volume_std = np.nanstd(volume_m3, axis=0)
        mvolume_rpl_zscore = np.divide(np.subtract(mvolume_rpl.values[-237*n:, :], volume_mean), volume_std)  # 做zscore
        mrev = np.multiply(mrtns[-237*n:, :], mvolume_rpl_zscore * 1000)  # "收益"
        mvolume_rpl = mvolume_rpl.values[-237*n:, :]
