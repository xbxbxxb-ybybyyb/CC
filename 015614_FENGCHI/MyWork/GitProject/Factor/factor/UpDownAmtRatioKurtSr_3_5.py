...
class UpDownAmtRatioKurtSr_3_5(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        mamt_short[mclose_rpl.values < mopen_rpl.values] = mamt_rpl.values[mclose_rpl.values < mopen_rpl.values]  # 空头成交量
        ...
        if aggtype == 'std':
            ...
            alpha = skew(mamt_long, axis=0, nan_policy='omit').__array__() / skew(mamt_short, axis=0, nan_policy='omit').__array__()
        elif aggtype == 'kurt':
            alpha = kurtosis(mamt_long, axis=0, nan_policy='omit').__array__() / kurtosis(mamt_short, axis=0, nan_policy='omit').__array__()
        else:
