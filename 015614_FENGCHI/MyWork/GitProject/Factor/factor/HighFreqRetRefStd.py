...
import numpy as np
import pandas as pd
class HighFreqRetRefStd(BaseFactor):
    # 因子频率，。默认为日频因子， 可不设置
    factor_type = "FIX"
    ...
    def reform(self,temp_result):
        factor = temp_result
        n = 15
        res = (factor - factor.rolling(window=n,min_periods=3).mean())/(factor.rolling(window=n,min_periods=3).std())
        return res
