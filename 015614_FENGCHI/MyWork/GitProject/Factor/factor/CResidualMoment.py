...
class CResidualMoment(BaseFactor):
    #  定义因子参数
    ptype = 'mean'
    # 因子频率，默认为日频因子， 可不设置
    factor_type = "FIX"
    ...
    def calc_single(self, database):
        ...
        close = min_close.resample("5T", label="right").last().dropna(axis=0, how='all')
        ret = ((close - close.shift(1)) / (close.shift(1))).iloc[1:,:]
        residual = Util.rolling_process(ret, 'regrmse', window=5, min_periods=5)
        residual = residual.iloc[-48 * (self.minute_lag):, :]  
        ans = residual.sum() / residual.std()
        return ans
