...
class FactorMin137_mean(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        data1_r_temp=(data1.values-data1.shift(periods=1).values)/abs(data1.shift(periods=1).values)   
        data1_r = pd.DataFrame(data1_r_temp, columns=data1.columns, index=data1.index)     
        data2_r_temp=(data2.values-data2.shift(periods=1).values)/abs(data2.shift(periods=1).values)  

        ##放量的
        data1_r_up=data1_r[pd.DataFrame(data2_r_temp>0, columns=data1_r.columns, index=data1_r.index)].cumsum()
        
        ##填充na
        data1_r_up.fillna(method='pad',inplace=True)
        
        ##缩量的收益率
        data1_r_down=data1_r[pd.DataFrame(data2_r_temp<0, columns=data1_r.columns, index=data1_r.index)].cumsum()
        
        data1_r_down.fillna(method='pad',inplace=True)
        
        def MaxDrawdown(return_list):
          return_list=return_list.dropna()
        ...
        ##计算最大回撤
        data1_r_up_max=data1_r_up.apply(MaxDrawdown)
        data1_r_down_max=data1_r_down.apply(MaxDrawdown)
        
        ans=data1_r_up_max/data1_r_down_max
