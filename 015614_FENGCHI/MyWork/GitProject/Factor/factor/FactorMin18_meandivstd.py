...
class FactorMin18_meandivstd(BaseFactor):
    #  定义因子参数
    # 因子频率，。默认为日频因子， 可不设置
    factor_type = "FIX"
    ...
    def calc_single(self, database):
        ...
        def corr1(x,y):
             ...
             c=np.nanmean(a*b,axis=0)/(np.nanstd(a,axis=0)*np.nanstd(b,axis=0))     
             c[np.isinf(c)]=np.nan 
             return c
        ...
        ##计算价差
        ##5分钟变化运算：
        data1=(data1-data1.shift(periods=5))/abs(data1.shift(periods=5))        
        data2=(data2-data2.shift(periods=5))/abs(data2.shift(periods=5))  
        
        ## 
        data1=data1.drop(data1.index[0:5])
        ##
        data2=data2.drop(data2.index[0:5])
        
        data1=data1[(data1<0) & (data2<0)]
        data2=data2[(data1<0) & (data2<0)]

        ans=pd.Series(corr1(data1,data2),index= data1.columns)      
