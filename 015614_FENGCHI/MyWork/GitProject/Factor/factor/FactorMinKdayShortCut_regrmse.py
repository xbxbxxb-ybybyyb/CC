...
class FactorMinKdayShortCut_regrmse(BaseFactor):
    #  定义因子参数
    ptype = 'regrmse'
    ...
    def calc_single(self, database):
        ...
        minute_close = Util.data_filter(min_forward_adj(close_df), limit_status, method="minute")
        minute_open = Util.data_filter(min_forward_adj(open_df), limit_status, method="minute")
        minute_high = Util.data_filter(min_forward_adj(high_df), limit_status, method="minute")
        minute_low = Util.data_filter(min_forward_adj(low_df), limit_status, method="minute")

        shortcut = 2 * (minute_high.values - minute_low.values) - abs(minute_open.values - minute_close.values)
        ans_df = np.nansum(shortcut, axis=0) / minute_low.iloc[-1, :]
        return ans_df
