...
class OverBuySellSkewRegrmse_1_15(BaseFactor):
    #  定义因子参数
    ...
    depend_data = ["FactorData.Basic_factor.limit_status_minute", "FactorData.Basic_factor.close_minute", "FactorData.Basic_factor.high_minute",
    ...
    def calc_single(self, database):
        ...
        overbuysell = np.divide(np.subtract(mhigh_rpl.values, mclose_rpl.values), np.subtract(mhigh_rpl.values, mlow_rpl.values))
        np.place(overbuysell, np.isinf(overbuysell), np.nan)  # 出现最高价等于最低价的情形，此时大多为无交易量情形
        overbuysell = overbuysell[-237//m:, :]  # 取出n天

        alpha = self.agg(overbuysell, 'skew')
