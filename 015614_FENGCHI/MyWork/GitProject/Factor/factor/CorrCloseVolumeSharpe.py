...
class CorrCloseVolumeSharpe(BaseFactor):
    #  定义因子参数

    # 因子频率，默认为日频因子， 可不设置
    factor_type = "DAY"
    # 依赖的平台原始数据，包括FactorData和MarketData接口中的数据。 默认为空，必须设置
    depend_data = ["FactorData.Basic_factor.volume_by_share", "FactorData.Basic_factor.close", "FactorData.Basic_factor.adjfactor"]
    ...
    def calc_single(self, database):
        ...
        cv_corr = ut.rolling_corr(close_adj, volume, 10)
        cv_corr[np.isinf(cv_corr)] = np.nan
        cv_corr[(cv_corr > 1) | (cv_corr < -1)] = np.nan
        ans = -cv_corr.iloc[-30:].mean() / cv_corr.iloc[-30:].std()
        return ans
