...
class MinCapitalGainBetaZscore(BaseFactor):
    factor_type = "FIX"
    ...
    def calc_single(self, single_database):
        ...
        arr = - array_coef(rel_price, high_df.shift(1)).values * rel_price.std().values / high_df.shift(1).std().values
        result = pd.Series(arr, index=rel_price.columns)
        return result

    def reform(self, temp_result):
        arr = (temp_result.values - temp_result.rolling(self.reform_window).mean().values) / temp_result.rolling(self.reform_window).std().values
