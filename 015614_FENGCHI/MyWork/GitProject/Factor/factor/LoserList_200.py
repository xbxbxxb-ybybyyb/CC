...
class LoserList_200(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        ans = pd.Series(pct_chg.rank(axis=1, ascending=True).values[0],index=pct_chg.columns)
        return -ans

    # 针对播放后的结果，进行相关的rolling等操作。所用的前序数据长度应为reform_window。默认不修改temp_result， 可不重写。
    def reform(self, temp_result):
         return temp_result.rolling(self.reform_window).apply(self.__count_smaller_than_n)

    def __count_smaller_than_n(self,series, m=200):  # 序列中小于m的次数
        num = np.sum(series <= m)
        return num