...
class Last30MaxClimbBias20d(BaseFactor):
    factor_type = "FIX"
    ...
    def calc_single(self, single_database):
        ...
        min2here = vwap.expanding().min()
        climb2here = pd.DataFrame(vwap.values / min2here.values - 1, index=vwap.index, columns=vwap.columns)
    ...
    def reform(self, temp_result):
        arr = - (temp_result.values - temp_result.rolling(self.reform_window).mean().values) / \
              temp_result.rolling(self.reform_window).std().values
