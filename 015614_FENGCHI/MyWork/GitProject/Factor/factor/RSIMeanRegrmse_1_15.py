...
class RSIMeanRegrmse_1_15(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        mrtns = np.divide(np.diff(mclose_rpl.values, axis=0), mclose_rpl.values[:-1, :])  # 5分钟收益率
        mrtns_pos = mrtns.copy()
        np.place(mrtns_pos, mrtns_pos < 0, 0)
        mrtns_pos_r = pd.DataFrame(mrtns_pos, columns=mclose.columns.to_list()).rolling(window=m, min_periods=m).sum()
        mrtns_r = pd.DataFrame(mrtns, columns=mclose.columns.to_list()).abs().rolling(window=m, min_periods=m).sum()
        strength = np.divide(mrtns_pos_r.values[-237*n//5:, :], mrtns_r.values[-237*n//5:, :])
        np.place(strength, np.isinf(strength), np.nan)  # 出现inf可能是因为停牌

        alpha = self.agg(strength, 'mean')
