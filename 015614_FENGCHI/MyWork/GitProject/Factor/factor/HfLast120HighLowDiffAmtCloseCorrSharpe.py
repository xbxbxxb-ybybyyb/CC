"""
    ...
    *因子名 : HfLast120HighLowDiffAmtCloseCorrSharpe
    *因子功能描述 : 开盘小时高低价成交额之差与分钟收盘价的负相关系数的夏普率; 分钟高低价成交额差值变化与股价变化背离，说明上涨动能衰竭,夏普率越高，说明趋势越大且稳定。
...
class HfLast120HighLowDiffAmtCloseCorrSharpe(BaseFactor):
    ...
    #     super(HfLast120HighLowDiffAmtCloseCorrSharpe_13h, self).__init__(json_path)
        
    # def definition(self, MinuteHigh, MinuteLow, MinuteClose, MinuteVolume):

    #     factor_values = self.minute_help(self.minute, 'HfLast120HighLowDiffAmtCloseCorrSharpe_13hHelp', MinuteHigh, MinuteLow, MinuteClose, MinuteVolume)
    #     factor_values = factor_values.rolling(window=5,min_periods=1).mean() / factor_values.rolling(window=5,min_periods=1).std()
    ...
    #     df_factor.loc[current_date] = -minute_swing_diff_amt[-120:].corrwith(minute_close[-120:])
