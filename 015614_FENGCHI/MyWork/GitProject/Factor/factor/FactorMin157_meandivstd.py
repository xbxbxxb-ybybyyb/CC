...
class FactorMin157_meandivstd(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        def get_regression_beta(y):
            
            x=data_compare[y.name]
            x=np.array(x)
            y=np.array(y)
            ...
            beta = np.cov(x_,y_, bias=True)[0, 1] / np.var(x_)
            
            return beta
        ##收益率
        data1_r=pd.DataFrame((data1.values-data1.shift().values)/data1.shift().values,columns=data1.columns,index=data1.index)
                
        data2_up=data2[data2>data2.shift()]
        data1_r_up=data1_r[data2>data2.shift()]
        data_compare=data2_up
              
        ##进行回归
        up_beta=data1_r_up.apply(get_regression_beta)
        
        ##缩量
        data2_down=data2[data2<data2.shift()]
        data1_r_down=data1_r[data2<data2.shift()]
        data_compare=data2_down

        down_beta=data1_r_down.apply(get_regression_beta)


        ans =up_beta-down_beta
