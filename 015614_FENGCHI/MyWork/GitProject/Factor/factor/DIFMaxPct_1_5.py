...
class DIFMaxPct_1_5(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        dif = np.subtract(mclose_ma12, mclose_ma26).values
        dif = dif[-237//m:, :]  # 取出n天

        alpha = self.agg(dif, 'max')
        alpha = pd.Series(alpha, index=mclose.columns.to_list())

        return alpha

    def reform(self, temp_result):
        factor_values = rolling_process(temp_result, 'pct', window=1)
        factor_values[factor_values.values > 1e9] = np.nan
