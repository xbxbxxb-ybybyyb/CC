...
class FactorMin143_meandivstd(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        def get_regression_params(y):
            ...
            x=np.arange(len(y))
            beta = np.cov(y, x, bias=True)[0, 1] / np.var(x)
            return beta
        ##放量的和缩量的
        data1_up=data1[data2>data2.shift()]
        data1_up_beta=data1_up.apply(get_regression_params)
        data1_down=data1[data2<data2.shift()]
        data1_down_beta=data1_down.apply(get_regression_params)

        ans=data1_up_beta-data1_down_beta
