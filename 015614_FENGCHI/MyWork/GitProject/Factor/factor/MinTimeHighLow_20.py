...
class MinTimeHighLow_20(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        minute_high.index = np.arange(0, 242, 1)
        minute_low.index = np.arange(0, 242, 1)

        minute_high_time = minute_high.idxmax()
        minute_low_time = minute_low.idxmin()
        ans = minute_high_time - minute_low_time

        return -ans

    # 针对播放后的结果，进行相关的rolling等操作。所用的前序数据长度应为reform_window。默认不修改temp_result， 可不重写。
    def reform(self, temp_result):
        return temp_result.rolling(20).mean()
