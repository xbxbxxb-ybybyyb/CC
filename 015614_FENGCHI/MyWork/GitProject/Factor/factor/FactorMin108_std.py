...
class FactorMin108_std(BaseFactor):
    #  定义因子参数

    # 因子频率，。默认为日频因子， 可不设置
    factor_type = "FIX"
    ##fix_times = ["1000","1100","1330","1400","1430"]
    ...
    def calc_single(self, database):
        ...
        ##最近30分钟的和之前的比较
        data1_r=(data1.values-data1.shift().values)/data1.shift().values
        data2_r=(data2.values-data2.shift().values)/data2.shift().values
        
        condition=pd.DataFrame((data1_r>0) & (data2_r>0),columns=data1.columns,index=data1.index)       
        
        ans=condition.apply(sum)/condition.shape[0]
