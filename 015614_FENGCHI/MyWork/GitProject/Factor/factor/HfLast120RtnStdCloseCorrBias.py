...
class HfLast120RtnStdCloseCorrBias(BaseFactor):
    factor_type = "FIX"
    ...
    def calc_single(self, database):
        ...
        minute_rtn_std = pd.DataFrame(minute_rtn, columns=close.columns).rolling(window=5,min_periods=1).std().values
        ans = - self.array_coef(minute_close[-120:], minute_rtn_std[-120:])
