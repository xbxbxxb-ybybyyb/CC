...
class CorrAmpAmt_5(BaseFactor):
    #  定义因子参数

    # 因子频率，默认为日频因子， 可不设置
    factor_type = "FIX"
    # 依赖的平台原始数据，包括FactorData和MarketData接口中的数据。 默认为空，必须设置
    depend_data = ["FactorData.Basic_factor.high_minute","FactorData.Basic_factor.low_minute","FactorData.Basic_factor.amt_minute"]
    ...
    def calc_single(self, database):
        ...
        def arraycoef(x, y):
            delta_x = x - x.mean(axis=0)
            delta_y = y - y.mean(axis=0)
            multi = (delta_x*delta_y).mean(axis=0)/(delta_x.std(axis=0)*delta_y.std(axis=0))
            return multi
        ...
        minute30_amt = minute_amt.replace([0], 0.001).resample("30T", label="right", fill_method=None).sum().replace([0], np.nan).dropna(axis=0, how="all").values[-8*(self.lag):,:]

        high_m_low = minute30_high - minute30_low
       # panduan = high_m_low.shape[0] != minute30_amt.shape[0]
        ans_df = arraycoef(minute30_amt,high_m_low)
