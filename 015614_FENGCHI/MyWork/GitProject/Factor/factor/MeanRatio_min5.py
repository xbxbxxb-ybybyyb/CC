...
class MeanRatio_min5(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        minute30_close = minute_close.resample("30T", label="right").last().dropna(axis=0, how='all').iloc[
                       -8 * (self.lag)-1:, :]
        delta = (minute30_close - minute30_close.shift(1)).iloc[1:,:]
        df_factor = delta.mean(axis=0) / abs(delta).sum(axis=0)
        return df_factor
