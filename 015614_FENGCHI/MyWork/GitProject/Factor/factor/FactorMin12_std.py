...
class FactorMin12_std(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        arr = data2_temp < 0
        data1_cal = data1[pd.DataFrame(arr, columns=data1.columns, index=data1.index)]
        data2_cal = data2[pd.DataFrame(arr, columns=data2.columns, index=data2.index)]   
    
        ans=Util.array_coef(data1_cal,data2_cal)
        return ans
