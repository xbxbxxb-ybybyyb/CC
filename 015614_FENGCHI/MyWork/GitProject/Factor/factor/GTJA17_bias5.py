...
class GTJA17_bias5(BaseFactor):
    #  定义因子参数

    # 因子频率，默认为日频因子， 可不设置
    factor_type = "FIX"
    # 依赖的平台原始数据，包括FactorData和MarketData接口中的数据。 默认为空，必须设置
    depend_data = ["FactorData.Basic_factor.amt_minute","FactorData.Basic_factor.volume_minute","FactorData.Basic_factor.close_minute",'FactorData.Basic_factor.adjfactor',"FactorData.Basic_factor.vwap","FactorData.Basic_factor.close","FactorData.Basic_factor.limit_status_minute","FactorData.Basic_factor.Data_limit_pctg"]
    ...
    def calc_single(self, database):
        ...
        current_date = min_volume.index[-1].strftime('%Y-%m-%d')
        adj_factor = database.depend_data['FactorData.Basic_factor.adjfactor'].copy()
        close = database.depend_data["FactorData.Basic_factor.close"].copy()*adj_factor
        vwap = database.depend_data["FactorData.Basic_factor.vwap"].copy()*adj_factor
        ...
        ans = np.power(np.max(vwap[-15:,:], axis=0) - vwap[-1,:], close[-1,:] - close[-6,:])
        ans_df = pd.Series(ans, index=min_amt.columns)
        ans_df = ans_df
        return ans_df
