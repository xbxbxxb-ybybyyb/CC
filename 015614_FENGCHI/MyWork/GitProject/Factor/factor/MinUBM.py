...
class MinUBM(BaseFactor):  # 派生一个因子类
    ...
    def calc_single(self, database):
        ...
        vrs_flag1 = (volume_rs > np.nanpercentile(volume_rs,80,axis=0))
        v1 = vrs_flag1 * min_volume.values
        p1 = vrs_flag1 * price_ratio.values
        df_ratio = pd.DataFrame((v1*p1)/np.nansum(v1,axis=0), index=vwap.index, columns=vwap.columns)
        # print(df_ratio)
        return df_ratio.sum()
     

    def  reform(self, temp_result):
        A = -temp_result.rolling(180,1).mean()
        return A
