...
class Tick_bsdiff_ret_skew_top_passive_orderamt_corr3_daily(BaseFactor):
    #  定义因子参数

    # 因子频率，默认为日频因子， 可不设置
    # 依赖的平台原始数据，包括FactorData和MarketData接口中的数据。 默认为空，必须设置
    depend_data = ["FactorData.Basic_factor.passivebuyorderamt_minute","FactorData.Basic_factor.passivesellorderamt_minute","FactorData.Basic_factor.close_minute","FactorData.Basic_factor.amt_minute","FactorData.Basic_factor.volume_minute","FactorData.Basic_factor.high_minute","FactorData.Basic_factor.low_minute"]
    ...
    def calc_single(self, database):
        ...
        if mini_factor_name == 'ret':
            close = database.depend_data['FactorData.Basic_factor.close_minute'];close = min_forward_adj(close)
        ...
        if mini_factor_name == 'hl':
            high = database.depend_data['FactorData.Basic_factor.high_minute'];high = min_forward_adj(high)
            low = database.depend_data['FactorData.Basic_factor.low_minute'];low = min_forward_adj(low)
