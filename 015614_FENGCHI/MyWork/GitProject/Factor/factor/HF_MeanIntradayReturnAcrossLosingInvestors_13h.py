...
class HF_MeanIntradayReturnAcrossLosingInvestors_13h(BaseFactor):

    """
    * 因子名：HF_MeanIntradayReturnAcrossLosingInvestors_13h
    * 因子功能描述：
    *     因子含义：计算当前分钟线路径上到收盘(10h/13h/14h)时的收益，并按成交额为权重计算所有浮亏的平均收益率.
    *     该因子反应了所有浮亏的平均收益率,值越大越好.因子值越大,"套牢"投资者的浮亏越小,第二天抛售止损的压力越小. 
    ...
    * 修改原因：尚未修改
    ...
    def calc_single(self, database):
        ...
        return_df_small = pd.DataFrame(arr,index=open_df.index,columns=open_df.columns)        
        
        total_lossing_tov = tov_df[return_df_small].sum(axis = 0)        
        result = (return_df * tov_df)[return_df_small].sum(axis = 0) / total_lossing_tov
