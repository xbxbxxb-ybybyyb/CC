...
class Ret10Max_SR5(BaseFactor):
    ...
    def calc_single(self, database):
        ...
        minute_ret = minute_close.pct_change(periods=10)
        ans = minute_ret.max(axis=0)
        return ans

    def reform(self, temp_result):
        
        factor_values = temp_result  # 传入这里的函数每天都会调用一次播放数据计算中间量
        factor_values = factor_values.rolling(self.reform_window, min_periods=1).mean() / factor_values.rolling(self.reform_window, min_periods=1).std()
