...
class TickFactor_RawActBuyOrderVolSkew(BaseFactor):
    # 因子频率，默认为日频因子， 可不设置
    factor_type = "FIX"
    # #fix_times = ["1000", "1030", "1100", "1300", "1330", "1400", "1430"]

    # 依赖的平台原始数据，包括FactorData和MarketData接口中的数据。 默认为空，必须设置
    depend_data = ["FactorData.Basic_factor.activebuyordervol_minute", "FactorData.Basic_factor.limit_status_minute"]
    ...
    def calc_single(self, database):
        ...
        min_activebuyordervol = Util.data_filter(activebuyordervol, limit_status, method="minute")
        ans = min_activebuyordervol.rolling(window=5, min_periods=2).mean().skew()
        return ans
