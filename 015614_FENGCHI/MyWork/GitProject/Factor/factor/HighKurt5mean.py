...
class HighKurt5mean(BaseFactor):
    #  定义因子参数

    # 因子频率，默认为日频因子， 可不设置
    factor_type = "FIX"
    # 依赖的平台原始数据，包括FactorData和MarketData接口中的数据。 默认为空，必须设置
    depend_data = ["FactorData.Basic_factor.high_minute","FactorData.Basic_factor.limit_status_minute"]
    ...
    def calc_single(self, database):
        ...
        high_min = high_min.dropna(how='all')
        if self.factype == 'skew':
            ans = np.sqrt(high_min).skew()
        elif self.factype == 'kurt':
            ans = np.sqrt(high_min).kurt()
        elif self.factype == 'std':
            ans = np.sqrt(high_min).std()
        return ans
