...
class FactorMin112_meandivstd(BaseFactor):
    #  定义因子参数

    # 因子频率，。默认为日频因子， 可不设置
    factor_type = "FIX"
    ##fix_times = ["1030","1300","1330","1400"]
    ...
    def calc_single(self, database):
        ...
        condition1= pd.DataFrame((data1_rcur.values<0) & (data2_rcur.values<0),columns=data1_rcur.columns,index=data1_rcur.index)    

        per1=condition1.apply(sum)/condition1.shape[0]
       
        condition2= pd.DataFrame((data1_rlast.values<0) & (data2_rlast.values<0),columns=data1_rlast.columns,index=data1_rlast.index)     
        
        per2=condition2.apply(sum)/condition2.shape[0]      
