...
class DebtToAsset_std_3y(BaseFactor):
    ...
    def calc_single(self, database):
        ...
        asset = WIND_AShareBalanceSheet['TOT_ASSETS'].unstack().reindex(columns=is_valid.columns)
        debt = WIND_AShareBalanceSheet['TOT_LIAB'].unstack().reindex(columns=is_valid.columns)
        
        debttoasset = debt/asset

        res = debttoasset.rolling(12).std()
