...
class GTJA5(BaseFactor):
    #  定义因子参数

    # 因子频率，默认为日频因子， 可不设置
    factor_type = "FIX"
    # 依赖的平台原始数据，包括FactorData和MarketData接口中的数据。 默认为空，必须设置
    depend_data = ["FactorData.Basic_factor.high_minute","FactorData.Basic_factor.volume_minute","FactorData.Basic_factor.adjfactor","FactorData.Basic_factor.high","FactorData.Basic_factor.volume_by_share","FactorData.Basic_factor.limit_status_minute","FactorData.Basic_factor.Data_limit_pctg"]
    ...
    def calc_single(self, database):
        ...
        current_date = minute_high.index[-1].strftime('%Y-%m-%d')
        ...
        day_volume = (database.depend_data['FactorData.Basic_factor.volume_by_share']).copy()
        day_volume = data_filter(day_volume, limit_pctg, method='day')
        day_volume = day_volume * (1/8)

        day_high.loc[pd.Timestamp(current_date),:] = minute_high.max()
        day_volume.loc[pd.Timestamp(current_date),:] = minute_volume.sum()
        high_rank = day_high.rolling(window=5).apply(func=rank).iloc[-5:,:].values
        volume_rank = day_volume.rolling(window=5).apply(func=rank).iloc[-5:,:].values
        ans_df = arraycoef(high_rank, volume_rank)

        df_factor = pd.DataFrame(index=[pd.Timestamp(current_date)], columns=minute_high.columns)
