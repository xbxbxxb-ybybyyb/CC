...
class RevExclu4mean(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        na_index = np.concatenate([np.arange(i * 237, i * 237 + 30) for i in range(int(ret.shape[0]/237)+1)])
        ret.iloc[na_index, :] = np.nan
        ret = ret.iloc[-237:, :].copy()
        ret = ret.dropna(how='all')
        ret_plus_1 = pd.DataFrame(ret.values + 1, index=ret.index, columns=ret.columns)
        ans = ret_plus_1.prod(axis=0) - 1
        return ans
