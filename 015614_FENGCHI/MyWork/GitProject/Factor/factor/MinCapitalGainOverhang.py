...
class MinCapitalGainOverhang(BaseFactor):
    factor_type = "FIX"
    ...
    def calc_single(self, single_database):
        ...
        amt_df = amt_minute.loc[pre_date].iloc[-30:].append(amt_minute.loc[compute_date])
        volume_df = volume_minute.loc[pre_date].iloc[-30:].append(volume_minute.loc[compute_date])
        ...
        result = - array_coef(rel_price, high_df.shift(1))
        return result
