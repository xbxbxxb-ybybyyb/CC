...
class subrr2adjwms_intraday_5(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        subrr = pd.DataFrame((subret.values) / subrange.values, index=subrange.index, columns=subrange.columns)
        subrr_crt = pd.DataFrame((subret_crt.values) / subrange_crt.values, index=subrange_crt.index,
                                 columns=subrange_crt.columns)
        alpha_all = pd.Series(np.nanmean(subrr.values, axis=0) / np.nanstd(subrr.values, axis=0), index=subrr.columns)
        ...
        alpha_all_rank = alpha_all.rank(pct=True)
        alpha_all[(alpha_crt_rank < 0.15) & (alpha_all_rank < 0.5)] = alpha_all.median()
        return alpha_all
