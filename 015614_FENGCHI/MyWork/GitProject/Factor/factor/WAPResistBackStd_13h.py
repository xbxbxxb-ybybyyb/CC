...
class WAPResistBackStd_13h(BaseFactor):

    '''
    * WAPResistBackStd_13h
    * 描述：120min的High-WAP与WAP-Low的比值的std
    * 逻辑：当WAP离支撑位近，离阻力位远表示涨幅空间越大，反弹越强。当一段时间内的反弹强度一致表示股票反弹可信度越高
    * 因子参数：分钟数据的收高低、换手、体量
    * 作者：孔剑阳
    * 日期：2019.10.10
    * 函数修改日期：尚未修改
    ...
    * 修改原因：尚未修改
    ...
    def calc_single(self, database):
        ...
        TWAP = c.rolling(120,1).mean()
        HIGH = h.rolling(120,1).max()
        LOW = l.rolling(120,1).min()
        VWAP = (a / v).rolling(120,1).mean()
        WAP = (TWAP +VWAP) / (2 * np.ones(VWAP.shape))
        RankMinute = ((HIGH-WAP)/(WAP-LOW)).std()
        return -RankMinute
