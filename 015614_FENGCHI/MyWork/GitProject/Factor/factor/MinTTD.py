...
class MinTTD(BaseFactor):  # 派生一个因子类
    ...
    def calc_single(self, database):
        ...
        amt = database.depend_data['FactorData.Basic_factor.amt_minute'].iloc[-120:,:]
        volume = database.depend_data['FactorData.Basic_factor.volume_minute'].iloc[-120:,:]

        volume = volume.replace(0.,np.nan)
        vol = volume.values
        am = amt.values
        top = (vol > np.nanpercentile(vol,90,axis=0))
        tail = (vol < np.nanpercentile(vol,10,axis=0))
    ...
    def  reform(self, temp_result):
        A = temp_result.rolling(5,1).max() - temp_result
        return -A.rolling(5,1).mean()
