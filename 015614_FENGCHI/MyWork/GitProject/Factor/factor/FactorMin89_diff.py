...
class FactorMin89_diff(BaseFactor):
    #  定义因子参数
    # 因子频率，。默认为日频因子， 可不设置
    factor_type = "FIX"
    ...
    def calc_single(self, database):
        ...
        ar_temp=pd.DataFrame((data2.values-data4.values)/(data4.values-data3.values),columns=data1.columns,index=data1.index)      
        
        ##系数如果是无穷的话就赋值为0
        ar_temp[np.isinf(ar_temp)]=np.nan
