...
import numpy as np
import pandas as pd
class IndustryExcessReturnStd5d(BaseFactor):
    # 因子频率，。默认为日频因子， 可不设置
    factor_type = "FIX"
    # 依赖的平台原始数据，包括FactorData和MarketData接口中的数据。 默认为空，必须设置
    depend_data = ["FactorData.Basic_factor.close_adj_minute","FactorData.Basic_factor.sw_indcode1"]
    ...
    def calc_single(self, database):
        ...
        factor = self.minute_help(MinuteClose,industry_code_all)
        return factor
    
    def reform(self,temp_result):
        factor = temp_result
        res = -factor.rolling(5).std()
        return res   
    def minute_help(self ,MinuteClose,industry_code_all):
        ...
        tmp = pd.concat([industry_today, re], axis = 1)
