...
class FactorMin227_meandivstd(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        def max_second(x):           
            ...
            if len(x)>0:
               ...
               x_max=max(x)
               x_min=second(x)
               if ((not x_max==None) & (not x_min ==None)):
                  index_max=x.index(x_max)/len(x)
                  index_min=x.index(x_min)/len(x)
                  index_max_min=index_max-index_min
        ...
        ##last
        data1_ms=data1.apply(max_second) 
