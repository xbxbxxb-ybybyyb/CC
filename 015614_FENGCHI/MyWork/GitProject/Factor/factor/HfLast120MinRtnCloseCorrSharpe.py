...
class HfLast120MinRtnCloseCorrSharpe(BaseFactor):
    factor_type = "FIX"
    ...
    def calc_single(self, database):
        ...
        low_5min = low.rolling(window=5, min_periods=1).min()
        minute_min_rtn = np.log(close.values / low_5min.values)
        ans = - self.array_coef(minute_min_rtn[-120:], close.values[-120:])
