...
class FactorMin218_diffdivstd(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        ##成交量大于均值
        data2_up=data2>data2.mean()
        
        ##成交量小于均值
        data2_down=data2<data2.mean()
        
        data2_up_sum=data2_up.sum()
        
        data2_down_sum=data2_down.sum()
        
        ans =data2_up_sum/data2_down_sum
                                        
        return ans
        
# 针对播放后的结果，进行相关的rolling等操作。所用的前序数据长度应为reform_window。默认不修改temp_result， 可不重写。
    def reform(self, temp_result):
        return Util.rolling_process(temp_result,ptype = 'diffdivstd',window=self.reform_window)
