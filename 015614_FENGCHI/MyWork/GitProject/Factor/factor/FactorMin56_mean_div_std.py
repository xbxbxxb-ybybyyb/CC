...
class FactorMin56_mean_div_std(BaseFactor):
    #  定义因子参数
    # 因子频率，。默认为日频因子， 可不设置
    factor_type = "FIX"
    ...
    def calc_single(self, database):
        ...
        iliq=pd.DataFrame(abs(data1_cal.values)*(10**6)/data2.values,columns=data1_cal.columns,index=data1_cal.index)
        
        
        iliq_up=iliq[pd.DataFrame(data1_temp>0, columns=data1.columns, index=data1.index) ]
        
        iliq_down=iliq[pd.DataFrame(data1_temp<0, columns=data1.columns, index=data1.index) ]  
        
        ans =iliq_up.std()/iliq_down.std()
