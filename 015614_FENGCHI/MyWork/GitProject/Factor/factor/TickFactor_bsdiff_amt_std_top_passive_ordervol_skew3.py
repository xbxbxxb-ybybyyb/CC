...
class TickFactor_bsdiff_amt_std_top_passive_ordervol_skew3(BaseFactor):
    #  定义因子参数

    # 因子频率，默认为日频因子， 可不设置
    factor_type = "FIX"
    # 依赖的平台原始数据，包括FactorData和MarketData接口中的数据。 默认为空，必须设置
    depend_data = ["FactorData.Basic_factor.passivebuyordervol_minute","FactorData.Basic_factor.passivesellordervol_minute","FactorData.Basic_factor.close_minute","FactorData.Basic_factor.amt_minute","FactorData.Basic_factor.volume_minute","FactorData.Basic_factor.high_minute","FactorData.Basic_factor.low_minute","FactorData.Basic_factor.limit_status_minute"]
    # 计算每个时点的因子所需要前移的数据窗口大小
    # 例如，为日频因子，lag=3表示计算某一日的因子值需要依赖前三个交易日和当日的数据，默认为0，可不设置
    lag = 1
    reform_window=5
    mini_factor='amt_std_top'

    theobject='passive_ordervol'
    sub_window=5
    stat='skew'
    n=3
    ...
    def calc_single(self, database):
        ...
        obj_buy_term,obj_sell_term = self.get_object_term()
        obj_buy = self.data_filter2(database.depend_data[obj_buy_term], limit_status, method='minute')
        obj_sell =  self.data_filter2(database.depend_data[obj_sell_term], limit_status, method='minute')

        amt_df =  self.data_filter2(database.depend_data['FactorData.Basic_factor.amt_minute'], limit_status, method='minute')
    ...
    def get_object_term(self):
        if '_' in self.theobject:
            object_part1 = self.theobject.split('_')[0]
            object_part2 = self.theobject.split('_')[1]
            object_buy = [i for i in self.depend_data if object_part1 in i and object_part2 in i and 'buy' in i][0]
            object_sell = [i for i in self.depend_data if object_part1 in i and object_part2 in i and 'sell' in i][0]
        else:
            object_buy = [i for i in self.depend_data if self.theobject in i and 'buy' in i][0]
            object_sell = [i for i in self.depend_data if self.theobject in i and 'sell' in i][0]
        return object_buy,object_sell
