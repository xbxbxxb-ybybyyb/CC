...
class HFPVCorrBias(BaseFactor):

    '''
    * 因子名：HFPVCorrBias_13h
    * 逻辑：最高价和最低价的差和均价相关性的偏度。高低差越大说明短期波动大，炒作可能性更高，价格和其相关性越小受其影响越小，获得超额概率越大。
    * 因子参数：分钟数据高低量额
    * 作者：xust
    * 日期：2019.7.12
    * 函数修改日期：尚未修改
    ...
    * 修改原因：尚未修改
    ...
    def reform(self, temp_result):
        df = temp_result
        df = (df - df.rolling(self.reform_window).mean()) / df.rolling(self.reform_window).std()
        return df
