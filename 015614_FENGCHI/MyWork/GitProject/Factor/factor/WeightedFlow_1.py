...
class WeightedFlow_1(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        minute30_close = minute_close.resample("30T", label="right").last().dropna(axis=0, how='all').iloc[-1,:].values
        ...
        minute30_high = minute_high.resample("30T", label="right").max().dropna(axis=0, how="all").iloc[-1,:].values
        ...
        minute30_low = minute_low.resample("30T", label="right").min().dropna(axis=0, how="all").iloc[-1,:].values
        minute_volume = data_filter(database.depend_data['FactorData.Basic_factor.volume_minute'],limit_status,method='minute')
        minute30_volume = minute_volume.replace([0], 0.001).resample("30T", label="right").sum().replace([0], np.nan).dropna(axis=0, how="all").iloc[-1,:].values

        ans_df = (minute30_high+minute30_close+minute30_low) / 3 * minute30_volume
