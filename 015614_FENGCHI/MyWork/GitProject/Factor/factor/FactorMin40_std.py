...
class FactorMin40_std(BaseFactor):
    #  定义因子参数
    # 因子频率，。默认为日频因子， 可不设置
    factor_type = "FIX"
    ...
    def calc_single(self, database):
        ...
        data1_up=data1[(data2_r>0)]
        
        data1_down=data1[(data2_r<0)]
...
# 针对播放后的结果，进行相关的rolling等操作。所用的前序数据长度应为reform_window。默认不修改temp_result， 可不重写。
    def reform(self, temp_result):
        temp_result_std=temp_result.rolling(self.reform_window).std()
        temp_result_std[['002102.SZ']]=np.nan
        return temp_result_std
