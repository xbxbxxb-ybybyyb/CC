...
class RetGather0p9mean5(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        threshold = ret_min.quantile(self.threshold,axis=0)
        filter = np.zeros(ret_min.shape)
        filter[ret_min.values - threshold.values >= 0] = 1
        part1 = pd.DataFrame((ret_min*filter/filter).values**2,index=ret_min.index,columns=ret_min.columns).sum(axis=0)
        part2 = pd.DataFrame(ret_min.values**2,index=ret_min.index,columns=ret_min.columns).sum(axis=0)
        ans = part1/part2
        return ans
