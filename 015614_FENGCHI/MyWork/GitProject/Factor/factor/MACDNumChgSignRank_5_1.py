...
class MACDNumChgSignRank_5_1(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        macd = macd[-237*5-1:, :]  # 取出n天的MACD柱

        macd_sign = (np.diff(np.sign(macd), axis=0) == 2) | (np.diff(np.sign(macd), axis=0) == -2)
        alpha = np.nansum(macd_sign, axis=0)  # 这里出现的很多0是由于macd全都是nan导致的
