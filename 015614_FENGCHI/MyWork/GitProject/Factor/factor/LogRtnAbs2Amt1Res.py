...
class LogRtnAbs2Amt1Res(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        mclose5m = mclose.iloc[-237:, :].resample('5T').asfreq().dropna(axis=0, how='all')  # 取出5min的close
        mrtns5m = mclose5m.values[1:, :] / mclose5m.values[:-1, :] - 1    # 5min收益率
        rtns = np.log(np.prod(np.abs(mrtns5m) + 1, axis=0))  # 累积对数收益率（绝对值）
        mamt5m = np.nansum(mamt.values[-237:, :], axis=0)  # 求n天成交额
