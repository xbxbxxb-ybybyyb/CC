...
"""
* 因子名：MinPriceBeta
* 因子功能描述：计算分钟最高价与最低价价格比率与收盘价之间的beta，是一个反转因子，该值越大则越容易往下跌
...
class MinPriceBeta(BaseFactor):

    factor_type = 'FIX'             # 声明因子类型为FIX
    depend_data = ['FactorData.Basic_factor.high_minute','FactorData.Basic_factor.low_minute','FactorData.Basic_factor.close_minute']    # 声明因子计算需要依赖的数据字段，必需设置
    ...
    def calc_single(self, database):
        ...
        close_df = database.depend_data['FactorData.Basic_factor.close_minute'].rank(axis=1)
        ...
        hl_df = (high_df/low_df).rank(axis=1)
        hf_close_corr = Util.array_coef(close_df, hl_df)
        result = -hf_close_corr*hl_df.std(axis=0)/close_df.std(axis=0)
        return result
    ...
    #     close_df = MinuteClose.loc[compute_date].rank(axis=1)
    #     high_df = MinuteHigh.loc[compute_date]
    #     low_df = MinuteLow.loc[compute_date]
    #     hl_df = (high_df/low_df).rank(axis=1)
    #     result = hl_df.corrwith(close_df,axis=0)*hl_df.std(axis=0)/close_df.std(axis=0)
    #     return result