...
resample_date = {2017: [20170101, 20171231],
                 2018: [20180101, 20180231],
...
sw1_code_dict = {'801010.SI': '农林牧渔',
                 ...
                 '801880.SI': '汽车',
                 '801890.SI': '机械设备'}
sw1_code_list = list(sw1_code_dict.keys())
...
sw_factor = fd.get_factor_value('WIND_ASWSindexEOD', ['S_DQ_PRECLOSE', 'S_DQ_CLOSE'],
                                S_INFO_WINDCODE=list(sw1_code_dict.keys()),
                                TRADE_DT=['>=20160101'])
...
# 申万一级行业指数21天波动率
yearly_sw_std = pd.DataFrame(index=resample_date.keys(), columns=sw1_code_list)
...
yearly_sw_std = yearly_sw_std.rename(columns=sw1_code_dict)
...
yearly_ind_std = pd.DataFrame(index=pd.MultiIndex.from_product([['mean', 'count', '10%分位数', '30%分位数', '50%分位数', '70%分位数', '90%分位数'], list(resample_date.keys())]), columns=indName.sw_level1.keys())
...
yearly_ind_std = yearly_ind_std.rename(columns=indName.sw_level1)
yearly_ind_std = yearly_ind_std.swaplevel(0, 1)

output_dict = {'申万一级行业指数21天波动率': yearly_sw_std,
               '申万一级成分股21天波动率情况': yearly_ind_std}

util.save_dict2xls(output_dict, other_stats_path, '行业波动率统计结果.xlsx')
