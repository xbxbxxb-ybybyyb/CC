...
from SimiStock.dataApi.stockList import trans_windcode2int, trans_int2windcode
from SimiStock.dataApi.indName import sw_level1, citics_level1
from SimiStock.dataApi.tradeDate import get_trade_date_interval, trans_datetime2int, get_minute_index, get_recent_trade_date, \
...
def get_minute_1factor(factor, start_datetime=None, end_datetime=None, minute_interval=1, code_list=None,
    ...
    if isinstance(end_datetime, str):
        if end_datetime.isdigit():
            end_datetime = int(end_datetime)
    base_start = trade_minutes[1 + minute_interval] if minute_interval > 1 else 925
    ...
    start_date = start_datetime // 10000 if start_datetime / 100000000 >= 1 else start_datetime
    end_date = end_datetime // 10000 if end_datetime / 100000000 >= 1 else end_datetime
    date_list = get_date_range(start_date, end_date)
    start_time = start_datetime % 10000 if start_datetime / 100000000 >= 1 else base_start
    end_time = end_datetime % 10000 if start_datetime / 100000000 >= 1 else 1500
    if start_time < date_list[0]:
        start_datetime = date_list[0] * 10000 + base_start
    else:
        start_datetime = start_date * 10000 + start_time
    if end_date > date_list[-1]:
        end_datetime = date_list[-1] * 10000 + 1500
    else:
        end_datetime = end_date * 10000 + end_time
