...
class SimiMethod1(SimiMethodRollingBase):
    ...
    def simi_strategy(self, stk_id, date_tuple_list):
        ...
        for date_tuple in date_tuple_list:
            ...
            # corr_copy = corr.reindex(index=concept_list).copy()
            # corr = self.arr_min_max_transfer(corr.values)
            ...
            if first_flag:
                if corr_stats['大于基础阈值次数'].sum() == 0:  # 若不满足基本条件，返回空
                    return list()
                else:
                    first_flag = False
            ...
            if manzu_list:
                for manzu in manzu_list:
                    if 0.8 < s_corr[manzu] < corr_base_low:
                        manzu_idx = s_corr.index.tolist().index(manzu)

            tmp_list2 = s_corr[(s_corr > 0.8) & (s_corr <= corr_high)].index.tolist()
...
if __name__ == '__main__':
    ...
    for param in param_list:
        ...
        save_name = f'{filename}_part2_result.pkl'
