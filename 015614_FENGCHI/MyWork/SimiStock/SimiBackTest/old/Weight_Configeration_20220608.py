...
class WeightedConfiguration(object):
    ...
    def stock_statisc_data(self, code, trade_date, ind='old'):
        ...
        # 获取对应行业
        concept_name = indName.sw_level1[self.concept_df.loc[cal_date, code]] + '（SW）'
        # 获取观测日期
        ...
        for type_way in ['similarity', 'pct_chg', 'turn', 'fluc']:
            ...
            if type_way == 'fluc':
                ...
                result.loc['中期行业个股波动率'] = self.middle_flu.loc[:cal_date, similarity_list].iloc[-10:].mean().mean()
                ...
                result.loc['短期行业个股波动率'] = self.short_flu.loc[:cal_date, similarity_list].iloc[-10:].mean().mean()
    ...
    def Result_Test(self, date, Original_code, para, way, test_name, profit, mode=['real', 'var'], index=False):
        ...
        Result.loc['if_trade', test_name] = self.Risk_Estimation(history_trade.loc[before_trade_days], profit,
                                                                 mode=mode)
    ...
    def Test_Difference_Time(self, factor_name, func, pca, same_weight, before_days, way, mode, num_list,
        ...
        if statistic == True:
            ...
            # middle_ind = ['基础化工', '医药生物', '电力设备', '传媒', '食品饮料', '国防军工', '轻工制造', '交通运输', '机械设备', '计算机','电子','农林牧渔','有色金属','通信']

            # 确定一共有多少个行业
            # 最好的行业：剔除后，主动参与组+剩余部分筛选参与组
            select_buy_best_result = ind_select(select_buy_result_discount, best_ind, SW='old')
            other_best_result = ind_select(other_result_discount, best_ind, SW='old')
            # 中间的行业，提出后，筛选参与组
            after_del_middle_result = ind_select(after_del_result_discount, middle_ind, SW='old')  # 全部行业都要用的
    ...
    def Get_Real_Trade(self, Data_Result):
        # 原始数据，用于获取成交金额
        ...
        # 样本拆分3：根据参与情况筛选
        best_ind = ['公用事业', '家用电器', '建筑装饰', '房地产', '汽车', '商业贸易', '采掘', '银行', '非银金融', '钢铁']
        middle_ind = ['化工', '医药生物', '电气设备', '传媒', '食品饮料', '国防军工', '轻工制造', '交通运输', '机械设备', '计算机', '电子', '农林牧渔', '有色金属',
                      '通信']
        # 新行业
        # best_ind = ['非银金融','公用事业','汽车','房地产','建筑装饰','商贸零售','钢铁','煤炭','银行','家用电器']
        # middle_ind = ['基础化工', '医药生物', '电力设备', '传媒', '食品饮料', '国防军工', '轻工制造', '交通运输', '机械设备', '计算机','电子','农林牧渔','有色金属','通信']
        # 最好的行业：剔除后，主动参与组+剩余部分筛选参与组
        select_buy_best_result = ind_select(select_buy_result_discount, best_ind, SW='old').reset_index().drop('index',
                                                                                                               axis=1)
        other_best_result = ind_select(other_result_discount, best_ind, SW='old').reset_index().drop('index', axis=1)
        # 中间的行业，提出后，筛选参与组
        after_del_middle_result = ind_select(after_del_result_discount, middle_ind, SW='old').reset_index().drop(
            'index', axis=1)  # 全部行业都要用的

        # 如果是主动参与组，那么就直接全部参与
        best_money = [1500, 4500]
        use_buy_money = [1000, 3000]
        select_money = [500, 1500]
        model_use = ['5_average', '5_meanvar', '5_curve']
        ...
        real_buy = real_buy[
            ['code', 'trade_date', 'discount', 'future_error', 'real_buy_money', 'single_money', 'max_money']]
        ...
        ############################ 统计结果 ###################################
        single_way_buy = real_buy[real_buy['max_money'] == 3000]
        single_way_buy = real_buy.copy()
        # 计算日均占资问题
        ...
        # 计算个股次数占比情况
        buy_num = real_buy['code'].value_counts().sort_values()
        buy_num.value_counts()
        (buy_num <= 3).sum()
        (buy_num >= 10).sum()

        real_buy[real_buy['code'].isin(buy_num[buy_num >= 10].index.to_list())]['real_buy_money'].sum()
        real_buy[real_buy['code'].isin(buy_num[buy_num >= 10].index.to_list())]['profit'].sum()
    ...
    def New_TradeOrNot(self, prepare_date, func, pca, same_weight, before_days=120, mode=['trade_real', 'var'],
        ...
        new_best_ind = ['非银金融', '公用事业', '汽车', '房地产', '建筑装饰', '商贸零售', '钢铁', '煤炭', '银行', '家用电器']
        new_middle_ind = ['基础化工', '医药生物', '电力设备', '传媒', '食品饮料', '国防军工', '轻工制造', '交通运输', '机械设备', '计算机', '电子', '农林牧渔',
                          '有色金属', '通信']
...
# 运行普通的结果：
# factor_name = '新版本_14_(0.6, 1)_(0.8, 1)_(120, 120)_95_20170101_20201231_result'
# Data_Result, statistic_data = self.Rol_AllResult(factor_name, choice, func, pca, same_weight,before_days,way, mode,num_list
#                      ,run='multi',save_name='run')

# 运行普通结果+年度结果（或者特定行业结果）
factor_name = "报告用老版本_14_(0.6, 1)_(0.7, 1)_(120, 120)_95_20170101_20201231_result"
# factor_name = "报告用老版本_14_(0.6, 1)_(0.7, 1)_(120, 120)_95_20210101_20211031_result"
before_days = 120
# for before_days in [30,60,90,120,150,180,210,240]:
self.Test_Difference_Time(factor_name, func, pca, same_weight, before_days, way, mode, num_list, choice, statistic)
"""
########################## 新出的跟踪大宗结果 ####################################
prepare_date = 20220527  # 昨天，比如今天上午收到大宗，这个日期为昨天
if_new = True  # if_new表示含义：即是否重新输出结果，还是用以前的加总
...
New_Result = self.New_TradeOrNot(prepare_date, func='error', pca=False, same_weight=True, before_days=120,
