...
class WeightedConfiguration(object):
    ...
    def cal_history_result(self, history_data, func, pca, same_weight, before_days,way,mode, num_list=[2, 3, 4, 5, 6, 7, 8, 9]):
        ...
        for num in num_list:
            if len(similarity_list) >= num:
                ...
                all_result[fit_name1] = self.Result_Test(date=trade_date, Original_code=Original_code, para=fit_weighted1, way=way, test_name=fit_name1, profit=profit, mode=mode)
                # 第二种：均值-方差法
                try:
                    ...
                    all_result[fit_name2] = self.Result_Test(date=trade_date, Original_code=Original_code, para=fit_weighted2, way=way, test_name=fit_name2, profit=profit, mode=mode)
                except:
    ...
    def TradeOrNot(self,prepare_date,func,pca, same_weight,before_days=120,mode=['trade_real', 'var'],if_new=False):
        ...
        if os.path.exists(txTest_path + factor_name + '.pkl') == True:
...
########################## 新出的跟踪大宗结果 ####################################
prepare_date = 20220505 # 昨天，比如今天上午收到大宗，这个日期为昨天
