...
class SimiMethod1(SimiMethodRollingBase):
    ...
    @staticmethod
    def maxdrawdown(arr):
        tmp_arr = arr.bfill().ffill()
        i = np.argmax((np.maximum.accumulate(tmp_arr) - tmp_arr) / np.maximum.accumulate(tmp_arr))  # end of the period
        j = np.argmax(tmp_arr[:i])
        return 1 - tmp_arr[i] / tmp_arr[j]

    def get_trend_score(self, stk_id, train_date_list, concept_list):
        close = getData.get_daily_1factor('close_badj', date_list=train_date_list)
        pre_close = getData.get_daily_1factor('pre_close_badj', date_list=train_date_list)
        stk_close = close[stk_id]
        other_stk_close = close[concept_list]

        stk_ytm = (1 + self.pctchg.loc[train_date_list, stk_id] / 100).cumprod().iloc[-1]
        other_stk_ytm = (1 + self.pctchg.loc[train_date_list, concept_list] / 100).cumprod().iloc[-1].values
        ytm_dist = abs(other_stk_ytm - stk_ytm)

        stk_pct_std = self.pctchg[stk_id].std()
        other_stk_pct_std = self.pctchg[concept_list].std().values
        std_dist = abs(other_stk_pct_std - stk_pct_std)

        stk_mdd = self.maxdrawdown(stk_close)
        other_stk_mdd = np.array([self.maxdrawdown(close[other_stk]) for other_stk in concept_list])
        mdd_dist = abs(other_stk_mdd - stk_mdd)

        stk_ret = close.loc[train_date_list[-1], stk_id] / pre_close.loc[train_date_list[0], stk_id] - 1
        stk_pct_mean = self.pctchg[stk_id].mean()
        stk_pct_std = self.pctchg[stk_id].std()
        other_stk_ret = other_stk_close.loc[train_date_list[1], concept_list] / \
                        pre_close.loc[train_date_list[0], concept_list] - 1
        other_stk_pct_mean = self.pctchg[concept_list].mean()
        other_stk_pct_std = self.pctchg[concept_list].std()

        stk_zscore = (stk_ret - stk_pct_mean) / stk_pct_std
        other_stk_corr = (other_stk_ret - other_stk_pct_mean) / other_stk_pct_std
        single_corr = (stk_zscore * other_stk_corr * 2 / (stk_zscore ** 2 + other_stk_corr ** 2)).values

        trend_corr = -self.arr_min_max_transfer(ytm_dist) - self.arr_min_max_transfer(std_dist) - \
                     self.arr_min_max_transfer(mdd_dist) + self.arr_min_max_transfer(single_corr)
        trend_corr = pd.DataFrame(trend_corr, index=concept_list).reindex(self.barra_code_list).values
        return self.arr_min_max_transfer(trend_corr[:, 0])
    ...
    def simi_strategy(self, stk_id, date_tuple_list):
        ...
        for date_tuple in date_tuple_list:
            ...
            trend_score = self.get_trend_score(stk_id, date_list, concept_list)

            ret = corr * weight_list[0] + trend_score * weight_list[1] + barra_score * weight_list[2]
...
if __name__ == '__main__':
    param_dict = {'method_name': ['叠加走势相似度'],
                  'concept': ['SW1'],
                  'hedge_max_num': [7],
    ...
    for param in param_list:
        ...
        # result = smt1.get_hedge_list(mode='multi', kernal_num=cpu_count())
        # result = smt1.get_hedge_list(mode='serial', kernal_num=cpu_count())
        result = smt1.get_simi_stock(601009, 20170324)
