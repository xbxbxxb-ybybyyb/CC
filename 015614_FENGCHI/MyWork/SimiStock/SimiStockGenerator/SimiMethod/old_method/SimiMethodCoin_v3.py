...
class SimiMethod1(SimiMethodRollingBase):
    ...
    def get_coint_score(self, stk_id, train_date_list, concept_list):
        close = getData.get_daily_1factor('close_badj', date_list=train_date_list)
        s_coint = pd.Series(index=concept_list)
        for stk in concept_list:
            stk_close = close[stk_id].ffill().bfill()
            other_stk_close = close[stk].ffill().bfill()
            _, pvalue, _ = sm.tsa.stattools.coint(stk_close, other_stk_close)
            if pvalue > 0.1:
                continue
            s_coint[stk] = pvalue
        s_coint = s_coint.reindex(self.barra_code_list).values
        coint_values = self.arr_min_max_transfer(s_coint)
        return coint_values
    ...
    def simi_strategy(self, stk_id, date_tuple_list):
        ...
        for date_tuple in date_tuple_list:
            ...
            coint_score = self.get_coint_score(stk_id, date_list, concept_list)

            barra_score = self.get_barra_score(stk_id, calc_date, concept_list)

            ret = coint_score * weight_list[0] + barra_score * weight_list[1]
            ret_s = pd.Series(ret, index=self.barra_code_list)

            ret_s = ret_s[concept_list].dropna().sort_values(ascending=False)
