...
计算每个样本当期120天的最大相似度
20220425:
对所有股票前10日相关性的均值排序，选前5名，这五名在未来120日的前10日相关性的均值，分别对应求差值（带正负号），
计算偏离均值，以及偏离最小值（最大负偏离），以及分行业统计
"""
...
class SimiMethodTest(SimiMethodRollingBase):
    ...
    def simi_strategy(self, stk_id, date_tuple_list):
        ...
        for date_tuple in date_tuple_list:
            ...
            history_corr = pd.DataFrame(corr_stats['_mean'][hedge_list], index=hedge_list)
            _, future_corr_stats = self.judge_corr_stability(stk_id, start_date, end_date,
                                                             concept_list, pre_times=self.corr_rolling_nums)
            future_corr = pd.DataFrame(future_corr_stats['_mean'][hedge_list], index=hedge_list)
            corr_diff = future_corr - history_corr
            corr_diff_mean = corr_diff.mean()[0]
            corr_diff_min = corr_diff.min()[0]
            corr_diff_max = corr_diff.max()[0]
            diff_num = len(corr_diff.query('_mean < -0.1'))
            diff_perc = diff_num / self.top_n
            append_dict = {'calc_date': calc_date,
                           'start_date': start_date,
                           'end_date': end_date,
                           'hedge_list': stk_list,
                           'hedge_value': [self.top_n, corr_diff_mean, corr_diff_min, corr_diff_max,
                                           diff_num, diff_perc]}
...
if __name__ == '__main__':
    ...
    for param in param_list:
        ...
        save_name = f'{filename}_corrResultV2.pkl'
