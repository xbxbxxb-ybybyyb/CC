...
class SimiMethodTest(SimiMethodRollingBase):
    ...
    def simi_strategy(self, stk_id, date_tuple_list):
        ...
        for date_tuple in date_tuple_list:
            ...
            history_stk_list = corr_stats['_mean'].sort_values(ascending=False).index.tolist()[:self.top_n]
            _, future_corr_stats = self.judge_corr_stability(stk_id, start_date, end_date,
                                                             concept_list, pre_times=self.corr_rolling_nums)
            future_stk_list = future_corr_stats['_mean'].sort_values(ascending=False).index.tolist()[:self.top_n]

            common_stk_list = list(set(history_stk_list).intersection(set(future_stk_list)))
            append_dict = {'calc_date': calc_date,
                           'start_date': start_date,
                           'end_date': end_date,
                           'hedge_list': common_stk_list,
                           'history_stk_list': list(history_stk_list),
                           'future_stk_list': list(future_stk_list),
                           'hedge_value': [self.top_n, len(common_stk_list), len(common_stk_list) / self.top_n]}
...
if __name__ == '__main__':
    ...
    for param in param_list:
        ...
        save_name = f'{filename}_corrResultV4.pkl'
