...
"""
20220518:
去掉之前K线相似度中走势相似度不合理的那块，并将涨跌幅修改为价格的相似性
对价格进行净值化处理
"""
...
class SimiMethod1(SimiMethodRollingBase):
    ...
    def get_kline_score(self, stk_id, train_date_list, concept_list):
        pre_close_badj = getData.get_daily_1factor('pre_close_badj', date_list=train_date_list)
        close = getData.get_daily_1factor('close_badj', date_list=train_date_list) / pre_close_badj - 1
        open = getData.get_daily_1factor('open_badj', date_list=train_date_list) / pre_close_badj - 1
        high = getData.get_daily_1factor('high_badj', date_list=train_date_list) / pre_close_badj - 1
        low = getData.get_daily_1factor('low_badj', date_list=train_date_list) / pre_close_badj - 1
...
if __name__ == '__main__':
    param_dict = {'method_name': ['2nd叠加K线相似度3'],
                  'concept': ['SW1'],
                  'hedge_max_num': [7],
...
#         end_date_ = 20210930
        ...
        # result = smt1.get_simi_stock(601198, 20180919)
