...
def backtest_1method(filename, output_name=None, duration=120, hedge_num=None, start_date=20180101, end_date=20200631,
    ...
    method_name = filename_param[0]
    weight_kind = filename_param[3]
    hedge_max_num = filename_param[1]
    corr_threshold = filename_param[2]
    ...
    summary['大于0.65胜率'] = df.groupby(groupby_columns).apply(lambda x: x['大于0.65个数'].sum() / x['总个数'].sum())
    summary['大于0.6胜率'] = df.groupby(groupby_columns).apply(lambda x: x['大于0.6个数'].sum() / x['总个数'].sum())
    summary['method_name'] = method_name
    summary['weight_kind'] = weight_kind
    summary['hedge_num'] = hedge_num
    summary['hedge_max_num'] = hedge_max_num
    summary['corr_threshold'] = corr_threshold
