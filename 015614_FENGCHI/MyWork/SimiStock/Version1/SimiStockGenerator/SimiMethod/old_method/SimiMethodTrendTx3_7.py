...
class SimiStyleMethod(SimiMethodBase):
    ...
    def simi_strategy(self, stk_id, trade_date, concept_list):
        if self.base_sample and ((trade_date, stk_id) not in self.base_sample):
            return trade_date, trade_date, list(), list(), list()
        old_trade_date = trade_date
        bad_date_list = tradeDate.get_date_range(20180101, 20182131)
        trade_date = bad_date_list[np.random.randint(len(bad_date_list))]
        ...
        if len(ret_list) > 0 and corr_low < s_corr.iloc[0] <= corr_high:
            return trade_date, old_trade_date, ret_list, [1] * len(ret_list), ret_s[ret_list].tolist()
        else:
            return trade_date, old_trade_date, list(), list(), list()
...
#                   'base_file': [(6, 7)],
    ...
    for param in param_list:
        ...
        weight_kind = param[5]
        base_file = param[6]
        base_name = f'{method_name}_{hedge_max_num}_{base_file}_{weight_kind}_95_20200101_20201231_result.pkl'
        ...
        save_name = f'{method_name}_{hedge_max_num}_{corr_threshold}_{weight_kind}_95_{start_date}_{end_date}_to2018_result.pkl'
