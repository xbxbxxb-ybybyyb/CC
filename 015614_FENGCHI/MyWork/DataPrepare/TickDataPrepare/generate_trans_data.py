...
date_code = [x[:-4] for x in os.listdir('/arch1/group/800442/800319/LimitTickData2/ReceiveDelay/')]
date_code = list(filter(lambda x: '20210731' >= x[9:16] >= '20210301', date_code))
date_code_list = sorted([x[-8:] + x[:9] for x in date_code])
date_code_list = date_code_list
...
#     os.makedirs('/data/group/800442/800319/LimitTradeData/%s' % item)
...
def prepare_trade_data(date_code, md):
    ...
    for item in _trade_items:
        np.save('/data/group/800442/800319/LimitTradeData/%s/%s%s' % (item, _code, _date), eval(item))
...
def _get_trade_1dc(date, code, address='/data/group/800442/800319/LimitTradeData/'):
...
def get_transaction_data(code, date, address='/data/group/800442/800319/LimitTradeData/'):
