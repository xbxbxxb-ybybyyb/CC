...
"""
price在日内vwap均线上方的净面积
"""
import pandas as pd

#TTransaction(逐笔成交类因子)示例 Todo:注意TTransaction类因子需要控制低耗时
def factor_fc_v_z_p_area(transaction_df, return_fillna_dic=False):
    factor_name = 'fc_v_z_p_area'
    ...
    v_z_area = (transaction_df['TradePrice'] / vwap - 1).sum()  # 未考虑CYB
...
if __name__ == '__main__':
    ...
    factor_df=factor_fc_v_z_p_area(start_date,end_date,IO)
    factor_path = '/data/user/015614/factor/'
    factor_df.to_hdf(factor_path + 'fc_v_z_p_area.h5', key='fc_v_z_p_area', mode='w')
