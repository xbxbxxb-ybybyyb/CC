...
# Date ：2023/3/13 9:54
"""
price高于vwap的trade中的数量占比
"""
import pandas as pd

#TTransaction(逐笔成交类因子)示例 Todo:注意TTransaction类因子需要控制低耗时
def factor_fc_v_z_ts_p(transaction_df, return_fillna_dic=False):
    factor_name = 'fc_v_z_ts_p'
    ...
    v_z_area = (transaction_df['TradePrice'] >= vwap).mean()
...
if __name__ == '__main__':
    import IO
    start_date, end_date=20181201, 20181231
    factor_df=factor_fc_v_z_ts_p(start_date,end_date,IO)
    factor_path = '/data/user/015614/factor/'
    factor_df.to_hdf(factor_path + 'fc_v_z_ts_p.h5', key='fc_v_z_ts_p', mode='w')
