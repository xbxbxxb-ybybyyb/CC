...
class FactorParamSearch:
    ...
    def wrapper(self, param_list):
        for param_tuple in tqdm(param_list):
            ...
            factor_df = run_factor(self.calc_factor, factor_name, 'Next1mTransaction',
    ...
    @staticmethod
    def calc_factor(transaction_df, param_tuple, return_fillna_dic=False):
        factor_name = str(param_tuple)
        ...
        time1 = fun_get_time(int(ul_time), -param1)
        time2 = fun_get_time(int(ul_time), -param2)
        transaction_df1 = transaction_df.query(f'MDTime >= {time1}')
        transaction_df2 = transaction_df.query(f'MDTime >= {time2}')
        ret = transaction_df2['TradeMoney'].sum() / (transaction_df1['TradeMoney'].sum() - transaction_df2['TradeMoney'].sum())
...
if __name__ == '__main__':
    note = 'dig_Next1mTransaction1'
    fps = FactorParamSearch(note=note)
    param_list = [[10, 12, 15, 20, 24, 30, 40, 50, 60],
                  [3, 6, 9, 12, 15, 20, 30, 40, 50, 60], ]
