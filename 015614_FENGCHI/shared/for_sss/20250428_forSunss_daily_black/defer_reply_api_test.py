...
if len(issuing_stock_list) > 0:
    ...
    for stock in issuing_stock_list:
        if stock[-2:] not in ['SZ', 'SH', 'BJ']:
            continue
        info1 = nd.getNewsInfoByStockCode(stock[:-3], year_list=[year], data_source='TNEWS')
        info2 = nd.getAnnouncement(stock, start_date=str(last_year_period), end_date=str(lastdate))
        info2 = info2[info2['ENTRYTIME'].map(lambda x: x.to)]
        ...
        info1 = info1[info1['textcategory'].astype('str').str.startswith('2')]
        if len(info) == 0:
            continue
