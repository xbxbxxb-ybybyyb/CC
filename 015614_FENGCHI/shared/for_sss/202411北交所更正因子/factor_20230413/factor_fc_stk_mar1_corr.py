...
def factor_fc_stk_mar1_corr(start_date, end_date, IO, return_fillna_dic=False):
    factor_name='fc_stk_mar1_corr'

    if return_fillna_dic:
        return {factor_name: 0, 'data': ['MD', 'AIndexEODPrices']}
    # 计算全部股票在全部时间区间上的因子值，之后会在run_factor_demo函数中进行向后平移一天和样本的筛选
    # -------------------------------------------------------------------------------------------------------------------
    start_date_ = int(s.tradingday(str(start_date), -100)[0])
    ...
    ret[mask] = ret[mask] / 2
    mask2 = ret.index.map(lambda x: (x[1][-2:] == 'BJ'))
    ret[mask2] = ret[mask2] / 3
    ...
    start_date = s.tradingday(start_date, -2)[0]  # 往前多算一天
    for dat in s.tradingday(start_date, end_date):
        ...
        ret_market_var = np.nanvar(tmp_market.values[-40:], axis=0)
