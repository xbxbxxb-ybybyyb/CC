...
if __name__ == "__main__":
    ...
    import time
    import os
    #(os.path.exists('/data/group/800463/日内强势股/log_parse/因子耗时/实盘触发标签汇总New_%s.xlsx'%Adate) == False) |\
    while (os.path.exists('/data/group/800463/日内强势股/log_parse/因子耗时/实盘触发标签汇总New_%s.xlsx'%Adate) == False) |\
            (os.path.exists('/data/group/800463/日内强势股/实盘分析记录/日内强势股总买入记录/日内强势股总买入记录New-%s.xlsx'%(date))==False) |\
          (os.path.exists('/data/group/800463/日内强势股/实盘分析记录/日内强势股总卖出记录/日内强势股总卖出记录New-%s.xlsx'%(date))==False):
        print('calESRate-v4New等待标签汇总中')
        time.sleep(60)
