...
def interval_trade(sell_vol, vol_pct, tick_df, dl_price, ul_price):
...
def cal_sell_trade_result(mdp, nextday, mdp_code, sell_vol, last_close, date_list, vol_pct, sell_minute_interval=5):
    ...
    try:
        close_price = tick_md_df['LastPx'][tick_md_df['LastPx']!=0].iloc[-1]
    except IndexError:
        close_price = tick_md_df['LastPx'].iloc[-1]
    if (sell_vol <= 0.0001):#已经卖完了
        return open_price / pre_close - 1, 0, [], [-1], [], 1
    ...
    for i in range(len(minute_list)-1):
        ...
        vol_list, price_list, tot_acculated_vol, touch_ul_tmp = interval_trade(tot_acculated_vol+normal_vol_list[i], sell_pct, interval_tick_data, dl_price, ul_price)
    ...
    if (sum(trade_vol_list) - sell_vol)> -1: #买到足够量
        touch_ul = -1
        return pct_chg_t1, 1, [nextday], [touch_ul], [sell_vol], 1
    else:
        ...
        next_pct_chg_t1, next_sell_len, next_sell_date_list, touch_ul_next_list, sell_vol_list, finish_indicator = cal_sell_trade_result(mdp, next_nextday, next_mdp_code, next_sell_vol, next_last_close,
                                                date_list, vol_pct)
        today_pct_chg = close_price / pre_close - 1
        return (pct_chg_t1 * (sum(trade_vol_list) / sell_vol)) + (
                    (1 + today_pct_chg) * (1 + next_pct_chg_t1) - 1) * (1 - sum(trade_vol_list) / sell_vol), 1 + next_sell_len, [nextday] + next_sell_date_list, [touch_ul] + touch_ul_next_list, [sum(trade_vol_list)]+sell_vol_list, finish_indicator
...
def cal_LabelProfit_zt(stock, date, ZT_Time, mdp, param={}):
    ...
    result['pct_t1'], result['sell_length'], result['sell_date'] , result['touch_ul'] , result['sell_vol'], result['finish_indicator']= \
