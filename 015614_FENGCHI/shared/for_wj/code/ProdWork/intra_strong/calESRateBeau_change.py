...
def get_buyDfTotalInfo(buyDf,Adate,strategy='jupiter'):
    ...
    buyDf['成交笔数/挂单笔数'] = buyDf.apply(lambda x: x['买入当日成交笔数（o32）']/x['买入当日挂单笔数（o32）'] if x['买入当日挂单笔数（o32）']>0 else np.nan,axis=1)
    ...
    #buyResDf.loc['日均买入当日收益率'] = buyDf.groupby('发生日期')['买入当日收益率(%)'].mean().mean()
    buyResDf.loc['次均成交额均值'] = buyDf[buyDf['成交数量']>0]['成交金额'].mean()
    #buyResDf.loc['日均成交额均值'] = buyDf.groupby('发生日期')['成交金额'].sum().mean()
    # buyResDf.loc['次均成交笔数/挂单笔数'] = '%.2f%%'%(buyDf['成交笔数/挂单笔数'].mean()*100)
    buyResDf.loc['10点前突破比例'] = '%.2f%%'%(len(buyDf[buyDf['买入当日突破时间'].apply(lambda x:int(x))<100000000])/len(buyDf)*100)
...
#    --------------------------------------------------画图------------------------------------------------
    ...
    #Average_Daily_Buy_Money['Date'] = Average_Daily_Buy_Money.index.tolist()
    #Average_Daily_Buy_Money['Date'] = [dt.strptime(d,'%Y/%m/%d').date() for d in pd.to_datetime(Average_Daily_Buy_Money['Date']).tolist()]
    ...
    # Average_Daily_Buy_Money.plot(kind = 'bar',figsize=(fig_width, fig_height))
    # ---------------------------让坐标看得清---------------------------
    ax = plt.gca()
    ...
    DPS.hold(True)
    if strategy == 'jupiter':
        Daily_lzt_Positive_Stocks = pd.DataFrame(buyDf[(buyDf['前涨停信号']!=0) & (buyDf['前涨停信号'].notnull())].groupby('发生日期').apply(len)).reindex(Daily_Positive_Stocks.index)#(buyDf.groupby('发生日期').apply(len).index)
    ...
    DPS.legend(loc='best')
    # Daily_Positive_Stocks.plot(kind = 'bar',figsize=(fig_width, fig_height))
    # ---------------------------让坐标看得清---------------------------
    ax = plt.gca()
    for label in ax.get_xticklabels():
        label.set_visible(False)
    ...
    DHA = fig.add_subplot(2, 7, 6)
    title_cus_day = "Daily hold amount"
    # buyDf['买入当天持仓金额']=buyDf['买入当天持仓金额'].astype(float)
    Daily_hold_amt = pd.DataFrame(buyDf.groupby('发生日期')['买入当天持仓金额'].sum()).reindex(tot_buy_dt).fillna(
        0)  # .rolling(rolldays_num,1).mean()
    ...
    DAR = fig.add_subplot(2, 7, 13)
    title_cus_day = "Daily Deal Ratio (Rolling %d days)"%rolldays_num
    buy_lnzt = buyDf[buyDf['last_is_zt']==False]
    #daily_pred_sum = pd.DataFrame(buy_lnzt.groupby('发生日期')['委托方向'].count()).reindex(tot_buy_dt).fillna(0)
    #daily_attend_sum = pd.DataFrame(buy_lnzt[buy_lnzt['成交数量']>0].groupby('发生日期')['委托方向'].count()).reindex(tot_buy_dt).fillna(0)
    ...
    signal_label = buyDf.groupby('发生日期').apply(lambda x:pd.Series({'形态4数量':(x['形态']==4).sum()
                                                       ...
                                                       ,'买入当日十点前突破数量':x['买入当日突破时间'].apply(lambda x:int(x)<100000000).sum()}))
    ...
    # tmp_df = Label_summary.copy()
    # tmp_df['oldindex'] = tmp_df.index.tolist()
    # tmp_df['dt'] = tmp_df['dt'].apply(lambda x: pd.Timestamp(x))
    # tmp_df.set_index(['dt', 'Ticker'], inplace=True)
    # label_zt = tmp_df['label_T_is_zt'].unstack().fillna(0)
    # label_zt_last1 = label_zt.shift(1)
    # label_zt_last2 = label_zt_last1.shift(1)
    # label_3_zt = ((label_zt == 1) & (label_zt_last1 == 1) & (label_zt_last2 == 1)).stack()
    # label_2_zt = ((label_zt == 1) & (label_zt_last1 == 1) ).stack()#& (label_zt_last2 == 0)
    # label_1_zt = ((label_zt == 1) & (label_zt_last1 == 0)).stack()# & (label_zt_last2 == 0)
    # print(label_1_zt.sum(),label_2_zt.sum())#,label_3_zt.sum())
    # tmp_df['label_zt_lianxu_in3days'] = 0
    # #tmp_df.loc[label_3_zt[label_3_zt==True].index,'label_zt_lianxu_in3days']= 3
    # tmp_df.loc[label_2_zt[label_2_zt==True].index,'label_zt_lianxu_in3days']= 2
    # tmp_df.loc[label_1_zt[label_1_zt==True].index, 'label_zt_lianxu_in3days']=1
    # ret_df = tmp_df.set_index(['oldindex'])
    ...
    if 'ZTBysModel_local_prob' in Label_summary.columns.tolist():
        Label_summary = Label_summary[Label_summary['ZTBysModel_local_prob'].isna()]
    #Label_summary = pd.concat([ Label_summary,ret_df[['label_zt_lianxu_in3days']]],axis = 1, join_axes = [Label_summary.index])
    ...
    actbuyday = fig.add_subplot(2, 7, 5)
    ...
    L4O2ULs = fig.add_subplot(2,7,10)
    ...
    ZT3O2UL  = fig.add_subplot(2,7,14)
    ...
    Label_summary_stats_rolling_3_2022 = Label_summary_stats.loc['2022-01-04':].sort_index()#[Label_summary_stats_rolling_3['dt'] >= '2022-01-04']
    x = np.arange(len(Label_summary_stats_rolling_3_2022))
    total_width,n = 0.8,2#0.3,3
    width = total_width/n
    ...
    if strategy == 'jupiter':
        '''ZT3O2UL.plot(all_zt_1_o2ul_rolling_3[['label_1_zt_o2ul']], 'r', all_zt_2_o2ul_rolling_3[['label_2_zt_o2ul']], 'g',
                 all_zt_3_o2ul_rolling_3[['label_3_zt_o2ul']], 'b')
        ZT3O2UL.legend([ 'CZT1', 'CZT2', 'CZT3'],loc='best')'''
    ...
    '''if strategy == 'jupiter':
        ZT3O2UL.vlines('2020-06-03', ymin=-10, ymax=20, colors='black', linestyles='--')
        ZT3O2UL.vlines('2021-08-25', ymin=-10, ymax=20, colors='black', linestyles='--')
        ZT3O2UL.vlines('2022-02-22', ymin=-10, ymax=20, colors='black', linestyles='--')'''
    # ---------------------------让坐标看得清---------------------------
    ax = plt.gca()
    for label in ax.get_xticklabels():
        label.set_visible(False)
...
def get_DfTodayInfo(buyDf,buyDate,strategy = 'jupiter'):
    ...
    today_tot_sample['buy_amt'] = factor_df_new_pct['buy_amt'].reindex(new_pct_basic_file.index).values
    today_tot_sample['pct_T'] = factor_df_new_pct['pct_t'].reindex(new_pct_basic_file.index).values
    md_close_pre_close = IO.read_data([Adate, Adate], columns=['pre_close', 'close','high'] ,
    ...
    for index, row in today_tot_sample.iterrows():
        ...
        ZT_Time = int(row['ZT_Time'])
        '''if (len(today_tot_sample) > 0) and \
                (stockCode in today_tot_sample['Unnamed: 0'].values):
            resDfIndex = today_tot_sample[today_tot_sample['Unnamed: 0'] == stockCode].index
            if len(resDfIndex) > 1:
                resDfIndex = resDfIndex[-1:]
        else:  # 如果存在正常的数据
            # 针对当天在o32中没有但在log中有的票进行数据填充
            resDfIndex, stock_code = len(today_tot_sample), row['Unnamed: 0']'''

        #print('计算形态',index,resDfIndex,index==resDfIndex)
        pre_close, close, ul_price = md_close_pre_close.loc[Adate, stockCode].values

        today_tot_sample.loc[index, '形态'] = cal_Basic_zt(mdp, stockCode, Adate, pre_close, close)['label_pattern'].values[0]
    if strategy == 'jupiter':
        today_tot_sample.reset_index().to_excel('/data/group/800463/日内强势股/log_parse/中间数据/jupiter每日触发样本_%s.xlsx'%str(Adate))
    else:
        today_tot_sample.reset_index().to_excel(
            '/data/group/800463/日内强势股/log_parse/中间数据/jupiter每日触发样本New_%s.xlsx' % str(Adate))
    ...
    todayBuyResDf.loc['今日形态4比例'] = '%.2f%%' % ((today_tot_sample['形态'] == 4).sum() / len(today_tot_sample) * 100)
    todayBuyResDf.loc['基础样本成交率(300,30)'] = '%.2f%%' % ((today_tot_sample['buy_amt'] > 0).sum() / len(today_tot_sample) * 100)
...
def get_buyDfTodayInfo(buyDf,buyDate,strategy='jupiter'):
    ...
    if len(todayOrderDf)==0:
        return
    if len(todayBuyDf)>0:
        todayBuyDf['成交笔数/挂单笔数'] = todayBuyDf.apply(lambda x: x['买入当日成交笔数（o32）']/x['买入当日挂单笔数（o32）'] if x['买入当日挂单笔数（o32）']>0 else np.nan,axis=1)
    todayBuyResDf = pd.DataFrame(columns=['value'])
    ...
    todayBuyResDf.loc['10点前突破比例'] = '0.00%' if len(todayBuyDf)==0 else format(len(todayBuyDf[todayBuyDf['买入当日突破时间'].apply(lambda x:int(x))<100000000])/len(todayBuyDf),'.00%')
...
def get_sellDfTotalInfo(sellDf,Adate,fig,strategy='jupiter'):
    ...
    sellDf_2021 = sellDf[(sellDf['买入日期']>='2022-12-30') & (sellDf['买入日期']<'2023-12-31')]
    sellResDf.loc['2023累计盈利'] = sellDf_2021[(sellDf_2021['是否全部卖出']==1)|(sellDf_2021['是否全部卖出']==0)]['卖出部分盈利金额'].sum()
    ...
    # active_sell_sample = sellSampleDf[sellSampleDf['买入日期']>='2020-07-21'].copy()
    # sellResDf.loc['动态调整规模后次均等权卖出收益率(%)'] = '%.2f%%'%(active_sell_sample['卖出部分收益率(%)'].mean())
    # sellResDf.loc['动态调整规模后次均成交额加权收益率(%)'] = '%.2f%%'%(active_sell_sample['加权收益率'].sum()/active_sell_sample['买入金额'].sum())
    # if len(sellSampleDf) == 0:
    #     sellResDf.loc['累计胜率'] = 0
    # else:
    #     sellResDf.loc['累计胜率'] = '%.2f%%'%(len(sellSampleDf[sellSampleDf['理论是否预测正确']==1])/len(sellSampleDf)*100)
    sellResDf.loc['累计参与率'] = '0.00%' if len(labels_zt)==0 else '%.2f%%' % (len(labels_zt[labels_zt['shouldBuySignal']==1]) / len(labels_zt) * 100)
    sellResDf.loc['累计胜率'] = '0.00%'if len(sellSampleDf)==0 else '%.2f%%' % (len(sellSampleDf[(sellSampleDf['理论是否正收益'] == 1) & (sellSampleDf['买入当天是否收盘涨停'] == 1)]) / len(sellSampleDf) * 100)
    sellResDf.loc['累计交易胜率'] = '0.00%'if len(sellSampleDf)==0 else '%.2f%%' % (len(sellSampleDf[sellSampleDf['实际是否正收益'] == 1]) / len(sellSampleDf) * 100)
    sellResDf.loc['预测为1实际为1平均买入金额'] = 0 if len(sellSampleDf)==0 else str(round(sellSampleDf[sellSampleDf['理论是否预测正确']==1]['买入金额'].mean(),2))
    sellResDf.loc['预测为1实际为1平均卖出收益率（%）'] = '0.00%'if len(sellSampleDf)==0 else '%.2f%%'%(sellSampleDf[sellSampleDf['理论是否预测正确']==1]['卖出部分收益率(%)'].mean())
    sellResDf.loc['预测为1实际为1次均成交额加权卖出收益率（%）'] = '0.00%'if len(sellSampleDf)==0 else '%.2f%%'%(sellSampleDf[sellSampleDf['理论是否预测正确']==1]['卖出部分盈利金额'].sum()/sellSampleDf[sellSampleDf['理论是否预测正确']==1]['买入金额'].sum()*100)

    sellResDf.loc['预测为1实际为0平均买入金额'] = 0 if len(sellSampleDf)==0 else str(round(sellSampleDf[sellSampleDf['理论是否预测正确']==0]['买入金额'].mean(),2))
    sellResDf.loc['预测为1实际为0平均卖出收益率（%）'] = '0.00%'if len(sellSampleDf)==0 else '%.2f%%'%(sellSampleDf[sellSampleDf['理论是否预测正确']==0]['卖出部分收益率(%)'].mean())
    sellResDf.loc['预测为1实际为0次均成交额加权卖出收益率（%）'] ='0.00%'if len(sellSampleDf)==0 else  '%.2f%%'%(sellSampleDf[sellSampleDf['理论是否预测正确']==0]['卖出部分盈利金额'].sum()/sellSampleDf[sellSampleDf['理论是否预测正确']==0]['买入金额'].sum()*100)
    sellResDf.loc['预测为1样本盈亏比'] = 0 if len(sellSampleDf)==0 else str(abs(round((sellSampleDf[sellSampleDf['理论是否预测正确']==1]['卖出部分盈利金额'].mean())/sellSampleDf[sellSampleDf['理论是否预测正确']==0]['卖出部分盈利金额'].mean(),2)))
    new_sell_sample = sellSampleDf[(sellSampleDf['买入日期']>='2020-05-22')&(sellSampleDf['TN_o2ul'].notnull())]
    # sellResDf.loc['实际收益率与o2ul差等权均值'] = '%.4f%%'%((new_sell_sample['卖出部分收益率(%)'] - new_sell_sample['TN_o2ul']).mean())
    # sellResDf.loc['实际收益率与o2ul差成交额加权均值'] = '%.4f%%'%(((new_sell_sample['卖出部分收益率(%)']*new_sell_sample['买入金额']).sum() - (new_sell_sample['TN_o2ul']*new_sell_sample['买入金额']).sum())/\
    #                                       (new_sell_sample['买入金额'].sum()))

    old_sell_sample = sellSampleDf[sellSampleDf['买入日期']>='2020-05-22'].copy()
    for index, row in old_sell_sample.iterrows():
        stock,date = row['证券代码'], row['买入日期']
        old_sell_sample.loc[index,'old_pct'] = labels_zt.set_index(['dt','Ticker']).loc[date,stock]['old_pct']*100-0.2
        old_sell_sample.loc[index,'old_finished'] = labels_zt.set_index(['dt','Ticker']).loc[date,stock]['old_finish_indicator']
    old_sell_sample_cal = old_sell_sample[old_sell_sample['old_finished'] == 1]
    sellResDf.loc['实际收益率与第一版卖出差等权均值'] ='0.00%'if len(old_sell_sample_cal)==0 else  '%.4f%%'%((old_sell_sample_cal['卖出部分收益率(%)'] - old_sell_sample_cal['old_pct']).mean())
    sellResDf.loc['实际收益率与第一版卖出差成交额加权均值'] = '0.00%'if len(old_sell_sample_cal)==0 else  '%.4f%%'%(((old_sell_sample_cal['卖出部分收益率(%)']*old_sell_sample_cal['买入金额']).sum()
                                                     - (old_sell_sample_cal['old_pct']*old_sell_sample_cal['买入金额']).sum())/(old_sell_sample_cal['买入金额'].sum()))

    #    --------------------------------------------------画图------------------------------------------------
    tot_sell_dt = sorted(list(sellSampleDf['买入日期'].unique()))
    CPs = fig.add_subplot(2,7,4)
    ...
    save_df = pd.concat([tempDf[['cumulated_profit']],lnonzt_sellDf[['lnonzt_cumulated_profit']],lzt_sellDf[['lzt_cumulated_profit']]],axis=1)
    # save_df.to_excel('/data/user/013550/Jupiter/%s策略按日收益明细_%s.xlsx'%(strategy,str(Adate)))
    ...
    actsellsample = fig.add_subplot(2, 7, 12)
    title_cus_day = 'Average Profit(%)'+ ' (Rolling %s samples)'%str(rollnums_num)
    #sellSampleDf.sort_values(by=['买入日期'],inplace=True)
    sellSampleDf['Sample'] = list(range(1, len(sellSampleDf) + 1))#[str(x) for x in list(range(1, len(sellSampleDf) + 1))]
    #sellSampleDf['Sample_str'] = sellSampleDf['Sample'].astype(str)
    sellSampleDf.set_index(['Sample'], inplace=True)


    sell_df = pd.DataFrame(sellSampleDf['卖出部分收益率(%)'].rolling(rollnums_num,5).mean().dropna())#.sort_index()#.reindex(sellSampleDf.iloc[rollnums_num:].index).fillna(0)
    ...
    #xticks = actsellsample.set_xticks(range(len(sell_df)))
    #xticks_label = actsellsample.set_xticklabels([str(x) for x in list(range(rollnums_num,len(sellSampleDf)+1))])
    xticks = actsellsample.set_xticks(range(len(sellSampleDf)))
    xticks_label = actsellsample.set_xticklabels([str(x) for x in list(range(1,len(sellSampleDf)+1))])
    #actsellsample.set_xticks(range(len(sellSampleDf)),[str(x) for x in list(range(1,len(sellSampleDf)+1))])
...
def get_sellDfTodayInfo(sellDf,sellDate):
    ...
    #todaySellResDf.loc['卖出盈利（元）'] = ((0.999*todaySellDf['卖出成交均价'].apply(return_last_comma) - todaySellDf['买入成交均价'])* todaySellDf['卖出数量'].apply(return_last_comma)).sum()
    todaySellResDf.loc['卖出盈利（元）'] = ((0.999*todaySellDf['卖出成交均价'].apply(return_last_comma) - todaySellDf['买入成交均价'])* todaySellDf['卖出数量'].apply(return_last_comma)).sum()-((0.0000887*(todaySellDf['卖出成交均价'].apply(return_last_comma)+todaySellDf['买入成交均价']))* todaySellDf['卖出数量'].apply(return_last_comma)).sum()
...
def get_cybDfTotalInfo(cybsellDf,Adate,strategy='jupiter'):
    ...
    active_sell_sample = sellSampleDf[sellSampleDf['买入日期'] >= '2020-07-21'].copy()
    #    sellResDf.loc['累计卖出胜率'] = '%.2f%%'%(len(sellSampleDf[(sellSampleDf['实际是否正收益']==1)&(sellSampleDf['买入当天是否收盘涨停']==1)])/len(sellSampleDf)*100)
    if len(sellSampleDf) == 0:
    ...
    new_sell_sample = sellSampleDf[(sellSampleDf['买入日期'] >= '2020-05-22') & (sellSampleDf['TN_o2ul'].notnull())]
    # cybsellResDf.loc['实际收益率与o2ul差等权均值'] = '%.4f%%' % ((new_sell_sample['卖出部分收益率(%)'] - new_sell_sample['TN_o2ul']).mean())
    # cybsellResDf.loc['实际收益率与o2ul差成交额加权均值'] = '%.4f%%' % (
    #             ((new_sell_sample['卖出部分收益率(%)'] * new_sell_sample['买入金额']).sum() - (new_sell_sample['TN_o2ul'] * new_sell_sample['买入金额']).sum()) / \
    #             (new_sell_sample['买入金额'].sum()))
    # import datetime as dt
    # Adate = dt.datetime.now().strftime('%Y') + '-' + dt.datetime.now().strftime('%m') + '-' + dt.datetime.now().strftime('%d')
    # date = '20200612'
    ...
    for index, row in old_sell_sample.iterrows():
        ...
        old_sell_sample.loc[index, 'old_finished'] = labels_zt.set_index(['dt', 'Ticker']).loc[date, stock]['old_finish_indicator']
    old_sell_sample_cal = old_sell_sample[old_sell_sample['old_finished'] == 1]
    cybsellResDf.loc['实际收益率与第一版卖出差等权均值'] = '%.4f%%' % ((old_sell_sample_cal['卖出部分收益率(%)'] - old_sell_sample_cal['old_pct']).mean())
    cybsellResDf.loc['实际收益率与第一版卖出差成交额加权均值'] = '%.4f%%' % (((old_sell_sample_cal['卖出部分收益率(%)'] * old_sell_sample_cal['买入金额']).sum()
                                                        - (old_sell_sample_cal['old_pct'] * old_sell_sample_cal['买入金额']).sum()) / (
                                                           old_sell_sample_cal['买入金额'].sum()))
...
def get_lztDfTotalInfo(cybsellDf,Adate,strategy='jupiter'):
    ...
    cybsellResDf.loc['累计全部卖出样本数'] = len(sellSampleDf)
    cybsellResDf.loc['累计盈利'] = cybsellDf[(cybsellDf['是否全部卖出'] == 1) | (cybsellDf['是否全部卖出'] == 0)]['卖出部分盈利金额'].sum()
    cybsellDf_2021 = cybsellDf[(cybsellDf['买入日期']>='2022-12-30') & (cybsellDf['买入日期']<'2023-12-31')]
    cybsellResDf.loc['2023累计盈利'] = cybsellDf_2021[(cybsellDf_2021['是否全部卖出']==1)|(cybsellDf_2021['是否全部卖出']==0)]['卖出部分盈利金额'].sum()
    cybsellResDf.loc['次均等权卖出收益率(%)'] = '%.2f%%' % (sellSampleDf['卖出部分收益率(%)'].mean())
    cybsellResDf.loc['次均成交额加权收益率(%)'] = '%.2f%%' % (sellSampleDf['加权收益率'].sum() / sellSampleDf['买入金额'].sum())
    active_sell_sample = sellSampleDf[sellSampleDf['买入日期'] >= '2020-07-21'].copy()
    #    sellResDf.loc['累计卖出胜率'] = '%.2f%%'%(len(sellSampleDf[(sellSampleDf['实际是否正收益']==1)&(sellSampleDf['买入当天是否收盘涨停']==1)])/len(sellSampleDf)*100)
    cybsellResDf.loc['累计参与率'] = '%.2f%%' % (len(labels_zt[(labels_zt['shouldBuySignal']==1)&(labels_zt['ZTBysModel_local_prob'].notnull())]) / len(labels_zt[labels_zt['ZTBysModel_local_prob'].notnull()]) * 100)
    cybsellResDf.loc['累计胜率'] = '%.2f%%' % (
                len(sellSampleDf[(sellSampleDf['理论是否正收益'] == 1) & (sellSampleDf['买入当天是否收盘涨停'] == 1)]) / len(sellSampleDf) * 100)
...
def get_lnonztDfTotalInfo(cybsellDf,Adate,strategy= 'jupiter'):
    ...
    if strategy == 'jupiter':
        labels_zt = pd.read_excel('/data/group/800463/日内强势股/log_parse/因子耗时/实盘触发标签汇总_%s.xlsx' % Adate)
        labels_zt = labels_zt[labels_zt['ZTBysModel_local_prob'].isnull()]
    else:
        labels_zt = pd.read_excel('/data/group/800463/日内强势股/log_parse/因子耗时/实盘触发标签汇总New_%s.xlsx' % Adate)
    cybsellDf.replace(np.nan,'',inplace=True)
    cybsellResDf = pd.DataFrame(columns=['value'])
    ...
    active_sell_sample = sellSampleDf[sellSampleDf['买入日期'] >= '2020-07-21'].copy()
    #    sellResDf.loc['累计卖出胜率'] = '%.2f%%'%(len(sellSampleDf[(sellSampleDf['实际是否正收益']==1)&(sellSampleDf['买入当天是否收盘涨停']==1)])/len(sellSampleDf)*100)
    #cybsellResDf.loc['累计参与率'] = '%.2f%%' % (len(labels_zt[labels_zt['shouldBuySignal']&(labels_zt['ZTBysModel_local_prob'].isnull())]) / len(labels_zt[labels_zt['ZTBysModel_local_prob'].isnull()]) * 100)
