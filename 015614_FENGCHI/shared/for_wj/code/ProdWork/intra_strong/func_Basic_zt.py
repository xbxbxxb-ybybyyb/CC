...
def cal_Basic_zt(mdp, stock, date, pre_close, close, tolerance_period=10):
    ...
    '''if ZT_Time==ZT_Time:
        ...
        max_rise_pct = np.nan'''
    df = pd.DataFrame([[ZT_Time, is_zt, high_price, ul_price,]],
                      columns=['ZT_Time', 'is_zt', 'high_price', 'ul_price'])
...
def fun_common_time_interval(time1, time2):
    time3 = float(np.floor(time1 / 10000000) * 3600 + np.floor(np.mod(time1, 10000000) / 100000) * 60 + np.mod(time1,
                                                                                                               100000) / 1000)
    time4 = float(np.floor(time2 / 10000000) * 3600 + np.floor(np.mod(time2, 10000000) / 100000) * 60 + np.mod(time2,
                                                                                                               100000) / 1000)
...
def cal_pattern(md_df, ZT_Time, ul_price, close_price, tolerance_period):
    #close_price = md_df['TradePrice'][md_df['TradePrice']>0].iloc[-1]
    ...
    if len(after_zt_not_zt_md_df) == 0:
        ...
        if sum(after_zt_md_df['MDTime'] > lastUL_end_Time) == 0: #最后一次涨停直至收盘
            lastUL_end_Time = 150000000
        ...
        if lastUL_end_Time == firstUL_end_Time: #第一次涨停就是最后一次涨停
            ...
            last_no_zt_before_last_zt_index = after_zt_md_df[(after_zt_md_df['TradeIndex']<lastUL_end_Time_index) & (after_zt_md_df['TradePrice']<ul_price)].iloc[-1]['TradeIndex'] # 最后一次涨停前非涨停的时间
            lastUL_Time = after_zt_md_df[(after_zt_md_df['TradeIndex']>=last_no_zt_before_last_zt_index) & (after_zt_md_df['TradePrice']==ul_price)].iloc[0]['MDTime'] # 最后一次涨停开始时间
