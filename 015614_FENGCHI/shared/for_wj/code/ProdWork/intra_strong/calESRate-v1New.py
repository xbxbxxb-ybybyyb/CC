...
from ProdWork.Param_config_data import thred_dict_jup001_v2 as thred_dict
import sys
if len(sys.argv) > 1:
    date = sys.argv[1]
else:
...
if lastdate >= '20220510':
    model_columns = [
                     'Hml0XgbModel',
                     'Hml1XgbModel',
                     'Hml2PMMLModel',#'Hml2XgbModel',
                     'RisePctHighDjModel',
                     'RisePctLowDjModel',
                     'Hml0XgbWjModel',
                     'Hml1XgbWjModel',
...
if lastdate >= '20230213':
    ...
    model_columns = list(set(
        model_columns + last_model_columns))

# 获取昨天的标签汇总、今天的因子耗时和模型差异
if lastdate <= '20220516':
    Labels_prod_summary_old = pd.DataFrame(columns = need_columns)
else:
    Labels_prod_summary_old = pd.read_excel('/data/group/800463/日内强势股/log_parse/因子耗时/实盘触发标签汇总New_%s.xlsx'%Alastdate)
...
print(model_columns)
raw_last_date_factor_time_cost = pd.read_excel('/data/group/800463/日内强势股/log_parse/因子耗时/因子耗时_%s_prod.xlsx'%Alastdate, sheet_name='因子耗时New')
...
raw_last_date_model_compare = pd.read_excel('/data/group/800463/日内强势股/log_parse/模型差异/%s/模型差异New_%s_prod.xlsx' % (lastdate, lastdate)
...
Labels_prod_summary_new = pd.concat([Labels_prod_summary_old,
        ...
        .reset_index()[need_columns + model_columns + ['old_finish_indicator' ,'old_pct','new_finish_indicator','new_pct','new_absolute_profit']]
# 对于o2ul还未算出的样本进行计算
...
md_data = IO.read_data([20220518, end_date_], columns=['pre_close', 'open', 'high', 'low','close','vwap', 'adjfactor'],
...
# md_data['new_300'] = (md_data.reset_index()['Ticker'].apply(lambda x:x[0]=='3') & (md_data.reset_index()['dt']>='20200824')).values
# md_data.loc[md_data['new_300'],'ul_price'] = np.floor(md_data.loc[md_data['new_300'],'pre_close'] * 100 * 1.2 + 0.5) / 100
# md_data.loc[md_data['new_300'],'trigger_price'] = md_data.loc[md_data['new_300'],'ul_price'] - 0.01
# md_data['open'], md_data['close'] = md_data['open'] * md_data['adjfactor'], md_data['close'] * md_data['adjfactor']
# md_data['vwap'], md_data['pre_close'] = md_data['vwap'] * md_data['adjfactor'], md_data['pre_close'] * md_data['adjfactor']
# md_data['high'], md_data['low'] = md_data['high'] * md_data['adjfactor'], md_data['low'] * md_data['adjfactor']
# md_data['ul_price'] = md_data['ul_price'] * md_data['adjfactor']
...
if __name__ == "__main__":
    ...
    for dates in s.tradingday(old_pct_start_date.strftime('%Y%m%d'),old_pct_end_date.strftime('%Y%m%d')):
        date_basic = pd.read_hdf('/data/group/800463/project/project1_prod/generalStrong_v3/Basic_zt_test/Basic_zt_001.h5').loc[pd.Timestamp(dates):pd.Timestamp(dates)]
