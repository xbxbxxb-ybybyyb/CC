...
if __name__ == "__main__":
    ...
    import os
    #
    while (os.path.exists('/data/group/800463/日内强势股/log_parse/因子耗时/实盘触发项目二标签汇总_%s.xlsx'%Adate) == False) |\
            (os.path.exists('/data/group/800463/日内强势股/log_parse/因子耗时/实盘触发项目二931标签汇总_%s.xlsx'%Adate) == False) |\
          (os.path.exists('/data/group/800463/日内强势股/实盘分析记录/日内强势股总买入记录/项目二总买入记录-%s.xlsx'%(date))==False) |\
    ...
    buysellDf_joined = buysellDf_joined[['买入日期', '证券代码', '证券名称', '卖出日期', '买入数量', '买入金额','买入当天持仓金额', '买入成交均价', '买入当天开盘涨幅(%)',
                                           ...
                                           '理论是否正收益', '理论是否预测正确', '卖出金额占市场比', '总卖出数量', '前日形态', 'TN_v2o10', '买入时点',
    ...
    return_path_930 = "/data/group/800463/日内强势股/实盘分析记录/日内强势股成交记录/收益画图/%s_%s_p2_930.png" % (Adate, '生产环境')
    ...
    f.encoding = 'GB2312'
    ftp_upload(f,'XQuant/013550/temp/saturn成交记录-%s.xlsx' % date,
               '/data/group/800463/日内强势股/实盘分析记录/日内强势股成交记录/saturn成交记录-%s.xlsx' % date)'''
