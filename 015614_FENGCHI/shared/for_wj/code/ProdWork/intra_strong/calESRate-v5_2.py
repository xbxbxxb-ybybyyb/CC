...
need_columns_tot = ['dt', 'Ticker', '前日形态', 'TN_v2o10d1', 'p2shouldBuySignal', '买入时点', 'T_c2o10d1', 'finish_indicator', 'absolute_profit',
                    'close_zt', 'high_zt', 'T_o2pre']
need_columns = ['dt', 'Ticker', '前日形态', 'TN_v2o10d1', 'p2shouldBuySignal', '买入时点']
if date >= '20210622':
    model_columns = ['saturn931OpenPctHighDjModel_local_prob', 'saturn931OpenPctHighWjModel_local_prob', 'saturn931OpenPctLowDjModel_local_prob',
                     'saturn931OpenPctLowWjModel_local_prob', 'saturn931Pat3DjModel_local_prob', 'saturn931Pat3XgbModel_local_prob',
                     'saturn931Pat4DjModel_local_prob', 'saturn931Pat4XgbModel_local_prob','saturn931Pct5HighWjModel_local_prob',
                     'saturn931Pct5LowWjModel_local_prob','saturn931Ret2oHighDjModel_local_prob','saturn931Ret2oHighPMMLModel_local_prob',
                     'saturn931Ret2oLowDjModel_local_prob','saturn931Ret2oLowPMMLModel_local_prob','saturn931TotalDjModel_local_prob',
                     'saturn931TotalWjModel_local_prob']
...
raw_last_date_factor_time_cost_pj2 = pd.read_excel('/data/group/800463/日内强势股/log_parse/因子耗时/因子耗时_%s_prod.xlsx' % Alastdate, sheet_name='项目二931样本')
raw_last_date_model_compare_pj2 = pd.read_excel('/data/group/800463/日内强势股/log_parse/模型差异/%s/模型差异_%s_prod_pj2_931.xlsx' % (lastdate, lastdate)
...
if len(v2o10d1_nan_samples) != 0:
    ...
    for i in v2o10d1_nan_samples.index:
        ...
        saturn_basic_hf_info = pd.read_hdf('/data/group/800463/project/project2_prod/daily_data/Basic/Basic_closed_hf_finish.h5')#.loc[pd.Timestamp(str(buy_date)):]
        #saturn_basic_hf_info = pd.read_pickle('/data/group/800463/project/project2_prod/daily_data/%s_v5/saturn_factor_v5_%s.pkl' % (buy_date, buy_date))
...
if __name__ == "__main__":
    ...
    for dates in s.tradingday(old_pct_start_date.strftime('%Y%m%d'), old_pct_end_date.strftime('%Y%m%d')):
        date_basic = pd.read_hdf('/data/group/800463/project/project2_prod/daily_data/Basic/Basic_closed_hf_finish.h5')#.loc[(pd.Timestamp(dates)):]#pd.read_hdf('/data/group/800463/project/project2_prod/everyday_Basic_v2/%s_%s/Basic_closed_hf_finish_%s_%s.h5' % (dates, dates, , dates))
