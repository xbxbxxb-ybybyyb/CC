...
need_columns_tot = ['dt', 'Ticker', '前日形态', 'TN_v2o10d1', 'p3shouldBuySignal', '买入时点', 'T_c2o10d1', 'finish_indicator', 'absolute_profit',
                    'close_zt', 'high_zt', 'T_o2pre']
need_columns = ['dt', 'Ticker', '前日形态', 'TN_v2o10d1', 'p3shouldBuySignal', '买入时点']
if date >= '20211125' and date <='20220128':
    ...
    model_columns = [x+'_local_prob' for x in list(model_rename_pj3_dict.keys())]
elif date>'20220128':
    model_rename_pj3_dict = {'ceres931cbMoreDjModel': 'cbMoreDjModel',
                             'ceres931cbOneDjModel': 'cbOneDjModel',
                             'ceres931Pct5HighDjModel': 'highPct5DjModel',
...
if lastdate <= '20211126':
    Labels_prod_summary_old = pd.DataFrame(columns = need_columns_tot)
else:
    Labels_prod_summary_old = pd.read_excel('/data/group/800463/日内强势股/log_parse/因子耗时/实盘触发项目三931标签汇总_%s.xlsx' % Alastdate)
if 'twap2o10d1' not in Labels_prod_summary_old.columns:
    Labels_prod_summary_old['twap2o10d1'] = np.nan

raw_last_date_factor_time_cost_pj2 = pd.read_excel('/data/group/800463/日内强势股/log_parse/因子耗时/因子耗时_%s_prod.xlsx' % Alastdate, sheet_name='Ceres931样本')
#if os.path.exists('/data/group/800463/日内强势股/log_parse/模型差异/%s/模型差异_%s_prod_pj3_931.xlsx' % (lastdate, lastdate)):
raw_last_date_model_compare_pj2 = pd.read_excel('/data/group/800463/日内强势股/log_parse/模型差异/%s/模型差异_%s_prod_pj3_931.xlsx' % (lastdate, lastdate)
...
raw_last_date_model_compare_pj2 = raw_last_date_model_compare_pj2[['Unnamed: 0'] + model_columns + ['买入时点']].rename(columns={'Unnamed: 0': 'Ticker'}) * 1
...
if len(raw_last_date_factor_time_cost_pj2) != 0:
    ...
    for index, row in raw_last_date_factor_time_cost_pj2.iterrows():
        ...
        saturn_basic_info = pd.read_hdf(
            '/data/group/800463/project/project3_prod/daily_data/%s_v2/Basic_night_finish_%s_%s.h5' % ( lastdate, lastdate, lastdate))
        raw_last_date_factor_time_cost_pj2.loc[index, '前日形态'] = saturn_basic_info.loc[lastdate, stock_code]['lcb_label_pattern']
    # 将耗时、模型预测拼入标签汇总
    ...
    Labels_prod_summary_new = pd.concat([Labels_prod_summary_old,
                                         raw_last_date_factor_time_cost_pj2.set_index(['dt', 'Ticker']).join(
                                             raw_last_date_model_compare_pj2.set_index(['dt', 'Ticker'])).reset_index()[need_columns + model_columns]]) \
        .reset_index()[need_columns_tot + model_columns + ['twap2o10d1']]
else:
...
if len(v2o10d1_nan_samples) != 0:
    ...
    for i in v2o10d1_nan_samples.index:
        ...
        saturn_basic_hf_info = pd.read_hdf('/data/group/800463/project/project3_prod/daily_data/%s_v2/Basic_closed_hf_finish_%s_%s.h5' % (buy_date, buy_date, buy_date))
        ...
        T_c2o10d1 = (vwap_data['vwap'].loc[buy_date, stock] / this_buy_price - 1) * 100
...
if __name__ == "__main__":
    param = {'buy_vol_pct': 0.2, 'sell_vol_pct': 0.1, 'max_amt': 500 * 10000, 'cover_amt': 1500, 'p2_type': '931'}
    # -----计算卖出收益-----
    ...
    for dates in s.tradingday(old_pct_start_date.strftime('%Y%m%d'), old_pct_end_date.strftime('%Y%m%d')):
        date_basic = pd.read_hdf('/data/group/800463/project/project3_prod/daily_data/%s_v2/Basic_closed_hf_finish_%s_%s.h5' % (dates, dates, dates))
        old_pct_basic_file = pd.concat([old_pct_basic_file,
                                        date_basic.loc[
                                            not_finished_old_holdings[not_finished_old_holdings['dt'] == dates].set_index(['dt', 'Ticker']).index]])
    ...
    for index, row in not_finished_old_holdings.iterrows():
        buy_date = row['dt']
        stock = row['Ticker']
        Labels_prod_summary_new.loc[index, ['twap2o10d1', 'finish_indicator', 'absolute_profit']] = \
            factor_df_twap.rename(columns={'pct': 'twap2o10d1'}).loc[pd.Timestamp(buy_date), stock][['twap2o10d1', 'finish_indicator', 'absolute_profit']]
    ...
    Labels_prod_summary_new.to_excel('/data/group/800463/日内强势股/log_parse/因子耗时/实盘触发项目三931标签汇总_%s.xlsx' % Adate, index=False)
    print('create file %s!!!!!!!!!!!!'%'/data/group/800463/日内强势股/log_parse/因子耗时/实盘触发项目三931标签汇总_%s.xlsx' % Adate)
