...
def get_zt_time_dic(df,trade_date_h):
    df_need = df[(df['order_direction'] == 'SplitLastShot')|
                 (df['order_direction'] == 'JupiterFirstOrder')| (df['order_direction']  == 'MRiskSplitLastShotBuy')|(df['order_direction']  == 'MRiskSplitShotBuy')]
    df_need['ZT_Time_dt'] = df_need['ZT_Time'].apply(lambda x: dt.datetime.strptime(trade_date_h+' '+inttime2str(x), '%Y-%m-%d %H:%M:%S'))
...
def plot_trade_data(fig,indicator,x_plot_number, trade_data, xticks_num, save_pic):
    ...
    plt.plot([pic_start_index, pic_start_index], [min(trade_data['price_df']['price']), max(trade_data['price_df']['price'])])
    plt.plot([pic_end_index, pic_end_index], [min(trade_data['price_df']['price']), max(trade_data['price_df']['price'])])
    ...
    if len(trade_df)>0:
        ...
        for i in range(len(trade_data['trade_df']['place_order_time_list'])):
            place_x = get_position_index(trade_data['price_df'], trade_data['trade_df']['place_order_time_list'].iloc[i], require_y = False)
        ...
        try:
            ...
            for i in range(len(trade_data['trade_df']['filled_time_list'])):
                trade_x = get_position_index(trade_data['price_df'], trade_data['trade_df']['filled_time_list'].iloc[i], require_y = False)
...
import time
while not os.path.exists(MD_data_prod_dir + '%s_MD.success' % trade_date):
...
if os.path.exists('/data/group/800463/日内强势股/实盘分析记录/每日突破/每日突破_%s_%s.xlsx' % (trade_date, 'prod')):
    df = pd.read_excel('/data/group/800463/日内强势股/实盘分析记录/每日突破/每日突破_%s_%s.xlsx' % (trade_date, 'prod'))
    if len(df):
        ZT_Time_all = get_zt_time_dic(df,trade_date_h)
    else:
        ZT_Time_all = pd.DataFrame()
else:
    df = pd.DataFrame()
#if len(ZT_Time_all)>
num_stocks_2_plot = len(set(trade_record_df[(trade_record_df['orderType'] == 'SplitLastShot')|(trade_record_df['orderType']  == 'MRiskSplitShot')|
                                            (trade_record_df['orderType'] == 'JupiterFirstOrder')| (trade_record_df['orderType']  == 'MRiskSplitLastShotBuy')|(trade_record_df['orderType']  == 'MRiskSplitShotBuy')]['stockcode']))
...
indicator = 0
code_list = list(set(trade_record_df[(trade_record_df['orderType'] == 'SplitLastShot')|(trade_record_df['orderType']  == 'MRiskSplitShot')|
                                     (trade_record_df['orderType'] == 'JupiterFirstOrder')| (trade_record_df['orderType']  == 'MRiskSplitLastShotBuy')|(trade_record_df['orderType']  == 'MRiskSplitShotBuy')]['stockcode']))[:40]
print(code_list)
for code in code_list:
    indicator += 1
    print(code)
    df = trade_record_df[(trade_record_df['stockcode'] == code)&((trade_record_df['orderType'] == 'SplitLastShot')|(trade_record_df['orderType']  == 'MRiskSplitShot')|
                                                                 (trade_record_df['orderType'] == 'JupiterFirstOrder')| (trade_record_df['orderType']  == 'MRiskSplitLastShotBuy')|(trade_record_df['orderType']  == 'MRiskSplitShotBuy'))].copy()
    trade_dict = dict()
    ...
    trade_dict['place_order_price'] = [df[df['ordStatus'] == 'PENDING_NEW']\
        [df[df['ordStatus'] == 'PENDING_NEW']['transactionTime'] == last_order_time]['price'].max()]
    if 'FILLED' in df['ordStatus'].values:
        ...
        [df[df['ordStatus'] == 'FILLED']['transactionTime'] == filled_order_time]['avgPx'].max()]
        ...
        [df[df['ordStatus'] == 'PARTIALLY_FILLED']['transactionTime'] == partially_filled_order_time]['avgPx'].max()]
    else:
    ...
    trade_end_time = max(trade_df['filled_time_list'].dropna()) if len(trade_df['filled_time_list'].dropna())>0 else trade_start_time
    ...
    if len(tick_data)>0:
        ...
        price_df['time'] = price_df['time'].apply(lambda x:dt.datetime.strptime(trade_date+str(x)[:~2], '%Y%m%d%H%M%S'))

        trade_data = {'trade_df':trade_df, 'price_df':price_df, 'zt_time':ZT_Time_all[code], 'code':code, 'envir':envir,\
                      'trade_date':trade_date, 'pic_interval':[pic_start_time, pic_end_time], 'price_limit':tick_data[tick_data['MaxPx']!=0][['MinPx', 'MaxPx']].values[0]}
...
plt.savefig("/data/group/800463/日内强势股/实盘分析记录/日内强势股成交记录/成交画图/%s_%s.png"%(trade_date, envir),dpi=80)
print('create jupiter png in %s!!!!!!!!!!'%"/data/group/800463/日内强势股/实盘分析记录/日内强势股成交记录/成交画图/%s_%s.png"%(trade_date, envir))
