    ...
    for nowdate in datelist:
        ...
        # for environment in ['UAT_lite']:#['prod','UAT','SZEX','SHEX','UAT_lite','UAT_other','night','test1'][:6]: # 可选择 UAT 和 night
            ...
            for model_name in list(All_model_predict_data_diff_out_dict.keys()):
                if 'ZT' in model_name:  # 去掉旧的ZT模型  # 20230612 by fengc: 去掉旧的ZT模型（jupiterZ）
                    continue
            ...
            excel_saver(excel_save_dict,'/data/group/800463/日内强势股/log_parse/模型差异/%s/模型差异_%s_%s.xlsx'%(tradeDatestr,tradeDatestr,environment))