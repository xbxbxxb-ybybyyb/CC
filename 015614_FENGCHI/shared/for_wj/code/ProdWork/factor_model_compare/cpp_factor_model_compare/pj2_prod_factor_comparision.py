...
if __name__ == '__main__':
    print('==========pj2_prod_factor_comparision==========')
    ...
    for environment in env_list:
        ...
        basic_pj2_path = '/data/group/800463/project/project2_prod/daily_data/%s_v5/Basic_closed_hf_finish_%s_%s.h5' % (date, date, date)
        # 需要陈睿的每日本地因子数据
        #local_factor_path = '/data/group/800463/project/project2_prod/everyday_Factor_v2/T_day_factor/%s_%s/' % (date, date)


        # 需要陈睿的每日本地因子数据
        local_factor_path = '/data/group/800463/project/project2_prod/daily_data/%s_v5/saturn_factor_v5_%s.pkl' % (str(date), str(date))
        # 需要传入日志解析中的因子值数据

        # 需要传入日志解析中的因子值数据
        java_factor_path = '/data/group/800463/日内强势股/cpp_log_parse/因子数据/项目二因子数据_%s_%s.xlsx' % (date_h, environment)
        # 输出路径
