...
if __name__ == '__main__':
    ...
    # for environment in ['UAT_lite']:  # ['night']:#['UAT_lite']:#
        ...
        inf_df = pd.read_excel('/data/group/800463/日内强势股/cpp_log_parse/因子耗时/因子耗时_%s_%s.xlsx' % (tradeDate, environment),
                               sheetname='项目二931样本')
        ...
        excel_saver(excel_save_dict, '/data/group/800463/日内强势股/cpp_log_parse/模型差异/%s/模型差异_%s_%s_pj2_931.xlsx' % (
    ...
    for environment in environment_lst[:1] + environment_lst[2:4]:  ## night
        ...
        #------------------------------读取log中因子耗时文件中各个模型的预测数据------------------------------
        inf_df = pd.read_excel( '/data/group/800463/日内强势股/cpp_log_parse/因子耗时/因子耗时_%s_%s.xlsx'%(tradeDate, environment),sheetname = '项目二931样本')
        inf_df = inf_df.rename(columns = {'Unnamed: 0':'Ticker'}).set_index('Ticker')
        inf_df = inf_df.loc[~inf_df.index.duplicated(keep='first')].sort_index()
        #UAT中的信号
        log_signal = inf_df[['p2shouldBuySignal']].rename(columns = {'p2shouldBuySignal':environment+'_signal'})
        ...
        check_dir('/data/group/800463/日内强势股/cpp_log_parse/模型差异/%s'%tradeDatestr)
        excel_saver(excel_save_dict,'/data/group/800463/日内强势股/cpp_log_parse/模型差异/%s/模型差异_%s_%s_pj2_931.xlsx'%(tradeDatestr,tradeDatestr,environment))
