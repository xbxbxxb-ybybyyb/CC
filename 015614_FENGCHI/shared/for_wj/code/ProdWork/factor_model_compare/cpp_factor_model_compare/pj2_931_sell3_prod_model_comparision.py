...
from ProdWork.Param_config_data import thred_dict_pj2_931_sell3 as thred_dict
if __name__ == '__main__':
    print('===========pj2_931_sell3_prod_model_comparision===========')
    if sys.argv[1]:
        nowdate = sys.argv[1]
    else:
    ...
    for nowdate in datelist:
        ...
        if sys.argv[2:]:
            ...
            env_list = ['UAT','night','UAT_50_51','UAT_49_53','prod','UAT','SZEX','SHEX','UAT_50_51','UAT_49_53','UAT_lite'][:3]
        for environment in env_list:#['prod','UAT','SZEX','SHEX','SZEX_udp','UAT_other','night4','test1'][1:2]: # 可选择 UAT 和 night
            ...
            inf_df = pd.read_excel( '/data/group/800463/日内强势股/cpp_log_parse/因子耗时/因子耗时_%s_%s.xlsx'%(tradeDate, environment),sheet_name = 'Sell3样本')
            ...
            prod_model = ['totalRegFSRSXgbSkkModel',
                         'totalRegXgbWjModel',
            ...
            log_model=['totalSkkXgbFSRSModel', 'totalWjXgbModel', 'totalSkkXgbFSV11Model', 'totalSkkXgbFSV8Model','totalXbcXgbModel', 'totalSkkXgbModel']
            ...
            pred_path = '/data/group/800463/wangj/model_signal/Sell3/prod_v1/%s/' % (tradeDatestr)+ '%s_%s_Sell%s2_fac_20230330_new_daily_pred.csv' % (tradeDatestr, tradeDatestr, '3')
            ...
            excel_saver(excel_save_dict,'/data/group/800463/日内强势股/cpp_log_parse/模型差异/%s/卖出Sell3模型差异_%s_%s.xlsx'%(tradeDatestr,tradeDatestr,environment))
