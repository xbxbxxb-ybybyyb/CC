...
def factor_comparison(local_data_path, local_factor_path,UAT_factor_path,daily_zuhe_path,date,date_hyphen
    ...
    #python_factors = pd.read_hdf(local_factor_path + 'all_factor_zt_merge_%s_%s.h5' % (date, date))
    #python_factors = pd.read_pickle(local_factor_path + 'all_factor_zt_merge_v2212_%s_europa.pkl' % (date))
    #if environment in ['UAT','UAT_50_51','UAT_49_53','UAT_lite','UAT_other']:
    # TODO：少森给这个文件路径
    python_factors = pd.read_pickle(local_factor_path + 'all_factor_zt_merge_v2304_%s_europa.pkl' % (date))
    #python_factors = pd.read_pickle('/data/group/800463/project/project1_prod/generalStrong_v2/daily_data_20201211/%s/'%(date) +\
    #                                'all_factor_zt_merge_%s_1211.pkl' % date)
    basic_df = python_factors.copy()#pd.read_hdf(local_basic_path + 'Basic_zt_%s_%s.h5' % (date, date))
    python_factors = python_factors[(basic_df['high_price'] < basic_df['trigger_price']) & (basic_df['ZT_Time'] <= 143000000)]
    UAT_factors = pd.read_excel(UAT_factor_path + '因子数据New_%s-%s-%s_%s.xlsx' % (Year, Month, Day,environment))
    UAT_factors = UAT_factors[~UAT_factors['Unnamed: 0'].duplicated()]  # note by fengc: 如果日志为空，这里就直接报错出去，竟然没处理= =
    ...
    #jn_factors = pd.read_pickle('/data/group/800463/xiely/generalStrong_v1/jn_factors_v8.pkl').tolist()
    #jz_factors = pd.read_pickle('/data/group/800463/xiely/generalStrong_v1/jz_factors_v8.pkl').tolist()
    python_factors_joined = python_factors.reindex(UAT_factors.index)[list(set(python_factors.columns.tolist())-set(label_cols))]
    '''Lzt_nan_count = UAT_factors.loc[python_factors_joined[python_factors_joined['last_is_zt'] == 1].index][jz_factors].isnull().sum(axis=1)
    LNzt_nan_count = UAT_factors.loc[python_factors_joined[python_factors_joined['last_is_zt'] == 0].index][jn_factors].isnull().sum(axis=1)
    Lzt_nan_sample = Lzt_nan_count[ (Lzt_nan_count != 0)]
    LNzt_nan_sample = LNzt_nan_count[ (LNzt_nan_count != 0)]'''
    ...
    try:
        ...
        #----------------------UAT用组合文件---------------------
        # total_zuhe_files = os.listdir(daily_zuhe_path+date)
        # total_zuhe = []
        # for file in total_zuhe_files:
        #     total_zuhe += list(pd.read_excel(daily_zuhe_path+date+'/'+file)['证券代码'])
        #----------------------实盘用param-----------------------
        # zuhe_file = pd.read_csv('/data/group/800463/日内强势股/实盘测试参数/param-%s-%s.csv' % (date,environment),encoding = 'gbk')
    ...
    ZT_Time_UAT = pd.read_excel(factor_time_path+'因子耗时_%s_%s.xlsx'%(date_hyphen,environment), sheet_name='因子耗时New')
    ...
    # ----------------------计算样本量与计算数据的拼接--------------------------------
    def cal_data_num(df):
        out_dict = {'Allday_Trans_Data': len(df),
                    'Allday_Unique_Buy': len(np.unique(df['TradeBuyNo'])),
                    'Allday_Unique_Sell': len(np.unique(df['TradeSellNo']))}
        return pd.Series(out_dict)
    stats = local_data.groupby(level=['dt','Ticker']).apply(cal_data_num)
    Local_data_stats = ZT_Time_UAT[['factor_time_cost','ZT_Time','system_time']].join(stats)
    # ----------------------检查重合的样本的因子值有没有差异---------------------
    ...
    output_dict = {'差值大于1e-8':all_diff_samples.reset_index(),
                   ...
                   '因子遗失情况':check_missing(UAT_missing_factors_abnormal,python_missing_factors,'因子',environment)}
    ...
    excel_name = os.path.join(output_folder, 'Factor_diff_New_%s_%s_3.xlsx'%(date,environment))
    try:
...
if __name__ == '__main__':
    print('============jup001_prod_factor_comparision===========')
    ...
    # date = '20230525'  # TODO:night夜间仿真，修改成下一天的日期，比如对比25号的回放，这里要改成26号下一个交易日
    datelist =s.tradingday(date, date)
    for date in datelist:
        #date='20230308'
        comparison_date = s.tradingday(date,-2)[0]
        #comparison_date = '20220429'
        ...
        if sys.argv[2:]:
            ...
            # environment_lst = ['prod','SHEX','SZEX','UAT','UAT_50_51','UAT_49_53','UAT_lite','UAT_other','night1', 'test']
            environment_lst = ['prod','SHEX','SZEX','UAT','UAT_50_51','UAT_49_53']
        # TODO: 部署到生产后， in environment_lst   如果是夜间仿真对比，就改成night-N night4
        for environment in environment_lst:#environment_lst[:7]:#['UAT','night4','test1'][0:1]:#environment_lst[:4]:
        # for environment in ['UAT_lite']:#environment_lst[:7]:#['UAT','night4','test1'][0:1]:#environment_lst[:4]:
            print('jup001_prod_factor_comparision', date, environment)
            # 是否计算样本偏差
            all_sample_environment = True
            # 需要王伟地的每日本地因子数据
            #local_factor_path = '/data/group/800463/project/project1_prod/generalStrong_v3/daily_data/%s/' % (date)
            #local_factor_path = '/data/group/800463/project/project1_prod/right_v2212/daily_data/%s_europa/'%(str(date))#'/data/group/800463/project/project1_prod/right_v2212/daily_data/%s_europa/' % (str(date))
            # TODO: local的路径，每次迭代 少森给这个路径
            local_factor_path = '/data/group/800463/project/project1_prod/right_v2304/daily_data/%s_europa/' % (str(date))

            #local_basic_path = '/data/group/800463/project/project1_prod/generalStrong_v3/daily_data/%s/'%(date)
            local_basic_path = local_factor_path
            #local_factor_path = '/data/group/800463/project/project1_prod/generalStrong_v2/daily_data_20201211/%s/'%(date)
            # 需要传入日志解析中的因子值数据
            # UAT_factor_path = '/data/user/013550/%s_factor_data/'%(environment)
            UAT_factor_path = '/data/group/800463/日内强势股/cpp_log_parse/因子数据/'
            # 输出路径
            # output_folder = '/data/user/013550/%s_factor_test_result/%s_%s/' % (environment,date,environment)
            output_folder = '/data/group/800463/日内强势股/cpp_log_parse/因子差异/%s_%s/' % (date, environment)
            # 需要传入谢璐遥的每日组合
            ...
            factor_time_path = '/data/group/800463/日内强势股/cpp_log_parse/因子耗时/'
            ...
            factor_comparison(local_data_path = local_data_path, local_factor_path=local_factor_path, UAT_factor_path=UAT_factor_path
                              ...
                              ,date=date ,date_hyphen=date_hyphen, all_sample_environment = all_sample_environment,environment = environment, local_basic_path = local_basic_path)
            print('finished')
