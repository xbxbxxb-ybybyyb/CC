...
def factor_comparison(basic_pj2_path, local_factor_path,java_factor_path,date
    ...
    for trade_time in ['930','931']:
        java_factor = pd.read_excel(java_factor_path,sheet_name = trade_time).rename(columns = {'Unnamed: 0':'Ticker'}).set_index('Ticker')
        # ----------------------检查因子数量有没有差异---------------------
        ...
        excel_name = os.path.join(output_folder, 'Factor_diff_pj3_%s_%s_%s.xlsx' % (trade_time, date, environment))
        try:
...
if __name__ == '__main__':
    print('===========pj3_prod_factor_comparision==========')
    ...
    if sys.argv[2:]:
        ...
        env_list =  ['prod', 'SHEX', 'SZEX', 'UAT', 'UAT_50_51', 'UAT_49_53', 'UAT_lite', 'UAT_other', 'night1', 'test'][:6]

    #date = '20221107'
    print('date = %s' % date)
    date_h = pd.Timestamp(date).strftime('%Y-%m-%d')
    print(date_h)
    for environment in env_list:#['UAT']:#sel_environment[:2]+sel_environment[4:]:#['prod','UAT']: # nightsel_environment[:2]+s
        print(environment)
        # 是否计算样本偏差
        all_sample_environment = True
        basic_pj3_path = '/data/group/800463/project/project3_prod/daily_data/%s_v3/Basic_closed_hf_finish_%s_%s.h5' % (date,date, date) # T-1的样本
        # 需要陈睿的每日本地因子数据
        local_factor_path ='/data/group/800463/project/project3_prod/daily_data/%s_v3/ceres_factor_v3_%s.pkl' % (str(date), str(date))# '/data/group/800463/project/project3_prod/daily_data/%s/ceres_factor_v0902_%s.pkl' % (date, date) # T日因子
        # 需要传入日志解析中的因子值数据
        java_factor_path = '/data/group/800463/日内强势股/cpp_log_parse/因子数据/项目三因子数据_%s_%s.xlsx' % (date_h, environment)
        # 输出路径
        output_folder = '/data/group/800463/日内强势股/cpp_log_parse/因子差异/%s_%s/' % (date, environment)
        factor_comparison(basic_pj2_path = basic_pj3_path, local_factor_path = local_factor_path,java_factor_path = java_factor_path,date = date,
