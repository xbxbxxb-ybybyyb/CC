...
def factor_comparison(local_data_path, local_factor_path,UAT_factor_path,daily_zuhe_path,date,date_hyphen
    ...
    python_factors = pd.read_pickle(local_factor_path + 'all_factor_zt_merge_v2212_%s.pkl' % (date))
    ...
    basic_df = pd.read_hdf(local_basic_path + 'Basic_zt_%s_%s.h5' % (date, date))
    python_factors = python_factors[(basic_df['high_price'] < basic_df['ul_price']) & (basic_df['ZT_Time'] <= 143000000)]
...
if __name__ == '__main__':
    print('==========jup_prod_factor_comparision==========')
    ...
    for date in datelist:
        ...
        if sys.argv[2:]:
            ...
            env_list = ['SHEX', 'SZEX', 'UAT', 'UAT_50_51', 'UAT_49_53', 'UAT_lite']
        ...
        # for environment in ['UAT_lite']:  #
            ...
            local_factor_path = '/data/group/800463/project/project1_prod/factor_manager_v2/daily_data/%s_v8/'%date
            local_basic_path = '/data/group/800463/project/project1_prod/generalStrong_v3/daily_data/%s/'%(date)
            local_factor_path = '/data/group/800463/project/project1_prod/right_v2212/daily_data/%s/'%(str(date))#'/data/group/800463/project/project1_prod/right_v2212/daily_data/%s/'%(date)
            # 需要传入日志解析中的因子值数据
