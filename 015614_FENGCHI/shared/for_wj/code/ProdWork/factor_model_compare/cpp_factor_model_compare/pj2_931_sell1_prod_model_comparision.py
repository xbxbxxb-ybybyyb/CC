...
if __name__ == '__main__':
    print('===============pj2_931_sell1_prod_model_comparision==========')
    if sys.argv[1]:
        nowdate = sys.argv[1]
    else:
    ...
    for nowdate in datelist:
        ...
        # for environment in ['UAT_lite']:#['prod','UAT','SZEX','SHEX','SZEX_udp','UAT_other','night4','test1'][1:2]: # 可选择 UAT 和 night
            ...
            inf_df = pd.read_excel( '/data/group/800463/日内强势股/cpp_log_parse/因子耗时/因子耗时_%s_%s.xlsx'%(tradeDate, environment),sheet_name = 'Sell1样本')
            ...
            excel_saver(excel_save_dict,'/data/group/800463/日内强势股/cpp_log_parse/模型差异/%s/卖出Sell1模型差异_%s_%s.xlsx'%(tradeDatestr,tradeDatestr,environment))
