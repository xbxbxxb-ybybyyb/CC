...
if __name__ == "__main__":
    ...
    for date in s.tradingday(begindate, enddate):
        ...
        for environment in environment_lst: #[:2] + environment_lst[4:]:#['night']:#['UAT','UAT_other']:#['night','night_other']:#['night_origin3']:#['night_origin3','night_50_51','night_49_53']:#['UAT_lite']:#
            ...
            print('create file %s' % '/data/group/800463/日内强势股/log_parse/因子数据/因子数据_%s_%s.xlsx' % (tradeDate, environment))
            ...
            print('create file %s'%'/data/group/800463/日内强势股/log_parse/因子数据/因子数据New_%s_%s.xlsx' % (tradeDate, environment))
            excel_saver({'930':factor_df_pj2_930,
                     '931':factor_df_pj2_931},'/data/group/800463/日内强势股/log_parse/因子数据/项目二因子数据_%s_%s.xlsx' % (tradeDate, environment))
            ...
            # factor_df_all.to_excel('/data/group/800463/日内强势股/log_parse/因子数据/全部因子数据_%s_%s.xlsx' % (tradeDate, environment))
            # factor_df_all.to_pickle('/data/group/800463/日内强势股/log_parse/因子数据/全部因子数据_%s_%s.pkl' % (tradeDate, environment))

            if len(except_df) > 0:
                except_df.to_excel('/data/group/800463/日内强势股/log_parse/异常报错/异常报错_%s_%s.xlsx' % (tradeDate, environment))
                print(
                    'create file %s' % '/data/group/800463/日内强势股/log_parse/异常报错/异常报错_%s_%s.xlsx' % (tradeDate, environment))
            ...
            if (environment == 'prod') | (environment == 'SHEX') | (environment in [ 'SZEX','SZEX_udp']):
                ...
                # -------------- 生产上才分析未成交的原因 -------------------
                if environment == 'prod':
                    hist_non_trade = pd.read_excel('/data/group/800463/日内强势股/log_parse/因子耗时/因子耗时_%s_%s.xlsx'% (lastdate_h, environment),sheetname = '未成交统计')
            ...
            if logparse.environment in ['prod','UAT']:
                ...
                ftp_upload(f,'XQuant/013550/temp/因子耗时_%s_%s.xlsx'%(tradeDate,environment),
                           '/data/group/800463/日内强势股/log_parse/因子耗时/因子耗时_%s_%s.xlsx'%(tradeDate,environment))
