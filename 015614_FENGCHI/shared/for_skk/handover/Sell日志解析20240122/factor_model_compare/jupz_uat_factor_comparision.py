...
def factor_comparison(basic_pj2_path, local_factor_path, java_factor_path, date, output_folder, all_sample_environment, environment):
    ...
    basic_pj2_filtered = basic_pj2.copy()
    ...
    java_factor = pd.read_excel(java_factor_path)
    ...
    excel_name = os.path.join(output_folder, 'Factor_diff_JupiterZ_%s_%s.xlsx' % (date, environment))
    try:
...
if __name__ == '__main__':
    print('==========jupz_local_factor_comparision==========')
    ...
    for date in s.tradingday(date,date):
        ...
        for environment in environment_lst:
            ...
            basic_pj2_path = '/data/group/800463/project/project1_prod/right_v2304/daily_data/%s/all_factor_zt_merge_v2304_%s.pkl' % (date, date)
            local_factor_path = '/data/group/800463/project/project1_prod/right_v2304/daily_data/%s/all_factor_zt_merge_v2304_%s.pkl' % (date, date)

            # 需要传入日志解析中的因子值数据
            java_factor_path = '/data/group/800463/日内强势股/sell_log_parse/因子数据/因子数据JupiterZ_%s_%s.xlsx' % (date_h, environment)

            output_folder = '/data/group/800463/日内强势股/sell_log_parse/因子差异/%s_%s/' % (date, environment)
            factor_comparison(basic_pj2_path=basic_pj2_path,
                              local_factor_path=local_factor_path,
                              java_factor_path=java_factor_path,
                              date=date,
                              output_folder=output_folder,
                              all_sample_environment=all_sample_environment,
