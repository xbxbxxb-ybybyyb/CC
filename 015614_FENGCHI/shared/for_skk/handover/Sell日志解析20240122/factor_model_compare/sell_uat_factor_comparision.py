...
def factor_comparison(basic_pj2_path, local_factor_path, java_factor_path, date
    ...
    for trade_time in ['930','931']:
        java_factor = pd.read_excel(java_factor_path, sheet_name='%s_Sellv1'%trade_time)
...
if __name__ == '__main__':
    ...
    for date in s.tradingday(date,date):
        ...
        for environment in environment_lst:
            ...
            # 需要传入日志解析中的因子值数据
            java_factor_path = '/data/group/800463/日内强势股/sell_log_parse/因子数据/卖出Sell因子数据_%s_%s.xlsx' % (date_h, environment)
