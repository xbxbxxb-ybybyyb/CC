...
from ProdWork.log_analyse.prod.sell.LogParse import LogParse
...
if __name__ == "__main__":
    ...
    for date in s.tradingday(begindate, enddate):
        ...
        for environment in environment_lst:
            ...
            if environment == 'UAT':
                file_name = f'/data/group/800463/StrategyLog/sim/SellStrategy-{tradeDate}.log.gz'
                # file_name = f'/data/group/800463/日内强势股/sell_log/sim/SellStrategy-{tradeDate}.log.gz'
            ...
            sz_log_fpath = f'/data/group/800463/StrategyLog/prd/SZEX.SellStrategy-{tradeDate}.log.gz'
            sh_log_fpath = f'/data/group/800463/StrategyLog/prd/SHEX.SellStrategy-{tradeDate}.log.gz'
            if environment == 'prod' and os.path.exists(sz_log_fpath):
                sz_file = gzip.GzipFile(sz_log_fpath)
                sz_lines = sz_file.readlines()
                copyfile(sz_log_fpath, '/data/group/800463/日内强势股/sell_log/SZEX.SellStrategy-%s.log.gz' % tradeDate)
                print(f'from {sz_log_fpath} to /data/group/800463/日内强势股/sell_log/SZEX.SellStrategy-{tradeDate}.log.gz 上传成功')
            else:
                sz_lines = []
            ...
            if environment == 'prod' and os.path.exists(sh_log_fpath):
                ...
                copyfile(sh_log_fpath, f'/data/group/800463/日内强势股/sell_log/SHEX.SellStrategy-{tradeDate}.log.gz')
                print(f'from {sh_log_fpath} to /data/group/800463/日内强势股/sell_log/SHEX.SellStrategy-{tradeDate}.log.gz 上传成功')
            else:
                sh_lines = []
            ...
            if environment == 'UAT' and os.path.exists(file_name):
                ...
                copyfile(file_name, '/data/group/800463/日内强势股/sell_log/sim/SellStrategy-%s.log.gz' % tradeDate)
                print('from %s to /data/group/800463/日内强势股/sell_log/sim/SellStrategy-%s.log.gz 上传成功' % (file_name, tradeDate))
            ...
            if environment not in ['prod']:
                if file_name.endswith('gz'):
                    ...
                    other_lines = list(filter(lambda x: str(x).startswith("2023-11-08T") and 'INFO' in str(x), other_lines))
            ...
            # 拆分日志
            log_split_path = f'/data/group/800463/日内强势股/sell_log_parse/日志拆分/{tradeDatestr}_{environment}环境/'
            ...
            # 解析日志开始
            inf_df_930_sellv1, inf_df_931_sellv1, inf_df_jupz, trade_dic, now_trade_dic, order_info_df, unfilled_info_df, new_trade_dic, \
            factor_df_930_sellv1, factor_df_931_sellv1, factor_df_jupz, \
            all_code_model_data_jupz, all_code_model_data_931_sellv1, \
            daily_df_jupz = lp_inst.start_parsing()

            except_df = lp_inst.get_except_df()
            save_index_sellv1 = list(set(factor_df_sellv1.index.tolist()))
            save_index_jupz = list(set(factor_df_jupz.index.tolist()))

            factor_df_sellv1.loc[save_index_sellv1].sort_index().to_excel('/data/group/800463/日内强势股/sell_log_parse/因子数据/因子数据Sellv1_%s_%s.xlsx' % (tradeDate, environment))
            print('create file %s' % '/data/group/800463/日内强势股/sell_log_parse/因子数据/因子数据Sellv1_%s_%s.xlsx' % (tradeDate, environment))
            factor_df_jupz.loc[save_index_jupz].sort_index().to_excel('/data/group/800463/日内强势股/sell_log_parse/因子数据/因子数据JupiterZ_%s_%s.xlsx' % (tradeDate, environment))
            print('create file %s' % '/data/group/800463/日内强势股/sell_log_parse/因子数据/因子数据JupiterZ_%s_%s.xlsx' % (tradeDate, environment))
            excel_saver({'Sellv1': factor_df_sellv1,
                         'JupiterZ': factor_df_jupz},
                        '/data/group/800463/日内强势股/log_parse/因子数据/卖出Sell1因子数据_%s_%s.xlsx' % (tradeDate, environment))

            if len(except_df) > 0:
                except_df.to_excel('/data/group/800463/日内强势股/sell_log_parse/异常报错/异常报错_%s_%s.xlsx' % (tradeDate, environment))
                print('create file %s' % '/data/group/800463/日内强势股/sell_log_parse/异常报错/异常报错_%s_%s.xlsx' % (tradeDate, environment))
            md_data_path = '/data/group/800463/日内强势股/sell_log_parse/行情数据/%s_%s/' % (tradeDate, environment)
            model_factor_data_path = '/data/group/800463/日内强势股/sell_log_parse/模型数据/%s_%s/' % (tradeDate, environment)
            ...
            model_total_keys_sellv1 = []
            for model_dict in all_code_model_data_sellv1.values():
                model_total_keys_sellv1 += list(model_dict.keys())
            model_total_keys_sellv1 = list(np.unique(np.array(model_total_keys_sellv1)))
            total_keys_sellv1 = list(set(trade_dic.keys()).union(set(now_trade_dic.keys())).union(set(model_total_keys_sellv1)))

            model_total_keys_jupz = []
            for model_dict in all_code_model_data_jupz.values():
                model_total_keys_jupz += list(model_dict.keys())
            model_total_keys_jupz = list(np.unique(np.array(model_total_keys_jupz)))
            total_keys001 = list(set(trade_dic.keys()).union(set(now_trade_dic.keys())).union(set(model_total_keys_jupz)))

            if 'ZT_Time' in inf_df_sellv1.columns.tolist():
                inf_df_sellv1 = inf_df_sellv1.sort_values('ZT_Time')
                print(inf_df_sellv1['ZT_Time'])
                inf_df_sellv1['ZT_Time_str'] = inf_df_sellv1['ZT_Time'].apply(inttime2str)
                daily_zt_df_sellv1['ZT_Time'] = inf_df_sellv1['ZT_Time'].loc[inf_df_sellv1.index]
            inf_df_sellv1['machine_code'] = pd.Series(lp_inst.machine_code_dict).loc[inf_df_sellv1.index].values
            if 'ZT_Time' in inf_df_jupz.columns.tolist():
                inf_df_jupz = inf_df_jupz.sort_values('ZT_Time')
                inf_df_jupz['ZT_Time_str'] = inf_df_jupz['ZT_Time'].apply(inttime2str)
                daily_zt_df_jupz['ZT_Time'] = inf_df_jupz['ZT_Time'].loc[inf_df_jupz.index]
            inf_df_jupz['machine_code'] = pd.Series(lp_inst.machine_code_dict).loc[inf_df_jupz.index].values

            tupo_excel_dic = {'每日Sellv1': daily_zt_df_sellv1,
                              '每日JupiterZ': daily_zt_df_jupz,
                              '每日订单': order_info_df,
                              '每日拒绝': unfilled_info_df}
            excel_saver(tupo_excel_dic, '/data/group/800463/日内强势股/sell_实盘分析记录/每日突破/每日突破_%s_%s.xlsx' % (tradeDatestr, environment))
            ...
            if len(order_info_df) == 0:
                ...
                buy_order_info_df = order_info_df[(order_info_df['orderType'] == 'SplitLastShot') | (order_info_df['orderType'] == 'MRiskSplitShot') |
                                                  (order_info_df['orderType'] == 'JupiterFirstOrder') | (order_info_df['orderType'] == 'MRiskSplitLastShotBuy') | (
            ...
            inf_df_dic = {'Sell1样本': inf_df_sellv1,
                          'JupiterZ样本': inf_df_jupz,
                          '买单统计': buy_statistics,
                          '卖单统计': sell_statistics,
                          '机器统计': pd.Series(lp_inst.machine_counter_dict)}

            excel_saver(inf_df_dic, '/data/group/800463/日内强势股/sell_log_parse/因子耗时/因子耗时_%s_%s.xlsx' % (tradeDate, environment))