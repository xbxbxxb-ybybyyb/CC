from dataApi.tradeDate import get_date_range, trade_minutes, get_sub_date_index, get_desample_minute_dict
...
class FactorTest(object):
    ...
    def test_factor(self, program, noprint=False, admin=None):
        ...
        if admin == 95238:
            return simple_result
        elif date_invalid_num & corr & ic_all_dtc & ic_all_dt & ic_all_c & top_ret & half_ic & dtc_all_sign:
            if not noprint:
                print(test_report)
...
class OutSampleTest(object):

    def __init__(self):

        random_state = np.random.RandomState(3251)
        self._sample = random_state.choice(72369024, 3000, replace=False)

        self._future_finite = np.load('/data/group/800442/800319/junkBigFactorPool/forward_data/future_finite.npy')
        self._future = np.load('/data/group/800442/800319/junkBigFactorPool/forward_data/future.npy')
        self._code_list = list(np.load('/data/group/800442/800319/junkBigFactorPool/forward_data/code_list.npy'))
        self._stock_pool = np.load('/data/group/800442/800319/junkBigFactorPool/forward_data/stock_pool.npy')

        calc_start_date = 20180801
    ...
    def batch_test(self):
        ...
        for name in wait_programs:
            program = load_pickle(address2 + name)
            if (program['corr_ignore']) or ('out_of_sample_check'in program.keys()):
                continue
            else:
                ...
                program['out_of_sample_check'] = self.__test_program(new_program)
                save_pickle(address2 + name, program)

    def __test_program(self, program):

        test_report = 'Out of Sample Report'.center(64, '*') + '\n'
        test_report += 'Introduction'.center(64, '-') + '\n'
        test_report += 'Raw File: ' + program['raw_file'] + '\n'

        address3 = '/data/group/800442/800319/junkBigFactorPool/level3_confirmed/'
        factor_address = '/data/group/800442/800319/junkBigFactorPool/forward_factor/'
        ...
        if program['program_complex']:
            ...
            if 'complex_simplified' in program:
                ...
                for basic_factor in get_program_factor(program['program_code']):
                    if basic_factor not in globals():
                        globals()[basic_factor] = np.load(
                            '/data/group/800442/800319/junkBigFactorPool/forward_data/%s.npy' % basic_factor)
                factor = eval(program['program_code'])
            else:
                indent = re.search('\n([ ]+)[\w+]', program['program_code'])[1]
                program_code = program['program_code'].replace('\n' + indent, '\n')
            ...
            program['program_code'] = program['program_code'].replace('\n', '').replace(' ', '').replace(',', ', ')
            if re.search('[\+\-\*/]', program['program_code']):
                raise SyntaxError("Please check your simple program code.")
        ...
        if not program['program_complex']:
            ...
            program_code = [x for x in program['program_code'].split('\n') if x != ''][-1] + '\n'
        ...
        dtc_all_sign = (Config['dtc_all_sign'][0][0] < simple_result['dtc_all_sign'] < Config['dtc_all_sign'][0][1]
            ...
            'dtc_all_sign'] < Config['dtc_all_sign'][1][1])
        half_ic = (simple_result['ic_half_dt'] < 0).sum() <= Config['ic_half'][1]

        program['corr_ignore'] = False
        corr = True
        if date_invalid_num & ic_all_dtc & ic_all_dt & ic_all_c & top_ret & half_ic & dtc_all_sign:

            corr_files3, ic_half_dt_compare, file_time = load_pickles(address3, ic_all_dt_every_code, factor_sample)
            corr_max = round(ic_half_dt_compare.max(), 4)

            if corr_max < Config['corr_limit']:
                corr = True
            elif (ic_half_dt_compare >= Config['corr_limit']).sum() > 1:
                corr = False
            else:
                corr_bench = np.arange(len(ic_half_dt_compare)).dot(ic_half_dt_compare >= Config['corr_limit'])
                corr_bench_file = address3, corr_files3[corr_bench]
                corr_bench_ic = load_pickle(corr_bench_file[0] + corr_bench_file[1])
                if (corr_bench_ic['ic_all_dt'] + corr_bench_ic['ic_all_c'] + 2 * corr_bench_ic['ic_all_dtc'] <
                        simple_result['ic_all_dt'] + simple_result['ic_all_c'] + 2 * simple_result['ic_all_dtc']):
                    corr = True
        ...
        test_report += 'Conclusion'.center(64, '-') + '\n'
        if date_invalid_num & ic_all_dtc & ic_all_dt & ic_all_c & top_ret & half_ic & dtc_all_sign & corr:
            test_report += '大者越大，强者恒强。\n'
            test_report += 'Congratulations! Your out of sample test SUCCEEDED as expected!\n'
            program.update(result)
            program.update({'factor_sample': factor_sample.astype(np.float32)})
            save_pickle(address3 + file_time, program)
            np.save(factor_address + '%s.npy' % file_time, factor)

        else:
            test_report += '行百里者，半于九十。\n'
            test_report += 'We regret to tell you that your out of sample test FAILED!\n'
        ...
        _ic_half_dt = list(simple_result['ic_half_dt'].round(4))
        test_report += 'Half Year IC:'.ljust(18) + str(_ic_half_dt[:2]) + '\n'
        test_report += ''.ljust(18) + str(_ic_half_dt[2:]).ljust(18)
        test_report += '  ' + judge[half_ic] + '\n'
        if date_invalid_num & ic_all_dtc & ic_all_dt & ic_all_c & top_ret & half_ic & dtc_all_sign:
            test_report += 'Corr Max:'.ljust(18) + str(round(corr_max, 4)
                                                         ).ljust(6) + ''.ljust(13) + judge[corr] + '\n'
        ...
        if date_invalid_num & ic_all_dtc & ic_all_dt & ic_all_c & top_ret & dtc_all_sign & half_ic & corr:
           send_message('015836', test_report)
           send_message(program['program_author'], program_code + ' 样本外测试通过')

        return date_invalid_num & ic_all_dtc & ic_all_dt & ic_all_c & top_ret & half_ic & dtc_all_sign & corr