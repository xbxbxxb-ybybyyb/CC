...
from HFfactor.MinFactorSuper.RealTime.UsefulList import MaterialList, LowFreqList, MinuteList, DesampleMethod, MaterialDistAdjust
...
local_path = '/data/group/800442/800319/strategy_HFfactor4/'
...
class MinFactorCalculator(object):
    ...
    def calc_sig_factor(self, name, formula, lag_days, bar, pre_bar, no_dt=False):
        ...
        finite = np.isfinite(self.FACTOR[name])
        bottleneck2.clip_array_2d(self.FACTOR[name])
        self.FACTOR[name][~ finite] = np.nan
    ...
    def calc_dp_sig_factor(self, name, formula, lag_days, bar, pre_bar, no_dt=False):
        ...
        finite = np.isfinite(self.M5FACTOR[name])
        bottleneck2.clip_array_2d(self.M5FACTOR[name])
        self.M5FACTOR[name][~ finite] = np.nan
...
if __name__ == '__main__':
    mfc = MinFactorCalculator(20210803)
    mfc.calc_bar_data(1000, 0, back_test=True, threads=48)