...
class StartWithLimitCashVolConsider(PortfolioStrategyBase):
    def __init__(self, long_signal,long_pred_ret, short_signal, start=20140101, end=20181231, stock_pool=None, target_point=None,
                 ...
                 cash_added={}, per_ratio_change={}, max_trigger_num={}, index_list=['SZZZ', 'SZCZ'],
                 condition_series={}, discount_ratio_series={}, intra_discount_ratio={},intra_discount_ratio2={}):
        ...
        self.long_signal = {i: long_signal[i].reindex(self.close.index) for i in long_signal}
        self.long_pred_ret = {i: long_pred_ret[i].reindex(self.close.index) for i in long_pred_ret}
        self.short_signal = {x: short_signal[x].reindex(self.close.index) for x in short_signal}
        self.data_flow['signal'] = None
        self.data_flow['short_signal'] = None
        ...
        self.intra_discount_ratio2 = pd.Series(intra_discount_ratio2).reindex(self.close.index).fillna(1)
        print(f"timing signal info {pd.DataFrame({'timing':self.intra_discount_ratio}).groupby('timing').size()}")
        self.loaded_cash = 0
    ...
    def daily_update(self, idx, date):
        ...
        self.data_flow['long_signal'] = {i: self.long_signal[i][self.date_idx * self.step:(self.date_idx + 1) * self.step] for i in self.long_signal}
        self.data_flow['short_signal'] = {x: self.short_signal[x][self.date_idx * self.step:(self.date_idx + 1) * self.step] for x in self.short_signal}
        self.data_flow['long_pred_ret'] = {i: self.long_pred_ret[i][self.date_idx * self.step:(self.date_idx + 1) * self.step] for i in self.long_signal}
        ...
        self.data_flow['intra_discount_ratio2'] = self.intra_discount_ratio2.iloc[self.date_idx * self.step:(self.date_idx + 1) * self.step]
        # 不在股票池里的股票
        ...
        for i in self.data_flow['long_signal']:
            if self.data_flow['long_signal'][i].index[0][0] != self.date or self.data_flow['long_signal'][i].index[-1][0] != self.date:
                raise Exception('Broadcast date and signal date are not match!')
        for i in self.data_flow['short_signal']:
            if self.data_flow['short_signal'][i].index[0][0] != self.date or self.data_flow['short_signal'][i].index[-1][0] != self.date:
                raise Exception(f'Broadcast date and signal date are not match!')
        self.data_flow['short_signal'] = {x: dict(self.data_flow['short_signal'][x].T[self.date]) for x in self.data_flow['short_signal']}
        for w_d in self.data_flow['short_signal']:
            for t_point in self.data_flow['short_signal'][w_d]:
                self.data_flow['short_signal'][w_d][t_point] = self.data_flow['short_signal'][w_d][t_point].dropna().index.tolist()
    ...
    def holding_more_bars(self, trigger_stk=set()):
        ...
        for temp_window in range(1, len(self.trading_point) - time_idx):
            temp_signal = self.data_flow['long_signal'][temp_window][time_idx:time_idx + 1].T[(date, time_point)]
            temp_signal = set(temp_signal[temp_signal].index)
    ...
    def bar_handler(self, bar_close):
        ...
        long_signal = self.data_flow['long_signal'][future_window][time_idx:time_idx + 1].T[(date, time_point)]
        pred_ret = self.data_flow['long_pred_ret'][future_window][time_idx:time_idx + 1].T[(date, time_point)]
        if self.discount_ratio<=1:
            signal = pred_ret[long_signal]
        else:
            signal = pred_ret
        signal = signal.dropna()
        trigger_stk = set(signal.index)
        if self.discount_ratio == 0:
            self.forbidden_stk = self.forbidden_stk.union(trigger_stk)
        trigger_stk = trigger_stk - self.forbidden_stk
        # 没有半路看跌信号的看涨信号
        all_short = set()
        for i in range(1, len(self.trading_point) - time_idx + 1):
            temp_short = self.data_flow['short_signal'][i][time_point]
        ...
        bar_basic_indicator = {
            ...
            'not_long': self.discount_ratio<=1,
            'not_long2':self.not_long2<=1
        }
        ...
        bar_basic_indicator['terminal_flag'] = eval(self.condition, bar_basic_indicator)    # eval的第二个参数 globals()
        ...
        if self.cash < self.per_amt:
            ...
            target_amt = target_amt[target_amt.cumsum() < self.cash]    # 按顺序买入，如果到了哪个没有足够cash了，那个就不买了
    ...
    def run_backtest(self, kernel=10):
        ...
        for date_idx, date in enumerate(bar):
            ...
            for time_idx, time_point in enumerate(self.trading_point):
                ...
                self.bar_point = date_idx * self.step + time_idx    # 总的bar的index，把日期的也算进去了
                ...
                self.not_long2 = self.data_flow['intra_discount_ratio2'].iloc[[time_idx]][(date, time_point)]
                self.forbidden_stk = set([])
            ...
            self.barly_holding_info[(date, 1500)] = self.holding.copy() # 设定收盘时的持仓情况
            self.cash_series[self.date] = self.cash # 设定当天收盘后的现金情况
            self.holding_num[self.date] = len(self.holding) # 设定当天收盘时的持仓数量
            daily_close = self.daily_info['close'][self.date_idx:self.date_idx + 1].T[self.date][list(self.holding.keys())]
            self.accout_value = (daily_close * pd.Series(self.holding)).sum() + self.cash   # 当天收盘时的持仓+现金
            self.holding_value[(self.date, 1500)] = self.accout_value   # 当天收盘时的持仓+现金
