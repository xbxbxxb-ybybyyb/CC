...
condition_map = {
    ...
    'NoCondition':'False',
    'NotLongCondition':'(((bar_first_trigger_num/bar_ratio)>(0.15*pool_num)) or ((bar_cum_first_trigger_num/barly_cum_ratio)>(0.15*pool_num))) and not_long',
    'NotLongConditionNoTiming':'(((bar_first_trigger_num/bar_ratio)>(0.15*pool_num)) or ((bar_cum_first_trigger_num/barly_cum_ratio)>(0.15*pool_num))) and not_long2',
}
...
def calc_back_test_record(per_amt_ratio, initial_cash, pool_num, deal_ratio, tag, alpha_pool_tag, timing_tag, down_definition,condition_tag):
    start = 20170101
    end = 20211130
    # end = 20171231
    condition_series = {
        get_pre_trade_date(start):condition_map[condition_tag]#'(((bar_first_trigger_num/bar_ratio)>(0.15*pool_num)) or ((bar_cum_first_trigger_num/barly_cum_ratio)>(0.15*pool_num))) and not_long'
    }
    ...
    timing_signal2 = timing_signal.copy()
    if condition_tag=='NotLongConditionNoTiming':
        timing_signal = pd.Series()

    tag = f'{tag}_{alpha_pool_tag}_{timing_tag}_{condition_tag}_NonfixWindow'  # _{down_signal_ratio:.2f}_{signal_threshold:.2f}_{down_definition:.3f}_NonFixSignal1000Base8Bar'
    ...
    base_dir = '/data/group/800442/800319/MarketTiming/NonFixWindowMannualSignalCondition2/'
    ...
    if run_flag:
        ...
        instance = StartWithLimitCashVolConsider(long_signal,long_pred_ret, short_signal, start, end, stock_pool=alpha_pool, target_point=bar_list, buy_cost=cost, sell_cost=cost,
                                                 ...
                                                 down_swing_threshold=down_definition, intra_discount_ratio=timing_signal,
                                                 intra_discount_ratio2=timing_signal2)
...
# tm_tag = 'long_short3'
# tm_tag = 'NoTiming'
tm_tag,c_tag = eval(AIMR.getParam())
# tm_tag = 'EMDT_trend_fix_signal'
# tm_tag = 'XGB300'
print(tm_tag)
calc_back_test_record(0.005, 2e8, 600, 0.1, 'XGB_DTC_Matrix_Light_CatNonFixWindow', 'CS_XGB_OLS_condition_style_rank_ex20', tm_tag, 0,c_tag)
print(tm_tag)
