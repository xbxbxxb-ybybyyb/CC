...
base_path = '/data/group/800442/800319/MarketTiming/NonFixWindowWithLongLimitNumFake100/'
# base_path = '/data/group/800442/800319/MarketTiming/NonFixWindowWithLongFakeV1/'
all_file = f'{base_path}/非固定时点框架根据胜率模拟信号对比.xlsx'
# file_list = sorted(list(filter(lambda x: x.startswith('_LongV1'), os.listdir(base_path))))
file_list = sorted(list(filter(lambda x: x.startswith('M'), os.listdir(base_path))))
...
for file_name in file_list:
    clf_name = file_name.replace('s20170101_e20211130_XGB_DTC_Matrix_Light_CatNonFixWindow_CS_XGB_OLS_condition_style_rank_ex20_', '').replace('_NonfixWindow_10bp_cost.xlsx', '')  # [108:-28]
