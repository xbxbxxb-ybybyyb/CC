...
from R2D2.ChangingCash.StartWithLimitCashVolConsiderNonFixSignal8BarExtraContitionChangingCashByRatioLimitShortLongV1 import StartWithLimitCashVolConsider, \
...
# 获取参数
if len(sys.argv) > 1:
    search_times = sys.argv[1]
    tag_name = sys.argv[2]
    search_flag = True
else:
    search_flag = False

debug = False    # TODO:进行每次修改
if debug:
    search_times = 0
    tag_name = 'XGB400_dc'
    search_flag = True

hx_signal_path = f'/data/group/800442/800319/Timing/BackTest/Signal/hyper_search_{tag_name}/{search_times}'
out_signal_path = '/data/group/800442/800319/Timing/BackTest/FormatedSignal/'   # 信号转换后的路径地址

hx_signal_list = [f'long_pred_{tag_name}', f'short_pred_{tag_name}', f'{tag_name}']
for each in hx_signal_list:
    ...
    print(f'已保存到{out_signal_path}/{each}.pkl')
...
long_param = {
    i: f'/data/user/015664/AFuckingTrigger/限制买入和持仓/信号存储/long_nonfix_window_8barOriginFactorExpandForCondition//signal_long_XGB_DTC_Matrix_Light_Cat_Future_{i}_Bar_pct_0.05.pkl'
    for
    i in range(1, 9)}
short_param = {
    i: f'/data/user/015664/AFuckingTrigger/限制买入和持仓/信号存储/short_8barOriginFactorExpandForCondition/signal_short_XGB_DTC_Matrix_Light_Cat_Future_{i}_Bar_pct_0.pkl'
    for i in
    range(1, 8)}
...
def calc_back_test_record(per_amt_ratio, initial_cash, pool_num, deal_ratio, tag, alpha_pool_tag, timing_tag, down_definition):
    ...
    if timing_tag == 'NoTiming':
        ...
        timing_signal_path = '/data/group/800442/800319/Timing/BackTest/FormatedSignal/'
        timing_signal = pd.Series(pd.read_pickle(f'{timing_signal_path}{timing_tag}.pkl'))
    tag = f'{tag}_{alpha_pool_tag}_{timing_tag}_NonfixWindow'
    file_name = '_LongV1_Max100_%s_%dbp_cost' % (tag, int(10000 * cost))
    base_dir = '/data/group/800442/800319/MarketTiming/NonFixWindowMannualSignal2/'
    ...
    if not search_flag:
        ...
        if not os.path.exists(f'{base_dir}/FC_hyper_search/'):
            os.makedirs(f'{base_dir}/FC_hyper_search/', exist_ok=True)
        out_path = f'{base_dir}/FC_hyper_search/{file_name}_{search_times}.xlsx'
    ...
    #     res = pd.read_pickle(record_file)
    #     if len(res)==5:
    #         record, cash_series, holding_num = res[:3]
    #         run_flag = False
    ...
    if run_flag:
        ...
        append_pool = {}
        pool_path = '/data/group/800442/800319/StrategyBackup/code_list/'
        ...
        for i in range(1, 9):
            ...
            long_pred_ret[i],long_signal[i] = temp_long_pred_ret,temp_long_sigal.fillna(False)
        ...
        instance = StartWithLimitCashVolConsider(long_signal,long_pred_ret, short_signal, start, end, stock_pool=alpha_pool, target_point=bar_list, buy_cost=cost, sell_cost=cost,
        ...
        barly_holding = instance.barly_holding_info
        pd.to_pickle([record, cash_series, holding_num, instance.condition_series,barly_holding], record_file)
...
timing_tag_list = ['EMDT_trend_fix_signal', 'SLM_fix_signal', 'SLM_fix_signal_V2', 'NoTiming']
...
if search_flag:
    hyper_search_signal_path = f'/data/group/800442/800319/Timing/BackTest/Signal/hyper_search_{tag_name}/'
    if not os.path.exists(hyper_search_signal_path):
        os.makedirs(hyper_search_signal_path)
    shutil.copyfile(out_path, hyper_search_signal_path + f'bt_result_{search_times}.xlsx')
    print(f'已复制到{hyper_search_signal_path}bt_result_{search_times}.xlsx')
