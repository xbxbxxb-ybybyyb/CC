...
def calc_back_test_record(per_amt_ratio, initial_cash, pool_num, deal_ratio, tag, alpha_pool_tag, timing_tag, down_definition):
    ...
    if timing_tag == 'NoTiming':
        ...
        timing_signal_path = '/data/group/800442/800319/Timing/ReportFactor/Strategy/'
        longshort, ttag = timing_tag.split('_', 1)


        timing_signal = pd.read_pickle(f'{timing_signal_path}{ttag}.pkl')
        timing_signal.index = timing_signal.index.astype(int)
        timing_signal.columns = timing_signal.columns.astype(int)
        timing_signal = timing_signal.stack()
        if longshort == 'long':
            timing_signal = timing_signal.replace(-1, 0) + 1
        elif longshort == 'short':
            timing_signal = timing_signal.replace(1, 0) + 1
        else:
            timing_signal = timing_signal + 1
        print(f'{timing_signal_path}{ttag}.pkl',timing_signal.max(),timing_signal.min(),timing_signal.sum())
        # return
    ...
    base_dir = '/data/group/800442/800319/Timing/ReportFactor/XuqiBacktest_Loop/'
    ...
    if not os.path.exists(f'{base_dir}/intra_discount/'):
        os.makedirs(f'{base_dir}/intra_discount/')
    pd.to_pickle(instance.intra_discount_ratio,f'{base_dir}/intra_discount/{file_name}.pkl')
...
timing_tag_list = [
    'DTW_daily_signal',
    'Diffusion_combined_daily_signal',
    'Diffusion_high_daily_signal',
    'Diffusion_kdj_daily_signal',
    'Diffusion_ma_daily_signal',
    'Diffusion_roc_daily_signal',
    'EMDT_reverse_signal',
    'EMDT_trend_fix_signal',
    'EMDT_trend_reverse_signal',
    'EMDT_trend_signal',
    'Hurst_daily_trend_signal',
    'SLM_day_signal',
    'SLM_fix_signal',
    'SMT_GF_abs_dis_signal_5d',
    'SMT_GF_corr_signal_5d',
    'SMT_GF_euc_dis_signal_5d',
    'TA_BBOT_daily_signal',
    'TA_BTOP_daily_signal',
    'TA_HS_daily_signal',
    'TA_RTOP_daily_signal',
    'TA_TBOT_daily_signal',
    'TA_TTOP_daily_signal',
]
test_direction = ['long', 'short', 'longshort']
from xquant.compute.aimr import AIMR

def _func(ttag):
    calc_back_test_record(0.005, 2e8, 600, 0.1, 'XGB_DTC_Matrix_Light_CatNonFixWindow',
                          'CS_XGB_OLS_condition_style_rank_ex20', ttag, 0)
...
todo_list = [td + '_' + tag for tag in timing_tag_list for td in test_direction]
# from dataApi import aimr
# idd, parts = eval(aimr.getParam())
# play_aimr(idd, parts, _func, todo_list)
# from xquant.compute.aimr import AIMR
