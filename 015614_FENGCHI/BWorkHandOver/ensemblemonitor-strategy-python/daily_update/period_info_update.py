...
local_config_path = '/data/group/800442//800319/strategy_local_path_offline/'
...
try:
    ...
    if end<20210601:
        period_info = get_rolling_index(start, 20210525, 200, 10) + get_rolling_index(20200728,20210527,200,1)[:-1] + get_rolling_index(20200730,end,200,3)
    elif end<20210617:
        period_info = get_rolling_index(start, 20210525, 200, 10) + get_rolling_index(20200728, 20210527, 200, 1)[:-1] + \
                      get_rolling_index(20200730, end, 200, 3)[:1] + get_rolling_index(20200804, end, 200, 12)
    else:
        period_info = get_rolling_index(start, 20210525, 200, 10) + get_rolling_index(20200728, 20210527, 200, 1)[:-1] + \
                      get_rolling_index(20200730, end, 200, 3)[:1] + get_rolling_index(20200804, end, 200, 12)[:1] + get_rolling_index(20200820, end, 200, 10)
    print(period_info)
    ...
    if last_cell[1] == last_cell[2] and last_cell[2] == last_cell[3]:
        ...
        if end < 20210601:
            period_info = get_rolling_index(start, 20210525, 200, 10) + get_rolling_index(20200728, 20210527, 200, 1)[:-1] +\
                          get_rolling_index(20200730, get_pre_trade_date(end), 200, 3)

        elif end < 20210617:
            period_info = get_rolling_index(start, 20210525, 200, 10) + get_rolling_index(20200728, 20210527, 200, 1)[:-1] + \
                          get_rolling_index(20200730, get_pre_trade_date(end), 200, 3)[:1] + get_rolling_index(20200804, get_pre_trade_date(end), 200, 12)
        else:
            period_info = get_rolling_index(start, 20210525, 200, 10) + get_rolling_index(20200728, 20210527, 200, 1)[:-1] + \
                          get_rolling_index(20200730, get_pre_trade_date(end), 200, 3)[:1] + get_rolling_index(20200804, get_pre_trade_date(end), 200, 12)[:1] + get_rolling_index(20200820, get_pre_trade_date(end), 200, 10)
        # else:
        #     period_info = get_rolling_index(start, 20210525, 200, 10) + get_rolling_index(20200728, 20210527, 200, 1)[:-1] + \
        #                   get_rolling_index(20200730, get_pre_trade_date(end), 200, 3)[:1] + get_rolling_index(start, get_pre_trade_date(end), 200, 10)[132:]
