...
from StrongStockModel.dataApi.stockList import trans_int2windcode, trans_windcode2int, clean_stock_list, get_all_stock_ever_appear
# from StrongStockModel.dataApi.getData import get_minute_1factor, get_daily_1factor
from StrongStockModel.dataApi.getData import get_minute_pickle, get_daily_1factor
...
init_conf_path, holding_info_path, vol_info_path, code_list_path, restrict_list_path, local_config_path, hyper_param_path, model_config_path, path_for_930 = \
    [path_conf[x] for x in
     'init_conf_path, holding_info_path, vol_info_path, code_list_path, restrict_list_path, local_config_path, hyper_param_path, model_config_path, path_for_930'.split(', ')]
...
def daily_update(date, T_plus_1_date, n_threads=10):
    """
    ...
    code_list = get_all_stock_ever_appear(date)  # get_code_list(date, restrict_list_path)
    ...
    print('vol done')
    rd_mean, rd_std = get_factor_mv(date, factor_list, code_list, n_threads=n_threads)
...
daily_update(pre_date, today, n_threads=24)
...
if today == 20220309:
    ...
    send_message(['015664'],'ignore zero stock done')
"""
