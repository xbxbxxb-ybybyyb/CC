...
from active_pool_since20210908.Application930ActivePool import Application930

config_path = get_path_conf('/data/group/800442/800319/EMExternalPoolTrace/strategy_local_path_TX/',True)
indi_list = "local_config_path, holding_info_path, hyper_param_path, code_list_path, model_config_path, buy_time_info_path, vol_info_path, init_conf_path, daily_out_path, ratio_path"
...
class Application:
    ...
    def __init__(self, date, log=print):
        ...
        file_list = sorted(list(filter(lambda x: x < 'model_conf%d.pkl' % date and x.endswith('.pkl'), file_list)))
        if not file_list:
        ...
        # unavailable_pool = {str(x).zfill(6) + '.SZ' if x < 400000 else str(x) + '.SH' for x in unavailable_pool}
        ...
        self.factor_mean = factor_mean.loc[factor_list, self.pool_with_over_night_stk].T
        self.factor_std = factor_std.loc[factor_list, self.pool_with_over_night_stk].T
        self.pred_ret = {}
        self.signal = {}
        self.factor = {}
        self.time = None
    ...
    def get_first_target_plan(self):
        ...
        # TODO 每天保存一个930 和 7个bar持仓比例分配的文件，用于计算当天实际Holding里的哪些是930的，哪些是7个bar的
    ...
    def predict_by_multi_model(self, factor, time_point=None):
        ...
        for each in self.model_list:
            ...
            if 0 in filtered_factor[tag].shape:
                res[each] = pd.Series(index=filtered_factor[tag].index)
