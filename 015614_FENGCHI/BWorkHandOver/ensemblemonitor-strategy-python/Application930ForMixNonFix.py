...
# from online_conf import non_fix_930_path,non_fix_output_path,non_fix_in_path,non_fix_path
...
sub_local_config_path = non_fix_930_path
...
class Application930:
    ...
    def __init__(self, date, log=print):
        ...
        if portfolio_id is None:
            portfolio_id = -1
        unavailable_pool = pd.read_pickle(f'{non_fix_in_path}/{date}/restrict_list{pre_date}.pkl')

        signal = pd.read_pickle(f'{non_fix_path}morning_model/val_sign/{date}.pkl')
        code_list = signal.index.tolist()
        # close = pd.read_pickle(f'{market_data_path}{date}/close.pkl')
    ...
    def holding_info_update(self, holding):
        ...
        if not os.path.exists(f'{sub_output_path}{self.date}/'):
            os.makedirs(f'{sub_output_path}{self.date}/')
