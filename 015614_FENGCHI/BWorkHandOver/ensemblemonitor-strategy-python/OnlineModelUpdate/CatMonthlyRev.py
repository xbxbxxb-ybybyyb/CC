...
class XGBRegressionFactorEvalYearly(ModelNewLoading):
    ...
    def train_model(self, X_train, y_train, params, end_date=None):
        key_list = set(params.keys()).intersection(
            set(['loss_function', 'iterations', 'learning_rate', 'depth', 'l2_leaf_reg', 'eval_metric', 'task_type', 'random_seed']))
        ...
        if 'load local model' in params and os.path.exists(params['model_conf_path'] + '%d.json' % end_date):
            ...
            model.fit(X=train_features,y=train_label['actual_label'],
                      verbose=True)
...
def main_window_search(i, indicator):
    ...
    out_file = '/data/group/800442/800319/Faamonitor/PL/all_mkt_ts_norm/MonthlySelected/CatNoEarlyV4FactorList_%s_train%d_test%d_factor_num%d.pkl' % (
    ...
    best_param_clf_xgb = {'loss_function': 'MAE', 'iterations': 200, 'learning_rate': 0.03,
                          'depth': 6, 'l2_leaf_reg': 3, 'eval_metric': 'MAE', 'task_type': 'GPU',
                          'random_seed': 1}
...
# i = int(AIMR.getParam())
#
# for ind_name in ['ic_dt', 'ic_tc', 'ic_dtc']:
#     # for ind_name in ['top_ret'][::-1]:
#     main_window_search(i, ind_name)


idx_list = list(range(138))[24:][::-1]
for i in tqdm(idx_list[:len(idx_list)//2][::-1]):
    for ind_name in ['ic_dt']:#['ic_dt', 'ic_tc', 'ic_dtc']:
        # for ind_name in ['top_ret'][::-1]:
        main_window_search(i, ind_name)
        gc.collect()
