...
class XGBRegressionFactorEvalYearly(ModelNewLoading):
    ...
    def train_model(self, X_train, y_train, params, end_date=None):
        ...
        val_date = [date_list[i] for i in list(range(-40,0))[::-1]]
        ...
        if 'load local model' in params and os.path.exists(params['model_conf_path'] + '%d.json' % end_date):
            ...
            model = xgb.train(args_param, d_train, num_boost_round=params['n_estimators'], evals=[(d_val, 'd_eval')], verbose_eval=True)
...
def main_window_search(i, indicator):
    ...
    out_file = '/data/group/800319/Faamonitor/PL/all_mkt_ts_norm/MonthlySelected/XGBV4FactorList40Val_%s_train%d_test%d_factor_num%d.pkl' % (
...
for i in tqdm(list(range(133))[::-1]):
    for ind_name in ['ic_c','ic_t','ic_d']:
...
# factor_list_path = '/data/group/800319/Faamonitor/PL/all_mkt_ts_norm/MonthlySelected/XGBV4FactorList_%s_train200_test10_factor_num400_factor_list/'
#
