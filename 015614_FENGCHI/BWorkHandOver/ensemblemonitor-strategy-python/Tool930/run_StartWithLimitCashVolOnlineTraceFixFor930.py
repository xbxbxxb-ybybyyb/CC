...
code_list_path, local_config_path = [path_conf[x] for x in ['code_list_path', 'local_config_path']]
...
today = 20210330  # int(datetime.date.today().strftime('%Y%m%d'))
...
for date in get_date_range(get_pre_trade_date(backtest_start_date), pre_date):
    temp_pool = pd.read_pickle(f'/data/group/800319/strategy_local_path/code_list_no688/{date}.pkl')
...
# unavailable_pool = pd.read_pickle(f'{local_config_path}restrict_list.pkl')#set(isolation_pool).union(set(black_name_list))
...
instance = StartWithLimitCashVolConsiderChangingCash(pred_ret, backtest_start_date, pre_date, stock_pool=alpha_pool, target_point=bar_list, buy_cost=cost, sell_cost=cost,
                                                     ...
                                                     deal_percent=deal_ratio, initial_cash=2000000, cash_added=cash_flow)
...
# pd.to_pickle([instance.sell_order_record, instance.buy_order_record],
#              '/data/user/015664/AFuckingTrigger/限制买入和持仓/实盘回测线下跟踪/validation_file/%sO_buy_sell_order_record.pkl' % tag)
...
out_path = '/data/user/015664/AFuckingTrigger/限制买入和持仓/实盘回测线下跟踪/930/%sVolConsiderOnlineLimit_UpBuy100_%dbp_cost_%d.xlsx' % (tag, int(10000 * cost), pre_date)
...
#
# for date in get_date_range(20210302,20210330):
#     if date==20210329:
#         continue
#     shutil.copy('/data/user/015664/AFuckingTrigger/限制买入和持仓/信号存储/实盘跟踪线下信号//signal_OutSample_XGB_Cat_Light_OnlineTest_20210329.pkl',
#                 '/data/user/015664/AFuckingTrigger/限制买入和持仓/信号存储/实盘跟踪线下信号//signal_OutSample_XGB_Cat_Light_OnlineTest_%d.pkl'%date)
