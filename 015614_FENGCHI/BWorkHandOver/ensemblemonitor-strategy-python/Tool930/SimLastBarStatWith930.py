...
import os
from dataApi.getData import get_pre_trade_date, trans_int2windcode
import configparser
...
def main_stat(date, T_plus_1, final_holding, div_cash=0, cash_add=0):
    ...
    daily_initial_generation(T_plus_1_date=T_plus_1, date=date, barly_max_buy=100, stk_min_amt=0.2, per_signal_ratio=0.005, order_ratio=0.1, cash_add=cash_add)
...
if __name__ == '__main__':
    ...
    # init_at_first_day(20210921,reinitial_930=True,initial_cash_930=3000000)
    today = 20210928#int(datetime.date.today().strftime('%Y%m%d'))
    import time
