...
from StrongStockModel.backtest.StrategyBackTest.DerivativeStrategy.StartWithLimitCashVolConsider930 import StartWithLimitCashVolConsider930, InitailCashBasedEvaluationHelper
...
pred_ret = {}
for date in get_date_range(20210302, 20210430):
    pred_ret[date] = pd.read_pickle( f'/data/group/800319/strategy_local_path3/morning_model/val_sign/{date}.pkl')
pred_ret = pd.DataFrame(pred_ret).T
pred_ret.columns = pred_ret.columns.map(lambda x : int(x[:-3]))
pred_ret.index = pd.MultiIndex.from_tuples([(x,930) for x in pred_ret.index])
...
tag = tag.replace('RevTriggerFilterHolding', '930PoolTop600Out')
...
instance = StartWithLimitCashVolConsider930(pred_ret, 20210302, 20210430, stock_pool=alpha_pool, target_point=bar_list, buy_cost=cost, sell_cost=cost,
...
pd.to_pickle([instance.daily_holding, instance.daily_buy_time_info, instance.daily_conf], '/data/group/800319/strategy_local_path/FolderFor930/fake_sample_new_frame_out_sample.pkl')
...
#
# daily_holding,daily_buy_time_info,daily_conf = pd.read_pickle('/data/group/800319/strategy_local_path/FolderFor930/fake_sample_new_frame.pkl')
# vol_info = pd.read_pickle(deal_price_path + 'vol_rolling_future_30min_sum_5day_mean.pkl')
# vol_info = vol_info.swaplevel(0,1).loc[930].shift(1).loc[20201101:]
...
#     pd.to_pickle(vol_info.loc[date].fillna(0), f'/data/group/800319/strategy_local_path3_for_930/FolderFor930/{date}/StrategyIn/vol_info{date}.pkl')
#
