...
"""
matrix_0406 = pd.read_pickle('/data/group/800319/strategy_local_path3_ForMatrix/relation_matrix/20211116.pkl')['sw1']
factor_0406 = pd.read_pickle('/data/group/800319/strategy_local_path3_ForMatrix/daily_output/20211116/all_factor_1000.pkl').T
...
offline = temp_res[14][0]
offline_count = count[14][0]
...
compare_sum = pd.DataFrame({'offline':offline,'online':online},index=_code_list)
compare = pd.DataFrame({'offline':offline/offline_count,'online':online/online_count},index=_code_list)
compare_norm_factor = pd.DataFrame({'offline':factor_arr[14,0,:],
                             'online':factor_0406.loc[factor_name].fillna(0)},index=_code_list)
pd.Series(factor_arr[14,0,:],index=_code_list)[pd.Series(relation_arr[14,1,:],index=_code_list).astype(bool)]
factor_0406.loc[factor_name].fillna(0)[matrix_0406.fillna(0).iloc[:,1].astype(bool)]#.sum()
compare_same_industry = pd.DataFrame({
    'offline':pd.Series(factor_arr[14,0,:],index=_code_list)[pd.Series(relation_arr[14,1,:],index=_code_list).astype(bool)],
    'online':factor_0406.loc[factor_name][matrix_0406.fillna(0).iloc[:,1].astype(bool)]
})

compare_same_industry[compare_same_industry['offline'].isin([0])]
compare_same_industry[compare_same_industry['online'].isnull()]
"""
##################
