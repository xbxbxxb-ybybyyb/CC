...
try:
    ...
    if not os.path.exists(f'{local_config_path}/portfolio_file/{date}/'):
        os.makedirs(f'{local_config_path}/portfolio_file/{date}/')
    portfolio_file.to_excel(f'{local_config_path}/portfolio_file/{date}/EMPortFile{date}_{account}_with_930.xlsx')
    send_message(['015664'],'仿真交易组合生成成功')
