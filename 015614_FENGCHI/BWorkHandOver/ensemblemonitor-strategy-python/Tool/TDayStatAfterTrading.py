...
def get_all_trading_record(date):
    ...
    if os.path.exists(f'{daily_out_path}{date}/last_bar_holding.pkl'):
        ...
        try:
            ...
            fake_holding = pd.read_pickle(f'{local_config_path}fake_barly_info/{date}/1500.pkl').set_index('Symbol')
    pre_time = None
    pre_holding = None
...
def profit_stat(stat_date,buy_cost=0.0001,sell_cost=0.0011):
    ...
    return record
import datetime
day = 20210706#int(datetime.date.today().strftime('%Y%m%d'))#20210310
