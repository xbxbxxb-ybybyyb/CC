...
import pandas as pd
# from online_conf import local_config_path,daily_out_path,buy_time_info_path
...
def get_all_trading_record(date):
    ...
    if os.path.exists(f'{non_fix_output_path}{date}/last_bar_holding.pkl'):
        fake_holding = pd.read_pickle(f'{non_fix_output_path}{date}/last_bar_holding.pkl')
        fake_holding.index.names = ['Symbol']
    else:
        fake_holding = pd.read_pickle(f'{non_fix_output_path}{date}/final_summary.pkl')['barly_holding_info'][1000].set_index('Symbol')
...
def profit_stat(stat_date,buy_cost=0.0001,sell_cost=0.0011):
    ...
    if os.path.exists(f'{daily_out_path}{stat_date}/last_bar_holding.pkl'):
        ...
        try:
            ...
            fake_output = pd.read_pickle(f'{local_config_path}fake_barly_info/{stat_date}/1500.pkl').set_index('Symbol')
    ...
    return record
import datetime
day = 20220411#int(datetime.date.today().strftime('%Y%m%d'))#20210310
out_path = '/data/user/015664/AFuckingTrigger/限制买入和持仓/仿真回测线下跟踪NonFix/'
if not os.path.exists(f'{out_path}{day}/'):
    os.makedirs(f'{out_path}{day}/')
