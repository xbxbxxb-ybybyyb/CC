...
while True:
    try:
        app = Application(date=date)
        lm.sendMessage(f'930信号数量:{len(app.app930.signal)}, 930持仓数量: {len(app.app930.holding)} {len(app.involved_instance)}')
        lm.sendMessage(f'barly_max_buy {app.barly_max_buy,app.app930.barly_max_buy}')
        break
    except:
