...
pre_date = 20210430#get_pre_trade_date(today)
...
period_list = end_list[end_list.index(start):]
...
alpha_pool = pd.read_pickle('/data/group/800319/AlphaPool/CS_OLS_condition_style_rank_ex20_F600T488P1_future925t241h135d_uniform20t50.pkl')
# alpha_pool.columns = [int(x[:-3]) for x in alpha_pool.columns]
alpha_pool = alpha_pool.reindex(original_pool.columns, axis=1).fillna(False).shift(1).loc[backtest_start_date:].rank(ascending=False,axis=1)<600
...
tag = tag.replace('RevTriggerFilterHolding', 'future925t241h135d')
...
out_path = '/data/user/015664/AFuckingTrigger/限制买入和持仓/样本外韩旭股票池实验/%sVolConsiderOnlineLimit_UpBuy100_%dbp_cost_%d.xlsx' % (tag, int(10000 * cost), pre_date)
