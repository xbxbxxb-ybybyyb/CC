...
def daily_initial_generation(T_plus_1_date,date,barly_max_buy,stk_min_amt,per_signal_ratio,order_ratio,afternoon_holding,portfolio):
    ...
    if POSITION_CHANGING:
        send_message(['015664'],f'FIX:{date}收盘现金+股票市值 {round(account_cap,2)}，账面相对前日收益{round(account_cap - pre_account_values,2)}，相对前日收益 {round((account_cap / pre_account_values - 1 )* 100,4)}%，持仓股票 {len(holding_series)} 只,其中0股{len(holding_series[holding_series<100])}只，持仓市值 {round(cap.sum(),2)}， 剩余现金 {round(cash,2)}')
    else:
        send_message(['015664','003186','016385','015836','011669'],f'FIX:{date}收盘现金+股票市值 {round(account_cap,2)}，账面相对前日收益{round(account_cap - pre_account_values,2)}，相对前日收益 {round((account_cap / pre_account_values - 1 )* 100,4)}%，持仓股票 {len(holding_series)} 只,其中0股{len(holding_series[holding_series<100])}只，持仓市值 {round(cap.sum(),2)}， 剩余现金 {round(cash,2)}')
...
def main(actual_cash):
    today = 20211111#int(datetime.date.today().strftime('%Y%m%d'))
...
if __name__=="__main__":
    main(-14651263.975549705 +  36447028.55)#202101015
