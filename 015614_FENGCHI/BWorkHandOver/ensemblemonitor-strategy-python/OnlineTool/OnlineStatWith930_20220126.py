...
def get_holding_930(date, final_total_holding, cash_added=0, div_cash=0, holding_added=pd.Series()):
    ...
    holding = final_total_holding.copy()#summary['barly_holding_info'][1000]
    holding['SellAvailable'] = 0
...
def AfterHoursStat(pre_date, today, T_plus_1_day, portfolio, cash_add, actual_all_cash, cash_add_930):
    ...
    trading_record.loc[trading_record['委托时间'] > '10:00:00','委托时间'] = '09:35:00'
    ...
    summary = pd.read_pickle(path_conf['daily_out_path']+f'{today}.pkl' )
    for bar in summary['barly_total_holding_info']:
        if bar==930:
            continue
        else:
            summary['barly_total_holding_info'][bar] = final_total_holding
    pd.to_pickle(summary,path_conf['daily_out_path']+f'{today}.pkl')
    ...
    daily_initial_generation930(T_plus_1_date=get_pre_trade_date(today, -1), date=today, barly_max_buy=0, stk_min_amt=0.2, per_signal_ratio=0.005, order_ratio=0.1)
    ...
    daily_initial_generation(T_plus_1_date=T_plus_1_day, date=today, barly_max_buy=0, stk_min_amt=0.2, per_signal_ratio=0.005,
...
def main(actual_cash):
    ...
    print(today)
    os.system(f'cp -r /data/user/015664_old/StrategyOutput/{today}*  /data/user/015664/StrategyOutput/')
    os.system(f'cp -r /data/user/015664_old/StrategyOutput/out_930/{today}*  /data/user/015664/StrategyOutput/out_930/')
...
if __name__ == "__main__":
    main(44890478.57 - 1002743.11-3000000-3000000+25288-4000000+300000)  # 20220124
