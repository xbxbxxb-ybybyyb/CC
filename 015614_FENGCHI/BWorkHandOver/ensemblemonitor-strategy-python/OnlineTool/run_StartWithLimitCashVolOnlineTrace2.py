...
if False:  # os.path.exists(f'/data/user/015664/AFuckingTrigger/限制买入和持仓/信号存储/实盘跟踪线下信号/signal_OutSample_{tag}_{pre_date}.pkl'):
    ...
    for per_date in sorted(list(pct_threshold_change.keys())) + [pre_date]:
        if per_date == pre_date:
            signal, pred_ret = get_signal_by_val_pct_threshold_integration_NoMaxThreshold(pct_threshold, [x.replace('.pkl', '_val_pred/') for x in file_list], file_list, per_date,
                                                                           'actual_label', 'new',
                                                                           tail=2)
        else:
