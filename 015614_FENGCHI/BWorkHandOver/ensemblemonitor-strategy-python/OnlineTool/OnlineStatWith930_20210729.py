...
def get_holding_and_buy_time(pre_holding, afternoon_portfolio, trading_record, date, port_id, start_cash,force_sold_930):
    ...
    calced_holding = pre_holding.reindex(stk_list).fillna(0) + buy_record['累计成交数量'].reindex(stk_list).fillna(0) - \
                     sell_record['累计成交数量'].reindex(stk_list).fillna(0) + force_sold_930.reindex(stk_list).fillna(0)
...
def calc_profit_decomposition(date,holding, pre_holding, buy_time_info_, sell_time_info, pre_buy_time_info,afternoon_holding):
    ...
    for each in buy_time_info_summary:
...
def daily_initial_generation(T_plus_1_date,date,barly_max_buy,stk_min_amt,per_signal_ratio,order_ratio,afternoon_holding,portfolio):
    ...
    lm.sendMessage(f'FIX:{date}收盘现金+股票市值 {round(account_cap,2)}，账面相对前日收益{round(account_cap - pre_account_values,2)}，相对前日收益 {round((account_cap / pre_account_values - 1 )* 100,4)}%，持仓股票 {len(holding_series)} 只,其中0股{len(holding_series[holding_series<100])}只，持仓市值 {round(cap.sum(),2)}， 剩余现金 {round(cash,2)}')
...
def get_holding_930(date, final_total_holding, cash_added=0,div_cash=0,holding_added=pd.Series(),after_noon_file =None):
    ...
    if after_noon_file is not None:
        after_noon_file
    ...
    if after_noon_file is not None:
        holding_over = detail.loc[holding_part.index, '隔夜持仓'].astype(int)
        holding_over = holding_over[holding_over>0]
        #[(detail.loc[holding_part.index, '隔夜持仓'] > 0)&~(np.isclose(detail.loc[holding_part.index, '隔夜持仓'] > 0,0)) ].astype(int)
        force_sold = holding_over.loc[(holding_over!=after_noon_file.loc[holding_over.index,'持仓']) & (after_noon_file.loc[holding_over.index,'持仓']==0)]
        price = after_noon_file.loc[force_sold.index,'当日卖金额']/after_noon_file.loc[force_sold.index,'当日卖量']

        detail.loc[force_sold.index,'隔夜持仓']-= force_sold
        detail['手动卖出量'] = force_sold
        detail['手动卖出价格'] = price
        detail['手动卖出收益'] = force_sold
        detail.loc[force_sold.index,'手动卖出收益'] = force_sold*(price-detail.loc[force_sold.index,'昨收价'])
            #after_noon_file.loc[force_sold.index,'当日卖费用']*force_sold/after_noon_file.loc[force_sold.index,'当日卖量']

        holding.loc[force_sold.index, 'NetPosition'] -= force_sold
        holding.loc[force_sold.index, 'SellAvailable'] -= force_sold
        holding.loc[force_sold.index,'TotalSellAmount'] += force_sold*price
    ...
    send_file(['015664','015836'], f'/data/user/015664/AFuckingTrigger/对比930/{date}/逐笔收益930_{date}.xlsx')
    ...
    if (set(holding.keys()) - set(buy_time_info.keys()))!=set(['cash']):
        raise Exception('Extra holding no buy time info')
    buy_time_keys = list(buy_time_info.keys())
    for each in buy_time_keys:
    ...
    return holding_df,left_cash,force_sold
...
def AfterHoursStat(pre_date,today,T_plus_1_day,portfolio,cash_add,actual_all_cash,cash_add_930):
    ...
    final_total_holding = afternoon_portfolio[[ '持仓','当日买金额', '当日卖金额', '当日买费用', '当日卖费用']].rename(columns={
        '持仓':'NetPosition', '当日买金额':'TotalBuyAmount', '当日卖金额':'TotalSellAmount', '当日买费用':'buy_cost', '当日卖费用':'sell_cost'
    })
    final_holding_930,cash_930,force_sold_930 = get_holding_930(today,final_total_holding,cash_added=cash_add_930,
                                                 div_cash=extra_cash_930,holding_added = extra_holding_930[extra_holding_930>0],
                                                 after_noon_file = afternoon_portfolio)
    daily_initial_generation930(T_plus_1_date=get_pre_trade_date(today,-1), date=today, barly_max_buy=100, stk_min_amt=0.2, per_signal_ratio=0.00375, order_ratio=0.1)
    ...
    final_fix_holding = final_fix_holding.rename(columns={
        'NetPosition':'持仓', 'TotalBuyAmount':'当日买金额',  'TotalSellAmount':'当日卖金额',  'buy_cost':'当日买费用', 'sell_cost':'当日卖费用'
    })
    ...
    holding,pre_holding,buy_time_info,sell_time_info,afternoon_holding = get_holding_and_buy_time(morning_fix_portfolio, final_fix_holding,
                                                                                                  ...
                                                                                                  today,portfolio,start_cash+extra_cash_fix.sum(),force_sold_930=force_sold_930)
    ...
    daily_initial_generation(T_plus_1_date=T_plus_1_day,date=today,barly_max_buy=28,stk_min_amt=0.2,per_signal_ratio=0.00167,
...
if __name__=="__main__":
    main(118944397.75-50000000-7500000)
    # main(150351227.59 -50000000-7500000)
