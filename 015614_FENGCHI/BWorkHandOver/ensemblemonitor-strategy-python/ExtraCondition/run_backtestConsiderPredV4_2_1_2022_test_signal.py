...
def calc_back_test_record(pct_threshold, per_amt_ratio, initial_cash, pool_num, deal_ratio, tag, alpha_pool_tag,
    ...
    signal_file = f'/data/user/015664/AFuckingTrigger/限制买入和持仓/信号存储/signal_out_sample_2021_%s_%.2f.pkl' % (tag, pct_threshold)
    ...
    out_path = '/data/user/015664/AFuckingTrigger/限制买入和持仓/PredConsider2021/%sVolConsider_UpBuy100_%dbp_cost_oldwayout.xlsx' % (tag, int(10000 * cost))
    ...
    if os.path.exists(signal_file):
        ...
        for each in file_list:
            if not os.path.exists(each):
                ...
                res = pd.read_pickle(each)
                if res.index[-1][0]<end:
                    out_signal(each.replace('.pkl', '/'))
        signal, pred_ret = get_signal_by_val_pct_threshold_integration_NoMaxThreshold(pct_threshold, [x.replace('.pkl', '_val_pred/') for x in file_list], file_list, get_pre_trade_date(start,45),
                                                                                      'actual_label',
                                                                                      'new',
                                                                                      head=None, end=end)
        # pred_ret.index.set_levels()
        pd.to_pickle([signal, pred_ret], signal_file)
    pred_ret[~signal.fillna(False)] = np.nan
    from dataApi.tradeDate import  get_date_range
    codel_list_path = '/data/group/800319/strategy_local_path3_ForExtra/code_list/'
    date_list = get_date_range(get_pre_trade_date(start),end)
    alpha_pool = {}
    for date in date_list:
        temp_pool = pd.read_pickle(f'{codel_list_path}{date}.pkl')
        alpha_pool[date] = pd.Series(True,index=[int(x[:6]) for x in temp_pool])
    alpha_pool = pd.DataFrame(alpha_pool).T.shift(1).loc[start:end].fillna(False)
...
if __name__ == '__main__':
    ...
    d_threshold,s_ratio,sw_thre = (0.5,0.2,0)#eval(AIMR.getParam())#(0.2,1)
    import datetime
