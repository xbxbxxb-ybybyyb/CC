...
def calc_back_test_record(pct_threshold, per_amt_ratio, initial_cash, pool_num, deal_ratio, tag, alpha_pool_tag,
    ...
    signal_file = f'/data/user/015664/AFuckingTrigger/限制买入和持仓/信号存储/signal_new_out_sample_2021_%s_%.2f.pkl' % (tag, pct_threshold)
    ...
    out_path = '/data/user/015664/AFuckingTrigger/限制买入和持仓/PredConsider2021_out_sample/%sVolConsider_UpBuy100_%dbp_cost.xlsx' % (tag, int(10000 * cost))
    ...
    if os.path.exists(signal_file):
        ...
        signal, pred_ret,_ = get_signal_by_val_pct_threshold_long_integration(pct_threshold, [x.replace('.pkl', '_val_pred/') for x in file_list], [x.replace('.pkl', '/') for x in file_list], get_pre_trade_date(start,45),
                                                                                      'actual_label',
                                                                                      'new',
                                                                                      head=None, end=end)
