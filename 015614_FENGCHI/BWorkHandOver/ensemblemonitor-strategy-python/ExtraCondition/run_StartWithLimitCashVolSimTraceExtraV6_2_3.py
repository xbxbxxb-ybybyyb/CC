...
path_conf = get_path_conf('/data/group/800319/strategy_local_path3_ForExtraV6_2_3/')
...
backtest_start_date = 20210913
initial_cash = 20000000
per_amt_ratio = 0.005
index_pct_change, signal_threshold, swing_threshold = (-1, 0.2, 0)
per_ratio_change = {
}
...
cash_flow = {}
max_trigger_num = {}
tag = 'XGBMonthlyV4WithSWMeanFixFixMIX_Cat_Light_Val'
file_list = para[tag]
print(file_list)
deal_ratio = 0.1
tag = tag + '_OnlineTest'

# today = 2021#int(datetime.date.today().strftime('%Y%m%d'))

pre_date = 20210928#get_pre_trade_date(today)
...
base_dir =  '/data/user/015664/AFuckingTrigger/限制买入和持仓/仿真回测ExtraV6_2_3/'
...
unavailable_pool = set(map(trans_windcode2int,unavailable_pool))
if os.path.exists(signal_file):
    ...
    for each in file_list:
        ...
        if temp.index[-1][0]<pre_date:
            print(f'reintegration {each}')
            out_signal(base_path=each.replace('.pkl','/'), end_date=pre_date)

    signal,pred_ret = get_signal_by_val_pct_threshold_integration_NoMaxThreshold(0.05, [x.replace('.pkl', '_val_pred/') for x in file_list], file_list, get_pre_trade_date(backtest_start_date,25),
                                                               'actual_label','new',head=None, end=pre_date)
    signal[list(unavailable_pool)] = False
    print(signal.shape)
...
print(f'{base_dir}record/barly_condition_indicator_%sOnlineTracing.pkl' % tag)
instance = StartWithLimitCashVolConsider(pred_ret, backtest_start_date, pre_date, stock_pool=alpha_pool, target_point=bar_list, buy_cost=cost, sell_cost=cost,
                                         per_amt_ratio=per_amt_ratio, append_param={'deal_price_path': deal_price_path + 'deal_price_vwap_30min_FullSample.pkl',
                                                                                                'not close pickle':0},
                                         deal_percent=deal_ratio, initial_cash=initial_cash, signal_threshold=signal_threshold, index_pct_change=index_pct_change,
                                         down_swing_threshold=swing_threshold)
...
pd.to_pickle(instance.barly_condition_indicator,f'{base_dir}record/barly_condition_indicator_%sOnlineTracing.pkl' % tag)
