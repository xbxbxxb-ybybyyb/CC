...
for time_point in [1000,1030,1100,1300,1330,1400,1430]:
    threshold = 0.003469407016857877
    offline_pred = pd.DataFrame(offline_pred)
    offline_1000 = offline_pred.loc[time_point]
    offline_1000.index = offline_1000.index.map(trans_int2windcode)
    offline_1000 = offline_1000.loc[pred[time_point].index,pred[time_point].columns]
    compare = pd.DataFrame({'online':pred[time_point].mean(axis=1),'offline':offline_1000.mean(axis=1)})
    signal = compare>threshold
    check = compare[~(signal['online']==signal['offline'])]
    online_1000 = pred[time_point][offline_1000.columns]
    # diff_res[time_point] = pd.DataFrame({'online':online_1000.loc[check.index].stack(),'offline':offline_1000.loc[check.index].stack()}).stack()
    diff_res[time_point] = pd.DataFrame({'online':online_1000.stack(),'offline':offline_1000.stack()}).stack()

bar_res = diff_res[1000].unstack(level=[2,1])
offline,online = bar_res['offline'],bar_res['online']
offline.corrwith(online)


diff_res = pd.DataFrame(diff_res).stack()

online,offline = diff_res.unstack(level=[0,-1]).swaplevel(0,1).loc['online'].T, diff_res.unstack(level=[0,-1]).swaplevel(0,1).loc['offline'].T
...
pred_ret = diff_res.groupby(level=[0,2,3]).mean()

# prob_id = '600563.SH'
# prob_time_point = 1330
# diff_res.loc[prob_id].unstack(level=1).swaplevel().loc[prob_time_point]#.swaplevel().swaplevel()


diff_res = diff_res.unstack(level=1)
diff_res = diff_res.swaplevel(1,2)
out_file = f'./线上线下有差异的信号比对{date}_rebodcast.xlsx'
diff_res.sort_index().to_excel(out_file)
...
model_tag = 'XGB_C_Matrix'
time_point = 1000
factor_list = model_conf[0][model_tag][2]['fix']
online_saved_factor = pd.read_pickle(daily_out_path+f'{date}/factor/{time_point}.pkl')
...
if 'Matrix' in model_tag:
    train_feature, train_label, test_feature, test_label, train_time = get_dataset((get_pre_trade_date(date,5),get_pre_trade_date(date,5)),(get_pre_trade_date(date,4),date),[factor_list,factor_list],
                                                    ...
                                                    '/data/group/800442/800319/HFfactor/CrossIndutryMeanShiftBackUp20220419/data/'])
    ...
    offline_factor = offline_factor.loc[date].loc[time_point]#.loc[online_factor.index, online_factor.columns]
    ...
    offline_factor = offline_factor.loc[time_point].loc[online_factor.index, online_factor.columns]
    # offline_factor = offline_factor*factor_direction[offline_factor.columns]

online_factor.corrwith(offline_factor).sort_values(ascending=False)
'VolaRatio10Mean', 'VolumeDownChange_13h'
# factor_name = 'DIFMaxRaw_1_5_sw1'
factor_name = 'zhy_fix_146_sw1'
compare_factor = pd.DataFrame({
    'online':online_factor[factor_name],
    'offline':offline_factor[factor_name]
})
backuped_factor = list(filter(lambda x : os.path.exists(f'/data/group/800442/800319/HFfactor/CrossIndutryMeanShiftBackUp20220419/data/{x}.npy'),
                                      online_factor.columns.tolist()))
online_factor.corrwith(offline_factor).loc[backuped_factor].sort_values(ascending=False)
...
model.load_model(f'{model_list[model_tag][:-1]}_model_conf/{update_date}.json')
...
recieve_time = {}
for each in online_factor.columns:
