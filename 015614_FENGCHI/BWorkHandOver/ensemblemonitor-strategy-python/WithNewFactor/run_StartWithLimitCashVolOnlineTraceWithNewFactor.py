...
path_conf = get_path_conf(f'/data/group/800319/strategy_local_path3_ForMix20210803_V20210907/')
...
out_dir = '/data/user/015664/AFuckingTrigger/限制买入和持仓/仿真回测线下跟踪/'
if not os.path.exists(out_dir):
    os.makedirs(f'{out_dir}/record/')
    os.makedirs(f'{out_dir}/daily_res_pn/')

last_buy_time = {x: instance.last_buy_time[x][0] * 10000 + instance.last_buy_time[x][1] for x in instance.holding}
pd.to_pickle([record, cash_series, holding_num], f'{out_dir}/record/%sOnlineTracing.pkl' % tag)
...
out_path = f'{out_dir}/%sVolConsiderOnlineLimit_UpBuy100_%dbp_cost_%d.xlsx' % (tag, int(10000 * cost), pre_date)
_, res_pn = helper.one_wave_run(record, cash_series, 48, output_path=out_path, signal_record_save=True, holding_num=holding_num)

pd.to_pickle([res_pn, last_buy_time], f'{out_dir}/daily_res_pn/{pre_date}.pkl')
print(out_path)
