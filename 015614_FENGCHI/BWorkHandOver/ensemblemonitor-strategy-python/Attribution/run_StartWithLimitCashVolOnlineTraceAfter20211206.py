...
final_tag = strategy_tag + '_OnlineTest'+ 'ActivePoolUnionAlpha_deal_ratio_%.1f_per_ratio_%.4f' % (deal_ratio, per_amt_ratio)
...
alpha_pool = alpha_pool1.reindex(union_stk, axis=1).fillna(False) + alpha_pool2.reindex(union_stk, axis=1).fillna(False)
