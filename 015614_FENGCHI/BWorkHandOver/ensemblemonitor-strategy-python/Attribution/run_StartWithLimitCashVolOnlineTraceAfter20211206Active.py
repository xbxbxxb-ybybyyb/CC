...
final_tag = strategy_tag + '_OnlineTest'+ 'ActivePoolOnly_deal_ratio_%.1f_per_ratio_%.4f' % (deal_ratio, per_amt_ratio)
...
for date in get_date_range(get_pre_trade_date(backtest_start_date), pre_date):
    ...
    alpha_pool1.append(temp_pool)
alpha_pool1 = pd.concat(alpha_pool1)
alpha_pool1.columns = [int(x[:-3]) for x in alpha_pool1.columns]

# alpha_pool2 = pd.read_pickle('/data/group/800442/800319/AlphaPool/OLS_XGB200_to20211231.pkl').rank(ascending=False,axis=1).shift(1).loc[backtest_start_date:pre_date]<600

# union_stk = sorted(list(set(alpha_pool1.columns).union(set(alpha_pool2.columns).intersection(set(original_pool.columns)))))
alpha_pool = alpha_pool1#alpha_pool1.reindex(union_stk, axis=1).fillna(False) + alpha_pool2.reindex(union_stk, axis=1).fillna(False)

alpha_pool = alpha_pool>0.5
