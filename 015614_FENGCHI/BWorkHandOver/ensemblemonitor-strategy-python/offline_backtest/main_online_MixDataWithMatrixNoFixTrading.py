...
import traceback
from OnlineTool.OnlineStatWith930_2022 import get_info
...
def backtest_by_day(date):
    ...
    morning_portfolio, afternoon_portfolio, trading_record = get_info(date, '201001')

    holding_info = app.total_holding_info[930].set_index('Symbol')
    holding_info['PortfolioNO'] = '201001'
    afternoon_portfolio = afternoon_portfolio[[ '当日买金额(净价)','当日卖金额(净价)','持仓', 'T日指令可用数量']]
    holding_info.loc[afternoon_portfolio.index,'NetPosition'] = afternoon_portfolio['持仓']
    holding_info.loc[afternoon_portfolio.index,'SellAvailable'] = afternoon_portfolio['T日指令可用数量']
    holding_info.loc[afternoon_portfolio.index,'TotalBuyAmount'] = afternoon_portfolio['当日买金额(净价)']
    holding_info.loc[afternoon_portfolio.index,'TotalSellAmount'] = afternoon_portfolio['当日卖金额(净价)']
...
backtest_list = [(x, get_pre_trade_date(x, -1)) for x in [20220324]]
