...
def backtest_by_day(date):
    ...
    holding_info.loc[last_buy.index, 'TotalBuyAmount'] = holding_info.loc[last_buy.index, 'TotalBuyAmount'] + last_buy * vwap.loc[last_buy.index] * (1 + 0.001)
    holding_info.loc[last_sell.index, 'TotalSellAmount'] = holding_info.loc[last_sell.index, 'TotalSellAmount'] + last_sell * vwap.loc[last_sell.index] * (1 - 0.001)
    # stk_lst = ['002192.SZ','300919.SZ','002850.SZ','002460.SZ','601666.SH','002781.SZ','002812.SZ']
    # sellamt_list = [11319.00,11533.00,14392.00,12535.00,7880.00,8320.00,21589.00]
    # holding_info.loc[stk_lst, 'TotalSellAmount'] = sellamt_list
    # holding_info.loc[stk_lst, 'NetPosition'] = 0
    # holding_info.loc[stk_lst, 'SellAvailable'] = 0
...
backtest_list = [(x, get_pre_trade_date(x, -1)) for x in [20220420]]
cash_flow = {
