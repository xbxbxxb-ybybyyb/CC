...
from StrongStockModel.model.ModelResultLoadingTool import get_signal_by_val_pct_threshold_long_integration,get_signal_by_val_pct_threshold_short_integration
import os
...
def generate_long_signal(pct,param_map,start,end,out_path):
    if not os.path.exists(out_path):
        os.makedirs(out_path)
    res = {}
    for future_window in range(1,9):
        if future_window==8:
        ...
        if os.path.exists(out_file):
            ...
            subset_path_list=[x[:-1] + '_val_pred/' for x in param_map[future_window]],start=start,end=end,threshold_tag=threshold_tag)
...
def generate_short_signal(pct,param_map,start,end,out_path):
    if not os.path.exists(out_path):
        os.makedirs(out_path)
    res = {}
    for future_window in range(1,8):
...
def calc_back_test_record(long_threshold, short_threshold, per_amt_ratio, initial_cash, deal_ratio, tag, alpha_pool_tag, base_dir):
    ...
    for date in date_list:
        ...
        temp_pool = get_nonfix_in_val('code_list',date,'/data/group/800319/strategy_local_path_nonfixBackUp20220322/')
        restrict_list = get_nonfix_in_val('restrict_list',date,'/data/group/800319/strategy_local_path_nonfixBackUp20220322/')
...
if __name__ == '__main__':
    ...
    calc_back_test_record(long_threshold=0.05, short_threshold=0, per_amt_ratio=0.005, initial_cash=50000000,
                          deal_ratio=0.1, tag='XGB_DTC_Matrix_Light_Cat', base_dir='/data/user/015664/AFuckingTrigger/限制买入和持仓/仿真回测NonFix8ModelSince0316/',
                          alpha_pool_tag=None)
