...
import pandas as pd
from ApplicationNonFix import Application
import configparser
from online_conf import realtime_path
from NonFixWindow.dailyLastBarStat import main_stat, main_stat_930
import os
import numpy as np
from dataApi.getData import get_minute_1factor, trans_int2windcode,get_minute_pickle
import traceback
# from online_conf import non_fix_path,non_fix_in_path,non_fix_930_path,non_fix_model_conf_path,non_fix_output_path
...
def main(date_time, app, holding_info=None):
    ...
    if not app.pre_time is None:
        holding_info = pd.read_pickle(f'{non_fix_path}fake_barly_info/{app.date}/{app.pre_time}.pkl')
        ...
        holding_info = holding_info.reset_index().rename(columns={'index': 'Symbol'}).fillna(0)
    if not os.path.exists(f'{non_fix_path}fake_barly_info/{app.date}/'):
        os.makedirs(f'{non_fix_path}fake_barly_info/{app.date}/')

    pd.to_pickle(holding_info, f'{non_fix_path}fake_barly_info/{app.date}/{app.time}.pkl')
    if res:
        # 更新持仓
        if time == 1000:
            print(1)
...
def backtest_by_day(date):
    ...
    # close = get_minute_pickle('close',date_list=[date]).loc[(date,930):(date,959)]#, start_datetime=date * 10000 + 930, end_datetime=date * 10000 + 959)
    ...
    for each in app.left_holding_bars:
        if type(app.left_holding_bars[each]) != tuple:
            print(each, app.left_holding_bars[each])
...
backtest_list = [(x, get_pre_trade_date(x, -1)) for x in [20220421]]#[3:]
# backtest_list = [(x, get_pre_trade_date(x, -1)) for x in [20220303]]#[3:]
# backtest_list = [(x, get_pre_trade_date(x, -1)) for x in get_date_range(20220113,20220127)]#[3:]
cash_flow = {
...
if __name__ == "__main__":
    ...
    for date, T_plust_1 in backtest_list:
        ...
        if date in cash_flow:
            cash_added_fix,cash_added_930 = cash_flow[date]
        else:
            cash_added_fix,cash_added_930 = 0,0
        main_stat(date=date, T_plus_1=T_plust_1,cash_added=cash_added_fix)
        main_stat_930(date=date, T_plus_1=T_plust_1,cash_added=cash_added_930)
