...
def calc_back_test_record(start,end,long_threshold, short_threshold, per_amt_ratio, initial_cash, deal_ratio, tag, alpha_pool_tag, base_dir):
    ...
    for date in date_list:
        ...
        temp_pool = pd.read_pickle(f'/data/user/015664/StrategyBackUp/strategy_local_path3DailyBackup/code_list/{get_pre_trade_date(date)}.pkl')#get_nonfix_in_val('code_list',date,strats_path)
        restrict_list = set([])
    ...
    instance = StartWithLimitCashVolConsider(long_signal, short_signal, start, end, stock_pool=alpha_pool, target_point=bar_list, buy_cost=cost, sell_cost=cost,
                                             ...
                                             stk_min_amt=stk_min_amt, condition_series=condition_series, down_swing_threshold=down_definition,max_trigger_num={})
...
if __name__ == '__main__':
    ...
    import time
    # time.sleep(60*30)
    b_dir = '/data/user/015664/AFuckingTrigger/限制买入和持仓/实盘跟踪NonFix8ModelConditionFrom20220422NoTimingV2/'
    strats_path = '/data/group/800319/strategy_local_path3/'
    backtest_start_date = 20220104
    pre_date = get_pre_trade_date(int(datetime.date.today().strftime('%Y%m%d')))
    calc_back_test_record(start=backtest_start_date,end=pre_date,long_threshold=0.05, short_threshold=0, per_amt_ratio=0.005, initial_cash=50000000,
