...
def get_singal(pct,tag,start,end,out_path):
    ...
    signal = signal.reindex(index).notnull()
    return signal
...
def prepare_condition_V4(date, signal, signal_threshold, down_signal_ratio, down_condition):
    ...
    barly_r_20d_ratio, _ = calc_recent_count(signal, 20)
...
condition_change_history={
    ...
    20220421:(prepare_condition_V4,(0.15,0.5,0)),
    20220422:(prepare_condition_V6_2_1_selfdefine_index,(0.15,-0.008,0,'SZCZ','SZCZ')),
    20220526:(prepare_condition_V4,(0.15,0.5,0)),
}

def generation(today):
    para_list = [
    ...
    # para_list.append()
    signal = get_singal(0.05, 'XGB_DTC_Matrix_Light_Cat', get_pre_trade_date(today, 30), today,
    f'/data/user/015664/AFuckingTrigger/限制买入和持仓/信号存储/非固定时点近30日WithCatLight/')
    for func, para, extra_tag in para_list:
        condition = func(get_pre_trade_date(today, -1), signal, *para)
        save_nonfix_in_val(condition, f'condition{extra_tag}', today, non_fix_path)
        # pd.to_pickle(condition, f'{out_path}{today}.pkl')
    send_message(['015664', '015614'], f'Condition {today} done')
    send_message(['015664', '015614'], f'{condition[0][1000]}')
...
if __name__ == '__main__':
    ...
    #####TODO for t_day in [get_recent_trade_date()]:#get_date_range(20220302,20220316):#
    for t_day in [get_recent_trade_date(dividing_point=23)]:#get_date_range(20220302,20220316):#
        ...
        print(get_pre_trade_date(t_day, -1))
        generation(t_day)
