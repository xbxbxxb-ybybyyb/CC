...
def get_holding_num(date_list,tag,base_dir,strategy_base_path):
    holding_vol = {}
    record = pd.read_pickle(f'{base_dir}/record/record_{tag}_{date_list[-1]}_VolConsider_UpBuy100_10bp_cost.pkl')
    offline_holding_info = record[-1]
    ...
    for date in tqdm(date_list,desc='统计持仓重合情况'):
        for bar in [1000,1030,1100,1300,1330,1400,1430,1500]:
            if bar == 1500:
                ...
                bar_holding = bar_holding[bar_holding>0]
            else:
...
def get_intersec(date,file_name,strategy_base_path):
    ...
    for i in range(1,9):
        ...
        offline_pred_ret[i] = temp_long[1].loc[date].reindex(code_list,axis=1).fillna(False).stack()
        if i>=8:
            continue
    ...
    offline_extra = offline_long & ~online_long
    online_extra = ~offline_long & online_long



    difference = pd.DataFrame({
    'online':online_pred_ret[offline_extra | online_extra].stack(),
    'offline':offline_pred_ret[offline_extra | online_extra].stack(),
    })
    ...
    return signal_info.reindex([ '线上看多信号数量','线下看多信号数量','线上线下看多信号交集','线上看多重合率','线下看多重合率',
        '线上看空信号数量','线下看空信号数量','线上线下看空信号交集','线上看空重合率','线下看空重合率'],axis=1),difference
...
def get_signal_stat_recent(date_list,tag,signal_date,base_dir,strategy_base):
    ...
    for date in tqdm(date_list,desc='统计信号交集...'):
        signal_stat[date],diff[date] = get_intersec(date, f'{base_dir}/信号/{signal_date}/',strategy_base)
    signal_stat_df = pd.DataFrame({x:signal_stat[x].stack() for x in signal_stat}).T
    return signal_stat_df,diff
...
def main_compare(today=None,tag= 'XGB_Cat_Light',start_backtest_date = 20210406,
    ...
    signal_stat,diff = get_signal_stat_recent(date_list,tag,signal_date= pre_date,base_dir=base_dir,strategy_base=strategy_base_path)
    ...
    with pd.ExcelWriter(out_diff_name) as writer:
        key_list = sorted(list(diff.keys()))
        for date in key_list:
            diff[date].to_excel(writer,sheet_name=f'{date}')
    writer.close()
    if send_diff:
