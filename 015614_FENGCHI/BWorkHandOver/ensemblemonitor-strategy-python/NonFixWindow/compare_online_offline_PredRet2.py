...
prob_id = '600133.SH'
comp_prob_stk = pd.DataFrame({
    'online':online_1000.loc[prob_id],
    'offline':offline_1000.loc[prob_id]
})
comp_prob_stk.mean()


model_tag = 'XGB_D_Matrix'
...
# fix_factor_list = pd.read_pickle('/data/group/800442/800319/Faamonitor/PL/all_mkt_ts_norm/MonthlySelectedMDDImproveV8BarWith1DayLabel_20220406Integration/Future_8_bar/XGBV4ReversalResReselect_ic_t_train200_test10_factor_num400/XGBV4ReversalResReselect_ic_t_train200_test10_factor_num400_factor_list/20220330.pkl')
# fix_factor_list = np.load(f'{(model_list[model_tag]%future_window)[:-1]}_train_features/{update_date}.npy').tolist()
# factor_list2 = np.load('/data/group/800442/800319/wyl/model_record/a01_factorlist/factors_400_20220302_ic_all_t.npy').tolist()

# fix_factor_list_2 = np.load(f'{non_fix_path}model/')
...
compare = pd.DataFrame({
    'online':online_facotr_1000.loc[prob_id],
    'offline':offline_factor_1000.loc[prob_id]
})
# prob_factor = abs(compare['online'] - compare['offline']).sort_values().index[-30:]
# compare.loc[prob_factor]
...
},index=offline_factor_1000.index)
res_compare.corr()
res_compare.loc[prob_id]
