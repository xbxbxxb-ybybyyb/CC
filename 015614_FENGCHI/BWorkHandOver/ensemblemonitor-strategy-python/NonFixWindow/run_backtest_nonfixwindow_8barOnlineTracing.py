...
def calc_back_test_record(long_threshold, short_threshold, per_amt_ratio, initial_cash, deal_ratio, tag, alpha_pool_tag, base_dir):
    ...
    for i in range(1, 9):
        ...
        long_signal_saved[i] = temp_long_pred_ret[temp_long_sigal.fillna(False)]
        print(f'window {i} {temp_long_sigal.index[0], temp_long_sigal.index[-1]}')
        if i < 8:
            temp_signal, temp_pred_ret = pd.read_pickle(short_param[i])[:2]
            short_signal_saved[i] = temp_pred_ret[temp_signal.fillna(False)]
...
if __name__ == '__main__':
    ...
    calc_back_test_record(long_threshold=0.05, short_threshold=0, per_amt_ratio=0.005, initial_cash=50000000,
                          deal_ratio=0.1, tag='XGB_DTC_Matrix', base_dir='/data/user/015664/AFuckingTrigger/限制买入和持仓/仿真回测NonFix剔除问题因子/', alpha_pool_tag=None)
