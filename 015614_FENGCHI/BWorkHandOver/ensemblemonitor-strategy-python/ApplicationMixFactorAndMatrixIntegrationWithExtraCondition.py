...
class Application:
    ...
    def __init__(self, date, log=print):
        ...
        # 加载成交量
        # TODO delete
        # if os.path.exists(vol_info_path + '%d_backup.pkl' % pre_date):
        #     vol_info = pd.read_pickle(vol_info_path + '%d_backup.pkl' % pre_date)
        #     self.log('using backup vol')
        # else:
        vol_info = pd.read_pickle(vol_info_path + '%d.pkl' % pre_date)
        # 加载复权因子
        adj_factor = pd.read_pickle(f'{realtime_path}market_data/{pre_date}/adjfactor.pkl')
        ...
        self.ratio = ratio
        self.matrix = pd.read_pickle(f'{matrix_conf}{pre_date}.pkl')
        ...
        self.index_map = pd.read_pickle(f'{local_config_path}/index_map.pkl')
        # pd.to_pickle({
        #     '000001.SH': 'SZZS',
        #     '000905.SH': 'ZZ500',
        #     '399005.SZ': 'ZX100',
        ...
        #     '000852.SH': 'ZZ1000',
        #     '399001.SZ': 'SZCZ'
        # }, f'{local_config_path}/index_map.pkl')
        self.condition, self.down_definition = pd.read_pickle(f'{condition_path}/{pre_date}.pkl')

        # '000001.SH,000905.SH,399005.SZ,000016.SH,000906.SH,399006.SZ,000300.SH,000985.CSI,399300.SZ,000832.CSI,000985.SH,399905.SZ,000852.SH,399001.SZ'
    ...
    def predict_by_multi_model(self, factor, factor_5min, matrix_factor, time_point=None):
        ...
        for each in self.model_list:
            ...
            for k in self.factor_map[each].keys():
                if k.startswith('matrix_'):
                    matrix_flag = k[7:]
                    temp_matrix = matrix_factor[matrix_flag]
                    temp_matrix = temp_matrix[self.factor_map[each][k]]
            ...
            if tag not in filtered_factor:
                ...
                if factor_list_5min or factor_df_list_matrix:
        ...
        # if self.backtest:
        if not os.path.exists(f'{daily_out_path}{self.date}/factor/'):
            os.mkdir(f'{daily_out_path}{self.date}/factor/')
        model_factor = {x: filtered_factor[model_tag_map[x]] for x in model_tag_map}
        pd.to_pickle(model_factor, f'{daily_out_path}{self.date}/factor/{self.time}.pkl')
    ...
    def load_factor(self, time_point, factor_path, backtest=False):
        ...
        # TODO: DELETE
        pd.to_pickle(factor, f'{daily_out_path}{self.date}/all_factor_{self.time}.pkl')
        fix_factor = self.factor_direction * factor.reindex(self.pool_with_over_night_stk, axis=0)
        matrix_factor = {}
        for each in self.matrix:
            ...
            matrix_factor[each] = pd.DataFrame(temp_res / count, index=self.matrix[each].index, columns=factor.columns).reindex(fix_factor.index, axis=0).reindex(factor.columns,
                                                                                                                                                                  axis=1)
    ...
    def bar_handler(self, signal=None):
        ...
        #########################################
        if self.date == 20210915 and self.time == 1000:
            print(1)
        try:
            self.update_basic_indicator(signal, close)
