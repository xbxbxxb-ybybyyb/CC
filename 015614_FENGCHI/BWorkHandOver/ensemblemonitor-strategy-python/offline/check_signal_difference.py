...
import pandas as pd
import numpy as np
from online_conf import code_list_path,holding_info_path,daily_out_path
...
for date,pre_date in list(zip(date_list,pre_date_list)):
    signal_stat[date],XOR_dict[date] = get_intersec(date,pre_date)
    print(date)
...
signal_stat.to_excel('/data/user/015664/AFuckingTrigger/online_stat_v20210209Fake_online/信号重合统计.xlsx')
pd.to_pickle(XOR_dict,'/data/user/015664/AFuckingTrigger/online_stat_v20210209Fake_online/XOR.pkl')