...
def get_daily_corr(date_list):
    max_date = max(list(filter(lambda x: os.path.exists(f'/data/user/015664/AFuckingTrigger/限制买入和持仓/信号存储/实盘跟踪线下信号/signal_OutSample_XGB_Cat_Light_SWMatrix_OnlineTest_{x}.pkl'),date_list)))
    signal,pred_ret = pd.read_pickle(f'/data/user/015664/AFuckingTrigger/限制买入和持仓/信号存储/实盘跟踪线下信号/signal_OutSample_XGB_Cat_Light_SWMatrix_OnlineTest_{max_date}.pkl')
...
def main():
    ...
    end = 20211214#get_pre_trade_date(int(datetime.date.today().strftime('%Y%m%d')))
    ...
    out_file = f'/data/user/015664/AFuckingTrigger/信号分析/模型信号分析_{end}_summaryV2021.xlsx'
