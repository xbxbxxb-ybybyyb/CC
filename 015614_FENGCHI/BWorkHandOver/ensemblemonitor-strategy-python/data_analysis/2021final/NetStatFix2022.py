...
today=20220406
pre_date = 20220331
start_backtest_date = 20220113
date_list = get_date_range(start_backtest_date, pre_date)
tag = 'XGB_Cat_Light_SWMatrix'
...
account_info['当日收盘后中增加资金'] = pd.Series(cash_flow).reindex(sorted(list(set(cash_flow.keys()).union(account_info.index)))).fillna(0)
account_info.loc[sorted([get_pre_trade_date(x,-1) for x in list(set(cash_flow.keys()))][1:] + sorted(list(set(cash_flow.keys())))[1:]),'追踪误差'] = np.nan
account_info['仓位变化前市值'] = account_info['线上账户总市值'] -account_info['当日收盘后中增加资金']
...
for idx,date in enumerate(date_list):
    if os.path.exists(signal_stat_path % (date, date)):
        daily_signal = pd.read_excel(signal_stat_path % (date, date), sheet_name=None)
    else:
        daily_signal = {
            '委托成交明细': pd.DataFrame(),
            '收益明细': pd.DataFrame(columns=['Unnamed: 0', 'buy_signal_time', 'sell_signal_time', '今收价', '成交价格',
                                          '成交金额', '昨收价', '类型', '累计成交数量', '证券代码', '贡献收益', '费用', '费后收益'])
        }
...
account_info['仓位'] = account_info['线上持仓市值']/account_info['仓位变化前市值']
