...
pct_threshold_change = {
    get_pre_trade_date(start_backtest_date):0.005,20210525: 0.04,20210527:0.05,
}
...
account_info['按占资计算净值FIX'] = 1+pd.Series(1,index=account_info.index).cumsum()*account_info['累计收益FIX']/account_info['线上持仓市值'].cumsum()#(account_info['不考虑规模按占资计算收益率']+1).cumprod()
...
out_file = './历史净值跟踪_日频_月频2021FINAL.xlsx'
