...
def get_signal_stat(date_list):
    ...
    return daily_signal_stat, daily_signal_win_rate, opputunity
...
def main():
    ...
    daily_signal_stat, daily_signal_win_rate, opputunity = get_signal_stat(date_list)
    corr,recent_5_day_corr = get_daily_corr(date_list)
    corr = pd.DataFrame({'IC':corr,'近五日IC':recent_5_day_corr})
    out_file = f'/data/user/015664/AFuckingTrigger/信号分析/模型信号分析_{end}.xlsx'
    with pd.ExcelWriter(out_file) as writer:
        pd.concat([daily_signal_stat, daily_signal_win_rate, opputunity,corr], axis=1).to_excel(writer, '信号收益命中情况')

    writer.close()

    send_file(['015664','003186'], out_file)
