...
class MRFixFactorTest(object):
    ...
    def set_future(self, future_days=1, delay_min=1, order_keep_min=30):
        path = '/data/group/800442/800319/junkData/StrongStock/external_data/MRFixFactorTest/'
        conf_file = f'{path}{self.calc_start_date}_{self.test_start_date}_{self.end_date}future_{future_days}_delay{delay_min}_kept_min{order_keep_min}.pkl'
        if os.path.exists(conf_file):
            para = pd.read_pickle(conf_file)
            self.future = para['future']
            self.future2 = para['future2']
            self.future_finite = para['future_finite']
            self.nolimit = para['nolimit']
            self.future_days = para['future_days']
            self.delay_min = para['delay_min']
            self.order_keep_min = para['order_keep_min']
            return
        ...
        pd.to_pickle(dict(future = self.future,
        future2 = self.future2,
        future_finite = self.future_finite,
...
if __name__ == '__main__':
    ...
    def _load_pickle_frame(file_name, date_list, code_list):
        ...
        factor_address = '/data/group/800442/800319/HFfactor/RealTimeFixRollCrosslize/data_df/'

        factor = pd.read_pickle(f'{factor_address}{file_name}.pkl')
        factor = factor.loc[date_list].reindex(code_list,axis=1)
        factor = factor.values.reshape((factor.shape[0]//7,7,factor.shape[-1]))

        return factor


    self = MRFixFactorTest(20140801, 20140801)
    self.set_stock_pool()
    e = time.time()
    self.set_future()
    total = time.time() - e
    ...
    factor_address = '/data/group/800442/800319/HFfactor/RealTimeFixRollCrosslize/data_df/'
    result_address = '/data/group/800442/800319/HFfactor/RealTimeFixRollCrosslize/ic_month_test/'
    fix_factor_list = list(map(lambda x : x.replace('.pkl',''),os.listdir(factor_address)))
    ...
    def _func(sub_list, line=0):
        for name in sub_list:
            if os.path.exists(f'{result_address}/{name}'):
                continue
            factor = _load_pickle_frame(name, self.test_date_list, self.code_list)
            result = self.test_factor(factor, move_window=12,standardize_days=0)
            pd.to_pickle(result, f'{result_address}/{name}')
            print(time.strftime('%Y%m%d %H:%M:%S'), name)

    _func(fix_factor_list)

    # factor = _load_pickle_frame(fix_factor_list[0],self.test_date_list,self.code_list)
    # e = time.time()
    # result = self.test_factor(factor, move_window=12,standardize_days=0)
    # total = time.time() - e
    # print(total)
