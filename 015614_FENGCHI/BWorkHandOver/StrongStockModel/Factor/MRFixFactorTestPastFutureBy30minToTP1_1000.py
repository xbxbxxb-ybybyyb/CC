...
class MRFixFactorTest(object):
    ...
    def test_factor(self, factor, bar_n, standardize_days=40, move_window=12,
        ...
        if standardize_days:
            ...
            factor[~ factor_finite] = 0
            del d_cf, d_cf2, rd_cf, rd_cf2, rd_cn
            for i in range(1,factor.shape[1]+1):
                if i==bar_n:
                    pass
                else:
                    factor[:,-i,:] = np.nan
...
if __name__ == '__main__':
    ...
    bar_num = 1
    today = 20211220#get_pre_trade_date(20140801,-50)#get_recent_trade_date()
    self = MRFixFactorTest(20140601, 20140801, today)
    self.set_stock_pool()
    self.set_future_by_30min(future_bar=bar_num,order_keep_min=10)
    factor_address = '/data/group/800442/800319/HFfactor/RealTimeFixRollRobust/data/'
    result_address = f'/data/group/800442/800319/HFfactor/RealTimeFixRollRobust/ic_future_{bar_num}_bar_only_to_1000/'
    ...
    def _func(sub_list, line=0):
        for name in sub_list:
            ...
            result = self.test_factor(factor,bar_n=bar_num, move_window=12)
