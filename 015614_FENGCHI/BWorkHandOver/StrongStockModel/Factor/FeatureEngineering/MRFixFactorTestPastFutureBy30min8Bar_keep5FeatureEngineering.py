...
import bottleneck
import gc
from StrongStockModel.Factor.FeatureEngineering.FactorCategory import perfect_distr,OK_distr,to_diff
...
class MRFixFactorTest(object):
    ...
    def set_future_by_30min(self,future_bar,delay_min=1, order_keep_min=30):
        ...
        future,future_date_list = _get_future_by_30min(self.test_date_list,self.code_list,future_bar=future_bar,delay_min=delay_min,order_keep_min=order_keep_min)
...
if __name__ == '__main__':
    ...
    today = 20220510#20211231#get_pre_trade_date(20140801,-50)#get_recent_trade_date()
    self = MRFixFactorTest(20140601, 20140801, today)
    self.set_stock_pool()
    self.set_future_by_30min(future_bar=bar_num,order_keep_min=5)
    factor_address = '/data/group/800442/800319/HFfactorFE/FixFactorTSFeatureEngineering/data/'
    result_address = f'/data/group/800442/800319/HFfactorFE/FixFactorTSFeatureEngineering/ic_future_{bar_num}_bar_8bar/'
    ...
    # fix_factor_list = get_fix_factor_list(True, factor_address)
    from operators import dt_delay
    import itertools
    from dataApi.LoadingTool import trans_df2arr
    from dataApi.sendInfo import send_message
    index = pd.MultiIndex.from_tuples(list(itertools.product(self.test_date_list,[1000, 1030, 1100, 1300, 1330, 1400, 1430])))
    pad_index = pd.MultiIndex.from_tuples(list(itertools.product(self.calc_date_list,[1000, 1030, 1100, 1300, 1330, 1400, 1430])))
    # index.shape
    def _func(sub_list, line=0):
        for name in sub_list:
            factor = _load_pickle_frame(name, self.calc_date_list, self.code_list)
            if os.path.exists(f'{factor_address}/{name}.npy') and os.path.exists(f'{result_address}/{name}'):
                continue
            if name in perfect_distr:
                pass
            elif name in OK_distr:
                factor_smth = OK_distr_process(factor,40)
                factor = np.pad(factor_smth,((factor.shape[0]-factor_smth.shape[0],0),(0,0),(0,0)),constant_values=0,mode='constant')
            elif name in to_diff:
                factor = factor - dt_delay(factor,1)
            else:
                continue
            result,factor_norm = self.test_factor(factor, move_window=12)
            try:
                factor_df = pd.DataFrame(factor_norm.reshape(factor_norm.shape[0]*factor_norm.shape[1],factor_norm.shape[2]),
                                     index=index,columns=self.code_list).reindex(pad_index).fillna(0)
            except:
                send_message(['015664'],f'{name} index wrong')
                print(f'{name} index wrong')
                continue

            factor_arr = trans_df2arr(factor_df,start_date=20140801,end_date=self.test_date_list[-1],roll=True)
            factor_arr = np.ascontiguousarray(factor_arr.astype('float32'))
            np.save(f'{factor_address}/{name}.npy',factor_arr)
    ...
    multidask(10, _func, perfect_distr+OK_distr+to_diff)
