...
class XGBRegressionFactorEvalYearly(ModelNonFixWindow):
    ...
    def get_fix_factor_evaluation(self, num, end_index):
        ...
        factor_evaluation = pd.read_pickle(f'{root_path}external_data/FutureBarBy30MinFeatureEngineering_8bar/Future_{self.future_bar_num}_bar/{self.eval_indicator}.pkl')
        print(f'{root_path}external_data/FutureBarBy30MinFeatureEngineering_8bar/Future_{self.future_bar_num}_bar/{self.eval_indicator}.pkl')
...
def main_window_search(i, indicator,future_bar_num,label_path):
    ...
    out_file =f'/data/group/800442/800319/Faamonitor/PL/FeatureEngineering/DiffExtraClip/Future_{future_bar_num}_bar/{target_file}/{target_file}.pkl'
    ...
    model = XGBRegressionFactorEvalYearly(train_start, test_end, None, feature_address='/data/group/800442/800319/HFfactorFE/FixFactorTSFeatureEngineering/data/',
...
print(morning)
print(para_list[idx])
idx_list = list(range(147))[24:]

for i in tqdm(idx_list):
    for f_bar in tqdm(list(range(1,8))):
