...
class XGBRegression(ModelBase):
    ...
    #     return factor_list
    def get_fix_factor_evaluation(self,num):
        if self.indicator=='sdt_adjusted':
            return self.get_std_adj_selected(num)
        factor_info = pd.read_excel('/data/user/015664/AFuckingTrigger/select1127.xlsx')
        return factor_info['name'].apply(lambda x : str(x).zfill(4)).tolist()

    def get_std_adj_selected(self,num):
        ...
        selected = check.sort_values('score', ascending=False)[:3*num]
        ...
        return selected.index.tolist()[:num]
    ...
    def train_model(self, X_train, y_train, params, end_date=None):
        ...
        if 'load local model' in params and os.path.exists(params['model_conf_path'] + '%d.json' % train_end):
            ...
            if train_features.shape[0]>25000000:
                train_features, train_label = train_features[-25000000:],train_label[-25000000:]
