...
def fit_model(i, output_path):
    path_dict = dict(
        ...
        base_model_res_path_test = '/data/group/800319/Faamonitor/PL/all_mkt_ts_norm/FeatureEngineeringExplore/XGB_DTCGWC_HX_RegIntegrationOnlyTest_base_model_res_test_set/'
    )
    ...
    X_test,y_test = base_model_res.loc[test_date_list],y.loc[test_date_list]
    pd.to_pickle([X_test,y_test],path_dict['base_model_res_path_test'] + '%d.pkl' % train_end)

    exist_test_set = sorted(list(filter(lambda x : x< '%d.pkl' % train_end,os.listdir(path_dict['base_model_res_path_test']))))
    stack_linear = DecisionTreeRegressor()
    params = {'criterion': 'mse', 'max_depth': None, 'max_features': None, 'max_leaf_nodes': None,
              'min_impurity_decrease': 0.0, 'min_impurity_split': None, 'min_samples_leaf': 5000, 'min_samples_split': 5000,
              'min_weight_fraction_leaf': 0.001, 'presort': False, 'random_state': None, 'splitter': 'best'}
    stack_linear.set_params(**params)


    if len(exist_test_set)>=5:
        print('exist train')
        X_train,y_train = [],[]
        for each in exist_test_set[-10:]:
            temp_X,temp_y = pd.read_pickle(path_dict['base_model_res_path_test']+each)
            X_train.append(temp_X)
            y_train.append(temp_y)
        X_train, y_train = pd.concat(X_train), pd.concat(y_train)
        train_date_list = sorted(list(set([x[0] for x in X_train.index])))
        train_date_list = list(set(train_date_list) - set(val_date_list))
        X_train,y_train = X_train.loc[train_date_list],y_train.loc[train_date_list]
        stack_linear.fit(X_train,y_train)
    else:
...
out_path = '/data/group/800319/Faamonitor/PL/all_mkt_ts_norm/FeatureEngineeringExplore/XGB_DTCGWC_HX_TreeIntegrationOnlyTest/'
...
for idx in idx_list:#tqdm(idx_list):
