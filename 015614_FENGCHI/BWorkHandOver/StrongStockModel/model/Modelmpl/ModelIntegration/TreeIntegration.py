...
out_path = '/data/group/800319/Faamonitor/PL/all_mkt_ts_norm/FeatureEngineeringExplore/XGB_DTCGWC_HX_ByTree/'
...
conf = configparser.ConfigParser()
conf.read('/data/group/800319/strategy_local_path_offline/period_info.ini')
para_list = eval(conf['period_info']['period_info'])[:73]
model_path = '/data/group/800319/Faamonitor/PL/all_mkt_ts_norm/FeatureEngineeringExplore/GBDTOnly_train400_test10_factornum400_eval_ic_all_t/'
para_list = list(filter(lambda x : not os.path.exists(model_path+'%d.pkl'%x[1][1]),para_list))
# idx_list = [x[0] for x in para_list]
# idx_list = list(filter(lambda x : x<73,idx_list))

idx_list = [x[0] for x in para_list]
idx_list = [idx_list[len(idx_list)*i//4:(i+1)*len(idx_list)//4] for i in range(4)]
#idx_list = idx_list[len(idx_list)*i//7:len(idx_list)*(i+1)//7]
...
AIMR.runTasks('./model/DTCOnlineTest/RFFixOnly.py',json.dumps(params))
print("end")
"""