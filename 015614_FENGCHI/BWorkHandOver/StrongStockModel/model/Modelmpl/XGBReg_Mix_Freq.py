# @Time : 2020/11/19 9:25
# @Author : Zhichen Lu
# @File : XGBReg_Mix_Freq.py
...
minute_dict = get_desample_minute_dict(30)
minute_dict = pd.Series(minute_dict).shift(31)
time_5min_list =  sorted(list(set(pd.Series(get_desample_minute_dict(5)))))
class XGBRegressionMixFreq(ModelBase):
    def __init__(self, start=20170103, end=20191231, stock_pool=None, feature_address=None):
        ...
        self.dp_5min = DataPrepare(idx_address='/data/group/800319/JunkSmallFactor/')
        self.dp_fix = DataPrepare(idx_address='/data/group/800319/LittleJunkFix/')

    def get_dataset(self, train_idx, test_idx, factor_list, interday_factor, label_method, label_param={}, kernel=10):
        gc.collect()
        e = time.time()
        self.dp_fix.set_date_range(train_idx[0], test_idx[1])
        self.dp_5min.set_date_range(train_idx[0], test_idx[1])
        factor_list_5min, factor_list_fix = factor_list
        factor_5min = self.dp_5min.load_data(factor_list_5min + ['future'])
        factor_5min = factor_5min.swaplevel(0,1).loc[time_5min_list[5:-1]].swaplevel(0,1).sort_values(['date','code','time'])
        fix_factor = self.dp_fix.load_data(factor_list_fix)
        if fix_factor.shape[0]//7!=factor_5min.shape[0]//42:
            raise Exception('Data shapes are not match')
        # check = factor_5min.swaplevel(0,1).loc[[1000,1030,1100,1300,1330,1400,1430]].swaplevel(0,1)
        load_time = time.time() - e
        fix_factor_arr = fix_factor.values.reshape(fix_factor.shape[0]//7,7,fix_factor.shape[1])
        factor_5min_arr = factor_5min.values.reshape(factor_5min.shape[0]//42,42,factor_5min.shape[1])

        fix_factor_arr_expend = np.concatenate([np.concatenate([fix_factor_arr[:,[i],:] for j in range(6)],axis=1) for i in range(fix_factor_arr.shape[1])],axis=1)
        factor = pd.DataFrame(np.concatenate([factor_5min_arr,fix_factor_arr_expend],axis=-1).
                                  reshape((factor_5min.shape[0],factor_5min.shape[-1]+fix_factor.shape[-1])),
                                  index=factor_5min.index,columns=factor_5min.columns.tolist()+fix_factor.columns.tolist())
        train_feature = factor.loc[train_idx[0]:train_idx[-1]]
        train_label = pd.DataFrame(train_feature.pop('future'))
        test_feature = factor.loc[test_idx[0]:test_idx[-1]]
    ...
    def get_fix_factor_evaluation(self, num):
        ...
        factor_list = res.sort_values('ic_all_dtc', ascending=False).index.tolist()[:num//2]
        factor_list_5min = [str(x).zfill(4) for x in factor_list]
        print('JT Factor!')

        factor_list_fix = pd.read_pickle(fix_factor_true_send_ic_sort_path).index.tolist()[:num-len(factor_list_5min)]
        print('factor_num:', len(factor_list_5min)+len(factor_list_fix))
        return (factor_list_5min,factor_list_fix)
    ...
    def train_model(self, X_train, y_train, params, end_date=None):
        ...
        if 'load local model' in params and os.path.exists(params['model_conf_path'] + '%d.json' % train_end):
            model = xgb.Booster(args_param)
            model.load_model(params['model_conf_path'] + '%d.json' % train_end)
            print('load from local', train_end)
        else:
            train_features, train_label = X_train.loc[date_list], y_train.loc[date_list]
            if train_features.shape[0]>20000000:
                train_features, train_label = train_features[-20000000:],train_label[-20000000:]
        ...
        if 'val_pred_path' in params:
            ...
            print(val_features.shape,'val_shape')
