# @Time : 2020/12/28 10:37
# @Author : Zhichen Lu
# @File : XGBRegressionFactorEvalRolling.py
import sys
...
class XGBRegressionFactorEvalRolling(ModelBase):
    ...
    def train_model(self, X_train, y_train, params, end_date=None,best_model=None):
        ...
        args_param = {params[x] for x in ['booster', 'colsample_bytree', 'gamma', 'max_depth', 'min_child_weight', 'n_estimators', 'sampling_method', 'subsample', 'tree_method']}
    ...
    def rolling_train_and_predict(self, params={}, period=10, predict_period=10, label_methodology='fix_window', label_param={}, factor_nums=200, kernel=10):
        ...
        for idx, cell_idx in bar:
            ...
            if len(X_train) > 2000 and len(set(y_train[y_train.columns[0]])) > 1:
                ...
                model = self.train_model(X_train, y_train, params, test_start_idx,best_model)
            if model is None:
                continue
...
def main_window_search():
    ...
    factor_num = 400
    N = 40
    indicator_list = [ 'ic_all_tc', 'ic_all_dc', 'ic_all_d', 'ic_all_c']
    indicator = 'ic_all_t'
    ...
    model = XGBRegressionFactorEvalRolling(20150309, 20181231, None, feature_address=all_mkt_preprocessed_ts_norm_by_date_path,factor_eval_indicator=indicator)
    ...
    # model = XGBRegressionFactorEvalRolling(20160711,20181231, None, feature_address=all_mkt_preprocessed_ts_norm_by_date_path, factor_eval_indicator=indicator)
    # out_file = '/data/group/800319/Faamonitor/PL/all_mkt_ts_norm/FactorEval/XGBFactorEval_%s_train%d_test%d_factor_num%d_norm_window_%d.pkl' % (
    #     indicator, train_period, test_period, factor_num, N)

    print(out_file)
    best_param_clf_xgb = {'booster': 'gbtree', 'colsample_bytree': 0.500208272281293, 'gamma': 0.17299255046340967, 'max_depth': 7, 'min_child_weight': 8255, 'n_estimators': 100, 'sampling_method': 'gradient_based', 'subsample': 0.8681101227137867, 'tree_method': 'gpu_hist'}
