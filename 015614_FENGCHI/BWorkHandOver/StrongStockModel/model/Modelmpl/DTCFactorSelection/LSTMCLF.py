...
class LSTMCLF(ModelBase):
    ...
    def Network(self, input_shape, param=None):
        ...
        out = Dense(2, activation='softmax')(full_conn)
    ...
    def train_model(self, X, y_train, params, end_date=None):
        ...
        if 'load local model' in params and os.path.exists(params['model_conf_path'] + '%d.h5' % 20170508):
            model.load_weights(params['model_conf_path'] + '%d.h5' % 20170508)
            print('load model from local 20170508')
        else:
            model.fit(train_features,to_categorical((train_label>0)*1) , epochs=params['nb_epoch']//2, \
                      batch_size=params['batch_size'], verbose=1, \
                      shuffle=True, callbacks=[train_log], validation_split=0.05)
        ...
        if 'val_pred_path' in params:
            ...
            val_labels['prediction'] =model.predict(val_features).argmax(axis=1)
