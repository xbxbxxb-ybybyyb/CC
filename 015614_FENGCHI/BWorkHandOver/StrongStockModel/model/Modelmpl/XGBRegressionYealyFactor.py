...
class XGBRegressionYealyFactor(ModelBase):
    ...
    def get_fix_factor_evaluation(self, num,end_date):
        factor_list = pd.read_pickle('/data/group/800319/Strong_stock/ic_sort_yearly_window40.pkl')
        factor_list = factor_list.loc[end_date//10000].sort_values(ascending=False)
        if num > len(factor_list):
            num = len(factor_list)
            print('the max length of fix_factor_list is %d' % (len(factor_list)))
        return factor_list.index.tolist()[:num]
    ...
    def get_dataset(self, train_idx, test_idx, fix_factor_list, interday_factor, label_method, label_param={}, kernel=10):
        ...
        print('subclass get dataset')
        return super().get_dataset(train_idx, test_idx, fix_factor_list, interday_factor, label_method, label_param, kernel)
