...
class XGB5min(ModelBase):
    def __init__(self, start=20170103, end=20191231,factor_num=100, stock_pool=None, feature_address='/arch1/group/800442/800319/HFfactor/DTC2021/data/',indicator=None):
    ...
    def get_dataset(self, train_idx, test_idx, fix_factor_list, interday_factor, label_method, label_param={}, kernel=10):
        ...
        X_train, y_train = self.loader.load_fix_point(X_train, y_train,d_train,t_train,c_train,self.factor_list)
        X_test, y_test = self.loader.load_fix_point(X_test, y_test, d_test, t_test, c_test,self.factor_list)
        X_pred, y_pred = self.loader.load_fix_point(X_pred, y_pred, d_pred, t_pred, c_pred,self.factor_list)
    ...
    def train_model(self, X, y, params, end_date=None):
        ...
        if 'val_pred_path' in params:
            ...
            val_labels = pd.DataFrame({'actual_label':val_labels['actual_label'],'prediction':model.predict(d_val)})
...
def main_window_search(i, indicator):
    ...
    test_period = 10
    factor_num = 400
    tag = f'XGB5minFixPoint_{indicator}_train{train_period}_test{test_period}_factor_num{factor_num}'
...
for i in tqdm(list(range(133))):
    for ind_name in [ 'ic_half_d', 'ic_half_t', 'ic_half_c']:
        ...
        gc.collect()
        if datetime.datetime.now()>=datetime.datetime(2021, 7, 1, 6, 00, 54, 416101):
            break
