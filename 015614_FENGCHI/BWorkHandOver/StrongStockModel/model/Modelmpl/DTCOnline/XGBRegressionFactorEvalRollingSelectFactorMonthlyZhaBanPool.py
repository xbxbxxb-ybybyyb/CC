...
class XGBRegressionFactorEvalYearly(ModelNewLoading):
    ...
    def get_dataset(self, train_idx, test_idx, fix_factor_list, interday_factor, label_method, label_param={}, kernel=10):
        ...
        gc.collect()
        e = time.time()
        factor_direction = pd.read_pickle('/data/group/800319/strategy_local_path/factor_direction.pkl')[fix_factor_list].values
        ...
        if train_idx[-1] == test_idx[0] and train_idx[-1] == test_idx[-1]:
            train_feature, train_label, nolimit_train, train_idx_date, train_idx_code, train_idx_time = load_fix_data(train_idx[0], get_pre_trade_date(train_idx[-1]),
                                                                                                                      fix_factor_list,nolimit_tag='__nolimit_zhaban')
        else:
            train_feature, train_label, nolimit_train, train_idx_date, train_idx_code, train_idx_time = load_fix_data(train_idx[0], train_idx[-1], fix_factor_list,nolimit_tag='__nolimit_zhaban')
        ...
        if train_idx[-1] == test_idx[0] and train_idx[-1] == test_idx[-1]:
            ...
            if test_idx[-1] >= today:
                test_feature, test_label, test_nolimit, test_idx_date, test_idx_code, test_idx_time = load_fix_data(start_date=test_idx[0], end_date=get_pre_trade_date(today),
                                                                                                                    factor_list=fix_factor_list, return_idx=True,nolimit_tag='__nolimit_zhaban')
            else:
    ...
    def train_model(self, X_train, y_train, params, end_date=None):
        ...
        if 'load local model' in params and os.path.exists(params['model_conf_path'] + '%d.json' % end_date):
            ...
            d_val = xgb.DMatrix(val_features,label=val_labels.values)
            # d_eval = xgb.DMatrix(train_features, label=train_label.values)
            model = xgb.train(args_param, d_train, num_boost_round=params['n_estimators'], evals=[(d_val, 'd_eval')], early_stopping_rounds=15, verbose_eval=False)
...
def main_window_search(i, indicator):
    ...
    out_file = '/data/group/800319/Faamonitor/PL/all_mkt_ts_norm/MonthlySelected/XGB_ZhaBan_%s_train%d_test%d_factor_num%d.pkl' % (
...
for i in tqdm(list(range(134))):
    for ind_name in ['ic_d'][::-1]:
