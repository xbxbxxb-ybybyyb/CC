    ...
    def rolling_train_and_predict(self, params={}, period=10, predict_period=10, label_methodology='fix_window', label_param={}, factor_nums=200, kernel=10):
        ...
        for idx, cell_idx in bar:
            ...
            X_train, y_train, X_test, y_test, feature_engineering_time = \
                self.get_dataset((get_pre_trade_date(train_end_idx, 55), get_pre_trade_date(train_end_idx, 15)), (test_start_idx, test_end_idx),
...
def main(i):
    ...
    out_file = '/data/group/800319/Faamonitor/PL/all_mkt_ts_norm/FeatureEngineeringExplore/NNExtractorRecent40Sigmoid_%s_train%d_test%d_factor_num%d_norm_window_%d.pkl' % (
    ...
    if os.path.exists(base_dir + '%d.pkl' % train_end):
        ...
        # return
    print(out_file)
    model = NN_redefine(train_start, test_end, None, feature_address='/data/group/800319/HFfactor/RealTimeFixRollRobust/data/', factor_eval_indicator=indicator, factor_num=factor_num)
...
idx_origin = list(range(73))
idx_origin = [idx_origin[len(idx_origin) * i // 40:(i + 1) * len(idx_origin) // 40] for i in range(40)]