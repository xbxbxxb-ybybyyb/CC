...
def load_dataset(start,end,split_date):
    if os.path.exists(f'{base_dir}train_set.pkl') and os.path.exists(f'{base_dir}test_set.pkl'):
        return
    ...
    fix_factor_list = eval_indicator.loc[target_date].apply(abs).sort_values(ascending=False).index.tolist()[:FACTOR_NUM]
...
def train_LSTM(hidden_dim,rec_layers,full_conn_dims,dropout):
    ...
    if os.path.exists(f'{out_dir}/{tag}.pkl') or os.path.exists(f'{out_dir}/{model_dir}.pkl') :
        return
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
    if not os.path.exists(model_dir):
        os.makedirs(model_dir)
    lstm = LSTMModel(FACTOR_NUM,hidden_dim, rec_layers, full_conn_dims, dropout)
...
base_dir = '/data/user/015664/ModelFile/lstm_fixFIXSEED300/'
