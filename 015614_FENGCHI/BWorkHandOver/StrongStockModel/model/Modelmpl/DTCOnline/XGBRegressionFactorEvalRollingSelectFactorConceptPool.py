# @Time : 2020/9/17 9:22
# @Author : Zhichen Lu
# @File : XGBRegressionFactorEvalYearlyConceptPool.py
import sys
...
class XGBRegressionFactorEvalYearlyCustomPool(ModelNewLoading):
    def __init__(self, start=20170103, end=20191231, stock_pool=None, feature_address=None, factor_eval_indicator=None, factor_num=None,pool_name='_active_stock_pool'):
        super().__init__(start, end, stock_pool, feature_address, factor_eval_indicator, factor_num=factor_num)
        self.custom_pool_name = pool_name
    ...
    def get_dataset(self, train_idx, test_idx, fix_factor_list, interday_factor, label_method, label_param={}, kernel=10):
        ...
        gc.collect()
        e = time.time()
        X, y, nolimit, idx_date, idx_time, idx_code = self.dp.load_data(start_date=train_idx[0], end_date=test_idx[-1], return_idx=True)
        stock_pool = self.dp.load_custom_pool(self.custom_pool_name,start_date=train_idx[0],end_date=test_idx[-1])
        X, y, idx_date, idx_time, idx_code = self.dp.feature_engineering(X, y, stock_pool, idx_date, idx_time, idx_code)
        ...
        train_feature, train_label = pd.DataFrame(train_feature, index=train_index.index, columns=fix_factor_list), pd.DataFrame({'actual_label': train_label},
                                                                                                                                 index=train_index.index)
        test_feature, test_label = pd.DataFrame(test_feature, index=test_index.index, columns=fix_factor_list), pd.DataFrame({'actual_label': test_label}, index=test_index.index)
...
def main_window_search(i,indicator):
    ...
    out_file = '/data/group/800319/Faamonitor/PL/all_mkt_ts_norm/FactorEvalOnlineTestVRobustOpt/XGBFactorEvalYearlyParam10Concept_%s_train%d_test%d_factor_num%d_norm_window_%d.pkl' % (
        indicator, train_period, test_period, factor_num, N)
    ...
    best_param_clf_xgb = {'booster': 'gbtree', 'colsample_bytree': 0.8, 'eta': 0.1, 'gamma': 0.17761168444070607, 'max_depth': 16, 'min_child_weight': 1551, 'n_estimators': 100, 'sampling_method': 'gradient_based', 'subsample': 0.8, 'tree_method': 'gpu_hist'}
    ...
    model = XGBRegressionFactorEvalYearlyCustomPool(train_start, test_end, None, feature_address='/data/group/800319/HFfactor/RealTimeFixRollRobust/data/',
                                          factor_eval_indicator=indicator,
                                          factor_num=factor_num,pool_name='_active_stock_pool')
...
# for i in [117,123]:
#     for ind_name in ['ic_half_d','ic_half_t','ic_half_c']:
#         main_window_search(i, ind_name)