...
class GBDTLR(ModelNewLoading):
    ...
    def train_model(self, X_train, y_train, params, end_date=None):
        ...
        if 'load local model' in params and os.path.exists(params['model_conf_path'] + '%d.pkl' % end_date):
            ...
            lgb_train = lgb.Dataset(train_features[:40000], train_label[:40000])
            lgb_eval = lgb.Dataset(train_features[40000:50000],train_label[40000:50000],reference=lgb_train)

            gbm = lgb.train(params,
                            lgb_train,
                            num_boost_round=100,
                            valid_sets=lgb_train)
            gbm.predict(train_features[40000:50000])
            y_pred = gbm.predict(train_features[:40000],pred_leaf=True)
            y_pred.shape

            model = GradientBoostingRegressor()
            model.fit(train_features[:400],train_label[:400])
...
def main_window_search(i,indicator):
    ...
    out_file = '/data/group/800319/Faamonitor/PL/all_mkt_ts_norm/FeatureEngineeringExplore/GBDTLR_%s_train%d_test%d_factor_num%d_norm_window_%d.pkl' % (
        indicator, train_period, test_period, factor_num, N)
    ...
    model = GBDTLR(train_start, test_end, None, feature_address='/data/group/800319/HFfactor/RealTimeFixRollRobust/data/',
