...
class XGBRegressionFactorEval(ModelNewLoading):
    ...
    def train_model(self, X_train, y_train, params, end_date=None):
        ...
        if 'load local model' in params and os.path.exists(params['model_conf_path'] + '%d.json' % end_date):
            ...
            model = xgb.train(args_param, d_train, num_boost_round=params['n_estimators'], verbose_eval=False,early_stopping_rounds=15)
...
def main_window_search(i,indicator):
    ...
    out_file = '/data/group/800319/Faamonitor/PL/all_mkt_ts_norm/FactorEvalOnlineTestVRobustNoFuture20210121/XGBFactorEvalNewPara95_%s_train%d_test%d_factor_num%d_norm_window_%d.pkl' % (
        indicator, train_period, test_period, factor_num, N)
    ...
    if os.path.exists(base_dir + '%d.pkl' % train_end):
        ...
        # return
    print(out_file)
    # best_param_clf_xgb = {'booster': 'gbtree', 'colsample_bytree': 0.8,
    #                       'max_depth': 4, 'nthread': -1, 'scale_pos_weight': 1,
    ...
    model = XGBRegressionFactorEval(train_start, test_end, None, feature_address='/data/group/800319/FixRollRobust/FixRoll/data/',
