...
nnic = TrainWrapper([200, 100, 1])

# X = torch.from_numpy(X_train.values).float()
# y = torch.from_numpy(y_train['actual_label'].values).float()

nnic.train(X=torch.from_numpy(X_train.values).float(),y=torch.from_numpy(y_train['actual_label'].values).float(),
...
pred_val = nnic.model(torch.from_numpy(X_train[-150000:].values))
pred_val = pred_val.detach().numpy()
label_val = y_train[-150000:]
label_val['pred'] = pred_val[:,0]
label_val.corr()


middle_out = nnic.model.last_hidden(torch.from_numpy(X_train[-150000:].values))
