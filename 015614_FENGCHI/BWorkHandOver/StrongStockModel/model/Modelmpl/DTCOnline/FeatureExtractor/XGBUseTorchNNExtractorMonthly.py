...
class XGBRegressionFactorEvalYearly(ModelNewLoading):
    ...
    def get_dataset(self, train_idx, test_idx, fix_factor_list, interday_factor, label_method, label_param={}, kernel=10):
        ...
        _,_,test_feature, test_label, _ = super().get_dataset((train_idx[-1],train_idx[-1]),test_idx,fix_factor_list, interday_factor, label_method, label_param, kernel)
    ...
    def predict(self, model, X_test, end_date=None,label_param={}):
        ...
        wraper.reinitial(X_test.shape[1])
        wraper.model.float()
        wraper.model.load_state_dict(torch.load(label_param['model_conf_path_NN'] + f'{end_date}.pkl'))

        X_test = pd.DataFrame(wraper.model.last_hidden(torch.from_numpy(X_test.values).float()).detach().numpy(),index=X_test.index)
...
def main_window_search(i, indicator):
    ...
    tag = 'XGBUseNNExtraTroch_%s_train%d_test%d_factor_num%d' % (indicator, train_period, test_period, factor_num)
    out_file = f'/data/group/800442/800319/Strong_stock/C3PO/{tag}/{tag}.pkl'

    tag_NN = 'NNExtractorTorch_%s_train%d_test%d_factor_num%d' % (indicator, 100, 200, 800)
    out_file_NN = f'/data/group/800442/800319/Strong_stock/C3PO/ParamOpt/{tag_NN}/{tag_NN}.pkl'

    base_dir = out_file.replace('.pkl', '/')
    train_start, train_end, test_start, test_end = para_list[i][1]
    if not os.path.exists(out_file_NN.replace('.pkl',f'/{train_end}.pkl')):
        print('no exist feature')
        return
    ...
    best_param_clf_xgb = {
        'booster': 'gbtree', 'colsample_bytree': 0.8, 'eta': 0.1, 'gamma': 0.17761168444070607,
    ...
    label = model.rolling_train_and_predict(params=best_param_clf_xgb, period=len(get_date_range(train_start,train_end)), predict_period=len(get_date_range(test_start,test_end)),
