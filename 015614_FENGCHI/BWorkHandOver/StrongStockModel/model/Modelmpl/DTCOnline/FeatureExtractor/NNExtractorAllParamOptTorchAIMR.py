...
para = eval(AIMR.getParam())
if datetime.datetime.now()>=datetime.datetime(2021, 8, 23, 6, 10, 29, 11109):
    pass
else:
    main(*para)
...
# conf = configparser.ConfigParser()
# conf.read('/data//group/800442/800319/strategy_local_path_offline/period_info.ini')
# para_list  = list(itertools.product(list(range(138))[24:],['ic_c','ic_d','ic_c']))
# period_list = eval(conf['period_info']['period_info'])
# target='/data/group/800442/800319/Strong_stock/C3PO/ParamOpt/NNExtractorTorch_%s_train100_test200_factor_num800/NNExtractorTorch_ic_c_train100_test200_factor_num800/%d.pkl'
# para_list = list(filter(lambda x : not os.path.exists(target%(x[1],period_list[x[0]][1][1])),para_list))
