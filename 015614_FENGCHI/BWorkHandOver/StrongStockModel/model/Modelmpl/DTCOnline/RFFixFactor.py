...
# 'min_weight_fraction_leaf': 0.001, 'n_estimators': 100, 'presort': 'auto', 'random_state': None, 'subsample': 1.0, 'verbose': 1, 'warm_start': False}

random_forest_param = {'bootstrap': True, 'criterion': 'mse', 'max_depth': None, 'max_features': 0.45, 'max_leaf_nodes': 400,
 'min_impurity_decrease': 0.0, 'min_impurity_split': None, 'min_samples_leaf': 10000, 'min_samples_split': 5000, 'min_weight_fraction_leaf': 0.0,
 'n_estimators': 100, 'n_jobs': -1, 'oob_score': False, 'random_state': None, 'verbose': 1, 'warm_start': False}
...
def fit_model(i,output_path,indicator,num):
    ...
    if not os.path.exists(path_dict['model_conf_path']+'%d.pkl'%train_end):
        ...
        model.set_params(**random_forest_param)
        print('training...')
        e = time.time()
...
# for idx in tqdm(idx_list,desc=eval_indicator):
...
#     gc.collect()
idx_list = list(range(73))
idx_list = [idx_list[len(idx_list)*i//5:(i+1)*len(idx_list)//5] for i in range(5)]
idx_tuple_list = [[28, 39], [52, 53], [57, 66], [67, 68, 72]]#list(zip(*idx_list))
for idx_list in idx_tuple_list[::-1]:
    for idx in tqdm(idx_list[::-1],desc=str(idx_list)):
        out_path = f'/data/group/800319/Faamonitor/PL/all_mkt_ts_norm/FeatureEngineeringExplore/GBDTOnly_train400_test10_factornum{factor_num}_eval_{eval_indicator}/'
        print(out_path)
        fit_model(idx,out_path,eval_indicator,factor_num)
        gc.collect()
