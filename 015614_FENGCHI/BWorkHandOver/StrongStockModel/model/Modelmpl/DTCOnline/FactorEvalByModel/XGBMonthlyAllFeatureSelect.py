...
def main_window_search(i, indicator):
    ...
    out_file = '/data/group/800442/800319/Faamonitor/PL/all_mkt_ts_norm/AllFeatureSelect/XGBAllFeature_%s_train%d_test%d.pkl' % (
        indicator, train_period, test_period)
    ...
    label = model.rolling_train_and_predict(params=best_param_clf_xgb, period=train_period, predict_period=test_period,
                                            label_param={'kind': 'reg'}, kernel=15, factor_nums=None)
...
for i in tqdm(list(range(138))[24:][::-1]):
