...
class XGBRegressionFactorEvalYearly(ModelNewLoading):
    ...
    def get_fix_factor_evaluation(self,param, num, end_index):
        # factor_list = set(list(map(lambda x : x.replace('.npy',''),os.listdir(self.feature_address)))).intersection(self.using_factor_list)#self.using_factor_list
        EVAL_RES_PATH = '/data/group/800442/800319/Faamonitor/PL/all_mkt_ts_norm/AllFeatureSelect/XGBAllFeature_AllFactor_train200_test10_EvalRes/'
        factor_eval_res = pd.read_pickle(f'{EVAL_RES_PATH}/{end_index}.pkl')
        factor_list = factor_eval_res[self.eval_indicator].dropna().sort_values(ascending=False).index.tolist()[:num]
        if len(factor_list)!=num:
            print(f'{end_index} only have {len(factor_list)} factors')
        print(f'factor num {len(factor_list)}')
        return sorted(list(factor_list))
    ...
    def train_model(self, X_train, y_train, params, end_date=None):
        ...
        val_date = date_list[-50:-1]#[date_list[i] for i in [-1, -3, -5, -7, -9, -11]]
        ...
        if 'val_pred_path' in params:
            ...
            pd.to_pickle([val_features,val_labels], params['val_pred_path'] + '%d.pkl' % end_date)
    ...
    def rolling_train_and_predict(self, params={}, period=10, predict_period=10, label_methodology='fix_window', label_param={}, factor_nums=200, kernel=10):
        ...
        for idx, cell_idx in bar:
            ...
            if os.path.exists(params['feature_path'] + '%d.pkl' % train_end_idx):
                ...
                if len(fix_factor_list)!=factor_nums:
                    print(train_end_idx,'wrong re train')
                    os.remove(params['feature_path'] + '%d.pkl' % train_end_idx)
                    os.remove(params['model_conf_path'] + '%d.json' % train_end_idx)
                    os.remove(params['val_pred_path'] + '%d.pkl' % train_end_idx)
                    os.remove(params['val_pred_path'].replace('_val_pred','') + '%d.pkl' % train_end_idx)
                    fix_factor_list = self.get_fix_factor_evaluation(params,factor_nums, train_end_idx)
                else:
                    return None
            else:
                fix_factor_list = self.get_fix_factor_evaluation(params,factor_nums, train_end_idx)
...
def main_window_search(i, indicator):
    ...
    test_period = 10
    factor_num = 400
    tag_name = f'XGB_{indicator}_train{train_period}_test{test_period}_factorNum{factor_num}'
    out_file = f'/data/group/800442/800319/Faamonitor/PL/all_mkt_ts_norm/DoubleEnsembleSelect/{tag_name}/{tag_name}.pkl'
...
for i in tqdm(list(range(134))[24:]):
    for ind in ['DEGain', 'corr_diff', 'mae_diff']:
        main_window_search(i,ind)
        gc.collect()
        # p = Process(target=main_window_search,args=(i, 'AllFactor'))
        # p.start()
        # p.join()
        # del p
        # gc.collect()
