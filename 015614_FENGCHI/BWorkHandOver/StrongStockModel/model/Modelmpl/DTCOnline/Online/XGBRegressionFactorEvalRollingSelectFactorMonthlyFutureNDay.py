...
class XGBRegressionFactorEvalYearly(ModelNewLoading):
    ...
    def get_dataset(self, train_idx, test_idx, fix_factor_list, interday_factor, label_method, label_param={}, kernel=10):
        window = label_param['future']
        label = pd.read_pickle(f'{root_path}labels/future_{int(window)}.pkl')
        gc.collect()
        e = time.time()
        ...
        train_label = trans_df2arr(label.loc[train_idx[0]:train_idx[-1]],train_idx[0],train_idx[-1])
        # check_train_label = trans_df2arr(label.loc[train_idx[0]:train_idx[-1]],start_date=train_idx[0],end_date=train_idx[-1],code_list=label.columns.tolist())
        ...
        if train_idx[-1] == test_idx[0] and train_idx[-1] == test_idx[-1]:
            ...
            test_label = trans_df2arr(label.loc[test_idx[0]:test_idx[1]],test_idx[0],test_idx[1])
...
def main_window_search(i, indicator,window):
    train_period = 200
    test_period = 10
    factor_num = 400

    tag = f'XGBV4FactorList_Future{window}_{indicator}_train{train_period}_test{test_period}_factor_num{factor_num}'
    out_file = f'/data/group/800442/800319/Faamonitor/PL/all_mkt_ts_norm/MonthlySelected/{tag}.pkl'
    ...
    label = model.rolling_train_and_predict(params=best_param_clf_xgb, period=train_period, predict_period=test_period,
                                            label_param={'future': window}, kernel=15, factor_nums=factor_num)
    pd.to_pickle(label, base_dir + '%d.pkl' % train_end)
    print(base_dir + '%d.pkl' % train_end)

for i in tqdm(list(range(24,133))):
    for ind_name in ['ic_d','ic_c', 'ic_t']:

        main_window_search(i, ind_name,int(3*240))
from dataApi.sendInfo import send_message

send_message(['015664'],f'GPU release in {datetime.datetime.now()}')
