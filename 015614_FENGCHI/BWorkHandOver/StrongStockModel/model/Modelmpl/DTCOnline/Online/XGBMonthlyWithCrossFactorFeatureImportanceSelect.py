...
class XGBRegressionFactorEvalYearly(ModelNewLoading):
    ...
    def get_dataset(self, train_idx, test_idx, fix_factor_list, interday_factor, label_method, label_param={}, kernel=10):
        ...
        if train_idx[-1] == test_idx[0] and train_idx[-1] == test_idx[-1]:
            test_feature, test_label = pd.DataFrame(columns=col_list), pd.DataFrame(columns=col_list)
        else:
            if test_idx[-1] >= today:
    ...
    def rolling_train_and_predict(self, params={}, period=10, predict_period=10, label_methodology='fix_window', label_param={}, factor_nums=200, kernel=10,cross_res_path='/arch1/group/800442/800319/AAcross/factor_result/1min/20140701_20210531/'):
        ...
        for idx, cell_idx in bar:
            ...
            cross_res_path = '/arch1/group/800442/800319/AAcross/factor_result_rerun6/1min/20140701_20210531/'

            cross_eval = pd.read_pickle(f'/data/group/800442/800319/Faamonitor/PL/all_mkt_ts_norm/CrossFactor20220125/XGBOnlyCross_ic_c_train200_test10_factor_num0/XGBOnlyCross_ic_c_train200_test10_factor_num0_model_conf/eval_res/{train_end_idx}.pkl')
            indicator_map = {'ic_d':'weight','ic_t':'gain','ic_c':'cover'}

            cross_factor_list = sorted(cross_eval[indicator_map[self.eval_indicator]].sort_values(ascending=False).index.tolist()[:factor_nums])

            cross_factor_list = list(filter(lambda x : os.path.exists(f'/data/group/800442/800319/HFfactor/CrossFactor/data/{x}.npy'),cross_factor_list))
            print(f'cross {len(cross_factor_list)}')
            print(f'cross factor {len(cross_factor_list)}')
...
def main_window_search(i, indicator):
    ...
    tag = 'XGBMixByFeatureImportance_%s_train%d_test%d_factor_num%d' % (indicator, train_period, test_period, factor_num)
    out_file = f'/data/group/800442/800319/Faamonitor/PL/all_mkt_ts_norm/CrossFactor20220125/{tag}/{tag}.pkl'
...
for i in tqdm(list(range(134))[24:][::-1]):
    for ind_name in ['ic_d','ic_c', 'ic_t'][::-1]:
