...
class XGBRegressionFactorEvalYearly(ModelNewLoading):
    def __init__(self, start=20170103, end=20191231, stock_pool=None, feature_address=None, factor_eval_indicator=None, factor_num=None):
        ...
        self.using_factor_list = list(map(lambda x : x.replace('.npy',''),os.listdir(feature_address)))
    ...
    def get_fix_factor_evaluation(self, num, end_index):
        ...
        enhanced_minute_factor = pd.read_excel(f'{root_path}external_data/增强K因子差异.xlsx',sheet_name='FIX')['logic'].tolist()
        enhanced_minute_factor = set(enhanced_minute_factor).intersection(self.using_factor_list)

        target_date = max(list(filter(lambda x : x<end_index,factor_evaluation.index.tolist())))
        factor_evaluation = factor_evaluation[list(set(factor_evaluation.columns)-set(enhanced_minute_factor))]
        ...
        if 'ret' in self.eval_indicator:
            ...
            raise Exception('')
        factor_list = factor_evaluation.index.tolist()[:num-len(enhanced_minute_factor)]
        factor_list = sorted(list(set(factor_list).union(enhanced_minute_factor)))
...
def main_window_search(i, indicator):
    ...
    out_file = '/data/group/800442/800319/Faamonitor/PL/all_mkt_ts_norm/MonthlySelected/XGBEnhanced_%s_train%d_test%d_factor_num%d.pkl' % (
...
for i in tqdm(list(range(133))):
    for ind_name in ['ic_d', 'ic_t', 'ic_c']:
        if datetime.datetime.now()>=datetime.datetime(2021, 7, 14, 6, 0, 0, 0):
            send_message(['015664'],'GPU释放中')
            break
        main_window_search(i, ind_name)
    if datetime.datetime.now() >= datetime.datetime(2021, 7, 14, 6, 0, 0, 0):
        break
send_message(['015664'],'GPU已经释放')
