...
class LGBRegressionFactorEvalYearly(ModelNewLoading):
    ...
    def train_model(self, X_train, y_train, params, end_date=None):
        key_list = ['boosting','first_metric_only', 'colsample_bytree', 'learning_rate', 'min_gain_to_split', 'max_depth', 'min_child_weight', 'n_estimators', 'subsample', 'device_type', 'objective']#set(list(CatBoostRegressor().get_params().keys())).intersection(set(list(params.keys())))
        ...
        if 'load local model' in params and os.path.exists(params['model_conf_path'] + '%d.json' % end_date):
            ...
            print('start training',model.get_params())
            model.fit(X=train_features[:-50000],y=train_label[:-50000],
                      eval_set=[(train_features[-50000:],train_label[-50000:])],
...
def main_window_search(i, indicator):
    ...
    out_file = '/data/group/800319/Faamonitor/PL/all_mkt_ts_norm/MonthlySelected/LGB_%s_train%d_test%d_factor_num%d.pkl' % (
    ...
    best_param_clf_xgb = {'boosting': 'dart', 'colsample_bytree': 0.8, 'learning_rate': 0.03, 'min_gain_to_split': 0.17761168444070607,
     'max_depth': 8, 'min_child_weight': 1551, 'n_estimators': 100,
     'subsample': 0.8,'device_type':'gpu','objective':'rmse','first_metric_only':True}
    ...
    model = LGBRegressionFactorEvalYearly(train_start, test_end, None, feature_address='/data/group/800319/HFfactor/RealTimeFixRollRobust/data/',
...
for i in tqdm(list(range(135))):
    present = datetime.datetime.now()
    if present.hour==6 and present.minute>=45:
        send_message(['015664'],'lgb stop')
        break
    if present.hour>6 and present.day>14:
        send_message(['015664'],'lgb stop')
        break
