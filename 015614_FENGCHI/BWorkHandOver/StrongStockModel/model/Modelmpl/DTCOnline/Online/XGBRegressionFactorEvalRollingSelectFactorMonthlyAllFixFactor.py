...
class XGBRegressionFactorEvalYearly(ModelNewLoading):
    def __init__(self, start=20170103, end=20191231, stock_pool=None, feature_address=None, factor_eval_indicator=None, factor_num=None):
        ...
        self.using_factor_list = [x.replace('.npy','') for x in os.listdir(feature_address)]
...
def main_window_search(i, indicator):
    ...
    out_file = '/arch1/group/800442/800319/Faamonitor/PL/all_mkt_ts_norm/MonthlySelected/XGBVALLFixSelected_%s_train%d_test%d_factor_num%d.pkl' % (
...
for i in tqdm(list(range(133))[::-1]):
    for ind_name in ['ic_d', 'ic_t', 'ic_c'][::-1]:
    # for ind_name in ['top_ret'][::-1]:
        main_window_search(i, ind_name)
        gc.collect()
    # if datetime.datetime.now()>=datetime.datetime(2021, 7, 2, 6, 00, 54, 416101):
    #     break