...
def main_window_search(i, indicator):
    ...
    out_file = '/data/group/800319/Faamonitor/PL/all_mkt_ts_norm/FactorEvalOnlineTestVRobustOpt/XGBFactorEvalYearlyParam10_%s_train%d_test%d_factor_num%d_norm_window_%d.pkl' % (
        indicator, 200, 10, factor_num, N)
...
# def get_fix_factor_evaluation( num,eval_indicator, end_index):
#
#     factor_evaluation = pd.read_pickle(root_path + 'external_data/ic_half.pkl')  # .set_index('name')
#     factor_evaluation = pd.DataFrame(factor_evaluation)
...
# factor_2021 = {}
# factor_intersection = {}
# for each in ['ic_half_d', 'ic_half_t', 'ic_half_c']:
#     factor_20200102[each] = get_fix_factor_evaluation(400,each,20200102)
#     factor_2021[each] = get_fix_factor_evaluation(400,each,20210501)
#     factor_intersection[each] = len(set(factor_20200102[each]).intersection(set(factor_2021[each])))
