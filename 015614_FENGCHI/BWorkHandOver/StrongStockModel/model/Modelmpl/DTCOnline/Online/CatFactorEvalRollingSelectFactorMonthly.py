...
class CatRegressionFactorEvalYearly(ModelNewLoading):
    ...
    def train_model(self, X_train, y_train, params, end_date=None):
        key_list = ['task_type', 'iterations', 'depth', 'loss_function', 'eval_metric', 'learning_rate']#set(list(CatBoostRegressor().get_params().keys())).intersection(set(list(params.keys())))
        ...
        if 'load local model' in params and os.path.exists(params['model_conf_path'] + '%d.json' % end_date):
            ...
            model.fit(X=train_features[:-50000],y=train_label[:-50000],
                      ...
                      verbose=True,early_stopping_rounds=10,use_best_model=True)
...
def main_window_search(i, indicator):
    ...
    out_file = '/data/group/800319/Faamonitor/PL/all_mkt_ts_norm/MonthlySelected/Cat_%s_train%d_test%d_factor_num%d.pkl' % (
    ...
    best_param_clf_xgb = {'task_type':'GPU','iterations': 200,'depth':5,'loss_function': 'RMSE','eval_metric':'RMSE','learning_rate':0.01}
    ...
    model = CatRegressionFactorEvalYearly(train_start, test_end, None, feature_address='/data/group/800319/HFfactor/RealTimeFixRollRobust/data/',
