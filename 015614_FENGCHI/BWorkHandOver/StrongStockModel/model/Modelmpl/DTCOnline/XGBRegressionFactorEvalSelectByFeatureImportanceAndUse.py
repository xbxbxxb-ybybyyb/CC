...
def fit_model(i,output_path,indicator,num):
    ...
    feature_eval_path = '/data/group/800319/Faamonitor/PL/all_mkt_ts_norm/FeatureEngineeringExplore/XGBAllFactor_train50_test50feature_eval/'
    path_dict = dict(
    ...
    feature_eval_res = pd.read_pickle(feature_eval_path+'%d.pkl'%train_end)
    using_factor_list = sorted(feature_eval_res.sort_values(indicator,ascending=False).index.tolist())
...
eval_indicator = indicators[0]
factor_num = 400

idx_list = list(range(73,128))#[::-1]

for idx in tqdm(idx_list):
    # fit_model(idx,out_path,eval_indicator,factor_num)
    for eval_indicator in indicators:
        out_path = f'/data/group/800319/Faamonitor/PL/all_mkt_ts_norm/FeatureEngineeringExplore/XGBAllFactor_train400_test10_factornum{factor_num}_eval_{eval_indicator}/'
        print(out_path)
