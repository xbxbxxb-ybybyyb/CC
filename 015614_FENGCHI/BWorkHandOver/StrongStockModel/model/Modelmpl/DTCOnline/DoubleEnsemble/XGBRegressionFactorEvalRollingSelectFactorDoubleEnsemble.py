...
class XGBRegressionFactorEvalYearly(ModelNewLoading):
    ...
    def train_model(self, X_train, y_train, params, end_date=None):
        ...
        if 'load local model' in params and os.path.exists(params['model_conf_path'] + '%d/model_%d.json' % (end_date,params['n_estimators'])):
            ...
            d_train = xgb.DMatrix(train_features, label=train_label['actual_label'],weight=sample_weight)
            check_point = xgb.callback.TrainingCheckPoint(params['model_conf_path'] + '%d/' % end_date,iterations=1)
            model = xgb.train(args_param, d_train, num_boost_round=params['n_estimators'], verbose_eval=True,callbacks=[check_point])
            model.save_model(params['model_conf_path'] + '%d/model_%d.json' % (end_date,params['n_estimators']))
...
def main_window_search(i,round_num, indicator):
    ...
    out_file = '/data/group/800319/Faamonitor/PL/all_mkt_ts_norm/DoubleEnsemble/XGBDoubleEnsemble_%s_train%d_test%d_factor%d.pkl' % (
    ...
    best_param_clf_xgb = {'booster': 'gbtree', 'colsample_bytree': 0.5243169632466114, 'gamma': 0.1856029577071825, 'max_depth': 7, 'min_child_weight': 9360, 'n_estimators': 100,
                          'sampling_method': 'gradient_based', 'subsample': 0.9672279176597204, 'tree_method': 'gpu_hist','bin_num':1000,'round_num':round_num}
...
for ind_name in ['ic_half_t','ic_half_d','ic_half_c']:
    for round_id in range(1,6):
        for i in list(range(73))[::-1]:
