...
class XGBRegressionFactorEvalYearly(ModelNewLoading):
    ...
    def get_dataset(self, train_idx, test_idx, fix_factor_list, interday_factor, label_method, label_param={}, kernel=10):
        ...
        if train_idx[-1] == test_idx[0] and train_idx[-1] == test_idx[-1]:
            train_feature, train_label, nolimit_train, train_idx_date, train_idx_code, train_idx_time = load_fix_data_selfdefined_label(train_idx[0], get_pre_trade_date(train_idx[-1]),
                                                                                                                      fix_factor_list,address=self.feature_address,label_path=self.label_path)
        else:
            train_feature, train_label, nolimit_train, train_idx_date, train_idx_code, train_idx_time = load_fix_data_selfdefined_label(train_idx[0], train_idx[-1], fix_factor_list,address=self.feature_address,label_path=self.label_path)
        ...
        if train_idx[-1] == test_idx[0] and train_idx[-1] == test_idx[-1]:
            ...
            if test_idx[-1] >= today:
                test_feature, test_label, test_nolimit, test_idx_date, test_idx_code, test_idx_time = load_fix_data_selfdefined_label(start_date=test_idx[0], end_date=get_pre_trade_date(today),
                                                                                                                    factor_list=fix_factor_list, return_idx=True,address=self.feature_address,label_path=self.label_path)
            else:
                test_feature, test_label, test_nolimit, test_idx_date, test_idx_code, test_idx_time = load_fix_data_selfdefined_label(start_date=test_idx[0], end_date=test_idx[-1],
                                                                                                                    factor_list=fix_factor_list, return_idx=True,address=self.feature_address,label_path=self.label_path)
...
def main_window_search(i, indicator,future_bar_num):
    ...
    out_file =f'/data/group/800442/800319/Faamonitor/PL/all_mkt_ts_norm/MonthlySelectedMDDImproveV2_20220120/Future_{future_bar_num}_bar/{target_file}/{target_file}.pkl'
    ...
    model = XGBRegressionFactorEvalYearly(train_start, test_end, None, feature_address='/data/group/800442/800319/HFfactor/RealTimeFixRollRobustReversalRes/data/',
                                          ...
                                          factor_num=factor_num,label_path='/data/group/800442/800319/HFfactor/ForDerivativeLabel/data/',future_bar_num=future_bar_num)
