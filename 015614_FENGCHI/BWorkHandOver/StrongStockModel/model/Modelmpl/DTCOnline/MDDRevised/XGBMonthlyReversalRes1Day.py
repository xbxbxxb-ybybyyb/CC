...
def main_window_search(i, indicator):
    ...
    test_period = 10
    factor_num = 400
    target_file = 'XGBV4ReversalRes1Day_%s_train%d_test%d_factor_num%d' % (indicator, train_period, test_period, factor_num)
    out_file =f'/data/group/800442/800319/Faamonitor/PL/all_mkt_ts_norm/MonthlySelectedMDDImprove/{target_file}/{target_file}.pkl'
    ...
    model = XGBRegressionFactorEvalYearly(train_start, test_end, None, feature_address='/data/group/800442/800319/HFfactor/RealTimeFixRollRobustReversalResRct1Day/data/',
...
for i in list(range(len(para_list)-2))[24:]:
    for ind_name in ['ic_d','ic_c', 'ic_t']:
        # for ind_name in ['top_ret'][::-1]:
        main_window_search(i, ind_name)
    if i>=134:
        send_message(['015664'],'Model for backtest is done')
