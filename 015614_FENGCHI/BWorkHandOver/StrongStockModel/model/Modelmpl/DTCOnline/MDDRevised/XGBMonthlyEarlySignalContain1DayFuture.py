...
class XGBRegressionFactorEvalYearly(ModelNewLoading):
    ...
    def get_fix_factor_evaluation(self, num, end_index):
        factor_evaluation = pd.read_pickle(f'{root_path}external_data/FutureBarBy30Min/Future_{self.future_bar_num}_bar/{self.eval_indicator}.pkl')
        print(f'{root_path}external_data/FutureBarBy30Min/Future_{self.future_bar_num}_bar/{self.eval_indicator}.pkl')
...
def main_window_search(i, indicator,future_bar_num):
    ...
    out_file =f'/data/group/800442/800319/Faamonitor/PL/all_mkt_ts_norm/MonthlySelectedMDDImproveV3With1DayLabel_20220126/Future_{future_bar_num}_bar/{target_file}/{target_file}.pkl'
    ...
    model = XGBRegressionFactorEvalYearly(train_start, test_end, None, feature_address='/data/group/800442/800319/HFfactor/RealTimeFixRollRobustReversalRes/data/',
                                          ...
                                          factor_num=factor_num,label_path='/data/group/800442/800319/HFfactor/ForDerivativeLabel_7bar/data/',future_bar_num=future_bar_num)
...
bar = tqdm(total=6*len(idx_list)*3,desc='---------------------------')
...
# pool = Pool(10)
# res_dic = {}
for f_bar in range(1,7):
    for i in idx_list:
        for ind_name in ['ic_d','ic_c', 'ic_t']:
            # res_dic[(i,ind_name,f_bar)] = pool.apply_async(main_window_search,args=(i,ind_name,f_bar))
