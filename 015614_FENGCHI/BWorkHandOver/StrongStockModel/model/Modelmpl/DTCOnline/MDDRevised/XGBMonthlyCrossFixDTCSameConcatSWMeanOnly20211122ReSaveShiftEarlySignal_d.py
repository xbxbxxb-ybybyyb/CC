...
def get_dataset(train_idx, test_idx, fix_factor_lists,feature_addresses,label_path):
    ...
    if train_idx[-1] == test_idx[0] and train_idx[-1] == test_idx[-1]:
        train_feature, train_label = load_dataset_from_multiple_add_selfdefine_label(train_idx[0],
                                        get_pre_trade_date(train_idx[-1]), fix_factor_lists, feature_addresses,label_path=label_path)
    else:
        train_feature, train_label = load_dataset_from_multiple_add_selfdefine_label(train_idx[0],
                                        train_idx[-1], fix_factor_lists, feature_addresses,label_path=label_path)
    ...
    if train_idx[-1] == test_idx[0] and train_idx[-1] == test_idx[-1]:
        ...
        if test_idx[-1] >= today:
            test_feature, test_label = load_dataset_from_multiple_add_selfdefine_label(test_idx[0],
                                today, fix_factor_lists, feature_addresses,tail_no_future=True,label_path=label_path)
        else:
            test_feature, test_label = load_dataset_from_multiple_add_selfdefine_label(test_idx[0],
                                test_idx[-1], fix_factor_lists, feature_addresses,label_path=label_path)
...
def main_window_search(i, indicator,future_bar_num):
    ...
    out_file =f'/data/group/800442/800319/Faamonitor/PL/all_mkt_ts_norm/MonthlySelectedMDDImproveV2_20220125SWMean/Future_{future_bar_num}_bar/{target_file}/{target_file}.pkl'
...
for f_bar in range(1,7):
    for i in tqdm(idx_list):
        for ind_name in ['ic_d','ic_t','ic_c']:
            ...
            if now.strftime('%H%M') > '0610' and now.strftime('%H%M') < '0930' and get_recent_trade_date(int(now.strftime('%Y%m%d'))) == int(now.strftime('%Y%m%d')):
                # break
                pass
