...
def get_dataset(train_idx, test_idx, fix_factor_lists, feature_addresses):
    ...
    if train_idx[-1] == test_idx[0] and train_idx[-1] == test_idx[-1]:
        ...
        if test_idx[-1] >= today:
            test_feature, test_label = load_dataset_from_multiple_add(test_idx[0], get_pre_trade_date(today), fix_factor_lists, feature_addresses)
        else:
...
class XGBRegressionFactorEvalYearly(ModelNewLoading):
    ...
    def train_model(self, X_train, y_train, params, end_date=None):
        ...
        if 'load local model' in params and os.path.exists(params['model_load_path'] + '%d.json' % end_date):
            print('load from local', end_date)
            model = xgb.Booster(args_param)
            model.load_model(params['model_load_path'] + '%d.json' % end_date)
            model.set_param('predictor', 'cpu_predictor')
            print('load from local', params['model_load_path'] + '%d.json' % end_date)
            import shutil
            shutil.copy(params['model_load_path'] + '%d.json' % end_date,params['model_conf_path'] + '%d.json' % end_date)
            print('------------------')
            print((params['model_load_path'] + '%d.json' % end_date,params['model_conf_path'] + '%d.json' % end_date))
            print('--------------------')
            # return model
        else:
    ...
    def rolling_train_and_predict(self, params={}, period=10, predict_period=10, label_methodology='fix_window', label_param={}, factor_nums=200, kernel=10):
        ...
        for idx, cell_idx in bar:
            ...
            if 'load local model' in params and os.path.exists(params['model_load_path'] + '%d.json' % train_end_idx):
                print('exist model and load short version')
                X_train, y_train, X_test, y_test, feature_engineering_time = \
                get_dataset((get_pre_trade_date(train_end_idx,15), train_end_idx), (test_start_idx, test_end_idx),
...
def main_window_search(i, indicator):
    ...
    out_file = '/data/group/800442/800319/Faamonitor/PL/all_mkt_ts_norm/CrossFix/XGBV4FactorListMixCrossSWMeanOnlyNoShiftReSave20211122/XGBV4FactorListMixCrossSWMeanOnlyOldModelNewFeature20211123_%s_train%d_test%d_factor_num%d.pkl' % (
    ...
    best_param_clf_xgb['model_load_path'] = '/data/group/800442/800319/Faamonitor/PL/all_mkt_ts_norm/CrossFix/XGBV4FactorListMixCrossSWMeanOnlyShiftReSave20211122/XGBV4FactorListMixCrossSWMeanOnlyShiftReSave20211122_%s_train200_test10_factor_num400_model_conf/'%indicator
...
#
# i = int(AIMR.getParam())
# main_window_search(i,'ic_t')
# idx_list = list(range(134))[24:]
...
# cp -r /data/group/800442/800319/Faamonitor/PL/all_mkt_ts_norm/CrossFix/XGBV4FactorListMixCrossSWMeanOnly_ic_d_train200_test10_factor_num400_model_conf/ ./XGBV4FactorListMixCrossSWMeanOnlyOldModelNewFeature20211109_ic_d_train200_test10_factor_num400_model_conf/
# cp -r /data/group/800442/800319/Faamonitor/PL/all_mkt_ts_norm/CrossFix/XGBV4FactorListMixCrossSWMeanOnly_ic_t_train200_test10_factor_num400_model_conf/ ./XGBV4FactorListMixCrossSWMeanOnlyOldModelNewFeature20211109_ic_t_train200_test10_factor_num400_model_conf/
# cp -r /data/group/800442/800319/Faamonitor/PL/all_mkt_ts_norm/CrossFix/XGBV4FactorListMixCrossSWMeanOnly_ic_c_train200_test10_factor_num400_model_conf/ ./XGBV4FactorListMixCrossSWMeanOnlyOldModelNewFeature20211109_ic_c_train200_test10_factor_num400_model_conf/
