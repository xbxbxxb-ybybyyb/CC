...
def main_window_search(i, indicator, barra_factor):
    train_period = 200
    test_period = 10
    factor_num = 400
    factor_dir = f'/data/group/800442/800319/HFfactor/BarraCrosslize/{barra_factor}/data/'
    factor_list = set(os.listdir(factor_dir)) - set(['idx_date.npy', 'idx_time.npy', 'idx_code.npy', 'future.npy', 'nolimit.npy'])
    if len(factor_list) < 1059:
        print('factor not ready')
        return 'factor not ready'
    out_file = f'/data/group/800442/800319/Faamonitor/PL/all_mkt_ts_norm/CrossFix/Barra/XGBV4_{barra_factor}MeanOnly/' \
        f'XGBV4_{barra_factor}MeanOnly_{indicator}_train{train_period}_test{test_period}_factor_num{factor_num}.pkl'
...
idx_list = list(range(147))[24:][::-1]  # [:1]
barra_factor_list = sorted(os.listdir('/data/group/800442/800319/HFfactor/BarraCrosslize/'))
barra_factor_list.remove('MidCapitalization')
for fac_barra in barra_factor_list:
    for i in tqdm(idx_list):
        for ind_name in ['ic_d', 'ic_t', 'ic_c']:
            res = main_window_search(i, ind_name, fac_barra)
            gc.collect()
    send_message(['015664'], f'{fac_barra}XGBMatrix done')
