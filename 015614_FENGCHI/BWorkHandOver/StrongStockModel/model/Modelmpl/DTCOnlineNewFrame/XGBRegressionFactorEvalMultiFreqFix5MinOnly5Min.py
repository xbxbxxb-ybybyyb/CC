...
def get_5min_factor_evaluation(num,end_index,eval_indicator):
    eval_res = pd.read_pickle(f'/arch1/group/800442/800319/junkData/StrongStock/external_data/factor5min_eval/{eval_indicator}.pkl')
...
def fit_model(i,output_path,indicator_daily):
    ...
    fix_factor_list = []#get_fix_factor_evaluation(0,train_end,eval_indicator=indicator_fix)
    min5_factor_list = get_5min_factor_evaluation(400,train_end,eval_indicator=indicator_daily)
...
for idx in tqdm(idx_list):
    for daily_indicator in ['ic_half_d','ic_half_t','ic_half_c']:
        out_path = f'/arch1/group/800442/800319/Faamonitor/PL/all_mkt_ts_norm/Fix5minMulti/XGBFix5minOnly_train200_test10_{daily_indicator}/'
        fit_model(idx,out_path,daily_indicator)
        gc.collect()
