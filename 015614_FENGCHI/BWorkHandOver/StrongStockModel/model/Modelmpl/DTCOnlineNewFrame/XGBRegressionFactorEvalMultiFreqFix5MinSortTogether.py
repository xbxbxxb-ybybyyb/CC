...
conf.read('/arch1/group/800442/800319/strategy_local_path_offline/period_info.ini')
...
using_factor_list = pd.read_pickle('/arch1/group/800442/800319/junkData/StrongStock/external_data/available_factor_list.pkl')
...
def get_mix_factor_evaluation(num, end_index, fix_eval_indicator,min5_factor_inficator):
    factor_evaluation = pd.read_pickle(f'{root_path}external_data/moon_v2/{fix_eval_indicator}.pkl')
    ...
    if 'ret' in fix_eval_indicator:
        print('ret')
        factor_evaluation = factor_evaluation.loc[target_date].sort_values(ascending=False)
    elif 'ic' in fix_eval_indicator:
        print('ic')
        factor_evaluation = factor_evaluation.loc[target_date].apply(abs).sort_values(ascending=False)
    else:
        raise Exception('')

    eval_res = pd.read_pickle(f'/arch1/group/800442/800319/junkData/StrongStock/external_data/factor5min_eval/{min5_factor_inficator}.pkl')
    target_date = max(list(filter(lambda x: x < end_index, eval_res.index)))
    mix_res = pd.concat([factor_evaluation,eval_res.loc[target_date]]).sort_values(ascending=False)
    mix_factor_list = mix_res.index.tolist()[:num]
    fix_factor = sorted(list(set(mix_factor_list).intersection(set(factor_evaluation.index))))
    min5_factor = sorted(list(set(mix_factor_list).intersection(set(eval_res.columns))))

    return fix_factor,min5_factor
...
def fit_model(i,output_path,indicator_fix,indicator_daily):
    ...
    fix_factor_list,min5_factor_list = get_mix_factor_evaluation(400, train_end, fix_eval_indicator=indicator_fix,min5_factor_inficator=indicator_daily)
...
for idx in tqdm(idx_list):
    ...
    out_path = f'/arch1/group/800442/800319/Faamonitor/PL/all_mkt_ts_norm/Fix5minMulti/XGBMultiFreqFix5minSortTogether_train200_test10_{fix_indicator}_{daily_indicator}/'
...
# union_stk_list = set()
# for file_path in [
#     '/arch1/group/800442/800319/Faamonitor/PL/all_mkt_ts_norm/Fix5minMulti/XGBMultiFreqFix5min_train200_test10_ic_d_ic_half_d_factor_list/',
#     '/arch1/group/800442/800319/Faamonitor/PL/all_mkt_ts_norm/Fix5minMulti/XGBMultiFreqFix5min_train200_test10_ic_t_ic_half_t_factor_list/',
#     '/arch1/group/800442/800319/Faamonitor/PL/all_mkt_ts_norm/Fix5minMulti/XGBMultiFreqFix5min_train200_test10_ic_c_ic_half_c_factor_list/',
# ]:
