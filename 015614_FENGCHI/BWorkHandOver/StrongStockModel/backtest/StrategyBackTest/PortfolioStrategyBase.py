...
class PortfolioStrategyBase:
    ...
    def __init__(self, start=20140101, end=20181231, stock_pool=None, target_point=None,
                 buy_cost=0.001, sell_cost=0.001, per_amt=500000, append_param={}):
        ...
        if 'not close pickle' in append_param:
        ...
        pause = get_daily_1factor('pause', date_list=self.date_list, code_list=self.stk_list).fillna(False)
        flatten = pd.read_pickle('/data/group/800442/800319/junkData/IntraFactorModel/DataForTplusN/open_flatten.pkl').loc[self.date_list, self.stk_list]
        ...
        deal_price = deal_price.shift(1).swaplevel(0, 1).loc[self.trading_point].swaplevel(0, 1)
        ...
        self.re_initial(self.stock_pool.columns.tolist())
    ...
    def re_initial(self, stk_list=None):
        self.holding = {}
        self.available = {}
...
class EvaluationHelper:
    ...
    def evaluate_daily(self, holding_num, kernel=10):
        ...
        daily_info['benchmark'] = wind_a['S_DQ_CLOSE'].loc[daily_info.index]
        daily_info['benchmark_pct_change'] = wind_a['S_DQ_CLOSE'].pct_change().loc[daily_info.index]
        ...
        daily_info['持仓股票数'] = pd.Series(holding_num)
    ...
    def evaluate_by_signal(self, kernel=10):
        ...
        # pd.to_pickle(eval_signal_record,'/data/user/015664/AFuckingTrigger/限制买入和持仓/InitialCashBased/eval_singal_record.pkl')
        ...
        # eval_signal_record['收益率*cash_occupy'] = (eval_signal_record['收益率'] * eval_signal_record['cash_occupy'])
        eval_signal_record['平均每分钟收益率*cash_occupy'] = (eval_signal_record['平均每分钟收益率'] * eval_signal_record['cash_occupy'])
        eval_signal_record['胜*cash_occupy'] = eval_signal_record['胜率'] * eval_signal_record['cash_occupy']
        eval_signal_result['收益率'] = eval_signal_record['profit'].sum() / eval_signal_record['cash_occupy'].sum()
        eval_signal_result['平均每分钟收益率'] = eval_signal_record['平均每分钟收益率*cash_occupy'].sum() / eval_signal_record['cash_occupy'].sum()
        eval_signal_result['胜率'] = eval_signal_record['胜*cash_occupy'].sum() / eval_signal_record['cash_occupy'].sum()
        win_part = eval_signal_record[eval_signal_record['收益率'] > 0][['profit', 'cash_occupy']].sum()
        lose_part = eval_signal_record[eval_signal_record['收益率'] < 0][['profit', 'cash_occupy']].sum()
        eval_signal_result['盈利交易收益均值'] = win_part['profit'] / win_part['cash_occupy']
        eval_signal_result['亏损交易收益均值'] = lose_part['profit'] / lose_part['cash_occupy']  # eval_signal_record[eval_signal_record['收益率'] < 0]['收益率'].mean()
        eval_signal_result['盈亏比(收益率)'] = -1 * eval_signal_result['盈利交易收益均值'] / eval_signal_result['亏损交易收益均值']
        del win_part, lose_part
        win_part = eval_signal_record[eval_signal_record['平均每分钟收益率'] > 0][['平均每分钟收益率*cash_occupy', 'cash_occupy']].sum()
        lose_part = eval_signal_record[eval_signal_record['平均每分钟收益率'] < 0][['平均每分钟收益率*cash_occupy', 'cash_occupy']].sum()
        eval_signal_result['盈利交易平均每分钟收益均值'] = win_part['平均每分钟收益率*cash_occupy'] / win_part['cash_occupy']  # eval_signal_record[eval_signal_record['收益率'] > 0]['平均每分钟收益率'].mean()
        eval_signal_result['亏损交易平均每分钟收益均值'] = lose_part['平均每分钟收益率*cash_occupy'] / lose_part['cash_occupy']  # eval_signal_record[eval_signal_record['收益率'] < 0]['平均每分钟收益率'].mean()
        del win_part, lose_part
        ...
        yearly_temp_count = eval_signal_record[['profit', '平均每分钟收益率*cash_occupy', '胜*cash_occupy', 'cash_occupy', 'end_year']].groupby('end_year').sum()
        yearly_eval_signal_result = (yearly_temp_count.T / yearly_temp_count['cash_occupy']).T.rename(columns={'profit': '收益率',
                                                                                                               '平均每分钟收益率*cash_occupy': '平均每分钟收益率',
                                                                                                               '胜*cash_occupy': '胜率'}).drop('cash_occupy', axis=1)
        ...
        win_part = eval_signal_record[eval_signal_record['收益率'] > 0][['profit', '平均每分钟收益率*cash_occupy', 'cash_occupy', 'end_year']].groupby('end_year').sum()
        lose_part = eval_signal_record[eval_signal_record['收益率'] < 0][['profit', '平均每分钟收益率*cash_occupy', 'cash_occupy', 'end_year']].groupby('end_year').sum()
        win_part = (win_part.T / win_part['cash_occupy']).T.drop('cash_occupy', axis=1)
        lose_part = (lose_part.T / lose_part['cash_occupy']).T.drop('cash_occupy', axis=1)
        win_loss_rate = -1 * (win_part / lose_part)
        yearly_eval_signal_result = pd.concat([yearly_eval_signal_result,
                                               win_part.rename(columns={'profit': '盈利交易收益均值', '平均每分钟收益率*cash_occupy': '盈利交易平均每分钟收益均值'}),
                                               lose_part.rename(columns={'profit': '亏损交易收益均值', '平均每分钟收益率*cash_occupy': '亏损交易平均每分钟收益均值'}),
                                               win_loss_rate.rename(columns={'profit': '盈亏比(收益率)', '平均每分钟收益率*cash_occupy': '盈亏比(平均每分钟收益率)'})], axis=1)
    ...
    def one_wave_run(self, record, holding_num, kernel=10, output_path=None, signal_record_save=False):
        """
        一次运行回测和评估并输出
        ...
        self.evaluation_result['每日持仓统计'], self.evaluation_result['持仓综合统计'] = self.evaluate_daily(holding_num, kernel)
...
class InitailCashBasedEvaluationHelper:
    ...
    def evaluate_daily(self, daily_cash, kernel=10, holding_num=None):
        # record = self.record._getvalue()
        daily_result = {}
        e = time.time()
        stk_list = list(self.record.keys())
        ...
        def update(*param):
            ...
            if pbar.last_print_n == len(stk_list):
                pbar.close()
        # self.evaluate_stk_by_day(600702)
        self.pool_dict = Manager().dict()
        pool = Pool(kernel)
        for i in range(len(stk_list)):
            stk = stk_list[i]
        ...
        for k in daily_result:
            try:
                ...
                daily_result[k] = self.evaluate_stk_by_day(k)
        res_pn = pd.Panel(daily_result)
        # pd.to_pickle([res_pn, daily_cash], '/data/user/015664/AFuckingTrigger/限制买入和持仓/NoFutureInfoResShift/record/XGB_Light_daily_res_pn_0105_0127OnlineLimit.pkl')
        # pd.to_pickle([res_pn,daily_cash], '/data/user/015664/AFuckingTrigger/限制买入和持仓/InitialCashBased/XGB_daily_res_pn.pkl')
        # res_pn,_ = pd.read_pickle('/data/user/015664/AFuckingTrigger/限制买入和持仓/InitialCashBased/XGB_daily_res_pn.pkl')
        if 0 in res_pn.shape:
            return pd.DataFrame(), pd.DataFrame(),res_pn
        daily_info = res_pn.sum(axis='items')
        daily_cash.index = daily_cash.index.astype(int)
        ####取出买入操作次数，卖出操作次数矩阵
        daily_info['收盘账户市值'] = daily_info['收盘持仓市值'].fillna(method='pad') + daily_cash
        daily_info['账户资金'] = daily_cash
        daily_info['买入股票数'] = res_pn.loc[:, :, '买入操作次数'].replace(0, np.nan).count(axis=1)
        daily_info['卖出股票数'] = res_pn.loc[:, :, '卖出操作次数'].replace(0, np.nan).count(axis=1)
        # check = res_pn.loc[:, :, '卖出操作次数'].replace(0, np.nan).loc[20160205]   recor_797 = self.record[797]
        daily_info['当日收益'] = daily_info['收盘持仓市值'].diff() - daily_info['扣费后净买入']
        daily_info['当日收益率'] = daily_info['收盘账户市值'].pct_change()
        daily_info['择时收益率'] = daily_info['择时收益'] / daily_info['收盘账户市值']
        initial_cash = None
        if len(daily_info) > 0:
            daily_info.at[daily_info.index[0], '当日收益'] = daily_info['收盘持仓市值'].tolist()[0] - daily_info['扣费后净买入'].tolist()[0]
            initial_cash = daily_info['收盘账户市值'].tolist()[0] - daily_info.at[daily_info.index[0], '当日收益']
            daily_info.at[daily_info.index[0], '当日收益率'] = daily_info.at[daily_info.index[0], '当日收益'] / initial_cash
            daily_info.at[daily_info.index[0], '择时收益率'] = daily_info.at[daily_info.index[0], '择时收益'] / initial_cash
        daily_info = pd.concat([daily_info.drop(['择时收益', '择时收益率'], axis=1), daily_info[['择时收益', '择时收益率']]], axis=1)
        daily_info['累积收益'] = daily_info['当日收益'].fillna(0).cumsum()
        daily_info['账户净值'] = daily_info['收盘账户市值'] / initial_cash
        wind_a = s.get_factor_value('WIND_AIndexWindIndustriesEOD', S_INFO_WINDCODE=['881001.WI'], trade_dt=[f'>={min(daily_info.index.tolist())}',f'<={max(daily_info.index.tolist())}'])[
            ['TRADE_DT', 'S_DQ_CLOSE']].set_index('TRADE_DT')
        wind_a.index = wind_a.index.astype(int)
        daily_info['benchmark'] = wind_a
        daily_info['benchmark_pct_change'] = daily_info['benchmark'].pct_change()
        daily_info['基准收益比'] = daily_info['当日收益率'] / daily_info['benchmark_pct_change']

        daily_info['year'] = [x // 10000 for x in daily_info.index.tolist()]
        # daily_info = daily_info[daily_info['账户净值'].fillna(0) != 0]
        yearly_stat = {}

        # daily_info = daily_info.drop(date_list[-1], axis=0)
        for year in sorted(list(set(daily_info['year']))) + ['全时段']:
            if year == '全时段':
                temp_info = daily_info
            else:
            ...
            if len(temp_info) == 0:
                continue
            date_list = get_date_range(int(temp_info.index[0]) // 10000 * 10000 + 101, int(temp_info.index[-1]) // 10000 * 10000 + 1231)
            temp_stat = pd.Series()
            temp_stat['相对基准收益收益比(涨)'] = temp_info[temp_info['benchmark_pct_change'] > 0]['当日收益率'].mean() / temp_info[temp_info['benchmark_pct_change'] > 0][
                'benchmark_pct_change'].mean()
            temp_stat['相对基准收益收益比(跌)'] = temp_info[temp_info['benchmark_pct_change'] < 0]['当日收益率'].mean() / temp_info[temp_info['benchmark_pct_change'] < 0][
                'benchmark_pct_change'].mean()
            temp_stat['相对基准收益盈亏比'] = temp_stat['相对基准收益收益比(涨)'] / temp_stat['相对基准收益收益比(跌)']
            temp_stat['根据大盘涨跌计算盈亏比'] = -1 * temp_info[temp_info['benchmark_pct_change'] > 0]['当日收益率'].mean() / temp_info[temp_info['benchmark_pct_change'] < 0]['当日收益率'].mean()
            temp_stat['根据大盘涨跌计算账面盈亏比'] = -1 * temp_info[temp_info['benchmark_pct_change'] > 0]['当日收益'].mean() / temp_info[temp_info['benchmark_pct_change'] < 0]['当日收益'].mean()
            temp_stat['收益率盈亏比'] = -1 * temp_info[temp_info['当日收益率'] > 0]['当日收益率'].mean() / temp_info[temp_info['当日收益率'] < 0]['当日收益率'].mean()
            temp_stat['日胜率'] = (temp_info['当日收益率'] > 0).sum() / ((temp_info['当日收益率'] > 0).sum() + (temp_info['当日收益率'] < 0).sum())
            temp_stat['账面收益盈亏比'] = -1 * temp_info[temp_info['当日收益'] > 0]['当日收益'].mean() / temp_info[temp_info['当日收益'] < 0]['当日收益'].mean()
            temp_stat['收益率均值'], temp_stat['收益率波动'] = temp_info['当日收益率'].mean(), temp_info['当日收益率'].std()
            temp_stat['年化收益'] = (temp_info['账户净值'].tolist()[-1] / temp_info['账户净值'].tolist()[0]) ** (244. / len(date_list)) - 1
            temp_stat['年化夏普'] = (244 ** 0.5) * temp_stat['收益率均值'] / temp_stat['收益率波动']
            temp_stat['净值最大回撤'] = (1 - temp_info['账户净值'] / temp_info['账户净值'].cummax()).max()
            temp_stat['累计账面最大亏损'] = (temp_info['累积收益'].cummax() - temp_info['累积收益']).max()
            yearly_stat[year] = temp_stat
        yearly_stat = pd.DataFrame(yearly_stat)
        date_list = get_date_range(daily_info.index[0], daily_info.index[-1])
        ...
        daily_info = daily_info.reindex(date_list)
        daily_info['账户净值'] = daily_info['账户净值'].fillna(method='pad')
        daily_info['收盘账户市值'] = daily_info['收盘账户市值'].fillna(method='pad')
        if not holding_num is None:
            daily_info['持仓股票数'] = holding_num
        daily_info = daily_info.fillna(0)
        daily_info.index = daily_info.index.astype(str)
        daily_info = daily_info.drop(['benchmark', 'benchmark_pct_change', '基准收益比', 'year'], axis=1)
        return daily_info, yearly_stat, res_pn
    ...
    def one_wave_run(self, record, daily_cash, kernel=10, output_path=None, signal_record_save=False, holding_num=None):
        """
        一次运行回测和评估并输出
        :param stk_list: 股票列表 [int,int,...]
        :param start:int
        ...
        for k in record:
            self.record[k] = record[k]
        self.evaluation_result['每日持仓统计'], self.evaluation_result['持仓综合统计'], res_pn = self.evaluate_daily(daily_cash, kernel, holding_num=holding_num)
        self.evaluation_result['逐笔持仓统计'], self.evaluation_result['逐笔持仓综合统计'] = self.evaluate_by_signal(kernel)
        ...
        if not output_path is None:
            with pd.ExcelWriter(output_path) as writer:
                for each in self.evaluation_result:
                    ...
                    self.evaluation_result[each].to_excel(writer, each)
            writer.close()
        return self.evaluation_result, res_pn
