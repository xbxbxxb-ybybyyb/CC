...
from StrongStockModel.dataApi.getData import get_daily_1factor,get_minute_pickle
# from StrongStockModel.dataApi.getData import get_minute_1factor, get_daily_1factor,get_minute_pickle
from StrongStockModel.conf.path_config import deal_price_path, root_path
from dataApi.usefulTools import delay
from dataApi.sendInfo import send_message
import itertools,datetime,traceback
...
def get_recent_vol_info(start,end, rolling_window, bar_list, order_keep_min=30, delay_min=1):
    ...
    vol = get_core(get_minute_pickle('volume', date_list=date_list, code_list=code_list)) / _adjfactor
    ...
    close = get_core(get_minute_pickle('close',date_list=date_list,code_list=code_list))
...
try:
    ...
    if stock_pool.index[-1]!=date:
       lm.sendMessage('vol adj 股票池截止日期不是当日')
       raise Exception('vol adj 股票池截止日期不是当日')
    vol_info = get_recent_vol_info(stock_pool.index[0],stock_pool.index[-1], 5, [930,1000,1030,1100,1300,1330,1400,1430])
    pd.to_pickle(vol_info,deal_price_path + 'vol_rolling_future_30min_sum_5day_mean.pkl')
    lm.sendMessage(f'{date}vol adj 更新成功%d'%stock_pool.index[-1])
    send_message(['015664'], f'{date}vol adj 更新成功%d'%stock_pool.index[-1])
except:
    lm.sendMessage(f"{date}vol adj 更新失败！！！！！！！！！！！！！！！")
    send_message(['015664'], f"{date}vol adj 更新失败！！！！！！！！！！！！！！！")
