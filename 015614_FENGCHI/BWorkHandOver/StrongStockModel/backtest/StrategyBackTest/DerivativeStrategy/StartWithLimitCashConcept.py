...
class StartWithLimitCashConcept(PortfolioStrategyBase):
    def __init__(self, signal, start=20140101, end=20181231, stock_pool=None, target_point=None,
                 buy_cost=0.001, sell_cost=0.001, per_amt_ratio=0.0025, append_param={}, initial_cash=200000000,barly_max_buy=100,concept=None):
        ...
        if concept is None:
            self.active_stock = pd.DataFrame(True,index=self.date_list,columns=self.stk_list)
        else:
            self.active_stock = concept
    ...
    def daily_update(self, idx, date):
        ...
        print(len(self.daily_active_stk))
