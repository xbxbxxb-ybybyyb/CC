...
"""
down_percent:下跌股票数占满仓股票数比例
signal_percent:买入信号数
"""
...
class StartWithLimitCashVolConsider(PortfolioStrategyBase):
    def __init__(self, signal, start=20140101, end=20181231, stock_pool=None, target_point=None,
                 ...
                 deal_percent=0.1, stk_min_amt=None,down_threshold=0.3,signal_threshold=0.3):
    ...
    def bar_handler(self):
        ...
        down_ratio = (ret<0).sum()/pre_close.count()
        if down_ratio>self.down_threshold and signal_count>self.signal_threshold/self.per_amt_ratio:
            signal = pd.Series()
