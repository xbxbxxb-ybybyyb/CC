...
class StartWithLimitCashVolConsider(PortfolioStrategyBase):
    ...
    def bar_handler(self):
        ...
        if date==20220224 and time_point==1330:
            print(1)
        ...
        for each in self.bench_ret:
            ...
            bar_basic_indicator[f'{each}_ret'] = self.data_flow[f'{each}_ret'][(date, time_point)] #= self.bench_ret.iloc[self.date_idx * self.step:(self.date_idx + 1) * self.step]
            ...
            bar_basic_indicator[f'{each}_MA5_to_MA10'] = self.MA5_minus_MA10[each].iloc[[date_idx]][date] #= self.bench_ret.iloc[self.date_idx * self.step:(self.date_idx + 1) * self.step]
