...
class StartWithLimitCashVolConsider(PortfolioStrategyBase):
    def __init__(self, signal, start=20140101, end=20181231, stock_pool=None, target_point=None,
        ...
        bench_ret = bench_arr/delay(bench_arr[:,:,None],1)[:,[-1],0]-1
    ...
    def bar_handler(self):
        ...
        eval_all_day_signal_ratio = eval_all_day_signal_count/self.da
        ...
        if down_signal_ratio<self.index_pct_change and eval_all_day_signal_count>self.signal_threshold*len(self.today_pool):
            signal = pd.Series()
