...
class StartWithLimitCashVolConsider(PortfolioStrategyBase):
    ...
    def bar_handler(self):
        ...
        if self.cash < self.per_amt:
            not_buy = True
            for stk in sell_stk:
                sell_vol = self.holding[stk]#min(historical_future_vol[stk], self.holding[stk])
            ...
            orderable_vol = target_vol.copy()#pd.concat([target_vol, historical_future_vol[list(trigger_stk)]], axis=1).min(axis=1)
    ...
    def run_backtest(self, kernel=10):
        ...
        for date_idx, date in enumerate(bar):
            ...
            for time_idx, time_point in enumerate(self.trading_point):
                temp_deal_price = self.data_flow['deal_price'][time_idx:time_idx + 1].T
                if (date,time_point) not in temp_deal_price.columns.tolist():
                    raise Exception(f'{time_idx}:{time_idx + 1} col is {temp_deal_price.columns.tolist()}, no {(date,time_point)}')
                self.bar_dealprice = temp_deal_price[(date, time_point)]
