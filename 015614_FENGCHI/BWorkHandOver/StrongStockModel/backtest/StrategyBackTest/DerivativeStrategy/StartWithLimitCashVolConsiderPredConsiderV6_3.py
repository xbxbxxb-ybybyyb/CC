...
class StartWithLimitCashVolConsider(PortfolioStrategyBase):
    def __init__(self, signal, start=20140101, end=20181231, stock_pool=None, target_point=None,
        ...
        bench_close = get_minute_1factor('close',type='bench',start_datetime=get_pre_trade_date(start),end_datetime=end,base_date=20100101).fillna(method='pad')
        bench_arr = bench_close['ZZ500'].values.reshape(bench_close.shape[0]//242,242)
        move_mean = []
        for i in range(1,6):
            move_mean.append(delay(bench_arr[:,[-1],None],i))
        move_mean = np.concatenate(move_mean,axis=-1)
        move_mean = np.nanmean(move_mean,axis=-1)
        bench_ret = bench_arr/move_mean - 1
    ...
    def bar_handler(self):
        ...
        signal_condition = eval_all_day_signal_count2>self.signal_threshold*len(self.today_pool)
