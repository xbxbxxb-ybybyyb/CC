...
class StartWithLimitCashVolConsider(PortfolioStrategyBase):
    def __init__(self, signal,inter_signal, start=20140101, end=20181231, stock_pool=None, target_point=None,
        ...
        self.signal = {i:signal[i].reindex(self.close.index) for i in signal}
    ...
    def buy_action(self, stk, vol=None):
        ...
        if deal_vol > 0:
            if not np.isnan(deal_price):
                self.left_holding_bars[stk] = len(self.trading_point) - self.datetime[-1]
                self.cash -= deal_vol * deal_price * (1 + self.buy_cost)
            else:
                raise Exception('Unexpected')
        return deal_vol

    def holding_another_round(self, stk,bar_nums):
        self.left_holding_bars[stk]+= len(self.trading_point) - self.datetime[-1]
    ...
    def holding_more_bars(self,trigger_stk=set()):
        ...
        for temp_window in range(2, len(self.trading_point) - time_idx):
