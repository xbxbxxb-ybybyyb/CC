...
class StartWithLimitCashVolConsiderPreSold(PortfolioStrategyBase):
    def __init__(self, signal,sold_signal, start=20140101, end=20181231, stock_pool=None, target_point=None,
        ...
        self.sold_signal = sold_signal.reindex(self.close.index)
    ...
    def daily_update(self, idx, date):
        ...
        self.data_flow['sold_signal'] = self.sold_signal[self.date_idx * self.step:(self.date_idx + 1) * self.step]
        ...
        if self.data_flow['sold_signal'].index[0][0] != self.date or self.data_flow['sold_signal'].index[-1][0] != self.date:
            raise Exception('Broadcast date and signal date are not match!')
    ...
    def bar_handler(self):
        ...
        sold_signal = self.data_flow['sold_signal'][time_idx:time_idx + 1].T[(date, time_point)]
        ...
        unsoldable_stk = set()
        for stk in sell_stk:
            if self.soldable_judge(stk) or sold_signal[stk]:
                pass
            else:
                unsoldable_stk.add(stk)
        sell_stk = list(set(sell_stk) - unsoldable_stk)
