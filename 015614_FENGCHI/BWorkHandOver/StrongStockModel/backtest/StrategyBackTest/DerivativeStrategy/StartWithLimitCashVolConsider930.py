...
class StartWithLimitCashVolConsider930(PortfolioStrategyBase):
    def __init__(self, signal, start=20140101, end=20181231, stock_pool=None, target_point=None,
        ...
        self.past_5day_future_30min_vol = pd.read_pickle(deal_price_path + 'vol_rolling_future_30min_sum_5day_mean.pkl')
        self.past_5day_future_30min_vol = self.past_5day_future_30min_vol.shift(len(self.past_5day_future_30min_vol.index.levels[1])).reindex(self.close.index)
        ...
        self.order_info = {}
        self.pre_dail_info['close'] = get_daily_1factor('close',
                                                        date_list=get_date_range(get_pre_trade_date(self.date_list[0]),self.date_list[-1]),
                                                        code_list=self.stk_list).shift(1)[1:]
    ...
    def bar_handler(self):
        ...
        # 盘中涨跌停
        bar_close = self.pre_dail_info['close'][self.date_idx:self.date_idx + 1].T[date]#self.data_flow['close'][time_idx:time_idx + 1].T[(date, time_point)]
