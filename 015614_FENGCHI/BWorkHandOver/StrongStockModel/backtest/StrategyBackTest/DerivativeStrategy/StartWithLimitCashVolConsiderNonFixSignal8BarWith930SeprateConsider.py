...
import bottleneck
from StrongStockModel.backtest.StrategyBackTest.ExtraConditionTool import calc_recent_count,delay,get_barly_trigger
...
class StartWithLimitCashVolConsider(PortfolioStrategyBase):
    def __init__(self, long_signal, short_signal, start=20140101, end=20181231, stock_pool=None, target_point=None,
        ...
        self.bar_signal_ratio_ratio, _ = calc_recent_count(np.isfinite(long_signal[8]),bars=8)
        signal_count = {i: long_signal[i].count(axis=1) for i in long_signal}
        signal_count = pd.DataFrame(signal_count)
        signal_count_arr = signal_count.sort_index(axis=1).values.reshape(signal_count.shape[0] // 8, 8, signal_count.shape[1])
    ...
    def buy_action(self, stk, vol=None):
        ...
        if deal_vol > 0:
            if not np.isnan(deal_price):
                self.left_holding_bars[stk] = min(len(self.trading_point) - self.datetime[-1] +1,8)
    ...
    def holding_more_bars(self,trigger_stk=set()):
        ...
        for stk in holding_one_more_round:
            self.left_holding_bars[stk] = min(len(self.trading_point) - time_idx +1,8)
    ...
    def bar_handler(self):
        ...
        if date==20220124 and time_point==1000:
            print(1)
        future_window = min(len(self.trading_point) - time_idx +1,8)
        ...
        future_8_singal = set(self.data_flow['signal'][8][time_idx:time_idx + 1].T[(date, time_point)].dropna().index)
        pool_signal = future_8_singal.intersection(set(self.today_pool))  # trigger_stk.intersection(set(self.today_pool))
        ...
        # for each in bar_basic_indicator.keys():
        #     exec(f'{each}={bar_basic_indicator[each]}'.replace('=nan', '=np.nan'))
        #     print(f'{each}={bar_basic_indicator[each]}'.replace('=nan', '=np.nan'))
    ...
    def run_backtest(self, kernel=10):
        ...
        for date_idx, date in enumerate(bar):
            ...
            if 930 not in self.trading_point:
