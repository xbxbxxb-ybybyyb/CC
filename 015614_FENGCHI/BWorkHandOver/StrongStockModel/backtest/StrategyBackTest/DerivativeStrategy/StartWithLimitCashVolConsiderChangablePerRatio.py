...
class StartWithLimitCashVolConsiderChangablePerRatio(PortfolioStrategyBase):
    def __init__(self, signal, start=20140101, end=20181231, stock_pool=None, target_point=None,
                 ...
                 deal_percent=0.1, stk_min_amt=None, cash_added={}, per_ratio_change={}, max_trigger_num={}):
        per_amt = initial_cash * per_amt_ratio // 10000 * 10000
        ...
        self.max_trigger_num = max_trigger_num
    ...
    def run_backtest(self, kernel=10):
        ...
        for date_idx, date in enumerate(bar):
            ...
            if date in self.max_trigger_num:
                self.barly_max_buy = self.max_trigger_num[date]
            self.holding_series[date] = self.holding.copy()
            self.last_buy_time_series[date] = self.last_buy_time.copy()
...
signal, pred_ret = pd.read_pickle('/data/user/015664/AFuckingTrigger/限制买入和持仓/信号存储/signal_XGBMonthly_Cat_LightWithoutMax5_0.05.pkl')
pred_ret[~signal] = np.nan
alpha_pool = pd.read_pickle('/data/group/800319/AlphaPool/CS_XGB|OLS_condition_style_rank_ex20_F600T488P1_future925t241h123d_uniform20t50.pkl').shift(1).loc[start:end].rank(ascending=False, axis=1) < 600
alpha_pool = alpha_pool.loc[start:end]
...
instance = StartWithLimitCashVolConsiderChangablePerRatio(pred_ret, start, end, stock_pool=alpha_pool, target_point=bar_list, buy_cost=cost, sell_cost=cost,
                                                          ...
                                                          deal_percent=deal_ratio, initial_cash=initial_cash, per_ratio_change=per_ratio_change)
...
out_path = './test.xlsx'#define your file .xlsx
