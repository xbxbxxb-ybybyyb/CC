...
class StartWithLimitCashVolConsider(PortfolioStrategyBase):
    def __init__(self, signal, start=20140101, end=20181231, stock_pool=None, target_point=None,
                 ...
                 deal_percent=0.1, stk_min_amt=None, index_pct_change=0.3, signal_threshold=0.3, down_swing_threshold=0):
        ...
        self.index_pct_change= index_pct_change
        self.signal_threshold = signal_threshold
        self.down_swing_threshold = down_swing_threshold
        self.daily_trigger_num = []
        ...
        bench_arr = bench_close['SZZZ'].values.reshape(bench_close.shape[0]//242,242)
        ...
        self.MA5_minus_MA10 = MA5_minus_MA10.loc[self.date_list]
        self.bench_ret = pd.Series(bench_ret.flatten(),index=bench_close.index).shift(1).loc[self.close.index]
        self.bench = pd.Series(bench_arr.flatten(),index=bench_close.index).shift(1).loc[self.close.index]
        self.MA5 = pd.Series(move_mean5[:,0],index = bench_close.index.levels[0]).loc[self.date_list]
    ...
    def daily_update(self, idx, date):
        ...
        self.data_flow['barly_index_ret'] = self.bench_ret.iloc[self.date_idx * self.step:(self.date_idx + 1) * self.step]
        self.data_flow['barly_index_close'] = self.bench.iloc[self.date_idx * self.step:(self.date_idx + 1) * self.step]
    ...
    def bar_handler(self):
        ...
        down_signal_ratio = self.data_flow['barly_index_ret'][(date,time_point)]#len(down_signal)/len(pool_signal) if len(pool_signal) else 0
        ...
        down_condition = down_signal_ratio<self.index_pct_change or self.MA5_minus_MA10.iloc[[date_idx]][date]<0
        if  signal_condition and down_condition:
        ...
        self.barly_condition_indicator[(date, time_point)] = {
            ...
            'bar_trigger_signal': len(pool_signal),
            ...
            'SZZS_pct':self.data_flow['barly_index_ret'][(date,time_point)],
            'SZZS':self.data_flow['barly_index_close'][(date,time_point)],
            'MA5_minus_MA10':self.MA5_minus_MA10.iloc[[date_idx]][date],
            'MA5':self.MA5.iloc[[date_idx]][date],

            'terminal_flag': down_condition and signal_condition,
        }
    ...
    def run_backtest(self, kernel=10):
        ...
        self.past_10_day_avg_trigger_num = np.nan
        for date_idx, date in enumerate(bar):
            ...
            for time_idx, time_point in enumerate(self.trading_point):
                ...
                self.bar_handler()
                # print(self.datetime)
            self.daily_trigger_num.append(len(self.already_triggered))
            self.past_10_day_avg_trigger_num = pd.Series(self.daily_trigger_num[-10:]).mean()
