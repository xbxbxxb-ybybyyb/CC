...
base_path = '/data/user/015664/AFuckingTrigger/限制买入和持仓/NonFixWindowTo2111/'
all_file = f'{base_path}/非固定持仓_3模型残差_20220316.xlsx'
# file_list = list(filter(lambda x: x.startswith('XGB') and 'Out' not in x, os.listdir(base_path)))
file_list = sorted(list(filter(lambda x: x.endswith('xlsx') and 'Res' in x , os.listdir(base_path))))
file_list2 = sorted(list(filter(lambda x: x.endswith('xlsx') and 'Origin' in x and 'XGB_DTC' in x and 'Matrix' not in x, os.listdir(base_path))))
file_list = sorted(file_list+file_list2)
