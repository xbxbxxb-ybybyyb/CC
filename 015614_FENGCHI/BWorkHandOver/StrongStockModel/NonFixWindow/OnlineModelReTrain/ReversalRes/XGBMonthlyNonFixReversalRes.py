...
class XGBRegressionFactorEvalYearly(ModelNewLoading):
    ...
    def get_fix_factor_evaluation(self, num, end_index):
        ...
        factor_evaluation = pd.read_pickle(f'{root_path}external_data/FutureBarBy30Min_8barReversalRes/Future_{self.future_bar_num}_bar/{self.eval_indicator}.pkl')
...
def main_window_search(i, indicator, future_bar_num, label_path):
    ...
    out_file = f'/data/group/800442/800319/Faamonitor/PL/all_mkt_ts_norm/MonthlySelectedMDDImprove8BarReversalRes/Future_{future_bar_num}_bar/{target_file}/{target_file}.pkl'
    ...
    model = XGBRegressionFactorEvalYearly(train_start, test_end, None, feature_address='/data/group/800442/800319/HFfactor/RealTimeFixRollRobustReversalResRct1Day8Bar/data/',
    ...
    for each in ['idx_date.npy', 'idx_time.npy', 'idx_code.npy', 'future.npy', 'nolimit.npy']:
        if not os.path.exists(f'/data/group/800442/800319/HFfactor/RealTimeFixRollRobustReversalResRct1Day8Bar/data/{each}'):
            shutil.copy(f'/data/group/800442/800319/HFfactor/RealTimeFixRollRobust/data/{each}',
                        f'/data/group/800442/800319/HFfactor/RealTimeFixRollRobustReversalResRct1Day8Bar/data/{each}')
    ...
    for f_bar in tqdm(list(range(1, 9))):
        for ind_name in ['ic_d']:
            try:
                ...
                send_message(['015664'], f'{ind_name, f_bar, i} fail')
