...
def calc_back_test_record(pct_threshold, per_amt_ratio, initial_cash, pool_num, deal_ratio, FIX_tag, TAG_930, alpha_pool_tag):
    ...
    if True:#not os.path.exists('/data/user/015664/AFuckingTrigger/限制买入和持仓/Upgrade/record/record_%sVolConsider_UpBuy100_%dbp_cost.pkl' % (tag, int(10000 * cost))):
        ...
        pd.to_pickle([record, cash_series, holding_num],
                     f'{base_dir}/record/record_%sVolConsider_UpBuy100_%dbp_cost.pkl' % (tag, int(10000 * cost)))
...
calc_back_test_record(*(each + (0.1,'XGB_DTC_Matrix_Light_Cat',None, 'CS_XGB_OLS_condition_style_rank_ex20')))
