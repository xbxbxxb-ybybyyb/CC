...
from StrongStockModel.backtest.StrategyBackTest.DerivativeStrategy.StartWithLimitCashVolConsiderNonFixSignal8BarExtraContition8WindowSignal import StartWithLimitCashVolConsider, InitailCashBasedEvaluationHelper
import pandas as pd
...
condition = {
    ...
    'v6_2_1':'(((bar_first_trigger_num/bar_ratio)>(0.15*pool_num)) or ((bar_cum_first_trigger_num/barly_cum_ratio)>(0.15*pool_num)))and ((SZZZ/SZZZ_MA5 -1)<-0.008 or SZZZ_MA5_to_MA10<0)',
}


def calc_back_test_record(per_amt_ratio, initial_cash, pool_num, deal_ratio, tag, alpha_pool_tag,signal_threshold,down_definition,STAGE):
    start = 20170101
    end = 20211231

    condition_series = {
        get_pre_trade_date(start): f'(((bar_first_trigger_num/bar_ratio)>({signal_threshold}*pool_num)) or '
        f'((bar_cum_first_trigger_num/barly_cum_ratio)>({signal_threshold}*pool_num)))and '
        f'((SZZZ/SZZZ_MA5 -1)<{down_definition} or SZZZ_MA5_to_MA10<0)'

    }
    if signal_threshold == 1 and down_definition == -1:
        condition_series = {
            get_pre_trade_date(start): f'False'

        }
    tag = f'{tag}_{alpha_pool_tag}_{signal_threshold:.2f}_{down_definition:.3f}_NonFixSignal1000Base8Bar'
    print(tag)
    stk_min_amt = min(round(initial_cash * per_amt_ratio, -5) * 0.2, 1000000)
    file_name = '%s_%dbp_cost' % (tag, int(10000 * cost))
    base_dir = '/data/user/015664/AFuckingTrigger/限制买入和持仓/NonFixWindowConditionV6_2_3_8WindowSignalRev20220406/'
    ...
    if not os.path.exists(record_file) and STAGE==0:
        ...
        send_message(['015664'],f'{signal_threshold,down_definition} no record')
        return
    else:
        print(f'{signal_threshold,down_definition} record exist in stage 0')
        return
    if STAGE==0:
        return
...
# d_definition,s_threshold = para_list[10]#eval(aimr_multitimes.getParam())#0.15,0.5,0,0
# calc_back_test_record(0.005, 2e8, 600, 0.1, 'XGB_DTC_Matrix_Light_CatNonFixWindow', 'CS_XGB_OLS_condition_style_rank_ex20',
#                       s_threshold, d_definition, 0)
# calc_back_test_record(0.005, 2e8, 600, 0.1, 'XGB_DTC_Matrix_Light_CatNonFixWindow', 'CS_XGB_OLS_condition_style_rank_ex20',
#                       s_threshold, d_definition, 1)
# d_definition, s_threshold = para_list[10]  # eval(aimr_multitimes.getParam())#0.15,0.5,0,0
# calc_back_test_record(0.005, 2e8, 600, 0.1, 'XGB_DTC_Matrix_Light_CatNonFixWindow', 'CS_XGB_OLS_condition_style_rank_ex20',
#                       s_threshold, d_definition, 0)
# calc_back_test_record(0.005, 2e8, 600, 0.1, 'XGB_DTC_Matrix_Light_CatNonFixWindow', 'CS_XGB_OLS_condition_style_rank_ex20',
...
if get_forbidden_tag():
    ...
    d_definition,s_threshold,stage = eval(aimr_multitimes.getParam())#0.15,0.5,0,0

    calc_back_test_record(0.005, 2e8, 600,0.1, 'XGB_DTC_Matrix_Light_CatNonFixWindow', 'CS_XGB_OLS_condition_style_rank_ex20',
                          s_threshold,d_definition,stage)
