...
def calc_back_test_record(per_amt_ratio, initial_cash, pool_num, deal_ratio, tag, alpha_pool_tag,down_signal_ratio,signal_threshold,down_definition):
    start = 20170101
    end = 20211231
    ...
    if True:#not os.path.exists(f'/data/user/015664/AFuckingTrigger/限制买入和持仓/NonFixWindow/record_{file_name}.pkl'):
        ...
        pd.to_pickle([record,cash_series,holding_num],f'/data/user/015664/AFuckingTrigger/限制买入和持仓/NonFixWindowCondition/record_{file_name}.pkl')
    else:
        print('record exist')
    ...
    out_path = f'/data/user/015664/AFuckingTrigger/限制买入和持仓/NonFixWindowCondition/{file_name}.xlsx'
    for each in record:
...
d_signal_ratio,s_threshold,d_definition = 0.15,0.5,0
calc_back_test_record(0.005, 2e8, 600,0.1, 'XGB_DTC_Matrix_Light_CatNonFixWindow_8BarOriginFactor', 'CS_XGB_OLS_condition_style_rank_ex20',
                      d_signal_ratio,s_threshold,d_definition)
