...
from StrongStockModel.backtest.StrategyBackTest.DerivativeStrategy.StartWithLimitCashVolConsiderNonFixSignal8BarExtraContition import StartWithLimitCashVolConsider, \
...
def calc_back_test_record(pct, per_amt_ratio, initial_cash, pool_num, deal_ratio, model_tag, alpha_pool_tag, down_signal_ratio, signal_threshold, down_definition, STAGE):
    ...
    tag = f'{model_tag}_{pct:.3f}_per{per_amt_ratio:.3f}_{alpha_pool_tag}_{down_signal_ratio:.2f}_{signal_threshold:.2f}_{down_definition:.3f}_Cash_1e{initial_cash}'
    file_name = '%s_%dbp_cost' % (tag, int(10000 * cost))
    base_dir = '/data/user/015664/AFuckingTrigger/限制买入和持仓/NonFixWindowConditionPctV4_2_1CashThreshold/'
    ...
    pd.DataFrame().to_excel(out_path)
    os.remove(out_path)
...
para_list = [(5e8, 0.005, 0.5, 0.15, 0, 0.03),
             (3e8, 0.01, 0.5, 0.15, 0, 0.03),
             (2e8, 0.005, 0.5, 0.15, 0, 0.05),
             (3e8, 0.005, 0.5, 0.15, 0, 0.03),
             (5e8, 0.01, 0.5, 0.15, 0, 0.03),
             ]
# i_cash,per_s_ratio,d_signal_ratio,s_threshold,d_definition,pct_thre =(5e8,0.005,0.5,0.15,0,0.03)
# i_cash,per_s_ratio,d_signal_ratio,s_threshold,d_definition,pct_thre =(3e8,0.01,0.5,0.15,0,0.03)

# if get_forbidden_tag():
#     pass
# else:
i_cash, per_s_ratio, d_signal_ratio, s_threshold, d_definition, pct_thre = para_list[-1]  # eval(aimr_multitimes.getParam())#0.15,0.5,0,0
calc_back_test_record(pct_thre, per_s_ratio, i_cash, 600, 0.1, 'XGB_DTC_Matrix_Light_Cat', 'CS_XGB_OLS_condition_style_rank_ex20',
