...
from StrongStockModel.backtest.StrategyBackTest.DerivativeStrategy.StartWithLimitCashVolConsiderNonFixSignal8BarWith930AllTo1000 import StartWithLimitCashVolConsider, InitailCashBasedEvaluationHelper
import pandas as pd
...
def calc_back_test_record(pct,per_amt_ratio, initial_cash, pool_num, deal_ratio, FIX_tag,TAG_930, alpha_pool_tag):
    start = 20170101
    end = 20211231
    # end = 20171231
    long_signal,short_signal,final_tag = get_nonfix_signal_with_930(pct_thre,0,start,end,FIX_tag,TAG_930,max_future=9)
    # long_signal[8] = long_signal[8].swaplevel(0,1)
    # long_signal[8].loc[930,:] = np.nan
    # long_signal[8] = long_signal[8].swaplevel(0,1)
    tag = 'AllTo1000_'+final_tag + alpha_pool_tag + f's{start}_e{end}'
