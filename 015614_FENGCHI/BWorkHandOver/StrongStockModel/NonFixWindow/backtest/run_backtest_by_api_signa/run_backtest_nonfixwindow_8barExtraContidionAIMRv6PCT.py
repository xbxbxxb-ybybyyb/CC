...
def calc_back_test_record(pct,per_amt_ratio, initial_cash, pool_num, deal_ratio, model_tag, alpha_pool_tag,signal_threshold,down_definition,STAGE):
    start = 20170101
    end = 20211231
    ...
    tag = f'{model_tag}_{alpha_pool_tag}_{pct:.3f}_{signal_threshold:.2f}_{down_definition:.3f}_NonFixSignal1000Base8Bar'
    print(tag)
    stk_min_amt = min(round(initial_cash * per_amt_ratio, -5) * 0.2, 1000000)
    file_name = '%s_%dbp_cost' % (tag, int(10000 * cost))
    base_dir = '/data/user/015664/AFuckingTrigger/限制买入和持仓/NonFixWindowConditionPctV6_2_1/'
...
para_list_stage0= [x+(0,) for x in para_list]
para_list_stage1= [x+(1,) for x in para_list]
...
if get_forbidden_tag():
    ...
    d_definition,s_threshold,pct_thre,stage = eval(aimr_multitimes.getParam())#0.15,0.5,0,0
