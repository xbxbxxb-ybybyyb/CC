...
def calc_back_test_record(pct,per_amt_ratio, initial_cash, pool_num, deal_ratio, tag, alpha_pool_tag):
    start = 20170101
    end = 20211231
    long_signal = generate_long_signal(pct,base_model_param[tag],start,end,f'{signal_path}/{tag}/long/')
    short_signal = generate_short_signal(0,base_model_param[tag],start,end,f'{signal_path}/{tag}/short/')
...
calc_back_test_record(*(each + (0.1, 'XGB_DTC_Matrix_Light_Cat', 'CS_XGB_OLS_condition_style_rank_ex20')))
