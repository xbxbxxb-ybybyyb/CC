...
from StrongStockModel.model.ModelResultLoadingTool import generate_long_signal
import pandas as pd
...
# long_param = {i: f'/data/user/015664/AFuckingTrigger/限制买入和持仓/信号存储/long_nonfix_window_8bar1DayRevResFactorExpandForCondition//signal_long_XGB_DTC_Future_{i}_Bar_pct_0.05.pkl' for i in range(1,9)}
# short_param = {i:f'/data/user/015664/AFuckingTrigger/限制买入和持仓/信号存储/long_nonfix_window_8bar1DayRevResFactorExpandForCondition//signal_short_XGB_DTC_Future_{i}_Bar_pct_0.pkl' for i in range(1,8)}
...
def calc_back_test_record(pct_threshold, per_amt_ratio, initial_cash, pool_num, deal_ratio, tag, alpha_pool_tag):
    ...
    # signal, pred_ret = pd.read_pickle(f'/data/user/015664/AFuckingTrigger/限制买入和持仓/信号存储/long_nonfix_window_8bar1DayRevResFactorExpandForCondition//signal_long_XGBReversalRes_DTC_Future_8_Bar_pct_0.05.pkl')[:2]
    # signal, pred_ret = pd.read_pickle(long_param[8])[:2]
    signal_out_path = f'/data/user/015664/AFuckingTrigger/限制买入和持仓/信号存储/long_nonfix_window_8bar_{tag}/'
    if not os.path.exists(signal_out_path):
        os.makedirs(signal_out_path)
    long_signal = generate_long_signal(pct_threshold,base_model_param[tag],start,end,signal_out_path)
    pred_ret = long_signal[8]
    tag = tag +  f'{alpha_pool_tag}Top{pool_num}_real{pool_num}_deal_ratio_%.1f_per_ratio_%.4f_threshold_%.2f_inital_%d' % (deal_ratio, per_amt_ratio, pct_threshold, int(initial_cash))
    ...
    if True:#not os.path.exists('/data/user/015664/AFuckingTrigger/限制买入和持仓/Upgrade/record/record_%sVolConsider_UpBuy100_%dbp_cost.pkl' % (tag, int(10000 * cost))):
        ...
        pd.to_pickle([record, cash_series, holding_num],
                     '/data/user/015664/AFuckingTrigger/限制买入和持仓/Upgrade/record/record_%sVolConsider_UpBuy100_%dbp_cost.pkl' % (tag, int(10000 * cost)))
...
'XGB_ResPreMinute_DTC':{
    x: [
f'/data/group/800442/800319/Faamonitor/PL/all_mkt_ts_norm/MonthlySelectedMDDImprove8BarReversalResPreMinute/Future_{x}_bar/XGBV4ReversalResReselect_ic_d_train200_test10_factor_num400/XGBV4ReversalResReselect_ic_d_train200_test10_factor_num400/',
f'/data/group/800442/800319/Faamonitor/PL/all_mkt_ts_norm/MonthlySelectedMDDImprove8BarReversalResPreMinute/Future_{x}_bar/XGBV4ReversalResReselect_ic_t_train200_test10_factor_num400/XGBV4ReversalResReselect_ic_t_train200_test10_factor_num400/',
f'/data/group/800442/800319/Faamonitor/PL/all_mkt_ts_norm/MonthlySelectedMDDImprove8BarReversalResPreMinute/Future_{x}_bar/XGBV4ReversalResReselect_ic_c_train200_test10_factor_num400/XGBV4ReversalResReselect_ic_c_train200_test10_factor_num400/',
...
calc_back_test_record(*(each + (0.1,'XGB_ResPreMinute_DTC', 'CS_XGB_OLS_condition_style_rank_ex20')))
