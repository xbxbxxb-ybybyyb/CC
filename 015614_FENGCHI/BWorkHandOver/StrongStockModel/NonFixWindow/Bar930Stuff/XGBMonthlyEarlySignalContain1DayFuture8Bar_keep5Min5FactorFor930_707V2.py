...
class XGBRegressionFactorEvalYearly(ModelNewLoading):
    ...
    def get_dataset(self, train_idx, test_idx, fix_factor_list, interday_factor, label_method, label_param={}, kernel=10):
        ...
        if train_idx[-1] == test_idx[0] and train_idx[-1] == test_idx[-1]:
            train_feature, train_label, nolimit_train, train_idx_date, train_idx_code, train_idx_time,y_1day_train,y_1day_origin_train = load_5min_8bar_with_selfdeined_label(train_idx[0], get_pre_trade_date(train_idx[-1]),
                                                                                                                      fix_factor_list,address=self.feature_address,label_path=self.label_path,return_1day_label=True,freq=47)
        else:
            train_feature, train_label, nolimit_train, train_idx_date, train_idx_code, train_idx_time,y_1day_train,y_1day_origin_train = load_5min_8bar_with_selfdeined_label(train_idx[0], train_idx[-1],
                                                                                                fix_factor_list,address=self.feature_address,label_path=self.label_path,return_1day_label=True,freq=47)
        ...
        if train_idx[-1] == test_idx[0] and train_idx[-1] == test_idx[-1]:
            ...
            if test_idx[-1] >= today:
                test_feature, test_label, test_nolimit, test_idx_date, test_idx_code, test_idx_time,y_1day,y_1day_origin = load_5min_8bar_with_selfdeined_label(start_date=test_idx[0], end_date=get_pre_trade_date(today),
                                    factor_list=fix_factor_list, return_idx=True,address=self.feature_address,label_path=self.label_path,return_1day_label=True,freq=47)
            else:
    ...
    def get_fix_factor_evaluation(self, num, end_index):
        ...
        factor_evaluation = pd.read_pickle(f'{root_path}external_data/FutureBarBy30Min_8barFor5mins930_707/Future_{self.future_bar_num}_bar/{self.eval_indicator}.pkl')
...
def main_window_search(i, indicator,future_bar_num,label_path):
    ...
    out_file =f'/data/group/800442/800319/Faamonitor/PL/all_mkt_ts_norm/NonFix8BarWith930Use5Mins_707V20220330/Future_{future_bar_num}_bar/{target_file}/{target_file}.pkl'
    ...
    model = XGBRegressionFactorEvalYearly(train_start, test_end, None, feature_address='/arch1/group/800442/800319/MinFactorSuper/FactorData/Factor/',
...
for i in tqdm(range(21,len(para_list)-20))[::-1]:
    for f_bar in [9]:#list(range(1, 9)):
        for ind_name in tag_list:
            ...
            #                  callback=update)
            main_window_search(i, ind_name,f_bar,'/data/group/800442/800319/HFfactor/ForDerivativeLabel8BarFor5MinsV47_keep5/data/')
