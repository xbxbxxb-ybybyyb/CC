...
from dataApi.FixFactorRollPrepare import load_5min_8bar_with_selfdeined_label, feature_engineering
import numpy as np
...
class XGBRegressionFactorEvalYearly(ModelNewLoading):
    ...
    def get_dataset(self, train_idx, test_idx, fix_factor_list, interday_factor, label_method, label_param={}, kernel=10):
        ...
        if train_idx[-1] == test_idx[0] and train_idx[-1] == test_idx[-1]:
            train_feature, train_label, nolimit_train, train_idx_date, train_idx_code, train_idx_time,y_1day_train,y_1day_origin_train = load_5min_8bar_with_selfdeined_label(train_idx[0], get_pre_trade_date(train_idx[-1]),
                                                                                                                      fix_factor_list,address=self.feature_address,label_path=self.label_path,return_1day_label=True)
        else:
            train_feature, train_label, nolimit_train, train_idx_date, train_idx_code, train_idx_time,y_1day_train,y_1day_origin_train = load_5min_8bar_with_selfdeined_label(train_idx[0], train_idx[-1], fix_factor_list,address=self.feature_address,label_path=self.label_path,return_1day_label=True)
        ...
        if train_idx[-1] == test_idx[0] and train_idx[-1] == test_idx[-1]:
            ...
            if test_idx[-1] >= today:
                test_feature, test_label, test_nolimit, test_idx_date, test_idx_code, test_idx_time,y_1day,y_1day_origin = load_5min_8bar_with_selfdeined_label(start_date=test_idx[0], end_date=get_pre_trade_date(today),
                                                                                                                    factor_list=fix_factor_list, return_idx=True,address=self.feature_address,label_path=self.label_path,return_1day_label=True)
            else:
                test_feature, test_label, test_nolimit, test_idx_date, test_idx_code, test_idx_time,y_1day,y_1day_origin = load_5min_8bar_with_selfdeined_label(start_date=test_idx[0], end_date=test_idx[-1],
                                                                                                                    factor_list=fix_factor_list, return_idx=True,address=self.feature_address,label_path=self.label_path,return_1day_label=True)
        ...
        return train_feature.loc[train_idx[0]:train_idx[-1]], train_label.loc[train_idx[0]:train_idx[-1]], \
               test_feature.loc[test_idx[0]:test_idx[-1]], test_label.loc[test_idx[0]:test_idx[-1]], time.time() - e
    ...
    def get_fix_factor_evaluation(self, num, end_index):
        ...
        factor_evaluation = pd.read_pickle(f'{root_path}external_data/FutureBarBy30Min_8barFor5mins930/Future_{self.future_bar_num}_bar/{self.eval_indicator}.pkl')
        ...
        for idx, cell_idx in bar:
            ...
            if os.path.exists(params['feature_path'] + '%d.pkl' % train_end_idx):
                ...
                X_train, y_train, X_test, y_test, feature_engineering_time = \
                    self.get_dataset((get_pre_trade_date(train_end_idx,14), train_end_idx), (test_start_idx, test_end_idx),
...
def main_window_search(i, indicator,future_bar_num,label_path):
    ...
    test_period = 10
    factor_num = 400
    target_file = 'XGB_5mins_%s_train%d_test%d_factor_num%d' % (indicator, train_period, test_period, factor_num)
    out_file =f'/data/group/800442/800319/Faamonitor/PL/all_mkt_ts_norm/NonFix8BarWith930Use5Mins/Future_{future_bar_num}_bar/{target_file}/{target_file}.pkl'
    ...
    model = XGBRegressionFactorEvalYearly(train_start, test_end, None, feature_address='/arch1/group/800442/800319/HFfactor/DTC2021/data/',
...
bar = tqdm(total=(len(para_list)-5)*8)
...
tag_list = ['ic_d','ic_t','ic_c']
for i in range(21,len(para_list)-5):
    for f_bar in list(range(1, 2)):
        for ind_name in tag_list:
            # pool.apply_async(main_window_search,args=(i, ind_name,f_bar,'/data/group/800442/800319/HFfactor/ForDerivativeLabel8BarFor5Mins_keep5/data/'),
            #                  callback=update)
            main_window_search(i, ind_name,f_bar,'/data/group/800442/800319/HFfactor/ForDerivativeLabel8BarFor5Mins_keep5/data/')
pool.close()
pool.join()

# tag_list = ['ic_d','ic_t','ic_c']
#
# for i in tqdm(range(20,len(para_list)-5)):
#     for f_bar in tqdm(list(range(1, 9))):
#         for ind_name in tag_list:
#             main_window_search(i, ind_name,f_bar,label_path='/data/group/800442/800319/HFfactor/ForDerivativeLabel8BarFor5Mins_keep5/data/')
#             gc.collect()
...
send_message(['015664'],'XGBMatrix for sim done')
