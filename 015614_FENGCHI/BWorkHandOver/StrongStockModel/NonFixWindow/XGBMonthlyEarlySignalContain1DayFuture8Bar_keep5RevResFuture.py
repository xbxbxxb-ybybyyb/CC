...
class XGBRegressionFactorEvalYearly(ModelNonFixWindow):
    def __init__(self, start=20170103, end=20191231, stock_pool=None, feature_address=None, factor_eval_indicator=None, factor_num=None,label_path=None,future_bar_num=None):
        ...
        if label_path is None or future_bar_num is None:
            ...
            self.label_path = f'{label_path}/{future_bar_num}.npy'
    ...
    #     today = get_recent_trade_date()#get_pre_trade_date(today)
    #     if train_idx[-1] == test_idx[0] and train_idx[-1] == test_idx[-1]:
    ...
    def get_fix_factor_evaluation(self, num, end_index):
        ...
        factor_evaluation = pd.read_pickle(f'{root_path}external_data/FutureBarBy30Min_8bar/Future_{self.future_bar_num[7:8]}_bar/{self.eval_indicator}.pkl')
...
def main_window_search(i, indicator,future_bar_num,tail,label_path):
    train_period = 200
    test_period = 10
    factor_num = 400
    target_file = 'XGB_%s_train%d_test%d_factor_num%d_%s' % (indicator, train_period, test_period, factor_num,tail)
    out_file =f'/data/group/800442/800319/IntraExp/RevResFuture/Future_{future_bar_num}_bar/{target_file}/{target_file}.pkl'
    ...
    model = XGBRegressionFactorEvalYearly(train_start, test_end, None, feature_address='/data/group/800442/800319/HFfactor/RealTimeFixRollRobust/data/',
                                          ...
                                          factor_num=factor_num,label_path=label_path,future_bar_num=f'future_{future_bar_num}_bar_{tail}')
...
idx_list = list(range(49,74))#[24:][::-1][:1]

for i in tqdm(idx_list):
    for f_bar in tqdm(list(range(8, 9))):
        for ind_name in ['ic_d','ic_t','ic_c']:
            for tail in ['regW1', 'regW1barly', 'regW280', 'regW35', 'regW7']:
                main_window_search(i, ind_name,f_bar,tail,label_path='/data/group/800442/800319/HFfactor/ForDerivativeLabel8Bar_keep5ReversalRes/data/')
                gc.collect()
