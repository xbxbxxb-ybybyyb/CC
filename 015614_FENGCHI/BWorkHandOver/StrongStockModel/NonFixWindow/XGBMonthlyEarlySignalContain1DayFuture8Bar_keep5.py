...
class XGBRegressionFactorEvalYearly(ModelNewLoading):
    ...
    def get_dataset(self, train_idx, test_idx, fix_factor_list, interday_factor, label_method, label_param={}, kernel=10):
        ...
        if train_idx[-1] == test_idx[0] and train_idx[-1] == test_idx[-1]:
            train_feature, train_label, nolimit_train, train_idx_date, train_idx_code, train_idx_time,y_1day_train = load_fix_data_selfdefined_label(train_idx[0], get_pre_trade_date(train_idx[-1]),
                                                                                                                      fix_factor_list,address=self.feature_address,label_path=self.label_path,return_1day_label=True)
        else:
            train_feature, train_label, nolimit_train, train_idx_date, train_idx_code, train_idx_time,y_1day_train = load_fix_data_selfdefined_label(train_idx[0], train_idx[-1], fix_factor_list,address=self.feature_address,label_path=self.label_path,return_1day_label=True)
        ...
        if train_idx[-1] == test_idx[0] and train_idx[-1] == test_idx[-1]:
            ...
            if test_idx[-1] >= today:
                test_feature, test_label, test_nolimit, test_idx_date, test_idx_code, test_idx_time,y_1day = load_fix_data_selfdefined_label(start_date=test_idx[0], end_date=get_pre_trade_date(today),
                ...
                test_feature, test_label, test_nolimit, test_idx_date, test_idx_code, test_idx_time,y_1day = load_fix_data_selfdefined_label(start_date=test_idx[0], end_date=test_idx[-1],
                                                                                                                    factor_list=fix_factor_list, return_idx=True,address=self.feature_address,label_path=self.label_path,return_1day_label=True)
...
def main_window_search(i, indicator,future_bar_num):
    train_period = 200
    test_period = 10
    factor_num = 400
    target_file = 'XGBV4ReversalResReselect_%s_train%d_test%d_factor_num%d' % (indicator, train_period, test_period, factor_num)
    out_file =f'/data/group/800442/800319/Faamonitor/PL/all_mkt_ts_norm/MonthlySelectedMDDImproveV8BarWith1DayLabel_20220215_keep5/Future_{future_bar_num}_bar/{target_file}/{target_file}.pkl'
    ...
    model = XGBRegressionFactorEvalYearly(train_start, test_end, None, feature_address='/data/group/800442/800319/HFfactor/RealTimeFixRollRobustReversalRes/data/',
                                          factor_eval_indicator=indicator,
                                          factor_num=factor_num,label_path='/data/group/800442/800319/HFfactor/ForDerivativeLabel8Bar_keep5/data/',future_bar_num=future_bar_num)
...
for f_bar in [8]:#list(range(1,9)):
    for i in tqdm(idx_list):
        for ind_name in ['ic_d','ic_c', 'ic_t']:
            main_window_search(i, ind_name,future_bar_num=f_bar)
...
new = f'/data/group/800442/800319/Faamonitor/PL/all_mkt_ts_norm/MonthlySelectedMDDImproveV8BarWith1DayLabel_20220215_keep5/Future_8_bar/XGBV4ReversalResReselect_ic_{tag}_train200_test10_factor_num400/XGBV4ReversalResReselect_ic_{tag}_train200_test10_factor_num400/'
old = f'/data/group/800442/800319/Faamonitor/PL/all_mkt_ts_norm/MonthlySelected/XGBV4FactorList_ic_{tag}_train200_test10_factor_num400/'
file_list = sorted(os.listdir(new))

old_res = []
new_res = []

for each in file_list:
    old_list = pd.read_pickle(f'{old}{each}')
    new_list = pd.read_pickle(f'{new}{each}')
    break
    old_res.append(old_list)
    new_res.append(new_list)

old_res = pd.concat(old_res)
