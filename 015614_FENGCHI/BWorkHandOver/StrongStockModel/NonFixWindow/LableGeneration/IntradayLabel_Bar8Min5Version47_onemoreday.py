...
def get_future_by_30min_2day(test_date_list, code_list,future_bar=1, delay_min=1, order_keep_min=30):
...
def save_bar_future(start,end,f_bar,out_path,order_keep_min):
    ...
    for each in ['idx_date.npy', 'idx_time.npy', 'idx_code.npy', 'future.npy', 'nolimit.npy']:
        shutil.copy(f'/arch1/group/800442/800319/MinFactorSuper/FactorData/Factor/{each}',
                    f'{out_path}/{each}')
    ...
    if f_bar>8 :
        future_short_window = get_future_by_30min_2day(date_list, code_list, future_bar=f_bar, delay_min=1, order_keep_min=order_keep_min)
    ...
    # check = future_short_window_df.swaplevel(0,1).loc[period_list].swaplevel(0,1)
    # future_short_window_df.loc[get_pre_trade_date(date_list[-1],-1)] = np.nan
    future_short_window_arr = trans_df2arr(future_short_window_df, start_date=start, end_date=get_pre_trade_date(end,-1),freq=len(out_period_list), roll=False,
                                           address='/arch1/group/800442/800319/MinFactorSuper/FactorData/Factor/')
...
future_bar = 9#int(AIMR.getParam())
latest_day = get_pre_trade_date(get_recent_trade_date(20220315))
save_bar_future(20140604,latest_day,f_bar=future_bar,
                out_path='/data/group/800442/800319/HFfactor/ForDerivativeLabel8BarFor5MinsV47_keep5/data/',order_keep_min=5)
