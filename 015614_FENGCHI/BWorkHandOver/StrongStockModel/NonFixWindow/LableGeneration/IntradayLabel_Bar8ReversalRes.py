...
def save_bar_future(start,end,f_bar,out_path,order_keep_min,reg_len):
    ...
    out_file = f'{out_path}future_{f_bar}_bar_regW{reg_len}.npy'
    ...
    alpha,beta = [np.pad(x,((2,0)),constant_values=np.nan,mode='constant') for x in [alpha[:-2,-1],beta[:-2,-1]]]
    y_ht = alpha[:,None,None] + beta[:,None,None] * recent_pct_change
