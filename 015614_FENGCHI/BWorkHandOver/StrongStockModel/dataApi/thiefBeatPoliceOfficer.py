...
if xquantEnv == 0:
    ...
    TRANSACTION_CONFIG = {
        "ApplicationConfig": "IStationDubboService",
        "IStation_service_interface": 'com.huatai.fast.business.manager.api.IManagerDubboService',
        "IP_CONFIG": "168.6.5.21:2181,168.6.5.22:2181,168.8.189.50:2181,168.8.189.51:2181,168.160.36.11:2181"
    }
    ...
    if os.path.exists("/etc/.config/octopus/__jurisdictionData_config.py"):
        try:
            ...
            if 'raylet' in ",".join(p.cmdline()):
                return json.loads(open("/etc/.config/octopus/__jurisdictionData_config.py").readlines()[0].split("=")[1].strip())
        except:
