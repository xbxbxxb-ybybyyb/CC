...
class XGBRegressionFactorEvalYearly(ModelNonFixWindow):
    ...
    def get_fix_factor_evaluation(self, num, end_index):
        ...
        if 'ret' in self.eval_indicator:
            ...
            raise Exception('')
        factor_list = factor_evaluation.index.tolist()[:num * 2]
    ...
    def train_model(self, X_train, y_train, params, end_date=None):
        ...
        pd.to_pickle(factor_list, params['feature_path'] + '%d_1.pkl' % end_date)
        model_1_path = params['model_conf_path'] + '%d_1.json' % end_date
        model_2_path = params['model_conf_path'] + '%d.json' % end_date
        train_features = None
        if False:  # os.path.exists(model_1_path):
            pass
        else:
            ...
            model.save_model(model_1_path)
            print(params['model_conf_path'] + '%d.json' % end_date)
            del d_train, d_eval  # ,train_features, train_label
            gc.collect()
        ...
        factor_list2 = sorted(score.index.tolist()[params['idx']:][:len(factor_list) // 2])
        print(f'final factor list len {len(factor_list2)}')
        pd.to_pickle(factor_list2, params['feature_path'] + '%d.pkl' % end_date)
        del model
        gc.collect()
        if os.path.exists(model_2_path):
            ...
            if train_features is None:
                train_features, train_label = X_train[factor_list2].loc[date_list], y_train.loc[date_list]
            else:
                train_features = train_features[factor_list2]
            d_train = xgb.DMatrix(train_features[:-50000], label=train_label[:-50000]['actual_label'])
            d_eval = xgb.DMatrix(train_features[-50000:], label=train_label[-50000:]['actual_label'])
            model = xgb.train(args_param, d_train, num_boost_round=params['n_estimators'], evals=[(d_eval, 'd_eval')], early_stopping_rounds=15, verbose_eval=False)
            model.save_model(model_2_path)
            del d_train, d_eval
            gc.collect()
        ...
        if 'val_pred_path' in params:
            ...
            val_features, val_labels = X_train[factor_list2].loc[val_date[1:]], y_train.loc[val_date[1:]]
    ...
    def rolling_train_and_predict(self, params={}, period=10, predict_period=10, label_methodology='fix_window', label_param={}, factor_nums=200, kernel=10):
        ...
        for idx, cell_idx in bar:
            ...
            if len(X_train) > 2000 and len(set(y_train[y_train.columns[0]])) > 1:
                ...
                model, factor_list = self.train_model(X_train, y_train, params, train_end_idx)
            if model is None:
                continue
            ...
            if len(X_test) == 0:
                ...
                pred_label = self.predict(model, X_test[factor_list], train_end_idx)
...
def main_window_search(i, indicator, future_bar_num, label_path, drop, score_type):
    ...
    out_file = f'/data/group/800442/800319/IntraExp/MDDPeriodFactorInfluence/DropByFIRetrain_{score_type}_roll/Future_{future_bar_num}_bar/{target_file}/{target_file}.pkl'
...
dr = 0
# pool = Pool()
for s_type in ['total_cover']:  # ,'weight', 'gain', 'cover', 'total_gain']:
    for i in tqdm(list(range(49,74))[::-1]):
