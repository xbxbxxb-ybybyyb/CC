            ...
            if os.path.exists(params['feature_path'] + '%d.pkl' % train_end_idx):
                ...
                X_train, y_train, X_test, y_test, feature_engineering_time = \
                    self.get_dataset((train_start_idx, train_end_idx), (test_start_idx, test_end_idx),
                                     fix_factor_list, None, label_methodology, label_param, kernel=kernel)
            X_train[fix_factor_list[:label_param['drop']]] = 0
            X_test[fix_factor_list[:label_param['drop']]] = 0
            gc.collect()
...
def main_window_search(i, indicator,future_bar_num,label_path,drop):
    ...
    out_file =f'/data/group/800442/800319/IntraExp/MDDPeriodFactorInfluence/Future_{future_bar_num}_bar/{target_file}/{target_file}.pkl'
