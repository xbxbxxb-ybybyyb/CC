    ...
    def rolling_train_and_predict(self, params={}, period=10, predict_period=10, label_methodology='fix_window', label_param={}, factor_nums=200, kernel=10):
        ...
        for idx, cell_idx in bar:
            ...
            for date in [20180201,20180202,20180206,20180207]:
                ...
                temp_list = ((abs(check)>4).sum()/check.shape[0]).sort_values().index[-idx:].tolist()
                drop_list += temp_list
            # extra_ratio = pd.DataFrame(extra_ratio)
            drop_list = list(set(drop_list))
            X_train[drop_list] = np.nan
            X_test[drop_list] = np.nan
            gc.collect()
...
def main_window_search(i, indicator,future_bar_num,label_path,drop):
    train_period = 200
    test_period = 10
    factor_num = 400
    # drop = 10
    target_file = 'XGB_%s_train%d_test%d_factor_num%d_drop%d' % (indicator, train_period, test_period, factor_num,drop)
    out_file =f'/data/group/800442/800319/IntraExp/MDDPeriodFactorInfluence/DropSub10ByExtraRatioRetrain/Future_{future_bar_num}_bar/{target_file}/{target_file}.pkl'
...
for dr in range(5,21,3):
    main_window_search(i, ind_name,f_bar,
        label_path='/data/group/800442/800319/HFfactor/ForDerivativeLabel8Bar_keep5/data/',drop=dr)
