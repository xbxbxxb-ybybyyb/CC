...
all_file = './V6_2_1单笔1.5%_原参数寻优.xlsx'
base_path = '/data/user/015664/AFuckingTrigger/限制买入和持仓/PredConsiderV6_2_1PoolSeekingReParamSeeking/'
...
file_list = sorted(list(filter(lambda x: 'per5' in x or 'per15' in x, file_list)))
# import shutil

# shutil.copy('/data/user/015664/AFuckingTrigger/限制买入和持仓/PredConsiderV2PrameterSeeking/XGBMonthlyV4_Cat_Light_ValWithoutMax5_deal0.10_per5.0bp_thre5_cash2e+08_Top600_start20170101_end20210531_s_thre1.00_d_thre1.00VolConsider_UpBuy100_10bp_cost.xlsx',
# f'{base_path}XGBMonthlyV4_Cat_Light_ValWithoutMax5_deal0.10_per5.0bp_thre5_cash2e+08_Top600_start20170101_end20210531_s_thre1.00_d_thre1.00_swing0VolConsider_UpBuy100_10bp_cost.xlsx')
...
for file_name in file_list:
    ...
    clf_name = file_name[105+14:].replace('VolConsider_UpBuy100_10bp_cost.xlsx', '')
...
sensi_eval_res['signal_threshold'] = sensi_eval_res['模型'].apply(lambda x: x.strip('s_thre').split('_d_thre')[0])
sensi_eval_res['down_signal_ratio_threshold'] = sensi_eval_res['模型'].apply(lambda x: x.strip('s_thre').split('_d_thre')[1])

sensi_eval_res = sensi_eval_res.set_index(['index','signal_threshold', 'down_signal_ratio_threshold']).drop('模型', axis=1)

sensi_eval_res_signal = signaly_stat.loc['收益率'].reset_index()
sensi_eval_res_signal['signal_threshold'] = sensi_eval_res_signal['模型'].apply(lambda x: x.strip('s_thre').split('_d_thre')[0])
sensi_eval_res_signal['down_signal_ratio_threshold'] = sensi_eval_res_signal['模型'].apply(lambda x: x.strip('s_thre').split('_d_thre')[1])
sensi_eval_res_signal = sensi_eval_res_signal.set_index(['signal_threshold', 'down_signal_ratio_threshold']).drop('模型', axis=1)

# all_file = './考虑行情和信号量寻优V6_2_1InSample.xlsx'  # f'{base_path}/不同股票池_不同信号版本_不限制预测值千5_20211108.xlsx'
