...
all_file = f'{base_path}策略参数寻优_初始{initial_cash}_股票池{pool_num}.xlsx'
...
sensitive_compare['per_ratio'], sensitive_compare['threshold'] = sensitive_compare['per_ratio'].apply(lambda x: eval(x[0])), sensitive_compare['per_ratio'].apply(
    lambda x: eval(x[1]))
...
pivote_file = f'{base_path}策略参数寻优_敏感性分析_含分时点统计_初始{initial_cash}_股票池{pool_num}.xlsx'
