...
base_path = '/data/user/015664/AFuckingTrigger/限制买入和持仓/PoolNumCompare20211112/'
...
for file_name in file_list:
    clf_name = file_name.replace('XGBMonthlyV4_Cat_LightWithoutMax5CS_XGB_OLS_condition_style_rank_ex20Top','')
    clf_name = clf_name.replace('start20170101_end20210531VolConsider_UpBuy100_10bp_cost.xlsx','')
    clf_name = clf_name.split('_')[0]
    clf_name = 'Top'+clf_name.zfill(4)
...
net_compare = pd.concat([net_compare.sort_index(axis=1), indexes / indexes.loc[indexes.index[0]]], axis=1)
...
all_file = f'{base_path}/不同股票池数量对比.xlsx'
