...
base_path = '/data/user/015664/AFuckingTrigger/限制买入和持仓/OnlineEraSensitivityCalcFinishRatio/'
...
# pool_num = 400
file_list = list(filter(lambda x: '.xlsx' in x and x.startswith('XGB_lightGBM_CatBoostWithMax5threshold') and 'inital_%d'%initial_cash in x, file_list))
...
# file_list = list(filter(lambda x: f'Top{pool_num}_real{pool_num}' in x, file_list))
...
for file_name in file_list:
    ...
    deal_ratio = para.loc[file_name,'deal_ratio']
    clf_name = f'股票池_{pool_num}_委托占比{round(per_ratio*100,1)}%_阈值{int(threshold*100)}%'
    # clf_name = f'成交假设_{int(deal_ratio*100)}%'
    # clf_name = file_name.replace(f'XGB_lightGBM_CatBoostWithMax5thresholdAlphaTriggerPoolV3Top{pool_num}_real{pool_num}_deal_ratio_0.1_',
    #                              '')
    # clf_name = clf_name.replace('_inital_%dVolConsider_UpBuy100_10bp_cost.xlsx'%initial_cash, '')
    ...
    order_detail = all_data['逐笔持仓统计']
    order_detail['time_point'] = order_detail['start'] % 10000
    group = order_detail.groupby(['year','time_point'])
    signal_ratio[clf_name] = group.size()
    signal_ratio[clf_name] = signal_ratio[clf_name]/signal_ratio[clf_name].sum()
    barly_signal_profit[clf_name] = group.mean()['收益率']
...
for year in list(set(net_compare['year'])) + [-1]:
    ...
    cum_mdd = ((cummax - temp_prof_comp) / cummax).drop('year', axis=1)
...
# all_file = f'{base_path}策略参数寻优_不同股票池_threshold_0.04_per_ratio_0.02_.xlsx'
all_file = f'{base_path}策略参数寻优_初始规模{initial_cash}_不同成交占比.xlsx'
...
sensitive_compare['per_ratio'] = sensitive_compare.index.map(lambda x: ''.join(list(filter(lambda i : i in num_str+['_'],x[1]))).strip('_').split('_'))
sensitive_compare['pool_num'],sensitive_compare['per_ratio'], sensitive_compare['threshold'] = sensitive_compare['per_ratio'].apply(lambda x: x[0]), \
                                                                                               sensitive_compare['per_ratio'].apply(lambda x: x[1]),\
                                                                                                sensitive_compare['per_ratio'].apply(lambda x: x[2])

sensitivity_stat = {}
for items in ['净值最大回撤', '年化夏普', '年化收益', '收益率波动', '收益率盈亏比', '日胜率']:
    sensitivity_stat[items] = sensitive_compare.loc[items].pivot_table(index=['pool_num','per_ratio'], columns='threshold', values='全时段')

cash_occupy_stat = cash_occupy_compare.mean().to_frame()
cash_occupy_stat['per_ratio'] = cash_occupy_stat.index.map(lambda x: ''.join(list(filter(lambda i : i in num_str+['_'],x))).strip('_').split('_'))
cash_occupy_stat['pool_num'],cash_occupy_stat['per_ratio'], cash_occupy_stat['threshold'] = cash_occupy_stat['per_ratio'].apply(lambda x: x[0]), \
                                                               cash_occupy_stat['per_ratio'].apply(lambda x: x[1]),\
                                                                cash_occupy_stat['per_ratio'].apply(lambda x: x[2])
sensitivity_stat['平均占资'] = cash_occupy_stat.pivot_table(index=['pool_num','per_ratio'], columns='threshold', values=0)

signal_profit = signaly_stat.loc['收益率']
signal_profit['per_ratio'] = signal_profit.index.map(lambda x: ''.join(list(filter(lambda i : i in num_str+['_'],x))).strip('_').split('_'))
signal_profit['pool_num'],signal_profit['per_ratio'], signal_profit['threshold'] = signal_profit['per_ratio'].apply(lambda x: x[0]), \
                                                                                    signal_profit['per_ratio'].apply(lambda x: x[1]),\
                                                                                    signal_profit['per_ratio'].apply(lambda x: x[2])

sensitivity_stat['单信号收益'] = signal_profit.pivot_table(index=['pool_num','per_ratio'], columns='threshold', values='全时段')

# barly_signal_profit = pd.DataFrame(barly_signal_profit).T
# signal_ratio = pd.DataFrame(signal_ratio).T
# signal_ratio['per_ratio'] = signal_ratio.index.map(lambda x: x.replace('per_ratio_', '').replace('threshold_', '').split('_'))
# barly_signal_profit['per_ratio'] = barly_signal_profit.index.map(lambda x: x.replace('per_ratio_', '').replace('threshold_', '').split('_'))

# barly_signal_profit['per_ratio'], barly_signal_profit['threshold'] = barly_signal_profit['per_ratio'].apply(lambda x: eval(x[0])), barly_signal_profit['per_ratio'].apply(lambda x: eval(x[1]))
# signal_ratio['per_ratio'], signal_ratio['threshold'] = signal_ratio['per_ratio'].apply(lambda x: eval(x[0])), signal_ratio['per_ratio'].apply(lambda x: eval(x[1]))

# barly_signal_profit_compare = barly_signal_profit.set_index(['per_ratio','threshold']).stack().to_frame().pivot_table(index=['time_point','per_ratio'],columns='threshold',values=0)
# signal_ratio = signal_ratio.set_index(['per_ratio','threshold'])#.stack().to_frame().pivot_table(index=['time_point','per_ratio'],columns='threshold',values=0)


# signal_by_threshold = {}
# for pct_threshold in [0.03,0.04,0.05,0.06,0.07,0.08,0.09]:
#     signal,_ = pd.read_pickle('/data/user/015664/AFuckingTrigger/限制买入和持仓/信号存储/signal_XGB_lightGBM_CatBoostWithMax5threshold_%.2f.pkl'%pct_threshold)
#     signal = signal.loc[:20181231]
#     signal = signal.stack().to_frame().reset_index()
#     signal['year'] = signal['date']//10000
#     signal_by_threshold[pct_threshold] = signal.groupby(['year','time']).sum()[0]
#
# signal_by_threshold = pd.DataFrame(signal_by_threshold)

# signal_ratio_dict = {}
# for year in set([x[0] for x in signal_ratio.columns]):
#     yearly = signal_ratio[year].stack().to_frame().pivot_table(index=['time_point','per_ratio'],columns='threshold',values=0)
#     yearly = yearly/yearly.groupby('per_ratio').sum()
#     temp = signal_by_threshold.loc[year].copy()
#     temp.index = pd.MultiIndex.from_tuples([(x,'原始信号') for x in temp.index])
#     temp = temp/temp.sum()
#     yearly = pd.concat([yearly,temp]).sort_index()
#     signal_ratio_dict[year] = yearly




# for pct_threshold in
# signal,_pred_ret = pd.read_pickle()

pivote_file = f'{base_path}策略参数寻优_敏感性分析_含分时点统计_初始{initial_cash}.xlsx'
with pd.ExcelWriter(pivote_file) as writer:
    for each in sensitivity_stat:
        sensitivity_stat[each].to_excel(writer, sheet_name=each)

    # signal_ratio.to_excel(writer,'每个时点信号比例')
    # barly_signal_profit_compare.to_excel(writer, '每个时点信号收益统计')
    # for year in signal_ratio_dict:
    #     signal_ratio_dict[year].to_excel(writer,'%d每个时点信号比例'%year)
writer.close()


send_file(['015664'],all_file)
send_file(['015664'],pivote_file)
