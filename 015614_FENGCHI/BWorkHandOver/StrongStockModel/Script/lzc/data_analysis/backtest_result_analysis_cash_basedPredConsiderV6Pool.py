...
all_file = './V4_2_1_参数用于调整未来成交，过去N日固定10%.xlsx'
base_path = '/data/user/015664/AFuckingTrigger/限制买入和持仓/PredConsiderV4_2_1PoolSeekingV3/'
...
for file_name in file_list:
    ...
    idx = file_name.index('Top')
    clf_name = file_name[52:68] + '_'+file_name[idx:idx+6]
...
sensi_eval_res['成交假设'] = sensi_eval_res['模型'].apply(lambda x: x.strip('deal').split('_per')[0])
sensi_eval_res['单笔委托'] = sensi_eval_res['模型'].apply(lambda x: x.strip('deal').split('_per')[1][:3]).astype(float)*0.001
sensi_eval_res['股票池数量'] = sensi_eval_res['模型'].apply(lambda x: x.strip('deal').split('_Top')[1])
sensi_eval_res = sensi_eval_res.set_index(['index', '股票池数量', '成交假设', '单笔委托']).drop('模型', axis=1)

sensi_eval_res_signal = signaly_stat.loc['收益率'].reset_index()
sensi_eval_res_signal['成交假设'] = sensi_eval_res_signal['模型'].apply(lambda x: x.strip('deal').split('_per')[0])
sensi_eval_res_signal['单笔委托'] = sensi_eval_res_signal['模型'].apply(lambda x: x.strip('deal').split('_per')[1][:3]).astype(float)*0.001
sensi_eval_res_signal['股票池数量'] = sensi_eval_res_signal['模型'].apply(lambda x: x.strip('deal').split('_Top')[1])
sensi_eval_res_signal = sensi_eval_res_signal.set_index(['股票池数量', '成交假设', '单笔委托']).drop('模型', axis=1)
...
with pd.ExcelWriter(all_file) as writer:
    ...
    sensi_eval_res.loc['年化收益'].unstack(level=[0]).T.to_excel(writer, sheet_name='年化收益 网格')
    sensi_eval_res.loc['净值最大回撤'].unstack(level=[0]).T.to_excel(writer, sheet_name='净值最大回撤 网格')
    (sensi_eval_res.loc['年化收益'] / sensi_eval_res.loc['净值最大回撤']).unstack(level=[0]).T.to_excel(writer, sheet_name='CalMar 网格')
    sensi_eval_res_signal.unstack(level=0).T.to_excel(writer, sheet_name='信号收益 网格')
