...
base_path = '/data/user/015664/AFuckingTrigger/限制买入和持仓/PredConsiderV3_1PoolPerSigPrameterSeeking/'
...
for file_name in file_list:
    ...
    if 's_thre1.0' in file_name:
        clf_name = 'peroldTopold'
    else:
        clf_name = file_name[61:].replace('_thre5_cash2e+08_','')[:15].strip('_')
...
sensi_eval_res['signal_threshold'] = sensi_eval_res['模型'].apply(lambda x: x.strip('per').split('Top')[0])
sensi_eval_res['down_signal_ratio_threshold'] = sensi_eval_res['模型'].apply(lambda x: x.strip('per').split('Top')[1].strip('_'))
# sensi_eval_res['swing_threshold'] = sensi_eval_res['模型'].apply(lambda x: x.strip('per').split('_swing')[1])
...
sensi_eval_res_signal['signal_threshold'] = sensi_eval_res_signal['模型'].apply(lambda x: x.strip('per').split('Top')[0])
sensi_eval_res_signal['down_signal_ratio_threshold'] = sensi_eval_res_signal['模型'].apply(lambda x: x.strip('per').split('Top')[1].strip('_'))
# sensi_eval_res_signal['swing_threshold'] = sensi_eval_res_signal['模型'].apply(lambda x: x.strip('per').split('_swing')[1])
sensi_eval_res_signal = sensi_eval_res_signal.set_index([ 'signal_threshold', 'down_signal_ratio_threshold']).drop('模型', axis=1)

all_file = './考虑行情和信号量寻优V3_1PoolPerSigRev.xlsx'  # f'{base_path}/不同股票池_不同信号版本_不限制预测值千5_20211108.xlsx'
