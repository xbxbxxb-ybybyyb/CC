...
import numpy as np
import pandas as pd
from StrongStockModel.model.Modelmpl.XGBRegressionYealyFactor import XGBRegressionYealyFactor
...
def main_window_search():
    ...
    model = XGBRegressionYealyFactor(20150101, 20181231, None, feature_address=all_mkt_preprocessed_ts_norm_by_date_path)
    out_file = '/data/group/800319/Faamonitor/PL/all_mkt_ts_norm/reg_norm_window_compare/XGBYearlyFactor_train%d_test%d_factor_num%d_norm_window_%d.pkl' % (train_period, test_period, factor_num,N)
    print(out_file)
