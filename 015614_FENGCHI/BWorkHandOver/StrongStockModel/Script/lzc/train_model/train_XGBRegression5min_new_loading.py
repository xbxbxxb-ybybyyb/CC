...
import numpy as np
import pandas as pd
from StrongStockModel.model.Modelmpl.XGBRegression5min_new_loading import XGBRegression
...
def main_window_search(i):
    ...
    factor_num = 100
    N = 40
    indicator = 'sdt_adjusted'
    all_mkt_preprocessed_ts_norm_by_date_path = '/data/group/800319/junkBigFactorPool/back_factor'#'/data/group/800319/ModelSmallFactor/'  # '/data/group/800319/junkData/StrongStock/processed_factor_all_pool_by_date/ts_norm_%d_and_binary/' % N
    out_file = '/data/group/800319/Faamonitor/PL/all_mkt_ts_norm/5min/FactorEval/XGB5minFactorEval_%s_train%d_test%d_factor_num%d_norm_window_%d.pkl' % (
    ...
    if os.path.exists(base_dir+'%d.pkl'%train_end):
        print(base_dir+'%d.pkl'%train_end,'exist')
        return
    ...
    model = XGBRegression(train_start, test_end, None, feature_address=all_mkt_preprocessed_ts_norm_by_date_path,indicator=indicator)
