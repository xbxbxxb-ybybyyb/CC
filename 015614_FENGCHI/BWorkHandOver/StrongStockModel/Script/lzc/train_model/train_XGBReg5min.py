...
import numpy as np
import pandas as pd
from StrongStockModel.model.Modelmpl.XGBRegression5min import XGBRegression5min
...
def main_window_search(i):
    ...
    para = param_list[i][1]
    print(para)
    out_file = '/data/group/800319/Faamonitor/PL/all_mkt_ts_norm/reg_norm_window_compare/XGB5min180_train%d_test%d_factor_num%d_norm_window_%d.pkl' % (
        train_period, test_period, factor_num, N)
    base_dir = out_file.replace('.pkl', '/')
    if not os.path.exists(base_dir):
        os.mkdir(base_dir)
    if os.path.exists(base_dir + '%d.pkl' % para[1]):
        print(base_dir + '%d.pkl' % para[1], 'eixst')
        return
    print(base_dir + '%d.pkl' % para[1], 'start')
    model = XGBRegression5min(para[0], para[-1], None, feature_address=all_mkt_preprocessed_ts_norm_by_date_path)
