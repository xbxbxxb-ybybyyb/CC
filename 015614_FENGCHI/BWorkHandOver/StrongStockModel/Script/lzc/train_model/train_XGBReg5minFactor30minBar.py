...
import numpy as np
import pandas as pd
from StrongStockModel.model.Modelmpl.XGBRegression5minFactor30minBar import XGBRegression5minFactor30minBar
...
def main_window_search(i):
    ...
    factor_num = 312
    N = 40
    all_mkt_preprocessed_ts_norm_by_date_path = None#root_path + 'processed_factor_all_pool_by_date_5min/ts_norm_%d_append/'%N

    # model = XGBRegression5min(20150309, 20181231, None, feature_address=all_mkt_preprocessed_ts_norm_by_date_path) #200
    out_file = '/data/group/800319/Faamonitor/PL/all_mkt_ts_norm/5min/XGB5minFactor30minBarTNManual_train%d_test%d_factor_num%d_norm_window_%d.pkl' % (
    train_period, test_period, factor_num, N)
    # para = para_list[i][-1]
    # train_start,train_end,test_start,test_end = para
    model = XGBRegression5minFactor30minBar(20150309, 20181231, None, feature_address=all_mkt_preprocessed_ts_norm_by_date_path)
    print(out_file)
    ...
    pithces_dir = out_file.replace('.pkl','/')
    if not os.path.exists(pithces_dir):
        os.mkdir(pithces_dir)
    pd.to_pickle(label,out_file)
    # pd.to_pickle(label, pithces_dir+'%d.pkl'%train_end)
