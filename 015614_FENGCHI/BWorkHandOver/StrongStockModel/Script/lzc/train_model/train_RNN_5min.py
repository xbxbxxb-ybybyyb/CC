...
import numpy as np
import pandas as pd
from StrongStockModel.model.Modelmpl.RNNBase import RNNBase,param_rnn
...
def main(i):
    ...
    test_period = 10
    factor_num = 100
    out_file = '/data/group/800319/Faamonitor/PL/all_mkt_ts_norm/NN_param_optimization/RNN5mintdse_train%d_test%d_factor_num%d_norm_window_%d.pkl' % (
    train_period, test_period, factor_num, N)
    print(out_file)
    para = param_list[i][1]
    print(para)
    if os.path.exists(out_file.replace('.pkl','_%d.pkl'%para[1])):
        print(out_file.replace('.pkl','_%d.pkl'%para[1]),'exist')
        return
    model = RNNBase(para[0], para[-1], None, feature_address='/data/group/800319/JunkSmallFactor/')

    # best_param_clf_xgb['objective'] = 'reg:squarederror'
    param_rnn['val_pred_path'] = out_file.replace('.pkl','_val_pred/')
    param_rnn['train_log_path'] = out_file.replace('.pkl', '_train_log/')
    param_rnn['model_conf_path'] = out_file.replace('.pkl', '_model_conf/')
    param_rnn['load local model'] = True
    # best_param_clf_xgb['train_pred_path'] = out_file.replace('.pkl','_train_pred/')
    label = model.rolling_train_and_predict(params=param_rnn, period=train_period, predict_period=test_period,
                                            label_param={'kind': 'reg'}, kernel=15,factor_nums=factor_num)
    pd.to_pickle(label,out_file.replace('.pkl','_%d.pkl'%para[1]))
    print(out_file.replace('.pkl','_%d.pkl'%para[1]))
    # os.mkdir('/data/group/800319/Faamonitor/PL/')

idx = 20#int(AIMR.getParam())
main(idx)
