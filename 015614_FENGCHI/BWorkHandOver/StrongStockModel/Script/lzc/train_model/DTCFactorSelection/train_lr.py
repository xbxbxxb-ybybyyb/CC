...
import numpy as np
import pandas as pd
from StrongStockModel.model.Modelmpl.DTCFactorSelection.Linear import LinearReg
...
def main():
    ...
    factor_num = 400
    N = 40
    indicator_list = ['ic_all_t', 'ic_all_d', 'ic_all_c']
    indicator = 'ic_all_t'
    # para = param_list[i][1]
    ...
    # model = LinearReg(20150309, 20181231, None, feature_address=all_mkt_preprocessed_ts_norm_by_date_path, factor_eval_indicator=indicator)
    # out_sample
    out_file = '/data/group/800319/Faamonitor/PL/all_mkt_ts_norm/FactorEvalOutSample/LinearFactorEval_%s_train%d_test%d_factor_num%d_norm_window_%d.pkl' % (
        indicator, train_period, test_period, factor_num, N)
    ...
    model = LinearReg(20180309, 20201030, None, feature_address=all_mkt_preprocessed_ts_norm_by_date_path, factor_eval_indicator=indicator)
    print(out_file)
    best_param = {'n_jobs': -1}
