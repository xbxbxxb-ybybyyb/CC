...
import numpy as np
import pandas as pd
from StrongStockModel.model.Modelmpl.DTCFactorSelection.XGBRegressionFactorEval import XGBRegressionFactorEval
...
def main_window_search():
    ...
    model = XGBRegressionFactorEval(20160711,20181231, None, feature_address=all_mkt_preprocessed_ts_norm_by_date_path, factor_eval_indicator=indicator)
    out_file = '/data/group/800319/Faamonitor/PL/all_mkt_ts_norm/FactorEval/XGBFactorEval_%s_train%d_test%d_factor_num%d_norm_window_%d.pkl' % (
        indicator, train_period, test_period, factor_num, N)
