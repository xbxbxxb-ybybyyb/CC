...
import numpy as np
import pandas as pd
from StrongStockModel.model.Modelmpl.XGBReg_Mix_Freq import XGBRegressionMixFreq
...
para_list = list(filter(lambda x :
            not os.path.exists('/data/group/800319/Faamonitor/PL/all_mkt_ts_norm/MixFreq/XGBMixFreq_train200_test10_factor_num200_norm_window_40/%d.pkl'%x[1][1]),param_list))
...
def main_window_search(i):
    ...
    factor_num = 200
    N = 40
    out_file = '/data/group/800319/Faamonitor/PL/all_mkt_ts_norm/MixFreq/XGBMixFreq_train%d_test%d_factor_num%d_norm_window_%d.pkl' % (
    ...
    all_mkt_preprocessed_ts_norm_by_date_path = None#'/data/group/800319/junkData/StrongStock/processed_factor_all_pool_by_date/ts_norm_%d_and_binary/' % N
    model = XGBRegressionMixFreq(train_start,test_end, None, feature_address=all_mkt_preprocessed_ts_norm_by_date_path)
