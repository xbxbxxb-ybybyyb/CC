...
'XGB_D_HalfYear':[    '/data/group/800442/800319/Faamonitor/PL/all_mkt_ts_norm/FactorEvalOnlineTestVRobustOpt/XGBFactorEvalYearlyParam10_ic_half_d_train200_test10_factor_num400_norm_window_40.pkl'],
'XGB_T_HalfYear':[    '/data/group/800442/800319/Faamonitor/PL/all_mkt_ts_norm/FactorEvalOnlineTestVRobustOpt/XGBFactorEvalYearlyParam10_ic_half_t_train200_test10_factor_num400_norm_window_40.pkl'],
'XGB_C_HalfYear':[    '/data/group/800442/800319/Faamonitor/PL/all_mkt_ts_norm/FactorEvalOnlineTestVRobustOpt/XGBFactorEvalYearlyParam10_ic_half_c_train200_test10_factor_num400_norm_window_40.pkl'],
...
def calc_back_test_record(pct_threshold, per_amt_ratio, initial_cash, pool_num, deal_ratio, tags, alpha_pool_tag):
    # file_list = para[tag]
    # print(file_list)
    start = 20170101
    end = 20210531
    ...
    for tag in tags:
        tag = tag + 'WithoutMax5'
        if (pct_threshold*1000)%(0.01*1000)==0:
        ...
        if os.path.exists(signal_file):
            ...
            # raise Exception(f'{tag} signal dose not exist')
            print(f'{tag} {pct_threshold} not exist-------------------')
            file_list = para[tag.strip( 'WithoutMax5')]
        ...
        signals[tag],pred_rets[tag] = signal,pred_ret

    signals,pred_rets = pd.Panel(signals), pd.Panel(pred_rets)
    pred_ret_count = pred_rets.count(axis=0)
    pred_ret_mean = pred_rets.mean(axis=0)
    signal_sum = signals.sum(axis=0)
    signal = signal_sum.eq(len(tags))
    pred_ret = pred_ret_mean[pred_ret_count.eq(len(tags))]
    tag = 'InterSection'+'_'.join(tags)
    # pred_ret = pd.read_pickle('/data/group/800442/800319/信号存储/20200111LinearCompareMV.pkl')
    tag = tag + 'RevTriggerFilterHolding_deal_ratio_%.1f_per_ratio_%.4f_threshold_%.3f_inital_%d' % (deal_ratio, per_amt_ratio, pct_threshold, int(initial_cash))
...
pct = eval(AIMR.getParam())

each = (pct, 0.005, 2e8, 600)
calc_back_test_record(*(each + (0.1, ['Cat','XGB_C','XGB_D','XBG_T','Light'], 'CS_XGB_OLS_condition_style_rank_ex20')))
