# @Time : 2021/2/1 10:27
# @Author : Zhichen Lu
# @File : run_NumLimitVolConsider.py
...
from StrongStockModel.backtest.StrategyBackTest.DerivativeStrategy.LimitHoldingAndBarTriggerWith240MoreHoldingVolConsider import LimitHoldingAndBarTriggerWith240MoreHoldingVolConsider,EvaluationHelper
...
if not os.path.exists('/data/user/015664/AFuckingTrigger/限制买入和持仓/信号存储/signal_%s.pkl'%tag):
...
tag = tag+'LimitUp_deal_ratio_%.1f_per_amt_%d_threshold_%f.2'%(deal_ratio,per_amt,pct_threshold)
...
tag = tag.replace('LimitUp','LimitUp_AlphaTriggerPoolV3AbsTrigger')

instance = LimitHoldingAndBarTriggerWith240MoreHoldingVolConsider(pred_ret, 20160101, 20181231,stock_pool=alpha_pool,target_point=bar_list,buy_cost=cost, sell_cost=cost,
                                         per_amt=per_amt,append_param={'deal_price_path':deal_price_path+'deal_price_vwap_30min_20160104_20181228.pkl'},
                                         deal_percent=deal_ratio,barly_max_buy=up_buy,max_holding=up_holding)
record = instance.run_backtest()
holding_num = instance.holding_num
# pd.to_pickle([record,],'/data/user/015664/AFuckingTrigger/限制买入和持仓/InitialCashBasedVolConsiderOnlineTest/record_nofuture/%sInSample.pkl'%tag)

pd.to_pickle([record,holding_num],'/data/user/015664/AFuckingTrigger/限制买入和持仓/NoFutureInfoResShiftLimitHoldingBased/record/record_%s.pkl'%tag)
...
out_path = '/data/user/015664/AFuckingTrigger/限制买入和持仓/NoFutureInfoResShiftLimitHoldingBased/%sVolConsider_UpBuy%d_UpHolding%d_%dbp_cost.xlsx' % (tag, int(10000 * cost),up_buy,up_holding)
helper.one_wave_run(record,holding_num,48,output_path=out_path ,signal_record_save=True)
