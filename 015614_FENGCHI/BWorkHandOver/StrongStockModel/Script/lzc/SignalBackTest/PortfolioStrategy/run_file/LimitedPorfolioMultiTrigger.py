...
class SignalBasedPortfolioLimitUpMultiTrigger(PortfolioStrategyBase):
...
def get_signal_by_val_pct_threshold_dynamic(pct, subset_path, signal_file_name):
    """
    在验证集上计算pct处于多少分位数，然后用分位数在预测值上的阈值作为分类阈值
    :param pct:
    :param subset_path:
    ...
    subset = {}
    for date in point_list:
        subset[date] = pd.read_pickle(subset_path + '%d.pkl' % date).rename(columns={0:'actual_label'})
        # date_list = sorted(list(set([x[0] for x in subset.index])))
        # subset = subset.loc[date_list[:-1]]
        # th = (subset[0] < pct).sum() / subset.shape[0]
        # threshold_series.loc[date, 'threshold'] = max(subset['prediction'].quantile(th), 0.005)
...
def main():
    ...
    signal, pred_ret = get_signal_by_val_pct_threshold_dynamic(pct_threshold, signal_file.replace('.pkl', '_val_pred/'), signal_file)
    pred_ret[~signal.eq(1)] = np.nan
    ...
    instance = SignalBasedPortfolioLimitUpMultiTrigger(pred_ret, 20160101, 20181231, target_point=[1000, 1030, 1100, 1300, 1330, 1400, 1430], buy_cost=cost, sell_cost=cost)
    helper = EvaluationHelper()
    import time
    e = time.time()
    record = instance.run_backtest(48)  # pd.read_pickle('/data/user/015664/AFuckingTrigger/port_record.pkl')
    # pd.to_pickle(record, '/data/user/015664/AFuckingTrigger/port_record2.pkl')
    helper.one_wave_run(record, kernel=48, output_path='/data/user/015664/AFuckingTrigger/PortfolioLimitUp.xlsx', signal_record_save=True)
    print(time.time() - e)
