# @Time : 2020/10/29 11:24
# @Author : Zhichen Lu
# @File : DynamicSignal.py
...
def get_signal_by_val_pct_dynamic_threshold_integration(pct, subset_path_list, signal_file_name_list):
    ...
    return signal, pred_ret.set_index(['date', 'time']).drop('index', axis=1), threshold_series
...
def main(max_holding, daily_max_buy):
    ...
    signal_file = fnm_list[0].replace('lr', 'lr_XGB_integration')
    signal, pred_ret, threshold = get_signal_by_val_pct_dynamic_threshold_integration(pct_threshold, sp_list, fnm_list)
    pd.to_pickle([signal, pred_ret, threshold], '/data/user/015664/AFuckingTrigger/saved_signal/lr_XGB_integration_dynaic_threshold.pkl')
    ...
    helper.one_wave_run(record, kernel=24,
                        output_path='/data/user/015664/AFuckingTrigger/限制买入和持仓/lr_XGB_DynaicThreshold_UpHolding%d_UpBuy%d.xlsx' % (max_holding, daily_max_buy),
