...
class SignalBasedPortfolioLimitUpWithConcept(PortfolioStrategyBase):
    ...
    def daily_update(self, idx, date):
        ...
        self.daily_active_stk = self.daily_active_stk[self.daily_active_stk].tolist()
...
def main(max_holding, daily_max_buy):
    ...
    file_list = ['/data/group/800319/Faamonitor/PL/all_mkt_ts_norm/FactorEval/LinearFactorEval_ic_all_d_train200_test10_factor_num400_norm_window_40.pkl',
    ...
    for signal_file in [file_list[-1]]:
        subpath = signal_file.replace('.pkl', '_val_pred/')
        sp_list.append(subpath)
        fnm_list.append(signal_file)
    ...
    out_file = '/data/user/015664/AFuckingTrigger/限制买入和持仓/FactorEval/Linear_T_SeperateEnsemble_InSample_UpHolding%d_UpBuy%d_%dbp_cost.xlsx' % (max_holding, daily_max_buy,int(10000*cost))
