...
class SignalBasedPortfolioLimitUp(PortfolioStrategyBase):
    ...
    def bar_handler(self):
        ...
        if holding_num >= self.max_holding_num:
            return
        trigget_num = min(len(trigger_stk), self.daily_max_buy_num,self.max_holding_num//len(self.trading_point))
...
def get_signal_by_val_pct_threshold(pct, subset_path, signal_file_name):
    ...
    for date in point_list:
        ...
        subset = subset.swaplevel(0,1).loc[target_point_list].swaplevel(0,1)
    ...
    pred_ret = pred_ret.pivot_table(index=['date', 'time'], columns='code', values='prediction').replace(-1, 0).fillna(0).sort_index()
...
"""
def get_signal_by_val_pct_dynamic_threshold_integration(pct, subset_path_list, signal_file_name_list):
    # subset_path_list, signal_file_name_list = [subpath,subpath.replace('lr','XGB')],[signal_file,signal_file.replace('lr','XGB')]
    point_set_list = [set(os.listdir(subset_path)) for subset_path in subset_path_list]
    for i in range(len(point_set_list)):
        for j in range(i + 1, len(point_set_list)):
    ...
    for date in point_list:
        temp_subset = {}
        for subset_path in subset_path_list:
            temp_subset[subset_path] = pd.read_pickle(subset_path + '%d.pkl' % date).rename(columns={0: 'actual_label'})
        temp_subset = pd.Panel(temp_subset)
        subset_sum = temp_subset.sum(axis=0)
        subset_count = temp_subset.count(axis=0)
        temp_subset = subset_sum / subset_count
        subset[date] = temp_subset
        # th = (temp_subset[0]<pct).sum()/temp_subset.shape[0]
        # threshold_series.loc[date, 'threshold'] = max(temp_subset['prediction'].quantile(th),0.005)

    pred_ret = {}
    for signal_file_name in signal_file_name_list:
        pred_ret[signal_file_name] = pd.read_pickle(signal_file_name)
    pred_ret = pd.Panel(pred_ret)
    signal_count = pred_ret.count(axis=0)
    signal_sum = pred_ret.sum(axis=0)
    pred_ret = signal_sum / signal_count
    pred_ret[signal_count.eq(0)] = np.nan

    ##############

    date_list = sorted(list(set([x[0] for x in pred_ret.index])))
    start = sorted(list(filter(lambda x: x > min(list(subset.keys())), date_list)))[0]
    date_list = date_list[date_list.index(start):]
    threshold_series = {}

    val_set = None
    val_date_list = None
    val_date_list_series = {}
    record_date_list = [max(list(filter(lambda i: i < date, point_list))) for date in date_list]
    record_val_date = None
    for val_date, date in zip(record_date_list, date_list):
        if record_val_date != val_date:
            val_set = subset[val_date]
            record_val_date = val_date
        th = (val_set['actual_label'] < pct).sum() / val_set.shape[0]
        threshold_series[date] = max(val_set['prediction'].quantile(th), 0.005)
        if val_date_list is None:
            val_date_list = sorted(list(set([x[0] for x in val_set.index])))
        val_date_list_series[date] = val_date_list
        val_date_list = val_date_list[1:]
        val_set = val_set.loc[val_date_list]
        val_set = val_set.append(pred_ret.loc[[date]])
        val_date_list.append(date)

    threshold_series = pd.DataFrame({'threshold': threshold_series, 'val_date_list': {x: '_'.join(sorted(map(str, val_date_list_series[x]))) for x in val_date_list_series}})

    pred_ret = pred_ret.reset_index().pivot_table(index=['level_0', 'level_2'], columns='level_1', values='prediction')  # .replace(-1, 0).fillna(0).sort_index()

    pred_ret['date'] = [x[0] for x in pred_ret.index]
    pred_ret['time'] = [x[1] for x in pred_ret.index]
    pred_ret['index'] = pred_ret['date'].apply(lambda x: max(list(filter(lambda i: i < x, point_list))))
    signal = pd.merge(pred_ret, threshold_series[['threshold']].reset_index(), 'left', 'index').set_index(['date', 'time'])

    signal = (signal.drop(['index', 'threshold'], axis=1).T - signal['threshold']).T
    signal = (signal > 0)

    return signal, pred_ret, threshold_series
...
def main(max_holding,daily_max_buy):
    ...
    #'/data/group/800319/Faamonitor/PL/all_mkt_ts_norm/5min/FactorEval/XGB5minFactorEval_ic_all_t_train200_test10_factor_num100_norm_window_40.pkl'

    # signal_file = '/data/group/800319/Faamonitor/PL/all_mkt_ts_norm/reg_norm_window_compare/XGB5min68_origin_train%d_test%d_factor_num%d_norm_window_%d.pkl' % (train_period, test_period, factor_num,N)
    subpath = signal_file.replace('.pkl', '_val_pred/')#.replace('XGB5min68','XGB5min68_retest')
    # best_param_clf_xgb['train_pred_path'] = signal_file.replace('.pkl','_train_pred/')
    signal, pred_ret = get_signal_by_val_pct_threshold(pct_threshold, signal_file.replace('.pkl', '_val_pred/'), signal_file)
    pd.to_pickle([signal, pred_ret], '/data/user/015664/AFuckingTrigger/saved_signal/XGB5min30BarFactorEval_ICT_signal_%d_threshold.pkl'%(int(pct_threshold*100)))
    # signal, pred_ret = pd.read_pickle('/data/user/015664/AFuckingTrigger/saved_signal/XGB5minNewLoading180_v2_signal.pkl')
    # pd.to_pickle(signal,'/data/user/015664/AFuckingTrigger/XGB5min_signal.pkl')
    # signal,pred_ret,threshold = get_signal_by_val_pct_dynamic_threshold_integration(pct_threshold, sp_list, fnm_list)
    ...
    pred_ret = pred_ret.swaplevel(0,1).loc[target_point_list].swaplevel(0,1)
    instance = SignalBasedPortfolioLimitUp(pred_ret, 20160101, 20181231, target_point=target_point_list,
    ...
    # pd.to_pickle(record, '/data/user/015664/AFuckingTrigger/port_limit_record.pkl')
    helper.one_wave_run(record, kernel=24,
                        output_path='/data/user/015664/AFuckingTrigger/限制买入和持仓/5min/XGB5minFactorEvalRev240_ICTf103_UpHolding%d_UpBuy%d_threshold_%dpct.xlsx'%(max_holding,daily_max_buy,int(pct_threshold*100)),
                        signal_record_save=True)
    print(time.time() - e)

target_point_list = [1000, 1030, 1100, 1300, 1330, 1400, 1430]
para_list = []
