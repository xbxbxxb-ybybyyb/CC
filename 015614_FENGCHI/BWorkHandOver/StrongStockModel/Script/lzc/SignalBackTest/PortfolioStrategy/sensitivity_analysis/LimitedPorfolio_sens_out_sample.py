...
class SignalBasedPortfolioLimitUp(PortfolioStrategyBase):
    ...
    def __init__(self, signal, start=20140101, end=20181231, stock_pool=None, target_point=None, buy_cost=0.001, sell_cost=0.001, per_amt=500000,append_param={}, max_holding=600, daily_max_buy=400):
...
def get_signal_by_val_pct_threshold_integration(pct, subset_path_list, signal_file_name_list):
    ...
    for i in range(len(point_set_list)):
        for j in range(i + 1, len(point_set_list)):
            point_list = sorted(list(point_set_list[i]))
            if point_set_list[i] == point_set_list[j]:
                point_list = sorted(list(point_set_list[i]))
            else:
                print('something wrong')
                # raise Exception('Change point of models are not match!!')
    point_list_dict = {x:sorted(os.listdir(x)) for x in subset_path_list}
    # point_list = os.listdir(subset_path_list[0])
    point_list = sorted([int(x[:-4]) for x in point_list])
    threshold_series = pd.DataFrame(columns=['threshold'], index=point_list)
    for idx,date in enumerate(point_list):
        subset = {}
        for subset_path in subset_path_list:
            subset[subset_path] = pd.read_pickle(subset_path + point_list_dict[subset_path][idx])
...
def main(max_holding, daily_max_buy):
    ...
    sp_list, fnm_list = [], []
    for signal_file in ['/data/group/800319/Faamonitor/PL/all_mkt_ts_norm/out_of_sample/lr/lr_train199_test1_factor_num400_norm_window_40.pkl',
                   '/data/group/800319/Faamonitor/PL/all_mkt_ts_norm/out_of_sample/NN/NNOutSample_train199_test1_factor_num400_norm_window_40.pkl',
                   '/data/group/800319/Faamonitor/PL/all_mkt_ts_norm/out_of_sample/XGB/XGB_train199_test1_factor_num400_norm_window_40.pkl']:
        # signal_file = '/data/group/800319/Faamonitor/PL/all_mkt_ts_norm/out_of_sample/lr/lr_train199_test1_factor_num400_norm_window_40.pkl'
        # signal_file = '/data/group/800319/Faamonitor/PL/all_mkt_ts_norm/NN_param_optimization/NNCorrStdAllPeriod_train%d_test%d_factor_num%d_norm_window_%d.pkl' % \
        #               (train_period, test_period, factor_num, N)
    ...
    signal, pred_ret = get_signal_by_val_pct_threshold_integration(pct_threshold, sp_list, fnm_list)
    ...
    deal_price_para_list = ['vwap_%dmin'%window for window in [5,10,20,30]]#['twap_%dmin' % window for window in [5, 20, 30]] + ['vwap_%dmin' % window for window in [5, 10, 20, 30]]
    deal_price_path = '/data/group/800319/junkData/StrongStock/deal_price/deal_price_%s_20190102_20201030.pkl' % deal_price_para_list[idx]
    print(deal_price_path)
    instance = SignalBasedPortfolioLimitUp(pred_ret, 20190101, 20201031, target_point=[1000, 1030, 1100, 1300, 1330, 1400, 1430],
                                           buy_cost=cost, sell_cost=cost, max_holding=max_holding, daily_max_buy=daily_max_buy,
                                           append_param={'deal_price_path':deal_price_path})
    ...
    pd.to_pickle(record, '/data/user/015664/AFuckingTrigger/限制买入和持仓/敏感性分析/record/%s_record_outsample_lr_XGB_NN.pkl'%deal_price_para_list[idx])
    helper.one_wave_run(record, kernel=24, output_path='/data/user/015664/AFuckingTrigger/限制买入和持仓/敏感性分析/lr_XGB_integration_OutSample_UpHolding%d_UpBuy%d_%dbp_cost.xlsx' % (max_holding, daily_max_buy,int(cost*10000)),
