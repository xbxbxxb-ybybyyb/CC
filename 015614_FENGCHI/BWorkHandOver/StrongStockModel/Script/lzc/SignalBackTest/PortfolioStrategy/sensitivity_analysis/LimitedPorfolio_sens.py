...
def main(max_holding, daily_max_buy):
    ...
    N = 40
    sp_list, fnm_list = [], []
    model_list = ['XGB','lr','NNCorrStdAllPeriod']
    ...
    deal_price_path = '/data/group/800319/junkData/StrongStock/deal_price/deal_price_%s_20160104_20181228.pkl'%deal_price_para_list[idx]
    ...
    pd.to_pickle(record, '/data/user/015664/AFuckingTrigger/限制买入和持仓/敏感性分析/record/%s_record_insample_lr_XGB_NN.pkl'%deal_price_para_list[idx])
    helper.one_wave_run(record, kernel=24, output_path='/data/user/015664/AFuckingTrigger/限制买入和持仓/敏感性分析/%sInSample_UpHolding%d_UpBuy%d_%dbp_cost.xlsx' % ('_'.join(model_list),max_holding, daily_max_buy,int(10000*cost)),
