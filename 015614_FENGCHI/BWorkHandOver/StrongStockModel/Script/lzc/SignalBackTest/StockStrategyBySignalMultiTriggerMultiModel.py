# @Time : 2020/10/12 8:32
# @Author : Zhichen Lu
# @File : StockStrategyBySignalMultiTriggerMultiModel.py
...
#    is_valid = strong_stock
    ...
    print(file_name)
    base_path = root_path + 'backtest_result_all_mkt_%dbp_cost_new_framework20201009/'%int(cost*10000)
    if not os.path.exists(base_path):
        os.mkdir(base_path)
    # output_path = root_path + base_path+'%s_%.2f.xlsx' % (file_name,th)
    output_path = base_path+'%s_%d_pct_val_threshold.xlsx' % (file_name,int(pct_threshold*100))
...
#    signal_file = '/data/group/800319/Faamonitor/PL/all_mkt_ts_norm/reg_ic_factor_selection/lr_train%d_test%d_factor_num%d.pkl' % (train_period, test_period, factor_num)
    ...
    sp_list, fnm_list = [],[]
    for md in ['lr','XGB','NN_redefine_hyper04']:
    ...
    signal_file = fnm_list[0].replace('lr','lr_XGB_NN_integration')
