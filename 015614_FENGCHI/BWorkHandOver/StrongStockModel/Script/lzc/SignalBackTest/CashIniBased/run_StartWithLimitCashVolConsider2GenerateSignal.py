...
def calc(pct_threshold,per_amt_ratio):
    ...
    if os.path.exists('/data/user/015664/AFuckingTrigger/限制买入和持仓/OnlineEraSensitivity/record/record_%sVolConsider_UpBuy100_%dbp_cost.pkl' % (tag, int(10000 * cost))):
        print(tag,'record exist')
        return True
    ...
    pd.to_pickle([record,cash_series,holding_num], '/data/user/015664/AFuckingTrigger/限制买入和持仓/OnlineEraSensitivity/record/record_%sVolConsider_UpBuy100_%dbp_cost.pkl' % (tag, int(10000 * cost)))
    return  True
...
total = 7
import gc
for each in para_list[i*len(para_list)//total:(i+1)*len(para_list)//total]:
    calc(*each)
    gc.collect()
