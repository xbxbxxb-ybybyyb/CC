...
# @Author : Zhichen Lu
# @File : run_StartWithLimitCashSingleModel.py
...
para={
    ...
    'XGB_D':list(filter(lambda x : 'ic_all_d' in x,file_list_xgb)),
    'XGB_T':list(filter(lambda x : 'ic_all_t' in x,file_list_xgb)),
    'XGB_C':list(filter(lambda x : 'ic_all_c' in x,file_list_xgb))
}
per_amt_ratio = 0.005
tag = 'XGB_D'
