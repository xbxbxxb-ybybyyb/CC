...
def calc_back_test_record(pct_threshold, per_amt_ratio, initial_cash, pool_num, deal_ratio, tag, alpha_pool_tag):
    ...
    if pool_dict[alpha_pool_tag] is None:
        ...
        if pool_dict[alpha_pool_tag].shape[0]>0:
            alpha_pool = pool_dict[alpha_pool_tag].shift(1).loc[start:end]
        else:
            alpha_pool = pool_dict[alpha_pool_tag]
...
#all_mkt
#calc_back_test_record(*(each + (0.1, 'XGBMonthlyV4_Cat_Light_Val', 'all_mkt')))
calc_back_test_record(*(each + (0.1, 'XGBWithSWSHIFReSaveTWithOrigin_Cat_Light', 'all_mkt')))
#calc_back_test_record(*(each + (0.1, 'XGBMonthlyV4_Cat_Light_Val', 'XGB_OLS_style_ex20')))
