...
def calc_back_test_record(pct_threshold, per_amt_ratio, initial_cash, pool_num, deal_ratio, tag, alpha_pool_tag):
    ...
    signal_file = '/data/user/015664/AFuckingTrigger/限制买入和持仓/信号存储/signal_newfunc_%s_%.2f.pkl' % (tag, pct_threshold)
    if os.path.exists(signal_file):
        ...
        #                                                                'new',
        #                                                                head=135)
        res = get_signal_by_val_pct_threshold_long_integration(pct_threshold, [x.replace('.pkl', '_val_pred/') for x in file_list],
                                                                            [x.replace('.pkl', '/') for x in file_list], start,
                                                                                      'actual_label',
                                                                                      'new',
                                                                                      head=None, end=end)
