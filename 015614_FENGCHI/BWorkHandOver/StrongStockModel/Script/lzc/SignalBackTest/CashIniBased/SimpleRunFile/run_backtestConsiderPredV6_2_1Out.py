...
def calc_back_test_record(pct_threshold, per_amt_ratio, initial_cash, pool_num, deal_ratio, tag, alpha_pool_tag,down_signal_ratio,signal_threshold):
    ...
    out_path = '/data/user/015664/AFuckingTrigger/限制买入和持仓/PredConsiderV6_2_1PrameterSeekingOutSample/%sVolConsider_UpBuy100_%dbp_cost.xlsx' % (tag, int(10000 * cost))
...
if __name__ == '__main__':
    ...
    import itertools
    para_list = [(-1,1)]+list(itertools.product([round(x*0.001,3) for x in range(-20,21,4)], [0.05,0.1,0.15,0.2,0.25,0.3,0.35,0.4]))
    from xquant.compute.aimr import AIMR
    d_threshold,s_threshold = (-1,1)#eval(AIMR.getParam())#(0.2,1)
    import datetime
