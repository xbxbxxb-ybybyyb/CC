...
def calc_back_test_record(pct_threshold, per_amt_ratio, initial_cash, pool_num, deal_ratio, tag, alpha_pool_tag):
    ...
    high = get_daily_1factor('high',code_list=pred_ret.columns.tolist())
    low = get_daily_1factor('low',code_list=pred_ret.columns.tolist())
    close = get_daily_1factor('close',code_list=pred_ret.columns.tolist())
    out_condition = (((high - close)/(high - low))<0.3) & (high>10)
    out_condition = out_condition.shift(1).loc[get_date_range(start,end)].fillna(False)
    import itertools
    fix_index = list(itertools.product(out_condition.index.tolist(),bar_list))
    out_condition.index = [(x,1000) for x in out_condition.index]
    out_condition = out_condition.reindex(fix_index).fillna(method='pad')
    out_condition.index = pd.MultiIndex.from_tuples(out_condition.index.tolist())
    pred_ret = pred_ret.loc[start:end]
    pred_ret[~out_condition] = np.nan
...
pool_dict['次新股']  = pd.read_pickle('/data/group/800319/Faamonitor/次新股.pkl')
pool_dict['次新股'].index = pool_dict['次新股'].index.map(int)
...
calc_back_test_record(*(each + (0.1, 'XGBMonthly_DTC', '次新股')))
