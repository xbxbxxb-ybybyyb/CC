...
from StrongStockModel.backtest.StrategyBackTest.DerivativeStrategy.StartWithLimitCashVolConsiderPredConsiderV3_1 import StartWithLimitCashVolConsider, InitailCashBasedEvaluationHelper
...
def calc_back_test_record(pct_threshold, per_amt_ratio, initial_cash, pool_num, deal_ratio, tag, alpha_pool_tag,
    ...
    out_path = '/data/user/015664/AFuckingTrigger/限制买入和持仓/PredConsiderV3_1PoolPerSigPrameterSeeking/%sVolConsider_UpBuy100_%dbp_cost.xlsx' % (tag, int(10000 * cost))
...
if __name__ == '__main__':
    ...
    import itertools
    # para_list = [(1,1,0)]+list(itertools.product([0.3,0.5,0.7,0.9],[0.03,0.05,0.07,0.1,0.13,0.15,0.2],[-0.01,-0.005,0,0.005,0.01]))
    para_list = list(itertools.product([0.005,0.01,0.015,0.2],[200,400,600]))
    from xquant.compute.aimr import AIMR
    d_threshold,s_ratio,sw_thre = (0.3,0.15,0)#eval(AIMR.getParam())#(0.2,1)
    per_sig_ratio,p_num = eval(AIMR.getParam())
    import datetime
    calc_back_test_record(0.05, per_sig_ratio, 2e8, p_num, 0.1, 'XGBWithSWSHIFReSaveTWithOrigin_Cat_Light', 'CS_XGB_OLS_condition_style_rank_ex20',
