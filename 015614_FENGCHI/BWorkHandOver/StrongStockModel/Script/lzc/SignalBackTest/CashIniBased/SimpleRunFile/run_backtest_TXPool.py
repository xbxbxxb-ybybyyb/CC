...
def calc_back_test_record(pct_threshold, per_amt_ratio, initial_cash, pool_num, deal_ratio, tag, alpha_pool_tag):
    ...
    target_file = f'/data/user/015664/AFuckingTrigger/限制买入和持仓/信号存储/signal_{tag}_{pct_threshold:.2f}_{start}_{end}.pkl'
    # target_before = list(filter(lambda x : x.startswith('XGBMonthlyV4_Cat_Light_Val'),os.listdir('/data/user/015664/AFuckingTrigger/限制买入和持仓/信号存储/')))
    # signal_before,pred_ret_before = pd.read_pickle('/data/user/015664/AFuckingTrigger/限制买入和持仓/信号存储/signal_XGBMonthlyV4_Cat_Light_ValWithoutMax5_0.05.pkl')
    # signal_after,pred_ret_after = pd.read_pickle('/data/user/015664/AFuckingTrigger/限制买入和持仓/信号存储/signal_XGBMonthlyV4_Cat_Light_ValTXWithoutMax5_0.05_20210301_20210715.pkl')
    # signal = pd.concat([signal_before.loc[:20210226],signal_after])
    # pred_ret = pd.concat([pred_ret_before.loc[:20210226],pred_ret_after])
    # pd.to_pickle([signal,pred_ret],f'/data/user/015664/AFuckingTrigger/限制买入和持仓/信号存储/signal_{tag}_{pct_threshold:.2f}_{20170103}_{end}.pkl')

    if os.path.exists(target_file):
        print(pct_threshold, 'signal exist')
        signal, pred_ret = pd.read_pickle(target_file)
    else:
        ...
        signal, pred_ret = get_signal_by_val_pct_threshold_integration_NoMaxThreshold(pct_threshold, [x.replace('.pkl', '_val_pred/') for x in file_list], file_list, start,
                                                                                      ...
                                                                                      'new',
                                                                                      head=None, end=end)
        pd.to_pickle([signal, pred_ret], target_file)
    ...
    if pool_dict[alpha_pool_tag] is None:
        ...
        alpha_pool = pool_dict[alpha_pool_tag].loc[start:end]
    ...
    if alpha_pool_tag=='CS_XGB_OLS_condition_style_rank_ex20':
        code_list_path = '/data/group/800319/strategy_local_path3_protect20210827/code_list/'
        append_pool = []
        for date in get_date_range(20210402, end):
            temp_pool = pd.read_pickle(f'{code_list_path}{date}.pkl')
            temp_pool = pd.DataFrame(True, index=[date], columns=temp_pool)
            append_pool.append(temp_pool)
        append_pool = pd.concat(append_pool)
        append_pool.columns = [int(x[:-3]) for x in append_pool.columns]
        append_pool = append_pool.reindex(alpha_pool.columns, axis=1).fillna(False).shift(1).loc[20210406:]
        append_pool = append_pool.drop(list(set(alpha_pool.index).intersection(set(append_pool.index))), axis=0)
        alpha_pool = pd.concat([alpha_pool,append_pool]).fillna(False)
    ...
    out_path = '/data/group/800442/800319/BackTestResForTX/%s.xlsx' % tag[:50]
    for each in record:
    ...
    send_file(['015664','015624'], out_path)
...
pool_dict['TXPOOL'] = pd.read_pickle('/data/group/800442/800319/Test_Pool_tx.pkl')
old_alpha_pool = pd.read_pickle('/data/group/800442/800319/AlphaPool/alpha20210402.pkl')
pool_dict['TXPOOL&OldAlphaPool'] = old_alpha_pool&pool_dict['TXPOOL']


each = (0.05, 0.005, 2e8, 600)
# calc_back_test_record(*(each + (0.1, 'XGBMonthlyV4_Cat_Light_ValTX', 'CS_XGB_OLS_condition_style_rank_ex20')))
for i in range(1,2):
    pool_dict['Pool%d'%i] = pd.read_pickle('/data/group/800442/800319/pool%d.pkl'%i)
    calc_back_test_record(*(each + (0.1, 'XGBMonthlyV4_Cat_Light_ValTX', 'Pool%d'%i)))

# calc_back_test_record(*(each + (0.1, 'XGBMonthlyV4_Cat_Light_ValTX', 'TXPOOL&Alpha')))
# send_file(['00155'],'都跑完啦！')