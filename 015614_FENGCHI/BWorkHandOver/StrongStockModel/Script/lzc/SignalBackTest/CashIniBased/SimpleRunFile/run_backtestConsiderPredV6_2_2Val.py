...
from StrongStockModel.backtest.StrategyBackTest.DerivativeStrategy.StartWithLimitCashVolConsiderPredConsiderV6_2_2Val import StartWithLimitCashVolConsider, InitailCashBasedEvaluationHelper
...
def calc_back_test_record(pct_threshold, per_amt_ratio, initial_cash, pool_num, deal_ratio, tag, alpha_pool_tag,down_signal_ratio,signal_threshold):
    ...
    out_path = '/data/user/015664/AFuckingTrigger/限制买入和持仓/PredConsiderV6_2_2ValPrameterSeekingInSample/%sVolConsider_UpBuy100_%dbp_cost.xlsx' % (tag, int(10000 * cost))
...
if __name__ == '__main__':
    ...
    d_threshold,s_threshold = (0,0.15)#eval(AIMR.getParam())#(0.2,1)
    import datetime
    ...
    if False:#now.strftime('%H%M') > '0610' and now.strftime('%H%M') < '0930' and get_recent_trade_date(int(now.strftime('%Y%m%d'))) == int(now.strftime('%Y%m%d')):
        pass
    else:
