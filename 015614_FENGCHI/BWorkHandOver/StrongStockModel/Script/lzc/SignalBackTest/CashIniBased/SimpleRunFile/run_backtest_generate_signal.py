...
for each in wyl_list:
    new_tag = os.path.split(each)[-1].replace('lightgbm','').replace('.pkl','')
    para[f'With{new_tag}'] = para['XGBWithSWSHIFReSaveTWithOrigin_Cat_Light']+[each]

para['WithAllLGB'] = para['XGBWithSWSHIFReSaveTWithOrigin_Cat_Light'] + wyl_list
...
target_list = list(filter(lambda x : x.startswith('With'),list(para.keys())))
...
for key_val in target_list:
    # calc_back_test_record(*(each + (0.1, key_val, 'CS_XGB_OLS_condition_style_rank_ex20')))
    # gc.collect()
...
send_message(['015664'],'Signal Done')