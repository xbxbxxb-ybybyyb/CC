...
def calc_back_test_record(pct_threshold, per_amt_ratio, initial_cash, pool_num, deal_ratio, tag, alpha_pool_tag):
    ...
    tag = tag + f'{start}_{end}Val20'  # +'WithMax5threshold' #
        ...
        for each in file_list:
            if not os.path.exists(each):
                ...
                if check.index[-1][0]<end:
                    out_signal(each.replace('.pkl', '/'))
        signal, pred_ret = get_signal_by_val_pct_threshold_integration_NoMaxThreshold(pct_threshold, [x.replace('.pkl', '_val_predrecent_5/') for x in file_list], file_list, start,
                                                                                      'actual_label',
                                                                                      'new',
                                                                                      head=None, end=end)
    ...
    res_date_list = sorted(list(set(date_list) - set(alpha_pool.index)))
    res_date_list = [get_pre_trade_date(res_date_list[0])]+res_date_list
    append_pool = []
    for date in res_date_list:
        temp_pool = pd.read_pickle(f'/data/group/800319/strategy_local_path3/code_list/{date}.pkl')
        temp_pool = pd.DataFrame(True, index=[date], columns=temp_pool)
        append_pool.append(temp_pool)
    append_pool = pd.concat(append_pool).fillna(False).shift(1).loc[res_date_list[1]:res_date_list[-1]]
    append_pool.columns = append_pool.columns.map(lambda x : int(x[:6]))
    alpha_pool = pd.concat([alpha_pool,append_pool]).sort_index() > 0.5
...
calc_back_test_record(*(each + (0.1, 'XGBMonthly_Recent5', 'CS_XGB_OLS_condition_style_rank_ex20')))
