...
from StrongStockModel.backtest.StrategyBackTest.DerivativeStrategy.StartWithLimitCashVolConsiderPredConsiderV4 import StartWithLimitCashVolConsider, InitailCashBasedEvaluationHelper
...
def calc_back_test_record(pct_threshold, per_amt_ratio, initial_cash, pool_num, deal_ratio, tag, alpha_pool_tag,
    ...
    out_path = '/data/user/015664/AFuckingTrigger/限制买入和持仓/PredConsiderV4PrameterSeekingInSample/%sVolConsider_UpBuy100_%dbp_cost.xlsx' % (tag, int(10000 * cost))
...
if __name__ == '__main__':
    ...
    import itertools
    para_list = list(itertools.product([ 0.5, 0.6, 0.7, 0.8, 0.9], [0.1,0.15,0.2,0.25,0.3], [-0.01, -0.005, 0, 0.005, 0.01]))

    from xquant.compute.aimr import AIMR
    d_threshold,s_ratio,sw_thre = para_list[0]#eval(AIMR.getParam())#(0.2,1)
    import datetime
    'XGBWithSWSHIFReSaveTWithOrigin_Cat_Light'
    if False:#"#datetime.datetime.now()>datetime.datetime(2021, 12, 13, 6, 15, 15, 698337):
        pass
    else:
