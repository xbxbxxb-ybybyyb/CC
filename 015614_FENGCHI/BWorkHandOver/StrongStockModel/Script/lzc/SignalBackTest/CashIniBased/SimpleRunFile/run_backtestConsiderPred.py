...
from StrongStockModel.backtest.StrategyBackTest.DerivativeStrategy.StartWithLimitCashVolConsiderPredConsider import StartWithLimitCashVolConsider, InitailCashBasedEvaluationHelper
...
def calc_back_test_record(pct_threshold, per_amt_ratio, initial_cash, pool_num, deal_ratio, tag, alpha_pool_tag, signal_threshold, down_threshold):
    file_list = para[tag]
    print(file_list)
    start = 20170101
    end = 20210531
    ...
    tag = f'{tag}_deal{deal_ratio:.2f}_per{per_amt_ratio * 1000}bp_thre{int(pct_threshold * 100)}_cash{initial_cash:.0e}_Top600_start{start}_end{end}_' \
        f's_thre{signal_threshold:.2f}_d_thre{down_threshold:.2f}'
    out_path = '/data/user/015664/AFuckingTrigger/限制买入和持仓/PredConsiderPrameterSeeking/%sVolConsider_UpBuy100_%dbp_cost.xlsx' % (tag, int(10000 * cost))
    ...
    if not os.path.exists(
        ...
        instance = StartWithLimitCashVolConsider(pred_ret, start, end, stock_pool=alpha_pool, target_point=bar_list, buy_cost=cost, sell_cost=cost,
                                                 ...
                                                 stk_min_amt=stk_min_amt, signal_threshold=signal_threshold, down_threshold=down_threshold)
...
import itertools
para_list = list(itertools.product([0.2,0.3,0.4],[0.5,0.75,1,1.25,1.5,1.75,2]))#+(1,4000)
...
calc_back_test_record(0.05, 0.005, 2e8, 600,0.1, 'XGBMonthlyV4_Cat_Light_Val', 'CS_XGB_OLS_condition_style_rank_ex20',
                      down_threshold=d_threshold,signal_threshold=s_ratio)
