...
from StrongStockModel.backtest.StrategyBackTest.DerivativeStrategy.StartWithLimitCashVolConsiderPredConsiderV4_1 import StartWithLimitCashVolConsider, InitailCashBasedEvaluationHelper
...
def calc_back_test_record(pct_threshold, per_amt_ratio, initial_cash, pool_num, deal_ratio, tag, alpha_pool_tag,
    ...
    out_path = '/data/user/015664/AFuckingTrigger/限制买入和持仓/PredConsiderV4_1PoolPerSigPrameterSeeking/%sVolConsider_UpBuy100_%dbp_cost.xlsx' % (tag, int(10000 * cost))
...
if __name__ == '__main__':
    ...
    d_threshold, s_ratio, sw_thre = (0.5, 0.25, 0)  # eval(AIMR.getParam())#(0.2,1)
    per_sig_ratio, p_num = eval(AIMR.getParam())
    import datetime
    now = datetime.datetime.now()
    if now.strftime('%H%M') > '0610' and now.strftime('%H%M') < '0930':
        pass
    else:
