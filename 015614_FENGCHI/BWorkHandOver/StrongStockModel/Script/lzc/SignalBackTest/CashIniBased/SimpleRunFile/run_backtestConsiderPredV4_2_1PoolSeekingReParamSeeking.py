...
def calc_back_test_record(pct_threshold, per_amt_ratio, initial_cash, pool_num, deal_ratio, tag, alpha_pool_tag,
    ...
    out_path = '/data/user/015664/AFuckingTrigger/限制买入和持仓/PredConsiderV4_2_1PoolSeekingReParamSeeking/%sVolConsider_UpBuy100_%dbp_cost.xlsx' % (tag, int(10000 * cost))
    ...
    if not os.path.exists(f'{record_path}/{record_name}'):
...
if __name__ == '__main__':
    ...
    para_list = [(600,0.005,0.1,1,1,-1)]+list(itertools.product([600], [ 0.01, 0.015], [0.2],[0.3,0.4,0.45,0.5,0.55, 0.6, 0.7, 0.8, 0.9], [0.1, 0.15, 0.2, 0.25, 0.3], [-0.01, -0.005, 0, 0.005, 0.01]))


    # d_threshold,s_ratio,sw_thre = 0.2,0.5,0#(0.4,0.1,0)#eval(AIMR.getParam())#(0.2,1)
    # para_list = list(itertools.product([200, 300, 400, 500, 600], [0.005, 0.007, 0.01, 0.013, 0.015], [0.1, 0.2, 0.3, 0.4]))
    p_num,p_ratio,d_ratio,d_threshold,s_ratio,sw_thre = eval(aimr_multitimes.getParam())
    from dataApi.tradeDate import get_recent_trade_date
    import datetime
    now = datetime.datetime.now()
    HHMM = int(now.strftime('%H%M'))
    no_running_period = [(610, 900), (1555, 1930)]
