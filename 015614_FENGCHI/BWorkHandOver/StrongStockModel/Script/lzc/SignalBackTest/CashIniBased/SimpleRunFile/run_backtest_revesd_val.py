...
from StrongStockModel.model.ModelResultLoadingTool import get_signal_by_revised_distribution
import pandas as pd
...
def calc_back_test_record(pct_threshold, per_amt_ratio, initial_cash, pool_num, deal_ratio, tag, alpha_pool_tag):
    ...
    tag = tag + f'{start}_{end}ReviedDistribution'  # +'WithMax5threshold' #
    ...
    if os.path.exists(signal_file):
        ...
        #                                                                'new',
        #                                                                head=135)
        signal,pred_ret = get_signal_by_revised_distribution(pct_threshold, [x.replace('.pkl', '_val_pred/') for x in file_list],
                                                             file_list, start,get_subset=False,head=None,tail=None,end=end)
