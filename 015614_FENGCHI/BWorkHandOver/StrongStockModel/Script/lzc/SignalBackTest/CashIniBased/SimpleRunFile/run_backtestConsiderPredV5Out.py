...
from StrongStockModel.backtest.StrategyBackTest.DerivativeStrategy.StartWithLimitCashVolConsiderPredConsiderV5 import StartWithLimitCashVolConsider, InitailCashBasedEvaluationHelper
...
def calc_back_test_record(pct_threshold, per_amt_ratio, initial_cash, pool_num, deal_ratio, tag, alpha_pool_tag,down_signal_ratio):
    file_list = para[tag]
    print(file_list)
    start = 20200102
    end = 20210531
    ...
    tag = f'{tag}_deal{deal_ratio:.2f}_per{per_amt_ratio * 1000}bp_thre{int(pct_threshold * 100)}_cash{initial_cash:.0e}_Top600_start{start}_end{end}_' \
        f'd_thre{down_signal_ratio:.2f}'
    out_path = '/data/user/015664/AFuckingTrigger/限制买入和持仓/PredConsiderV5PrameterSeekingOutSample/%sVolConsider_UpBuy100_%dbp_cost.xlsx' % (tag, int(10000 * cost))
    ...
    if not os.path.exists(
        ...
        instance = StartWithLimitCashVolConsider(pred_ret, start, end, stock_pool=alpha_pool, target_point=bar_list, buy_cost=cost, sell_cost=cost,
                                                 ...
                                                 stk_min_amt=stk_min_amt, index_pct_change=down_signal_ratio)
...
if __name__ == '__main__':
    ...
    import itertools
    para_list = [round(x*0.001,3) for x in range(-20,21,2)]
    from xquant.compute.aimr import AIMR
    d_threshold = eval(AIMR.getParam())#(0.2,1)
    import datetime
    'XGBWithSWSHIFReSaveTWithOrigin_Cat_Light'
