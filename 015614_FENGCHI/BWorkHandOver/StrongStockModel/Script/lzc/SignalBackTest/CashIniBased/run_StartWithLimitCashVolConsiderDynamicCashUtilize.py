...
from StrongStockModel.backtest.StrategyBackTest.DerivativeStrategy.StartWithLimitCashVolConsiderDynamicCashUtilize import StartWithLimitCashVolConsiderDynamicCashUtilize,InitailCashBasedEvaluationHelper
...
tag = tag+'RevTriggerFilterHolding_deal_ratio_%.1f_per_ratio_%s_threshold_%.2f'%(deal_ratio,str(per_amt_ratio),pct_threshold)
pred_ret[~signal] = np.nan
alpha_pool = pd.read_pickle('/data/group/800319/信号存储/daily_stock_score_vTrigger_20210127.pkl').loc[20160101:20181231]
original_pool = pd.read_pickle('/data/group/800319/junkData/StrongStock/stock_pool_without_limit_up_down.pkl').loc[20160101:20181231]
original_pool = original_pool.drop(alpha_pool.index,axis=0)
alpha_pool = pd.concat([original_pool,alpha_pool]).sort_index()>0.5
tag = tag.replace('RevTriggerFilterHolding','RevTriggerFilterHolding_AlphaTriggerPool')
...
instance = StartWithLimitCashVolConsiderDynamicCashUtilize(pred_ret, 20160101, 20181231,stock_pool=alpha_pool,target_point=bar_list,buy_cost=cost, sell_cost=cost,
...
out_path = '/data/user/015664/AFuckingTrigger/限制买入和持仓/NoFutureInfoResDnymicCashUtilize/%sVolConsider_UpBuy100_%dbp_cost.xlsx' % (tag, int(10000 * cost))
