...
if os.path.exists('/data/user/015664/AFuckingTrigger/限制买入和持仓/信号存储/signal_OutSample_%s.pkl'%tag):
    signal, pred_ret = get_signal_by_val_pct_threshold_integration(pct_threshold,[x.replace('.pkl','_val_pred/') for x in file_list],file_list,20190102,'actual_label','new',tail=51)
    pd.to_pickle([signal, pred_ret],'/data/user/015664/AFuckingTrigger/限制买入和持仓/信号存储/signal_OutSample_%s.pkl'%tag)
else:
    signal, pred_ret = pd.read_pickle('/data/user/015664/AFuckingTrigger/限制买入和持仓/信号存储/signal_OutSample_%s.pkl'%tag)
...
instance = StartWithLimitCashVolConsider(pred_ret, 20210105,20210127,target_point=bar_list,buy_cost=cost, sell_cost=cost,
...
# pd.to_pickle(record,'/data/user/015664/AFuckingTrigger/限制买入和持仓/InitialCashBasedVolConsider/record/%sOutSample.pkl'%tag)
...
out_path = '/data/user/015664/AFuckingTrigger/限制买入和持仓/NoFutureInfoResShift/%sVolConsider0105_0127_UpBuy100_%dbp_cost.xlsx' % (tag, int(10000 * cost))
