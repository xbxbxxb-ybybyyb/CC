...
from StrongStockModel.backtest.StrategyBackTest.DerivativeStrategy.StartWithLimitCashVolConsiderPredConsiderV6_2_1 import StartWithLimitCashVolConsider, InitailCashBasedEvaluationHelper
...
def calc_back_test_record(pct_threshold, per_amt_ratio, initial_cash, pool_num, deal_ratio, tag, alpha_pool_tag,down_signal_ratio,signal_threshold):
    ...
    tag = f'{tag}_deal{deal_ratio:.2f}_per{per_amt_ratio * 1000}bp_thre{int(pct_threshold * 100)}_cash{initial_cash:.0e}_Top{pool_num}_start{start}_end{end}_' \
        f's_thre{signal_threshold:.2f}_d_thre{down_signal_ratio:.3f}'
    out_path = '/data/user/015664/AFuckingTrigger/限制买入和持仓/PredConsiderV6_2_1PoolSeeking/%sVolConsider_UpBuy100_%dbp_cost.xlsx' % (tag, int(10000 * cost))
    record_path, record_name = os.path.split(out_path)
    record_name = record_name.replace('.xlsx', 'pkl')
    record_path = f'{record_path}/record/'
    ...
    if True:#not os.path.exists(f'{record_path}/{record_name}'):
        instance = StartWithLimitCashVolConsider(pred_ret, start, end, stock_pool=alpha_pool, target_point=bar_list, buy_cost=cost, sell_cost=cost,
                                                 ...
                                                 stk_min_amt=stk_min_amt, index_pct_change=down_signal_ratio,signal_threshold=signal_threshold)
        ...
        pd.to_pickle([record, cash_series, holding_num],f'{record_path}/{record_name}')
        pd.to_pickle(instance.barly_condition_indicator,f'{record_path}/record_{record_name}')
        from dataApi.sendInfo import send_message
        send_message(['015664'],f'{record_path}/record_{record_name}')
    else:
        record, cash_series, holding_num = pd.read_pickle(f'{record_path}/{record_name}')
    if STAGE==1:
        return
...
if __name__ == '__main__':
    ...
    p_num, p_ratio, d_ratio = 600,0.01,0.3#eval(aimr_multitimes.getParam())
    d_threshold,s_threshold = (-0.008,0.15)#eval(AIMR.getParam())#(0.2,1)
    import datetime
    from dataApi.tradeDate import get_recent_trade_date
    now = datetime.datetime.now()
    HHMM = int(now.strftime('%H%M'))
    no_running_period = [(610, 930)]
    ...
    if forbiden_period and trading_day:
        ...
        calc_back_test_record(0.05, p_ratio, 2e8, p_num, d_ratio, 'XGBWithSWSHIFReSaveTWithOrigin_Cat_Light', 'CS_XGB_OLS_condition_style_rank_ex20',
