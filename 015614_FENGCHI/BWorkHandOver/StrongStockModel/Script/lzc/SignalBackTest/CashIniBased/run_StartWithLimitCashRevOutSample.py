...
#signal,pred_ret = get_signal_by_zscore_integration(file_list[0],threshold=pct_threshold)
signal, pred_ret = get_signal_by_val_pct_threshold_integration(pct_threshold,[x.replace('.pkl','_val_pred/') for x in file_list],file_list,20160104,'actual_label','new',tail=50)
pd.to_pickle([signal, pred_ret],'/data/user/015664/AFuckingTrigger/限制买入和持仓/信号存储/signal_%s_out.pkl'%tag)
...
instance = StartWithLimitCashVolConsider(pred_ret, 20190101,20201031,target_point=bar_list,buy_cost=cost, sell_cost=cost,
...
out_path = '/data/user/015664/AFuckingTrigger/限制买入和持仓/InitialCashBasedVolConsider/%sVolConsider_UpBuy100_%dbp_cost.xlsx' % (tag, int(10000 * cost))
