...
if not os.path.exists('/data/user/015664/AFuckingTrigger/限制买入和持仓/信号存储/signal_OutSample2021_%s.pkl' % tag):
    signal, pred_ret = get_signal_by_val_pct_threshold_integration(pct_threshold, [x.replace('.pkl', '_val_pred/') for x in file_list], file_list, 20210105, 'actual_label', 'new',
                                                                   tail=4)
    pd.to_pickle([signal, pred_ret], '/data/user/015664/AFuckingTrigger/限制买入和持仓/信号存储/signal_OutSample2021_%s.pkl' % tag)
else:
    signal, pred_ret = pd.read_pickle('/data/user/015664/AFuckingTrigger/限制买入和持仓/信号存储/signal_OutSample2021_%s.pkl' % tag)

from dataApi.tradeDate import get_date_range
real_time_path = '/data/group/800002/realtime/alpha/market_data/'
limit_status = pd.DataFrame()
for date in get_date_range(20210105, 20210223):
...
pool_list = ['20210224AlphaPool',
 '20210224AlphaOrActivePool',
 '20210224ActivePool']
pool_name = pool_list[2]
alpha_pool = pd.read_pickle('/data/group/800319/信号存储/%s.pkl'%pool_name).shift(1).loc[20210105:20210223]
original_pool = pd.read_pickle('/data/group/800319/junkData/StrongStock/stock_pool_without_limit_up_down.pkl').shift(1).loc[20210105:20210223]
...
tag = tag.replace('RevTriggerFilterHolding', f'{pool_name}Top600')


instance = StartWithLimitCashVolConsider(pred_ret, 20210105, 20210223, stock_pool=alpha_pool, target_point=bar_list, buy_cost=cost, sell_cost=cost,
...
pd.to_pickle([record,cash_series,holding_num],f'/data/user/015664/AFuckingTrigger/限制买入和持仓/PoolCompare/record/record_{tag}.pkl')
...
out_path = '/data/user/015664/AFuckingTrigger/限制买入和持仓/PoolCompare/%sVolConsiderOnlineLimit_UpBuy100_%dbp_cost.xlsx' % (tag, int(10000 * cost))
