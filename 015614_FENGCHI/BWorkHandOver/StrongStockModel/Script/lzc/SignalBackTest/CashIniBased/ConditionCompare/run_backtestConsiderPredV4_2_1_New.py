...
from StrongStockModel.backtest.StrategyBackTest.DerivativeStrategy.StartWithLimitCashVolConsiderPredConsiderV7ChangableCash import StartWithLimitCashVolConsider, InitailCashBasedEvaluationHelper
...
def calc_back_test_record(pct_threshold, per_amt_ratio, initial_cash, pool_num, deal_ratio, tag, alpha_pool_tag,
    ...
    tag = f'New_{tag}_deal{deal_ratio:.2f}_per{per_amt_ratio * 1000}bp_thre{int(pct_threshold * 100)}_cash{initial_cash:.0e}_Top600_start{start}_end{end}_' \
        f's_thre{signal_threshold:.2f}_d_thre{down_signal_ratio:.2f}_swing{swing_threshold:.3f}'
    out_path = '/data/user/015664/AFuckingTrigger/盘中终止条件前后对比/PredConsiderV4_2_1FullSample/%sVolConsider_UpBuy100_%dbp_cost.xlsx' % (tag, int(10000 * cost))
    print(out_path)
    ...
    if not os.path.exists(
        ...
        instance = StartWithLimitCashVolConsider(pred_ret, start, end, stock_pool=alpha_pool, target_point=bar_list, buy_cost=cost, sell_cost=cost,
                                                 ...
                                                 stk_min_amt=stk_min_amt,down_swing_threshold=swing_threshold,condition_series=condition_series)
        ...
        pd.to_pickle([record, cash_series, holding_num,instance.barly_condition_indicator],
                     '/data/user/015664/AFuckingTrigger/盘中终止条件前后对比/PredConsiderV4_2_1FullSample/record/record_%sVolConsider_UpBuy100_%dbp_cost.pkl' % (tag, int(10000 * cost)))
    else:
...
if __name__ == '__main__':
    ...
    import itertools
    para_list = list(itertools.product([ 0.3,0.4,0.5, 0.6, 0.7, 0.8, 0.9], [0.1,0.15,0.2,0.25,0.3], [-0.01, -0.005, 0, 0.005, 0.01]))

    from xquant.compute.aimr import AIMR
    d_threshold,s_ratio,sw_thre = (0.5,0.15,0)#eval(AIMR.getParam())#(0.2,1)
    import datetime
