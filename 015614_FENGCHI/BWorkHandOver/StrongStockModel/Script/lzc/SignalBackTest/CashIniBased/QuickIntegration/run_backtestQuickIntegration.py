...
def calc_back_test_record(pct_threshold, per_amt_ratio, initial_cash, pool_num, deal_ratio, tag, alpha_pool_tag):
    ...
    signal_file = '/data/user/015664/AFuckingTrigger/限制买入和持仓/信号存储/signal_ValOld_%s_%.2f.pkl' % (tag, pct_threshold)
    if os.path.exists(signal_file):
        ...
        res = get_signal_by_val_pct_threshold_long_integration(
            pct_threshold, [x.replace('.pkl', '_val_pred/') for x in file_list],
            [x.replace('.pkl', '/') for x in file_list], start,'actual_label','new',head=None, end=end)
    ...
    tag = f'{tag}_{alpha_pool_tag}Top{pool_num}_real{pool_num}_%.1f_per_ratio_%.4f_threshold_%.2f_inital_%d' % (deal_ratio, per_amt_ratio, pct_threshold, int(initial_cash))
    ...
    file_name = '%s_ValOld_VolConsider_UpBuy100_%dbp_cost' % (tag, int(10000 * cost))
...
calc_back_test_record(*(each + (0.1,f'XGB_DTC_OnlyVal5', 'CS_XGB_OLS_condition_style_rank_ex20')))
