...
from StrongStockModel.model.ModelResultLoadingTool import get_signal_by_val_pct_threshold_long_integration_threshold_by_history
import pandas as pd
...
def calc_back_test_record(pct_threshold, per_amt_ratio, initial_cash, pool_num, deal_ratio, tag, alpha_pool_tag,val_day):
    file_list = para[tag]
    print(file_list)
    start = 20170101
    end = 20211231
    ...
    signal_file = '/data/user/015664/AFuckingTrigger/限制买入和持仓/信号存储/signal_Val%d_%s_%.2f.pkl' % (val_day,tag, pct_threshold)
    if os.path.exists(signal_file):

        print(pct_threshold, 'signal exist')
        signal, pred_ret = pd.read_pickle(signal_file)[:2]
    else:
        send_message(['015664'],f'{val_day} not exist')
        res = get_signal_by_val_pct_threshold_long_integration_threshold_by_history(
            pct_threshold, [x.replace('.pkl', '_val_pred/') for x in file_list],
            [x.replace('.pkl', '/') for x in file_list], start,'actual_label','new',head=None, end=end,val_period=val_day)
    ...
    signal = signal>0.5
    pred_ret[~signal.fillna(False).astype(bool)] = np.nan
    ...
    tag = f'{tag}_{alpha_pool_tag}Top{pool_num}_ValLen{val_day}_%.1f_per_ratio_%.4f_threshold_%.2f_inital_%d' % (deal_ratio, per_amt_ratio, pct_threshold, int(initial_cash))
    ...
    out_path = f'/data/user/015664/AFuckingTrigger/限制买入和持仓/ValDayCompare/{file_name}.xlsx'
    for each in record:
...
vla_len = 50#int( AIMR.getParam())
print(vla_len)
calc_back_test_record(*(each + (0.1,f'XGB_DTC_OnlyVal5', 'CS_XGB_OLS_condition_style_rank_ex20',vla_len)))
# calc_back_test_record(*(each + (0.1,f'XGB_DTC_OnlyVal5', 'CS_XGB_OLS_condition_style_rank_ex20',20)))
# calc_back_test_record(*(each + (0.1,f'XGB_DTC_OnlyVal5', 'CS_XGB_OLS_condition_style_rank_ex20',200)))
