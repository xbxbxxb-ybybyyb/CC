...
def calc_back_test_record(pct_threshold, per_amt_ratio, initial_cash, pool_num, deal_ratio, tag, alpha_pool_tag):
    ...
    signal, pred_ret = pd.read_pickle(f'/data/user/015664/AFuckingTrigger/限制买入和持仓/信号存储/long_nonfix_window//signal_long_XGB_DTC_Future_7_Bar_pct_0.05.pkl' )[:2]
    ...
    inter_signal = {}
    for i in range(1,7):
        temp_signal, temp_pred_ret = pd.read_pickle(f'/data/user/015664/AFuckingTrigger/限制买入和持仓/信号存储/short_v2//signal_short_XGB_DTC_Future_{i}_Bar_pct_0.pkl')[:2]
...
calc_back_test_record(*(each + (0.1, 'XGB_DTC_OnlyEarlyStop', 'CS_XGB_OLS_condition_style_rank_ex20')))
