...
from StrongStockModel.backtest.StrategyBackTest.DerivativeStrategy.StartWithLimitCashVolConsiderNonFixSignal import StartWithLimitCashVolConsider, InitailCashBasedEvaluationHelper
import pandas as pd
...
cost = 0.001
long_param = {i: f'/data/user/015664/AFuckingTrigger/限制买入和持仓/信号存储/long_nonfix_window//signal_long_XGB_DTC_Future_{i}_Bar_pct_0.05.pkl' for i in range(1,8)}
short_param = {i:f'/data/user/015664/AFuckingTrigger/限制买入和持仓/信号存储/short_v2//signal_short_XGB_DTC_Future_{i}_Bar_pct_0.pkl' for i in range(1,7)}

def calc_back_test_record(per_amt_ratio, initial_cash, pool_num, deal_ratio, tag, alpha_pool_tag):
    start = 20170101
    end = 20210531

    tag = tag + alpha_pool_tag+'NonFixSignal1000Base'
    ...
    out_path = f'/data/user/015664/AFuckingTrigger/限制买入和持仓/NonFixWindow/{file_name}.xlsx'
    for each in record:
