...
def out_file(factor_name,shift,out_path_io,all_date_list,code_list,overlap_from=None):
    ...
    if not os.path.exists(f'{out_path_io}{factor_name}.npy'):
        ...
        # tail = np.argwhere(idx_date==overlap_from).min()
        exist_data = np.memmap(f'{out_path_io}{factor_name}.npy',dtype='float32',offset=128,mode='r')
        if not overlap_from is None:
            tail = (idx_date < overlap_from).sum()
            exist_data = exist_data[:tail*time_len]
    ...
    relationlize_factor = factor_matmul(norm_factor, date_list, 'SW1',shift=shift,code_list=code_list)
...
def main(out_path_io):
    ...
    today = 20220419#get_recent_trade_date()
    ...
    f_name =  aimr_multitimes.getParam()
    print(f_name,'start')
    out_file(f_name,shift=True,out_path_io=out_path_io,all_date_list=_all_date_list,code_list=_code_list,overlap_from=None)
    print(f_name,'end')


if __name__ == '__main__':
    main('/data/group/800442/800319/HFfactor/CrossIndutryMeanShiftV20220420/data/')
