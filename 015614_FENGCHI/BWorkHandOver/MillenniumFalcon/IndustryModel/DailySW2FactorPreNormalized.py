...
for each in ['mean','std','zscore']:
    if not os.path.exists(f'{out_path}{each}'):
        os.makedirs(f'{out_path}{each}')
...
def get_all_stk_future_avg(future_days=1, delay_min=1, order_keep_min=1):
...
def out_factor(factor_name):
    ...
    factor = ((factor - np.nanmean(factor, axis=-1)[:,:,None]) / np.nanstd(factor, axis=-1)[:,:,None])
    group_factor = get_daily_1factor('SW2')#.shift(1)
    group_factor = group_factor.loc[_date_list, _code_list].values[:, None, :]
...
def main(n):
    ...
    pool.close()
    pool.join()
# factor_list = factor_list[:800]
# out_factor(factor_list[10])

# main(36)

# for i in tqdm(range(1, 6)):
#     stk_intraday_all_avg_label = get_all_stk_future_avg(future_days=i)
#     pd.to_pickle(stk_intraday_all_avg_label, f'{out_path}label/lable_group_stk_future_avg_{i}.pkl')
#
#
# # def arrlize():
