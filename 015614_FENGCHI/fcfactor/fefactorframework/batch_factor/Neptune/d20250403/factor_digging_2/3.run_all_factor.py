...
idx_list = np.arange(0, len(factor_list), 10)  # 0-10已完成

for idx in range(len(idx_list)):
    print(f'-----------{idx_list[idx]}:{min(len(factor_list), idx_list[idx+1])}------------')
    batch_factor_list = factor_list[idx_list[idx]:min(len(factor_list), idx_list[idx+1])]
