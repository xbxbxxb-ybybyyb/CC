...
class factor_fc_T1_n20240321_2(BaseFactor):
    owner = 'fc'
    strategy_name = "neptune"
    factor_name = sys._getframe().f_code.co_name[7:]
    fill_na_value = 0
    need_pre_calculate_T_N = True
    factor_explain = "ZT日大单买入金额中位值相对于前日之差"
    zcz_adjusted = "否"
    logic_type = ""
    low_cost = "是"
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        b = md_data['BUY_VALUE_LARGE_ORDER'].unstack().rolling(1, min_periods=1).median().stack()
        a = md_data['BUY_VALUE_LARGE_ORDER'].unstack().rolling(2, min_periods=1).median().stack()
