...
class factor_fc_T1_n20240321_9(BaseFactor):
    owner = 'fc'
    strategy_name = "neptune"
    factor_name = sys._getframe().f_code.co_name[7:]
    fill_na_value = 0
    need_pre_calculate_T_N = True
    factor_explain = "昨日中户卖出金额相对于近2日中位值的差值"
    zcz_adjusted = "否"
    logic_type = ""
    low_cost = "是"
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        b = md_data['SELL_VALUE_MED_ORDER'].unstack().rolling(1, min_periods=1).median().stack()
        a = md_data['SELL_VALUE_MED_ORDER'].unstack().rolling(2, min_periods=1).median().stack()
