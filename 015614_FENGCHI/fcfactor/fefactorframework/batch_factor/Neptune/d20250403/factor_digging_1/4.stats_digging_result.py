...
# factor_fpath_list.append('fc_n20250403_139.pkl')
# factor_value_list.append('fc_n20250403_139.h5')
res_list = []

for pkl in tqdm(factor_fpath_list):
...
factor_df_list = []
for factor_fpath in tqdm(factor_value_list):
...
for factor_fpath in tqdm(week_submit_factor_fpath_list):
...
for factor_fpath in tqdm(has_submit_factors):
...
all_factor = pd.concat(factor_df_list, axis=1)
# all_factor = all_factor.T.drop_duplicates().T   # 如果出现重复的列，就用这种方法删除，不然相关性计算出来是个列表
print('拼接完成')
...
for idx1, factor_name1 in enumerate(tqdm(all_factor.columns.tolist())):
    for idx2, factor_name2 in enumerate(all_factor.columns.tolist()):
        if idx1 > idx2:
            # if factor_name1 == 'fc_n20250403_139':
            #     print(1)
...
for index in tqdm(res_df.index):
    print(index)
    try:
        self_highest_corr = corr_res.loc[index.replace('/', '%')].fillna(1.0).sort_values(ascending=False)
