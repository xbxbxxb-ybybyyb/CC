...
param1, param2, param3 = '', 1, 240   # 配置超参数
...
class factor_fc_n20250522_1(BaseFactor):
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True:
            ...
            df1_ori['period'] = df1_ori['MDDate'].apply(lambda x: get_report_period(x))
            res1 = df1_ori['FIX_ASSETS'].groupby(['dt', 'Ticker']).apply(lambda x: x.tail(4).sum()).to_frame(name='res1')

            df2_ori = database['MD_CHINA_STOCK_DAILY_WIND']
            res2 = df2_ori['mkt_cap_ard'].unstack().rolling(5, 1).max().iloc[[-1]]

            res1['res2'] = res2.stack()
            res1[self.factor_name] = res1['res1'] / res1['res2'].replace(0, np.nan)

            database['pre_T_N'] = res1[[self.factor_name]]
            return database
