...
for factor in has_submit_factors:
    ...
    for idx in range(len(res_df2)):
        ...
        if factor in row['self_high_corr'].split(','):   # TODO: 这里要修改一下
...
while True:
    ...
    if len(tmp_res_df) > 0:
        ...
        for idx2 in range(1, len(tmp_res_df)):
            ...
            if name in row['self_high_corr'].split(','):
...
output_dict = {'score': res_df,
               'corr': corr_res,
               'filter': res_df2}
save_dict2xls(output_dict, root_path, f'因子寻优结果_{note}_{start_num}-{start_num+50}.xlsx')
