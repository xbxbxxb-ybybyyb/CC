...
start_date = 20160101
end_date = 20191231
europa_df = IO.read_data([start_date, end_date], alt='/data/group/800463/data/project1_public/factor_lib_v3/sft_update_europa_filter_20160101_20191231.h5')

root_path = '/data/user/015614/fefactorframework/europa_d20240801_factor_digging_1/factor_test/europa/'
factor_value = f'/data/user/015614/fefactorframework/europa_d20240801_factor_digging_1/factor_value/europa/'
note = '_part1'
...
factor_value_list = list(filter(lambda x: x.endswith('.h5'), sorted(value_list)))
res_list = []
...
for pkl in factor_fpath_list:
    ...
    nan_num = pkl_res['factor_information'].loc['Max Time Cost (Day)', 'DesCription']
    same_rate = pkl_res['pre_check_sta'].loc['same_rate']
    ...
    mixed_diff_score = pkl_res['check_score_res'].loc['score', 'mixed_diff_score']
    mixed_stability_score = pkl_res['check_score_res'].loc['score', 'mixed_stability_score']
...
for factor_fpath in factor_value_list:
    ...
    if factor_fpath != 'factor.pkl':
        ...
        if tmp.shape[0] != europa_df.shape[0]:
            print(f'{factor_fpath}样本长度不匹配')
        tmp = tmp.reindex(index=europa_df.index)
        factor_df_list.append(tmp)

#%% 计算和本周已提交因子的相关性
week_submit_date = 20240801
week_root_path = f'/data/user/015614/factor/d{week_submit_date}_Europa/'
...
has_submit_factors = [
    'fc_ttickab_n20240801_125',
    'fc_ttickab_n20240801_254',
    'fc_ttickab_n20240801_195',
]
...
res_df2 = res_df2.sort_values('score', ascending=False).query('score > 32 & highest_corr < 0.695 & high_corr_s_num == 0')
res_df2['drop'] = 0
res_df2['commit'] = 0
