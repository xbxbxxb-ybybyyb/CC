...
class factor_fc_n20250410_7(BaseFactor):
    owner = 'fc'
    strategy_name = "neptune"
    factor_name = sys._getframe().f_code.co_name[7:]
    fill_na_value = 1e8
    need_pre_calculate_T_N = True
    factor_explain = "过去10日中单净主动买入量均值" # 因子逻辑解释
    ...
    t_1_factor_data = [
        {'name': 'AShareMoneyFlow', # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 360,
         'column': ["VOLUME_DIFF_MED_TRADER_ACT"]
    }]
    ...
    def pre_calculate_T_N_data(self, database):
        if database["skip"] == True:
            ...
            res = df_ori["VOLUME_DIFF_MED_TRADER_ACT"].unstack().rolling(10, 1).apply(mean_).stack()
