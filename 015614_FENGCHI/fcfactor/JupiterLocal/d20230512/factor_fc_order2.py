# coding: utf-8
# Author：fengchi863
# Date ：2023/5/8 19:04

import numpy as np
import pandas as pd


def factor_fc_order2(df, param_tuple=(), return_fillna_dic=False):
    factor_name = 'fc_order2'
    if return_fillna_dic:
        return {factor_name: 0.0}
    # -------------------------------------------------------------------------------------------------------------------
    pre_close = df['pre_close'].iloc[0]
    dt, Ticker = df.index[0]
    ff_shares = df['ff_shares'].iloc[0]
    df = df[df['OrderType'].isin([1, 2]) & df['OrderBSFlag'].isin([1])]
    zcz = (Ticker[0] == '3' and dt.strftime('%Y-%m-%d') >= '2020-08-24') or (Ticker[0:2] == '68')
    df['limit_max'] = np.floor(pre_close * 1.1 * 100 + 0.5) / 100 if not zcz else np.floor(pre_close * 1.2 * 100 + 0.5) / 100
    df['limit_min'] = np.floor(pre_close * 0.9 * 100 + 0.5) / 100 if not zcz else np.floor(pre_close * 0.8 * 100 + 0.5) / 100
    df = df.query('limit_min <= OrderPrice <= limit_max')
    ul6_limit_max = np.floor(pre_close * 1.092 * 100 + 0.5) / 100 if not zcz else np.floor(pre_close * 1.184 * 100 + 0.5) / 100
    df['TradeMoney'] = df['OrderPrice'] * df['OrderQty']

    if len(df) > 100:
        # factor = df['OrderQty'].sort_values()[-30:].sum() / ff_shares   # 38.17 0.705freeturn
        # factor = df['OrderQty'].sort_values()[-20:].sum() / ff_shares   # 38.17 通过相关性
        # factor = df['OrderQty'].iloc[-100:].sort_values()[-10:].sum() / ff_shares  # 已经加入

        # last10 = df['OrderQty'].iloc[-100:].sort_values()[-10:]
        # factor = last10.std() / last10.mean() # 不行，序列太短是不可以的

        factor = (df.iloc[-3:]['OrderQty'].sum() + df.iloc[-10:-4]['OrderQty'].mean()) / ff_shares # 65 9


    else:
        factor = 0
    # print(df['Lag_Time'][0])
    # print(df.groupby('OrderType')['MDTime'].count())

    factor_dict = {factor_name: factor}
    # ---------------------------------------------------------------------------------------------------------------
    return pd.Series(factor_dict)
