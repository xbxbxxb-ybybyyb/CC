...
def cal_dt_price(pre_close_dataframe):
    ...
    pre_close_dataframe['ul_price'] = np.floor(pre_close_dataframe['pre_close'] * 100 * 0.9 + 0.5) / 100
    pre_close_dataframe.loc[(after_824 & cyb)| kcb, 'ul_price'] = np.floor(pre_close_dataframe['pre_close'] * 100 * 0.8 + 0.5) / 100
...
# T-1日类因子
def factor_fc_t_1_dt_c(start_date, end_date, IO, return_fillna_dic=False):
    import sys
    ...
    md_data['dt_price'] = cal_dt_price(md_data[['pre_close']])
    md_data['is_dt'] = md_data['close'] == md_data['dt_price']

    stock_pool = cal_stock_list(start_date=start_date,
                                end_date=end_date,
                                IO=IO)

    dt = md_data['is_dt'].unstack().fillna(False)
    dt = dt & stock_pool.reindex(index=dt.index, columns=dt.columns)

    by_day = pd.Series(np.nansum(dt.values, axis=1), index=dt.index)
    by_day.name = fname
...
if __name__ == '__main__':
    import IO
    factor = factor_fc_t_1_dt_c(20220901, 20220930, IO=IO, return_fillna_dic=False)
    pass