...
# T-1日类因子
def factor_fc_t_1_lb_ratio(start_date, end_date, IO, return_fillna_dic=False):
    import sys
    ...
    daily_lb_num = pd.Series(np.nansum(lb.values >= 2, axis=1) / np.nansum(np.pad(zt.values[:-1], ((1,0),(0,0)), mode='constant', constant_values=True), axis=1), index=lb.index)
...
if __name__ == '__main__':
    import IO
    factor = factor_fc_t_1_lb_ratio(20221001, 20221211, IO=IO, return_fillna_dic=False)
    pass