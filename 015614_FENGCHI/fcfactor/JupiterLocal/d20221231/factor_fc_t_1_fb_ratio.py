...
# T-1日类因子
def factor_fc_t_1_fb_ratio(start_date, end_date, IO, return_fillna_dic=False):
    import sys
    ...
    md_data = IO.read_data([start_date_, end_date], columns=['close', 'high', 'pre_close','pct_chg'], alt='/data/group/800080/warehouse/prod/MD/CHINA_STOCK/DAILY/WIND/MD_CHINA_STOCK_DAILY_WIND.h5')
    ...
    md_data['is_cb'] = md_data['high'] == md_data['ul_price']
    ...
    cb = md_data['is_cb'].unstack().fillna(False)
    zt = zt & stock_pool.reindex(index=zt.index, columns=zt.columns)
    cb = cb & stock_pool.reindex(index=cb.index, columns=zt.columns)

    by_day = pd.Series(np.nansum(zt.values, axis=1) / np.nansum(cb.values, axis=1), index=zt.index)
    by_day.name = fname
...
if __name__ == '__main__':
    import IO
    factor = factor_fc_t_1_fb_ratio(20221001, 20221211, IO=IO, return_fillna_dic=False)
    pass