...
def factor_fc_trans_order6(df, param_tuple=(), return_fillna_dic=False):
    import sys
    ...
    order_df = order_df.query('OrderBSFlag == 1')  # 买入委托

    # 根据order的筛选，在trans中找到对应的部分
    trans_df = trans_df.query(f'TradeBuyNo in {order_df["OrderIndex"].tolist()}')  # 逐笔成交中卖出Index在OrderIndex中的部分
    trans_df = trans_df.query('buy_flag == 1')
    ...
    # -------------------买入委托中，逐笔成交中主动买入的比例--35.08 7.87 和sss一个因子高相关sss_to_rawactratio_buy_s60 62.33分-----------------------------------------------------
