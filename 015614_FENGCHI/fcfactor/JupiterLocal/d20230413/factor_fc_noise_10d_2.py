...
def factor_fc_noise_10d_2(start_date, end_date, IO, return_fillna_dic=False):
    factor_name = 'fc_noise_10d_2'

    if return_fillna_dic:
        return {factor_name: 0, 'data': ['MD']}

    start_date_ = int(s.tradingday(str(start_date), -80)[0])  # 向前取的天数至少大于要用到的数据日期数+1天
    ...
    price_diff = (close - opn) / close.shift(1)
