...
def factor_fc_stk_mar_corr2(start_date, end_date, IO, return_fillna_dic=False):
    factor_name='fc_stk_mar_corr2'

    if return_fillna_dic:
        return {factor_name: 0, 'data': ['MD', 'AIndexEODPrices']}
    # 计算全部股票在全部时间区间上的因子值，之后会在run_factor_demo函数中进行向后平移一天和样本的筛选
    # -------------------------------------------------------------------------------------------------------------------
    start_date_ = int(s.tradingday(str(start_date), -41)[0])
    ...
    index_close = index_data['S_DQ_CLOSE'].shift(1)
    index_pre_close = index_data['S_DQ_CLOSE'].shift(1)
    ret = (close - opn) / pre_close
    ret_market = (index_close - index_opn) / index_pre_close
