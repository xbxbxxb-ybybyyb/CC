...
def weighted_smaller_than_n(series, m=225):
    num = series <= m  # 序列中小于n的次数
    weight = np.arange(1, (22 + 1), 1) / 22
    temp = (num * weight).sum()  # 序列中小于n的加权次数
    return temp

def factor_fc_winner_225(start_date, end_date, IO, return_fillna_dic=False):
    factor_name='fc_winner_225'
    ...
    ans = pd.DataFrame(pct_chg.rank(ascending=False, axis=1).values, index=pct_chg.index, columns=pct_chg.columns)
