...
def run_factor(func, factor_name, factor_type, start_date, end_date, basic_file_path, result_path, append_next_tradingday=False, interval_res=True,
               ...
               data_path_dic= {'TTransaction': '/data/group/800463/data/project1_prod/transaction_zt_bs/',
    ...
    if factor_type in ['TTransaction','TOrder','LastTouchTTick', 'T-1_factor']:
        # 个股因子
        ...
        if factor_type in ['TTransaction', 'TOrder']:
        ...
        elif factor_type in ['LastTouchTTick']:
            tradingday_list = s.tradingday(start_date, end_date)
            factor_df_list = []
            ...
            for tradingday, num in zip(tradingday_list, range(len(tradingday_list))):
                ...
                for index, inf in basic_dic[tradingday].iterrows():
                    ...
                    if factor_type in ['TTouchTTrans']:
                        filter_data = filter_data[filter_data['zt_time']<zt_time]
...
if __name__ == '__main__':
    ...
    basic_file_path = '/data/group/800463/data/project1_public/Basic_zt.h5'
    result_path = '/data/user/015614/factor/'
    # import Factor.factor.factor_test_last_factor as func_tmp
    ...
    # import Factor.factor.factor_test_trade_factor as func_tmp
    ...
    factor_name_list = ['fc_tick_pup_mean', 'fc_v_z_ap_area', 'fc_v_z_p_area', 'fc_v_z_ts_p']
    factor_name_list = ['fc_buy_amt_pct', 'fc_buy_amt_pct_1m', 'fc_conc_ratio_10s', 'fc_net_buy_str', 'fc_net_buy_str_1m']
    factor_name_list = ['fc_net_buy_str_1m']
    for factor_name in factor_name_list:
        mod_name = f'Factor.factor.d20230314.factor_{factor_name}'
        module = importlib.import_module(mod_name)
        func = getattr(module, f'factor_{factor_name}')
        factor_df = run_factor(func, factor_name, 'TTransaction', start_date, end_date, basic_file_path, result_path, interval_res=False)