...
class strongFactorTest:
    def __init__(self, start_date, end_date, segment_number=20, style_list=['freeturn', 'ZT_Time'], cal_mi = ['value', 'class', 'mixed'],
                 time_interval=[93000000, 143000000], last_is_zt=False, test_sample='zt',
                 first_over_threshold=False, break_is_zt=False,
                 first_is_zt=False, open_is_zt=False, Flag_SH_SZ=None, filter_factor=None,
                 ):
    ...
    def load_data(self):
        if self.test_sample == 'zt':
            df = IO.read_data([self.start_date, self.end_date], alt='/data/group/800463/data/project1_public/sft_base_data_zt_v2.h5')
            df['first_is_zt'] = (df['ul_price'] == df['high_price'])
        ...
        if self.test_sample == 'bt':
            ...
            all_filter = (time_interval_filter & last_is_zt_filter & first_over_threshold_filter & break_is_zt_filter & sh_sz_filter)
            ...
            all_filter = (time_interval_filter & last_is_zt_filter & open_is_zt_filter & first_is_zt_filter & sh_sz_filter)
        #样本筛选
        df = df[all_filter]
        df['label_pattern'] = (df['label_pattern'] ==4).astype(int)
        df = df.rename(columns = {'label_pattern':'class', 'label_TN_o2ul':'value'})
    ...
    def factor_test(self, factor_df, result_path, factor_corr_test=False, generate_pdf=True, tag=''):
        ...
        if self.factor_corr_test:
            ...
            self.result_dic['factor_corr_summary'] = self.result_dic['factor_corr_summary'][(self.result_dic['factor_corr_summary']['factor_corr']>0.7) | (self.result_dic['factor_corr_summary']['max']>0.9)]
    ...
    def generate_pdf(self, result_dic, save_path):
        ...
        t_other_sta = generate_table(result_dic['other_sta'], col_type=['pct2'], axis=0, reformat_type=True)
        if self.last_is_zt:
...
if __name__ == '__main__':
    # sft = strongFactorTest(20160101, 20181231)
    res_path = '/data/user/015614/factor/'

    # factor_df0 = pd.read_hdf(res_path + 'test_last_factor.h5')
    # sft.factor_test(factor_df0, result_path=res_path, factor_corr_test=True, generate_pdf=True)

    # factor_df1 = pd.read_hdf(res_path + 'fc_v_z_area.h5')
    # sft.factor_test(factor_df1, result_path=res_path, factor_corr_test=True, generate_pdf=True)

    # factor_name_list = ['fc_conc_ratio_1m', 'fc_conc_ratio_30s', 'fc_tick_pup_mean', 'fc_v_z_ap_area', 'fc_v_z_p_area', 'fc_v_z_ts_p']
    # factor_name_list = ['fc_buy_amt_pct', 'fc_buy_amt_pct_1m', 'fc_conc_ratio_10s', 'fc_net_buy_str', 'fc_net_buy_str_1m']
    ...
    # 测试自身相关性
    submit_factor_list = ['fc_v_z_ts_p', 'fc_buy_amt_pct_1m', 'fc_conc_ratio_10s', 'fc_conc_ratio_30s', 'fc_net_buy_str_1m']
    factor_df_list = list()
    for factor_name in submit_factor_list:
        tmp = pd.read_hdf(res_path + f'{factor_name}.h5')
        factor_df_list.append(tmp)
    all_factors = pd.concat(factor_df_list, axis=1)
    corr = all_factors.corr('spearman')