...
def factor_fc_trans_tickab1(df, param_tuple=(), return_fillna_dic=False):
    import sys
    ...
    if len(min_trans_qty) > 0 and len(min_tick_nums) > 0:
        ...
        factor = factor_s.mean() / ff_shares
    else:
        factor = 0

    print(factor_name, dt.strftime('%Y%m%d'), factor)
    factor_dict = {factor_name: factor}
    # ------------------------------成交数量除以成交笔数，每10秒平均每笔交易的量对应换手 40.33 7.44%------------------------------------------
    # 和float_shares相关性很高，感觉不应该取平均
