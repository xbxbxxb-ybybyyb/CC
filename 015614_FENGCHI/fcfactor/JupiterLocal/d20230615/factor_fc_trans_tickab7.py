...
def factor_fc_trans_tickab7(df, param_tuple=(), return_fillna_dic=False):
    import sys
    ...
    min_trans_qty = tmp_trans_df.query('buy_flag == 0').groupby('m')['TradeQty'].sum(min_count=1)
    ...
    # ------------------------------成交数量除以成交笔数，最近一小段时间平均每笔交易的卖单量对应换手--1.88 -0.61 没有高相关，但是卖单就表现这么差----------------------------------------
