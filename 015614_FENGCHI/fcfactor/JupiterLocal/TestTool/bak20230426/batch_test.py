...
factor_name_list = list(map(lambda x: x[7: -3], os.listdir(f'/data/user/015614/fcfactor/JupiterLocal/d{date}/')))
factor_name_list = list(filter(lambda x: x.startswith('fc'), factor_name_list))
factor_name_list = ['fc_swing7_60']
recalc_flag = True
...
for factor_name in tqdm(factor_name_list):
    ...
    if not recalc_flag and os.path.exists(res_path + f'{factor_name}.h5'):
        ...
        mod_name = f'JupiterLocal.d{date}.factor_{factor_name}'
        module = importlib.import_module(mod_name)
        func = getattr(module, f'factor_{factor_name}')
        factor_df = run_factor(func, factor_name, factor_type, start_date, end_date, basic_file_path, res_path, interval_res=False)
