...
if __name__ == '__main__':
    ...
    start_date, end_date = 20191231, 20191231
    ...
    # check1 = pd.read_hdf('/data/user/015614/factor/fc_loser_225_1.h5')
    # check2 = pd.read_hdf('/data/user/015614/factor/fc_loser_225.h5')
    # check1 = check1.loc[pd.to_datetime(str(20191231))]
    # check1-check2
    ...
    factor_type, no, multi = 'T-1_factor', 1, True
    print(factor_type, f'no=={no}')

    if factor_type == 'T-1_factor':
        mod_name = f'Jupiter.d20230413.factor_fc_stk_mar1_corr'
        module = importlib.import_module(mod_name)
        func = getattr(module, f'factor_fc_stk_mar1_corr')
        factor_name = f'fc_stk_mar1_corr'
        factor_df = run_factor(func, factor_name, factor_type, start_date, end_date, basic_file_path, result_path, param_tuple=(), interval_res=False, multi=False)

    if factor_type == 'TTransaction':
        mod_name = f'JupiterLocal.d{date}.factor_fc_trans{no}'
        module = importlib.import_module(mod_name)
        func = getattr(module, f'factor_fc_trans{no}')
        factor_name = f'fc_trans{no}'
    ...
    elif factor_type == 'TOrder':
        mod_name = f'JupiterLocal.d{date}.factor_fc_order{no}'
        module = importlib.import_module(mod_name)
        func = getattr(module, f'factor_fc_order{no}')
        factor_name = f'fc_order{no}'
        factor_df = run_factor(func, factor_name, factor_type, start_date, end_date, basic_file_path, result_path, param_tuple=(), interval_res=False, multi=multi)

    elif factor_type == 'TTransaction_TOrder':
        mod_name = f'JupiterLocal.d{date}.factor_fc_trans_order{no}'
        module = importlib.import_module(mod_name)
        func = getattr(module, f'factor_fc_trans_order{no}')
        factor_name = f'fc_trans_order{no}'
        factor_df = run_factor(func, factor_name, factor_type, start_date, end_date, basic_file_path, result_path, param_tuple=(), interval_res=False, multi=multi)

    elif factor_type == 'TTransaction_TTickab':
        mod_name = f'JupiterLocal.d{date}.factor_fc_trans_tickab{no}'
        module = importlib.import_module(mod_name)
        func = getattr(module, f'factor_fc_trans_tickab{no}')
        factor_name = f'fc_trans_tickab{no}'
        factor_df = run_factor(func, factor_name, factor_type, start_date, end_date, basic_file_path, result_path, param_tuple=(), interval_res=False, multi=multi)

    elif factor_type == 'TTickab' and  no == 1:
        mod_name = f'JupiterLocal.d{date}.factor_fc_tickab{no}'
        module = importlib.import_module(mod_name)
        func = getattr(module, f'factor_fc_tickab{no}')
        factor_name = f'fc_tickab{no}'
        factor_df = run_factor(func, factor_name, factor_type, start_date, end_date, basic_file_path, result_path, param_tuple=(), interval_res=False, multi=multi)
