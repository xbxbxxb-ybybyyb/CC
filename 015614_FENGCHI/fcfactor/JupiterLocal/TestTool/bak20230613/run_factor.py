...
def run_factor(func, factor_name, factor_type, start_date, end_date, basic_file_path, result_path, append_next_tradingday=False, interval_res=True,
    ...
    if factor_type in ['TTransaction', 'TTickab','T-1_factor','TOrder','LastTouchTTick','MarketTTick','Market1TTick','MarketIndTTick']:
        # 个股因子
        ...
        if factor_type in ['TTransaction', 'TTickab', 'TOrder']:
            ...
            for tradingday, num in zip(tradingday_list, range(len(tradingday_list))):
                ...
                if factor_type in ['TTickab','TOrder']:
                    #策略使用transaction触发, 由于tick、order相较于transaction延迟，截取时将时间戳提前一定时间
                    flag_dic={0:'SH',1:'SZ'}
                    if factor_type in ['TTickab']:
                        lag_dic = {'SH': 1000, 'SZ': 2000}  # ms
                    elif factor_type in ['TOrder']:
                        lag_dic = {'SH': 300, 'SZ': 30}  # ms
                    tmp=basic_df[['ZT_Time','Flag_SH_SZ']].loc[tradingday]
                    data['Lag_Time']=tmp.apply(lambda x:get_before_time(int(x['ZT_Time']),lag_dic[flag_dic[x['Flag_SH_SZ']]]),axis=1)
                    data=data[data['MDTime']<=data['Lag_Time']]
...
if __name__ == '__main__':
    ...
    # import JupiterLocal.TestTool.factor_test_last_factor as func_tmp
    ...
    import JupiterLocal.TestTool.factor_test_trade_factor as func_tmp
    factor_df1 = run_factor(func_tmp.factor_test_trade_factor, 'test_trade_factor', 'TTransaction',
                           start_date, end_date, basic_file_path, result_path, interval_res=False)

    # import JupiterLocal.d20230322.factor_fc_o2pre as func_tmp
    # factor_df1 = run_factor(func_tmp.factor_fc_o2pre, 'fc_o2pre', 'TTransaction',
    #                         start_date, end_date, basic_file_path, result_path, interval_res=False)
