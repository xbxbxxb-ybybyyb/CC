...
class FactorParamSearch:
    ...
    def calc_factor(df, param_tuple, return_fillna_dic=False):
        ...
        df_jh = df[df['MDTime'] < 93000000].copy()
        df['act_flag'] = (df['TradeBuyNo'] > df['TradeSellNo']).astype(float)
        df = df[df['MDTime'] >= 93000000]

        df_act_sell = df[df['act_flag'] == 0]
        if len(df_act_sell) == 0:
            purbuy = df.copy()
        else:
            purbuy = df[df['TradeIndex'] > df_act_sell['TradeIndex'].iloc[-1]].copy()
        maxsize = purbuy.groupby('TradeBuyNo')['TradeQty'].sum().max()
        if len(df_jh) > 0:
            before_maxsize = df_jh.groupby('TradeBuyNo')['TradeQty'].sum().max()
        else:
            before_maxsize = 1

        factor = np.log(maxsize / before_maxsize)
        factor_dict = {fname: factor}
