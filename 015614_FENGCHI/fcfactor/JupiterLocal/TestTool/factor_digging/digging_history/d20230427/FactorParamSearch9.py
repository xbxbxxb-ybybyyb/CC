...
class FactorParamSearch:
    ...
    def calc_factor(df, param_tuple, return_fillna_dic=False):
        ...
        buy_df = df[df['buy_flag'] == 1]
        group_buy_df = buy_df.groupby('TradeBuyNo').agg({'TradeMoney': sum})   # 保留大户

        if len(group_buy_df) == 0:
            ret = 0.0001
        else:
            ml_group_buy = group_buy_df.query(f'TradeMoney > {param1}')
            ml_group_buy = df.query(f'TradeBuyNo in {ml_group_buy.index.tolist()}')
            group_buy_df['TradeIndexMin'] = ml_group_buy.groupby('TradeBuyNo').agg({'TradeIndex': min})
            group_buy_df['TradeIndexMax'] = ml_group_buy.groupby('TradeBuyNo').agg({'TradeIndex': max})
            if len(group_buy_df) > 1:
                last1_index = group_buy_df.iloc[-1]['TradeIndexMin']
                last2_index = group_buy_df.iloc[-2]['TradeIndexMax']
                last1_dt_mdtime = df.query(f'TradeIndex == {last1_index}').iloc[0]['MDTime']
                last2_dt_mdtime = df.query(f'TradeIndex == {last2_index}').iloc[0]['MDTime']

                last1_dt = dt.strptime(str(int(last1_dt_mdtime)), '%H%M%S%f')
                last2_dt = dt.strptime(str(int(last2_dt_mdtime)), '%H%M%S%f')

                if last1_dt < last2_dt:
                    ret = 0.0001
                else:
                    time_delta = last1_dt - last2_dt
                    ret = time_delta.seconds + time_delta.microseconds / 1000000
                    if last1_dt_mdtime > 120000000 > last2_dt_mdtime:
                        ret -= 5400
            else:
                ret = 0.0001
        ...
        if func1 == '倒数':
            if ret == 0:
                ret = 0.0001
            factor_dict = {factor_name: 1 / ret}
        elif func1 == '对数':
            factor_dict = {factor_name: np.log(1 + ret)}
        elif func1 == '平方根':
            factor_dict = {factor_name: np.sqrt(ret)}
        elif func1 == 'None':
