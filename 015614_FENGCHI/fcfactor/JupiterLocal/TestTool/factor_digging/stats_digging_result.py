...
root_path = '/data/user/015614/factor/factor_digging_20230613151325_修复bug后市场相关性因子&填充中位值&添加新的指数/'
...
for index in res_df.index:
    self_highest_corr = corr_res.loc[index].sort_values(ascending=False)
    res_df.loc[index, 'self_high_corr'] = self_highest_corr.iloc[0]
    res_df.loc[index, 'self_high_factor'] = self_highest_corr.index[0]
