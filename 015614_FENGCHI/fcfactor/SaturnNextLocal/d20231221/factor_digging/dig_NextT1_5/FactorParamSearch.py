...
class FactorParamSearch:
    ...
    def calc_factor(start_date, end_date, IO, param_tuple, return_fillna_dic=False):
        ...
        def sub_std(x, multiplier=1):
            x = x - x.std() * multiplier
            return x
        ...
        res_std = md_data['res'].unstack().rolling(param1, min_periods=5).std().stack()

        factor_df = pd.DataFrame()
        factor_df[factor_name] = (md_data['res'] - res_std * param2).unstack().rolling(param3, min_periods=1).median().stack()
        return factor_df
...
if __name__ == '__main__':
    ...
    param_list = [[1, 2, 3, 6, 10, 20, 30, 40, 60, 90, 120, 240],
                  [0.2, 0.5, 0.8, 1, 2, 3, -1, -2, -3],
                  [1, 2, 3, 6, 10, 20, 30, 40, 60, 90, 120, 240]]
    param_list = list(product(*param_list))
    # param_list = list(filter(lambda x: x[1] > x[0], param_list))
    multiprocess(24, fps.wrapper, param_list)
    # fps.wrapper([(1, 0.2, 1)]) # 用于调试