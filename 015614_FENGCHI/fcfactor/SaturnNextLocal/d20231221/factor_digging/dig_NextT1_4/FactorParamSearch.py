...
class FactorParamSearch:
    ...
    def calc_factor(start_date, end_date, IO, param_tuple, return_fillna_dic=False):
        ...
        c2v_std = md_data['c2v'].unstack().rolling(param1, min_periods=5).std().stack()

        factor_df = pd.DataFrame()
        factor_df[factor_name] = (md_data['c2v'] - c2v_std * param2).unstack().rolling(param3, min_periods=1).median().stack()
        return factor_df
