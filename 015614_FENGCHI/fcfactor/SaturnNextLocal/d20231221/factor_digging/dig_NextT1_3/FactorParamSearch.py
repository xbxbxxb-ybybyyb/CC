...
class FactorParamSearch:
    ...
    def calc_factor(start_date, end_date, IO, param_tuple, return_fillna_dic=False):
        ...
        if return_fillna_dic:
            return {factor_name: 4, 'data': ['minute5']}
            # -------------------------------------------------------------------------------------------------------------------
        start_date = int(s.tradingday(str(start_date), -240)[0])
        high_minute5 = IO.read_data([start_date, end_date], alt='/data/group/800463/data/generalStrong/minute5/high.h5')
        low_minute5 = IO.read_data([start_date, end_date], alt='/data/group/800463/data/generalStrong/minute5/low.h5')
        h2l = high_minute5 / low_minute5 - 1
        res = h2l.std(axis=1)

        factor_df = pd.DataFrame()
        factor_df[factor_name] = res.unstack().rolling(param1, 1).mean().stack() * 1000
        return factor_df
