...
class FactorParamSearch:
    ...
    def wrapper(self, param_list):
        for param_tuple in tqdm(param_list):
            ...
            factor_df = run_factor(self.calc_factor, factor_name, 'Next1mTickab',
    ...
    def calc_factor(df, param_tuple, return_fillna_dic=False):
        ...
        trade_big = df['WindowValue'].quantile(param2)
        trade_small = df['WindowValue'].quantile(param3)
        ...
        if param4 == 'big':
            ...
            value = df_small['factor'].mean()

        # value = df['factor'].mean() - df['factor'].std()
