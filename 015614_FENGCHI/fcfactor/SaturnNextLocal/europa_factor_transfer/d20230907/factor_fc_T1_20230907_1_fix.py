...
def factor_fc_T1_20230907_1_fix(start_date, end_date, IO, param_tuple=(), return_fillna_dic=False):
    import sys
    ...
    factor_df[factor_name] = md_data['CLOSE_MONEYFLOW_PCT_VALUE'].unstack().diff().rolling(120, min_periods=1).mean().stack() * 1e6
    # ---------------------------------------------------------14:30后的资金净流入金额/流通市值----------------------------------------------------------
    """
    波动性太低，进行修改 样本内得分 22.75 -0.06
    """
    return factor_df
