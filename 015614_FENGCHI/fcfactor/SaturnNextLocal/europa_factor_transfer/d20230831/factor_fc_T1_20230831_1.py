...
def factor_fc_T1_20230831_1(start_date, end_date, IO, param_tuple=(), return_fillna_dic=False):
    import sys
    ...
    factor_df[factor_name] = md_data['CLOSE_MONEYFLOW_PCT_VALUE'].unstack().diff().rolling(120, min_periods=1).mean().stack()
    # ---------------------------------------------------------14:30后的资金净流入金额/流通市值----------------------------------------------------------
    """
    样本内得分 22.75 -0.06
    """
