...
def factor_fc_ttickab_20231019_11(df, param_tuple=(), return_fillna_dic=False):
    import sys
    factor_name = sys._getframe().f_code.co_name[7:]
    # ------------------------------------------------近20分钟按成交量去除尾部与去除头部后对应vwap的涨跌幅差异---------------------------------------------------------
    ...
    if len(df) > 0:
        small, big = df['TotalValueTrade'].quantile([0.1, 0.9])
        small, big = round_(small, 2), round_(big, 2)
        df_small = df[df['TotalValueTrade'] >= small]
        if len(df_small) > 20 * 20: df_small = df_small.head(20 * 20) # clip
        df_big = df[df['TotalValueTrade'] <= big]
        if len(df_big) > 20 * 20: df_big = df_big.head(20 * 20)
    ...
    """
    31.75 0.079
    =====>>>> 31.75 0.07900235688783018 -0.04061612226206218 0.20437950421262818 qyh_ttick_apct_a2b_1m，wj_TTick_10_h2prec_maxmin4 0.6706，0.6146
    """
