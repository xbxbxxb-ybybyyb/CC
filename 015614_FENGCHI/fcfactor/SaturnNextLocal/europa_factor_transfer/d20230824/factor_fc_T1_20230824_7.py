...
def factor_fc_T1_20230824_7(start_date, end_date, IO, param_tuple=(), return_fillna_dic=False):
    import sys
    ...
    factor_df[factor_name] = md_data['MONEYFLOW_PCT_VALUE'].unstack().diff().rolling(60, min_periods=1).mean().stack()
    # ------------------------------------------------------当日主动资金净流入金额/流通市值 日差值取平均-------------------------------------------------------------
    """
    样本内得分 17.9583 -0.034
    """
    return factor_df
