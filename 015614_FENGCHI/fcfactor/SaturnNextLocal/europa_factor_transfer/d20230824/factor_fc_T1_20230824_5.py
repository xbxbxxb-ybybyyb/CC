...
def factor_fc_T1_20230824_5(start_date, end_date, IO, param_tuple=(), return_fillna_dic=False):
    import sys
    ...
    factor_df[factor_name] = md_data['MONEYFLOW_PCT_VOLUME'].unstack().diff().rolling(90, min_periods=1).mean().stack()
    # ----------------------------------------------------当日主动资金净流入量/流通股本 日差值取平均---------------------------------------------------------------
    """
    样本内得分 19.125 -0.045
    """
    return factor_df
