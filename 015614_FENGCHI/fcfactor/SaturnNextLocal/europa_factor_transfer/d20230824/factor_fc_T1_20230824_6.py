...
def factor_fc_T1_20230824_6(start_date, end_date, IO, param_tuple=(), return_fillna_dic=False):
    import sys
    ...
    factor_df[factor_name] = md_data['S_MFD_INFLOWVOLUME'].unstack().diff().rolling(6, min_periods=1).mean().stack()
    # ----------------------------------------------机构买入总量+大户买入总量-(机构卖出总量+大户卖出总量) 日差值取平均---------------------------------------------------------------------
    """
    样本内得分 18.29 -0.045
    """
    return factor_df
