...
class FactorParamSearch:
    ...
    def wrapper(self, param_list):
        for param_tuple in tqdm(param_list):
            ...
            factor_df = run_factor(self.calc_factor, factor_name, 'TallTrans',
                           self.start_date, self.end_date, self.basic_file_path, self.res_path, param_tuple=param_tuple, interval_res=False, multi=True)
    ...
    def calc_factor(df, param_tuple, return_fillna_dic=False):
        ...
        res = df.groupby(['TradePrice'])['TradeQty'].sum().reset_index().corr(method='spearman').values[0][1]
...
if __name__ == '__main__':
    ...
    param_list1 = [[1], [3, 5, 10, 20, 30, 60, 180, 300, 600, 1200, 1800, 3000]
                   ]
    param_list2 = [[2], [50, 100, 200, 500, 1000, 1200, 1500, 2000]]
    param_list1 = list(product(*param_list1))
    param_list2 = list(product(*param_list2))
    param_list = param_list1 + param_list2

    # multiprocess(24, fps.wrapper, param_list)
    fps.wrapper([(1, 3)]) # 用于调试