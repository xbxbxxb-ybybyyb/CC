...
def factor_fc_TallTick_10(df, return_fillna_dic=False):
    import sys
    ...
    df['LowPx'] = (df['LowPx'] - df['pre_close']) / df['pre_close']

    if df.shape[0] > 0:
        factor = (df['LowPx'] - df['WeightedAvgMid']).std()
    else:
        factor = 0

    print(factor_name, dt.strftime('%Y%m%d'), factor)
    factor_dict = {factor_name: factor}
    # -------------------------------------------------每个Tick买卖均价均值与最低价差的标准差 12.04 -4.48-------------------------------------------------------
