...
def factor_fc_Next1mTransaction_3(df, return_fillna_dic=False):
    ...
    if len(df) == 0:
        ...
        df_low = df[df['TradeQty'] >= df['TradeQty'].quantile(0.7)]

        factor = ((df_low['TradePrice'] - df_low['TradePrice'].min()) / pre_close).max()

        factor_dict = {factor_name: factor}
    print(factor_name, dt.strftime('%Y%m%d'), factor)
    #----------------------------930-931之间大单部分成交价格相对于最低价的最大值（负向，涨的越多，收益越低？） 16.67 -5.69-观察只在最大值附近单调性很强--和伟地一个迁移的因子相关性高-----------------------------
