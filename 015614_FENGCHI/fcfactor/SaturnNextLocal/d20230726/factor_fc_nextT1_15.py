...
def factor_fc_nextT1_15(start_date, end_date, IO, return_fillna_dic=False):
    import sys
    ...
    # -------------------------------------------------------------------------------------------------------------------
    start_date_ = int(s.tradingday(str(start_date), -1)[0])
    md_data = IO.read_data([start_date_, end_date], columns=['CLOSE_NET_INFLOW_RATE_VOLUME'], alt='/data/group/800080/warehouse/prod/DATABASE/WIND/AShareMoneyFlow/AShareMoneyFlow.h5')

    factor_df = pd.DataFrame()
    factor_df[factor_name] = md_data['CLOSE_NET_INFLOW_RATE_VOLUME']
    # -----------------------------------------------------------------尾盘资金流入率(量)(%)----------------------------------------------------
    """
    1.6666666666666667 -0.000535432078711052
计算本地高相关：
回测耗时 21.925840377807617
    """
    return factor_df
"""
AShareMoneyFlow:
