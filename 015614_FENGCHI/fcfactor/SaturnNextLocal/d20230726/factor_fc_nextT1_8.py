...
def factor_fc_nextT1_8(start_date, end_date, IO, return_fillna_dic=False):
    import sys
    ...
    # -------------------------------------------------------------------------------------------------------------------
    start_date_ = int(s.tradingday(str(start_date), -51)[0])
    md_data = IO.read_data([start_date_, end_date], columns = ['BookToPrice'], alt='/data/group/800080/warehouse/prod/MD/CHINA_STOCK/DAILY/WIND/MD_CHINA_STOCK_DAILY_WIND.h5')

    md_data.loc[md_data['pct_chg'] > 10, 'pct_chg'] = 10
    md_data.loc[md_data['pct_chg'] < -10, 'pct_chg'] = -10

    factor_df = pd.DataFrame()
    factor_df[factor_name] = md_data['pct_chg'].unstack().rolling(50, min_periods=3).apply(lambda x: calc_mdd(x)).stack()
    # -------------------------------------------------------过去50天的最大回撤--------------------------------------------------------------
    """
10.0 -0.0378066418329332  
计算本地高相关：
fc_nextT1_7.h5 0.8934973983223616
    """
    return factor_df
