...
def factor_fc_nextT1_21(start_date, end_date, IO, return_fillna_dic=False):
    import sys
    ...
    md_data = IO.read_data([start_date_, end_date], columns=['S_MFD_INFLOW_OPENVOLUME'], alt='/data/group/800080/warehouse/prod/DATABASE/WIND/AShareMoneyFlow/AShareMoneyFlow.h5')

    factor_df = pd.DataFrame()
    factor_df[factor_name] = md_data['S_MFD_INFLOW_OPENVOLUME'].unstack().diff().rolling(6, min_periods=1).mean().stack()
    # ------------------------------------------------------委买总量(手)---------------------------------------------------------------
    """
     3.3333333333333335 0.014413021414608044
    """
    return factor_df
"""
AShareMoneyFlow:
