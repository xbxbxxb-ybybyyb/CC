...
class FactorParamSearch:
    ...
    def calc_factor(df, param_tuple, return_fillna_dic=False):
        ...
        seg_price = df['LastPx'].quantile(param2)
        if len(df) > 0:
            df[param1] = df[param1].diff().diff().fillna(0)
            part_df1 = df.query(f'LastPx >= {seg_price}')
            part_df2 = df.query(f'LastPx <= {seg_price}')
            res = np.divide(part_df1[param1].mean(), part_df2[param1].mean())
            if np.isinf(res):
                res = 0
        else:
            res = 0
...
if __name__ == '__main__':
    ...
    param_list = [['NumTrades', 'Buy1OrderQty',
         'Buy2OrderQty',
         'Buy3OrderQty',
         'Buy4OrderQty',
         'Buy5OrderQty',
