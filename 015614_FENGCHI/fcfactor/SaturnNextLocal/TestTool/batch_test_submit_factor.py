...
if __name__ == '__main__':
    ...
    result_path = f'/data/user/015614/factor/d{date}_saturnNext/'
    ...
    factor_name_list = list(filter(lambda x: str(x).startswith('factor_'), os.listdir(f'/data/user/015614/fcfactor/SaturnNext/d{date}/')))
    ...
    factor_fname_list = [f'factor_fc_TallTick_20240104_{x}' for x in range(1, 6)]

    this_submit_factor_dict = dict()
    for factor_fname in factor_fname_list:
        print(f'当前测试因子：{factor_fname}')
        mod_name = f'SaturnNext.d{date}.{factor_fname}'
