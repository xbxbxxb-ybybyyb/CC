...
def factor_fc_trans_20231019_7(df, return_fillna_dic=False):
    import sys
    factor_name = sys._getframe().f_code.co_name[7:]
    # -------------------------------------------------突破前20分钟大于价格中位值部分的vwap与卖单号秩相关性--------------------------------------------------------
    ...
    start_time = max(93000000, fun_get_time(ZT_Time, -1200))
    df = df.query(f'MDTime > {start_time}')
    mid_price = round_(np.median(np.unique(df['TradePrice'])), 6)
    ...
    """
    52.7 -0.085
    =====>>>> 52.708333333333336 -0.08519895371430354 -0.06098906114235415 0.2975081411147643 wwd_t_up_half_ask_corr_pid，fc_trans_order_b_ratio_in_s 0.6197，0.5887
    """
