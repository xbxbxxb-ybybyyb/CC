...
def factor_fc_order_20231123_7(df, return_fillna_dic=False):
    import sys
    factor_name = sys._getframe().f_code.co_name[7:]

    if return_fillna_dic:
        return {factor_name: 0}
    # ---------------------------------------------------涨停前5分钟相对于前20分钟内，逐笔委托卖出中间价的价格变化率------------------------------------------------------
    ...
    dt_price = np.floor(pre_close * 100 * (0.9 - zcz * 0.1) + 0.5 + 1e-8) / 100
    zt_price = np.floor(pre_close * 100 * (1.1 + zcz * 0.1) + 0.5 + 1e-8) / 100
    df = df.query(f'OrderPrice <= {zt_price} & OrderPrice >= {dt_price}')
    df['OrderMoney'] = df['OrderPrice'] * df['OrderQty']
    df = df.query('OrderBSFlag==2')  # 卖出

    start_time1 = max(fun_get_time(zt_time, -300), 93000000)
    ...
    """
    55.91 -0.098
    =====>>>> 55.91666666666667 -0.09851924020161276 815.5320860039396 4580.728005318932 fc_trans_20231019_12，skk_TTickab_LPR_roll2vwap_std_a 0.6794，0.6762
    """
    """
    MDTime: 时间
