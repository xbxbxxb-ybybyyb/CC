...
def factor_fc_ttickab_20230921_19(df, return_fillna_dic=False):
    import sys
    ...
    if len(df) > 0:
        res = (df['p2v'][df['p2v'] > round_(df['p2v'].quantile(0.95), 6)].mean() - df['p2v'][df['p2v'] < round_(df['p2v'].quantile(0.05), 6)].mean()) / pre_close
    else:
        res = np.nan
