...
def factor_fc_ttickab_20231026_19(df, param_tuple=(), return_fillna_dic=False):
    import sys
    ...
    df['itvl_pct'] = ((df['LastPx'] - df['LastPx'].shift(20)) / pre_close)
    if zcz: df['itvl_pct'] /= 2
    df['itvl_pct_flag'] = df['itvl_pct'] >= 0.01 - 1e-6

    if len(df) <= 20:
        res = 0
    else:
        df_head = df.head(df.shape[0] - 20)
        res1 = df_head['itvl_pct_flag'].shape[0] / df_head.shape[0]
        df_tail = df.tail(20)
        res2 = df_tail['itvl_pct_flag'].shape[0] / df_tail.shape[0]
        res = res2 - res1
