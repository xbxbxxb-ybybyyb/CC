...
def factor_fc_T1_20230824_4_fix(start_date, end_date, IO, param_tuple=(), return_fillna_dic=False):
    import sys
    ...
    factor_df[factor_name] = md_data['CLOSE_MONEYFLOW_PCT_VALUE'].unstack().diff().rolling(60, min_periods=1).mean().stack() * 1e6
    # ----------------------------------------------------------14:30后的资金净流入金额/流通市值 日差值取平均---------------------------------------------------------
    """
    样本内得分 20.04 -0.049
    """
    return factor_df
