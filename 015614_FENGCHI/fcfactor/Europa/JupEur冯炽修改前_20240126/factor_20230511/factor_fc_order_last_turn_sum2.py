...
def factor_fc_order_last_turn_sum2(df, param_tuple=(), return_fillna_dic=False):
    factor_name = 'fc_order_last_turn_sum2'
    ...
    if len(df) > 50:
        factor = (df.iloc[-1]['OrderQty'] + df.iloc[-10:-2]['OrderQty'].mean()) / ff_shares    # 最后一笔 和 倒数第2-10笔买单的均值
    else:
        factor = 0
    # print(df['Lag_Time'][0])
