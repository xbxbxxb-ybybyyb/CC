...
def factor_fc_order_last50_qty_ratio(df, param_tuple=(), return_fillna_dic=False):
    factor_name = 'fc_order_last50_qty_ratio'
    ...
    if len(df) > 50:
        factor = df.iloc[-50:]['OrderQty'].sum() / df['OrderQty'].sum()
    else:
        factor = 0
    # print(df['Lag_Time'][0])
