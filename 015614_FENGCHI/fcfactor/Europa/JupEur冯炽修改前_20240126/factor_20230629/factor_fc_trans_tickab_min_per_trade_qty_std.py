...
def factor_fc_trans_tickab_min_per_trade_qty_std(df, param_tuple=(), return_fillna_dic=False):
    import sys
    ...
    if len(min_trans_qty) > 0 and len(min_tick_nums) > 0:
        ...
        factor = factor_s.std() ** 2 / ff_shares
    else:
        factor = 0

    #print(factor_name, dt.strftime('%Y%m%d'), factor)
    factor_dict = {factor_name: factor}
    # ------------------------------成交数量除以成交笔数，每10秒平均每笔交易的量 方差对应换手-33.33 7.22 没有高相关，但是19年得分偏低-----------------------------------------
