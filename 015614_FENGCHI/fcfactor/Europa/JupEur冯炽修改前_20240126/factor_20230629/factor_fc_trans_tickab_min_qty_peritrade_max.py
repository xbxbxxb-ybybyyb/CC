...
def factor_fc_trans_tickab_min_qty_peritrade_max(df, param_tuple=(), return_fillna_dic=False):
    import sys
    ...
    if len(min_trans_qty) > 0 and len(min_tick_nums) > 0:
        ...
        factor = factor_s.max() / ff_shares
    else:
        factor = 0

    #print(factor_name, dt.strftime('%Y%m%d'), factor)
    factor_dict = {factor_name: factor}
    # ------------------------------成交数量除以成交笔数，每分钟平均每笔交易的量 最大值对应换手--50.33 8.38 没有高相关----------------------------------------
