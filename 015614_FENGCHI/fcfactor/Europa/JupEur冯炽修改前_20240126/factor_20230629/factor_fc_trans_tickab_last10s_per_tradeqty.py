...
def factor_fc_trans_tickab_last10s_per_tradeqty(df, param_tuple=(), return_fillna_dic=False):
    import sys
    ...
    min_trans_qty = tmp_trans_df.groupby('m')['TradeQty'].sum(min_count=1)
    ...
    if len(min_trans_qty) > 0 and len(min_tick_nums) > 0:
        ...
        target = factor_s.iloc[-1] if ~np.isnan(factor_s.iloc[-1]) else factor_s.iloc[-2]
    ...
    # ------------------------------成交数量除以成交笔数，最近一个10秒平均每笔交易的量对应换手--57.71 8.57 没有高相关----------------------------------------
