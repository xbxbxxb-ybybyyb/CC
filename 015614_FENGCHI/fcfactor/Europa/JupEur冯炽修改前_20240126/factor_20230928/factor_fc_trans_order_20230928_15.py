...
def round_(x, n=0):
    ...
    return res
def factor_fc_trans_order_20230928_15(df, param_tuple=(), return_fillna_dic=False):
    import sys
    factor_name = sys._getframe().f_code.co_name[7:]
    if return_fillna_dic:
        return {factor_name: 0.0}
    # ----------------------------------------------------近10秒比前10秒卖出委托量中成交比例的比值-------------------------------------------------------------
    dt, Ticker = df.index[0]
    pre_close = df['pre_close'].iloc[0]
    zcz = (Ticker[0] == '3' and dt.strftime('%Y-%m-%d') >= '2020-08-24') or (Ticker[0:2] == '68')
    ul_price = np.floor(pre_close * 100 * 1.1 + 0.5) / 100 if not zcz else np.floor(pre_close * 100 * 1.2 + 0.5) / 100
    dt_price = np.floor(pre_close * 100 * 0.9 + 0.5) / 100 if not zcz else np.floor(pre_close * 100 * 0.8 + 0.5) / 100
    ...
    factor=round_(factor,6)
    factor = 0 if factor > 250 else factor
    factor_dict = {factor_name: factor}
    """
    33.62 0.062 进行了调整，提高了得分
    =====>>>> 36.75 0.07433567327334384 6.224533334323255 17.9443979600056 fc_trans_20230907_3，t_old_ask_vol_1d5_t1 0.4768，0.4327
    """
