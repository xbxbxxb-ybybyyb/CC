...
def factor_fc_trans_20231019_1(df, param_tuple=(), return_fillna_dic=False):
    import sys
    ...
    short_time = fun_shift_time(ZT_Time, -min(10000, int(dis2opn / 20)))
    long_time = fun_shift_time(ZT_Time, -min(1200000, int(dis2opn / 2)))
    # short_time = max(93000000, fun_get_time(ZT_Time, -min(10000, int(dis2opn / 20)) / 1000))
    # long_time = max(93000000, fun_get_time(ZT_Time, -min(1200000, int(dis2opn / 2)) / 1000))
    ...
    """
    77.58 0.12
    =====>>>> 77.58333333333334 0.12168785810609432 -0.38712767108732093 0.4239695587813139 xbc_20231012_6，Long_short_volatility_diff 0.6871，0.6783
    """
