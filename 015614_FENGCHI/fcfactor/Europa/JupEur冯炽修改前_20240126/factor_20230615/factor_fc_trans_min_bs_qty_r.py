...
def factor_fc_trans_min_bs_qty_r(df, param_tuple=(), return_fillna_dic=False):
    # 逐笔成交分钟买单量与分钟卖单量的相关性 33.71 得分刚过30线
    ...
    if len(min_buy_qty) == 0:
        ...
        factor = pearsonr(min_buy_qty.loc[index], min_sell_qty.loc[index])[0]   # spearonr的效果回测效果很差
