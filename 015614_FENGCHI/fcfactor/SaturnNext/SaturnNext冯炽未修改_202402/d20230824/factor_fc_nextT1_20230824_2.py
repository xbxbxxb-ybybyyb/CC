...
def factor_fc_nextT1_20230824_2(start_date, end_date, IO, return_fillna_dic=False):
    import sys
    ...
    md_data = IO.read_data([start_date_, end_date], columns=['SELL_VALUE_SMALL_ORDER'], alt='/data/group/800080/warehouse/prod/DATABASE/WIND/AShareMoneyFlow/AShareMoneyFlow.h5')

    factor_df = pd.DataFrame()
    factor_df[factor_name] = md_data['SELL_VALUE_SMALL_ORDER'].unstack().diff().rolling(40, min_periods=1).mean().stack()
    # ------------------------------------------------------散户主动卖出金额 近40日差值均值---------------------------------------------------------------
    """
    17.54 -0.043
    """
