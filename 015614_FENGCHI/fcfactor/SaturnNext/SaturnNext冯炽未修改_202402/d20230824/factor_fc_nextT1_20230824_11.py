...
def factor_fc_nextT1_20230824_11(start_date, end_date, IO, return_fillna_dic=False):
    import sys
    ...
    res = (close / vwap - 1).mean(axis=1)

    res_mean = pd.DataFrame(res.unstack().rolling(5, 2).mean().stack())
    res_std = pd.DataFrame(res.unstack().rolling(5, 2).std().stack())
    res_cv = res_std / res_mean
    res_cv.columns = [factor_name]
    factor_df = res_cv
    # ------------------------------------------------------五分钟频close与vwap的涨跌幅日内均值的五日CV---------------------------------------------------------------
    """
    22.20 0.050127
    """
    return factor_df
