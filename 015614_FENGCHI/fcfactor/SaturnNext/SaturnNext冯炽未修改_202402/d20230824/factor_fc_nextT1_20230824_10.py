...
def factor_fc_nextT1_20230824_10(start_date, end_date, IO, return_fillna_dic=False):
    import sys
    ...
    res = pd.DataFrame(res.unstack().rolling(25, 2).mean().stack())
    res.columns = [factor_name]
    factor_df = res
    # ------------------------------------------------------五分钟频close与vwap的涨跌幅日内均值的25日均值---------------------------------------------------------------
    """
    15.875 -0.04
    """
    return factor_df
