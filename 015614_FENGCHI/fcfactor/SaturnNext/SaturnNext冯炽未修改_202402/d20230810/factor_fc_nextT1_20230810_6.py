...
def factor_fc_nextT1_20230810_6(start_date, end_date, IO, return_fillna_dic=False):
    import sys
    ...
    md_data = IO.read_data([start_date_, end_date], columns=['SELL_VOLUME_MED_ORDER_ACT'], alt='/data/group/800080/warehouse/prod/DATABASE/WIND/AShareMoneyFlow/AShareMoneyFlow.h5')

    factor_df = pd.DataFrame()
    factor_df[factor_name] = md_data['SELL_VOLUME_MED_ORDER_ACT'].unstack().diff().rolling(60, min_periods=1).mean().stack()
    # ------------------------------------------------------中户卖出总量近60日差值均值---------------------------------------------------------------
    """
     19.29 -0.04
    """
