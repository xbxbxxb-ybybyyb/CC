...
def factor_fc_nextT1_20230810_5(start_date, end_date, IO, return_fillna_dic=False):
    import sys
    ...
    # -------------------------------------------------------------------------------------------------------------------
    start_date_ = int(s.tradingday(str(start_date), -90)[0])
    md_data = IO.read_data([start_date_, end_date], columns=['BUY_VOLUME_LARGE_ORDER_ACT'], alt='/data/group/800080/warehouse/prod/DATABASE/WIND/AShareMoneyFlow/AShareMoneyFlow.h5')

    factor_df = pd.DataFrame()
    factor_df[factor_name] = md_data['BUY_VOLUME_LARGE_ORDER_ACT'].unstack().diff().rolling(60, min_periods=1).mean().stack()
    # ------------------------------------------------------大户主动买入总量近60日差值均值---------------------------------------------------------------
    """
     19.29 -0.04
    """
