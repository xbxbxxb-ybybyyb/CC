...
def factor_fc_nextT1_20231221_21(start_date, end_date, IO, return_fillna_dic=False):
    import sys
    ...
    c2v_std = md_data['c2v'].unstack().rolling(6, min_periods=5).std().stack()

    factor_df = pd.DataFrame()
    factor_df[factor_name] = (md_data['c2v'] - 2 * c2v_std).unstack().rolling(6, min_periods=1).median().stack()
    # ---------------------------------------------------------------------------------------------------------------------
    """
    close与vwap的差值，变换后取中位值
    20.45 -0.050
    =====>>>> 20.4583 -0.05052 -0.026108273888244344 0.017249687517557764 qyh_next_md_20231130_3，skk_roll_pdown8 0.6367，0.5604
    """
    return factor_df