...
def factor_fc_nextT1_20231221_39(start_date, end_date, IO, return_fillna_dic=False):
    import sys
    ...
    res_std = md_data['res'].unstack().rolling(30, min_periods=5).std().stack()

    factor_df = pd.DataFrame()
    factor_df[factor_name] = (md_data['res'] - res_std * 2).unstack().rolling(6, min_periods=1).median().stack()
    # ---------------------------------------------------------------------------------------------------------------------
    """
    日涨幅与换手乘积滚动中位值
    18.083 -0.035
    =====>>>> 18.083 -0.0354 -35.90287301102985 70.92388517164764 qyh_md_amt_5_20，qyh_next_md_20231130_3 0.5346，0.4768
    """
    return factor_df