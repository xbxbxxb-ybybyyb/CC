...
def factor_fc_TallTick_20231229_22(df, return_fillna_dic=False):
    import sys
    ...
    if len(df) > 0:
        df['Buy2NumOrders'] = df['Buy2NumOrders'].diff().abs().fillna(0).apply(lambda x: round_(x, 6))
        part_df1 = df.query(f'TotalValueTrade >= {seg_threshold}')
        part_df2 = df.query(f'TotalValueTrade <= {seg_threshold}')
        res = np.divide(part_df1['Buy2NumOrders'].mean(), part_df2['Buy2NumOrders'].mean())
        if np.isinf(res):
            res = 0
    else:
        res = 0
    ...
    # ---------------------------------------------------------------------------------------------------------------
    """
    根据成交额对全天进行截断，不同区间上的买单委托笔数的比值
    16.083 -0.049 
    =====>>>> 16.083333333333332 -0.04954235136561973 4.6444723212337164 28.42676923622998 pre_close，tradingday 0.5389，0.5318
    """
