...
def factor_fc_nextT1_20230824_5(start_date, end_date, IO, return_fillna_dic=False):
    import sys
    ...
    res = pd.DataFrame(res.unstack().rolling(15, 2).mean().stack())
    res.columns = [factor_name]
    factor_df = res
    # ------------------------------------------------------五分钟频open与vwap的涨跌幅日内均值的15日均值---------------------------------------------------------------
    """
    22.125 0.061
    """
    return factor_df
