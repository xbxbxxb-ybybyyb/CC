...
def factor_fc_nextT1_20230824_6(start_date, end_date, IO, return_fillna_dic=False):
    import sys
    ...
    res = pd.DataFrame(res.unstack().rolling(4, 2).mean().stack())
    res.columns = [factor_name]
    factor_df = res
    # ------------------------------------------------------五分钟频open与vwap的涨跌幅日内均值的4日均值---------------------------------------------------------------
    """
    17.041 0.048
    """
    return factor_df
