...
def factor_fc_nextT1_20230824_9(start_date, end_date, IO, return_fillna_dic=False):
    import sys
    ...
    res = pd.DataFrame(res.unstack().rolling(30, 2).mean().stack())
    res.columns = [factor_name]
    factor_df = res
    # ------------------------------------------------------五分钟频open与vwap的涨跌幅日内均值的30日均值---------------------------------------------------------------
    """
    17.75 0.04
    """
    return factor_df
