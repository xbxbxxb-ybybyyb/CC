...
def factor_fc_nextT1_20231221_34(start_date, end_date, IO, return_fillna_dic=False):
    import sys
    ...
    res_std = md_data['res'].unstack().rolling(20, min_periods=5).std().stack()

    factor_df = pd.DataFrame()
    factor_df[factor_name] = (md_data['res'] - res_std * 0.2).unstack().rolling(20, min_periods=1).median().stack()
    # ---------------------------------------------------------------------------------------------------------------------
    """
    过去20日涨跌幅与成交量的均值的中位值
    19.541 -0.0463 
    =====>>>> 19.541666666666668 -0.04631765293084578 -2.2817833039098327 10.315791813575286 qyh_md_amt_5_20，skk_turn_r_incre_std 0.3907，0.374
    """
    return factor_df