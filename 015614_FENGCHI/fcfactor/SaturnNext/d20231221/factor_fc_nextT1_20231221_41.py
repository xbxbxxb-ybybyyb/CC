...
def factor_fc_nextT1_20231221_41(start_date, end_date, IO, return_fillna_dic=False):
    import sys
    ...
    res_std = md_data['res'].unstack().rolling(6, min_periods=5).std().stack()

    factor_df = pd.DataFrame()
    factor_df[factor_name] = (md_data['res'] - res_std * 0.2).unstack().rolling(120, min_periods=1).median().stack()
    # ---------------------------------------------------------------------------------------------------------------------
    """
    PV中性化滚动120日中位值
    14.958 -0.0318
    =====>>>> 14.958333333333332 -0.03181791809489326 -1.526088690468917 4.980654267846782 float_shares，Next_float_shares 0.5162，0.5158
    """
    return factor_df