...
def factor_fc_nextT1_20231221_7(start_date, end_date, IO, return_fillna_dic=False):
    import sys
    ...
    c2v_std = md_data['c2v'].unstack().rolling(30, min_periods=5).std().stack()

    factor_df = pd.DataFrame()
    factor_df[factor_name] = (md_data['c2v'] - c2v_std * 0.2).unstack().rolling(20, min_periods=1).median().stack()
    # ---------------------------------------------------------------------------------------------------------------------
    """
    近一个月收盘价相对于vwap的距离的平均
    39.041 -0.0660 
    =====>>>> 39.04166666666667 -0.0660367967807836 -0.001985217362477623 0.003791178184995228 sss_c2vwap_20，skk_20231207_62 0.6346，0.6247
    """
    return factor_df