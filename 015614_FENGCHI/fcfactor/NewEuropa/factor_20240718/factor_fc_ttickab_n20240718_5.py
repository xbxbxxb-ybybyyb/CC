...
class factor_fc_ttickab_n20240718_5(BaseFactor):
    owner = 'fc'
    strategy_name = "europa"
    factor_name = sys._getframe().f_code.co_name[7:]
    fill_na_value = 0
    need_pre_calculate_T_N = True
    factor_explain = "最近100分钟内滚动最高价与滚动最低价的分位数差异"
    zcz_adjusted = "否"
    logic_type = ""
    low_cost = "是"
    ...
    def calculate(self, database):
        if database["skip"] == True:
            ...
            if len(tick_df) > 0:
                res = np.percentile(tick_df["h2l"][-80:], 80) - np.percentile(tick_df["h2l"][-80:], 20)
            else:
                res = None
            # print(res)
