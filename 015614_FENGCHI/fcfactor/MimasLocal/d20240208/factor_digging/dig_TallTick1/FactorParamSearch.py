...
import pandas as pd
from MimasLocal.TestTool.project_2_factor_test_origin import pj2FactorTest, send_message
...
class FactorParamSearch:
    ...
    def calc_factor(df, param_tuple, return_fillna_dic=False):
        ...
        seg_threshold = round_(df[param1].quantile(param2), 6)
        if len(df) > 0:
            df[param3] = df[param3].diff().abs().fillna(0)
            part_df1 = df.query(f'{param1} >= {seg_threshold}')
            part_df2 = df.query(f'{param1} <= {seg_threshold}')
            if part_df2[param3].mean() != 0 and part_df1[param3].mean() != 0:
                res1 = part_df1[param3].std() / part_df1[param3].mean()
                res2 = part_df2[param3].std() / part_df2[param3].mean()
                if res2 != 0:
                    res = res1 / res2
                else:
                    res = 0
...
if __name__ == '__main__':
    note = 'dig_20240208_Mimas_TallTick1'
    ...
    # fps.wrapper([('TotalOfferQty', 0.75, 'Sell1NumOrders')]) # 用于调试