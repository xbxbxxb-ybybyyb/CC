...
class pj2FactorTest:
    ...
    def load_data(self):
        ...
        df = pd.read_pickle(sft_basic_path).loc[pd.to_datetime(str(self.start_date)):pd.to_datetime(str(self.end_date))]
        ...
        if buy_type == '0940':
            all_filter = all_filter & ((df['Next_day_first_ZT_Time'] <= 94000000) == False)& ((df['Next_day_first_DT_Time'] <= 94000000) == False) & available_for_reaction_filter
    ...
    def factor_test(self, factor_df, result_path, factor_corr_test=False, generate_pdf=True):
        ...
        print(round(self.result_dic['check_score_res'].loc['score', 'tot_score'], 3),
              round(self.result_dic['corr_sta'].loc['corr_tot', 'value'], 3),
