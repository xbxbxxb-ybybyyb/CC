...
from MimasLocal.TestTool.run_factor_demo import run_factor
...
def judge_factor_type(factor_name):
    ...
    if factor_name in ['fc_Next1mTransaction_btp_max_pct',
                       'fc_Next1mTransaction_stp_max_pct',
                       ]:
        return 'Next1mTransaction'
    if factor_name in ['fc_TallTrans_btp_max_pct',
                       'fc_TallTrans_p2ul_area_mean_std2',
                       'fc_TallTrans_p2ul_pstd',
                       'fc_TallTrans_stp_max_pct',
                       'fc_TallTrans_p2ul_area_mean_std_v2'
                       ]:
        return 'TallTrans'
    if factor_name_list[2].startswith('202'):
        factor_type = factor_name_list[1]
    if factor_type == 'nextT1':
        return 'Next_T-1_factor'
    elif factor_type == 'next1mTrans':
        return 'Next1mTransaction'
    elif factor_type == 'Next1mTrans':
        return 'Next1mTransaction'
    elif factor_type == 'next1mTick':
        return 'Next1mTickab'
    elif factor_type == 'TallTick':
        return 'TallTick'
    elif factor_type == 'TallTrans':
        return 'TallTrans'
    elif factor_type == 'trans_order':
        return 'TTransaction_TOrder'
    elif factor_type == 'Next1mTransaction':
        return 'Next1mTransaction'
...
if __name__ == '__main__':
    ...
    fixed_factor_list = pd.read_excel('/data/user/015614/fcfactor/Mimas/Mimas冯炽修改_202402/因子列表_fc.xlsx', index_col=1)
    fixed_factor_list = fixed_factor_list.query('是否需要修改==1')['factor_name'].tolist()
    for date in [20230706, 20230713, 20230720, 20231130, 20231207, 20231214,
                 20231221, 20231229, 20240104, 20240111, 20240125, 20240201]:
        basic_file_path = '/data/group/800463/data/project2_public/next_factor_lib/Basic_next_hf_finish_20160101_20191231.h5'
        result_path = f'/data/user/015614/factor/Mimas_nofix_20240126/'
        os.makedirs(result_path, exist_ok=True)
        factor_df = pd.DataFrame()
        sft = pj2FactorTest(start_date, end_date)

        factor_name_list = list(filter(lambda x: str(x).startswith('factor_'), os.listdir(f'/data/user/015614/fcfactor/Mimas/Mimas冯炽修改_202402/factor_{date}/')))
        ...
        for factor_fname in factor_fname_list:
            ...
            if factor_fname[7:] not in fixed_factor_list:
                ...
                continue
            mod_name = f'Mimas.Mimas冯炽修改前_202402.factor_{date}.{factor_fname}'
