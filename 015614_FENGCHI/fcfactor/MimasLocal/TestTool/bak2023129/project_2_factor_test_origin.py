...
class pj2FactorTest:
    def __init__(self, start_date, end_date, segment_number=20, style_list=['T_free_turn', 'Next_o2pre'], cal_mi = False,
    ...
    def load_data(self):
        ...
        sft_basic_path = '/data/group/800463/data/project2_public/next_factor_lib/sft_update_next.h5'
        ...
        if buy_type == '0931':
            all_filter = all_filter & ((df['Next_day_first_ZT_Time'] <= 93100000) == False)& ((df['Next_day_first_ZT_Time'] <= 93100000) == False) & available_for_reaction_filter
        if buy_type == '0940':
            all_filter = all_filter & ((df['Next_day_first_ZT_Time'] <= 94000000) == False)& ((df['Next_day_first_ZT_Time'] <= 93100000) == False) & available_for_reaction_filter
    ...
    def factor_test(self, factor_df, result_path, factor_corr_test=False, generate_pdf=True):
        ...
        print(self.result_dic['check_score_res'].loc['score', 'tot_score'],
              self.result_dic['corr_sta'].loc['corr_tot', 'value'],
              '，'.join(self.result_dic['factor_corr'].query('factor_corr >= 0.7').index.tolist()),
              '，'.join(self.result_dic['factor_corr'].query('factor_corr >= 0.7').loc[:, 'factor_corr'].map(str).values.tolist()))
...
if __name__ == '__main__':
    ...
    factor_fpath = '/data/user/015614/factor/fc_nextT1_16.h5'
    # factor_fpath = '/data/user/015614/factor/fc_TallTrans_9.h5'
    # factor_fpath = '/data/user/015614/factor/fc_TallTick_4.h5'
    # factor_fpath = '/data/user/015614/factor/fc_Next1mTransaction_3.h5'
    factor_df = pd.read_hdf(factor_fpath)
    print(factor_df.describe())

    factor_test = pj2FactorTest(start_date, end_date)
    factor_test.factor_test(factor_df, res_path, factor_corr_test=True)
