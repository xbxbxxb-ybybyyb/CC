...
if __name__ == '__main__':
    ...
    result_path = f'/data/user/015614/factor/d{date}_Mimas/'
    ...
    factor_name_list = list(filter(lambda x: str(x).startswith('factor_'), os.listdir(f'/data/user/015614/fcfactor/Mimas/factor_{date}/')))
    ...
    factor_fname_list = [f'factor_fc_TallTick_20240208_{x}' for x in range(6, 12)]

    this_submit_factor_dict = dict()
    for factor_fname in factor_fname_list:
        print(f'当前测试因子：{factor_fname}')
        mod_name = f'Mimas.factor_{date}.{factor_fname}'
