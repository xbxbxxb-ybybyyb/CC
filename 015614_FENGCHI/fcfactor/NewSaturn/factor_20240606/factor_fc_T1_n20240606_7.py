...
class factor_fc_T1_n20240606_7(BaseFactor):
    ...
    factor_explain = "昨日散户主动买入金额相对于近1年的中位值的差值"
    zcz_adjusted = "否"
    logic_type = ""
    low_cost = "是"
    ...
    t_1_factor_data = [
        {'name': 'AShareMoneyFlow',  # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 250,  # 注意为正数
         'column': ['BUY_VALUE_SMALL_ORDER_ACT']}
    ]
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        b = md_data['BUY_VALUE_SMALL_ORDER_ACT'].unstack().rolling(1, min_periods=1).median().stack()
        a = md_data['BUY_VALUE_SMALL_ORDER_ACT'].unstack().rolling(240, min_periods=1).median().stack()
