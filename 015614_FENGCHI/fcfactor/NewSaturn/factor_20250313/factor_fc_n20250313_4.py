...
class factor_fc_n20250313_4(BaseFactor):
    owner = 'fc'
    strategy_name = "saturn"
    factor_name = sys._getframe().f_code.co_name[7:]
    fill_na_value = 0
    need_pre_calculate_T_N = False
    factor_explain = "不同价格区间撤单量区分度"
    zcz_adjusted = "否"
    logic_type = ""
    low_cost = "是"
    ...
    def calculate(self, database):
        if database["skip"] == True:
            ...
            seg_threshold = df['OrderPrice'].quantile(0.25)
            if len(df) > 0:
            ...
            res1 = part_df1['OrderQty'].sum()
            res2 = part_df2['OrderQty'].sum()
