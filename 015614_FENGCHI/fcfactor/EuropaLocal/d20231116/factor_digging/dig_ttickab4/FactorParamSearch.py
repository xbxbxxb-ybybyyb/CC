...
class FactorParamSearch:
    ...
    def calc_factor(df, param_tuple, return_fillna_dic=False):
        ...
        def calc_res(part_df_):
            ret = part_df_[param2].diff().diff().fillna(0)
            return ret.kurt()
        ...
        if len(df) > 0:
            df['TickVolumeTrade'].iloc[0] = df['TotalVolumeTrade'].iloc[0]
        ...
        if len(up_index) > 0:
            part_df1 = df.query(f's10 >= {up_index[min(len(up_index) - 1, param1)]}')
            part_df2 = df.copy()
