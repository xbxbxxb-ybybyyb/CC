...
def judge_factor_type(factor_name):
    ...
    if factor_name_list[2].startswith('las'):
...
# check1 = pd.read_hdf('/data/user/015614/factor/20230511/fc_trans_lastest_area_ms.h5')
# check2 = pd.read_hdf('/data/user/015614/factor/20230511/fc_trans_lastest_area_ms_fix.h5')
# abs_check = pd.Series(abs(check1.values - check2.values).reshape(-1)).sort_values(ascending=False)

# check1 = pd.read_hdf('/data/user/015614/factor/20230511/fc_order_last50_t5_ratio.h5')
# check2 = pd.read_hdf('/data/user/015614/factor/20230511/fc_order_last50_t5_ratio_fix.h5')
# abs_check = pd.Series(abs(check1.values - check2.values).reshape(-1)).sort_values(ascending=False)
...
if __name__ == '__main__':
    ...
    result_path = f'/data/user/015614/factor/{date}/'
    ...
    factor_name_list = list(filter(lambda x: str(x).startswith('factor_'), os.listdir(f'/data/user/015614/fcfactor/Jupiter/d{date}/')))
    ...
    # factor_fname_list = ['factor_fc_order_20230921_3','factor_fc_order_20230921_4']
    # factor_fname_list = ['factor_fc_trans_order_20230907_8_fix_fix', 'factor_fc_trans_order_20230907_9_fix_fix']
    # factor_fname_list = ['factor_fc_trans_20230928_6', 'factor_fc_trans_20230928_7', 'factor_fc_trans_20230928_5']
    # factor_fname_list = ['factor_fc_trans_lastest_area_ms', 'factor_fc_trans_lastest_area_ms_fix']
    factor_fname_list = ['factor_fc_order_last50_t5_ratio', 'factor_fc_order_last50_t5_ratio_fix']

    this_submit_factor_dict = dict()
    for factor_fname in factor_fname_list:
        print(f'当前测试因子：{factor_fname}')
        mod_name = f'Jupiter.d{date}.{factor_fname}'
