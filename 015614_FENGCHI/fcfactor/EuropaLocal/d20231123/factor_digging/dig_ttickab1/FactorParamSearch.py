...
class FactorParamSearch:
    ...
    def calc_factor(df, param_tuple, return_fillna_dic=False):
        ...
        start_time1 = max(fun_get_time(zt_time, -param2), 93000000)
        start_time2 = max(fun_get_time(zt_time, -param3), 93000000)
        part_df1 = df.query(f'MDTime >= {start_time1}')
        part_df2 = df.query(f'{start_time1} >= MDTime >= {start_time2}')

        def calc_res(df_):
            ret = ((df_[param1] - df_['LowPx'] + 0.0001) / (df_['HighPx'] - df_['LowPx'] + 1e-4)).sum()
            return ret
...
if __name__ == '__main__':
    ...
    param_list = [['WeightedAvgBidPx', 'WeightedAvgOfferPx', 'Buy1Price', 'Sell1Price', 'LastPx'],
                  [5, 10, 20, 30, 60, 90, 120, 180, 300, 600, 1200, 1800, 3600, 7200],
                  [5, 10, 20, 30, 60, 90, 120, 180, 300, 600, 1200, 1800, 3600, 7200]
                  ]
    ...
    # fps.wrapper([('WeightedAvgBidPx', 10, 20)]) # 用于调试