...
root_path = '/data/user/015614/factor/dig_TTrans6_20231123104714_寻出1个已提交，还剩一半/'
note = root_path.split('_')[-1][:-1] + '_part2'
file_list = os.listdir(root_path)
xlsx_list = list(filter(lambda x: x.endswith('.xlsx') and not x.startswith('因子寻优结果'), sorted(file_list)))[390:]
factor_fpath_list = list(filter(lambda x: x.endswith('.pkl'), sorted(file_list)))[390:]
res_list = []
