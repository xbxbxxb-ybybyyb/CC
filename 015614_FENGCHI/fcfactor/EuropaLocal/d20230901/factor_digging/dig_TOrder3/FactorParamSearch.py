...
class FactorParamSearch:
    ...
    def calc_factor(df, param_tuple, return_fillna_dic=False):
        ...
        if len(df) > param1:
            factor = df.iloc[-param1:]['OrderQty'].sum() / df['OrderQty'].sum()
        else:
            factor = 0
...
if __name__ == '__main__':
    note = '20230905测试_TOrder3_TOrder中的买改为卖'
    fps = FactorParamSearch(note=note)
    param_list = [[3, 5, 10, 20, 25, 30, 40, 50, 60, 70, 80, 90, 100, 120, 150, 180, 200, 220, 250, 300, 400, 500, 800, 1000]]
