...
class FactorParamSearch:
    ...
    def wrapper(self, param_list):
        for param_tuple in tqdm(param_list):
            ...
            factor_df = run_factor(self.calc_factor, factor_name, 'TTransaction_TTickab',
    ...
    def calc_factor(df, param_tuple, return_fillna_dic=False):
        ...
        trans_df['10s'] = trans_df['MDTime'] // 10000
        trans_df['s'] = trans_df['MDTime'] // 1000
        trans_df['s10'] = trans_df['MDTime'] // 100
        trans_df['s100'] = trans_df['MDTime'] // 10
        ...
        tick_df['m'] = tick_df['MDTime'] // 100000
        tick_df['10s'] = tick_df['MDTime'] // 10000
        tick_df['s'] = tick_df['MDTime'] // 1000
        tick_df['s10'] = tick_df['MDTime'] // 100
        tick_df['s100'] = tick_df['MDTime'] // 10
        ...
        min_trans_qty = tmp_trans_df.query(f'buy_flag == {param2}').groupby(param3)['TradeQty'].sum(min_count=1)
        min_tick_nums = tmp_tick_df.groupby(param3)['tickNumTrades'].sum(min_count=1)
        ...
        if len(min_trans_qty) > 0 and len(min_tick_nums) > 0:
            ...
            target = factor_s.iloc[-param4:].sum()
