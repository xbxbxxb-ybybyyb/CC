...
root_path = '/data/user/015614/factor/factor_digging_20230907195359_20230907测试/'
note = 'part9_4000-4500'
file_list = os.listdir(root_path)
# xlsx_list = sorted(list(filter(lambda x: x.endswith('.xlsx'), file_list)))[1000:1500]
# factor_fpath_list = sorted(list(filter(lambda x: x.endswith('.pkl'), file_list)))[1000:1500]
# xlsx_list = sorted(list(filter(lambda x: x.endswith('.xlsx'), file_list)))[1500:2000]
# factor_fpath_list = sorted(list(filter(lambda x: x.endswith('.pkl'), file_list)))[1500:2000]
# xlsx_list = sorted(list(filter(lambda x: x.endswith('.xlsx'), file_list)))[2500:3000]
# factor_fpath_list = sorted(list(filter(lambda x: x.endswith('.pkl'), file_list)))[2500:3000]
# xlsx_list = sorted(list(filter(lambda x: x.endswith('.xlsx'), file_list)))[3000:3500]
# factor_fpath_list = sorted(list(filter(lambda x: x.endswith('.pkl'), file_list)))[3000:3500]
# xlsx_list = sorted(list(filter(lambda x: x.endswith('.xlsx'), file_list)))[3500:4000]
# factor_fpath_list = sorted(list(filter(lambda x: x.endswith('.pkl'), file_list)))[3500:4000]
xlsx_list = sorted(list(filter(lambda x: x.endswith('.xlsx') and x.startswith('('), file_list)))[4000:4500]
factor_fpath_list = sorted(list(filter(lambda x: x.endswith('.pkl'), file_list)))[4000:4500]
res_list = []
...
for idx1, factor_name1 in enumerate(all_factor.columns.tolist()):
    print(f'{idx1}/{len(all_factor.columns)}')
...
for index in res_df.index:
    ...
    res_df.loc[index, 'self_high_corr'] = ','.join(self_highest_corr[self_highest_corr > 0.68].index.tolist())
    res_df.loc[index, 'self_high_factor'] = ','.join(self_highest_corr[self_highest_corr > 0.68].map(lambda x: str(round(x, 3))).tolist())
