...
class FactorParamSearch:
    ...
    def calc_factor(start_date, end_date, IO, param_tuple, return_fillna_dic=False):
        ...
        ##########################################
        a = md_data[param1].unstack().rolling(param2, min_periods=1).mean().stack()
        b = md_data[param1].unstack().rolling(param3, min_periods=1).mean().stack()
        factor_df[factor_name] = b - a
        factor_df[factor_name] = factor_df[factor_name].unstack().rolling(param4, min_periods=1).mean().stack()
        return factor_df
...
if __name__ == '__main__':
    ...
    param_list = [['pct_chg', 'turn', 'vwap', 'amt'],
    ...
    # fps.wrapper([('pct_chg', 5, 3, 2)]) # 用于调试