...
class FactorParamSearch:
    ...
    def calc_factor(df, param_tuple, return_fillna_dic=False):
        ...
        itvl = min([2400, int(df.shape[0] / 2), param1])
        df['ItvlValueTrade'] = df['TotalValueTrade'] - df['TotalValueTrade'].shift(itvl).fillna(0)
        df['ItvlVolumeTrade'] = df['TotalVolumeTrade'] - df['TotalVolumeTrade'].shift(itvl).fillna(0)
        df['tick_vwap'] = df['ItvlValueTrade'] / df['ItvlVolumeTrade']
        df['dis_bo2vwap'] = (df['WeightedAvgOfferPx'] + df['WeightedAvgBidPx'] - 2 * df['tick_vwap']) / pre_close
        if zcz: df['dis_bo2vwap'] /= 2

        cut_loc = min(param2, (len(df) - df['dis_bo2vwap'].isna().sum()))
        res = df['dis_bo2vwap'][-cut_loc:].mean()
...
if __name__ == '__main__':
    ...
    param_list = [[1, 2, 3, 5, 10, 20, 40, 60],   # 几个tick
