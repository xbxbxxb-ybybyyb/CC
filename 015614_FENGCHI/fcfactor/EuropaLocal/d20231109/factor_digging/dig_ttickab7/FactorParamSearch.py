...
class FactorParamSearch:
    ...
    def calc_factor(df, param_tuple, return_fillna_dic=False):
        ...
        def calc_res(part_df):
            ...
            res = (part_df['LastPx'] - part_df['WeightedAvgBidPx']) / pre_close / (int(zcz) + 1)
            return np.log(res.iloc[-1] / res.mean()) if res.mean() != 0 else 1
        ...
        part_df1 = df.iloc[:min(len(df) // 3 * 2, len(df) - param1)]
...
if __name__ == '__main__':
    ...
    param_list = [[20, 40, 60, 90, 120, 300, 600, 1200, 2400, 3600]]
