...
class FactorParamSearch:
    ...
    def calc_factor(df, param_tuple, return_fillna_dic=False):
        ...
        def calc_res(part_df_):
            if len(part_df_) == 0: return 0
            part_df = part_df_.copy()
            part_df['ValueTrade'] = part_df['TotalValueTrade'].iloc[-1] - part_df['TotalValueTrade'].iloc[0]
            part_df['VolumeTrade'] = part_df['TotalVolumeTrade'].iloc[-1] - part_df['TotalVolumeTrade'].iloc[0]
            part_df['vwap'] = part_df['ValueTrade'] / part_df['VolumeTrade']
            res = ((part_df["WeightedAvgOfferPx"] + part_df["WeightedAvgBidPx"] - part_df['vwap'] * 2) / (part_df["LastPx"]) * 100 - 100) / (zcz + 1)
            return res.mean()
        ...
        df = df.query('MDTime >= 93000000 & NumTrades > 0')

        part_df1 = df.iloc[min(len(df) // param2 * (param2-2), len(df) - param1 - 20) : min(len(df) // param2 * (param2-1), len(df) - param1)]
        part_df2 = df.iloc[-min(len(df), param1):]
        ret = calc_res(part_df1) * calc_res(part_df2)
...
if __name__ == '__main__':
    ...
    param_list = [[3, 5, 8, 10, 20, 40, 60, 90, 120, 300, 600, 1200, 2400, 3600],
                  [3, 4, 5, 6, 7]]
