...
class FactorParamSearch:
    ...
    def calc_factor(df, param_tuple, return_fillna_dic=False):
        ...
        def calc_res(part_df_):
            ...
            part_df['pctchg'] = (part_df[param3] - part_df['WeightedAvgOfferPx']) / pre_close / (zcz + 1)
            res = part_df['pctchg'].std() / part_df['pctchg'].mean() if part_df['pctchg'].mean() != 0 else np.nan
            return res
        ...
        # 对全天分段
        part_df1 = df.iloc[min(len(df) // 2, param2):]
...
if __name__ == '__main__':
    ...
    param_list = [[3, 5, 8, 10, 20, 40, 60, 90, 120, 300, 600, 1200, 2400, 3600],
                  [40, 60, 90, 120, 300, 600, 1200, 2400, 3600],
                  ['Buy1Price', 'Sell1Price', 'Sell5Price', 'Buy5Price']]
    ...
    # fps.wrapper([(3, 40, 'Buy1Price')]) # 用于调试