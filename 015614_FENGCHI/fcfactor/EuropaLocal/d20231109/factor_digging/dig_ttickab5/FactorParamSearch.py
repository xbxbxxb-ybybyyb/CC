...
class FactorParamSearch:
    ...
    def calc_factor(df, param_tuple, return_fillna_dic=False):
        ...
        def calc_res(part_df):
            if len(part_df) == 0: return 9
            res =  (part_df['LastPx'] - df['WeightedAvgBidPx']) / pre_close / (zcz + 1)
            return np.log(res.iloc[-1] / (res.mean() + 1e-3))
        ...
        part_df1 = part_df.iloc[-min(len(part_df), param2):]
        part_df2 = part_df.iloc[:min(len(part_df), param2)]
...
if __name__ == '__main__':
    ...
    param_list = [[60, 90, 120, 300, 600, 1200, 1800, 3600, 4800, 7200],
                  [10, 20, 30, 40, 50]]
