...
class FactorParamSearch:
    ...
    def calc_factor(df, param_tuple, return_fillna_dic=False):
        ...
        def calc_res(part_df):
            return (part_df['high_pctchg'] - part_df['low_pctchg']).std()
        ...
        df['high_pctchg'] = (df['HighPx'] - df['pre_close']) / pre_close / (int(zcz) + 1)
        df['low_pctchg'] = (df['LowPx'] - df['pre_close']) / pre_close / (int(zcz) + 1)

        part_df1 = df.iloc[-min(len(df), param1):]
        part_df2 = df.iloc[:min(len(df) // 2, param1)]
...
if __name__ == '__main__':
    ...
    param_list = [3, 5, 10, 20, 40, 60, 100, 200, 600, 1200, 2400],   # 几个tick 20个tick 1min
