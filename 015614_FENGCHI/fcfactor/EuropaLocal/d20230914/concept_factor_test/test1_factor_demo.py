...
class strongFactorTest:
    ...
    def load_data(self):
        df = IO.read_data([self.start_date, self.end_date], alt='/data/group/800463/data/project1_public/factor_lib_v2/sft_update_europa.h5')
        if self.filter_name != '':
            df = IO.read_data([self.start_date, self.end_date], alt='/data/group/800463/data/project1_public/factor_lib_v2/sft_update_europa_%s.h5' % self.filter_name)
        ...
        df['label_pattern'] = (df['label_pattern'] == 4).astype(int)
        df = df.rename(columns={'label_pattern': 'class', 'label_TN_931o2ul': 'value'})
        df['value'] = df['value'] * 100
        df['mixed'] = ((df['value'] > 0) & (df['label_T_is_zt'] == 1)).apply(lambda x: 1 if x else 0)
    ...
    def factor_test(self, factor_df, result_path, factor_corr_test=False, generate_pdf=True, tag=''):
        ...
        print(self.basic_df['factor'].describe())
        ...
        if 'ZT_Time' in self.result_dic['double_group_sort']:
            self.result_dic['check_diff_res'], self.result_dic['check_score_res'] = self.check_score(self.result_dic['double_group_sort']['ZT_Time'].copy(), self.result_dic['corr_month'].copy(),
                                                                                                     np.sign(self.result_dic['corr_sta'].loc['corr_tot']))
            self.result_dic['check_score_list'] = self.check_score_list(self.result_dic['double_group_sort_dic'].copy(), self.result_dic['corr_month_dic'].copy(),
                                                                        np.sign(self.result_dic['corr_sta'].loc['corr_tot']))
        ...
        if self.factor_corr_test:
            ...
            self.result_dic['factor_corr_summary'] = self.result_dic['factor_corr_summary'][
                (self.result_dic['factor_corr_summary']['factor_corr'] > 0.7) | (self.result_dic['factor_corr_summary']['max'] > 0.9)]
    ...
    def generate_pdf(self, result_dic, save_path):
        ...
        # h_score_list_sta
        p_score_list_sta = generate_plot(result_dic['check_score_list'][['mixed_diff_score', 'mixed_stability_score', 'value_diff_score', 'value_stability_score']], plot_name='Interval Score',
                                         x_label='interval', y_label='score', color=['#525252', '#1f4068', '#900d0d', '#e23e57'], plot_type='bar', stacked=True)
        elements += [h_score_list_sta, p_score_list_sta]

        h_monthly_corr = Paragraph('Monthly Corr', pdf_styles['Heading4'])
        p_monthly_corr = generate_plot(result_dic['corr_month'][['value', 'class', 'mixed']], plot_name='Monthly Corr - Class&Mixed', x_label='month', y_label='corr of class&mixed', plot_type='bar',
                                       color=['#e23e57', '#1b7fbd', '#525252'], rot=25)
        ...
        p_group_test_cm_with_style0 = generate_plot(self.result_dic['group_tot_time_sort'][self.style_list[0]][['class', 'mixed']],
                                                    plot_name='Group Test - Class&Mixed - %s Neutralization' % (self.style_list[0]),
                                                    x_label='Group', y_label='class&mixed', plot_type='bar', color=['#1b7fbd', '#525252'])
    ...
    def check_score(self, double_group_df, month_df, dir, label_test=['value', 'mixed']):
        ...
        for label in label_test:
            ...
            diff_res_df['%s_high_reverse' % (label)], diff_res_df['%s_mid_reverse' % (label)] = diff_res_df['%s_diff' % (label)] * diff_dic[label]['reverse'][1], diff_res_df['%s_diff' % (label)] * \
                                                                                                diff_dic[label]['reverse'][0]
            diff_res_df['%s_standard' % (label)] = 1
            diff_res_df.loc[
                ((diff_res_df['%s_diff' % (label)] > diff_dic[label]['diff'][0]) & (diff_res_df['%s_reverse' % (label)] < diff_res_df['%s_mid_reverse' % (label)])), '%s_standard' % (label)] = 2
            diff_res_df.loc[
                ((diff_res_df['%s_diff' % (label)] > diff_dic[label]['diff'][1]) & (diff_res_df['%s_reverse' % (label)] < diff_res_df['%s_mid_reverse' % (label)])), '%s_standard' % (label)] = 3
    ...
    def get_factor_information(self):
        ...
        factor_information['Factor Info'] = pd.Series({'Factor Name': self.factor_name,
                                                       ...
                                                       'Nan|Inf Count': len(self.factor_df) - len(self.factor_df[self.factor_df['factor'] == self.factor_df['factor']]) + np.sum(
...
if __name__ == '__main__':
    ...
    concept_list = ['所属Wind概念数量',
                    ...
                    '昨日所属SW2涨停个数',
                    '昨日所属SW2触板个数']
    print(concept_list[idx-1])
    # factor_df = pd.read_hdf('/data/user/015614/factor/fc_order1.h5')
    # factor_df = pd.read_hdf('/data/user/015614/factor/fc_trans1.h5')
    # factor_df = pd.read_hdf('/data/user/015614/factor/fc_tickab1.h5')
    # factor_df = pd.read_hdf('/data/user/015614/factor/fc_trans_order1.h5')
    # factor_df = pd.read_hdf('/data/user/015614/factor/fc_trans_tickab9.h5')
    # factor_df = pd.read_hdf('/data/user/015614/factor/fc_T1_20230824_1.h5')
    factor_df = pd.read_hdf(f'/data/user/015614/factor/fc_T1_concept_test_{idx}.h5', key=f'fc_concept_test_{idx}')

    # print(factor_df.describe())
    # factor_df[factor_df < 0.004] = 0.04
    # factor_df.quantile(14/20)
    # np.quantile(np.array(factor_df.iloc[:, 0].tolist()), 0.68)
    # factor_df1 = (factor_df0 - np.median(factor_df0)) ** 2
    # print(factor_df1.describe())
    sft.factor_test(factor_df, result_path=res_path, factor_corr_test=True, generate_pdf=True)
