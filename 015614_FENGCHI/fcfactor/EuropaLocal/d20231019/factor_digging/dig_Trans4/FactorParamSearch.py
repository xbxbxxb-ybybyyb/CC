...
class FactorParamSearch:
    ...
    def calc_factor(df, param_tuple, return_fillna_dic=False):
        ...
        if len(df) > 0:
            big, small = df['TotalValueTrade'].quantile([param1, 1 - param1])
            big, small = round_(big, 2), round_(small, 2)
            df_big = df[df['TotalValueTrade'] >= big]
            if len(df_big) > param2: df_big = df_big.head(param2)
            df_small = df[df['TotalValueTrade'] <= small]
            if len(df_small) > param2: df_small = df_small.head(param2)

            if df_big['TotalValueTrade'].sum() > 0:
                pct1 = (df_big['TotalValueTrade'].sum() / df_big['TotalVolumeTrade'].sum()) / pre_close - 1
            else:
                pct1 = np.nan
            if df_small['TotalValueTrade'].sum() > 0:
                pct2 = (df_small['TotalValueTrade'].sum() / df_small['TotalVolumeTrade'].sum())  / pre_close - 1
            else:
                pct2 = np.nan
            res = pct1 - pct2
        else:
...
if __name__ == '__main__':
    ...
    param_list = [[0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4],
                 [20, 30, 40, 50, 80, 100, 120, 150, 200, 400, 600, 1200]]
