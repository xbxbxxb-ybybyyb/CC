...
class FactorParamSearch:
    ...
    def calc_factor(df, param_tuple, return_fillna_dic=False):
        ...
        if switch == 1:
            shift_time = param1
            start_time = max(fun_get_time(ZT_Time, -shift_time), 93000000)
            part_df = df.query(f'MDTime >= {start_time}')

        if switch == 2:
            shift_trans_num = param1
            part_df = df.iloc[-min(len(df), shift_trans_num):]

        if len(part_df.groupby(['TradePrice'])['TradeQty'].sum()) <= 1:
            res = 0
        else:
            res = part_df.groupby(['TradePrice'])['TradeQty'].sum().reset_index().corr(method='spearman').values[0][1]
