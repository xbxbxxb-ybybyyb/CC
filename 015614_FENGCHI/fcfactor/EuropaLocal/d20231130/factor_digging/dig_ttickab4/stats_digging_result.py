...
root_path = '/data/user/015614/factor/dig_TTickab4_20231201103218/'
note = root_path.split('_')[-1][:-1]
file_list = os.listdir(root_path)
xlsx_list = list(filter(lambda x: x.endswith('.xlsx') and not x.startswith('因子'), sorted(file_list)))[:400]
factor_fpath_list = list(filter(lambda x: x.endswith('.pkl'), sorted(file_list)))[:400]
res_list = []
...
week_submit_date = 20231130
week_root_path = f'/data/user/015614/factor/{week_submit_date}/'
...
# 挑选样本
res_df2 = res_df.copy()
res_df2 = res_df2.sort_values('score', ascending=False).query('score > 30 & highest_corr < 0.695 & high_corr_s_num == 0')
res_df2['drop'] = 0
res_df2['commit'] = 0
while True:
