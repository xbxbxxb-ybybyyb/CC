...
class FactorParamSearch:
    ...
    def calc_factor(df, param_tuple, return_fillna_dic=False):
        ...
        if type(param_tuple) == tuple:
            param1, param2, param3, param4 = param_tuple
        else:
            param1, param2, param3, param4 = param_tuple
        ...
        part_df = df.copy()
        part_df[param4] = part_df[param4].diff().diff().fillna(0)
        part_df[param1] = (part_df[param1] / pre_close - 1) / (1 + zcz)
        part_df = part_df.query(f'{param1} >= {param2}')
        part_df[param3] = (part_df[param3] - pre_close) / pre_close / (1 + zcz)
        part_df = part_df.query(f'{param4} > 0')
        res = part_df[param3].mean() / part_df[param4].mean() * 1e5
...
if __name__ == '__main__':
    ...
    param_list = [['WeightedAvgBidPx', 'WeightedAvgOfferPx', 'LastPx', 'vwap'],
                  [0.04, 0.05, 0.06, 0.07, 0.08, 0.09],
                  [f'Sell{x}Price' for x in range(1, 11) if x % 2 == 0],
                  ['TotalValueTrade', 'TotalVolumeTrade', 'TotalOfferQty', 'TotalBidQty', 'NumTrades']
                  ]
    ...
    # fps.wrapper([(14400, 0.08, 'Sell5Price', 'TotalVolumeTrade')]) # 用于调试