...
class FactorParamSearch:
    ...
    def calc_factor(df, param_tuple, return_fillna_dic=False):
        ...
        part_df = df.query(f'TradeMoney >= {param2} & MDTime >= {start_time}')

        part_df['pct'] = (part_df['TradePrice'] / pre_close - 1) / (zcz + 1)
        part_df['pct_abs'] = part_df['pct'].diff().diff().abs()
        res = part_df['pct_abs'].dropna().mean() * 1e4
        # print(res)
...
if __name__ == '__main__':
    ...
    param_list = [[10, 30, 60, 90, 120, 300, 600, 1200, 1800, 3600, 6000, 7200],
