...
class FactorParamSearch:
    ...
    def calc_factor(df, param_tuple, return_fillna_dic=False):
        factor_name = str(param_tuple)
        # param1: 涨幅阈值 0.002, 0.005, 0.008, 0.01           1min对应20个tick，1h对应1200个tick
        # param2: shift tick个数 1, 3, 5, 10, 20, 30, 40, 50, 60
        # param3: 拉升时的范围  3, 10, 20, 60, 100, 200, 600, 1200
        ...
        df['itvl_pct'] = ((df['LastPx'] - df['LastPx'].shift(param2)) / pre_close).apply(lambda x: round_(x, 4))
        if zcz: df['itvl_pct'] /= 2

        if len(df) <= param3:
            res = 0
        else:
            df_head = df.head(df.shape[0] - param3)
            res1 = df_head.query(f'itvl_pct >= {param1}').shape[0] / df_head.shape[0]
            df_tail = df.tail(param3)
            res2 = df_tail.query(f'itvl_pct >= {param1}').shape[0] / df_tail.shape[0]
            res = res2 - res1
...
if __name__ == '__main__':
    ...
    param_list = [[0.002, 0.005, 0.008, 0.01],  # 4
                  [1, 3, 5, 10, 20, 30, 40, 50, 60], # 9
                  [3, 10, 20, 60, 100, 200, 600, 1200]] # 8
