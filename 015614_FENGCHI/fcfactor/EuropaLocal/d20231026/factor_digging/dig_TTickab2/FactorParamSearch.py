...
class FactorParamSearch:
    ...
    def calc_factor(df, param_tuple, return_fillna_dic=False):
        ...
        start_time = max(93000000, fun_get_time(ZT_Time, -param1 * 60))
        df = df.query(f'MDTime > {start_time}')

        df['min'] = df['MDTime'].map(lambda x: x // 1e5)
        df['10s'] = df['MDTime'].map(lambda x: x // 1e4)
        df['30s'] = df['MDTime'].map(lambda x: x // 3e4)
        df['pct'] = df[param2] / pre_close - 1
        if zcz: df['pct'] /= 2
        group_df = df.groupby(param3)['pct'].mean()
...
if __name__ == '__main__':
    ...
    param_list = [[5, 10, 30, 60, 120, 240],  # 6
                  ['Buy1Price', 'Buy3Price', 'Buy5Price', 'Sell1Price', 'Sell5Price'], # 5
                  ['min', '10s', '30s']] # 8
    ...
    # fps.wrapper([(5, 'Buy1Price', 'min')]) # 用于调试