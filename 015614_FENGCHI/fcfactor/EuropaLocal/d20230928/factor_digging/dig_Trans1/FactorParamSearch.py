...
class FactorParamSearch:
    ...
    def calc_factor(df, param_tuple, return_fillna_dic=False):
        ...
        group_sell_df = sell_df.groupby('TradeSellNo').agg({'TradeMoney': sum,
                                                            'TradeIndex': max})  # 去除散户
        mid_big_group_sell = group_sell_df.query(f'TradeMoney > {param1}')
        last_index = mid_big_group_sell['TradeIndex'].iloc[-param2] if len(mid_big_group_sell) >= param2 else 0
...
if __name__ == '__main__':
    ...
    param_list = [[0, 50000, 100000, 200000, 500000],
                  [1, 2, 3, 4, 5]
                 ]
