...
class FactorParamSearch:
    ...
    def calc_factor(df, param_tuple, return_fillna_dic=False):
        ...
        short_time = max(fun_get_time(zt_time, -param1), 93000000)
        long_time = max(fun_get_time(zt_time, -param2), 93000000)
        short_df = df.query(f'MDTime >= {short_time}')
        long_df = df.query(f'MDTime >= {long_time}')

        if len(long_df) != 0:
            ret = short_df.groupby('TradeBuyNo').sum()['TradeQty'].mean() / long_df.groupby('TradeBuyNo').sum()['TradeQty'].std()
        else:
            ret = 1.0
...
if __name__ == '__main__':
    ...
    param_list = [[5, 10, 15, 30, 60, 90, 120, 300, 600, 1200],
                  [10, 15, 30, 60, 120, 300, 600, 1200, 6000]
                 ]
