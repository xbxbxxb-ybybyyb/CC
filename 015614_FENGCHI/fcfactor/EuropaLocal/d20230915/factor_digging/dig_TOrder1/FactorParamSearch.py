...
class FactorParamSearch:
    def __init__(self, note=None):
        ...
        self.res_path = f'/data/user/015614/factor/factor_digging_20230915102228_20230914测试_order分组_已提交一部分/'
    ...
    def calc_factor(df, param_tuple, return_fillna_dic=False):
        ...
        if param1:
            df = df[df['OrderBSFlag'].isin([1])]

        df = df[df['MDTime'] >= max(fun_get_time(zt_time, -param2), 93000000)]
        df = generate_group(df, group_num=param3)
        group_df = df.groupby('group_id')['OrderQty'].sum()
        factor = group_df[-param4:].sum() / group_df[:param4].sum()
