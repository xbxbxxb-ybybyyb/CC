...
class FactorParamSearch:
    ...
    def calc_factor(df, param_tuple, return_fillna_dic=False):
        ...
        df['p2v'] = df[param1] - df['WeightedAvgOfferPx']
        df = df[df['MDTime'] >= max(fun_get_time(zt_time, -param2), 93000000)]

        if len(df) > 0:
            res = (df['p2v'][df['p2v'] > df['p2v'].quantile(param3)].mean() - df[df < df.quantile(1-param3)]['p2v'].mean()) / pre_close
        else:
            res = np.nan
...
if __name__ == '__main__':
    ...
    param_list = [['Buy1Price', 'Buy2Price', 'Buy3Price', 'Buy4Price', 'Buy5Price', 'Sell1Price'],
                  [60, 90, 120, 180, 360, 480, 600, 900, 1200, 1800, 2400, 3000, 3600, 7200],
    ...
    # fps.wrapper([('Buy1Price', 10, 0.2)]) # 用于调试