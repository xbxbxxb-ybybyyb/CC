...
class FactorParamSearch:
    ...
    def calc_factor(df, param_tuple, return_fillna_dic=False):
        ...
        df1 = df[df['MDTime'] >= max(fun_get_time(zt_time, -param2), 93000000)]
        df1['pct'] = df1[param1] / pre_close - 1
        df1['pct'] = df1['pct'] / 2 if zcz else df1['pct']
        df1['pct'] = df1['pct'].apply(lambda x: round_(x, 5))
        res1 = df1['pct'].kurt()

        df2 = df[df['MDTime'] >= max(fun_get_time(zt_time, -param3), 93000000)]
        df2['pct'] = df2[param1] / pre_close - 1
        df2['pct'] = df2['pct'] / 2 if zcz else df2['pct']
        df2['pct'] = df2['pct'].apply(lambda x: round_(x, 5))
        res2 = df2['pct'].kurt()
...
if __name__ == '__main__':
    ...
    param_list = [['WeightedAvgBidPx', 'WeightedAvgOfferPx'],
                  [10, 20, 30, 60, 90, 120, 180, 360, 480, 600],
                  [15, 20, 30, 60, 90, 120, 180, 360, 480, 600, 1200]
                 ]
    ...
    # fps.wrapper([('WeightedAvgBidPx', 10, 15)]) # 用于调试