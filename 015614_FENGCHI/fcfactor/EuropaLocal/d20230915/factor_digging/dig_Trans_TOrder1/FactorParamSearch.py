...
class FactorParamSearch:
    ...
    def calc_factor(df, param_tuple, return_fillna_dic=False):
        ...
        order_df = order_df.query(f'OrderBSFlag == 1')  # 买入委托
        ...
        trans_factor1 = part_trans_df1['TradeQty'].std()
        trans_factor2 = part_trans_df2['TradeQty'].std()
        trans_factor3 = part_trans_df3['TradeQty'].std()
        order_factor1 = part_order_df1['OrderQty'].std()
        order_factor2 = part_order_df2['OrderQty'].std()
        order_factor3 = part_order_df3['OrderQty'].std()

        if np.isnan(trans_factor1): trans_factor1 = 0
        if np.isnan(trans_factor2): trans_factor2 = 0
        if np.isnan(trans_factor3): trans_factor3 = 0
        if np.isnan(order_factor1): order_factor1 = 0
        if np.isnan(order_factor2): order_factor2 = 0
        if np.isnan(order_factor3): order_factor3 = 0

        factor = (trans_factor1 - order_factor2 + 0.001) / (trans_factor1 + order_factor2 + 0.001)
...
if __name__ == '__main__':
    ...
    param_list = [[3, 5, 8, 10, 12, 15, 18, 21, 30, 45, 60],
                  [20, 30, 40, 50, 60, 80, 100, 120, 150, 180],
                  [30, 36, 40, 50, 60, 70, 80, 90, 100, 120, 150, 180, 240, 300, 360, 420, 480],]
    param_list = list(product(*param_list))

    # 检测已有的因子

    exist_factor_param_list = list(filter(lambda x: str(x).endswith('.pdf'), os.listdir(fps.res_path)))
    exist_factor_param_list = list(map(lambda x: eval(str(x).split('_')[0]), exist_factor_param_list))
    param_list = list(set(param_list).difference(set(exist_factor_param_list)))

    param_list = list(filter(lambda x: (x[0] < x[1]) & (x[1] < x[2]) , param_list))
    multiprocess(10, fps.wrapper, param_list)
    # multiprocess(1, fps.wrapper, param_list[0])
    # fps.wrapper([(3, 20, 30)]) # 用于调试