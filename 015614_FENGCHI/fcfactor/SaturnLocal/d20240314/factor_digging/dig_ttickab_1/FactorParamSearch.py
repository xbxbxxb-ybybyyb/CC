...
class FactorParamSearch:
    ...
    def calc_factor(df, param_tuple, return_fillna_dic=False):
        ...
        deal_df = df.query('TotalVolumeTrade != 0')
        jhjj_volume = deal_df['TotalVolumeTrade'].values[0] if len(deal_df) != 0 else 1.0

        part_df = df.query(f'MDTime >= {param1}')
        order_list = list(range(1, param2))
        part_order_qty = (part_df[[f'Buy{x}OrderQty' for x in order_list]]).sum().sum() if len(part_df) != 0 else 1

        res = jhjj_volume / (jhjj_volume + part_order_qty)
...
if __name__ == '__main__':
    note = 'dig_20240314_Saturn_tick_1'
