...
def run_factor(func, factor_name, factor_type, start_date, end_date, basic_file_path, result_path, append_next_tradingday=False, interval_res=True,
               t=None, last_factor_dic = None, param_tuple=None, multi=False,
    ...
    if factor_type in  ['TTransaction', 'TTickab','T1mTransaction','T1mTickab','T10mTransaction','T10mTickab',
            ...
            for i in range(len(tradingday_list)):
                ...
                if factor_type[-2:] == 'cs':
                    ...
                    if (t=='t') and (last_factor_dic is not  None):
                        tmp_df = data.groupby(level=[0, 1]).apply(lambda x: func(x, last_value=last_factor_dic[x.iloc[0].name], param_tuple=param_tuple))
                    else:
                        tmp_df = data.groupby(level=[0, 1]).apply(lambda x: func(x, param_tuple=param_tuple))
        ...
        last_factor_df = run_factor(func, 'last_'+factor_name, last_data_type, start_date, end_date, basic_file_path, result_path, append_next_tradingday, interval_res, param_tuple=param_tuple, t='last')
        if append_next_tradingday == False: #非日常更新的情况
            last_factor_dic = dict(last_factor_df['last_'+factor_name])
            result_df = run_factor(func, factor_name, t_data_type, start_date, end_date, basic_file_path, result_path, append_next_tradingday, interval_res, param_tuple=param_tuple, t='t', last_factor_dic=last_factor_dic)
        else:
