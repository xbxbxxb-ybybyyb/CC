...
week_date = 20240509
root_path = f'/data/user/015614/fcfactor/Saturn/factor_{week_date}/'
...
res = pd.DataFrame(columns=['factor_name', 'factor_type', 'factor_owner', '因子逻辑', '提交时间', 'emotion', '填充值', 'T-1日类别', '逻辑类别', '是否针对注册制做调整'])
for factor_idx, factor_fpath in enumerate(factor_fpath_list):
    ...
    res.loc[factor_idx, '是否针对注册制做调整'] = '否' if zcz_times < 2 else '是'
    res.loc[factor_idx, '提交时间'] = week_date

res.to_excel(f'/data/user/015614/junkData/{week_date}_fc.xlsx', index=0)
