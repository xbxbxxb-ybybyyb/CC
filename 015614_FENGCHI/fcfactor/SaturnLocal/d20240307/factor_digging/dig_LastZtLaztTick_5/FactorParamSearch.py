...
class FactorParamSearch:
    ...
    def calc_factor(df, param_tuple, return_fillna_dic=False):
        ...
        i = param2
        activeness = df['tick_volume'] / (df[['Buy%dOrderQty' % (j + 1) for j in range(i + 1)]].sum(axis=1) + df[['Sell%dOrderQty' % (j + 1) for j in range(i + 1)]].sum(axis=1))
        compact_ratio = (df[['Buy%dOrderQty' % (j + 1) for j in range(i + 1)]].sum(axis=1) / df[['Sell%dOrderQty' % (j + 1) for j in range(i + 1)]].sum(axis=1))
        activeness[np.abs(activeness) == np.inf] = 0
        compact_ratio[np.abs(compact_ratio) == np.inf] = 0

        if activeness.std() != 0:
            res1 = activeness.std() / activeness.mean()
        else:
            res1 = 0

        if compact_ratio.std() != 0:
            res2 = compact_ratio.std() / compact_ratio.mean()
        else:
            res2 = 0
        res = res1 + res2
...
if __name__ == '__main__':
    note = 'dig_20240307_Saturn_LastZtLastTick_5'
    fps = FactorParamSearch(note=note)
    param_list = [list(range(1, 4)),
                  list(range(2, 11))
                  ]
