...
class FactorParamSearch:
    ...
    def calc_factor(df, param_tuple, return_fillna_dic=False):
        ...
        # TODO: fix
        time_gap = param2
        zt_time_end = fun_get_time(zt_time, time_gap)
        df2 = df.query(f'{zt_time} <= MDTime <= {zt_time_end}')

        if len(df) >= param1:
            after_zt_amt = (df2['TotalBidQty'] * df2['WeightedAvgBidPx']).head(param1).sum()
        else:
...
if __name__ == '__main__':
    note = 'dig_20240307_Saturn_LastZtLastTick_7'
    fps = FactorParamSearch(note=note)
    param_list = [[1, 2, 3],
                  [360, 420, 480, 600, 900, 1200]
                  ]
