...
class FactorParamSearch:
    ...
    def calc_factor(df, param_tuple, return_fillna_dic=False):
        ...
        df = df.query(f'{param2} != 0')
        df = df.query('130000000 <= MDTime <= 150000000')

        df['tick_delta'] = df[param1] - df[param1].shift().fillna(0)
        if len(df) == 0:
            res = 0
        else:
            if len(df.groupby(param2).sum()['tick_delta']) <= 2:
                res = 0
            else:
                df['bid_ratio'] = df['TotalBidQty'] / (df['TotalBidQty'] + df['TotalOfferQty'])
                res = df[['bid_ratio', param2]].corr(method='spearman').values[0][1]
        # print(res)
...
if __name__ == '__main__':
    note = 'dig_20240307_Saturn_LastZtLastTick_2'
    fps = FactorParamSearch(note=note)
    param_list = [['TotalVolumeTrade', 'TotalValueTrade', 'TotalBidQty', 'TotalOfferQty'],
                  [f'Buy{x}Price' for x in range(10)] + ['LastPx'],
                  ]
    ...
    # multi = True
    # multi = False
    # fps.wrapper([('TotalVolumeTrade', 'Buy1Price')]) # 用于调试