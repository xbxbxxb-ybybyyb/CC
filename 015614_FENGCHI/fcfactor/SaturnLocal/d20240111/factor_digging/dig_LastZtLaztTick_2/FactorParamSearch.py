...
class FactorParamSearch:
    def __init__(self, note=None):
        ...
        note = note
        self.basic_file_path = '/data/group/800463/data/project2_public/factor_lib/Basic_closed_hf_finish_20160101_20191231.h5'
    ...
    def wrapper(self, param_list):
        for param_tuple in tqdm(param_list):
            ...
            factor_df = run_factor(self.calc_factor, factor_name, 'LastZtLastTick',
                           self.start_date, self.end_date, self.basic_file_path, self.res_path, param_tuple=param_tuple, interval_res=False, multi=multi)
    ...
    def calc_factor(df, param_tuple, return_fillna_dic=False):
        ...
        zt_time = df.query(f'LastPx == {ul_price}')['MDTime'].iloc[0]
        zcz = ((Ticker[0:2] == '30') & (dt.strftime('%Y%m%d') >= '20200824')) | (Ticker[0:2] == '68')
        start_time = min(fun_get_time(zt_time, param1), 150000000) # zt往后一小时
        part_df = df.query(f'{start_time} <= MDTime <= 150000000')

        part_df = part_df.query(f'{param2} > 0 & {param3} > 0')

        if len(df) > 0:
            part_df['price_diff'] = (part_df[param3] - part_df[param2]) / pre_close # gap
            if zcz: part_df['price_diff'] /= part_df['price_diff']
            res = part_df['price_diff'].mean()
        else:
            res = np.nan

        factor_dict = {factor_name: res * 1e3}
        # ---------------------------------------------------------------------------------------------------------------
        """
...
if __name__ == '__main__':
    note = 'dig_Saturn_LastZtLastTick_2'
    fps = FactorParamSearch(note=note)
    param_list = [[600, 1800, 3600],
        [f'Buy{x}Price' for x in range(1, 11)],
        [f'Sell{x}Price' for x in range(1, 11)]
                  ]
    ...
    # multi = True
    # multi = False
    # fps.wrapper([(600, 'Buy1Price', 'Sell1Price')]) # 用于调试