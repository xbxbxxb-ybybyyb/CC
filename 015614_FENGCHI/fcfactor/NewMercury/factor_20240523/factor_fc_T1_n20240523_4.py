...
class factor_fc_T1_n20240523_4(BaseFactor):
    owner = 'fc'
    strategy_name = "mercury"
    factor_name = sys._getframe().f_code.co_name[7:]
    fill_na_value = 0
    need_pre_calculate_T_N = True
    factor_explain = "过去1日散户主动卖出总量相对于近30日的中位值差异"
    zcz_adjusted = "否"
    logic_type = ""
    low_cost = "是"
    ...
    t_1_factor_data = [
        {'name': 'AShareMoneyFlow',  # 同一个因子内的name不能重复，同一批计算因子间不同表(以路径为准，与lag、column无关)不能相同名字（可以相同表叫不同名字）
         ...
         'lag': 250,  # 注意为正数
         'column': ['SELL_VOLUME_SMALL_ORDER_ACT']}
    ]
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        b = md_data['SELL_VOLUME_SMALL_ORDER_ACT'].unstack().rolling(1, min_periods=1).median().stack()
        a = md_data['SELL_VOLUME_SMALL_ORDER_ACT'].unstack().rolling(30, min_periods=1).median().stack()
