...
class factor_fc_T1_n20240531_4(BaseFactor):
    owner = 'fc'
    strategy_name = "mercury"
    factor_name = sys._getframe().f_code.co_name[7:]
    fill_na_value = 0
    need_pre_calculate_T_N = True
    factor_explain = "昨日中单买入总手数相对于最近3日的差值"
    zcz_adjusted = "否"
    logic_type = ""
    low_cost = "是"
    ...
    def pre_calculate_T_N_data(self, database):
        ...
        b = md_data['BUY_VOLUME_MED_ORDER'].unstack().rolling(1, min_periods=1).median().stack()
        a = md_data['BUY_VOLUME_MED_ORDER'].unstack().rolling(3, min_periods=1).median().stack()
