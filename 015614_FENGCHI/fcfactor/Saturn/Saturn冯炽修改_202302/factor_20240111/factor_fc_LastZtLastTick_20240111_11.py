...
def factor_fc_LastZtLastTick_20240111_11(df, return_fillna_dic=False):
    import sys
    ...
    if len(df) > 0:
        df['Buy5OrderQty'] = df['Buy5OrderQty'].diff().abs().fillna(0).apply(lambda x: round_(x, 6))
        part_df1 = df.query(f'TotalValueTrade >= {seg_threshold}')
        part_df2 = df.query(f'TotalValueTrade <= {seg_threshold}')
        if part_df2['Buy5OrderQty'].max() != 0:
            res = part_df1['Buy5OrderQty'].max() / part_df2['Buy5OrderQty'].max()
        else:
            res = 0
    else:
        res = 0
    ...
    """
    不同区间上主买委托量的变化情况
    =====>>>> 15.16 -0.047 10.36464048586602 51.89577265985704 wj_last_bid，wd_lzt_act_near_qty_pct 0.4156，0.4156
    """
