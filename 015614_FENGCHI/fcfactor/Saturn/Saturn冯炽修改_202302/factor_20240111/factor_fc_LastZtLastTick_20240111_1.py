...
def factor_fc_LastZtLastTick_20240111_1(df, return_fillna_dic=False):
    import sys
    ...
    if len(df) > 0:
        df['Sell4OrderQty'] = df['Sell4OrderQty'].diff().abs().fillna(0).apply(lambda x: round_(x, 6))
        part_df1 = df.query(f'TotalVolumeTrade >= {seg_threshold}')
        part_df2 = df.query(f'TotalVolumeTrade <= {seg_threshold}')
        if part_df2['Sell4OrderQty'].max() != 0:
            res = part_df1['Sell4OrderQty'].max() / part_df2['Sell4OrderQty'].max()
        else:
            res = 0
    else:
        res = 0
    ...
    # ---------------------------------------------------------------------------------------------------------------
    """
    按照买入额进行分层，不同层级之间卖盘委托量变化率的最大值的差异
    =====>>>> 34.125 0.048 74.00675271651092 2898.473043994536 wd_lzkcs_pct_sum_d_mean，sss_tk_buyqty_ratio1d10_open_mean 0.314，0.2765
    """
