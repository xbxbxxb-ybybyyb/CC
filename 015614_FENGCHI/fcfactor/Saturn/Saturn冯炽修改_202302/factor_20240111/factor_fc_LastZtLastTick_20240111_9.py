...
def factor_fc_LastZtLastTick_20240111_9(df, return_fillna_dic=False):
    import sys
    ...
    seg_threshold = round_(df['TotalValueTrade'].quantile(0.1), 6)
    if len(df) > 0:
        df['Buy9OrderQty'] = df['Buy9OrderQty'].diff().abs().fillna(0).apply(lambda x: round_(x, 6))
        part_df1 = df.query(f'TotalValueTrade >= {seg_threshold}')
        part_df2 = df.query(f'TotalValueTrade <= {seg_threshold}')
        if part_df2['Buy9OrderQty'].max() != 0:
            res = part_df1['Buy9OrderQty'].max() / part_df2['Buy9OrderQty'].max()
        else:
            res = 0
    else:
        res = 0
    ...
    # ---------------------------------------------------------------------------------------------------------------
    """
    按照成交额进行划分后盘口中买盘部分增长率最大值
    =====>>>> 16.0 -0.0515 17.675296930031493 213.07397936059164 wd_lzt_act_near_qty_pct，wd_t1_down_ret_sum 0.4508，0.4197
    """
