...
def factor_fc_LastZtLastTick_20240111_7(df, return_fillna_dic=False):
    import sys
    ...
    if len(df) > 0:
        ...
        if part_df2['Buy1OrderQty'].max() != 0:
            res = part_df1['Buy1OrderQty'].max() / part_df2['Buy1OrderQty'].max()
        else:
            res = 0
    else:
        res = 0
    ...
    # ---------------------------------------------------------------------------------------------------------------
    """
    根据单位时刻买单成交量进行头尾部划分，两个区间上买单委托单的数量斜率最大的比值
    =====>>>> 18.5 0.0624 4.388974651379151 9.371303632934683 sss_tk_qtybs_all_lastdmean，Lzt_pj2k_corr_d_3_1 0.3089，0.3078
    """
