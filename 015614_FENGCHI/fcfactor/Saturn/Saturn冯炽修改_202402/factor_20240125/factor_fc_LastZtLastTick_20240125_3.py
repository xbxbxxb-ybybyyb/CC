...
def factor_fc_LastZtLastTick_20240125_3(df, return_fillna_dic=False):
    import sys
    ...
    if len(df) > 0:
        df['Buy10OrderQty'] = df['Buy10OrderQty'].diff().abs().fillna(0).apply(lambda x: round_(x, 6))
        part_df1 = df.query(f'TotalOfferQty >= {seg_threshold}')
        part_df2 = df.query(f'TotalOfferQty <= {seg_threshold}')
        if part_df2['Buy10OrderQty'].mean() != 0:
            res = part_df1['Buy10OrderQty'].mean() / part_df2['Buy10OrderQty'].mean()
        else:
            res = 0
    else:
        res = 0
    ...
    # ---------------------------------------------------------------------------------------------------------------
    """
    按照主买成交量进行头尾部区间买盘委托量对比
    =====>>>> 33.208 -0.074 52.33829879221423 5891.802679116099 Lzt_pj2k_sell2_compact_mean，fm_abnormal_extent_120 0.5757，0.5512
    """
