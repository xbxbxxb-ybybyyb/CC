...
def factor_fc_LastZtLastTick_20240125_8(df, return_fillna_dic=False):
    import sys
    ...
    if len(df) > 0:
        df['Buy2OrderQty'] = df['Buy2OrderQty'].diff().abs().fillna(0).apply(lambda x: round_(x, 6))
        part_df1 = df.query(f'TotalOfferQty >= {seg_threshold}')
        part_df2 = df.query(f'TotalOfferQty <= {seg_threshold}')
        if part_df2['Buy2OrderQty'].mean() != 0:
            res = part_df1['Buy2OrderQty'].mean() / part_df2['Buy2OrderQty'].mean()
        else:
            res = 0
    else:
        res = 0
    ...
    # ---------------------------------------------------------------------------------------------------------------
    """
    按主买成交量两个区间上买盘委托变化率对比
    =====>>>> 18.0 0.068 1.548006947911353 1.0974904799829475 wd_lzo_near_max_pct_bid，wd_lzk_after_max_bid_rate 0.3901，0.3691
    """
