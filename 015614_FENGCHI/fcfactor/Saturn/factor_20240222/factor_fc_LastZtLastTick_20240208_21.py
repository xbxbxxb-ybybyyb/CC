...
def factor_fc_LastZtLastTick_20240208_21(df, return_fillna_dic=False):
    import sys
    ...
    if len(df) > 0:
        df['Buy5OrderQty'] = df['Buy5OrderQty'].diff().abs().fillna(0)
        part_df1 = df.query(f'TotalBidQty >= {seg_threshold}')
        part_df2 = df.query(f'TotalBidQty <= {seg_threshold}')
        if part_df2['Buy5OrderQty'].mean() != 0 and part_df1['Buy5OrderQty'].mean() != 0:
            res1 = part_df1['Buy5OrderQty'].std() / part_df1['Buy5OrderQty'].mean()
            res2 = part_df2['Buy5OrderQty'].std() / part_df2['Buy5OrderQty'].mean()
            if res2 != 0:
                res = res1 / res2
            else:
                res = 0
    ...
    """
    按主卖量，不同买盘盘口委托差异
    =====>>>> 25.542 -0.063 1.502330022715026 0.47009526897976545 fc_LastZtLastTick_20240125_14，fc_LastZtLastTick_20240125_6 0.464，0.4598
    """
