...
def factor_fc_LastZtLastTick_20240208_41(df, return_fillna_dic=False):
    import sys
    ...
    if len(df) > 0:
        df['Buy8OrderQty'] = df['Buy8OrderQty'].diff().abs().fillna(0)
        part_df1 = df.query(f'TotalBidQty >= {seg_threshold}')
        part_df2 = df.query(f'TotalBidQty <= {seg_threshold}')
        if part_df2['Buy8OrderQty'].mean() != 0 and part_df1['Buy8OrderQty'].mean() != 0:
            res1 = part_df1['Buy8OrderQty'].std() / part_df1['Buy8OrderQty'].mean()
            res2 = part_df2['Buy8OrderQty'].std() / part_df2['Buy8OrderQty'].mean()
            if res2 != 0:
                res = res1 / res2
            else:
                res = 0
    ...
    """
    按主卖量进行分段，头尾部买盘变化率
    =====>>>> 15.083 0.055 0.5128749913138766 0.2668866326201581 fc_LastZtLastTick_20240125_1，Lzt_latest_last_order_time_cost 0.4911，0.4537
    """
