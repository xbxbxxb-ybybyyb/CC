...
def factor_fc_LastZtLastTick_20240208_7(df, return_fillna_dic=False):
    import sys
    ...
    if len(df) > 0:
        ...
        if part_df2['Sell3OrderQty'].mean() != 0 and part_df1['Sell3OrderQty'].mean() != 0:
            res1 = part_df1['Sell3OrderQty'].std() / part_df1['Sell3OrderQty'].mean()
            res2 = part_df2['Sell3OrderQty'].std() / part_df2['Sell3OrderQty'].mean()
            if res2 != 0:
                res = res1 / res2
            else:
                res = 0
    ...
    """
    按照主卖成交进行对比，盘口委托变化比值
    =====>>>> 50.917 0.086 0.6809584282955873 0.26989203065591705 fc_LastZtLastTick_20240125_1，Lzt_pj2k_sell2_compact_mean 0.6128，0.5686
    """
