...
def factor_fc_LastZtLastTick_20240208_32(df, return_fillna_dic=False):
    import sys
    ...
    if len(df) > 0:
        df['Buy4OrderQty'] = df['Buy4OrderQty'].diff().abs().fillna(0)
        part_df1 = df.query(f'TotalBidQty >= {seg_threshold}')
        part_df2 = df.query(f'TotalBidQty <= {seg_threshold}')
        if part_df2['Buy4OrderQty'].mean() != 0 and part_df1['Buy4OrderQty'].mean() != 0:
            res1 = part_df1['Buy4OrderQty'].std() / part_df1['Buy4OrderQty'].mean()
            res2 = part_df2['Buy4OrderQty'].std() / part_df2['Buy4OrderQty'].mean()
            if res2 != 0:
                res = res1 / res2
            else:
                res = 0
    ...
    # ---------------------------------------------------------------------------------------------------------------
    """
    全天卖盘成交量划分区间，买盘委托变更情况
    =====>>>> 16.833 -0.05 1.5092544453948245 0.47187472423020627 fc_LastZtLastTick_20240125_14，fc_LastZtLastTick_20240125_6 0.4594，0.4571
    """
