...
def factor_fc_LastZtLastTick_20240208_19(df, return_fillna_dic=False):
    import sys
    ...
    if len(df) > 0:
        df['Sell6OrderQty'] = df['Sell6OrderQty'].diff().abs().fillna(0)
        part_df1 = df.query(f'TotalOfferQty >= {seg_threshold}')
        part_df2 = df.query(f'TotalOfferQty <= {seg_threshold}')
        if part_df2['Sell6OrderQty'].mean() != 0 and part_df1['Sell6OrderQty'].mean() != 0:
            res1 = part_df1['Sell6OrderQty'].std() / part_df1['Sell6OrderQty'].mean()
            res2 = part_df2['Sell6OrderQty'].std() / part_df2['Sell6OrderQty'].mean()
            if res2 != 0:
                res = res1 / res2
            else:
                res = 0
    ...
    """
    根据主买成交量分层，卖盘委托变化值分布
    =====>>>> 25.75 -0.066 0.7774370540651816 0.2679175024523109 xbc_20240125_6，xbc_20240125_0 0.4944，0.4781
    """
