...
def factor_fc_LastZtLastTick_20240208_29(df, return_fillna_dic=False):
    import sys
    ...
    if len(df) > 0:
        df['Buy9OrderQty'] = df['Buy9OrderQty'].diff().abs().fillna(0)
        part_df1 = df.query(f'TotalBidQty >= {seg_threshold}')
        part_df2 = df.query(f'TotalBidQty <= {seg_threshold}')
        if part_df2['Buy9OrderQty'].mean() != 0 and part_df1['Buy9OrderQty'].mean() != 0:
            res1 = part_df1['Buy9OrderQty'].std() / part_df1['Buy9OrderQty'].mean()
            res2 = part_df2['Buy9OrderQty'].std() / part_df2['Buy9OrderQty'].mean()
            if res2 != 0:
                res = res1 / res2
            else:
                res = 0
    ...
    """
    不同区间委托分布差异
    =====>>>> 19.333 -0.056 1.4775021134110176 0.4505518471125143 fc_LastZtLastTick_20240125_6，fc_LastZtLastTick_20240125_14 0.466，0.4643
    """
