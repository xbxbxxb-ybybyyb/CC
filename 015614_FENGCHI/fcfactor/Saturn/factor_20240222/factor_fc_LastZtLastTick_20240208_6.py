...
def factor_fc_LastZtLastTick_20240208_6(df, return_fillna_dic=False):
    import sys
    ...
    if len(df) > 0:
        df['Sell5OrderQty'] = df['Sell5OrderQty'].diff().abs().fillna(0)
        part_df1 = df.query(f'TotalBidQty >= {seg_threshold}')
        part_df2 = df.query(f'TotalBidQty <= {seg_threshold}')
        if part_df2['Sell5OrderQty'].mean() != 0 and part_df1['Sell5OrderQty'].mean() != 0:
            res1 = part_df1['Sell5OrderQty'].std() / part_df1['Sell5OrderQty'].mean()
            res2 = part_df2['Sell5OrderQty'].std() / part_df2['Sell5OrderQty'].mean()
            if res2 != 0:
                res = res1 / res2
            else:
                res = 0
    ...
    # ---------------------------------------------------------------------------------------------------------------
    """
    按照主卖成交量分层，不同层盘口委托挂单变化分布
    =====>>>> 52.167 0.086 0.6596682489903226 0.2559849367547984 fc_LastZtLastTick_20240125_1，Lzt_pj2k_sell2_compact_mean 0.6108，0.5708
    """
