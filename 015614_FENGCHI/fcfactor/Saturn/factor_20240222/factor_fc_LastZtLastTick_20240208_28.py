...
def factor_fc_LastZtLastTick_20240208_28(df, return_fillna_dic=False):
    import sys
    ...
    if len(df) > 0:
        ...
        if part_df2['Sell4OrderQty'].mean() != 0 and part_df1['Sell4OrderQty'].mean() != 0:
            res1 = part_df1['Sell4OrderQty'].std() / part_df1['Sell4OrderQty'].mean()
            res2 = part_df2['Sell4OrderQty'].std() / part_df2['Sell4OrderQty'].mean()
            if res2 != 0:
                res = res1 / res2
            else:
                res = 0
    ...
    # ---------------------------------------------------------------------------------------------------------------
    """
    按照主买成交量计算卖盘中委托diff的对比
    =====>>>> 23.958 -0.037 0.6194372464706118 0.2790247496588163 fc_LastZtLastTick_20240125_3，fc_LastZtLastTick_20240125_8，wj_last_down_wvadpct 0.7109，0.4658，0.428
    """
