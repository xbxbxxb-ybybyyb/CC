...
def factor_fc_LastZtLastTick_20240208_18(df, return_fillna_dic=False):
    import sys
    ...
    if len(df) > 0:
        df['Buy6OrderQty'] = df['Buy6OrderQty'].diff().abs().fillna(0)
        part_df1 = df.query(f'TotalBidQty >= {seg_threshold}')
        part_df2 = df.query(f'TotalBidQty <= {seg_threshold}')
        if part_df2['Buy6OrderQty'].mean() != 0 and part_df1['Buy6OrderQty'].mean() != 0:
            res1 = part_df1['Buy6OrderQty'].std() / part_df1['Buy6OrderQty'].mean()
            res2 = part_df2['Buy6OrderQty'].std() / part_df2['Buy6OrderQty'].mean()
            if res2 != 0:
                res = res1 / res2
            else:
                res = 0
    ...
    """
    不同阶段买盘委托变化
    =====>>>> 26.583 -0.061 1.4955532343983586 0.46154893433257294 fc_LastZtLastTick_20240125_6，fc_LastZtLastTick_20240125_14 0.4645，0.4626
    """
