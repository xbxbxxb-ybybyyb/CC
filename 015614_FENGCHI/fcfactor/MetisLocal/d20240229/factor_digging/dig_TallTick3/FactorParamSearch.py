...
class FactorParamSearch:
    ...
    def calc_factor(df, param_tuple, return_fillna_dic=False):
        ...
        if len(df) > 0:
            ...
            if part_df1[param1].mean() != 0:
                res1 = part_df1[param1].std() / part_df1[param1].mean()
            else:
                res1 = np.nan

            if part_df2[param1].mean() != 0:
                res2 = part_df2[param1].std() / part_df2[param1].mean()
            else:
                res2 = np.nan
...
if __name__ == '__main__':
    note = 'dig_20240229_Metis_ttickab3'
