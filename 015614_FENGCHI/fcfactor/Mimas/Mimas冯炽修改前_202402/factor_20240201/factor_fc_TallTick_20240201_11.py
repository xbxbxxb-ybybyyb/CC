...
def factor_fc_TallTick_20240201_11(df, return_fillna_dic=False):
    import sys
    ...
    if len(df) > 0:
        df['Buy3OrderQty'] = df['Buy3OrderQty'].diff().abs().fillna(0)
        part_df1 = df.query(f'TotalVolumeTrade >= {seg_threshold}')
        part_df2 = df.query(f'TotalVolumeTrade <= {seg_threshold}')
        if part_df2['Buy3OrderQty'].mean() != 0 and part_df1['Buy3OrderQty'].mean() != 0:
            res1 = part_df1['Buy3OrderQty'].std() / part_df1['Buy3OrderQty'].mean()
            res2 = part_df2['Buy3OrderQty'].std() / part_df2['Buy3OrderQty'].mean()
            if res2 != 0:
                res = res1 / res2
            else:
                res = 0
    ...
    # ---------------------------------------------------------------------------------------------------------------
    """
    不同委托量头尾部区间上买盘委托情况的对比
    13.083 -0.03  
    =====>>>> 13.083 -0.03 0.727162131899344 0.24157808727234548 fc_TallTick_20240104_5，fc_TallTick_20240104_3 0.4123，0.3875
    """
