...
def factor_fc_TallTick_20231229_19(df, return_fillna_dic=False):
    import sys
    ...
    seg_threshold = df['TotalOfferQty'].quantile(0.25)
    if len(df) > 0:
        df['Sell8NumOrders'] = df['Sell8NumOrders'].diff().abs().fillna(0)
        part_df1 = df.query(f'TotalOfferQty >= {seg_threshold}')
        part_df2 = df.query(f'TotalOfferQty <= {seg_threshold}')
        res = np.divide(part_df1['Sell8NumOrders'].mean(), part_df2['Sell8NumOrders'].mean())
        if np.isinf(res):
            res = 0
    else:
        res = 0
    ...
    # ---------------------------------------------------------------------------------------------------------------
    """
    T-1日全天tick成交量分段盘口委托数之比
    15.541 -0.0376
    =====>>>> 15.541666666666668 -0.037672554715547736 0.48588699293910487 0.2892399807405689 xbc_20231207_21，dt_last_zt_1 0.3962，0.3845
    """
