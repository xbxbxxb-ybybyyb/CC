...
def factor_fc_TallTick_20231229_27(df, return_fillna_dic=False):
    import sys
    ...
    if len(df) > 0:
        ...
        res = np.divide(part_df1['NumTrades'].mean(), part_df2['NumTrades'].mean())
        if np.isinf(res):
            res = 0
    else:
        res = 0
    ...
    # ---------------------------------------------------------------------------------------------------------------
    """
    T-1日按成交量分头尾部后两部分成交次数的均值比例
    21.375 -0.0442
    =====>>>> 21.375 -0.0442396958745696 8.39702505450026 8.722831276710242 qyh_m5_vol_s2m，qyh_talltick_amt_cct 0.6516，0.6493
    """
