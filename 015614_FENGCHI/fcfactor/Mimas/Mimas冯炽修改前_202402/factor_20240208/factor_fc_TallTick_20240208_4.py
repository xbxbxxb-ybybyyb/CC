...
def factor_fc_TallTick_20240208_4(df, return_fillna_dic=False):
    import sys
    ...
    seg_threshold = round_(df['TotalOfferQty'].quantile(0.75), 6)
    if len(df) > 0:
        df['Sell5NumOrders'] = df['Sell5NumOrders'].diff().abs().fillna(0)
        part_df1 = df.query(f'TotalOfferQty >= {seg_threshold}')
        part_df2 = df.query(f'TotalOfferQty <= {seg_threshold}')
        if part_df2['Sell5NumOrders'].mean() != 0 and part_df1['Sell5NumOrders'].mean() != 0:
            res1 = part_df1['Sell5NumOrders'].std() / part_df1['Sell5NumOrders'].mean()
            res2 = part_df2['Sell5NumOrders'].std() / part_df2['Sell5NumOrders'].mean()
            if res2 != 0:
                res = res1 / res2
            else:
                res = 0
    ...
    主买委托分布变化
    14.0 -0.039  
    =====>>>> 14.0 -0.039 0.564208179152817 0.3205637473403741 fc_TallTick_20240201_6，fc_TallTick_20231229_22 0.6189，0.5613
    """
