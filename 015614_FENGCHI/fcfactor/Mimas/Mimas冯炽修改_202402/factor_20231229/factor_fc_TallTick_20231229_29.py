...
def factor_fc_TallTick_20231229_29(df, return_fillna_dic=False):
    import sys
    ...
    if len(df) > 0:
        df['Sell1NumOrders'] = df['Sell1NumOrders'].diff().abs().fillna(0).apply(lambda x: round_(x, 6))
        part_df1 = df.query(f'TotalOfferQty >= {seg_threshold}')
        part_df2 = df.query(f'TotalOfferQty <= {seg_threshold}')
        if part_df2['Sell1NumOrders'].mean() != 0:
            res = part_df1['Sell1NumOrders'].mean() / part_df2['Sell1NumOrders'].mean()
        else:
            res = 0
    else:
        res = 0
    ...
    # ---------------------------------------------------------------------------------------------------------------
    """
    根据买入成交量分段不同区间上卖1委托单数量均值的比率
    30.916 0.0647 
    =====>>>> 30.916666666666668 0.06475538863477744 1.6972995640982855 0.7603455300096279 wj_TTick_h2c_val，sss_tk_1pmidv_diffret_all_rise 0.6338，0.6202
    """
