...
def factor_fc_TallTick_20231229_39(df, return_fillna_dic=False):
    import sys
    ...
    if len(df) > 0:
        df['Buy5NumOrders'] = df['Buy5NumOrders'].diff().abs().fillna(0).apply(lambda x: round_(x, 6))
        part_df1 = df.query(f'TotalOfferQty >= {seg_threshold}')
        part_df2 = df.query(f'TotalOfferQty <= {seg_threshold}')
        if part_df2['Buy5NumOrders'].mean() != 0:
            res = part_df1['Buy5NumOrders'].mean() / part_df2['Buy5NumOrders'].mean()
        else:
            res = 0
    else:
        res = 0
    ...
    # ---------------------------------------------------------------------------------------------------------------
    """
    买入量头尾部两段区间内买盘委托笔数的差异情况
    16.5833 -0.0385
    =====>>>> 16.583333333333336 -0.03850267549036786 0.5387406873615498 0.7839595707579812 xbc_20231207_21，dt_last_zt_1 0.4017，0.3799
    """
