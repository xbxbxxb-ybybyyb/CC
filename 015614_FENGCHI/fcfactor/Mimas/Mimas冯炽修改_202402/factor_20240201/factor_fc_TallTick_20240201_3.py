...
def factor_fc_TallTick_20240201_3(df, return_fillna_dic=False):
    import sys
    ...
    seg_threshold = round_(df['TotalValueTrade'].quantile(0.75), 6)
    if len(df) > 0:
        df['NumTrades'] = df['NumTrades'].diff().abs().fillna(0)
        part_df1 = df.query(f'TotalValueTrade >= {seg_threshold}')
        part_df2 = df.query(f'TotalValueTrade <= {seg_threshold}')
        if part_df2['NumTrades'].mean() != 0 and part_df1['NumTrades'].mean() != 0:
            res1 = part_df1['NumTrades'].std() / part_df1['NumTrades'].mean()
            res2 = part_df2['NumTrades'].std() / part_df2['NumTrades'].mean()
            if res2 != 0:
                res = res1 / res2
            else:
                res = 0
    ...
    # ---------------------------------------------------------------------------------------------------------------
    """
    对下午Tick数据按照成交额进行分层，每层之间成交笔数的CV之比
    21.042 -0.02  
    =====>>>> 21.042 -0.02 1.0085374952580877 0.5579543348829985 wj_last_5mv2hlv_pctmed，zwh_20231221_017 0.6507，0.5322
    """
