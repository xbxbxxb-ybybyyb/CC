...
def factor_fc_TallTick_20240201_12(df, return_fillna_dic=False):
    import sys
    ...
    if len(df) > 0:
        df['Sell1OrderQty'] = df['Sell1OrderQty'].diff().abs().fillna(0)
        part_df1 = df.query(f'TotalValueTrade >= {seg_threshold}')
        part_df2 = df.query(f'TotalValueTrade <= {seg_threshold}')
        if part_df2['Sell1OrderQty'].mean() != 0 and part_df1['Sell1OrderQty'].mean() != 0:
            res1 = part_df1['Sell1OrderQty'].std() / part_df1['Sell1OrderQty'].mean()
            res2 = part_df2['Sell1OrderQty'].std() / part_df2['Sell1OrderQty'].mean()
            if res2 != 0:
                res = res1 / res2
            else:
                res = 0
    ...
    # ---------------------------------------------------------------------------------------------------------------
    """
    按照下午成交额进行分层，不同区间内卖盘委托diff的分布变化
    12.542 0.011  
    =====>>>> 12.542 0.011 0.5495610747125337 0.24272657795798816 qyh_talltick_rlength_a25_up，qyh_talltick_amt_cct 0.4645，0.3969
    """
