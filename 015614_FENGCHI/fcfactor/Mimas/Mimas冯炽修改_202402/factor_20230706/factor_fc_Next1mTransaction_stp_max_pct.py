...
def factor_fc_Next1mTransaction_stp_max_pct(df, return_fillna_dic=False):
    ...
    if len(df) == 0:
        ...
        df_low = df[df['TradeQty'] <= round_(df['TradeQty'].quantile(0.3), 6)]
    ...
    #----------------------------930-931之间小单部分成交价格相对于最低价的最大值（负向，涨的越多，收益越低？） 16.42 -5.68--------------------------------
    #----------------------------stp small trade price--------------------------------
