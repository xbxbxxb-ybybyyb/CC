...
if __name__ == '__main__':
    start_date, end_date = 20220101, 20220826
    week_start_date, week_end_date = 20220822, 20220826
    Friday = 20220826
    ...
    daily_zt_num = zt.sum(axis=1)
    daily_filter_zt_num = filter_zt.sum(axis=1)
    daily_zb_num = zb.sum(axis=1)
    daily_filter_zb_num = filter_zb.sum(axis=1)
    daily_zb_pctchg = daily_zb_num / (daily_zt_num + daily_zb_num)
    daily_zt_pctchg = daily_zt_num / (daily_zt_num + daily_zb_num)
    daily_filter_zt_pctchg = daily_filter_zt_num / (daily_filter_zt_num + daily_filter_zb_num)
    daily_filter_zb_pctchg = daily_filter_zb_num / (daily_filter_zt_num + daily_filter_zb_num)
    ...
    def plot_twinx(data, col1, col2, output_path, file_name):
        ...
        ax2 = ax1.twinx()
        label1 = col1[2:] if '筛选' in col1 else col1
        label2 = col2[2:] if '筛选' in col2 else col2
        ...
        ax1.set_ylabel(col1, fontsize=20)
        ax2.set_ylabel(col2, fontsize=20)
        ax1.legend(loc=2, borderaxespad=1.).set_zorder(2)
        ax2.legend(loc=1, borderaxespad=1.).set_zorder(2)
        plt.title(f'{col1[:2]}相关走势图', fontsize='large')
        ...
        for ytick in ax2.get_yticklabels():
            ytick.set_fontsize(20)
        plt.xticks(range(0, len(data)), data.index.astype(str).tolist())
    ...
    plot_twinx(output_df, '筛选涨停数', '筛选封板率', zt_plot_data_path, f'涨停数_封板率_{Friday}.jpg')
    plot_twinx(output_df, '筛选炸板数', '筛选炸板率', zt_plot_data_path, f'炸板数_炸板率_{Friday}.jpg')
    ...
    #%% 根据成交记录来进行标粗
    deal_record = pd.read_excel('/data/group/800463/日内强势股/实盘分析记录/日内强势股成交记录/jupiter成交记录-20220826.xlsx', sheet_name='累计买入明细')
    deal_record = deal_record.query('发生日期 >= "2022-08-19"')
    ...
    zb_info = daily_zb_dict['20220826']
