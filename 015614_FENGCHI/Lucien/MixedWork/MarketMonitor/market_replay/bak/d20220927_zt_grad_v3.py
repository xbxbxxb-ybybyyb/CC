...
if __name__ == '__main__':
    start_date, end_date = 20220101, 20220923
    week_start_date, week_end_date = 20220919, 20220923
    Friday = 20220923
    ...
    for week_date in week_date_list:
        tmp_jupiter_data = pd.read_pickle(f'/data/user/015614/daily/basic/basic_wind_sw_history/BlockData/daily_max_pctchg_concept/{week_date}.pkl')
    ...
    def format_stock(stk_id, stk_name, jupiter_dict=jupiter_dict, sw2_dict=sw2_dict):
        ...
        return f'{stk_name}({jupiter_dict[stk_code].split(",")[0] if stk_code in jupiter_dict.keys() else ""})'
    ...
    for _date in week_date_list:
        ...
        zt_info['所属概念'] = zt_info.index.map(lambda x: jupiter_dict[to_wc(x)].split(",")[0] if to_wc(x) in jupiter_dict.keys() else "")
    ...
    jupiter_buy_record.query('证券名称 == "新华百货"')
    europa_buy_record.query('证券名称 == "新华百货"')
    ...
    for _date in week_date_list:
        ...
        tmp_append_list = tmp_jupiter_buy_record['证券代码'].tolist() + tmp_europa_buy_record['证券代码'].tolist() + tmp_jupiter_sell_record['证券代码'].tolist() + \
            tmp_europa_sell_record['证券代码'].tolist()
    ...
    zb_info = daily_zb_dict['20220923']
    ...
    len(tmp)
    FileUtil.save_df2xls(tmp, zt_data_path, '周五炸板的概念.xlsx')
