...
if __name__ == '__main__':
    start_date, end_date = 20240101, 20240729
    week_start_date, week_end_date = 20240715, end_date
    ...
    ths_dict = pd.read_json(f'/data/user/015614/daily/同花顺数据/同花顺概念排名/everyday/同花顺概念排名20230517.json', typ='dict')
    ...
    for week_date in week_date_list:
        tmp_jupiter_data = pd.read_pickle(f'/data/user/015614/daily/basic/basic_wind_sw_history2/BlockData/daily_max_pctchg_concept/jupiter/{week_date}.pkl')
    ...
    stk_pool = stockList.clean_stock_list(no_ST=True, least_live_days=10, least_normal_days=10, no_pause=True, least_recover_days=0, start_date=shift_start_date, end_date=end_date)
    ...
    filter_zt_stack_watcher = filter_zt_stack.groupby(['mddate', '概念名称']).count()
    stats_dict['概念集中度按日明细'] = filter_zt_stack_watcher
    ...
    weekly_zt_stack_num = weekly_zt_stack_num[weekly_zt_stack_num >= 2].sort_values(ascending=False)
    weekly_zt_stack_num_indexes = weekly_zt_stack_num.index.tolist()
    if '' in weekly_zt_stack_num_indexes:
    ...
    stats_dict['上周和全年对比结果'] = output_df
    ...
    for _date in week_date_list:
        ...
        zt_info['所属概念'] = zt_info.index.map(lambda x: jupiter_data.loc[pd.to_datetime(str(_date)), to_wc(x)]['概念名称'] if ((pd.to_datetime(str(_date)), to_wc(x)) in jupiter_data.index) and type(jupiter_data.loc[pd.to_datetime(str(_date)), to_wc(x)]['概念名称']) == str else "")
    ...
    for _date in week_date_list:
        ...
        # zb_info['所属概念'] = zb_info.index.map(lambda x: jupiter_dict[to_wc(x)].split(",")[0] if to_wc(x) in jupiter_dict.keys() and type(jupiter_dict[to_wc(x)]) == str else "")
        zb_info['所属概念'] = zb_info.index.map(lambda x: jupiter_data.loc[pd.to_datetime(str(_date)), to_wc(x)]['概念名称'] if ((pd.to_datetime(str(_date)), to_wc(x)) in jupiter_data.index) and type(jupiter_data.loc[pd.to_datetime(str(_date)), to_wc(x)]['概念名称']) == str else "")
    ...
    FileUtil.save_dict2xls(stats_dict, zt_data_path, '统计表格.xlsx')
    ...
    week_start_date_str = pd.to_datetime(str(week_start_date)).strftime('%Y-%m-%d')
    week_end_date_str = pd.to_datetime(str(week_end_date)).strftime('%Y-%m-%d')
    tmp_jup_no_buy_amt_list = jupiter_buy_record.query(f'发生日期 >= "{week_start_date_str}" & 发生日期 <= "{week_end_date_str}" & 成交金额 == 0')
    tmp_eur_no_buy_amt_list = europa_buy_record.query(f'发生日期 >= "{week_start_date_str}" & 发生日期 <= "{week_end_date_str}" & 成交金额 == 0')
    tmp_entrust_list = tmp_jup_no_buy_amt_list['证券代码'].tolist() + tmp_eur_no_buy_amt_list['证券代码'].tolist()
    # jupiter_buy_record.query('证券名称 == "三七互娱"').iloc[-1].T
    # europa_buy_record.query('证券名称 == "三七互娱"').iloc[-1].T

    deal_stk_list = tmp_entrust_list
    for _date in week_date_list:
        for _grad in lb_grad.index:
            if _grad in lb.T[_date].tolist():
                lb_list = lb.loc[_date][lb.loc[_date] == _grad].index.tolist()
    ...
    hot_stk_str = '长江传媒、皖新传媒、掌阅科技、中国出版、中视传媒、秦港股份、广深铁路、国新文化、设计总院、宝明科技、杭州热电'  # 以顿号、分隔，放入外部分析资料中的市场活跃个股名称
    ...
    # zb_info = zb_info.drop('000615')
    # zb_info.loc['600636', '所属概念'] = '教育'

    # 分析某一天炸板的个股所处行业、概念以及o2ul均值
    # label = pd.read_hdf('/data/group/800463/project/project1_prod/generalStrong_v3/Label_zt/Label_zt.h5')
    label = pd.read_hdf('/data/group/800463/project/project1_prod/left_v2212/Label_zt/Label_zt.h5')
    ...
    tmp1.to_excel(f'/data/user/015614/shared/for_qyh/复盘/{_date}炸板.xlsx')
    send_file(tmp1)

    ######
    lb_flag = lb[lb > 0]
    mkt_cap = getData.get_daily_1factor('a_mkt_cap', date_list=date_list, code_list=lb_flag.columns.tolist())
    lb_mkt_cap = lb_flag * mkt_cap
    check = lb_mkt_cap.mean(axis=1)
    send_file(pd.DataFrame(check))