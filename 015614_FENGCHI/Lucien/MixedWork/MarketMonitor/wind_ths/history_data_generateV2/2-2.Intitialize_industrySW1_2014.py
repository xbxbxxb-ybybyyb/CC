...
"""读取2014版本申万二级行业"""
sw2014_code = pd.read_excel(path_block + '申万一级行业2014版.xlsx', index_col=0)
...
AShareSWIndustriesClass2['industry'] = AShareSWIndustriesClass2['industry'].apply(lambda x: x[:4].ljust(16, '0'))
...
for sw_code in tqdm(industry_list):
    ...
    os.makedirs(path_block + 'sw20141_each_block/', exist_ok=True)
    data.to_pickle(path_block + 'sw20141_each_block/' + sw_code + '.pkl')  # 把二级行业指数加入到概念板块中