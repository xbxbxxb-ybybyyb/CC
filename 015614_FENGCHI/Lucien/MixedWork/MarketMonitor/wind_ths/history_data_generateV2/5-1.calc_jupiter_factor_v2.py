...
# datelist = tradeDate.get_date_range(20150101, 20211212)
# datelist = tradeDate.get_date_range(20211212, 20221027)
# has_datelist = list(map(lambda x: int(x[:8]), os.listdir('/data/user/015614/daily/basic/basic_wind_sw_history2/BlockData/daily_min_concept_num_concept_v20230403/jupiter/')))
# not_datelist = list(set(datelist).difference(set(has_datelist)))
# not_datelist = sorted(not_datelist)
...
sw1_name = pd.read_excel('/data/user/015614/daily/basic/basic_wind_sw_history2/BlockData/申万一级行业2014版.xlsx', index_col=0)
sw1_name_dict1 = sw1_name['sw_name'].to_dict()
sw1_name = pd.read_excel('/data/user/015614/daily/basic/basic_wind_sw_history2/BlockData/申万一级行业2021版.xlsx', index_col=0)
sw1_name_dict2 = sw1_name['sw_name'].to_dict()

if end_date == 20211212:
    name_dict = dict(list(wind_name_dict.items()) + list(sw2_name_dict2.items()) + list(sw2_name_dict1.items()) + list(sw1_name_dict1.items()))
else:
    name_dict = dict(list(wind_name_dict.items()) + list(sw2_name_dict1.items()) + list(sw2_name_dict2.items()) + list(sw1_name_dict2.items()))
...
def calc_stk_index(trade_date):
    ...
    for trade_date in trade_date_list:
        ...
        # 筛选wind和sw概念，防止下面筛选时混在一起
        concept_wind_df = concept_df[list(filter(lambda x: x.endswith('WI'), concept_df.columns.tolist()))]

        # 对Wind成分股数量做限制10-100
        daily_concept_stk_num = concept_wind_df.sum(axis=0)
        match_concept_list = daily_concept_stk_num[(daily_concept_stk_num <= 100) & (daily_concept_stk_num > 10)].index.tolist()

        for stk_code in stk_list:
            concept_list = concept_df.loc[stk_code][concept_df.loc[stk_code] == 1].index.tolist()
            sw_list = list(filter(lambda x: x.endswith('SI'), concept_list))
            if len(sw_list) != 0:
                match_sw = sw_list[0]

                # 对申万做筛选，如果所在一级行业成分股小于100，那么用一级行业
                if trade_date > 20211212:
                    sw1 = pd.read_pickle(block_path + 'sw2021_2_1_dict.pkl')[match_sw]
                    date_sw1 = pd.read_pickle(block_path + f'sw20211_each_block/{sw1}.pkl')
                    # 如果所属申万一级行业成分股数量小于100，或者申万二级行业（如林业）没有行情
                    if date_sw1.loc[pd.to_datetime(str(trade_date))].sum()[0] < 100 or match_sw not in has_kline_concept:
                        match_sw = sw1  # 更换为SI
                else:
                    sw1 = pd.read_pickle(block_path + 'sw2014_2_1_dict.pkl')[match_sw]
                    date_sw1 = pd.read_pickle(block_path + f'sw20141_each_block/{sw1}.pkl')
                    if date_sw1.loc[pd.to_datetime(str(trade_date))].sum()[0] < 100 or match_sw not in has_kline_concept:
                        match_sw = sw1  # 更换为SI
            else:
                match_sw = ''
            ...
            wind_concept_list = list(set(concept_list).difference(set(no_kline_concept_list)).intersection(set(match_concept_list)))

            # 判断重合概念
            coin_wind_concept_df = concept_df[wind_concept_list]
            ret_mul = coin_wind_concept_df.values.T.dot(coin_wind_concept_df.values)
            ret_sum = coin_wind_concept_df.values.sum(axis=0)
            ret = pd.DataFrame(ret_mul / ret_sum, index=wind_concept_list, columns=wind_concept_list)
            match_wind_concept_list = list()
            for idx, wind_concept in enumerate(wind_concept_list):
                if (ret[wind_concept].iloc[:idx] > 0.6).sum() > 0:
                    highest_two = ret[wind_concept].iloc[:idx].sort_values(ascending=False).index.tolist()
                    match_wind_concept_list.append([highest_two[0], wind_concept])
                else:
                    match_wind_concept_list.append([wind_concept])

            wind_concept_list = ['&'.join(x) if len(x) > 1 else x[0] for x in match_wind_concept_list]

            if stk_code not in stk_concept_list_dict.keys():
                stk_concept_list_dict[stk_code] = list(set(wind_concept_list + [match_sw])) if len(sw_list) != 0 else list(set(wind_concept_list))
            else:
                tmp_list = stk_concept_list_dict[stk_code]
                tmp_list.extend(wind_concept_list)
                if len(sw_list) != 0:
                    tmp_list.extend([match_sw])
    ...
    # 特殊处理：林业申万没有行情，如果遇到直接剔除
    if '801011.SI' in concept_count_df.index and trade_date > 20211212:
        concept_count_df = concept_count_df.drop('801011.SI')
    concept1_list = list(filter(lambda x: '&' not in x, concept_count_df.index.tolist()))
    concept2_list = list(filter(lambda x: '&' in x, concept_count_df.index.tolist()))
    concept_count_df.loc[concept1_list, 'concept_pctchg'] = concept_count_df.loc[concept1_list].index.map(lambda x: concept_factor_wind.loc[x, 'pctchg'] if x.endswith('WI') else concept_factor_sw.loc[x, 'pctchg']).values
    concept_count_df.loc[concept2_list, 'concept_pctchg'] = concept_count_df.loc[concept2_list].index.map(lambda x: (concept_factor_wind.loc[x.split('&')[0], 'pctchg'] + concept_factor_wind.loc[x.split('&')[1], 'pctchg']) / 2)
    concept_count_df = concept_count_df.sort_values(['count', 'concept_pctchg'], ascending=False)

    stk_concept_dict = dict()
    for concept in concept_count_df.index.tolist():
        for stk_code in stk_list:
    ...
    deal_data['概念名称'] = deal_data['概念代码'].map(lambda x: name_dict[x] if '&' not in x else f'{name_dict[x.split("&")[0]]}&{name_dict[x.split("&")[1]]}')
    ...
    os.makedirs(block_path + f'daily_min_concept_num_concept_v20230403/jupiter/', exist_ok=True)
    deal_data.to_pickle(block_path + f'daily_min_concept_num_concept_v20230403/jupiter/{trade_date}.pkl')
...
if __name__ == '__main__':
    ...
    # calc_stk_index(20220802)
    # calc_stk_index(20150105)
