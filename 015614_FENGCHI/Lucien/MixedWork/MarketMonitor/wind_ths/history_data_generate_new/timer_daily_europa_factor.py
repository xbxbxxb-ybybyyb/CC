...
# 初始化参数
path = '/data/user/015614/daily/basic/basic_wind_sw_history3/'
path_block = path + 'BlockData/'
if len(sys.argv) > 1:
    today_date = sys.argv[1]
    print(f'当前计算{today_date}...')
# today_date = '20220915' # 目前全量跑是从中秋节前的部分，20220913为中秋节后第一天
else:
...
member3 = fd.get_factor_value('WIND_AIndexMembersWIND', S_CON_INDATE=['>=' + "20230101", '<=' + "20231231"], F_INFO_WINDCODE="like'884%'")
member4 = fd.get_factor_value('WIND_AIndexMembersWIND', S_CON_INDATE=['>=' + "20240101", '<=' + "20241231"], F_INFO_WINDCODE="like'884%'")
member5 = fd.get_factor_value('WIND_AIndexMembersWIND', S_CON_INDATE='>=20240101', F_INFO_WINDCODE="like'884%'")
wind_member = pd.concat([member99, member0, member00, member01, member02, member03, member1, member2, member3, member4, member5], ignore_index=True)
...
# stock_list = list(set(member0['S_CON_WINDCODE'].tolist()))
# stock_list = list(filter(lambda x: str(x)[-2:] in ['SH', 'SZ'], stock_list))
# demo = fd.get_factor_value('WIND_AIndexMembersWIND', S_CON_WINDCODE=stock_list[:2], S_CON_INDATE='>=20230101', F_INFO_WINDCODE="like'884%'")
...
FileUtil.save_list2pkl(wind_concept_list, '/data/user/015614/daily/basic/basic_wind_sw_history3/BlockData/', '最新2015至今全量Wind列表.pkl')
...
if os.path.exists(path_block + 'daily_Wind&SW/' + f'{today_date}.pkl'):
    ...
    for concept in tqdm(concept_list):
        ...
        tmp_member = list(set(tmp_member))
...
# 每个交易日22:08分左右更新
# basic_zt_fname = '/data/group/800463/project/project1_prod/generalStrong_v3/Basic_zt_test/Basic_zt_001.h5'
#
# try:
...
def calc_stk_index(trade_date):
    ...
    FileUtil.save_df2pkl(deal_data, path_block + 'daily_max_pctchg_concept/europa/', f'{trade_date}.pkl')
    check = pd.read_hdf('/data/group/800463/fengc/daily/concept/europa_concept.h5')
    if check.index.get_level_values(0)[-1] != pd.to_datetime(str(today_date)):
        IO.pd_hdf5_writer(deal_data, '/data/group/800463/fengc/daily/concept/europa_concept.h5', dataset='concept', append=True)

calc_stk_index(int(today_date))
# calc_stk_index(20230213)
print(f'{today_date}所属概念计算已完成...{time.time() - t1}')
send_message(f'{today_date} europa日频概念数据已更新...')