...
ccpt_daily_data2 = fd.get_factor_value('WIND_AIndexWindIndustriesEOD', TRADE_DT=['>=' + "20170101", '<=' + "20181231"], S_INFO_WINDCODE="like'884%'")
...
ccpt_daily_data6 = fd.get_factor_value('WIND_AIndexWindIndustriesEOD', TRADE_DT=['>=' + "20210101", '<=' + now_date], S_INFO_WINDCODE="like'884%'")
ccpt_daily_data = pd.concat([ccpt_daily_data1, ccpt_daily_data2, ccpt_daily_data3, ccpt_daily_data4, ccpt_daily_data5, ccpt_daily_data6], axis=0)
...
print('总耗时', time.time() - t1)  # 20150101-20220902 查询耗时504秒=8min
...
print('耗时：', time.time() - t1)  # 20150101-20220902 拼接耗时1064秒=17min