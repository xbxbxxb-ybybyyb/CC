...
# 筛选其中是WI的板块
t1 = time.time()
block_path = '/data/user/015614/daily/basic/basic_wind_sw_history/BlockData/each_block/'
...
FileUtil.save_list2pkl(sw2_concept_list, '/data/user/015614/daily/basic/basic_wind_sw_history/BlockData/', '最新2015至今全量申万二级行业列表.pkl')

# today_date = fd.tradingday(dt.datetime.now().strftime('%Y%m%d'), -1)[0]
today_date = 20220927   # TODO：每次重跑这里要改
yesterday_date = fd.tradingday(dt.datetime.now().strftime('%Y%m%d'), -2)[0]
date_list = tradeDate.get_date_range(20150101, today_date)
...
"""【放弃】第一种方式，构建好数组，往里放：光是构建的这个空数组就占用了上G的内存，速度极慢"""
# t1 = time.time()
# res = pd.DataFrame(index=pd.MultiIndex.from_product([date_list, has_appear_stk_list]), columns=concept_list)
# print('第一种方案创建DataFrame耗时', time.time() -  t1)
"""【放弃】第二种方式，以values的list，然后进行concatenate，处理一个数据需要2.5分钟，共500个概念，要1250分钟，20小时"""
# t1 = time.time()
# value_list = list()
# for fname in tqdm(file_list[:10]):
#     data = pd.read_pickle(block_path + fname)
#     data2 = data.reindex(index=pd.MultiIndex.from_product([pd.to_datetime(list(map(str, date_list))), has_appear_stk_list]))
#     data_values = data2.values
#     value_list.append(data_values)
# print('重索引耗时', time.time() -  t1)
# t1 = time.time()
# value_res = np.concatenate(value_list, axis=1)
# res = pd.DataFrame(value_res, index=pd.MultiIndex.from_product([pd.to_datetime(list(map(str, date_list))), has_appear_stk_list]),
#                    columns=list(map(lambda x: x[:-4], file_list)))
...
#     os.makedirs('/data/user/015614/daily/basic/basic_wind_sw_history/BlockData/daily_Wind&SW/', exist_ok=True)
#     res.to_pickle('/data/user/015614/daily/basic/basic_wind_sw_history/BlockData/daily_Wind&SW/' + f'{date}.pkl')
# print('总计耗时：', time.time() - t1)  # 20个小时！！！要拼接成index是个股，columns是概念的DataFrame
