...
for idx in tqdm(range(len(deal_data))):
    ...
    concept_pctchg = pd.DataFrame(index=all_wind_concept + sw_concept_list, columns=['pctchg'])
    trade_dt, stk_code = deal['dt'], deal['Ticker']
    # trade_dt, stk_code = pd.to_datetime('2022-08-23'), '600777.SH'
    try:
        ...
        concept_pctchg = pd.DataFrame(index=wind_concept + sw_concept, columns=['pctchg'])
        if wind_concept:
...
for dt, concept, concept_name in tqdm(group_data.index):
    ...
    zt_num = zt.loc[int(dt.strftime('%Y%m%d')), list(map(stockList.trans_windcode2int, concept_member_list))].sum()
    group_data.loc[(dt, concept, concept_name), '涨停数量'] = zt_num

    concept_zt_s = zt.loc[int(dt.strftime('%Y%m%d')), list(map(stockList.trans_windcode2int, concept_member_list))]
