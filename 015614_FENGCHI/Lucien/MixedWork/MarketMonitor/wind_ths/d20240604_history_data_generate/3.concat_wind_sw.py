...
FileUtil.save_list2pkl(wind_concept_list, '/data/user/015614/daily/basic/basic_wind_sw_history20240619/BlockData/', '最新2015至今全量Wind列表.pkl')
code_concept_dict = wind_concept[['S_INFO_WINDCODE', 'S_INFO_NAME']].set_index('S_INFO_WINDCODE').to_dict()['S_INFO_NAME']

# 筛选其中是WI的板块
t1 = time.time()
block_path = '/data/user/015614/daily/basic/basic_wind_sw_history20240619/BlockData/each_block/'
...
FileUtil.save_list2pkl(sw2_concept_list, '/data/user/015614/daily/basic/basic_wind_sw_history20240619/BlockData/', '最新2015至今全量申万二级行业列表.pkl')

today_date = int(fd.tradingday(dt.datetime.now().strftime('%Y%m%d'), -1)[0])
# today_date = 20221027   # TODO：每次重跑这里要改
yesterday_date = fd.tradingday(dt.datetime.now().strftime('%Y%m%d'), -2)[0]
date_list = tradeDate.get_date_range(20240525, 20240618)
...
def concat_block(date):
    ...
    os.makedirs('/data/user/015614/daily/basic/basic_wind_sw_history20240619/BlockData/daily_Wind&SW/', exist_ok=True)
    res.to_pickle('/data/user/015614/daily/basic/basic_wind_sw_history20240619/BlockData/daily_Wind&SW/' + f'{date}.pkl')
