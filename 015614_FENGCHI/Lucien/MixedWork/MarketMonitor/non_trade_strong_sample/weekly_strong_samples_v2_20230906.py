...
for date_str in date_str_list:
    ...
    if 'MRisk_info' not in factor_time_cost_df_jup: factor_time_cost_df_jup['MRisk_info'] = ''
    if 'MRisk_info' not in factor_time_cost_df_eur: factor_time_cost_df_eur['MRisk_info'] = ''

    if len(factor_time_cost_df_jup) != 0:
    ...
    if len(factor_time_cost_df_eur) != 0:
...
"""未成交强势股不会出现在成交记录里面，这里犯糊涂了"""
# week_date_next = get_pre_trade_date(week_end_date, -1)
# week_date_next_str_func = lambda x: x.strftime('%Y-%m-%d')
# eur_deal_info = pd.read_excel(f'/data/group/800463/日内强势股/实盘分析记录/日内强势股成交记录/Europa成交记录-{week_end_date}.xlsx', sheet_name='累计卖出明细')
# jup_deal_info = pd.read_excel(f'/data/group/800463/日内强势股/实盘分析记录/日内强势股成交记录/jupiter成交记录-{week_end_date}.xlsx', sheet_name='累计卖出明细')
# eur_deal_info['Ticker'] = eur_deal_info['证券代码']
# eur_deal_info['dt'] = eur_deal_info['买入日期'].apply(lambda x: pd.to_datetime(x))
# eur_deal_info = eur_deal_info.set_index(['dt', 'Ticker'])
# jup_deal_info['Ticker'] = jup_deal_info['证券代码']
# jup_deal_info['dt'] = jup_deal_info['买入日期'].apply(lambda x: pd.to_datetime(x))
# jup_deal_info = jup_deal_info.set_index(['dt', 'Ticker'])
# jup_first2['是否已经卖完'] = jup_first2.apply(lambda x: jup_deal_info.loc[(x['dt'], x['Ticker']), '是否全部卖出'], axis=1)
