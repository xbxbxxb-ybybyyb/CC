...
manual_black = manual_black.query(f'出池时间 == "" & 入池时间 >= "{start_time}"')

manual_black['gap_days'] = manual_black['入池时间'].apply(lambda x: tradeDate.get_trade_date_interval(today_dt, int(x.strftime("%Y%m%d"))))
