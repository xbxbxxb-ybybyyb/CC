...
if today_new_super_notice.shape[0] > 0:
    for idx in range(len(today_new_super_notice)):
        ...
        if stock_id not in manual_black['证券代码'].tolist() and not stock_code.startswith('688') and '*' not in stock_name and '债券' not in today_new_super_notice.iloc[idx].loc['texttitle']:
    ...
    manual_black_raw['出池时间'] = manual_black_raw['出池时间'].apply(lambda x: pd.to_datetime(x).strftime('%Y/%m/%d') if type(x) in [pd.Timestamp, str] else nan)
