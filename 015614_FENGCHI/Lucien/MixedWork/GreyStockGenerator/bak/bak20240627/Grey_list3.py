...
# 持仓
position_file = f'/data/group/800463/position/O45_组合证券_{date}.xlsx'
while not os.path.exists(position_file):
    time.sleep(20)
lm.sendMessage('position文件已出现')
while True:
    try:
        joined_holding = pd.read_excel(position_file)['证券代码'].map(lambda x: Int2Wc(int(x))).tolist()
...
black_list['股票代码'] = black_list['股票代码'].map(lambda x: Int2Wc(int(x)))
