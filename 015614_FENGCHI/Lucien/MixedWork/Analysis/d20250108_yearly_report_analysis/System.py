...
def get_latest_yugao(type=None, early_year=20200101, late_year=20211231):
    fd = FactorData()
    yugao_table_name = 'AShareProfitNotice'
    raw_yugao_df = fd.get_factor_value('WIND_' + yugao_table_name,
                                       S_PROFITNOTICE_PERIOD=['>=%d' % early_year, '<=%d' % late_year])
    col = ['S_INFO_WINDCODE', 'S_PROFITNOTICE_FIRSTANNDATE', 'S_PROFITNOTICE_DATE', 'S_PROFITNOTICE_PERIOD',
           ...
           'S_PROFITNOTICE_INCOMEMAX', 'S_PROFITNOTICE_INCOMEMIN', 'DEDUCTEDSALESMIN', 'DEDUCTEDSALESMAX']
    ...
    rename_dict = {'S_INFO_WINDCODE': '股票代码',
                   ...
                   'S_PROFITNOTICE_INCOMEMAX': '预告营业收入上限',
                   'S_PROFITNOTICE_INCOMEMIN': '预告营业收入下限',
                   'DEDUCTEDSALESMIN': '预告扣除后营业收入下限',
                   'DEDUCTEDSALESMAX': '预告扣除后营业收入上限'}
    ...
    date_list = tradeDate.get_date_range(20140101, 20251231)
...
def get_latest_nianbao(type=None, early_year=20201231, late_year=20211231):
    if type == 'all':
        report_date = 1231
    elif type == 'q1':
        report_date = 331
    ...
    raw_date_predict = fd.get_factor_value('WIND_' + date_predict_table_name,
                                           report_period=['>=%d' % early_year, '<=%d' % late_year])
    col = ['S_INFO_WINDCODE', 'REPORT_PERIOD', 'S_STM_PREDICT_ISSUINGDATE', 'S_STM_ACTUAL_ISSUINGDATE', 'ANN_DT']
    ...
    raw_date_predict['年报预计披露日首次交易日'] = raw_date_predict['年报预计披露日'].apply(lambda x: min(
        [y for y in date_list if y >= int(x)]) if x == x else np.nan)
    raw_date_predict['预计披露日期首次交易日'] = raw_date_predict['预计披露日期公告日'].apply(lambda x: min(
        [y for y in date_list if y >= int(x)]) if x == x else np.nan)
...
def get_latest_kuaibao(early_year=20201231):
    ...
    raw_kuaibao_df = fd.get_factor_value('WIND_' + kuaibao_table_name,
                                         report_period=['>=%d' % early_year, '<=20251231'])
