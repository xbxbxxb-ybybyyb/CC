...
#     info = nd.getAnnouncement([stock], str(20201201), str(20250306))
#     # nd.getAnnouncement(['000040.SZ'], '20231231', '20240523')
#     if len(info) == 0:
#         continue
#
#     info = info[info['PUBDATE'] >= pd.to_datetime('2020' + '1201')]
...
from xquant.factordata import FactorData
fd = FactorData()
date_list = get_date_range(20201101, 20250228)
...
"""
存在一只票20210731 603111 没有ST，但是发布了公告书后一直没有发布决定书
"""
merge_df = merge_df.loc[~merge_df['dt2'].isna()]
merge_df = merge_df.query('dt2 > dt')   # 找出决定书在告知书之后的样本
check = merge_df.sort_values(['Ticker', 'dt', 'dt2']).drop_duplicates(['dt', 'Ticker'], keep='first')

print('决定书才ST：', check[(check['告知书就ST']==False) & (check['决定书才风险警示']==True) & (check['告知书时已经是ST']==False)].shape[0])
print('没有决定书：', check[(check['决定书才风险警示'].isna())].shape[0])   # 8个 发布了告知书到现在没有发布决定书，多数为退市个股，也有部分最近刚发布告知书
print('告知书时已经是ST：', check[(check['告知书时已经是ST']==True)].shape[0])
print('告知书就ST：', check[(check['告知书就ST']==True)].shape[0])
print('总样本数：', check.shape[0]) # 51个
...
res_dict = {'原始数据': check2,
            '策略样本情况': ress2}
from LucienUtil.FileUtil import FileUtil
FileUtil.save_dict2xls(res_dict, '/data/user/015614/junkData/', '数据情况.xlsx')
from dataApi.sendInfo import send_file
send_file(f'/data/user/015614/junkData/数据情况.xlsx')