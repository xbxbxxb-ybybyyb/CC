...
date_list = get_date_range(20231101, 20240630)
date_list = list(map(lambda x: str(x), date_list))
yugao_all_df = get_latest_nianbao(type='all', early_year=20230101, late_year=20231231)
...
yugao_all_df = yugao_all_df.query('年报实际披露日首次交易日 > 0')
print(yugao_all_df.shape)
print(len(stock_list))

yugao_all_df['距离0430距离'] = yugao_all_df['年报实际披露日首次交易日'].apply(lambda x: tradeDate.get_trade_date_interval(x, base_date=20240430) + 1)
...
res = yugao_all_df[['mddate', 'stock', '距离范围',
                    'pct1', 'pct5', 'pct10',
...
res.to_excel('/data/user/015614/junkData/2023年度年报披露结果.xlsx')
...
FileUtil.save_dict2xls(output_dict, '/data/user/015614/junkData/', '年报披露日公告后表现统计.xlsx')
from dataApi.sendInfo import send_file
send_file('/data/user/015614/junkData/年报披露日公告后表现统计.xlsx')
