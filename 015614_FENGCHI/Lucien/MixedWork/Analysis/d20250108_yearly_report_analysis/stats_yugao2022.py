...
date_list = get_date_range(20221101, 20230630)
date_list = list(map(lambda x: str(x), date_list))
yugao_all_df = get_latest_yugao(type='all', early_year=20220101, late_year=20221231)
...
yugao_all_df = yugao_all_df.query('预告首次交易日 <= 20231231')
...
yugao_all_df = yugao_all_df.set_index(['预告首次交易日', '股票代码'])
...
res = yugao_all_df[['mddate', 'stock', '预告类型',
                    'pct1', 'pct5', 'pct10',
...
res.to_excel('/data/user/015614/junkData/2022年度业绩预告结果.xlsx')

res2 = res.groupby('预告类型').agg({'mddate': 'count',
...
FileUtil.save_dict2xls(output_dict, '/data/user/015614/junkData/', '业绩预告后表现统计2022.xlsx')
from dataApi.sendInfo import send_file
send_file('/data/user/015614/junkData/业绩预告后表现统计2022.xlsx')
