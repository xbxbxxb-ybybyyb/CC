...
ipo_data = ipo_data[ipo_data['code'].apply(lambda x: x[:2] in ['60', '30', '00', '68'])]  # 筛选上交所和深交所股票，不包括科创板
...
wrong_content_df = pd.DataFrame()
for stock in all_sample:
    print(stock)
    # if stock != '600589.SH':
    #     continue
    # data_info = nd.getNewsInfoByStockCode(stock[:~2], data_source='TNEWS')
    data_info = nd.getAnnouncement([stock], str(last_year_period), str(nowdate))
    ...
    if len(data_info) != 0:
        ...
        if len(jyfxts_warning) != 0:
            for index, row in jyfxts_warning.iterrows():
                tt = row['TEXTTITLE']
                id = row['ORIGINALCODE']
                news_date = dt.datetime.strftime(row.name[0], '%Y%m%d')
                newsBody = nd.getAnnouncementContent([str(id)])
                if len(newsBody) == 0:
                    ...
                    if ('交易风险' in tt) & ('撤销' not in tt):
                        ...
                        newsBody = newsBody.reset_index()['CONTENT'].loc[0]
                        # if newsBody == '':
                        #     send_message(f'交易风险提示以下公告为空：{nowdate} {stock} {tt}')
                        wrong_content_df = wrong_content_df.append(pd.Series([news_date, stock, id, tt]), ignore_index=True)
                        if ('交易' in newsBody) & (('涨停' in newsBody) | ('偏离' in newsBody)):
                            jyfxts_warning.loc[index, 'jyfxts_indicator'] = 1
                        else:
                            jyfxts_warning.loc[index, 'jyfxts_indicator'] = 0
            jyfxts_warning = jyfxts_warning[jyfxts_warning['jyfxts_indicator'] == 1]
            jyfxts_tot = pd.concat([jyfxts_tot, jyfxts_warning[['ORIGINALCODE', 'jyfxts_indicator']]])
...
joined_condition_1 = (ret_40_next >= 1.5) & ((ycbd_20 >= 2) | (jyfxts_20 >= 1))
...
is_zcz = pd.Series(ycbd_10.index.get_level_values(1).map(lambda x: x.startswith('3') or x.startswith('68')), index=ycbd_10.index, dtype=bool)
joined_condition1 = ((ret_largest_10_next >= 0.3) & ((ycbd_10 >= 2) | (jyfxts_10 >= 1))) & (~is_zcz)
joined_condition2 = ((ret_largest_10_next >= 0.5) & ((ycbd_10 >= 2) | (jyfxts_10 >= 1))) & is_zcz
joined_condition = joined_condition1 | joined_condition2
...
#    return out[['证券名称','证券代码','异常波动公告数', '风险提示公告数', '10日最大涨幅', '上一个提示日期', '公告总数','after_not_ul_len']]


# to_wwd(joined_condition_5_days_fulfilled_need)\
#    .to_excel(path_user+'abnormal_notice_list_%s.xlsx'%nextdate,index=False)
# to_wwd(joined_condition_5_days_fulfilled_need)\
#    .to_excel(path_group+'abnormal_notice_list/abnormal_notice_list_%s.xlsx'%nextdate,index=False)
...
excel_saver({'Sheet1':out,
             '备选检查':out2},
            path_group+'abnormal_notice_list/abnormal_notice_list_%s.xlsx'%nextdate,index = False)


from xquant.xqutils.helper import link

lm = link.LinkMessage()
message = f'{nowdate}异常波动公告上传成功：' + str(len(out)) + '只股票'
