...
"""
计算Saturn样本和异常波动的交集数量，寻找一个最优参数转折点
"""
...
# date_list = tradeDate.get_date_range(20211231, 20221231)
# date_list = tradeDate.get_date_range(20230101, 20230705)
# date_list = tradeDate.get_date_range(20230706, 20230921)
date_list = tradeDate.get_date_range(20230922, 20231009)
# date_list = tradeDate.get_date_range(20211231, 20211231)
# date_list = tradeDate.get_date_range(20200701, 20211231)
date_list = tradeDate.get_date_range(20200701, 20211231)
path_user = '/data/user/015614/JunkWorkList/灰名单生成/异常波动测试V20240816/'
os.makedirs(path_user, exist_ok=True)
# path_group = '/data/group/800463/stock_list/abnormal_notice_list/'
path_group = '/data/user/015614/daily/灰名单生成/异常波动历史测试/'
rolling_period = 1
...
for nowdate in tqdm(date_list):
    ...
    df = pd.read_excel(path_group + f'abnormal_notice_list_{nowdate}.xlsx', sheet_name='备选检查', index_col=0)
    ...
    df['40日涨跌幅'] = Ret_40.loc[pd.to_datetime(str(date))].loc[df.index]
    df['10日最大涨跌幅'] = Ret_10_largest.loc[pd.to_datetime(str(date))].loc[df.index]

    df.to_excel(path_user + f'abnormal_notice_{nowdate}.xlsx')
    ...
    # lm = link.LinkMessage()
    # message = '异常波动公告上传成功：' + str(len(out)) + '只股票'
