...
if os.path.exists('/data/user/015614/junkData/news_info.pkl'):
    news_info = pd.read_pickle('/data/user/015614/junkData/news_info.pkl')
else:
    print(1)
...
# 对可能提到截止日期的股票单独处理
for index, row in news_info_st2.iterrows():
    news_body = nd.getAnnouncementContent([row['newsID']])['newsBody'].iloc[0]
    print(news_body)
    news_body = news_body.replace('\r', '').replace('\n', '').replace(' ', '')  # 去除换行符空白符
    for text in news_body.split('。'):
        if '风险警示' in text:
            stock_date2.loc[index, 'begin'] = row['dt']
            stock_date2.loc[index, 'end'] = min(stock_date1.loc[index, '10days_later'], date_dt)
