...
weighted_check_num = np.zeros((stk_len, stk_len))  # AB超过7%次数
zt_check_num = np.zeros((stk_len, stk_len))   # AB涨停次数
weighted_zt_check_num = np.zeros((stk_len, stk_len))   # AB涨停次数
weighted_check_pct = np.zeros((stk_len, stk_len))
...
for interval_month in list(set(interval_month_list)):
    log_sum += np.log(interval_month + 1)
...
for _date in tqdm(date_list_1year):
    ...
    _w = np.log(max(interval_month_list) - interval_month + 2) / np.sum(np.log(idx) for idx in list(np.array(list(set(interval_month_list))) + 1))
    # print(_w)
...
for _date in tqdm(date_list_1year):
    ...
    _w = np.log(max(interval_month_list) - interval_month + 2) / np.sum(np.log(idx) for idx in list(np.array(list(set(interval_month_list))) + 1))
    # print(_w)
    weighted_check_pct += (tmp_check * _w / np.array([check_num[i][i] for i in range(len(check_num))]))    # 第一行作为除以AA、BB、CC、DD的比例
...
for stk_id in tqdm(res_df.index):
    ...
    tmp_res = tmp_res[['index', 'num']].apply(lambda x: str(int(x['index'])) + '(' + str(round(x['num'], 2)) + ')', axis=1)
...
res_df.to_excel('/data/user/015614/shared/for_wys/20220301-20230228个股按规则筛选前20个V4_20230308.xlsx')