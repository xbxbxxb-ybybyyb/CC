...
date_list = tradeDate.get_date_range(20230101, 20240816)
path_user = '/data/user/015614/daily/灰名单生成/异常波动历史测试V20240816/'
...
def wrapper(dat_list):
    for _dat in tqdm(dat_list):
        try:
            ...
            # 1、条件一（新加的条件）
            ycbd_20 = ycbd_tot['ycbd_indicator'].reindex(all_index).unstack().fillna(0).rolling(5, 5).sum().stack()
            jyfxts_20 = jyfxts_tot['jyfxts_indicator'].reindex(all_index).unstack().fillna(0).rolling(5, 5).sum().stack()
            ...
            # 2、条件二（原本的条件）
            ycbd_10 = ycbd_tot['ycbd_indicator'].reindex(all_index).unstack().fillna(0).rolling(5, 5).sum().stack()
            jyfxts_10 = jyfxts_tot['jyfxts_indicator'].reindex(all_index).unstack().fillna(0).rolling(5, 5).sum().stack()
...
# wrapper([20230110])
import os
file_list = os.listdir('/data/user/015614/daily/灰名单生成/异常波动历史测试V20240816/')
