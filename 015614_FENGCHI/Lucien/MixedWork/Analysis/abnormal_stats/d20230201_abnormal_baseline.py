...
"""
20230201: 测试不同参数的异常波动数量情况，初期目标：设置几组参数，跑出每一日的结果，然后分组画出曲线图
需不需要计算他们的未来N日收益情况再看要求
"""
...
root_path = '/data/user/015614/junkData/参数组合3/'

def get_abnormal_1day(dat):
    nowdate = str(dat)
    nextdate = nowdate
    ...
    # 5日、7日、10日、40日涨跌幅
    # Ret_5 = pd.DataFrame(close_adj.unstack().rolling(6, 6).apply(cal_return).stack(), columns=['Ret_5']).reindex(f_data.index)
    # Ret_7 = pd.DataFrame(close_adj.unstack().rolling(8, 8).apply(cal_return).stack(), columns=['Ret_7']).reindex(f_data.index)
    # Ret_10 = pd.DataFrame(close_adj.unstack().rolling(11, 11).apply(cal_return).stack(), columns=['Ret_10']).reindex(f_data.index)
    Ret_40 = pd.DataFrame(close_adj.unstack().rolling(41, 41).apply(cal_return).stack(), columns=['Ret_40']).reindex(f_data.index)
    ...
    # 1、条件一（新加的条件）
    ycbd_20 = ycbd_tot['ycbd_indicator'].reindex(all_index).unstack().fillna(0).rolling(PARAM_A, PARAM_A).sum().stack()
    jyfxts_20 = jyfxts_tot['jyfxts_indicator'].reindex(all_index).unstack().fillna(0).rolling(PARAM_A, PARAM_A).sum().stack()
    ret_40_next = fun_append_next_tradingday(Ret_40['Ret_40']).unstack().shift().stack()
    joined_condition_1 = (ret_40_next >= PARAM_C) & ((ycbd_20 + jyfxts_20) >= 1)
    joined_condition_1_need = joined_condition_1[joined_condition_1].loc[nowdate]

    # 2、条件二（原本的条件）
    ycbd_10 = ycbd_tot['ycbd_indicator'].reindex(all_index).unstack().fillna(0).rolling(PARAM_D, PARAM_D).sum().stack()
    jyfxts_10 = jyfxts_tot['jyfxts_indicator'].reindex(all_index).unstack().fillna(0).rolling(PARAM_D, PARAM_D).sum().stack()

    ret_largest_10_next = fun_append_next_tradingday(Ret_10_largest['Ret_10_largest']).unstack().shift().stack()
    joined_condition = (ret_largest_10_next >= PARAM_F) & ((ycbd_10 + jyfxts_10) >= PARAM_G)
    # joined_condition = (ret_largest_10_next>=0.3) & (ycbd_10>=1) & (jyfxts_10>=1)
    # joined_condition[joined_condition.reset_index()['Ticker'].apply(lambda x:x[~0] == 'H').values] = \
    #    (ret_largest_10_next>=0.3) & ((ycbd_10+jyfxts_10)>=1)
    ...
    FileUtil.save_df2xls(out, root_path, f'{dat}.xlsx')

def wrapper(dats):
    pbar = tqdm(range(len(dats)))
    for idx in pbar:
        trade_date = dats[idx]
        pbar.set_description('并行生成中|%s' % trade_date)
        get_abnormal_1day(trade_date)

if __name__ == '__main__':
    exist_date_list = list(map(lambda x: int(x[:-5]), os.listdir(root_path)))
    date_list = list(set(date_list).difference(set(exist_date_list)))
    SpeedUtil.multiprocess(20, wrapper, date_list)
    # get_abnormal_1day(20220414)