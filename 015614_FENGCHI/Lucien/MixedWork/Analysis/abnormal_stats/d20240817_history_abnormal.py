...
def wrapper(dat_list):
    for _dat in tqdm(dat_list):
        try:
            ...
            ycbd_tot = pd.read_pickle('/data/user/015614/daily/灰名单生成/中间数据/ycbd_tot_20240820.pkl')
            jyfxts_tot = pd.read_pickle('/data/user/015614/daily/灰名单生成/中间数据/jyfxts_tot_20240820.pkl')

            all_index = fun_append_next_tradingday(f_data).index
            # 1、条件一（新加的条件）
            ycbd_20 = ycbd_tot['ycbd_indicator'].reindex(all_index).unstack().fillna(0).rolling(6, 6).sum().stack()
            jyfxts_20 = jyfxts_tot['jyfxts_indicator'].reindex(all_index).unstack().fillna(0).rolling(6, 6).sum().stack()
            ...
            # 2、条件二（原本的条件）
            ycbd_10 = ycbd_tot['ycbd_indicator'].reindex(all_index).unstack().fillna(0).rolling(6, 6).sum().stack()
            jyfxts_10 = jyfxts_tot['jyfxts_indicator'].reindex(all_index).unstack().fillna(0).rolling(6, 6).sum().stack()
...
# wrapper([20240809])
import os
file_list = os.listdir('/data/user/015614/daily/灰名单生成/异常波动历史测试V20240817/')
