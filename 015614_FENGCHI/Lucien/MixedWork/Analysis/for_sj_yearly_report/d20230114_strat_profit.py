...
strategy = 'jupiter'    # Europa从5.18开始

# date_list = get_date_range(20220101, 20221231)
...
for _dat in tqdm(date_list):
    ...
    try:
        if _dat > 20220307:
            ...
            daily_sell_profit = daily_profit.iloc[18, 1]
            profit_list.append(daily_sell_profit)
    except:
        print(f'{_dat}没有卖出盈利')
...
h1_date_list = get_date_range(20220101, 20220630)
h2_date_list = get_date_range(20220701, 20221231)
calc_mdd(profit_s.loc[h1_date_list])
calc_mdd(profit_s.loc[h2_date_list])

for _dat in date_list[:20]:
    daily_profit = pd.read_excel(root_path + f'{strategy}成交记录-{_dat}.xlsx', sheet_name='今日汇总情况')
    daily_sell_profit = daily_profit.iloc[18, 1]
    profit_list.append(daily_sell_profit)