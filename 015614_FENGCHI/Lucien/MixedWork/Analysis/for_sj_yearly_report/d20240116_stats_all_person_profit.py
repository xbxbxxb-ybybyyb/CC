...
track_path = '/data/user/015614/daily/复盘/策略模拟跟踪/20231229/'

eur_fpath = track_path + 'Europa模型跟踪v2_20221230_20231229.xlsx'  # v1 v2 v3 v4
sellv1_fpath = track_path + 'JupiterNSell模型跟踪v1_20221230_20231229.xlsx' # v1
sellv3_fpath = track_path + 'JupiterNSell34模型跟踪v2_20221230_20231229.xlsx'   # v1
jup_fpath = track_path + 'jupiter模型跟踪v2_20221230_20231229.xlsx' # v8 v9
sat_fpath = track_path + 's1模型跟踪v6_20221230_20231229.xlsx' # v5 v6
jupz_fpath = track_path + 'JupiterZ模型跟踪v1_20221230_20231229.xlsx'   # v1
metis_fpath = track_path + 'Metis模型跟踪v1_20221230_20231229.xlsx' # v1

strategy_list = [eur_fpath, jup_fpath, sat_fpath, jupz_fpath, sellv1_fpath, sellv3_fpath, metis_fpath]
version_list = ['v1', 'v2', 'v3', 'v4', 'v5', 'v6', 'v7', 'v8', 'v9']
strategy_name_list = ['Europa', 'Jupiter', 'Saturn', 'JupiterZ', 'Sell_v1', 'Sell_v3', 'Metis']
...
for idx, strat_fpath in enumerate(strategy_list):
    ...
    if strategy_name == 'Europa':
        ...
        v4 = pd.read_excel(strat_fpath, sheet_name='本地样本跟踪v4modelpool')
        for idx2 in range(len(v4)):
            res.loc[(strategy_name, 'v4', idx2), :] = v4.loc[idx2][need_cols].values
...
FileUtil.save_dict2xls(res_dict, '/data/user/015614/', '2023年终模型评估结果.xlsx')