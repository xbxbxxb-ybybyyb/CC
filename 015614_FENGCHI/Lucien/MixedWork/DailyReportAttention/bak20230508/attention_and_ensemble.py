...
team_addr_dict = {
    '王敬': '/data/group/800463/wangj/for_fc/daily_summary/',
    '谢璐遥': '/data/group/800463/xiely/daily/summary/',
    ...
    if cur_time > t_list[0]:
        ...
        if len(not_submit_no_list) >= 3:
            ...
            if  203000 < cur_time < 210500:
                send_message(f'【日小结提醒】请发送今天的日小结，目前还剩3位同事未交~({",".join(not_submit_name_list)})')
        if len(not_submit_no_list) == 2:
            send_message('【日小结提醒】请发送今天的日小结，目前还剩2位同事未交！', users=not_submit_no_list)
            if 203000 < cur_time < 210500:
                send_message(f'【日小结提醒】请发送今天的日小结，目前还剩2位同事未交~({",".join(not_submit_name_list)})')
        if len(not_submit_no_list) == 1:
            send_message('【日小结提醒】请发送今天的日小结，目前还剩1位同事未交，就差你啦', users=not_submit_no_list)
            if 203000 < cur_time < 210500:
                send_message(f'【日小结提醒】请发送今天的日小结，目前还剩1位同事未交~({",".join(not_submit_name_list)})')
