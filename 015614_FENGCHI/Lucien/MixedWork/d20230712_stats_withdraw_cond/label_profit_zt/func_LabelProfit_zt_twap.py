...
def hf_preprocessing(data_type, md_df, btTime=None):
    if (data_type == 'Stock') or (data_type == 'StockAllDay') or (data_type == 'StockAllDayNoTradingPhaseCode'):    #TICK
        ...
        md_df = md_df[['MDDate', 'MDTime', 'OrderIndex', 'OrderPrice', 'OrderQty', 'OrderBSFlag','OrderType', 'HTSCSecurityID']]
        md_df['MDTime'] = md_df['MDTime'].astype(int)
        return md_df

def cal_shot_trade_result(zt_trans_df, zt_order_df, trans_reach_zt_time, lag_ms, max_amt, close_price, ul_price):
    ...
    if place_time > 145700000:
        ...
        last_order_before_place = zt_order_df[zt_order_df['MDTime'] >= place_time].iloc[0]
        last_order_index_before_place = last_order_before_place['OrderIndex']
        last_index_before_place = min(last_trans_index_before_place, last_order_index_before_place)

        trans_df_after_place = zt_trans_df[zt_trans_df['TradeBuyNo'] > last_index_before_place]
    ...
    result = {'target_vol': target_vol, 'buy_vol': shot_vol, 'buy_amt': shot_amt, 'pct_t': close_price / ul_price -1}
...
def interval_trade(sell_vol, vol_pct, tick_df, dl_price, ul_price, cover_amt):
    ...
    for index, tick_data in tick_df.iterrows():
        if tick_data['LastPx'] == dl_price:
            ...
            now_trade_vol = max(tick_data['vol'] * vol_pct,0)
...
def cal_sell_trade_result(mdp, nextday, mdp_code, sell_vol, last_close, date_list, vol_pct, cover_amt, sell_minute_interval=5):
    if nextday!=nextday: #目前没有下一交易日
        ...
        if (len(tick_md_df) == 0) or (tick_md_df['vol'].min()<-10000) or (tick_md_df['amt'].min()<-10000): #或者下个交易日没有数据
            return 0, 0, [], [-2], []
    pre_close, open_price, ul_price = tick_md_df['PreClosePx'].iloc[-1], tick_md_df['OpenPx'].iloc[-1], tick_md_df['MaxPx'].iloc[-1]
    dl_price, close_price = tick_md_df['MinPx'].iloc[-1], tick_md_df['LastPx'].iloc[-1]
    if sell_vol <= 0.0001: # 已经卖完了
        return open_price / pre_close - 1, 0, [], [-1], []
    ...
    if (sum(trade_vol_list) - sell_vol)> -1: #买到足够量
        touch_ul = -1
        return pct_chg_t1, 1, [nextday], [touch_ul], [sell_vol]
    else:
        ...
        next_pct_chg_t1, next_sell_len, next_sell_date_list, touch_ul_next_list, sell_vol_list = cal_sell_trade_result(mdp, next_nextday, next_mdp_code, next_sell_vol, next_last_close,
                                                date_list, vol_pct, cover_amt)
        today_pct_chg = close_price / pre_close - 1
        return (pct_chg_t1 * (sum(trade_vol_list) / sell_vol)) + (
                    (1 + today_pct_chg) * (1 + next_pct_chg_t1) - 1) * (1 - sum(trade_vol_list) / sell_vol), 1 + next_sell_len, [nextday] + next_sell_date_list, [touch_ul] + touch_ul_next_list, [sum(trade_vol_list)]+sell_vol_list
...
def cal_LabelProfit_zt(stock, date, ZT_Time, mdp, param={}):
    ...
    ul_price, close_price, cover_amt = param['ul_price'], param['close_price'], param['cover_amt']
    max_amt = min(max_amt, max_vol * 10000 * ul_price)

    trans_df = mdp.get_data_by_date('Transaction', stock, date)
    trans_df = hf_preprocessing('TransactionAllDay', trans_df)
    zt_trans_df = trans_df[(trans_df['MDTime'] >= ZT_Time) & (trans_df['TradePrice'] != 0)].copy() # 去除深圳撤单
    order_df = mdp.get_data_by_date('Order', stock, date)
    if stock[-2:] == 'SH':
        order_df = order_df.drop(columns=['OrderIndex']).rename(columns={'OrderNO':'OrderIndex'})
    order_df = hf_preprocessing('OrderAllDay', order_df)
    zt_order_df = order_df[(order_df['MDTime'] >= ZT_Time) & (order_df['OrderType'] != 10)].copy() # 去除上海撤单
    result = cal_shot_trade_result(zt_trans_df=zt_trans_df,
                                   zt_order_df=zt_order_df,
                                   ...
                                   ul_price=ul_price)
    try:
        first_ul_trans = zt_trans_df.iloc[0]
        result['delta_ms'] = cal_time_delta(int(first_ul_trans['MDTime']), first_ul_trans['ReceiveDateTime'] % 1000000000)
    except:
        result['delta_ms'] = np.nan

    #次日卖出部分
    nextday = get_next_day(date, date_list)
    mdp_code = change_code_for_mdp(code=stock, date=date)
    result['pct_t1'], result['sell_length'], result['sell_date'] , result['touch_ul'] , result['sell_vol']= \
        cal_sell_trade_result(mdp=mdp, nextday=nextday, mdp_code=mdp_code, sell_vol=result['target_vol'],
                              last_close=close_price, date_list=date_list, vol_pct=sell_vol_pct, cover_amt=cover_amt)
    result['sell_date'] = str([int(day) for day in result['sell_date']])
    result['sell_vol'] = str([int(vol) for vol in result['sell_vol']])
    result['touch_ul'] = str([int(vol) for vol in result['touch_ul']])
