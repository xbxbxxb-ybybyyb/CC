...
# 共分为三个日期，仿真日期sim_date创建日期create_date完成仿真日期finish_date
import sys
...
digit2factorSet = {'300001': '168.62.9.55', '300002': '168.62.1.38', '300003': '168.62.1.39', '300004': '168.62.1.82',
                   ...
                   '300009': '168.62.1.80', '300010': '168.62.9.53', '300011': '168.62.1.77', '300012': '168.62.1.78',
                   '300013': '168.62.1.79', '300014': '168.62.1.81'}
ip2kernal = {'168.62.9.55': 96, '168.62.1.38': 96, '168.62.1.39': 96, '168.62.1.82': 48,
             '168.62.1.83': 48, '168.62.1.84': 48, '168.62.1.85': 48, '168.62.1.86': 48,
             '168.62.1.80': 48, '168.62.9.53': 96, '168.62.1.77': 48, '168.62.1.78': 48,
             '168.62.1.79': 48, '168.62.1.81': 48}
ip_check_dict = {
    '20230424': [55, 38, 39, 77, 78, 79, 80, 81, 82, 83, 84, 85, 86],
    '': []
}

class SimLogParseCpp:
    def __init__(self):
    ...
    def dropdup(self, parse_file_fpath):
        wangj_log = ''
        xiely_log = ''
        lines = self.get_lines4dropup(parse_file_fpath)

        print('开始给wj和xly去重日志并保存')
        if len(lines) > 0:
            for line in tqdm(lines):
                machine_code = line[line.find('[StrongStrategy-algo'):line.find('-n0]')]
                machine_code = machine_code.split('-')[-1]
                if machine_code[-2:] in ['53', '55', '84']:
                    wangj_log += line
                if machine_code[-2:] in ['53', '55', '84'] and 'ParamsLog' not in line and 'OpenPX' not in line:
                    xiely_log += line
            # wangj_log = '\n'.join(pd.Series(wangj_log.split('\n')).drop_duplicates(keep='first').tolist())
            # xiely_log = '\n'.join(pd.Series(xiely_log.split('\n')).drop_duplicates(keep='first').tolist())
    ...
    def parse_1uatlog(self, sim_date, create_date, prefix='uat_lite'):
        lines = self.get_lines(str(sim_date), f'{prefix}-{create_date}')
        flag = prefix + str(create_date)

        try:
            jpt_zone_dict = pd.read_pickle(f'/data/group/800463/xiely/save-file/forFc/daily/jpt_zone_dict_{sim_date}_{create_date}')
        except:
            jpt_zone_dict = pd.read_pickle(f'/data/group/800463/xiely/save-file/forFc/daily/jpt_zone_dict_20230331_20230414')
        try:
            eur_zone_dict = pd.read_pickle(f'/data/group/800463/xiely/save-file/forFc/daily/eur_zone_dict_{sim_date}_{create_date}')
        except:
            eur_zone_dict = pd.read_pickle(f'/data/group/800463/xiely/save-file/forFc/daily/eur_zone_dict_20230331_20230414')

        # 对ip-factor class map进行处理
        ...
        jpt_zone_df = pd.DataFrame(pd.Series(jpt_zone_dict))
        jpt_zone_df = jpt_zone_df.reset_index()
        jpt_zone_df.columns = ['ip', 'class_list']
        jpt_zone_df['class_name'] = jpt_zone_df[['ip', 'class_list']].apply(lambda x: f'factor_{len(x["class_list"].split(" "))}_{ip2kernal[x["ip"]]}', axis=1)
        jpt_zone_df = jpt_zone_df.set_index('ip')

        eur_zone_df = pd.DataFrame(pd.Series(eur_zone_dict))
        eur_zone_df = eur_zone_df.reset_index()
        eur_zone_df.columns = ['ip', 'class_list']
        eur_zone_df['class_name'] = eur_zone_df[['ip', 'class_list']].apply(lambda x: f'factor_{len(x["class_list"].split(" "))}_{ip2kernal[x["ip"]]}', axis=1)
        eur_zone_df = eur_zone_df.set_index('ip')
        ...
        if len(lines) > 0:
            ...
            for line in tqdm(lines):
                machine_code = line[line.find('[StrongStrategy-algo'):line.find('-n0]')]
                machine_code = machine_code.split('-')[-1]
                if 'INFO  c.h.s.s.StrongTradeExecutor - Order info:' in line and 'orderType=SaturnBuy' not in line and 'placeType=amendMRiskSplitLastShot' not in line and 'placeType=amendSplitLastShot' not in line and 'orderType=Sell' not in line:
                ...
                if 'Triggered: reachedZTTime=' in line:
                    ...
                    if 'JupiterN' in line and 'JupiterNew' not in line:
                        jpt_zone_df.loc[ip_code, 'factor_list_len'] = len(factor_list)
                    if 'JupiterNew' in line:
                        eur_zone_df.loc[ip_code, 'factor_list_len'] = len(factor_list)
                    ...
                    startCalculateBy_df.loc[len(startCalculateBy_df)] = [sim_date, symbol, source, startCalculateBy, ZTTradeTime, systemTimeNow, systemTimeZT, trigger_timeDelay, wait_time,
                                                                         machine_code]
            ...
            jpt_zone_df = jpt_zone_df.reset_index()
            jpt_zone_df['class_name'] = jpt_zone_df[['ip', 'factor_list_len']].apply(lambda x: f'factor_{int(x["factor_list_len"])}_{ip2kernal[x["ip"]]}' if not np.isnan(x["factor_list_len"]) else
            f'factor_ip{x["ip"].split(".")[-1]}_{ip2kernal[x["ip"]]}', axis=1)
            jpt_zone_df = jpt_zone_df.set_index('ip')

            eur_zone_df = eur_zone_df.reset_index()
            eur_zone_df['class_name'] = eur_zone_df[['ip', 'factor_list_len']].apply(lambda x: f'factor_{int(x["factor_list_len"])}_{ip2kernal[x["ip"]]}' if not np.isnan(x["factor_list_len"]) else
            f'factor_ip{x["ip"].split(".")[-1]}_{ip2kernal[x["ip"]]}', axis=1)
            eur_zone_df = eur_zone_df.set_index('ip')

            combined_df5_jup = combined_df5.query('source == "JupiterN"')
            combined_df5_eur = combined_df5.query('source == "JupiterNew"')
            combined_df5_jup['group'] = combined_df5_jup['machine_code'].replace(udp2machine).replace(jpt_zone_df['class_name'].to_dict())
            combined_df5_eur['group'] = combined_df5_eur['machine_code'].replace(udp2machine).replace(eur_zone_df['class_name'].to_dict())
            combined_df5 = pd.concat([combined_df5_jup, combined_df5_eur], axis=0)
    ...
    def timer_mission(self, drop_sim_date_list=[]):
        log_file_cache = list()
        while True:
            ...
            for filename in filename_list:
                ...
                # 临时过滤一部分文件
                if int(filename[15:23]) in drop_sim_date_list:
                    continue
                ...
                if filename.endswith('.zip'):
                    ...
                    self.unzip_file(log_path + filename)
                    ...
                    # 步骤2：
                    self.parse_1uatlog(sim_date, create_date)
                    print(f'解析完成{sim_date}_{create_date}_{date}仿真日志')
                    send_message(f'解析完成{sim_date}_{create_date}_{date}仿真日志')

                    self.dropdup(output_log_path + f'StrongStrategy-%s-%s-%s-uat_lite-%s.log.gz' % (sim_date[:4], sim_date[4:6], sim_date[6:8], create_date))
...
if __name__ == '__main__':
    ...
    # # 单独跑一个
    # filename = 'StrongStrategy-20230425-20230425200826-472-0-00000000.zip'
    # sim_date = filename.split('-')[1]
    # create_date = filename.split('-')[2][:8]
    # slp.unzip_file(log_path + filename)
    # for filename2 in os.listdir(log_path):
    #     if '.log.gz' in filename2:
    #         shutil.copy(log_path + filename2, output_log_path + r'StrongStrategy-%s-%s-%s-%s-%s.log.gz' % (sim_date[:4], sim_date[4:6], sim_date[6:8], 'uat_lite', create_date))
    #         os.remove(log_path + filename2)
    #         os.remove(log_path + 'StrongStrategy-%s.zip' % sim_date)
    # # slp.parse_1uatlog(sim_date, create_date)
    # slp.dropdup(output_log_path + f'StrongStrategy-%s-%s-%s-uat_lite-%s.log.gz' % (sim_date[:4], sim_date[4:6], sim_date[6:8], create_date))
    #
    # # 单独跑多个
    # sim_date_list = []
    # create_date = []
    ...
    #
    # # 过滤几个文件，跑全部
    # finish_date = 20230417
    # log_path = f'/data/group/800463/sim-lite-log/{finish_date}/'
    # drop_date_list = []
    # slp.timer_mission()
