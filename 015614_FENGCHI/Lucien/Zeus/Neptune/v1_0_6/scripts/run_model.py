...
from Zeus.Neptune.v1_0_6.scripts.check_path_is_ok import check_path_is_ok
...
# fs_version = ['rffs']
# fs_version = ['fsv10']
config_flag = [f'config{x}' for x in range(1,6)]
# config_flag = [f'config10']
# config_flag = [f'config{x}' for x in [2, 5, 8, 11, 14]]
# config_flag = [f'config{x}' for x in [6, 7, 8, 9, 10]]
# config_flag = [f'config{x}' for x in [6]]
...
def wrapper(param_list):
    for param in tqdm(param_list):
        ...
        os.system(f'python3 /data/user/015614/Lucien/Zeus/Neptune/v1_0_6/scripts/model_all.py {argv_list}')
...
param_list = list(filter(lambda x: x[2] + '_' + x[1] in [f'config1_fsv10', 'config1_fsv11', 'config2_fsv11', 'config3_fsv11', 'config2_fsv8', 'config3_fsv8'], param_list))
