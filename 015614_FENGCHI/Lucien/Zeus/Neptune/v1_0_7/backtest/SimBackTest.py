...
from Zeus.Neptune.v1_0_7.config.path_conf import *
import math
import time
import os
...
class SimBackTest:
    def __init__(self, pred_fpath_list, fit_fpath_list, date_dict, period, config, profit_data_fpath=None, label=None, data_fpath=None, attend_ratio_range=(10, 45), save_flag=True, multi_attend=True):
    ...
    def state_machine(self):
        ...
        if STRATEGY_NAME == 'Neptune':
            profit_df['pct'] = profit_df['pct']
            # if self.config in ['config1', 'config2', 'config3', 'config4', 'config5']:
            #     profit_df['pct'] = profit_df['label_t2o9d1_pos']
            # else:
            #     profit_df['pct'] = profit_df['label_t2o10dc_pos']
    ...
    def calc_model_mingan(self, _test_df, _fit_df):
        ...
        for pred_reg in test_attend_df.index.tolist():
            ...
            try:
                ...
                fit_attend_df.loc[pred_reg, '收益风险比'] = round(abs(fit_attend_df.loc[pred_reg, '累计盈利'] / fit_attend_df.loc[pred_reg, '最大回撤']).iloc[0], 4)
            try:
