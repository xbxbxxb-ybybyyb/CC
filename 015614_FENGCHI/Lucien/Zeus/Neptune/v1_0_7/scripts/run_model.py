...
from Zeus.Neptune.v1_0_7.scripts.check_path_is_ok import check_path_is_ok
...
# fs_version = ['rffs']
fs_version = ['fsv10']
# config_flag = [f'config{x}' for x in range(1,4)]
# config_flag = [f'config10']
config_flag = [f'config{x}' for x in [1,4,5]]
...
def wrapper(param_list):
    for param in tqdm(param_list):
        ...
        os.system(f'python3 /data/user/015614/Lucien/Zeus/Neptune/v1_0_7/scripts/model_all.py {argv_list}')
...
# param_list = list(filter(lambda x: x[2] + '_' + x[1] in [f'config{x}_fsv10' for x in range(1, 11)] + ['config1_fsv8', 'config6_fsv8'], param_list))
