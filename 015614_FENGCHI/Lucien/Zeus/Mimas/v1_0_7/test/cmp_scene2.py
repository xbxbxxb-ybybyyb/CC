...
import pandas as pd
import numpy as np
from Zeus.Mimas.v1_0_7.config.path_conf import *
import os
...
model_name_list = os.listdir('/data/user/015614/Zeus/pred/Mimas/v1_0_7/')
...
for model_name in model_name_list:
    csv_fpath_list = os.listdir(f'/data/user/015614/Zeus/pred/Mimas/v1_0_7/{model_name}/')
    for period in period_list:
        ...
        csv_df = pd.read_csv(f'/data/user/015614/Zeus/pred/Mimas/v1_0_7/{model_name}/{test_start_date}~{test_end_date}.csv', index_col=0)
        ...
        os.makedirs(f'/data/user/015614/Zeus/pred/Mimas/v1_0_7_scene/{model_name}', exist_ok=True)
        new_csv_df['prediction'] = new_csv_df['pred_Reg'] >= new_threshold
        new_csv_df.to_csv(f'/data/user/015614/Zeus/pred/Mimas/v1_0_7_scene/{model_name}/{test_start_date}~{test_end_date}.csv')

        csv_df = pd.read_csv(f'/data/user/015614/Zeus/pred/Mimas/v1_0_7/{model_name}/{fit_start_date}~{fit_end_date}.csv', index_col=0)
        new_csv_df = csv_df.loc[list(set(csv_df.index.tolist()).intersection(scene_df_index))]
        new_csv_df['prediction'] = new_csv_df['pred_Reg'] >= new_threshold
        new_csv_df.to_csv(f'/data/user/015614/Zeus/pred/Mimas/v1_0_7_scene/{model_name}/{fit_start_date}~{fit_end_date}.csv')
