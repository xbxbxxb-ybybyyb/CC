...
class SimBackTest:
    ...
    @staticmethod
    def adjust_buy_amt(df):
        _df = df.copy()
        group = _df.query('prediction == 1').groupby('datelist')['label_buy_amt'].sum()
        max_amt = 3e8
        up_days = group[group > max_amt].index.tolist()
        # print(up_days)
        daily_signal_num = _df.query('prediction == 1').groupby('datelist')['label_pct'].count().reindex(list(sorted(set(_df['datelist'])))).fillna(0)
        _df['label_buy_amt'] = _df[['datelist', 'label_buy_amt']].apply(lambda x: min(x['label_buy_amt'], max_amt / daily_signal_num.loc[x['datelist']]), axis=1)
        _df['label_profit_cost'] = _df['label_pct_cost'] * _df['label_buy_amt']
        return _df
    ...
    def single_backtest(self, pred_fpath, fit_fpath):
        ...
        # """根据2024年市场情况，修改买入金额"""
        # test_df = self.adjust_buy_amt(test_df)
        # fit_df = self.adjust_buy_amt(fit_df)
