...
from Zeus.Mimas.v1_0_19.scripts.check_path_is_ok import check_path_is_ok
...
def wrapper(param_list):
    for param in tqdm(param_list):
        ...
        os.system(f'python3 /data/user/015614/Lucien/Zeus/Mimas/v1_0_19/scripts/model_all.py {argv_list}')
...
# param_list = list(filter(lambda x: x[2] + '_' + x[1] in ['config1_fsv8', 'config1_fsv11', 'config2_fsv8', 'config2_fsv11',
#                                                          'config3_fsv8', 'config3_fsv11', 'config4_fsv8', 'config4_fsv11'], param_list))
# param_list = list(filter(lambda x: x[2] + '_' + x[1] in ['config1_fsv10', 'config1_fsv11', 'config2_fsv10', 'config2_fsv8'], param_list))
# param_list = list(filter(lambda x: x[2] + '_' + x[1] in ['config1_fsrs', 'config1_fsv10', 'config2_fsrs', 'config2_fsv8'], param_list))
param_list = list(filter(lambda x: x[2] + '_' + x[1] in ['config1_fsrs', 'config1_fsv10', 'config1_fsv11', 'config2_fsv11'], param_list))
