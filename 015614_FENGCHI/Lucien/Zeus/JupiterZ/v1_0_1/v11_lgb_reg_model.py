...
import pandas as pd
import numpy as np
from Zeus.JupiterZ.v1_0_1.path_conf import *
from Zeus.JupiterZ.v1_0_1.hyper_param_space import hyper_lgb_reg_params
from Zeus.BTLite.BTLite2 import BTLite2
import lightgbm as lgb
from sklearn.metrics import roc_curve, auc
from LucienUtil.FileUtil import FileUtil
from Zeus.JupiterZ.v1_0_1.my_logger import MyLogger
import warnings
import os
import json
import joblib
...
fs_version = 'fsv11' # fsv8, fsv10, fsv11
lowcost_version = 'all' # all
afterZ_flag = 'True'
...
# lowcost_version = sys.argv[3]
# afterZ_flag = sys.argv[4]
# SUB_VERSION = f'v{PERIOD[-1]}'
# print(PERIOD, fs_version, lowcost_version, afterZ_flag)

hyper_search_mode = False

param = {}
best_threshold = -0.0015
if PERIOD[-1] == '1':
    param ={'boosting_type': 'gbdt', 'class_weight': None, 'colsample_bytree': 0.5, 'device': 'gpu', 'gpu_device_id': 0, 'gpu_platform_id': 1, 'gpu_use_dp': True, 'learning_rate': 0.005, 'max_depth': 6.0, 'metric': {'auc', 'binary_logloss'}, 'min_child_samples': 4.0, 'min_child_weight': 0.001, 'min_split_gain': 0.0, 'n_estimators': 1200.0, 'n_jobs': -1, 'num_leaves': 36.0, 'random_state': 2022, 'reg_alpha': 1.0, 'reg_lambda': 0.6, 'subsample': 0.9, 'subsample_freq': 0, 'verbosity': -1}
    best_threshold = -0.0015
elif PERIOD[-1] == '2':
    param ={'boosting_type': 'gbdt', 'class_weight': None, 'colsample_bytree': 0.5, 'device': 'gpu', 'gpu_device_id': 0, 'gpu_platform_id': 1, 'gpu_use_dp': True, 'learning_rate': 0.005, 'max_depth': 6.0, 'metric': {'auc', 'binary_logloss'}, 'min_child_samples': 4.0, 'min_child_weight': 0.001, 'min_split_gain': 0.0, 'n_estimators': 1200.0, 'n_jobs': -1, 'num_leaves': 36.0, 'random_state': 2022, 'reg_alpha': 1.0, 'reg_lambda': 0.6, 'subsample': 0.9, 'subsample_freq': 0, 'verbosity': -1}
    best_threshold = -0.0015
    ...
    param ={'boosting_type': 'gbdt', 'class_weight': None, 'colsample_bytree': 0.5, 'device': 'gpu', 'gpu_device_id': 0, 'gpu_platform_id': 1, 'gpu_use_dp': True, 'learning_rate': 0.005, 'max_depth': 6.0, 'metric': {'binary_logloss', 'auc'}, 'min_child_samples': 4.0, 'min_child_weight': 0.001, 'min_split_gain': 0.0, 'n_estimators': 1200.0, 'n_jobs': -1, 'num_leaves': 36.0, 'random_state': 2022, 'reg_alpha': 1.0, 'reg_lambda': 0.6, 'subsample': 0.9, 'subsample_freq': 0, 'verbosity': -1}
    best_threshold = -8.2e-05
elif PERIOD[-1] == '5':
    param = {'boosting_type': 'gbdt', 'class_weight': None, 'colsample_bytree': 0.5, 'device': 'gpu', 'gpu_device_id': 0, 'gpu_platform_id': 1, 'gpu_use_dp': True, 'learning_rate': 0.005, 'max_depth': 6.0, 'metric': {'binary_logloss', 'auc'}, 'min_child_samples': 4.0, 'min_child_weight': 0.001, 'min_split_gain': 0.0, 'n_estimators': 1200.0, 'n_jobs': -1, 'num_leaves': 36.0, 'random_state': 2022, 'reg_alpha': 1.0, 'reg_lambda': 0.6, 'subsample': 0.9, 'subsample_freq': 0, 'verbosity': -1}
    best_threshold = 0.0022
hyper_params = hyper_lgb_reg_params

class LgbRegModel:
    def __init__(self, sub_version='v1'):
        if afterZ_flag == 'True':
            self.model_name = f'{fs_version}_afterZ_{lowcost_version}_LgbRegModel'
        else:
            self.model_name = f'{fs_version}_{lowcost_version}_LgbRegModel'
        self.strategy_name = 'JupiterZ'
        self.version = 'v1_0_1'
        ...
        self.scaler = None
        if afterZ_flag == 'True':
            self.label = 'label_diff_pct_afterZ'
        else:
    ...
    def get_dataset(self, path):
        ...
        y = pd.read_pickle(profit_data_fpath)
        y = y[[self.label]]
    ...
    def filter_factor(self, xgb_imptc_fpath=None, factor_score_fpath=factor_score_fpath):
        ...
        if fs_version == 'rffs':
            if lowcost_version == 'lowCost':
                if afterZ_flag == 'True':
                    factor_list = pd.read_pickle(f'/data/user/015614/Zeus/factor_select/JupiterZ/v1_0_1/rffs_afterZ_{lowcost_version}_{PERIOD}.pkl')
                else:
                    factor_list = pd.read_pickle(f'/data/user/015614/Zeus/factor_select/JupiterZ/v1_0_1/rffs_{lowcost_version}_{PERIOD}.pkl')
            else:
                if afterZ_flag == 'True':
                    factor_list = pd.read_pickle(f'/data/user/015614/Zeus/factor_select/JupiterZ/v1_0_1/rffs_afterZ_{PERIOD}.pkl')
                else:
                    factor_list = pd.read_pickle(f'/data/user/015614/Zeus/factor_select/JupiterZ/v1_0_1/rffs_{PERIOD}.pkl')
        else:
    ...
    def get_train_and_test_data(self):
        ...
        if lowcost_version == 'lowCost':
            if afterZ_flag == 'True':
                if fs_version == 'fsv8':
                    filtered_factor = self.filter_factor(xgb_imptc_fpath=eval(f'xgb_imptc_lowCost_{PERIOD}_fpath'))
                elif fs_version == 'fsv10':
                    filtered_factor = self.filter_factor(xgb_imptc_fpath=eval(f'xgb_imptc_fsv10_lowCost_{PERIOD}_fpath'))
                elif fs_version == 'fsv11':
                    filtered_factor = self.filter_factor(xgb_imptc_fpath=eval(f'xgb_imptc_fsv11_lowCost_{PERIOD}_fpath'))
                else:
                    filtered_factor = self.filter_factor(xgb_imptc_fpath=eval(f'xgb_imptc_lowCost_{PERIOD}_fpath'))
            else:
                if fs_version == 'fsv8':
                    filtered_factor = self.filter_factor(xgb_imptc_fpath=eval(f'xgb_imptc_afterZ_lowCost_{PERIOD}_fpath'))
                elif fs_version == 'fsv10':
                    filtered_factor = self.filter_factor(xgb_imptc_fpath=eval(f'xgb_imptc_fsv10_afterZ_lowCost_{PERIOD}_fpath'))
                elif fs_version == 'fsv11':
                    filtered_factor = self.filter_factor(xgb_imptc_fpath=eval(f'xgb_imptc_fsv11_afterZ_lowCost_{PERIOD}_fpath'))
                else:
                    filtered_factor = self.filter_factor(xgb_imptc_fpath=eval(f'xgb_imptc_lowCost_{PERIOD}_fpath'))
        else:
            if afterZ_flag == 'True':
                if fs_version == 'fsv8':
                    ...
                    filtered_factor = self.filter_factor(xgb_imptc_fpath=eval(f'xgb_imptc_fsv10_{PERIOD}_fpath'))
                elif fs_version == 'fsv11':
                    filtered_factor = self.filter_factor(xgb_imptc_fpath=eval(f'xgb_imptc_fsv11_{PERIOD}_fpath'))
                else:
                    filtered_factor = self.filter_factor(xgb_imptc_fpath=eval(f'xgb_imptc_lowCost_{PERIOD}_fpath'))
            else:
                if fs_version == 'fsv8':
                    filtered_factor = self.filter_factor(xgb_imptc_fpath=eval(f'xgb_imptc_afterZ_{PERIOD}_fpath'))
                elif fs_version == 'fsv10':
                    filtered_factor = self.filter_factor(xgb_imptc_fpath=eval(f'xgb_imptc_fsv10_afterZ_{PERIOD}_fpath'))
                elif fs_version == 'fsv11':
                    filtered_factor = self.filter_factor(xgb_imptc_fpath=eval(f'xgb_imptc_fsv11_afterZ_{PERIOD}_fpath'))
                else:
                    filtered_factor = self.filter_factor(xgb_imptc_fpath=eval(f'xgb_imptc_lowCost_{PERIOD}_fpath'))
    ...
    def model_eval(self, y_true, y_pred, score_threshold=0.0, stage='test'):
        ...
        if not hyper_search_mode and stage is not 'no_draw':
            import matplotlib.pyplot as plt
    ...
    def tscv_eval_model(self, param):
        ...
        for idx, cv_list in enumerate(rolling_cv_month_list):
            ...
            self.train_model(X_train_scaled, y_train, param, X_valid=X_test_scaled, y_valid=y_valid, stage='no_draw')
            y_valid_pred = self.predict(X_test_scaled)

            eval_auc, eval_precision, eval_recall, eval_rmse, eval_ic, y_pred_label = self.model_eval(y_valid, y_valid_pred, score_threshold=best_threshold, stage='no_draw')
    ...
    def start_train(self, param):
        ...
        test_auc, test_precision, test_recall, test_rmse, test_ic, y_test_pred_label = self.model_eval(self.y_test, y_test_pred, score_threshold=best_threshold, stage='test')
        ...
        fit_auc, fit_precision, fit_recall, fit_rmse, fit_ic, y_fit_pred_label = self.model_eval(self.y_fit, y_fit_pred, score_threshold=best_threshold, stage='fit')
...
if hyper_search_mode:
    ...
    # # joblib.dump(inst.model.booster_, junk_path + 'period4_LgbRegModel.pkl')
    # inst.model.save_model(junk_path + 'LgbRegModel.pkl')
