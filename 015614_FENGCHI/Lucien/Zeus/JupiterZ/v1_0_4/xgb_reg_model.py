...
from Zeus.JupiterZ.v1_0_4.hyper_param_space import hyper_xgb_reg_params
...
SUB_VERSION = f'v{PERIOD[-1]}'
print(PERIOD, fs_version, lowcost_version, afterZ_flag, 'xgb')
...
class XgbRegModel:
    def __init__(self, sub_version='v1'):
        if afterZ_flag == 'True':
            self.model_name = f'{fs_version}_afterZ_{lowcost_version}_XgbRegModel'
        else:
            self.model_name = f'{fs_version}_{lowcost_version}_XgbRegModel'
        ...
        if not hyper_search_mode:
            if PERIOD[-1] == '1':
                ...
                self.best_threshold = -0.007517
            elif PERIOD[-1] == '2':
                self.param = pd.read_excel(self.log_path + f'{self.strategy_name}_{self.version}_{self.model_name}_{self.sub_version}_.xlsx', index_col=0).iloc[0]['整体参数']
                self.best_threshold = -0.005671
            elif PERIOD[-1] == '3':
                self.param = pd.read_excel(self.log_path + f'{self.strategy_name}_{self.version}_{self.model_name}_{self.sub_version}_.xlsx', index_col=0).iloc[0]['整体参数']
                self.best_threshold = -0.003022
            elif PERIOD[-1] == '4':
                self.param = pd.read_excel(self.log_path + f'{self.strategy_name}_{self.version}_{self.model_name}_{self.sub_version}_.xlsx', index_col=0).iloc[0]['整体参数']
