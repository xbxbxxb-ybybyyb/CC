...
import pandas as pd
import numpy as np
from Zeus.JupiterZ.v2_0_7.strat_conf import strategy_name, strategy_version
from Zeus.JupiterZ.v2_0_7.path_conf import profit_data_fpath, bt_out_path, date_config, data_all_fpath
import math
import time
import os
...
class SimBackTest:
    def __init__(self, pred_fpath_list, fit_fpath_list, date_dict, attend_ratio_range=(10, 45), save_flag=True, multi_attend=True):
    ...
    def state_machine(self):
        if strategy_name == 'JupiterZ':
            self.profit_fpath = profit_data_fpath
            self.label_fpath = data_all_fpath
            self.cost_pct = 0.002
    ...
    def wrapper(self, pred_fpath_tuple_list):
        for idx in tqdm(range(len(pred_fpath_tuple_list))):
            ...
            stats_df, model_test_mingan, model_fit_mingan = self.single_backtest(pred_fpath, fit_fpath)
            output_dict = {'汇总结果': stats_df, 'test': model_test_mingan, 'fit': model_fit_mingan}
            FileUtil.save_dict2xls(output_dict, os.path.dirname(pred_fpath), '/bt_result.xlsx', verbose=False)

    def start_backtest(self, multi=True):
        stats_df, model_test_mingan, model_fit_mingan = None, None, None
        if multi:
...
if __name__ == '__main__':
    sbt = SimBackTest(pred_fpath_list=[f'/data/user/015614/Zeus/pred/{strategy_name}/{strategy_version}/fsrs_pct_AllXgbRegModel/model/period3/seed_0/20201201~20210531.csv'],
                      fit_fpath_list=[f'/data/user/015614/Zeus/pred/{strategy_name}/{strategy_version}/fsrs_pct_AllXgbRegModel/model/period3/seed_0/20210501~20210531.csv'],
