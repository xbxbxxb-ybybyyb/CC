...
import pandas as pd
import numpy as np
from Zeus.JupiterZ.v1_0_5.path_conf import *
from Zeus.JupiterZ.v1_0_5.hyper_param_space import hyper_lgb_reg_params
...
from Zeus.JupiterZ.v1_0_5.my_logger import MyLogger
import warnings
import os
import json
import joblib
...
SUB_VERSION = f'v{PERIOD[-1]}'
print(PERIOD, fs_version, lowcost_version, afterZ_flag, 'lgb')
...
class LgbRegModel:
    def __init__(self, sub_version='v1'):
        ...
        self.log_path = log_path + f'{self.strategy_name}/v1_0_4/{self.model_name}/'
        ...
        if not hyper_search_mode:
            if PERIOD[-1] == '1':
                ...
                self.param = pd.read_excel(self.log_path + f'{self.strategy_name}_{self.version}_{self.model_name}_{self.sub_version}_.xlsx', index_col=0).iloc[0]['整体参数']
                self.best_threshold = 0.000208
            elif PERIOD[-1] == '5':
                self.param = pd.read_excel(self.log_path + f'{self.strategy_name}_v1_0_4_{self.model_name}_{self.sub_version}_.xlsx', index_col=0).iloc[0]['整体参数']
                self.best_threshold = -4.8e-05
    ...
    def filter_factor(self, xgb_imptc_fpath=None, factor_score_fpath=factor_score_fpath):
        ...
        if fs_version == 'rffs':
            if lowcost_version == 'lowCost':
                if afterZ_flag == 'True':
                    factor_list = pd.read_pickle(f'/data/user/015614/Zeus/factor_select/JupiterZ/v1_0_5/rffs_afterZ_{lowcost_version}_{PERIOD}.pkl')
                else:
                    factor_list = pd.read_pickle(f'/data/user/015614/Zeus/factor_select/JupiterZ/v1_0_5/rffs_{lowcost_version}_{PERIOD}.pkl')
            else:
                if afterZ_flag == 'True':
                    factor_list = pd.read_pickle(f'/data/user/015614/Zeus/factor_select/JupiterZ/v1_0_5/rffs_afterZ_{PERIOD}.pkl')
                else:
                    factor_list = pd.read_pickle(f'/data/user/015614/Zeus/factor_select/JupiterZ/v1_0_5/rffs_{PERIOD}.pkl')
        else:
