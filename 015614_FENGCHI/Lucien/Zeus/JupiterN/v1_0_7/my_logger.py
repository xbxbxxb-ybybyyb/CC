...
class MyLogger:
    def __init__(self, strategy_name=None, version=None, sub_version=None, model_name=None, remarks=None):
        ...
        log_file_path = os.path.join(log_path, f'{strategy_name}_{version}_{model_name}_{sub_version}.log')
        if os.path.exists(log_file_path):
            is_remove = input(f'{log_file_path}已存在，是否删除(y/n)?')
            if is_remove is 'y':
