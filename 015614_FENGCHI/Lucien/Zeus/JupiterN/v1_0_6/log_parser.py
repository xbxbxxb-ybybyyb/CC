...
def log_parse(log_path=None,
    ...
    output_dict = {'调参': multi,
                   '指标相关性': multi.iloc[:10].drop('整体参数', axis=1).corr(method='spearman')}

    FileUtil.save_dict2xls(output_dict, log_path, f'{log_fname[:-4]}_{note}.xlsx')

if __name__ == '__main__':
    log_parse(log_path='/data/user/015614/Zeus/logs/JupiterN/v1_0_6/LgbRegModel/',
              log_fname='JupiterN_v1_0_6_LgbRegModel_v1.log',
              note='')
    log_parse(log_path='/data/user/015614/Zeus/logs/JupiterN/v1_0_6/LgbRegModel/',
              log_fname='JupiterN_v1_0_6_LgbRegModel_v2.log',
              note='')