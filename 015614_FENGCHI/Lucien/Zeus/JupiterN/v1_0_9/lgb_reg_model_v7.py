...
"""
GapWalkForward(n_splits=5, max_train_size=30, test_size=3)  滚动时序验证，train训练集为30个月的滚动窗口，即每次剔除较远时间的数据，只保留距离valid最近的30个月数据作为训练集
"""

import pandas as pd
import numpy as np
from Zeus.JupiterN.v1_0_9.path_conf import *
from Zeus.JupiterN.v1_0_9.hyper_param_space import hyper_lgb_reg_params
from Zeus.BTLite.BTLite2 import BTLite2
import lightgbm as lgb
from sklearn.metrics import roc_curve, auc
from LucienUtil.FileUtil import FileUtil
from Zeus.JupiterN.v1_0_9.my_logger import MyLogger
...
class LgbRegModel:
    ...
    def model_eval(y_true, y_pred, score_threshold=0):
        ...
        return eval_auc, eval_precision, eval_recall, eval_rmse, y_pred_label

    def kcv_eval_model(self, param):
        gap_kfold = GapWalkForward(n_splits=5, max_train_size=30, test_size=3)
        eval_auc_list, eval_precision_list, eval_recall_list, eval_rmse_list = list(), list(), list(), list()
        for idx, (train_month_arr, valid_month_arr) in enumerate(gap_kfold.split(self.train_months)):
            train_month_list = list(np.array(self.train_months)[train_month_arr])
            valid_month_list = list(np.array(self.train_months)[valid_month_arr])
            ...
            X_valid = X_train_all.query(f'trade_month in {valid_month_list}').drop('trade_month', axis=1)
            y_train = y_train_all.query(f'trade_month in {train_month_list}').drop('trade_month', axis=1)
            y_valid = y_train_all.query(f'trade_month in {valid_month_list}').drop('trade_month', axis=1)
            ...
            eval_auc, eval_precision, eval_recall, eval_rmse, y_pred_label = self.model_eval(y_valid, y_valid_pred, score_threshold=best_threshold)
            print(f'第{idx + 1}折结果：{round(eval_auc, 4)}, {round(eval_precision, 4)}, {round(eval_recall, 4)}, {round(eval_rmse, 4)}')
        ...
        print(f'该参数下表现均值为：{round(eval_auc_mean, 4)}, {round(eval_auc_std, 4)}, {round(eval_precision_mean, 4)}, '
              f'{round(eval_recall_mean, 4)}, {round(eval_rmse_mean, 4)}')
        return eval_auc_mean, eval_auc_std, eval_precision_mean, eval_recall_mean, eval_rmse_mean
    ...
    def start_train(self, param):
        ...
        valid_auc, valid_auc_std, valid_precision, valid_recall, valid_rmse = self.kcv_eval_model(param=param_copy)
        ...
        test_auc, test_precision, test_recall, test_rmse, y_test_pred_label = self.model_eval(self.y_test, y_test_pred, score_threshold=best_threshold)
        ...
        fit_auc, fit_precision, fit_recall, fit_rmse, y_fit_pred_label = self.model_eval(self.y_fit, y_fit_pred, score_threshold=best_threshold)
        print(f'fit区间结果：{round(fit_auc, 4)}, {round(fit_precision, 4)}, {round(fit_recall, 4)}, {round(fit_rmse, 4)}')
        self.y_fit_pred = y_fit_pred

        return valid_auc, ((valid_auc, valid_auc_std, valid_precision, valid_recall, valid_rmse),
                           (test_auc, test_precision, test_recall, test_rmse),
                           (fit_auc, fit_precision, fit_recall, fit_rmse))
