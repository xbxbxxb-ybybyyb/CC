...
def wrapper(param_list):
    for param in tqdm(param_list):
        ...
        os.system(f'python3 /data/user/015614/Lucien/Zeus/JupiterN/v3_0_2/scripts/model_all.py {argv_list}')
...
# param_list = list(filter(lambda x: x[1] + '_' + x[2] in ['fsv8_config3', 'fsrs_config3'], param_list))
# param_list = list(filter(lambda x: x[1] + '_' + x[2] in ['fsv8_config1'], param_list))
param_list = list(filter(lambda x: x[1] + '_' + x[2] in ['fsv8_config4'], param_list))
