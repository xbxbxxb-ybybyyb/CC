...
import pandas as pd
import numpy as np
from Zeus.JupiterN.v1_0_5.path_conf import *
from Zeus.JupiterN.v1_0_5.hyper_param_space import hyper_lgb_reg_params
from Zeus.BTLite.BTLite2 import BTLite2
from lightgbm import LGBMRegressor
import lightgbm as lgb
from sklearn.metrics import roc_curve, auc
from LucienUtil.FileUtil import FileUtil
from Zeus.JupiterN.v1_0_5.my_logger import MyLogger
import warnings
import os
import json
import joblib
...
param = {}
if PERIOD[-1] is '1':
    param = {'boosting_type': 'gbdt', 'class_weight': None, 'colsample_bytree': 0.6, 'device': 'gpu', 'gpu_device_id': 0, 'gpu_platform_id': 1, 'gpu_use_dp': True, 'learning_rate': 0.005, 'max_depth': 9.0, 'metric': {'binary_logloss', 'auc', 'rmse'}, 'min_child_samples': 8.0, 'min_child_weight': 0.001, 'min_split_gain': 0.0, 'n_estimators': 3100.0, 'n_jobs': -1, 'num_leaves': 40.0, 'random_state': 2022, 'reg_alpha': 0.7, 'reg_lambda': 0.6, 'subsample': 0.9, 'subsample_freq': 0, 'verbosity': -1}
    best_threshold = 0
elif PERIOD[-1] is '2':
    param = {'boosting_type': 'gbdt', 'class_weight': None, 'colsample_bytree': 0.6, 'device': 'gpu', 'gpu_device_id': 0, 'gpu_platform_id': 1, 'gpu_use_dp': True, 'learning_rate': 0.005, 'max_depth': 5.0, 'metric': {'auc', 'binary_logloss'}, 'min_child_samples': 6.0, 'min_child_weight': 0.001, 'min_split_gain': 0.0, 'n_estimators': 2200.0, 'n_jobs': -1, 'num_leaves': 16.0, 'random_state': 2022, 'reg_alpha': 0.8, 'reg_lambda': 1.0, 'subsample': 0.6, 'subsample_freq': 0, 'verbosity': -1}
    best_threshold = 0
