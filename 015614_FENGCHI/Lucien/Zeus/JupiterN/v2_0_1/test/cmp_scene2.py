...
import pandas as pd
import numpy as np
from Zeus.JupiterN.v2_0_1.config.path_conf import *
import os
...
model_name_list = os.listdir('/data/user/015614/Zeus/pred/JupiterN/v2_0_1/')
...
for model_name in model_name_list:
    csv_fpath_list = os.listdir(f'/data/user/015614/Zeus/pred/JupiterN/v2_0_1/{model_name}/')
    for period in period_list:
        ...
        csv_df = pd.read_csv(f'/data/user/015614/Zeus/pred/JupiterN/v2_0_1/{model_name}/{test_start_date}~{test_end_date}.csv', index_col=0)
        ...
        os.makedirs(f'/data/user/015614/Zeus/pred/JupiterN/v2_0_1_scene/{model_name}', exist_ok=True)
        new_csv_df['prediction'] = new_csv_df['pred_Reg'] >= new_threshold
        new_csv_df.to_csv(f'/data/user/015614/Zeus/pred/JupiterN/v2_0_1_scene/{model_name}/{test_start_date}~{test_end_date}.csv')

        csv_df = pd.read_csv(f'/data/user/015614/Zeus/pred/JupiterN/v2_0_1/{model_name}/{fit_start_date}~{fit_end_date}.csv', index_col=0)
        new_csv_df = csv_df.loc[list(set(csv_df.index.tolist()).intersection(scene_df_index))]
        new_csv_df['prediction'] = new_csv_df['pred_Reg'] >= new_threshold
        new_csv_df.to_csv(f'/data/user/015614/Zeus/pred/JupiterN/v2_0_1_scene/{model_name}/{fit_start_date}~{fit_end_date}.csv')
