...
from Zeus.Saturn.v3_2_0.DataPrepare import DataPrepare
...
trade_months_ = list(map(lambda x: x // 100, tradeDate.trade_months))
...
class ModelBase:
    ...
    def rolling_train_and_predict(self, len_month=3, rolling_type='valid', param=None,
        ...
        rolling_train_test_list = self.get_rolling_index_by_samples(len_month, rolling_type)
    ...
    # 改为按月份滚动，从20180701开始每三个月进行一次滚动
    def get_rolling_index_by_samples(self, len_month=3, rolling_type='valid', rolling_start_month=201807):
        rolling_train_test_list = list()

        y = self.y.copy()
        y['trade_date'] = self.y.index.get_level_values(0).strftime('%Y%m%d').astype(int).tolist()
        y['trade_month'] = y['trade_date'] // 100
        ...
        # 首轮
        train_start_month = 201601
        train_end_month = trade_months_[trade_months_.index(rolling_start_month) - 1]
        test_start_month = trade_months_[trade_months_.index(train_end_month) + 1]
        test_end_month = trade_months_[trade_months_.index(train_end_month) + 3]
        tmp_train_y = y.query(f'trade_month >= {train_start_month} & trade_month <= {train_end_month}')
        tmp_test_y = y.query(f'trade_month >= {test_start_month} & trade_month <= {test_end_month}')

        train_start_idx = 0
        train_end_idx = len(tmp_train_y)
        test_start_idx = len(tmp_train_y) + 1
        test_end_idx = len(tmp_train_y) + len(tmp_test_y)

        while test_end_month <= self.test_end_date // 100:
            rolling_train_test_list.append([train_start_idx, train_end_idx, test_start_idx, test_end_idx])

            train_end_month = trade_months_[trade_months_.index(train_end_month) + 3]
            test_start_month = trade_months_[trade_months_.index(train_end_month) + 1]
            test_end_month = trade_months_[trade_months_.index(train_end_month) + 3]
            tmp_train_y = y.query(f'trade_month >= {train_start_month} & trade_month <= {train_end_month}')
            tmp_test_y = y.query(f'trade_month >= {test_start_month} & trade_month <= {test_end_month}')
