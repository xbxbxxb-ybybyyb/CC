...
def wrapper(param_list):
    for param in tqdm(param_list):
        ...
        os.system(f'python3 /data/user/015614/Lucien/Zeus/Saturn/v5_0_5/scripts/model_all.py {argv_list}')
...
param_list = list(filter(lambda x: x[1] + '_' + x[2] in ['fsci_config1', 'fsv8_config1', 'fsrs_config2', 'fsv11_config1'], param_list))
# param_list = list(filter(lambda x: x[1] + '_' + x[2] in ['fsrs_config4'], param_list))
# param_list = list(filter(lambda x: x[1] + '_' + x[2] in ['fsrs_config5', 'fsv11_config5', 'fsv8_config5', 'fsci_config5'], param_list))
# param_list = list(filter(lambda x: x[1] + '_' + x[2] in ['fsv8_config2'], param_list))
