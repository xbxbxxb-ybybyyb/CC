...
from Zeus.Saturn.v3_0_12.DataPrepare import DataPrepare
...
class ModelBase:
    ...
    @staticmethod
    def format_label_bylevel(x):
        if 0.12 > abs(x) >= 0.1:
            x = np.sign(x) * 0.1
        elif 0.14 > abs(x) >= 0.12:
            x = np.sign(x) * 0.12
        elif 0.16 > abs(x) >= 0.14:
            x = np.sign(x) * 0.14
        elif 0.18 > abs(x) >= 0.16:
            x = np.sign(x) * 0.16
        elif 0.2 > abs(x) >= 0.18:
            x = np.sign(x) * 0.18
        elif 0.25 > abs(x) >= 0.2:
    ...
    def get_dateset(self):
        ...
        # 处理数据集，这里要根据数据集进行变化
        y = pd.read_hdf('/data/group/800463/project/project2_prod/profit_backtest/p2_profit_931_0.20_0.10_500_1500.h5')
        y = y[['pct']]
        y.columns = [self.label]
        X = samples.drop(samples.filter(regex='label*').columns.tolist(), axis=1)
        y = y.reindex(index=X.index)
        y = y.applymap(self.format_label_bylevel)
