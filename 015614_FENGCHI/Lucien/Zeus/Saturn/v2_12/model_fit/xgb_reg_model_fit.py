...
from Zeus.Saturn.v2_12.Tool import multiprocess
from Zeus.Saturn.v2_12.hyper_param_space import hyper_xgb_reg_params_4fit, multi_xgb_reg_params
from Zeus.Saturn.v2_12.hyper_param_space import model_params
from Zeus.Saturn.v2_12.models.xgb_reg_model import XGBRegModel
from Zeus.Saturn.v2_12.my_logger import MyLogger
from Zeus.Saturn.v2_12.path_conf import *
# from Zeus.Saturn.v2_12.BackTest.SimpleEvalLaunch import EvalLaunch
from Zeus.Saturn.v2_12.BackTest.EvalLaunch import EvalLaunch
...
hyper_remark = '不滚动版本_正常顺序(测试在valid上调好阈值用在test上是否可行)_fit'
...
def tuning_objective(param):
    ...
    print(f'使用train时的因子数量{len(factor_list)}个')
    ...
    mse, mae, mae2, r2, evs = mf_inst.calc_model_reg_score(y_train, y_train_pred)
    ...
    mse_, mae_, mae2_, r2_, evs_ = mf_inst.calc_model_reg_score(y_valid, y_valid_pred)
    ...
    _mse, _mae, _mae2, _r2, _evs = mf_inst.calc_model_reg_score(y_test, y_test_pred)
    ...
    # 整合valid以及pred，保存
    # valid_test_pred = list(np.concatenate([y_train_pred > score_threshold, y_test_pred > score_threshold]))
    # valid_test_pred_prob = list(np.concatenate([y_train_pred, y_test_pred]))
    # valid_test_pred_df = pd.DataFrame(index=X_train.index).append(pd.DataFrame(index=X_test.index))
...
if __name__ == '__main__':
    if hyper_search_mode:
        ...
        best_param = fmin(tuning_objective,
                          space=hyper_xgb_reg_params_4fit,
                          algo=tpe.suggest,
                          trials=trials,
                          max_evals=max_evals,
                          verbose=True,
