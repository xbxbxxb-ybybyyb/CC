...
from Zeus.Saturn.v2_2.Tool import multiprocess, get_interval_data
from Zeus.Saturn.v2_2.hyper_param_space import hyper_xgb_reg_params, multi_xgb_reg_params
from Zeus.Saturn.v2_2.hyper_param_space import model_params
from Zeus.Saturn.v2_2.models.xgb_reg_model import XGBRegModel
from Zeus.Saturn.v2_2.my_logger import MyLogger
from Zeus.Saturn.v2_2.path_conf import *
from Zeus.Saturn.v2_2.BackTest.EvalLaunch import EvalLaunch
...
hyper_remark = '修复refer_num bug后进行测试'
...
def tuning_objective(param):
    ...
    # 和filter_v3版本样本对齐
    # v6_samples = pd.read_csv(fc_path + 'Zeus/pred/SaturnS1/xgb_reg_model/v2_6/20190102~20200630_xgbRegModel_v1.csv', index_col=0)
    # valid_test_pred_df = valid_test_pred_df.reindex(index=v6_samples.index)

    # 和filter_v3_down版本样本对齐
    # v6_samples = pd.read_csv(fc_path + 'Zeus/pred/Saturn_down/xgb_reg_model/v2_7/20190102~20200630_xgbRegModel_v1.csv', index_col=0)
    # valid_test_pred_df = valid_test_pred_df.reindex(index=v6_samples.index)

    # 和filter_v4版本样本对齐
    # v6_samples = pd.read_csv(fc_path + 'Zeus/pred/SaturnS1/xgb_reg_model/v2_9/20190102~20200630_xgbRegModel_v1.csv', index_col=0)
    # valid_test_pred_df = valid_test_pred_df.reindex(index=v6_samples.index)

    # 和935时点样本对齐
    # s5_samples = pd.read_csv(fc_path + 'Zeus/pred/SaturnS5/xgb_reg_model/v2_14/20190102~20200630_xgbRegModel_v1.csv', index_col=0)
    # valid_test_pred_df = valid_test_pred_df.reindex(index=s5_samples.index)
    # 和935时点样本对齐2
    # s5_samples = pd.read_csv(fc_path + 'Zeus/pred/SaturnS5/xgb_reg_model/v2_15/20190102~20200630_xgbRegModel_v1.csv', index_col=0)
    # valid_test_pred_df = valid_test_pred_df.reindex(index=s5_samples.index)

    # 和加入93030因子后的样本对齐
    v6_1_samples = pd.read_csv(fc_path + 'Zeus/pred/SaturnS1/xgb_reg_model/v2_16/20190102~20200630_xgbRegModel_v1.csv', index_col=0)
    valid_test_pred_df = valid_test_pred_df.reindex(index=v6_1_samples.index)
    ...
    if hyper_search_mode or multi_grid_search_mode:
        ...
        if fit_cheat_mode:
            ...
            factor_list = FileUtil.read_list(factor_path + f'{strategy_name}/{model_name}/{version}/',
                                             f'{model_name}_factor_list2.pkl')
