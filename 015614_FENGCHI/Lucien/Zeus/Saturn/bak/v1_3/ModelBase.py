...
from Zeus.Saturn.v1_3.DataPrepare import DataPrepare
...
class ModelBase:
    def __init__(self,
        ...
        if factor_score_path:
            factor_list = self.select_factor()
            self.factor_list = sorted(list(set(factor_list).intersection(self.factor_list)))
    ...
    def get_dateset(self):
        dp = DataPrepare()
        samples = dp.get_samples(self.saturn_data_path)
    ...
    def select_factor(self):
        if self.factor_score_path:
            factor_list = self.load_factor_score().set_index('factor_name')['区间1-in-mixed'].dropna()
            factor_list = factor_list[factor_list > 0]
            if self.factor_num > len(factor_list):
                factor_list = factor_list
            else:
                factor_list = factor_list.sort_values(ascending=False)[:self.factor_num]
            return factor_list.index.tolist()
        else:
