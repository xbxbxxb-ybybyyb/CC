...
from Zeus.Saturn.v3_0.ModelBase import ModelBase
...
class XGBRegModel(ModelBase):
    ...
    def my_eval(preds, dtrain):
        ...
        ic = -1 * stats.spearmanr(label, preds)[0]  # 这里改成斯皮尔曼相关性，皮尔逊是线性关系
        return 'my_eval', ic
    ...
    def train_model(self, X_train, y_train, X_valid, y_valid, param):
        ...
        train = [X_train.values, y_train.values.ravel()]
        ...
        model.fit(X_train.values, y_train.values.ravel(), early_stopping_rounds=early_stopping_rounds,
                  eval_set=eval_set, eval_metric=self.my_eval, verbose=False)
