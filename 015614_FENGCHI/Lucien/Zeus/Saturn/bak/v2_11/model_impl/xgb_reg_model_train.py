...
from Zeus.Saturn.v2_11.Tool import multiprocess, get_interval_data
from Zeus.Saturn.v2_11.hyper_param_space import hyper_xgb_reg_params, multi_xgb_reg_params
from Zeus.Saturn.v2_11.hyper_param_space import model_params
from Zeus.Saturn.v2_11.models.xgb_reg_model import XGBRegModel
from Zeus.Saturn.v2_11.my_logger import MyLogger
from Zeus.Saturn.v2_11.path_conf import *
from Zeus.Saturn.v2_11.BackTest.SimpleEvalLaunch import EvalLaunch
...
def tuning_objective(param):
    ...
    date_config = dict(train_start_date=0,
                       train_end_date=21019,
                       valid_start_date=21019,
                       valid_end_date=25948,
                       test_start_date=25948,
                       test_end_date=33958)
    ...
    selected_factor_list = used_factor_list
    used_factor_list = sorted(list(set(used_factor_list).intersection(set(selected_factor_list))))
    print(f'My因子筛选数量{len(selected_factor_list)}个，最终使用的因子数量{len(used_factor_list)}个')
    ...
    if hyper_search_mode or multi_grid_search_mode:
        if test_cheat_mode:
            eval_inst = EvalLaunch(date_config=date_config2,
                                   strategy_name=strategy_name,
                                   sel_model_names=['xgbRegModel'],
                                   valid_path_list=[output_path + pred_fname],
                                   pred_path_list=[output_path + pred_fname],
        ...
        if fit_cheat_mode:
            date_config = dict(train_start_date=0,
                               train_end_date=25948,
                               valid_start_date=25948,
                               valid_end_date=33958,
                               test_start_date=33958,
                               test_end_date=39142)
            date_config2 = dict(train_start_date=20160104,
                               train_end_date=20181231,
                               valid_start_date=20190102,
