...
from Zeus.Saturn.v3_1_0.path_conf import factor_score_fpath, filter_factor_fpath, factor_path
...
factor_score = factor_score.loc[~pd.isna(factor_score['out_score'])]
factor_score = factor_score.reindex(index=xgb_imptc_factor['factor_name'].tolist())
# 采用标准化处理因子顺序
# factor_score['区间1-out-value'] = scaler.fit_transform(factor_score['区间1-out-value'].values[:, None])
# 使用rank排名
factor_score['out_score'] = factor_score['out_score'].rank()

score = pd.concat([factor_score], axis=1)
score['total_score'] = score['out_score']
