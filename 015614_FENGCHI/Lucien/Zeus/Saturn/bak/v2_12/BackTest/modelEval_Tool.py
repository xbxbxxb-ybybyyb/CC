...
class modelEval_Tool:
    ...
    def select_label(self):
        ...
        if self.strategy == 'SaturnS0&SaturnS1':
            ...
            if self.strategy in ['SaturnS1', 'SaturnS0','Saturn','Saturn_down']:
    ...
    def generate_series_data(self):
        ...
        if self.strategy in ['SaturnS1', 'SaturnS0','Saturn','Saturn_down']: # ,'CeresS1'
        ...
        totalResDf, daily_data,statistic_by_sample,daily_data_valid,model_mingan,model_inmingan,group_proba_all = self.generate_test_report(model,model_valid,scene_flag,predcol)
        ...
        return totalResDf.fillna(0), daily_data,statistic_by_sample,daily_data_valid,model_mingan,model_inmingan,group_proba_all
    def generate_test_report(self,pred_out, pred_in,scene_flag,probacol):
        ...
        group_proba_all.to_excel(writer, sheet_name = '预测值分层统计')
        model_valid_mingan.to_excel(writer, sheet_name='样本内不同参与率表现')
        model_mingan.to_excel(writer, sheet_name = '不同参与率表现')
        writer.save()
        return totalResDf_monthly, daily_data,statistic_by_sample_all.set_index(['datelist','stockID']),statistic_by_day_valid,model_mingan,model_valid_mingan,group_proba_all
