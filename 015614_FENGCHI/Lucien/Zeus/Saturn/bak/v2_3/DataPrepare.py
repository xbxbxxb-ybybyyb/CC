...
from Zeus.Saturn.v2_3.path_conf import saturn_data_test_fpath
import pandas as pd
...
class DataPrepare:
    ...
    @staticmethod
    def get_samples(saturn_data_fpath=saturn_data_test_fpath, profit_fpath=None):
        samples = pd.read_pickle(saturn_data_fpath)
        profit = pd.read_hdf(profit_fpath)
        samples = pd.concat([samples, profit[['pct']]], join='inner', join_axes=[samples.index], axis=1)
        samples = samples.rename({'pct': 'label_pct'}, axis=1)
