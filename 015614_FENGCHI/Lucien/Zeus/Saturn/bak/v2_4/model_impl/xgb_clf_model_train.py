...
hyper_remark = '发现fit两个数据不匹配后进行测试'
    ...
    if hyper_search_mode or multi_grid_search_mode:
        ...
        if fit_cheat_mode:
            ...
            mf_inst = XGBClfModel(model_name=model_name,
                                  ...
                                  label='label_mixed')
            # 读取train时的因子
            factor_list = FileUtil.read_list(factor_path + f'{strategy_name}/{model_name}/{version}/', f'{model_name}_factor_list2.pkl')
            ...
            _, _, _, _, y_valid_prob, y_test_prob, _, y_valid_clf, y_test_clf = \
                get_interval_data(mf_inst.y, y_pred_prob_all, y_pred_clf_all, date_config)
            # 整合valid以及pred，保存
            valid_test_pred = pd.concat([y_valid_clf, y_test_clf], axis=0)
            valid_test_pred_prob = pd.concat([y_valid_prob, y_test_prob], axis=0)
