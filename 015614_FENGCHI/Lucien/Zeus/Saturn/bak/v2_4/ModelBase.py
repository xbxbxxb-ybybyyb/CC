...
from Zeus.Saturn.v2_4.DataPrepare import DataPrepare
...
class ModelBase:
    def __init__(self,
        ...
        dp = DataPrepare()
        self.samples = dp.get_samples(self.saturn_data_path, self.profit_path)
        self.y = None
    ...
    def rolling_train_and_predict(self, len_train=4000, len_test=1000, rolling_type='valid', param=None,
        ...
        for idx, train_test_idx_list in enumerate(rolling_train_test_list):
            ...
            y_train = y.iloc[train_start_idx:train_end_idx]     # 这里可以观测第一轮是否已经超过
            X_test = X.iloc[test_start_idx:test_end_idx]
            self.train_model(X_train, y_train, param)
            _, y_test_prob_pred = self.model_predict(X_test.values)
            _, y_train_prob_pred = self.model_predict(X_train.values)
    ...
    def get_rolling_index_by_samples(self, len_train=4000, len_test=1000, rolling_type='valid'):
        ...
        if rolling_train_test_list[-1][-1] < samples_len:
            rolling_train_test_list.append([train_start_idx, train_end_idx, test_start_idx, samples_len - 1])
