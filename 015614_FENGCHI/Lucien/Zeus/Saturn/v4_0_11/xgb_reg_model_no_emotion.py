...
import pandas as pd
import numpy as np
from Zeus.Saturn.v4_0_11.path_conf2 import *
from Zeus.Saturn.v4_0_11.hyper_param_space import hyper_xgb_reg_params
...
from Zeus.Saturn.v4_0_11.my_logger import MyLogger
...
fs_version = 'fsrs' # fsv8, fsv10, fsv11
abs_flag = 'None' # _abs None
...
if 'None' in abs_flag:
    abs_flag = ''
print(PERIOD, fs_version, abs_flag)
...
class XgbRegModel:
    def __init__(self, sub_version='v1'):
        self.model_name = f'{fs_version}{abs_flag}_XgbRegProdModel'
        self.strategy_name = 'Saturn'
        self.version = 'v4_0_11'
        ...
        if not hyper_search_mode:
            if PERIOD[-1] == '1':
                ...
                self.param = pd.read_excel(self.log_path + f'{self.strategy_name}_{self.version}_{self.model_name}_{self.sub_version}_.xlsx', index_col=0).iloc[0]['整体参数']
                self.best_threshold = 0.012041  # fsv11
                self.best_threshold = 0.011441  # rffs
                self.best_threshold = 0.009249  # fsrs

            elif PERIOD[-1] == '5':
                # self.param = pd.read_excel(self.log_path + f'{self.strategy_name}_{self.version}_{self.model_name}_{self.sub_version}_.xlsx', index_col=0).iloc[0]['整体参数']
                self.param = pd.read_excel('/data/user/015614/Zeus/logs/Saturn/v4_0_11/fsrs_XgbRegModel/Saturn_v4_0_11_fsrs_XgbRegModel_v5_.xlsx', index_col=0).iloc[0]['整体参数']
                # self.best_threshold = 0.009652  # fsv11
                # self.best_threshold = 0.006776  # rffs
                self.best_threshold = 0.008067  # fsrs
        ...
        self.model = None
        self.scaler = None
        self.label = 'label_v2o10d1_graded'
    ...
    def filter_factor(self, xgb_imptc_fpath=None, factor_score_fpath=factor_score_fpath):
        ...
        if fs_version == 'rffs':
            factor_list = pd.read_pickle(f'/data/user/015614/Zeus/factor_select/Saturn/v4_0_11/rffs_{PERIOD}{abs_flag}.pkl')
        elif fs_version == 'fsrs':
            factor_list = pd.read_excel(eval(f'fsrs_imptc_{PERIOD}{abs_flag}_fpath'), index_col=0)
    ...
    def get_train_and_test_data(self):
        ...
        if fs_version in ['fsv8', 'fsv10', 'fsv11']:
            filtered_factor = self.filter_factor(xgb_imptc_fpath=eval(f'xgb_imptc_{fs_version}_{PERIOD}{abs_flag}_fpath'))
        else:
            filtered_factor = self.filter_factor(xgb_imptc_fpath=eval(f'xgb_imptc_fsv8_{PERIOD}{abs_flag}_fpath'))
...
# btl = BTLite2(strategy_name='SaturnS1',
