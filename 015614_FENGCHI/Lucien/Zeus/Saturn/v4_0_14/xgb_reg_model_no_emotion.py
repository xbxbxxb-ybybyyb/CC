...
import pandas as pd
import numpy as np
from Zeus.Saturn.v4_0_14.path_conf import *
from Zeus.Saturn.v4_0_14.hyper_param_space import hyper_xgb_reg_params
...
from Zeus.Saturn.v4_0_14.my_logger import MyLogger
...
class XgbRegModel:
    def __init__(self, sub_version='v1'):
        self.model_name = f'{fs_version}{abs_flag}_XgbRegModel'
        self.strategy_name = 'Saturn'
        self.version = 'v4_0_14'
        ...
        if not hyper_search_mode:
            if PERIOD[-1] == '1':
                ...
                self.param = pd.read_excel(self.log_path + f'{self.strategy_name}_{self.version}_{self.model_name}_{self.sub_version}_.xlsx', index_col=0).iloc[0]['整体参数']
                self.best_threshold = 0.009576  #
            elif PERIOD[-1] == '6':
                self.param = pd.read_excel(self.log_path + f'{self.strategy_name}_{self.version}_{self.model_name}_{self.sub_version}_.xlsx', index_col=0).iloc[0]['整体参数']
                # self.best_threshold = 0.010903  # fsv8
                self.best_threshold = 0.010945  # fsv11
                # self.best_threshold = 0.009273  # rffs
    ...
    def filter_factor(self, xgb_imptc_fpath=None, factor_score_fpath=factor_score_fpath):
        ...
        if fs_version == 'rffs':
            factor_list = pd.read_pickle(f'/data/user/015614/Zeus/factor_select/Saturn/v4_0_14/rffs_{PERIOD}{abs_flag}.pkl')
