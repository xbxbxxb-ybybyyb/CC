...
from Zeus.Saturn.v2_10.DataPrepare import DataPrepare
...
class ModelBase:
    ...
    def get_dateset(self):
        ...
        # 这里要保证样本文件中存在这几天的数据
        X_train = X.iloc[self.train_start_date:self.train_end_date]
        y_train = y.iloc[self.train_start_date:self.train_end_date]
        X_valid = X.iloc[self.valid_start_date:self.valid_end_date]
        y_valid = y.iloc[self.valid_start_date:self.valid_end_date]
        X_test = X.iloc[self.test_start_date:self.test_end_date]
        y_test = y.iloc[self.test_start_date:self.test_end_date]
    ...
    def get_rolling_index_by_samples(self, len_train=4000, len_test=1000, rolling_type='valid'):
        ...
        while test_end_idx < samples_len:
            rolling_train_test_list.append([0, train_end_idx, test_start_idx, test_end_idx])
        ...
        if rolling_train_test_list[-1][-1] < samples_len:
            rolling_train_test_list.append([0, train_end_idx, test_start_idx, samples_len])
    ...
    def select_factor(self):
        if self.factor_score_path:
            # 剔除
            factor_filter = self.load_factor_score().set_index('factor_name')['out_mic'].dropna()
            factor_filter = factor_filter[factor_filter >= 0.07].index.tolist()

            # 排序
            factor_s = self.load_factor_score().set_index('factor_name')['out_score'].dropna()
            ...
            factor_list = pd.DataFrame(factor_s3).reset_index().sort_values(['out_score', 'factor_name'], ascending=False)
    ...
    def get_score_threshold(y_train, y_train_prob_pred, threshold=0.06, refer_num=1000):
        ...
        pred_threshold = np.percentile(y_train_prob_pred[:-refer_num], (1 - _threshold) * 100)  # ！！！！！# 这里有问题
