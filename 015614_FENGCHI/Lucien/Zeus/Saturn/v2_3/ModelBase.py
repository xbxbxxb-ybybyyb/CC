...
from Zeus.Saturn.v2_3.DataPrepare import DataPrepare
...
class ModelBase:
    ...
    def rolling_train_and_predict(self, len_train=4000, len_test=1000, rolling_type='valid', param=None,
        ...
        # 统计每个月的样本数
        # tmp_X = X.copy()
        # tmp_X['trade_month'] = X.index.get_level_values(0).strftime('%Y%m').astype(int).tolist()
        # stats_X = tmp_X.groupby('trade_month')['trade_month'].count()
        # print(f'训练接样本数{stats_X[stats_X.index <= 201812].sum()}个')
