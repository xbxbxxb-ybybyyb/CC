...
"""
采用FSRS因子版本，madminmax处理方式，label_pct_graded，回测三个区间
"""

import pandas as pd
import numpy as np
from Zeus.Europa.v1_0_32.path_conf import *
from Zeus.Europa.v1_0_32.hyper_param_space import hyper_lr_reg_params
...
# param_test ={'factor_num': 2020.0, 'fit_intercept': True, 'n_jobs': 10, 'normalize': True, 'score_threshold': -0.00324574709395806}
# param_fit ={'factor_num': 2020.0, 'fit_intercept': False, 'n_jobs': 10, 'normalize': True, 'score_threshold': 0.00103393425902647}

# param_test = {'factor_num': 2020.0, 'fit_intercept': False, 'n_jobs': 10, 'normalize': True, 'score_threshold': 0.00127372144706346}
# param_fit ={'factor_num': 2020.0, 'fit_intercept': False, 'n_jobs': 10, 'normalize': True, 'score_threshold': 0.00386091212574983}

param_test ={'factor_num': 2020.0, 'fit_intercept': True, 'n_jobs': 10, 'normalize': True, 'score_threshold': 0.00300348928818725}
param_fit ={'factor_num': 2020.0, 'fit_intercept': True, 'n_jobs': 10, 'normalize': True, 'score_threshold': 0.000880972458031096}

# period4 test 作废
# param_test = {'factor_num': 2020.0, 'fit_intercept': True, 'n_jobs': 10, 'normalize': True, 'score_threshold': 0.00170782834152571}
# param_fit = {'factor_num': 2020.0, 'fit_intercept': True, 'n_jobs': 10, 'normalize': True, 'score_threshold': 0.00170782834152571}

# param_test = {'factor_num': 2020.0, 'fit_intercept': True, 'n_jobs': 10, 'normalize': True, 'score_threshold': 0.00458630507435262}
# param_fit = {'factor_num': 2020.0, 'fit_intercept': True, 'n_jobs': 10, 'normalize': True, 'score_threshold': 0.00458630507435262}

hyper_params = hyper_lr_reg_params

class LrRegModel:
    def __init__(self, mode='test', sub_version='v1'):
        self.model_name = 'LrRegModel'
    ...
    def filter_factor(self, xgb_imptc_fpath=None, factor_score_fpath=factor_score_fpath):
        factor_list = self.fun_filter_factor4(xgb_imptc_fpath=xgb_imptc_fpath, factor_score_fpath=factor_score_fpath)
        return factor_list
    ...
    def fun_filter_factor4(self, xgb_imptc_fpath=None, factor_score_fpath=None):
        """叠加lowCost筛选，周五下午已经叫孙少森添加了这部分限制，并重新筛选了RS因子"""
        filter_factor = pd.read_excel(xgb_imptc_fpath, index_col=0)
        _xgb_imptc_factor = filter_factor.query('select==1')
        factor_list = _xgb_imptc_factor.index.tolist()

        # lowcost_list = pd.read_excel(factor_score_fpath, index_col=0).query('lowCost==1')['factor_name'].tolist()
        # factor_list = list(set(factor_list).intersection(set(lowcost_list)))
        print(f'use {len(factor_list)} factors')
    ...
    def get_train_and_test_data(self):
        ...
        filtered_factor = self.filter_factor(xgb_imptc_fpath=eval(f'fsrs_imptc_{PERIOD}_fpath'), factor_score_fpath=factor_score_fpath)
    ...
    def xbc_start_rolling_halfyear_train(self, param):
        ...
        valid_test_pred_df = valid_test_pred_df.sort_index()
        return valid_test_pred_df


inst_test = LrRegModel(mode='test', sub_version=SUB_VERSION)
inst_fit = LrRegModel(mode='fit', sub_version=SUB_VERSION)
...
if hyper_search_mode:
    ...
    # inst_fit = inst_test
    # joblib.dump(inst_fit.model, junk_path + 'period4_LrRegModel.pkl')
