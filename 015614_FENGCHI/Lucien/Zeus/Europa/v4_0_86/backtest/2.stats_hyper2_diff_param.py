...
if __name__ == '__main__':
    ...
    hyper_root_path = f'/data/user/015614/Zeus/pred/Europa/{STRATEGY_VERSION}/config7/'
    ...
    for model_name in model_names:
        ...
        for search_time in hyper_list:
            if not os.path.exists(hyper_root_path + model_name + f'/hyper/{search_time}/bt_result_{PERIOD}.xlsx'):
                continue

            with open(hyper_root_path + model_name + f'/hyper/{search_time}/param.json', 'r') as f_obj:
                param = json.load(f_obj)

            bt_result_dict = pd.read_excel(hyper_root_path + model_name + f'/hyper/{search_time}/bt_result_{PERIOD}.xlsx', index_col=0, sheet_name=None)
            print(search_time)
