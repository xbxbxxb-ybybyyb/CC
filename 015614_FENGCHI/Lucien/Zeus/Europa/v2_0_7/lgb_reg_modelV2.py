...
import pandas as pd
import numpy as np
from Zeus.Europa.v2_0_1.path_conf import *  # 5个情绪因子的版本
from Zeus.Europa.v2_0_7.hyper_param_space import hyper_lgb_reg_params
from Zeus.BTLite.BTLite2 import BTLite2
import lightgbm as lgb
from sklearn.metrics import roc_curve, auc
from LucienUtil.FileUtil import FileUtil
from Zeus.Europa.v2_0_7.my_logger import MyLogger
import warnings
import os
import json
import joblib
...
best_threshold = 0
if PERIOD[-1] is '1':
    param = {'boosting_type': 'gbdt', 'class_weight': None, 'colsample_bytree': 0.7, 'device': 'gpu', 'gpu_device_id': 0, 'gpu_platform_id': 1, 'gpu_use_dp': True, 'learning_rate': 0.005, 'max_depth': 8.0, 'metric': {'auc', 'binary_logloss'}, 'min_child_samples': 16.0, 'min_child_weight': 0.001, 'min_split_gain': 0.0, 'n_estimators': 4000.0, 'n_jobs': -1, 'num_leaves': 28.0, 'random_state': 2022, 'reg_alpha': 0.4, 'reg_lambda': 0.2, 'subsample': 0.6, 'subsample_freq': 0, 'verbosity': -1}
    best_threshold = 0
elif PERIOD[-1] is '2':
    param = {'boosting_type': 'gbdt', 'class_weight': None, 'colsample_bytree': 0.5, 'device': 'gpu', 'gpu_device_id': 0, 'gpu_platform_id': 1, 'gpu_use_dp': True, 'learning_rate': 0.005, 'max_depth': 5.0, 'metric': {'binary_logloss', 'auc'}, 'min_child_samples': 4.0, 'min_child_weight': 0.001, 'min_split_gain': 0.0, 'n_estimators': 3500.0, 'n_jobs': -1, 'num_leaves': 20.0, 'random_state': 2022, 'reg_alpha': 0.8, 'reg_lambda': 0.0, 'subsample': 0.6, 'subsample_freq': 0, 'verbosity': -1}
    best_threshold = 0
elif PERIOD[-1] is '3':
    param = {'boosting_type': 'gbdt', 'class_weight': None, 'colsample_bytree': 0.6, 'device': 'gpu', 'gpu_device_id': 0, 'gpu_platform_id': 1, 'gpu_use_dp': True, 'learning_rate': 0.005, 'max_depth': 9.0, 'metric': {'auc', 'binary_logloss'}, 'min_child_samples': 14.0, 'min_child_weight': 0.001, 'min_split_gain': 0.0, 'n_estimators': 2700.0, 'n_jobs': -1, 'num_leaves': 32.0, 'random_state': 2022, 'reg_alpha': 0.7, 'reg_lambda': 0.5, 'subsample': 0.9, 'subsample_freq': 0, 'verbosity': -1}
    best_threshold = 0
hyper_params = hyper_lgb_reg_params

class LgbRegModel:
    def __init__(self, sub_version='v1'):
        self.model_name = 'LgbRegModelV2'   # old版本，重新设置100轮寻优进行查找
