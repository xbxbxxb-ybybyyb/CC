...
from Zeus.Europa.v2_0_15.hyper_param_space import hyper_lr_reg_params
...
if PERIOD[-1] is '1':
    param = {'fit_intercept': False, 'n_jobs': 10, 'normalize': True}
    best_threshold = -0.006028
elif PERIOD[-1] is '2':
    param = {'fit_intercept': False, 'n_jobs': 10, 'normalize': True}
    best_threshold = -0.003967
elif PERIOD[-1] is '3':
    param = {'fit_intercept': False, 'n_jobs': 10, 'normalize': True}
    best_threshold = -0.005583
elif PERIOD[-1] is '4':
    param ={'fit_intercept': False, 'n_jobs': 10, 'normalize': True}
    best_threshold = -0.001624
elif PERIOD[-1] is '5':
    param = {'fit_intercept': False, 'n_jobs': 10, 'normalize': True}
    best_threshold = -0.001964
hyper_params = hyper_lr_reg_params

class LinRegModel:
    def __init__(self, sub_version='v1'):
        self.model_name = 'LinFsrsRegModel'
    ...
    def filter_factor(self, xgb_imptc_fpath=None, factor_score_fpath=factor_score_fpath):
        ...
        if 'V_DNoBActMin4' in factor_list:
            factor_list.remove('V_DNoBActMin4')
    ...
    def fun_filter_factor(self, xgb_imptc_fpath=None, factor_score_fpath=None):
        ...
        _xgb_imptc_factor = filter_factor_df.query('select==1')
    ...
    def get_train_and_test_data(self):
        ...
        filtered_factor = self.filter_factor(xgb_imptc_fpath=eval(f'fsrsv2_imptc_{PERIOD}_fpath'))
    ...
    def train_model(self, X_train, y_train, param, X_valid=None, y_valid=None, stage='no_draw'):
        lr_model = LinearRegression(param)
        model = lr_model.fit(X_train.dropna(how='all', axis=1), y_train)

        self.model = model

    def predict(self, X_other):
        _y_pred = self.model.predict(X_other.dropna(how='all', axis=1))
        return _y_pred

    def model_eval(self, y_true, y_pred, score_threshold=0.0, stage='test'):
        y_true_label = y_true.values.reshape(-1) > 0
        y_pred_label = y_pred.reshape(-1) > score_threshold
        ...
        eval_ic = pd.DataFrame([y_true.values.reshape(-1), y_pred.reshape(-1)]).T.corr('spearman').iloc[0, 1]
...
inst = LinRegModel(sub_version=SUB_VERSION)
...
def tuning_params(param):
    ...
    y_test_pred_df = inst.tranfrom_pred_data(inst.y_test_pred.reshape(-1), index=inst.y_test.index, score_threshold=best_threshold)
    FileUtil.save_df2csv(y_test_pred_df, inst.hyper_pred_out_path, inst.test_pred_fname)

    y_fit_pred_df = inst.tranfrom_pred_data(inst.y_fit_pred.reshape(-1), index=inst.y_fit.index, score_threshold=best_threshold)
...
if hyper_search_mode:
    ...
    joblib.dump(inst.model, junk_path + 'LinRegModel.pkl')
