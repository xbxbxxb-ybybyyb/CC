...
from Zeus.Europa.v2_0_13.hyper_param_space import hyper_lgb_reg_params
from Zeus.BTLite.BTLite2 import BTLite2
import lightgbm as lgb
from sklearn.metrics import roc_curve, auc
from LucienUtil.FileUtil import FileUtil
from Zeus.Europa.v2_0_13.my_logger import MyLogger
import warnings
import os
import json
import joblib
...
if PERIOD[-1] is '1':
    ...
    best_threshold = -0.007232
elif PERIOD[-1] is '2':
    param ={'boosting_type': 'gbdt', 'class_weight': None, 'colsample_bytree': 0.8, 'device': 'gpu', 'gpu_device_id': 0, 'gpu_platform_id': 1, 'gpu_use_dp': True, 'learning_rate': 0.005, 'max_depth': 8.0, 'metric': {'binary_logloss', 'auc'}, 'min_child_samples': 14.0, 'min_child_weight': 0.001, 'min_split_gain': 0.0, 'n_estimators': 2700.0, 'n_jobs': -1, 'num_leaves': 16.0, 'random_state': 2022, 'reg_alpha': 0.7, 'reg_lambda': 0.3, 'subsample': 0.5, 'subsample_freq': 0, 'verbosity': -1}
    best_threshold = -0.004772
elif PERIOD[-1] is '3':
    ...
    best_threshold = -0.003091
elif PERIOD[-1] is '4':
    param ={'boosting_type': 'gbdt', 'class_weight': None, 'colsample_bytree': 0.6, 'device': 'gpu', 'gpu_device_id': 0, 'gpu_platform_id': 1, 'gpu_use_dp': True, 'learning_rate': 0.005, 'max_depth': 9.0, 'metric': {'auc', 'binary_logloss'}, 'min_child_samples': 8.0, 'min_child_weight': 0.001, 'min_split_gain': 0.0, 'n_estimators': 2300.0, 'n_jobs': -1, 'num_leaves': 40.0, 'random_state': 2022, 'reg_alpha': 0.9, 'reg_lambda': 0.6, 'subsample': 1.0, 'subsample_freq': 0, 'verbosity': -1}
    best_threshold = -0.001632
elif PERIOD[-1] is '5':
    param = {'boosting_type': 'gbdt', 'class_weight': None, 'colsample_bytree': 0.5, 'device': 'gpu', 'gpu_device_id': 0, 'gpu_platform_id': 1, 'gpu_use_dp': True, 'learning_rate': 0.005, 'max_depth': 8.0, 'metric': {'auc', 'binary_logloss'}, 'min_child_samples': 16.0, 'min_child_weight': 0.001, 'min_split_gain': 0.0, 'n_estimators': 3100.0, 'n_jobs': -1, 'num_leaves': 40.0, 'random_state': 2022, 'reg_alpha': 1.0, 'reg_lambda': 0.7, 'subsample': 0.6, 'subsample_freq': 0, 'verbosity': -1}
    best_threshold = -0.001104
hyper_params = hyper_lgb_reg_params

class LgbRegModel:
    def __init__(self, sub_version='v1'):
        self.model_name = 'LgbRegModelV11'
        self.strategy_name = 'Europa'
        self.version = 'v2_0_13'
        ...
        self.get_dataset(junk_path + 'factor_df_all_20160101_20220630_replace4factors.pkl')
    ...
    def filter_factor(self, xgb_imptc_fpath=None, factor_score_fpath=factor_score_fpath):
        ...
        factor_list = pd.read_pickle(f'/data/user/015614/Zeus/factor_select/Europa/v2_0_13/rf_fs_list_{PERIOD}.pkl')
        factor_list = list(set(factor_list).difference(set(list(['wyc_buyPMDay253_mean', 't_emo_ave_pct_adj', 't_emo_ave_pct2open_adj']))))
        factor_list.extend(['wyc_buyPMDay253', 't_emo_ave_pct', 'Jlzt_kcs_mbzs_alt'])
...
if hyper_search_mode:
    ...
    # # joblib.dump(inst.model.booster_, junk_path + 'LgbRegModel.pkl')
