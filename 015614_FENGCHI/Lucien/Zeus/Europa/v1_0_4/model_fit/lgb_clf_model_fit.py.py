...
from Zeus.Europa.v1_0_4.Tool import multiprocess
from Zeus.Europa.v1_0_4.hyper_param_space import hyper_lgb_clf_params, multi_lgb_clf_params
from Zeus.Europa.v1_0_4.models.lgb_clf_model import LGBClfModel
from Zeus.Europa.v1_0_4.my_logger import MyLogger
from Zeus.Europa.v1_0_4.path_conf import *
...
pred_type = 'fit'   # 这个参数只在not_search_but_backtest为True时起作用，可选test/fit，给敬姐回测框架传的参数，定义文件名
...
version = 'v1_0_4'
model_name = 'lgb_clf_model'
hump_model_name = 'lgbClfModel'
hyper_remark = ''
verbose = False     # 控制是否输出内容
search_time = 0
fit_fname = f'20190102~20201231_{hump_model_name}_v1.csv'
...
def tuning_objective(param):
    ...
    mf_inst = LGBClfModel(model_name=model_name,
    ...
    # 读取train时的因子
    selected_factor_list = FileUtil.read_list(factor_path + f'{strategy_name}/{model_name}/{version}/', f'factor_list.pkl')
    ...
    # 评估部分
    y_train_pred = mf_inst.model_predict(X_train.values)[:, 0]
    ...
    y_valid_pred = mf_inst.model_predict(X_valid.values)[:, 0]
    ...
    y_test_pred = mf_inst.model_predict(X_test.values)[:, 0]
    ...
    # 和v6版本样本对齐， TODO: 是否对齐
    # v6_samples = pd.read_csv(fc_path + 'Zeus/pred/Europa/lgb_reg_model/v3_0_37/20190102~20201231_lgbRegModel_v1.csv', index_col=0)
...
if __name__ == '__main__':
    if hyper_search_mode:
        ...
        multi_params_list = list(multi_lgb_clf_params.values())
        params = list(product(*multi_params_list))
        print(f'超参数的组合个数为{len(params)}个')
        grid_params = [dict(zip(list(multi_lgb_clf_params.keys()), _param)) for _param in params]
        ...
        from Zeus.Europa.v1_0_4.hyper_param_space import model_params_fit

        param = model_params_fit[model_name]
        if not param:
            from Zeus.Europa.v1_0_4.hyper_param_space import model_params
