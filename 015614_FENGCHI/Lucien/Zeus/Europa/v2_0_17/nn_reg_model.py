...
if PERIOD[-1] is '1':
    ...
    best_threshold = -0.008229
elif PERIOD[-1] is '2':
    param ={'fit_intercept': False, 'n_jobs': 10, 'normalize': True}
    best_threshold = -0.003727
elif PERIOD[-1] is '3':
    param ={'fit_intercept': False, 'n_jobs': 10, 'normalize': True}
    best_threshold = -0.00332
elif PERIOD[-1] is '4':
    param ={'fit_intercept': False, 'n_jobs': 10, 'normalize': True}
    best_threshold = -0.003324
elif PERIOD[-1] is '5':
    param = {'fit_intercept': False, 'n_jobs': 10, 'normalize': True}
    best_threshold = -0.001503
hyper_params = hyper_lr_reg_params


class SimpleNet(nn.Module):
    def __init__(self, in_dim, hidden_1_size, hidden_2_size, out_dim, dropout=0.4):
...
class LinRegModel:
    ...
    def tscv_eval_model(self, param):
        ...
        for idx, cv_list in enumerate(rolling_cv_month_list):
            ...
            y_valid_pred = self.predict(X_test_scaled.values)
