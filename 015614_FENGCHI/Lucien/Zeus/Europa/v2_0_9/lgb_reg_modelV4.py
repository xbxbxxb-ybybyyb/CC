...
import pandas as pd
import numpy as np
from Zeus.Europa.v2_0_9.path_conf import *
from Zeus.Europa.v2_0_9.hyper_param_space import hyper_lgb_clf_params
from Zeus.BTLite.BTLite2 import BTLite2
import lightgbm as lgb
from sklearn.metrics import roc_curve, auc
from LucienUtil.FileUtil import FileUtil
from Zeus.Europa.v2_0_9.my_logger import MyLogger
import warnings
import os
import json
import joblib
...
if PERIOD[-1] is '1':
    param = {'boosting_type': 'gbdt', 'class_weight': None, 'colsample_bytree': 0.5, 'device': 'gpu', 'gpu_device_id': 0, 'gpu_platform_id': 1, 'gpu_use_dp': True, 'learning_rate': 0.005, 'max_depth': 6.0, 'metric': {'auc', 'binary_logloss'}, 'min_child_samples': 4.0, 'min_child_weight': 0.001, 'min_split_gain': 0.0, 'n_estimators': 2400.0, 'n_jobs': -1, 'num_leaves': 36.0, 'objective': 'binary', 'random_state': 2022, 'reg_alpha': 1.0, 'reg_lambda': 0.6, 'subsample': 0.9, 'subsample_freq': 0, 'verbosity': -1}
    best_threshold = 0.349353
elif PERIOD[-1] is '2':
    param ={'boosting_type': 'gbdt', 'class_weight': None, 'colsample_bytree': 0.6, 'device': 'gpu', 'gpu_device_id': 0, 'gpu_platform_id': 1, 'gpu_use_dp': True, 'learning_rate': 0.005, 'max_depth': 9.0, 'metric': {'auc', 'binary_logloss'}, 'min_child_samples': 18.0, 'min_child_weight': 0.001, 'min_split_gain': 0.0, 'n_estimators': 3200.0, 'n_jobs': -1, 'num_leaves': 20.0, 'objective': 'binary', 'random_state': 2022, 'reg_alpha': 0.0, 'reg_lambda': 0.5, 'subsample': 0.5, 'subsample_freq': 0, 'verbosity': -1}
    best_threshold = 0.358927
elif PERIOD[-1] is '3':
    param = {'boosting_type': 'gbdt', 'class_weight': None, 'colsample_bytree': 0.5, 'device': 'gpu', 'gpu_device_id': 0, 'gpu_platform_id': 1, 'gpu_use_dp': True, 'learning_rate': 0.005, 'max_depth': 6.0, 'metric': {'auc', 'binary_logloss'}, 'min_child_samples': 10.0, 'min_child_weight': 0.001, 'min_split_gain': 0.0, 'n_estimators': 3300.0, 'n_jobs': -1, 'num_leaves': 36.0, 'objective': 'binary', 'random_state': 2022, 'reg_alpha': 0.7, 'reg_lambda': 0.2, 'subsample': 0.9, 'subsample_freq': 0, 'verbosity': -1}
    best_threshold = 0.420417
...
class LgbRegModel:
    ...
    def filter_factor(self, xgb_imptc_fpath=None, factor_score_fpath=factor_score_fpath):
        ...
        factor_list = pd.read_pickle(f'/data/user/015614/Zeus/factor_select/Europa/v2_0_9/rf_fs_clf_list_{PERIOD}.pkl')
    ...
    def get_train_and_test_data(self):
        ...
        filtered_factor = self.filter_factor(xgb_imptc_fpath=eval(f'xgb_imptc_mix_{PERIOD}_fpath'))
    ...
    def model_eval(y_true, y_pred, score_threshold=0):
        y_true_label = y_true.values.reshape(-1)
