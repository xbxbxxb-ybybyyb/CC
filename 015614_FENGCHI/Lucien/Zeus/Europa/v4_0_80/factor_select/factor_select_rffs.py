...
from Zeus.Europa.v4_0_80.config.strat_conf import *
import pandas as pd
import numpy as np
import importlib
...
module_name = f'Zeus.Europa.v4_0_80.path_conf'
module = importlib.import_module(module_name)

class FactorSelect:
    def __init__(self, period, label, data_fpath, profit_data_fpath, factor_score_fpath):
        ...
        self.factor_score_fpath = factor_score_fpath
    ...
    def get_dataset(self, path):
        ...
        factor_info = pd.read_excel(self.factor_score_fpath, index_col=0).set_index('factor_name')
    ...
    def launch_1period(self):
        FS = FactorSelect(self.start_date, self.end_date)
...
factor_score_fpath = PT['factor_score_fpath']
fs = FactorSelect(period, label, data_fpath, profit_data_fpath, factor_score_fpath)
...
FileUtil.save_list2pkl(selected_factor_list, factor_select_path + f'{fs.strategy_name}/{fs.version}/{config}/', f'rffs_{period}.pkl')