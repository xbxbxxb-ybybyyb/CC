...
import json
import pandas as pd
from Zeus.Europa.v4_0_54.strat_conf import strategy_version
from Zeus.Europa.v4_0_54.path_conf import *
from Zeus.Europa.v4_0_54.hyper_param_space import xgb_fixed_params, lgb_fixed_params
...
if __name__ == '__main__':
    ...
    for model_name in model_names:
        ...
        best_seed_list = model_param_res.sort_values('平均收益风险比', ascending=False).groupby(_watch_params)['seed'].agg('first').tolist()
        groupby_param_res = model_param_res.groupby(_watch_params).agg({'累计扣费总收益': 'mean',
                                                                       '最大回撤': 'mean',
                                                                       '收益风险比': 'mean',
                                                                       '收益夏普比率': 'mean',
                                                                       '平均收益风险比': 'mean',
                                                                       '平均收益夏普比率': 'mean'}).sort_values(_watch_params).reset_index()
    ...
    FileUtil.save_df2xls(res_df, '/data/user/015614/junkData/', f'hyper2_res_{version}.xlsx')
    send_file(f'/data/user/015614/junkData/hyper2_res_{version}.xlsx')