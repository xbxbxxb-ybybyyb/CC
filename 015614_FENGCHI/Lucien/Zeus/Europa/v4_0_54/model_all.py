...
from Zeus.Europa.v4_0_54.backtest.sim_backtest.SimBackTest import SimBackTest

import pandas as pd
import numpy as np
from tqdm import tqdm
from joblib import Parallel, delayed
from Zeus.Europa.v4_0_54.path_conf import *
from Zeus.Europa.v4_0_54.hyper_param_space import *
import xgboost as xgb
import lightgbm as lgb
from Zeus.Europa.v4_0_54.models.MlpRegModel import MlpRegModel
from sklearn.metrics import roc_curve, auc
from LucienUtil.FileUtil import FileUtil
from Zeus.Europa.v4_0_54.my_logger import MyLogger
import warnings
import random
import os
...
if len(sys.argv) > 1:
    ...
    period = 'period4'
    fs_version = 'fsv11' # fsv8\fsv10\fsv11\rffs
    label_flag = 'pct'
    model_select = 'XgbRegModel'
    scaler_select = 'scaler2'
    hyper_search_mode = 2
    attend_ratio = 40
    seed = 0
...
if hyper_search_mode == 2:
    if model_select == 'XgbRegModel':
        hyper2_xgb_reg_params_list = [add(fixed_params, {'num_boost_round': x}) for x in range(300, 1500, 100)]
    elif model_select == 'LgbRegModel':
        hyper2_lgb_reg_params_list = [add(fixed_params, {'n_estimators': x}) for x in range(300, 1500, 100)]
...
class MyModelTrain:
    def __init__(self, model_select='XgbRegModel'):
        self.model_name = f'{fs_version}_{scaler_select}_{label_flag}_All{model_select}'
        self.model_select = model_select
        self.strategy_name = 'Europa'
        self.version = 'v4_0_54'
    ...
    def start_train(self, param):
        ...
        train_auc, train_precision, train_recall, train_rmse, train_ic, y_train_pred_label = self.model_eval(self.y_train, y_train_pred, score_threshold=self.best_threshold, stage='train')
        print(f'train区间结果：{round(train_auc, 4)}, {round(train_precision, 4)}, {round(train_recall, 4)}, {round(train_rmse, 4)}')
        ...
        if seed == 0 and not period.endswith('roll'):
            ...
            self.best_threshold = pd.read_json(f'/data/user/015614/Zeus/pred/Europa/v4_0_54/{self.model_name}/model/{period.replace("_roll", "")}/seed_0/_score_threshold.json').iloc[0, 0]
        ...
        return test_auc, ((test_auc, test_precision, test_recall, test_rmse, test_ic),
                         (fit_auc, fit_precision, fit_recall, fit_rmse, fit_ic),
                          (train_auc, train_precision, train_recall, train_rmse, train_ic))
    ...
    def wrapper(params):
        for param in tqdm(params):
            ...
            if in_time:
                ...
                stats_df['训练集prec'] = float(eval_tuple[2][1])
