...
from Zeus.Europa.v4_0_8.backtest.sim_backtest.SimBackTest import SimBackTest

import pandas as pd
import numpy as np
from Zeus.Europa.v4_0_8.path_conf import *
from Zeus.Europa.v4_0_8.hyper_param_space import *
import xgboost as xgb
import lightgbm as lgb
from sklearn.metrics import roc_curve, auc
from LucienUtil.FileUtil import FileUtil
from Zeus.Europa.v4_0_8.my_logger import MyLogger
import warnings
import random
import os
...
class MyModelTrain:
    ...
    def select_factor(self):
        if fs_version in ['fsv8', 'fsv10', 'fsv11']:
            ...
            factor_list = pd.read_excel(eval(f'fsrs_imptc_{label_flag}_{period.replace("_roll", "")}_fpath'), index_col=0)
    ...
    def start_train(self, param):
        ...
        if seed == 0 and not period.endswith('roll'):
            ...
            self.best_threshold = pd.read_json(f'/data/user/015614/Zeus/pred/Europa/v4_0_8/{self.model_name}/model/{period.replace("_roll", "")}/seed_0/_score_threshold.json').iloc[0, 0]
...
if hyper_search_mode:
    ...
    sbt = SimBackTest(pred_fpath_list=[y_test_pred_df],
                      ...
                      save_flag=False,
                      multi_attend=True)
    stats_df, _, _ = sbt.start_backtest(multi=False)
    stats_df['累计扣费总收益'] /= 1e8
    stats_df['最大回撤'] /= 1e8
    stats_df = stats_df.map(lambda x: round(x, 2))
    stats_df['基础样本数量'] = int(stats_df['基础样本数量'])
    print(stats_df[['累计扣费总收益', '最大回撤', '收益风险比', '收益夏普比率', '预测值与标签RankIC', '基础样本数量', '扣费后收益率胜率']].to_dict())
    ...
    if inst.model_select in ['XgbRegModel', 'LgbRegModel']:
        ...
        FileUtil.save_df2csv(y_test_pred_df, model_path, inst.test_pred_fname)
        FileUtil.save_df2csv(y_fit_pred_df, model_path, inst.fit_pred_fname)