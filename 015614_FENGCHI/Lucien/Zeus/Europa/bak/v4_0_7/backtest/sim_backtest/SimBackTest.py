...
import pandas as pd
import numpy as np
from Zeus.Europa.v4_0_7.strat_conf import strategy_name, strategy_version
from Zeus.Europa.v4_0_7.path_conf import profit_data_fpath, bt_out_path, date_config
import math
import time
...
class SimBackTest:
    ...
    def single_backtest(self, pred_fpath, fit_fpath):
        if type(pred_fpath) is str:
            test_df = pd.read_csv(pred_fpath, index_col=0)
        else:
            test_df = pred_fpath

        if type(fit_fpath) is str:
    ...
    def start_backtest(self, multi=True):
        if multi:
            ...
            for idx in range(len(self.pred_fpath_list)):
                ...
                self.single_backtest(pred_fpath, fit_fpath)

if __name__ == '__main__':
    sbt = SimBackTest(pred_fpath_list=['/data/user/015614/Zeus/pred/Europa/v4_0_7/fsrs_pct_AllXgbRegModel/model/period3/seed_0/20201201~20210531.csv'],
                      fit_fpath_list=['/data/user/015614/Zeus/pred/Europa/v4_0_7/fsrs_pct_AllXgbRegModel/model/period3/seed_0/20210501~20210531.csv'],
                      date_dict=date_config['period3'],
