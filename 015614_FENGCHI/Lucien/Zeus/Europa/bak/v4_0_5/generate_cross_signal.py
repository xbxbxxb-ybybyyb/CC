...
for period in ['period1', 'period2', 'period3']:
    for pred_type in ['test', 'fit']:
        ...
        valid_path_list = [f'/data/user/015614/Zeus/pred/Europa/v4_0_5/{x}/{out_begin}~{out_end}.csv' for x in sel_model_names]
        all_valid_path_list = [f'/data/user/015614/Zeus/pred/Europa/v4_0_5/{x}/{out_begin}~{out_end}.csv' for x in sel_all_model_names]
        ...
        for idx, valid_path in enumerate(valid_path_list):
            ...
            cross_signal.to_csv('/data/user/015614/Zeus/pred/Europa/v4_0_5/' + f'{valid_path.split("/")[-2]}Cross/' + os.path.basename(valid_path))