...
import pandas as pd
import numpy as np
from Zeus.Europa.v2_1_1.path_conf import *
from Zeus.Europa.v2_1_1.hyper_param_space import hyper_xgb_reg_params
...
from Zeus.Europa.v2_1_1.my_logger import MyLogger
import warnings
import os
...
param = {'alpha': 0.6, 'booster': 'gbtree', 'colsample_bytree': 0.6, 'eta': 0.005, 'eval_metric': 'rmse', 'evals_result': ('auc', 'rmse'), 'gamma': 0, 'lambda': 0.2, 'max_depth': 8.0, 'min_child_weight': 6.0, 'n_jobs': -1, 'num_boost_round': 1600.0, 'objective': 'reg:linear', 'random_state': 2022, 'scale_pos_weight': 1.0, 'silent': True, 'subsample': 0.9, 'tree_method': 'gpu_hist'}
best_threshold = 0.0
...
class XgbRegModel:
    def __init__(self, sub_version='v1'):
        self.model_name = 'XgbFSV8RegModel'
        ...
        self.search_time = 0
        self.date_dict = date_config['period1']
    ...
    # 使用month进行tscv
    def get_tscv(self, rolling_size=10, valid_size=10, min_train_size=360):
        month_list = self.train_days
    ...
    def get_train_and_test_data(self):
        ...
        self.train_days = list(sorted(set(X_train['trade_date'].tolist())))
    ...
    def train_model(self, X_train, y_train, param, X_valid=None, y_valid=None, stage='no_draw'):
        ...
        if not hyper_search_mode and stage is 'draw':
            ...
            plt.savefig(self.eval_path + f'auc_1.png', bbox_inches='tight', pad_inches=0.0)
    ...
    def model_eval(self, y_true, y_pred, score_threshold=0.0, stage='test'):
        ...
        if not hyper_search_mode and stage is not 'no_draw':
            ...
            plt.savefig(self.eval_path + f'roc_curve_1_{stage}.png', bbox_inches='tight', pad_inches=0.0)
    ...
    def rolling_train_model(self, param):
        ...
        for idx, cv_list in enumerate(rolling_cv_month_list):
            ...
            X_train_all['trade_month'] = X_train_all.index.get_level_values(0).strftime('%Y%m%d').astype(int)
            y_train_all['trade_month'] = y_train_all.index.get_level_values(0).strftime('%Y%m%d').astype(int)
            ...
            th_date_list = tradeDate.get_date_range(train_start_month, train_end_month)[-10:] # 最后10天的表现
            X_th = X_train_all.query(f'trade_month in {th_date_list}').drop('trade_month', axis=1)
            y_th = y_train_all.query(f'trade_month in {th_date_list}').drop('trade_month', axis=1)
            X_train = X_train.drop(X_th.index)
            y_train = y_train.drop(y_th.index)

            # 滚动因子筛选
            # 根据最近一段时间表现进行因子筛选
            select_num = 200
            near_samples_num = 2000
            rf = RandomForestRegressor(n_estimators=100, random_state=2023, n_jobs=-1, max_depth=3)
            rf.fit(X_train.iloc[-near_samples_num:], y_train.iloc[-near_samples_num:])
            factor_imptc_df = pd.Series(rf.feature_importances_, index=X_train.columns).sort_values(ascending=False)
            select_factor_list = factor_imptc_df.iloc[:select_num].index.tolist()
            X_train = X_train[select_factor_list]
            X_valid = X_valid[select_factor_list]
            X_th = X_th[select_factor_list]

            X_train_scaled, X_test_scaled = self.func_trans_train_test_minmaxmadscaler(X_train, X_valid)
            X_train_scaled, X_th_scaled = self.func_trans_train_test_minmaxmadscaler(X_train, X_th)
            ...
            dth = xgb.DMatrix(X_th_scaled)
            y_th_pred = self.predict(dth)

            # 根据前10日的阈值设置后面的结果
            threshold = 0.005
            label_quantile_ratio = ((y_th <= threshold).sum() / len(y_th)).values[0]
            adapt_threshold = np.quantile(y_th_pred, label_quantile_ratio)
            y_valid_signal = y_valid_pred > adapt_threshold
            print('该折正样本比例', 1 - label_quantile_ratio, '该折参与率：', y_valid_signal.sum() / len(y_valid_signal))

            y_pred_list.append(pd.DataFrame(y_valid_pred, index=y_valid.index, columns=['pred_Reg']))
            y_signal_list.append(pd.DataFrame(y_valid_signal, index=y_valid.index, columns=['prediction']))   # TODO：加入index
            ...
            print(f'第{idx + 1}折结果({valid_start_month}, {valid_end_month})：{round(adapt_threshold, 4)}, {round(eval_auc, 4)}, {round(eval_precision, 4)}, {round(eval_recall, 4)}, {round(eval_rmse, 4)}, {round(eval_ic, 4)}')
        ...
        y_signal = pd.concat(y_signal_list, axis=0)
    ...
    def start_train(self, param):
        ...
        valid_auc, valid_auc_std, valid_precision, valid_recall, valid_rmse, valid_ic, y_pred, y_signal = self.rolling_train_model(param=param_copy)

        return valid_auc, ((valid_auc, valid_auc_std, valid_precision, valid_recall, valid_rmse, valid_ic), ), y_pred, y_signal

inst = XgbRegModel(sub_version='v1')
...
valid_auc, eval_tuple, y_pred, y_signal = inst.start_train(param)

all_pred = pd.concat([y_pred, y_signal], axis=1)
all_pred['stockID'] = all_pred.index.get_level_values(1)
all_pred['datelist'] = all_pred.index.get_level_values(0).strftime('%Y%m%d')
all_pred['Indexs'] = all_pred['stockID'] + ' ' + all_pred['datelist']
all_pred = all_pred.set_index('Indexs', drop=True)
FileUtil.save_df2csv(all_pred, inst.pred_out_path, 'all_pred.csv')