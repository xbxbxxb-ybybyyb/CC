...
from Zeus.Europa.v2_0_11.hyper_param_space import hyper_cat_reg_params
from Zeus.BTLite.BTLite2 import BTLite2
import lightgbm as lgb
from catboost import CatBoostRegressor
from sklearn.metrics import roc_curve, auc
from LucienUtil.FileUtil import FileUtil
from Zeus.Europa.v2_0_11.my_logger import MyLogger
import warnings
import os
import json
import joblib
...
if PERIOD[-1] is '1':
    param = {'boosting_type': 'gbdt', 'class_weight': None, 'colsample_bytree': 0.7, 'device': 'gpu', 'gpu_device_id': 0, 'gpu_platform_id': 1, 'gpu_use_dp': True, 'learning_rate': 0.005, 'max_depth': 8.0, 'metric': {'binary_logloss', 'auc'}, 'min_child_samples': 20.0, 'min_child_weight': 0.001, 'min_split_gain': 0.0, 'n_estimators': 3600.0, 'n_jobs': -1, 'num_leaves': 24.0, 'random_state': 2022, 'reg_alpha': 0.9, 'reg_lambda': 0.1, 'subsample': 1.0, 'subsample_freq': 0, 'verbosity': -1}
    best_threshold = -0.009802
elif PERIOD[-1] is '2':
    param = {'boosting_type': 'gbdt', 'class_weight': None, 'colsample_bytree': 0.5, 'device': 'gpu', 'gpu_device_id': 0, 'gpu_platform_id': 1, 'gpu_use_dp': True, 'learning_rate': 0.005, 'max_depth': 9.0, 'metric': {'auc', 'binary_logloss'}, 'min_child_samples': 14.0, 'min_child_weight': 0.001, 'min_split_gain': 0.0, 'n_estimators': 3200.0, 'n_jobs': -1, 'num_leaves': 36.0, 'random_state': 2022, 'reg_alpha': 0.9, 'reg_lambda': 0.2, 'subsample': 0.8, 'subsample_freq': 0, 'verbosity': -1}
    best_threshold = -0.006207
elif PERIOD[-1] is '3':
    param = {'boosting_type': 'gbdt', 'class_weight': None, 'colsample_bytree': 0.5, 'device': 'gpu', 'gpu_device_id': 0, 'gpu_platform_id': 1, 'gpu_use_dp': True, 'learning_rate': 0.005, 'max_depth': 9.0, 'metric': {'auc', 'binary_logloss'}, 'min_child_samples': 12.0, 'min_child_weight': 0.001, 'min_split_gain': 0.0, 'n_estimators': 3400.0, 'n_jobs': -1, 'num_leaves': 40.0, 'random_state': 2022, 'reg_alpha': 0.8, 'reg_lambda': 0.0, 'subsample': 1.0, 'subsample_freq': 0, 'verbosity': -1}
    best_threshold = -0.004335
...
class CatRegModel:
    ...
    def train_model(self, X_train, y_train, param, X_valid=None, y_valid=None, stage='no_draw'):
        ...
        model.fit(X_train.values, y_train.values.ravel(), verbose=False)
...
inst = CatRegModel(sub_version=SUB_VERSION)
