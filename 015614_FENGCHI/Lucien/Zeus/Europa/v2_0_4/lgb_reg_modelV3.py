...
import pandas as pd
import numpy as np
from Zeus.Europa.v2_0_1.path_conf import *  # 和5个情绪因子的进行对比
from Zeus.Europa.v2_0_4.hyper_param_space import hyper_lgb_reg_params
from Zeus.BTLite.BTLite2 import BTLite2
import lightgbm as lgb
from sklearn.metrics import roc_curve, auc
from LucienUtil.FileUtil import FileUtil
from Zeus.Europa.v2_0_4.my_logger import MyLogger
import warnings
import os
import json
import joblib
...
if PERIOD[-1] is '1':
    ...
    best_threshold = -0.005751
elif PERIOD[-1] is '3':
    param = {'boosting_type': 'gbdt', 'class_weight': None, 'colsample_bytree': 0.5, 'device': 'gpu', 'gpu_device_id': 0, 'gpu_platform_id': 1, 'gpu_use_dp': True, 'learning_rate': 0.005, 'max_depth': 6.0, 'metric': {'binary_logloss', 'auc'}, 'min_child_samples': 4.0, 'min_child_weight': 0.001, 'min_split_gain': 0.0, 'n_estimators': 2400.0, 'n_jobs': -1, 'num_leaves': 36.0, 'random_state': 2022, 'reg_alpha': 1.0, 'reg_lambda': 0.6, 'subsample': 0.9, 'subsample_freq': 0, 'verbosity': -1}
    best_threshold = -0.001497
