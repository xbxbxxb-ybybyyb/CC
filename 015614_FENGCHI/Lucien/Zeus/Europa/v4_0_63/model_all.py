...
from Zeus.Europa.v4_0_63.backtest.sim_backtest.SimBackTest import SimBackTest

import pandas as pd
import numpy as np
from tqdm import tqdm
from joblib import Parallel, delayed
from Zeus.Europa.v4_0_63.path_conf import *
from Zeus.Europa.v4_0_63.hyper_param_space import *
import xgboost as xgb
import lightgbm as lgb
from Zeus.Europa.v4_0_63.models.MlpRegModel import MlpRegModel
from sklearn.metrics import roc_curve, auc
from LucienUtil.FileUtil import FileUtil
from Zeus.Europa.v4_0_63.my_logger import MyLogger
import warnings
import random
import os
...
if len(sys.argv) > 1:
    ...
    period = 'period1'
    fs_version = 'allfactor' # fsv8\fsv10\fsv11\rffs
    label_flag = 'pct'
...
class MyModelTrain:
    ...
    def select_factor(self):
        ...
        elif fs_version == 'allfactor':
            factor_list = pd.read_excel(eval(f'fsrs_imptc_{label_flag}_{period.replace("_roll", "")}_fpath'))['factor_name'].tolist()
        elif fs_version == '3kindfactor':
            factor_list1 = pd.read_excel(eval(f'xgb_imptc_{label_flag}_fsv8_{period.replace("_roll", "")}_fpath'), index_col=0).query('corr_selected==1')['factor_name'].tolist()
            factor_list2 = pd.read_excel(eval(f'xgb_imptc_{label_flag}_fsv10_{period.replace("_roll", "")}_fpath'), index_col=0).query('corr_selected==1')['factor_name'].tolist()
            factor_list3 = pd.read_excel(eval(f'xgb_imptc_{label_flag}_fsv11_{period.replace("_roll", "")}_fpath'), index_col=0).query('corr_selected==1')['factor_name'].tolist()
            factor_list = list(sorted(list(set(factor_list1 + factor_list2 + factor_list3))))
        else:
            try:
    ...
    def start_train(self, param):
        ...
        if seed == 0 and not period.endswith('roll'):
            ...
            self.best_threshold = pd.read_json(f'/data/user/015614/Zeus/pred/Europa/v4_0_63/{self.model_name}/model/{period.replace("_roll", "")}/seed_0/_score_threshold.json').iloc[0, 0]
