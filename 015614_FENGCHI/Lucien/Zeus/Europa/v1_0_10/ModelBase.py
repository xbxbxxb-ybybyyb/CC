...
from Zeus.Europa.v1_0_10.DataPrepare import DataPrepare
...
from dataApi.tradeDate import trade_months2
random.seed(2022)
...
class ModelBase:
    ...
    def get_dateset(self):
        ...
        X = samples.drop(samples.filter(regex='label*').columns.tolist(), axis=1)
        ...
        X = X.query(f'trade_date >= {self.train_start_date} & trade_date <= {self.test_end_date}')
        y = y.query(f'trade_date >= {self.train_start_date} & trade_date <= {self.test_end_date}')

        X = self.ts_standard(X)
    ...
    def ts_standard(self, X):
        # 传进来的X带有trade_date
        X_copy = X.copy()
        X_copy['trade_month'] = X_copy['trade_date'] // 100
        rolling_date_list = trade_months2
        normalized = np.array([])
        rolling_date_list = list(filter(lambda x: self.train_start_date // 100 <= x <= self.test_end_date // 100, rolling_date_list))
        rolling_date_list.extend([trade_months2[trade_months2.index(rolling_date_list[-1]) + 1]])
        for idx, start_month in enumerate(rolling_date_list[:-2]):
            end_month = rolling_date_list[idx + 1]
            next_start_month = end_month
            next_end_month = rolling_date_list[idx + 2]
            scaler = StandardScaler()
            scaler = scaler.fit(X_copy.query(f'trade_month >= {start_month} & trade_month < {end_month}').values)
            if idx == 0:
                normalized = X_copy.query('trade_month >= @start_month & trade_month < @end_month').values

            tmp_normalized = scaler.transform(X_copy.query(f'trade_month >= {next_start_month} & trade_month < {next_end_month}'))
            normalized = np.concatenate((normalized, tmp_normalized), axis=0)
        X2 = pd.DataFrame(normalized, index=X_copy.index, columns=X_copy.columns)
        X2['trade_month'] = X_copy['trade_month'].tolist()
        X2['trade_date'] = X_copy['trade_date'].tolist()
        X2 = X2.drop('trade_month', axis=1)
        return X2
