...
from Zeus.Europa.v1_0_11.Tool import multiprocess
from Zeus.Europa.v1_0_11.hyper_param_space import hyper_lr_reg_params, multi_lr_reg_params
from Zeus.Europa.v1_0_11.hyper_param_space import model_params
from Zeus.Europa.v1_0_11.models.lr_reg_model import LrRegModel
from Zeus.Europa.v1_0_11.my_logger import MyLogger
from Zeus.Europa.v1_0_11.path_conf import *
...
test_hyper_param_fpath = '2022-11-08 11：21：42_Europa_v1_0_9_lgb_reg_modeltest.xlsx'
...
def tuning_objective(param):
    ...
    mf_inst = LrRegModel(model_name=model_name,
    ...
    # 如果是在寻优，进行下面的步骤
    if hyper_search_mode or multi_grid_search_mode or use_test_param_mode:
        if test_cheat_mode:
            eval_inst = EvalLaunch(date_config=date_config,
                                   strategy_name=strategy_name,
                                   sel_model_names=['LrRegModel'],
            ...
            model_inmingan_copy = model_inmingan_copy.query('0.2 < 实际参与率 < 0.5')
            ...
            model_mingan_copy = model_mingan_copy.query('0.2 < 实际参与率 < 0.5')
            ...
            winrate = round(model_eval.loc['扣费后收益率胜率'][0], 4) if model_eval.shape[0] > 0 else 0
            ...
            model_eval, model_inmingan, model_mingan, merge_attend_metric = pd.DataFrame(), pd.DataFrame(), pd.DataFrame(), pd.DataFrame()
            eval_inst = None
            watch_scores = dict()

        if fit_cheat_mode:
            date_config = dict(train_start_date=20160101,
                               train_end_date=20200930,
                               valid_start_date=20201001,
                               valid_end_date=20210630,
            ...
            model_inmingan_copy = model_inmingan.copy()
            model_inmingan_copy['fit_indicator'] = model_inmingan_copy['收益风险比'] * model_inmingan_copy['扣费收益率胜率'] * model_inmingan_copy['实际参与率']
            model_inmingan_copy = model_inmingan_copy.query('0.2 < 实际参与率 < 0.5')  # 控制参与率在0.1-0.3之间
            model_inmingan_copy = model_inmingan_copy.sort_values(['fit_indicator'], ascending=False).iloc[0]
            in_adapt_score_threshold = model_inmingan_copy.name
            model_mingan_copy_ = model_mingan.loc[in_adapt_score_threshold]
            ...
            model_mingan_copy = model_mingan.copy()
            model_mingan_copy['fit_indicator'] = model_mingan_copy['收益风险比'] * model_mingan_copy['扣费收益率胜率'] * model_mingan_copy['实际参与率']
            model_mingan_copy = model_mingan_copy.query('0.2 < 实际参与率 < 0.5')    # 控制参与率在0.1-0.3之间
            model_mingan_copy = model_mingan_copy.sort_values(['fit_indicator'], ascending=False).iloc[0]
        ...
        if hyper_search_mode:   # 如果是hyperopt寻优，那么返回目标值
            ...
            search_time += 1
            return -ic if test_cheat_mode is True else -_ic
...
if __name__ == '__main__':
    if hyper_search_mode:
        ...
        if hyper_remark:
            param_abs_fpath = '/data/user/015614/Zeus/logs/' + f'{strategy_name}/{version}/{model_name}_{hyper_remark}/{test_hyper_param_fpath}'
        else:
            param_abs_fpath = '/data/user/015614/Zeus/logs/' + f'{strategy_name}/{version}/{model_name}/{test_hyper_param_fpath}'
        if not os.path.exists(param_abs_fpath):
            print('test区间寻优参数表格不存在')
        test_params = pd.read_excel(param_abs_fpath, index_col=0).sort_index()
        test_params_list = test_params['整体参数'].tolist()
        for param in test_params_list:
            param_dict = eval(param)
            tuning_objective(param_dict)
    elif multi_grid_search_mode:
        ...
        multi_params_list = list(multi_lr_reg_params.values())
        params = list(product(*multi_params_list))
        print(f'超参数的组合个数为{len(params)}个')
        grid_params = [dict(zip(list(multi_lr_reg_params.keys()), _param)) for _param in params]
