...
import pandas as pd
import numpy as np
from Zeus.Europa.v2_0_19.path_conf import *
from Zeus.Europa.v2_0_19.hyper_param_space import hyper_lgb_reg_params
...
from Zeus.Europa.v2_0_19.my_logger import MyLogger
...
# emo_flag = '_emotion' # _emotion _noEmotion
...
SUB_VERSION = f'v{PERIOD[-1]}'
emo_flag = sys.argv[3]
print(PERIOD, fs_version, emo_flag)
...
class LgbRegModel:
    def __init__(self, sub_version='v1'):
        self.model_name = f'{fs_version}{emo_flag}_LgbRegModel'
        self.strategy_name = 'Europa'
        self.version = 'v2_0_19'
        ...
        if not hyper_search_mode:
            if PERIOD[-1] == '1':
                ...
                self.param = pd.read_excel(self.log_path + f'{self.strategy_name}_{self.version}_{self.model_name}_{self.sub_version}_.xlsx', index_col=0).iloc[0]['整体参数']
                self.best_threshold = 0.005325
                self.best_threshold = 0.004333
            elif PERIOD[-1] == '6':
                self.param = pd.read_excel(self.log_path + f'{self.strategy_name}_{self.version}_{self.model_name}_{self.sub_version}_.xlsx', index_col=0).iloc[0]['整体参数']
                self.best_threshold = 0.005325
    ...
    def filter_factor(self, xgb_imptc_fpath=None, factor_score_fpath=factor_score_fpath):
        ...
        if fs_version == 'rffs':
            factor_list = pd.read_pickle(f'/data/user/015614/Zeus/factor_select/Europa/v2_0_19/rffs{emo_flag}_{PERIOD}.pkl')
        elif fs_version == 'fsrs':
            factor_list = pd.read_excel(eval(f'fsrs_imptc{emo_flag}_{PERIOD}_fpath'), index_col=0)
    ...
    def get_train_and_test_data(self):
        ...
        if fs_version in ['fsv8', 'fsv10', 'fsv11']:
            filtered_factor = self.filter_factor(xgb_imptc_fpath=eval(f'xgb_imptc{emo_flag}_{fs_version}_{PERIOD}_fpath'))
        else:
            filtered_factor = self.filter_factor(xgb_imptc_fpath=eval(f'xgb_imptc{emo_flag}_fsv8_{PERIOD}_fpath'))
...
# btl = BTLite2(strategy_name='EuropaS1',
