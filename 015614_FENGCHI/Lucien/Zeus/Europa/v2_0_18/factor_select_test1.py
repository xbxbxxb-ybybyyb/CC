...
class FactorSelect:
    ...
    def get_dataset(self, path):
        ...
        # lowcost_factor_list = pd.read_excel('/data/group/800463/sunss/europa/20230317/europa策略低耗时列表.xlsx')['factor_name'].tolist()
        # t1_factor_list = pd.read_excel(factor_score_fpath).query('t == "T-1"')['factor_name'].tolist()
        # lowcost_factor_list.extend(t1_factor_list)
        # factor_list = list(set(lowcost_factor_list).intersection(set(X.columns.tolist())))

        # 20230411更新：自己筛选低耗时因子也必须在打分文件里选
        lowcost_factor_list = pd.read_excel(factor_score_fpath).query('lowCost==1')['factor_name'].tolist()
        factor_list = list(set(lowcost_factor_list).intersection(set(X.columns.tolist())))
        X = X[factor_list]
    ...
    def kcv_factor_imptc_eval(self):
        ...
        factor_imptc_df = factor_imptc_df.sort_index()
        for idx, (train_month_arr, _) in enumerate(gap_kfold.split(self.train_months)):
            ...
            # factor_imptc_df.loc[:, idx] = list(self.model.get_score(importance_type='gain').values())
            factor_imptc_df.loc[:, idx] = list(self.model.get_score(importance_type='weight').values())
...
if __name__ == '__main__':
    ...
    def launch_1period(period):
        ...
        # 中间结果
        FileUtil.save_df2pkl(factor_imptc_df, factor_select_path + f'{FS.strategy_name}/{FS.version}/', f'tmp_rf_cv_score_{period}_test1_default.pkl')
        ...
        FileUtil.save_df2pkl(stats, factor_select_path + f'{FS.strategy_name}/{FS.version}/', f'tmp_rf_cv_score_{period}_test1_default.pkl')
        selected_factor_list = stats.query(f'mean >= 280').index.tolist()
        FileUtil.save_list2pkl(selected_factor_list, factor_select_path + f'{FS.strategy_name}/{FS.version}/', f'rf_fs_list_{period}_test1_default.pkl')
        # selected_factor_list = stats.query(f'mean >= 165').index.tolist()
        # FileUtil.save_list2pkl(selected_factor_list, factor_select_path + f'{FS.strategy_name}/{FS.version}/', f'rf_fs_list_{period}_test1_165.pkl')
