...
from Zeus.Europa.v2_0_18.hyper_param_space import *
...
class MyModelTrain:
    def __init__(self, model_select='XgbRegModel'):
        self.model_name = f'{fs_version}{label_flag}_NoEmotion{model_select}'
        ...
        if not hyper_search_mode:
            if os.path.exists(self.log_path + f'{self.strategy_name}_{self.version}_{self.model_name}_{period}.xlsx'):
                ...
                self.param = "{'alpha': 0.6, 'booster': 'gbtree', 'colsample_bytree': 0.6, 'eta': 0.005, 'eval_metric': 'rmse', 'evals_result': ('auc', 'rmse'), 'gamma': 0, 'lambda': 0.2, 'max_depth': 8, 'min_child_weight': 6.0, 'n_jobs': -1, 'num_boost_round': 1600.0, 'objective': 'reg:linear', 'random_state': 2022, 'scale_pos_weight': 1.0, 'silent': True, 'subsample': 0.9, 'tree_method': 'gpu_hist'}" # 固定参数
        else:
            self.param = "{'alpha': 0.6, 'booster': 'gbtree', 'colsample_bytree': 0.6, 'eta': 0.005, 'eval_metric': 'rmse', 'evals_result': ('auc', 'rmse'), 'gamma': 0, 'lambda': 0.2, 'max_depth': 8, 'min_child_weight': 6.0, 'n_jobs': -1, 'num_boost_round': 1600.0, 'objective': 'reg:linear', 'random_state': 2022, 'scale_pos_weight': 1.0, 'silent': True, 'subsample': 0.9, 'tree_method': 'gpu_hist'}" # 固定参数
        ...
        if 'pct_after' in label_flag:
            self.label = 'label_pct_after_graded'
        else:
        ...
        self.test_pred_fname = f'{self.date_dict["test_start_date"]}~{self.date_dict["test_end_date"]}_{self.model_name}.csv'
        self.fit_pred_fname = f'{self.date_dict["fit_start_date"]}~{self.date_dict["fit_end_date"]}_{self.model_name}.csv'
    ...
    def select_factor(self, xgb_imptc_fpath=None):
        if fs_version == 'rffs':
            ...
            emotion_factors = factor_score.query('factor_owner == "emotion" or factor_owner == "emotion_time"')['factor_name'].tolist()
            factor_list = list(set(factor_list).difference(set(emotion_factors)))
        ...
        elif fs_version in ['fsv8', 'fsv10', 'fsv11']:
            xgb_imptc_fpath = eval(f'xgb_imptc{label_flag}_{fs_version}_{period}_fpath')
