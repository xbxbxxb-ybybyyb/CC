...
class modelEval_Tool:
    ...
    def select_profit(self):
        if self.strategy not in ['SaturnS0&SaturnS1','SaturnS1&SaturnS3']:
            if self.strategy == 'JupiterN':
                ...
                self.cost_pct = 0.002   # TODO：为了对比，设置为0.003，后面降为0.002，已降低
                self.attend_min = 10
                self.attend_max = 61
                self.group_ratio = 0.2
                self.extreme_thres = 0.2
            elif self.strategy == 'JupiterNSell':
                self.profit_path = '/data/group/800463/project/project1_prod/LabelProfit_fixvol/LabelProfit_zt_pct_931_0.10_800_190_SH450_SZ100.h5'
                self.cost_pct = 0.003
                self.attend_min = 10
                self.attend_max = 61
                self.group_ratio = 0.2
                ...
                self.cost_pct = 0.002
                self.attend_min = 10#5#20
                self.attend_max = 41#81#41
                self.group_ratio = 0.2
                self.extreme_thres = 0.2
    ...
    def select_label(self):
        ...
        if self.strategy == 'SaturnS0&SaturnS1':
            ...
            if self.strategy == 'JupiterN':
                ...
                #self.label_path = '/data/user/018107/share_file/for_wj/%s/factor_df_all_20160101_20220930.pkl' % 'jupiter'
                self.label_rev = 'label_TN_o2ul'
                self.label_o2o10 = 'label_pattern'
            elif self.strategy == 'JupiterNSell':
                self.label_path = '/data/group/800463/sunss/jupiter_931o2ul/20230201/factor_df_all_20160101_20201231.pkl'
                ...
                #self.label_path = '/data/user/018107/share_file/for_wj/%s/factor_df_all_20160101_20220930.pkl'%'europa'
                self.label_rev = 'label_TN_o2ul'
                self.label_o2o10 = 'label_pattern'
            elif self.strategy == 'SaturnS0':
            ...
            if 'label_T_close_is_zt' not in label_data.columns.tolist() and self.strategy in ['JupiterN','Europa','jupiter003','JupiterNSell']:
                label_data['label_T_close_is_zt'] = label_data['label_T_is_zt']
            if 'label_Next_close_is_zt' not in label_data.columns.tolist()  and self.strategy in ['JupiterN','Europa','jupiter003','JupiterNSell']:
    ...
    def generate_test_report(self,pred_out, pred_in,scene_flag,probacol):
        ...
        if self.strategy not in ['JupiterN', 'jupiter_sell','Europa','jupiter003','JupiterNSell']:
        ...
        if (self.indi_str.find('stack')<0 and self.indi_str.find('vote')<0 and tradedate_length <len(hfactor.tradingday(self.begindate,self.enddate)) and self.vote_type==False):
    ...
    def fun_statistic_by_day(self,start_date, end_date, by_samples,pred_out):
        ...
        by_day['滚动5日基础收益率(扣除成本)'] = by_day['5日基础收益率'] /by_day['5日基础数量']
        ...
        by_day['滚动5日收益率(扣除成本)'] = (by_sample.groupby('datelist').sum()['收益率(扣除成本)'].reindex(by_day.index).fillna(0)).rolling(5,5).sum() / (by_sample.groupby('datelist').count()['stockID'].reindex(by_day.index).fillna(0)).rolling(5,5).sum()
    ...
    def cal_model_mingan(self,sel_raw_data,factor,step=1):
        ...
        if self.indi_str.find('scene')<0:
            ...
            # test_sell_df = pd.read_csv('/data/user/015614/Zeus/pred/ProjectSell/v1_0_1/LgbRegModel/20191001~20200331_LgbRegModel_v1.csv', index_col=0)
            test_sell_df = pd.read_csv('/data/user/015614/Zeus/pred/ProjectSell/v1_0_1/LgbRegModel/20200401~20201231_LgbRegModel_v1.csv', index_col=0)
            from dataApi.tradeDate import get_pre_trade_date
            test_sell_df['last_dt'] = test_sell_df['datelist'].apply(lambda x: get_pre_trade_date(x, 1))
            test_sell_df['Indexs'] = test_sell_df[['stockID', 'last_dt']].apply(lambda x: x['stockID'] + ' ' + str(x['last_dt']), axis=1)
            test_sell_df = test_sell_df.set_index(['Indexs'])
            test_sell_df = test_sell_df.rename(columns={'prediction': 'sell_prediction'})
            ...
            for group_indi_str in plot_data.index.tolist():
                ...
                if self.strategy == 'JupiterNSell':
                    group_df_sell = pd.merge(group_df, test_sell_df[['sell_prediction', 'label_diff_pct']], on='Indexs')
                    gain_profit = (group_df_sell.query('sell_prediction == 0')['label_diff_pct'] * group_df_sell.query('sell_prediction == 0')['label_buy_amt']).sum()
                    plot_data.loc[group_indi_str, '卖出收益增益'] = -gain_profit
                elif self.strategy == 'JupiterN':
                    group_df_sell = pd.merge(group_df, test_sell_df[['sell_prediction', 'label_diff_pct']], on='Indexs')
                    gain_profit = (group_df_sell.query('sell_prediction == 1')['label_diff_pct'] * group_df_sell.query('sell_prediction == 1')['label_buy_amt']).sum()
                    plot_data.loc[group_indi_str, '卖出收益增益'] = gain_profit
    ...
    def combine_res_inout(self,pred_sample_data,sample_data,statistic_without_extreme_value,statistic_by_sample,statistic_by_day,pred_in,flag_scene,proba_col):
        ...
        if 'label_zuhe' not in pred_sample_data.columns.tolist() and self.strategy in ['JupiterN','Europa','jupiter003','JupiterNSell']:
        ...
        if 'label_pat_zuhe' not in pred_sample_data.columns.tolist() and self.strategy in ['JupiterN','Europa','jupiter003','JupiterNSell']:
        ...
        if self.strategy in ['JupiterN','Europa','jupiter003','JupiterNSell']:
        ...
        totalResDf.loc['滚动5日收益率增强均值'] = float((statistic_by_day['滚动5日收益率(扣除成本)']-statistic_by_day['滚动5日基础收益率(扣除成本)']).mean())
