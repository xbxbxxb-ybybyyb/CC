...
import pandas as pd
import numpy as np
from Zeus.JupiterNSell.v1_0_1.path_conf import *
from Zeus.JupiterNSell.v1_0_1.hyper_param_space import hyper_lgb_clf_params
from Zeus.BTLite.BTLite import BTLite
from lightgbm import LGBMClassifier
from LucienUtil.FileUtil import FileUtil
from Zeus.JupiterNSell.v1_0_1.my_logger import MyLogger
import warnings
import os
...
param_test = {}
param_fit = {}
if PERIOD[-1] == '1' and SUB_VERSION[-1] == '0':
...
elif PERIOD[-1] == '1' and SUB_VERSION[-1] == '1':
...
elif PERIOD[-1] == '1' and SUB_VERSION[-1] == '2':
...
hyper_params = hyper_lgb_clf_params

class LgbClfModel:
    def __init__(self, mode='test', sub_version='v1'):
        self.model_name = 'LgbClfModel'
        self.strategy_name = 'JupiterNSell'
        self.version = 'v1_0_1'
        ...
        self.model = None
        self.scaler = None
        self.label = 'label_TN_931o2ul_mix'
    ...
    def predict(self, X_other):
        _y_pred = self.model.predict_proba(X_other)[slice(None), 1]
        return _y_pred
...
inst_test = LgbClfModel(mode='test', sub_version=SUB_VERSION)
inst_fit = LgbClfModel(mode='fit', sub_version=SUB_VERSION)
...
def tuning_params(param):
    ...
    adapt_profit_sharpe_ratio = round(model_test_mingan_copy['收益夏普比率'], 4) if model_test_mingan_copy.shape[0] > 0 else 0
    ...
    adapt_profit_sharpe_ratio_ = round(model_test_mingan_copy_['收益夏普比率'], 4) if model_test_mingan_copy_.shape[0] > 0 else 0
    ...
    watch_scores = {
        ...
        '自适应收益夏普比率': adapt_profit_sharpe_ratio,
        ...
        'R自适应收益夏普比率': adapt_profit_sharpe_ratio_,
    ...
    if fit_cheat_mode:
        ...
        _adapt_profit_sharpe_ratio = round(model_test_mingan_copy['收益夏普比率'], 4) if model_test_mingan_copy.shape[0] > 0 else 0
        ...
        _adapt_profit_sharpe_ratio_ = round(model_test_mingan_copy_['收益夏普比率'], 4) if model_test_mingan_copy_.shape[0] > 0 else 0
        ...
        watch_scores.update({
            ...
            '自适应收益夏普比率2': _adapt_profit_sharpe_ratio,
            ...
            'R自适应收益夏普比率2': _adapt_profit_sharpe_ratio_,
...
if hyper_search_mode:
    ...
    joblib.dump(inst_fit.model.booster_, junk_path + 'period5_LgbClfModel.pkl')
