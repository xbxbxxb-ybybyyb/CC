...
# from Zeus.Ceres.v1_0_11.backtest.SimBackTest import SimBackTest
...
import importlib
from Zeus.Ceres.v1_0_11.backtest.SimBackTest import SimBackTest
from Zeus.Ceres.v1_0_11.config.strat_conf import *
from Zeus.Ceres.v1_0_11.scripts.hyper_param_space import *
from Zeus.Ceres.v1_0_11.label_generate.fetch_label import fetch_label
import xgboost as xgb
import lightgbm as lgb
from Zeus.Ceres.v1_0_11.models.MlpRegModel import MlpRegModel
from sklearn.metrics import roc_curve, auc
from LucienUtil.FileUtil import FileUtil
from Zeus.Ceres.v1_0_11.scripts.my_logger import MyLogger
import warnings
import random
import os
...
module_name = f'Zeus.Ceres.v1_0_11.config.path_conf'
...
if hyper_search_mode == 2:
    tree_num_list = list(range(300, 1000, 200)) + [1000, 1200, 1500, 1800, 2100, 2700, 3000, 4000, 5000, 7500, 10000]
    if model_select == 'XgbRegModel':
...
class MyModelTrain:
    ...
    def start_train(self, param):
        ...
        if seed == 0 and not period.endswith('roll'):
            ...
            self.best_threshold = pd.read_json(f'/data/user/015614/Zeus/pred/Ceres/v1_0_11/{config_flag}/{self.model_name}/model/{period.replace("_roll", "")}/seed_0/_score_threshold.json').iloc[0, 0]
...
def tuning_params(hyper_params_list, in_time=True, multi=True):
    def wrapper(params):
        for param in tqdm(params):
            ...
            if in_time:
                sbt = SimBackTest(pred_fpath_list=[inst.hyper_pred_out_path + inst.test_pred_fname],
                                  fit_fpath_list=[inst.hyper_pred_out_path + inst.fit_pred_fname],
            ...
            # }
                ...
                if my_logger:
                    ...
                    print(f'{config_flag} {model_select} num_boost_round: {param["num_boost_round"]} | seed: {param["seed"]}: {watch_scores}')
            inst.search_time += 1

    param_list = list()
    for param_dict in hyper_params_list:
        for seed in range(2000, 2001):  # 设置5个随机种子，然后取结果的平均，稳定波动
...
if hyper_search_mode == 1:
    ...
    if model_select == 'XgbRegModel':
        tuning_params(hyper2_xgb_reg_params_list, in_time=True)
    elif model_select == 'LgbRegModel':
        tuning_params(hyper2_lgb_reg_params_list, in_time=True)
