...
from Zeus.Ceres.v2_0_2.config.path_conf import *
import math
import time
import os
...
class SimBackTest:
    def __init__(self, pred_fpath_list, fit_fpath_list, date_dict, period, config, profit_data_fpath=None, data_fpath=None, attend_ratio_range=(10, 45), save_flag=True, multi_attend=True):
    ...
    def state_machine(self):
        ...
        # profit_df = profit_df.reset_index().query('num==1').set_index(['dt', 'Ticker'])
        label_df = pd.read_pickle(self.label_fpath)
        # label_df = label_df.reset_index().query('num==1').set_index(['dt', 'Ticker'])

        if STRATEGY_NAME == 'ProjectSell':
            profit_df['pct'] = profit_df['label_diff_pct']
        # if self.config in ['config1', 'config2']:
        #     profit_df['pct'] = profit_df['label_pct_diff']
        ...
        if 'label_pct_cost' not in profit_df.columns:
            ...
            profit_df['Indexs'] = profit_df['stockID'].astype(str) + ' ' + (profit_df['datelist'].astype(int)).astype(str) + ' ' + \
                                  (profit_df['num'].astype(int)).astype(str)
    ...
    def wrapper(self, pred_fpath_tuple_list):
        for idx in tqdm(range(len(pred_fpath_tuple_list))):
            if idx % 5 == 0: print(f'回测中：目前为第{idx}个')
    ...
    def start_backtest(self, multi=True):
        ...
        if multi:
            ...
            # pred_fpath_tuple_list = list(tuple(zip(self.pred_fpath_list, self.fit_fpath_list)))
            # self.wrapper(pred_fpath_tuple_list)
...
if __name__ == '__main__':
    sbt = SimBackTest(pred_fpath_list=['/data/user/015614/Zeus/pred/Ceres/v2_0_2/config1/fsv8_s1_Xgb/period1/hyper/0/20201001~20210331.csv'],
                      fit_fpath_list=['/data/user/015614/Zeus/pred/Ceres/v2_0_2/config1/fsv8_s1_Xgb/period1/hyper/0/20210301~20210331.csv'],
                      data_fpath='/data/group/800463/sunss/ceres/20250901_vnum/factor_df_vnum_20160101_20210331.pkl',
                      config='config1',
                      profit_data_fpath='/data/group/800463/sunss/ceres/20250901_vnum/profit_vnum_0.10_0.10_500_1500_250_20.h5',
                      date_dict=DATE_CONFIG['period1'],
                      period='period1',
