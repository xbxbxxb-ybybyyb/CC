...
from Zeus.Ceres.v2_0_2.backtest.backtest import start_backtest
...
import importlib
from Zeus.Ceres.v2_0_2.backtest.SimBackTest import SimBackTest
from Zeus.Ceres.v2_0_2.config.strat_conf import *
from Zeus.Ceres.v2_0_2.scripts.hyper_param_space import *
from Zeus.Ceres.v2_0_2.label_generate.fetch_label import fetch_label
import xgboost as xgb
import lightgbm as lgb
from Zeus.Ceres.v2_0_2.models.MlpRegModel import MlpRegModel
from sklearn.metrics import roc_curve, auc
from LucienUtil.FileUtil import FileUtil
from Zeus.Ceres.v2_0_2.scripts.my_logger import MyLogger
import warnings
import random
import os
...
if len(sys.argv) > 1:
    ...
    hyper_search_mode = 2   # 0表示直接训练，1表示寻很多参数，2表示寻数的棵树和随机种子
    attend_ratio = 40
    seed = 0
    gpu_id = 0
print(period, fs_version, config_flag, label_trans, model_select, scaler_select, hyper_search_mode, attend_ratio, gpu_id)
os.environ['CUDA_VISIBLE_DEVICES'] = str(gpu_id)
module_name = f'Zeus.Ceres.v2_0_2.config.path_conf'
...
# filter
if 'filter_data_fpath' in PT.keys():
    filter_data_fpath = PT['filter_data_fpath']
else:
    filter_data_fpath = None
profit_data_fpath = PT['profit_data_fpath']
fsv8_fpath = PT[f'xgb_fsv8_fpath']
fsv10_fpath = PT[f'xgb_fsv10_fpath']
fsv11_fpath = PT[f'xgb_fsv11_fpath']
fsrs_fpath = PT[f'fsrs_fpath']
...
version: like v1_0_0...
strategy_name: like Ceres, Jupiter...
"""
class MyModelTrain:
    def __init__(self, model_select='XgbRegModel'):
        ...
        self.strategy_name = 'Ceres'
        self.version = STRATEGY_VERSION
        self.attend_ratio = attend_ratio
        self.best_threshold = 0
        self.period = period
        self.prefix_path = f'{self.strategy_name}/{self.version}/{config_flag}/{self.model_name}/{self.period}/'
        ...
        if hyper_search_mode == 0:
            # 获取寻优之后保存在hyper文件夹下的最佳参数
            with open(f'/data/user/015614/Zeus/pred/{self.prefix_path.replace("_roll", "")}/hyper/best_params.json') as f:
                best_params = json.load(f)
            self.param = eval(best_params['best_param'])
        ...
        self.X_train_scaled, self.X_test_scaled, self.X_fit_scaled = None, None, None
        self.prepare_data()
    ...
    def select_factor(self):
        if fs_version in ['fsv8', 'fsv10', 'fsv11']:
            xgb_imptc_fpath = eval(eval(f'{fs_version}_fpath'))
            ...
            xgb_imptc_fpath = eval(eval(f'{fs_version}_fpath'))
            factor_list = pd.read_excel(xgb_imptc_fpath).set_index('factor_name')
        ...
        if len(factor_list) > 550:
            factor_list = factor_list[:550]

        print(f'{period} {config_flag} {label} | use {fs_version} which includes {len(factor_list)} factors and use No.{gpu_id} GPU')
    ...
    def tranfrom_pred_data(y_test_pred, index, score_threshold=0.0):
        ...
        test_pred_df['num'] = test_pred_df.index.get_level_values(2).map(str)

        test_pred_df['Indexs'] = test_pred_df['stockID'] + ' ' + test_pred_df['datelist'] + ' ' + test_pred_df['num']
        if filter_data_fpath:
            filter_data = pd.read_pickle(filter_data_fpath)
            test_pred_df = test_pred_df.loc[list(set(test_pred_df.index).intersection(set(filter_data.index)))]
    ...
    def prepare_data(self):
        ...
        self.X_train_scaled, self.X_test_scaled, self.X_fit_scaled = X_train_scaled, X_test_scaled, X_fit_scaled


    def start_train(self, param):
        param_copy = param.copy()

        self.train_model(self.X_train_scaled, self.y_train, param_copy, self.X_test_scaled, self.y_test, stage='draw')

        dtrain = self.wrap_test_data(self.X_train_scaled)
        ...
        dtest = self.wrap_test_data(self.X_test_scaled)
        ...
        if seed == 0 and not period.endswith('roll'):
            ...
            self.best_threshold = pd.read_json(f'/data/user/015614/Zeus/pred/{self.prefix_path.replace("_roll", "")}/model/seed_0/_score_threshold.json').iloc[0, 0]
        ...
        dfit = self.wrap_test_data(self.X_fit_scaled)
...
# btl = BTLite2(strategy_name='Ceres',
...
def wrapper(idx, params):
    inst.hyper_pred_out_path = inst.pred_out_path + f'hyper/{idx}/'
    if idx % 5 == 0:
        print(f'{period}_{config_flag}_{idx}')
    os.makedirs(inst.hyper_pred_out_path, exist_ok=True)
    with open(inst.hyper_pred_out_path + f'param.json', 'w') as f_obj:
        json.dump(params, f_obj)

    valid_auc, eval_tuple = inst.start_train(params)
    inst.model.save_model(inst.hyper_pred_out_path + f'{inst.model_select}.pkl')    # 保存模型，方便后续fit
    ...
    with open(inst.hyper_pred_out_path + '_factorName.json', 'w') as f:
        json.dump(inst.factor_list, f, ensure_ascii=False, indent=2)
    # 保存scaler结果
    factor_scaler_info.to_json(inst.hyper_pred_out_path + '_factorScaler.json', orient='records', lines=False, double_precision=15)
    # 保存因子列表名称
    factor_scaler_info['factorName'].to_json(inst.hyper_pred_out_path + '_factorName.json', orient='values')
    # 保存最佳阈值和scaler名字
    pd.Series([inst.best_threshold, scaler_select]).to_json(inst.hyper_pred_out_path + '_score_threshold.json', orient='records', lines=False, double_precision=15)
    """end"""

    with open(inst.hyper_pred_out_path + f'train_result_{period}.json', 'w') as f_obj:    # 保存train、test、fit的结果
        json.dump(eval_tuple, f_obj)

def tuning_params(hyper_params_list, multi=True):
    param_tuple_list = [(x, y) for x, y in zip(np.arange(len(hyper_params_list)), hyper_params_list)]
    print(f'{period}_{config_flag}本次搜索参数空间个数为{len(param_tuple_list)}个')
    if not multi:
        t1 = time.time()
        for idx, params in param_tuple_list:
            wrapper(idx, params)
        print(f'{period}_{config_flag}本次参数空间搜索耗时{time.time() - t1}秒')
    else:
        t1 = time.time()
        job_list = list()
        for idx, params in param_tuple_list:
            job_list.append(delayed(wrapper)(idx, params))
        # multi_work = Parallel(n_jobs=5, backend='multiprocessing')
        multi_work = Parallel(n_jobs=20, backend='loky')
        multi_work(job_list)
        print(f'本次参数空间搜索耗时{time.time() - t1}秒')

    # print('-------开始进行全部回测--------')
    # start_backtest('/data/user/015614/Zeus/pred/Ceres/v2_0_2/', period)


if hyper_search_mode == 1:
    if model_select == 'XgbRegModel':
        tuning_params(hyper_xgb_reg_params_list)
elif hyper_search_mode == 2:
    if model_select == 'XgbRegModel':
        tuning_params(hyper_xgb_reg_params_list, multi=False)   # GPU训练使用单进程更好，串行反而看起来变慢了
    ...
    """保存模型"""
    if inst.model_select in ['XgbRegModel']:
        os.makedirs(model_save_path + f'{inst.prefix_path}/model/seed_{seed}/', exist_ok=True)
        model_path = model_save_path + f'{inst.prefix_path}/model/seed_{seed}/'
