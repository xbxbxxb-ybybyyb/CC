...
import time
import pandas as pd
from Zeus.Ceres.v1_0_13.config.strat_conf import DATE_CONFIG
from Zeus.Ceres.v1_0_13.submit_model.infer import infer
...
def start_backtest(pred_dir, period):
    ...
    for dirpath, dirnames, filenames in os.walk(pred_dir):
        for filename in filenames:
            if 'Model' in filename:
                ...
                threshold_file = dirpath + '/_score_threshold.json.json'
                model_file = dirpath + '/' + model
    ...
    for config in config_set:   # 不同的config可能对应不同的收益文件
        ...
        for idx, *tmp_multi_tuple in tmp_multi_tuple_list:
            result, _ = infer(data_df, tmp_multi_tuple[0], tmp_multi_tuple[1], begin_date, end_date, tmp_multi_tuple[2], tmp_multi_tuple[3])
        print(f'本次参数空间搜索耗时{time.time() - t1}秒')


if __name__ == '__main__':
    start_backtest('/data/user/015614/Zeus/pred/Ceres/v1_0_13/', 'period6_fit')