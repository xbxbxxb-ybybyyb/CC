...
import pandas as pd
import numpy as np
from Zeus.ProjectSell.v1_0_3.path_conf import *
from Zeus.ProjectSell.v1_0_3.hyper_param_space import hyper_xgb_reg_params
...
from Zeus.ProjectSell.v1_0_3.my_logger import MyLogger
import warnings
import os
import json
import joblib
...
if PERIOD[-1] is '1':
    param = {'alpha': 0.6, 'booster': 'gbtree', 'colsample_bytree': 0.6, 'eta': 0.005, 'eval_metric': 'rmse', 'evals_result': ('auc', 'rmse'), 'gamma': 0, 'lambda': 0.2, 'max_depth': 8.0, 'min_child_weight': 6.0, 'n_jobs': -1, 'num_boost_round': 3200.0, 'objective': 'reg:linear', 'random_state': 2022, 'scale_pos_weight': 1.0, 'silent': True, 'subsample': 0.9, 'tree_method': 'gpu_hist'}
    best_threshold = -0.007761
elif PERIOD[-1] is '2':
    param = {'alpha': 0.2, 'booster': 'gbtree', 'colsample_bytree': 0.8, 'eta': 0.005, 'eval_metric': 'rmse', 'evals_result': ('auc', 'rmse'), 'gamma': 0, 'lambda': 0.4, 'max_depth': 8.0, 'min_child_weight': 4.0, 'n_jobs': -1, 'num_boost_round': 2900.0, 'objective': 'reg:linear', 'random_state': 2022, 'scale_pos_weight': 1.0, 'silent': True, 'subsample': 0.7, 'tree_method': 'gpu_hist'}
    best_threshold = -0.005474
elif PERIOD[-1] is '3':
    param = {'alpha': 0.5, 'booster': 'gbtree', 'colsample_bytree': 0.7, 'eta': 0.005, 'eval_metric': 'rmse', 'evals_result': ('auc', 'rmse'), 'gamma': 0, 'lambda': 0.7, 'max_depth': 9.0, 'min_child_weight': 5.0, 'n_jobs': -1, 'num_boost_round': 2400.0, 'objective': 'reg:linear', 'random_state': 2022, 'scale_pos_weight': 1.0, 'silent': True, 'subsample': 0.9, 'tree_method': 'gpu_hist'}
    best_threshold = -0.000779
elif PERIOD[-1] is '4':
    param = {'alpha': 1.0, 'booster': 'gbtree', 'colsample_bytree': 0.6, 'eta': 0.005, 'eval_metric': 'rmse', 'evals_result': ('auc', 'rmse'), 'gamma': 0, 'lambda': 0.9, 'max_depth': 7.0, 'min_child_weight': 1.0, 'n_jobs': -1, 'num_boost_round': 2300.0, 'objective': 'reg:linear', 'random_state': 2022, 'scale_pos_weight': 1.0, 'silent': True, 'subsample': 0.6, 'tree_method': 'gpu_hist'}
    best_threshold = 0.002513
elif PERIOD[-1] is '5':
    param = {'alpha': 0.9, 'booster': 'gbtree', 'colsample_bytree': 0.5, 'eta': 0.005, 'eval_metric': 'rmse', 'evals_result': ('auc', 'rmse'), 'gamma': 0, 'lambda': 0.6, 'max_depth': 6.0, 'min_child_weight': 5.0, 'n_jobs': -1, 'num_boost_round': 2200.0, 'objective': 'reg:linear', 'random_state': 2022, 'scale_pos_weight': 1.0, 'silent': True, 'subsample': 0.5, 'tree_method': 'gpu_hist'}
    best_threshold = 0.005239
hyper_params = hyper_xgb_reg_params

class XgbRegModel:
    def __init__(self, sub_version='v1'):
        self.model_name = 'XgbRegModelV3_test'  # 用于测试drop数据处理后为nan的影响
        ...
        self.model = None
        self.scaler = None
        self.label = 'label_diff_pct_next'
    ...
    def get_dataset(self, path):
        ...
        y = pd.read_hdf('/data/group/800463/sunss/project_sell/newData/Sell_pct_0.10_800_190_SH450_SZ100.h5')
        y = y[[self.label]]
    ...
    def filter_factor(self, xgb_imptc_fpath=None, factor_score_fpath=factor_score_fpath):
        ...
        factor_list = pd.read_pickle(f'/data/user/015614/Zeus/factor_select/ProjectSell/v1_0_3/rf_fs_list_next_{PERIOD}.pkl')
        # factor_list.remove('saturn_high_before20')
    ...
    def start_train(self, param):
        ...
        X_train_scaled['saturn_high_before20'] = 0
        X_test_scaled['saturn_high_before20'] = 0
...
if hyper_search_mode:
    ...
    inst.model.save_model(junk_path + 'period4_XgbRegModel.pkl')
