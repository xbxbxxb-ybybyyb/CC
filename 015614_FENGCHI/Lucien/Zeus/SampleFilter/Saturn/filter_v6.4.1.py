...
"""
根据概念进行划分
剔除炸板率大于50%且触板数量大于等于3的概念（表示概念在日内出现退潮）
或将炸板率大于50%更改为：板块内
"""
...
zb_pct = jupiter_sample.groupby(['trade_date', '概念名称']).sum()['is_zb'] / jupiter_sample.groupby(['trade_date', '概念名称']).count()['is_zb']
drop_indexes = zb_pct[zb_pct > 0.5].index # 得到要drop的概念，eg.('20150105', '化学制品')
...
filtered_saturn_sample = saturn_reindexed.reset_index().set_index(['dt', 'Ticker'])

#%% 进行样本统计 date20220930
stats_df = pd.DataFrame(index=['FilterV6.4', '全样本'])
stats_df.loc['FilterV6.4', '样本个数'] = len(filtered_saturn_sample)
stats_df.loc['FilterV6.4', 'pct胜率'] = (filtered_saturn_sample['pct'] > 0).sum() / len(filtered_saturn_sample)
stats_df.loc['FilterV6.4', 'pct均值'] = filtered_saturn_sample['pct'].mean()
stats_df.loc['FilterV6.4', 'pct中位数'] = filtered_saturn_sample['pct'].median()
stats_df.loc['FilterV6.4', 'pct_std'] = filtered_saturn_sample['pct'].std()
stats_df.loc['FilterV6.4', 'label胜率'] = (filtered_saturn_sample['label_v2o10d1'] > 0).sum() / len(filtered_saturn_sample)
stats_df.loc['FilterV6.4', 'label均值'] = filtered_saturn_sample['label_v2o10d1'].mean()
stats_df.loc['FilterV6.4', 'label中位数'] = filtered_saturn_sample['label_v2o10d1'].median()
stats_df.loc['FilterV6.4', 'label_std'] = filtered_saturn_sample['label_v2o10d1'].std()
stats_df.loc['FilterV6.4', 'T日收盘涨停比例'] = filtered_saturn_sample['label_T_close_is_zt'].sum() / len(filtered_saturn_sample)
