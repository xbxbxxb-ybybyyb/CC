...
jupiter = jupiter.query('prediction == 1')
...
check = pd.concat([profit, label], axis=1)
...
# 只筛选jupiter信号为1的样本
check = check.reindex(index=jupiter.index)
check.query('datelist >= "20210701" & datelist <= "20211231" & buy_vol == 0')['pct'].describe()
check.query('datelist >= "20210701" & datelist <= "20211231" & buy_vol == 0')['label_TN_o2ul'].describe()
...
period4_test = '20210701~20211231_Europa_fac_20221116_FSV8_all_pct_graded_lowCost_period4_all_merge_test_模型评价_20230111.xlsx'
