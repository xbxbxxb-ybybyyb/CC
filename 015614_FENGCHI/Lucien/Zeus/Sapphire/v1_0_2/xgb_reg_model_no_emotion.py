...
import pandas as pd
import numpy as np
from Zeus.Sapphire.v1_0_2.path_conf import *
from Zeus.Sapphire.v1_0_2.hyper_param_space import hyper_xgb_reg_params
...
from Zeus.Sapphire.v1_0_2.my_logger import MyLogger
...
if len(sys.argv) > 1:
    ...
    print(PERIOD, fs_version, label_flag)
else:
    PERIOD = 'period1'
    SUB_VERSION = f'v{PERIOD[-1]}'
...
class XgbRegModel:
    def __init__(self, sub_version='v1'):
        self.model_name = f'{fs_version}{label_flag}_XgbRegModel'
        ...
        if not hyper_search_mode:
            if PERIOD[-1] == '1':
                ...
                self.param = pd.read_excel(self.log_path + f'{self.strategy_name}_{self.version}_{self.model_name}_{self.sub_version}_.xlsx', index_col=0).iloc[0]['整体参数']
                # self.best_threshold = 0.008754 # fsv8
                self.best_threshold = 0.010686 # rffs
            elif PERIOD[-1] == '5':
                self.param = pd.read_excel(self.log_path + f'{self.strategy_name}_{self.version}_{self.model_name}_{self.sub_version}_.xlsx', index_col=0).iloc[0]['整体参数']
                self.best_threshold = 0
            elif PERIOD[-1] == '6':
        ...
        self.model = None
        self.scaler = None
        if 'pct_after' in label_flag:
            self.label = 'label_diff_pct_after_graded'
        else:
            self.label = 'label_diff_pct_graded'
    ...
    def filter_factor(self, xgb_imptc_fpath=None, factor_score_fpath=None):
        # TODO：使用哪种因子版本，统一的版本还是自己开发的版本
        if fs_version == 'rffs':
            factor_list = pd.read_pickle(f'/data/user/015614/Zeus/factor_select/Sapphire/v1_0_2/rffs_{PERIOD}.pkl')
        elif fs_version == 'fsrs':
            factor_list = pd.read_excel(eval(f'fsrs_imptc{label_flag}_{PERIOD}_fpath'), index_col=0)
    ...
    def get_train_and_test_data(self):
        ...
        if fs_version in ['fsv8', 'fsv10', 'fsv11']:
            filtered_factor = self.filter_factor(xgb_imptc_fpath=eval(f'xgb_imptc{label_flag}_{fs_version}_{PERIOD}_fpath'))
        else:
            filtered_factor = self.filter_factor(xgb_imptc_fpath=eval(f'xgb_imptc{label_flag}_fsv8_{PERIOD}_fpath'))
    ...
    def tranfrom_pred_data(self, y_test_pred, index, score_threshold=0.0):
        ...
        test_pred_df['datelist'] = test_pred_df.index.map(lambda x: profit_data.loc[x, 'dt_last_zt_1_ts']).map(lambda x: x.strftime('%Y%m%d'))
        # test_pred_df['datelist'] = test_pred_df.index.map(lambda x: self.X.loc[x, 'saturn_dt_last_zt_1']).map(int).map(str)
...
if hyper_search_mode:
    ...
    score_threshold = np.percentile(inst.y_test_pred, 100 - 30)
    inst.best_threshold = score_threshold
