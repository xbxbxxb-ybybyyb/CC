...
from Zeus.Sinope.v1_0_19.backtest.sim_backtest.SimBackTest import SimBackTest

import pandas as pd
import numpy as np
from tqdm import tqdm
from joblib import Parallel, delayed
from Zeus.Sinope.v1_0_19.path_conf import *
from Zeus.Sinope.v1_0_19.hyper_param_space import *
import xgboost as xgb
import lightgbm as lgb
from Zeus.Sinope.v1_0_19.models.MlpRegModel import MlpRegModel
from sklearn.metrics import roc_curve, auc
from LucienUtil.FileUtil import FileUtil
from Zeus.Sinope.v1_0_19.my_logger import MyLogger
import warnings
import random
import os
...
if len(sys.argv) > 1:
    ...
    scaler_select = sys.argv[5]
    hyper_search_mode = int(sys.argv[6])
    attend_ratio = int(sys.argv[7])
    seed = int(sys.argv[8])
else:
    period = 'period5'
    fs_version = 'fsv11_add32' # fsv8\fsv10\fsv11\rffs
    label_flag = 'pct'
    ...
    attend_ratio = 40
    seed = 0
print(period, fs_version, label_flag, model_select, scaler_select, hyper_search_mode, attend_ratio)

"""确定选用的参数"""
if model_select == 'XgbRegModel':
    fixed_params = fixed_xgb_reg_param[fs_version.replace('_add32', '')]
elif model_select == 'LgbRegModel':
    fixed_params = fixed_lgb_reg_param[fs_version.replace('_add32', '')]
    # os.environ['CUDA_LAUNCH_BLOCKING'] = '2'
elif model_select == 'MlpRegModel':
    fixed_params = fixed_mlp_reg_param[fs_version.replace('_add32', '')]
...
version: like v1_0_0...
strategy_name: like Sinope, Jupiter...
"""
class MyModelTrain:
    def __init__(self, model_select='XgbRegModel'):
        ...
        model_select_abbr = model_select
        self.model_name = f'{fs_version}_{scaler_select_abbr}_{model_select_abbr}'

        self.model_select = model_select
        self.strategy_name = 'Sinope'
        self.version = 'v1_0_19'
    ...
    def select_factor(self):
        ...
        if fs_version.__contains__('_add32'):
            fs_version = fs_version.replace('_add32', '')  # add32只能以_add32添加在因子筛选结尾
            add32_flag = True
        else:
            fs_version = fs_version

        if fs_version in ['fsv8', 'fsv10', 'fsv11']:
            xgb_imptc_fpath = eval(f'xgb_imptc_{label_flag}_{fs_version}_{period.replace("_roll", "")}_fpath')
            ...
            factor_list = pd.read_excel(eval(f'fsrs_imptc_{label_flag}_{period.replace("_roll", "")}_fpath')).set_index('factor_name')
    ...
    def get_train_and_test_data(self):
        ...
        if add32_flag:
            add_fea = pd.read_csv(f'/data/group/800463/xubc/extract_feature/sinope/fac_20240714/sinope_fea0731_{period[-1]}.csv')
            add_fea['dt'] = add_fea['dt'].apply(lambda x: pd.to_datetime(x))
            add_fea = add_fea.set_index(['dt', 'Ticker'])
            X_copy = X_copy.join(add_fea, how='left')
            self.factor_list = self.factor_list + [f'c{x}' for x in range(0, 32)]
    ...
    def start_train(self, param):
        ...
        if seed == 0 and not period.endswith('roll'):
            ...
            self.best_threshold = pd.read_json(f'/data/user/015614/Zeus/pred/Sinope/v1_0_19/{self.model_name}/model/{period.replace("_roll", "")}/seed_0/_score_threshold.json').iloc[0, 0]
...
# btl = BTLite2(strategy_name='Sinope',
        ...
        for param in tqdm(params):
            ...
            if in_time:
                sbt = SimBackTest(pred_fpath_list=[y_test_pred_df],
                                  ...
                                  attend_ratio_range=(10, 45),
                                  save_flag=False,
                                  multi_attend=True)
                stats_df, model_test_mingan, _ = sbt.start_backtest(multi=False)
                ...
                stats_df['训练集auc'] = float(eval_tuple[2][0])
                stats_df['训练集acc'] = float(eval_tuple[2][1])
            ...
            # }
                stats_df['param'] = str(param)
                FileUtil.save_df2csv(stats_df, inst.hyper_pred_out_path, '', verbose=False)

                watch_scores = stats_df[['收益风险比', '收益夏普比率', '平均收益风险比', '平均收益夏普比率', '累计扣费总收益', '最大回撤', '预测值与标签RankIC', '基础样本数量', '扣费后收益率胜率',
                                         '训练集auc', '训练集prec']].to_dict()
...
if hyper_search_mode == 1:
    ...
    if inst.model_select in ['XgbRegModel', 'LgbRegModel']:
        ...
        """
        在这里改变保存的阈值，根据900多个样本那个去进行更改
        """
        scene_samples_fpath = data_all_fpath
        ...
        new_csv_df = y_test_pred_df.loc[list(set(y_test_pred_df.index.tolist()).intersection(scene_df_index))]
        y_test_pred_df = new_csv_df
        new_threshold = np.percentile(new_csv_df['pred_Reg'].values, 60)

        pd.Series([new_threshold, scaler_select]).to_json(model_path + '_score_threshold.json', orient='records', lines=False, double_precision=15)
        FileUtil.save_df2csv(y_test_pred_df, inst.pred_out_path, inst.test_pred_fname, verbose=False)
