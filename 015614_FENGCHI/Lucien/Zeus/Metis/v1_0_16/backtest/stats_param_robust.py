...
import pandas as pd
import numpy as np
from Zeus.Metis.v1_0_16.path_conf import *
import math
...
model_names = os.listdir('/data/user/015614/Zeus/pred/Metis/v1_0_16/')
...
def backtest(pred_df):
    profit_data = pd.read_hdf('/data/group/800463/sunss/profit/metis/20230817/v1/LabelProfit_zt_twap_0.15_2000_300_SH250_SZ20.h5')
...
pred_out_path = pred_out_path + 'Metis/v1_0_16/'
...
for model_name in model_names:
    for idx in range(90):
        ...
        if idx % 3 == 0:
            res_s.name = f'{model_name}_{idx}_method1'
        elif idx % 3 == 1:
            res_s.name = f'{model_name}_{idx}_method2'
        elif idx % 3 == 2:
            res_s.name = f'{model_name}_{idx}_method3'
        summary_df = summary_df.append(res_s)

summary_df['method'] = summary_df.index.map(lambda x: x[-7:])
summary_df['model_name'] = summary_df.index.map(lambda x: '_'.join(x.split('_')[:3]))
summary_df['epoch'] = summary_df.index.map(lambda x: int(x.split('_')[3]) // 3)

watch_list = ['平均收益风险比', '平均收益夏普比率', '平均累计收益', '平均收益率均值']
win_times_s = pd.Series(0, index=pd.MultiIndex.from_product([model_names, watch_list]))
summary_df = summary_df.set_index(['model_name', 'epoch', 'method'])
epoch_times = 30
for model_name in model_names:
    for epoch in range(epoch_times):
        for watch in watch_list:
            if summary_df.loc[(model_name, epoch, 'method2'), watch] > summary_df.loc[(model_name, epoch, 'method3'), watch]:
                 win_times_s.loc[(model_name, watch)] += 1

win_ratio_s = win_times_s / epoch_times
...
FileUtil.save_dict2xls(output_dict, '/data/user/015614/junkData/', f'{period}.xlsx')
send_file(f'/data/user/015614/junkData/{period}.xlsx')