...
import pandas as pd
import numpy as np
from Zeus.Metis.v1_0_1.path_conf import *
from Zeus.Metis.v1_0_1.hyper_param_space import hyper_lgb_reg_params
...
from Zeus.Metis.v1_0_1.my_logger import MyLogger
...
class LgbRegModel:
    def __init__(self, sub_version='v1'):
        ...
        if not hyper_search_mode:
            if PERIOD[-1] == '1':
                ...
                self.param = pd.read_excel(self.log_path + f'{self.strategy_name}_{self.version}_{self.model_name}_{self.sub_version}_.xlsx', index_col=0).iloc[0]['整体参数']
                self.best_threshold = 0.005715  # fsv11
            elif PERIOD[-1] == '5':
                self.param = pd.read_excel(self.log_path + f'{self.strategy_name}_{self.version}_{self.model_name}_{self.sub_version}_.xlsx', index_col=0).iloc[0]['整体参数']
                self.best_threshold = 0
            elif PERIOD[-1] == '6':
    ...
    def filter_factor(self, xgb_imptc_fpath=None, factor_score_fpath=None):
        ...
        if fs_version == 'rffs':
            factor_list = pd.read_pickle(f'/data/user/015614/Zeus/factor_select/Metis/v1_0_1/rffs_{PERIOD}.pkl')
