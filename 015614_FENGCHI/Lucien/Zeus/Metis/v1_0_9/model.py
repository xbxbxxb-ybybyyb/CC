...
import pandas as pd
import numpy as np
from Zeus.Metis.v1_0_9.path_conf import *
from Zeus.Metis.v1_0_9.hyper_param_space import *
import xgboost as xgb
import lightgbm as lgb
from sklearn.metrics import roc_curve, auc
from LucienUtil.FileUtil import FileUtil
from Zeus.Metis.v1_0_9.my_logger import MyLogger
import warnings
import random
import os
...
class MyModelTrain:
    ...
    def start_train(self, param):
        ...
        """加载保存下来的模型进行验证"""
        # model_fpath = '/data/user/015614/shared/for_wj/d20230906_Metis_V2_V20230821_period5/fsrs_pct_LgbRegModel/model/period5/LgbRegModel.pkl'
        # lgb_model = lgb.Booster(model_file=model_fpath)
        # self.model = lgb_model

        """加载王敬处理后的因子路径，与本地因子进行对比，或者进行模型预测"""
        # wj_scaled_factor_fpath = '/data/group/800463/wangj/for_fc/fsrslgb_scaledata.pkl'
        # scaled_factor = pd.read_pickle(wj_scaled_factor_fpath)
        # scaled_factor = scaled_factor[X_test_scaled.columns.tolist()]
        # factor_diff = (scaled_factor - X_test_scaled).applymap(abs).sum(axis=0).sort_values()
        # X_test_scaled = scaled_factor

        dtest = self.wrap_test_data(X_test_scaled)
        y_test_pred = self.predict(dtest)
        self.best_threshold = np.percentile(y_test_pred, 100 - self.attend_ratio)
        self.best_threshold = pd.read_json('/data/user/015614/shared/for_wj/d20230906_Metis_V2_V20230821_period5/fsrs_pct_LgbRegModel/model/period5/_score_threshold.json')[0].values[0]
