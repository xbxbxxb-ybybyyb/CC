...
import pandas as pd
import numpy as np
from Zeus.Metis.v1_0_2.path_conf import *
from Zeus.Metis.v1_0_2.hyper_param_space import hyper_lgb_reg_params
...
from Zeus.Metis.v1_0_2.my_logger import MyLogger
...
class LgbRegModel:
    def __init__(self, sub_version='v1'):
        self.model_name = f'{fs_version}{label_flag}_LgbRegModel'
        ...
        if not hyper_search_mode:
            if PERIOD[-1] == '1':
                ...
                self.param = pd.read_excel(self.log_path + f'{self.strategy_name}_{self.version}_{self.model_name}_{self.sub_version}_.xlsx', index_col=0).iloc[0]['整体参数']
                self.best_threshold = 0.4404  # fsv11
            elif PERIOD[-1] == '5':
                self.param = pd.read_excel(self.log_path + f'{self.strategy_name}_{self.version}_{self.model_name}_{self.sub_version}_.xlsx', index_col=0).iloc[0]['整体参数']
                self.best_threshold = 0
            elif PERIOD[-1] == '6':
        ...
        self.model = None
        self.scaler = None
        if 'pct1' in label_flag:
            self.label = 'label_diff_pct_v1'
        else:
    ...
    def filter_factor(self, xgb_imptc_fpath=None, factor_score_fpath=None):
        ...
        if fs_version == 'rffs':
            factor_list = pd.read_pickle(f'/data/user/015614/Zeus/factor_select/Metis/v1_0_2/rffs_{PERIOD}.pkl')
    ...
    def tranfrom_pred_data(y_test_pred, index, score_threshold=0.0):
        ...
        test_pred_df['name'] = test_pred_df.index.get_level_values(2).tolist()
        test_pred_df = test_pred_df.query('name == "metis"')
...
# btl = BTLite2(strategy_name='MetisS1',
