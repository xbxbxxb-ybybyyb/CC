...
from Zeus.Europa.v1_0_24.path_conf import date_config, junk_path
import pandas as pd
import numpy as np
import math
import time
...
class BTLite:
    def __init__(self, valid_fpath, test_fpath, strategy_name, model_name, date_dict, bt_save_path, mode='serial', save_flag=False):
        """
        :param mode: mode可选multi和serial
        """
        self.valid_fpath = valid_fpath
        ...
        self.valid_start_date = date_dict['valid_start_date']
        self.valid_end_date = date_dict['valid_end_date']
        ...
        self.valid_df, self.test_df, self.profit_df, self.label_df, self.cost_pct = None, None, None, None, None
        self.extreme_thres, self.attend_min, self.attend_max = None, None, None
        self.state_machine()

    def set_valid_fpath(self, value):
        self.valid_fpath = value
    ...
    def state_machine(self):
        if self.strategy_name is 'Europa':
            ...
            self.cost_pct = 0.002
            self.attend_min = 10
            self.attend_max = 61    # TODO：change
            self.extreme_thres = 0.2
    ...
    def data_prepare(self):
        valid_df = pd.read_csv(self.valid_fpath, index_col=0).query(f'datelist >= {self.date_dict["valid_start_date"]} & datelist <= {self.date_dict["valid_end_date"]}')
        test_df = pd.read_csv(self.test_fpath, index_col=0).query(f'datelist >= {self.date_dict["test_start_date"]} & datelist <= {self.date_dict["test_end_date"]}')

        self.valid_df = self._concat_label_profit(valid_df)
        self.test_df = self._concat_label_profit(test_df)
    ...
    def _concat_label_profit(self, df):
        ...
        df_copy['ZT_Time'] = self.label_df['ZT_Time']
        return df_copy
    ...
    def calc_model_mingan(self, _valid_df, _test_df):
        valid_df = _valid_df.copy()
        test_df = _test_df.copy()
        attend_pct_list = list(range(self.attend_min, self.attend_max, 1))
        valid_attend_df = pd.DataFrame(index=attend_pct_list)
        valid_df = valid_df.sort_values(by='pred_Reg', ascending=False)
        test_df = test_df.sort_values(by='pred_Reg', ascending=False)
        valid_totalnum = valid_df.shape[0]
        test_totalnum = test_df.shape[0]

        # 分配group_id
        group_id_list = []  # 分成从20-40之间(不同策略不同)
        for attend_pct in attend_pct_list:
            ratio_num = math.ceil(valid_totalnum * attend_pct / 100)
            if attend_pct == self.attend_min:
                tmp_num = ratio_num
            elif ratio_num >= valid_totalnum:
                tmp_num = valid_totalnum - math.ceil(valid_totalnum * (attend_pct - 1) / 100)
            else:
                tmp_num = ratio_num - math.ceil(valid_totalnum * (attend_pct - 1) / 100)

            group_id_list = group_id_list + [attend_pct] * tmp_num
        group_id_list = group_id_list + (valid_totalnum - len(group_id_list)) * [attend_pct + 1]
        valid_df['group_id'] = group_id_list
        date_list = [int(x) for x in fd.tradingday(str(self.valid_start_date), str(self.valid_end_date))]

        for attend_pct in attend_pct_list:
            group_df = valid_df.query(f'group_id <= {attend_pct}')
            group_df_daily = group_df.groupby('datelist').sum()[['label_profit_cost']].reindex(date_list).fillna(0)
            valid_attend_df.loc[attend_pct, '实际参与率'] = round(len(group_df) / valid_totalnum, 4)
            valid_attend_df.loc[attend_pct, '因子值范围'] = str(float(group_df.min()['pred_Reg']))
            valid_attend_df.loc[attend_pct, '扣费收益率胜率'] = round((group_df['label_pct_cost'] > 0).mean(), 4)
            valid_attend_df.loc[attend_pct, '扣费收益率'] = round(group_df['label_pct_cost'].mean(), 4)
            valid_attend_df.loc[attend_pct, '累计盈利'] = int(group_df.sum()['label_profit_cost'])
            valid_attend_df.loc[attend_pct, '最大回撤'] = int(self.calc_mdd(group_df_daily['label_profit_cost'].values))
            valid_attend_df.loc[attend_pct, '收益风险比'] = round(abs(valid_attend_df.loc[attend_pct, '累计盈利'] / valid_attend_df.loc[attend_pct, '最大回撤']), 4)
            valid_attend_df.loc[attend_pct, '夏普比率'] = self.calc_sharp(valid_df, ref_col='收益率(扣除成本)')
            valid_attend_df.loc[attend_pct, '收益夏普比率'] = self.calc_sharp(valid_df, ref_col='盈亏金额(扣除成本)')

        valid_attend_df = valid_attend_df.set_index(['因子值范围'])

        # 开始计算test_df
        test_attend_df = pd.DataFrame(index=valid_attend_df.index)
        ...
        for pred_reg in test_attend_df.index.tolist():
            group_df = test_df.query(f'pred_Reg >= {pred_reg}')
            group_df_daily = group_df.groupby('datelist')[['label_profit_cost']].sum().reindex(date_list).fillna(0)
            test_attend_df.loc[pred_reg, '实际参与率'] = round(len(group_df) / test_totalnum, 4)
            test_attend_df.loc[pred_reg, '因子值范围'] = str(float(group_df.min()['pred_Reg']))
            test_attend_df.loc[pred_reg, '扣费收益率胜率'] = round((group_df['label_pct_cost'] > 0).mean(), 4)
            test_attend_df.loc[pred_reg, '扣费收益率'] = round(group_df['label_pct_cost'].mean(), 4)
            test_attend_df.loc[pred_reg, '累计盈利'] = int(group_df.sum()['label_profit_cost'])
            test_attend_df.loc[pred_reg, '最大回撤'] = int(self.calc_mdd(group_df_daily['label_profit_cost'].values))
            test_attend_df.loc[pred_reg, '收益风险比'] = round(abs(test_attend_df.loc[pred_reg, '累计盈利'] / test_attend_df.loc[pred_reg, '最大回撤']), 4)
            test_attend_df.loc[pred_reg, '夏普比率'] = self.calc_sharp(group_df, ref_col='收益率(扣除成本)')
            test_attend_df.loc[pred_reg, '收益夏普比率'] = self.calc_sharp(group_df, ref_col='盈亏金额(扣除成本)')

        return valid_attend_df, test_attend_df
    ...
    def calc_model_bt_result(self, _test_df, _test_stats_by_samples1, _test_stats_by_day):
        """
        :param _test_df: 保存的预测记录prediction|pred_Reg|stockID|datelist，拼接了一部分列，如label_pct_cost|label_profit_cost|label_buy_amt|label_pct|ZT_Time
        ...
        :return:
        """
        test_df, test_stats_by_samples1, test_stats_by_day = _test_df.copy(), _test_stats_by_samples1.copy(), _test_stats_by_day.copy()
        test_df['盈亏金额(扣除成本)'] = test_stats_by_samples1['盈亏金额(扣除成本)']
        stats_s = pd.Series()

        stats_s['基础样本数量'] = test_df.shape[0]
        ...
        stats_s['样本参与率'] = test_stats_by_samples1.query('prediction == 1').shape[0] / test_df.shape[0]
        ...
        stats_s['预测值与标签IC'] = test_df.query('label_buy_amt>0')[['label_pct_cost', 'pred_Reg']].corr().iloc[0, 1]
        stats_s['预测值与标签RankIC'] = test_df.query('label_buy_amt>0')[['label_pct_cost', 'pred_Reg']].corr('spearman').iloc[0, 1]
        return stats_s
    ...
    def start_backtest(self):
        ...
        model_valid_mingan, model_test_mingan = self.calc_model_mingan(_valid_df=valid_df, _test_df=test_df)

        if self.save_flag:
            save_dict = {'模型结果': stats_df, '样本内不同参与率统计': model_valid_mingan, '样本外不同参与率统计': model_test_mingan}
            FileUtil.save_dict2xls(save_dict, self.bt_save_path)

        return stats_df, model_valid_mingan, model_test_mingan

if __name__ == '__main__':
    t1 = time.time()
    # btl = BTLite(valid_fpath='/data/user/015614/Zeus/pred/Europa/v1_0_24/LgbRegModel/20210401~20211231_LgbRegModel_v2.csv',
    #              test_fpath='/data/user/015614/Zeus/pred/Europa/v1_0_24/LgbRegModel/20210401~20211231_LgbRegModel_v2.csv',
    #              strategy_name='Europa',
    #              model_name='SingleTestModel',
    #              date_dict=date_config['period4_test'],
    #              bt_save_path=junk_path + '回测结果/')
    btl = BTLite(valid_fpath='/data/user/015614/Zeus/pred/Europa/v1_0_29/LgbRegModel/20200401~20201231_LgbRegModel_v2.csv',
                 test_fpath='/data/user/015614/Zeus/pred/Europa/v1_0_29/LgbRegModel/20200401~20201231_LgbRegModel_v2.csv',
                 strategy_name='Europa',
                 model_name='SingleTestModel',
                 date_dict=date_config['period2_test'],
                 bt_save_path=junk_path + '回测结果/')
    t2 = time.time()
    stats_df, model_valid_mingan, model_test_mingan = btl.start_backtest()
    print('初始化时间：', time.time() - t1)
    print('回测时间：', time.time() - t2)
