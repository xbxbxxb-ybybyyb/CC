...
class modelEval_Tool:
    ...
    def select_profit(self):
        if self.strategy not in ['SaturnS0&SaturnS1','SaturnS1&SaturnS3']:
            if self.strategy == 'JupiterN':
                ...
                self.cost_pct = 0.002   # TODO：为了对比，设置为0.003，后面降为0.002
                self.attend_min = 20
                self.attend_max = 51
                ...
                self.cost_pct = 0.002
                self.attend_min = 20 #10#5#20
                self.attend_max = 51#81#41
                self.group_ratio = 0.2
                self.extreme_thres = 0.1 # 0.2
            ...
            if self.profit_path.endswith('.h5'):
                ...
                # TODO：self.pred_data
                all0_index = self.pred_data.query('s3d_prediction == 0').index
                stk_list = list(map(lambda x: x[:9], all0_index))
                dt_list = list(map(lambda x: pd.to_datetime(str(x[9:])), all0_index))
                all0_index = pd.DataFrame([dt_list, stk_list]).T.set_index([0, 1]).index
                profit_data.loc[all0_index, 'buy_amt'] = 0
            else:
    ...
    def select_label(self):
        ...
        if self.strategy == 'SaturnS0&SaturnS1':
            ...
            if self.strategy == 'JupiterN':
                ...
                self.label_path = '/data/user/018107/share_file/for_wj/europa/20230329/factor_df_all_20160101_20230331_fit.pkl'
                self.label_path = '/data/group/800463/sunss/europa/20230329_new/factor_df_all_20160101_20220930.pkl'
                self.label_rev = 'label_TN_o2ul'
                self.label_o2o10 = 'label_pattern'
            elif self.strategy == 'SaturnS0':
            ...
            # if self.strategy in ['SaturnS1', 'SaturnS0','Saturn','Saturn_down','SaturnS5','SaturnS3','SaturnS0&SaturnS1','SaturnS1&SaturnS3']:
            #     #To2pre_df = pd.read_hdf('/data/group/800463/project/project2_prod/daily_data/Basic/Basic_closed_hf_finish.h5')
            #     To2pre_df = pd.read_pickle('/data/group/800463/sunss/saturn/20230524/factor_df_931_20160101_20211231_graded.pkl')
            ...
            #     label_data = pd.concat([label_data, To2pre_df[['T_o2pre', 'pre_close', 'lzt_label_pattern']],T1o2pre_df], axis=1).reindex(label_data.index)#,join_axes=[label_data.index])
            ...
            if 'label_T_close_is_zt' not in label_data.columns.tolist() and (self.strategy in ['JupiterN','Europa','jupiter003','JupiterNSell','JupiterNSell34','JupiterZ','EuropaSell','EuropaSellEnhance'] or self.strategy.find( 'JupiterZSell')>=0):
                label_data['label_T_close_is_zt'] = label_data['label_T_is_zt']
            if 'label_Next_close_is_zt' not in label_data.columns.tolist()  and (self.strategy in ['JupiterN','Europa','jupiter003','JupiterNSell','JupiterNSell34','JupiterZ','EuropaSell','EuropaSellEnhance'] or self.strategy.find( 'JupiterZSell')>=0):
    ...
    def generate_test_report(self,pred_out, pred_in,scene_flag,probacol):
        ...
        if self.strategy not in ['JupiterN', 'jupiter_sell','Europa','jupiter003','JupiterNSell','JupiterNSell34','JupiterZ','EuropaSell','EuropaSellEnhance'] and self.strategy.find( 'JupiterZSell')<0:
    ...
    def cal_sharp(self,day_data,pct_col='收益率(扣除成本)',stepdate=3):
        ...
        sharp = (mean_ret / std_ret) * math.sqrt(250)#abs(mean_ret / std_ret) * math.sqrt(250)
        return sharp
    ...
    def combine_res_inout(self,pred_sample_data,sample_data,statistic_without_extreme_value,statistic_by_sample,statistic_by_day,pred_in,flag_scene,proba_col):
        ...
        if 'label_zuhe' not in pred_sample_data.columns.tolist() and (self.strategy in ['JupiterN','Europa','jupiter003','JupiterNSell','JupiterNSell34','JupiterZ','EuropaSell','EuropaSellEnhance'] or self.strategy.find('JupiterZSell')>=0):
        ...
        if 'label_pat_zuhe' not in pred_sample_data.columns.tolist() and (self.strategy in ['JupiterN','Europa','jupiter003','JupiterNSell','JupiterNSell34','JupiterZ','EuropaSell','EuropaSellEnhance']  or self.strategy.find('JupiterZSell')>=0):
        ...
        if self.strategy in ['JupiterN','Europa','jupiter003','JupiterNSell','JupiterNSell34','JupiterZ','EuropaSell','EuropaSellEnhance'] or self.strategy.find('JupiterZSell')>=0:
