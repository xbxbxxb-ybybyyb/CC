...
class modelEval_Tool:
    def __init__(self,strategy_name,pred_data,valid_data,model_name,begindate, enddate,in_begindate, in_enddate,savepath,label_name='label_pct_cost',scene_flag=''):
    ...
    def select_label(self):
        ...
        if self.strategy == 'SaturnS0&SaturnS1':
            ...
            if self.strategy in ['SaturnS1', 'SaturnS0','Saturn','Saturn_down','SaturnS5','SaturnS3']:
    ...
    def generate_series_data(self):
        ...
        if self.strategy in ['SaturnS1', 'SaturnS0','Saturn','Saturn_down','SaturnS5','SaturnS3']: # ,'CeresS1'
        ...
        totalResDf, daily_data,statistic_by_sample, model_mingan, model_inmingan = self.generate_test_report(model,model_valid,scene_flag,predcol)
        daily_data.columns = [self.indi_str]# [i + '_%s' % str(self.indi_str) for i in daily_data.columns.tolist()]
        if totalResDf.shape[0] > 0:
            totalResDf.columns = [x.replace('模型表现',self.indi_str) for x in totalResDf.columns.tolist()]
        # model_mingan = pd.concat([pd.DataFrame([self.indi_str],index=['模型名称'])])
        return totalResDf.fillna(0), daily_data, statistic_by_sample, model_mingan, model_inmingan
    def generate_test_report(self,pred_out, pred_in,scene_flag,probacol):
        ...
        if self.indi_str.find('vote')<0:
            ...
            model_valid_mingan = self.cal_model_mingan(pred_in_without_extreme, self.predcol)
            model_mingan = self.cal_model_mingan_basedout(pred_out_without_extreme, self.predcol)
        ...
        """这里是分月份统计的，用不到"""
        # if self.indi_str.find('vote')<0 and tradedate_length <len(hfactor.tradingday(self.begindate,self.enddate)):
        ...
        #                                                                 '投资金额')
        # totalResDf_only_extreme, _, _, _ = self.combine_res_inout(
        ...
        model_valid_mingan.to_excel(writer, sheet_name='样本内不同参与率表现')
        model_mingan.to_excel(writer, sheet_name='不同参与率表现')
        writer.save()
        return totalResDf_monthly, daily_data,statistic_by_sample_all.set_index(['datelist','stockID']), model_mingan, model_valid_mingan
    def fun_statistic_by_day(self,start_date, end_date, by_samples,pred_out):
        ...
        #by_day['基础收益率(扣除成本)'] = pred_out.groupby('datelist').mean()['label_profit_cost'].reindex(by_day.index)
