...
class modelEval_Tool:
    ...
    def generate_series_data(self):
        ...
        return totalResDf.fillna(0), daily_data, statistic_by_sample, model_mingan
    def generate_test_report(self,pred_out, pred_in,scene_flag,probacol):
        ...
        #totalResDf_without_extreme.to_excel(writer, sheet_name = '极值处理模型结果')
        # totalResDf_only_extreme.to_excel(writer, sheet_name = '极值部分模型结果')
        model_mingan.to_excel(writer, sheet_name='不同参与率表现')
        writer.save()
        return totalResDf_monthly, daily_data,statistic_by_sample_all.set_index(['datelist','stockID']), model_mingan
