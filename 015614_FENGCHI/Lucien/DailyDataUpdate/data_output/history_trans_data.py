...
def save_data(stk_code_list, dat, md):
    for stk_code in tqdm(stk_code_list):
        print(stk_code)
        trans_df = md.get_data_by_date('Transaction', stk_code, dat)
        if trans_df.shape[0] == 0:
            continue
        else:
            trans_df = trans_df.sort_values(by=['MDTime', 'TradeIndex'])
        ...
        need_col = ['MDDate',
                    ...
                    'TradeBuyNo',
                    'TradeSellNo',
                    'TradeType',  # 为1表示撤单，深圳才有
                    'TradeBSFlag',  # 华泰自定义，重命名
                    'TradePrice',
                    'TradeQty',
                    'TradeMoney',
        ...
        if 'ChannelNo' in trans_df.columns.tolist():
            need_col += ['ChannelNo', 'ApplSeqNum']
            rename_dict = {
                'TradeBSFlag': 'TradeDirection',
                'HTSCSecurityID': 'Symbol',
                'ChannelNo': 'Channel',
                'ApplSeqNum': 'RecordID'
            }
        else:
        ...
        trans_df = trans_df.rename(rename_dict, axis=1)

        year = dat[:4]
        year_month = dat[:6]

        output_dir = f'/data/user/015614/daily/data/Transaction/{year}/{year_month}/{dat}/'
        os.makedirs(output_dir, exist_ok=True)
        trans_df.to_csv(output_dir + f'{stk_code}.csv', index=False)
...
date_list = fd.tradingday(20160112, 20161231)
for dat in date_list:
    ...
    output_dir = f'/data/user/015614/daily/data/Transaction/{year}/{year_month}/{dat}/' # NOTE: 千万不能改，后面有删除

    create_zip_with_max_compression(output_dir, f'/data/user/015614/daily/data/Transaction/{year}/{year_month}/{dat}.zip')
