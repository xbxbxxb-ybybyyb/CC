...
channel_no_dict = sh_channel_no_dict1
SH_df0 = pd.DataFrame()
...
SH_df = pd.concat([SH_df0], sort=False)
SZ_df = pd.concat([SZ_df0], sort=False)
print('需要买入的股票数量:SH%s（第一份%s）；SZ%s（第一份%s）' % (len(SH_df), len(SH_df0), len(SZ_df), len(SZ_df0)))
...
SZ_all_df['duplicates'] = SZ_all_df['证券代码'].duplicated()
message = '孔老师，今天warehouse1需要交易%s只（其中上海%s，深圳%s），第二批需要交易%s只（其中上海%s，深圳%s）' \
...
if not os.path.exists(path):
    os.makedirs(path)
SH_all_df1.to_excel('%s/%s_one_hand_warehouse1_SH1.xlsx' % (path, today), index=False)
SZ_all_df1.to_excel('%s/%s_one_hand_warehouse1_SZ1.xlsx' % (path, today), index=False)
SH_all_df2.to_excel('%s/%s_one_hand_warehouse1_SH2.xlsx' % (path, today), index=False)
SZ_all_df2.to_excel('%s/%s_one_hand_warehouse1_SZ2.xlsx' % (path, today), index=False)
