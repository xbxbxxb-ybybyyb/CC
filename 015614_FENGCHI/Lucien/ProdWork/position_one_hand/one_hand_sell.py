...
hold_sh1 = hold_sh[hold_sh['组合编号'].isin(sh_channel_no_list1)].copy()
hold_sh2 = hold_sh[hold_sh['组合编号'].isin(sh_channel_no_list2)].copy()
print(hold_sh1['组合编号'].value_counts())
print(hold_sh2['组合编号'].value_counts())

# 第一份底仓
channel_no_dict1 = sh_channel_no_dict1
...
# 第二份底仓
channel_no_dict2 = sh_channel_no_dict2
...
condition = (~hold_inf['组合编号'].astype(str).str.startswith('200')) & (hold_inf['当前数量'] > 0) &\
            ((hold_inf['当前数量'] < 100) | ((hold_inf['当前数量'] < 200) & (hold_inf['证券代码'].str.startswith('68')))
             | (~hold_inf['证券代码'].isin(stock_list)))
