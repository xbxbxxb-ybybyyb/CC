...
from ProdWork.CommonTools import excel_saver, ftp_download,ftp_upload,getValueByKeyFromLine,strTime2MDTime,isException,cal_time_delta,trans_str2dic,getMriskFlag,getRejectReason,getMriskInfo, getRiskViolateRemark
class LogParse_Tool:
    ...
    def log_preprocess(self, log_line):
        mock_lines = list(filter(lambda x: 'isMock=1' in str(x), log_line))
        print(f'共有{len(mock_lines)}个实例属于mock票')
        algo_code_set = set()
    ...
    def getAlgoByalgoFromLine(self, line):
        if self.environment == 'prod_test':
            ...
            return line[line.index('StrongStrategy-algo-') + 20:line.index('-n0]')]
    ...
    def getMachineNoFromLine(self,line):
        if  (self.environment == 'UAT_lite') or ('uat_lite' in self.environment):
            ...
            return line[line.index('168.7.') + 6:line.index('-n0]')]
        else:
            #return line[line.index('1') + 1:line.index('-n0]')]
            try:
    ...
    def get_algo_code_dict(self):
        ...
        for line in self.lines:
            try:
                ...
                if ('symbol=' in line) & (algo_code not in algo_code_dict.keys()):
    ...
    def get_machine_code_dict(self):
        ...
        for line in self.lines:
            try:
                algo_code = self.getAlgoCode(line)
                try:
                    stock_code = self.algo_code_dict[algo_code]
    ...
    def split_log_algo(self):
        ...
        for line in self.lines:
            try:
                ...
                if key in self.algo_code_dict.keys():
                    if key not in resultDic:
                        resultDic[key] = [line]
                    else:
                        resultDic[key].append(line)
            except ValueError:
                pass
        return resultDic
    ...
    def get_inf_from_log(self, use_zuhe, zuhe_list):
        ...
        for key, log in self.log_dic.items():
            ...
            if key_code != '601688.SH':
                ...
                for line in log:
                    ...
                    if ('Calculate factors' in line) & ('saturn' not in line)& ('ceres' not in line)&('JupiterNew' not in line): # jupiter因子耗时
                        ...
                        trade = {'TradePrice': float(getValueByKeyFromLine(trade_str, by='price')),
                                 ...
                                 'TradeBSFlag': (getValueByKeyFromLine(trade_str, by='side') == 'Offer') + 1,
                                 'TradeType': int(getValueByKeyFromLine(trade_str, by='type') == 'Canceled'),
                                 'log_ReceiveDateTime': trade_str[11:13] + trade_str[14:16] + trade_str[
                                                                                              17:19] + trade_str[
                                                                                                       20:23]}
                    ...
                    #     print(key_code, 'jupiter001', inf_dic001['MRisk_info'])
                        ...
                        if model_name + '_tot_time_cost' in inf_dic.keys():
                            inf_dic[model_name + '_tot_time_cost'] = inf_dic[
                                                                         model_name + '_tot_time_cost'] + time_cost
                        else:
                        ...
                        inf_dic['filledTradeList'] = getValueByKeyFromLine(line, by='filledTradeList',
                                                                           form='(.*?)[;\n]')
                        inf_dic['lxjjFillList'] = getValueByKeyFromLine(line, by='lxjjFillList', form='(.*?)[;\n]')
                        inf_dic['last5SecFillList'] = getValueByKeyFromLine(line, by='last5SecFillList',
                                                                            form='(.*?)[;\n]')
                        inf_dic['last30SecFillList'] = getValueByKeyFromLine(line, by='last30SecFillList',
                                                                             form='(.*?)[;\n]')
                        inf_dic['last2MinFillList'] = getValueByKeyFromLine(line, by='last2MinFillList',
                                                                            form='(.*?)[;\n]')
                        inf_dic['last5MinFillList'] = getValueByKeyFromLine(line, by='last5MinFillList',
                                                                            form='(.*?)[;\n]')
                    elif ('market data stat info' in line) &('JupiterNew' in line)  :
                        inf_dic001['filledTradeList'] = getValueByKeyFromLine(line, by='filledTradeList',
                                                                           form='(.*?)[;\n]')
                        inf_dic001['lxjjFillList'] = getValueByKeyFromLine(line, by='lxjjFillList', form='(.*?)[;\n]')
                        inf_dic001['last5SecFillList'] = getValueByKeyFromLine(line, by='last5SecFillList',
                                                                            form='(.*?)[;\n]')
                        inf_dic001['last30SecFillList'] = getValueByKeyFromLine(line, by='last30SecFillList',
                                                                             form='(.*?)[;\n]')
                        inf_dic001['last2MinFillList'] = getValueByKeyFromLine(line, by='last2MinFillList',
                                                                            form='(.*?)[;\n]')
                        inf_dic001['last5MinFillList'] = getValueByKeyFromLine(line, by='last5MinFillList',
                                                                            form='(.*?)[;\n]')
                    ########################### saturn930模型信息 ###########################
                ...
                if 'isSkip' not in inf_dic.keys():
                    inf_dic['isSkip'] = 0
                if 'isSkip' not in inf_dic001.keys():
                    inf_dic001['isSkip'] = 0
