...
class LogParse_Tool:
    ...
    def getAlgoByalgoFromLine(self, line):
        if 'test' in self.environment:
            return line[line.index('JupiterStrategy-algo-') + 25:line.index(']')]
            ...
            return re.findall(r"JupiterStrategy-algo-(.*?)]", line)[0]

    def getAlgoCode(self, line):
        if 'test' in self.environment:
            return re.findall(r"JupiterStrategy-algo-(.*)] INFO", line)[0] # line[line.index('StrongStrategy-StrongStrategy-') + 30:line.index('-n0]')]
    ...
    def get_inf_from_log(self):
        ...
        for key, log in self.log_dic.items():
            ...
            if key_code != '601688.SH':
                if key_code == '000066.SZ':
                    print(1)
                inf_dic = {}
                factor_dic_all = {}
                daily_zt_dic = {}
                inf_dic['totalOrderAmt'] = 0 # NOTE: 20250714修改，同一只票可能有多次进入那个循环，需要累加
                for line in log:
                    order_count = 0
                    if 'Calculate factors' in line: # jupiter因子耗时
                        inf_dic['factor_time_cost'] = float(getValueByKeyFromLine(line, by='timeCost'))

                    elif 'Triggered' in line: # jupiter触发信息
                    ...
                    elif ('parentPath' in line) & ('Single model' not in line):
                    ...
                    elif ('Order info' in line) & ('rejected' not in line) & ('placeType=odd_sell' not in line):
                        order_count += 1
                        ...
                        if (quantity > 0) & (action_type != 'JupiterNew') & ((trade_type == 'SplitLastShot') | (trade_type == 'JupiterFirstOrder')|
                                                                             (trade_type == 'MRiskSplitLastShotBuy')| (order_type == 'MRiskSplitShot')|
                                                                             (trade_type == 'FirstSplitLastShot')| (trade_type == 'SecondSplitLastShot')): # 捕捉jupiter的下单
        ...
        return inf_df, factor_df, trade_dic, now_trade_dic, all_code_model_data, daily_zt_df, order_info_df, unfilled_info_df, new_trade_dic