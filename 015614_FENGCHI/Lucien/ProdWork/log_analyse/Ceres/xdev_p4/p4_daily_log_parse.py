...
from ProdWork.log_analyse.Ceres.xdev_p4.LogParse import LogParse
    ...
    for date in s.tradingday(begindate, enddate):
        ...
        for environment in environment_lst:
            ...
            if environment in ['thread', 'xdev', 'xdev2']:
                ...
                # file_name = f'/data/user/013551/forXT/Ceres/log/{nowdate}_type2.log'
                file_name = f'/data/user/015614/shared/for_XT/Ceres/logs/{nowdate}_1-10.log'
            ...
            # 解析日志开始
            inf_df_ceres, trade_dic, now_trade_dic, order_info_df, unfilled_info_df, new_trade_dic, factor_df_ceres, factor_df_ceres930, daily_zt_df_ceres, all_code_model_data_ceres = \
            ...
            tupo_excel_dic = {'每日突破P4': daily_zt_df_ceres,
                              '每日订单': order_info_df,
            ...
            if (environment == 'prod') | (environment == 'SHEX') | (environment =='SZEX'):
                ...
                inf_df_dic = {'因子耗时P4': inf_df_ceres,
                              '买单统计': buy_statistics,
                ...
                # -------------- 生产上才分析未成交的原因 -------------------
                if environment == 'prod':
                    hist_non_trade = pd.read_excel('/data/group/800463/日内强势股/p4_log_parse/因子耗时/因子耗时_%s_%s.xlsx'% (lastdate_h, environment), sheetname='未成交统计')
