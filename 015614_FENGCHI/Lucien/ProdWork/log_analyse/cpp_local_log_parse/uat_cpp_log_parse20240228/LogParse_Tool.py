...
from ProdWork.CommonTools import excel_saver, ftp_download,ftp_upload,getValueByKeyFromLine, getValueByKeyFromLine2,strTime2MDTime,isException,cal_time_delta,trans_str2dic,getMriskFlag,getRejectReason,getMriskInfo, format_unix2dt, format_lastMatchTime2dt, getRiskViolateRemark
class LogParse_Tool:
    ...
    def getAlgoCode(self, line):
        if self.environment == 'prod_test':
            ...
            return re.findall(r"EventDrivenStrategy-algo-(.*)] INFO", line)[0] # line[line.index('StrongStrategy-StrongStrategy-') + 30:line.index('-n0]')]
    ...
    def getMachineNoFromLine(self,line):
        ...
        elif self.environment in ['SHEX', 'SHEX_beta', 'prod']:
            return line[line.index('168.'):line.index(']')]
        ...
        for key, log in self.log_dic.items():
            ...
            if key_code != '601688.SH':
                ...
                for line in log:
                    ...
                    elif ('market data stat info' in line) & ('Metis' in line):
                    ...
                    ########################### ceres930模型信息 ###########################
                        ...
                        inf_dic_pj3_930['p3factor_time_cost'] = float(getValueByKeyFromLine(line, by='timeCost'))
                        ...
                        inf_dic_pj3_930['p3model_time_cost'] = float(getValueByKeyFromLine(line, by='timeCost'))
                    ########################### ceres930模型信息 ###########################
                        ...
                        inf_dic_pj3_931['p3factor_time_cost'] = float(getValueByKeyFromLine(line, by='timeCost'))
                        ...
                        inf_dic_pj3_931['p3model_time_cost'] = float(getValueByKeyFromLine(line, by='timeCost'))
