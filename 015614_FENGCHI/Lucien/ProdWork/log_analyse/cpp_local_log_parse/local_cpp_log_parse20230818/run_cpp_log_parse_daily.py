...
from ProdWork.log_analyse.local_cpp_log_parse20230818.LogParse_Tool import LogParse_Tool
...
if __name__ == "__main__":
    ...
    for date in s.tradingday(begindate, enddate):
        ...
        # environment_lst =['SZEX', 'SHEX', 'SHEX_beta']
        ...
        for environment in environment_lst:
            ...
            if environment == 'test':
                # file_name = '/data/group/800463/xiely/save-file/forFc/log/log_local_20230726.txt'
                # file_name = '/data/group/800463/StrategyLog/sim/EventDrivenCpp-2023-08-17.log.gz'
                # file_name = '/data/group/800463/StrategyLog/sim/EventDrivenStrategy-2023-08-17.log.gz'
                file_name = '/data/group/800463/StrategyLog/sim/EventDrivenCpp-2023-08-22.log.gz'
            print(file_name)
            ...
            if environment == 'prod' and os.path.exists(o45file_name):
                ...
                copyfile(o45file_name,'/data/group/800463/日内强势股/cpp_log/StrongStrategy-%s-SZEX.log.gz' % tradeDate)
                print('from %s to /data/group/800463/日内强势股/cpp_log/StrongStrategy-%s-SZEX.log.gz 上传成功' % (o45file_name, tradeDate))
            else:
                o45_lines = []
            if environment == 'prod' and os.path.exists(o45file_SHname):
                ...
                copyfile(o45file_SHname, '/data/group/800463/日内强势股/cpp_log/StrongStrategy-%s-SHEX.log.gz' % tradeDate)
                print('from %s to /data/group/800463/日内强势股/cpp_log/StrongStrategy-%s-SHEX.log.gz 上传成功' % (o45file_SHname, tradeDate))
            else:
                o45_SHlines = []
            if environment not in  ['prod']:
                g_file = gzip.GzipFile(file_name)   # for gz
                # g_file = open(file_name)  # for txt
                lines = g_file.readlines()
                # lines = list(filter(lambda x: str(x).find("2022-02-14T"), lines))
                # lines = list(filter(lambda x: str(x).startswith('2023-'), lines))
                lines = list(filter(lambda x: str(x).startswith("b'2023-"), lines))
            else:
                lines = []
