...
from ProdWork.CommonTools import excel_saver, cal_time_delta, inttime2str, getAmiBySymbolFromLine
from ProdWork.log_analyse.Saturn.local.LogParse import LogParse
    ...
    for date in s.tradingday(begindate, enddate):
        ...
        for environment in environment_lst:
            ...
            if environment == 'UAT':
                ...
                file_name = f'/data/group/800463/日内强势股/saturn_log/sim/SaturnStrategy-{tradeDate}.log.gz'
            ...
            if environment == 'test':
                ...
                file_name = f'/data/user/013551/forXT/Leda/log_20240408/SaturnStrategy.log'
            if environment == 'thread':
                # file_name = f'/data/user/013551/forXT/log/local_log/eventdriven_log/20231108-xdev38-saturn.log'
                # file_name = f'/data/user/013551/forXT/log/local_log/saturn/saturn_log_xdev_20240119.log'
                # file_name = f'/data/group/800463/xiely/save-file/log/{tradeDate.replace("-", "")}_shsz.log'
                # file_name = f'/data/group/800463/xiely/save-file/forFc/log/{tradeDate.replace("-", "")}_saturn.log'
                # file_name = f'/data/group/800463/xiely/save-file/forFc/log/20240226/saturn_shsz_{tradeDate.replace("-", "")}.log'
                # file_name = f'/data/user/018107/share_file/for_fc/日志/saturn-20240219-sh.log'
                # file_name = f'/data/group/800463/xiely/save-file/forFc/log/saturn_xdev/saturn_20240327.log'
                file_name = f'/data/user/013551/forXT/Saturn/log/20240419/saturn_log_{nowdate}.log'
                file_name = f'/data/group/800463/StrategyLog/sim/sorted/SaturnStrategy-0124.log'
                file_name = f'/data/user/013551/forXT/Saturn/log/20240717/Test5.log'
                file_name = f'/data/user/013551/forXT/Saturn/log/20240911_test/{nowdate}/saturn-incomp.log'
                file_name = f'/data/user/013551/forXT/Saturn/log/20240918_test/{nowdate}-new.log'
            ...
            if environment == 'prod' and os.path.exists(sz_log_fpath):
                ...
                copyfile(sz_log_fpath, '/data/group/800463/日内强势股/saturn_log/SZEX.SaturnStrategy-%s.log.gz' % tradeDate)
                print(f'from {sz_log_fpath} to /data/group/800463/日内强势股/saturn_log/SZEX.SaturnStrategy-{tradeDate}.log.gz 上传成功')
            else:
                sz_lines = []
            ...
            if environment == 'prod' and os.path.exists(sh_log_fpath):
                ...
                copyfile(sh_log_fpath, f'/data/group/800463/日内强势股/saturn_log/SHEX.SaturnStrategy-{tradeDate}.log.gz')
                print(f'from {sh_log_fpath} to /data/group/800463/日内强势股/saturn_log/SHEX.SaturnStrategy-{tradeDate}.log.gz 上传成功')
            else:
                sh_lines = []
            ...
            #     copyfile(file_name, '/data/group/800463/日内强势股/saturn_log/sim/SaturnStrategy-%s.log.gz' % tradeDate)
            #     print('from %s to /data/group/800463/日内强势股/saturn_log/sim/SaturnStrategy-%s.log.gz 上传成功' % (file_name, tradeDate))
            ...
            if environment not in ['prod']:
                if file_name.endswith('gz'):
                    ...
                    # other_lines = list(filter(lambda x: str(x).startswith(f"[20") and 'info' in str(x), other_lines))
                    # other_lines = list(filter(lambda x: 'INFO' in str(x), other_lines))
                    other_lines = list(filter(lambda x: 'INFO' in str(x), other_lines))
            ...
            elif environment in ['UAT', 'night', 'test', 'thread', 'thread2']:
                inf_df_dic = {'因子耗时Saturn': inf_df_saturn,
