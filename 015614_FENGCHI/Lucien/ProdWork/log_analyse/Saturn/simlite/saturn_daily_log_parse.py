...
if __name__ == "__main__":
    ...
    for date in s.tradingday(begindate, enddate):
        ...
        for environment in environment_lst:
            ...
            if environment == 'simlite':
                file_name = f'/data/group/800463/sim-lite-log/20240201/SaturnStrategy-20240131210900-587-0-00000000.log'
            print(file_name)
            ...
            if environment not in ['prod']:
                if file_name.endswith('gz'):
                    ...
                    other_lines = list(filter(lambda x: str(x).startswith("2024-01-31T") and 'INFO' in str(x), other_lines))
                    other_lines = list(filter(lambda x: 'SaturnStrategy' in str(x), other_lines))
                # other_lines = other_file.readlines()
                # other_lines = list(filter(lambda x: str(x).startswith("2024-"), other_lines))
            ...
            elif environment in ['UAT', 'night', 'test', 'thread', 'simlite']:
                inf_df_dic = {'因子耗时Saturn': inf_df_saturn,
