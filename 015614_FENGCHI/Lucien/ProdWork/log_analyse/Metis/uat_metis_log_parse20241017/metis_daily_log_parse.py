...
from ProdWork.log_analyse.Metis.uat_metis_log_parse20241017.LogParse import LogParse
    ...
    for date in s.tradingday(begindate, enddate):
        ...
        for environment in environment_lst:
            ...
            if environment == 'UAT':
                ...
                file_name = f'/data/group/800463/StrategyLog/sim/MetisStrategy-2024-10-26.log.gz'
                file_name = f'/data/group/800463/StrategyLog/prd/MetisStrategy-168.26.5.67.log.gz'
                file_name = f'/data/group/800463/StrategyLog/prd/MetisStrategy-20250218-168-26-5-67.log.gz'
            ...
            if environment == 'test':
                ...
                file_name = '/data/user/013551/forXT/Metis/logs/20241017/20241009/20241009_metis.log'
            ...
            if environment not in  ['prod']:
                if file_name.endswith('gz'):
                    ...
                    other_file = open(file_name, encoding='utf-8')
                #     other_lines = other_file.readlines()
                #     other_lines = list(filter(lambda x: str(x).startswith("[2024-") and 'INFO' in str(x), other_lines))
                #     other_lines = list(filter(lambda x: 'thread' in str(x), other_lines))
                other_lines = other_file.readlines()
                other_lines = list(filter(lambda x: str(x).startswith("b'2025-"), other_lines))  # UAT
                # other_lines = list(filter(lambda x: str(x).startswith("[2024-") and 'info' in str(x), other_lines))
                # other_lines = list(filter(lambda x: 'thread' in str(x), other_lines))
            else:
                other_lines = []
            ...
            if (environment == 'prod') | (environment == 'SHEX') | (environment =='SZEX'):
                ...
                # -------------- 生产上才分析未成交的原因 -------------------
                if environment == 'prod':
                    hist_non_trade = pd.read_excel('/data/group/800463/日内强势股/metis_log_parse/因子耗时/因子耗时_%s_%s.xlsx'% (lastdate_h, environment), sheetname='未成交统计')
