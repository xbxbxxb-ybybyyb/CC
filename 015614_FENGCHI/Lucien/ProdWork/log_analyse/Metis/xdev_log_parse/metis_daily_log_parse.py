...
from ProdWork.log_analyse.Metis.xdev_log_parse.LogParse import LogParse
    ...
    for date in s.tradingday(begindate, enddate):
        ...
        for environment in environment_lst:
            ...
            if environment == 'test':
                ...
                file_name = f'/data/user/013551/forXT/Metis/logs/20241023/metis_log_{tradeDatestr}.log'
            ...
            if environment not in  ['prod']:
                if file_name.endswith('gz'):
                    ...
                    other_lines = list(filter(lambda x: 'thread' in str(x) and 'info' in str(x), other_lines))
                    # other_lines = list(filter(lambda x: 'thread' in str(x), other_lines))
                # other_lines = other_file.readlines()
                # other_lines = list(filter(lambda x: str(x).startswith("b'2023-"), other_lines))
                # other_lines = list(filter(lambda x: 'thread' in str(x), other_lines))
            else:
                other_lines = []
