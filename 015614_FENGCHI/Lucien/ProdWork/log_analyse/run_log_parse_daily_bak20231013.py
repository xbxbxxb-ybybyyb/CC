...
from ProdWork.CommonTools import excel_saver, ftp_download,ftp_upload,cal_time_delta,inttime2str
from ProdWork.log_analyse.LogParse_Tool import LogParse_Tool
...
if __name__ == "__main__":
    ...
    for date in s.tradingday(begindate, enddate):
        ...
        environment_lst = ['prod', 'SZEX', 'SHEX', 'UAT', 'UAT_49_53', 'UAT_50_51']  # ,'UAT_75_77_78','UAT_79_81_82','UAT_50_51_83','UAT_lite','UAT_other']# ['prod','UAT','night','test1','SZEX','SHEX','UAT_49_53','UAT_50_51','UAT_lite','UAT_other']#[:1]#
        ...
        for environment in environment_lst: #[:2] + environment_lst[4:]:#['night']:#['UAT','UAT_other']:#['night','night_other']:#['night_origin3']:#['night_origin3','night_50_51','night_49_53']:#['UAT_lite']:#
            ...
            if environment == 'prod' and os.path.exists(o45file_name):
                ...
                o45_lines = list(filter(lambda x: str(x).startswith("b'2023-"), o45_lines))
            ...
            if environment == 'prod' and os.path.exists(o45file_SHname):
                ...
                o45_SHlines = list(filter(lambda x: str(x).startswith("b'2023-"), o45_SHlines))
            ...
            if environment not in  ['prod']:
                ...
                lines = list(filter(lambda x: str(x).startswith("b'2023-"), lines)) # TODO: 2024年要更换一下吧?
                # lines = list(filter(lambda x: str(x).startswith('2023-'), lines)) # TODO: 2024年要更换一下吧?
            else:
                lines = []
            ...
            for code, log in logparse.log_dic.items():
                if code in logparse.algo_code_dict.keys():
                    ...
                    with open(log_split_file_path + '%s-%s-%s环境.txt' % (tradeDatestr, code_code, environment), 'w') as file_handle:
                        for line in log:
            ...
            if (environment == 'prod') | (environment == 'SHEX') | (environment in [ 'SZEX','SZEX_udp']):
                ...
                # -------------- 生产上才分析未成交的原因 -------------------
                if environment == 'prod':
                    hist_non_trade = pd.read_excel('/data/group/800463/日内强势股/java_log_parse/因子耗时/因子耗时_%s_%s.xlsx'% (lastdate_h, environment),sheetname = '未成交统计')
