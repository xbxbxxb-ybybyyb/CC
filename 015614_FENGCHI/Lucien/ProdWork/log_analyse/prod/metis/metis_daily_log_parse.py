...
from ProdWork.log_analyse.prod.metis.LogParse import LogParse
...
if __name__ == "__main__":
    ...
    for date in s.tradingday(begindate, enddate):
        ...
        for environment in environment_lst:
            ...
            if environment == 'UAT':
                file_list = list(sorted(list(filter(lambda x: f'{tradeDate}' in x and 'MetisStrategy' in x, os.listdir('/data/group/800463/StrategyLog/xsim/')))))
                file_name = '/data/group/800463/StrategyLog/xsim/' + file_list[0]
                # file_name = f'/data/group/800463/StrategyLog/sim/MetisStrategy-{tradeDate}.log.gz'
                # file_name = f'/data/group/800463/StrategyLog/sim/log_bak/MetisStrategy-2024-10-19.log.gz'
            if environment == 'night':
                file_list = list(sorted(list(filter(lambda x: f'{tradeDate}' in x and 'MetisStrategy' in x, os.listdir('/data/group/800463/StrategyLog/xsim/')))))
                file_name = '/data/group/800463/StrategyLog/xsim/' + file_list[-1]
                # file_name = f'/data/group/800463/StrategyLog/sim/MetisStrategy-{tradeDate}.log.gz'
                # file_name = f'/data/group/800463/StrategyLog/sim/log_bak/MetisStrategy-2024-10-12.log.gz'
            if environment == 'SHEX':
            ...
            if environment == 'test':
                ...
                file_name = f'/data/group/800463/日内强势股/metis_log/sim/EventDrivenStrategy-{tradeDate}.gz'
            if environment in ['thread', 'xdev']:
                # file_name = f'/data/user/013551/forXT/log/local_log/eventdriven_log/20231108-xdev38-metis.log'
                file_name = f'/data/user/013551/forXT/Metis/logs/dev_log/metis/20250804/{date}/metis_{date}.log'
            print(file_name)
            ...
            sz_log_fpath = f'/data/group/800463/StrategyLog/prd/SZEX.MetisStrategy-{tradeDate}.log.gz'
            sh_log_fpath = f'/data/group/800463/StrategyLog/prd/SHEX.MetisStrategy-{tradeDate}.log.gz'
            if environment == 'prod' and os.path.exists(sz_log_fpath):
                sz_file = gzip.GzipFile(sz_log_fpath)
                sz_lines = sz_file.readlines()
                # copyfile(sz_log_fpath, '/data/group/800463/日内强势股/metis_log/SZEX.MetisStrategy-%s.log.gz' % tradeDate)
                # print(f'from {sz_log_fpath} to /data/group/800463/日内强势股/metis_log/SZEX.MetisStrategy-{tradeDate}.log.gz 上传成功')
            else:
                sz_lines = []
            ...
            if environment == 'prod' and os.path.exists(sh_log_fpath):
                ...
                # copyfile(sh_log_fpath, f'/data/group/800463/日内强势股/metis_log/SHEX.MetisStrategy-{tradeDate}.log.gz')
                # print(f'from {sh_log_fpath} to /data/group/800463/日内强势股/metis_log/SHEX.MetisStrategy-{tradeDate}.log.gz 上传成功')
            else:
                sh_lines = []
            ...
            if environment == 'UAT' and os.path.exists(file_name):
                ...
                # copyfile(file_name, '/data/group/800463/日内强势股/metis_log/sim/MetisStrategy-%s.log.gz' % tradeDate)
                # print('from %s to /data/group/800463/日内强势股/metis_log/sim/MetisStrategy-%s.log.gz 上传成功' % (file_name, tradeDate))
            ...
            # for line in lines:
            #     if ('Model prediction' in str(line)) & ('parentPath' not in str(line)) & ('Metis' in str(line)):
            #         print(1)
            lp_inst = LogParse(environment, tradeDatestr, lines)
            print('日志数据读取完毕\n', len(lp_inst.lines))

            # 拆分日志
            log_split_path = f'/data/group/800463/日内强势股/metis_log_parse/日志拆分/{tradeDatestr}_{environment}环境/'
            ...
            # 解析日志开始
            inf_df_metis, trade_dic, now_trade_dic, order_info_df, unfilled_info_df, new_trade_dic, factor_df_metis, daily_zt_df_metis, all_code_model_data_metis = \
                lp_inst.start_parsing()

            except_df = lp_inst.get_except_df()
            save_index_metis = list(set(factor_df_metis.index.tolist()))

            # TODO: 因为没有isSkip和isMock两个key，先注释掉以下4行
            inf_df_metis = inf_df_metis.copy()
            ...
            # daily_zt_df_metis = daily_zt_df_metis.drop_duplicates([])

            factor_df_metis.loc[save_index_metis].sort_index().to_excel('/data/group/800463/日内强势股/metis_log_parse/因子数据/因子数据Metis_%s_%s.xlsx' % (tradeDate, environment))
            print('create file %s' % '/data/group/800463/日内强势股/metis_log_parse/因子数据/因子数据Metis_%s_%s.xlsx' % (tradeDate, environment))

            if len(except_df) > 0:
                except_df.to_excel('/data/group/800463/日内强势股/metis_log_parse/异常报错/异常报错_%s_%s.xlsx' % (tradeDate, environment))
                print('create file %s' % '/data/group/800463/日内强势股/metis_log_parse/异常报错/异常报错_%s_%s.xlsx' % (tradeDate, environment))
            md_data_path = '/data/group/800463/日内强势股/metis_log_parse/行情数据/%s_%s/' % (tradeDate, environment)
            model_factor_data_path = '/data/group/800463/日内强势股/metis_log_parse/模型数据/%s_%s/' % (tradeDate, environment)
            ...
            if 'ZT_Time' in inf_df_metis.columns.tolist():
                ...
                # 保证取set的时候顺序一致
                inf_df_metis = inf_df_metis.sort_index()
                daily_zt_df_metis = daily_zt_df_metis.sort_index()
                daily_zt_df_metis['ZT_Time'] = inf_df_metis['ZT_Time'].loc[list(sorted(set(daily_zt_df_metis.index)))].values
            inf_df_metis['machine_code'] = pd.Series(lp_inst.machine_code_dict).loc[inf_df_metis.index].values

            tupo_excel_dic = {'每日突破Metis': daily_zt_df_metis,
                              '每日订单': order_info_df,
                              '每日拒绝': unfilled_info_df}
            excel_saver(tupo_excel_dic, '/data/group/800463/日内强势股/metis_log_parse/每日突破/每日突破_%s_%s.xlsx' % (tradeDatestr, environment))
            ...
            inf_df_dic = {'因子耗时Metis': inf_df_metis,
                          '机器统计': pd.Series(lp_inst.machine_counter_dict)}

            excel_saver(inf_df_dic, '/data/group/800463/日内强势股/metis_log_parse/因子耗时/因子耗时_%s_%s.xlsx' % (tradeDate, environment))
            if environment == 'prod':
                from dataApi.sendInfo import send_message
                send_message(f'{tradeDate} 实盘metis日志已解析')