...
from ProdWork.log_analyse.prod.ceres.LogParse import LogParse
...
if __name__ == "__main__":
    ...
    for date in s.tradingday(begindate, enddate):
        ...
        for environment in environment_lst:
            ...
            daily_tupo = pd.read_excel('/data/group/800463/日内强势股/ceres_log_parse/每日突破/每日突破_%s_%s.xlsx' % (tradeDatestr, environment), sheet_name='每日目标', index_col=0)
            daily_tupo = daily_tupo.set_index('index')
            inf_df_ceres['target_amt'] = inf_df_ceres.index.map(lambda x: daily_tupo.loc[x, 'target_amt'].max() if x in daily_tupo.index else 0)
