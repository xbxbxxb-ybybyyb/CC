...
class LogParse:
    ...
    def filter_triggered_line(self, log_line):
        ...
        this_type_stocks_list = list(map(lambda x: getValueByKeyFromLine2(str(x), by='symbol'), this_type_stocks_lines))
    ...
    def start_parsing(self):
        ...
        for key, log in self.log_dic.items():
            ...
            if key_code != '601688.SH':
                ...
                # # %% 临时计算正确的quantity
                # place_line = filter(lambda x: 'NewOrderPlace' in str(x), log)
                # # place_line =
                ...
                for line in log:
                    ...
                    if ('Order info' in line) & ('rejected' not in line):  #  触发并准备下单
                        ...
                        if len(action_type) == 0: action_type = 'Ceres'
                        ...
                        if (quantity > 0) & (action_type != 'JupiterNew') & (trade_type == 'A'): # 捕捉jupiter的下单
                            ...
                            daily_zt_ceres_dic['totalOrderAmt'] = float(daily_zt_ceres_dic['quantity'])*float(daily_zt_ceres_dic['price'])
