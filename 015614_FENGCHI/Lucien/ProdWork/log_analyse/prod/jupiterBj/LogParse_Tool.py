...
class LogParse_Tool:
    ...
    def getAlgoByalgoFromLine(self, line):
        if 'test' in self.environment:
            return line[line.index('algo-') + 25:line.index(']')]
    ...
    def getAlgoCode(self, line):
        if 'test' in self.environment:
            return re.findall(r"algo-(.*)] INFO", line)[0] # line[line.index('StrongStrategy-StrongStrategy-') + 30:line.index('-n0]')]
