...
from ProdWork.log_analyse.prod.jupiterBj.LogParse_Tool import LogParse_Tool
...
if __name__ == "__main__":
    ...
    for date in s.tradingday(begindate, enddate):
        ...
        for environment in environment_lst:
            ...
            # ------------ 按环境加载日志的目录 ---------------
            import gzip
            file_name = f'/data/group/800463/StrategyLog/prd/{environment}.JupiterStrategy_BJ-{tradeDate}.log.gz'
            if environment == 'UAT':
                file_list = list(sorted(list(filter(lambda x: f'{tradeDate}' in x and 'JupiterStrategy_BJ' in x, os.listdir('/data/group/800463/StrategyLog/xsim/')))))
            ...
            o45file_name = '/data/group/800463/StrategyLog/prd/SZEX.JupiterStrategy_BJ-' + tradeDate + '.log.gz' # '/data/group/800463/日内强势股/log/StrongStrategy-%s-SZEX.log.gz'#
            # o45file_name = '/data/group/800463/xiely/日内强势股/log/JupiterStrategy-' + tradeDate + '-SZEX.log.gz'
            o45file_SHname = '/data/group/800463/StrategyLog/prd/SHEX.JupiterStrategy_BJ-' + tradeDate + '.log.gz'
            ...
            lines = list(filter(lambda x: str(x).startswith("b'2025-"), lines))  # TODO: 2026年要更换一下吧?

            logparse = LogParse_Tool(environment, tradeDatestr, lines)
            print('日志数据读取完毕\n', len(logparse.lines))

            # NOTE: 由于jupiterBj策略经常几天没有一个触发样本，所以这里直接采用没有触发就保存空的方式
            # 拆分日志
            ...
            if len(inf_df) == 0:
                inf_df = pd.DataFrame(columns=inf_df.columns.tolist() + ['买入时形态', 'shouldBuySignal', 'ZT_Time'])
            ...
            if environment == 'prod':
                ...
                send_message(f'{tradeDate} 实盘jupiterBj日志已解析')