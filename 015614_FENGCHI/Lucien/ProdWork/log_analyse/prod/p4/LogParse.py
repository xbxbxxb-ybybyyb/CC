...
class LogParse:
    ...
    def getSymbolFromStartSuccessLine(line):
        return line[line.index('P4Strategy-CPP for ') + 29 : line.index(' Version')]
    ...
    def filter_triggered_line(self, log_line):
        ...
        #　筛选type==1的标签
        this_type_stocks_lines = list(filter(lambda x: 'sample_type=2' in str(x), log_line))
        this_type_stocks_list = list(map(lambda x: getValueByKeyFromLine2(str(x), by='symbol'), this_type_stocks_lines))    # 耗时长

        algo_code_set = set()
        new_lines = list(filter(lambda x: getValueByKeyFromLine2(str(x), by='symbol') in this_type_stocks_list, log_line))
    ...
    def start_parsing(self):
            ...
            if key_code != '601688.SH':
                ...
                for line in log:
                    ...
                    if ('calculate factors' in line) & ('ceresKey=931' in line):
                        inf_dic_p4['factor_time_cost'] = float(getValueByKeyFromLine(line, by='timeCost'))
                    if ('calculate factors' in line) & ('ceresKey=930' in line):
                        inf_dic_p4930['factor_time_cost'] = float(getValueByKeyFromLine(line, by='timeCost'))

                    if 'model prediction' in line:
                        # inf_dic_p4['ZT_Time'] = int(float(getValueByKeyFromLine(line, by='reachedZTTime')))
                        inf_dic_p4['ZT_Time'] = 93100000

                    if ('total unscaled tday-factors' in line) & ('ceresKey=931' in line):
                        factor_dic_p4 = trans_str2dic(line[line.index('{'): line.index(', }')] + '}')
                    if ('ceres factor' in line) & ('ceresKey=930' in line):
                        factor_dic_p4930 = trans_str2dic(line[line.index('{'): line.index(', }')] + '}')
                    ...
                    if ('ceres model prediction' in line) & ('parentPath' not in line):
                        if 'shouldBuy=' in line:
                            inf_dic_p4['shouldBuySignal'] = bool(int(getValueByKeyFromLine(line, by='shouldBuy')))
                        else:
                            inf_dic_p4['shouldBuySignal'] = bool(getValueByKeyFromLine(line, by='shouldBuySignal'))   # 这一行其实没用
                        if 'cost=' in line:
                            inf_dic_p4['model_time_cost'] = float(getValueByKeyFromLine(line, by='cost'))
                        else:
                            inf_dic_p4['model_time_cost'] = float(getValueByKeyFromLine(line, by='timeCost'))

                    if ('riskSummary' in line) & ('拉抬打压' in line):
                        inf_dic_p4['MRisk_info'] = '触发拉抬打压'

                    if ('predict sum_signals' in line) & ('parentPath' not in line):
                        inf_dic_p4['sum_signals'] = getValueByKeyFromLine(line, by='predict sum_signals')

                    if ('parentPath' in line) & ('probability' in line):
                        model_name = getValueByKeyFromLine(line, by='parentPath')
                        if model_name not in all_code_model_data_p4.keys():
                            all_code_model_data_p4[model_name] = {}
                        code = getValueByKeyFromLine(line, by='symbol')
                        inf_dic_p4[model_name + '_probability'] = getValueByKeyFromLine(line, by='probability')
                    ...
                    if ('Order info' in line) & ('rejected' not in line):  #  触发并准备下单
                        ...
                        if len(action_type) == 0: action_type = 'P4'
                        ...
                        if (quantity > 0) & (action_type != 'JupiterNew') & (trade_type=='A'): # 捕捉jupiter的下单
                            daily_zt_p4_dic['order_direction'] = trade_type
                            daily_zt_p4_dic['quantity'] = getValueByKeyFromLine(line, by='quantity')
                            daily_zt_p4_dic['targetAmt'] = getValueByKeyFromLine(line, by='targetAmt')
                            daily_zt_p4_dic['nowPrice'] = getValueByKeyFromLine(line, by='nowPrice')
                            daily_zt_p4_dic['highLimitPrice'] = getValueByKeyFromLine(line, by='highLimitPrice')
                            daily_zt_p4_dic['price'] = getValueByKeyFromLine(line, by='price')
                            # daily_zt_p4_dic['splitOrderNum'] = getValueByKeyFromLine(line, by='splitOrderNum')
                            daily_zt_p4_dic['totalOrderAmt'] = float(daily_zt_p4_dic['quantity'])*float(daily_zt_p4_dic['price'])
                            inf_dic_p4['totalOrderAmt'] = daily_zt_p4_dic['totalOrderAmt']
                            daily_zt_ser = pd.Series(daily_zt_p4_dic, name=key_code)
                            daily_zt_p4_list.append(daily_zt_ser)
                    ...
                    if ('market data stat info' in line) & ('Metis' in line):   # 计算因子使用到的行情条数，原来是对比因子不一致时使用
                        inf_dic_p4['filledTradeList'] = getValueByKeyFromLine(line, by='filledTradeList', form='(.*?)[;\n]')
                        inf_dic_p4['lxjjFillList'] = getValueByKeyFromLine(line, by='lxjjFillList', form='(.*?)[;\n]')
                        inf_dic_p4['last5SecFillList'] = getValueByKeyFromLine(line, by='last5SecFillList', form='(.*?)[;\n]')
                        inf_dic_p4['last30SecFillList'] = getValueByKeyFromLine(line, by='last30SecFillList', form='(.*?)[;\n]')
                        inf_dic_p4['last2MinFillList'] = getValueByKeyFromLine(line, by='last2MinFillList', form='(.*?)[;\n]')
                        inf_dic_p4['last5MinFillList'] = getValueByKeyFromLine(line, by='last5MinFillList', form='(.*?)[;\n]')
                ...
                if len(inf_dic_p4) != 0:
                    inf_ser_p4 = pd.Series(inf_dic_p4, name=key_code)
                    inf_list_p4.append(inf_ser_p4)

                if len(factor_dic_p4) != 0:
                    factor_ser_p4 = pd.Series(factor_dic_p4, name=key_code)
                    factor_list_p4.append(factor_ser_p4)
                if len(factor_dic_p4930) != 0:
                    factor_ser_p4930 = pd.Series(factor_dic_p4930, name=key_code)
                    factor_list_p4930.append(factor_ser_p4930)

        if len(inf_list_p4) == 0:
            print('没有p4样本！！！')
            inf_df_p4 = pd.DataFrame()
        else:
            inf_df_p4 = pd.concat(inf_list_p4, axis=1).T.sort_index()
            if 'MRisk_info' in inf_df_p4.columns:
                inf_df_p4_mrisk_info = inf_df_p4[~inf_df_p4['MRisk_info'].isna()]
                inf_df_p4_mrisk_info = inf_df_p4_mrisk_info.reset_index().drop_duplicates('index').set_index('index')
                inf_df_p4.loc[inf_df_p4_mrisk_info.index, 'MRisk_info'] = inf_df_p4_mrisk_info.loc[inf_df_p4_mrisk_info.index, 'MRisk_info']
            if 'ZT_Time' in inf_df_p4.columns:
                inf_df_p4 = inf_df_p4.loc[inf_df_p4['ZT_Time'].notnull()]
                inf_df_p4 = inf_df_p4.query('ZT_Time > 0')
            else:
                inf_df_p4 = pd.DataFrame()


        if len(factor_list_p4)==0:
            factor_df_p4 = pd.DataFrame()
        else:
            factor_df_p4 = pd.concat(factor_list_p4, axis=1).T.sort_index()
        if len(factor_list_p4930)==0:
            factor_df_p4930 = pd.DataFrame()
        else:
            factor_df_p4930 = pd.concat(factor_list_p4930, axis=1).T.sort_index()

        if len(daily_zt_p4_list) != 0:
            daily_zt_df_p4 = pd.concat(daily_zt_p4_list, axis=1).T.sort_index()
        else:
            daily_zt_df_p4 = pd.DataFrame()
        ...
        return inf_df_p4, trade_dic, now_trade_dic, order_info_df, unfilled_info_df, new_trade_dic, factor_df_p4, factor_df_p4930, daily_zt_df_p4, all_code_model_data_p4