...
from ProdWork.log_analyse.prod.jupiter.LogParse_Tool import LogParse_Tool
    ...
    for date in s.tradingday(begindate, enddate):
        ...
        for environment in environment_lst:
            ...
            if environment == 'UAT':
                ...
                # file_name = f'/data/group/800463/StrategyLog/sim/JupiterStrategy-{tradeDate}.log.gz'
                # file_name = '/data/group/800463/StrategyLog/xsim/JupiterStrategy-2025-07-03-20250703090942-604-0-00000000.log.gz'
                # file_name = f'/data/group/800463/StrategyLog/sim/log_bak/JupiterStrategy-2024-10-19.log.gz'
            ...
            if 'night' in environment:
                ...
                # file_name = '/data/group/800463/StrategyLog/sim/JupiterStrategy-%s.log.gz' % tradeDate
                # file_name = '/data/group/800463/StrategyLog/sim/log_bak/JupiterStrategy-2024-10-12.log.gz'
                # file_name = '/data/group/800463/StrategyLog/xsim/JupiterStrategy-2025-07-03-20250703090942-604-0-00000000.log.gz'
                file_list = list(sorted(list(filter(lambda x: f'{tradeDate}' in x and 'JupiterStrategy' in x and 'BJ' not in x, os.listdir('/data/group/800463/StrategyLog/xsim/')))))
            ...
            if environment == 'prod' and os.path.exists(o45file_SHname):
                ...
                # print('from %s to /data/group/800463/日内强势股/log/SHEX.JupiterStrategy-%s.log.gz 上传成功' % (o45file_SHname, tradeDate))
            else:
                o45_SHlines = []
            ...
            lines = list(filter(lambda x: str(x).startswith("b'2025-"), lines))  # TODO: 2025年要更换一下吧?
            ...
            factor_df.loc[save_index].sort_index().to_excel('/data/group/800463/日内强势股/jupiter_log_parse/因子数据/因子数据_%s_%s.xlsx' % (tradeDate, environment))
            ...
            if (environment == 'prod') | (environment == 'SHEX') | (environment in [ 'SZEX','SZEX_udp', 'SHEX_beta']):
                ...
                # -------------- 生产上才分析未成交的原因 -------------------
                if environment == 'prod':
                    hist_non_trade = pd.read_excel('/data/group/800463/日内强势股/jupiter_log_parse/因子耗时/因子耗时_%s_%s.xlsx'% (lastdate_h, environment),sheetname = '未成交统计')
