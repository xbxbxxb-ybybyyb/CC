...
from ProdWork.log_analyse.prod.saturn.LogParse import LogParse
...
if __name__ == "__main__":
    ...
    for date in s.tradingday(begindate, enddate):
        ...
        for environment in environment_lst:
            ...
            if environment == 'UAT':
                file_list = list(sorted(list(filter(lambda x: f'{tradeDate}' in x and 'SaturnStrategy' in x, os.listdir('/data/group/800463/StrategyLog/xsim/')))))
                file_name = '/data/group/800463/StrategyLog/xsim/' + file_list[0]
                # file_name = f'/data/group/800463/StrategyLog/sim/SaturnStrategy-{tradeDate}.log.gz'
                # file_name = f'/data/group/800463/StrategyLog/sim/log_bak/SaturnStrategy-2024-10-19.log.gz'
                # file_name = f'/data/group/800463/StrategyLog/sim/sorted/SaturnStrategy-{tradeDate}.log.gz'
                # file_name = f'/data/group/800463/日内强势股/log/sim/SaturnStrategy-{tradeDate}.log.gz'
            if environment.__contains__('night'):
                # file_name = f'/data/group/800463/StrategyLog/sim/SaturnStrategy-{tradeDate}.log.gz'
                # file_name = f'/data/group/800463/StrategyLog/sim/log_bak/SaturnStrategy-2024-10-12.log.gz'
                # file_name = f'/data/group/800463/StrategyLog/sim/SaturnStrategy-2025-03-05.log.gz'
                file_list = list(sorted(list(filter(lambda x: f'{tradeDate}' in x and 'SaturnStrategy' in x, os.listdir('/data/group/800463/StrategyLog/xsim/')))))
                file_name = '/data/group/800463/StrategyLog/xsim/' + file_list[-1]
            if environment == 'test':
                file_name = '/data/user/015614/shared/for_XT/Ceres/logs/20250811/saturn-20250701.log'
                # file_name = f'/data/group/800463/日内强势股/saturn_log/sim/EventDrivenStrategy-{tradeDate}.gz'
            if environment == 'thread':
                file_name = f'/data/user/013551/forXT/log/local_log/eventdriven_log/20231108-xdev38-saturn.log'
            print(file_name)
            ...
            sz_log_fpath = f'/data/group/800463/StrategyLog/prd/SZEX.SaturnStrategy-{tradeDate}.log.gz'
            sh_log_fpath = f'/data/group/800463/StrategyLog/prd/SHEX.SaturnStrategy-{tradeDate}.log.gz'
            if environment == 'prod' and os.path.exists(sz_log_fpath):
                sz_file = gzip.GzipFile(sz_log_fpath)
                sz_lines = sz_file.readlines()
                # copyfile(sz_log_fpath, '/data/group/800463/日内强势股/log/SZEX.SaturnStrategy-%s.log.gz' % tradeDate)
                # print(f'from {sz_log_fpath} to /data/group/800463/日内强势股/log/SZEX.SaturnStrategy-{tradeDate}.log.gz 上传成功')
            else:
                sz_lines = []
            ...
            if environment == 'prod' and os.path.exists(sh_log_fpath):
                ...
                # copyfile(sh_log_fpath, f'/data/group/800463/日内强势股/log/SHEX.SaturnStrategy-{tradeDate}.log.gz')
                # print(f'from {sh_log_fpath} to /data/group/800463/日内强势股/log/SHEX.SaturnStrategy-{tradeDate}.log.gz 上传成功')
            else:
                sh_lines = []

            if environment in ['UAT', 'night'] and os.path.exists(file_name):
                uat_file = gzip.GzipFile(file_name)
                uat_lines = uat_file.readlines()
                # if environment == 'UAT':
                #     copyfile(file_name, '/data/group/800463/日内强势股/log/sim/SaturnStrategy-%s.log.gz' % tradeDate)
                # elif environment == 'night':
                #     copyfile(file_name, '/data/group/800463/日内强势股/log/sim/SaturnStrategy-%s-night.log.gz' % tradeDate)
                # print('from %s to /data/group/800463/日内强势股/log/sim/SaturnStrategy-%s.log.gz 上传成功' % (file_name, tradeDate))
            ...
            if environment not in ['prod']:
                if file_name.endswith('gz'):
                    ...
                    other_lines = other_file.readlines()
                    # other_lines = list(filter(lambda x: str(x).startswith("2025-08-11T") and 'INFO' in str(x), other_lines))
                    # other_lines = list(filter(lambda x: 'thread' in str(x), other_lines))
                # other_file = open(file_name)
                # other_lines = other_file.readlines()
                other_lines = list(filter(lambda x: str(x).startswith("b'2025-"), other_lines))
                # other_lines = list(filter(lambda x: str(x).startswith("2025-"), other_lines))
                # other_lines = list(filter(lambda x: 'thread' in str(x), other_lines))
            else:
                other_lines = []
            ...
            # 查看缺失哪部分日志
            # for line in lines:
            #     line_str = bytes.decode(line, errors='ignore')
            #     # if '603928.SH' in line_str and 'probability' in line_str:
            #     if '603928.SH' in line_str:
            #         print(line_str)
            lp_inst = LogParse(environment, tradeDatestr, lines)
            print('日志数据读取完毕\n', len(lp_inst.lines))

            # 拆分日志
            log_split_path = f'/data/group/800463/日内强势股/saturn_log_parse/日志拆分/{tradeDatestr}_{environment}环境/'
            ...
            # 解析日志开始
            inf_df_saturn, trade_dic, now_trade_dic, order_info_df, unfilled_info_df, new_trade_dic, factor_df_saturn, factor_df_saturn930, daily_zt_df_saturn, all_code_model_data_saturn = \
                lp_inst.start_parsing()

            spec_col = list(filter(lambda x: x.endswith('_probability'), inf_df_saturn.columns.tolist()))[0]
            inf_df_saturn = inf_df_saturn.reset_index().loc[inf_df_saturn.reset_index()[spec_col].dropna().index].set_index('index')
            inf_df_saturn.index.names = ['Unnamed: 0']  # 为了和之前java统一

            except_df = lp_inst.get_except_df()
            save_index_saturn = list(set(factor_df_saturn.index.tolist()))

            # TODO: 因为没有isSkip和isMock两个key，先注释掉以下4行
            inf_df_saturn = inf_df_saturn.copy()
            inf_df_saturn = inf_df_saturn.loc[list(set(inf_df_saturn.index.tolist()) & set(save_index_saturn))].sort_index()
            save_index_saturn = inf_df_saturn.index.tolist()
            daily_zt_df_saturn = daily_zt_df_saturn.loc[list(set(daily_zt_df_saturn.index.tolist()) & set(save_index_saturn))].sort_index()

            excel_saver({'930': factor_df_saturn930.loc[save_index_saturn].sort_index(),
                         '931': factor_df_saturn.loc[save_index_saturn].sort_index()},
                        '/data/group/800463/日内强势股/saturn_log_parse/因子数据/因子数据Saturn_%s_%s.xlsx' % (tradeDate, environment))
            print('create file %s' % '/data/group/800463/日内强势股/saturn_log_parse/因子数据/因子数据Saturn_%s_%s.xlsx' % (tradeDate, environment))

            if len(except_df) > 0:
                except_df.to_excel('/data/group/800463/日内强势股/saturn_log_parse/异常报错/异常报错_%s_%s.xlsx' % (tradeDate, environment))
                print('create file %s' % '/data/group/800463/日内强势股/saturn_log_parse/异常报错/异常报错_%s_%s.xlsx' % (tradeDate, environment))
            md_data_path = '/data/group/800463/日内强势股/saturn_log_parse/行情数据/%s_%s/' % (tradeDate, environment)
            model_factor_data_path = '/data/group/800463/日内强势股/saturn_log_parse/模型数据/%s_%s/' % (tradeDate, environment)
            ...
            model_total_keys_saturn = []
            for model_dict in all_code_model_data_saturn.values():
                model_total_keys_saturn += list(model_dict.keys())
            model_total_keys_saturn = list(np.unique(np.array(model_total_keys_saturn)))
            total_keys_saturn = list(set(trade_dic.keys()).union(set(now_trade_dic.keys())).union(set(model_total_keys_saturn)))

            if 'ZT_Time' in inf_df_saturn.columns.tolist():
                inf_df_saturn = inf_df_saturn.sort_values('ZT_Time')
                inf_df_saturn['ZT_Time_str'] = inf_df_saturn['ZT_Time'].apply(inttime2str)
                daily_zt_df_saturn['ZT_Time'] = inf_df_saturn['ZT_Time'].loc[daily_zt_df_saturn.index]
            inf_df_saturn['machine_code'] = pd.Series(lp_inst.machine_code_dict).loc[inf_df_saturn.index].values

            tupo_excel_dic = {'每日突破Saturn': daily_zt_df_saturn,
                              '每日订单': order_info_df,
                              '每日拒绝': unfilled_info_df}
            excel_saver(tupo_excel_dic, '/data/group/800463/日内强势股/saturn_log_parse/每日突破/每日突破_%s_%s.xlsx' % (tradeDatestr, environment))
            ...
            if (environment == 'prod') | (environment == 'SHEX') | (environment =='SZEX'):
                ...
                inf_df_dic = {'因子耗时Saturn': inf_df_saturn,
                              '买单统计': buy_statistics,
                ...
                # -------------- 生产上才分析未成交的原因 -------------------
                if environment == 'prod':
                    hist_non_trade = pd.read_excel('/data/group/800463/日内强势股/saturn_log_parse/因子耗时/因子耗时_%s_%s.xlsx'% (lastdate_h, environment), sheetname='未成交统计')
            ...
            elif environment in ['UAT', 'night', 'test', 'thread', 'night2', 'night3']:
                inf_df_dic = {'因子耗时Saturn': inf_df_saturn,
                              '项目930': inf_df_saturn,
                              '机器统计': pd.Series(pd.Series(lp_inst.machine_counter_dict))}

            excel_saver(inf_df_dic, '/data/group/800463/日内强势股/saturn_log_parse/因子耗时/因子耗时_%s_%s.xlsx' % (tradeDate, environment))
            if environment == 'prod':
                from dataApi.sendInfo import send_message
                send_message(f'{tradeDate} 实盘saturn日志已解析')
