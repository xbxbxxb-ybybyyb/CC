        ...
        for key, log in self.log_dic.items():
            ...
            if key_code != '601688.SH':
                ...
                for line in log:
                    ...
                    if ('calculate factors' in line) & ('saturnKey=931' in line):
                        inf_dic_saturn['p2factor_time_cost'] = float(getValueByKeyFromLine(line, by='timeCost'))
                    if ('calculate factors' in line) & ('saturnKey=930' in line):
                        inf_dic_saturn930['p2factor_time_cost'] = float(getValueByKeyFromLine(line, by='timeCost'))
                    ...
                    if ('total unscaled tday-factors' in line) & ('saturnKey=931' in line):
                        factor_dic_saturn = trans_str2dic(line[line.index('{'): line.index(', }')] + '}')
                    if ('saturn factor' in line) & ('saturnKey=930' in line):
                    ...
                    if ('saturn model prediction' in line) & ('parentPath' not in line):
                        if 'shouldBuy=' in line:
                            inf_dic_saturn['p2shouldBuySignal'] = bool(int(getValueByKeyFromLine(line, by='shouldBuy')))
                        else:
                            inf_dic_saturn['p2shouldBuySignal'] = bool(int(getValueByKeyFromLine(line, by='shouldBuySignal')))   # 这一行其实没用
                        if 'cost=' in line:
                            inf_dic_saturn['p2model_time_cost'] = float(getValueByKeyFromLine(line, by='cost'))
                        else:
                            inf_dic_saturn['p2model_time_cost'] = float(getValueByKeyFromLine(line, by='timeCost'))
                    ...
                    if ('Order info' in line) & ('rejected' not in line):  #  触发并准备下单
                        ...
                        if placeType == 'A' and order_type == 'SaturnBuy':
                            ...
                            daily_zt_saturn_dic['totalOrderAmt'] = getValueByKeyFromLine(line, by='totalOrderAmt')
                            daily_zt_ser = pd.Series(daily_zt_saturn_dic, name=key_code)
                            daily_zt_saturn_list.append(daily_zt_ser)

                    if ('saturn model predict' in line) and ('saturnKey=931' in line) and ('parentPath' not in line) and ('shouldBuy' not in line):
                        inf_dic_saturn['totalOrderAmt'] = float(getValueByKeyFromLine(line, by='targetAmt'))
