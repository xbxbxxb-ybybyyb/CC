...
if __name__ == "__main__":
    ...
    for date in s.tradingday(begindate, enddate):
        ...
        for environment in environment_lst:
            ...
            daily_df_jupz, daily_df_sellv1 = lp_inst.start_parsing()

            except_df = lp_inst.get_except_df()
            save_index_sellv1 = list(set(factor_df_931_sellv1.index.tolist()))
            ...
            excel_saver({'930_Sellv1': factor_df_930_sellv1.loc[save_index_sellv1].sort_index(),
                         '931_Sellv1': factor_df_931_sellv1.loc[save_index_sellv1].sort_index(),
                         'JupiterZ': factor_df_jupz},
                        '/data/group/800463/日内强势股/sell_log_parse/因子数据/卖出Sell因子数据_%s_%s.xlsx' % (tradeDate, environment))
            ...
            for model_dict in all_code_model_data_931_sellv1.values():
            ...
            if 'ZT_Time' in inf_df_931_sellv1.columns.tolist():
                inf_df_931_sellv1 = inf_df_931_sellv1.sort_values('ZT_Time')
                print(inf_df_931_sellv1['ZT_Time'])
                inf_df_931_sellv1['ZT_Time_str'] = inf_df_931_sellv1['ZT_Time'].apply(inttime2str)
                daily_df_sellv1['ZT_Time'] = inf_df_931_sellv1['ZT_Time'].loc[inf_df_931_sellv1.index]
            inf_df_931_sellv1['machine_code'] = pd.Series(lp_inst.machine_code_dict).loc[inf_df_931_sellv1.index].values
            if 'ZT_Time' in inf_df_jupz.columns.tolist():
                ...
                daily_df_jupz['ZT_Time'] = inf_df_jupz['ZT_Time'].loc[inf_df_jupz.index]
            inf_df_jupz['machine_code'] = pd.Series(lp_inst.machine_code_dict).loc[inf_df_jupz.index].values

            tupo_excel_dic = {'每日Sellv1': daily_df_sellv1,
                              '每日JupiterZ': daily_df_jupz,
                              '每日订单': order_info_df,
            ...
            inf_df_dic = {'Sell1样本': inf_df_931_sellv1,
