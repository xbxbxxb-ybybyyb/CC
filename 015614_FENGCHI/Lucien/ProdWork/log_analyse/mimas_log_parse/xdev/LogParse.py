...
class LogParse:
    ...
    def getSymbolFromStartSuccessLine(line):
        return line[line.index('MimasStrategy-CPP for ') + 29 : line.index(' Version')]
    ...
    def filter_triggered_line(self, log_line):
        triggered_lines = list(filter(lambda x: 'mimas model prediction' in str(x), log_line))
    ...
    def start_parsing(self):
        inf_list_mimas = []
        factor_list_mimas = []
        factor_list_mimas930 = []
        trade_dic = {}
        daily_zt_mimas_list = []
        now_trade_dic = {}
        new_trade_dic = {}
        order_info_dic = {}
        all_code_model_data_mimas = {}
        unfilled_info_dic = {}
        for key, log in self.log_dic.items():
            ...
            if key_code != '601688.SH':
                ...
                for line in log:
                    ...
                    if ('calculate factors' in line) & ('mimasKey=931' in line):
                        inf_dic_mimas['factor_time_cost'] = float(getValueByKeyFromLine(line, by='timeCost'))
                    if ('calculate factors' in line) & ('mimasKey=930' in line):
                        inf_dic_mimas930['factor_time_cost'] = float(getValueByKeyFromLine(line, by='timeCost'))

                    if 'model prediction' in line:
                        # inf_dic_mimas['ZT_Time'] = int(float(getValueByKeyFromLine(line, by='reachedZTTime')))
                        inf_dic_mimas['ZT_Time'] = 93100000

                    if ('total unscaled tday-factors' in line) & ('mimasKey=931' in line):
                        factor_dic_mimas = trans_str2dic(line[line.index('{'): line.index(', }')] + '}')
                    if ('mimas factor' in line) & ('mimasKey=930' in line):
                        factor_dic_mimas930 = trans_str2dic(line[line.index('{'): line.index(', }')] + '}')
                    ...
                    if ('mimas model prediction' in line) & ('parentPath' not in line):
                        if 'shouldBuy=' in line:
                            inf_dic_mimas['shouldBuySignal'] = bool(int(getValueByKeyFromLine(line, by='shouldBuy')))
                        else:
                            inf_dic_mimas['shouldBuySignal'] = bool(getValueByKeyFromLine(line, by='shouldBuySignal'))   # 这一行其实没用
                        if 'cost=' in line:
                            inf_dic_mimas['model_time_cost'] = float(getValueByKeyFromLine(line, by='cost'))
                        else:
                            inf_dic_mimas['model_time_cost'] = float(getValueByKeyFromLine(line, by='timeCost'))

                    if ('riskSummary' in line) & ('拉抬打压' in line):
                        inf_dic_mimas['MRisk_info'] = '触发拉抬打压'

                    if ('predict sum_signals' in line) & ('parentPath' not in line):
                        inf_dic_mimas['sum_signals'] = getValueByKeyFromLine(line, by='predict sum_signals')

                    if ('parentPath' in line) & ('probability' in line):
                        model_name = getValueByKeyFromLine(line, by='parentPath')
                        if model_name not in all_code_model_data_mimas.keys():
                            all_code_model_data_mimas[model_name] = {}
                        code = getValueByKeyFromLine(line, by='symbol')
                        inf_dic_mimas[model_name + '_probability'] = getValueByKeyFromLine(line, by='probability')
                    ...
                    if ('Order info' in line) & ('rejected' not in line):  #  触发并准备下单
                        ...
                        if (quantity > 0) & (action_type != 'JupiterNew') & ((trade_type == 'SplitLastShot') | (trade_type == 'JupiterFirstOrder')| (trade_type == 'MRiskSplitLastShotBuy')| (order_type == 'MRiskSplitShot')): # 捕捉jupiter的下单
                            daily_zt_mimas_dic['order_direction'] = trade_type
                            daily_zt_mimas_dic['quantity'] = getValueByKeyFromLine(line, by='quantity')
                            daily_zt_mimas_dic['targetAmt'] = getValueByKeyFromLine(line, by='targetAmt')
                            daily_zt_mimas_dic['nowPrice'] = getValueByKeyFromLine(line, by='nowPrice')
                            daily_zt_mimas_dic['highLimitPrice'] = getValueByKeyFromLine(line, by='highLimitPrice')
                            daily_zt_mimas_dic['price'] = getValueByKeyFromLine(line, by='price')
                            daily_zt_mimas_dic['splitOrderNum'] = getValueByKeyFromLine(line, by='splitOrderNum')
                            daily_zt_mimas_dic['totalOrderAmt'] = float(daily_zt_mimas_dic['quantity'])*float(daily_zt_mimas_dic['price'])*float(daily_zt_mimas_dic['splitOrderNum'])
                            inf_dic_mimas['totalOrderAmt'] = daily_zt_mimas_dic['totalOrderAmt']
                            daily_zt_ser = pd.Series(daily_zt_mimas_dic, name=key_code)
                            daily_zt_mimas_list.append(daily_zt_ser)
                    ...
                    if ('market data stat info' in line) & ('Metis' in line):   # 计算因子使用到的行情条数，原来是对比因子不一致时使用
                        inf_dic_mimas['filledTradeList'] = getValueByKeyFromLine(line, by='filledTradeList', form='(.*?)[;\n]')
                        inf_dic_mimas['lxjjFillList'] = getValueByKeyFromLine(line, by='lxjjFillList', form='(.*?)[;\n]')
                        inf_dic_mimas['last5SecFillList'] = getValueByKeyFromLine(line, by='last5SecFillList', form='(.*?)[;\n]')
                        inf_dic_mimas['last30SecFillList'] = getValueByKeyFromLine(line, by='last30SecFillList', form='(.*?)[;\n]')
                        inf_dic_mimas['last2MinFillList'] = getValueByKeyFromLine(line, by='last2MinFillList', form='(.*?)[;\n]')
                        inf_dic_mimas['last5MinFillList'] = getValueByKeyFromLine(line, by='last5MinFillList', form='(.*?)[;\n]')
                ...
                if len(inf_dic_mimas) != 0:
                    inf_ser_mimas = pd.Series(inf_dic_mimas, name=key_code)
                    inf_list_mimas.append(inf_ser_mimas)

                if len(factor_dic_mimas) != 0:
                    factor_ser_mimas = pd.Series(factor_dic_mimas, name=key_code)
                    factor_list_mimas.append(factor_ser_mimas)
                if len(factor_dic_mimas930) != 0:
                    factor_ser_mimas930 = pd.Series(factor_dic_mimas930, name=key_code)
                    factor_list_mimas930.append(factor_ser_mimas930)

        if len(inf_list_mimas) == 0:
            print('没有mimas样本！！！')
            inf_df_mimas = pd.DataFrame()
        else:
            inf_df_mimas = pd.concat(inf_list_mimas, axis=1).T.sort_index()
            if 'MRisk_info' in inf_df_mimas.columns:
                inf_df_mimas_mrisk_info = inf_df_mimas[~inf_df_mimas['MRisk_info'].isna()]
                inf_df_mimas_mrisk_info = inf_df_mimas_mrisk_info.reset_index().drop_duplicates('index').set_index('index')
                inf_df_mimas.loc[inf_df_mimas_mrisk_info.index, 'MRisk_info'] = inf_df_mimas_mrisk_info.loc[inf_df_mimas_mrisk_info.index, 'MRisk_info']
            if 'ZT_Time' in inf_df_mimas.columns:
                inf_df_mimas = inf_df_mimas.loc[inf_df_mimas['ZT_Time'].notnull()]
                inf_df_mimas = inf_df_mimas.query('ZT_Time > 0')
            else:
                inf_df_mimas = pd.DataFrame()


        if len(factor_list_mimas)==0:
            factor_df_mimas = pd.DataFrame()
        else:
            factor_df_mimas = pd.concat(factor_list_mimas, axis=1).T.sort_index()
        if len(factor_list_mimas930)==0:
            factor_df_mimas930 = pd.DataFrame()
        else:
            factor_df_mimas930 = pd.concat(factor_list_mimas930, axis=1).T.sort_index()

        if len(daily_zt_mimas_list) != 0:
            daily_zt_df_mimas = pd.concat(daily_zt_mimas_list, axis=1).T.sort_index()
        else:
            daily_zt_df_mimas = pd.DataFrame()
        ...
        return inf_df_mimas, trade_dic, now_trade_dic, order_info_df, unfilled_info_df, new_trade_dic, factor_df_mimas, factor_df_mimas930, daily_zt_df_mimas, all_code_model_data_mimas