...
def factor_comparison(basic_ceres_path, local_factor_path,java_factor_path, filter_factor_path, date
    ...
    while not os.path.exists(basic_ceres_path):
        ...
        time.sleep(60)
    basic_ceres = pd.read_hdf(basic_ceres_path)
    ...
    for trade_time in ['931']:
        ...
        excel_name = os.path.join(output_folder, 'Factor_diff_ceres_%s_%s_%s.xlsx' % (trade_time, date, environment))
        try:
            excel_saver(output_dict, excel_name)
            # 检测差异
            if (all_diff_samples['ratio_diff'] > 1e-4).sum() > 0:
                send_message(f'{nowdate} Ceres {environment} 存在因子差异 {(all_diff_samples["ratio_diff"] > 1e-5).sum()}')
            else:
                continue
        except:
            print('saving error!!!!!!!!!!!!')
...
if __name__ == '__main__':
    print('==========ceres_uat_factor_comparision==========')
    ...
    for environment in env_list:
        ...
        basic_ceres_path = '/data/group/800463/project/project3_prod/daily_data/%s_v4/Basic_closed_hf_finish_%s_%s.h5' % (date, date, date)
        # 需要陈睿的每日本地因子数据
        local_factor_path = '/data/group/800463/project/project3_prod/daily_data/%s_v4/ceres_factor_v4_%s.pkl' % (str(date), str(date))
        # 需要传入日志解析中的因子值数据
        java_factor_path = '/data/group/800463/日内强势股/ceres_log_parse/因子数据/因子数据Ceres_%s_%s.xlsx' % (date_h, environment)
        # filter的样本文件来源
        filter_factor_path = f'/data/group/800463/project/project3_prod/daily_data/{date}_v4/ceres_factor_v4_{date}.pkl'
        # 输出路径
        output_folder = '/data/group/800463/日内强势股/ceres_log_parse/因子差异/%s_%s/' % (date, environment)
        factor_comparison(basic_ceres_path=basic_ceres_path, local_factor_path=local_factor_path, java_factor_path=java_factor_path, filter_factor_path=filter_factor_path,
