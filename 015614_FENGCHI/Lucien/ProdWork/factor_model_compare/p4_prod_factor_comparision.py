...
def factor_comparison(basic_ceres_path, local_factor_path,java_factor_path, filter_factor_path, date
                      ,output_folder,all_sample_environment,environment):
    while not os.path.exists(basic_ceres_path):
        print(f'缺失文件：{basic_ceres_path}')
        time.sleep(60)
    basic_ceres = pd.read_pickle(basic_ceres_path)
    ...
    def merge_filter(df, delay=False):
        ...
        if delay:
            delay_10_filter = (df['label_T_day_first_ZT_Time'] <= 94000000) == False
    ...
    basic_ceres_filtered = merge_filter(basic_ceres)
    ...
    # basic_ceres_filtered = basic_ceres_filtered.loc[list(set(basic_ceres_filtered.index).intersection(set(filter_samples)))]
    ...
    for trade_time in ['931']:
        ...
        # ----------------------检查样本数量和自由流通股本有没有差异---------------------
        local_samples = list(basic_ceres_filtered.reset_index()['Ticker'])
        ...
        excel_name = os.path.join(output_folder, 'Factor_diff_p4_%s_%s_%s.xlsx' % (trade_time, date, environment))
        try:
            excel_saver(output_dict, excel_name)
            # 检测差异
            if (all_diff_samples['ratio_diff'] > 1e-4).sum() > 0:
                send_message(f'{nowdate} P4 {environment} 存在因子差异 {(all_diff_samples["ratio_diff"] > 1e-5).sum()}')
            else:
                continue
        except:
            print('saving error!!!!!!!!!!!!')
...
if __name__ == '__main__':
    print('==========ceres_factor_comparision==========')
    ...
    for environment in env_list:
        ...
        basic_ceres_path = '/data/group/800463/project/project4_prod/daily_data/%s_v1/basic_p4_%s_%s.h5' % (date, date, date)

        # 需要陈睿的每日本地因子数据
        local_factor_path = '/data/group/800463/project/project4_prod/daily_data/%s_v1/p4_factor_v1_%s.pkl' % (str(date), str(date))
        # 需要传入日志解析中的因子值数据
        java_factor_path = '/data/group/800463/日内强势股/p4_log_parse/因子数据/因子数据P4_%s_%s.xlsx' % (date_h, environment)
        # filter的样本文件来源
        filter_factor_path = f'/data/group/800463/project/project4_prod/daily_data/{date}_v1/p4_factor_v1_{date}.pkl'
        # 输出路径
        output_folder = '/data/group/800463/日内强势股/p4_log_parse/因子差异/%s_%s/' % (date, environment)
        factor_comparison(basic_ceres_path=local_factor_path, local_factor_path=local_factor_path, java_factor_path=java_factor_path, filter_factor_path=filter_factor_path,
