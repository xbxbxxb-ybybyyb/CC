...
if __name__ == '__main__':
    ...
    for nowdate in datelist:
        ...
        for environment in env_list:
            ...
            all_df = pd.read_pickle(f'/data/group/800463/project/project1_prod/right_v2212/daily_data/{tradeDateInt}/all_factor_zt_merge_v2212_{tradeDateInt}.pkl')
            all_df['is_sample_jup'] = (all_df['ZT_Time'] >= 93000000) &\
                                     (all_df['ZT_Time'] <= 143000000) &\
                                     (all_df['open_is_zt'] == 0) &\
                                     (all_df['T_o2pre'] >= -0.05) &\
                                     (all_df['after_not_ul_len'] > 10) &\
                                     (all_df['pre_close'] >= 1.6) &\
            ...
            excel_save_dict['本地投票结果'] = local_predict.join(log_signal).join(total_reg_model_predict_result).join(total_cla_model_predict_result).join(total_ZT_model_predict_result).join(all_df[['is_sample_jup']])
