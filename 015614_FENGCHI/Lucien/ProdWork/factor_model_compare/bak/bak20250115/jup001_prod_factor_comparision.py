...
def factor_comparison(local_data_path, local_factor_path,UAT_factor_path,daily_zuhe_path,date,date_hyphen
    ...
    # TODO：少森给这个文件路径
    while not os.path.exists(local_factor_path + 'all_factor_zt_merge_v2304_with_kcb_%s_europa.pkl' % (date)):
        print(f'缺失{local_factor_path}' + f'all_factor_zt_merge_v2304_with_kcb_{date}_europa.pkl')
        time.sleep(60)
    python_factors = pd.read_pickle(local_factor_path + 'all_factor_zt_merge_v2304_with_kcb_%s_europa.pkl' % (date))
    ...
    label_cols = list(set(label_cols).difference(set(['Min_max_rate'])))    # 剔除一个因子
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        # date = '20240813'  # TODO:night夜间仿真，修改成下一天的日期，比如对比25号的回放，这里要改成26号下一个交易日
    ...
    for date in datelist:
        ...
        for environment in environment_lst:
            ...
            local_factor_path = '/data/group/800463/project/project1_prod/right_v2304_kcb/daily_data/%s_europa/' % (str(date))
            ...
            # prod
            local_data_path = '/data/group/800463/data/project1_prod/transaction_jupiter/'
