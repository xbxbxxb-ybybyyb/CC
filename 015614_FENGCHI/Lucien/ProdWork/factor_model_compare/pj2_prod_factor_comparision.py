    ...
    for trade_time in ['931']:
        ...
        try:
            ...
            if (all_diff_samples['ratio_diff'] > 1e-4).sum() > 0:
                send_message(f'{nowdate} Saturn {environment} 存在因子差异 {(all_diff_samples["ratio_diff"] > 1e-5).sum()}')
            else:
                continue
        except:
            print('saving error!!!!!!!!!!!!')
...
if __name__ == '__main__':
    ...
    for environment in env_list:
        ...
        basic_pj2_path = '/data/group/800463/project/project2_prod/daily_data/%s_v7/Basic_closed_hf_finish_%s_%s.h5' % (date, date, date)
        # 需要陈睿的每日本地因子数据
        local_factor_path = '/data/group/800463/project/project2_prod/daily_data/%s_v7/saturn_factor_v7_%s.pkl' % (str(date), str(date))
        # 需要传入日志解析中的因子值数据
        java_factor_path = '/data/group/800463/日内强势股/saturn_log_parse/因子数据/因子数据Saturn_%s_%s.xlsx' % (date_h, environment)
        # filter的样本文件来源
        filter_factor_path = f'/data/group/800463/project/project2_prod/daily_data/{date}_v6/saturn_factor_v7_{date}.pkl'
        # 输出路径
