...
def factor_comparison(local_data_path, local_factor_path,UAT_factor_path,daily_zuhe_path,date,date_hyphen
    ...
    while not os.path.exists(local_factor_path + 'all_factor_zt_merge_v2412_%s.pkl' % date):
        print('缺失' + local_factor_path + 'all_factor_zt_merge_v2412_%s.pkl' % date)
        time.sleep(60)
    python_factors = pd.read_pickle(local_factor_path + 'all_factor_zt_merge_v2412_%s.pkl' % date)
    ...
    try:
        f_data = IO.read_data([date, date], columns=['open','pre_close'], alt='/data/group/800080/warehouse_event/prod/MD/CHINA_STOCK/DAILY/WIND/MD_CHINA_STOCK_DAILY_WIND.h5')
...
if __name__ == '__main__':
    ...
    for date in datelist:
        ...
        # for environment in ['UAT_lite']:  #
            ...
            local_factor_path = '/data/group/800463/project/project1_prod/right_v2412/daily_data/%s/'%(str(date))#'/data/group/800463/project/project1_prod/right_v2212/daily_data/%s/'%(date)
            # 需要传入日志解析中的因子值数据
            # UAT_factor_path = '/data/user/013550/%s_factor_data/'%(environment)
            UAT_factor_path = '/data/group/800463/日内强势股/jupiter_log_parse/因子数据/'
            # 输出路径
            # output_folder = '/data/user/013550/%s_factor_test_result/%s_%s/' % (environment,date,environment)
            output_folder = '/data/group/800463/日内强势股/jupiter_log_parse/因子差异/%s_%s/' % (date,environment)
            # 需要传入谢璐遥的每日组合
            ...
            factor_time_path = '/data/group/800463/日内强势股/jupiter_log_parse/因子耗时/'
