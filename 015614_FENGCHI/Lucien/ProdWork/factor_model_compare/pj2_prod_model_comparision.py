...
from ProdWork.Param_config_data import thred_dict_pj2_931_v7 as thred_dict
...
if __name__ == '__main__':
    ...
    for environment in env_list:
        ...
        prod_model = ['totalRegv2o10d1_FSV8XgbWjModel',
                      'totalRegv2o10d1_FSV11XgbWjModel',
                      'totalRegbase_FSV11XgbWjModel',
                      'totalRegFSV8XgbXbcModel',
                      'totalRegXbcXgbXbcModel',
                      'totalRegFSRSXgbXbcModel']
        log_model = list(thred_dict.keys())
        log_model = ['totalRegv2o10d1_FSV8XgbWjModel',
                      'totalRegv2o10d1_FSV11XgbWjModel',
                      'totalRegbase_FSV11XgbWjModel',
                      'totalRegFSV8XgbXbcModel',
                      'totalRegXbcXgbXbcModel',
        ...
        # TODO: wj提供，本地模型路径
        pred_path = '/data/group/800463/wangj/model_signal/Saturn/S1_v7/%s/%s_%s_saturn_fac_20241129_daily_pred_prodmodel.csv' % (tradeDatestr, tradeDatestr, tradeDatestr)
        ...
        # ------------------------------总结df------------------------------------------------
        saturn_factor_fpath = f'/data/group/800463/project/project2_prod/daily_data/{tradeDatestr}_v7/saturn_factor_v7_{tradeDatestr}.pkl'
        ...
        def jupiter_voting(reg_stacking, cla_stacking):
            condition1 = reg_stacking >= 1  # 此次版本阈值为3  # 20240703更改为大于等于4
            return condition1
        ...
        diff_num = len(list(filter(lambda x: '模型存在概率差异' in x[0], list(summary_dict.values()))))
        if diff_num > 0:
            send_message(f'{nowdate} Saturn {environment} 存在模型差异 {diff_num} !!!!!!')
        else:
            send_message(f'{nowdate} Saturn {environment} 不存在模型差异')