...
def factor_comparison(local_data_path, local_factor_path,UAT_factor_path,daily_zuhe_path,date,date_hyphen
    ...
    # TODO：少森给这个文件路径
    while not os.path.exists(local_factor_path + f'all_factor_zt_merge_v2309_{date}_metis.pkl'):
        print('缺失' + local_factor_path + f'all_factor_zt_merge_v2309_{date}_metis.pkl')
        time.sleep(60)
    python_factors = pd.read_pickle(local_factor_path + f'all_factor_zt_merge_v2309_{date}_metis.pkl')
    basic_df = python_factors.copy()
    python_factors = python_factors[(basic_df['high_price'] < basic_df['ul_price']) & (basic_df['ZT_Time'] <= 143000000) & (basic_df['trigger_time'] <= 143000000)]
    UAT_factors = pd.read_excel(UAT_factor_path + '因子数据Metis_%s-%s-%s_%s.xlsx' % (Year, Month, Day, environment))
    ...
    ZT_Time_UAT = pd.read_excel(factor_time_path+'因子耗时_%s_%s.xlsx'%(date_hyphen,environment), sheet_name='因子耗时Metis')
    ...
    Local_data_stats = ZT_Time_UAT[['factor_time_cost','ZT_Time']].join(stats)
    # ----------------------检查重合的样本的因子值有没有差异---------------------
    ...
    excel_name = os.path.join(output_folder, 'Factor_diff_Metis_%s_%s_3.xlsx'%(date,environment))
    try:
...
if __name__ == '__main__':
    print('============metis_prod_factor_comparision===========')
    ...
    for date in datelist:
        ...
        for environment in environment_lst:
            print('metis_prod_factor_comparision', date, environment)
            # 是否计算样本偏差
            all_sample_environment = True
            # TODO: local的路径，每次迭代 少森给这个路径
            local_factor_path = f'/data/group/800463/project/project1_prod/right_v2309/daily_data/{date}_metis/'
            local_basic_path = local_factor_path
            # 需要传入日志解析中的因子值数据
            UAT_factor_path = '/data/group/800463/日内强势股/metis_log_parse/因子数据/'
            # 输出路径
            output_folder = '/data/group/800463/日内强势股/metis_log_parse/因子差异/%s_%s/' % (date, environment)
            # 需要传入谢璐遥的每日组合
            ...
            local_data_path = '/data/group/800463/data/project1_prod/transaction_metis/'

            # 需要传入日志解析中的因子耗时数据
            factor_time_path = '/data/group/800463/日内强势股/metis_log_parse/因子耗时/'
