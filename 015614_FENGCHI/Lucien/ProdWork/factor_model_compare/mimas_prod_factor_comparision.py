...
def factor_comparison(basic_mimas_path, local_factor_path,java_factor_path, filter_factor_path, date
                      ,output_folder,all_sample_environment,environment):
    while not os.path.exists(basic_mimas_path):
        print(f'缺失文件：{basic_mimas_path}')
        time.sleep(60)
    basic_mimas = pd.read_hdf(basic_mimas_path)
    ...
    basic_mimas_filtered = merge_filter(basic_mimas)
    ...
    # basic_mimas_filtered = basic_mimas_filtered.loc[list(set(basic_mimas_filtered.index).intersection(set(filter_samples)))]
    ...
    for trade_time in ['931']:
        ...
        # ----------------------检查样本数量和自由流通股本有没有差异---------------------
        local_samples = list(basic_mimas_filtered.reset_index()['Ticker'])
        ...
        excel_name = os.path.join(output_folder, 'Factor_diff_mimas_%s_%s_%s.xlsx' % (trade_time, date, environment))
        try:
            excel_saver(output_dict, excel_name)
            # 检测差异
            if (all_diff_samples['ratio_diff'] > 1e-4).sum() > 0:
                send_message(f'{nowdate} Mimas {environment} 存在因子差异 {(all_diff_samples["ratio_diff"] > 1e-5).sum()}')
            else:
                continue
        except:
            print('saving error!!!!!!!!!!!!')
...
if __name__ == '__main__':
    print('==========mimas_uat_factor_comparision==========')
    ...
    for environment in env_list:
        ...
        basic_mimas_path = '/data/group/800463/project/project2_prod/daily_data/%s_mimas_v1/Basic_next_hf_finish_%s_%s.h5' % (date, date, date)
        # 需要陈睿的每日本地因子数据
        local_factor_path = f'/data/group/800463/project/project2_prod/daily_data/{date}_mimas_v1/mimas_factor_v1_{date}.pkl'
        # 需要传入日志解析中的因子值数据
        java_factor_path = '/data/group/800463/日内强势股/mimas_log_parse/因子数据/因子数据Mimas_%s_%s.xlsx' % (date_h, environment)
        # filter的样本文件来源
        filter_factor_path = f'/data/group/800463/project/project3_prod/daily_data/{date}_v4/mimas_factor_v4_{date}.pkl'
        # 输出路径
        output_folder = '/data/group/800463/日内强势股/mimas_log_parse/因子差异/%s_%s/' % (date, environment)
        factor_comparison(basic_mimas_path=basic_mimas_path, local_factor_path=local_factor_path, java_factor_path=java_factor_path, filter_factor_path=filter_factor_path,
