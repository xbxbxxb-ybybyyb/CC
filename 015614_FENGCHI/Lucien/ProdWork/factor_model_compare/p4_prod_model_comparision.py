...
from ProdWork.Param_config_data import thred_dict_p4_v1 as thred_dict

if __name__ == '__main__':
    print('=============ceres_uat_model_comparision=============')
    if len(sys.argv) > 1:
        nowdate = sys.argv[1]
    else:
        nowdate = dt.datetime.now().strftime('%Y%m%d')
        # nowdate = '20250530'    # 第二天
    ...
    for environment in env_list:
        ...
        inf_df = pd.read_excel('/data/group/800463/日内强势股/p4_log_parse/因子耗时/因子耗时_%s_%s.xlsx' % (tradeDate, environment),
                               sheetname='因子耗时P4')
        ...
        prod_model = ['totalRegbase_FSRSXgbZwhModel',
                      'totalRegbase_FSV11XgbSkkModel',
                      'totalRegbase_FSV8XgbFcModel',
                      'totalRegbasemix_FSV11XgbWjModel',
                      'totalRegbase_SkkXgbSkkModel',
                      'totalRegbase_XbcXgbXbcModel']
        log_model = list(thred_dict.keys())
        log_model = ['totalRegbase_FSRSXgbZwhModel',
                      'totalRegbase_FSV11XgbSkkModel',
                      'totalRegbase_FSV8XgbFcModel',
                      'totalRegbasemix_FSV11XgbWjModel',
                      'totalRegbase_SkkXgbSkkModel',
        ...
        # TODO: wj提供，本地模型路径
        pred_path = '/data/group/800463/wangj/model_signal/P4/ns1_v1/%s/%s_%s_p4_fac_20250324_daily_pred_prodmodel.csv' % (tradeDatestr, tradeDatestr, tradeDatestr)
        ...
        # ------------------------------总结df------------------------------------------------
        saturn_factor_fpath = f'/data/group/800463/project/project4_prod/daily_data/{tradeDatestr}_v1/p4_factor_v1_{tradeDatestr}.pkl'
        ...
        param['is_sample_p4'] = ((param['label_T_day_first_ZT_Time'] <= 93010000) == False) & \
                                 ((param['label_T_day_first_DT_Time'] <= 93010000) == False) & \
                                 (param['label_T_open_is_zt'] == 0) & \
                                 (param['label_T_first_trans_ZT'] != 1) & \
                                 (param['saturn_after_not_ul_len'] > 10) & \
                                 (param['label_T_open_is_dt'] == 0) & \
                                 (~param['T_day_93010_10_twap_before_ZT'].isin([-1, -3])) & \
                                 (param['saturn_t931_s_filter'] == 1) & \
                                 (param['last_is_break2'] == 1) & \
        ...
        # # 添加当天是否含有策略参数
        # # param_list = list(map(lambda x: x[:9], os.listdir(f'/data/user/013551/forXT/Ceres/param/20250514_type2/{nowdate}/Ceres/{nowdate}_ceres')))
        # param_list = os.listdir(f'/data/user/013551/forXT/Ceres/param/param_new/CeresStrategy/backend_sz_type2_{nowdate}/stock_params/') + \
        #          os.listdir(f'/data/user/013551/forXT/Ceres/param/param_new/CeresStrategy/backend_sh_type2_{nowdate}/stock_params/')
        ...
        # excel_save_dict['本地投票结果'] = local_predict.join(log_signal).join(param[['is_sample_ceres', 'has_param']]).join(total_reg_model_predict_result)
        excel_save_dict['本地投票结果'] = local_predict.join(log_signal).join(param[['is_sample_p4']]).join(total_reg_model_predict_result)

        """添加filter列 20230803 Ceres上线后有个filter"""
        ...
        check_dir('/data/group/800463/日内强势股/p4_log_parse/模型差异/%s' % tradeDatestr)
        excel_saver(excel_save_dict, '/data/group/800463/日内强势股/p4_log_parse/模型差异/%s/模型差异_%s_%s_p4_931.xlsx' % (tradeDatestr, tradeDatestr, environment))
        # 监测是否存在模型差异
        from dataApi.sendInfo import send_message

        diff_num = len(list(filter(lambda x: '模型存在概率差异' in x, list(summary_dict.values()))))
        if diff_num == 0:
            send_message(f'{nowdate} P4 {environment} 存在模型差异 {diff_num} !!!!!!')
        else:
            send_message(f'{nowdate} P4 {environment} 不存在模型差异')