...
from ProdWork.Param_config_data import thred_dict_jup001_v4 as thred_dict   # TODO: import 最新的
if __name__ == '__main__':
    ...
    for nowdate in datelist:
        ...
        for environment in env_list:
            ...
            # prod_model = ['totalRegpct2_FSV8XgbWjModel',
            #               'totalRegp7_FSZWHXgbZwhModel',
            #               'totalRegpct2_FSRSXgbZwhModel',
            #               'sceneRegbase_cnbo_FSV10XgbXlyModel',
            #               'sceneRegp7_cnbo_FSRSXgbXbcModel',
            #               'totalRegp7_FSV10XgbWjModel']
            prod_model = ['totalRegbase_under3_FSV10XgbXlyModel', 'totalRegbase_up2_FSV10XgbXlyModel',
                         'totalRegp7_FSV10XgbWjModel',
                         'totalRegp7_FSZWHXgbZwhModel',
                         'totalRegp7_under3_FSRSXgbXbcModel', 'totalRegp7_up2_FSRSXgbXbcModel',
                         'totalRegpct2_FSRSXgbZwhModel',
                         'totalRegpct2_FSV8XgbWjModel']
            log_model = list(thred_dict.keys())
            log_model = ['totalRegbase_under3_FSV10XgbXlyModel', 'totalRegbase_up2_FSV10XgbXlyModel',
                         'totalRegp7_FSV10XgbWjModel',
                         'totalRegp7_FSZWHXgbZwhModel',
                         'totalRegp7_under3_FSRSXgbXbcModel', 'totalRegp7_up2_FSRSXgbXbcModel',
                         'totalRegpct2_FSRSXgbZwhModel',
            ...
            #TODO: wj提供，本地模型路径
            pred_path = '/data/group/800463/wangj/model_signal/Jupiter001/prod_v4/%s/%s_%s_europa_fac_20240901_daily_pred.csv'%(tradeDatestr, tradeDatestr, tradeDatestr)
            import time
            ...
            # TODO: sss提供
            param = pd.read_pickle(f'/data/group/800463/project/project1_prod/right_v2412/daily_data/{nowdate}_europa/all_factor_zt_merge_v2412_{nowdate}_europa.pkl')
            ...
            param['is_sample_europa'] = ((param['ZT_Time'] >= 93000000) &
                                         (param['ZT_Time'] <= 143000000) &
                                         (param['open_is_zt'] == 0) &
                                         (param['T_o2pre'] >= -0.05) &
                                         (param['after_not_ul_len'] > 10) &
                                         (param['pre_close'] >= 2) &
                                         (param['high_price'] < (param['trigger_price'])) &
                                         (param['last_is_zt'] == 0) &
                                         (param['min_is_dt']==0))

            # 生成本地投票结果
            # reg_stacking = total_reg_model_predict_result.sum(axis = 1)   # NOTE: 20250108更改为读取敬姐文件中的vote_sum_pred
            reg_stacking = All_model_predict_data['vote_sum_pred'].reindex(All_model_predict_data.index)
            ...
            for model_name in prod_model:
                excel_save_dict[model_name.replace('totalReg', '')] = All_model_predict_data_diff_out_dict[model_rename_dict[model_name]]
