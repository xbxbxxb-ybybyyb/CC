...
def factor_comparison(local_data_path, local_factor_path, UAT_factor_path, date, date_hyphen,
                      factor_time_path, output_folder, all_sample_environment, environment):
    import time
    ...
    while not os.path.exists(local_factor_path):
        print('缺失' + local_factor_path)
        time.sleep(60)
    python_factors = pd.read_pickle(local_factor_path)
    basic_df = python_factors.copy()
    python_factors = python_factors[(basic_df['ZT_Time'] >= 93000000) &
                                    (basic_df['ZT_Time'] <= 143000000) &
                                    (basic_df['open_is_zt'] == 0) &
                                    (basic_df['T_o2pre'] >= -0.05) &
                                    (basic_df['after_not_ul_len'] > 10) &
                                    (basic_df['pre_close'] >= 1.6) &
                                    (basic_df['high_price'] < (basic_df['ul_price'])) &
                                    (basic_df['min_is_dt'] == 0) &
                                    (basic_df['last_is_zt'] == 1) &
                                    (basic_df['saturn_lzt_day_pattern'].isin([3,4]))]
    ...
    try:
        f_data = IO.read_data([date, date], columns=['open', 'pre_close'],
    ...
    ZT_Time_UAT = pd.read_excel(factor_time_path + '因子耗时_%s_%s.xlsx' % (date_hyphen, environment), sheet_name='因子耗时')
    ...
    excel_name = os.path.join(output_folder, 'Factor_diff_Leda_%s_%s.xlsx' % (date, environment))
    try:
...
if __name__ == '__main__':
    print('============leda_uat_factor_comparision===========')
    ...
    for date in datelist:
        ...
        for environment in environment_lst:
            print('leda_factor_comparision', date, environment)
            # 是否计算样本偏差
            all_sample_environment = True
            # TODO: local的路径，每次迭代 少森给这个路径
            local_factor_path = f'/data/group/800463/project/project1_prod/right_v2310/daily_data/{date}/leda_v2310_{date}.pkl'
            local_basic_path = local_factor_path
            # 需要传入日志解析中的因子值数据
            UAT_factor_path = '/data/group/800463/日内强势股/leda_log_parse/因子数据/'
            output_folder = f'/data/group/800463/日内强势股/leda_log_parse/因子差异/{date}_{environment}/'
            # 需要王伟地的每日本地行情数据（这里主要需要突破时间）
            local_data_path = '/data/group/800463/data/project1_prod/transaction_jupiter/'
            factor_time_path = '/data/group/800463/日内强势股/leda_log_parse/因子耗时/'

            factor_comparison(local_data_path=local_data_path, local_factor_path=local_factor_path, UAT_factor_path=UAT_factor_path,
                              factor_time_path=factor_time_path, output_folder=output_folder,
