...
from ProdWork.Param_config_data import thred_dict_ceres_v4 as thred_dict
...
if __name__ == '__main__':
    ...
    for environment in env_list:
        ...
        inf_df = pd.read_excel('/data/group/800463/日内强势股/ceres_log_parse/因子耗时/因子耗时_%s_%s.xlsx' % (tradeDate, environment),
                               sheetname='因子耗时Ceres')
        ...
        prod_model = ['totalRegbase_FSV10XgbXbcModel',
                      'totalRegbase_FSV8XgbFcModel',
                      'totalRegbase_FSV8XgbXbcModel',
                      'totalRegbasemix_FSV11XgbWjModel',
                      'totalRegbasemix_XbcXgbXbcModel',
                      'totalRegpctp5_FSV11XgbWjModel']
        log_model = list(thred_dict.keys())
        log_model = ['totalRegbase_FSV10XgbXbcModel',
                      'totalRegbase_FSV8XgbFcModel',
                      'totalRegbase_FSV8XgbXbcModel',
                      'totalRegbasemix_FSV11XgbWjModel',
                      'totalRegbasemix_XbcXgbXbcModel',
        ...
        # TODO: wj提供，本地模型路径
        pred_path = '/data/group/800463/wangj/model_signal/Ceres/S1_v4/%s/%s_%s_ceres_fac_20250324_daily_pred_prodmodel.csv' % (tradeDatestr, tradeDatestr, tradeDatestr)
        ...
        # ------------------------------总结df------------------------------------------------
        saturn_factor_fpath = f'/data/group/800463/project/project3_prod/daily_data/{tradeDatestr}_v4/ceres_factor_v4_{tradeDatestr}.pkl'
        ...
        param['is_sample_ceres'] = ((param['label_T_day_first_ZT_Time'] <= 93100000) == False) & \
                                 ...
                                 (~param['T_day_931_10_twap_before_ZT'].isin([-1, -3])) & \
        ...
        # param_list = os.listdir(f'/data/user/013551/forXT/Ceres/param/param_new/CeresStrategy/backend_sz_type1_{nowdate}/stock_params/') + \
        #     os.listdir(f'/data/user/013551/forXT/Ceres/param/param_new/CeresStrategy/backend_sh_type1_{nowdate}/stock_params/')
        ...
        excel_save_dict['本地投票结果'] = local_predict.join(log_signal).join(param[['is_sample_ceres']]).join(total_reg_model_predict_result)
        ...
        check_dir('/data/group/800463/日内强势股/ceres_log_parse/模型差异/%s' % tradeDatestr)
        excel_saver(excel_save_dict, '/data/group/800463/日内强势股/ceres_log_parse/模型差异/%s/模型差异_%s_%s_ceres_931.xlsx' % (tradeDatestr, tradeDatestr, environment))
        from dataApi.sendInfo import send_message
        # 监测是否存在模型差异
        diff_num = len(list(filter(lambda x: '模型存在概率差异' in x, list(summary_dict.values()))))
        if diff_num == 0:
            send_message(f'{nowdate} Ceres {environment} 存在模型差异 {diff_num} !!!!!!')
        else:
            send_message(f'{nowdate} Ceres {environment} 不存在模型差异')