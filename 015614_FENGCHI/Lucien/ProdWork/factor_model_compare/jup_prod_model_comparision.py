...
from ProdWork.Param_config_data import thred_dict_jup_v10 as thred_dict
    ...
    for nowdate in datelist:
        ...
        for environment in env_list:
            ...
            inf_df = pd.read_excel(
                '/data/group/800463/日内强势股/jupiter_log_parse/因子耗时/因子耗时_%s_%s.xlsx' % (tradeDate, environment),
                sheet_name='因子耗时')
            ...
            prod_model = ['totalRegp5_FSZWHXgbZwhModel',
            ...
            log_model = ['totalRegp5_FSZWHXgbZwhModel',
                         'totalRegp5_FSRSXgbXbcModel',
                         'totalRegp5_FSV8XgbZwhModel',
                         'totalRegp5_FSRSXgbFcModel',
                         'totalRegp5_FSV8XgbWjModel',
            ...
            # TODO: sss提供
            all_df = pd.read_pickle(f'/data/group/800463/project/project1_prod/right_v2412/daily_data/{nowdate}/all_factor_zt_merge_v2412_{nowdate}.pkl')
            md = IO.read_data([nowdate, nowdate], columns=['open'], alt='/data/group/800080/warehouse/prod/MD/CHINA_STOCK/DAILY/WIND/MD_CHINA_STOCK_DAILY_WIND.h5')
            all_df['open_price'] = md['open']
            all_df['is_sample_jupiter'] = ((all_df['ZT_Time'] >= 93000000) &
            ...
            def jupiter_voting(reg_stacking, cla_stacking):
                condition1 = reg_stacking >= 3  # 集成阈值，根据实盘情况修改
            ...
            excel_save_dict['本地投票结果'] = local_predict.join(log_signal).join(total_reg_model_predict_result).join(total_cla_model_predict_result).join(total_ZT_model_predict_result).join(all_df[['is_sample_jupiter']])
            excel_save_dict['线上投票结果'] = local_predict.join(log_signal)

            """去除因子耗时中的JupiterZ"""
            tradeDateInt = int(tradeDate.replace('-', ''))
            local_factor_fpath = '/data/group/800463/project/project1_prod/right_v2412/daily_data/%s/all_factor_zt_merge_v2412_%s.pkl' % (tradeDateInt, tradeDateInt)
            import time

            while not os.path.exists(local_factor_fpath):
                print(f'缺失{local_factor_fpath}')
                time.sleep(60)
            jup_samples_index = pd.read_pickle(local_factor_fpath).query('last_is_zt==0').index
            excel_save_dict['本地投票结果'] = excel_save_dict['本地投票结果'].loc[jup_samples_index.get_level_values(1).tolist()]
            excel_save_dict['线上投票结果'] = excel_save_dict['线上投票结果'].loc[jup_samples_index.get_level_values(1).tolist()]
            ...
            check_dir('/data/group/800463/日内强势股/jupiter_log_parse/模型差异/%s/'%tradeDatestr)

            excel_saver(excel_save_dict,'/data/group/800463/日内强势股/jupiter_log_parse/模型差异/%s/模型差异_%s_%s.xlsx'%(tradeDatestr,tradeDatestr,environment))
            if environment == 'prod':
                from dataApi.sendInfo import send_message
                send_message(f'{nowdate} jupiter生成完毕')