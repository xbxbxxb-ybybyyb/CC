...
def generate_zuhe_uat(choose_param_df,date, saturn_config_Df, ceres_config_Df):
    ...
    for stock in nosell_zuhe_df.index.tolist():
        if '.SH' in stock:
            sh_stockIDs.append(stock)
    zuhe_commonPath = r'/data/group/800463/xiely/daily/daily-zuhe-prod-O45-SH-new/'
    for filename in os.listdir(zuhe_commonPath):
        os.remove(os.path.join(zuhe_commonPath,filename))
    nosell_zuhe_df.loc[sh_stockIDs[:500]].to_excel(zuhe_commonPath + 'new-O45组合-SH-first-'+date+'.xlsx',header=True, encoding='gbk')
    nosell_zuhe_df.loc[sh_stockIDs[500:1000]].to_excel(zuhe_commonPath + 'new-O45组合-SH-second-'+date+'.xlsx',header=True, encoding='gbk')
    nosell_zuhe_df.loc[sh_stockIDs[1000:]].to_excel(zuhe_commonPath + 'new-O45组合-SH-third-'+date+'.xlsx',header=True, encoding='gbk')
    if len(sell_zuhe_df)>0:
        sell_zuhe_df.to_excel(zuhe_commonPath + 'new-O45组合-SH-sell-'+date+'.xlsx',header=True, encoding='gbk')
    if len(saturn_zuhe_df)>0:
        saturn_zuhe_df.to_excel(zuhe_commonPath + 'new-O45组合-SH-saturn-'+date+'.xlsx',header=True, encoding='gbk')
    if len(ceres_zuhe_df)>0:
        ceres_zuhe_df.to_excel(zuhe_commonPath + 'new-O45组合-SH-ceres-'+date+'.xlsx',header=True, encoding='gbk')
...
def generate_paramsJson():
    ...
    filepath = commonPath + 'excels/param-'+date+'-prod-O45-SH-new.xlsx'
