        ...
        for WJ_model_name in WJ_model_name_all:
            ...
            if tradeDatestr >= v5_pj2_start_date and WJ_model_name.find('931') < 0:
                ...
                if WJ_model_name.find('931') > 0:
                    ...
                    saturn_model_rename_dict[WJ_model_name], tradeDatestr)
                else:
                    wj_new_models_path = '/data/group/800463/wangj/model_signal/Saturn/S0/%s/%s' % (
                    saturn_model_rename_dict[WJ_model_name], tradeDatestr)
                # 核对模型的预测数据
...
merged_basic = sat_model_track_v6.combine_labelandprofit()
# NOTE: 20231030 给孙康康用来做卖出分析
from dataApi.sendInfo import send_file
from LucienUtil.FileUtil import FileUtil
output_dict = dict()
output_dict['v5'] = v5_saturn_tot_local_s1
output_dict['v6'] = v6_saturn_tot_local_s1
output_dict['sat_sample'] = merged_basic
FileUtil.save_dict2xls(output_dict, '/data/user/015614/junkData/', 'saturn_signal.xlsx')
send_file('/data/user/015614/junkData/saturn_signal.xlsx')
