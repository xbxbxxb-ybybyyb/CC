...
if not os.path.exists(output_path + f'{start_date}-{today_date}.pkl'):
    ...
    md_data.loc[md_data['bj'], 'ul_price'] = np.floor(md_data.loc[md_data['bj'], 'pre_close'] * 100 * 1.3 + 1e-8) / 100
    md_data.loc[md_data['bj'], 'next_ul_price'] = np.floor(md_data.loc[md_data['bj'], 'close'] * 100 * 1.3 + 1e-8) / 100
...
print(f'{today_date}当日所有基础数据已存储并保存')
print(f'0-2.calc_md_data耗时{round(time.time() - t1, 6)}秒')

# %% 测试数据，检验任意股票的涨停价计算是否正确
check = pd.read_pickle(output_path + f'20250116-20250220.pkl')
