...
from ProdWork.intra_strong.update.recordUtil import getExtraBuyInfo, write_excel
...
lm = link.LinkMessage()
from ProdWork.CommonTools import number2stockcode, judge_updatedate
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        # date = '20250411' # 若未在当个交易日晚上运行程序，需要在次日早上修改date
    ...
    historyFileBj = '/data/group/800463/日内强势股/实盘分析记录/日内强势股总买入记录/日内强势股总买入记录Bj-%s.xlsx' % lastdate
    ...
    if os.path.exists(historyFileBj):
        historyBuyDf_Bj = pd.read_excel(historyFileBj, sheet_name='总买入记录')
        historyBuyDf_Bj = historyBuyDf_Bj[historyBuyDf_Bj['证券名称'].notnull()]
    else:
        historyBuyDf_Bj = pd.DataFrame(columns = historyBuyDf.columns)
    ...
    if os.path.exists(logFile):
        ...
        logDf_Bj = pd.read_excel(logFile, sheet_name='每日突破Bj')
        logDf_Bj['actionSource'] = 'JupiterBj'
        ...
        if len(logDf_Bj)>0:
            logDf_Bj = logDf_Bj[logDf_Bj['ZT_Time']!=0]
        if len(logDf001)>0:
    ...
    def updatePositionBuy(buyDf, resDf, resDf_Bj, resDf_pj2, resDf_pj3, logDf, buy_orderDf, resDf001, logDf001, buy_metis_orderDf, logDf_metis, logDf_leda):
        ...
        if len(logDf_Bj) == 0: buy_log_Df_jupiterBj = pd.DataFrame()
        else: buy_log_Df_jupiterBj = logDf_Bj[(logDf_Bj['order_direction'] == 'SplitLastShot')|(logDf_Bj['order_direction'] == 'JupiterFirstOrder')| (logDf_Bj['order_direction']  == 'MRiskSplitLastShotBuy')|(logDf_Bj['order_direction']  == 'MRiskSplitShotBuy')]
        ...
        buy_orderDf_jupiterBj = buy_orderDf[(buy_orderDf['actionSource'] == 'JupiterBJ') &
                                          ((buy_orderDf['orderType'] == 'SplitLastShot') |
                                           (buy_orderDf['orderType'] == 'MRiskSplitShot') |
                                           (buy_orderDf['orderType'] == 'JupiterFirstOrder') |
                                           (buy_orderDf['orderType'] == 'MRiskSplitLastShotBuy') |
        ...
        if len(buyDf)==0:
            ...
            buy_orderDf_jupiterBj_info = buy_orderDf_tot_info[buy_orderDf_tot_info['stockcode'].apply(lambda x:x in set(buy_orderDf_jupiterBj['stockcode']))].set_index('stockcode')
            ...
            buyDf_jupiterBj = buyDf[buyDf['证券代码'].apply(lambda x:x in list(buy_orderDf_jupiterBj_info.index))]
            ...
            buyDf_jupiterBj = getExtraBuyInfo(buyDf_jupiterBj)
        ...
        for index,row in buyDf_jupiter.iterrows():
            ...
            resDf.loc[dummy_resDfindex, '买入当日收益率(%)'] = resDf.loc[dummy_resDfindex, '买入当天收盘价'] / resDf.loc[dummy_resDfindex, '成交均价'] - 1

        for index,row in buyDf_jupiterBj.iterrows():
            dummy_resDfindex = len(resDf_Bj)
            stock_code = row['证券代码']
            resDf_Bj.loc[dummy_resDfindex, commonInfoColumns] = row[commonInfoColumns].values
            sel_order = buy_orderDf_jupiterBj[((buy_orderDf_jupiterBj['ordStatus']=='PARTIALLY_FILLED') |
                                             (buy_orderDf_jupiterBj['ordStatus']=='FILLED')) &
                                             (buy_orderDf_jupiterBj['stockcode']==stock_code)]
            resDf_Bj.loc[dummy_resDfindex, ['成交数量', '成交金额']] = sel_order[['lastQty', 'lastAmt']].sum().values
            resDf_Bj.loc[dummy_resDfindex, '成交均价'] = resDf_Bj.loc[dummy_resDfindex, '成交金额'] / resDf_Bj.loc[dummy_resDfindex, '成交数量']
            resDf_Bj.loc[dummy_resDfindex, '买入当日收益率(%)'] = resDf_Bj.loc[dummy_resDfindex, '买入当天收盘价'] / resDf_Bj.loc[dummy_resDfindex, '成交均价'] - 1
        ...
        for index, row in buyDf_metis.iterrows():
            ...
            resDf_metis.loc[dummy_resDfindex, '买入当日收益率(%)'] = resDf_metis.loc[dummy_resDfindex, '买入当天收盘价'] / resDf_metis.loc[dummy_resDfindex, '成交均价'] - 1
        ...
        for index, row in buyDf_jupiter001.iterrows():
            ...
            resDf001.loc[dummy_resDfindex, '买入当日收益率(%)'] = resDf001.loc[dummy_resDfindex, '买入当天收盘价'] / resDf001.loc[dummy_resDfindex, '成交均价'] - 1
        ...
        for index,row in buyDf_saturn.iterrows():
            ...
            resDf_pj2.loc[dummy_resDfpj2_index, '买入当日收益率(%)'] = resDf_pj2.loc[dummy_resDfpj2_index, '买入当天收盘价'] / resDf_pj2.loc[dummy_resDfpj2_index, '成交均价'] - 1
        ...
        for index, row in buyDf_leda.iterrows():
            ...
            resDf_leda.loc[dummy_resDfindex, '买入当日收益率(%)'] = resDf_leda.loc[dummy_resDfindex, '买入当天收盘价'] / resDf_leda.loc[dummy_resDfindex, '成交均价'] - 1
        ...
        # 针对jupiterBj进行额外信息的填充
        for index, row in buy_log_Df_jupiterBj.iterrows():
            stockCode = row['Unnamed: 0']
            ZT_Time = row['ZT_Time']
            print(stockCode, ZT_Time, '额外信息补充')
            if (len(resDf_Bj[resDf_Bj['发生日期'] == Adate]) > 0) and \
                    (stockCode in resDf_Bj[resDf_Bj['发生日期'] == Adate]['证券代码'].values):
                resDfIndex = resDf_Bj[resDf_Bj['证券代码'] == stockCode].index
                if len(resDfIndex) > 1:
                    resDfIndex = resDfIndex[-1:]
            else:  # 如果存在正常的数据
                # 针对当天在o32中没有但在log中有的票进行数据填充
                resDfIndex, stock_code = len(resDf), row['Unnamed: 0']
                resDf_Bj.loc[resDfIndex, ['证券代码', '证券名称']] = stock_code, name_data.loc[lastdate, stock_code]['STOCK_NAME'].values[0]
                resDf_Bj.loc[resDfIndex, ['发生日期', '委托方向', '成交数量', '成交金额']] = Adate, '买入', 0, 0
            # 其他columns，计算买入日形态和突破时间
            try:
                resDf_Bj.loc[resDfIndex, '形态'] = tot_pattern_df.loc[(pd.to_datetime(date), stockCode), 'label_pattern']
            except:
                print(f'{date}-{stockCode}需要重新计算形态数据！')
            resDf_Bj.loc[resDfIndex, '买入当日突破时间'] = ZT_Time
            resDf_Bj.loc[resDfIndex, '委托金额'] = buy_log_Df_jupiterBj.loc[index, 'totalOrderAmt']
        # 针对jupNew进行额外信息的填充
        ...
        resDf_Bj['涨跌幅(%)'] = resDf_Bj['涨跌幅(%)'].astype(float)
        resDf_pj2['涨跌幅(%)'] = resDf_pj2['涨跌幅(%)'].astype(float)
        resDf_pj3['涨跌幅(%)'] = resDf_pj3['涨跌幅(%)'].astype(float)
        return resDf, resDf_Bj, resDf_pj2, resDf_pj3, resDf001, resDf_metis, resDf_leda
    ...
    def writeExcelBj(resDf):
        writePath = f'/data/group/800463/日内强势股/实盘分析记录/日内强势股总买入记录/日内强势股总买入记录Bj-{date}.xlsx'
        sheetname = '总买入记录'
        resDf.replace(np.nan,'',inplace=True)
        print('write sheet %s for %s!!!!!!!' % (sheetname,writePath))
        write_excel(resDf, writePath, sheetname)
    ...
    resDf1, resDf1_Bj, resDf1_pj2, resDf1_pj3, resDf1001, resDf1_metis, resDf1_leda = updatePositionBuy(todayBuyDf.copy(), historyBuyDf.copy(), historyBuyDf_Bj.copy(),
                                                                historyBuyDf_pj2.copy(), historyBuyDf_pj3.copy(), logDf.copy(), buy_orderDf.copy(), historyBuyDf001.copy(),
    ...
    if True in resDf1_Bj['形态'].isnull():
        unfilled_pattern = resDf1_Bj[resDf1_Bj['形态'].isnull()]
        for index,row in unfilled_pattern.iterrows():
            stock_code, pre_date= row[['证券代码','发生日期']].values
            pre_date = pre_date[0:4] + pre_date[5:7] + pre_date[8:]
            # print(stock_code, pre_date)
            try:
                resDf1_Bj.loc[index, '形态'] = tot_pattern_df.loc[(pd.to_datetime(pre_date), stock_code), 'label_pattern']
            except:
                print(f'{pre_date}-{stock_code}需要重新计算形态数据！')
    ...
    for i in rjf.index:
        ...
        # 如果项目三买了则jupiter不买，所以先判断是否在今日的项目三
            ...
            if len(resDf1_Bj[((resDf1_Bj['证券代码'] == rjf.loc[i]['证券代码']) & (resDf1_Bj['发生日期'] == rjf.loc[i]['发生日期']))]) > 0:
                buy_index = resDf1_Bj[((resDf1_Bj['证券代码'] == rjf.loc[i]['证券代码']) & (resDf1_Bj['发生日期'] == rjf.loc[i]['发生日期']))].index
                resDf1_Bj.loc[buy_index, '未完成原因'] = rjf.loc[i]['未完成原因']
    ...
    #%% jupiter北交所的信号信息
    factor_time_cost = pd.read_excel('/data/group/800463/日内强势股/log_parse/因子耗时/因子耗时_%s_prod.xlsx' % Adate, sheet_name='因子耗时Bj')
    from ProdWork.Param_config_data import thred_dict_jup_bjs_v1 as thred_dict

    model_list = list(thred_dict.keys())
    today_index = resDf1_Bj[resDf1_Bj['发生日期'] == Adate].index
    if '拉抬打压信息' not in resDf1_Bj.columns.tolist():
        resDf1_Bj['拉抬打压信息'] = np.nan
    for i in today_index:
        stock = resDf1_Bj.loc[i, '证券代码']
        this_stock_cla_model_score = 0
        this_stock_reg_model_score = 0
        this_stock_ZT_model_score = 0
        for model in model_list:
                ...
                if 'ZTBysModel_probability' not in factor_time_cost[factor_time_cost['Unnamed: 0'] == stock].columns:
                    ...
                    if factor_time_cost[factor_time_cost['Unnamed: 0'] == stock]['ZTBysModel_probability'].isnull().sum() != 1:
                        ...
                        if 'Cla' in model:
                            ...
                            this_stock_reg_model_score += model_res
        resDf1_Bj.loc[i, '回归信号'] = this_stock_reg_model_score
        resDf1_Bj.loc[i, '分类信号'] = this_stock_cla_model_score
        resDf1_Bj.loc[i, '前涨停信号'] = this_stock_ZT_model_score
        # if np.isnan(factor_time_cost[factor_time_cost['Unnamed: 0'] == stock]['MRisk_info'].values[0])==False:
        if 'MRisk_info' in factor_time_cost.columns.tolist():
            try:  # 这个try针对20240208手工暂停情况进行的处理，后续可以 剔除
                resDf1_Bj.loc[i, '拉抬打压信息'] = factor_time_cost[factor_time_cost['Unnamed: 0'] == stock]['MRisk_info'].values[0]
            except:
                resDf1_Bj.loc[i, '拉抬打压信息'] = ''

    resDf1_Bj['买入当天持仓金额'] = resDf1_Bj['成交数量'] * resDf1_Bj['买入当天收盘价']
    ...
    f_data['bjs'] = (f_data.reset_index()['Ticker'].apply(lambda x: x[-2:] == 'BJ')).values
    ...
    is_zt[f_data['bjs']] = (f_data['close'] == np.floor(f_data['pre_close'] * 100 * 1.3 + 1e-8) / 100)
    ...
    sellDf_Bj_copy = resDf1_Bj.copy()
    sellDf_Bj_copy['买入日期'] = sellDf_Bj_copy['发生日期'].apply(lambda x: pd.Timestamp(x))
    sellDf_Bj_copy = sellDf_Bj_copy.rename(columns={'买入日期': 'dt', '证券代码': 'Ticker'}).set_index(['dt', 'Ticker'])
    resDf1_Bj['last_is_zt'] = last_is_zt.reindex(sellDf_Bj_copy.index).values
    ...
    # 读取因子耗时文件
    from ProdWork.Param_config_data import *
    ...
    # -------------------------在jupiterBj买入汇总中添加o2ul的计算-------------------------------------------
    if 'TN_o2ul' not in resDf1_Bj.columns:
        resDf1_Bj['TN_o2ul'] = np.nan
    o2ul_nan_samples = resDf1_Bj[resDf1_Bj['TN_o2ul'].isnull()]

    # date_ini = o2ul_nan_samples['发生日期'].apply(lambda x:x[:4]+x[5:7]+x[8:10]).min()
    # end_date = o2ul_nan_samples['发生日期'].apply(lambda x:x[:4]+x[5:7]+x[8:10]).max()
    resDf1_Bj_copy = resDf1_Bj.copy()
    resDf1_Bj_copy['发生日期'] = resDf1_Bj_copy['发生日期'].apply(lambda x: pd.Timestamp(x[:4] + x[5:7] + x[8:10]))
    tmp_df = 100 * md_data.reindex(resDf1_Bj_copy.rename(columns={'发生日期': 'dt',
                                                               '证券代码': 'Ticker'}).set_index(['dt', 'Ticker']).index)['label_TN_o2ul']
    for i in o2ul_nan_samples.index:
        buy_date = resDf1_Bj.loc[i]['发生日期']
        stock = resDf1_Bj.loc[i]['证券代码']
        resDf1_Bj.loc[i, 'TN_o2ul'] = tmp_df.loc[buy_date, stock]
