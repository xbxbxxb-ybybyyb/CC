...
if len(sys.argv) > 1:
    ...
    # date = '20250507' # 若未在当个交易日晚上运行程序，需要在次日早上修改date
print('jupiterBj更新实盘触发标签文件：current date = %s' % date)
...
Labels_prod_summary_old = pd.read_excel('/data/group/800463/日内强势股/log_parse/因子耗时/实盘触发标签汇总Bj_%s.xlsx' % Alastdate)

raw_last_date_factor_time_cost = pd.read_excel('/data/group/800463/日内强势股/jupiterBj_log_parse/因子耗时/因子耗时_%s_prod.xlsx' % Alastdate)
raw_last_date_factor_time_cost = raw_last_date_factor_time_cost[~raw_last_date_factor_time_cost['Unnamed: 0'].duplicated()]

raw_last_date_model_compare = pd.read_excel(f'/data/group/800463/日内强势股/jupiterBj_log_parse/模型差异/{lastdate}/模型差异_{lastdate}_prod.xlsx', sheet_name='本地投票结果').rename(columns = {'Ticker':'Unnamed: 0'})
# 获取模型的预测结果
...
sell_record = pd.read_excel(f'/data/group/800463/日内强势股/实盘分析记录/日内强势股总卖出记录/日内强势股总卖出记录Bj-{date}.xlsx').set_index(['买入日期', '证券代码'])['买入时形态']
for index, row in raw_last_date_factor_time_cost.iterrows():
    ...
    try:
        ...
        raw_last_date_factor_time_cost.loc[index, '买入时形态'] = 4
...
# 对于o2ul还未算出的样本进行计算，其实就是筛选昨天买入的进行计算o2ul
...
tmp_df = md_data.reindex(Labels_prod_summary_new_copy.set_index(['dt','Ticker']).index)['label_TN_o2ul']
for idx in o2ul_nan_samples.index:
    buy_date = Labels_prod_summary_new.loc[idx]['dt']
    stock = Labels_prod_summary_new.loc[idx]['Ticker']
    Labels_prod_summary_new.loc[idx, 'TN_o2ul'] = tmp_df.loc[buy_date.strftime('%Y%m%d'), stock] * 100

Labels_prod_summary_new.to_excel('/data/group/800463/日内强势股/log_parse/因子耗时/实盘触发标签汇总Bj_%s.xlsx' % Adate, index=False)
print('create file %s!!!!!!!!!!' % '/data/group/800463/日内强势股/log_parse/因子耗时/实盘触发标签汇总Bj_%s.xlsx' % Adate)
print(f'2-7.label_summary_jupBj耗时{round(time.time() - t1, 6)}秒')