...
from ProdWork.intra_strong.update.func_Basic_zt import cal_Basic_zt
if len(sys.argv) > 1:
...
while True:
    md_data = pd.read_pickle('/data/group/800463/日内强势股/实盘分析记录/日内强势股成交记录/前置数据/md_data_wind/' + f'{start_date}-{today_date}.pkl')
...
jupiterBj_samples = pd.read_excel(f'/data/group/800463/日内强势股/log_parse/因子耗时/因子耗时_{today_date_str}_prod.xlsx', sheet_name='因子耗时Bj', index_col=0)
...
jupBj_lattern = pd.read_hdf('/data/group/800463/project/project1_prod/left_v2310/Basic_zt/Basic_zt_BJ.h5')[['label_pattern']]
...
new_samples = list(sorted(list(set(jupiter_samples.index).union(europa_samples.index).union(metis_samples.index).union(leda_samples.index).union(jupiterBj_samples.index))))
...
def get_stk_pattern(stk_code_list):
    ...
    for stk_code in tqdm(stk_code_list):
        # if idx < 6:
        #     continue
        pre_close, close, ul_price = md_data.loc[pd.to_datetime(today_date), stk_code][['pre_close', 'close', 'ul_price']].values
        #
        # # 计算ul_price
        # if stk_code[-2:] == 'BJ':
        #     ul_price = np.floor(pre_close * 100 * 1.3 + 1e-8) / 100
        # elif stk_code[0] == '3' or stk_code[:3] == '688':
        #     ul_price =
        ret_list.append(cal_Basic_zt(sd, mdp, stk_code, today_date, pre_close, close, ul_price)['label_pattern'].values[0])
    return ret_list

ret_dict = SpeedUtil.multiprocess(24, get_stk_pattern, new_samples)
# ret_dict = get_stk_pattern(new_samples[-1:])
ret_result = dict()
ret_list = list()
for k in ret_dict:
    try:
...
# ret_list = get_stk_pattern(new_samples[-1:])
...
tot_lattern = pd.concat([jup_lattern, eur_lattern, metis_lattern, today_pattern, jupBj_lattern], axis=0).sort_index().reset_index().drop_duplicates(subset=['dt', 'Ticker'], keep='first').set_index(['dt', 'Ticker']).sort_index()
...
#%% 检查数据
check = pd.read_pickle(output_path + f'{today_date}.pkl')
check = check.reset_index()
check['is_BJ'] = check.Ticker.apply(lambda x: x[-3:])