...
jupiterBj_record_path = '/data/group/800463/日内强势股/jupiterBj_log_parse/'
...
for envir in enviroment_list:
    ...
    jupiterBj_log_parse_fpath = jupiterBj_path + f'因子耗时/因子耗时_{today_date_str}_{envir}.xlsx'
    ...
    while not os.path.exists(jupiter_log_parse_fpath):
        ...
        time.sleep(30)
    while not os.path.exists(jupiterBj_log_parse_fpath):    # 有可能文件内为空，但是文件必须要有
        send_message(f'{today_date} jupiterBj北交所因子耗时文件缺失')
        time.sleep(30)
    ...
    jupiterBj_factor_cost_df_dict = pd.read_excel(jupiterBj_log_parse_fpath, sheet_name=None, index_col=0)
    ...
    total_factor_cost_df_dict['因子耗时Bj'] = jupiterBj_factor_cost_df_dict['因子耗时']
    ...
    jupiterBj_tupo_df_dict = pd.read_excel(jupiterBj_record_path + f'每日突破/每日突破_{today_date}_{envir}.xlsx', sheet_name=None, index_col=0)
    ...
    jupBj_deal = jupiterBj_tupo_df_dict['每日订单']
    ...
    cpp_tupo_df_dict['每日订单'] = pd.concat([saturn_deal, eur_deal, jup_deal, jupBj_deal, leda_deal, ceres_deal, p4_deal], axis=0)
    ...
    cpp_tupo_df_dict['每日突破Bj'] = jupiterBj_tupo_df_dict['每日突破']
    ...
    jupiterBj_reject = jupiterBj_tupo_df_dict['每日拒绝']
    ceres_reject = ceres_tupo_df_dict['每日拒绝']
    p4_reject = p4_tupo_df_dict['每日拒绝']
    if len(saturn_reject) != 0 or len(leda_reject) != 0 or len(jupiter_reject) != 0 or len(jupiterBj_reject) != 0 or len(ceres_reject) != 0 or len(p4_reject) != 0:
        concat = pd.concat([cpp_reject, leda_reject, saturn_reject, jupiter_reject, jupiterBj_reject, ceres_reject, p4_reject], axis=0)
    else:
        concat = cpp_reject
