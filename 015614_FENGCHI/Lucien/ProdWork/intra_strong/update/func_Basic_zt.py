...
def cal_pattern(md_df, ZT_Time, ul_price, close_price, tolerance_period):
    md_df = md_df.sort_values(by=['MDTime','TradeIndex'], ascending=True)
    ...
    return df
"""
#%% 20250122 改用少森的函数：因为北交所没有逐笔成交数据，所以这里使用tick
def cal_Basic_zt(sd, mdp, stock, date, pre_close, close, ul_price, tolerance_period=10):
    if stock[-2:] == 'BJ':
        xdb_df = sd.get_tickex(date, stock)
        rename_dic = {'md_time': 'MDTime', 'high_px': 'HighPx', 'last_px': 'LastPx'}
        all_md_df = xdb_df.rename(columns=rename_dic)
    else:
        all_md_df = mdp.get_data_by_date('Stock', stock, date)
    all_md_df['MDTime'] = all_md_df['MDTime'].astype(int)
    all_md_df = all_md_df[all_md_df['LastPx'] > 0]
    ZT_Time = all_md_df[all_md_df['HighPx'] == ul_price]['MDTime'].min()

    df = pd.DataFrame([[ZT_Time]], columns=['ZT_Time'])
    df.index = pd.Index([(pd.Timestamp(date), stock)], name=['dt', 'Ticker'])
    if (stock[-2:] == 'SH') and (date < '20180820'):  # 老规则，强行变成尾盘集合竞价
        all_md_df.loc[all_md_df['MDTime'] > 145700000, 'LastPx'] = close
    pattern_df = cal_pattern(all_md_df, ZT_Time, ul_price, close, tolerance_period)
    pattern_df.index = df.index
    df = df.join(pattern_df)
    return df
...
def cal_pattern(md_df, ZT_Time, ul_price, close_price, tolerance_period):
    if abs(close_price-ul_price)>0.0001: # 未收盘涨停
        ...
        last_not_zt_time = md_df[(md_df['LastPx']!=ul_price)].iloc[-1]['MDTime']
        if len(md_df[(md_df['MDTime']>=last_not_zt_time) & (md_df['LastPx']==ul_price)])>0:
            last_zt_start_time = md_df[(md_df['MDTime']>=last_not_zt_time) & (md_df['LastPx']==ul_price)].iloc[0]['MDTime']
        else:
            #xdb的tick数据暂不包含1500之后的，可能集合竞价才恢复涨停，而集合竞价的时间可能超过1500。
            last_zt_start_time=150000000
        not_zt_len = fun_common_time_interval(ZT_Time, last_zt_start_time)
        if not_zt_len>tolerance_period:
            label_pattern = 3
        else:
            label_pattern = 4
    #首次涨停结束的时间
    after_zt_md_df = md_df[md_df['MDTime']>=ZT_Time].copy()
    after_zt_not_zt_md_df = after_zt_md_df[(after_zt_md_df['LastPx']<ul_price)]
    if len(after_zt_not_zt_md_df) == 0:
        firstUL_end_Time = 150000000
    else:
        firstUL_end_Time = after_zt_not_zt_md_df.iloc[0]['MDTime']

    df = pd.DataFrame([[label_pattern, firstUL_end_Time]], columns=['label_pattern', 'label_firstUL_end_Time'])
    return df