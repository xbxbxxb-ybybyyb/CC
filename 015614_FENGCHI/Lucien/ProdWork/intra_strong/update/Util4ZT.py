...
year_start_date = today_date[:4] + '0101'   # TODO: 由于部分样本还没有，仍是按去年计算，明天修改
year_start_date = '20240101'
...
#    --------------------------------------------------画图------------------------------------------------
    ...
    elif strategy == 'jupiterBj':
        Label_summary = pd.read_excel('/data/group/800463/日内强势股/log_parse/因子耗时/实盘触发标签汇总Bj_%s.xlsx' % Adate)
...
def get_DfTodayInfo(buyDf, buyDate, strategy='jupiter'):
    ...
    if strategy == 'jupiter':
        ...
        today_tot_sample = pd.read_excel('/data/group/800463/日内强势股/log_parse/因子耗时/因子耗时_%s_prod.xlsx' % buyDate, sheet_name='因子耗时Bj')
    elif strategy == 'Metis':
        today_tot_sample = pd.read_excel('/data/group/800463/日内强势股/metis_log_parse/因子耗时/因子耗时_%s_prod.xlsx' % buyDate, sheet_name='因子耗时Metis')
    elif strategy == 'Leda':
        today_tot_sample = pd.read_excel('/data/group/800463/日内强势股/leda_log_parse/因子耗时/因子耗时_%s_prod.xlsx' % buyDate, sheet_name='因子耗时')
...
def get_sellDfTotalInfo(sellDf,Adate,fig,strategy='jupiter'):
    if strategy == 'jupiter':
        ...
        labels_zt = pd.read_excel('/data/group/800463/日内强势股/log_parse/因子耗时/实盘触发标签汇总Bj_%s.xlsx' % Adate)
    elif strategy == 'jupiterNew':
    ...
    sellDf_2021 = sellDf[(sellDf['买入日期']>='2024-12-31') & (sellDf['买入日期']<'2025-12-31')]
    sellResDf.loc['2025累计盈利'] = sellDf_2021[(sellDf_2021['是否全部卖出']==1)|(sellDf_2021['是否全部卖出']==0)]['卖出部分盈利金额'].sum()
    ...
    if strategy == 'jupiterBj':
        sell_df = pd.DataFrame(sellSampleDf['卖出部分收益率(%)'].rolling(rollnums_num, 1).mean().dropna())
    else:
    ...
    if strategy == 'Leda':
        ...
        sell_df_roll = cal_cumsum_mean(sellSampleDf['卖出部分收益率(%)'], 1)  # 因为北交所样本数量太少
    else:
    ...
    if strategy == 'jupiter':
        ...
        plt.savefig("/data/group/800463/日内强势股/实盘分析记录/日内强势股成交记录/收益画图/%s_Bj_%s.png" % (Adate, '生产环境'), dpi=120)
    elif strategy == 'Metis':
...
def get_lztDfTotalInfo(cybsellDf,Adate,strategy='jupiter'):
    ...
    cybsellResDf.loc['累计全部卖出样本数'] = len(sellSampleDf)
    cybsellResDf.loc['累计盈利'] = cybsellDf[(cybsellDf['是否全部卖出'] == 1) | (cybsellDf['是否全部卖出'] == 0)]['卖出部分盈利金额'].sum()
    cybsellDf_2021 = cybsellDf[(cybsellDf['买入日期']>='2024-12-31') & (cybsellDf['买入日期']<'2025-12-31')]
    cybsellResDf.loc['2025累计盈利'] = cybsellDf_2021[(cybsellDf_2021['是否全部卖出']==1)|(cybsellDf_2021['是否全部卖出']==0)]['卖出部分盈利金额'].sum()
    cybsellResDf.loc['次均等权卖出收益率(%)'] = '%.2f%%' % (sellSampleDf['卖出部分收益率(%)'].mean())
    cybsellResDf.loc['次均成交额加权收益率(%)'] = '%.2f%%' % (sellSampleDf['加权收益率'].sum() / sellSampleDf['买入金额'].sum())
