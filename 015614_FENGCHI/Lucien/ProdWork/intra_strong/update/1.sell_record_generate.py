...
from ProdWork.intra_strong.update.recordUtil import getExtraBuyInfo, getExtraSellInfo, calProfit, calProfit_pj2
...
def updatePositionBuy(buyDf, resDf, resDf_Bj, resDf001, resDf_metis, resDf_leda, resDf_pj2, buy_orderDf):
    ...
    for index, row in buyDf_jupiterBj.iterrows():
        ...
        stock_code = row['证券代码']
        resDf_Bj.loc[dummy_resDfindex, buy_commonInfoColumns_rename] = row[buy_commonInfoColumns].values
        ...
        resDf_Bj.loc[dummy_resDfindex, ['买入数量', '买入金额']] = sel_order[['lastQty', 'lastAmt']].sum().values
        resDf_Bj.loc[dummy_resDfindex, '买入成交均价'] = resDf_Bj.loc[dummy_resDfindex, '买入金额'] / resDf_Bj.loc[dummy_resDfindex, '买入数量']
        resDf_Bj.loc[dummy_resDfindex, '买入当日收益率(%)'] = resDf_Bj.loc[dummy_resDfindex, '买入当天收盘价'] / resDf_Bj.loc[dummy_resDfindex, '买入成交均价'] - 1
    ...
    return resDf[resDf['买入数量'] > 0], resDf_Bj[resDf_Bj['买入数量'] > 0], resDf_pj2, resDf001[resDf001['买入数量'] > 0], resDf_metis[resDf_metis['买入数量'] > 0], resDf_leda[resDf_leda['买入数量'] > 0]

def updatePositionSell(sellDf, resDf1, resDf1_Bj, resDf1001, resDf1_metis, resDf1_leda, resDf1_pj2, date, dead_circle):
    ...
    resDf1_Bj.replace(np.nan, '', inplace=True)
    ...
    for index, row in sellDf.iterrows():
        ...
        def jupiterBj_sell_one_day_holding(sold_qty, resDf1_Bj):  # 对一天的jupiter进行卖出
            historyIndex = resDf1_Bj[(resDf1_Bj['证券代码'] == stockCode) & (resDf1_Bj['是否全部卖出'] != 1)].index[0]
            totalSold_indicator = resDf1_Bj.loc[historyIndex]['是否全部卖出']
            if resDf1_Bj.loc[historyIndex, '总卖出数量'] == '':
                sold_already = 0
            else:
                sold_already = resDf1_Bj.loc[historyIndex, '总卖出数量']
            qty_allocate_here = min((int(resDf1_Bj.loc[historyIndex, '买入数量']) - sold_already), sold_qty)
            if totalSold_indicator == '' or resDf1_Bj.loc[historyIndex, '总卖出数量'] == '':  # 没卖出过
                resDf1_Bj.loc[historyIndex, sellCommonColumns] = sellDf.loc[index, CommonInfoColumns].values
                resDf1_Bj.loc[historyIndex, '卖出数量'] = qty_allocate_here
                resDf1_Bj.loc[historyIndex, '卖出金额'] = qty_allocate_here * sellDf.loc[index, '成交均价']
                resDf1_Bj.loc[historyIndex, '总卖出数量'] = qty_allocate_here
                resDf1_Bj.loc[historyIndex, '卖出金额占市场比'] = qty_allocate_here / market_volume['volume'].loc[date, stockCode]
            elif totalSold_indicator == 0:  # 之前未全部卖出
                resDf1_Bj.loc[historyIndex, '卖出日期'] += ',' + str(sellDf.loc[index, '发生日期'])
                resDf1_Bj.loc[historyIndex, '卖出数量'] = str(resDf1_Bj.loc[historyIndex, '卖出数量']) + ',' + str(qty_allocate_here)
                resDf1_Bj.loc[historyIndex, '卖出成交均价'] = str(resDf1_Bj.loc[historyIndex, '卖出成交均价']) + ',' + str(sellDf.loc[index, '成交均价'])
                resDf1_Bj.loc[historyIndex, '卖出金额'] = str(resDf1_Bj.loc[historyIndex, '卖出金额']) + ',' + str(qty_allocate_here * sellDf.loc[index, '成交均价'])
                resDf1_Bj.loc[historyIndex, '卖出日期开盘价'] = str(resDf1_Bj.loc[historyIndex, '卖出日期开盘价']) + ',' + str(sellDf.loc[index, '卖出日期开盘价'])
                resDf1_Bj.loc[historyIndex, '总卖出数量'] = resDf1_Bj.loc[historyIndex, '总卖出数量'] + qty_allocate_here
                resDf1_Bj.loc[historyIndex, '卖出金额占市场比'] = str(resDf1_Bj.loc[historyIndex, '卖出金额占市场比']) + ',' + str(qty_allocate_here / market_volume['volume'].loc[date, stockCode])
            if resDf1_Bj.loc[historyIndex, '总卖出数量'] >= resDf1_Bj.loc[historyIndex, '买入数量']: # NOTE：jupiterBj暂时没做提前小单这个功能
                resDf1_Bj.loc[historyIndex, '是否全部卖出'] = 1
            volume_left_to_be_sell = sold_qty - qty_allocate_here
            return volume_left_to_be_sell, resDf1_Bj
        ...
        while volume_left_to_be_sell > 1e-5: # 1108 只剩100股，大家时间都一样
            ...
            if len(resDf1_Bj[(resDf1_Bj['证券代码'] == stockCode) & (resDf1_Bj['是否全部卖出'] != 1)]['买入日期']) > 0:
                jupiterBj_min_date = resDf1_Bj[(resDf1_Bj['证券代码'] == stockCode) & (resDf1_Bj['是否全部卖出'] != 1)]['买入日期'].min()
            else:
                jupiterBj_min_date = '2030-12-31'
            ...
            # NOTE: 针对北交所的个股，目前只有jupiterBj这一个策略有，所以没有在全部可能里面加入北交所
            if (len(resDf1[(resDf1['证券代码'] == stockCode) & (resDf1['是否全部卖出'] != 1) & (resDf1['买入日期'] != Adate)]['买入日期']) > 0) and \
            ...
            elif (len(resDf1[(resDf1['证券代码'] == stockCode) & (resDf1['是否全部卖出'] != 1) & (resDf1['买入日期'] != Adate)]['买入日期']) == 0) and \
                    (len(resDf1_Bj[(resDf1_Bj['证券代码'] == stockCode) & (resDf1_Bj['是否全部卖出'] != 1) & (resDf1_Bj['买入日期'] != Adate)]['买入日期']) > 0) and \
                    (len(resDf1001[(resDf1001['证券代码'] == stockCode) & (resDf1001['是否全部卖出'] != 1) & (resDf1001['买入日期'] != Adate)]['买入日期']) == 0) and \
                    (len(resDf1_metis[(resDf1_metis['证券代码'] == stockCode) & (resDf1_metis['是否全部卖出'] != 1) & (resDf1_metis['买入日期'] != Adate)]['买入日期']) == 0) and\
                    (len(resDf1_leda[(resDf1_leda['证券代码'] == stockCode) & (resDf1_leda['是否全部卖出'] != 1) & (resDf1_leda['买入日期'] != Adate)]['买入日期']) == 0) and\
                    (len(resDf1_pj2[(resDf1_pj2['证券代码'] == stockCode) & (resDf1_pj2['是否全部卖出'] != 1) & (resDf1_pj2['买入日期'] != Adate)]['买入日期']) == 0):
                print(f'该标的{stockCode}只有jupBj存在持仓')
                volume_left_to_be_sell, resDf1_Bj = jupiterBj_sell_one_day_holding(volume_left_to_be_sell, resDf1_Bj)
                ...
                # 该标的至少2个策略存在持仓, 先卖早买入的部分，北交所的不可能进入到这个else里面来
                min_date = pd.Series([jupiter_min_date, jupiter_min_date001, metis_min_date, leda_min_date, saturn_min_date]).sort_values().iloc[0]
                max_date = pd.Series([jupiter_min_date, jupiter_min_date001, metis_min_date, leda_min_date, saturn_min_date]).sort_values(ascending=False).iloc[0]
                ...
                #     volume_left_to_be_sell, resDf1 = jupiter_sell_one_day_holding(volume_left_to_be_sell, resDf1)
                    ...
                    break
    return resDf1, resDf1_Bj, resDf1_pj2, resDf1001, resDf1_metis, resDf1_leda
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        # date = '20250811' # 若未在当个交易日晚上运行程序，需要在次日早上修改date
    ...
    historyFileBj = '/data/group/800463/日内强势股/实盘分析记录/日内强势股总卖出记录/日内强势股总卖出记录Bj-%s.xlsx' % lastdate
    ...
    if os.path.exists(historyFileBj):
        historySellDf_Bj = deal_Unnamed(pd.read_excel(historyFileBj, sheet_name='总卖出记录'))
        if len(historySellDf_Bj) > 0:
            historySellDf_Bj['是否全部卖出'] = historySellDf_Bj.apply(lambda x: 0 if x['卖出比例'] != '100.00%' else 1, axis=1)
        else:
            historySellDf_Bj = pd.DataFrame(columns=historySellDf.columns)
    else:
        historySellDf_Bj = pd.DataFrame(columns=historySellDf.columns)
    ...
    if os.path.exists(logFile):
        ...
        logDf_Bj = pd.read_excel(f'/data/group/800463/日内强势股/jupiterBj_log_parse/每日突破/每日突破_{date}_prod.xlsx', sheet_name='每日突破')
        if len(logDf_Bj) > 0:
    ...
    orderDf_jupiterBj = orderDf[(orderDf['actionSource'] == 'JupiterBJ') &
                              ((orderDf['orderType'] == 'SplitLastShot') |
                               (orderDf['orderType'] == 'MRiskSplitShot') |
                               (orderDf['orderType'] == 'JupiterFirstOrder') |
                               (orderDf['orderType'] == 'MRiskSplitLastShotBuy') |
    ...
    todayBuyDf_jupiterBj = todayBuyDf[todayBuyDf['证券代码'].apply(lambda x: x in list(np.unique(orderDf_jupiterBj['stockcode'])))]
    ...
    if '买入时形态' not in resDf.columns:
        resDf['买入时形态'] = np.nan
    resDf_Bj = historySellDf_Bj.copy()
    if '买入时形态' not in resDf_Bj.columns:
        resDf_Bj['买入时形态'] = np.nan
    ...
    resDf1, resDf1_Bj, resDf1_pj2, resDf1001, resDf1_metis, resDf1_leda = updatePositionBuy(todayBuyDf.copy(),
                                                                                          historySellDf.copy(),
                                                                                          historySellDf_Bj.copy(),
    ...
    h_resDf1, h_resDf1_Bj, h_resDf1_pj2, h_resDf1001, h_resDf1_metis, h_resDf1_leda = \
        resDf1.copy(), resDf1_Bj.copy(), resDf1_pj2.copy(), resDf1001.copy(), resDf1_metis.copy(), resDf1_leda.copy()
    sellDf = todaySellDf.copy()

    resDf2, resDf2_Bj, resDf2_pj2, resDf2001, resDf2_metis, resDf2_leda = \
        updatePositionSell(todaySellDf.copy(), resDf1, resDf1_Bj, resDf1001, resDf1_metis, resDf1_leda, resDf1_pj2, date, dead_circle=0)
    # print('updatePositionSell:resdf2:', resDf2.shape, resDf2.iloc[0])
    resDf2.replace('', np.nan, inplace=True)
    resDf2_Bj.replace('', np.nan, inplace=True)
    ...
    # --------------------------添加JupiterBj买入形态和o2ul信息------------------------------
    if True in resDf2_Bj['买入时形态'].isnull():
        unfilled_pattern = resDf2_Bj[resDf2_Bj['买入时形态'].isnull()]
        for index, row in unfilled_pattern.iterrows():
            stock_code, pre_date = row[['证券代码', '买入日期']].values
            pre_date = pre_date[0:4] + pre_date[5:7] + pre_date[8:]
            if (pd.to_datetime(pre_date), stock_code) in tot_pattern_df.index:
                resDf2_Bj.loc[index, '买入时形态'] = tot_pattern_df.loc[(pd.to_datetime(pre_date), stock_code), 'label_pattern']
            else:
                print(stock_code, pre_date, '计算买入时形态')
    ...
    if 'TN_o2ul' not in resDf2_Bj.columns:
        resDf2_Bj['TN_o2ul'] = np.nan
    o2ul_nan_samples = resDf2_Bj[resDf2_Bj['TN_o2ul'].isnull()]

    if len(o2ul_nan_samples) != 0:
        # date_ini = o2ul_nan_samples['买入日期'].apply(lambda x: x[:4] + x[5:7] + x[8:10]).min()
        # end_date = o2ul_nan_samples['买入日期'].apply(lambda x: x[:4] + x[5:7] + x[8:10]).max()
        resDf2_Bj_copy = resDf2_Bj.copy()
        resDf2_Bj_copy['买入日期'] = resDf2_Bj_copy['买入日期'].apply(lambda x: pd.Timestamp(x[:4] + x[5:7] + x[8:10]))
        tmp_df = 100 * md_data.reindex(resDf2_Bj_copy.rename(columns={'买入日期': 'dt', '证券代码': 'Ticker'}).set_index(['dt', 'Ticker']).index)['label_TN_o2ul']
        for i in o2ul_nan_samples.index:
            buy_date = resDf2_Bj.loc[i]['买入日期']
            stock = resDf2_Bj.loc[i]['证券代码']
            resDf2_Bj.loc[i, 'TN_o2ul'] = tmp_df.loc[buy_date, stock]
    ...
    resDf3_Bj = calProfit(resDf2_Bj.copy())
    ...
    resDf3_Bj.replace('', np.nan, inplace=True)
    ...
    resDf3_Bj['卖出比例'] = (resDf3_Bj['卖出数量'].apply(cal_sum).astype(float) / resDf3_Bj['买入数量']).fillna(0).apply(lambda x: '%.2f%%' % (x * 100))
    resDf3_Bj.replace(np.nan, '', inplace=True)
    resDf3_Bj['卖出比例'] = resDf3_Bj['卖出比例'].apply(lambda x: '100.00%' if float(x[:-1]) == 100 else x) # NOTE: 北交所没有开发提前下单的需求
    ...
    resDf3_Bj[['证券名称', '证券代码', '买入日期', '卖出日期', '买入数量', '买入金额', '买入成交均价', '买入当天涨停价',
            '买入当天是否收盘涨停', '买入当天收盘价', '买入当日收益率(%)', '卖出数量', '卖出成交均价', '卖出金额',
            '是否全部卖出', '卖出比例', '卖出部分盈利金额', '卖出部分收益率(%)', '实际是否正收益', '卖出日期开盘价',
            '卖出挂单笔数', '卖出成交笔数', '卖出开始时间', '卖出结束时间', '卖出金额占市场比', '未完成原因',
            '总卖出数量', '买入时形态', 'TN_o2ul']].to_excel('/data/group/800463/日内强势股/实盘分析记录/日内强势股总卖出记录/日内强势股总卖出记录Bj-%s.xlsx' % date, sheet_name='总卖出记录', index=False)

    print('create sheet 总卖出记录 for  %s' % '/data/group/800463/日内强势股/实盘分析记录/日内强势股总卖出记录/日内强势股总卖出记录Bj-%s.xlsx' % date)
