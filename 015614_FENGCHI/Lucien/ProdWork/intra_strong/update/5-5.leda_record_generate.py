...
from ProdWork.intra_strong.update.Util4ZT import get_buyDfTotalInfo, get_buyDfTodayInfo, get_sellDfTotalInfo, get_sellDfTodayInfo, get_lnonztDfTotalInfo, get_DfTodayInfo
...
if __name__ == "__main__":
    ...
    while not os.path.exists(f'/data/group/800463/日内强势股/实盘分析记录/日内强势股总买入记录/日内强势股总买入记录Metis-{date}.xlsx'):
        print(f'缺失：日内强势股总买入记录Leda-{date}.xlsx')
        time.sleep(60)
    while not os.path.exists(f'/data/group/800463/日内强势股/实盘分析记录/日内强势股总卖出记录/日内强势股总卖出记录Leda-{date}.xlsx'):
        print(f'缺失：日内强势股总卖出记录Leda-{date}.xlsx')
        time.sleep(60)
    ...
    buyDf = pd.read_excel(f'/data/group/800463/日内强势股/实盘分析记录/日内强势股总买入记录/日内强势股总买入记录Leda-{date}.xlsx')
    sellDf = pd.read_excel(f'/data/group/800463/日内强势股/实盘分析记录/日内强势股总卖出记录/日内强势股总卖出记录Leda-{date}.xlsx')
    cybsellDf = sellDf[sellDf['证券代码'].apply(lambda x:x[0:3] == '300') & (sellDf['买入日期'] >= '2020-08-24')]
    writePath = f'/data/group/800463/日内强势股/实盘分析记录/日内强势股成交记录/Leda成交记录-{date}.xlsx'
    ...
    lnonztsellDf = sellDf[sellDf['last_is_zt']==True]
    lztsellDf = sellDf[sellDf['last_is_zt']==True]

    buyResDf, fig, _ = get_buyDfTotalInfo(buyDf.copy(), Adate, 'Leda')
    todayResDf = get_DfTodayInfo(buyDf.copy(), Adate,'Leda')
    # todayResDf = pd.DataFrame()   # 如果当天没有买入，比如20230913发生事故导致策略没有启动，那么替换上一行为这一行
    todayBuyResDf = get_buyDfTodayInfo(buyDf.copy(), Adate,'Leda')
    sellResDf = get_sellDfTotalInfo(sellDf.copy(), Adate, fig, 'Leda')  # TODO：恢复
    todaySellResDf = get_sellDfTodayInfo(sellDf.copy(), Adate)# ,'Leda')
    # cyb_sellResDf = get_cybDfTotalInfo(cybsellDf.copy(),Adate,'Leda')
    lnonzt_sellResDf = get_lnonztDfTotalInfo(lnonztsellDf.copy(), Adate, 'Leda')

    buyDf['买入当日突破时间'] = buyDf['买入当日突破时间'].astype(str)
    buyDf.rename(columns={'涨跌幅(%)':'买入当日涨跌幅(%)'},inplace=True)

    return_path = f"/data/group/800463/日内强势股/实盘分析记录/日内强势股成交记录/收益画图/{Adate}_Leda_生产环境.png"
    # trade_path = f"/data/group/800463/日内强势股/实盘分析记录/日内强势股成交记录/成交画图/{date}_Leda_生产环境.png"
    ...
    def write_excel(path):
        ...
        end_idx = write_excel_helpTotal_graph(worksheet2,lnonzt_sellResDf,2,3,'Leda卖出汇总',wformat1,merge_format,return_path)
    ...
    print(f'5-4.leda_record_generate运行耗时{round(time.time() - t1, 2)}秒')