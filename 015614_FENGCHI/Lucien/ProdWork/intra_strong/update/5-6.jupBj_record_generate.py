...
import pandas as pd
import numpy as np
from ProdWork.intra_strong.update.recordUtil import write_excel_help, write_excel_helpTotal, write_excel_helpTotal_graph
from ProdWork.intra_strong.update.Util4ZT import get_buyDfTotalInfo,get_buyDfTodayInfo,get_sellDfTotalInfo,get_sellDfTodayInfo,get_lztDfTotalInfo,get_lnonztDfTotalInfo,get_DfTodayInfo
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        # date = '20250207'  # 若未在当个交易日晚上运行程序，需要在次日早上修改date
    ...
    while (os.path.exists('/data/group/800463/日内强势股/实盘分析记录/日内强势股总买入记录/日内强势股总买入记录Bj-%s.xlsx'%(date))==False) |\
          (os.path.exists('/data/group/800463/日内强势股/实盘分析记录/日内强势股总卖出记录/日内强势股总卖出记录Bj-%s.xlsx'%(date))==False):
        print('5-6.jupiterBj等待标签汇总中')
        time.sleep(60)

    buyDf = pd.read_excel('/data/group/800463/日内强势股/实盘分析记录/日内强势股总买入记录/日内强势股总买入记录Bj-%s.xlsx' % date)
    sellDf = pd.read_excel('/data/group/800463/日内强势股/实盘分析记录/日内强势股总卖出记录/日内强势股总卖出记录Bj-%s.xlsx' % date)
    # cybsellDf = sellDf[sellDf['证券代码'].apply(lambda x:x[0:3] == '300') & (sellDf['买入日期'] >= '2020-08-24')]
    writePath = '/data/group/800463/日内强势股/实盘分析记录/日内强势股成交记录/jupiterBj成交记录-%s.xlsx' % date
    ...
    buyResDf,fig, all_zt_df = get_buyDfTotalInfo(buyDf.copy(), Adate, strategy='jupiterBj')
    todayResDf = get_DfTodayInfo(buyDf.copy(), Adate, strategy='jupiterBj')
    todayBuyResDf = get_buyDfTodayInfo(buyDf.copy(), Adate, strategy='jupiterBj')
    sellResDf = get_sellDfTotalInfo(sellDf.copy(), Adate, fig, strategy='jupiterBj')
    _, fig_thisyear, _ = get_buyDfTotalInfo(buyDf[buyDf['发生日期'] > '2022-01-01'].copy(), Adate, strategy='jupiterBj')
    _ = get_sellDfTotalInfo(sellDf[sellDf['买入日期'] > '2022-01-01'].copy(), Adate, fig_thisyear, strategy='jupiterBj')
    todaySellResDf = get_sellDfTodayInfo(sellDf.copy(), Adate)
    lnonzt_sellResDf = get_lnonztDfTotalInfo(lnonztsellDf.copy(), Adate, strategy='jupiterBj')

    buyDf['买入当日突破时间'] = buyDf['买入当日突破时间'].astype(str)
    buyDf.rename(columns={'涨跌幅(%)':'买入当日涨跌幅(%)'}, inplace=True)
     # TODO: continue修改
    return_path = "/data/group/800463/日内强势股/实盘分析记录/日内强势股成交记录/收益画图/%s_Bj_%s.png" % (Adate, '生产环境')
    # trade_path = "/data/group/800463/日内强势股/实盘分析记录/日内强势股成交记录/成交画图/%s_%s.png"%(date,'生产环境')
    sell_path = "/data/group/800463/日内强势股/实盘分析记录/日内强势股成交记录/卖出画图/%s_Bj_%s.png"%(date,'生产环境')
    ...
    print(f'5-6.jupBj_record_generate运行耗时{round(time.time() - t1, 2)}秒')
