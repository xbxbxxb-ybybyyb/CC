...
from ProdWork.intra_strong.add_mimas.recordUtil import getExtraBuyInfo, write_excel
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        # date = '20250710' # 若未在当个交易日晚上运行程序，需要在次日早上修改date
    ...
    leda_sz_queryFile = '/data/group/800463/xiely/daily-track-cpp/%s-SZEX/接口查询统计_20240401_%s_SZEX_Leda.xlsx' % (date, date)
    leda_sh_queryFile = '/data/group/800463/xiely/daily-track-cpp/%s-SHEX/接口查询统计_20240401_%s_SHEX_Leda.xlsx' % (date, date)
    ...
    flag_ledaconp = judge_updatedate(jup_conceptFile, date)    # Metis的绝对属于Jupiter的子集
    ...
    flag_ledash = os.path.exists(leda_sh_queryFile)
    flag_ledasz = os.path.exists(leda_sh_queryFile)
    ...
    while flag_jupsz == False or flag_jupsh == False or flag_eursz == False or flag_eursh == False or \
            flag_metissh == False or flag_metissz == False or flag_ledash == False or flag_ledasz == False:
        now_time = dt.datetime.now()
        now_time_int = int(now_time.strftime('%H%M%S'))
        now_time_str = now_time.strftime('%H:%M:%S')
        print('%s, 等待接口查询数据中！！！！！！' % now_time_str)
        print(flag_jupsz, flag_jupsh, flag_eursz, flag_eursh, flag_metissz, flag_metissh, flag_ledash, flag_ledasz)
        ...
        flag_ledash = os.path.exists(leda_sh_queryFile)
        flag_ledasz = os.path.exists(leda_sh_queryFile)
    message = f'接口查询数据已成功生成~~~~~~~~~~{now_time}'
    lm.sendMessage(message)
    ...
    leda_query = cal_querymetric(leda_sz_queryFile, leda_sh_queryFile)
    # 只在交易日的当天20点之前不等概念数据，其他时间都等
    # Metis样本以及Leda样本都使用JupiterConcept
    ...
    historyFile_leda = '/data/group/800463/日内强势股/实盘分析记录/日内强势股总买入记录/日内强势股总买入记录Leda-%s.xlsx' % lastdate
    historyFile_pj2 = '/data/group/800463/日内强势股/实盘分析记录/日内强势股总买入记录/项目二总买入记录-%s.xlsx' % lastdate
    historyFile_ceres = '/data/group/800463/日内强势股/实盘分析记录/日内强势股总买入记录/日内强势股总买入记录Ceres-%s.xlsx' % lastdate
    historyFile_p4 = '/data/group/800463/日内强势股/实盘分析记录/日内强势股总买入记录/日内强势股总买入记录P4-%s.xlsx' % lastdate
    historyFile_mimas = '/data/group/800463/日内强势股/实盘分析记录/日内强势股总买入记录/日内强势股总买入记录Mimas-%s.xlsx' % lastdate
    ...
    if os.path.exists(historyFile_leda):
        historyBuyDf_leda = pd.read_excel(historyFile_leda, sheet_name='总买入记录')
        historyBuyDf_leda = historyBuyDf_leda[historyBuyDf_leda['证券名称'].notnull()]
    else:
        historyBuyDf_leda = pd.DataFrame(columns=historyBuyDf.columns)
    ...
    if os.path.exists(historyFile_ceres):
        historyBuyDf_ceres = pd.read_excel(historyFile_ceres, sheet_name='总买入记录')
        historyBuyDf_ceres = historyBuyDf_ceres[historyBuyDf_ceres['证券名称'].notnull()]
    else:
        historyBuyDf_ceres = pd.DataFrame(columns=historyBuyDf_pj2.columns)
        
    if os.path.exists(historyFile_p4):
        historyBuyDf_p4 = pd.read_excel(historyFile_p4, sheet_name='总买入记录')
        historyBuyDf_p4 = historyBuyDf_p4[historyBuyDf_p4['证券名称'].notnull()]
    else:
        historyBuyDf_p4 = pd.DataFrame(columns=historyBuyDf_pj2.columns)
        
    if os.path.exists(historyFile_mimas):
        historyBuyDf_mimas = pd.read_excel(historyFile_mimas, sheet_name='总买入记录')
        historyBuyDf_mimas = historyBuyDf_mimas[historyBuyDf_mimas['证券名称'].notnull()]
    else:
        historyBuyDf_mimas = pd.DataFrame(columns=historyBuyDf_pj2.columns)
    ...
    if os.path.exists(logFile):
        ...
        logDf_leda = pd.read_excel(logFile, sheet_name='每日突破Leda')
        ...
        logDf_ceres = pd.read_excel(logFile, sheet_name='每日突破Ceres')
        if len(logDf_ceres) > 0:
            logDf_ceres = logDf_ceres[logDf_ceres['quantity'] != 0]
        logDf_p4 = pd.read_excel(logFile, sheet_name='每日突破P4')
        if len(logDf_p4) > 0:
            logDf_p4 = logDf_p4[logDf_p4['quantity'] != 0]
        logDf_mimas = pd.read_excel(logFile, sheet_name='每日突破Mimas')
        if len(logDf_mimas) > 0:
            logDf_mimas = logDf_mimas[logDf_mimas['quantity'] != 0]
        ...
        logDf_p4 = pd.DataFrame()
        logDf_mimas = pd.DataFrame()
    ...
    commonInfoColumns = ['证券名称','证券代码','发生日期','委托方向','成交数量','成交金额','成交均价','涨跌幅(%)','买入当天是否收盘涨停',
                         '买入当天收盘价','买入当日收益率(%)','买入当天涨停价']

    resDf = historyBuyDf.copy()
    resDf001 = historyBuyDf001.copy()
    resDf_metis = historyBuyDf_metis.copy()
    resDf_leda = historyBuyDf_leda.copy()
    resDf_pj2 = historyBuyDf_pj2.copy()
    resDf_ceres = historyBuyDf_ceres.copy()
    resDf_p4 = historyBuyDf_p4.copy()
    resDf_mimas = historyBuyDf_mimas.copy()
    buyDf = todayBuyDf.copy()

    def updatePositionBuy(buyDf, resDf, resDf_pj2, resDf_ceres, resDf_p4, resDf_mimas, logDf, buy_orderDf, resDf001, logDf001, buy_metis_orderDf, logDf_metis, logDf_leda):
        ...
        signal_info_p4 = pd.read_excel('/data/group/800463/日内强势股/log_parse/因子耗时/因子耗时_%s_prod.xlsx' % Adate,sheet_name='P4样本')
        signal_info_mimas = pd.read_excel('/data/group/800463/日内强势股/log_parse/因子耗时/因子耗时_%s_prod.xlsx' % Adate,sheet_name='Mimas样本')
        ...
        else: buy_log_Df_jupiter = logDf[(logDf['order_direction'] == 'SplitLastShot')|(logDf['order_direction'] == 'JupiterFirstOrder')| (logDf['order_direction']  == 'MRiskSplitLastShotBuy')|(logDf['order_direction']  == 'MRiskSplitShotBuy')|(logDf['order_direction']  == 'FirstSplitLastShot')|(logDf['order_direction']  == 'SecondSplitLastShot')]
        ...
        if len(logDf_leda) == 0: buy_log_Df_leda = pd.DataFrame()
        else: buy_log_Df_leda = logDf_leda[(logDf_leda['order_direction'] == 'SplitLastShot')|(logDf_leda['order_direction'] == 'JupiterFirstOrder')| (logDf_leda['order_direction']  == 'MRiskSplitLastShotBuy')|(logDf_leda['order_direction']  == 'MRiskSplitShotBuy')]
        ...
        if len(logDf_p4) == 0: buy_log_Df_p4 = pd.DataFrame()
        else: buy_log_Df_p4 = logDf_p4[logDf_p4['order_direction'] == 'A']
        if len(logDf_mimas) == 0: buy_log_Df_mimas = pd.DataFrame()
        else: buy_log_Df_mimas = logDf_mimas[logDf_mimas['order_direction'] == 'A']
        ...
        buy_orderDf_jupiter = buy_orderDf[(buy_orderDf['actionSource'] == 'JupiterN') &
                                          ((buy_orderDf['orderType'] == 'SplitLastShot') |
                                           ...
                                           (buy_orderDf['orderType'] == 'FirstSplitLastShot') |
                                           (buy_orderDf['orderType'] == 'SecondSplitLastShot'))]
        ...
        buy_orderDf_p4 = buy_orderDf[buy_orderDf['orderType'] == 'P4Buy']
        buy_orderDf_mimas = buy_orderDf[buy_orderDf['orderType'] == 'MimasBuy']
        ...
        if len(buyDf)==0:
            ...
            buy_orderDf_p4_info = buy_orderDf_tot_info[buy_orderDf_tot_info['stockcode'].apply(lambda x: x in set(buy_orderDf_p4['stockcode']))].set_index('stockcode')
            buy_orderDf_p4_info = buy_orderDf_p4_info[buy_orderDf_p4_info['deal_amt'] != 0]
            buy_orderDf_mimas_info = buy_orderDf_tot_info[buy_orderDf_tot_info['stockcode'].apply(lambda x: x in set(buy_orderDf_mimas['stockcode']))].set_index('stockcode')
            ...
            buyDf_p4 = buyDf[buyDf['证券代码'].apply(lambda x: x in list(buy_orderDf_p4_info.index))]
            buyDf_mimas = buyDf[buyDf['证券代码'].apply(lambda x: x in list(buy_orderDf_mimas_info.index))]
        ...
        for index, row in buyDf_jupiter.iterrows():
            ...
            resDf.loc[dummy_resDfindex, '买入当日收益率(%)'] = (resDf.loc[dummy_resDfindex, '买入当天收盘价'] / resDf.loc[dummy_resDfindex, '成交均价'] - 1) * 100
        ...
        for index, row in buyDf_metis.iterrows():
            ...
            resDf_metis.loc[dummy_resDfindex, '买入当日收益率(%)'] = (resDf_metis.loc[dummy_resDfindex, '买入当天收盘价'] / resDf_metis.loc[dummy_resDfindex, '成交均价'] - 1) * 100
        ...
        for index,row in buyDf_saturn.iterrows():
            ...
            resDf_pj2.loc[dummy_resDfpj2_index, ['成交数量', '成交金额']] = sel_order[['lastQty', 'lastAmt']].sum().values
            resDf_pj2.loc[dummy_resDfpj2_index, '成交均价'] = resDf_pj2.loc[dummy_resDfpj2_index, '成交金额'] / resDf_pj2.loc[dummy_resDfpj2_index, '成交数量']
            ...
            # resDf_pj2.loc[dummy_resDfpj2_index, ['委托金额']] = 0
            resDf_pj2.loc[dummy_resDfpj2_index, '买入当日收益率(%)'] = (resDf_pj2.loc[dummy_resDfpj2_index, '买入当天收盘价'] / resDf_pj2.loc[dummy_resDfpj2_index, '成交均价'] - 1) * 100

        for index, row in buyDf_leda.iterrows():
            dummy_resDfindex = len(resDf_leda)
            stock_code = row['证券代码']
            resDf_leda.loc[dummy_resDfindex, commonInfoColumns] = row[commonInfoColumns].values
            flag = 0
            buy_vol = float(buy_orderDf_leda_info.loc[stock_code]['deal_vol'])
            buy_amt = float(buy_orderDf_leda_info.loc[stock_code]['deal_amt'])
            if stock_code in resDf_pj2[resDf_pj2['发生日期'] == Adate]['证券代码'].tolist():
                buy_vol = buy_vol - float(resDf_pj2[(resDf_pj2['发生日期'] == Adate) & (resDf_pj2['证券代码'] == stock_code)]['成交数量'])
                buy_amt = buy_amt - float(resDf_pj2[(resDf_pj2['发生日期'] == Adate) & (resDf_pj2['证券代码'] == stock_code)]['成交金额'])
                flag = 1
            resDf_leda.loc[dummy_resDfindex, '成交数量'] = buy_vol
            resDf_leda.loc[dummy_resDfindex, '成交金额'] = buy_amt
            if flag == 0:
                resDf_leda.loc[dummy_resDfindex, ['成交数量', '成交金额', '成交均价']] = buy_orderDf_leda_info.loc[stock_code][['deal_vol', 'deal_amt', 'deal_vwap']].values
            resDf_leda.loc[dummy_resDfindex, '成交均价'] = resDf_leda.loc[dummy_resDfindex, '成交金额'] / resDf_leda.loc[dummy_resDfindex, '成交数量']
            resDf_leda.loc[dummy_resDfindex, '买入当日收益率(%)'] = (resDf_leda.loc[dummy_resDfindex, '买入当天收盘价'] / resDf_leda.loc[dummy_resDfindex, '成交均价'] - 1) * 100

        for index,row in buyDf_ceres.iterrows():
            dummy_resDfceres_index = len(resDf_ceres)
            stock_code = row['证券代码']
            resDf_ceres.loc[dummy_resDfceres_index, commonInfoColumns + buyExtraColumns] = row[commonInfoColumns + buyExtraColumns].values
            sel_order = buy_orderDf_ceres[((buy_orderDf_ceres['ordStatus'] == 'PARTIALLY_FILLED') |
                                            (buy_orderDf_ceres['ordStatus'] == 'FILLED')) &
                                           (buy_orderDf_ceres['stockcode'] == stock_code)]
            resDf_ceres.loc[dummy_resDfceres_index, ['成交数量', '成交金额']] = sel_order[['lastQty', 'lastAmt']].sum().values
            resDf_ceres.loc[dummy_resDfceres_index, '成交均价'] = resDf_ceres.loc[dummy_resDfceres_index, '成交金额'] / resDf_ceres.loc[dummy_resDfceres_index, '成交数量']
            resDf_ceres.loc[dummy_resDfceres_index, ['买入时点']] = '931'
            index_in_timecost = signal_info_ceres[signal_info_ceres['Unnamed: 0'] == stock_code].index.tolist()[0]
            resDf_ceres.loc[dummy_resDfceres_index, ['委托金额']] = float(signal_info_ceres.loc[index_in_timecost][['target_amt']])
            # resDf_ceres.loc[dummy_resDfceres_index, ['委托金额']] = 0
            resDf_ceres.loc[dummy_resDfceres_index, '买入当日收益率(%)'] = (resDf_ceres.loc[dummy_resDfceres_index, '买入当天收盘价'] / resDf_ceres.loc[dummy_resDfceres_index, '成交均价'] - 1) * 100
            
        for index,row in buyDf_p4.iterrows():
            dummy_resDfp4_index = len(resDf_p4)
            stock_code = row['证券代码']
            resDf_p4.loc[dummy_resDfp4_index, commonInfoColumns + buyExtraColumns] = row[commonInfoColumns + buyExtraColumns].values
            sel_order = buy_orderDf_p4[((buy_orderDf_p4['ordStatus'] == 'PARTIALLY_FILLED') |
                                            (buy_orderDf_p4['ordStatus'] == 'FILLED')) &
                                           (buy_orderDf_p4['stockcode'] == stock_code)]
            resDf_p4.loc[dummy_resDfp4_index, ['成交数量', '成交金额']] = sel_order[['lastQty', 'lastAmt']].sum().values
            resDf_p4.loc[dummy_resDfp4_index, '成交均价'] = resDf_p4.loc[dummy_resDfp4_index, '成交金额'] / resDf_p4.loc[dummy_resDfp4_index, '成交数量']
            resDf_p4.loc[dummy_resDfp4_index, ['买入时点']] = '931'
            index_in_timecost = signal_info_p4[signal_info_p4['Unnamed: 0'] == stock_code].index.tolist()[0]
            resDf_p4.loc[dummy_resDfp4_index, ['委托金额']] = float(signal_info_p4.loc[index_in_timecost][['target_amt']])
            # resDf_p4.loc[dummy_resDfp4_index, ['委托金额']] = 0
            resDf_p4.loc[dummy_resDfp4_index, '买入当日收益率(%)'] = (resDf_p4.loc[dummy_resDfp4_index, '买入当天收盘价'] / resDf_p4.loc[dummy_resDfp4_index, '成交均价'] - 1) * 100
        
        for index,row in buyDf_mimas.iterrows():
            dummy_resDfmimas_index = len(resDf_mimas)
            stock_code = row['证券代码']
            resDf_mimas.loc[dummy_resDfmimas_index, commonInfoColumns + buyExtraColumns] = row[commonInfoColumns + buyExtraColumns].values
            sel_order = buy_orderDf_mimas[((buy_orderDf_mimas['ordStatus'] == 'PARTIALLY_FILLED') |
                                            (buy_orderDf_mimas['ordStatus'] == 'FILLED')) &
                                           (buy_orderDf_mimas['stockcode'] == stock_code)]
            resDf_mimas.loc[dummy_resDfmimas_index, ['成交数量', '成交金额']] = sel_order[['lastQty', 'lastAmt']].sum().values
            resDf_mimas.loc[dummy_resDfmimas_index, '成交均价'] = resDf_mimas.loc[dummy_resDfmimas_index, '成交金额'] / resDf_mimas.loc[dummy_resDfmimas_index, '成交数量']
            resDf_mimas.loc[dummy_resDfmimas_index, ['买入时点']] = '931'
            index_in_timecost = signal_info_mimas[signal_info_mimas['Unnamed: 0'] == stock_code].index.tolist()[0]
            resDf_mimas.loc[dummy_resDfmimas_index, ['委托金额']] = float(signal_info_mimas.loc[index_in_timecost][['target_amt']])
            # resDf_mimas.loc[dummy_resDfmimas_index, ['委托金额']] = 0
            resDf_mimas.loc[dummy_resDfmimas_index, '买入当日收益率(%)'] = (resDf_mimas.loc[dummy_resDfmimas_index, '买入当天收盘价'] / resDf_mimas.loc[dummy_resDfmimas_index, '成交均价'] - 1) * 100
        ...
        for index, row in buyDf_jupiter001.iterrows():
            ...
            flag = 0
            buy_vol = float(buy_orderDf_jupiter001_info.loc[stock_code]['deal_vol'])
            buy_amt = float(buy_orderDf_jupiter001_info.loc[stock_code]['deal_amt'])
            if stock_code in resDf[resDf['发生日期'] == Adate]['证券代码'].tolist():
                buy_vol = buy_vol - float(resDf[(resDf['发生日期'] == Adate) & (resDf['证券代码'] == stock_code)]['成交数量'])
                buy_amt = buy_amt - float(resDf[(resDf['发生日期'] == Adate) & (resDf['证券代码'] == stock_code)]['成交金额'])
                flag = 1
            if stock_code in resDf_metis[resDf_metis['发生日期'] == Adate]['证券代码'].tolist():
                buy_vol = buy_vol - float(resDf_metis[(resDf_metis['发生日期'] == Adate) & (resDf_metis['证券代码'] == stock_code)]['成交数量'])
                buy_amt = buy_amt - float(resDf_metis[(resDf_metis['发生日期'] == Adate) & (resDf_metis['证券代码'] == stock_code)]['成交金额'])
                flag = 1
            if stock_code in resDf_leda[resDf_leda['发生日期'] == Adate]['证券代码'].tolist():
                buy_vol = buy_vol - float(resDf_leda[(resDf_leda['发生日期'] == Adate) & (resDf_leda['证券代码'] == stock_code)]['成交数量'])
                buy_amt = buy_amt - float(resDf_leda[(resDf_leda['发生日期'] == Adate) & (resDf_leda['证券代码'] == stock_code)]['成交金额'])
                flag = 1
            if stock_code in resDf_ceres[resDf_ceres['发生日期'] == Adate]['证券代码'].tolist():
                buy_vol = buy_vol - float(resDf_ceres[(resDf_ceres['发生日期'] == Adate) & (resDf_ceres['证券代码'] == stock_code)]['成交数量'])
                buy_amt = buy_amt - float(resDf_ceres[(resDf_ceres['发生日期'] == Adate) & (resDf_ceres['证券代码'] == stock_code)]['成交金额'])
                flag = 1
            if stock_code in resDf_p4[resDf_p4['发生日期'] == Adate]['证券代码'].tolist():
                buy_vol = buy_vol - float(resDf_p4[(resDf_p4['发生日期'] == Adate) & (resDf_p4['证券代码'] == stock_code)]['成交数量'])
                buy_amt = buy_amt - float(resDf_p4[(resDf_p4['发生日期'] == Adate) & (resDf_p4['证券代码'] == stock_code)]['成交金额'])
                flag = 1
            if stock_code in resDf_mimas[resDf_mimas['发生日期'] == Adate]['证券代码'].tolist():
                buy_vol = buy_vol - float(resDf_mimas[(resDf_mimas['发生日期'] == Adate) & (resDf_mimas['证券代码'] == stock_code)]['成交数量'])
                buy_amt = buy_amt - float(resDf_mimas[(resDf_mimas['发生日期'] == Adate) & (resDf_mimas['证券代码'] == stock_code)]['成交金额'])
                flag = 1
            resDf001.loc[dummy_resDfindex, '成交数量'] = buy_vol
            resDf001.loc[dummy_resDfindex, '成交金额'] = buy_amt if buy_amt >= 0 else 0
            if flag == 0:
            ...
            resDf001.loc[dummy_resDfindex, '买入成交均价'] = resDf001.loc[dummy_resDfindex, '成交金额'] / resDf001.loc[dummy_resDfindex, '成交数量']
            resDf001.loc[dummy_resDfindex, '买入当日收益率(%)'] = (resDf001.loc[dummy_resDfindex, '买入当天收盘价'] / resDf001.loc[dummy_resDfindex, '成交均价'] - 1) * 100
        ...
        for index,row in buy_log_Df_jupiter.iterrows():
            ...
            resDf.loc[resDfIndex, '委托金额'] = buy_log_Df_jupiter.loc[buy_log_Df_jupiter['Unnamed: 0']==stockCode]['totalOrderAmt'].sum()
        #%% 针对jupNew进行额外信息的填充
        for index, row in buy_log_Df_jupiter001.iterrows():
            ...
            if (len(resDf001[resDf001['发生日期'] == Adate]) > 0) and (stockCode in resDf001[resDf001['发生日期'] == Adate]['证券代码'].values):
                ...
                # resDfIndex, stock_code = len(resDf001), row['index']
        ...
        #%% 针对Leda进行额外信息的填充
        for index, row in buy_log_Df_leda.iterrows():
            stockCode = row['Unnamed: 0']
            ZT_Time = row['ZT_Time']
            if (len(resDf_leda[resDf_leda['发生日期'] == Adate]) > 0) and (stockCode in resDf_leda[resDf_leda['发生日期'] == Adate]['证券代码'].values):
                resDfIndex = resDf_leda[resDf_leda['证券代码'] == stockCode].index
                if len(resDfIndex) > 1:
                    resDfIndex = resDfIndex[-1:]
            else:  # 如果存在正常的数据
                # 针对当天在o32中没有但在log中有的票进行数据填充
                resDfIndex, stock_code = len(resDf_leda), row['Unnamed: 0']
                resDf_leda.loc[resDfIndex, ['证券代码', '证券名称']] = stock_code, name_data.loc[lastdate, stock_code]['STOCK_NAME'].values[0]
                resDf_leda.loc[resDfIndex, ['发生日期', '委托方向', '成交数量', '成交金额']] = Adate, '买入', 0, 0
            # 其他columns，计算买入日形态和突破时间
            try:
                resDf_leda.loc[resDfIndex, '形态'] = tot_pattern_df.loc[(pd.to_datetime(date), stockCode), 'label_pattern']
            except:
                print(f'{date}-{stockCode}需要重新计算形态数据！')
            resDf_leda.loc[resDfIndex, '买入当日突破时间'] = ZT_Time
            resDf_leda.loc[resDfIndex, '委托金额'] = buy_log_Df_leda.loc[index, 'totalOrderAmt']
        #%% 针对项目二930进行额外信息的填充
        if len(buy_log_Df_saturn) == 0:
            pass
        else:
            buy_log_Df_saturn = buy_log_Df_saturn.loc[~buy_log_Df_saturn['Unnamed: 0'].duplicated(keep='first')]
        saturn_basic_info = pd.read_hdf('/data/group/800463/project/project2_prod/daily_data/%s_v6/Basic_night_finish_%s_%s.h5'%(date,date,date))
        ...
        resDf_ceres['涨跌幅(%)'] = resDf_ceres['涨跌幅(%)'].astype(float)
        resDf_p4['涨跌幅(%)'] = resDf_p4['涨跌幅(%)'].astype(float)
        
        return resDf, resDf_pj2, resDf_ceres, resDf_p4, resDf_mimas, resDf001, resDf_metis, resDf_leda
    ...
    def writeExcelMetis(resDf):
        ...
        print('write sheet %s for %s!!!!!!!' % (sheetname, writePath))
        write_excel(resDf, writePath, sheetname)
    def writeExcelLeda(resDf):
        writePath = f'/data/group/800463/日内强势股/实盘分析记录/日内强势股总买入记录/日内强势股总买入记录Leda-{date}.xlsx'
        sheetname = '总买入记录'
        resDf.replace(np.nan, '', inplace=True)
        print('write sheet %s for %s!!!!!!!' % (sheetname, writePath))
        write_excel(resDf, writePath, sheetname)
    def writeExcel_pj2(resDf):
        ...
        print('write sheet %s for %s!!!!!!!' % (sheetname, writePath))
        write_excel(resDf, writePath, sheetname)
    def writeExcel_ceres(resDf):
        writePath = f'/data/group/800463/日内强势股/实盘分析记录/日内强势股总买入记录/日内强势股总买入记录Ceres-{date}.xlsx'
        sheetname = '总买入记录'
        resDf.replace(np.nan,'',inplace=True)
        # resDf = resDf.drop(['前日形态', 'TN_v2o10', 'TN_v2o10nd1'], axis=1)
        print('write sheet %s for %s!!!!!!!' % (sheetname, writePath))
        write_excel(resDf, writePath, sheetname)
    def writeExcel_p4(resDf):
        writePath = f'/data/group/800463/日内强势股/实盘分析记录/日内强势股总买入记录/日内强势股总买入记录P4-{date}.xlsx'
        sheetname = '总买入记录'
        resDf.replace(np.nan,'',inplace=True)
        print('write sheet %s for %s!!!!!!!' % (sheetname, writePath))
        write_excel(resDf, writePath, sheetname)
    def writeExcel_mimas(resDf):
        writePath = f'/data/group/800463/日内强势股/实盘分析记录/日内强势股总买入记录/日内强势股总买入记录Mimas-{date}.xlsx'
        sheetname = '总买入记录'
        resDf.replace(np.nan,'',inplace=True)
        print('write sheet %s for %s!!!!!!!' % (sheetname, writePath))
        write_excel(resDf, writePath, sheetname)
    def orderrejectlog2reason(buy_orderDf, buy_rejectDf):
        ...
        if (len(buy_orderDf) == len(buy_rejectDf)) & (len(buy_rejectDf) == 0):
            ...
            buy_orderDf_for_reject = buy_orderDf[buy_orderDf['orderType'] != 'SmallTest']   # 筛选所有的单子
            ...
            partially_filled_stocks = list(set(partially_filled_stocks).difference(set(filled_stocks.index)))   # 一些filled的个股被当作未成交

            """20240515修改这里，所有的都当做unfilled"""
            unfilled_stocks = filled_stocks[filled_stocks == False] # 不是filled( )也不是partially filled，那就unfilled
            all_unfilled_stocks = buy_orderDf_for_reject.groupby('stockcode').sum()
            ...
            if len(buy_rejectDf) > 0:
                ...
                if 'reason' in buy_rejectDf.columns.tolist():
                    buy_rejectDf['riskSummary'] = buy_rejectDf['riskSummary'].fillna(buy_rejectDf['riskViolateRemark']).fillna(buy_rejectDf['reason'])
                else:
                    buy_rejectDf['riskSummary'] = buy_rejectDf['riskSummary'].fillna(buy_rejectDf['riskViolateRemark'])
                # reject_info = buy_rejectDf.groupby('stockcode').apply(lambda x: str(list(np.unique(x['riskSummary'].astype(str))))).reindex(all_unfilled_stocks.index).dropna()
                reject_info = buy_rejectDf.groupby('stockcode').apply(lambda x: str(list(np.unique(x['riskSummary'].astype(str))))).dropna()
            ...
            reject_info_out = pd.concat([reject_info, unfilled_info_df], axis = 0).reset_index(drop=True)
            return reject_info_out

    resDf1, resDf1_pj2, resDf1_ceres, resDf1_p4, resDf1_mimas, resDf1001, resDf1_metis, resDf1_leda = updatePositionBuy(todayBuyDf.copy(), historyBuyDf.copy(), historyBuyDf_pj2.copy(),
                                                                 historyBuyDf_ceres.copy(), historyBuyDf_p4.copy(), historyBuyDf_mimas.copy(), logDf.copy(), buy_orderDf.copy(), historyBuyDf001.copy(),
                                                                 logDf001.copy(), buy_metis_orderDf.copy(), logDf_metis.copy(), logDf_leda.copy())
    ...
    if True in resDf_leda['形态'].isnull():
        unfilled_pattern = resDf_leda[resDf_leda['形态'].isnull()]
        for index,row in unfilled_pattern.iterrows():
            stock_code, pre_date= row[['证券代码','发生日期']].values
            pre_date = pre_date[0:4] + pre_date[5:7] + pre_date[8:]
            # print(stock_code, pre_date)
            try:
                resDf_leda.loc[index, '形态'] = tot_pattern_df.loc[(pd.to_datetime(pre_date), stock_code), 'label_pattern']
            except:
                print(f'{pre_date}-{stock_code}需要重新计算形态数据！')
    ...
    for i in rjf.index:
        ...
        # 如果项目三买了则jupiter不买，所以先判断是否在今日的项目三
        elif len(resDf1_ceres[((resDf1_ceres['证券代码'] == rjf.loc[i]['证券代码']) & (resDf1_ceres['发生日期'] == rjf.loc[i]['发生日期']))]) != 0:
            buy_index_ceres = resDf1_ceres[((resDf1_ceres['证券代码'] == rjf.loc[i]['证券代码']) & (resDf1_ceres['发生日期'] == rjf.loc[i]['发生日期']))].index
            resDf1_ceres.loc[buy_index_ceres,'未完成原因'] = rjf.loc[i]['未完成原因']
        elif len(resDf1_p4[((resDf1_p4['证券代码'] == rjf.loc[i]['证券代码']) & (resDf1_p4['发生日期'] == rjf.loc[i]['发生日期']))]) != 0:
            buy_index_p4 = resDf1_p4[((resDf1_p4['证券代码'] == rjf.loc[i]['证券代码']) & (resDf1_p4['发生日期'] == rjf.loc[i]['发生日期']))].index
            resDf1_p4.loc[buy_index_p4,'未完成原因'] = rjf.loc[i]['未完成原因']
        elif len(resDf1_mimas[((resDf1_mimas['证券代码'] == rjf.loc[i]['证券代码']) & (resDf1_mimas['发生日期'] == rjf.loc[i]['发生日期']))]) != 0:
            buy_index_mimas = resDf1_mimas[((resDf1_mimas['证券代码'] == rjf.loc[i]['证券代码']) & (resDf1_mimas['发生日期'] == rjf.loc[i]['发生日期']))].index
            resDf1_mimas.loc[buy_index_mimas,'未完成原因'] = rjf.loc[i]['未完成原因']
        else:
            ...
            if len(resDf1001[((resDf1001['证券代码'] == rjf.loc[i]['证券代码']) & (resDf1001['发生日期'] == rjf.loc[i]['发生日期']))]) > 0:
            ...
            if len(resDf1_leda[((resDf1_leda['证券代码'] == rjf.loc[i]['证券代码']) & (resDf1_leda['发生日期'] == rjf.loc[i]['发生日期']))]) > 0:
                buy_index_leda = resDf1_leda[((resDf1_leda['证券代码'] == rjf.loc[i]['证券代码']) & (resDf1_leda['发生日期'] == rjf.loc[i]['发生日期']))].index
                resDf1_leda.loc[buy_index_leda, '未完成原因'] = rjf.loc[i]['未完成原因']
    ...
    from ProdWork.Param_config_data import thred_dict_jup001_v4 as thred_dict001
    ...
    #%% Leda
    from ProdWork.Param_config_data import thred_dict_leda_v1 as thred_dict_leda

    model_list_leda = list(thred_dict_leda.keys())
    today_index_leda = resDf1_leda.query(f'发生日期 == "{Adate}"').index
    factor_time_cost_leda = pd.read_excel(f'/data/group/800463/日内强势股/leda_log_parse/因子耗时/因子耗时_{Adate}_prod.xlsx', sheet_name='因子耗时')
    if '拉抬打压信息' not in resDf1_leda.columns.tolist():
        resDf1_leda['拉抬打压信息'] = np.nan

    for i in today_index_leda:
        stock = resDf1_leda.loc[i, '证券代码']
        this_stock_cla_model_score = 0
        this_stock_reg_model_score = 0
        this_stock_ZT_model_score = 0
        for model in model_list_leda:
            if model + '_probability' in factor_time_cost_leda.columns:
                factor_time_cost_leda[model + '_probability'] = factor_time_cost_leda[model + '_probability'].astype(float)
                model_res = (factor_time_cost_leda[factor_time_cost_leda['Unnamed: 0'] == stock][model + '_probability'].values >=
                             thred_dict_leda[model]).sum()
                if 'ZTBysModel_probability' not in factor_time_cost_leda[factor_time_cost_leda['Unnamed: 0'] == stock].columns:
                    if 'Cla' in model:
                        this_stock_cla_model_score += model_res
                    else:
                        this_stock_reg_model_score += model_res
                else:
                    if factor_time_cost_leda[factor_time_cost_leda['Unnamed: 0'] == stock]['ZTBysModel_probability'].isnull().sum() != 1:
                        if 'ZT' in model:
                            this_stock_ZT_model_score += model_res
                    else:
                        if 'Cla' in model:
                            ...
                            this_stock_reg_model_score += model_res
        resDf1_leda.loc[i, '回归信号'] = this_stock_reg_model_score
        resDf1_leda.loc[i, '分类信号'] = this_stock_cla_model_score
        resDf1001.loc[i, '前涨停信号'] = this_stock_ZT_model_score
        # if np.isnan(factor_time_cost001[factor_time_cost001['Unnamed: 0'] == stock]['MRisk_info'].values[0]) == False:
        if 'MRisk_info' in factor_time_cost_leda.columns.tolist():
            resDf1_leda.loc[i, '拉抬打压信息'] = factor_time_cost_leda[factor_time_cost_leda['Unnamed: 0'] == stock]['MRisk_info'].values[0]
    resDf1_leda['买入当天持仓金额'] = resDf1_leda['成交数量'] * resDf1_leda['买入当天收盘价']
    ...
    f_data['zcz'] = (((f_data.reset_index()['Ticker'].apply(lambda x: x[0] == '3')) & (
    ...
    sellDf_leda_copy = resDf1_leda.copy()
    sellDf_leda_copy['买入日期'] = sellDf_leda_copy['发生日期'].apply(lambda x: pd.Timestamp(x))
    sellDf_leda_copy = sellDf_leda_copy.rename(columns={'买入日期': 'dt', '证券代码': 'Ticker'}).set_index(['dt', 'Ticker'])
    resDf1_leda['last_is_zt'] = last_is_zt.reindex(sellDf_leda_copy.index).values

    # --------------------------添加Saturn各个模型的预测并汇总到回归模型和分类模型---------------------
    # 读取因子耗时文件
    today_index_pj2 = resDf1_pj2[(resDf1_pj2['发生日期']==Adate)].index
    for trade_time in ['930', '931'][1:]:
        factor_time_cost_pj2 = pd.read_excel('/data/group/800463/日内强势股/log_parse/因子耗时/因子耗时_%s_prod.xlsx' % Adate, sheet_name='项目二%s样本'%trade_time, index_col=0)
        for i in today_index_pj2:
            stock = resDf1_pj2.loc[i, '证券代码']
            resDf1_pj2.loc[i, '回归信号'] = factor_time_cost_pj2.loc[stock, 'sum_signals']

    # --------------------------添加Ceres各个模型的预测并汇总到回归模型和分类模型---------------------
    today_index_ceres = resDf1_ceres[(resDf1_ceres['发生日期'] == Adate)].index
    factor_time_cost_ceres = pd.read_excel('/data/group/800463/日内强势股/log_parse/因子耗时/因子耗时_%s_prod.xlsx' % Adate, sheet_name='Ceres样本', index_col=0)
    for i in today_index_ceres:
        stock = resDf1_ceres.loc[i, '证券代码']
        resDf1_ceres.loc[i, '回归信号'] = factor_time_cost_ceres.loc[stock, 'sum_signals']

    # --------------------------添加P4各个模型的预测并汇总到回归模型和分类模型---------------------
    today_index_p4 = resDf1_p4[(resDf1_p4['发生日期'] == Adate)].index
    factor_time_cost_p4 = pd.read_excel('/data/group/800463/日内强势股/log_parse/因子耗时/因子耗时_%s_prod.xlsx' % Adate, sheet_name='P4样本', index_col=0)
    for i in today_index_p4:
        stock = resDf1_p4.loc[i, '证券代码']
        resDf1_p4.loc[i, '回归信号'] = factor_time_cost_p4.loc[stock, 'sum_signals']

    # --------------------------添加Mimas各个模型的预测并汇总到回归模型和分类模型---------------------
    today_index_mimas = resDf1_mimas[(resDf1_mimas['发生日期'] == Adate)].index
    factor_time_cost_mimas = pd.read_excel('/data/group/800463/日内强势股/log_parse/因子耗时/因子耗时_%s_prod.xlsx' % Adate, sheet_name='Mimas样本', index_col=0)
    for i in today_index_mimas:
        stock = resDf1_mimas.loc[i, '证券代码']
        resDf1_mimas.loc[i, '回归信号'] = factor_time_cost_mimas.loc[stock, 'sum_signals']
    ...
    for i in o2ul_nan_samples.index:
        ...
        # resDf1.loc[i,'TN_o2ul'] = tmp_df.loc[buy_date,stock].values[0]
    ...
    # -------------------------在Leda买入汇总中添加o2ul的计算-------------------------------------------
    if 'TN_o2ul' not in resDf1_leda.columns:
        resDf1_leda['TN_o2ul'] = np.nan
    o2ul_nan_leda_samples = resDf1_leda[resDf1_leda['TN_o2ul'].isnull()]
    # date_ini_leda = o2ul_nan_leda_samples['发生日期'].apply(lambda x: x[:4] + x[5:7] + x[8:10]).min()
    # end_date_leda = o2ul_nan_leda_samples['发生日期'].apply(lambda x: x[:4] + x[5:7] + x[8:10]).max()
    resDf1_leda_copy = resDf1_leda.copy()
    resDf1_leda_copy['发生日期'] = resDf1_leda_copy['发生日期'].apply(lambda x: pd.Timestamp(x[:4] + x[5:7] + x[8:10]))
    tmp_df = 100 * md_data.reindex(resDf1_leda_copy.rename(columns={'发生日期': 'dt', '证券代码': 'Ticker'}).set_index(['dt', 'Ticker']).index)['label_TN_o2ul']
    for i in o2ul_nan_leda_samples.index:
        buy_date = resDf1_leda.loc[i]['发生日期']
        stock = resDf1_leda.loc[i]['证券代码']
        resDf1_leda.loc[i, 'TN_o2ul'] = tmp_df.loc[buy_date, stock]

    # -------------------------在Saturn买入汇总中添加o2ul的计算-------------------------------------------
    if 'TN_v2o10' not in resDf1_pj2.columns:
        resDf1_pj2['TN_v2o10'] = np.nan
    if 'TN_v2o10dh1' not in resDf1_pj2.columns:
        resDf1_pj2['TN_v2o10dh1'] = np.nan
    ...
    v2o10dh1_nan_samples_not_today = resDf1_pj2[resDf1_pj2['TN_v2o10dh1'].isnull() & (resDf1_pj2['发生日期']!=Adate) & (resDf1_pj2['买入时点']=='931')]

    trade_time_list = ['930', '931']
    need_sample_list = [v2o10_nan_samples_not_today, v2o10dh1_nan_samples_not_today]
    for i in range(2):
        ...
        if len(v2o10_nan_samples_not_today) !=0:
            ...
            for index, row in nan_samples_here.iterrows():
                ...
                if trade_time == '930':
                    ...
                    resDf1_pj2.loc[index,'TN_v2o10dh1'] = this_label

    # -------------------------在Ceres买入汇总中添加TN_v2o10dh1的计算-------------------------------------------
    if 'TN_v2o10dh1' not in resDf1_ceres.columns:
        resDf1_ceres['TN_v2o10dh1'] = np.nan
    resDf1_ceres['买入当天持仓金额'] = resDf1_ceres['成交数量'] * resDf1_ceres['买入当天收盘价']
    v2o10dh1_nan_samples_not_today = resDf1_ceres[resDf1_ceres['TN_v2o10dh1'].isnull() & (resDf1_ceres['发生日期'] != Adate)]

    if len(v2o10dh1_nan_samples_not_today) != 0:
        for index, row in v2o10dh1_nan_samples_not_today.iterrows():
            stock = row['证券代码']
            buy_A_date = row['发生日期']
            buy_date = buy_A_date[0:4] + buy_A_date[5:7] + buy_A_date[8:10]
            ceres_basic_hf_info = pd.read_hdf('/data/group/800463/project/project3_prod/daily_data/Label/label.h5')
            this_v2o10d1 = ceres_basic_hf_info.loc[(pd.to_datetime(buy_date), stock), 'label_v2o10d1']
            resDf1_ceres.loc[index, 'TN_v2o10d1'] = this_v2o10d1

    # -------------------------在P4买入汇总中添加TN_v2o10nd1的计算-------------------------------------------
    if 'TN_v2o10nd1' not in resDf1_p4.columns:
        resDf1_p4['TN_v2o10nd1'] = np.nan
    resDf1_p4['买入当天持仓金额'] = resDf1_p4['成交数量'] * resDf1_p4['买入当天收盘价']
    v2o10dh1_nan_samples_not_today = resDf1_p4[resDf1_p4['TN_v2o10nd1'].isnull() & (resDf1_p4['发生日期'] != Adate)]
    ...
    if len(v2o10dh1_nan_samples_not_today) != 0:
        for index, row in v2o10dh1_nan_samples_not_today.iterrows():
            ...
            buy_A_date = row['发生日期']
            buy_date = buy_A_date[0:4] + buy_A_date[5:7] + buy_A_date[8:10]
            p4_basic_hf_info = pd.read_hdf('/data/group/800463/project/project4_prod/daily_data/Label/label_p4.h5')
            try:
                this_v2o10nd1 = p4_basic_hf_info.loc[(pd.to_datetime(buy_date), stock), 'label_v2o10nd1']
            except:
                this_v2o10nd1 = np.nan
            resDf1_p4.loc[index, 'TN_v2o10nd1'] = this_v2o10nd1

    # -------------------------在Mimas买入汇总中添加TN_v2o10dh1的计算-------------------------------------------
    if 'TN_v2o10dh1' not in resDf1_mimas.columns:
        resDf1_mimas['TN_v2o10dh1'] = np.nan
    resDf1_mimas['买入当天持仓金额'] = resDf1_mimas['成交数量'] * resDf1_mimas['买入当天收盘价']
    v2o10dh1_nan_samples_not_today = resDf1_mimas[resDf1_mimas['TN_v2o10dh1'].isnull() & (resDf1_mimas['发生日期'] != Adate)]
    ...
    if len(v2o10dh1_nan_samples_not_today) != 0:
        for index, row in v2o10dh1_nan_samples_not_today.iterrows():
            ...
            buy_A_date = row['发生日期']
            buy_date = buy_A_date[0:4] + buy_A_date[5:7] + buy_A_date[8:10]
            mimas_basic_hf_info = pd.read_hdf('/data/group/800463/project/project2_prod/daily_data/Label/label_Next.h5')
            try:
                this_v2o10dh1 = mimas_basic_hf_info.loc[(pd.to_datetime(buy_date), stock), 'label_v2o10dh1']
            except:
                this_v2o10dh1 = np.nan
            resDf1_mimas.loc[index, 'TN_v2o10dh1'] = this_v2o10dh1
    ...
    writeExcel_ceres(resDf1_ceres)
    writeExcel_p4(resDf1_p4)
    writeExcel_mimas(resDf1_mimas)
    ...
    # Leda
    basic_file_raw = resDf1_leda.copy()
    basic_file_raw.replace('', np.nan, inplace=True)

    # --------------------------------------------------------------------
    basic_file_raw_add = basic_file_raw.copy()
    unfilled_left = basic_file_raw[(basic_file_raw['发生日期']==Adate) & (basic_file_raw['成交数量']==0) & (basic_file_raw['未完成原因'].isna()) &
                                   (basic_file_raw['last_is_zt'] == 0)].index.tolist()
    basic_file_raw.loc[unfilled_left, '未完成原因'] = '本策略未成交'
    basic_file_raw_add = basic_file_raw_add.set_index(['发生日期', '证券代码'])
    if flag_eurconp:
        ...
        concept_eur.set_index(['发生日期', 'Ticker'], inplace=True)
        basic_file_raw_add['概念名称'] = concept_eur.loc[basic_file_raw_add.index, '概念名称']
    sel_indx = list(set(basic_file_raw_add.index.tolist()) & set(leda_query.index.tolist()))
    basic_file_raw_add['是否触发查询'] = 0
    basic_file_raw_add['是否之前有成交'] = 0
    basic_file_raw_add.loc[sel_indx, '是否触发查询'] = leda_query.loc[sel_indx, 'Flag_query']
    basic_file_raw_add.loc[sel_indx, '是否之前有成交'] = leda_query.loc[sel_indx, 'Flag_has_trade_last3min']
    writeExcelLeda(basic_file_raw_add.reset_index())
