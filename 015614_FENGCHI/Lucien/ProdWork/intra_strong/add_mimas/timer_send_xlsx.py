...
while True:
    if os.path.exists(root_path + f'Europa成交记录-{today_date}.xlsx') and \
        ...
        os.path.exists(root_path + f'Leda成交记录-{today_date}.xlsx') and \
        os.path.exists(root_path + f'Ceres成交记录-{today_date}.xlsx') and \
        os.path.exists(root_path + f'P4成交记录-{today_date}.xlsx') and \
        os.path.exists(root_path + f'Mimas成交记录-{today_date}.xlsx')\
            :
        ...
        send_file(root_path + f'Ceres成交记录-{today_date}.xlsx')
        send_file(root_path + f'P4成交记录-{today_date}.xlsx')
        send_file(root_path + f'Mimas成交记录-{today_date}.xlsx')
        # send_file('/data/group/800463/日内强势股/实盘分析记录/日内强势股总买入记录/' + f'日内强势股总买入记录Ceres-{today_date}.xlsx')
        # send_file('/data/group/800463/日内强势股/实盘分析记录/日内强势股总买入记录/' + f'日内强势股总买入记录P4-{today_date}.xlsx')
        break
    time.sleep(30)
...
ceres_buy = pd.read_excel(root_path + f'Ceres成交记录-{today_date}.xlsx', sheet_name='累计卖出明细')
p4_buy = pd.read_excel(root_path + f'P4成交记录-{today_date}.xlsx', sheet_name='累计卖出明细')
mimas_buy = pd.read_excel(root_path + f'Mimas成交记录-{today_date}.xlsx', sheet_name='累计卖出明细')
...
ceres_buy['买入日期'] = ceres_buy['买入日期'].apply(lambda x: x.replace('-', ''))
p4_buy['买入日期'] = p4_buy['买入日期'].apply(lambda x: x.replace('-', ''))
mimas_buy['买入日期'] = mimas_buy['买入日期'].apply(lambda x: x.replace('-', ''))
...
ceres_buy_list = ceres_buy.query(f'买入日期 == "{today_date}"')['证券代码'].tolist()
p4_buy_list = p4_buy.query(f'买入日期 == "{today_date}"')['证券代码'].tolist()
