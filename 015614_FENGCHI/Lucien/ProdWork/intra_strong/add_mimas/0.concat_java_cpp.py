...
mimas_path = '/data/group/800463/日内强势股/mimas_log_parse/'  # 20250728上线
...
mimas_record_path = '/data/group/800463/日内强势股/mimas_log_parse/'
...
if len(sys.argv) > 1:
    ...
    # date = '20250812' # 若未在当个交易日晚上运行程序，需要在次日早上修改date
...
for envir in enviroment_list:
    ...
    mimas_log_parse_fpath = mimas_path + f'因子耗时/因子耗时_{today_date_str}_{envir}.xlsx'
    ...
    while not os.path.exists(p4_log_parse_fpath):
        ...
        time.sleep(120)
    while not os.path.exists(mimas_log_parse_fpath):
        send_message(f'{today_date} mimas因子耗时文件缺失')
        time.sleep(120)
    ...
    mimas_factor_cost_df_dict = pd.read_excel(mimas_log_parse_fpath, sheet_name=None, index_col=0)
    ...
    total_factor_cost_df_dict['Mimas样本'] = mimas_factor_cost_df_dict['因子耗时Mimas']
    ...
    mimas_tupo_df_dict = pd.read_excel(mimas_record_path + f'每日突破/每日突破_{today_date}_{envir}.xlsx', sheet_name=None, index_col=0)
    ...
    mimas_deal = mimas_tupo_df_dict['每日订单']
    p4_deal['orderType'] = 'P4Buy'  # NOTE：因为Ceres和P4的订单中都是CeresBuy，在这一步拼接时一定要区分出来哪个是P4
    cpp_tupo_df_dict['每日订单'] = pd.concat([saturn_deal, eur_deal, jup_deal, leda_deal, ceres_deal, p4_deal, mimas_deal], axis=0)
    ...
    cpp_tupo_df_dict['每日突破Mimas'] = mimas_tupo_df_dict['每日突破Mimas']
    ...
    mimas_reject = mimas_tupo_df_dict['每日拒绝']
    if len(saturn_reject) != 0 or len(leda_reject) != 0 or len(jupiter_reject) != 0 or len(ceres_reject) != 0 or len(p4_reject) != 0 or len(mimas_reject) != 0:
        concat = pd.concat([cpp_reject, leda_reject, saturn_reject, jupiter_reject, ceres_reject, p4_reject, mimas_reject], axis=0)
    else:
        concat = cpp_reject
