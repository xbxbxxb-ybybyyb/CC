...
from ProdWork.intra_strong.add_mimas.recordUtil import getExtraBuyInfo, getExtraSellInfo, calProfit, calProfit_pj2, trans_any2code
...
def updatePositionBuy(buyDf, resDf, resDf001, resDf_metis, resDf_leda, resDf_pj2, resDf_ceres, resDf_p4, resDf_mimas, buy_orderDf):
    # 添加买入的分钟数据
    ...
    for index, row in buyDf_jupiter.iterrows():
        ...
        resDf.loc[dummy_resDfindex, '买入当日收益率(%)'] = (resDf.loc[dummy_resDfindex, '买入当天收盘价'] / resDf.loc[dummy_resDfindex, '买入成交均价'] - 1) * 100
    ...
    for index, row in buyDf_ceres.iterrows():
        ...
        stock_code = row['证券代码']
        resDf_ceres.loc[dummy_resDfceres_index, buy_commonInfoColumns_rename + buyExtraColumns] = row[buy_commonInfoColumns + buyExtraColumns].values
        ...
        resDf_ceres.loc[dummy_resDfceres_index, ['买入数量', '买入金额']] = sel_order[['lastQty', 'lastAmt']].sum().values
        resDf_ceres.loc[dummy_resDfceres_index, '买入成交均价'] = resDf_ceres.loc[dummy_resDfceres_index, '买入金额'] / resDf_ceres.loc[dummy_resDfceres_index, '买入数量']
        # resDf_ceres.loc[dummy_resDfceres_index, ['买入时点']] = '931'
        resDf_ceres.loc[dummy_resDfceres_index, '买入当日收益率(%)'] = (resDf_ceres.loc[dummy_resDfceres_index, '买入当天收盘价'] / resDf_ceres.loc[dummy_resDfceres_index, '买入成交均价'] - 1) * 100

    for index, row in buyDf_p4.iterrows():
        dummy_resDfp4_index = len(resDf_p4)
        stock_code = row['证券代码']
        resDf_p4.loc[dummy_resDfp4_index, buy_commonInfoColumns_rename + buyExtraColumns] = row[buy_commonInfoColumns + buyExtraColumns].values
        ...
        resDf_p4.loc[dummy_resDfp4_index, ['买入数量', '买入金额']] = sel_order[['lastQty', 'lastAmt']].sum().values
        resDf_p4.loc[dummy_resDfp4_index, '买入成交均价'] = resDf_p4.loc[dummy_resDfp4_index, '买入金额'] / resDf_p4.loc[dummy_resDfp4_index, '买入数量']
        # resDf_p4.loc[dummy_resDfp4_index, ['买入时点']] = '931'
        resDf_p4.loc[dummy_resDfp4_index, '买入当日收益率(%)'] = (resDf_p4.loc[dummy_resDfp4_index, '买入当天收盘价'] / resDf_p4.loc[dummy_resDfp4_index, '买入成交均价'] - 1) * 100
    
    # Mimas  
    for index, row in buyDf_mimas.iterrows():
        dummy_resDfmimas_index = len(resDf_mimas)
        stock_code = row['证券代码']
        resDf_mimas.loc[dummy_resDfmimas_index, buy_commonInfoColumns_rename + buyExtraColumns] = row[buy_commonInfoColumns + buyExtraColumns].values
        ...
        resDf_mimas.loc[dummy_resDfmimas_index, ['买入数量', '买入金额']] = sel_order[['lastQty', 'lastAmt']].sum().values
        resDf_mimas.loc[dummy_resDfmimas_index, '买入成交均价'] = resDf_mimas.loc[dummy_resDfmimas_index, '买入金额'] / resDf_mimas.loc[dummy_resDfmimas_index, '买入数量']
        # resDf_mimas.loc[dummy_resDfmimas_index, ['买入时点']] = '931'
        resDf_mimas.loc[dummy_resDfmimas_index, '买入当日收益率(%)'] = (resDf_mimas.loc[dummy_resDfmimas_index, '买入当天收盘价'] / resDf_mimas.loc[dummy_resDfmimas_index, '买入成交均价'] - 1) * 100
    ...
    #%% Europa: 买入方面，先匹配其他策略的，最后剩的放到europa策略里（以下匹配了Jupiter\Metis\Leda）
    for index, row in buyDf_jupiter001.iterrows():
        ...
        #     flag = 1
        if stock_code in resDf_ceres[resDf_ceres['买入日期'] == Adate]['证券代码'].tolist():
            buy_vol = buy_vol - float(resDf_ceres[(resDf_ceres['买入日期'] == Adate) & (resDf_ceres['证券代码'] == stock_code)]['买入数量'])
            buy_amt = buy_amt - float(resDf_ceres[(resDf_ceres['买入日期'] == Adate) & (resDf_ceres['证券代码'] == stock_code)]['买入金额'])
            flag = 1
        if stock_code in resDf_p4[resDf_p4['买入日期'] == Adate]['证券代码'].tolist():
            buy_vol = buy_vol - float(resDf_p4[(resDf_p4['买入日期'] == Adate) & (resDf_p4['证券代码'] == stock_code)]['买入数量'])
            buy_amt = buy_amt - float(resDf_p4[(resDf_p4['买入日期'] == Adate) & (resDf_p4['证券代码'] == stock_code)]['买入金额'])
            flag = 1
        if stock_code in resDf_mimas[resDf_mimas['买入日期'] == Adate]['证券代码'].tolist():
            buy_vol = buy_vol - float(resDf_mimas[(resDf_mimas['买入日期'] == Adate) & (resDf_mimas['证券代码'] == stock_code)]['买入数量'])
            buy_amt = buy_amt - float(resDf_mimas[(resDf_mimas['买入日期'] == Adate) & (resDf_mimas['证券代码'] == stock_code)]['买入金额'])
            flag = 1
    ...
    return resDf[resDf['买入数量'] > 0], resDf_pj2[resDf_pj2['买入数量'] > 0], resDf_ceres[resDf_ceres['买入数量'] > 0], resDf_p4[resDf_p4['买入数量'] > 0], resDf_mimas[resDf_mimas['买入数量'] > 0], \
           resDf001[resDf001['买入数量'] > 0], resDf_metis[resDf_metis['买入数量'] > 0], resDf_leda[resDf_leda['买入数量'] > 0]

def updatePositionSell(sellDf, resDf1, resDf1001, resDf1_metis, resDf1_leda, resDf1_pj2, resDf1_ceres, resDf1_p4, resDf1_mimas, date, dead_circle):
    sellCommonColumns = ['卖出日期', '卖出成交均价', '卖出日期开盘价']
    sellDf['证券代码'] = sellDf['证券代码'].apply(trans_any2code)
    ...
    if len(resDf1_ceres) == 0:
        resDf1_ceres = pd.DataFrame(columns=resDf1_pj2.columns.tolist())
    resDf1_ceres.replace(np.nan, '', inplace=True)

    if len(resDf1_p4) == 0:
        resDf1_p4 = pd.DataFrame(columns=resDf1_pj2.columns.tolist())
    resDf1_p4.replace(np.nan, '', inplace=True)
    
    if len(resDf1_mimas) == 0:
        resDf1_mimas = pd.DataFrame(columns=resDf1_pj2.columns.tolist())
    resDf1_mimas.replace(np.nan, '', inplace=True)
    ...
    for index, row in sellDf.iterrows():
        ...
        def saturn_sell_one_day_holding(sold_qty, resDf1_pj2):  # 对一天的saturn进行卖出
            ...
            if resDf1_pj2.loc[historyIndex, '买入金额'] == 0:
                resDf1_pj2.loc[historyIndex, '是否全部卖出'] = 1
                resDf1_pj2.loc[historyIndex, '卖出比例'] = "100.00%"
            volume_left_to_be_sell = sold_qty - qty_allocate_here
        ...
        while volume_left_to_be_sell > 1e-5: # 1108 只剩100股，大家时间都一样
            ...
            if len(resDf1_ceres[(resDf1_ceres['证券代码'] == stockCode) & (resDf1_ceres['是否全部卖出'] != 1)]['买入日期']) > 0:
                ceres_min_date = resDf1_ceres[(resDf1_ceres['证券代码'] == stockCode) & (resDf1_ceres['是否全部卖出'] != 1)]['买入日期'].min()
            else:
                ceres_min_date = '2030-12-31'
            if len(resDf1_p4[(resDf1_p4['证券代码'] == stockCode) & (resDf1_p4['是否全部卖出'] != 1)]['买入日期']) > 0:
                p4_min_date = resDf1_p4[(resDf1_p4['证券代码'] == stockCode) & (resDf1_p4['是否全部卖出'] != 1)]['买入日期'].min()
            else:
                p4_min_date = '2030-12-31'
            if len(resDf1_mimas[(resDf1_mimas['证券代码'] == stockCode) & (resDf1_mimas['是否全部卖出'] != 1)]['买入日期']) > 0:
                mimas_min_date = resDf1_mimas[(resDf1_mimas['证券代码'] == stockCode) & (resDf1_mimas['是否全部卖出'] != 1)]['买入日期'].min()
            else:
                mimas_min_date = '2030-12-31'
            ...
            if (len(resDf1[(resDf1['证券代码'] == stockCode) & (resDf1['是否全部卖出'] != 1) & (resDf1['买入日期'] != Adate)]['买入日期']) > 0) and \
                    ...
                    (len(resDf1_ceres[(resDf1_ceres['证券代码'] == stockCode) & (resDf1_ceres['是否全部卖出'] != 1) & (resDf1_ceres['买入日期'] != Adate)]['买入日期']) == 0) and\
                    (len(resDf1_p4[(resDf1_p4['证券代码'] == stockCode) & (resDf1_p4['是否全部卖出'] != 1) & (resDf1_p4['买入日期'] != Adate)]['买入日期']) == 0) and \
                    (len(resDf1_mimas[(resDf1_mimas['证券代码'] == stockCode) & (resDf1_mimas['是否全部卖出'] != 1) & (resDf1_mimas['买入日期'] != Adate)]['买入日期']) == 0):
                ...
                volume_left_to_be_sell, resDf1_ceres = saturn_sell_one_day_holding(volume_left_to_be_sell, resDf1_ceres)
            elif (len(resDf1001[(resDf1001['证券代码'] == stockCode) & (resDf1001['是否全部卖出'] != 1) & (resDf1001['买入日期'] != Adate)]['买入日期']) == 0) and \
                    (len(resDf1[(resDf1['证券代码'] == stockCode) & (resDf1['是否全部卖出'] != 1) & (resDf1['买入日期'] != Adate) & (resDf1['买入日期'] != Adate)]['买入日期']) == 0) and \
                    (len(resDf1_metis[(resDf1_metis['证券代码'] == stockCode) & (resDf1_metis['是否全部卖出'] != 1) & (resDf1_metis['买入日期'] != Adate)]['买入日期']) == 0) and \
                    (len(resDf1_leda[(resDf1_leda['证券代码'] == stockCode) & (resDf1_leda['是否全部卖出'] != 1) & (resDf1_leda['买入日期'] != Adate)]['买入日期']) == 0) and \
                    ...
                    (len(resDf1_p4[(resDf1_p4['证券代码'] == stockCode) & (resDf1_p4['是否全部卖出'] != 1) & (resDf1_p4['买入日期'] != Adate)]['买入日期']) > 0) and \
                    (len(resDf1_mimas[(resDf1_mimas['证券代码'] == stockCode) & (resDf1_mimas['是否全部卖出'] != 1) & (resDf1_mimas['买入日期'] != Adate)]['买入日期']) == 0):
                print(f'该标的{stockCode}只有p4存在持仓')
                volume_left_to_be_sell, resDf1_p4 = saturn_sell_one_day_holding(volume_left_to_be_sell, resDf1_p4)
            elif (len(resDf1001[(resDf1001['证券代码'] == stockCode) & (resDf1001['是否全部卖出'] != 1) & (resDf1001['买入日期'] != Adate)]['买入日期']) == 0) and \
                    (len(resDf1[(resDf1['证券代码'] == stockCode) & (resDf1['是否全部卖出'] != 1) & (resDf1['买入日期'] != Adate) & (resDf1['买入日期'] != Adate)]['买入日期']) == 0) and \
                    (len(resDf1_metis[(resDf1_metis['证券代码'] == stockCode) & (resDf1_metis['是否全部卖出'] != 1) & (resDf1_metis['买入日期'] != Adate)]['买入日期']) == 0) and \
                    (len(resDf1_leda[(resDf1_leda['证券代码'] == stockCode) & (resDf1_leda['是否全部卖出'] != 1) & (resDf1_leda['买入日期'] != Adate)]['买入日期']) == 0) and \
                    ...
                    (len(resDf1_p4[(resDf1_p4['证券代码'] == stockCode) & (resDf1_p4['是否全部卖出'] != 1) & (resDf1_p4['买入日期'] != Adate)]['买入日期']) == 0) and \
                    (len(resDf1_mimas[(resDf1_mimas['证券代码'] == stockCode) & (resDf1_mimas['是否全部卖出'] != 1) & (resDf1_mimas['买入日期'] != Adate)]['买入日期']) > 0):
                print(f'该标的{stockCode}只有mimas存在持仓')
                volume_left_to_be_sell, resDf1_mimas = saturn_sell_one_day_holding(volume_left_to_be_sell, resDf1_mimas)
            elif (len(resDf1001[(resDf1001['证券代码'] == stockCode) & (resDf1001['是否全部卖出'] != 1) & (resDf1001['买入日期'] != Adate)]['买入日期']) == 0) and \
                    (len(resDf1[(resDf1['证券代码'] == stockCode) & (resDf1['是否全部卖出'] != 1) & (resDf1['买入日期'] != Adate) & (resDf1['买入日期'] != Adate)]['买入日期']) == 0) and \
                    (len(resDf1_metis[(resDf1_metis['证券代码'] == stockCode) & (resDf1_metis['是否全部卖出'] != 1) & (resDf1_metis['买入日期'] != Adate)]['买入日期']) == 0) and \
                    (len(resDf1_leda[(resDf1_leda['证券代码'] == stockCode) & (resDf1_leda['是否全部卖出'] != 1) & (resDf1_leda['买入日期'] != Adate)]['买入日期']) > 0) and \
                ...
                min_date = pd.Series([jupiter_min_date, jupiter_min_date001, metis_min_date, leda_min_date, ceres_min_date, p4_min_date, mimas_min_date, saturn_min_date]).sort_values().iloc[0]
                max_date = pd.Series([jupiter_min_date, jupiter_min_date001, metis_min_date, leda_min_date, ceres_min_date, p4_min_date, mimas_min_date, saturn_min_date]).sort_values(ascending=False).iloc[0]
                ...
                # elif (ceres_min_date < jupiter_min_date) and (ceres_min_date < saturn_min_date) and (ceres_min_date < jupiter_min_date001) and (ceres_min_date < metis_min_date):
                    ...
                    break
    return resDf1, resDf1_pj2, resDf1_ceres, resDf1_p4, resDf1_mimas, resDf1001, resDf1_metis, resDf1_leda
...
if __name__ == "__main__":
    ...
    historyFile_ceres = '/data/group/800463/日内强势股/实盘分析记录/日内强势股总卖出记录/日内强势股总卖出记录Ceres-%s.xlsx' % lastdate
    historyFile_p4 = '/data/group/800463/日内强势股/实盘分析记录/日内强势股总卖出记录/日内强势股总卖出记录P4-%s.xlsx' % lastdate
    historyFile_mimas = '/data/group/800463/日内强势股/实盘分析记录/日内强势股总卖出记录/日内强势股总卖出记录Mimas-%s.xlsx' % lastdate
    ...
    if os.path.exists(historyFile_ceres):
        historySellDf_ceres = deal_Unnamed(pd.read_excel(historyFile_ceres, sheet_name='总卖出记录'))
        historySellDf_ceres['是否全部卖出'] = historySellDf_ceres.apply(lambda x: 0 if x['卖出比例'] != '100.00%' else 1, axis=1)
    else:
        historySellDf_ceres = pd.DataFrame(columns=historySellDf_pj2.columns)

    if os.path.exists(historyFile_p4):
        historySellDf_p4 = deal_Unnamed(pd.read_excel(historyFile_p4, sheet_name='总卖出记录'))
        historySellDf_p4['是否全部卖出'] = historySellDf_p4.apply(lambda x: 0 if x['卖出比例'] != '100.00%' else 1, axis=1)
    else:
        historySellDf_p4 = pd.DataFrame(columns=historySellDf_pj2.columns)
        
    if os.path.exists(historyFile_mimas):
        historySellDf_mimas = deal_Unnamed(pd.read_excel(historyFile_mimas, sheet_name='总卖出记录'))
        historySellDf_mimas['是否全部卖出'] = historySellDf_mimas.apply(lambda x: 0 if x['卖出比例'] != '100.00%' else 1, axis=1)
    else:
        historySellDf_mimas = pd.DataFrame(columns=historySellDf_pj2.columns)
    ...
    orderDf_p4 = orderDf[orderDf['orderType'] == 'P4Buy']
    orderDf_mimas = orderDf[orderDf['orderType'] == 'MimasBuy']
    ...
    resDf_ceres = historySellDf_ceres.copy()
    if '前日形态' not in resDf_ceres.columns:
        resDf_ceres['前日形态'] = np.nan

    buyDf = todayBuyDf.copy()
    sellDf_tot = todaySellDf.copy()

    resDf1, resDf1_pj2, resDf1_ceres, resDf1_p4, resDf1_mimas, resDf1001, resDf1_metis, resDf1_leda = updatePositionBuy(todayBuyDf.copy(),
                                                                                                      historySellDf.copy(),
                                                                                                      historySellDf001.copy(),
                                                                                                      historyFile_metis.copy(),
                                                                                                      historyFile_leda.copy(),
                                                                                                      historySellDf_pj2.copy(),
                                                                                                      historySellDf_ceres.copy(),
                                                                                                      historySellDf_p4.copy(),
                                                                                                      historySellDf_mimas.copy(),
                                                                                                      buy_orderDf)
    ...
    resDf2, resDf2_pj2, resDf2_ceres, resDf2_p4, resDf2_mimas, resDf2001, resDf2_metis, resDf2_leda = \
        updatePositionSell(todaySellDf.copy(), resDf1, resDf1001, resDf1_metis, resDf1_leda, resDf1_pj2, resDf1_ceres, resDf1_p4, resDf1_mimas, date, dead_circle=0)
    ...
    resDf2_ceres.replace('', np.nan, inplace=True)
    resDf2_p4.replace('', np.nan, inplace=True)
    resDf2_mimas.replace('', np.nan, inplace=True)
    ...
    if len(o2ul_nan_samples) != 0:
        ...
        for i in o2ul_nan_samples.index:
            ...
            resDf2.loc[i, 'TN_o2ul'] = tmp_df.loc[buy_date, stock]    #TODO: 这里为什么突然有问题？？加了.values[0]
            # resDf2.loc[i, 'TN_o2ul'] = tmp_df.loc[buy_date, stock].values[0]    #TODO: 这里为什么突然有问题？？加了.values[0]

    if 'TN_o2ul' not in resDf2_leda.columns:
        resDf2_leda['TN_o2ul'] = np.nan
    o2ul_nan_samples_leda = resDf2_leda[resDf2_leda['TN_o2ul'].isnull()]
    ...
    if len(o2ul_nan_samples_leda) != 0:
        ...
        for i in o2ul_nan_samples_leda.index:
            ...
            # resDf2_leda.loc[i, 'TN_o2ul'] = tmp_df.loc[buy_date, stock].values[0]
    ...
    #%% TODO：给Ceres和P4更新label_v2o10d1、label_v2o10nd1
    if 'TN_v2o10d1' not in resDf2_ceres.columns:
        resDf2_ceres['TN_v2o10d1'] = np.nan
    v2o10_nan_samples_not_today = resDf2_ceres[resDf2_ceres['TN_v2o10d1'].isnull() & (resDf2_ceres['买入日期'] != Adate)]
    if len(v2o10_nan_samples_not_today) != 0:
        for index, row in v2o10_nan_samples_not_today.iterrows():
            stock = row['证券代码']
            buy_A_date = row['买入日期']
            ...
            resDf2_ceres.loc[index, 'TN_v2o10d1'] = this_v2o10d1

    if 'TN_v2o10nd1' not in resDf2_p4.columns:
        resDf2_p4['TN_v2o10nd1'] = np.nan
    v2o10_nan_samples_not_today = resDf2_p4[resDf2_p4['TN_v2o10nd1'].isnull() & (resDf2_p4['买入日期'] != Adate)]
    if len(v2o10_nan_samples_not_today) != 0:
        for index, row in v2o10_nan_samples_not_today.iterrows():
            stock = row['证券代码']
            buy_A_date = row['买入日期']
            ...
            this_v2o10d1 = p4_basic_hf_info.loc[(pd.to_datetime(buy_date), stock), 'label_v2o10nd1']
            resDf2_p4.loc[index, 'TN_v2o10nd1'] = this_v2o10d1
            
    #%% 给Mimas更新
    if 'TN_v2o10dh1' not in resDf2_mimas.columns:
        resDf2_mimas['TN_v2o10dh1'] = np.nan
    v2o10_nan_samples_not_today = resDf2_mimas[resDf2_mimas['TN_v2o10dh1'].isnull() & (resDf2_mimas['买入日期'] != Adate)]
    if len(v2o10_nan_samples_not_today) != 0:
        for index, row in v2o10_nan_samples_not_today.iterrows():
            stock = row['证券代码']
            buy_A_date = row['买入日期']
            ...
            resDf2_mimas.loc[index, 'TN_v2o10dh1'] = this_v2o10dh1
    # -------------------------------------------------
    ...
    resDf3_ceres = calProfit_pj2(resDf2_ceres.copy(), 'ceres')
    resDf3_p4 = calProfit_pj2(resDf2_p4.copy(), 'p4')
    resDf3_mimas = calProfit_pj2(resDf2_mimas.copy(), 'mimas')
    ...
    resDf3_ceres.replace('', np.nan, inplace=True)
    resDf3_p4.replace('', np.nan, inplace=True)
    resDf3_mimas.replace('', np.nan, inplace=True)
    ...
    resDf3_ceres['卖出比例'] = (resDf3_ceres['卖出数量'].apply(cal_sum).astype(float) / resDf3_ceres['买入数量']).fillna(0).apply(lambda x: '%.2f%%' % (x * 100))
    resDf3_ceres.replace(np.nan, '', inplace=True)
    resDf3_p4['卖出比例'] = (resDf3_p4['卖出数量'].apply(cal_sum).astype(float) / resDf3_p4['买入数量']).fillna(0).apply(lambda x: '%.2f%%' % (x * 100))
    resDf3_p4.replace(np.nan, '', inplace=True)
    resDf3_mimas['卖出比例'] = (resDf3_mimas['卖出数量'].apply(cal_sum).astype(float) / resDf3_mimas['买入数量']).fillna(0).apply(lambda x: '%.2f%%' % (x * 100))
    resDf3_mimas.replace(np.nan, '', inplace=True)
    ...
    # ----------------------------增加Ceres样本的shouldBuySignal信号---------------------------------------
    today_ceres_signal = pd.read_excel('/data/group/800463/日内强势股/log_parse/因子耗时/因子耗时_%s_prod.xlsx' % Adate, sheet_name='Ceres样本')

    buysellDf_joined_today = resDf3_ceres[resDf3_ceres['买入日期'] == Adate]
    if len(today_ceres_signal) != 0:
        resDf3_ceres.loc[buysellDf_joined_today.index, 'shouldBuySignal'] = today_ceres_signal.set_index('Unnamed: 0').reindex(
            buysellDf_joined_today.rename(columns={'证券代码': 'Unnamed: 0'}).set_index('Unnamed: 0').index) \
                                                                    ['shouldBuySignal'].values * 1

    # ----------------------------增加P4样本的shouldBuySignal信号---------------------------------------
    today_p4_signal = pd.read_excel('/data/group/800463/日内强势股/log_parse/因子耗时/因子耗时_%s_prod.xlsx' % Adate, sheet_name='P4样本')

    buysellDf_joined_today = resDf3_p4[resDf3_p4['买入日期'] == Adate]
    if len(today_p4_signal) != 0:
        resDf3_p4.loc[buysellDf_joined_today.index, 'shouldBuySignal'] = today_p4_signal.set_index('Unnamed: 0').reindex(
            buysellDf_joined_today.rename(columns={'证券代码': 'Unnamed: 0'}).set_index('Unnamed: 0').index) \
                                                                                ['shouldBuySignal'].values * 1

    # ----------------------------增加Mimas样本的shouldBuySignal信号---------------------------------------
    today_mimas_signal = pd.read_excel('/data/group/800463/日内强势股/log_parse/因子耗时/因子耗时_%s_prod.xlsx' % Adate, sheet_name='Mimas样本')

    buysellDf_joined_today = resDf3_mimas[resDf3_mimas['买入日期'] == Adate]
    if len(today_mimas_signal) != 0:
        resDf3_mimas.loc[buysellDf_joined_today.index, 'shouldBuySignal'] = today_mimas_signal.set_index('Unnamed: 0').reindex(
            buysellDf_joined_today.rename(columns={'证券代码': 'Unnamed: 0'}).set_index('Unnamed: 0').index) \
                                                                                 ['shouldBuySignal'].values * 1

    resDf3_ceres[['证券名称', '证券代码', '买入日期', '卖出日期', '买入数量', '买入金额', '买入成交均价', '买入当天开盘价', '买入当天开盘涨幅(%)',
                '买入当天涨停价', '买入当天盘中是否涨停', '买入当天收盘价', '买入当天是否收盘涨停', '买入当日收益率(%)', '卖出数量', '卖出成交均价', '卖出金额',
                '是否全部卖出', '卖出比例', '卖出部分盈利金额', '卖出部分收益率(%)', '实际是否正收益', '卖出日期开盘价',
                '卖出金额占市场比', '总卖出数量', '前日形态', 'TN_v2o10d1', 'shouldBuySignal']].to_excel(
        '/data/group/800463/日内强势股/实盘分析记录/日内强势股总卖出记录/日内强势股总卖出记录Ceres-%s.xlsx' % date, sheet_name='总卖出记录', index=False)

    print('create sheet 总卖出记录 for  %s' % '/data/group/800463/日内强势股/实盘分析记录/日内强势股总卖出记录/日内强势股总卖出记录Ceres-%s.xlsx' % date)

    resDf3_p4[['证券名称', '证券代码', '买入日期', '卖出日期', '买入数量', '买入金额', '买入成交均价', '买入当天开盘价', '买入当天开盘涨幅(%)',
                  '买入当天涨停价', '买入当天盘中是否涨停', '买入当天收盘价', '买入当天是否收盘涨停', '买入当日收益率(%)', '卖出数量', '卖出成交均价', '卖出金额',
                  '是否全部卖出', '卖出比例', '卖出部分盈利金额', '卖出部分收益率(%)', '实际是否正收益', '卖出日期开盘价',
                  '卖出金额占市场比', '总卖出数量', '前日形态', 'TN_v2o10nd1', 'shouldBuySignal']].to_excel(
        '/data/group/800463/日内强势股/实盘分析记录/日内强势股总卖出记录/日内强势股总卖出记录P4-%s.xlsx' % date, sheet_name='总卖出记录', index=False)

    print('create sheet 总卖出记录 for  %s' % '/data/group/800463/日内强势股/实盘分析记录/日内强势股总卖出记录/日内强势股总卖出记录P4-%s.xlsx' % date)

    resDf3_mimas[['证券名称', '证券代码', '买入日期', '卖出日期', '买入数量', '买入金额', '买入成交均价', '买入当天开盘价', '买入当天开盘涨幅(%)',
               '买入当天涨停价', '买入当天盘中是否涨停', '买入当天收盘价', '买入当天是否收盘涨停', '买入当日收益率(%)', '卖出数量', '卖出成交均价', '卖出金额',
               '是否全部卖出', '卖出比例', '卖出部分盈利金额', '卖出部分收益率(%)', '实际是否正收益', '卖出日期开盘价',
               '卖出金额占市场比', '总卖出数量', '前日形态', 'TN_v2o10dh1', 'shouldBuySignal']].to_excel(
        '/data/group/800463/日内强势股/实盘分析记录/日内强势股总卖出记录/日内强势股总卖出记录Mimas-%s.xlsx' % date, sheet_name='总卖出记录', index=False)

    print('create sheet 总卖出记录 for  %s' % '/data/group/800463/日内强势股/实盘分析记录/日内强势股总卖出记录/日内强势股总卖出记录Mimas-%s.xlsx' % date)
