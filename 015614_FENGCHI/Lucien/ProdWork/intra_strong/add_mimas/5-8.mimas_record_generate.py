...
if __name__ == "__main__":
    ...
    while (os.path.exists('/data/group/800463/日内强势股/实盘分析记录/日内强势股总买入记录/日内强势股总买入记录Mimas-%s.xlsx'% date)==False) |\
          (os.path.exists('/data/group/800463/日内强势股/实盘分析记录/日内强势股总卖出记录/日内强势股总卖出记录Mimas-%s.xlsx'% date)==False):
        print(f'日内强势股总买入记录Mimas-{date}.xlsx 等待中')
        time.sleep(60)

    # ------------------------------------------------------新加模块结束------------------------------------------------------------
    buyDf = pd.read_excel('/data/group/800463/日内强势股/实盘分析记录/日内强势股总买入记录/日内强势股总买入记录Mimas-%s.xlsx'% date)
    sellDf = pd.read_excel('/data/group/800463/日内强势股/实盘分析记录/日内强势股总卖出记录/日内强势股总卖出记录Mimas-%s.xlsx'% date)
    ...
    buysellDf_joined = buysellDf_joined[['买入日期', '证券代码', '证券名称', '卖出日期', '买入数量', '买入金额','买入当天持仓金额', '买入成交均价', '买入当天开盘涨幅(%)',
                                           ...
                                           '卖出金额占市场比', '总卖出数量', 'TN_v2o10dh1', '回归信号','委托金额']]

    writePath = f'/data/group/800463/日内强势股/实盘分析记录/日内强势股成交记录/Mimas成交记录-{date}.xlsx'
    ...
    buyResDf, fig = get_buyDfTotalInfo(buyDf.copy(), Adate, strategy='Mimas')
    todayBuyResDf = get_buyDfTodayInfo(buyDf.copy(), Adate, strategy='Mimas')
    sellResDf = get_sellDfTotalInfo(sellDf.copy(), Adate, fig, strategy='Mimas')
    todaySellResDf = get_sellDfTodayInfo(sellDf.copy(), Adate)

    return_path = "/data/group/800463/日内强势股/实盘分析记录/日内强势股成交记录/收益画图/%s_%s_mimas.png" % (Adate, '生产环境')
    ...
    print(f'5-7.mimas_record_generate运行耗时{round(time.time() - t1, 2)}秒')
