...
def get_sellDfTotalInfo(sellDf,Adate,fig,trade_time,strategy='saturn'):
    ...
    if strategy=='saturn':
        labels_p2 = pd.read_excel('/data/group/800463/日内强势股/log_parse/因子耗时/实盘触发项目二931标签汇总_%s.xlsx' % Adate)
    # elif strategy=='ceres':
    ...
    if len(Label_summary.dt.unique())<=rolldays_num:
        ...
        signal_label = Label_summary[['dt', 'p2shouldBuySignal']].groupby('dt').apply(lambda x: x['p2shouldBuySignal'].sum() / x.count())[['p2shouldBuySignal']]
