...
print('metis更新实盘触发标签文件：current date = %s' % date)
...
if lastdate <= '20220516':
    ...
    Labels_prod_summary_old = pd.read_excel(f'/data/group/800463/日内强势股/metis_log_parse/因子耗时/实盘触发标签汇总Metis_{Alastdate}.xlsx')

model_columns = list(set(model_columns + Labels_prod_summary_old.filter(regex='_local_prob').columns.tolist()))
# print(model_columns)
raw_last_date_factor_time_cost = pd.read_excel(f'/data/group/800463/日内强势股/metis_log_parse/因子耗时/因子耗时_{Alastdate}_prod.xlsx', sheet_name='因子耗时Metis')
raw_last_date_factor_time_cost = raw_last_date_factor_time_cost[~raw_last_date_factor_time_cost['Unnamed: 0'].duplicated()]

raw_last_date_model_compare = pd.read_excel(f'/data/group/800463/日内强势股/metis_log_parse/模型差异/{lastdate}/模型差异Metis_{lastdate}_prod.xlsx',
...
Labels_prod_summary_new.to_excel('/data/group/800463/日内强势股/metis_log_parse/因子耗时/实盘触发标签汇总Metis_%s.xlsx' % Adate, index=False)
print('create file %s!!!!!!!!!!' % '/data/group/800463/日内强势股/metis_log_parse/因子耗时/实盘触发标签汇总Metis_%s.xlsx' % Adate)
print(f'2-5.label_summary_metis耗时{round(time.time() - t1, 6)}秒')