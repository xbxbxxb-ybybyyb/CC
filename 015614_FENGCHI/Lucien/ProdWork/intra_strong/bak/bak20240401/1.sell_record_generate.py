...
def updatePositionBuy(buyDf, resDf, resDf001, resDf_metis, resDf_pj2, resDf_pj3, buy_orderDf):
    ...
    for index, row in buyDf_jupiter001.iterrows():
        ...
        if stock_code in resDf[resDf['买入日期'] == Adate]['证券代码'].tolist() and stock_code in resDf_metis[resDf_metis['买入日期'] == Adate]['证券代码'].tolist():
            resDf001.loc[dummy_resDfindex, '买入数量'] = float(buy_orderDf_jupiter_info001.loc[stock_code]['deal_vol']) - float(resDf[(resDf['买入日期'] == Adate) & (resDf['证券代码'] == stock_code)]['买入数量']) - float(resDf_metis[(resDf_metis['买入日期'] == Adate) & (resDf_metis['证券代码'] == stock_code)]['买入数量'])
            resDf001.loc[dummy_resDfindex, '买入金额'] = float(buy_orderDf_jupiter_info001.loc[stock_code]['deal_amt']) - float(resDf[(resDf['买入日期'] == Adate) & (resDf['证券代码'] == stock_code)]['买入金额']) - float(resDf_metis[(resDf_metis['买入日期'] == Adate) & (resDf_metis['证券代码'] == stock_code)]['买入金额'])
        elif stock_code in resDf[resDf['买入日期'] == Adate]['证券代码'].tolist() and stock_code not in resDf_metis[resDf_metis['买入日期'] == Adate]['证券代码'].tolist():
        ...
        elif stock_code not in resDf[resDf['买入日期'] == Adate]['证券代码'].tolist() and stock_code in resDf_metis[resDf_metis['买入日期'] == Adate]['证券代码'].tolist():
            resDf001.loc[dummy_resDfindex, '买入数量'] = float(buy_orderDf_jupiter_info001.loc[stock_code]['deal_vol']) - float(resDf_metis[(resDf_metis['买入日期'] == Adate) & (resDf_metis['证券代码'] == stock_code)]['买入数量'])
            resDf001.loc[dummy_resDfindex, '买入金额'] = float(buy_orderDf_jupiter_info001.loc[stock_code]['deal_amt']) - float(resDf_metis[(resDf_metis['买入日期'] == Adate) & (resDf_metis['证券代码'] == stock_code)]['买入金额'])
        else:
    ...
    return resDf[resDf['买入数量'] > 0], resDf_pj2, resDf_pj3, resDf001[resDf001['买入数量'] > 0], resDf_metis[resDf_metis['买入数量'] > 0]

def updatePositionSell(sellDf, resDf1, resDf1001, resDf1_metis, resDf1_pj2, resDf1_pj3, date, dead_circle):
    ...
    for index, row in sellDf.iterrows():
        ...
        while volume_left_to_be_sell > 1e-5: # 1108 只剩100股，大家时间都一样
            ...
            if (len(resDf1[(resDf1['证券代码'] == stockCode) & (resDf1['是否全部卖出'] != 1) & (resDf1['买入日期'] != Adate)]['买入日期']) > 0) and \
                ...
                min_date = pd.Series([jupiter_min_date, jupiter_min_date001, metis_min_date, ceres_min_date, saturn_min_date]).sort_values().iloc[0]
                max_date = pd.Series([jupiter_min_date, jupiter_min_date001, metis_min_date, ceres_min_date, saturn_min_date]).sort_values(ascending=False).iloc[0]
                ...
                #     volume_left_to_be_sell, resDf1 = jupiter_sell_one_day_holding(volume_left_to_be_sell, resDf1)
                    ...
                    break
    return resDf1, resDf1_pj2, resDf1_pj3, resDf1001, resDf1_metis
...
if __name__ == "__main__":
    ...
    resDf1, resDf1_pj2, resDf1_pj3, resDf1001, resDf1_metis = updatePositionBuy(todayBuyDf.copy(),
                                                                              historySellDf.copy(),
    ...
    h_resDf1, h_resDf1_pj2, h_resDf1_pj3, h_resDf1001, h_resDf1_metis = resDf1.copy(), resDf1_pj2.copy(), resDf1_pj3.copy(), resDf1001.copy(), resDf1_metis.copy()
    sellDf = todaySellDf.copy()

    resDf2, resDf2_pj2, resDf2_pj3, resDf2001, resDf2_metis = updatePositionSell(todaySellDf.copy(), resDf1, resDf1001, resDf1_metis, resDf1_pj2, resDf1_pj3, date, dead_circle=0)
