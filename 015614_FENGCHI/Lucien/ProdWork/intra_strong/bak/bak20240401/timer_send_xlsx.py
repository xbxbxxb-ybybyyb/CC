...
while True:
    if os.path.exists(root_path + f'Europa成交记录-{today_date}.xlsx') and \
        ...
        os.path.exists(root_path + f'Metis成交记录-{today_date}.xlsx'):
...
today_record_list = list(set(set(eur_buy_list).union(set(jup_buy_list).union(sat_buy_list).union(set(metis_buy_list)))))
...
if set(today_record_list) == set(right_buy_list):
    ...
    send_message(','.join(right_record))
    send_message(','.join(record_right))
