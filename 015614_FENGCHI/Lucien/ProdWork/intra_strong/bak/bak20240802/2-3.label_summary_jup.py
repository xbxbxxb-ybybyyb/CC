...
if len(sys.argv) > 1:
    ...
    # date = '20240305' # 若未在当个交易日晚上运行程序，需要在次日早上修改date
print('jupiter更新实盘触发标签文件：current date = %s' % date)
...
raw_last_date_factor_time_cost = pd.read_excel('/data/group/800463/日内强势股/jupiter_log_parse/因子耗时/因子耗时_%s_prod.xlsx' % Alastdate)
raw_last_date_factor_time_cost = raw_last_date_factor_time_cost[~raw_last_date_factor_time_cost['Unnamed: 0'].duplicated()]

raw_last_date_model_compare = pd.read_excel(f'/data/group/800463/日内强势股/jupiter_log_parse/模型差异/{lastdate}/模型差异_{lastdate}_prod.xlsx', sheet_name='本地投票结果').rename(columns = {'Ticker':'Unnamed: 0'})
# 获取模型的预测结果
...
sell_record = pd.read_excel(f'/data/group/800463/日内强势股/实盘分析记录/日内强势股总卖出记录/日内强势股总卖出记录-{date}.xlsx').set_index(['买入日期', '证券代码'])['买入时形态']
...
print(f'2-3.label_summary_jup耗时{round(time.time() - t1, 6)}秒')