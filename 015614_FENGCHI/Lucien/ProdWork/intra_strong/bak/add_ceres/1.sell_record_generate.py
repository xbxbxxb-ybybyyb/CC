...
from ProdWork.intra_strong.add_ceres.recordUtil import getExtraBuyInfo, getExtraSellInfo, calProfit, calProfit_pj2, trans_any2code
...
def updatePositionBuy(buyDf, resDf, resDf001, resDf_metis, resDf_leda, resDf_pj2, resDf_ceres, resDf_p4, buy_orderDf):
    # 添加买入的分钟数据
    ...
    return resDf[resDf['买入数量'] > 0], resDf_pj2, resDf_ceres, resDf_p4, resDf001[resDf001['买入数量'] > 0], resDf_metis[resDf_metis['买入数量'] > 0], resDf_leda[resDf_leda['买入数量'] > 0]

def updatePositionSell(sellDf, resDf1, resDf1001, resDf1_metis, resDf1_leda, resDf1_pj2, resDf1_ceres, resDf1_p4, date, dead_circle):
    ...
    for index, row in sellDf.iterrows():
        ...
        while volume_left_to_be_sell > 1e-5: # 1108 只剩100股，大家时间都一样
            ...
            if (len(resDf1[(resDf1['证券代码'] == stockCode) & (resDf1['是否全部卖出'] != 1) & (resDf1['买入日期'] != Adate)]['买入日期']) > 0) and \
                    ...
                    (len(resDf1_p4[(resDf1_p4['证券代码'] == stockCode) & (resDf1_p4['是否全部卖出'] != 1) & (resDf1_p4['买入日期'] != Adate)]['买入日期']) == 0):
                ...
                min_date = pd.Series([jupiter_min_date, jupiter_min_date001, metis_min_date, leda_min_date, ceres_min_date, p4_min_date, saturn_min_date]).sort_values().iloc[0]
                max_date = pd.Series([jupiter_min_date, jupiter_min_date001, metis_min_date, leda_min_date, ceres_min_date, p4_min_date, saturn_min_date]).sort_values(ascending=False).iloc[0]
                ...
                # elif (ceres_min_date < jupiter_min_date) and (ceres_min_date < saturn_min_date) and (ceres_min_date < jupiter_min_date001) and (ceres_min_date < metis_min_date):
                    ...
                    break
    return resDf1, resDf1_pj2, resDf1_ceres, resDf1_p4, resDf1001, resDf1_metis, resDf1_leda
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        # date = '20250604' # 若未在当个交易日晚上运行程序，需要在次日早上修改date
    ...
    resDf1, resDf1_pj2, resDf1_ceres, resDf1_p4, resDf1001, resDf1_metis, resDf1_leda = updatePositionBuy(todayBuyDf.copy(),
                                                                                          historySellDf.copy(),
    ...
    resDf2, resDf2_pj2, resDf2_ceres, resDf2_p4, resDf2001, resDf2_metis, resDf2_leda = \
        updatePositionSell(todaySellDf.copy(), resDf1, resDf1001, resDf1_metis, resDf1_leda, resDf1_pj2, resDf1_ceres, resDf1_p4, date, dead_circle=0)
