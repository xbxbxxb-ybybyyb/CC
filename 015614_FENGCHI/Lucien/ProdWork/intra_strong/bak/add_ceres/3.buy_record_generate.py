...
from ProdWork.intra_strong.add_ceres.recordUtil import getExtraBuyInfo, write_excel
...
if __name__ == "__main__":
    ...
    def updatePositionBuy(buyDf, resDf, resDf_pj2, resDf_ceres, resDf_p4, logDf, buy_orderDf, resDf001, logDf001, buy_metis_orderDf, logDf_metis, logDf_leda):
        ...
        for index,row in buyDf_ceres.iterrows():
            ...
            resDf_ceres.loc[dummy_resDfceres_index, ['委托金额']] = float(signal_info_ceres.loc[index_in_timecost][['totalOrderAmt']])
        ...
        for index,row in buyDf_p4.iterrows():
            ...
            resDf_p4.loc[dummy_resDfp4_index, ['委托金额']] = float(signal_info_p4.loc[index_in_timecost][['totalOrderAmt']])
        ...
        return resDf, resDf_pj2, resDf_ceres, resDf_p4, resDf001, resDf_metis, resDf_leda
    ...
    def writeExcel_p4(resDf):
        ...
        sheetname = '总买入记录'
        resDf.replace(np.nan,'',inplace=True)
        # resDf = resDf.drop(['前日形态', 'TN_v2o10', 'TN_v2o10d1'], axis=1)
        print('write sheet %s for %s!!!!!!!' % (sheetname, writePath))
        write_excel(resDf, writePath, sheetname)
    ...
    resDf1, resDf1_pj2, resDf1_ceres, resDf1_p4, resDf1001, resDf1_metis, resDf1_leda = updatePositionBuy(todayBuyDf.copy(), historyBuyDf.copy(), historyBuyDf_pj2.copy(),
                                                                 historyBuyDf_ceres.copy(), historyBuyDf_p4.copy(), logDf.copy(), buy_orderDf.copy(), historyBuyDf001.copy(),
    ...
    # -------------------------在Ceres买入汇总中添加TN_v2o10d1的计算-------------------------------------------
    if 'TN_v2o10d1' not in resDf1_ceres.columns:
        resDf1_ceres['TN_v2o10d1'] = np.nan
    resDf1_ceres['买入当天持仓金额'] = resDf1_ceres['成交数量'] * resDf1_ceres['买入当天收盘价']
    v2o10d1_nan_samples_not_today = resDf1_ceres[resDf1_ceres['TN_v2o10d1'].isnull() & (resDf1_ceres['发生日期'] != Adate)]

    if len(v2o10d1_nan_samples_not_today) != 0:
        for index, row in v2o10d1_nan_samples_not_today.iterrows():
            stock = row['证券代码']
            buy_A_date = row['发生日期']
            buy_date = buy_A_date[0:4] + buy_A_date[5:7] + buy_A_date[8:10]
    ...
    v2o10d1_nan_samples_not_today = resDf1_p4[resDf1_p4['TN_v2o10nd1'].isnull() & (resDf1_p4['发生日期'] != Adate)]
