...
if __name__ == "__main__":
    ...
    def updatePositionBuy(buyDf, resDf, resDf_pj2, resDf_pj3, logDf, buy_orderDf, resDf001, logDf001, buy_metis_orderDf, logDf_metis, logDf_leda):
        ...
        for index, row in buyDf_leda.iterrows():
            ...
            sel_order = buy_orderDf_leda[((buy_orderDf_leda['ordStatus'] == 'PARTIALLY_FILLED') |
                                           (buy_orderDf_leda['ordStatus'] == 'FILLED')) &
                                          (buy_orderDf_leda['stockcode'] == stock_code)]
            resDf_leda.loc[dummy_resDfindex, ['成交数量', '成交金额']] = sel_order[['lastQty', 'lastAmt']].sum().values
        ...
        for index, row in buyDf_jupiter001.iterrows():
            ...
            resDf001.loc[dummy_resDfindex, '成交金额'] = buy_amt
            if flag == 0:
                resDf001.loc[dummy_resDfindex, ['成交数量', '成交金额', '成交均价']] = buy_orderDf_jupiter001_info.loc[stock_code][['deal_vol', 'deal_amt', 'deal_vwap']].values
    ...
    for i in rjf.index:
        ...
        # 如果项目三买了则jupiter不买，所以先判断是否在今日的项目三
            ...
            elif len(resDf1_leda[((resDf1_leda['证券代码'] == rjf.loc[i]['证券代码']) & (resDf1_leda['发生日期'] == rjf.loc[i]['发生日期']))]) > 0:
