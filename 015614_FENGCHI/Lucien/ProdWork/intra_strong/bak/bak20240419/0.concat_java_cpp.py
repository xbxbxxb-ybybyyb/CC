...
if len(sys.argv) > 1:
    ...
    # date = '20240207' # 若未在当个交易日晚上运行程序，需要在次日早上修改date
...
for envir in enviroment_list:
    ...
    cpp_tupo_df_dict['每日订单'] = pd.concat([saturn_deal, cpp_deal_jup, leda_deal], axis=0)
    ...
    if len(saturn_reject) != 0 or len(leda_reject) != 0:
        concat = pd.concat([cpp_reject, leda_reject, saturn_reject], axis=0)
    else:
        concat = cpp_reject
