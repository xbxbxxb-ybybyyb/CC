...
def excel_saver(output_dict, excel_name):
    ...
    dir_path = os.path.dirname(excel_name)
...
def trans_str2dic(str_line):
    ...
    str_line = str_line.replace('=inf', "=0") # 20240206 metis日志中多了inf
    str_line = str_line.replace(':inf', ":0") # 20240206 metis日志中多了inf
    try:
        ret = eval(str_line)
    except:
...
def getAmiBySymbolFromLine(line):
...
# 20240515 by fengc，拒单信息添加一个字段
def getRejectReason3(line, by1, by2, by3, form='(.*?)[,\n]'):
    if by1 not in line or by2 not in line or by3 not in line:
        return ''
    if by3 not in line:
        try:
            info_list = line.split(';')
            for tmp_line in info_list:
                if by1 not in tmp_line or by2 not in tmp_line or by3 not in tmp_line:
                    pass
                else:
                    operation = re.findall(r"%s=%s" % (by1, form), tmp_line)[0]
                    type = getValueByKeyFromLine(tmp_line, by=by2)
                    if str(operation).find('2') >= 0:
                        return type
                    else:
                        continue
            return getValueByKeyFromLine(line, by=by2)
        ...
        return getValueByKeyFromLine(line, by=by3)
