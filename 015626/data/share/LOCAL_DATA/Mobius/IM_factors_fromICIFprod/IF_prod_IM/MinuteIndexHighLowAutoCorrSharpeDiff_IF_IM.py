...
class MinuteIndexHighLowAutoCorrSharpeDiff_IF_IM(FutureFactor):
    ...
    def calculate(self, data):
        ...
        rtn_high = rtn[:, (auto_corr - np.percentile(auto_corr, 2 / 3 * 100))> 1e-9 ]
        rtn_low = rtn[:, (auto_corr - np.percentile(auto_corr, 1 / 3 * 100))< -1e-9]
