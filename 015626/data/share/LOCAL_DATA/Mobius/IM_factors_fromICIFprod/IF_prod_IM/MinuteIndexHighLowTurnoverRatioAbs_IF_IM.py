...
class MinuteIndexHighLowTurnoverRatioAbs_IF_IM(FutureFactor):
    '''
    ...
    def calculate(self, data):
        ...
        amount_0 = np.nanmean(np.where(close - high_threshold > 1e-9, amount, np.nan), axis=0)
        amount_1 = np.nanmean(np.where(close - low_threshold < -1e-9, amount, np.nan), axis=0)
