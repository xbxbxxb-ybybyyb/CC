...
class MinuteIndexHighLowAbsPxPathReturnDiff_IM(FutureFactor):
    '''
    Description: 
    Class: Group_Stat
    Author: jinpx
    ...
    def calculate(self, data):
        ...
        f = np.nanmean(r_sum[(AbsPxPath_mean_rank - 0.8) > 1e-9]) - np.nanmean(r_sum[(AbsPxPath_mean_rank - 0.2) < -1e-9])
