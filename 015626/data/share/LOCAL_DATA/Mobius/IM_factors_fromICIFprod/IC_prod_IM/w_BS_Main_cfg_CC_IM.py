# -*- coding: utf-8 -*-
"""
Created on Mon Jul 11 09:38:46 2022
...
class w_BS_Main_cfg_CC_IM(FutureFactor):
    ...
    def calculate(self, data):
        hclose = ts_mean(data['close'].iloc[-8:], 6)
        hret = (hclose/hclose.shift(1) - 1).iloc[-2:]
        hret_mask = ((hret > 0).values * data['weight'].values[-2:])
        stk_BuyUniqueOrderNum = data['BuyUniqueOrderNum'].values[-2:]
        stk_BuyTradeNum = data['BuyTradeNum'].values[-2:]
        stk_SellUniqueOrderNum = data['SellUniqueOrderNum'].values[-2:]
        stk_SellTradeNum = data['SellTradeNum'].values[-2:]        
        factor_raw = (stk_BuyUniqueOrderNum / r(stk_BuyTradeNum)) - (stk_SellUniqueOrderNum / r(stk_SellTradeNum))
        factor = -np.nanmean(factor_raw * hret_mask, axis = 0)
