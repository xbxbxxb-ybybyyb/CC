...
class Short_BS_11_CC_IM(FutureFactor):
    ...
    def calculate(self, data):
        ...
        sig1 = ma.array(stk_PxVolCorr, mask=(df_s - amount_mask<= 1e-9))
        sig1 = np.nanmean(sig1, axis=1)
        sig1 = bk.move_rank(sig1, 300, 150)
        sig2 = ma.array((stk_close - stk_open) / r(stk_AbsPxPath), mask=(df_s - amount_mask <= 1e-9))
