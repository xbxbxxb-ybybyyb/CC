...
if 'IC'in ticker.upper():
    ...
    multiplier = 300
    trail = '_if'
contract = pd.read_excel('/data/user/011477/Trade_Docs/%s/Mobius_%s/MobiusStrategy_%s_%s.xlsx'%(date.replace('-', ''),date.replace('-', ''), ticker[:2], date.replace('-', '')))['合约代码'].iloc[0] + 'E'

if 'IF' in ticker.upper():
    model_date_list = ['20221125_if_if_v6nl', '20221125_if_if_v6nl_orig']
else:    
    model_date_list = ['20221125_ic_ic_v7c', '20221125_ic_ic_v7c_orig', '20221125_ic_ic_short_v3c']

cash_dict = {}
cash_dict['20220715_ic_ic_prod_v6_fix'] = 0
cash_dict['20220715_ic_trade_v1_ic2'] = 0#2e8
cash_dict['20221021_ic_ic_short_v2'] = 0
cash_dict['20221125_ic_ic_short_v3c'] = 2e8 * 3 / 4
cash_dict['20221125_if_if_v6nl'] = 3.2e8
cash_dict['20221125_if_if_v6nl_orig'] = 1.3e8
cash_dict['20221125_ic_ic_v7c_orig'] = 2e8 * 3 / 4
cash_dict['20221125_ic_ic_v7c'] = 1.5e8

pos_divnum = 10


pos_dict = {}
pos_dict['20220715_ic_ic_prod_v6_fix'] = {(0, 0.0003) : (0.0, 0.0),
            (0.0003, 0.0004): (0.0, 0.333/pos_divnum),
            (0.0004, 0.0005) : (0, 0.666/pos_divnum),
            (0.0005, 0.0006): (0.333/pos_divnum, 1/pos_divnum),
            (0.0006, 0.0007): (0.666/pos_divnum, 1/pos_divnum),
            (0.0007, 100) : (1/pos_divnum, 1/pos_divnum)}

pos_dict['20220513_ic_trade_v1_ic2'] = {(0,      0.0004): (0,        0),
            (0.0004, 0.0005): (0,        0.333/pos_divnum),
            (0.0005, 0.0006): (0.333/pos_divnum,        0.666/pos_divnum),
            (0.0006, 0.0007): (0.666/pos_divnum,        1/pos_divnum),
            (0.0007, 100): (1/pos_divnum, 1/pos_divnum)}

pos_dict['20221021_ic_ic_short_v2'] = {(0,   0.1): (0,        0),
            (0.1,   0.2): (0,        0.5/pos_divnum),
            (0.2,   0.8): (0,        1/pos_divnum),
            (0.8, 0.9): (0.5/pos_divnum,        1/pos_divnum),
            (0.9, 100): (1/pos_divnum,     1/pos_divnum)}
            
pos_dict['20221125_ic_ic_short_v3c'] = {(0,   0.1): (0,        0),
            (0.1,   0.2): (0,        0.5/pos_divnum),
            (0.2,   0.8): (0,        1/pos_divnum),
            (0.8, 0.9): (0.5/pos_divnum,        1/pos_divnum),
            (0.9, 100): (1/pos_divnum,     1/pos_divnum)}
...
pos_dict['20221125_if_if_v6nl'] = {(0, 0.0002): (0.0, 0.0),
                (0.0002, 0.0003): (0.0, 0.333/10),
                (0.0003, 0.0004): (0.0/10, 0.666/10),
                (0.0004, 0.0005): (0.0/10, 1.0/10),
                (0.0005, 0.0006): (0.0/10, 1.0/10),
                (0.0006, 0.0007): (0.333/10, 1.0/10),
                (0.0007, 0.0008): (0.666/10, 1.0/10),
                (0.0008, 100): (1.0/10, 1.0/10)             
               }

pos_dict['20221125_if_if_v6nl_orig'] = {
            (0,   0.3): (0,        0),
            (0.3, 0.4): (0,        0.5/pos_divnum),
            (0.4, 0.8): (0,        1/pos_divnum),
            (0.8, 0.9): (0.5/pos_divnum,   1/pos_divnum),
            (0.9, 100): (1/pos_divnum,     1/pos_divnum)}

pos_dict['20221125_ic_ic_v7c_orig'] = {
            (0,   0.3): (0,        0),
            (0.3, 0.4): (0,        0.5/pos_divnum),
            (0.4, 0.8): (0,        1/pos_divnum),
            (0.8, 0.9): (0.5/pos_divnum,   1/pos_divnum),
...
pos_dict['20221125_ic_ic_v7c'] = {(0, 0.0002): (0.0, 0.0),
                (0.0002, 0.0003): (0.0, 0.333/10),
                (0.0003, 0.0004): (0.0/10, 0.666/10),
                (0.0004, 0.0005): (0.0/10, 1.0/10),
                (0.0005, 0.0006): (0.0/10, 1.0/10),
...
for i in range(len(model_date_list)):
    ...
    b.columns = ['rawsignal%s_python_%s' % (model_num[i], model_date)]
    python_model.append(b)
result = pd.concat(python_model, axis = 1)

for i in range(len(model_date_list)):
    model_date = model_date_list[i]
    if ('orig' in model_date) or ('short' in model_date):
...
for i, model_name in enumerate(model_date_list):
    ...
    ts = TS_BACK_TEST(signal_list1, save_signal_list = True, c_rate=2.8 / 100000, ticker = ticker, stop_loss = -0.005,tickslippage = 10, max_wait_tick_num = 4,
...
b = date.replace('-', '')
para = pd.read_excel('/data/user/016700/Data/para/Mobius_%s/MobiusStrategy_%s.xlsx'%(str(b).replace('-', ''), ticker[:2] + '_'+ str(b).replace('-', '')), sheet_name = '期初持仓列表')
...
para_tomorrow = pd.read_excel('/data/user/016700/Data/para/Mobius_%s/MobiusStrategy_%s.xlsx'%(str(tomorrow).replace('-', ''), ticker[:2] + '_'+ str(tomorrow).replace('-', '')), sheet_name = '期初持仓列表')
...
pnl_bf = (trading_stats1['发生金额(全价)'] - trading_stats1['settle'] * np.sign(trading_stats1['发生金额(全价)'])*multiplier).sum()
...
a = a.set_index('dt')
order_price = (a['成交价']*np.sign(a['发生金额'])).groupby(a.index).mean()
