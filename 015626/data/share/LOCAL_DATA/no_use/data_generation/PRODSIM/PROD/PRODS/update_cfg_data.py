...
# 获取股票当日的adjfactor
s = FactorData()
WIND_AShareEODPrices = s.get_factor_value('WIND_AShareEODPrices',factors = ['S_INFO_WINDCODE','S_DQ_ADJFACTOR'], trade_dt=[str(b)])
...
#WIND_AShareEODPrices = s.get_factor_value('WIND_AShareEODPrices',factors = ['TRADE_DT','S_INFO_WINDCODE','S_DQ_ADJFACTOR'], trade_dt=['>=%s'%c[-1] ]) 

# 这里写死了存托凭证，比较笨拙，后续想办法替换
...
temp_fs2['CHANGE_DT'] = temp_fs['CHANGE_DT'].astype('int')
...
ffill_list = ['open','high','low','close', 'adjfactor', 'float_shares','twap','weight']
...
def update_cfgdata(para):
    ...
    #         return
        ...
        tick = mdp.get_data_by_date("Stock", stock, str(date), ['1','2','3', '4','5'])

        fill_na_columns = ['Buy1Price', 'Buy2Price', 'Buy3Price', 'Buy4Price', 'Buy5Price', 'Sell1Price', 'Sell2Price', 'Sell3Price', 'Sell4Price', 'Sell5Price', 'LastPx']
        ...
        transaction = mdp.get_data_by_date("Transaction", stock, str(date), ['1','2','3', '4','5'])
        ...
        result = result[standard_columnslist]
        if os.path.exists(h5_path):
        ...
        lm = link.LinkMessage()
        lm.sendMessage([str(stock), str(e)])
        del lm

    return

# 获取时间段内需要进行更新的股票参数【stock，date，weight】
...
if __name__ == '__main__':
    ...
    end_date = edate
    #start_date, end_date = '20160101', '20210912'
    ...
    flag_check_path = '/data/group/800466/warehouse/prod/MD/MarketData/LOCAL_DATA/FLAGS/' + str(end_date) + '/' + str(end_date)+'_' + 'WEIGHT.success'
    while True:
    ...
    #for date in [20220118, 20220119, 20220120, 20220121]:
        update_by_date(date)
    flag_path_success = flag_root + str(end_date) + '_' + 'CFG.success'
