...
next_date = get_next_trading_date(start_date)
...
for TICKER in ['IC.CFE','IF.CFE','IH.CFE']:
    ...
    df00.index = [pd.to_datetime(next_date)]
    df00.index.name = 'dt'
    df00['expiration_days_00'] = df00['expiration_days_00']-1

    df01.index = [pd.to_datetime(next_date)]
    df01['expiration_days_01'] = df01['expiration_days_01']-1
    df01.index.name = 'dt'
    df = df00.join(df01)
...
indexdf['contract_main'] = indexdf.apply(lambda x:fd.get_change_date(x['Ticker'].split('.')[0], str(edate), 'ZL00')[0] + 'E', axis = 1)
