...
pos_dict['if_v7c'] =[[0.0, 0.2, 0.3, 0.8, 0.9],
                              [0.2, 0.3, 0.8, 0.9, 100.0],
                              [0, 0, 0, 5, 10],
                              [0, 5, 10, 10, 10]]
                            
pos_dict['if_v7_crn'] =[[0.0, 0.2, 0.3, 0.8, 0.9],
                              [0.2, 0.3, 0.8, 0.9, 100.0],
                              [0, 0, 0, 5, 10],
                              [0, 5, 10, 10, 10]]
...
for contract_cat in ['IF.CFE']:
    ...
    if 'IC' in contract_cat.upper():
        factor_init = '/data/group/800466/warehouse/prod/alpha/CHINA_FUTURES/MINUTE/model/model_trade/factor_definition/factor_definition_set_V4.0.1.json'
        xdjg = 1
        hycs = 200
        pjjk = pjjk_ic
        trail = ''
    ...
    paradict = {'合约代码': recent_future,
                '交易日期': int(next_tday),
                '行情分钟数据目录': '/data/group/800466/warehouse/prod/alpha/CHINA_FUTURES/mobius_data_for_prod/minuteData_sim/%s/' % str(next_tday),
                
                '历史因子文件目录': '/data/group/800466/warehouse/prod/alpha/CHINA_FUTURES/trade_files/%s/' % (str(next_tday) + '_' + str(model_date[0])),
                '因子配置文件': factor_init,
                '是否记录因子值': 'true',
    ...
    if sim == True:
        writer = pd.ExcelWriter(os.path.join(savepath, 'MobiusStrategy_%s_sim2.xlsx' % (contract_cat[:-4] + '_' +str(next_tday))))
    else:
