...
def get_500_300_list(date_str):
    ...
    hs300_stock_list = []
    try:
        #with pd.HDFStore(settings.WEIGHTO_FILE, mode='r') as hdf_store:
...
def generate_his_data(today_str, days, target_folder):
    ...
    AIMR.runTasks('/gen_mobius_data/generate_his_data.py', json.dumps(params))
    print('done!!!')
    b = time.time()
    lm = link.LinkMessage()
...
#    date_str = time.strftime('%Y%m%d', time.localtime(time.time()))
    ...
    while True:
        if (os.path.exists(flag_path1) == True) and (os.path.exists(flag_path2) == True) and (os.path.exists(flag_path3) == True) and (os.path.exists(flag_path4) == True):
            break
        time.sleep(60)
