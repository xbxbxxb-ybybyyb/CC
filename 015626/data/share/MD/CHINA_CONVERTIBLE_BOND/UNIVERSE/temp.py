...
amt_last10 = amt_last10.groupby(amt_last10.index.date).sum()
...
for t in tqdm(tickerlist):
    ...
    tdf['B_INFO_OUTSTANDINGBALANCE'] = tdf['B_INFO_OUTSTANDINGBALANCE'].fillna(method = 'ffill').fillna(method = 'bfill')
    try:
        t_rating = cbondrating.xs(t, level = 1)
        tdf = tdf.join(t_rating, how = 'outer')
        tdf['B_INFO_CREDITRATING'] = tdf['B_INFO_CREDITRATING'].fillna(method = 'ffill').fillna(method = 'bfill')
        tdf['B_CREDITRATING_CHANGE'] = tdf['B_CREDITRATING_CHANGE'].fillna(method = 'ffill').fillna(method = 'bfill')
    except Exception as e:
        print(e)
    try:
        stk_rating = stock_rating.xs(t, level = 1)
        tdf = tdf.join(stk_rating, how = 'outer')
        tdf[['B_INFO_CREDITRATING_stock']] = tdf['B_INFO_CREDITRATING_stock'].fillna(method = 'ffill').fillna(method = 'bfill')
        tdf['B_RATE_RATINGOUTLOOK_stock'] = tdf['B_RATE_RATINGOUTLOOK_stock'].fillna(method = 'ffill').fillna(method = 'bfill')
        tdf[['B_INFO_PRECREDITRATING_stock']] = tdf['B_INFO_PRECREDITRATING_stock'].fillna(method = 'ffill').fillna(method = 'bfill')
        tdf['B_CREDITRATING_CHANGE_stock'] = tdf['B_CREDITRATING_CHANGE_stock'].fillna(method = 'ffill').fillna(method = 'bfill')
    except Exception as e:
...
turnover_rate = (turnover_rate == 2).fillna(False)
...
a = date_mask[['B_INFO_OUTSTANDINGBALANCE', 'B_CREDITRATING_CHANGE', 'B_INFO_CREDITRATING', 'B_RATE_RATINGOUTLOOK_stock',
       'B_INFO_CREDITRATING_stock', 'B_INFO_PRECREDITRATING_stock',
       'B_CREDITRATING_CHANGE_stock',
       'LISTEDDATE', 'CONV_STARTDATE', 'CONV_ENDDATE', 'TRADE_DT_LAST',
       'stockcode']]
a = a.join(ccbondvaluation, how = 'left')

IO.pd_hdf5_writer(a, '/data/user/015626/data/share/MD/CHINA_CONVERTIBLE_BOND/UNIVERSE/temp.h5', dataset='temp', override = True)




'B_INFO_OUTSTANDINGBALANCE', 债券余额（元）
'B_INFO_CREDITRATING', 债券评级
'B_INFO_CREDITRATING_stock', 正股评级
'LISTEDDATE', 上市日期
'CONV_STARTDATE', 转股起始日期
'CONV_ENDDATE', 转股结束日期
'TRADE_DT_LAST', 最后交易日
'stockcode' 正股代码
