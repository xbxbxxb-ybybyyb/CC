...
def merge_evaluate(total_trade, total_daily_return, long_ret, short_ret, total_daily_profit, total_daily_open_value, trade_counts):
    ...
    results.loc[0, '累计收益'] = "{:.3e}".format(total_daily_profit.sum())
    ...
    results.loc[0, '每笔交易平均盈亏'] = format(trade['change'].mean(), '.3%')  # 每笔交易平均盈亏
...
def final_generate_pdf(trade_df, daily_df, _sdate, _edate, output_path):
    ...
    for ticker in ticker_list:
        ...
        if len(total_trade[total_trade['Ticker'] == ticker]) == 0 or ticker_daily_df['dailyret'].sum() == 0:
            ...
            _result_ticker = merge_evaluate(total_trade[total_trade['Ticker'] == ticker], ticker_daily_df['dailyret'], long_ticker_daily_df['dailypnl'].sum(), short_ticker_daily_df['dailypnl'].sum(), ticker_daily_df['dailypnl'], ticker_daily_df['open_value'], None)
    ...
    results_nodce = merge_evaluate(total_trade_nodce, total_daily_return_nodce, long_daily_df_nodce['dailypnl'].unstack().sum(axis = 1).sum(), short_daily_df_nodce['dailypnl'].unstack().sum(axis = 1).sum(), total_daily_profit_nodce, total_daily_open_value_nodce, daily_df_nodce['dailyret'].unstack().replace(0, np.nan).count(axis = 1))
    ...
    _rjys = rjys.loc[['累计收益', '年化收益', '最大回撤', '夏普比率', '平均日占资', '最大日占资', '平均日持仓数量', '最大日持仓数量', '胜率', '盈亏收益比', '平均持仓周期','做多收益', '做空收益']]
    ...
    trade_counts_nodce = daily_df_nodce['dailyret'].unstack().replace(0, np.nan).count(axis = 1)
    tc = pd.concat([trade_counts, trade_counts_nodce], axis = 1)
...
trade_df = pd.read_hdf('/dfs/group/800466/warehouse/test/share/trade_df_fix.h5')
daily_df = pd.read_hdf('/dfs/group/800466/warehouse/test/share/daily_df_fix.h5')

final_generate_pdf(trade_df, daily_df, 20250101, 20251231, output_path)