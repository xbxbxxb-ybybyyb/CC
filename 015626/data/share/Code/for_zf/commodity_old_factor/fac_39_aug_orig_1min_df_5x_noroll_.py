...
class fac_39_aug_orig_1min_df_5x_noroll_(FactorGenerator):
    def __init__(self):
        required_columns=['close', 'high', 'second_main_mask']

        super(fac_39_aug_orig_1min_df_5x_noroll_, self).__init__(required_columns=required_columns
                                  )
    ...
    def on_bar(self, data, aa, bb, ccc, ddd):
        ...
        rtn = (data['close'] - data['close'].shift(ccc))[mask].mean(axis = 1) 
        vol = rtn.rolling(aa, min_periods=1).std()
        co = cross_hub_num(data['close'], aa * 2)[mask].mean(axis = 1) + 1
        ret = (data['close'] - (data['high'].shift(ccc).rolling(aa, min_periods=1).max()))[mask].mean(axis = 1)
