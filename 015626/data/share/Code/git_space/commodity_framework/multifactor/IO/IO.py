...
def csv_dumper(file_list, hdf5, dataset, from_scratch=False, header_exist=True, factor_name='Jone', data_columns=True, **kwargs):
    """
    --- DESCRIPTION ---
    ...
    with pd.HDFStore(hdf5) as h5_store:
        ...
        for _file in file_list:
            ...
            if exist_flag:
                ...
                # Dump DataFrame
                h5_store.append(dataset, pd_file, data_columns=data_columns, **kwargs)
                h5_store.get_storer(dataset).attrs.modification_date = ts_meta
                print('%s appended to %s' % (date_ticker.strftime('%Y%m%d'), dataset))


def hdf5_replacer(file_list, hdf5, dataset, from_scratch=False, data_columns=True):
    """
    --- DESCRIPTION ---
    Reads in csv files as specified in file_list and tries to concatenate csv contents into one multiIndexed Pandas DataFrame
    ...
    with pd.HDFStore(hdf5) as h5_store:
        ...
        for _file in file_list:
            ...
            if not exist_flag:
                ...
                # Append new records
                h5_store.append(dataset, pd_file, data_columns=data_columns)
                h5_store.get_storer(dataset).attrs.modification_date = ts_meta
                print('%s appended to %s' % (date_ticker.strftime('%Y%m%d'), dataset))


def pd_hdf5_writer(pd_factor, hdf5, dataset=None, override=None, append=None, from_scratch=True,
                   override_overlap=True, date_column=None, min_itemsize=None, data_columns=True):
    """
    --- DESCRIPTION ---
    ...
    with pd.HDFStore(hdf5) as h5_store:
        ...
        if override:
            ...
            if not exist_flag:
                ...
                raise AssertionError
            h5_store.put(dataset, pd_factor, format='table', append=False, data_columns=data_columns, min_itemsize=min_itemsize)
            h5_store.get_storer(dataset).attrs.modification_date = ts_meta
            ...
            for date_ticker in dt_lst_to_process:
                ...
                if not exist_flag or override_overlap:
                    ...
                    h5_store.append(dataset, sliced_data, data_columns=data_columns)
                    h5_store.get_storer(dataset).attrs.modification_date = ts_meta
                    print('%s appended to %s' % (date_ticker.strftime('%Y%m%d'), dataset))
        else:
            # new stash
            ...
            if not exist_flag:
                h5_store.put(dataset, pd_factor, format='table', append=False,
                             data_columns=data_columns, min_itemsize=min_itemsize)
                h5_store.get_storer(dataset).attrs.modification_date = ts_meta
    ...
    with pd.HDFStore(abs_h5_path, 'r') as h5_store:
        ...
        if columns is not None:
            if not all([item in s_type_cols for item in columns]):
                warnings.warn('Illegal column item(s) found: %s' % ' '.join([item for item in columns if item not in s_type_cols]))
        else:
