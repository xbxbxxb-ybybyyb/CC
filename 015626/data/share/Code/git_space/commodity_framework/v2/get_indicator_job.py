...
#            print(para, 'no tick!')
        ...
        assert multiplier is not None, ticker
        ...
        tick = tick[(tick.TotalVolumeTrade != 0) & (tick.TotalValueTrade != 0) & (tick['LastPx'] < tick['PreClosePx'] * 1.3) & (tick['LastPx'] > tick['PreClosePx'] / 1.3) & \
...
#            print(para, 'no tick after select')
        ...
        assert abs(tick_vwap.mean() / midprice.mean() - 1) < 0.2,  para
        ...
        ptick_list = []
        for p in ptdays:
            ptick = fd.get_tickex(date, ticker).rename(columns = columns_mapping)['TotalVolumeTrade'].diff()
            if len(ptick) > 0:
                ptick_list.append(ptick)
        del(fd)
        if len(ptick_list) > 0:
            gigantic_t = np.nanmedian([x.quantile(0.95) for x in ptick_list])
            super_t = np.nanmedian([x.quantile(0.9) for x in ptick_list])
            big_t = np.nanmedian([x.quantile(0.8) for x in ptick_list])
            small_t = np.nanmedian([x[x > 0].quantile(0.8) for x in ptick_list])
        else:
        ...
        if night_end_time is not None:
            ...
            if len(night_auction) > 0:
                ...
                night_start_date = str(tickdf.between_time('195800','230000').index[0].date()).replace('-', '')
                night_index = pd.date_range(f'{night_start_date} 21:00:00',f'{night_end_date} {night_end_time}', freq=freq).to_list()
                day_index = night_index + day_index
...
freq = '1min'
start_date, end_date = 20100101, 20250514

dailym = IO.read_data([start_date, end_date],columns = ['contract', 'volume'], alt = '/dfs/group/800466/warehouse/prod/CHINA_COMMODITY/MD/DAILY/MD_MAIN_CHINA_COMMODITY_DAILY.h5')
dailys = IO.read_data([start_date, end_date],columns = ['contract', 'volume'], alt = '/dfs/group/800466/warehouse/prod/CHINA_COMMODITY/MD/DAILY/MD_SECONDMAIN_CHINA_COMMODITY_DAILY.h5')
...
#daily['Ticker'] = daily['Ticker'].apply(lambda x:x.replace('CZC', 'ZCE'))
#daily['wind_code'] = daily['wind_code'].apply(lambda x:x.replace('CZC', 'ZCE'))
