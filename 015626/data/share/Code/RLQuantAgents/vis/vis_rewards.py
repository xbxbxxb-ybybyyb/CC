...
if __name__ == '__main__':
    ...
    val_dir = '/home/appadmin/RLQuantAgents/engine/runs/20250428-170002_dmodel128_lr1e-05_test'
    f = open(os.path.join(val_dir,'val_minute_reward.pkl'),
             'rb')
    minute_data = pickle.load(f)
    f.close()
    f = open(os.path.join(val_dir, 'val_day_reward.pkl'), 'rb')
    day_data = pickle.load(f)
    f.close()
    f = open(os.path.join(val_dir, 'val_minute_pos.pkl'), 'rb')
    minute_pos_data = pickle.load(f)
    ...
    f = open(os.path.join(val_dir, 'val_minute_timestamp.pkl'),
             'rb')
    val_minute_time_stamp = pickle.load(f)
    f.close()
    f = open(os.path.join(val_dir, 'val_day_timestamp.pkl'),
             'rb')
    val_day_time_stamp = pickle.load(f)
    f.close()
    arr1 = np.array(minute_data)
    arr2 = np.array(day_data)
    assert len(val_day_time_stamp) == len(day_data)
    assert len(val_minute_time_stamp) == len(minute_pos_data)
    negative_count = 0
    positive_count = 0

    arr3 = np.array(minute_pos_data)
