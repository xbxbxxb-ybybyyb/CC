...
class TradingEnv(object):
    ...
    def _update_volatility(self):
        ...
        clean_price = prices[~np.isnan(prices)][-5:]
        self.volatility = np.std((clean_price[1:] - clean_price[:-1]) / clean_price[:-1]) * 1000
        self.trend = clean_price[0] - clean_price[-1]
    ...
    def get_panalty(self, action, trend):
        delta = 0
        if self.current_step != 0 and self.current_step != 1 and action == 0 and self.clock >= 5:
            delta = 0.8 * (math.sqrt(self.clock) - 9)
        # For Short Trading
        # if self.volatility > 0.65 and action >= 0.15 and trend > 10:
        #     delta = -4.5
        # if self.volatility > 0.65 and action <= -0.15 and trend <-10:
        #     delta = -4.5
        # if self.volatility > 0.65 and action >= 0.15 and trend < -10:
        ...
        # if self.volatility < 0.3 and action >= -0.01 and action < 0.01 and abs(trend) < 4.5:
        #     delta = -3.5
        # if self.volatility < 0.3 and abs(action)>=0.15:
        #     delta = 5

        if self.volatility < 0.3 and abs(action) >= 0.05:
            delta = 5
        # if self.volatility < 0.65 and abs(action) >= 0.05 and abs(trend) > 6:
        #     delta = -4.5
        # if action < 0 and trend > 15:
        #     delta = 5
        #
        # if action > 0 and trend < -15:
        #     delta = 5

        # if action < -0.15 and min(self.data.get_obs_price(self.current_step)) == \
        #         self.data.get_obs_price(self.current_step)[-1] and abs(trend) > 15:
        #     delta = -0.75
        #
        # if action > 0.15 and max(self.data.get_obs_price(self.current_step)) == \
        #         self.data.get_obs_price(self.current_step)[-1] and abs(trend) > 15:
        #     delta = -0.75



        # if action >= 0.15 and trend > 6.5:
        #     delta = -3.75

        # if self.volatility > 0.65 and abs(action) <= 0.10 and abs(self.position) < 0.8:
        #     delta = 3
        # if self.volatility < 0.45 and abs(action) >= 0.10:
        #     delta = 8.0

        # For long Trading
        # if self.volatility < 0.35 and abs(action) <=0.01:
        #     delta = -10
        #
        # if self.volatility > 0.70 and action >= 0.15 and trend > 10:
        #      delta = 1.5
        # if self.volatility > 0.70 and abs(action) >= 0.15:
        #      delta = -1.5
        # if self.volatility < 0.35 and action <= -0.15:
        #     delta = 8.0
        #
        # if self.volatility > 0.55 and action >= 0.15:
        #     delta = -3.8
        # if self.volatility > 0.55 and action <= -0.15:
        #     delta = -0.7
        return delta
    ...
    def step(self, action_idx):
        ...
        if abs(new_position) > 0.8:
            future_reward = future_reward * 3
        penalty = self.get_panalty(position_delta, self.trend)
        # future_reward = 0
