...
def add_turnover_rate_and_adj(df, stock):
    ...
    if stock == '300114.SZ':
        ashare = ashare_total.loc['302132.SZ'].reset_index(drop = True).sort_values(by = 'CHANGE_DT')
    else:
...
for date in cdate_list:
    ...
    print('update done')
    flag_check_path3 = flag_root + str(edate) + '_' + 'CFG_ICIF.success'
    print(flag_check_path3)
    while True:
        if (os.path.exists(flag_check_path3) == True) :
            print('start download data')
            break
        time.sleep(60)
