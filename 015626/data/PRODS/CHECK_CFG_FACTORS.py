...
def check_fill_ratio(date):
    ...
    temp = dc_ic.get_stock_data()['volume'].join(dc_if.get_stock_data()['volume']).join(dc_im.get_stock_data()['volume'])
    temp = temp > 0
    fill_ratio = temp.sum(axis = 1) / 1800

    if len(fill_ratio[fill_ratio < fill_ratio_t]) > fill_ratio_bar_t:
        print('stock fill ratio wrong:  %s' %  'volume')

    temp = dc_ic.get_stock_data()['BuyTradeMoney'].join(dc_if.get_stock_data()['BuyTradeMoney']).join(dc_im.get_stock_data()['BuyTradeMoney'])
    temp = temp + dc_ic.get_stock_data()['SellTradeMoney'].join(dc_if.get_stock_data()['SellTradeMoney']).join(dc_im.get_stock_data()['SellTradeMoney'])
    temp = temp + dc_ic.get_stock_data()['lo_amount'].join(dc_if.get_stock_data()['lo_amount']).join(dc_im.get_stock_data()['lo_amount'])
    temp = temp > 0
    fill_ratio = temp.sum(axis = 1) / 1800
