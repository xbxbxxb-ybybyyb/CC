...
model_name_list = ['%s_ic_ic_v7unifac_crn_trend'%full_date, '%s_im_im_v1unifac_crn_trend'%full_date, '%s_if_if_v7_crn_trend'%full_date]
...
version_ticker_dict = {'if_v7_crn':'IF.CFE', 'if_v7c':'IF.CFE', 'if_v7_crn_trend':'IF.CFE',
                       'ic_v7unifac_crn': 'IC.CFE', 'ic_v7unifac': 'IC.CFE', 'ic_v7unifac_crn_old': 'IC.CFE', 'ic_v7unifac_crn_trend': 'IC.CFE', 
                       'im_v1unifac_crn': 'IM.CFE', 'im_v1unifac_crn_trend': 'IM.CFE'}
model_date_use_dict = {
                       'if_v7_crn':'%s'%(model_date_dict['if_v7_crn']),
                       'if_v7_crn_trend':'%s'%(model_date_dict['if_v7_crn_trend']),
                       'if_v7c':'%s'%(model_date_dict['if_v7_crn']),
                       'ic_v7unifac_crn':'%s'%(model_date_dict['ic_v7unifac_crn']),
                       'ic_v7unifac_crn_trend':'%s'%(model_date_dict['ic_v7unifac_crn_trend']),
                       ...
                       'im_v1unifac_crn_trend':'%s'%(model_date_dict['im_v1unifac_crn_trend']),
                       }
...
def calc(ud, model_name):
    ...
    if '_trend' not in model_name:
        ...
        model_num = 25
        factor_dir_list = [
            '/data/group/800466/warehouse/prod/alpha/CHINA_FUTURES/MINUTE/TREND/minute_norm',
        ]       
    # load data
...
for datee in update_date_list:
    ...
    for version in version_list:
        ...
        save_path = '/data/group/800466/warehouse/prod/alpha/CHINA_FUTURES/trade_files/%s/historySignal' % (next_tday + '_' + model_date + trail)
...
flag_path = flag_rootpath + str(edate) + '/'
flag_path_success = flag_path  + 'MODEL.success'
with open(flag_path_success,'w') as file:
    pass 
