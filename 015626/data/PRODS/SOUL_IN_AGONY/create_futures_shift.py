...
# Save Path
# NO '/' AT THE END !
save_path = ('/dfs/group/800466/warehouse/prod/MD/MarketData/LOCAL_DATA/CSV/MINUTE/CHINA_FUTURES/tick_to_minute_shift_%s/ALL_CONTRACT'%str(ss)).format(ROOT_PATH)
...
def resample_and_update_csv(read_path, save_path):
    ...
    df_tick_data['dt'] = df_tick_data['dt'].apply(lambda x: x.split(' ')[0] + ' ' +switch_time(x.split(' ')[-1].replace(':', '').replace('.', '')[:-3]) + '000')
    df_tick_data['dt'] = df_tick_data['dt'].apply(lambda x1: x1[:13] + ':' + x1[13:15] + ':' + x1[15:17] + '.' + x1[17:])
    ...
    df_existed_minute_data.index = pd.to_datetime(df_existed_minute_data.index)
    ...
    if 'IF2308' in save_path:
        print(df_existed_minute_data.index[-10:])
        print(df_minute_data.index[-10:])
        print(df_merged_data.index[-10:])
    return
...
def print_csv(read_path, save_path = save_path, tt = tt, factor_number = factor_number, freq = freq):
    ...
    if len(w3) < 10:
        return
    w3['dt'] = w3['dt'].apply(lambda x: x.split(' ')[0] + ' ' +switch_time(x.split(' ')[-1].replace(':', '').replace('.', '')[:-3]) + '000')
    w3['dt'] = w3['dt'].apply(lambda x1: x1[:13] + ':' + x1[13:15] + ':' + x1[15:17] + '.' + x1[17:])
    w3_30 = w3.copy()
...
# start_date = '20160101'
# end_date = '20210331'
a, b, c = check_update_date(20241001, 20250314)
...
create_h5_dict = {'IC': [], 'IF': [], 'IH': [], 'IM': [],'T': []}
print(create_h5_dict)



def calc_and_store(path):
    try:
        #print_csv(path)
        ...
        return [variety, single_save_path]
    except Exception as e:
        print('error_{}'.format(path), e)
with Pool(24) as pool:
    dic_temp = pool.map(calc_and_store, updatelist)

for item in dic_temp:
    try:
        create_h5_dict[item[0]].append(item[1])
    except:
        pass


h5_root_path = '/dfs/group/800466/warehouse/prod/MD/MarketData/MD/CHINA_FUTURES/MINUTE_shift_%s/'%str(ss)
try:
    os.makedirs(h5_root_path)
except:
    pass
...
for k,v in create_h5_dict.items():
    ...
    #if 'IM' in k.upper():
    #    h5_path = h5_path.replace('/IM_', '/backup/IM_')
    nowdf = get_contract_df(v)
    #try:
    #    olddf = pd.read_hdf(h5_path)
    #    nowdf = drop_dup(pd.concat([olddf, nowdf])).sort_index()
    #    IO.pd_hdf5_writer(nowdf, h5_path, dataset = '{}_MINUTE'.format(k))
    #except:
    #    pass
