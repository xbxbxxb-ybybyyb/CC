...
contract_list = ['IC.CFE','IF.CFE', 'IM.CFE']
...
for date1 in [20240901, 20240902, 20240903, 20240904, 20240926, 20240927]:
    date = str(date1)
    for ticker in contract_list:
        ...
        if 'IC' in ticker:
            ...
            save_folder = next_tday + '_all_00'
        elif 'IF' in ticker:
            factor_folder = 'if_ever'
            save_folder = next_tday + '_all_if_00'
        elif 'IM' in ticker:
            factor_folder = 'im_unifac_ever'
            save_folder = next_tday + '_all_im_00'
        elif 'TREND' in ticker:
            factor_folder = 'TREND'
            save_folder = next_tday + '_trend_00'

        # gen factors
        factorlib_path = '/data/group/800466/warehouse/prod/alpha/CHINA_FUTURES/MINUTE/%s/minute_raw/'%factor_folder
        save_path = '/data/group/800466/warehouse/test/alpha/CHINA_FUTURES/trade_files/%s/historyFactor' %save_folder
        ...
        save_path = '/data/group/800466/warehouse/test/alpha/CHINA_FUTURES/trade_files/%s/historyNormFactor' %save_folder
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        # 生成文件
