...
#    target_trigger_time = pd.Timedelta(hours=14, minutes=47)
    def final_funcs_helper():
        logger1 = add_file_logger('Diamond_2_0', level=logging.DEBUG, file_name=os.path.join(log_path, datetime.datetime.now().strftime('%Y%m%d'),\
                                  'Dia_1429_' + datetime.datetime.now().strftime('%H%M%S') + '.log'))
...
#        raise Exception('edb data is not exists!')
    target_trigger_time_mid = pd.Timedelta(hours=trade_mid_time.hour, minutes=trade_mid_time.minute + 5)
    ...
    scheduler(mid_funcs_helper, target_trigger_time_mid, delay=2000)
