...
#                pickle.dump(save_dict, temp_input, protocol=pickle.HIGHEST_PROTOCOL)
#            return
#        save_pickle(hot_data, '/data/user/017024/')
...
def generate_para_excel(trade_date, trading_plan):
    ...
    plan['买卖方向'] = [afternoon_trade_direction[i[:2]] for i in plan['合约代码']]
    ...
    next_plan = plan.copy()
    next_plan['买卖方向'] = [morning_trade_direction[i[:2]] for i in plan['合约代码']]
...
def executor(trade_date=None, max_workers=12, mode = 'realtime', tag='factors'):
    ...
    trading_plan_2_3 = signal_dealer(factor_score['norm'], spot_close_dict, recent_contract, season_contract, volume_limit_dict, settlement_ratio_dict)
    print('-' * 60)
    print('Diamond 2.3 1429:')
    print(trading_plan_2_3[['Contract', 'Contract_Num', 'Signal', 'Settlement_Ratio']])
    lm.sendMessage('Diamond 2.3 1429:\n' + str(trading_plan_2_3[['Contract', 'Contract_Num', 'Signal', 'Settlement_Ratio']]))
    print('-' * 60)
    # dump factor
    ...
    generate_para_excel(trade_date, trading_plan_2_3.reset_index())
    # excel_transmission(trade_date)
    trading_plan_2_3.to_csv(os.path.join(trading_plan_savepath, '%s_%s_2_3.csv' % (trade_date, trade_stop_time.strftime('%H%M'))))
    # dump factor
...
#    alla_kline_part_a = pd.read_hdf(os.path.join(trade_root, 'hot', trade_date,
#                   'alla_kline_1min_%s_%s.h5' % (trade_start_time.strftime('%H%M%S'), trade_mid_time.strftime('%H%M%S'))))
#    alla_kline_part_b = pd.read_hdf(os.path.join(trade_root, 'hot', trade_date,
#                   'alla_kline_1min_%s_%s.h5' % (trade_mid_time.strftime('%H%M%S'), trade_stop_time.strftime('%H%M%S'))))
#    final_kline = pd.concat([alla_kline_part_a, alla_kline_part_b], axis=0, sort=False)
#    final_kline.to_hdf(os.path.join(trade_root, 'hot', trade_date,
#                       'alla_kline_1min_%s_%s.h5' % (trade_start_time.strftime('%H%M%S'), trade_stop_time.strftime('%H%M%S'))),
#                       key='alla_kline_1min', mode='w')
