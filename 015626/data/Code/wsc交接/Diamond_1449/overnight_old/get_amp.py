...
def get_amp_singleday(t0):
    ...
    ic_s = ma.getMDSecurityTickDataFrame(ic_rm, f"{t0_str}093000", f"{t0_str}094000", 1)['Buy1Price'].mean()
    if_b = ma.getMDSecurityTickDataFrame(if_rm, f"{t1_str}145000", f"{t1_str}150000", 1)['Sell1Price'].mean()
    if_s = ma.getMDSecurityTickDataFrame(if_rm, f"{t0_str}093000", f"{t0_str}094000", 1)['Buy1Price'].mean()
    ih_b = ma.getMDSecurityTickDataFrame(ih_rm, f"{t1_str}145000", f"{t1_str}150000", 1)['Sell1Price'].mean()
    ih_s = ma.getMDSecurityTickDataFrame(ih_rm, f"{t0_str}093000", f"{t0_str}094000", 1)['Buy1Price'].mean()
    ic_ret = ic_s / ic_b - 1
    if_ret = if_s / if_b - 1
    ih_ret = ih_s / ih_b - 1
    result = abs(ic_ret + if_ret + ih_ret) / 3
    print(ic_ret, if_ret, ih_ret)
...
def get_amp_last5d(t0=None):
    if t0 is None:
        t0 = pd.Timestamp.now()
    date_list = udt.get_trading_day_offset(t0, [-4, -3, -2, -1, 0])
    result = 0
