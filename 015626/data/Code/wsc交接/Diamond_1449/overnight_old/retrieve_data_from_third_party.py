...
def prepare_tickers(ref_date=None):
    ...
    return index_tickers + futures_tickers
...
def retrieve_alla_helper(start_time, stop_time, date=None, stock_list=None):
    ...
    if stock_list is None:
        stock_list = pd.read_pickle(os.path.join(hisdata_path, date_str, 'zz800_stock_list.pkl'))
    result_dict = concurrent_apply_func(retrieve_single_stock, stock_list, 24, date=date, start_time=start_time, stop_time=stop_time, void_log_flag=True)
...
def retrieve_misc_helper(start_time, stop_time, date=None):
    ...
    result_dict = concurrent_apply_func(retrieve_single_misc, ticker_list, 24, date=date, start_time=start_time, stop_time=stop_time, void_log_flag=True)
    result_df = pd.concat(result_dict.values()).sort_index()
