...
from utils_wsc.help_functions_wsc import calc_avg_bars
from utils_wsc.operators_wsc import ts_corr, shift_np
from sklearn.metrics import adjusted_mutual_info_score
from utils_wsc.SIF_Factor_Test21 import SIF_Factor_Test
...
def _calc_icir(y, y_pred, starttime_insample, endtime_insample, starttime_outsample, endtime_outsample, w):
    y_insample = y.loc[starttime_insample: endtime_insample]
    y_outsample = y.loc[starttime_outsample: endtime_outsample]
    y_pred_insample = y_pred.loc[starttime_insample: endtime_insample]
    y_pred_outsample = y_pred.loc[starttime_outsample: endtime_outsample]
    data_fullness_ratio = y_pred_insample.count() / y_insample.count()
    if not isinstance(data_fullness_ratio, float):
        data_fullness_ratio = min(data_fullness_ratio)
    if data_fullness_ratio < 0.9:
        icir_insample = np.nan
        icir_outsample = np.nan
    else:
        ic_insample = ts_corr(y_insample, y_pred_insample, 4800)
        icir_insample = ic_insample.mean() / ic_insample.std()
        ic_outsample = ts_corr(y_outsample, y_pred_outsample, 4800)
        icir_outsample = ic_outsample.mean() / ic_outsample.std()
    return icir_insample, icir_outsample


def _calc_rmse(y, y_pred, starttime_insample, endtime_insample, starttime_outsample, endtime_outsample, w):
    y_insample = y.loc[starttime_insample: endtime_insample]
    y_outsample = y.loc[starttime_outsample: endtime_outsample]
    y_pred_insample = y_pred.loc[starttime_insample: endtime_insample]
    y_pred_outsample = y_pred.loc[starttime_outsample: endtime_outsample]
    ...
    if data_fullness_ratio < 0.9:
        ...
        rmse_outsample = np.nan
    else:
        rmse_insample = np.sqrt((((y_insample - y_pred_insample) / y_insample) ** 2).mean())
        rmse_outsample = np.sqrt((((y_outsample - y_pred_outsample) / y_outsample) ** 2).mean())
    return rmse_insample, rmse_outsample


def _calc_ic_np(y, y_pred, starttime_insample, endtime_insample, starttime_outsample, endtime_outsample,
                data_drop_list, w):
    if data_drop_list is not None:
        starttime_insample -= len([i for i in data_drop_list if i < starttime_insample])
        endtime_insample -= len([i for i in data_drop_list if i < endtime_insample])
        starttime_outsample -= len([i for i in data_drop_list if i < starttime_outsample])
        endtime_outsample -= len([i for i in data_drop_list if i < endtime_outsample])
        y = np.delete(y, data_drop_list, axis=0)
        y_pred = np.delete(y_pred, data_drop_list, axis=0)
    y_insample = y[starttime_insample: endtime_insample]
    y_outsample = y[starttime_outsample: endtime_outsample]
    y_pred_insample = y_pred[starttime_insample: endtime_insample]
    y_pred_outsample = y_pred[starttime_outsample: endtime_outsample]
    data_nan_ratio = np.isnan(y_pred_insample).sum() / y_insample.shape[0]
    if data_nan_ratio < 0.1:
        if y.ndim == 2:
            ic_insample = np.full(y.shape[1], np.nan)
            ic_outsample = np.full(y.shape[1], np.nan)
            for i in range(y.shape[1]):
                ic_insample[i] = np.ma.corrcoef(np.ma.masked_invalid(y_insample[:, i]),
                                                np.ma.masked_invalid(y_pred_insample[:, i])).data[0, 1]
                ic_outsample[i] = np.ma.corrcoef(np.ma.masked_invalid(y_outsample[:, i]),
                                                 np.ma.masked_invalid(y_pred_outsample[:, i])).data[0, 1]
            return ic_insample, ic_outsample
        elif y.ndim == 1:
            ic_insample = np.ma.corrcoef(np.ma.masked_invalid(y_insample),
                                         np.ma.masked_invalid(y_pred_insample)).data[0, 1]
            ic_outsample = np.ma.corrcoef(np.ma.masked_invalid(y_outsample),
                                          np.ma.masked_invalid(y_pred_outsample)).data[0, 1]
            return np.array([ic_insample]), np.array([ic_outsample])
        else:
...
def _calc_ic_np_minus(y, y_pred, starttime_insample, endtime_insample, starttime_outsample, endtime_outsample,
                      data_drop_list, w):
    if data_drop_list is not None:
        starttime_insample -= len([i for i in data_drop_list if i < starttime_insample])
        endtime_insample -= len([i for i in data_drop_list if i < endtime_insample])
...
def _calc_icac_np(y, y_pred, starttime_insample, endtime_insample, starttime_outsample, endtime_outsample,
                  data_drop_list, w):
    if data_drop_list is not None:
        starttime_insample -= len([i for i in data_drop_list if i < starttime_insample])
        endtime_insample -= len([i for i in data_drop_list if i < endtime_insample])
    ...
    if data_nan_ratio < 0.1:
        if y.ndim == 2:
            icac_insample = np.full(y.shape[1], np.nan)
            icac_outsample = np.full(y.shape[1], np.nan)
            for i in range(y.shape[1]):
                temp_ic_insample = np.ma.corrcoef(np.ma.masked_invalid(y_insample[:, i]),
                                                  np.ma.masked_invalid(y_pred_insample[:, i])).data[0, 1]
                temp_ac_insample = np.ma.corrcoef(np.ma.masked_invalid(y_pred_insample[:, i]),
                                                  np.ma.masked_invalid(shift_np(y_pred_insample[:, i], 5))).data[0, 1]
                temp_ic_outsample = np.ma.corrcoef(np.ma.masked_invalid(y_outsample[:, i]),
                                                   np.ma.masked_invalid(y_pred_outsample[:, i])).data[0, 1]
                temp_ac_outsample = np.ma.corrcoef(np.ma.masked_invalid(y_pred_outsample[:, i]),
                                                   np.ma.masked_invalid(shift_np(y_pred_outsample[:, i], 5))).data[0, 1]
                icac_insample[i] = temp_ic_insample * temp_ac_insample
                icac_outsample[i] = temp_ic_outsample * temp_ac_outsample
            return icac_insample, icac_outsample
        elif y.ndim == 1:
            ic_insample = np.ma.corrcoef(np.ma.masked_invalid(y_insample),
                                         np.ma.masked_invalid(y_pred_insample)).data[0, 1]
            ac_insample = np.ma.corrcoef(np.ma.masked_invalid(y_pred_insample),
                                         np.ma.masked_invalid(shift_np(y_pred_insample, 5))).data[0, 1]
            ic_outsample = np.ma.corrcoef(np.ma.masked_invalid(y_outsample),
                                          np.ma.masked_invalid(y_pred_outsample)).data[0, 1]
            ac_outsample = np.ma.corrcoef(np.ma.masked_invalid(y_pred_outsample),
                                          np.ma.masked_invalid(shift_np(y_pred_outsample, 5))).data[0, 1]
            icac_insample = ic_insample * ac_insample
            icac_outsample = ic_outsample * ac_outsample
            return np.array([icac_insample]), np.array([icac_outsample])
        else:
            raise NotImplementedError
    else:
        if y.ndim == 2:
...
def _calc_icac_np_minus(y, y_pred, starttime_insample, endtime_insample, starttime_outsample, endtime_outsample,
                        data_drop_list, w):
    if data_drop_list is not None:
        starttime_insample -= len([i for i in data_drop_list if i < starttime_insample])
        endtime_insample -= len([i for i in data_drop_list if i < endtime_insample])
...
def _ht_adj(holding_time, time_cut=17.5):
    if (holding_time >= time_cut) & (holding_time < time_cut * 2):
        holding_time = time_cut
    elif (holding_time >= time_cut * 2) & (holding_time < time_cut * 3):
        holding_time = time_cut * 3 - holding_time
    elif holding_time >= time_cut * 3:
        holding_time = 0
    return holding_time


def _calc_icht_np(y, y_pred, starttime_insample, endtime_insample, starttime_outsample, endtime_outsample,
                  data_drop_list, w):
    if data_drop_list is not None:
        starttime_insample -= len([i for i in data_drop_list if i < starttime_insample])
        endtime_insample -= len([i for i in data_drop_list if i < endtime_insample])
    ...
    if data_nan_ratio < 0.1:
        if y.ndim == 2:
            icht_insample = np.full(y.shape[1], np.nan)
            icht_outsample = np.full(y.shape[1], np.nan)
            for i in range(y.shape[1]):
                temp_ht_insample = calc_avg_bars(y_pred_insample[:, i])
                temp_ht_outsample = calc_avg_bars(y_pred_outsample[:, i])
                temp_ht_adj_insample = _ht_adj(temp_ht_insample)
                temp_ht_adj_outsample = _ht_adj(temp_ht_outsample)
                temp_ic_insample = np.ma.corrcoef(np.ma.masked_invalid(y_insample[:, i]),
                                                  np.ma.masked_invalid(y_pred_insample[:, i])).data[0, 1]
                temp_ic_outsample = np.ma.corrcoef(np.ma.masked_invalid(y_outsample[:, i]),
                                                   np.ma.masked_invalid(y_pred_outsample[:, i])).data[0, 1]
                icht_insample[i] = temp_ic_insample * temp_ht_adj_insample
                icht_outsample[i] = temp_ic_outsample * temp_ht_adj_outsample
            return icht_insample, icht_outsample
        elif y.ndim == 1:
            temp_ht_insample = calc_avg_bars(y_pred_insample)
            temp_ht_outsample = calc_avg_bars(y_pred_outsample)
            temp_ht_adj_insample = _ht_adj(temp_ht_insample)
            temp_ht_adj_outsample = _ht_adj(temp_ht_outsample)
            ic_insample = np.ma.corrcoef(np.ma.masked_invalid(y_insample),
                                         np.ma.masked_invalid(y_pred_insample)).data[0, 1]
            ic_outsample = np.ma.corrcoef(np.ma.masked_invalid(y_outsample),
                                          np.ma.masked_invalid(y_pred_outsample)).data[0, 1]
            icht_insample = ic_insample * temp_ht_adj_insample
            icht_outsample = ic_outsample * temp_ht_adj_outsample
            return np.array([icht_insample]), np.array([icht_outsample])
        else:
            raise NotImplementedError
    else:
        if y.ndim == 2:
...
def _calc_icht_np_minus(y, y_pred, starttime_insample, endtime_insample, starttime_outsample, endtime_outsample,
                        data_drop_list, w):
    if data_drop_list is not None:
        starttime_insample -= len([i for i in data_drop_list if i < starttime_insample])
        endtime_insample -= len([i for i in data_drop_list if i < endtime_insample])
...
icir = _Fitness(function=_calc_icir,
                greater_is_better=True)
rmse = _Fitness(function=_calc_rmse,
                greater_is_better=False)
ic_np = _Fitness(function=_calc_ic_np,
                 greater_is_better=True)
ic_np_minus = _Fitness(function=_calc_ic_np_minus,
                       greater_is_better=True)
icac_np = _Fitness(function=_calc_icac_np,
                   greater_is_better=True)
icac_np_minus = _Fitness(function=_calc_icac_np_minus,
                         greater_is_better=True)
icht_np = _Fitness(function=_calc_icht_np,
                   greater_is_better=True)
icht_np_minus = _Fitness(function=_calc_icht_np_minus,
                         greater_is_better=True)
...
_fitness_map = {'pearson': weighted_pearson,
                ...
                'rmse': rmse,
                'ic_np': ic_np,
                'ic_np_minus': ic_np_minus,
                'icac_np': icac_np,
                'icac_np_minus': icac_np_minus,
                'icht_np': icht_np,
                'icht_np_minus': icht_np_minus}
