...
class BaseSymbolic(BaseEstimator, metaclass=ABCMeta):
    ...
    def fit(self, X, y, starttime_insample, endtime_insample, starttime_outsample, endtime_outsample,
            data_drop_list=None, sample_weight=None):
        ...
        if not ((isinstance(self.const_params_range, tuple) and
                 len(self.const_params_range) == 2) or self.const_params_range is None):
            raise ValueError('const_params_range should be a tuple with length two, '
                             'or None.')
