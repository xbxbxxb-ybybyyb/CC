...
def read_data(trading_days=None, columns=None, universe=None, mkttype=MktType.CHINA, dtype=DType.STOCK,
              ftype=FType.MD, dfreq=DFreq.DAILY, dsource=DSource.WIND, dtable = None, alt=None, h5root='/data/group/800080/warehouse/prod', max_workers=None):
    '''
    --- DESCRIPTION ---
    ...
    if pd_retrieved.shape[0]<1:
        print('No data be read!' + pd_retrieved.columns[0] + '_'+ trading_days[-1].strftime('%Y%m%d'))
        raise('check data')
...
def get_available_cols(mkttype=MktType.CHINA, dtype=DType.STOCK, ftype=FType.MD, dfreq=DFreq.DAILY, dsource=DSource.WIND, dtable=None, alt=None, h5root='Z:/warehouse/prod'):
    abs_h5_path = path_assembler(mkttype=mkttype, dtype=dtype, ftype=ftype, dfreq=dfreq, dsource=dsource, dtable=dtable, alt=alt, h5root=h5root)
    with pd.HDFStore(abs_h5_path, 'r') as h5_store:
        h5_root_keys = get_root_keys(h5_store)
        if len(h5_root_keys) == 1:
