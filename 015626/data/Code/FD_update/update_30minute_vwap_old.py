...
def twap_retrieve(date):
    ...
    df.reset_index(inplace=True)
    df.set_index('minute',inplace=True)
    a = df.groupby('Ticker')['close'].rolling(30).mean()
    df = pd.DataFrame(a)
    df.reset_index('Ticker',inplace=True)
...
def rolling(sdate=None, edate=None):
    ...
    h5_path = r'/data/group/800080/warehouse/prod/VD/CHINA_STOCK/DAILY/WIND/VD_CHINA_STOCK_DAILY_WIND.h5'

    fail_list = update_wind_daily_h5(cdate_list,factor_list,csv_path,h5_path)
    
    if len(fail_list) == 0:
        print('*' * 50)
        print('update 30minute vwap finish!')
    else:
        print('follows factors are wrong!!!')
        print(fail_list)
...
# rolling(20190322,20190325)


# just like main function
def update_30minute_vwap():
