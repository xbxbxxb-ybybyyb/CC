...
def check_data(path,mode):
    ...
    # mode = 'overwrite'
    try:
        df_test = IO.read_data([20000101,20180701],alt=test_path)
    ...
    df_prod.set_index(['dt','OBJECT_ID'],inplace=True)
    
    df_test.reset_index('dt',inplace=True)
    df_test.set_index(['dt','OBJECT_ID'],inplace=True)
...
def main():
    ...
    for path in path_list:
        if path not in ['AShareMarginTradeSum']:
            continue
