...
def second_job(sdate = None, edate = None):
    sdate,edate,cdate_list = check_update_date(20200601,20200715)
    ...
    table_dict = { 'QUARTERLY': [],'DAILY': daily_C_list + daily_list + new_daily_list}
#    table_dict = {'DAILY': ['WIND_AShareInsideHolder'],'QUARTERLY': []}


    for table_type in table_dict:
        for table_name in table_dict[table_type]:
            if table_name in ['WIND_ChinaMutualFundBondPortfolio','WIND_AShareEXRightDividendRecord','WIND_ChinaMutualFundStockPortfolio','WIND_AShareFreeFloatCalendar',
                'WIND_ChinaMutualFundIndPortfolio','WIND_ChinaMutualFundAssetPortfolio','WIND_ASharePledgeproportion']:
                continue
            if table_name in ['WIND_AShareIndustriesClassCITICS', 'WIND_AShareDescription', 'WIND_AShareIndustriesCode','WIND_AShareST',
                        'WIND_AShareCapitalization', 'WIND_AShareFreeFloat', 'WIND_AShareIPO', 'WIND_AShareAgency',
                         'WIND_AShareCOCapitaloperation', 'WIND_ASharePledgeproportion', 'WIND_AshareStockRepo', 'WIND_AShareCorporateFinance',
                         'WIND_AShareIssueCommAudit', 'WIND_AShareEquityDivision', 'WIND_AShareStaff',
                         'WIND_IPOCompRFA', 'WIND_IECMemberList', 'WIND_AShareLeadUnderwriter',
                         ...
                         'WIND_AShareMainandnoteitems','WIND_AIndexMembers','WIND_AShareConseption'] :
                continue
            aaa = IO.read_data([20200601,20200715], alt = '/data/group/800080/warehouse/prod/DATABASE/WIND/'+table_name[5:] + '/'+ table_name[5:] +'.h5')
            alist = aaa.index.get_level_values(0).unique().to_list()
            aintlist = [int(str(x)[:10].replace('-','')) for x in alist]
            timelist = list(set(cdate_list) - set(aintlist))
            print(table_name, timelist)
            timelist.sort()
            for t in timelist:
                instantiate(table_name, t, t, base_path = '/data/group/800080/warehouse/prod/', operation = 'append',table_dict=table_dict,update_h5_flag=True)
