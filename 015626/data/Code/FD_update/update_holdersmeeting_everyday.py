...
def holdersmeeting_job(sdate = None, edate = None):
    # sdate,edate,cdate_list = check_update_date(sdate = sdate, edate = edate)
                 
    table_dict = {'DAILY': ['WIND_AShareholdersmeeting'], 'QUARTERLY': []}
...
sdate = edate // 10000 * 10000 + 101

flag_root = '/data/user/015626/FLAG/' + str(edate) + '/'
if not os.path.exists(flag_root):
    os.makedirs(flag_root)
flag_path_start = flag_root + str(edate) + '_' + 'holdersmeeting_job.start'

with open(flag_path_start,'w') as file:
    pass 

holdersmeeting_job(sdate, edate)

flag_path_success = flag_root + str(edate) + '_' + 'holdersmeeting_job.success'
