...
def retriver(cdate_list, factor_list):
    for date in cdate_list:
        date = str(date)
        while not os.path.exists("/data/group/800080/warehouse/prod/LOCAL_DATA/CSV/stock_universe/SH50/" + date + '.csv'):
            logger.info('file is not exist, download from ftp')
            for factor in factor_list:
                try:
                    ftp.downloadFile("015626/index_weight/" + factor + '/' + date + '.csv', 
                    "/data/group/800080/warehouse/prod/LOCAL_DATA/CSV/stock_universe/" + factor + '/' + date + '.csv')
