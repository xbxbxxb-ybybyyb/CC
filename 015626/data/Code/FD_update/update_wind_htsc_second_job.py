...
class WIND_DATABASE:
    def __init__(self, table_name, sdate, edate, 
        ...
        if self.dfreq=='DAILY':
            ...
            raise Exception
        self.over_write_list = ['WIND_CCBondConversion','WIND_AShareIndustriesClassCITICS', 'WIND_AShareDescription', 'WIND_AShareIndustriesCode','WIND_AShareST',
                        'WIND_AShareCapitalization', 'WIND_AShareFreeFloat', 'WIND_AShareIPO', 'WIND_AShareAgency',
                         ...
                         'WIND_AShareMainandnoteitems','WIND_AIndexMembers','WIND_AShareConseption','WIND_CbondFSubjectcvf']  
    ...
    def csv2h5(self, update_list, min_size=0):
        ...
        """
        dat_fig_dict = {'WIND_CCBondConversion':{'dt':'date','Ticker':'S_INFO_WINDCODE'},
        
                        'WIND_CBondEODPrices':{'dt':'TRADE_DT','Ticker':'S_INFO_WINDCODE'},
        
                        'WIND_CBondRating':{'dt':'ANN_DT','Ticker':'S_INFO_WINDCODE'},
                        'WIND_CBondIssuerRating':{'dt':'ANN_DT','Ticker':'S_INFO_COMPCODE'},
        
                        'WIND_CCBondValuation':{'dt':'TRADE_DT','Ticker':'S_INFO_WINDCODE'},
                        'WIND_CCommodityFuturesPositions':{'dt':'TRADE_DT','Ticker':'S_INFO_WINDCODE'},
                        'WIND_CbondFSubjectcvf':{'dt':'ANN_DT','Ticker':'S_INFO_WINDCODE'},
                        'WIND_CbondFCTD':{'dt':'TRADE_DT','Ticker':'S_INFO_WINDCODE'},
                        'WIND_CBondFuturesEODPrices':{'dt':'TRADE_DT','Ticker':'S_INFO_WINDCODE'},
                        'WIND_CBondFValuation':{'dt':'TRADE_DT','Ticker':'S_INFO_WINDCODE'},
                        'WIND_CGBbenchmark':{'dt':'TRADE_DT','Ticker':'S_INFO_WINDCODE'},
                        
                        'WIND_ChinaOptionDescription':{'dt':'S_INFO_FTDATE','Ticker':'S_INFO_WINDCODE'},
                        'WIND_ChinaOptionValuation':{'dt':'TRADE_DT','Ticker':'S_INFO_WINDCODE'},
                        'WIND_ChinaOptionDailyStatistics':{'dt':'TRADE_DT','Ticker':'S_INFO_CODE'},
                        'WIND_ChinaOptionMemberStatistics':{'dt':'TRADE_DT','Ticker':'S_INFO_CODE'},
                        'WIND_ChinaOptionIndexEODPrices':{'dt':'TRADE_DT','Ticker':'S_INFO_WINDCODE'},
                        'WIND_ChinaOptionEODPrices':{'dt':'TRADE_DT','Ticker':'S_INFO_WINDCODE'},
                        'WIND_AShareholdersmeeting':{'dt':'ANN_DT','Ticker':'S_INFO_WINDCODE'},
            ...
            for fname in update_list:
                ...
                if len(dat1)<=min_size:
                    ...
                    if self.table_name == 'WIND_AShareProfitNotice':
                        ...
                        h5_store.append(self.table_name,dat,data_columns=True,  min_itemsize={'Ticker':10,'OPDATE':30})
                        ...
                        h5_store.append(self.table_name,dat,data_columns=True,min_itemsize={'S_INFO_BONDWINDCODE':15,'Ticker':20})
                        ...
                        dat.drop(['S_HOLDER_MEMO','OBJECT_ID','REPORT_PERIOD','S_HOLDER_NAT','S_INFO_COMPCODE'], axis=1, inplace=True)
                        ...
                        h5_store.append(self.table_name,dat,data_columns=True, min_itemsize={'S_HOLDER_NAME': 300, 
                                                                                            'S_HOLDER_ANAME':200,
                        ...
                        dat = dat.drop(['LATEST_OWN_QTY','MEETEVENT_ID','PLACEMENT_CURRENCY','PLACEMENT_FINANCING','PLACEMENT_NON_CURRENCY','S_INFO_COMPCODE','S_SHARE_LSTTYPENAME'], axis = 1)
                        ...
                        h5_store.append(self.table_name,dat,data_columns=True,min_itemsize={'STATEMENT_TYPE': 30})                                      
                    ...
                    elif self.table_name == 'WIND_ChinaOptionMemberStatistics':
                        h5_store.append(self.table_name,dat, data_columns=True,min_itemsize={'S_INFO_MEMBERNAME':40,'S_INFO_NAME':50})
                    elif self.table_name == 'WIND_ChinaOptionEODPrices':
                        h5_store.append(self.table_name,dat, data_columns=True,min_itemsize={'Ticker':30})
                    elif self.table_name == 'WIND_ChinaOptionIndexEODPrices':
                        h5_store.append(self.table_name,dat, data_columns=True,min_itemsize={'S_INFO_NAME':30,'SEC_ID':20})
                    elif self.table_name == 'WIND_ChinaOptionValuation':
                        h5_store.append(self.table_name,dat, data_columns=True,min_itemsize={'Ticker':20})
                    elif self.table_name == 'WIND_ChinaOptionDailyStatistics':
                        h5_store.append(self.table_name,dat, data_columns=True,min_itemsize={'S_INFO_NAME':50})
                    elif self.table_name == 'WIND_ChinaMutualFundIndPortfolio':
                        dat.drop(['OBJECT_ID'],axis=1,inplace=True)
                        dat['S_INFO_CSRCINDUSCODE'] = dat['S_INFO_CSRCINDUSCODE'].astype(str)
                        h5_store.append(self.table_name,dat, data_columns=True,min_itemsize={'Ticker':20,'S_INFO_CSRCINDUSCODE':10})
                    elif self.table_name == 'WIND_ChinaOptionDescription':
#                        dat.fillna('Nan', inplace = True)
#                        dat.drop(['ADJ_SIGN'],axis=1,inplace=True)
                        for k in dat.columns:
                            if k in ['S_INFO_STRIKEPRICE','S_INFO_LPRICE']:
                                continue
#                            dat[k].fillna('Nan', inplace = True)
                            dat[k] = dat[k].astype(str)
                        
                        h5_store.append(self.table_name,dat, data_columns=True,min_itemsize={'S_INFO_NAME':80,'Ticker':30,
                                                                                             'S_INFO_MATURITYDATE':15,
                                                                                            'S_INFO_EXERCISINGEND':15,
                                                                                            'S_INFO_LDDATE':15,
                                                                                            'S_INFO_CODE':25,
                                                                                            'S_INFO_EXNAME':100,
                                                                                            'S_INFO_SCCODE':30,
                    ...
                    elif self.table_name == 'WIND_CbondFCTD':
                        dat['SZ_CTD_WINDCODE'] = dat['SZ_CTD_WINDCODE'].astype(str)
                        dat['SH_CTD_WINDCODE'] = dat['SH_CTD_WINDCODE'].astype(str)
                        dat['IB_CTD_WINDCODE'] = dat['IB_CTD_WINDCODE'].astype(str)
                        h5_store.append(self.table_name,dat, data_columns=True, min_itemsize={'Ticker':14,'CTD_WINDCODE':14,'IB_CTD_WINDCODE':14})
                    elif self.table_name == 'WIND_CBondFuturesEODPrices':
                        h5_store.append(self.table_name,dat, data_columns=True, min_itemsize={'Ticker':14})
                    elif self.table_name == 'WIND_CBondFValuation':
                        h5_store.append(self.table_name,dat, data_columns=True, min_itemsize={'Ticker':14, 'DLS_WINDCODE':14})
                    elif self.table_name == 'WIND_CGBbenchmark':
                        h5_store.append(self.table_name,dat, data_columns=True, min_itemsize={'Ticker':14})
                    elif self.table_name == 'WIND_CCommodityFuturesPositions':
                        h5_store.append(self.table_name,dat, data_columns=True, min_itemsize={'FS_INFO_MEMBERNAME':50, 'SEC_ID':20})
                    elif self.table_name in ['WIND_CBondRating']:
                        dat = dat.drop(['B_INFO_CREDITRATINGEXPLAIN'], axis = 1)
                        dat['B_INFO_PRECREDITRATING'] = dat['B_INFO_PRECREDITRATING'].astype('str')
                        h5_store.append(self.table_name,dat,data_columns=True, min_itemsize={'B_INFO_PRECREDITRATING':10,'B_INFO_CREDITRATING':10,'Ticker':30})
                    elif self.table_name in ['WIND_CBondIssuerRating']:
                        dat = dat.drop(['B_INFO_CREDITRATINGEXPLAIN'], axis = 1)
                        dat['B_INFO_PRECREDITRATING'] = dat['B_INFO_PRECREDITRATING'].astype('str')
                        h5_store.append(self.table_name,dat,data_columns=True, min_itemsize={'B_INFO_PRECREDITRATING':10,'B_INFO_CREDITRATING':10,'Ticker':30,'S_INFO_COMPNAME':160})
    
                    elif self.table_name == 'WIND_CBondEODPrices':
                        h5_store.append(self.table_name,dat, data_columns=True, min_itemsize={'Ticker':20})
                    else:
                        h5_store.append(self.table_name,dat,data_columns=True)
    ...
    def get_df(self, date):
        ...
        if self.table_name in ['WIND_AShareFinancialIndicator', 'WIND_AShareProfitExpress', 'WIND_AShareTTMHis',
            ...
            df = s.get_factor_value(self.table_name, S_ProfitNotice_Period=[str(date)])
        ...
        elif self.table_name in ['WIND_CBondEODPrices','WIND_CCBondValuation','WIND_CCommodityFuturesPositions','WIND_ChinaOptionEODPrices', 'WIND_ChinaOptionIndexEODPrices', 'WIND_ChinaOptionValuation',
                            'WIND_ChinaOptionDailyStatistics', 'WIND_ChinaOptionMemberStatistics',
                            'WIND_CIndexFuturesPositions','WIND_CBondFuturesPositions','WIND_AShareEODPrices', 'WIND_AShareEODDerivativeIndicator','WIND_SHSCDailyStatistics',
                            ...
                            'WIND_AIndexIndustriesEODCITICS','WIND_AIndexValuation','WIND_SIndexPerformance','WIND_AIndexWindIndustriesEOD',
                                'WIND_CbondFCTD','WIND_CBondFuturesEODPrices','WIND_CBondFValuation','WIND_CGBbenchmark']:
        ...
        elif self.table_name in ['WIND_CBondRating','WIND_CBondIssuerRating','WIND_CbondFSubjectcvf','WIND_AShareholdersmeeting','WIND_CMFOtherPortfolio',  'WIND_AShareHolderNumber', 'WIND_AShareIllegality',
        ...
        elif self.table_name in ['WIND_ChinaOptionDescription']:
            df = s.get_factor_value(self.table_name, S_INFO_FTDATE=[str(date)])
        else:
            ...
            if self.table_name in self.over_write_list:
                try:
                    ...
                    if self.table_name == 'WIND_AIndexMembers':
                        ...
                        df = s.get_factor_value(self.table_name, S_ENDDATE=['>=20200701']) \
                        .append(s.get_factor_value(self.table_name, S_ENDDATE=['>=20191001','<20200701'])) \
                        ...
                        _dlist = [x.strftime('%Y%m%d') for x in pd.date_range('20160901', dt.datetime.now().date().strftime('%Y%m%d'), freq='20D')]
                        df = s.get_factor_value(self.table_name, ANN_DT=['>=%s' % _dlist[-1]]) 
                        for i in range(len(_dlist)-1,0,-1):
                            df = df.append(s.get_factor_value(self.table_name, ANN_DT=['>=%s' % _dlist[i-1],'<%s' % _dlist[i]]))
                        df = df.append(s.get_factor_value(self.table_name, ANN_DT=['<%s' % _dlist[0]]))
                    elif self.table_name == 'WIND_htzqedbdzzbs':
                        ...
                        df = s.get_factor_value(self.table_name, OPDATE=['>=20200701']) \
                        .append(s.get_factor_value(self.table_name, OPDATE=['>=20200415','<20200701'])) \
                        ...
                        .append(s.get_factor_value(self.table_name, OPDATE=['>=20190101','<20190401'])) \
                        .append(s.get_factor_value(self.table_name, OPDATE=['>=20180427','<20190101'])) \
...
def second_job(sdate = None, edate = None):
    ...
    daily_C_list = ['WIND_CCBondConversion','WIND_CBondEODPrices','WIND_CCBondValuation','WIND_CBondRating','WIND_CBondIssuerRating','WIND_CCommodityFuturesPositions','WIND_CIndexFuturesPositions','WIND_CBondFuturesPositions','WIND_CbondFSubjectcvf','WIND_CbondFCTD',
                    'WIND_CBondFuturesEODPrices','WIND_CBondFValuation','WIND_CGBbenchmark']
    ...
    daily_list =  ['WIND_AShareAnnInf','WIND_ASharePledgeproportion', 'WIND_AShareFreeFloatCalendar',
                ...
                #'WIND_ASarePlanTrade',这个表暂未更新，出了点莫名其妙的问题，没时间处理
    new_daily_list = ['WIND_AShareMarginTradeSum','WIND_AShareCapitalization','WIND_AShareFreeFloat', 'WIND_AShareIPO', 'WIND_AShareAgency',
       ...
       'WIND_htzqedbdzzbs','WIND_AIndexIndustriesEODCITICS','WIND_SIndexPerformance','WIND_BOIndexWeightsWIND',
                ...
                'WIND_AShareIllegality','WIND_AShareMajorEvent','WIND_AShareInsideHolder','WIND_SHSCDailyStatistics']
    ...
    ##WIND_ChinaOptionDescription
    new_daily_option_list = ['WIND_ChinaOptionEODPrices','WIND_ChinaOptionIndexEODPrices','WIND_ChinaOptionMemberStatistics', 'WIND_ChinaOptionValuation','WIND_ChinaOptionDailyStatistics',
                             'WIND_ChinaOptionDescription']
    ...
    table_dict = { 'QUARTERLY': qtr_list,'DAILY': daily_C_list + daily_list + new_daily_list + new_daily_option_list}
...
def fix_job(sdate=None,edate=None):
    sdate,edate,cdate_list = check_update_date(sdate = sdate, edate = edate)
    #
    first_daily_list = ['WIND_ChinaOptionDescription'] 
    
    table_dict = { 'DAILY':first_daily_list ,'QUARTERLY': []}
    ...
    logger.info('===============fix job upload finish======================')
