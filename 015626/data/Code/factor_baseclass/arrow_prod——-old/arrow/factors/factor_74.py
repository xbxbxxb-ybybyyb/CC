...
class factor_74(FactorGenerator):
    def __init__(self, *args, **kwargs):
        data_mode = 't'
        required_columns = ['transaction', 'order', 'order_raw']
        super(factor_74, self).__init__(*args, data_mode = data_mode, required_columns=required_columns, **kwargs)
    ...
    def on_bar(self, df):
        ...
        for stk in df['universe']:
            ...
            if stk[0] == '6':
                ...
                factor[stk] = -(data_o_r.OrderQty * data_o_r.OrderPrice)[(data_o_r.OrderType == 10) & (data_o_r.OrderBSFlag == 2)].sum() / (data_o.OrderQty * data_o.OrderPrice)[data_o.OrderBSFlag == 2].sum()
            else:
                ...
                factor[stk] = -(data_t.TradeQty * data_t.OrderPrice)[(data_t.TradeType == 1) & (data_t.TradeBSFlag == 2)].sum() / (data_o.OrderQty * data_o.OrderPrice)[data_o.OrderBSFlag == 2].sum()
