...
sys.path.insert(4, '/data/user/015626/data/Code/git_space/commodities_factors_2/factors_new_frame_so/')
...
class TorusProduction(object):
    ...
    def calculate_factor_model(self):
        ...
        for now_time in self.data_gentime['1']:
                    ...
                    while True:
                        ...
                        if os.path.exists(now_time_freq_path):
                            ...
                            for fac_name in self.factor_freq_dict[freq]:
                                ...
                                try:
                                    raw = self.factor_all[full_fac_name].calculate(_data)
            ...
            all_raw_factor_dict[_md_time] = copy.copy(now_raw_factor)
            all_norm_factor_dict[_md_time] = copy.copy(now_norm_factor)
        ...
        pd.DataFrame(factor_time_dict).T.to_csv(os.path.join(self.result_save_path, f'{self.date}_{self.ticker}_time_using_factor.csv'))
        pd.DataFrame(model_time_dict).T.to_csv(os.path.join(self.result_save_path, f'{self.date}_{self.ticker}_time_using_model.csv'))
...
if __name__ == '__main__':
    tp = TorusProduction('/data/user/015626/data/Code/torus_production/para/20250801/AU.SHF.json')
    tp.run()