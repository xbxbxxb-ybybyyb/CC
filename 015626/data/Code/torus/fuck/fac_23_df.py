...
class fac_23_df(FutureFactor):
    ...
    def calculate(self, data):
        ...
        num = cross_hub_num_array(c, 10) + 1.0
        c_diff = c[1:] - c[:-1]
        vol = nanstd_np(c_diff[-5:], ddof=1)
        vwtc = ho * cl / replace_zero(hl) / replace_zero(vol) / np.sqrt(num)
        self.vwtc_list.append(vwtc)

        sig1 = ema_1(np.array(self.vwtc_list[-(ddd * 3):]), ddd * 3, 1 / (ddd + 1))
        sig2 = ema_1(np.array(self.vwtc_list[-(bbb * 3):]), bbb * 3, 1 / (bbb + 1))
        sig = sig1 + 2 * sig2
        return sig
