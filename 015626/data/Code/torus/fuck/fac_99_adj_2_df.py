...
class fac_99_adj_2_df(FutureFactor):
    ...
    def __init__(self, ticker, freq):
        ...
        self.days_past = int(int(np.ceil(601 / self.bars_dict[self.ticker])) * freq)
        self.required_columns = [  'close', 'BidAskSpreadMean',  'buy_small_volume', 'sell_small_volume', 'buy_gigantic_volume', 'sell_gigantic_volume']
    ...
    def calculate(self, data):
        ...
        close_diff = data['buy_gigantic_volume'][-601:] + data['sell_gigantic_volume'][-601:]
        locallow = nanargmax_new(close_diff[-int(nanmax_np([1, aaa * coef])):]) 

        fac = data['buy_small_volume'][-601:] - data['sell_small_volume'][-601:]
