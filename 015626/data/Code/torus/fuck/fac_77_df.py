...
class fac_77_df(FutureFactor): 
    ...
    def __init__(self, ticker, freq = 1):
        ...
        self.required_columns = ['idmax_secmain', 'volume_after_max_secmain','idmin_secmain', 'volume_after_min_secmain', 'volume_secmain']
    ...
    def calculate(self, data):
        ...
        fac_raw = -data['idmin_secmain'] * (data['volume_secmain'] - data['volume_after_min_secmain'])  + data['idmax_secmain'] * (data['volume_secmain'] - data['volume_after_max_secmain'])

        fac_raw = nanforward_fill(fac_raw)

        factor = irr_filter(fac_raw[-25*5:], 25)[-1]
