...
class fac_43_df(FutureFactor):
    ...
    def calculate(self, data):
        ...
        vol = cross_hub_num_array(dclose, 10) + 1
        sig1 = ulb_zscore_mean / r(vol)
        return sig1
    ...
    def pre_calculate(self, data):
        ...
        for i in range(60, -1, -1):
            if len(data['high']) < 2 + i:
                ...
                self.ulb_zscore_list.append(np.nan)
                continue
            if i == 0:
                dhigh = nanmax_np(data['high'][-2:])
                dlow = nanmin_np(data['low'][-2:])
            ...
            roll_win = 10
            ma_win = 4
            upper = SMA1(np.array(self.high_list[-roll_win * 6:]), roll_win, 1)
            lower = SMA1(np.array(self.low_list[-roll_win * 6:]), roll_win, 1)
            ulb = upper - lower
            self.ulb_list.append(ulb)
            ulb_avg = nanmean_np(np.array(self.ulb_list[-roll_win:]))
            ulb_std = nanstd_np(np.array(self.ulb_list[-roll_win:]), ddof = 1)
