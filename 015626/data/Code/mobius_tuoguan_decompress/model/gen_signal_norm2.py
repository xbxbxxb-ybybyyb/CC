...
def get_history_price(ticker, end_date, offset):
    ...
    data_list = []
    for date in dates:
        data = get_contract_from_xdb(contract_root + date + "/contract_univ", "IndexDF")
        if data is None or data.empty:
            continue
        contracts = data[data['Ticker'] == ticker + '.CF']['contract_00'].values

        if len(contracts) != 1:
            logger.error("date={} has no contract 00", date)
        else:
            col = 'twap' if ticker == "IM" else "close"

            data = read_indicator_from_xdb(col, date, ticker, contracts[0], offset)
            if data is None or data.empty:
                continue
...
def rank_index(date_list, variety, offset):
    ...
    ticker_signal_dict = {}
    if variety == 'IM' and offset == '0':
        ticker_signal_dict = {'IM': ['20241213_im_im_v1unifac_crn', '20241213_im_im_v1unifac']} 
    else:
    ...
    for ticker, signal_group_list in ticker_signal_dict.items():
        ...
        for sample_date in date_list:
            ...
            for signal_group in signal_group_list:
                ...
                index_list = get_index_list(minute_ret, minute_std, sample_date, num_samples, min_quantile, max_quantile)
...
if __name__ == '__main__':
    ...
    trading_list = ['20250314']
    # rank_index(trading_list, "IC", '55')
    # rank_index(trading_list, "IF", '55')
