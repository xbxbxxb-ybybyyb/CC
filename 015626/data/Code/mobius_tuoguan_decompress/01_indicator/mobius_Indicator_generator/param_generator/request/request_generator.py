...
from param_generator.request.templates import *
...
class RequestGenerator:
	...
	def __init__(self, channel_name=None):
		...
		if channel_name is None:
			...
			self.channel_name = channel_name
			self.request_template = globals().get(f"template_request_{channel_name[:8]}")
	...
	def generate(self, date):
		...
		if (self.channel_name is not None) and ("stock" in self.channel_name):
			channel = self.channel_name.split("_")[-1]
