...
class TaskRunner(object):
    ...
    def prepare_data(self, data_center, days_past, date):
        ...
        date_list = [str(x)[:10] for x in udt.get_trading_date_range('19900101', date)][-days_past - 1:]
        ...
        if data_center.get_data_type() == 'IndexStock':
            ...
            stock_list = get_constituent_stock_list(index_code, date_list[-1].replace('-',''))
            ...
            prepared_data = pd.concat(data_list, axis=1, join='inner')
    ...
    def run_factor_multi_day(self, factor, variety, data_center, start_date, end_date, ncore=20):
        ...
        date_list = self.__get_available_dates(start_date, end_date)
        # date_list = [str(x)[:10] for x in udt.get_trading_date_range(start_date, end_date)]
        pool = Pool(ncore)
        tasks = []
    ...
    def __get_available_dates(self, start_date, end_date):
        ...
        return [str(x)[:10].replace('-','') for x in udt.get_trading_date_range(start_date, end_date)]