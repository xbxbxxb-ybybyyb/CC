...
class DataCenter(object):
    ...
    def __init__(self, variety, data_type, instrument_type, data_dict, start_date, end_date, days_past):
        ...
        self.__tick_stock_path = '/data/user/017024/share/data/cfg_data/tick'
        self.__index_weights_path = '/data/group/800002/FutureTrader/test/MD/UNIVERSE/INDEXWEIGHT_CHINA_STOCK_DAILY_CSI.h5'
    ...
    def get_stock_tick_data(self):
        return self.__stock_tick_data
    ...
    def get_stock_tick_fields(self, para):

        symbol = para[0]
        fields = para[1]
        try:
            df_stock = IO.read_data([self.__start_date, self.__end_date+'235959'],columns=fields,
                                    alt='{}/{}.h5'.format(self.__tick_stock_path, symbol)).reset_index(level = 1, drop = True).add_suffix('_tick')
        except:
            df_stock = pd.DataFrame()
        return [symbol, df_stock]

    # 获取成分股tick数据
    def load_stock_tick_data(self):
        stock_list = get_all_stocklist_by_period(self.__variety, self.__start_date, self.__end_date)

        print('Loading index stock tick data for {}...'.format(self.__variety))



        assert self.__data_dict.get('Stock_tick') is not None
        fields = self.__data_dict.get('Stock_tick')

        paralist = [[s, fields] for s in stock_list]

        tasks = []
        with Pool(20) as pool:
            tasks = pool.map(self.get_stock_tick_fields, paralist)

        self.__stock_tick_data = {s[0]:s[1] for s in tasks}
        del(tasks)
    ...
    def load_data(self):
        ...
        # load stock tick data
        if self.__data_type == 'IndexStock_tick' and self.__data_dict.get('Stock_tick') is not None:
            self.load_stock_tick_data()
