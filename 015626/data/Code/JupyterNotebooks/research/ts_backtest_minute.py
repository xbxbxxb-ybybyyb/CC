...
class TS_BACK_TEST:
    ...
    def __init__(self, signal_df, ticker='IC.CFE', kind = 'swap', price_kind='vwap', pos_dict = {(0, 0.4): (0.0, 0.0),
                                                                                 (0.4, 0.5): (0.0, 0.2/3),
                                                                                 (0.5, 0.6): (0.2/3, 0.4/3),
                                                                                 (0.6, 0.7): (0.4/3, 0.6/3),
                                                                                 (0.7, 0.8): (0.6/3, 0.8/3),
                 ...
                 initial_cash=1000000, leverge = 1, c_rate=0.0004, slippage=0.006, minimum_trading_unit = 0.001, price_precision=None,
                 std_filter_threshold = -100, minute_after_stop_loss = 1, max_hold_time = None, filter_series = None, start_date = 20210101, end_date = 20230630,
        ...
        if self.signal_df.shape[1] == 1:
            _data = pd.read_parquet(f'/dfs/user/015626/research/data/minute/{kind}_per_kind/{ticker}.parquet').rename(columns = {'volume':'volume_deal'})
            self.signal_df = self.signal_df.join(_data, how = 'left')
            self.signal_df['std_signal'] = 0

        # self.signal_df = self.signal_df.loc[pd.to_datetime(str(start_date)):pd.to_datetime(str(end_date))].sort_index()
        ...
        for x in self.signal_df.volume_deal.tolist()[-100:]:
            x = str(round(x, 9))
            if '.' in x:
                if len(x.split('.')[-1]) > minimum_trading_unit:
                    minimum_trading_unit = len(x.split('.')[-1])
        minimum_trading_unit = 1 / 10**(minimum_trading_unit)
...
#         self.para_pos_dict = self.get_target_pos_dict(entry_para, exit_para)
        ...
        global dt_idx, raw_idx, std_signal_idx, open_idx, close_idx ,low_idx ,vwap_idx ,twap_idx ,volume_idx ,amount_idx ,price_kind_idx, buy_vwap_idx, sell_vwap_idx, filter_socre_idx
        ...
        if price_kind != 'buy_sell':
            price_kind_idx = columns_list.index(price_kind)
        else:
            price_kind_idx = 0
        buy_vwap_idx = columns_list.index('buy_vwap')
        sell_vwap_idx = columns_list.index('sell_vwap')
    ...
    def back_test(self):
        ...
        for i in tqdm(range(1, length)):
            ...
            # if now_hold_num == 0 and max_hold_time_flag == 1 and (target_pos_state == -1 or target_open_num_max == 0):
            #     max_hold_time_flag = 0
            # elif now_hold_num == 0 and max_hold_time_flag == -1 and (target_pos_state == 1 or target_open_num_max == 0):
            #     max_hold_time_flag = 0
            ...
            #  (now_hold_num_state * pre_target_pos_state == -1) or (now_hold_num_abs > target_open_num_max) or (now_hold_num_state * pre_filter_socre == -1) or (now_hold_num != 0 and max_hold_time_flag != 0):
            ...
            #     elif now_hold_num_state * pre_filter_socre == -1:
            #         close_contract_num = min(now_hold_num_abs, self.close_num_permin)
            ...
            elif (stop_loss_flag and (now_hold_num==0)) or ((now_hold_num_abs >= target_open_num_min) and (now_hold_num_abs <= target_open_num_max)) or not open_flag or (max_hold_time_flag != 0):
                # 之前持仓的本分钟收益
                if now_hold_num != 0:
                    now_hold_num_profit = self.face_value * now_hold_num * (close - last_close)  # 此分钟收益
                else:
            ...
            elif (now_hold_num_abs < target_open_num_min) and open_flag and max_hold_time_flag == 0: # 开仓
                # 之前持仓的本分钟收益
                if now_hold_num != 0:
                    now_hold_num_profit = self.face_value * now_hold_num * (close - last_close)  # 此分钟收益
                else:
        ...
        return pnl, results, trade_df, totaltrade_df
