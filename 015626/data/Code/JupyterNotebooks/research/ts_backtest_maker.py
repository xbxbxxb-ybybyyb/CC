...
def make_deal(order_submitted_list, order_finished_list, buy1px, sell1px, lastpx, deal_px_vol, nowtime, now_sig, vwappx):
    ...
    for k in range(len(order_submitted_list)):
        ...
        if _order.action in ['buy_open', 'buy_close'] and _order.status in ['submit', 'partial_fill']:
            for p in plist:
    ...
    def back_test_singleday(self, date):
        ...
        for i in range(1, date_signal_lenth):
            ...
            if need_trade_num != 0:
                ...
                if need_trade_num > 0:
                    open_price = (buy1px + sell1px) / 2 # buy1px + tickslippage
                else:
                    open_price = (buy1px + sell1px) / 2 # sell1px - tickslippage
                tickdt_time = nowtime
