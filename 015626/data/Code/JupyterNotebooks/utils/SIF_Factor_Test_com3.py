...
class SIF_Factor_Test:
    def __init__(self, _df,in_t = None, out_t = None, factor_name=None, future_kind='contract_00', data_kind ='future',  factor_kind='5min', return_kind='', layers=4,
                 ...
                 morning_only = False, 
                 savepath='/data/user/015626/data/share/factor/factor_test/', ic_duration_lookback = False, ret_layers_min = 30):
        """
        ...
        if factor_kind in ['1min', '5min', '15min', '30min']:
            ...
            if type(df) == pd.DataFrame:
                if len(df.columns.tolist()) == 1:
                    if factor_kind == '1min':
                        datapath = '/dfs/group/800466/warehouse/prod/MD/CHINA_COMMODITY/' + 'MINUTE_1/INSAMPLE/' 
                        origindata_full = pd.read_pickle('/dfs/group/800466/warehouse/prod/MD/CHINA_COMMODITY/rets/ret1/' + self.ticker + '.pkl')
                        ...
                        df = df.join(origindata, how = 'inner')
                            ...
                            if return_kind == '':
                                ...
                                origindata_full = pd.read_hdf('/dfs/group/800466/warehouse/prod/MD/CHINA_COMMODITY/rets/ret5_' + return_kind + '_h5/' + self.ticker + '.h5')
                            origindata = origindata_full.loc[str(starttime) : str(endtime)]
                            df = df.join(origindata, how = 'inner')
                    elif factor_kind == '15min':
                            datapath = '/dfs/group/800466/warehouse/prod/MD/CHINA_COMMODITY/' + 'MINUTE_15/INSAMPLE/' 
                            ...
                            datapath = '/dfs/group/800466/warehouse/prod/MD/CHINA_COMMODITY/' + 'MINUTE_30/INSAMPLE/' 
        ...
        df.columns = ['raw', 'ret'] if len(df.columns) == 2 else ['raw', 'ret'] + ['ret%s'%i for i in [1,5,10,20,30,45,60]] + ['ret-%s'%i for i in [1,5,10,30,60]]
        if direction == 'long':
    ...
    def draw_result(self):
        ...
        if self.factor_kind in ['overnight', 'open_to_open']:
            ...
            ax1 = fig.add_subplot(fig_rows, 4, 6)
            xlist = [-60,-30,-10,-5,-1] + [1,5,10,20,30,45,60]
...
def load_data(ticker_list, suffix=['']):
    data_root = '/dfs/group/800466/warehouse/prod/MD/CHINA_COMMODITY/MINUTE_5/INSAMPLE'
    data_dict = {}
    for ticker in ticker_list:
        data = {}
        for ss in suffix:
            if ss == '':
                data_path = f'{data_root}/{ticker}.pkl'
            else:
                data_path = f'{data_root}/{ticker}_{ss}.pkl'
...
def calculate_result(ticker_factor, st, et, ret_layers_min, return_kind):
    try:
        ticker, factor = ticker_factor
        aaa = SIF_Factor_Test(factor, ticker=ticker, return_price_kind='twap', starttime=st, endtime=et, layers=4, ret_layers_min=ret_layers_min, return_kind=return_kind, save_image=False, show_image=False, savepath='')
        c, stats, pd_res30, pd_res30_20min = aaa.draw_result()
        c_ret_daily = c['ret'].groupby(c.index.date).sum()
        c_ret_daily.index = pd.to_datetime(c_ret_daily.index)
        ...
        stats_holder = []
        for ms in ['IC-1min', 'IC-5min', 'IC-10min',  'IC-30min',  'IC-60min', 'self_corr-shift(5)', 'ret_per_deal', 'ret_per_deal_long', 'ret_per_deal_short'] :
            stats_holder.append(stats[ms])
        stats_holder.append(np.nanmean([stats['avg_long_bars'] + stats['avg_short_bars']]))
        stats_holder = pd.Series(stats_holder, index = ['IC-1', 'IC-5', 'IC-10',  'IC-30',  'IC-60', 'AC-5', 'ret_per_deal', 'ret_per_deal_long', 'ret_per_deal_short', 'avg_bars'], name=ticker) 
        long_ret = c[c.bins == 3]['ret']
        short_ret = c[c.bins == 0]['ret']
        long_ret = long_ret.groupby(long_ret.index.date).sum()
        long_ret.index = pd.to_datetime(long_ret.index)
...
def generate_result(ticker_list, factor_dict, start_time, end_time, ret_layers_min, return_kind, n_jobs=1):
    item_list = [(ticker, factor_dict[ticker]) for ticker in ticker_list]
    if n_jobs > 1:
        with Pool(n_jobs) as pool:
            result_list = pool.map(partial(calculate_result, st=start_time, et=end_time, ret_layers_min=ret_layers_min, return_kind=return_kind), item_list)
    else:
        result_list = [calculate_result(item, st=start_time, et=end_time, ret_layers_min=ret_layers_min, return_kind=return_kind) for item in item_list]
...
def combine_result(ticker_list, result_dict, factor_name):
    ...
    combo_stats = pd.concat({k: v[3] for k, v in result_dict.items()}, axis=1)
    
    combo_curve = pd.concat({k: v[0] for k, v in result_dict.items()}, axis=1).cumsum().fillna(method='ffill')
    combo_curve_l = pd.concat({k: v[4] for k, v in result_dict.items()}, axis=1).cumsum().fillna(method='ffill')
    combo_curve_s = pd.concat({k: v[5] for k, v in result_dict.items()}, axis=1).cumsum().fillna(method='ffill')

    combo_duration = pd.concat({k: v[1] for k, v in result_dict.items()}, axis=1).iloc[5:]
    combo_duration.index = ['1', '5', '10', '20', '30', '45', '60']

    combo_layers = pd.concat({k: v[2] for k, v in result_dict.items()}, axis=1)
    combo_layers_n = pd.concat({k: v[6] for k, v in result_dict.items()}, axis=1)
    ...
    output = {'name': factor_name, 'cat_stats': cat_stats, 'cat_curve': cat_curve,
              'stats': combo_stats, 'curve': combo_curve, 'curve_long': combo_curve_l, 'curve_short': combo_curve_s,
              'ic_duration': combo_duration, 'layers': combo_layers,'layers_n': combo_layers_n}
    return output
...
def draw_result(result, show_plot=True, save_plot=False, save_path=None):
    ...
    for i, idx in enumerate(df.index):
        ax.text(0.0, 0.8 - 0.08 * (i + 1), idx, fontsize=text_size)
    for j, col in enumerate(df.columns):
        ax.text(0.2 + 0.15 * j, 0.8, col, fontsize=text_size)
    for i, idx in enumerate(df.index):
        for j, col in enumerate(df.columns):
            val = df.iloc[i, j]
            if np.isnan(val):
                ...
                val = f'{val:.5f}'
            ax.text(0.2 + 0.15 * j, 0.8 - 0.08 * (i + 1), val, fontsize=text_size)
    ...
    ax.legend(df.columns[1:4])
    ax.set_title('Cumulative Return', fontsize=title_size)
    ax.tick_params(labelsize=label_size)
    ax.grid()
    
    ax = fig.add_subplot(4, 2, 6)
    ss = result['curve_long'].mean(axis=1)
    ax.plot(ss, color='red', linewidth=line_width)
    ss = result['curve_short'].mean(axis=1)
    ax.plot(ss, color='green', linewidth=line_width)
    ax.legend(['Long_Ret', 'Short_Ret'])
    ax.set_title('Cumulative Return', fontsize=title_size)
    ax.tick_params(labelsize=label_size)
    ax.grid()
    
    ax = fig.add_subplot(4, 2, 7)
    ss = result['layers'].mean(axis=1)
    xlist = ss.index.tolist()
    ylist = ss.tolist()
    ax.bar(np.arange(len(xlist)), ylist, color='royalblue', width=0.8)
    ax.set_xticks(np.arange(len(xlist)))
    ax.set_xticklabels(xlist)    
    ax.set_title('Segment Return (1-min)', fontsize=title_size)
    ax.tick_params(labelsize=label_size)
    ax.grid()
    
    ax = fig.add_subplot(4, 2, 8)
    ss = result['layers_n'].mean(axis=1)
    xlist = ss.index.tolist()
    ylist = ss.tolist()
    ax.bar(np.arange(len(xlist)), ylist, color='royalblue', width=0.8)
    ax.set_xticks(np.arange(len(xlist)))
    ax.set_xticklabels(xlist)    
    ax.set_title('Segment Return (N-min)', fontsize=title_size)
    ax.tick_params(labelsize=label_size)
    ax.grid()
