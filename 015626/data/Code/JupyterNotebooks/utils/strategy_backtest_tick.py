...
class TS_BACK_TEST:
    ...
    def __init__(self, signal_df, ticker='IC.CFE', price_kind='vwap', pos_dict = {},
                 ...
                 delay_tick_num = 0,max_wait_tick_num = 0, hour_t=14, minute_t=30, closepos_minute = 50, save_path='/data/user/', name_prefix=''):
        """
...
#         self.para_pos_dict = self.get_target_pos_dict(entry_para, exit_para)
        ...
        self.closepos_minute = closepos_minute
    ...
    def back_test(self):
        ...
        for i in tqdm(range(1, length)):
            ...
            if now_date != pre_date:
                tickdf = pd.read_csv(os.path.join(tick_path, df.iloc[i]['contract_00'][:6], str(now_date).replace('-','') + '.csv'))[['dt','Buy1Price','Sell1Price','TotalVolumeTrade','LastPx']]
            ...
            if self.deal_volume_ratio > 0:
                self.open_num_permin = min(max(np.floor(df.iloc[i]['volume'] * self.deal_volume_ratio), 1), 50)
                self.close_num_permin = self.open_num_permin

            now_hold_num_state = np.sign(now_hold_num)
            now_hold_num_abs = abs(now_hold_num)

            if (nowtime.hour >= self.hour_t) and (nowtime.minute >= self.closepos_minute): #14：50后开始平仓
                target_pos = (0, 0)
            else:
             ...
             (now_hold_num_state * pre_target_pos_state == -1) or (now_hold_num_abs > target_open_num_max):
                ...
                for j in range(len(order_px_para)):
                    ...
                    if (nowtime.hour == 14) & (nowtime.minute == 57) & (j == (len(order_px_para) - 1)) & (now_hold_num != 0):
                        ...
                        if now_hold_num_state == 1:
                            deal_price = order_px_para[j][buy1px_idx] if deal_price > 0 else order_px_para[j][lastpx_idx]
                        elif now_hold_num_state == -1:
                            deal_price = order_px_para[j][sell1px_idx] if deal_price > 0 else order_px_para[j][lastpx_idx]
...
#                 now_hold_dealcount.append(deal_count)
    ...
    def prepare_data(self):
        ...
        # 每天只交易9:35-14:55时间段
        idx = df.index
        t1 = df.loc[(idx.hour == 9) & (idx.minute >= 37)]
...
#         t = t.set_index('dt')
        ...
        df['contract_00'] = df['contract_00'].fillna(method = 'bfill')
        return df
...
#         return self.para_pos_dict[float(str(abs(signal))[:3])]
    ...
    def strategy_evaluate(self, pnl, trade, trade_minute):
        ...
        results.loc[0, '做多胜率'] = format(len(longtrade[longtrade.change > 0]) / len(longtrade), '.2%') if len(longtrade) > 0 else np.nan # 胜率
        results.loc[0, '做空笔数'] = len(shorttrade)  
        results.loc[0, '做空胜率'] = format(len(shorttrade[shorttrade.change > 0]) / len(shorttrade), '.2%') if len(shorttrade) > 0 else np.nan   # 胜率
...
def change_sig(path, sdate = 20150101, edate = 20211204, change_ticker = 'IC.CFE'):
...
# final result adjustment
def change_sig_cc(path, sdate = 20150101, edate = 20211204, change_ticker = 'IC.CFE'):
    try:
        sigorg = pd.read_hdf(path)
    except:
        sigorg = pd.read_pickle(path)
    sigorg.index.name='dt'
    adjcom = get_sig_adj(sdate,edate,ticker=change_ticker)
    sig_res = sigorg*adjcom
    sig_res.name = 'sig_res'
...
# factor = pd.read_hdf('/data/user/012398/data/MD/sig_ic_adj_prod.h5')
# save_root_path = '/data/user/015626/data/share/factor/back_test/IC_ts/20211025/'

# pos_dict = {(0, 0.000001): (0, 0),
#                  (0.000001, 0.0007): (0, 1.0/10),
#                  (0.0007, 2.1): (1.0/10, 1.0/10)}

# name_prefix = 'sig_ic_adj_prod_7_outsample_20200801_20210531'
# save_path = os.path.join(save_root_path,  name_prefix)
# if not os.path.exists(save_path):
#     os.makedirs(save_path)
# a,b,c,d = TS_BACK_TEST(factor.loc['20200801':'20210531'],price_kind='twap',ticker='IC.CFE', slippage=0.6, initial_cash=5e8, save_path = save_path, name_prefix = name_prefix,
#           pos_dict=pos_dict, capital_use_rate = 1,stop_loss = -0.005,deal_volume_ratio = 1, 
#           tick_price_kind = 'tickslippage',
#           tickslippage = 1.2, max_wait_tick_num = 4, delay_tick_num=0).back_test() 