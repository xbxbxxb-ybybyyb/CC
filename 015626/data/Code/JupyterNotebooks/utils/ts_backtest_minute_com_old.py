...
class TS_BACK_TEST:
    ...
    def __init__(self, signal_df, ticker='IC.CFE', price_kind='vwap', pos_dict = {(0, 0.4): (0.0, 0.0),
        ...
        :param base_cash: 用atr来计算交易张数的资金
        :param c_rate: 交易费用
        :param slippage: 交易价格滑点仓
        :param save_path: 结果保存路径
        :param name_prefix: 结果csv命名前缀
        :param minimum_trading_unit: 最小交易单位
        :param minute_after_stop_loss: 止损后多久不交易
        :param max_hold_time: 单笔最长持有时间
        :param filter_series: 1 0 -1，只对开仓有效
        ...
        for i in tqdm(range(1, length)):
            ...
            if trading_flag and ((stop_loss_flag and (now_hold_num!=0)) or \
                ...
                if self.filter_series is not None:
                    if open_contract_num > 0 and open_contract_num * now_hold_num >= 0 and pre_filter_socre == -1:
                        open_contract_num = 0
                    if open_contract_num < 0 and open_contract_num * now_hold_num >= 0 and pre_filter_socre == 1:
                        open_contract_num = 0
