...
class SIF_Factor_Test:
    def __init__(self, _df, in_t=None, out_t=None, factor_name=None, future_kind='contract_00', data_kind='future',  factor_kind='5min', return_kind='raw', layers=4,
                 ...
                 morning_only=False, savepath='', ic_duration_lookback=False, ret_layers_min=30):
        """
        ...
        :param morning_only: 只看早盘
        :param ret_layers_min: 收益分层的时间周期，必须在1,5,10,20,30,45,60中指定
        """
        df = _df.copy()
        ...
        if factor_kind in ['1min','5s', '5min','15min', '30min']:
        ...
        assert factor_kind in ['1min', '5s', '5min','15min', '30min', 'overnight', 'open_to_open'], 'factor_kind must be in [1min, 5s, 5min, 15min, 30min, overnight, open_to_open]'
        ...
        self.ticker = ticker
        self.ret_layers_min = ret_layers_min
        if morning_only == True:
            df = df.between_time('0900', '1500')
        h5path = '/data/user/015626/data/share/MD/CHINA_FUTURES/'

        if factor_kind in ['1min', '5min', '15min', '30min']:
            if type(df) == pd.DataFrame:
                if len(df.columns.tolist()) == 1:
                    if factor_kind == '1min':
                        datapath = '/dfs/group/800466/warehouse/prod/MD/CHINA_COMMODITY/1MIN/INSAMPLE/' 
                        if return_kind == 'raw':
                            origindata_full = pd.read_hdf('/dfs/group/800466/warehouse/prod/MD/CHINA_COMMODITY/rets/ret1_raw_h5/' + self.ticker + '.h5')
                        elif return_kind == 'ratio':
                            origindata_full = pd.read_hdf('/dfs/group/800466/warehouse/prod/MD/CHINA_COMMODITY/rets/ret1_h5/' + self.ticker + '.h5')
                        else:
                            raise RuntimeError(f'return_kind={return_kind}')
                        origindata = origindata_full.loc[str(starttime) : str(endtime)]
                        df = df.join(origindata, how = 'inner')
                    elif factor_kind == '5min':
                        datapath = '/dfs/group/800466/warehouse/prod/MD/CHINA_COMMODITY/MINUTE_5/INSAMPLE/' 
                        if return_kind == 'raw':
                            origindata_full = pd.read_hdf('/dfs/group/800466/warehouse/prod/MD/CHINA_COMMODITY/rets/ret5_raw_h5/' + self.ticker + '.h5')
                        elif return_kind == 'ratio':
                            origindata_full = pd.read_hdf('/dfs/group/800466/warehouse/prod/MD/CHINA_COMMODITY/rets/ret5_h5/' + self.ticker + '.h5')
                        else:
                            raise RuntimeError(f'return_kind={return_kind}')
                        origindata = origindata_full.loc[str(starttime) : str(endtime)]
                        df = df.join(origindata, how = 'inner')
                    elif factor_kind == '15min':
                        datapath = '/dfs/group/800466/warehouse/prod/MD/CHINA_COMMODITY/15MIN/INSAMPLE/' 
                        origindata_full = pd.read_pickle('/dfs/group/800466/warehouse/prod/MD/CHINA_COMMODITY/rets/ret15/' + self.ticker + '.pkl')
                        origindata = origindata_full.loc[str(starttime) : str(endtime)]
                        df = df.join(origindata, how = 'inner')
                    elif factor_kind == '30min':
                        datapath = '/dfs/group/800466/warehouse/prod/MD/CHINA_COMMODITY/30MIN/INSAMPLE/' 
                        origindata_full = pd.read_pickle('/dfs/group/800466/warehouse/prod/MD/CHINA_COMMODITY/rets/ret30/' + self.ticker + '.pkl')
                        origindata = origindata_full.loc[str(starttime) : str(endtime)]
                        df = df.join(origindata, how = 'inner')
            elif len(df.columns.tolist()) == 3:
                df.columns = ['raw','ret','price']
                for i in [1,5,10,20,30,45,60,90,120,240,360,480]:
        ...
        df.columns = ['raw', 'ret'] if len(df.columns) == 2 else ['raw', 'ret'] + ['ret%s'%i for i in [1,5,10,20,30,45,60,90,120,240,360,480]] + ['ret-%s'%i for i in [1,5,10,30,60]]
        if direction == 'long':
        ...
        if factor_kind in ['overnight', 'open_to_open']:
            ...
            self.df = df.copy() #self.slice_by_minute(df, time_range)
        ...
        self.ic_duration_lookback = ic_duration_lookback
    ...
    def draw_result(self):
        ...
        if self.factor_kind in ['1min', '5s', '5min', '15min', '30min']:
        ...
        if self.factor_kind in ['5s', '1min', '5min', '15min', '30min']:       
            IC_duration_list = []
            IC_decay_list = [IC]
            for i in [1,5,10,20,30,45,60,90,120,240,360,480]:
                _df2 = _df.copy()#between_time(datetime.time(9,30), datetime.time(14, 56))
                IC_duration_list.append(round(_df2['raw'].corr(_df2['ret%s'%i]), 5))
            for i in range(1,11):
                _df3 = _df.copy()#.between_time(datetime.time(9,30+i), datetime.time(14, 56))
                IC_decay_list.append(round(_df3['raw_shift%s'%i].corr(_df3['ret']), 5))
            IC5 = IC_duration_list[1]
            IC10 = IC_duration_list[2]
            IC20 = IC_duration_list[3]
            IC30 = IC_duration_list[4]
            IC45 = IC_duration_list[5]
            IC60 = IC_duration_list[6]
            IC90 = IC_duration_list[7]
            IC120 = IC_duration_list[8]
            IC240 = IC_duration_list[9]
            IC360 = IC_duration_list[10]
            IC480 = IC_duration_list[11]
        ...
        assert int(self.ret_layers_min) in [1, 5, 10, 20, 30, 45, 60], 'ret_layers_min must be in 1, 5, 10, 20, 30, 45, 60'
        ps_return20 = df_copy['ret%s'%str(self.ret_layers_min)]
        ...
        pd_res30_20min, magic30_20min = self.ts_segment_test(ps_raw, ps_return20, layers=30, layer_lims=self.signal_lims)
        # 计算每组占据多长时间
        ...
        if IC5 == IC5:
            ...
            stats['IC-30min'] = IC30
            stats['IC-60min'] = IC60
            stats['IC-120min'] = IC120
            stats['IC-240min'] = IC240
            stats['IC-480min'] = IC480
        ...
        fig = plt.figure(figsize=(0.8*28, 0.8*5 * fig_rows))
        ...
        if self.factor_kind in ['1min', '5min', '15min', '30min']:
            plt.text(0.2, 0.3, 'avg_long_bars: ' + str(round(stats['avg_long_bars'], 2)) + ' bars', fontsize=text_fontsize)
            plt.text(0.2, 0.2, 'avg_short_bars: ' + str(round(stats['avg_short_bars'], 2)) + ' bars', fontsize=text_fontsize)
        ...
        plt.text(0.6, 0.7, 'annual_ret: ' + str(annual_ret), fontsize=text_fontsize)
        plt.text(0.6, 0.6, 'ret_before_fee: ' + str(ret_long_short), fontsize=text_fontsize)
        # if self.fee > 0:
        #     plt.text(0.6, 0.5, 'profit_after_fee: ' + str(after_fee_cumualtive_long_short_point), fontsize=text_fontsize)

        plt.text(0.6, 0.5, 'ret_per_deal: ' + str((ret_per_deal)), fontsize=text_fontsize)
        plt.text(0.6, 0.4, 'ret_per_deal_long: ' + str(ret_per_deal_long), fontsize=text_fontsize)
        plt.text(0.6, 0.3, 'ret_per_deal_short: ' + str((ret_per_deal_short)), fontsize=text_fontsize)
        plt.text(0.6, 0.2, 'long_short_mdd: ' + str((long_short_mdd)), fontsize=text_fontsize)
        ...
        if self.factor_kind in ['overnight', 'open_to_open']:
            ...
            ax1 = fig.add_subplot(fig_rows, 4, 6)
            xlist = [-60,-30,-10,-5,-1] + [1,5,10,20,30,45,60,90,120,240,360,480]
            ylist = IC_duration_list
            if not self.ic_duration_lookback:
                xlist = xlist[5:]
                ylist = ylist[5:]
            ax1.bar(np.arange(len(xlist)), ylist, color='dodgerblue')
            ax1.set_xticks(np.arange(len(xlist)))
        ...
        if self.show_image:
            plt.show()
        plt.close()
        return c, stats, pd_res30, pd_res30_20min


def prepare_data(cat_list, futures_data_insample, temp_object):
    cat_dict_list = []
    for i, cat in enumerate(cat_list):
        exec("cat_dict%s = {}"%str(i))
        exec("cat_dict%s['cat'] = cat"%str(i))
        exec("cat_dict_list.append(cat_dict%s)"%str(i))
    
    needed_list = temp_object.required_columns
    para_num = temp_object.on_bar.__code__.co_argcount - 2
    ...
    for item in needed_list:
        for i,item1 in enumerate(cat_dict_list):    
                exec("cat_dict%s[item] = futures_data_insample[cat_dict%s['cat']][item]"%(i, i))
    ...
    all_dict = []
    for item in cat_dict_list:
        all_dict.append(item)
    return all_dict


def calc_factor(cat, all_dict, temp_object):

    data_hf = all_dict[cat]
    factor0 = temp_object 
    factor = factor0.on_bar(data_hf)
    return factor

def calc_combo(cat, all_dict, temp_object, st, et, ret_layers_min):
    
    factor = calc_factor(cat, all_dict, temp_object)
        
    aaa = SIF_Factor_Test(factor,  ticker = cat, save_image = False, show_image = False, return_price_kind = 'twap',
              #direction = 'long',
              #time_range = [900, 1600],
              savepath = '/data/user/016700/Data/Factors/factor_reports/20220722/',
              starttime = st, endtime=et, layers = 4, 
              ret_layers_min = ret_layers_min,
             )
    
    c, stats, pd_res30, pd_res30_20min = aaa.draw_result()
    
    c_ret_daily = c['ret'].groupby(c.index.date).sum()
    c_ret_daily.index = pd.to_datetime(c_ret_daily.index)
    c_ret_daily.name = cat
    duration = pd.Series(stats['IC_duration'], name = cat)
    pd_res30.columns = [cat]
    pd_res30_20min.columns = [cat]
    stats_holder = []
    for ms in ['IC-1min', 'IC-5min', 'IC-10min',  'IC-30min',  'IC-60min', 'ret_per_deal', 'ret_per_deal_long', 'ret_per_deal_short'] :
        stats_holder.append(stats[ms])
    stats_holder.append(np.nanmean([stats['avg_long_bars'] + stats['avg_short_bars']]))
    stats_holder = pd.Series(stats_holder, index = ['IC-1min', 'IC-5min', 'IC-10min',  'IC-30min',  'IC-60min', 'ret_per_deal', 'ret_per_deal_long', 'ret_per_deal_short', 'avg_bars'], name = cat) 
    long_ret = c[c.bins == 3]['ret']
    short_ret = c[c.bins == 0]['ret']
    long_ret = long_ret.groupby(long_ret.index.date).sum()
    long_ret.index = pd.to_datetime(long_ret.index)
    short_ret = short_ret.groupby(short_ret.index.date).sum()
    short_ret.index = pd.to_datetime(short_ret.index)
    long_ret.name = cat
    short_ret.name = cat
    print(cat)
    return [c_ret_daily, duration, pd_res30, stats_holder, long_ret, short_ret, pd_res30_20min]
...
def load_data(ticker_list, factor_kind, root=None, suffix=['']):
    root_dict = {
        '1min': '/dfs/group/800466/warehouse/prod/MD/CHINA_COMMODITY/1MIN/INSAMPLE',
        '5min': '/dfs/group/800466/warehouse/prod/MD/CHINA_COMMODITY/MINUTE_5/INSAMPLE',
        '15min': '/dfs/group/800466/warehouse/prod/MD/CHINA_COMMODITY/15MIN/INSAMPLE',
        '30min': '/dfs/group/800466/warehouse/prod/MD/CHINA_COMMODITY/30MIN/INSAMPLE',
    }
    if root is None:
        root = root_dict[factor_kind]
    print(f'load data from {root}')
    ...
    for ticker in ticker_list:
        ...
        for ss in suffix:
            if ss == '':
                data_path = f'{root}/{ticker}.pkl'
            else:
                data_path = f'{root}/{ticker}_{ss}.pkl'
            data.update(pd.read_pickle(data_path))
        data_dict[ticker] = data
    return data_dict


def calculate_factor(data, factor_obj):
    # try:
    factor = factor_obj.on_bar(data)
...
def generate_factor(ticker_list, data_dict, factor_obj, n_jobs=1):
    item_list = prepare_data(ticker_list, data_dict, factor_obj)
    if n_jobs > 1:
        with Pool(n_jobs) as pool:
            factor_list = pool.map(partial(calculate_factor, factor_obj=factor_obj), item_list)
    else:
        factor_list = [calculate_factor(item, factor_obj=factor_obj) for item in item_list]
    factor_dict = {ticker: factor for ticker, factor in zip(ticker_list, factor_list)}
...
def define_cats(cat_list):
    assert len(cat_list) > 0, 'List Empty'
    selected_cats1 = ['NI.SHF', 'OI.CZC', 'EB.DCE', 'AP.CZC', 'PP.DCE']
    selected_cats2 = ['PG.DCE', 'EG.DCE', 'SN.SHF', 'J.DCE', 'P.DCE', 'SC.INE', 'AU.SHF','JM.DCE', 'SF.CZC', 'Y.DCE', 'RB.SHF', 'HC.SHF', 'JD.DCE', 'SM.CZC', 'FU.SHF', 'AG.SHF']
    selected_cats3 = list(set(cat_list) - set(selected_cats1) - set(selected_cats2))
    
    selected_cats1 = list(set(selected_cats1).intersection(set(cat_list)))
    selected_cats2 = list(set(selected_cats2).intersection(set(cat_list)))
    selected_cats3 = list(set(selected_cats3).intersection(set(cat_list)))
    return cat_list, selected_cats1, selected_cats2, selected_cats3


def calculate_result(ticker_factor, st, et, factor_kind, return_kind, ret_layers_min):
    try:
        ticker, factor = ticker_factor
        aaa = SIF_Factor_Test(factor, ticker=ticker, return_price_kind='twap', starttime=st, endtime=et, factor_kind=factor_kind, return_kind=return_kind, ret_layers_min=ret_layers_min, layers=4, save_image=False, show_image=False, savepath='')
        c, stats, pd_res30, pd_res30_20min = aaa.draw_result()
        
        c_ret_daily = c['ret'].groupby(c.index.date).sum()
        c_ret_daily.index = pd.to_datetime(c_ret_daily.index)
        c_ret_daily.name = ticker

        long_ret = c[c.bins == 3]['ret']
        long_ret = long_ret.groupby(long_ret.index.date).sum()
        long_ret.index = pd.to_datetime(long_ret.index)
        long_ret.name = ticker

        short_ret = c[c.bins == 0]['ret']
        short_ret = short_ret.groupby(short_ret.index.date).sum()
        short_ret.index = pd.to_datetime(short_ret.index)
        short_ret.name = ticker

        duration = pd.Series(stats['IC_duration'], name=ticker)

        pd_res30.columns = [ticker]
        pd_res30_20min.columns = [ticker]

        stats_holder = []
        for ms in ['IC-1min', 'IC-10min',  'IC-30min',  'IC-60min',  'IC-120min',  'IC-240min',  'IC-480min', 'self_corr-shift(5)', 'ret_per_deal', 'ret_per_deal_long', 'ret_per_deal_short'] :
            stats_holder.append(stats[ms])
        stats_holder.append(np.nanmean([stats['avg_long_bars'] + stats['avg_short_bars']]))
        stats_holder = pd.Series(stats_holder, index = ['IC-1', 'IC-10', 'IC-30', 'IC-60', 'IC-120', 'IC-240', 'IC-480', 'AC-5', 'ret_per_deal', 'ret_per_deal(long)', 'ret_per_deal(short)', 'avg_bars'], name=ticker) 
        
        result = [c_ret_daily, duration, pd_res30, stats_holder, long_ret, short_ret, pd_res30_20min]
    except Exception as e:
        print(e)
        result = None
    return result
...
def generate_result(ticker_list, factor_dict, start_time, end_time, factor_kind, return_kind, ret_layers_min=30, n_jobs=1):
    item_list = [(ticker, factor_dict[ticker]) for ticker in ticker_list]
    if n_jobs > 1:
        with Pool(n_jobs) as pool:
            result_list = pool.map(partial(calculate_result, st=start_time, et=end_time, factor_kind=factor_kind, return_kind=return_kind, ret_layers_min=ret_layers_min), item_list)
    else:
        result_list = [calculate_result(item, st=start_time, et=end_time, factor_kind=factor_kind, return_kind=return_kind, ret_layers_min=ret_layers_min) for item in item_list]
    result_dict = {ticker: result for ticker, result in zip(ticker_list, result_list)}
...
def combine_result(ticker_list, result_dict, factor_name):
    ticker_list = [ticker for ticker in ticker_list if result_dict[ticker] is not None]
    result_dict = {ticker: result_dict[ticker] for ticker in ticker_list}

    stats = pd.concat({k: v[3] for k, v in result_dict.items()}, axis=1)
    
    curve = pd.concat({k: v[0] for k, v in result_dict.items()}, axis=1).cumsum().fillna(method='ffill')
    curve_l = pd.concat({k: v[4] for k, v in result_dict.items()}, axis=1).cumsum().fillna(method='ffill')
    curve_s = pd.concat({k: v[5] for k, v in result_dict.items()}, axis=1).cumsum().fillna(method='ffill')

    ic_duration = pd.concat({k: v[1] for k, v in result_dict.items()}, axis=1).iloc[5:]  # [-60, -30, -10, -5, -1, 1, 5, ...]
    ic_duration.index = ['1', '5', '10', '20', '30', '45', '60', '90', '120', '240', '360', '480']

    layers = pd.concat({k: v[2] for k, v in result_dict.items()}, axis=1)
    layers_n = pd.concat({k: v[6] for k, v in result_dict.items()}, axis=1)

    # all_cats, selected_cats1, selected_cats2, selected_cats3 = define_cats(ticker_list)

    # stats_total = combo_stats.mean(axis=1)
    # stats_cats1 = combo_stats[selected_cats1].mean(axis=1)
    # stats_cats2 = combo_stats[selected_cats2].mean(axis=1)
    # stats_cats3 = combo_stats[selected_cats3].mean(axis=1)
    # cat_stats = pd.concat([stats_total, stats_cats1, stats_cats2, stats_cats3], axis=1)
    # cat_stats.columns = ['All', 'High_Vol', 'Mid_Vol', 'Low_Vol']

    # curve_total = combo_curve.mean(axis = 1)
    # curve_cats1 = combo_curve[selected_cats1].mean(axis = 1)
    # curve_cats2 = combo_curve[selected_cats2].mean(axis = 1)
    # curve_cats3 = combo_curve[selected_cats3].mean(axis = 1)
    # cat_curve = pd.concat([curve_total, curve_cats1, curve_cats2, curve_cats3], axis=1)
    # cat_curve.columns = ['All', 'High_Vol', 'Mid_Vol', 'Low_Vol']
    
    output = {'name': factor_name, 'stats': stats, 'curve': curve, 'curve_l': curve_l, 'curve_s': curve_s, 'ic_duration': ic_duration, 'layers': layers, 'layers_n': layers_n}
    return output


def draw_result(result, show_plot=True, save_plot=False, save_path=None):
    ...
    name = result['name']
    ax.text(0.0, 0.9, name, fontsize=head_size)

    stats = result['stats']
    df = pd.concat({'mean': stats.mean(axis=1), 'median': stats.median(axis=1)}, axis=1)
    df = df.loc[['IC-1', 'IC-10', 'IC-30', 'IC-60', 'IC-120', 'IC-240', 'IC-480']]
    for i, idx in enumerate(df.index):
        ax.text(0.0, 0.8 - 0.1 * (i + 1), idx, fontsize=text_size)
    for j, col in enumerate(df.columns):
        ax.text(0.4 + 0.3 * j, 0.8, col, fontsize=text_size)
    for i, idx in enumerate(df.index):
        for j, col in enumerate(df.columns):
            val = df.iloc[i, j]
            if np.isnan(val):
                ...
                val = f'{val:.4f}'
            ax.text(0.4 + 0.3 * j, 0.8 - 0.1 * (i + 1), val, fontsize=text_size)
    ...
    stats = result['stats']
    df = pd.concat({'mean': stats.mean(axis=1), 'median': stats.median(axis=1)}, axis=1)
    df = df.loc[['AC-5', 'ret_per_deal', 'ret_per_deal(long)', 'ret_per_deal(short)', 'avg_bars']]
    for i, idx in enumerate(df.index):
        ax.text(0.0, 0.8 - 0.1 * (i + 1), idx, fontsize=text_size)
    for j, col in enumerate(df.columns):
        ax.text(0.4 + 0.3 * j, 0.8, col, fontsize=text_size)
    ...
    ss = result['ic_duration'].mean(axis=1)
    xlist = ss.index.tolist()
    ylist = ss.tolist()
    ax.bar(np.arange(len(xlist)), ylist, color='royalblue', width=0.8)
    ax.set_xticks(np.arange(len(xlist)))
    ax.set_xticklabels(xlist)
    ax.set_title('IC Duration', fontsize=title_size)
    ax.tick_params(labelsize=label_size)
    ax.grid()

    ax = fig.add_subplot(num_rows, num_cols, 4)
    ss = result['curve'].mean(axis=1)
    ax.plot(ss, color='royalblue', linewidth=line_width)
    ss = result['curve_l'].mean(axis=1)
    ax.plot(ss, color='red', linewidth=line_width)
    ss = result['curve_s'].mean(axis=1)
    ax.plot(ss, color='green', linewidth=line_width)
    ax.legend(['Long Short', 'Long', 'Short'])
    ...
    kk = 'ret_per_deal'
    ss = result['stats'].loc[kk].sort_values(ascending=False)
    xlist = ss.index.tolist()
    xlist = [x.split('.')[0] for x in xlist]
    ylist = ss.tolist()
    zlist = [ss.mean()] * len(xlist)
    ax.bar(np.arange(len(xlist)), ylist, color='royalblue', width=0.8)
    ax.plot(np.arange(len(xlist)), zlist, color='royalblue', linestyle='--', linewidth=line_width)
    ax.set_xticks(np.arange(len(xlist)))
    ax.set_xticklabels(xlist)
    ax.set_title(kk, fontsize=title_size)
    ax.tick_params(labelsize=label_size)
    ax.grid()
    ...
    ax = fig.add_subplot(num_rows, num_cols, 8)
    kk = 'IC-120'
    ss = result['stats'].loc[kk].sort_values(ascending=False)
    xlist = ss.index.tolist()
    xlist = [x.split('.')[0] for x in xlist]
    ...
    ax = fig.add_subplot(num_rows, num_cols, 9)
    kk = 'IC-240'
    ss = result['stats'].loc[kk].sort_values(ascending=False)
    xlist = ss.index.tolist()
    xlist = [x.split('.')[0] for x in xlist]
    ...
    ax = fig.add_subplot(num_rows, num_cols, 10)
    kk = 'IC-480'
    ss = result['stats'].loc[kk].sort_values(ascending=False)
    xlist = ss.index.tolist()
    xlist = [x.split('.')[0] for x in xlist]
    ...
    if save_plot:
        ...
        plt.savefig(f'{save_path}/{name}.png', format='png', dpi=100)
    if show_plot:
        plt.show()
    plt.close()
    return None
