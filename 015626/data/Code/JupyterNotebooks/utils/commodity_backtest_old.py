...
def back_test(ticker, signal, filter_series=None, start_date=None, end_date=None, in_th=0.5, ot_th=0.5, in_end=2, session1=True, session2=True, overnight=True, freq='1min', num_processes=10):
    ...
    ref = ref.loc[start_date:end_date]
    ...
    date_list = price.groupby(price.index.date).mean().index.to_list()
    ...
    total_trade = total_trade[['ticker', 'pos', 'in_time', 'ot_time', 'in_price', 'ot_price', 'in_spread', 'ot_spread', 'in_fee', 'ot_fee',
                               'hold_bars', 'hold_time', 'change', 'return', 'net_return', 'itd_return', 'ovn_return', 'ss1_return', 'ss2_return']]

    result = get_result(prev_corr, next_corr, total_trade, date_list)
    return result
...
def get_result(prev_corr, next_corr, total_trade, date_list):
    raw_stats, raw_ret = get_stats(total_trade, date_list, ret_col='return')
    raw_stats_l, raw_ret_l = get_stats(total_trade[total_trade['pos'] > 0], date_list, ret_col='return')
    raw_stats_s, raw_ret_s = get_stats(total_trade[total_trade['pos'] < 0], date_list, ret_col='return')
    raw_cum_ret = raw_ret.cumsum()
    raw_cum_ret_l = raw_ret_l.cumsum()
    raw_cum_ret_s = raw_ret_s.cumsum()

    net_stats, net_ret = get_stats(total_trade, date_list, ret_col='net_return')
    net_stats_l, net_ret_l = get_stats(total_trade[total_trade['pos'] > 0], date_list, ret_col='net_return')
    net_stats_s, net_ret_s = get_stats(total_trade[total_trade['pos'] < 0], date_list, ret_col='net_return')
    net_cum_ret = net_ret.cumsum()
    net_cum_ret_l = net_ret_l.cumsum()
    net_cum_ret_s = net_ret_s.cumsum()

    itd_stats, itd_ret = get_stats(total_trade, date_list, ret_col='itd_return')
    ovn_stats, ovn_ret = get_stats(total_trade, date_list, ret_col='ovn_return')
    ss1_stats, ss1_ret = get_stats(total_trade, date_list, ret_col='ss1_return')
    ss2_stats, ss2_ret = get_stats(total_trade, date_list, ret_col='ss2_return')
...
def get_stats(total_trade, date_list, ret_col='return'):
    df = total_trade.copy()
    num_tickers = len(df['ticker'].drop_duplicates())
    if len(total_trade) > 0 and (df[ret_col].abs().sum() > 0):
        trade_ret = df[ret_col]
        trade_ret.index = df['ot_time']
        daily_ret = trade_ret.groupby(trade_ret.index.date).sum()
        daily_ret.index = pd.to_datetime(daily_ret.index)
        daily_ret = daily_ret.reindex(pd.to_datetime(date_list), fill_value=0)
        ...
        daily_ret = pd.Series(0.0, index=pd.to_datetime(date_list))
        stats = {
            'annual_return': '-',
            'sharpe_ratio': '-',
            'win_ratio': '-',
