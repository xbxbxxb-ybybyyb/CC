...
for i in univ.values:
    ...
    data_txn = data_txn[data_txn.TradeType == 0]
    data_txn['MDTime'] = [int(pd.Timestamp(x).strftime('%H%M%S%f')) for x in data_txn.dt]
    data_txn = data_txn[data_txn.MDTime < 92600000000]
    
    if (len(data) > 1) & (len(data_txn) > 1):
        px = data_txn.TradePrice.values[-1]
        qty = data_txn.TradeQty.sum()
    result.append([i[0], i[1], (px - data.Buy1Price.values[0])/data.PreClosePx.values[0]/(px * qty - data.Buy1Price.values[0] * (data.Buy1OrderQty.values[0]))])
