...
def add_turnover(df, stock):
    ...
    if stock == '689009.SH':
        ashare = pd.DataFrame([20201029.0, 50900.0000], index = ['CHANGE_DT','FLOAT_A_SHR']).T
    else:
...
flag_path_start = flag_root + str(flag_date) + '_' + 'IC_cfg_and_mask.start'
...
picklesavename = ticker[:2] + '_cfg_data_2020.pkl'
...
save_pickle(df20dict, '/data/user/015626/data/warehouse/prod/MD/CHINA_FUTURES/MINUTE/OUTSAMPLE/' + picklesavename)

flag_path_success = flag_root + str(flag_date) + '_' + 'IC_cfg_and_mask.success'
