...
def generate_para_excel(trade_date, trading_plan):
    ...
    if trading_plan['Signal'].iloc[0] > 0.05:
        plan['买卖方向'] = [afternoon_trade_direction_long[i[:2]] for i in plan['合约代码']]
    elif trading_plan['Signal'].iloc[0] <= 0.05:
...
def executor(trade_date=None, max_workers=12, mode = 'realtime', tag='factors'):
    ...
    trading_plan_1_0, trading_plan_2_2, trading_plan_3_0 = signal_dealer(factor_score['norm'], spot_close_dict, recent_contract, season_contract, volume_limit_dict, settlement_ratio_dict)
    print('-' * 60)
    print('Diamond 1.0:')
    print(trading_plan_1_0[['Contract', 'Contract_Num', 'Signal', 'Settlement_Ratio']])
    lm.sendMessage('Diamond 1.0:\n' + str(trading_plan_1_0[['Contract', 'Contract_Num', 'Signal', 'Settlement_Ratio']]))
    print('-' * 60)
    print('Diamond 2.2:')
    print(trading_plan_2_2[['Contract', 'Contract_Num', 'Signal', 'Settlement_Ratio']])
    lm.sendMessage('Diamond 2.2:\n' + str(trading_plan_2_2[['Contract', 'Contract_Num', 'Signal', 'Settlement_Ratio']]))
    print('-' * 60)
    print('Diamond 3.0:')
    print(trading_plan_3_0[['Contract', 'Contract_Num', 'Signal', 'Settlement_Ratio']])
    lm.sendMessage('Diamond 3.0:\n' + str(trading_plan_3_0[['Contract', 'Contract_Num', 'Signal', 'Settlement_Ratio']]))
    print('-' * 60)
    # dump factor
...
#    excel_transmission(trade_date)
    ...
    trading_plan_2_2.to_csv(os.path.join(trading_plan_savepath, '%s_%s_2_2.csv' % (trade_date, trade_stop_time.strftime('%H%M'))))
    trading_plan_3_0.to_csv(os.path.join(trading_plan_savepath, '%s_%s_3_0.csv' % (trade_date, trade_stop_time.strftime('%H%M'))))
    # dump factor
