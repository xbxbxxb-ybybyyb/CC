...
def get_signal_05(factor_series):
    factor_series_open = factor_series[factor_series >= 0.5]
    signal_raw = factor_series_open.shape[0] / factor_series.shape[0]
    return signal_raw
...
def get_signal_final_Diamond_2_1(factor_series):
    future_ic = get_signal(factor_series.reindex(TRADING_PLAN['future_ic_2_1']))
    spot_ic = get_signal(factor_series.reindex(TRADING_PLAN['spot_ic_2_1']))
    future_if = get_signal(factor_series.reindex(TRADING_PLAN['future_if_2_1']))
    spot_if = get_signal(factor_series.reindex(TRADING_PLAN['spot_if_2_1']))
    future_ih = get_signal(factor_series.reindex(TRADING_PLAN['future_ih_2_1']))
    spot_ih = get_signal(factor_series.reindex(TRADING_PLAN['spot_ih_2_1']))
    signal_ic = (future_ic + spot_ic) / 2
    signal_if = (future_if + spot_if) / 2
    signal_ih = (future_ih + spot_ih) / 2
    signal = (signal_ic + signal_if + signal_ih) / 3
...
def get_signal_final_Diamond_2_0_05(factor_series):
    future_ic = get_signal_05(factor_series.reindex(TRADING_PLAN['future_ic']))
    spot_ic = get_signal_05(factor_series.reindex(TRADING_PLAN['spot_ic']))
    future_if = get_signal_05(factor_series.reindex(TRADING_PLAN['future_if']))
    spot_if = get_signal_05(factor_series.reindex(TRADING_PLAN['spot_if']))
    future_ih = get_signal_05(factor_series.reindex(TRADING_PLAN['future_ih']))
    spot_ih = get_signal_05(factor_series.reindex(TRADING_PLAN['spot_ih']))
    signal_ic = (future_ic + spot_ic) / 2
    signal_if = (future_if + spot_if) / 2
    signal_ih = (future_ih + spot_ih) / 2
    signal = (signal_ic + signal_if + signal_ih) / 3
...
def get_signal_final_Diamond_2_2(factor_series):
    signal = get_signal(factor_series.reindex(TRADING_PLAN['Diamond_2_2']))
    return signal
    

def get_signal_final_Diamond_3_0(factor_series):
    signal = get_signal(factor_series.reindex(TRADING_PLAN['Diamond_3_0']))
    return signal
...
def calc_position_from_signal(signal, index_prices, recent_contract, season_contract, volume_limit_dict, settlement_ratio_dict):
    ...
    for k, v in index_prices.items():
        ...
        signal_temp = signal
        if signal_temp < 0.05:
            signal_temp = -0.167  # -0.21875, -0.175
        elif (signal_temp < 0.2) & (signal_temp > 0):
            signal_temp = 0
...
def signal_dealer(factor_series, index_prices, recent_contract, season_contract, volume_limit_dict, settlement_ratio_dict):
    settlement_ratio_temp = max(settlement_ratio_dict['IC.CFE'] - 1, settlement_ratio_dict['IF.CFE'] - 1, 0)
    need_money_51607 = int(max(round(5 + 200 * settlement_ratio_temp) * 100, 1000))
    signal_1_0 = get_signal_final_Diamond_1_0(factor_series)
    # lm.sendMessage(f'另类策略团队账户：51607留{need_money_51607}万，51606留400万，其余请转出')
    if signal_1_0 < 0.05:
        lm.sendMessage(f'另类策略团队账户：51607留{need_money_51607}万，51606留1600万，其余请转出')
    else:
        lm.sendMessage(f'另类策略团队账户：51607留{need_money_51607}万，51606留400万，其余请转出')
    
#    signal_2_0 = get_signal_final_Diamond_2_0(factor_series)
    # signal_2_1 = get_signal_final_Diamond_2_1(factor_series)
    signal_2_2 = get_signal_final_Diamond_2_2(factor_series)
#    signal_2_0_05 = get_signal_final_Diamond_2_0_05(factor_series)
    signal_3_0 = get_signal_final_Diamond_3_0(factor_series)
    trading_plan_1_0 = calc_position_from_signal(signal_1_0, index_prices, recent_contract, season_contract, volume_limit_dict, settlement_ratio_dict)
#    trading_plan_2_0 = calc_position_from_signal(signal_2_0, index_prices, recent_contract, season_contract, volume_limit_dict, settlement_ratio_dict)
    # trading_plan_2_1 = calc_position_from_signal(signal_2_1, index_prices, recent_contract, season_contract, volume_limit_dict, settlement_ratio_dict)
    trading_plan_2_2 = calc_position_from_signal(signal_2_2, index_prices, recent_contract, season_contract, volume_limit_dict, settlement_ratio_dict)
#    trading_plan_2_0_05 = calc_position_from_signal(signal_2_0_05, index_prices, recent_contract, season_contract, volume_limit_dict, settlement_ratio_dict)
    trading_plan_3_0 = calc_position_from_signal(signal_3_0, index_prices, recent_contract, season_contract, volume_limit_dict, settlement_ratio_dict)
    return trading_plan_1_0, trading_plan_2_2, trading_plan_3_0
