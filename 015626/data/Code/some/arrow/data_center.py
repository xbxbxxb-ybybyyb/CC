...
class HistoryData:
        ...
        for k,v in name_dict.items():
            ...
            for stk in self.universe:
                ...
                for date in self.tdays_list:
                    csv_path = os.path.join(stk_path, f'{date}.csv')
    ...
    def get_mad_startdate(self):
        ...
        for x in mad_periods:
            d_dict[x] = udt.get_trading_day_offset(self.next_tdate, -1 * (x - 1))[0].strftime('%Y%m%d')
    ...
    def get_dummy(self):
        ...
        univ = pd.read_pickle(universe_path).set_index('dt').loc[self.next_tdate].reset_index()
