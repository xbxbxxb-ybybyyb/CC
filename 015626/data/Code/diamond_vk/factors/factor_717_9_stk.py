...
import pandas as pd
# 更优波动率的均值
class factor_717_9_stk(FactorGenerator):
    def __init__(self, *args, **kwargs):
        required_columns = ['open_stk', 'high_stk', 'low_stk', 'close_stk', 'amount_stk']
        super(factor_717_9_stk, self).__init__(*args, required_columns=required_columns, **kwargs)

    def on_bar(self, df):
        tday = df['close_stk'].index.date[-1]
        opendf = df['open_stk'].loc[tday.strftime('%Y%m%d')].between_time(data_morning_begin, data_afternoon_end)
        close = df['close_stk'].loc[tday.strftime('%Y%m%d')].between_time(data_morning_begin, data_afternoon_end)
        high = df['high_stk'].loc[tday.strftime('%Y%m%d')].between_time(data_morning_begin, data_afternoon_end)
        low = df['low_stk'].loc[tday.strftime('%Y%m%d')].between_time(data_morning_begin, data_afternoon_end)
