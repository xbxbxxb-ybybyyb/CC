...
#    target_trigger_time = pd.Timedelta(hours=14, minutes=56)
    def final_funcs_helper():
        ...
        hot_path = os.path.join(hot_root, datetime.datetime.now().strftime('%Y%m%d'))
        out_path = os.path.join(hot_path, 'ccbond_stock_kline_1min_%s_%s.h5' % (morning_start_time.strftime('%H%M%S'), stock_ref_limit_end_time.strftime('%H%M%S')))
        while not os.path.exists(out_path):
            lm.sendMessage(str(datetime.datetime.now()) + " lack stock data")
            time.sleep(10)
