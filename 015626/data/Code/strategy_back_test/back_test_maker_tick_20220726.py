...
# 用estimate进行估计判断判断成交，如果可以成交，则以发单价成交，否则挂单
...
#             return 10
    ...
    def back_test_singleday(self, date):
        ...
        for i in range(1, date_signal_lenth):
            assert len(order_submitted_list) <= 1
    ...
    def back_test(self):
        ...
        if self.save_results:
            ...
            trade_df.to_csv(os.path.join(self.save_path, self.name_prefix + '_order_detail_alltick.csv'))
    ...
    for x in sigvalues:
        if state == 0:
            ...
            if x >= -out_t:
                ...
                rlist.append(-1)
    return pd.Series(rlist, index = sig.index)

# sig = pd.read_hdf('/data/user/017024/share/vars/for_wyc/tick_sig_new.h5')
# sig = pd.read_hdf('/data/user/012398/data/MD/sig_tick.h5')
# sig = pd.read_pickle('/data/user/016700/Data/temp_sig_tick.pkl')
sig = pd.read_pickle('/data/group/800466/tmp/lstm_tick/lstm_20tk_20ticks_ins.pkl').shift(-1)
start_date = 20200701
end_date = 20201231
sig = change_sig(sig, 0.15, -0.1)
name = 'cf_oos_0.15-0.1'
ts = TS_BACK_TEST(sig, in_t=0.9, submit_order_t=0.9, out_t=0.1, max_wait_tick_num_close_order = None,
                  initial_cash=1e7, cash_divnum=10,
                  c_rate=2.4 / 100000, ticker='IC.CFE',
                  start_date=start_date, end_date=end_date,
                  tickslippage_dict={(0, 0.3): 0,
                                     (0.3, 0.7): 0,
                                     (0.7, 0.8): 0,
                                     (0.8, 0.9): 0,
                                     (0.9, 100): 0},
                  trade_start_time=[9, 30], trade_end_time=[14, 50],
                  save_results = True,
                  save_path='/data/user/015626/data/share/factor/back_test/Tick/IC/20220720/%s_%s_%s' % (name, start_date, end_date),
                  name_prefix='test_%s' % name)
# finishdf = ts.back_test_singleday(20201208)
result = ts.back_test()