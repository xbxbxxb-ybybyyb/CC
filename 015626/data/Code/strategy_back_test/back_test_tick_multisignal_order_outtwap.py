...
class TS_BACK_TEST:
    ...
    def back_test_singleday(self, date):
        ...
        tickdf['flag_volume'] = (tickdf['TotalVolumeTrade'].rolling(120, min_periods = 60).sum() * 0.1).apply(lambda x:np.floor(x))
        tickdf['commission_num'] = tickdf['TotalVolumeTrade'].rolling(self.max_wait_tick_num * 2, min_periods = self.max_wait_tick_num).mean().apply(lambda x:max(min(np.floor(x), 4), 1))
        ...
        for i in range(1, date_signal_lenth):
            ...
            if need_trade_num == 0:#此分钟不交易
                ...
                if need_trade_num * pre_hold_num < 0 and abs(need_trade_num) <= abs(pre_hold_num):
                    # print(nowtime, 'pingcang')
                    ...
                    for z in range(len(order_px_para)):
                        ...
                        if (round(open_price,1) >= _deal_price) and (tickvolume > 0) and not makedealflag:
                            ...
                            deal_vol = min(1, open_contract_num)
                    ...
                    # 新的收益细节：
                    deal_weighted_price_close, deal_volume_close, deal_weighted_price_open, deal_volume_open = allocate_open_close(dealtotal_vol, pre_hold_num, deal_price_list, deal_volume_list, np.max([1, res_vol]))
