...
class TS_BACK_TEST:
    ...
    def __init__(self, signal_list, ticker='IC.CFE', start_date = None, end_date = None,
                 ...
                 max_open_value = None, in_th = 0.9, out_th = -0.2, cash_ratio_pertrade = 1, in_th2 = None, out_th2 = None, close_draw_down_th = None, close_volatility_th = None):
        """
        ...
        :param in_th,out_th 信号进出场参数，如0.9进 -0.2出
        :param cash_ratio_pertrade: 每笔交易分配多少资金，如0.1，或者1
        :param in_th2: 单仓位多头排列进场时所用，为None表示不启用
        :param close_th1: 单仓位出场时所用，仅支持负数，为None表示不启用。启用时参数dict和以往一样设置，不能出现负数，如果要9in-0.2out，则参数dict设置0.9进0出，close_th1设置-0.2
        :param out_th2 close_draw_down_th close_volatility_th 为趋势策略出场条件，其中close_draw_down_th close_volatility_th可以单独设置
                为None表示不作用
        ...
        self.close_volatility_th = close_volatility_th
        self.close_draw_down_th = close_draw_down_th
        self.in_th = in_th
        self.out_th = out_th
        self.cash_ratio_pertrade = cash_ratio_pertrade
        self.in_th2 = in_th2
        self.out_th2 = out_th2
        if self.close_volatility_th is not None or self.close_draw_down_th is not None:
            assert self.out_th2 is not None
        ...
        global dt_idx, raw_idx, contract_00_idx, open_idx, close_idx ,low_idx ,vwap_idx ,twap_idx ,pos_price_idx ,pos_min_idx ,pos_max_idx ,open_filter_idx, volatility_flag_idx, ma_lt_idx
        ...
        volatility_flag_idx = columns_list.index('volatility_flag')
        ma_lt_idx = columns_list.index('ma_lt')
        ...
        for sig_dict in self.signal_list:
            ...
            if len(signal_df.columns.tolist()) == 1:
                ...
                if md is None:
                    ...
                    _close = md['close'].copy()
                    ma1 = _close.rolling(240 * 1).mean()
                    ma2 = _close.rolling(240 * 2).mean()
                    ma3 = _close.rolling(240 * 4).mean()
                    ma_lt_l = (ma1 > ma2) & (ma2 > ma3)
                    ma_lt_s = (ma1 < ma2) & (ma2 < ma3)
                    md['ma_lt'] = ma_lt_l.astype('float') - ma_lt_s.astype('float')
                signal_df = signal_df.join(md, how='inner')
            else:
                clist = ['raw'] + signal_df.columns.tolist()[1:]
                signal_df.columns = clist

            # _close = signal_df['close'].copy()
            # ma1 = _close.rolling(240 * 1).mean()
            # ma2 = _close.rolling(240 * 2).mean()
            # ma3 = _close.rolling(240 * 4).mean()
            # ma_lt_l = (ma1 > ma2) & (ma2 > ma3)
            # ma_lt_s = (ma1 < ma2) & (ma2 < ma3)
            # signal_df['ma_lt'] = ma_lt_l.astype('float') - ma_lt_s.astype('float')

            signal_df = standard_index(signal_df)
            # signal_df = handle_holiday(signal_df)

            signal_df['_ret'] = signal_df['close'].pct_change()
            signal_df.loc[signal_df.index.time == pd.Timestamp('09:30').time(), '_ret'] = np.nan
            signal_df['volatility'] = signal_df['_ret'].abs().rolling(10, min_periods=1).mean()
            signal_df['volatility_flag'] = True
            if self.close_volatility_th is not None:
                signal_df.loc[signal_df['volatility'] > self.close_volatility_th, 'volatility_flag'] = False
            ...
            signal_df[['raw']] = signal_df[['raw']].fillna(0)
    ...
    def back_test_singleday(self, date):
        ...
        for i in range(1, date_signal_lenth):
            ...
            last_vwap = sig0[i - 1][vwap_idx]
            last_volatility_flag = sig0[i - 1][volatility_flag_idx]
            
            if pre_hold_num != 0:
                vwap_list_in_trade.append(last_vwap)
            ...
            for sig in signal_date_list:
                ...
                last_sig = sig['signal'][i - 1][raw_idx]
                last_ma_lt = sig['signal'][i - 1][ma_lt_idx]
                _pos_flag = ''
                if hold_num == 0:
                    if last_sig >= self.in_th or last_sig <= -1 * self.in_th:
                        _pos_flag = 'full_pos'
                    else:
                        _pos_flag = 'empty_pos'
                elif hold_num > 0:
                    if last_sig >= self.in_th:
                        _pos_flag = 'full_pos'
                    elif last_sig >= self.out_th and last_sig < self.in_th:
                        _pos_flag = 'maintain_pos'
                    else:
                        _pos_flag = 'empty_pos'
                else:
                    if last_sig <= -1 * self.in_th:
                        _pos_flag = 'full_pos'
                    elif last_sig > -1 * self.in_th and last_sig <= -1 * self.out_th:
                        _pos_flag = 'maintain_pos'
                    else:
                        _pos_flag = 'empty_pos'
                assert _pos_flag in ['full_pos', 'maintain_pos', 'empty_pos']
                if _pos_flag == 'full_pos':
                    pos_min = np.sign(last_sig) * np.floor(self.initial_cash * self.cash_ratio_pertrade / sig['signal'][i - 1][pos_price_idx] / self.face_value)
                    pos_max = pos_min
                elif _pos_flag == 'maintain_pos':
                    pos_max = np.sign(hold_num) * np.floor(self.initial_cash * self.cash_ratio_pertrade / sig['signal'][i - 1][pos_price_idx] / self.face_value)
                    pos_min = 0
                else:
                    pos_max, pos_min = 0, 0

                if self.in_th2 is not None:
                    if (last_ma_lt > 0 and last_sig > self.in_th2) or (last_ma_lt < 0 and last_sig < -1 * self.in_th2):
                        pos_min = np.sign(last_sig) * np.floor(self.initial_cash * self.cash_ratio_pertrade / sig['signal'][i - 1][pos_price_idx] / self.face_value)
                        pos_max = pos_min
            ...
            _close_flag = ''
            if pre_hold_num != 0 and self.close_volatility_th is not None:
                if not last_volatility_flag:
                    if pre_hold_num > 0 and last_sig < self.out_th2:
                        _close_flag += 'volitity'
                    elif pre_hold_num < 0 and last_sig > -1 * self.out_th2:
                        _close_flag += 'volitity'
                    
            if pre_hold_num != 0 and self.close_draw_down_th is not None:
                assert len(vwap_list_in_trade) > 0
                if pre_hold_num > 0:
                    if last_vwap / np.nanmax(vwap_list_in_trade) - 1 < -1 * self.close_draw_down_th and last_sig < self.out_th2:
                        _close_flag += 'draw_down'
                else:
                    if last_vwap / np.nanmin(vwap_list_in_trade) - 1 > self.close_draw_down_th and last_sig > -1 * self.out_th2:
                        _close_flag += 'draw_down'

            if len(_close_flag) > 0:
                need_trade_num_persig = []
            ...
            if need_trade_num == 0:#此分钟不交易
                ...
                trade_dict[deal_count] = {'deal_count': deal_count, 'pos': np.sign(now_hold_num), 'dealflag':dealflag, 'deal_time': nowtime,
                                          ...
                                          'putorder_num':putorder_num, 'dealtickcount': dealtickcount,'usetickcount': usetickcount,'tick_state':str(tick_state),'close':close,'last_close':last_close,
                                          'pre_hold_num':pre_hold_num, 'filter_score':filter_score, 'close_flag':_close_flag}
                ...
                if (pre_hold_num == 0) and (now_hold_num != 0):
                    ...
                    totaltrade_dict[totaldeal_count].update({'pos_close':np.sign(pre_hold_num),'close_time':nowtime,'profit_intradeal':profit_intradeal,
                                                            ...
                                                            'open_value_intratrade':open_value_intratrade, 'deal_fee':deal_fee_for_total_trade, 'close_filter_score':filter_score, 'close_flag':_close_flag})
                    deal_fee_for_total_trade = 0
                    profit_intradeal = 0
                    totaldeal_count += 1
                    max_hold_num_intrade = 0
    ...
    def back_test(self):
        ...
        trade_df = trade_df[['deal_time', 'pos', 'dealflag', 'deal_contract_num', 'deal_weighted_price_close', 'deal_volume_close',
         ...
         'open_value_intraday', 'putorder_num', 'dealtickcount', 'usetickcount', 'tick_state', 'filter_score', 'close_flag']]
        
        totaltrade_df = totaltrade_df[['totaltrade_count', 'pos',  'open_time', 'close_time', 'pos_close', 'profit_intradeal', 'deal_fee',
                                       'change','change_without_fee','change_of_open_value_intratrade','change_of_open_value_intratrade_without_fee', 
                                       'open_value_intratrade','max_hold_value', 'equity_curve', 'holding_time','open_filter_score','close_filter_score', 'close_flag']]
...
# 测试用例
'''
_ = pd.read_pickle('/data/user/015626/data/share/LOCAL_DATA/Mobius/IF_trend.pkl')

ap_st_in_th = 0.001
ap_lt_in_th = 0.001
_['long_filter'] = (_['ma_st'] > 0) & (_['ap_st'] < ap_st_in_th) & (_['ap_lt'] < ap_lt_in_th)
_['short_filter'] = (_['ma_st'] < 0) & (_['ap_st'] < ap_st_in_th) & (_['ap_lt'] < ap_lt_in_th)

_['filter'] = _['short_filter']
_.loc[_['sig'] >=0, 'filter'] = _['long_filter']

initial_cash1 = 5e6
back_test_sdate, back_test_edate = 20200101, 20240208

ticker = 'IF.CFE'
factor = _['sig'].copy()
filter_series = _['filter'].copy()

factor_name = 'trend_sig'
name_suffix = 'para90'
...
signal_list = [{'signal':signal1,'cash':initial_cash1, 'filter_series':filter_series}]


name = f'{name_suffix}_{factor_name}_{initial_cash1/1e8}_{sig_ot_th2}_{draw_down_th}_{ap_st_ot_th}'
a = TS_BACK_TEST(signal_list, ticker=ticker, start_date=back_test_sdate, end_date=back_test_edate, tickslippage = 0.8, 
               save_signal_list = False, save_path=f'/data/user/015626/data/share/LOCAL_DATA/Mobius/backtest_trend/{ticker}/{name}',
                 name_prefix=name, max_wait_tick_num = 2, trade_start_time = [9,49], trade_end_time = [14,56],no_opening_start_time = [14,41], closing_start_time = [14,55], 
              in_th = 0.9, out_th = -0.2, cash_ratio_pertrade = 1,
             in_th2 = 0.8, out_th2 = sig_ot_th2, close_draw_down_th = draw_down_th, close_volatility_th = ap_st_ot_th)
result = a.back_test()
'''