    ...
    def back_test_singleday(self, date):
        ...
        for i in range(1, date_signal_lenth):
            ...
            if need_trade_num == 0:#此分钟不交易
                ...
                open_tickvolume = pre_tickdf[0][volume_idx]
                open_tickvolume_flag = False if open_tickvolume <=1 else True
                ...
                for z in range(len(order_px_para)):
                    ...
                    if (round(open_price,1) >= _deal_price) and (tickvolume > 0) and open_tickvolume_flag and not makedealflag:
                        ...
                        if open_tickvolume_flag:
                            tick_state.append(0)
                        else:
                            tick_state.append(3)

                    wait_tick_num += 1    
                    if (wait_tick_num >= self.max_wait_tick_num) or makedealflag or (not open_tickvolume_flag):
                        if z - self.delay_tick_num < 0:
                            ...
                            open_tickvolume = pre_tickdf[z+1][volume_idx]
                        else:
                            if need_trade_num > 0:
                            ...
                            open_tickvolume = order_px_para[z-self.delay_tick_num][volume_idx]
                        open_tickvolume_flag = False if open_tickvolume <=1 else True
