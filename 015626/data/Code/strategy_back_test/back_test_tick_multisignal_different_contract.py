...
class TS_BACK_TEST:
    ...
    def __init__(self, signal_list, ticker='IC.CFE', start_date = None, end_date = None,
                 ...
                 max_open_value = None, contract_kind = 'recent_month'):
        """
        ...
        :param contract: 用什么合约进行回测，recent_month表示默认的提前3天切换的合约，00,01,02,03表示不提前切换的当月次月当季次季合约
        ...
        assert contract_kind in ['recent_month', '00', '01', '02', '03']
        ...
        for sig_dict in self.signal_list:
            ...
            if len(signal_df.columns.tolist()) == 1:
                ...
                if md is None:
                    ...
                    if self.contract_kind != 'recent_month':
                        md_all = IO.read_data([start_time, end_time], columns=['PROD_ID'], alt='/data/user/015626/data/share/MD/CHINA_FUTURES/MINUTE/XQUANT_MINUTE/MD_SIF_TICK_TO_MINUTE_ALL_CONTRACT.h5')
                        md_all = md_all[md_all['PROD_ID'] == self.ticker].reset_index(level = 1).drop(['PROD_ID'], axis = 1).rename(columns = {'Ticker': 'contract_00'})
                        md_all = md_all.groupby(md_all.index).apply(lambda x:x.iloc[int(self.contract_kind)])
                        md = md.drop(['contract_00'], axis = 1).join(md_all, how = 'left')
