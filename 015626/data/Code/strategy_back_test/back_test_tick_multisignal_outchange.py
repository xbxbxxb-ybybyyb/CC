...
class TS_BACK_TEST:
    ...
    def __init__(self, signal_list, ticker='IC.CFE', start_date = None, end_date = None,
        ...
        :param strict_close_pos: 是否严格平仓，默认为False，表示按照信号与仓位的对应关系来平仓。
                                 True的话并且当前信号小于进场阈值时:上一分钟未完成的平仓，如果按照当前分钟是不交易，或者继续平仓，但是平仓数量小于未完成平仓，
                                 则继续进行上一分钟的未完成平仓，如果当前分钟是继续平仓，平仓数量大于未完成平仓，或者开仓，则维持不变。
        ...
        self.strict_close_pos = strict_close_pos
        ...
        global dt_idx, raw_idx, contract_00_idx, open_idx, close_idx ,low_idx ,vwap_idx ,twap_idx ,pos_price_idx ,pos_min_idx ,pos_max_idx ,close_state_idx
        ...
        close_state_idx = columns_list.index('close_state')
        ...
        for sig_dict in self.signal_list:
            ...
            for k,v in pos_dict.items():
                if v[0] > 0:
                    open_t = k[0]
                    break
            assert open_t > 0, 'wrong pos_dict'
            ...
            signal_df['close_state'] = (abs(signal_df['raw']) < open_t).shift(1).fillna(False)
    ...
    def back_test_singleday(self, date):
        ...
        unfinished_num_pre_per_sig = [0] * len(signal_date_list)
        for i in range(1, date_signal_lenth):
            ...
            raw_signal_persig = []
            target_pos_persig = []
            for p in range(len(signal_date_list)):
                sig = signal_date_list[p]
                hold_num = sig['hold_num']
                pos_min = sig['signal'][i][pos_min_idx]
                pos_max = sig['signal'][i][pos_max_idx]
                close_state = sig['signal'][i][close_state_idx]
                trade_num_this_sig = self.get_trade_num(hold_num, pos_min, pos_max, nowtime, stop_loss_flag)
                unfinished_num_pre_this_sig = unfinished_num_pre_per_sig[p]
                if self.strict_close_pos and close_state and unfinished_num_pre_this_sig * hold_num < 0:
                    if trade_num_this_sig == 0 or unfinished_num_pre_this_sig / trade_num_this_sig > 1:
                        trade_num_this_sig = unfinished_num_pre_this_sig
            ...
            if need_trade_num == 0:#此分钟不交易
                ...
                # restotal_vol = need_trade_num - dealtotal_vol
                # # print(nowtime, need_trade_num, dealtotal_vol, restotal_vol)
                # # print(deal_price_list)
                # # print(deal_volume_list)
                # if restotal_vol != 0:
                #     _wprice = np.dot(deal_price_list, deal_volume_list) / np.sum(deal_volume_list)
                #     deal_vol = abs(restotal_vol)
                #     deal_price_list.append(_wprice)
                #     deal_volume_list.append(deal_vol)
                #     open_contract_num -= deal_vol

                #     open_value = _wprice * self.face_value * deal_vol
                #     open_fee = open_value * self.c_rate
                #     # open_value_intraday += (open_value + open_fee)
                #     # open_value_intratrade += (open_value + open_fee)
                #     if now_hold_num * need_trade_num >= 0:# 说明是开仓
                ...
                unfinished_num_pre_per_sig = list(np.array(need_trade_num_persig) - np.array(deal_vol_per_sig))
                # print(nowtime, dealtotal_vol, need_trade_num_persig, deal_vol_per_sig, unfinished_num_pre_per_sig)
                now_hold_num_persig = []
                for i2 in range(len(signal_date_list)):
