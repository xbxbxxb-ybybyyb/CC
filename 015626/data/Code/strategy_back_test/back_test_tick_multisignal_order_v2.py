...
class TS_BACK_TEST:
    ...
    def __init__(self, signal_list, ticker='IC.CFE', start_date = None, end_date = None,
                 ...
                 max_open_value = None, volume_limit_ratio_inorder = 0.1, volume_per_order = 3):
        """
        ...
        self.volume_limit_ratio_inorder = volume_limit_ratio_inorder
    ...
    def back_test_singleday(self, date):
        ...
        tickdf['flag_volume'] = (tickdf['TotalVolumeTrade'].rolling(120, min_periods = 60).sum() * self.volume_limit_ratio_inorder).apply(lambda x:max(np.floor(x), 20))
        tickdf['commission_num'] = tickdf['TotalVolumeTrade'].rolling(self.max_wait_tick_num * 2, min_periods = self.max_wait_tick_num).mean().apply(lambda x:np.floor(x))#.apply(lambda x:max(min(np.floor(x), self.volume_per_order), 1))
        ...
        for i in range(1, date_signal_lenth):
            ...
            if need_trade_num == 0:#此分钟不交易
                ...
                temp_commission_num = pre_tickdf[0][commission_num_idx]
                if tick_seq < 10:
                    temp_commission_num = max(temp_commission_num, 3)
                else:
                    temp_commission_num = max(min(temp_commission_num, self.volume_per_order), 1)
                open_commission_num = max(min(temp_commission_num, open_flag_volume - abs(dealtotal_vol), open_contract_num), 0)
                ...
                for z in range(len(order_px_para)):
                    ...
                    if wait_tick_num >= self.max_wait_tick_num or open_commission_num2 == 0:
                        if z - self.delay_tick_num < 0:
                            ...
                            temp_commission_num = pre_tickdf[z+1][commission_num_idx]
                            if tick_seq < 10:
                                temp_commission_num = max(temp_commission_num, 3)
                            else:
                                temp_commission_num = max(min(temp_commission_num, self.volume_per_order), 1)
                            ...
                            temp_commission_num = order_px_para[z-self.delay_tick_num][commission_num_idx]
                            if tick_seq < 10:
                                temp_commission_num = max(temp_commission_num, 3)
                            else:
                                temp_commission_num = max(min(temp_commission_num, self.volume_per_order), 1)
