...
class TS_BACK_TEST:
    ...
    def __init__(self, signal_list, ticker='IC.CFE', start_date = None, end_date = None,
        ...
        global dt_idx, raw_idx, contract_00_idx, close_idx ,pos_price_idx ,pos_min_idx ,pos_max_idx ,open_filter_idx
    ...
    def prepare_data(self):
        ...
        for sig_dict in self.signal_list:
            ...
            if len(signal_df.columns.tolist()) in [1, 2]:
                ...
                if md is None:
                    md = IO.read_data([start_time, end_time], columns=['contract_00', 'close', 'twap'],
    ...
    def back_test_singleday(self, date):
        ...
        tickdf = pd.read_csv('/data/user/015626/data/share/MD/CHINA_FUTURES/TICK/STOCK_INDEX_FUTURE/%s/%s.csv' % (contract, date), index_col=0, parse_dates=True)[['Buy1Price','Sell1Price','Buy1OrderQty','Sell1OrderQty','TotalVolumeTrade','LastPx']]
        ...
        tickdf['commission_num'] = tickdf['TotalVolumeTrade'].rolling(self.max_wait_tick_num * 2, min_periods = self.max_wait_tick_num).mean().apply(lambda x:max(min(np.floor(x*0.2), self.volume_per_order), 1))
            ...
            if need_trade_num == 0:#此分钟不交易
                ...
                for z in range(len(order_px_para)):
                    ...
                    if round(open_price,1) >= _deal_price and tickvolume > 0 and open_commission_num > 0 and orderbook_qty > 0:
                        ...
                        get_vol = np.floor(max(tickvolume, orderbook_qty) * 0.5)
