...
class TS_BACK_TEST:
    ...
    def __init__(self, signal_list, ticker='IC.CFE', start_date = None, end_date = None,
                 ...
                 max_open_value = None, close_th = None, close_draw_down_th = None, close_volatility_th = None):
        """
        ...
        :param close_th close_draw_down_th close_volatility_th 为趋势策略出场条件，其中close_draw_down_th close_volatility_th可以单独设置
                为None表示不作用
        ...
        if self.close_volatility_th is not None or self.close_draw_down_th is not None:
            assert self.close_th is not None
        ...
        global dt_idx, raw_idx, contract_00_idx, open_idx, close_idx ,low_idx ,vwap_idx ,twap_idx ,pos_price_idx ,pos_min_idx ,pos_max_idx ,open_filter_idx, volatility_flag_idx
        ...
        for i in range(1, date_signal_lenth):
            ...
            if pre_hold_num != 0 and self.close_volatility_th is not None:
                if not last_volatility_flag:
                    if pre_hold_num > 0 and last_sig < self.close_th:
                        _close_flag += 'volitity'
                    elif pre_hold_num < 0 and last_sig > -1 * self.close_th:
                        _close_flag += 'volitity'
                    
            if pre_hold_num != 0 and self.close_draw_down_th is not None:
                assert len(vwap_list_in_trade) > 0
                if pre_hold_num > 0:
                    if last_vwap / np.nanmax(vwap_list_in_trade) - 1 < -1 * self.close_draw_down_th and last_sig < self.close_th:
                        _close_flag += 'draw_down'
                else:
                    if last_vwap / np.nanmin(vwap_list_in_trade) - 1 > self.close_draw_down_th and last_sig > -1 * self.close_th:
                        _close_flag += 'draw_down'
...
signal_list = [{'signal':signal1,'pos_dict':pos_dict1,'cash':initial_cash1, 'filter_series':filter_series}]

name = f'{name_suffix}_{factor_name}_{initial_cash1/1e8}_{sig_ot_th2}_{draw_down_th}_{ap_st_ot_th}'
a = v_trend(signal_list, ticker=ticker, start_date=back_test_sdate, end_date=back_test_edate, tickslippage = 0.8, 
              ...
              close_th = sig_ot_th2, close_draw_down_th = draw_down_th, close_volatility_th = ap_st_ot_th)
result = a.back_test()
'''