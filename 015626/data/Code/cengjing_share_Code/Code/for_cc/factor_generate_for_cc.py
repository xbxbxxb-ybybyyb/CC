...
end_date_standard = '20210830' # 因子的截止日期
start_date_standard = '20160101' # 因子的开始日期
days_interval = 120 #每次的时间间隔

save_path = '/data/user/015626/data/share/factor/factor_gp/%s_prod/cfg_fullhistory/' % kind

import sys
sys.path.insert(4,'/data/user/015626/data/share/Code/factor_rewrite/factor_framework') # 框架路径
sys.path.insert(4,'/data/user/015626/data/share/Code/factor_gp/%s_factor' % str.lower(kind)) # 因子代码路径
...
fs = [f for f in os.listdir('/data/user/015626/data/share/Code/factor_gp/%s_factor' % str.lower(kind)) if f.endswith('.py')]
for f in fs:
