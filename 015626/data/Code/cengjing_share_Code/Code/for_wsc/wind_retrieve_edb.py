import sys
sys.path.insert(4, 'B://group/800466/trade/overnight/code/')
...
def retrieve_edb_helper(last_num_days=20):
    ...
    cache_path = os.path.join(trade_root, 'cache', 'edb.h5')
    cache = pd.read_hdf(cache_path)
    edb_pd = edb_pd.loc[~edb_pd.index.isin(cache.index)]
    cache.append(edb_pd).sort_index().to_hdf(cache_path, 'edb')
    return edb_pd
