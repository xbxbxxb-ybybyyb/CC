...
def get_continuous_df(date, df_list, variety_list, instrument_type):
    ...
    for i in range(len(variety_list)):
        ...
        df_continuous_data = df_continuous_data.between_time('9:30:00','14:56:00')
...
class DataCenter(object):
    ...
    def __init__(self, variety, data_type, instrument_type, data_dict, start_date, end_date, days_past):
        ...
        def get_start_date(sdt, days_past):
            date_list = sorted(get_trading_days('19900101',sdt))
            return date_list[-days_past-1]

        self.__data_type = data_type
        self.__instrument_type = instrument_type
        self.__start_date = get_start_date(start_date, days_past)
        self.__end_date = end_date
        self.__data_dict = data_dict
        self.__days_past = days_past
        ...
        self.__minute_index_path = '/data/group/800002/FutureTrader/test/MD/CHINA_INDEX/MINUTE'
        # self.__minute_future_path = '/data/group/800002/FutureTrader/test/data_zeshi'
        # self.__minute_index_path = '/data/group/800002/FutureTrader/test/data_zeshi'
        self.__minute_stock_path = '/data/group/800002/FutureTrader/test/MD/CHINA_STOCK/MINUTE'
        self.__index_weights_path = '/data/user/012913/IndexFuture/data_center/stock_weights.pickle'
        self.__minute_industry_path = '/data/group/800002/FutureTrader/test/MD/CHINA_INDUSTRY/MINUTE'
    ...
    def get_industry_data_dict(self):
        return self.__industry_data
    ...
    def load_stock_data(self):
        ...
        def check_all_na(df_all):
            date_list = [i.strftime('%Y%m%d') for i in np.unique(df_all.index.date)]
            for d in date_list:
                assert not df_all.loc[d].isna().all().all(), 'Empty Data on {}'.format(d)
        ...
        for i in fields:
            exec("df_temp = pd.DataFrame(np.transpose(data_{}),index=self.stock_time_index,columns=stock_list).astype('float')".format(i, i))
            exec("check_all_na(df_temp)")
            exec("self.get_stock_data()['{}'] = df_temp".format(i))

    def load_industry_data(self):
        industry_code_list = get_industry_code_list()
        assert self.__data_dict.get('Industry_Data') is not None

        fields = self.__data_dict.get('Industry_Data')

        data_list = [IO.read_data([self.__start_date, self.__end_date+'235959'], columns=fields, alt='{}/{}.h5'.format(self.__minute_industry_path, symbol)).unstack() for
                     symbol in industry_code_list]

        self.__industry_data = {'industry_{}'.format(i):pd.concat(data_list,axis=1,join='inner')[i] for i in fields}
    ...
    def load_data(self):
        ...
        if self.__data_dict.get('Index_Id') is not None:
            ...
            self.__index_data = pd.concat(index_data_list,axis=1,join='inner')
        ...
        if self.__data_dict.get('Other_Variety') is not None:
            ...
            for k,v in other_variety_dict.items():
                ...
                df_temp_variety = select_data_by_univ(data=self.__future_data_dict[k][v], variety=k, instrument_type=self.__instrument_type).reset_index().set_index('dt')[v]
                df_temp_variety.columns = ['{}_{}'.format(i,k) for i in v]
                other_variety_data_list.append(df_temp_variety)

            self.__other_variety_data = pd.concat(other_variety_data_list,axis=1,join='inner')
        ...
        # load industry data
        if self.__data_dict.get('Industry_Data') is not None:
