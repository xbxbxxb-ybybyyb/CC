...
s = FactorData()
_dlist = [x.strftime('%Y%m%d') for x in pd.date_range('20150101', dt.datetime.now().date().strftime('%Y%m%d'), freq='20D')]
WIND_AShareEODPrices = s.get_factor_value('WIND_AShareEODPrices',factors = ['TRADE_DT','S_INFO_WINDCODE','S_DQ_ADJFACTOR'], trade_dt=['>=%s' % _dlist[-1]]) 
for i in range(len(_dlist)-1,0,-1):
    WIND_AShareEODPrices = WIND_AShareEODPrices.append(s.get_factor_value('WIND_AShareEODPrices',factors = ['TRADE_DT','S_INFO_WINDCODE','S_DQ_ADJFACTOR'], trade_dt=['>=%s' % _dlist[i-1],'<%s' % _dlist[i]]))
WIND_AShareEODPrices = WIND_AShareEODPrices.append(s.get_factor_value('WIND_AShareEODPrices',factors = ['TRADE_DT','S_INFO_WINDCODE','S_DQ_ADJFACTOR'], trade_dt=['>=%s' % '20141210','<%s' % _dlist[0]]))
del(s)
# »ñÈ¡¹ÉÆ±µ±ÈÕµÄadjfactor
def get_adjfactor(stock, date):
    df = WIND_AShareEODPrices[WIND_AShareEODPrices['TRADE_DT'] == str(date)]
...
if __name__ == '__main__':
    ...
    start_date, end_date = '20170101', '20190501'
