...
para_path = f'/data/group/800466/trade/Arrow/para/Arrow_{next_tdate}'
path_in_sim = f'/data/cppParam/ArrowCppStrategy/Arrow_{next_tdate}'
...
def get_md_params_json():
    ...
    md_params_dict = {
        "ip": "168.9.100.93",
        "port": 18088,
        "user": "USERATSQUANTUDP12",
        "password": "x7E_+.nqMHJh",
        "backup": [
            {
                "ip": "168.9.65.25",
                "port": 18088
        ...
        "interface_ip": "100.68.84.39"
    }

    output_file = open(os.path.join(arrow_udp_path, 'arrow-udp-config-168.11.224.138.json'), 'w')
    json.dump(md_params_dict, output_file, indent = 4, ensure_ascii = False)
    output_file.close()

    udp_front_dict = {'path':os.path.join(path_in_sim,'md_params', 'arrow_udp', 'arrow-udp-config-168.11.224.138.json')}
...
params_dir = f'/data/group/800466/trade/Arrow/para/Arrow_{today}/strategy_params/arrow_strategy/'
...
if len(position) > 0:
    arrow_portfolio_sell = pd.DataFrame({'证券代码':position.证券代码, 
                                    '买入交易账户':[account_num] * len(position), 
                                    '卖出交易账户':[account_num] * len(position), 
...
arrow_portfolio_buy = pd.DataFrame({'证券代码':arrow_portfolio_buy.Ticker, 
                                '买入交易账户':[account_num] * len(arrow_portfolio_buy), 
                                '卖出交易账户':[account_num] * len(arrow_portfolio_buy), 
...
#     stk_need_sell = ','.join(get_txn_summary_buy(end_date)['证券代码'].tolist())
    ...
    fake_df = pd.DataFrame({'证券代码':fake_stk_list, 
                                        '买入交易账户':[account_num] * len(fake_stk_list), 
                                        '卖出交易账户':[account_num] * len(fake_stk_list), 
