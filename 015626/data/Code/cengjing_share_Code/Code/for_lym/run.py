...
calc_count = 0
while len(factors) > 0 and calc_count < 5:
    ...
    factors_done = [x[:-4] for x in os.listdir('./factors/factor_raw/') if x.endswith('.pkl')]
...
model_date_dict = {'arrow_prod':'20221206'}
...
# with open('./setting/params.json') as f:
...
buy_list = final_score[final_score > threshold].nlargest(daily_max_num).reset_index().Ticker.to_frame()
if len(buy_list) > daily_min_num:
	buy_list.to_csv('trade_list.csv', index = False)
# num = len(buy_list)
# blacklist_filtered = [x for x in buy_list if x in blacklist]
# if len(blacklist_filtered) > 0:
    # pd.DataFrame(blacklist_filtered).to_csv('./blacklist_filtered.csv', index = False, header = 0)

# buy_list = [x for x in buy_list if x not in blacklist]
