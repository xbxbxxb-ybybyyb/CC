...
class SIF_Factor_Test:
    def __init__(self, _df,in_t = None, out_t = None, factor_name=None, future_kind='contract_00', data_kind ='future',  factor_kind='5min', layers=4,
                ...
                if len(df.columns.tolist()) == 1:
                    if factor_kind == '1min':
                        ...
                        df = df.join(origindata, how = 'inner')
                            ...
                            origindata_full = pd.read_hdf('/dfs/group/800466/warehouse/prod/MD/CHINA_COMMODITY/rets/ret15/' + self.ticker + '.h5')
                            origindata = origindata_full.loc[str(starttime) : str(endtime)]
                            df = df.join(origindata, how = 'inner')
                    elif factor_kind == '30min':
                            datapath = '/dfs/group/800466/warehouse/prod/MD/CHINA_COMMODITY/' + 'MINUTE_30/INSAMPLE/' 
                            origindata_full = pd.read_hdf('/dfs/group/800466/warehouse/prod/MD/CHINA_COMMODITY/rets/ret30/' + self.ticker + '.h5')
                            origindata = origindata_full.loc[str(starttime) : str(endtime)]
                            df = df.join(origindata, how = 'inner')
...
# def generate_factor(ticker_list, data_dict, factor_obj):
#     input_list = prepare_data(ticker_list, data_dict, factor_obj)
#     with Pool(24) as pool:
#         holder = pool.map(partial(calculate_factor, factor_obj=factor_obj), input_list)
#     factor_dict = {ticker: factor for ticker, factor in zip(ticker_list, holder)}
#     return factor_dict


def generate_factor(ticker_list, data_dict, factor_obj, n_jobs=24):
    input_list = prepare_data(ticker_list, data_dict, factor_obj)
    if n_jobs > 1:
        with Pool(24) as pool:
            holder = pool.map(partial(calculate_factor, factor_obj=factor_obj), input_list)
    else:
        holder = [calculate_factor(data, factor_obj) for data in input_list]
    factor_dict = {ticker: factor for ticker, factor in zip(ticker_list, holder)}
    return factor_dict
...
def calculate_result(ticker_factor, st, et, ret_layers_min):
    ticker, factor = ticker_factor
    aaa = SIF_Factor_Test(factor, ticker=ticker, return_price_kind='twap', starttime=st, endtime=et, layers=4, ret_layers_min=ret_layers_min, save_image=False, show_image=False, savepath='')
    c, stats, pd_res30, pd_res30_20min = aaa.draw_result()
    c_ret_daily = c['ret'].groupby(c.index.date).sum()
    c_ret_daily.index = pd.to_datetime(c_ret_daily.index)
...
def generate_result(ticker_list, factor_dict, start_time, end_time, ret_layers_min):
    input_list = [(ticker, factor_dict[ticker]) for ticker in ticker_list]
    with Pool(24) as pool:
        holder = pool.map(partial(calculate_result, st=start_time, et=end_time, ret_layers_min=ret_layers_min), input_list)
    # holder = [calculate_result(item, st=start_time, et=end_time, ret_layers_min=ret_layers_min) for item in input_list]
    result_dict = {ticker: result for ticker, result in zip(ticker_list, holder)}

    factor_name = factor_dict[ticker_list[0]].columns[0]
