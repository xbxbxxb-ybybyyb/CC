...
def executor(trade_date=None, max_workers=24, tag='factors', factor_mode = 't'):
    ...
    pred_raw_df = mv_dict['long']
    short_pred_raw_df = mv_dict['short']

    final_score = pred_raw_df['stack']
    short_final_score = short_pred_raw_df['stack']
    late_trade_list = short_final_score[short_final_score < late_trade_threshold].index.get_level_values(1).tolist()
    ...
    short_pred_raw_df.reset_index(level = 0, drop = True).to_csv(os.path.join(short_model_value_path, '%s.csv' % str_date))
