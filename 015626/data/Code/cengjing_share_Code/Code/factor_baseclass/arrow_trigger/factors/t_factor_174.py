...
class t_factor_174(FactorGenerator):
    def __init__(self, *args, **kwargs):
        data_mode = 't'
        required_columns = ['transaction', 'order', 'order_raw']
        super(t_factor_174, self).__init__(*args, data_mode = data_mode, required_columns=required_columns, **kwargs)
    ...
    def on_bar(self, df):
        ...
        for stk in df['universe']:
            ...
            if stk.endswith('SH'):
                ...
                factor[stk] = -1 * len(data_raw) / len(data)
            else:
                data_txn = data_txn[(data_txn.TradeType == 1) & (data_txn.TradeBSFlag == 1)]
                factor[stk] = -1 * len(data_txn) / len(data)
