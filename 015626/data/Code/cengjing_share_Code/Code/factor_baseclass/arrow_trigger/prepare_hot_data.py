...
class PrepareHotData:
    def __init__(self, ref_date=None):
        ...
        self.trigger_dt_dict = univ[univ['dt'] == self.ref_date].set_index('Ticker')['trigger_dt'].to_dict()
    ...
    def get_all(self, kind = 'history'):
        ...
        if kind == 'history':
            ...
            _data_root = os.path.join(today_data_root, datetime.datetime.now().strftime('%Y%m%d'), 'data')
        all_data_dict = {}
        for k,v in name_dict.items():
            ...
            for stk in self.universe:
                ...
                if len(stk_data) > 0:
                    ...
                    stk_data = stk_data[(stk_data['dt'] >= pd.to_datetime(f'{self.ref_date} 091500')) & (stk_data['dt'] <= self.trigger_dt_dict[stk])]
                    if len(stk_data) > 0:
                        if k == 'transaction':
