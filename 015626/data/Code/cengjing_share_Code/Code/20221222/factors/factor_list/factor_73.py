...
for i in univ.values:
    ...
    if i[1][0] == '6':
        ...
        result.append([i[0], i[1], -(data_order_raw.OrderQty * data_order_raw.OrderPrice)[(data_order_raw.OrderType == 10) & (data_order_raw.OrderBSFlag == 1)].sum() / (data_order.OrderQty * data_order.OrderPrice)[data_order.OrderBSFlag == 1].sum()])
    else:
        ...
        result.append([i[0], i[1], -(data_txn.TradeQty * data_txn.OrderPrice)[(data_txn.TradeType == 1) & (data_txn.TradeBSFlag == 1)].sum() / (data_order.OrderQty * data_order.OrderPrice)[data_order.OrderBSFlag == 1].sum()])
