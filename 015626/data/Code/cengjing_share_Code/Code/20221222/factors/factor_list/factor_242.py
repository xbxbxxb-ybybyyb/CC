...
for i in univ.values:
    ...
    data_big_buy = data[(data.TradeMoney > 20000) & (data.TradeMoney < 100000) & (data.TradeBSFlag == 1)]
    data_big_sell = data[(data.TradeMoney > 20000) & (data.TradeMoney < 100000) & (data.TradeBSFlag == 2)]
    
    result.append([i[0], i[1], data_big_buy.TradeMoney.sum() / (data_big_buy.TradeMoney.sum() + data_big_sell.TradeMoney.sum())])
