# 回测fak单，slippage，ob_gear较小价格来发单
# -*- coding:UTF-8 -*-
import matplotlib
# matplotlib.use('Agg')
import pandas as pd
...
class TS_BACK_TEST:
    ...
    def back_test_singleday(self, date):
        ...
        for ii in range(5):
            tickdf = tickdf.round({f'Buy{ii + 1}Price':1, f'Sell{ii + 1}Price':1})
        idx_tickdf = tickdf.index
        tickdf_columns = tickdf.columns.tolist()
        buy1px_idx = tickdf_columns.index('Buy1Price')
        sell1px_idx = tickdf_columns.index('Sell1Price')
        volume_idx = tickdf_columns.index('TotalVolumeTrade')
        lastpx_idx = tickdf_columns.index('LastPx')
        midprice_idx = tickdf_columns.index('midprice')
        ...
        for i in range(1, date_signal_lenth):
            ...
            if need_trade_num == 0:#此分钟不交易
                ...
                for z in range(len(order_px_para)):
                    ...
                    if z >= 1:
                        if need_trade_num > 0:
                            z_1_pv = {order_px_para[z-1][tickdf_columns.index(f'Sell{w}Price')]:order_px_para[z-1][tickdf_columns.index(f'Sell{w}OrderQty')] for w in range(1,6)}
                        else:
                            z_1_pv = {order_px_para[z-1][tickdf_columns.index(f'Buy{w}Price')]:order_px_para[z-1][tickdf_columns.index(f'Buy{w}OrderQty')] for w in range(1,6)}
                        for ii in range(self.ob_gear):
                            ii += 1                        
                            _deal_price = order_px_para[z][tickdf_columns.index(f'Sell{ii}Price')] if need_trade_num > 0 else order_px_para[z][tickdf_columns.index(f'Buy{ii}Price')] * -1
                            _gear_vol = order_px_para[z][tickdf_columns.index(f'Sell{ii}OrderQty')] if need_trade_num > 0 else order_px_para[z][tickdf_columns.index(f'Buy{ii}OrderQty')]
                            if z > 1:
                                if need_trade_num > 0:
                                    _gear_vol = max(0, _gear_vol - z_1_pv.get(order_px_para[z][tickdf_columns.index(f'Sell{ii}Price')], 0))
                                else:
                                    _gear_vol = max(0, _gear_vol - z_1_pv.get(order_px_para[z][tickdf_columns.index(f'Buy{ii}Price')], 0))
                            if _gear_vol == 0:
                                continue
                            if (round(open_price,1) >= _deal_price):
                                ...
                                deal_vol = min(_gear_vol, open_contract_num)
...
# 子信号过滤示例
filter_series = pd.read_hdf('/data/group/800466/warehouse/prod/alpha/CHINA_FUTURES/MINUTE/IF_filters/minute_raw/futures_volume_if.h5')['futures_volume_if']
filter_series = (filter_series > 200).astype('int')

back_test_sdate, back_test_edate = 20230101, 20231201

ticker = 'IF.CFE'
factor_path = '/data/group/800466/warehouse/prod/alpha/CHINA_FUTURES/MINUTE/model/model_update/20230922_if_if_v7c/model_value/model_norm/20231127/pred_comb2.pkl'
factor = pd.read_pickle(factor_path)*2-1

factor_name = '20230922_if_if_v7c'
...
name = f'{factor_name}_{initial_cash1/1e8}'
a = TS_BACK_TEST(signal_list, ticker=ticker, start_date=back_test_sdate, end_date=back_test_edate, tickslippage = 0.6, save_signal_list = False, save_path=f'/data/user/015626/data/share/LOCAL_DATA/Mobius/backtest_1min/{ticker}/{name}',
