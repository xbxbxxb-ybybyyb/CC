...
# 先mask再rolling
class Crossing_Turns_ICIF_CC_IF_IH(FutureFactor):

    data_type = 'Future'
    days_past = 10
    data_dict = dict()
    data_dict['Continuous_Data'] = {'IH':['close', 'open', 'high', 'low', 'vwap']}
    normalize_size = 1200
...
#    num_range = '(-0.5, 1]'
    ...
    def calculate(self, data):
        ...
        hclose = (data['close_cont_IH'].values)[-1000:]
        hopen = (data['open_cont_IH'].values)[-1000:]
        hhigh = (data['high_cont_IH'].values)[-1000:]
        hlow = (data['low_cont_IH'].values)[-1000:]
        hvwap = (data['vwap_cont_IH'].values)[-1000:]
