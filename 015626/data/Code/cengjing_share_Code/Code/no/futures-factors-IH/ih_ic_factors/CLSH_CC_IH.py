...
class CLSH_CC_IH(FutureFactor):

    data_type = 'Future'
    instrument_type = 'recent'
    days_past = 6
    data_dict = dict()
    data_dict['Continuous_Data'] = {'IH':['close', 'share']}
    normalize_size = 242*3
    normalize_type = 'ts_rank'
    num_range = None
    
    def calculate(self, data):
        hclose = data['close_cont_IH'].iloc[-1001:].values
        temp1 = (np.where(np.diff(hclose)>0, 1, np.where(np.diff(hclose)<0, -1, 0)))
        
        hshare = data['share_cont_IH'].iloc[-1000:].values
