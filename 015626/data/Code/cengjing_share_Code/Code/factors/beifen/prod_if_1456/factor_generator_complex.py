...
class FactorGeneratorComplex:
    ...
    def __init__(self, factor_name='test', lookback_bars=5000, required_columns=None,
                 savepath='/data/user/015626/data/share/alpha/CHINA_FUTURES/MINUTE/IF_all_1456'):
    ...
    def prepare_hot_data(inst, start_date, end_date, use_cache = False, save_cache = False,ticker='IC.CFE',datakind='insample'):
        ...
        if use_cache:
            ...
            if ticker == 'IC.CFE':
                if datakind == 'insample':
                    cfg_stocks_data = read_pickle(os.path.join(insample_path,'IC_cfg_150416_200401_data1.pkl'))
                    cfg_hf_data = read_pickle(os.path.join(insample_path, 'IC_cfg_hf_data_150301_2004011.pkl'))
                elif datakind == 'outsample':        
                ...
                if datakind == 'insample':
                    cfg_stocks_data = read_pickle(os.path.join(insample_path,'IF_cfg_data_insample.pkl'))
                    cfg_hf_data = read_pickle(os.path.join(insample_path, 'IF_cfg_hf_data_insample.pkl'))
                elif datakind == 'outsample':        
