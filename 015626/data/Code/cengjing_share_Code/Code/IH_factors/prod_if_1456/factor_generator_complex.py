...
class FactorGeneratorComplex:
    ...
    def __init__(self, factor_name='test', lookback_bars=5000, required_columns=None,
                 savepath='/data/group/800466/warehouse/prod/alpha/CHINA_FUTURES/MINUTE/factors_base_oldframe_1456/IF_all/'):
    ...
    def prepare_hot_data(inst, start_date, end_date, use_cache = False, save_cache = False,ticker='IC.CFE',datakind='insample'):
        ...
        if use_cache:
            ...
            data_dict = {}
            insample_path = '/data/user/015626/data/warehouse/prod/MD/CHINA_FUTURES/MINUTE/'
            ...
            if datakind in ['insample','insample_ago']:
                index_data = read_pickle(os.path.join(insample_path, 'SPOT_DATA_120101_200901.pkl'))
            elif datakind == 'outsample':
                index_data = read_pickle(os.path.join(outsample_path, 'SPOT_DATA_2020.pkl'))
            
            if ticker == 'IC.CFE':
                if datakind == 'insample':
                    cfg_stocks_data = read_pickle(os.path.join(insample_path,'IC_cfg_150416_200401_data.pkl'))
                    cfg_hf_data = read_pickle(os.path.join(insample_path, 'IC_cfg_hf_data_150301_200401.pkl'))
                elif datakind == 'outsample':        
                    cfg_stocks_data = read_pickle(os.path.join(outsample_path,'IC_cfg_data_2020.pkl'))
                    cfg_hf_data = read_pickle(os.path.join(outsample_path, 'IC_cfg_hf_data_2020.pkl'))
                elif datakind == 'insample_ago':
                    cfg_stocks_data = read_pickle(os.path.join(insample_path,'IC_cfg_120101_150701_data.pkl'))
                    cfg_hf_data = read_pickle(os.path.join(insample_path, 'IC_cfg_hf_data_130701_150701.pkl'))
            elif ticker == 'IF.CFE':
                if datakind == 'insample':
                    cfg_stocks_data = read_pickle(os.path.join(insample_path,'IF_cfg_130101_200401_data.pkl'))
                    cfg_hf_data = read_pickle(os.path.join(insample_path, 'IF_cfg_hf_data_150301_200401.pkl'))
                elif datakind == 'outsample':        
                    cfg_stocks_data = read_pickle(os.path.join(outsample_path,'IF_cfg_data_2020.pkl'))
                    cfg_hf_data = read_pickle(os.path.join(outsample_path, 'IF_cfg_hf_data_2020.pkl'))
                elif datakind == 'insample_ago':
                    cfg_stocks_data = read_pickle(os.path.join(insample_path,'IF_cfg_130101_200401_data.pkl'))
                    cfg_hf_data = read_pickle(os.path.join(insample_path, 'IF_cfg_hf_data_130101_150701.pkl'))
                    
            data_dict = {**index_data,**cfg_stocks_data,**cfg_hf_data}
            for key in data_dict.keys():
                temp = data_dict[key].loc[start_date:end_date]
                data_dict[key] = temp.loc[temp.index.time != datetime.time(14,57)]
