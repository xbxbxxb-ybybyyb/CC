#目前实现的功能：
'''
(1)基于tick信号挂单进行回测，in_t表示进场信号，当tick上的信号超过此阈值时开始挂单，submit_order_t表示已经开始挂单后，信号如果回落至
in_t以下，但是仍在submit_order_t以上时，会保持挂单，直至持仓量+挂单量 = 目标持仓量。目前每次发单量仅支持1张。
(2)当信号低于out_t时，此时需要平仓出场，首先将所有开仓的挂单撤掉，然后挂单进行平仓，参数max_wait_tick_num_close_order表示平仓单经过
多少根tick没有成交时，对其进行撤单，根据最新盘口重新进行挂单, 此参数默认为None，表示平仓单不按照时间进行撤单.
(3)发单的价格为：买开买平时 = buy1price + tickslippage
             卖开卖平时 = sell1price - tickslippage
             tickslippage作为参数，是一个dict，表示信号范围对应的tickslippage
            当信号与持仓相反，需要挂单平仓时，tickslippage设定为10，达到市价平仓的效果
(4)成交规则为：当发单价格与当前盘口对价可以直接成交时，认定为市价单，以对价成交，
            其余委托单为限价单，成交价格为委托价
            限价单的成交规则为：按发单时间与成交估计的价量进行模拟撮合
                            1.现将成交估计的价格按照从小到大进行排序
                            2.遍历委托单，按照发单时间（此处还可优化，按照价格优先，时间优先进行便利）
                            3.当买单价格大于等于成交估计中的最小价格时，此委托单可以成交，同时更新成交估计中最小价格对应的成交量-1，
                            量为0时将此价格从成交估计中剔除
                            4.当卖单价格小于等于成交估计中的最大价格时，此委托单可以成交，同时更新成交估计中最大价格对应的成交量-1，
                            当量为0时将此价格从成交估计中剔除
(5)每日的目标仓位为initial_cash(初始资金) / cash_divnum(资金分成几份) / 近月合约当日开盘价 / 乘数
'''
...
def deal_estimate(vwap, amount, volume, lastpx, highpx, lowpx):
    if volume == 0:
        return []
    deal_price_list = []
    for x in [lastpx, highpx, lowpx]:
        if volume > 0:
            if x == x:
                deal_price_list.append((x, 1))
                amount = amount - x * 200 * 1
                volume -= 1
        else:
            break            
    vwap_buy1 = round(vwap - vwap % 0.2, 1)
    vwap_sell1 = vwap_buy1 + 0.2
    if volume > 0:
        vb1 = Symbol('vb1')
        vs1 = Symbol('vs1')
        result = solve([vb1 + vs1 - volume, vwap_buy1 * vb1 * 200 + vwap_sell1 * vs1 * 200 - amount],[vb1, vs1])
        vb1 = round(result[vb1],0)
        vs1 = volume - vb1
        deal_price_list.append((vwap_buy1, vb1))
        deal_price_list.append((vwap_sell1, vs1))
    deal_price_list.sort()
    return deal_price_list
...
class Order:
    def __init__(self, symbol=None, action=None, submit_price=None, fill_price=None, quantity=0, status=None,
                 ...
                 create_tick_pxlist=[], finish_tick_pxlist=[], hold_tick_num = 0):
        self.symbol = symbol
...
def make_deal(order_submitted_list, order_finished_list, buy1px, sell1px, lastpx, deal_px_vol, nowtime, now_sig):
    ...
    for k in range(len(order_submitted_list)):
        ...
        if _order.action in ['buy_open', 'buy_close'] and _order.status == 'submit':
            if plist[0] <= _order.submit_price:
                _order.status = 'fill'
                _order.fill_price = _order.submit_price
                ...
                res_v = deal_estimate[plist[0]] - _order.quantity
                if res_v > 0:
                    deal_estimate[plist[0]] = res_v
                else:
                    del(deal_estimate[plist[0]])
                    plist = plist[1:]
            ...
            if plist[-1] >= _order.submit_price:
                ...
                res_v = deal_estimate[plist[-1]] - _order.quantity
                if res_v > 0:
                    deal_estimate[plist[-1]] = res_v
                else:
...
class TS_BACK_TEST:
    ...
    def __init__(self, signal, in_t=0.9, submit_order_t=0.8, out_t=0.7,max_wait_tick_num_close_order = None, initial_cash=2e8, cash_divnum=10,
                 ticker='IC.CFE',
...
#         print('order_submitted_num:',order_submitted_num)
        ...
        if abs(pre_sig) >= self.in_t:
            ...
            put_order_status = 'buy_open_pos' if pre_sig > 0 else 'sell_open_pos'
            need_trade_num = target_pos - pre_hold_num - order_submitted_num
        elif abs(pre_sig) >= self.submit_order_t:
            if (put_order_status == 'buy_open_pos' and pre_sig > 0) or (put_order_status == 'sell_open_pos' and pre_sig < 0):
                target_pos = max_pos * np.sign(pre_sig)
                need_trade_num = target_pos - pre_hold_num - order_submitted_num
            else:
...
#         print(pre_hold_num, pre_sig, need_trade_num, put_order_status)
    ...
    def cancel_order(self,  order_submitted_list, order_finished_list, pre_sig, pre_hold_num, put_order_status, nowtime):
        ...
        if iscancel_openorder:
            ...
            cancel_action_list += ['sell_open']
        elif pre_sig < 0:
            cancel_action_list += ['buy_open']
            
        if iscancel_closeorder:
            cancel_action_list += ['buy_close', 'sell_close']
        elif pre_hold_num > 0:
            cancel_action_list += ['buy_close']
        elif pre_hold_num < 0:
            cancel_action_list += ['sell_close']
        ...
        if (self.max_wait_tick_num_close_order is not None) and (len(order_submitted_list) > 0):
            ...
            for k in range(len(order_submitted_list)):
                _order = order_submitted_list[k]
                if (_order.action in ['buy_close', 'sell_close']) and (_order.hold_tick_num >= self.max_wait_tick_num_close_order):
                    _order.status = 'cancel'
                    _order.finish_time = nowtime
                    _order.finish_sig = pre_sig
                    order_finished_list.append(_order)
    ...
    def back_test_singleday(self, date):
        ...
        max_pos = 1# np.floor(self.initial_cash / self.cash_divnum / (open_price_930 * self.face_value))
#         print(max_pos)
#         tickdf = pd.read_csv('/data/user/015626/data/share/MD/CHINA_FUTURES/TICK/STOCK_INDEX_FUTURE/%s/%s.csv' % (contract, date), index_col=0, parse_dates=True)
...
#             pre_sig = round(signal_date[i-1][raw_idx], 6)
            ...
            if len(order_submitted_list) > 0:
                # 判断成交
                order_submitted_list, order_finished_list, _fill_quantity_sum = make_deal(order_submitted_list, order_finished_list, buy1px, sell1px, lastpx, deal_px_vol, nowtime, now_sig)
...
#             print(nowtime, now_sig, now_hold_num, put_order_status)
#             for x in order_submitted_list:
#                 print(x.__dict__)
            need_trade_num, put_order_status = self.get_trade_num(order_submitted_list, now_sig,
                                                                                      max_pos, now_hold_num,
                                                                                      put_order_status, nowtime, date)
#             print(need_trade_num, put_order_status)
            ...
            if need_trade_num == 0:
                ...
                if need_trade_num > 0:
                    open_price = buy1px + tickslippage#sell1px#(buy1px + sell1px) / 2#buy1px + tickslippage
                else:
                    open_price = sell1px - tickslippage#buy1px#(buy1px + sell1px) / 2 # sell1px - tickslippage
                tickdt_time = nowtime
                ...
                if put_order_status in ['buy_open_pos', 'sell_open_pos']:  # 当需要挂单开仓时
                    ...
                    if need_trade_num_state == 1:
                        ...
                        need_trade_num -= 1
                        if open_price >= sell1px: # 报价超过盘口，按盘口价成交
                            _order.status = 'fill'
                            _order.fill_price = sell1px
                            _order.finish_sig = now_sig
                            _order.finish_time = nowtime
                            _order.finish_tick_pxlist = [buy1px, sell1px, lastpx]
                            order_finished_list.append(_order)
                        ...
                        need_trade_num += 1
                        if open_price <= buy1px: # 报价超过盘口，按盘口价成交
                            _order.status = 'fill'
                            _order.fill_price = buy1px
                            _order.finish_sig = now_sig
                            _order.finish_time = nowtime
                            _order.finish_tick_pxlist = [buy1px, sell1px, lastpx]
                            order_finished_list.append(_order)
        ...
        col_list = ['symbol', 'action', 'submit_price', 'fill_price', 'quantity', 'status', 'create_time',
                    'create_sig', 'finish_time','finish_sig', 'create_tick_pxlist', 'finish_tick_pxlist', 'hold_tick_num']
        finishdf = [
            [x.symbol, x.action, x.submit_price, x.fill_price, x.quantity, x.status, x.create_time, x.create_sig,
             x.finish_time, x.finish_sig, x.create_tick_pxlist, x.finish_tick_pxlist, x.hold_tick_num] for x in order_finished_list]
    ...
    def back_test(self):
        ...
        trade_df = pd.concat(rlist, axis=0).sort_values(by='create_time')

        totaltrade_df, total_order_counts, cancel_order_counts = self.trade_helper(trade_df)
        trade_df['hold_time_seconds'] = trade_df.apply(
            lambda x: get_timediff_seconds(x.create_time, x.finish_time), axis=1)
        trade_df['hold_time_minutes'] = trade_df['hold_time_seconds'] / 60
...
'''
sig = pd.read_pickle('/data/user/015626/data/share/MD/CHINA_FUTURES/TICK/sig_500ms.pkl')
sig = sig['pred']

ts = TS_BACK_TEST(sig, in_t=0.9, submit_order_t=0.8, out_t=0.7, max_wait_tick_num_close_order = 4,
                  initial_cash=3.5e8, cash_divnum=10,
                  c_rate=2.3 / 100000, ticker='IC.CFE',
                  start_date=20210927, end_date=20210927,
                  tickslippage_dict={(0, 0.3): 1,
                                     (0.3, 0.7): 0.4,
                                     (0.7, 0.8): 0.2,
                                     (0.8, 0.9): 0.2,
                                     (0.9, 100): 0},
                  trade_start_time=[9, 31], trade_end_time=[14, 56],
                  save_results = True,
                  save_path='/data/user/015626/data/share/LOCAL_DATA/Mobius/test20220710/new_closecancel/',
                  name_prefix='test')
# finishdf = ts.back_test_singleday(20210929)
result = ts.back_test()
'''