...
def reform_sh_order(df_order, df_transaction, append_cancel_orders):
    ...
    df_order_10 = df_order_10.merge(df_order_2[['OrderNO', 'OrderPrice']], left_on='OrderNO', right_on='OrderNO')
    df_order_10 = df_order_10.drop('OrderPrice_x', axis=1).rename(columns={'OrderPrice_y': 'OrderPrice'})
...
def get_h5_by_stock(stock):
    ...
    index_data_300 = IO.read_data(columns = ['close'], alt = '/sample/warehouse/prod/MD/MarketData/MD/CHINA_INDEX/MINUTE/000300.SH.h5')
    index_close_300 = index_data_300['close'].xs('000300.SH', level = 1)
    index_ret_300 = index_close_300.pct_change(1, fill_method=None)

    index_data_500 = IO.read_data(columns = ['close'], alt = '/sample/warehouse/prod/MD/MarketData/MD/CHINA_INDEX/MINUTE/000905.SH.h5')
    index_close_500 = index_data_500['close'].xs('000905.SH', level = 1)
    index_ret_500 = index_close_500.pct_change(1, fill_method=None)

    index_data_50 = IO.read_data(columns = ['close'], alt = '/sample/warehouse/prod/MD/MarketData/MD/CHINA_INDEX/MINUTE/000016.SH.h5')
...
#        print(para)
        ...
        pre6date = int(udt.get_trading_day_offset(str(date), -6)[0].strftime('%Y%m%d'))
        ...
        if os.path.exists(h5_path):
            IO.pd_hdf5_writer(result, h5_path, dataset = stock, append = True, data_columns=['dt', 'Ticker'])
        else:
            IO.pd_hdf5_writer(result, h5_path, dataset = stock, data_columns=['dt', 'Ticker'])
    except Exception as e:
        link_send_message(str(para) + str(e))
        print(para, result.dtypes.unique(), e)
...
rootpath = '/sample/warehouse/prod/MD/MarketData/LOCAL_DATA/CSV/MINUTE/CHINA_STOCK/tick_transaction_order_tominute_v4/'
h5_rootpath = '/sample/warehouse/prod/MD/MarketData/MD/CHINA_STOCK/MINUTE/'
weight_path = '/sample/warehouse/prod/MD/MarketData/MD/UNIVERSE/INDEXWEIGHT_CHINA_STOCK_DAILY_CSI.h5'

sdate,edate,cdate_list = check_update_date()

flag_rootpath = '/sample/warehouse/prod/MD/MarketData/LOCAL_DATA/FLAGS/'
...
while True:
    if (os.path.exists(flag_check_path1) == True) and (os.path.exists(flag_check_path2) == True):
...
for date in cdate_list:
    ...
    ashare_total = s.get_factor_value('WIND_AShareCapitalization', factors = ['S_INFO_WINDCODE','CHANGE_DT', 'FLOAT_A_SHR']).rename(columns = {'S_INFO_WINDCODE':'Ticker'}).set_index('Ticker')
    ashare_total['CHANGE_DT'] = ashare_total['CHANGE_DT'].astype('int')
    ashare_total.loc[['689009.SH'], 'FLOAT_A_SHR']  *= 10
    temp_fs = pd.DataFrame([20220601, 50900], index = ashare_total.columns, columns = ['689009.SH']).T
    temp_fs['CHANGE_DT'] = temp_fs['CHANGE_DT'].astype('int')
    temp_fs2 = pd.DataFrame([20210602, 50900], index = ashare_total.columns, columns = ['689009.SH']).T
    temp_fs2['CHANGE_DT'] = temp_fs2['CHANGE_DT'].astype('int')
    ...
    index_data_1000 = IO.read_data(columns = ['close'], alt = '/sample/warehouse/prod/MD/MarketData/MD/CHINA_INDEX/MINUTE/000852.SH.h5')
    ...
    weight_sh50 = IO.read_data([pre_32_date, date], columns = ['index_weight_sh50'], alt = '/sample/warehouse/prod/MD/MarketData/MD/UNIVERSE/INDEXWEIGHT_CHINA_STOCK_DAILY_CSI.h5')
...
flag_path_success = flag_root + str(edate) + '_' + 'CFG.success'
