...
def main():
    ...
    # 构建文件路径
    request_file = f"{base_path}/{sub_dir}/request.json"
    params_file = f"{base_path}/{sub_dir}/params.json"
    log_file = f"/dfs/user/666466/11_log/{sub_dir}/MobiusFactor.log"
    ...
    flag_path_start = flag_root + '/MobiusFactor.start'
    with open(flag_path_start, 'w') as file:
        pass

    l.sendMessage(f"[Mobius因子]Mobius Factor Calc Start With {tradedate} {offset}")
    # 启动Java程序
    ...
    flag_path_success = flag_root + '/MobiusFactor.success'
