...
def final_funcs():
#    trade_stop_delayed_time = delayed_time(trade_stop_time)
#    target_trigger_time = pd.Timedelta(hours=trade_stop_delayed_time.hour, minutes=trade_stop_delayed_time.minute)
...
#    lm.sendMessage(str(datetime.datetime.now()) + " end mdconstant job" )
    target_trigger_time = pd.Timedelta(hours=14, minutes=53)
    def get_stock_mdconstant_helper():
        lm.sendMessage(str(datetime.datetime.now()) + " start mdconstant job" )
        retrieve_mdconstant_helper()
        lm.sendMessage(str(datetime.datetime.now()) + " end mdconstant job" )
    scheduler(get_stock_mdconstant_helper, target_trigger_time, delay=2000)
