...
#         self.para_pos_dict = self.get_target_pos_dict(entry_para, exit_para)
        ...
        self.tickslippage_copy = tickslippage
    ...
    def back_test(self):
        ...
        for i in tqdm(range(1, length)):
            ...
            if now_date != pre_date:
                tickdf = pd.read_csv(os.path.join(tick_path, df.iloc[i]['WIND_CODE'][:6], str(now_date).replace('-','') + '.csv'))[['dt','Buy1Price','Sell1Price','TotalVolumeTrade']]
            ...
            if (nowtime.hour >= self.hour_t) and (nowtime.minute >= self.minute_t):
                target_pos = (0, 0)
                self.tickslippage = max(1.8, self.tickslippage)
            else:
                self.tickslippage = self.tickslippage_copy
...
#                 now_hold_dealcount.append(deal_count)
    ...
    def prepare_data(self):
        ...
        if len(self.signal_df.columns.tolist()) == 1:
            ...
            md = IO.read_data([start_time, end_time], columns=['WIND_CODE', 'open', 'high', 'low', 'close', 'vwap','volume'],
                              alt='/data/user/012245/warehouse/prod/MD/CHINA_FUTURES/MINUTE/SECONDMAIN/MD_CHINA_FUTURES_MINUTE_SECONDMAIN.h5')
            md = md.xs(self.ticker, level=1)
            
            df = self.signal_df.join(md, how='outer')
            df['volume'] = df['volume'].fillna(value = 0)
            for k in ['WIND_CODE', 'open', 'high', 'low', 'close', 'vwap']:
...
def get_tickresult(para):
    ...
    myticker = para[4]
    tkind = para[5]
    cashdict = {50000000:'',100000000:'_1yi', 500000000:'_5yi', 300000000:'_3yi'}
    
    
    factor = change_sig('/data/user/012315/share/ts/strategy/minute/res_20201211/%s/base_prod_nd/pred_comb_norm_nd_mod.h5' % (str.lower(myticker[:2])),change_ticker=myticker)    
    save_root_path = '/data/user/015626/data/share/factor/back_test/%s_ts/20201221_secondmain_max1.8_%s/' % (myticker[:2], tkind)
    ...
    sp = 0.6 if myticker == 'IC.CFE' else 0.4
    ...
    a,b,c,d = TS_BACK_TEST(factor.loc['20190501':'20200401'],price_kind='twap',ticker=myticker, slippage=sp, initial_cash=cash, save_path = save_path, name_prefix = name_prefix,
              pos_dict=pos_dict, capital_use_rate = 1,stop_loss = -0.005,deal_volume_ratio = 1, 
              tick_price_kind = tkind,
              tickslippage = tickslippage, max_wait_tick_num = max_wait_tick_num, delay_tick_num=delaynum).back_test() 


    name_prefix = '20170101_20200331' + cashdict[cash]
    save_path = os.path.join(save_root_path, sigtype + '_' + str(adj), name_prefix, h5[:-3] + '_%s_%s_%s' % (str(tickslippage), str(max_wait_tick_num), str(delaynum)))
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    a,b,c,d = TS_BACK_TEST(factor.loc['20170101':'20200331'],price_kind='twap',ticker=myticker, slippage=sp, initial_cash=cash, save_path = save_path, name_prefix = name_prefix,
              pos_dict=pos_dict, capital_use_rate = 1,stop_loss = -0.005,deal_volume_ratio = 1, 
              tick_price_kind = tkind,
              tickslippage = tickslippage, max_wait_tick_num = max_wait_tick_num, delay_tick_num=delaynum).back_test() 
    
    name_prefix = '20200401_20201130' + cashdict[cash]
    save_path = os.path.join(save_root_path, sigtype + '_' + str(adj), name_prefix, h5[:-3] + '_%s_%s_%s' % (str(tickslippage), str(max_wait_tick_num), str(delaynum)))
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    a,b,c,d = TS_BACK_TEST(factor.loc['20200401':'20201130'],price_kind='twap',ticker=myticker, slippage=sp, initial_cash=cash, save_path = save_path, name_prefix = name_prefix,
              pos_dict=pos_dict, capital_use_rate = 1,stop_loss = -0.005,deal_volume_ratio = 1, 
              tick_price_kind = tkind,
              tickslippage = tickslippage, max_wait_tick_num = max_wait_tick_num, delay_tick_num=delaynum).back_test() 
            
tslippagelist = [0.6, 0.8, 1.0, 1.2, 1.4, 1.6, 1.8, 2.0, 2.2]
waitlist = [0,4,10]
delaylist = [0]
cashlist = [500000000]
tickerlist = ['IC.CFE']
tick_price_kind_list = ['tickslippage']
paralist = []
for x in tslippagelist:
    for y in waitlist:
        for z in delaylist:
            for k in cashlist:
                for a1 in tickerlist:
                    for a2 in tick_price_kind_list:
                        paralist.append([x, y, z, k, a1, a2])
