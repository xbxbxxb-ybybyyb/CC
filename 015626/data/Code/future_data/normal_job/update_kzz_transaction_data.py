...
droplist = ['MDRecordID', 'MDReportID', 'MDStreamID', 'TradingPhaseCode', 'NumTrades', 
...
savepath = '/arch1/group/800466/warehouse/prod/MD/CHINA_CONVERTIBLE_BOND/Transaction/'
                    
def get_kzz_data_transaction(symbol, date):
    spath = os.path.join(savepath,symbol)
    if os.path.exists(os.path.join(spath, str(date) + '.csv')):
        return
    bd = BondData()
    result_min = bd.get_bond_data(symbol, "%s 090000000" % str(date), "%s 150000000" % str(date), 'TRANSACTION')
    del(bd)
    if len(result_min) == 0:
...
def get_kzz_data_by_date(date):
    ...
    for symbol in kzz_list:
        get_kzz_data_transaction(symbol, date)

sdate,edate,cdate_list = check_update_date(20170101,20210621)
