...
def get_cfg_hfdata(para):
    ...
    rootpath = '/arch0/group/800466/warehouse/prod/MD/CHINA_STOCK/Transaction_to_3s/CSV/ZZ500/'
...
#        transaction['minute'] = transaction.dt.map(lambda x: x.replace(second=0,microsecond=0))
        transaction['seconds_3s'] = transaction.dt.map(lambda x: x.replace(microsecond=0))
        index3s = pd.date_range(str(date)+'090000',str(date)+'150000', freq ='3S').tolist()
        transaction.loc[~transaction.seconds_3s.isin(index3s),'seconds_3s'] = np.nan
        transaction['seconds_3s'] = transaction['seconds_3s'].fillna(method = 'ffill')
        
        transaction = transaction[transaction.TradePrice != 0]
        selldf = transaction[transaction.TradeBSFlag == 2]
        sellorder_money = selldf.groupby(['seconds_3s', 'TradeBuyNo'])['TradeMoney','TradeQty'].sum().reset_index()
        ...
        sell_small_order = sell_small_order.groupby('seconds_3s').agg({'TradeBuyNo':'count', 'TradeMoney':'sum', 'TradeQty':'sum'}).rename(columns = {'TradeBuyNo':'sell_smallorder_count','TradeMoney':'sell_smallorder_money','TradeQty':'sell_smallorder_volume'})
        sell_mid_order = sell_mid_order.groupby('seconds_3s').agg({'TradeBuyNo':'count', 'TradeMoney':'sum', 'TradeQty':'sum'}).rename(columns = {'TradeBuyNo':'sell_midorder_count','TradeMoney':'sell_midorder_money','TradeQty':'sell_midorder_volume'})
        sell_big_order = sell_big_order.groupby('seconds_3s').agg({'TradeBuyNo':'count', 'TradeMoney':'sum', 'TradeQty':'sum'}).rename(columns = {'TradeBuyNo':'sell_bigorder_count','TradeMoney':'sell_bigorder_money','TradeQty':'sell_bigorder_volume'})
        sell_super_order = sell_super_order.groupby('seconds_3s').agg({'TradeBuyNo':'count', 'TradeMoney':'sum', 'TradeQty':'sum'}).rename(columns = {'TradeBuyNo':'sell_superorder_count','TradeMoney':'sell_superorder_money','TradeQty':'sell_superorder_volume'})

        selldf = selldf.groupby('seconds_3s').agg({'TradeMoney':'sum','TradeQty':'sum','TradePrice':'count','TradeSellNo':lambda x:len(x.unique())})
        ...
        buyorder_money = buydf.groupby(['seconds_3s', 'TradeBuyNo'])['TradeMoney','TradeQty'].sum().reset_index()
        ...
        buy_small_order = buy_small_order.groupby('seconds_3s').agg({'TradeBuyNo':'count', 'TradeMoney':'sum', 'TradeQty':'sum'}).rename(columns = {'TradeBuyNo':'buy_smallorder_count','TradeMoney':'buy_smallorder_money','TradeQty':'buy_smallorder_volume'})
        buy_mid_order = buy_mid_order.groupby('seconds_3s').agg({'TradeBuyNo':'count', 'TradeMoney':'sum', 'TradeQty':'sum'}).rename(columns = {'TradeBuyNo':'buy_midorder_count','TradeMoney':'buy_midorder_money','TradeQty':'buy_midorder_volume'})
        buy_big_order = buy_big_order.groupby('seconds_3s').agg({'TradeBuyNo':'count', 'TradeMoney':'sum', 'TradeQty':'sum'}).rename(columns = {'TradeBuyNo':'buy_bigorder_count','TradeMoney':'buy_bigorder_money','TradeQty':'buy_bigorder_volume'})
        buy_super_order = buy_super_order.groupby('seconds_3s').agg({'TradeBuyNo':'count', 'TradeMoney':'sum', 'TradeQty':'sum'}).rename(columns = {'TradeBuyNo':'buy_superorder_count','TradeMoney':'buy_superorder_money','TradeQty':'buy_superorder_volume'})

        buydf = buydf.groupby('seconds_3s').agg({'TradeMoney':'sum','TradeQty':'sum','TradePrice':'count','TradeBuyNo':lambda x:len(x.unique())})
    ...
    result.loc[str(date)+'093000':str(date)+'112959'].append(result.loc[str(date)+'130000':str(date)+'145959']).to_csv(filepath)
...
for ticker in ['IC.CFE']:
    ...
    paralist = get_target_list(ticker,20181228,20210517)
