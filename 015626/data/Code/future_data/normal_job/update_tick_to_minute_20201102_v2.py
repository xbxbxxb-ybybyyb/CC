...
@author: appadmin 增加了回测时要用到了三个volume字段以及midprice字段
"""
...
# Save Path
# NO '/' AT THE END !
save_path = '/data/user/015626/data/share/LOCAL_DATA/CSV/MINUTE/CHINA_FUTURES/tick_to_minute_for_backtest2/ALL_CONTRACT'

######################################################################################################################################
factor_number = 27
li = ['dt','tradingdate', 'open', 'close', 'high', 'low', 'value','volume', 'vwap', 'twap', 'position', 'ticktime']
li_factors = ['dt','OrderFlowImbalanceLv1','OrderFlowImbalanceRatioLv1', 'PriceMean', 'PriceVol', 'PriceSkew', 'PriceKurt', 'RetMean', 'RetVol', 'RetSkew', 'RetKurt', 'AbsDistance', 'VolumeMean', 'VolumeStd', 'HighVolumeCount', 'BidAskMean', 'BidAskVol', 'BASWeighted', 'BASSign', 'BASCorrV', 'TickCounts', 'LastMinuteMid', 'VolLtLp', 'VolLtVw', 'VolLtMid', 'VolGtLp', 'VolGtVw', 'VolGtMid']
import pandas as pd
import numpy as np
import time
                ...
                if count>0:
                    ...
                    if bl ==1 or count == 1:
                        ...
                        if count1 >= 2:
                            ...
                            if count1 > 2:
                                last_minute_price = data_dict_minute[count1-2][-1][last_px]
                                last_minute_vwap = data_dict_minute_concat[count1-2][8]
                                last_minute_mid = np.nanmean([data_dict_minute[count1-2][-1][s1p], data_dict_minute[count1-2][-1][b1p]])
                                vol_lt_lp = np.nansum(List_volume[(List_prce<last_minute_price)])
                                vol_lt_vw = np.nansum(List_volume[(List_prce<last_minute_vwap)])
                                vol_lt_mid = np.nansum(List_volume[(List_prce<last_minute_mid)])
                                vol_gt_lp = np.nansum(List_volume[(List_prce>last_minute_price)])
                                vol_gt_vw = np.nansum(List_volume[(List_prce>last_minute_vwap)])
                                vol_gt_mid = np.nansum(List_volume[(List_prce>last_minute_mid)])
                                
                                data_dict_factor_concat[count1-1][22] = vol_lt_lp
                                data_dict_factor_concat[count1-1][23] = vol_lt_vw
                                data_dict_factor_concat[count1-1][24] = vol_lt_mid
                                data_dict_factor_concat[count1-1][25] = vol_gt_lp
                                data_dict_factor_concat[count1-1][26] = vol_gt_vw
                                data_dict_factor_concat[count1-1][27] = vol_gt_mid
                            else:
                                data_dict_factor_concat[count1-1][22] = np.nan
                                data_dict_factor_concat[count1-1][23] = np.nan
                                data_dict_factor_concat[count1-1][24] = np.nan
                                data_dict_factor_concat[count1-1][25] = np.nan
                                data_dict_factor_concat[count1-1][26] = np.nan
                                data_dict_factor_concat[count1-1][27] = np.nan
                            ...
                            # Last Spread
                            data_dict_factor_concat[count1-1][21] = np.nanmean([data_dict_minute[count1-1][-1][s1p], data_dict_minute[count1-1][-1][b1p]])
...
pathlist = glob.glob('/data/user/015626/data/share/LOCAL_DATA/CSV/MINUTE/CHINA_FUTURES/tick_to_minute_for_backtest2/ALL_CONTRACT/*/*.csv')
downlist = ['/data/user/015626/data/share/MD/CHINA_FUTURES/TICK/STOCK_INDEX_FUTURE/' + x[-19:] for x in pathlist]
...
################################## -- update h5 -- #################################
rootpath = '/data/user/015626/data/share/MD/CHINA_FUTURES/for_backtest2/'
