...
def get_futuretick_bydate(sdate = None, edate = None):
    ...
    ic_list = ['IC'+str(a)+'.CFE' for a in range(1901,1913)]
    if_list = ['IF'+str(a)+'.CFE' for a in range(1901,1913)]
    ih_list = ['IH'+str(a)+'.CFE' for a in range(1901,1913)]
    wind_code_list = ic_list + if_list + ih_list    
    root_path = '/data/user/015626/data/share/future/fd_STOCK_INDEX_FUTURE'
    for i in range(len(cdate_list)):
        ...
        for ticker in wind_code_list:
           ...
           if delta > datetime.timedelta(1):
                ...
                df_yesterday = fd.get_future_data(maticker, yesterday1+"205500000", yesterday2 + "205500000",'TICK')
                df_today = fd.get_future_data(maticker, today1+"205500000", today2 + "205500000",'TICK')
                df = df_yesterday.append(df_today)
           else:
                df = fd.get_future_data(maticker, yesterday + "205500000", today + "205500000",'TICK')
...
get_futuretick_bydate(20180701, 20200101)