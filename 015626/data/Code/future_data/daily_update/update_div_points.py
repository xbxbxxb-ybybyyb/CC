...
idxdata = IO.read_data([start_date, end_date], columns=['close'], alt = '/data/group/800080/warehouse/prod/MD/CHINA_INDEX/DAILY/WIND/MD_CHINA_INDEX_DAILY_WIND.h5')

cdatelist = get_trading_date_range(start_date, end_date)
clist = [str(x)[:10].replace('-','') for x in cdatelist]

totalresult = pd.DataFrame()
for futurecode in ['IC','IF','IH']:
    if futurecode == 'IF':
        tmp = '000300.SH'
        aim = 'HS300'
    elif futurecode == 'IC':
