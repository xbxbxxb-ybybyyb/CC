...
def minute_flag_check(date,today):
    ...
    path5 = '/data/user/015626/data/share/LOCAL_DATA/FLAG/' + str(today) + '/' + str(today) + '_cfg_noon.success'
...
data_startdate = 20200701 # 生成的数据的起始日期
startdate, enddate = 20110101,21000101
pkl_savepath = '/data/user/015626/data/share/MD/CHINA_STOCK/prod_for_overnight/prod_pickle/'
flag_root = '/data/user/015626/data/share/LOCAL_DATA/FLAG/' + str(today_date).replace('-','') + '/'

if not os.path.exists(flag_root):
    os.makedirs(flag_root)
flag_path_start = flag_root + str(today_date).replace('-','') + '_' + '%s_cfg_and_mask_noondata_for_overnight.start' % ticker[:2]
...
# 读取今日股票数据
today_stock_mdata = pd.read_hdf('/data/user/015626/data/share/MD/CHINA_STOCK/prod_for_overnight/noon_minute_everyday/%s/%s_cfg_noon.h5' % (str(today_date).replace('-',''), str(today_date).replace('-',''))).astype('float').reset_index(level = 1)
...
# 生成mask
df20 = df20.loc[pd.to_datetime(str(data_startdate)):].sort_index()
a = df20.copy()
mask_dict_20 = {}
...
flag_path_success = flag_root + str(today_date).replace('-','') + '_' + '%s_cfg_and_mask_noondata_for_overnight.success' % ticker[:2]
