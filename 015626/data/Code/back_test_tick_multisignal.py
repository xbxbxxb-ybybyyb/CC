...
class TS_BACK_TEST:
    ...
    def __init__(self, signal_list, ticker='IC.CFE', start_date = None, end_date = None,
                 ...
                 stop_loss=-100, capital_use_rate = 3,  n_jobs = 24,
                 c_rate=2.5 / 100000, tickslippage = 1.2, vol_pertick = 1, 
                 delay_tick_num = 0,max_wait_tick_num = 4, no_opening_start_time = [14,30], closing_start_time = [14,54], 
                 trade_start_time = [9,38], trade_end_time = [14,56],
                 save_path='/data/user/', name_prefix=''):
        """
        :param signal_list: dict, 信号dataframe，index为分钟，如果只有一列，则认为此列为信号值，读取行情数据进行测试。
        ...
        global dt_idx, raw_idx, contract_00_idx, open_idx, close_idx ,low_idx ,volume_idx ,vwap_idx ,twap_idx ,pos_price_idx ,pos_min_idx ,pos_max_idx 
        ...
        volume_idx = columns_list.index('volume')
    ...
    def prepare_data(self):
        ...
        for sig_dict in self.signal_list:
            ...
            # 获取信号开始结束时间，获取行情数据
            start_time = int(str(signal_df.ix[[0]].index.values[0]).split('T')[0].replace('-', ''))
            end_time = str(signal_df.ix[[-1]].index.values[0]).split('T')[0]
    ...
    def get_trade_num(self, hold_num, pos_min, pos_max, now_time, stop_loss_flag):
        ...
        if (pos_min != 0) and (hold_num != 0) and (np.sign(hold_num) != np.sign(pos_min)):
            return 0 - hold_num
    ...
    def back_test(self):
        ...
        if self.save_path != None:
            ...
            daily_openvalue.to_csv(os.path.join(self.save_path, self.name_prefix + '_daily_openvalue.csv'), index=False)
