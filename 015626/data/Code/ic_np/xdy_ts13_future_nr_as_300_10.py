...
class xdy_ts13_future_nr_as_300_10(FutureFactor):
    ...
    def calculate(self, df):
        high = df['high_preadj'][-2391:].values
        factor = bk.move_max(high, 121, 60, axis = 0)[-2270:]
        factor = rolling_norm(factor, 3*242)[-1544:]
        factor = factor[15:] - factor[:-15]
        factor = bk.move_max(factor, 19, 9, axis = 0)[-1510:]
