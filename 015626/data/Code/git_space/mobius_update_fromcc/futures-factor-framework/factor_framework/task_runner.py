...
class TaskRunner(object):
    ...
    def __get_future_data(self, data_center, date_list, date):
        ...
        if data_center.get_data_dict().get('Continuous_Data') is not None:
            data_list.append(data_center.get_continuous_data_dict()[date].loc[date_list[0]:date_list[-1]])
    ...
    def run_factor_multi_day(self, factor, variety, data_center, start_date, end_date, ncore=24):
    ...
    def ts_rank(self, df1, d=4800):
        ...
        if d == 1:
            ...
            try:
                ...
                if isinstance(df1, pd.DataFrame):
                    output = pd.DataFrame(bk.move_rank(df1, window=d, min_count=1, axis=0),
                                          index=df1.index, columns=df1.columns)
                elif isinstance(df1, pd.Series):
                    output = pd.Series(bk.move_rank(df1, window=d, min_count=1, axis=0),
                                       index=df1.index, name=df1.name)
                return output
