# -*- coding: utf-8 -*-
"""
Created on Mon Aug 22 13:51:46 2022
...
class WBS_S_CC(FutureFactor):
    ...
    def calculate(self, data):
        ...
        w = data['WeightBuyOrderQtySumMean'].values[-30:] / data['WeightSellOrderQtySumMean'].values[-30:]
        #* data_dict['volume_300']
        df_s = np.nanstd(w, axis = 0)

        
        hclose = data['close'].iloc[-230:]
        hret = ts_pct_change(hclose, 1)
        hret[abs(hret)>10000] = np.nan
        hret = hret.ewm(8).mean().values[-1]# * weight[-1]
