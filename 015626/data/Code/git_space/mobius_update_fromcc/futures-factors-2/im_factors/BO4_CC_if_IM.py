# -*- coding: utf-8 -*-
"""
Created on Mon Dec  6 18:23:27 2021
...
class BO4_CC_if_IM(FutureFactor):
    ...
    def calculate(self, data):
        ...
        bool_df = stk_weight.gt(pd.Series(stk_weight.quantile(0.90, axis = 1)), axis=0)
        
        factor_raw = (data['buy_bigorder_money'].iloc[-107:].fillna(0)[bool_df]).values
        
        df_s = (ts_truncated_ema_span_1(factor_raw, 100, 20)*stk_weight)[-2:]

        hret = ts_pct_change(stk_close, 1)
        hret = (ts_truncated_ema_span_1(hret, 100, 25))[-2:]
