...
class MinuteDistanceAmountOLSBeta_IF_IH(FutureFactor):
    data_type = 'Future'
    instrument_type = 'recent'
    days_past = 1
    data_dict = {}
    data_dict['Index_Id'] = {'000016.SH': ['open', 'close', 'high', 'low', 'amount']}
    normalize_size = 20 * 237
    normalize_type = 'ts_rank'
    
    def calculate(self, data):
        lb = 20
        open_px = data['open_000016.SH'].values
        close = data['close_000016.SH'].values
        high = data['high_000016.SH'].values
        low = data['low_000016.SH'].values
        amount = data['amount_000016.SH'].values
        x = amount
