...
def send_link(message):
    LinkMessage(link_user_ids).sendMessage(str(message))
...
def str_date_parser(str_name):
    ...
    '''
    if any([isinstance(str_name, datetime.date), isinstance(str_name, datetime.datetime), isinstance(str_name, pd.Timestamp)]):
        return pd.Timestamp(str_name)
    if type(str_name) is int:
        str_name = str(str_name)
    if type(str_name) is str:
        if len(str_name) == 8:
            return pd.Timestamp(datetime.datetime.strptime(str_name, '%Y%m%d'))
        elif len(str_name) == 14:
            return pd.Timestamp(datetime.datetime.strptime(str_name, '%Y%m%d%H%M%S'))
        else:
...
def pred_helper(x_test,model_dict,pred='regression',check_time=False,return_itr=False,res_base_path=None):
...
def norm_3std(temp_factor):
    factor_prod = temp_factor.copy()
    factor_mean = factor_prod.mean() 
    factor_std = factor_prod.std()
    _up = factor_mean + 3 * factor_std
    _down = factor_mean - 3 * factor_std
    factor_clip = np_clip(factor_prod, _down, _up)
    return ((factor_clip - factor_clip.mean()) / factor_clip.std())
