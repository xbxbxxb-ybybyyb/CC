...
def get_arrow_black_list(four_factor, rule_data):
    ...
    d28 = data[(data['open_to_preclose']<0) & (data['filter_3']) & (data['last_day_close_to_preclose'] < -0.08) & (data['last_day_open_to_preclose'] < 0)]
...
def executor(trade_date=None, max_workers=24, tag='factors', factor_mode = 't'):
    ...
    if len(factor_input) == 0:
        pd.DataFrame().to_csv(os.path.join(factor_savepath, '%s_final_score.csv' % str_date))
        print('lenth of factor_input is 0')
        return
    ...
    pred_raw_df = mv_dict['long'].reset_index(level = 0, drop = True)
    short_pred_raw_df = mv_dict['short'].reset_index(level = 0, drop = True)
    ...
    final_score.to_csv(os.path.join(factor_savepath, '%s_final_score.csv' % str_date))
    ...
    factor_input.reset_index(level = 0, drop = True).to_csv(os.path.join(factor_savepath, '%s_factor_input.csv' % str_date))
    del(inst)
    return