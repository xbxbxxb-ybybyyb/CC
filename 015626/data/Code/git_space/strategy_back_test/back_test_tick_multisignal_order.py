...
# 增加期权 etf
# 增加多路径合并仓位回测
# 可以支持任意时间间隔的信号回测,但是计算的IC可能不准，需要最准确的ic需要自己传入量价数据
# 增加每日资金
import matplotlib
matplotlib.use('Agg')
# 全局设置 figure 面板颜色
...
def handle_holiday(df):
    ...
    t_mins_list = pd.date_range('09:30:00','09:59:59', freq='s').to_list()
...
class TS_BACK_TEST:
    ...
    def __init__(self, signal_list, ticker='IC.CFE', start_date = None, end_date = None,
                 ...
                 c_rate=0.00003, tickslippage = 1.2, 
                 ...
                 max_open_value = None, max_deal_volume_in_minute = 60, volume_per_order = 3, 
                 fee_fixed = 0, face_value = None, min_deal_vol_in_bar = None, ticker_kind='future',
                 tick_root_path = '/data/user/015626/data/share/MD/CHINA_FUTURES/TICK/STOCK_INDEX_FUTURE/', 
                 data_root_path = '/data/user/015626/data/share/MD/CHINA_FUTURES/MINUTE/XQUANT_MINUTE/MD_SIF_TICK_TO_MINUTE_RECENT_MONTH.h5'):
        """
        :param signal_list: 格式为[{'signal':signal1 * -1,'pos_dict':pos_dict1,'cash':initial_cash1}, {'signal':signal2,'pos_dict':pos_dict2,'cash':initial_cash2}]
                            ...
                            测试时不对信号值做任何处理，使用原始值。
                            需要开仓过滤时可以在dict中增加'filter_series'key，内容为[True,False]或者[0,1]，仅支持开仓过滤
        :param ticker: 交易品种
        :param start_date， end_date: 测试起始结束日期，为None就测全部时间
        :param stop_loss: 止损
        :param n_jobs: 并行使用多少个核
        :param c_rate: 交易费用比例，  fee_fixed是固定手续费，比如说每张几元， 这两个参数同时起作用，如果不用哪个需置为0
        ...
        :param max_deal_volume_in_minute: 每个bar的最大成交量
        :param volume_per_order: 每单最多发几张，当为1时表示twap算法
        :param max_open_value: 当日最大开仓金额，默认为None表示不限制，设置为1e8表示当日单边开仓超过1e8后不再进行开仓, 只平仓
        :param face_value: 面值，当为None时，内置IC、IF、IM、IH、MO、IO面值，非None时直接使用设置的参数
        :param min_deal_vol_in_bar: 每个bar至少成交几张，撮合不够的话剩余部分就用最后一根tick的对价成交
        :ticker_kind: 回测的品种，期货future 期权option
        :tick_root_path: 撮合的tick数据路径
        :data_root_path: 数据路径，指定每日撮合合约，close以及twap价格
        ...
        assert ticker_kind in ['future', 'option', 'etf']
        ...
        self.signal_list = copy.copy(signal_list)
        self.ticker = ticker
        face_value_dict = {'IC.CFE': 200,
                           ...
                           'MO.CFE': 100,
                           'IO.CFE': 100}
        self.face_value = face_value_dict[self.ticker] if face_value is None else face_value
        ...
        self.ticker_kind = ticker_kind

        self.tick_root_path = tick_root_path
        ...
        self.max_deal_volume_in_minute = max_deal_volume_in_minute
        self.volume_per_order = volume_per_order
        self.min_deal_vol_in_bar = min_deal_vol_in_bar
        ...
        global dt_idx, raw_idx, contract_00_idx, twap_idx, close_idx ,pos_price_idx ,pos_min_idx ,pos_max_idx ,open_filter_idx
        ...
        open_filter_idx = columns_list.index('open_filter')
    ...
    def prepare_data(self):
        if len(self.signal_list) > 1:
            ...
            for sig_dict in self.signal_list:
                ...
                merge_index |= set(signal_df.index.tolist())

            merge_index = sorted(list(merge_index))
            merge_index = pd.Index(merge_index, name = 'dt')
        ...
        for sig_dict in self.signal_list:
            ...
            md = None
            if len(signal_df.columns.tolist()) in [1, 2]:
                if len(signal_df.columns.tolist()) == 1:
                    signal_df.columns = ['raw']
                    signal_df['pos_cash'] = initial_cash
                if len(signal_df.columns.tolist()) == 2:
                    signal_df.columns = ['raw', 'pos_cash']
                if md is None:
                    if self.ticker_kind == 'etf':
                        md = IO.read_data([start_time, end_time],  select_str=f"'Ticker' == '{self.ticker}'", columns=['close', 'volume', 'amount'], alt = self.data_root_path)
                        md['twap'] = md['amount'] / md['volume'].replace(0, np.nan)
                        md['contract_00'] = self.ticker
                    else:
                        md = IO.read_data([start_time, end_time], columns=['contract_00', 'close', 'twap'], alt = self.data_root_path)
                    md = md.xs(self.ticker, level=1)
                signal_df = signal_df.join(md, how='outer').sort_index().fillna(method = 'ffill').reindex(signal_df.index)
            else:
                clist = ['raw'] + signal_df.columns.tolist()[1:]
                signal_df.columns = clist
                if 'pos_cash' not in clist:
                    signal_df['pos_cash'] = initial_cash

            # signal_df = standard_index(signal_df)
            signal_df = handle_holiday(signal_df)

            if self.ticker_kind == 'option':
                signal_df['pos_price'] = signal_df['contract_00'].apply(lambda x:int(x.split('-')[-1].split('.')[0]))
            else:
                pos_price = signal_df['close'].between_time('930','934')
                pos_price = round(pos_price.groupby(pos_price.index.date).mean(), 3)
            ...
            signal_df['pos_min'] = np.sign(signal_df['raw']) * np.floor(signal_df['raw'].apply(get_target_pos_from_signal_min_temp) * signal_df['pos_cash'] / signal_df['pos_price'] / self.face_value)
            get_target_pos_from_signal_max_temp = partial(get_target_pos_from_signal_max, pos_dict=pos_dict)
            signal_df['pos_max'] = np.sign(signal_df['raw']) * np.floor(signal_df['raw'].apply(get_target_pos_from_signal_max_temp) * signal_df['pos_cash'] / signal_df['pos_price'] / self.face_value)
            ...
            if 'filter_series' in sig_dict.keys():
                fs = sig_dict['filter_series'].fillna(0)
                if isinstance(fs, pd.Series):
                    fs = fs.to_frame()
                fs.columns = ['open_filter']
                fs = fs.astype('int')
                assert len(set(fs['open_filter'].unique().tolist()) - set([0,1])) == 0, 'filter_series must in [0, 1]'
                signal_df = signal_df.join(fs, how = 'left')
                signal_df['open_filter'] = signal_df['open_filter'].shift(1)
            else:
                signal_df['open_filter'] = np.nan

            signal_df['raw'] = signal_df['raw'].shift(1)
            if len(self.signal_list) > 1:
                signal_df = signal_df.reindex(merge_index).between_time(self.trade_start_time, self.trade_end_time).fillna(method = 'ffill')
                signal_df[['raw', 'pos_min', 'pos_max']] = signal_df[['raw', 'pos_min', 'pos_max']].fillna(0)
                signal_df[['pos_cash', 'contract_00', 'close', 'twap', 'pos_price']] = signal_df[['pos_cash', 'contract_00', 'close', 'twap', 'pos_price']].fillna(method = 'bfill')
    ...
    def back_test_singleday(self, date):
        ...
        contract = self.ticker if self.ticker_kind == 'etf' else signal_date_list[0]['signal'][0][contract_00_idx].split('.')[0]
        _tick_path = os.path.join(self.tick_root_path, '%s/%s.csv' % (contract, date))
        if not os.path.exists(_tick_path):
            _tick_path = os.path.join(self.tick_root_path, '%s.CF/%s.csv' % (contract, date))

        tickdf = pd.read_csv(_tick_path, index_col=0, parse_dates=True)[['Buy1Price','Sell1Price','Buy1OrderQty','Sell1OrderQty','TotalVolumeTrade','LastPx']]
        tickdf['TotalVolumeTrade'] = tickdf.TotalVolumeTrade.diff()
        tickdf['flag_volume'] = self.max_deal_volume_in_minute#(tickdf['TotalVolumeTrade'].rolling(120, min_periods = 60).sum() * self.max_deal_volume_in_minute).apply(lambda x:max(np.floor(x), 20))
        # tickdf['commission_num'] = tickdf['TotalVolumeTrade'].rolling(self.max_wait_tick_num * 2, min_periods = self.max_wait_tick_num).mean().apply(lambda x:max(min(np.floor(x), self.volume_per_order), 1))
        tickdf['commission_num'] = tickdf['TotalVolumeTrade'].rolling(2, min_periods = 1).mean().apply(lambda x:max(min(np.floor(x), self.volume_per_order), 1))
        ...
        tickdf = tickdf.round({'Buy1Price':3,'Sell1Price':3})
        idx_tickdf = tickdf.index
        tclist = tickdf.columns.tolist()
        buy1px_idx = tclist.index('Buy1Price')
        sell1px_idx = tclist.index('Sell1Price')
        volume_idx = tclist.index('TotalVolumeTrade')
        lastpx_idx = tclist.index('LastPx')
        midprice_idx = tclist.index('midprice')
        flag_volume_idx = tclist.index('flag_volume')
        commission_num_idx = tclist.index('commission_num')
        buy1qty_idx = tclist.index('Buy1OrderQty')
        sell1qty_idx = tclist.index('Sell1OrderQty')
        ...
        for i in range(1, date_signal_lenth):
            ...
            for sig in signal_date_list:
                ...
                filter_score = sig['signal'][i][open_filter_idx]
                trade_num_this_sig = self.get_trade_num(hold_num, pos_min, pos_max, nowtime, stop_loss_flag)
                if trade_num_this_sig != 0 and filter_score == 0 and (trade_num_this_sig * hold_num >= 0):
                    trade_num_this_sig = 0
                need_trade_num_persig.append(trade_num_this_sig)
            ...
            if need_trade_num == 0:#此分钟不交易
                ...
                if nowtime.hour == 11 and nowtime.minute == 29:
                ...
                open_flag_volume = pre_tickdf[0][flag_volume_idx]
                open_commission_num = max(min(pre_tickdf[0][commission_num_idx], open_flag_volume - abs(dealtotal_vol), open_contract_num), 0)
                open_commission_num2 = open_commission_num
                ...
                for z in range(len(order_px_para)):
                    ...
                    orderbook_qty = order_px_para[z][sell1qty_idx] if need_trade_num > 0 else order_px_para[z][buy1qty_idx]
                    if abs(abs(open_price) / valid_px - 1) >= 0.05 and self.ticker in ['IC.CFE', 'IF.CFE', 'IH.CFE', 'IM.CFE']:
                        if (z != (len(order_px_para) - 1)) and (open_contract_num != 0):
                            putorder_num += 1
                        tick_state.append([open_commission_num2, open_flag_volume, open_commission_num, round(open_price,1), _deal_price, tickvolume, orderbook_qty, 0, dealtotal_vol, need_trade_num])
                        continue

                    if round(open_price,1) >= _deal_price and tickvolume > 0 and open_commission_num > 0 and orderbook_qty > 0:
                        ...
                        # get_vol = max(np.floor(min(tickvolume, orderbook_qty) * 0.5), 1) if tickvolume * orderbook_qty > 0 else 0
                        get_vol = np.floor(max(tickvolume, orderbook_qty) * 0.5) if self.ticker_kind == 'future' else max(tickvolume, orderbook_qty)
                        deal_vol = min(open_commission_num, open_contract_num, get_vol)
                        open_commission_num -= deal_vol
                        ...
                        open_fee = open_value * self.c_rate + deal_vol * self.fee_fixed
                        ...
                        tick_state.append([open_commission_num2, open_flag_volume, open_commission_num, round(open_price,1), _deal_price, tickvolume, orderbook_qty, deal_vol, dealtotal_vol, need_trade_num])

                    else:# 没成交
                        tick_state.append([open_commission_num2, open_flag_volume, open_commission_num, round(open_price,1), _deal_price, tickvolume, orderbook_qty, 0, dealtotal_vol, need_trade_num])

                    wait_tick_num += 1    
                    if wait_tick_num >= self.max_wait_tick_num or open_commission_num2 == 0:
                        if z - self.delay_tick_num < 0:
                            ...
                            open_flag_volume = pre_tickdf[z+1][flag_volume_idx]
                            open_commission_num = max(min(pre_tickdf[z+1][commission_num_idx], open_flag_volume - abs(dealtotal_vol), open_contract_num), 0)
                            ...
                            open_flag_volume = order_px_para[z-self.delay_tick_num][flag_volume_idx]
                            open_commission_num = max(min(order_px_para[z-self.delay_tick_num][commission_num_idx], open_flag_volume - abs(dealtotal_vol), open_contract_num), 0)
                        open_commission_num2 = open_commission_num
                        if (z != (len(order_px_para) - 1)) and (open_contract_num != 0):
                            putorder_num += 1
                        wait_tick_num = 0
                ...
                if nowtime.time() == self.trade_end_time:
                    if now_hold_num != 0:
                        if now_hold_num > 0:
                            deal_price = order_px_para[-1][buy1px_idx] if len(order_px_para) > 0 else abs(open_price)
                        elif now_hold_num < 0:
                            deal_price = order_px_para[-1][sell1px_idx] if len(order_px_para) > 0 else abs(open_price)
                ...
                if self.min_deal_vol_in_bar is not None:
                    min_deal_vol = min(self.min_deal_vol_in_bar, abs(need_trade_num))
                    if abs(dealtotal_vol) < min_deal_vol:
                        
                        deal_price = twap
                        if deal_price != deal_price:
                            if need_trade_num < 0:
                                deal_price = order_px_para[-1][buy1px_idx] if len(order_px_para) > 0 else abs(open_price)
                            elif need_trade_num > 0:
                                deal_price = order_px_para[-1][sell1px_idx] if len(order_px_para) > 0 else abs(open_price)

                        deal_vol = min_deal_vol - abs(dealtotal_vol)
                        deal_price_list.append(deal_price)
                        deal_volume_list.append(deal_vol)
                        open_contract_num -= deal_vol
                ...
                # 新的收益细节：
                deal_weighted_price_close, deal_volume_close, deal_weighted_price_open, deal_volume_open = allocate_open_close(dealtotal_vol, pre_hold_num, deal_price_list, deal_volume_list, np.max(deal_volume_list) if len(deal_volume_list) > 0 else 1)
