...
# 限制在前几根tick成交，或者使用未来数据，成交这个分钟成交量比例上限
...
class TS_BACK_TEST:
    ...
    def __init__(self, signal_list, ticker='IC.CFE', start_date = None, end_date = None,
                 ...
                 max_open_value = None, cuohe_tick_num = None, deal_volume_ratio = 100, cuohe_tick_10s_period = 0):
        """
        ...
        self.cuohe_tick_num = cuohe_tick_num
        self.deal_volume_ratio = deal_volume_ratio
        self.cuohe_tick_10s_period = cuohe_tick_10s_period
        ...
        global dt_idx, raw_idx, contract_00_idx, open_idx, close_idx ,low_idx ,vwap_idx ,twap_idx ,pos_price_idx ,pos_min_idx ,pos_max_idx ,open_filter_idx, minute_volume_idx
        ...
        minute_volume_idx = columns_list.index('volume')
    ...
    def back_test_singleday(self, date):
        ...
        tickdf = tickdf.join(pd.DataFrame(index = pd.date_range(tickdf.index[0],tickdf.index[-1], freq='500L')), how = 'outer').sort_index()
        tickdf = tickdf.fillna(method = 'ffill')
        ...
        for i in range(1, date_signal_lenth):
            ...
            _volume_t = np.floor(sig0[i][minute_volume_idx] * self.deal_volume_ratio)
            ...
            if need_trade_num == 0:#此分钟不交易
                ...
                for z in range(len(order_px_para)):
                    if self.cuohe_tick_num is not None:
                        if z > self.cuohe_tick_num:
                            break
                    if (z >= self.cuohe_tick_10s_period * 20) and (z < (self.cuohe_tick_10s_period + 1) * 20):
                    ...
                    if (round(open_price,1) >= _deal_price) and (tickvolume > 0) and not makedealflag and deal_period_flag:
                    ...
                    if open_contract_num == 0:
                        break
                    if abs(dealtotal_vol) >= _volume_t:
                        break
