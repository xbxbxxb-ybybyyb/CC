...
class TS_BACK_TEST:
    ...
    def __init__(self, signal_df, ticker='IC.CFE', price_kind='vwap', pos_dict = {(0,   0.1): (0.0, 0.0),
                 ...
                 save_path='/data/user/', name_prefix='', show_image = True, save_csv = False, save_image = True):
        """ ！！！！！！！！！！！！！！ 默认参数的信号值在[-1,1]之间 ！！！！！！！！！！！
        ...
        :param stop_loss: 止损
        :param minute_after_stop_loss: 止损后多久不交易，设置为30就是止损后30分钟内不再开仓
        ...
        if self.signal_df.shape[1] == 1:
            self.signal_df.columns = ['raw']
            _data = IO.read_data([start_date, f'{end_date}235959'], universe=[ticker], columns = ['HTSCSecurityID', 'open', 'high', 'low', 'close', 'twap', 'vwap', 'volume', 'amount', 'position'], alt = '/dfs/group/800466/warehouse/prod/MD/CHINA_COMMODITY/1MIN/H5/MAIN_CHINA_COMMODITY_1MIN.h5').rename(columns = {'volume':'volume_deal'})
            # atr = IO.read_data([start_date, end_date], universe=[ticker], columns = ['ATR'], alt = '/dfs/group/800466/warehouse/prod/MD/CHINA_COMMODITY/UNIVERSE/CHINA_COMMODITY_MINUTE_ATR_preday.h5')
            self.signal_df = pd.concat([self.signal_df, _data.reset_index(level = 1, drop = True)], axis = 1) #self.signal_df.join(_data, how = 'left')

        self.multiplier_dict = pd.read_csv('/dfs/group/800466/warehouse/prod/MD/CHINA_COMMODITY/INFO/WIND_CFuturesContPro.csv', index_col='S_INFO_WINDCODE')['S_INFO_PUNIT'].to_dict()

        self.signal_df = self.signal_df.loc[str(start_date):str(end_date)].sort_index()


        price_precision = 0
        for x in self.signal_df.close.tolist()[-100:]:
            x = str(round(x, 9))
            if '.' in x:
                if len(x.split('.')[-1]) > price_precision:
                    price_precision = len(x.split('.')[-1])
        ...
        assert self.price_precision is not None
        ...
        global dt_idx, raw_idx, std_signal_idx, open_idx, close_idx ,low_idx ,vwap_idx ,twap_idx ,volume_idx ,amount_idx ,price_kind_idx, buy_vwap_idx, sell_vwap_idx, filter_socre_idx, HTSCSecurityID_idx, atr_idx
        ...
        HTSCSecurityID_idx = columns_list.index('HTSCSecurityID')
    ...
    def back_test(self):
        ...
        for i in tqdm(range(1, length)):
            ...
            if HTSCSecurityID != df[i-1][HTSCSecurityID_idx] and now_hold_num != 0:
                yicang_flag = True
            else:
                yicang_flag = False
            
            face_value = self.multiplier_dict[HTSCSecurityID.replace('.ZCE', '.CZC')]
            ...
            open_price = round(open_price, self.price_precision)
            if now_hold_num == 0:
            ...
            if trading_flag and ((stop_loss_flag and (now_hold_num!=0)) or \
                         (now_hold_num_state * pre_target_pos_state == -1) or (now_hold_num_abs > target_open_num_max) or (now_hold_num != 0 and max_hold_time_flag != 0)):
                ...
                while (close_contract_num > 0):
                    ...
                    close_value = close_price * face_value * this_deal_close_num
                    close_fee = close_value * self.c_rate
                    if yicang_flag:
                        thisdeal_thismin_profit = face_value * this_deal_close_num * (close_price - twap) * now_deal_pos_state - close_fee
                    else:
                        thisdeal_thismin_profit = face_value * this_deal_close_num * (close_price - last_close) * now_deal_pos_state - close_fee
                    close_profit_this_min += thisdeal_thismin_profit

                    trade_dict[wait_close_dealcount].update({'close_time': nowtime, 'close_price': close_price, 'close_contract':HTSCSecurityID,
                ...
                if now_hold_num != 0:
                    if yicang_flag:
                        now_hold_num_profit = face_value * now_hold_num * (close - twap)  # 此分钟收益
                        _fee = (last_close + twap) * face_value * abs(now_hold_num) * self.c_rate
                ...
                if (pre_hold_num != 0) and (now_hold_num == 0):
                    totaltrade_dict[totaldeal_count].update({'pos_close':np.sign(pre_hold_num),'close_time':nowtime,'profit_intradeal':profit_intradeal,'max_position':maxposition_intradeal, 'holding_time':now_hold_time, 'close_contract':HTSCSecurityID})
                    profit_intradeal = 0
                ...
                if open_contract_num == 0:
                    ...
                    trade_dict[deal_count] = {'deal_count': deal_count, 'pos': pre_target_pos_state, 'open_time': nowtime,
                                              ...
                                              'open_contract':HTSCSecurityID,
                                              'close_time': None, 'close_price': None,'close_contract':None,
        ...
        trade_df = trade_df[
            ['deal_count', 'pos', 'open_time', 'open_price', 'open_contract_num', 'now_hold_num', 
             ...
             'open_signal', 'now_hold_dealcount', 'open_contract', 'close_time', 'close_price', 'close_contract',
        ...
        totaltrade_df = totaltrade_df[['totaltrade_count', 'pos',  'open_time', 'close_time', 'pos_close', 'profit_intradeal', 'change',
                                      'equity_curve', 'holding_time', 'open_contract', 'close_contract', 'max_position']]
...
def draw_picture(daily_return, _result, _pnl, save_path, name, initial_cash, show_image, save_image):
    daily_df = daily_return[['daily_equty_curve']].fillna(0)
...
def merge_evaluate(total_trade, total_daily_return, ticker_num):
    ...
    if len(longtrade) > 0:
        results.loc[0, '做多收益'] = format(longtrade.change.sum() / ticker_num, '.4%')
        results.loc[0, '做多盈亏比'] = round(longtrade.loc[longtrade['change'] > 0]['change'].mean() / longtrade.loc[longtrade['change'] < 0]['change'].mean() * (-1), 2)  
    else:
        results.loc[0, '做多收益'] = np.nan
        results.loc[0, '做多盈亏比'] = np.nan
    if len(shorttrade) > 0:
        results.loc[0, '做空收益'] = format(shorttrade.change.sum() / ticker_num, '.4%')
        results.loc[0, '做空盈亏比'] = round(shorttrade.loc[shorttrade['change'] > 0]['change'].mean() / shorttrade.loc[shorttrade['change'] < 0]['change'].mean() * (-1), 2)  
    else:
        results.loc[0, '做空收益'] = np.nan
        results.loc[0, '做空盈亏比'] = np.nan
