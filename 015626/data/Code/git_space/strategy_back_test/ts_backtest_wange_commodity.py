...
class TS_BACK_TEST:
    ...
    def __init__(self, signal_df, ticker='IC.CFE', price_kind='vwap', pos_dict = {(0, 0.4): (0.0, 0.0),
                 ...
                 data_kind = 'future', stop_profit=100,
                 stop_loss=-100, capital_use_rate = 1,  open_num_permin=30, close_num_permin=30, deal_volume_ratio = 0,
...
#         self.para_pos_dict = self.get_target_pos_dict(entry_para, exit_para)
        ...
        face_value_dict = {'IC.CFE': 200,
                           ...
                           'IH.CFE': 300,
                           'IM.CFE': 200}
        self.face_value = face_value_dict.get(self.ticker, 1)
        ...
        for i in tqdm(range(1, length)):
            ...
            if (stop_loss_flag and (now_hold_num!=0)) or (((nowtime.hour == 14) and (nowtime.minute == 57)) and (now_hold_num != 0)) or \
             (now_hold_num_state * pre_target_pos_state == -1) or (now_hold_num_abs > target_open_num_max):
                ...
                if ((open_contract_num <=0 ) or (datetime.time(nowtime.hour, nowtime.minute) >= datetime.time(self.hour_t,self.minute_t))) or stop_loss_flag or (open_value_intraday > self.max_open_value):
                    pnl_dict[nowtime] = now_hold_num_profit
                    profit_intraday += now_hold_num_profit
                    profit_intradeal += now_hold_num_profit
                ...
                trade_dict[deal_count] = {'deal_count': deal_count, 'pos': pre_target_pos_state, 'open_time': nowtime,
                                          ...
                                          'open_signal': pre_raw,
                                          'open_value': open_value, 'open_fee': open_fee,'open_value_intraday':open_value_intraday,
...
#             daily_openvalue['fenzu'] = pd.cut(daily_openvalue['open_value_intraday'], bins=listBins, labels=alist, include_lowest=False)
...
#                 df = df.join(md_twap, how='inner')
        ...
        df = t.sort_index()
        df = self.handle_holiday(df)
        df = df.reset_index()
        return df

    def handle_holiday(self, df):
        datelist = udt.get_trading_date_range(df.index.tolist()[0].date(),df.index.tolist()[-1].date())
        datedict = {}
...
#         return {(0, 0.25):(0, 0),
#                (0.25, 0.3):(0, 0.1/3),
#                (0.3, 0.35):(0.1/3, 0.2/3),
#                (0.35, 0.4):(0.2/3, 0.3/3),
#                (0.4, 0.45):(0.3/3, 0.4/3),
#                (0.45, 0.5):(0.4/3, 0.5/3),
#                (0.5, 0.55):(0.5/3, 0.6/3),
#                (0.55, 0.6):(0.6/3, 0.7/3),
#                (0.6, 0.65):(0.7/3, 0.8/3),
#                (0.65, 0.7):(0.8/3, 0.9/3),
#                (0.7, 0.75):(0.9/3, 1.0/3),
#                (0.75, 1.01):(1.0/3, 1.0/3)} 
...
'''
# 测试IC
save_root_path = '/data/user/015626/data/share/factor/back_test/IC_ts/20201123/'
factor_path = '/data/user/012398/data/MD/sig_ic.h5'
factor = pd.read_hdf(factor_path)
factor = factor.loc['20170101':'20200401'].sort_index()

factor_name = factor_path.split('/')[-1].split('.')[0] + '_56789_5000w_insample'
...
pos_dict = {(0, 0.4): (0.0, 0.0),
                 ...
                 (0.8, 0.9): (0.8/3, 1.0/3),
                 (0.9, 1.9): (1.0/3, 1.0/3)}
ts = TS_BACK_TEST(factor,price_kind='twap',ticker='IC.CFE', slippage=0.6, initial_cash=50000000, save_path = save_path, name_prefix = name_prefix,
                  pos_dict=pos_dict, capital_use_rate = 1,stop_loss = -0.005)
...
'''
# 测试IF
save_root_path = '/data/user/015626/data/share/factor/back_test/IF_ts/20201123/'
factor_path = '/data/user/012398/data/MD/sig_if.h5'
factor = pd.read_hdf(factor_path)
# factor = factor / 1.8
factor = factor.loc['20170101':'20200401'].sort_index()

factor_name = factor_path.split('/')[-1].split('.')[0] + '_56789_5000w_insample_'
name_prefix = factor_name 
...
pos_dict = {(0, 0.4): (0.0, 0.0),
                 ...
                 (0.8, 0.9): (0.8/3, 1.0/3),
                 (0.9, 1.9): (1.0/3, 1.0/3)}
ts = TS_BACK_TEST(factor,price_kind='twap',ticker='IF.CFE', slippage=0.4, initial_cash=50000000, save_path = save_path, name_prefix = name_prefix,
                  pos_dict = pos_dict, capital_use_rate = 1,stop_loss = -0.005)
a, b, c, d =  ts.back_test()
print(b)
...
'''
# 测试IM data_kind 要设置为 spot. price_kind 设置为 twap_fake 表示使用假标签测试收益，twap表示指数twap测试收益
factor_path = '/data/group/800466/warehouse/prod/alpha/CHINA_FUTURES/MINUTE/model/model_update/20220715_ic_ic_prod_v6_fix/model_value/model_norm/20220718/pred_comb2.pkl'
factor = pd.read_pickle(factor_path) * 2 - 1 
factor = factor.loc['20210101':'20211231'].sort_index()

save_root_path = '/data/user/015626/data/share/factor/back_test/IC_ts/20221221_im/'

factor_name = factor_path.split('/')[-1].split('.')[0] + 'twap_fake'
name_prefix = factor_name 
...
pos_dict = {(0,   0.5): (0,        0),
            ...
            (0.7, 0.9): (0.5/10,   1/10),
            (0.9, 100): (1/10,     1/10)}
ts = TS_BACK_TEST(factor,price_kind='twap_fake',ticker='IM.CFE', data_kind = 'spot', slippage=0.6, initial_cash=2e8, save_path = save_path, name_prefix = name_prefix,
                  pos_dict=pos_dict, capital_use_rate = 1, c_rate=2.5 / 100000,  stop_loss = -0.005)
a, b, c, d =  ts.back_test()
print(b)
'''
...
save_root_path = '/data/user/015626/data/share/factor/back_test/IC_ts/20220719_im/'

factor_name = factor_path.split('/')[-1].split('.')[0] + '_insample'
name_prefix = factor_name 
...
pos_dict = {(0,   0.5): (0,        0),
            ...
            (0.7, 0.9): (0.5/10,   1/10),
            (0.9, 100): (1/10,     1/10)}
ts = TS_BACK_TEST(factor,price_kind='vwap',ticker='512100.SH', data_kind = 'etf', slippage=0, initial_cash=2e8, save_path = save_path, name_prefix = name_prefix,
                  pos_dict=pos_dict, capital_use_rate = 1, c_rate=2.5 / 100000,  stop_loss = -0.005,
                 open_num_permin=30, close_num_permin=30, deal_volume_ratio = 0.1)
a, b, c, d =  ts.back_test()
print(b)
'''