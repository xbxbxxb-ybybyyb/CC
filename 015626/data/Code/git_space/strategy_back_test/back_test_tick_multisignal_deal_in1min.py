...
class TS_BACK_TEST:
    ...
    def __init__(self, signal_list, ticker='IC.CFE', start_date = None, end_date = None,
                 ...
                 max_open_value = None, deal_in1min_kind = None):
        """
        ...
        :param deal_in1min_kind: 当为open时，表示开仓要在一分钟内开完，当为close时，表示平仓要在一分钟内平完，
                                 当为all时，表示开平都要在一分钟内搞定，一分钟交易不完时，剩余部分使用已成交的均价进行成交
        ...
        self.deal_in1min_kind = deal_in1min_kind
    ...
    def back_test_singleday(self, date):
        ...
        for i in range(1, date_signal_lenth):
            ...
            if need_trade_num == 0:#此分钟不交易
                ...
                if self.deal_in1min_kind is not None:
                    _res_volume = open_contract_num
                    _deal_weighted_price = np.multiply(np.array([deal_price_list]),np.array([deal_volume_list])).sum() / np.sum(deal_volume_list)
                    if need_trade_num < 0:
                        _deal_weighted_price *= -1
                    if _res_volume != 0:
                        if self.deal_in1min_kind == 'all':
                            deal_price = _deal_weighted_price
                            deal_vol = _res_volume
                            deal_price_list.append(deal_price * need_trade_num_state)
                            deal_volume_list.append(deal_vol)

                            open_value = deal_price * self.face_value * deal_vol * need_trade_num_state
                            open_fee = open_value * self.c_rate
                            if pre_hold_num * need_trade_num >= 0:# 说明是开仓
                                open_value_intraday += open_value 
                                open_value_intratrade += open_value 
                            ...
                            tick_state.append(2)
                        elif self.deal_in1min_kind == 'open' and pre_hold_num * need_trade_num >= 0:
                            deal_price = _deal_weighted_price
                            deal_vol = _res_volume
                            deal_price_list.append(deal_price * need_trade_num_state)
                            deal_volume_list.append(deal_vol)
                            ...
                            tick_state.append(2)
                        elif self.deal_in1min_kind == 'close' and pre_hold_num * need_trade_num < 0:
                            deal_price = _deal_weighted_price
                            deal_vol = _res_volume
                            deal_price_list.append(deal_price * need_trade_num_state)
                            deal_volume_list.append(deal_vol)
