...
def draw_picture(daily_return, open_value, _result, _pnl, save_path, name, initial_cash):
    ...
    plt.text(0.5, 0.1, 'average trade minutes:  ' + str(_result.loc['平均每日交易分钟数']), fontsize=text_fontsize)
    plt.text(0.5, 0, 'initial cash:  ' + str(initial_cash / 1e8) + 'e8', fontsize=text_fontsize)
...
class TS_BACK_TEST:
    ...
    def __init__(self, signal_list, ticker='IC.CFE', start_date = None, end_date = None,
                 ...
                 delay_tick_num = 0,max_wait_tick_num = 4, put_order_immediately = False, no_opening_start_time = [14,30], closing_start_time = [14,51], 
        ...
        :param put_order_immediately: 表示一个委托单成交后，是否马上发出下一单
        ...
        self.put_order_immediately = put_order_immediately
            ...
            if need_trade_num == 0:#此分钟不交易
                ...
                for z in range(len(order_px_para)):
                    ...
                    if (round(open_price,1) >= deal_price) and (tickvolume > 0) and not makedealflag:
                        deal_vol = min(self.vol_pertick, open_contract_num, tickvolume)
                    ...
                    wait_tick_num += 1    
                    if (wait_tick_num >= self.max_wait_tick_num) or (makedealflag and self.put_order_immediately):
                ...
                if nowtime.time() == self.trade_end_time:
                    if now_hold_num != 0:
                        if now_hold_num > 0:
                            deal_price = order_px_para[-1][buy1px_idx]
                        elif now_hold_num < 0:
                            deal_price = order_px_para[-1][sell1px_idx]
    ...
    def back_test(self):
        ...
        if self.save_path != None:
            ...
            draw_picture(daily_return, daily_openvalue, results['num'], pnl, self.save_path, self.name_prefix, self.initial_cash)
...
factor = pd.read_pickle('/data/group/800466/warehouse/prod/alpha/CHINA_FUTURES/MINUTE/model/model_update/20220826_ic_ic_v7c/model_value/model_norm/20220929/pred_comb2.pkl')
factor = factor * 2 - 1
...
today = today = str(int(datetime.datetime.now().strftime('%Y%m%d'))) + date_suffix
...
cash = 5e8
max_wait_tick_num = 4
name_prefix = '%s_%s_%s_%s_%s_%s_pos_price' % (signal_name, std_name, str(cash//1e8), str(max_wait_tick_num), start_date, end_date)
save_path = os.path.join(save_root_path,  name_prefix)

signal_list = [{'signal':factor,'pos_dict':pos_dict,'cash':cash}]
ts = TS_BACK_TEST(signal_list, save_signal_list = True, c_rate=3 / 100000, ticker = ticker, stop_loss = -0.005,tickslippage = 1.2, 
                  max_wait_tick_num = max_wait_tick_num, put_order_immediately = True,
