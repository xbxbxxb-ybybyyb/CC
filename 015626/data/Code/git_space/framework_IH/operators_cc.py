...
def ts_rank(df1, d = 1200,min_count=600):
    min_count = int(d/2)
    ...
    if d == 1:
        ...
        if isinstance(df1, pd.DataFrame):
            output = pd.DataFrame(bk.move_rank(df1, window=d, min_count=min_count, axis=0),
                                  index=df1.index, columns=df1.columns)
        elif isinstance(df1, pd.Series):
            output = pd.Series(bk.move_rank(df1, window=d, min_count=min_count, axis=0),
                               index=df1.index, name=df1.name)
    return output


def ts_mean(df1, d = 1200,min_count=600):
    min_count = int(d/2)
    # moving time-series rank for the past d periods
    assert isinstance(df1, pd.Series) or isinstance(df1, pd.DataFrame), 'input is not a dataframe or series'
    if d == 1:
        ...
        if isinstance(df1, pd.DataFrame):
            output = pd.DataFrame(bk.move_mean(df1, window=d, min_count=min_count, axis=0),
                                  index=df1.index, columns=df1.columns)
        elif isinstance(df1, pd.Series):
            output = pd.Series(bk.move_mean(df1, window=d, min_count=min_count, axis=0),
                               index=df1.index, name=df1.name)
    return output

def ts_sum(df1, d = 1200,min_count=600):
    min_count = int(d/2)
    # moving time-series rank for the past d periods
    assert isinstance(df1, pd.Series) or isinstance(df1, pd.DataFrame), 'input is not a dataframe or series'
    if d == 1:
        ...
        if isinstance(df1, pd.DataFrame):
            output = pd.DataFrame(bk.move_sum(df1, window=d, min_count=min_count, axis=0),
                                  index=df1.index, columns=df1.columns)
        elif isinstance(df1, pd.Series):
            output = pd.Series(bk.move_sum(df1, window=d, min_count=min_count, axis=0),
                               index=df1.index, name=df1.name)
    return output
...
def cross(df):
    print('00')
    df_temp = df.copy()
    df_temp[~np.isfinite(abs(df_temp))] = np.nan
    b = ((df_temp.isna()).sum(axis = 1)>650)
    df_temp.loc[b == True] = np.nan
    print('0')
    b = df_temp.quantile(0.99, axis = 1)
    print('1')
    c = df_temp.quantile(0.01, axis = 1)
    print('2')
    df_temp = df_temp.clip(c, b, axis=0)
    return df_temp
