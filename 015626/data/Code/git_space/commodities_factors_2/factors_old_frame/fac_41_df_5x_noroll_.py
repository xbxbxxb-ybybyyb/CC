...
# td
class fac_41_df_5x_noroll_(FactorGenerator):
    def __init__(self):
        required_columns=['high', 'main_mask']

        super(fac_41_df_5x_noroll_, self).__init__(required_columns=required_columns
                                  )
    ...
    def on_bar(self, data, aaa, bbb, ccc, ddd):
        ...
        if aaa == bbb:
            ...
            vol = data['high'].diff().rolling(bbb, min_periods = 5).std()[mask].mean(axis = 1)
