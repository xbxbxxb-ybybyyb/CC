...
class fac_54_df_20x_noroll_(FactorGenerator):
    def __init__(self):
        required_columns=['close', 'twap', 'main_mask']

        super(fac_54_df_20x_noroll_, self).__init__(required_columns=required_columns
                                  )
    ...
    def on_bar(self, data, aaa = 5, bbb = 45, ccc = 8, ddd = 5):
        ...
        price_level = rolling_norm(data['twap'],  aaa)
        price_std = (data['twap'].pct_change()).rolling(bbb, min_periods = 5).std()
        factor = (price_level / r(price_std))[mask].mean(axis = 1)
        factor = ts_rank(factor.to_frame().rolling(ccc, min_periods = 1).mean(), ddd)
