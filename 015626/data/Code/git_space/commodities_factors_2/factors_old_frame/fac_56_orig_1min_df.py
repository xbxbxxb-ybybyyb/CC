...
class fac_56_orig_1min_df(FactorGenerator):
    def __init__(self):
        required_columns=[ 'low', 'high', 'main_mask', 'close']

        super(fac_56_orig_1min_df, self).__init__(required_columns=required_columns
                                  )
    ...
    def on_bar(self, data, aa, bb, ccc):
        ...
        factor = ts_truncated_ema_1((temp * r((co)) / r(co2)).to_frame(), ccc * 3, 1/(1+ccc))
        #factor = (temp / r((co)) / r(co2)).rolling(ccc, min_periods = 1).mean()
