...
# wsc8_future_if
class fac_49_aug_orig_1min_df_10x_noroll_(FactorGenerator):
    def __init__(self):
        required_columns=['close', 'high', 'low', 'second_main_mask']

        super(fac_49_aug_orig_1min_df_10x_noroll_, self).__init__(required_columns=required_columns
                                  )
    ...
    def on_bar(self, data, a = 30, b = 80, c = 60, d = 4):
        ...
        close = data['close'][data['second_main_mask']].mean(axis = 1)
        high = data['high']
        low = data['low']
        low_n = ts_min(low, a)[data['second_main_mask']].mean(axis = 1)
        high_n = ts_max(high, a)[data['second_main_mask']].mean(axis = 1)
        temp1 = high_n - low_n
        ...
        factor = ts_truncated_ema_span_1(temp4, c*3, c)
        factor = ts_rank(factor, d * 300)
