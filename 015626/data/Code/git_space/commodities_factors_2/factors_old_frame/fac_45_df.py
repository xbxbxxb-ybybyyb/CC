...
# vma_std
class fac_45_df(FactorGenerator):
    def __init__(self):
        required_columns=['low', 'high', 'close', 'volume', 'main_mask']

        super(fac_45_df, self).__init__(required_columns=required_columns
                                  )
    ...
    def on_bar(self, data, aaa, bbb, ccc):
        ...
        cs = vwap_ma.rolling(int(coef / 2), min_periods = 5).corr(hclose)
        cl = vwap_ma.rolling(int(coef * 2) ,min_periods = 5).corr(hclose)
