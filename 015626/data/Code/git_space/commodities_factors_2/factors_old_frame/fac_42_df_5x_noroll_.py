...
# tr1_zf
class fac_42_df_5x_noroll_(FactorGenerator):
    def __init__(self):
        required_columns=['low', 'high', 'close', 'main_mask']

        super(fac_42_df_5x_noroll_, self).__init__(required_columns=required_columns
                                  )
    ...
    def on_bar(self, data, aaa, bbb, ccc):
        ...
        coef = int(np.nanmedian(mask.groupby(mask.index.date).count()))
        cs = sig.rolling(int(coef), min_periods = 5).corr(hclose)
        cl = sig.rolling(int(coef * 3) ,min_periods = 5).corr(hclose)
        sig[(cs <cl) | (cl < 0)] = 0
