...
class fac_57_df(FactorGenerator):
    def __init__(self):
        required_columns=['close', 'low', 'second_main_mask']

        super(fac_57_df, self).__init__(required_columns=required_columns
                                  )
        

    def on_bar(self, data, aa1, aa2, bb, ccc):
        aa1 = 5
        aa2 = 35
        bb = 2
        ccc = 300
        ...
        vol1 = rtn.rolling(aa1, min_periods=1).std()
        vol2 = rtn.rolling(aa2, min_periods=1).std()
        co = cross_hub_num(data['close'], 15)[mask].mean(axis = 1) + 1
        ret1 = (data['close'] - (data['low'].shift(1).rolling(aa1, min_periods=1).min()))[mask].mean(axis = 1)
        ret2 = (data['close'] - (data['low'].shift(1).rolling(aa2, min_periods=1).min()))[mask].mean(axis = 1)
