...
# tr1_zf
class fac_42_orig_1min_df_5x_(FactorGenerator):
    def __init__(self):
        required_columns=['low', 'high', 'close', 'main_mask']

        super(fac_42_orig_1min_df_5x_, self).__init__(required_columns=required_columns
                                  )
    ...
    def on_bar(self, data, aaa, bbb, ccc):
        ...
        sig1 = sig.rolling(int(np.sqrt(bbb)), min_periods = 1).mean() 
        sig = ts_rank(sig1, 300 * ccc)
