...
from help_functions_wsc import rolling_window_upgrade
from rolling_adj import *

# vma_std
class fac_58_5min_df(FactorGenerator):
    def __init__(self):
        required_columns=['close', 'low', 'second_main_mask', 'high', 'volume']

        super(fac_58_5min_df, self).__init__(required_columns=required_columns
                                  )
    ...
    def on_bar(self, data, aa, bb, ccc):
        ...
        score_raw1 = irr_ma(score_raw1, ma_win)
        ...
        co = (cross_hub_num(data['close'], 10)[mask].mean(axis = 1) / 3) + 1
