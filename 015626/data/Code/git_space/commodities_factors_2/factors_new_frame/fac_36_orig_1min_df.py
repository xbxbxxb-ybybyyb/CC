...
class fac_36_orig_1min_df(FutureFactor):
    ...
    def calculate(self, data):
        ...
        ema_short = ema_1(close[-short_win*5:],short_win*5,1/(short_win+1))

        ema_long = ema_1(close[-long_win*5:],long_win*5,1/(long_win+1))
        ...
        co3 = nanstd_np(cls_diff[-120:], ddof = 1)

        if abs(co3) < 1e-8:

            co3 = np.nan

        co2 = cross_hub_num_array(close[-130:],60) + 1

        factor = price_oscillator_raw_ma / np.sqrt(co3) / co2
    ...
    def pre_calculate(self, data):
        ...
        for i in range(3):
            ...
            if i == 0:
                ...
                close = data['close_secmain'][-(220+i):-i]
