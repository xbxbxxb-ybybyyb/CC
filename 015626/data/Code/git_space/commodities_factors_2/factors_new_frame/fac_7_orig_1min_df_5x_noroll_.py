...
class fac_7_orig_1min_df_5x_noroll_(FutureFactor):
    ...
    def calculate(self, data):
        ...
        sig = ema_1(np.array(self.hl_list[-(ccc * 3):]), ccc * 3, 1 / (ccc + 1))
        return sig
    ...
    def pre_calculate(self, data):
        ...
        for i in range(n):
            ...
            if len(h) > 0:
                ...
                self.ch_list.append(ch)
                self.cl_list.append(cl)
                if (len(self.ch_list) < bbb * 4) or (len(self.cl_list) < bbb * 4):
                    continue
                ch_ema = ema_span_1(np.array(self.ch_list[-(bbb * 4):]), bbb * 4, bbb)
                cl_ema = ema_span_1(np.array(self.cl_list[-(bbb * 4):]), bbb * 4, bbb)
                hl = ch_ema + cl_ema
