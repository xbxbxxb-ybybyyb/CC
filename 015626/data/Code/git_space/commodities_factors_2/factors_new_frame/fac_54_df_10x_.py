...
class fac_54_df_10x_(FutureFactor):
    ...
    def __init__(self, ticker, freq = 1):
        ...
        self.normalize_size = 12000

        self.normalize_type = 'ts_rank'

        self.days_past = int(freq) * 13
    ...
    def calculate(self, data):
        ...
        price_level = rolling_normalize_array(twap,2700)

        price_std = move_std_bk(rtn, window = 80, min_count = 5, ddof = 1)

        price_std[abs(price_std) < 1e-8] = np.nan

        factor_org = price_level[-30:] / price_std[-30:]
