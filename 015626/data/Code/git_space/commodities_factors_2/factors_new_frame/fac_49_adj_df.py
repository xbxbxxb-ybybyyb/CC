...
def irr_filter3(data, coef, bbb):
    window = bbb 
    sig1_list = data[-window * 6 :]
    print(sig1_list[-int(window * coef):])
    return irr_filter(sig1_list[-int(window * coef):],  int(window * coef))[-1]

class fac_49_adj_df(FutureFactor):
    ...
    def __init__(self, ticker, freq):
        ...
        self.days_past = int(7 * self.freq)
    ...
    def calculate(self, data):
        ...
        #if data['dt'][-1] == np.datetime64(pd.to_datetime('20170303210000')):
        #    print(temp4, irr_filter3(self.temp4_list, coef, c))
        sig = temp4 + irr_filter4(np.array(self.temp4_list), coef, c)
