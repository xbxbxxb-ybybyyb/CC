...
class Pos():
    def __init__(self, start_date = 20160101, end_date = 20250501):
        ...
        blacklist = ['ZC.ZCE','IMS.SHF','SCTAS.INE','TC.ZCE','EFS.CFE','AFS.CFE','LR.ZCE','JR.ZCE','PM.ZCE','RI.ZCE',
                        'BB.DCE','RS.ZCE','WR.SHF','WH.ZCE','FB.DCE','CY.ZCE','RR.DCE','PR.ZCE','LG.DCE','TS.CFE','TF.CFE','T.CFE','TL.CFE',
                        'BR.SHF','BU.SHF','FU.SHF','PB.SHF','SS.SHF','PG.DCE','PP.DCE','PF.ZCE','PK.ZCE','SF.ZCE','IH.CFE','IM.CFE']
...
#        univ_data = univ_data[univ_data['amt_1e10']]
        ...
        for ticker in tickerlist:            
            if ticker not in stdindict:
                stdindict[ticker] = 0.02
            #if ticker not in stdoutdict:
            #stdoutdict[ticker] = max(stdindict[ticker],0.03)
            #stdindict[ticker] = 0.02
            stdoutdict[ticker] = stdindict[ticker]+0.01
            #else:
            #    stdoutdict[ticker] = 0.03
    ...
    def calc_basis_ratio_all(self, start_date, end_date):
    ...
    def singlebacktest(self, ticker, use_univ = True):
        ...
        ### signal construct
        longin =  malongin & stdin
        shortin =  mashortin & stdin
    ...
    def get_trade_parameters_ticker(self, set_date, ticker, pos = 0):
        ...
        if (len(pricelist) < 59) | (len(rtlist) < 9):
...
#            contract_pre = data_ticker_pre.index[idx]
        ...
        action_parm = {'maincontract':contract_cur,'longin_down':price_longin_down,'longin_up':price_longin_up,'shortin_down':price_shortin_down,'shortin_up':price_shortin_up,
                      ...
                      'rtlist':str(rtlist),'trlist':str(trlist),'cls_main_org':cls_cur}
