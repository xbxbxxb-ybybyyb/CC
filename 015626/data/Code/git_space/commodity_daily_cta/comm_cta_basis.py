...
class Pos_basis():
    def __init__(self, start_date = 20160101, end_date = 20250501):
        ...
        self.std_df = pd.DataFrame(pd.concat(std_list,axis=1).stack(),columns = ['std_10d']).sort_index()        
        self.basis_df = None
    ...
    def singlebacktest(self, ticker, use_univ = True):
        ...
        basis_ticker = basis_ticker.rolling(5,min_periods = 5).mean()
        # multiplier
        mult = self.multip.loc[ticker,'multiplier']
        ...
        malongin = (mashort > mamid)
        mashortin = (mashort < mamid)
        
        malongout = (mashort < mamid)
        mashortout = (mashort < mamid)
        ...
        # basis signal
#        basislongin = (basis_ticker < -0.08)
#        basisshortin = (basis_ticker > 0.08)
#        basislongout = (basis_ticker > -0.03)
#        basisshortout = (basis_ticker < 0.03)
        basis_std = pd.concat([basis_ticker,rt_std],axis=1)
        #if ticker == 'TF.CFE':
        
        basislongin = (basis_std.iloc[:,0] < -basis_std.iloc[:,1] * 8)
        basisshortin = (basis_std.iloc[:,0] > basis_std.iloc[:,1] * 8)
        basislongout = (basis_std.iloc[:,0] > -basis_std.iloc[:,1] * 2)
        basisshortout = (basis_std.iloc[:,0] < basis_std.iloc[:,1] * 2)
                                
        ### signal construct
        #longin =  basislongin &  turtlelongin & stdin
        #shortin =  basisshortin & turtleshortin & stdin
        longin = basislongin & stdin & malongin
        shortin = basisshortin & stdin & mashortin
        
        #longout =  basislongout | turtlelongout | stdout
        #shortout = basisshortout | turtleshortout | stdout
        longout =  basislongout | stdout | turtlelongout
        shortout = basisshortout | stdout | turtleshortout
        
        longin.iloc[:21] = False
        shortin.iloc[:21] = False
