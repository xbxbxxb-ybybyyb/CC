...
class Pos_trend():
    def __init__(self, start_date = 20160101, end_date = 20250501):
        ...
        blacklist = ['ZC.ZCE','IMS.SHF','SCTAS.INE','TC.ZCE','EFS.CFE','AFS.CFE','LR.ZCE','JR.ZCE','PM.ZCE','RI.ZCE',
                        'BB.DCE','RS.ZCE','WR.SHF','WH.ZCE','FB.DCE','CY.ZCE','RR.DCE','PR.ZCE','LG.DCE','ME.ZCE','TS.CFE',
        ...
        for ticker in tickerlist:
            ...
            std_all = pd.concat(std_list,axis=1)
            std_all.columns.name = 'Ticker'
        self.std_df = pd.DataFrame(std_all.stack(),columns = ['std_10d']).sort_index()
        self.basis_df = []
    ...
    def calc_basis_ratio_all(self, start_date = None, end_date = None, use_cache = False):
        if use_cache:
            self.basis_df = pd.read_hdf('/data/group/800466/warehouse/prod/tradingstats/Spiral/data_5/basis_ratio.h5')
        else:
            trading_days = dt.get_trading_date_range(start_date, end_date)
            basis_df_list = []
            for td in trading_days:
                basis_df = self.calc_basis_ratio(td)
                basis_df['dt'] = td
                basis_df_list.append(basis_df)
            basis_df_all = pd.concat(basis_df_list,axis=0)
            basis_df_all = basis_df_all.reset_index().set_index(['dt','Ticker'])
            self.basis_df = basis_df_all
        return self             
    ...
    def singlebacktest(self, ticker, use_univ = True):
        ...
        basis_ticker = basis_ticker.rolling(5,min_periods = 3).mean()
        #basis_ticker = basis_ticker.fillna(method = 'pad', limit = 20)
        # multiplier
        ...
        # 2、calc signal
        mashort = clsadj.rolling(5).mean()
        mamidshort = clsadj.rolling(10).mean()
        mamid = clsadj.rolling(20).mean()
        mamidlong = clsadj.rolling(30).mean()
        malong = clsadj.rolling(60).mean()
        ...
        ## masignal
        malongin = ((clsadj > mashort)&(mashort > mamidshort)&(mamidshort > mamid) &(mamid > mamidlong)&(mamidlong > malong)).astype(int)
        mashortin = ((clsadj < mashort)&(mashort < mamidshort)&(mamidshort < mamid) &(mamid < mamidlong)&(mamidlong < malong)).astype(int)
        
        #malongin = ((clsadj > mashort)&(mashort > mamidshort)&(mamidshort > mamid) &(mamid > mamidlong)).astype(int)
        #mashortin = ((clsadj < mashort)&(mashort < mamidshort)&(mamidshort < mamid) &(mamid < mamidlong)).astype(int)
        ...
        # basis signal
        basis_std = pd.concat([basis_ticker,rt_std],axis=1)
        basis_upper = basis_std.iloc[:,1] * 8
        basis_upper = basis_upper.where(basis_upper < 0.05,other = 0.05)
        basis_lower = -basis_std.iloc[:,1] * 8
        basis_lower = basis_lower.where(basis_lower > -0.05,other = -0.05)
        
        basislongin = (basis_std.iloc[:,0] < basis_upper)
        basisshortin = (basis_std.iloc[:,0] > basis_lower)
        
        basislongout = (basis_std.iloc[:,0] > -basis_std.iloc[:,1] * 2)
        basisshortout = (basis_std.iloc[:,0] < basis_std.iloc[:,1] * 2)
        
        ### signal construct
        longin =  malongin & stdin & basislongin
        shortin =  mashortin & stdin & basisshortin
        
        #longout1 = turtlelongout
        #longout2 = dailyretlongout
        #longout3 = stdout
        
        longout = turtlelongout | dailyretlongout | stdout
        shortout = turtleshortout | dailyretshortout | stdout
        ...
        for i in range(1,len(prod_data)):
            ...
            #basis_ratio_ticker = basis_ticker.loc[cur_date]            
...
#            contract_pre = data_ticker_pre.index[idx]
    ...
    def get_trade_parameters(self, date, pos_df, omit_dce = True):        
        ...
        para_df1= para_df1.join(pos[['contract_holding','shares_holding']])
        para_df1['shares_holding'] = para_df1['shares_holding'].fillna(0)
        para_df1['curcontract'] = para_df1['curcontract'].where(pd.isna(para_df1['contract_holding']),other = para_df1['contract_holding'])
