...
class wsc_limit_14_no_st(FactorGenerator):
    ...
    def on_bar(self, data_dict):
        # 前一个交易日涨停的股票，今天开盘涨停但是尾盘开板的数量，除以前一天涨停的股票数量
        ...
        limit_judgement3 = (open_alla_daily_trun == limit_alla_daily)  # 判断开盘时股票是否涨停
        limit_judgement4 = limit_judgement1.shift(1) * limit_judgement2  # 前一个交易日涨停的股票今天尾盘是否涨停
        limit_judgement5 = limit_judgement1.shift(1) * limit_judgement3  # 前一个交易日涨停的股票今天开盘是否涨停
        
        
        factor = (limit_judgement5.sum(axis=1) - limit_judgement4.sum(axis=1)) / limit_judgement1.shift(1).sum(axis=1)
