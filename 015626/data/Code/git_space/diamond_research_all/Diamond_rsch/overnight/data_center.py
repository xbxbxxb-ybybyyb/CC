...
class HistoryData:
    def __init__(self, trade_date, date, hisdays=30, has_hist=False, need_raw=True):
        self.collector = dict()
    ...
    def get_stock_hisdata(self):
        ...
        for key in columns:
            target_data[key] = stk_full_mins_data[key].astype('float32')
    ...
    def get_all(self):
        print('sg-------------------------------------------------------------------------------------------------')
        if self.has_hist:
            hist_data_path = os.path.join(hisdata_path, self.trade_date.strftime('%Y%m%d'), 'history_%s.pkl' % minute_to_daily_tag)
            hist_data = diller(hist_data_path)[1]
            if 'raw_factors' in hist_data.keys():
                hist_data.pop('raw_factors')
            self.update_collector(hist_data)
        else:
...
class HotData:
    ...
    def get_all(self):
        a = pd.read_hdf(os.path.join(arch0_root, self.hotdir, self.ref_date.strftime('%Y%m%d'),
                       'alla_kline_1min_%s_%s.h5' % (trade_start_time.strftime('%H%M%S'), trade_mid_time.strftime('%H%M%S'))))
        b = pd.read_hdf(os.path.join(arch0_root, self.hotdir, self.ref_date.strftime('%Y%m%d'),
        ...
        mdconstant = pd.read_hdf(os.path.join(arch0_root, self.hotdir, self.ref_date.strftime('%Y%m%d'), 'mdconstant.h5'))
        misc_minute = pd.read_hdf(os.path.join(arch0_root, self.hotdir, self.ref_date.strftime('%Y%m%d'), 'misc_minute_%s.h5' % trade_stop_time.strftime('%H%M'))).sort_index()
        # format data
