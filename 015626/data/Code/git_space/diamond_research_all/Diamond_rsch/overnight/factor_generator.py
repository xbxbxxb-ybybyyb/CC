...
class FactorGenerator:
    ...
    def __init__(self, required_columns=None, ts_norm_method='ts_rank', ts_norm_bars=20, savepath=hisfactor_path, need_norm=True):
    ...
    @classmethod
    def prepare_hist_data(inst, trade_date=None, hisdays=15, has_hist=False, need_raw=True):
        ...
        hd = HistoryData(trade_date, ref_date, hisdays, has_hist, need_raw)
    ...
    @classmethod
    def merge_hot_data(inst, trade_date=None, mode='realtime', need_norm=True):
        if trade_date is None:
            trade_date =  pd.Timestamp.now().date()
        trade_date = IO.str_date_parser(trade_date)
        # load history data
...
def prepare_history(trade_date=None, hisdays=15, has_hist=False, need_raw=True):
    inst = FactorGenerator()
    inst.prepare_hist_data(trade_date=trade_date, hisdays=hisdays, has_hist=has_hist, need_raw=need_raw)
    inst.dump_hist_data()



def _get_factors(subcls, need_norm=True):
    print('calculating: ', subcls.__name__)
    return subcls(need_norm=need_norm).__callback__()


def executor(trade_date=None, max_workers=12, mode = 'realtime', tag='factors', need_norm=True):
    # load factors
    ...
    # merge hot data
    inst = FactorGenerator(need_norm=need_norm)
    inst.merge_hot_data(trade_date=trade_date, mode=mode, need_norm=need_norm)

    # import pdb; pdb.set_trace()
    get_factors = partial(_get_factors, need_norm=need_norm)
