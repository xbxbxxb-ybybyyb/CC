...
def prepare_edb_dummy(ref_date, last_num_days=20):
    ...
    out_path = os.path.join(arch0_root, 'hot_proof', end_date.strftime('%Y%m%d'), 'edb.h5')
...
def prepare_mdconstant_dummy(ref_date):
    ...
    out_path = os.path.join(arch0_root, 'hot_proof', ref_date.strftime('%Y%m%d'), 'mdconstant.h5')
    if not os.path.exists(os.path.dirname(out_path)):
        os.makedirs(os.path.dirname(out_path))
...
def prepare_misc_minute_dummy(ref_date):
    ...
    out_path = os.path.join(arch0_root, 'hot_proof', ref_date.strftime('%Y%m%d'), 'misc_minute_%s.h5' % trade_stop_time.strftime('%H%M'))
    if not os.path.exists(os.path.dirname(out_path)):
        os.makedirs(os.path.dirname(out_path))
...
def prepare_alla_kline_1min(ref_date):
    ...
    out_path = os.path.join(arch0_root, 'hot_proof', ref_date.strftime('%Y%m%d'))
