    ...
    def navigator(self,ts):
        ...
        if self.daily_opt or self.ts in self.transaction_days:
            if self.poke_func is not None:
                ...
                # filter blacklist for user weight
                custom_target_weight = self.poke_func(self.ts)
                # rescale custom target weight in case the sum is not properly scaled
                custom_portion = custom_target_weight.abs().sum()
                custom_portion = custom_portion if custom_portion <= 1 else 1
                custom_target_weight = vec_normalize(custom_target_weight, custom_portion)
                self.wt_target_dic[ts] = custom_target_weight
                self.opt_slicer(self.opt_func, self.ts)
            else:
                self.poke()
...
def opt_func(mu_path):
    ...
    def opt_func_helper(ts,benchmark):
        ...
        style_pd=IO.read_data(ts,ftype=FType.RISK,dsource=DSource.STYLEFACTOR)
