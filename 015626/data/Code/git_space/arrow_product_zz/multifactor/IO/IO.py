...
if platform.system() == 'Windows':
    sys_env = 'windows'
    data_source_path = r'Z:'  # disk Z name
else:
    sys_env = 'linux'
    data_source_path = '/data/group/800080'  # disk Z name
    data_save_path = '/data/user/012315'  # disk A name

h5_root_use = os.path.join(data_source_path, 'warehouse', 'prod')
...
def read_data(trading_days, columns=None, universe=None, mkttype=MktType.CHINA, dtype=DType.STOCK,
              ftype=FType.MD, dfreq=DFreq.DAILY, dsource=DSource.HTSC, dtable=None, alt=None, h5root=h5_root_use, max_workers=1):
    '''
    --- DESCRIPTION ---
...
def get_available_cols(mkttype=MktType.CHINA, dtype=DType.STOCK, ftype=FType.MD, dfreq=DFreq.DAILY, dsource=DSource.WIND, dtable=None, alt=None, h5root=h5_root_use):
    abs_h5_path = path_assembler(mkttype=mkttype, dtype=dtype, ftype=ftype, dfreq=dfreq, dsource=dsource, dtable=dtable, alt=alt, h5root=h5root)
    with pd.HDFStore(abs_h5_path, 'r') as h5_store:
        h5_root_keys = get_root_keys(h5_store)
        if len(h5_root_keys) == 1:
