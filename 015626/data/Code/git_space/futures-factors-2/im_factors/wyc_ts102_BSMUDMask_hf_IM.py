...
class wyc_ts102_BSMUDMask_hf_IM(FutureFactor):
    ...
    def calculate(self, data):
        buy = data['BuyTradeMoney'][-50:].rolling(20,min_periods = 15).sum().values[-30:]
        sell = data['SellTradeMoney'][-50:].rolling(20,min_periods = 15).sum().values[-30:]
        amount = data['amount'][-50:].rolling(20,min_periods = 15).sum().values[-30:]
        bs = buy-sell
        
        ret = data['close'][-36:].pct_change(5).replace([np.inf, -np.inf, 0], np.nan)[-30:].values
