...
def main():
    ...
    strategy = f'20241213_if_if_v7_crn_{path_id}'
    model_times = {
        'crn_cla': [1, 5, 10],
        'crn_reg': [1, 5, 10],
    ...
    factor_root = f'/data/group/800466/warehouse/test/alpha/CHINA_FUTURES/MINUTE/IF_shift_{path_id}/if_ever/minute_norm'
    ...
    strategy = f'20241213_ic_ic_v7unifac_crn_{path_id}'
    model_times = {
        'crn_cla': [1, 5, 10],
        'crn_reg': [1, 5, 10],
        'crn_spot_cla': [10, 20, 30],
    ...
    num_models = 2 * 5  # num_seeds x num_folds(5)
    factor_root = f'/data/group/800466/warehouse/test/alpha/CHINA_FUTURES/MINUTE/IC_shift_{path_id}/ic_unifac_ever/minute_norm'
    assert strategy.split('_')[0] == model_date
    update_model(strategy, model_times, num_models, factor_root, model_root, value_root, trade_date_list)

    strategy = f'20241213_im_im_v1unifac_crn_{path_id}'
    model_times = {
        'crn_cla': [1, 5, 10],
        'crn_reg': [1, 5, 10],
        'crn_spot_cla': [10, 20, 30],
    ...
    num_models = 2 * 5  # num_seeds x num_folds(5)
    factor_root = f'/data/group/800466/warehouse/test/alpha/CHINA_FUTURES/MINUTE/IM_shift_{path_id}/im_unifac_ever/minute_norm'
    assert strategy.split('_')[0] == model_date
    update_model(strategy, model_times, num_models, factor_root, model_root, value_root, trade_date_list)
...
if __name__ == '__main__':
    ...
    try:
        ...
        traceback.print_exc()
        link.LinkMessage().sendMessage('Error: model_value_mobius_20241213_path (data)')
