...
class Model(object):
    ...
    def train(self, x_train, y_train, m_train, x_valid, y_valid, m_valid):
        # set device
        self.move_to_gpu()

        # set loss function
        if self.config['objective'] == 'REG':
            self.criterion = nn.HuberLoss(delta=2.0)
        elif self.config['objective'] == 'CLA':
        ...
        # set scheduler
        self.scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, mode='min', factor=self.config['reduce_factor'], patience=self.config['update_period'], min_lr=self.config['minimum_lr'], verbose=False)
        ...
        train_indices = np.where(m_train > 0)[0]
        train_dataset = Subset(dataset=train_dataset, indices=train_indices)
        train_sampler = RandomSampler(data_source=train_dataset, replacement=True, num_samples=self.config['epoch_size'])
        train_loader = DataLoader(dataset=train_dataset, sampler=train_sampler, batch_size=self.config['batch_size'], drop_last=True)
        ...
        valid_indices = np.where(m_valid > 0)[0]
        valid_dataset = Subset(dataset=valid_dataset, indices=valid_indices)
        valid_sampler = SequentialSampler(data_source=valid_dataset)
        valid_loader = DataLoader(dataset=valid_dataset, sampler=valid_sampler, batch_size=self.config['batch_size'], drop_last=False)
        ...
        for epoch in range(self.config['num_epochs']):
            ...
            valid_corr, valid_std, valid_ac1, valid_ac5 = self.calculate_metrics(valid_output, valid_target)
            ...
            # print info
            basic_info = 'epoch: {:03d}, patience: {:02d}, train_loss: {:.4f}, valid_loss: {:.4f}, valid_corr: {:.4f}'.format(epoch, patience, train_loss, valid_loss, valid_corr)
            model_info = 'skip' if patience > 0 else 'save'
            extra_info = 'valid_std: {:.4f}, valid_ac1: {:.4f}, valid_ac5: {:.4f}, lr: {:.2e}, {}'.format(valid_std, valid_ac1, valid_ac5, self.optimizer.param_groups[0]['lr'], model_info)
            info = '{}, {}'.format(basic_info, extra_info)
    ...
    def predict(self, x):
        ...
        infer_loader = DataLoader(dataset=infer_dataset, sampler=infer_sampler, batch_size=self.config['batch_size'])
    ...
    def run_valid(self, valid_loader):
        self.network.eval()
        valid_samples = 0
        valid_loss = 0.0
        output_list = []
    ...
    def run_infer(self, infer_loader):
        self.network.eval()
        output_list = []
    ...
    def calculate_metrics(output, target):
        ...
        std = output.std()
        ac1 = output.corr(output.shift(1))
        ac5 = output.corr(output.shift(5))
        return corr, std, ac1, ac5
