...
from framework.model_v1 import Model
from framework.utils import normalize_return, fill_inf_and_nan, convert_to_prob, set_random_seed, save_pickle, load_pickle
...
def train(model_config, factor_base, ticker_type, update_date, return_time, random_seed, prod_version=True, save_info=True, show_info=True):
    ...
    for k, (train_index, valid_index) in enumerate(kf.split(fac_ins)):
        # set logger
        ...
        # process data
        fac_train_np, ret_train_np = fac_train.values, ret_train.values
        fac_valid_np, ret_valid_np = fac_valid.values, ret_valid.values
        if model_config['objective'] == 'BCE':
            ret_train_np = convert_to_prob(ret_train_np, a=model_config['coefficient'])
            ret_valid_np = convert_to_prob(ret_valid_np, a=model_config['coefficient'])
        ...
        # train model
        model.train(fac_train_np, ret_train_np, fac_valid_np, ret_valid_np)
