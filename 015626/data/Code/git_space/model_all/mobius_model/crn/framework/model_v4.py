...
class Model(object):
    ...
    def train(self, x_train, y_train, m_train, x_valid, y_valid, m_valid):
        ...
        clone_sampler = SequentialSampler(data_source=train_dataset)
        clone_loader = DataLoader(dataset=train_dataset, sampler=clone_sampler, batch_size=self.config['batch_size'], drop_last=False)
        ...
        for epoch in range(self.config['num_epochs']):
            self.run_train(train_loader)
            train_loss, train_output, train_target = self.run_valid(clone_loader)
            train_corr = self.calculate_metrics(train_output, train_target)
            ...
            info = 'epoch: {:03d}, patience: {:02d}, train_loss: {:.4f}, train_corr: {:.4f}, valid_loss: {:.4f}, valid_corr: {:.4f}, lr: {:.2e}'.format(epoch, patience, train_loss, train_corr, valid_loss, valid_corr, lr)
