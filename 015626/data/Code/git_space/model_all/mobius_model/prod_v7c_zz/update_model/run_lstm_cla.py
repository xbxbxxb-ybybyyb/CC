...
from strategy.fitting_model import pred_fit_lstm_cla_kf, get_train_test_sample, set_seed
...
from ts.utility.ts_utility import calc_hpr_recent, calc_hpr, calc_ts_truncation, ts_align_fitting_data, reduce_mem_usage
...
for suffix in suffix_list:
    ...
    if suffix in save_memory_model_list or suffix in use_generator_list:
        params['time_step'] = 10

    gpu_mem_size = 4
    fit_pred_func = partial(pred_fit_lstm_cla_kf, fold_num=fold_num, params=params, verbose=verbose, return_score=return_score,
                            return_misc=return_misc, plot_model=plot_model, track_feature_importance=track_feature_importance,
                            return_model=return_model,
                            res_iter_save_folder=res_iter_save_folder,
                            use_generator=False, bidirectional=False,
                            time_dist=False, shuffle=shuffle, rlrop_param=rlrop_param,
                            gpu_mem_size=gpu_mem_size)
    ...
    res_folder_dict = {iter_name: os.path.join(data_save_path, res_base_path, '%s_%d_r%d' % (model, iter_name, roll_day)) for iter_name in iter_list}
    res_dict_model = {}
    for iter_name in iter_list:
        ...
        res_iter_save_folder = res_folder_dict[iter_name]
        set_seed()
        fit_pred_func = partial(pred_fit_lstm_cla_kf, fold_num=fold_num, params=params, verbose=verbose, return_score=return_score,
                                return_misc=return_misc, plot_model=plot_model, track_feature_importance=track_feature_importance,
                                return_model=return_model,
                                res_iter_save_folder=res_iter_save_folder,
                                use_generator=use_generator, bidirectional=False,
                                time_dist=False, shuffle=shuffle, gpu_mem_size=gpu_mem_size)
