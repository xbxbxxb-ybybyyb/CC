...
from conf.templates_prod import *
...
class Generator:
    ...
    def write_params_file(self, mode, base_path, gen_trading_day, baseline_date=None, backend_params_dirpath=None,
                          mock=False, replay=False, strategy_name='MobiusCrossSectionCalculator', zone='${zone_id}', zone_ids=[]):
        if baseline_date is None:
            baseline_date = gen_trading_day
        params_generator = ParamsGenerator(self.real_env, self.user_ids, self.use_h5_data)
        content = params_generator.generate(gen_trading_day, baseline_date, mock)
        self.date_dir_path = os.path.join(base_path, gen_trading_day)
