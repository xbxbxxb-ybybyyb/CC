...
def CHECK(path = None, cat = 'IC'):
    ...
    if path == None:        
        ...
        default_path = '/data/user/015626/data/share/para/Mobius_%s/MobiusStrategy_%s_%s.xlsx'%(next_date, cat.upper(), next_date)
        if '_IC' in default_path:
            ...
            machine = '#503103'
        savepath = os.path.join('/data/user/015626/data/share/para/', 'Mobius_' + str(next_date))
...
if __name__ == '__main__':
    ...
    if UPLOAD == True:
        if np.nansum(holder) == 3:
            ...
            # 即老版本用来创建策略的excel文件
            excel_dict = {"IM": f"/data/user/015626/data/share/para/Mobius_{trade_date}/MobiusStrategy_IM_{trade_date}#503103.xlsx",
                          "IF": f"/data/user/015626/data/share/para/Mobius_{trade_date}/MobiusStrategy_IF_{trade_date}#503102.xlsx",
                          "IC": f"/data/user/015626/data/share/para/Mobius_{trade_date}/MobiusStrategy_IC_{trade_date}#503101.xlsx"}
            ...
            for k,v in factor_zone_dict.items():
                upload_strategy_file(strategy_id = "MobiusFactorStrategy", strategy_date = str(target_date), file_type = 1, 
                                        upload_file_path = os.path.join(json_path, f'{target_date}_factor_part{k}#{v}.json'), is_delete=False,  is_ready=1, disable_instance_validation=0, max_instance=1)
            for k,v in model_zone_dict.items():
                upload_strategy_file(strategy_id = "MobiusModelStrategy", strategy_date = str(target_date), file_type = 1, 
                                        upload_file_path = os.path.join(json_path, f'{target_date}_{k}_model#{v}.json'), is_delete=False,  is_ready=1, disable_instance_validation=0, max_instance=1)
            for k,v in trade_zone_dict.items():
                upload_strategy_file(strategy_id = "MobiusStrategy", strategy_date = str(target_date), file_type = 1, 
                                        upload_file_path = os.path.join(json_path, f'{target_date}_{k}_trade#{v}.json'), is_delete=False,  is_ready=1, disable_instance_validation=0, max_instance=1)
