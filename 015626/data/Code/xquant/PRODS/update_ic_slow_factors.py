...
factor_rootpath = '/data/user/016700/Trade_Codes/futures-factors-2/'
for folder in os.listdir(factor_rootpath):
    if kind == 'IF':
        ...
        if not folder.startswith('if_'):
            if not folder.startswith('im_'):
                sys.path.insert(4,os.path.join(factor_rootpath, folder))
    elif kind == 'IM':
        if folder.startswith('im_'):         
            sys.path.insert(4,os.path.join(factor_rootpath, folder))
...
impy = glob.glob(factor_rootpath + 'im_*/*.py')
icpy = list(set(allpy) - set(ifpy) - set(impy))
...
slow_factors = ['wyc_ts6_future_ar','wyc_ts50_future_nr_tr','wyc_ts5_future_nr_as' ,'wyc_ts5_future_nr_as_fast' ,'wyc_ts6_future_nr_cr' ,'wyc_ts6_future_ts' ,'xdy_ts13_future_nr_as_300_10' ,'xdy_ts13_future_nr_as_50_10' ,'xdy_ts15_future_nr_as' ,'xdy_ts1_spot_ar']
flist = [x for x in flist if x.__name__ in slow_factors]
...
def get_save_path(factor_name):
    ...
    if 'features_new' in folder:
        ...
        elif folder.startswith('im'):
            folder = 'IM_' + folder.split('_')[-1]
...
def minute_flag_check(date):
    path1 = flag_rootpath + str(date) + '/' + str(date) + '_CFG_ICIF.success'
#    path2 = flag_rootpath + str(date) + '/' + str(date) + '_INDUSTRY.success'
    ...
    path5 = flag_rootpath + str(date) + '/' + str(date) + '_tick_concat.success'
...
flag_path_start = flag_path + str(date) + '_%s_slow.start' % str.lower(kind)
...
for data_type in ['IndexStock']:
    ...
    def get_factor(factor):
        ...
        if os.path.exists(factor_raw_path):
            ...
            raw = drop_dup(origin_value.append(raw))
...
flag_path_success = flag_path + str(date) + '_' + '%s_slow.success' % str.lower(kind)
