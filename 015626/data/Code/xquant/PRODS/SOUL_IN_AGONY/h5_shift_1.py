...
#        print(para)
    ...
    tick = tick[(tick['MDTime'].astype(int) < 113000000) | (tick['MDTime'].astype(int) > 130000000)]
    tick['MDTime'] = tick['MDTime'].apply(lambda x: switch_time(x))
    ...
    transaction = transaction[(transaction['MDTime'].astype(int) < 113000000) | (transaction['MDTime'].astype(int) > 130000000)]
    transaction['MDTime'] = transaction['MDTime'].apply(lambda x: switch_time(x))
    ...
    order = order[(order['MDTime'].astype(int) < 113000000) | (order['MDTime'].astype(int) > 130000000)]
    order['MDTime'] = order['MDTime'].apply(lambda x: switch_time(x))
...
rootpath = '/dfs/group/800466/warehouse/prod/MD/MarketData/LOCAL_DATA/CSV/MINUTE/CHINA_STOCK/shift_%s/'%str(ss)
try:
    os.makedirs('/dfs/group/800466/warehouse/prod/MD/MarketData/LOCAL_DATA/CSV/MINUTE/CHINA_STOCK/shift_%s'%str(ss))
except:
    pass
h5_rootpath = '/dfs/group/800466/warehouse/prod/MD/MarketData/MD/CHINA_STOCK/new/MINUTE_shift_%s/'%str(ss)

try:
    os.makedirs(h5_rootpath)
except:
    pass
...
index_data_300 = IO.read_data(columns = ['close'], alt = '/dfs/group/800466/warehouse/prod/MD/MarketData/MD/CHINA_INDEX/MINUTE_shift_%s/000300.SH.h5'%str(ss))
index_close_300 = index_data_300['close'].xs('000300.SH', level = 1)
index_ret_300 = index_close_300.pct_change(1, fill_method=None)

index_data_500 = IO.read_data(columns = ['close'], alt = '/dfs/group/800466/warehouse/prod/MD/MarketData/MD/CHINA_INDEX/MINUTE_shift_%s/000905.SH.h5'%str(ss))
index_close_500 = index_data_500['close'].xs('000905.SH', level = 1)
index_ret_500 = index_close_500.pct_change(1, fill_method=None)

index_data_50 = IO.read_data(columns = ['close'], alt = '/dfs/group/800466/warehouse/prod/MD/MarketData/MD/CHINA_INDEX/MINUTE_shift_%s/000016.SH.h5'%str(ss))
index_close_50 = index_data_50['close'].xs('000016.SH', level = 1)
index_ret_50 = index_close_50.pct_change(1, fill_method=None)

index_data_1000 = IO.read_data(columns = ['close'], alt = '/dfs/group/800466/warehouse/prod/MD/MarketData/MD/CHINA_INDEX/MINUTE_shift_%s/000852.SH.h5'%str(ss))
...
def calc(date, stock):
    try:
        predate = int(udt.get_trading_day_offset(str(date), -3)[0].strftime('%Y%m%d'))

        WIND_AShareEODPrices = IO.read_data([predate, date], columns = ['S_DQ_ADJFACTOR'], alt = '/data/group/800080/warehouse/prod/DATABASE/WIND/AShareEODPrices/AShareEODPrices.h5')
        
        def get_adjfactor(stock, date,  WIND_AShareEODPrices =  WIND_AShareEODPrices):
            if stock == '689009.SH':
                return 1
            try:
...
weight_path = '/data/group/800466/warehouse/prod/MD/MarketData/MD/UNIVERSE/INDEXWEIGHT_CHINA_STOCK_DAILY_CSI.h5'
_ = get_target_list('IF.CFE', '20241008', '20250314') + get_target_list('IC.CFE', '20241008', '20250314') + get_target_list('IM.CFE', '20241008', '20250314')
_11 = pd.DataFrame(_)
_1 = _11.set_index([1, 0])
_1.index.names = ['stock', 'date']
stock_list = sorted(list(set(list(_11[1]))))


for stock in ['300360.SZ']:

        if True:

            #date_list = list(_1.xs(stock, level = 0).index)
            date_list = check_update_date(20250220, 20250314)[-1]
            print(date_list)
            print(date_list)
            print(stock, date_list[0], date_list[-1])
            hholder = Parallel(n_jobs= -1)(delayed(calc)(date, stock) for date in date_list)
            result = pd.concat(hholder).sort_index()
            result = result.reset_index().set_index('dt')
