...
for ticker in contract_list:
    ...
    if 'IC' in ticker:
        factor_folder = 'ic_unifac_ever'
        save_folder = next_tday + '_all'
    elif 'IF' in ticker:
        factor_folder = 'if_ever'
        save_folder = next_tday + '_all_if'
    elif 'IM' in ticker:
        factor_folder = 'im_unifac_ever'
        save_folder = next_tday + '_all_im'
    elif 'TREND' in ticker:
        factor_folder = 'TREND'
        save_folder = next_tday + '_trend'

    # gen factors
    factorlib_path = '/data/group/800466/warehouse/prod/alpha/CHINA_FUTURES/MINUTE/%s/minute_raw/'%factor_folder
    save_path = '/data/group/800466/warehouse/prod/alpha/CHINA_FUTURES/trade_files/%s/historyFactor' %save_folder
    ...
    norm_factorlib_path = '/data/group/800466/warehouse/prod/alpha/CHINA_FUTURES/MINUTE/%s/minute_norm/'%factor_folder
    dummies_path = '/data/group/800466/warehouse/prod/alpha/CHINA_FUTURES/MINUTE/dummies/minute_norm/'
    save_path = '/data/group/800466/warehouse/prod/alpha/CHINA_FUTURES/trade_files/%s/historyNormFactor' %save_folder
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    # 生成文件
...
lm = link.LinkMessage()
lm.sendMessage('PROD - FACTOR TRADE_FILES FINISHED!')
del lm 

flag_path_success = flag_rootpath + str(date) + '/' + 'trade_files_factors.success'
