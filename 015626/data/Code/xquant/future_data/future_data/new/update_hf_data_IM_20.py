...
#         sell_super_order = sell_super_order.groupby('minute').agg({'TradeBuyNo':'count', 'TradeMoney':'sum', 'TradeQty':'sum'}).rename(columns = {'TradeBuyNo':'sell_superorder_count','TradeMoney':'sell_superorder_money','TradeQty':'sell_superorder_volume'})
        ...
        transactiondf = pd.concat([selldf, buydf, sell_small_order_v2, sell_mid_order_v2, sell_big_order_v2, sell_super_order_v2, buy_small_order, buy_mid_order, buy_big_order, buy_super_order], axis = 1)
...
for ticker in ['IM.CFE']:
    rootpath = '/data/user/015626/data/share/LOCAL_DATA/CSV/MINUTE/CHINA_STOCK/tick_transaction_tominute_v2_zz1000/'
...
#     startdate,enddate,_ = check_update_date(int(str(tempdata.index[-1].date()).replace('-','')), None)
    startdate,enddate,_ = check_update_date(20201201, 20220721)
...
#        pool.map(get_cfg_hfdata, paralist)
    ...
    df.to_pickle('/data/user/015626/data/share/MD/CHINA_FUTURES/MINUTE/OUTSAMPLE_IM/temp_data/IM_cfg_hf_20201201_20220721.pkl')
