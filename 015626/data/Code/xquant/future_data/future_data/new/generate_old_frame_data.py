...
def get_stock(stock):
    ...
    sdf = IO.read_data([iw_date.index[0].strftime('%Y%m%d'), udt.get_trading_day_offset(iw_date.index[-1].strftime('%Y%m%d'),1)[0].strftime('%Y%m%d')], columns = [col], \
    ...
    return sdf[sdf['date'].isin(iw_date.index.date)][[stock]]
...
for col in tqdm(col_list):
    ...
    rdf = pd.concat(rlist, axis = 1, join= 'outer').sort_index()[stk_list]
