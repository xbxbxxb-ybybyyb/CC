...
standard_columns = ['open', 'high', 'low', 'close', 'twap', 'Buy1NumOrdersMean', 'Sell1NumOrdersMean', 'BidAskSpreadMean', 
                    ...
                    'sell_super_cancel_lo_hold_time', 'weight']

ffill_list = ['open','high','low','close','twap','weight']
...
def aggregate_transaction(transaction):
    ...
    if len(transaction) > 100:
        ...
        transactiondf = pd.concat([selldfgroup, buydfgroup, sell_small_order, sell_mid_order, sell_big_order, sell_super_order,sell_small_order_v2, sell_mid_order_v2, sell_big_order_v2, sell_super_order_v2, buy_small_order, buy_mid_order, buy_big_order, buy_super_order, market_map_limit_num, tran1, tran2], axis = 1)
...
def aggregate_order_SZ(order, transaction):
    ...
    if (len(order) > 100) & (len(transaction) > 100):
        ...
        orderdf = pd.concat([handle_normal_order(order_normal), handle_cancel_order(order_cancel)], axis = 1)
    return orderdf
...
for ticker in ['IM.CFE']:
    ...
    startdate,enddate,_ = check_update_date(20170103, 20170701)
    ...
    print(paralist)
    rootpath = '/data/user/015626/data/share/LOCAL_DATA/CSV/MINUTE/CHINA_STOCK/tick_transaction_order_tominute_zz1000/'
...
#     for i in range(0,len(standard_columns), 30):
#         clist = standard_columns[i:i+30]
#         print(clist)
