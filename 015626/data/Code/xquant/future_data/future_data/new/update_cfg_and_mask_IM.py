...
def ticker_match(ticker_num): # jit slow
    ...
    if ticker_num >= 800000:
     suffix = '.BJ'
    elif ticker_num >= 600000:
     suffix = '.SH'
    else:
     suffix = '.SZ'
...
def select_index(adf):
    ...
    t4 = adf.loc[(idx.hour == 14) & (idx.minute <= 56)]
    t = t1.append(t2).append(t3).append(t4)
    t = t.sort_index()
    return t

_ashare = IO.read_data([20080710, 21000101],columns = ['CHANGE_DT', 'FLOAT_A_SHR'], alt = '/data/group/800080/warehouse/prod/DATABASE/WIND/AShareCapitalization/AShareCapitalization.h5')
# calculate turnover
def add_turnover(df, stock):
    df = df.reset_index()
    df['CHANGE_DT'] = df.dt.apply(lambda x:int(str(x.date()).replace('-','')))
    try:
        ashare = _ashare.xs(stock, level = 1).reset_index()
    ...
    #     totaldf = totaldf.set_index(['dt','Ticker'])
        ...
        print(stock, e)
        totaldf = df.copy().set_index(['dt']).drop(['CHANGE_DT'], axis = 1)
        totaldf['float_shares'] = np.nan
        totaldf['turnover'] = np.nan
        return totaldf
...
def get_sp_stock(stock):
    ...
    stk_full_mins_data = stk_full_mins_data[stk_full_mins_data.dt >= 20160101]
...
with Pool(processes = 24) as pool:
    ...
    df20 = finaldf.loc[finaldf.index.get_level_values(0) >= pd.to_datetime('20170101')]
    df20.to_pickle('/data/user/015626/data/share/MD/CHINA_FUTURES/MINUTE/INSAMPLE_IM/temp_data/'+ticker[:2]+'_STOCKS_MINUTE_DATA_after2017.pkl')
    print('save_data done')
# 生成mask
df20 = df20.loc[pd.to_datetime('20170101'):].sort_index()
a = df20.copy()
mask_dict_20 = {}
...
picklesavename = ticker[:2] + '_cfg_data_insample.pkl'
...
for x in df20.columns:
    if 'PR' in x:
        continue
    print(x)
    df20[[x]].unstack().droplevel(0,axis =1).to_pickle('/data/user/015626/data/share/MD/CHINA_FUTURES/MINUTE/OUTSAMPLE_IM/cfg_preadj_zz1000/%s.pkl' % x)
    
for k,v in mask_dict_20.items():
    print(k)
    v.to_pickle('/data/user/015626/data/share/MD/CHINA_FUTURES/MINUTE/OUTSAMPLE_IM/cfg_preadj_zz1000/%s.pkl' % k)  
...
#for k in df20dict.keys():
#    a = df20dict[k]
#    idx = a.index
##    a = a.loc[~((idx.hour==14)&(idx.minute==57))]
#    df20dict[k] = a
#save_pickle(df20dict, '/data/user/015626/data/share/MD/CHINA_FUTURES/MINUTE/INSAMPLE/' + picklesavename)
#print(df20dict)