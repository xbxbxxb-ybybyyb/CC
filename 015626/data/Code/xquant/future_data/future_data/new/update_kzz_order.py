...
savepath = '/arch1/group/800466/warehouse/prod/MD/CHINA_CONVERTIBLE_BOND/Order/'
...
def get_kzz_data_tick(symbol, date):
    ...
    if os.path.exists(os.path.join(spath, str(date) + '.csv')):
        return
    bd = BondData()
    result_min = bd.get_bond_data(symbol, "%s 090000000" % str(date), "%s 150000000" % str(date), 'ORDER')
    del(bd)
    if len(result_min) == 0:
