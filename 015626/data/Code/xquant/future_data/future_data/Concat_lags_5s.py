...
# Save Path
# NO '/' AT THE END !
save_root_path = '/data/user/015626/data/share/LOCAL_DATA/CSV/FUTURES_5s/'
save_path_IC = save_root_path + 'IC'
save_path_IF = save_root_path + 'IF'
save_path_IH = save_root_path + 'IH'

begin_date, end_date, cdate_list = check_update_date(20200705,20200709)

print(begin_date, end_date, cdate_list)

begin_date = str(begin_date)
end_date = str(end_date)

print('check tick file')
flag = True
while(flag):
    if os.path.exists(os.path.join(read_path_IC, end_date + '.csv')):
        flag = False
print('check finish!')
    ...
    try:
            ...
            try:
                if count>0:
                    ...
                    if (int(data_dict[count][dt][11:13])*3600 + int(data_dict[count][dt][14:16])*60 + int(data_dict[count][dt][17:19]) - (int(temp[dt][11:13])*3600+int(temp[dt][14:16])*60+int(temp[dt][17:19])) >= n) or (int(data_dict[count][dt][17:19])%n == 0 and data_dict[count][dt][17:19] != temp[dt][17:19] ):
                        bl = 1
                    ...
                    if bl ==1 or count == 1:                                              
                        ...
                        if count1 >= 2:
                            ...
                            #High
                            data_dict_minute_concat[count1-1][4] = np.max(List_o[:, last_px].astype(float))
                            #Low
                            data_dict_minute_concat[count1-1][5] = np.min(List_o[:, last_px].astype(float))
...
def create_dict(w1,Contract, factor_number, l = li, l_factor = li_factors):
    ...
    df_0['dt'] = [(item[:18]+str(int(item[18])-int(item[18])%5)) for item in df_0['dt']]
    df_1['dt'] = [(item[:18]+str(int(item[18])-int(item[18])%5)) for item in df_1['dt']]
...
# 以下为更新csv数据为h5
h5rootpath = '/data/user/015626/data/share/MD/CHINA_FUTURES/5s'

def get_df(path, ticker):
    adf = pd.DataFrame()
    for c in cdate_list:
        a = pd.read_csv(os.path.join(path, str(c) + '.csv'))
        a = a.rename(columns = {a.columns.tolist()[0]:'dt'})
...
alldf = alldf.sort_values(['dt','Ticker'])
alldf = alldf.set_index(['dt','Ticker'])
alldf = alldf.sort_index()
IO.pd_hdf5_writer(alldf, os.path.join(h5rootpath, 'MD_STOCK_INDEX_FUTURES_5S_MAIN_ALL.h5'), dataset = 'all', append=True)
...
def select_index(df):
    ...
    t4 = df.loc[(idx.hour == 14) & (idx.minute <= 56)]
    t5 = df.loc[(idx.hour == 14) & (idx.minute == 57) & (idx.second == 0)]
    t = t1.append(t2).append(t3).append(t4).append(t5)
    t = t.sort_index()
    return t

alldf = select_index(alldf)
IO.pd_hdf5_writer(alldf, os.path.join(h5rootpath, 'MD_STOCK_INDEX_FUTURES_5S_MAIN.h5'), dataset = '5s', append=True)

def change_name(df, name):
    clist = df.columns.tolist()
    renamedict = {}
    for c in clist:
        renamedict[c] = c + '_' + name
    df = df.rename(columns = renamedict)
    return df
    
icdf = icdf.set_index(['dt','Ticker'])
icdf = change_name(select_index(icdf),'ic')
IO.pd_hdf5_writer(icdf, os.path.join(h5rootpath, 'MD_IC_5S_MAIN.h5'), dataset = 'ic', append=True)

ifdf = ifdf.set_index(['dt','Ticker'])
ifdf = change_name(select_index(ifdf),'if')
IO.pd_hdf5_writer(ifdf, os.path.join(h5rootpath, 'MD_IF_5S_MAIN.h5'), dataset = 'if', append=True)

ihdf = ihdf.set_index(['dt','Ticker'])
ihdf = change_name(select_index(ihdf),'ih')
IO.pd_hdf5_writer(ihdf, os.path.join(h5rootpath, 'MD_IH_5S_MAIN.h5'), dataset = 'ih', append=True)
