...
#     print(csvlist)
    if len(csvlist) > 0:
        ...
        df = pd.DataFrame(columns = ['dt','Ticker','MEETING_DT'])
    ...
    def Combine_Info(self, weights, predict_div_exdate, idx_close, tickers, div_info, predict_net_profit,
        ...
        for i in range(len(tickers)):
            ...
            if ticker in list(div_info.index):
                ...
                if (predict_net_profit[ticker] < 0) | (int(self.date) > int(self.date[: 4] + '0430')) | (ticker in ['689009.SH', '600925.SH', '301262.SZ', '301291.SZ']):
