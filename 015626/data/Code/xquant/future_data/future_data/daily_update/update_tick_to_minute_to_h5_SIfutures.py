...
df_temp = IO.read_data([20200101,21000101], alt = recentmonth_path)
sdate,edate,cdate_list = check_update_date(int(str(df_temp.reset_index().iloc[-1]['dt'])[:10].replace('-','')),None)
#sdate,edate,cdate_list = check_update_date(20241008, None)
...
flag_path_start = flag_root + str(end_date) + '_' + 'tick_to_minute_future_data_and_mask.start'
...
updatelist = []
for date in cdate_list[1:]:
    updatelist = updatelist + glob.glob('/data/user/015626/data/share/MD/CHINA_FUTURES/TICK/STOCK_INDEX_FUTURE/*/' + str(date) + '.csv')

for path in updatelist:
    print_csv(path)

############################## ----- write h5 ---- ###################################
create_h5_list = ['/data/user/015626/data/share/LOCAL_DATA/CSV/MINUTE/CHINA_FUTURES/tick_to_minute/ALL_CONTRACT/' + x[-19:] for x in updatelist]
...
def get_history_df(path):
    ...
    for contract in tqdm(os.listdir(path)):
        ...
        for col in ['open','high','low','close','position', 'vwap']:
...
def get_contract_df(pathlist):
    path = pathlist[0][:-19]
    contract_list = list(set([x[-19:-13] for x in pathlist]))
    date_csv_list = list(set([x[-12:] for x in pathlist]))
    date_csv_list.sort()
...
#                assert os.path.exists(csvpath), csvpath + ' not exists'
        ...
        mdf = index_df.join(mdf, how = 'left')
        for col in ['open','high','low','close','position', 'vwap', 'twap']:
            mdf[col] = mdf[col].fillna(method = 'ffill')
        mdf['close'] = mdf['close'].fillna(mdf['open'])
        mdf['vwap'] = mdf['vwap'].fillna(mdf['open'])
        mdf['twap'] = mdf['twap'].fillna(mdf['open'])   
        for col in ['volume','amount']:
            mdf[col] = mdf[col].fillna(value = 0)
...
nowdf = get_contract_df(create_h5_list)
IO.pd_hdf5_writer(nowdf, allcontract_path, dataset = 'contract', append = True)

#alldf_origin = IO.read_data([20100101, 21000101], alt=allcontract_path)
alldf_origin = nowdf
idf_origin = IO.read_data([start_date, end_date], alt='/data/user/015626/data/share/MD/CHINA_FUTURES/daily/MD_STOCK_INDEX_FUTURES_UNIVERSE.h5')
...
#rdf = rdf.sort_index()
#IO.pd_hdf5_writer(rdf, main_path, dataset='main', append = True)
...
# 以下是主力mask
#main_mask_dict = {}
#main_mask_name_dict = {'IC.CFE':'main_ic_mask','IF.CFE':'main_if_mask','IH.CFE':'main_ih_mask'}
#for t in ['IC.CFE','IF.CFE','IH.CFE']:
#    u = IO.read_data([20200101, 21000101], alt = '/data/user/015626/data/share/MD/CHINA_FUTURES/daily/MD_STOCK_INDEX_FUTURES_UNIVERSE.h5')
#    try:
#        u = u.xs(t, level = 1)[['contract_main']]
#    except:
#        continue
#    u['contract'] = u.contract_main.apply(lambda x:x[2:])
#    u = u.reset_index().rename(columns = {'dt':'date'})[['date','contract']].set_index('date')
...
#    main_mask_clist = main_mask.columns.tolist()
#    allclist = future_dict['open_if'].columns.tolist()
#    reslist = list(set(allclist) - set(main_mask_clist))
#    for c in reslist:
#        main_mask[c] = False
#    main_mask = main_mask.sort_index(axis = 1)
    
#    main_mask_dict[main_mask_name_dict[t]] = main_mask
...
flag_path_success = flag_root + str(end_date) + '_' + 'tick_to_minute_future_data_and_mask.success'
