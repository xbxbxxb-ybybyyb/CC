...
flag_path_start = flag_root + str(flag_date) + '_' + 'cfg_hf.start'
...
for ticker in ['IF.CFE','IC.CFE']:
    rootpath = '/data/user/015626/data/share/LOCAL_DATA/CSV/MINUTE/CHINA_STOCK/tick_transaction_tominute_v3/'
    outsample_path = '/data/user/015626/data/warehouse/prod/MD/CHINA_FUTURES/MINUTE/OUTSAMPLE/'
    temppath = '/data/user/015626/data/warehouse/prod/MD/CHINA_FUTURES/MINUTE/cfg_hf_data/%s_cfg_hf_2020.pkl' % (ticker[:2])
    tempdata = pd.read_pickle(temppath)
   
    startdate,enddate,_ = check_update_date(int(str(tempdata.index[-1].date()).replace('-','')), None)
#    startdate,enddate,_ = check_update_date(20230921, 20230922)
    ...
    print(ticker, 'concat data')
    df = pd.concat(dflist, axis = 0).sort_index()
    df = tempdata[:str(startdate)].append(df).sort_index()
    
    print(ticker, 'save origin data')
    df.to_pickle(temppath)
    print(ticker, 'save origin data done')

    suffix_dict = {'IC.CFE':'_500','IF.CFE':'_300','IH.CFE':'_50'}
    outdf = df.loc['20230101':]
...
#        out_dict[col+suffix_dict[ticker]] = outdf2[col]
    print(ticker, 'generate data')
    out_dict = {}
    for col in outdf.columns:
        out_dict[col+suffix_dict[ticker]] = outdf[col].unstack()
    ...
    print(ticker, 'save data')
    save_pickle(out_dict, os.path.join(outsample_path, '%s_cfg_hf_data_2023.pkl' % ticker[:2]))


flag_path_success = flag_root + str(flag_date) + '_' + 'cfg_hf.success'
