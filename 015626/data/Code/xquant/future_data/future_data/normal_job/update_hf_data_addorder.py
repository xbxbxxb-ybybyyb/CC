...
for ticker in ['IC.CFE', 'IF.CFE']:
    ...
    startdate,enddate,_ = check_update_date(20171130,20181201)
...
#        if x.endswith('_time'):
#            continue
#        newlist = [d[[x]] for d in dflist if (d is not None)]
#        df = pd.concat(newlist, axis = 0).sort_index()
#        df = df.unstack()[x]
#        df.to_pickle(os.path.join(insample_path,'%s_%s.pkl' % (x, suffix_dict[ticker])))

#    df = pd.concat(dflist, axis = 0).sort_index().add_suffix('_%s' % suffix_dict[ticker])
#    df = df.unstack()
