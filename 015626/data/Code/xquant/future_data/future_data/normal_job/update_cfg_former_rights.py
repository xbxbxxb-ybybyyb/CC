...
ticker = 'IF.CFE'
startdate, enddate = 20110101,20210101
savepath = '/data/user/015626/data/share/MD/CHINA_STOCK/'
...
stkdaily = IO.read_data([startdate, enddate],columns = ['adjfactor'], alt = '/data/group/800080/warehouse/prod/MD/CHINA_STOCK/DAILY/WIND/MD_CHINA_STOCK_DAILY_WIND.h5').unstack()
...
maskdf = (abs(stkdaily / stkdaily.shift(1) - 1) > 0.08)
maskdf.iloc[0,:] = True
...
sp_start_date['date'] = sp_start_date.dt.apply(lambda x:x - datetime.timedelta(days = 30))
...
def get_sp_stock(stock):
    ...
    for i in range(len(adj_chg_list)):
        ...
        if i == 0:
            ...
            tempdf1 = stk_full_mins_data.loc[(idx >= (adj_chgdate_list[i] - datetime.timedelta(days = 30))) & (idx < adj_chgdate_list[i])]
...
# nowlist = os.listdir(os.path.join(savepath, 'CFG_MINUTE', savenamedict[ticker]))
# nowstklist = [x[:-3] for x in nowlist]
# waitstklist = list(set(stklist) - set(nowstklist))
# for stock in tqdm(waitstklist):
#     get_sp_stock(stock)
...
with Pool(processes = 40) as pool:
    ...
    df20 = finaldf.loc[finaldf.index.get_level_values(0) >= pd.to_datetime('20160101')]
    df20.to_pickle('/data/user/015626/data/share/MD/CHINA_FUTURES/MINUTE/'+ticker[:2]+'_STOCKS_MINUTE_DATA_16_now_v3.pkl')
#     IO.pd_hdf5_writer(df20, '/data/user/015626/data/share/MD/CHINA_FUTURES/MINUTE/'+ticker[:2]+'_STOCKS_MINUTE_DATA_16_now_v3.h5', dataset = ticker[:2])
    df16 = finaldf.loc[finaldf.index.get_level_values(0) < pd.to_datetime('20160101')]
    df16.to_pickle('/data/user/015626/data/share/MD/CHINA_FUTURES/MINUTE/'+ticker[:2]+'_STOCKS_MINUTE_DATA_12_15_v3.pkl')
#     IO.pd_hdf5_writer(df16, '/data/user/015626/data/share/MD/CHINA_FUTURES/MINUTE/'+ticker[:2]+'_STOCKS_MINUTE_DATA_12_15_v3.h5', dataset = ticker[:2])
#     IO.pd_hdf5_writer(finaldf, os.path.join('/data/user/015626/data/share/MD/CHINA_FUTURES/MINUTE/',ticker[:2] + '_STOCKS_MINUTE_DATA_new.h5'), dataset = ticker[:2])


# update mask
df20 = df20.loc[pd.to_datetime('20161201'):pd.to_datetime('20200101')]
a = df20.copy()
mask_dict_20 = {}
ticker = 'IF.CFE'
...
picklesavename = 'IC_cfg_16_20_data.pkl'
...
index_data = IO.read_data([starttime, endtime], alt = '/data/user/015626/data/share/MD/CHINA_FUTURES/MINUTE/MD_STOCK_INDEX_SPOT_MINUTE.h5')
...
# save_pickle(mask_dict_20, '/data/user/015626/data/share/MD/CHINA_FUTURES/MINUTE/' + picklesavename)
...
save_pickle(df20dict, '/data/user/015626/data/share/MD/CHINA_FUTURES/MINUTE/' + 'IF_cfg_data_insample.pkl')