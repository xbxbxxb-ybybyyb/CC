...
savepath = '/arch1/group/800466/warehouse/prod/MD/CHINA_FUND/ETF/Tick/'
...
def get_kzz_data_tick(symbol, date):
    ...
    if os.path.exists(os.path.join(spath, str(date) + '.csv')):
        return
    fd = FundData()
    result_min = fd.get_fund_data(symbol, "%s 090000000" % str(date), "%s 150000000" % str(date), 'TICK')
    del(fd)
    if len(result_min) == 0:
...
def get_kzz_data_by_date(date):
    print(date)
    fd = FundData()
    kzz_list = fd.get_fund_set(str(date), 'ETF')
#    del(fd)
...
sdate,edate,cdate_list = check_update_date(20220101,20240821)
