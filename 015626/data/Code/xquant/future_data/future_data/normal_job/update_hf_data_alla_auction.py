...
def get_level2_data(para, append_cancel_orders=True, save_root_path = '/arch0/group/800466/warehouse/prod/MD/CHINA_STOCK_STOPPING/'):
    ...
    try:
        ...
        if dtype == 'Order':
            df = md.get_data_by_time_frame("Order", stock, f"{date} 091500000", f"{date} 092959999")
        elif dtype == 'Transaction':
            df = md.get_data_by_time_frame("Transaction", stock, f"{date} 091500000", f"{date} 092959999")
        elif dtype == 'Stock':
            df = md.get_data_by_time_frame("Stock", stock, f"{date} 091500000", f"{date} 092959999")
        else:
...
def retrieve_stopping_by_date(date, save_root_path = '/arch0/group/800466/warehouse/prod/MD/CHINA_STOCK_AUCTION_ALLA/'):
    print(date, ' start retrieve')
    stock_list = IO.read_data([date], columns = ['close']).index.get_level_values(1).tolist()
...
sdate,edate,cdate_list = check_update_date(20200101, 20220101)
