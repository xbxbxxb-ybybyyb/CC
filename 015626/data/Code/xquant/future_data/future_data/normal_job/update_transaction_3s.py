...
#        transaction['minute'] = transaction.dt.map(lambda x: x.replace(second=0,microsecond=0))
        transaction['seconds_10s'] = transaction.dt.map(lambda x: x.replace(microsecond=0))
        index3s = pd.date_range(str(date)+'090000',str(date)+'150000', freq ='10S').tolist()
        transaction.loc[~transaction.seconds_10s.isin(index3s),'seconds_10s'] = np.nan
        ...
        sell_small_order = sell_small_order.groupby('seconds_10s').agg({'TradeBuyNo':'count', 'TradeMoney':'sum', 'TradeQty':'sum'}).rename(columns = {'TradeBuyNo':'sell_smallorder_count','TradeMoney':'sell_smallorder_money','TradeQty':'sell_smallorder_volume'})
        sell_mid_order = sell_mid_order.groupby('seconds_10s').agg({'TradeBuyNo':'count', 'TradeMoney':'sum', 'TradeQty':'sum'}).rename(columns = {'TradeBuyNo':'sell_midorder_count','TradeMoney':'sell_midorder_money','TradeQty':'sell_midorder_volume'})
        sell_big_order = sell_big_order.groupby('seconds_10s').agg({'TradeBuyNo':'count', 'TradeMoney':'sum', 'TradeQty':'sum'}).rename(columns = {'TradeBuyNo':'sell_bigorder_count','TradeMoney':'sell_bigorder_money','TradeQty':'sell_bigorder_volume'})
        sell_super_order = sell_super_order.groupby('seconds_10s').agg({'TradeBuyNo':'count', 'TradeMoney':'sum', 'TradeQty':'sum'}).rename(columns = {'TradeBuyNo':'sell_superorder_count','TradeMoney':'sell_superorder_money','TradeQty':'sell_superorder_volume'})
    ...
    result.loc[str(date)+'093000':str(date)+'112959'].append(result.loc[str(date)+'130000':str(date)+'145559']).to_csv(filepath)
...
for ticker in ['IC.CFE']:
    ...
    paralist = get_target_list(ticker,20190401,20220616)
