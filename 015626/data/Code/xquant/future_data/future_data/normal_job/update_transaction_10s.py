...
def get_index_fromdate(date):
    t_mins_list = pd.date_range('09:30:00', '11:29:50', freq='10S').to_list() + pd.date_range('13:00:00','14:55:50',freq='10S').to_list()
...
for ticker in ['IC.CFE']:
    ...
    paralist = get_target_list(ticker,20190401,20210101)
...
#        dflist.append(get_csvdf(para))
    ...
    print('sort done!')
    dfnew.to_pickle('/data/user/015626/data/share/MD/CHINA_FUTURES/10s/zz500_cfg_10s.pkl')
    
    insample = dfnew.loc[:pd.to_datetime('20210101')].unstack()
