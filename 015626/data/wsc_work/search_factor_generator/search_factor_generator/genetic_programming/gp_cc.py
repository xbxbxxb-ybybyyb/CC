...
from utils_wsc.help_functions import read_pickle, rolling_norm
...
# 数据导入，需导入因子值和收益两项数据
path = '/data/user/012398/data/alpha/CHINA_FUTURES/MINUTE/IC_prod/'
factors = os.listdir(path)
factors = [os.path.join(path, i) for i in factors]
X = None
for i_name in factors:
    factor = pd.read_hdf(i_name).xs('IC.CFE', level=1)
    X = factor if X is None else pd.concat([X, factor], axis=1)
X = X.fillna(0)
XT = np.transpose(X)
...
pd.DataFrame(weighted_matrix).to_csv('/data/user/017024/GA_CC.csv')
