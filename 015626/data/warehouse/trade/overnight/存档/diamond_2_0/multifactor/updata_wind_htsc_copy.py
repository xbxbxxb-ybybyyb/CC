        ...
        if self.table_name == 'WIND_AShareMonthlyReportsofBrokers':
            ...
            if self.table_name in self.over_write_list:
                try:
                    ...
                    logger.warning('%s 出现了异常，异常信息如下：'%(self.table_name))
                    print(err)
                    df = s.get_factor_value(self.table_name, S_CON_INDATE=['>=20130101']).append(s.get_factor_value(self.table_name, S_CON_INDATE=['<20130101']))
                    df['date'] = self.cdate_list[-1]
                    save_name = os.path.join(self.table_csv_path,self.table_name + '.csv')
                    df.set_index('OBJECT_ID', inplace = True)
                    df.to_csv(save_name, sep=',', encoding='utf_8_sig')
...
# AShareMonthlyReportsofBrokers
# AShareManagementHoldReward
# AShareIndustriesClassCITICS
    
    qtr_list = []
    
    first_daily_list = ['WIND_AShareEODDerivativeIndicator']
