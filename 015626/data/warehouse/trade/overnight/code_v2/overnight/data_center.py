...
class HistoryData:
    ...
    def get_mask_for_future_data(self, start_date, end_date):
        contract = get_trade_contract(start_date, end_date, prod_id='IF.CFE')
...
class HotData:
    ...
    def get_all(self):
        ...
        recent_month_contract = get_current_futures_contract('IF.CFE', trade_date=self.ref_date.strftime('%Y%m%d'))
        recent_month_contract = re.sub("\D", "", recent_month_contract)
        mask = target_data['close_IF.CFE']
