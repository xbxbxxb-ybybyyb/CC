...
class FactorGenerator:
    ...
    def dump_hist_data(inst):
        ...
        diller(os.path.join(save_path, 'history_%s.pkl' % trade_stop_time.strftime('%H%M')), (inst.__trade_date__, inst.__data__, inst.__mdconstant__))
    ...
    def load_hist_data(inst, trade_date=None):
        ...
        _trade_date, _data, _mdconstant = diller(os.path.join(save_path, 'history_%s.pkl' % trade_stop_time.strftime('%H%M')))
...
def executor(trade_date=None, max_workers=12):
    # load factors
    ...
    trading_plan_1_0, trading_plan_2_0 = signal_dealer(factor_score['norm'], spot_close_dict, recent_contract, season_contract, volume_limit_dict)
    print('-' * 60)
    print('Diamond 1.0:')
    ...
    lm.sendMessage('Diamond 1.0:\n' + str(trading_plan_1_0[['Account_Num', 'Contract_Num','Seconds_Interval']]))
    lm.sendMessage('Diamond 2.0:\n' + str(trading_plan_2_0[['Account_Num', 'Contract_Num','Seconds_Interval']]))
    print('-' * 60)
    # dump factor
    ...
    # dump trading plan
    trading_plan_savepath = os.path.join(trading_plan_path, trade_date)
    if not os.path.exists(trading_plan_savepath):
        os.makedirs(trading_plan_savepath)
    trading_plan_1_0.to_csv(os.path.join(trading_plan_savepath, '%s_1_0.csv' % trade_date))
    trading_plan_2_0.to_csv(os.path.join(trading_plan_savepath, '%s_2_0.csv' % trade_date))
