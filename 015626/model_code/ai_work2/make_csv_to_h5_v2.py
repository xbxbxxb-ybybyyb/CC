...
def get_adjfactor(stock, date):
    s = FactorData()
    df = s.get_factor_value('WIND_AShareEODPrices',factors = ['S_INFO_WINDCODE','S_DQ_ADJFACTOR'], trade_dt=[str(date)])
    del(s)
...
def get_h5_by_stock(stock):
    try:
        ...
        if len(csvdf_list) == 0:
            link_send_message(stock + ' no csv!!!')
            print(stock, 'no csv!!!')
            return
        finaldf = pd.concat(csvdf_list, axis = 0).sort_index()

        finaldf = add_turnover_rate_and_adj(finaldf, stock)
        stk_ret = (finaldf['close'] * finaldf['adjfactor'] / finaldf.iloc[-1]['adjfactor']).pct_change(1, fill_method = None)
...
temp_fs2['CHANGE_DT'] = temp_fs2['CHANGE_DT'].astype('int')
ashare_total = pd.concat([ashare_total, temp_fs, temp_fs2])
adj_df = IO.read_data([20160101, 20230822],columns = ['adjfactor'], alt = '/data/group/800080/warehouse/prod/MD/CHINA_STOCK/DAILY/WIND/MD_CHINA_STOCK_DAILY_WIND.h5')
...
index_data_300 = IO.read_data([20160101, 20230822],columns = ['close'], alt = '/data/group/800466/warehouse/prod/MD/MarketData/MD/CHINA_INDEX/MINUTE/000300.SH.h5')
index_close_300 = index_data_300['close'].xs('000300.SH', level = 1)
index_ret_300 = index_close_300.pct_change(1, fill_method=None)

index_data_500 = IO.read_data([20160101, 20230822],columns = ['close'], alt = '/data/group/800466/warehouse/prod/MD/MarketData/MD/CHINA_INDEX/MINUTE/000905.SH.h5')
index_close_500 = index_data_500['close'].xs('000905.SH', level = 1)
index_ret_500 = index_close_500.pct_change(1, fill_method=None)

index_data_50 = IO.read_data([20160101, 20230822],columns = ['close'], alt = '/data/group/800466/warehouse/prod/MD/MarketData/MD/CHINA_INDEX/MINUTE/000016.SH.h5')
index_close_50 = index_data_50['close'].xs('000016.SH', level = 1)
index_ret_50 = index_close_50.pct_change(1, fill_method=None)

index_data_1000 = IO.read_data([20160101, 20230822],columns = ['close_spot'], alt = '/data/user/015626/data/share/MD/CHINA_FUTURES/MINUTE/XQUANT_MINUTE/MD_STOCK_INDEX_SPOT_MINUTE.h5')
...
csv_rootpath = '/data/user/015626/data/share/LOCAL_DATA/CSV/MINUTE/CHINA_STOCK/tick_transaction_order_tominute_v3/'
h5_path = '/data/group/800466/warehouse/prod/MD/MarketData/MD/CHINA_STOCK/MINUTE_sim2/'

weight = IO.read_data([20161130,20220822], alt = '/data/group/800080/warehouse/prod/INDEXWEIGHT/CHINA_STOCK/DAILY/CSI/INDEXWEIGHT_CHINA_STOCK_DAILY_CSI.h5')
weight = weight.sum(axis = 1)
ticker_list = weight[weight > 0].index.get_level_values(1).unique().tolist()
ticker_list.sort()
print('start concat h5')
with Pool(24) as pool:
    pool.map(get_h5_by_stock, ticker_list[1000:2000])

link_send_message('2000')