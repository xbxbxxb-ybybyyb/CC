...
m2 = pd.read_hdf('/data/user/015626/data/share/LOCAL_DATA/stock_open/M2_slippage.h5')
m2_yesterday = m2.unstack().shift(1).stack()
m2 = m2.loc['20220105':]

m2 = pd.concat([m2,m2_yesterday], axis = 1)
m2.columns = ['today','yesterday']

# m2 = m2.loc['20220530':]

md = IO.read_data([20211120,20220909],columns=['open','close'])
md['next_open'] = md['open'].unstack().shift(-1).stack()
m2 = m2.join(md, how = 'left')

# price_index_list = [-2,-1,0,1,2]
def get_dealinfo_submit(para):
    date = para[0]
    stock = para[1]
#     date = 20220105
#     stock = '000782.SZ'
    ...
    buy1price = tick.iloc[-1]['Buy1Price']
    ...
    for i in range(1, len(timedelta_list)):
        ...
        px_qty_list = sorted(px_qty_list, key=lambda x:x[0], reverse = True)
    ...
    submit_list = [[openpx + i*0.01, order_list[0]] for i in price_index_list]
    ...
    while deal_vol < np.sum(order_list) and order_idx < len(order_list) and deal_vol < target_vol:
        ...
        submit_list = sorted(submit_list, key = lambda x:x[0], reverse=False)
...
m2 = m2.dropna(subset = ['today'])
_m2 = m2.reset_index()
dtlist = [x.strftime('%Y%m%d') for x in _m2.dt.tolist()]
tickerlist = _m2.Ticker.tolist()
paralist = [x for x in zip(dtlist,tickerlist)]

pi_list = [[0,1,2,3,4],[-1,0,1,2,3],[-2,-1,0,1,2],[-3,-2,-1,0,1],[-4,-3,-2,-1,0],[-5,-4,-3,-2,-1],[-6,-5,-4,-3,-2]]
ov_list = [0.6, 0.8, 1, 1.2, 1.5, 2]
    ...
    for ordervol_ratio in ov_list:
        for chedan in [True]:
            ...
            with Pool(24) as pool:
                rlist = pool.map(get_dealinfo_submit, paralist)
            tranr = pd.concat(rlist, axis = 0)
            tranr['dt'] = pd.to_datetime(tranr['dt'])
            tranr = tranr.set_index(['dt', 'Ticker']).sort_index()
            m2dealinfo = m2.join(tranr, how = 'left')

            m2dealinfo['deal_px'] = m2dealinfo['deal_money'] / m2dealinfo['deal_vol']
            m2dealinfo['slippage'] = m2dealinfo['deal_px'] / m2dealinfo['open'] - 1
            slippage = m2dealinfo['slippage'].mean()

            print(price_index_list, ordervol_ratio, chedan, slippage)
            with open('/data/user/015626/data/share/LOCAL_DATA/stock_open/stock_open_new.txt', 'a') as file:
                file.write(str(datetime.datetime.now()) + '#'+ str(price_index_list) + '#' + str(ordervol_ratio)+ '#'+ str(chedan) + str(slippage) + '\r\n')            