...
end_date_standard = '20220601'
start_date_standard = '20150301'
days_interval = 60

save_path = '/data/group/800466/warehouse/test/alpha/CHINA_FUTURES/MINUTE/%s_IC_prod/' % kind

factor_path = '/data/user/015626/data/share/LOCAL_DATA/Mobius/IM_factors_fromICIFprod/IC_prod_IM/'
...
for dtype in ['Future', 'IndexStock']:
...
#            continue
        aaa = pd.read_hdf(os.path.join(save_path,'minute_norm',factor_name+'.h5'))
        if int(aaa.index[0].date().strftime('%Y%m%d')) < 20210101:
            continue
