...
def update_by_date(date, ncore = 24):
    ...
    today_uplist = get_target_list('IM.CFE',last_tday,date) 
    nexttday_uplist = get_nexttday_target_list('IM.CFE',date,date) 
...
h5_rootpath = '/data/group/800466/warehouse/prod/MD/MarketData/MD/CHINA_STOCK/MINUTE_sim2/'

link_send_message('kaishi')
# sdate,edate,cdate_list = check_update_date(20220822, 20220822)
sdate,edate = 20220822,20220822
cdate_list = [20220822]
date = edate
...
from xquant.factordata import FactorData
s = FactorData()
WIND_AShareEODPrices = s.get_factor_value('WIND_AShareEODPrices',factors = ['S_INFO_WINDCODE','S_DQ_ADJFACTOR'], trade_dt=[str(date)])
temp_adj = pd.DataFrame(['689009.SH', 1], index = WIND_AShareEODPrices.columns).T
WIND_AShareEODPrices = pd.concat([WIND_AShareEODPrices, temp_adj]).reset_index(drop = True)
ashare_total = s.get_factor_value('WIND_AShareCapitalization', factors = ['S_INFO_WINDCODE','CHANGE_DT', 'FLOAT_A_SHR']).rename(columns = {'S_INFO_WINDCODE':'Ticker'}).set_index('Ticker')
ashare_total['CHANGE_DT'] = ashare_total['CHANGE_DT'].astype('int')
temp_fs = pd.DataFrame([20220601, 50900], index = ashare_total.columns, columns = ['689009.SH']).T
temp_fs['CHANGE_DT'] = temp_fs['CHANGE_DT'].astype('int')
...
tomorrowdate = int(udt.get_trading_day_offset(str(date), 1)[0].strftime('%Y%m%d'))

index_data_300 = IO.read_data([pre6date, tomorrowdate], columns = ['close'], alt = '/data/group/800466/warehouse/prod/MD/MarketData/MD/CHINA_INDEX/MINUTE/000300.SH.h5')
index_close_300 = index_data_300['close'].xs('000300.SH', level = 1)
index_ret_300 = index_close_300.pct_change(1, fill_method=None)

index_data_500 = IO.read_data([pre6date, tomorrowdate], columns = ['close'], alt = '/data/group/800466/warehouse/prod/MD/MarketData/MD/CHINA_INDEX/MINUTE/000905.SH.h5')
index_close_500 = index_data_500['close'].xs('000905.SH', level = 1)
index_ret_500 = index_close_500.pct_change(1, fill_method=None)

index_data_50 = IO.read_data([pre6date, tomorrowdate], columns = ['close'], alt = '/data/group/800466/warehouse/prod/MD/MarketData/MD/CHINA_INDEX/MINUTE/000016.SH.h5')
index_close_50 = index_data_50['close'].xs('000016.SH', level = 1)
index_ret_50 = index_close_50.pct_change(1, fill_method=None)

index_data_1000 = IO.read_data([pre6date, tomorrowdate], columns = ['close_spot'], alt = '/data/user/015626/data/share/MD/CHINA_FUTURES/MINUTE/XQUANT_MINUTE/MD_STOCK_INDEX_SPOT_MINUTE.h5')
index_close_1000 = index_data_1000['close_spot'].xs('IM.CFE', level = 1).between_time('930','1456').sort_index()
index_ret_1000 = index_close_1000.pct_change(1, fill_method=None)
