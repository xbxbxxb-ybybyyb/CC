...
from .functions import _function_map, _Function, sigmoid
...
def _parallel_evolve(n_programs, parents, X, y, starttime_insample, endtime_insample, starttime_outsample,
                     endtime_outsample, data_drop_list, sample_weight, seeds, params):
    ...
    const_arities = params['const_arities']
    init_depth = params['init_depth']
    init_method = params['init_method']
    const_range = params['const_range']
    const_params_range = params['const_params_range']
    ...
    def _tournament():
        ...
        if np.isnan(fitness).sum() == len(fitness):
            parent_index = contenders[random_state.randint(0, tournament_size)]
        else:
    ...
    for i in range(n_programs):
        ...
        if parents is None:
            ...
            if method < method_probs[0]:
                # crossover
                # print('crossover')
                donor, donor_index = _tournament()  # donor是个_Program类
        ...
        program = _Program(function_set=function_set,
                           ...
                           const_params_range=const_params_range,
        ...
        program.raw_fitness_diverse_, program.validation_fitness_diverse_ = \
            program.raw_fitness(X, y, starttime_insample, endtime_insample, starttime_outsample, endtime_outsample,
                                data_drop_list, curr_sample_weight)
        # if isinstance(program.raw_fitness_diverse_, float) or isinstance(program.raw_fitness_diverse_, int):
        #     program.raw_fitness_ = program.raw_fitness_diverse_
        #     program.validation_fitness_ = program.validation_fitness_diverse_
        # else:
        #     program.raw_fitness_ = program.raw_fitness_diverse_.mean()
        #     program.validation_fitness_ = program.validation_fitness_diverse_.mean()
        program.raw_fitness_ = program.raw_fitness_diverse_.mean()
        program.validation_fitness_ = program.validation_fitness_diverse_.mean()
        if max_samples < n_samples:
            # Calculate OOB fitness
            program.oob_fitness_diverse_, program.oob_validation_fitness_diverse_ = \
                program.raw_fitness(X, y, starttime_insample, endtime_insample, starttime_outsample, endtime_outsample,
                                    data_drop_list, oob_sample_weight)
            # if isinstance(program.oob_fitness_diverse_, float) or isinstance(program.oob_fitness_diverse_, int):
            #     program.oob_fitness_ = program.oob_fitness_diverse_
            #     program.oob_validation_fitness = program.oob_validation_fitness_diverse_
            # else:
            #     program.oob_fitness_ = program.oob_fitness_diverse_.mean()
            #     program.oob_validation_fitness = program.oob_validation_fitness_diverse_.mean()
            program.oob_fitness_ = program.oob_fitness_diverse_.mean()
            program.oob_validation_fitness = program.oob_validation_fitness_diverse_.mean()
...
class BaseSymbolic(BaseEstimator, metaclass=ABCMeta):
    ...
    def __init__(self,
                 ...
                 verbose=0,
                 random_state=None,
                 factor_save_path='/data/user/017024/data/',
                 rolling_window=1200,
                 time_now=None,
                 threshold=None):
        ...
        self.factor_save_path = factor_save_path
    ...
    def _verbose_reporter(self, run_details=None):
        ...
        if run_details is None:
            print('    |{:^46}|{:^63}|'.format('Population Average',
                                               'Best Individual'))
            print('-' * 4 + ' ' + '-' * 46 + ' ' + '-' * 63 + ' ' + '-' * 10)
            line_format = '{:>4} {:>8} {:>12} {:>24} {:>8} {:>12} {:>24} {:>16} {:>10}'
            print(line_format.format('Gen', 'Length', 'Fitness', 'Validation Fitness', 'Length',
                                     'Fitness', 'Validation Fitness', 'OOB Fitness', 'Time Left'))
            ...
            oob_fitness = 'N/A'
            line_format = '{:4d} {:8.2f} {:12g} {:24g} {:8d} {:12g} {:24g} {:>16} {:>10}'
            if self.max_samples < 1.0:
                oob_fitness = run_details['best_oob_fitness'][-1]
                line_format = '{:4d} {:8.2f} {:12g} {:24g} {:8d} {:12g} {:24g} {:16g} {:>10}'

            print(line_format.format(run_details['generation'][-1],
                                     run_details['average_length'][-1],
                                     run_details['average_fitness'][-1],
                                     run_details['average_validation_fitness'][-1],
                                     run_details['best_length'][-1],
                                     run_details['best_fitness'][-1],
                                     run_details['best_validation_fitness'][-1],
                                     oob_fitness,
                                     remaining_time))

    def fit(self, X, y, starttime_insample, endtime_insample, starttime_outsample, endtime_outsample,
            data_drop_list=None, sample_weight=None, result_index = None):
        ...
        starttime_insample : pd.datetime
            training set start time
        endtime_insample : pd.datetime
            training set end time
        starttime_outsample : pd.datetime
            validation set start time
        endtime_outsample : pd.datetime
            validation set end time
        data_drop_list : list or None
            the index of data which needed to be dropout
        Returns
        -------
        self : object
            Returns self.
        ...
        for function in self._function_set:
            ...
            self._const_arities[_const_arity] = self._const_arities.get(_const_arity, [])
            self._const_arities[_const_arity].append(function)
        ...
        if not ((isinstance(self.const_params_range, tuple) and
                 len(self.const_params_range) == 2) or self.const_params_range is None or isinstance(self.const_params_range, list)):
            raise ValueError('const_range should be a tuple with length two, or None, or list')
        ...
        params['const_arities'] = self._const_arities
        ...
        for gen in range(prior_generations, self.generations):
            ...
            population = Parallel(n_jobs=n_jobs, max_nbytes='5G',
            ...
            population = list(itertools.chain.from_iterable(population))  # population是个list，里面的元素是_Program类

            fitness = [program.raw_fitness_ for program in population]
            validation_fitness = [program.validation_fitness_ for program in population]

            if self._metric.greater_is_better:
                fitness_detail_insample = [[str(program), program.raw_fitness_diverse_]
                                           for program in population if program.raw_fitness_ > self.threshold]
                fitness_detail_insample = pd.DataFrame([i[1] for i in fitness_detail_insample],
                                                       index=[i[0] for i in fitness_detail_insample])
                fitness_detail_outsample = [[str(program), program.validation_fitness_diverse_]
                                            for program in population if program.raw_fitness_ > self.threshold]
                fitness_detail_outsample = pd.DataFrame([i[1] for i in fitness_detail_outsample],
                                                        index=[i[0] for i in fitness_detail_outsample])
            else:
                fitness_detail_insample = [[str(program), program.raw_fitness_diverse_]
                                           for program in population if program.raw_fitness_ < self.threshold]
                fitness_detail_insample = pd.DataFrame([i[1] for i in fitness_detail_insample],
            ...
            #     if (self._metric.greater_is_better and program.raw_fitness_ > self.threshold) or (not self._metric.greater_is_better and program.raw_fitness_ < self.threshold):
            #         ff = pd.DataFrame(program.execute(X), index=result_index)
            #         ff.to_hdf(os.path.join(self.factor_save_path, 'factors', self.time_now,
            #                                                    str(gen) + '_' + str(program)) + '.h5', key = 'h5')
            fitness_detail_insample_unique = fitness_detail_insample[~fitness_detail_insample.index.duplicated()]
            fitness_detail_outsample_unique = fitness_detail_outsample[~fitness_detail_outsample.index.duplicated()]
            fitness_detail_insample_unique.to_csv(os.path.join(self.factor_save_path, 'fitness', self.time_now,
                                                               str(gen) + '_fitness_detail_insample') + '.csv')
            fitness_detail_outsample_unique.to_csv(os.path.join(self.factor_save_path, 'fitness', self.time_now,
                                                                str(gen) + '_fitness_detail_outsample') + '.csv')
            ...
            # Record run details
            if self._metric.greater_is_better:
                best_program = population[np.nanargmax(fitness)]
                # print(best_program)
            else:
                best_program = population[np.nanargmin(fitness)]
                # print(best_program)

            self.run_details_['generation'].append(gen)
            self.run_details_['average_length'].append(np.mean(length))
            self.run_details_['average_fitness'].append(np.nanmean(fitness))
            self.run_details_['average_validation_fitness'].append(np.nanmean(validation_fitness))
            self.run_details_['best_length'].append(best_program.length_)
            self.run_details_['best_fitness'].append(best_program.raw_fitness_)
            self.run_details_['best_validation_fitness'].append(best_program.validation_fitness_)
            oob_fitness = np.nan
            if self.max_samples < 1.0:
            ...
            if fitness_detail_insample.shape[0] > 0:
                print('{} factors is qualified in generation {}, repetition rate is {}'.format(
                    str(fitness_detail_outsample_unique.shape[0]), str(gen),
                    str(1 - fitness_detail_outsample_unique.shape[0] / fitness_detail_insample.shape[0])))
            else:
                print('No factors is qualified in generation {}'.format(str(gen)))
        ...
        #         #     if self._metric.greater_is_better:
        #         #         hall_of_fame = (fitness * (-1)).argsort()[:self.hall_of_fame]
        #         #     else:
        ...
        #         #         correlations = np.ma.corrcoef(np.ma.masked_invalid(evaluation)).data
