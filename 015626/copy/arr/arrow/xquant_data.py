...
def get_level2_data(para, save_path, dtype, append_cancel_orders=False, force_override=False, use_thirdparty = True):
    ...
    try:
        if use_thirdparty:
            ...
            if dtype == 'Order' or dtype =='Order_RAW':
                if dtype == 'Order' and '.SH' in stock:
                    df_transaction = md.get_data_by_date("Transaction", stock, str(date))
                    df_order = md.get_data_by_date("Order", stock, str(date))
                    df = reform_sh_order(df_order, df_transaction, append_cancel_orders=append_cancel_orders)
                else:
                    df = md.get_data_by_date("Order", stock, str(date))
            elif dtype == 'Transaction':
                df = md.get_data_by_date("Transaction", stock, str(date))
            elif dtype == 'Stock':
