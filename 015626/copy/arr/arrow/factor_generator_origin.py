...
class FactorGenerator:
    ...
    @classmethod
    def merge_hot_data(inst, trade_date=None, factor_mode = None, only_data = False, kind = 'history'):
        if trade_date is None:
            trade_date =  pd.Timestamp.now().date()
        trade_date = IO.str_date_parser(trade_date)
        # load history data
        ...
        if factor_mode == 't-1':
            ...
            data_t_1['daily_data'] = hist_data['daily_data']
...
# def get_model(model, fac):
#     pred_raw = pred_one_helper(fac, os.path.join(model_root,'%s.pkl' % model), model).iloc[:,0]
#     return {model: pred_raw}


# def model_predict(factor, max_workers = 24):
#     # path setting
...
#             rlist = pool.starmap(get_model, [(x, factor.copy()) for x in model_list])
#         for r in rlist:
#             pred_raw_dict.update(r)
#     else:
#         for model in model_list:
#             print(model)
#             model_save_itr = model_path_dict[model]
#             print(model_save_itr)
#             pred_raw = pred_one_helper(factor.copy(), model_save_itr, model)
#             pred_raw_dict[model] = pred_raw.iloc[:,0]
...
def model_predict(factor, model_kind = 'long'):
    # path setting
    model_sstime = time.time()
    print(f'start {model_kind} model predict')
    assert model_kind in ['long', 'short']
    if model_kind == 'long':
        this_model_list = model_list
        this_model_root = model_root
        this_stack_model_root = stack_model_root
        this_stack_model = stack_model
    else:
        this_model_list = short_model_list
        this_model_root = short_model_root
        this_stack_model_root = short_stack_model_root
        this_stack_model = short_stack_model

    pred_raw_dict = {}
    for model in this_model_list:
        pred_raw = pred_one_helper(factor.copy(), os.path.join(this_model_root,'%s.pkl' % model), model)
        pred_raw_dict[model] = pred_raw.iloc[:,0]
    pred_raw_df = pd.DataFrame(pred_raw_dict)

    x_test_stack = pred_raw_df[this_model_list]
    pred_stack = pred_one_helper(x_test_stack, os.path.join(this_stack_model_root, '%s.pkl' % this_stack_model), this_stack_model)
    pred_raw_dict['stack'] = pred_stack.iloc[:,0]
    pred_raw_df = pd.DataFrame(pred_raw_dict)
    print(f'end {model_kind} model predict, time use: ', time.time() - model_sstime)

    return {model_kind : pred_raw_df}
...
def get_arrow_black_list(four_factor, rule_data):
    ...
    d20 = data[(data['open_to_preclose']<0) & (data['last_day_tail10_close_to_low'] > 0.03)]
    d28 = data[(data['open_to_preclose']<0) & (data['filter_3']) & (data['last_day_close_to_preclose'] < 0) & (data['last_day_open_to_preclose'] < 0)]
    d29 = data[(data['open_to_preclose']<0) & (data['filter_1']) & (data['last_day_open_to_preclose'] > 0.08)]
    d_all = pd.concat([d20, d28, d29])
    return d_all.index.unique().to_list()

def get_universe_filter_list(f5, f123):
    f5.columns = ['filter_5']
    f5 = (f5 > -0.5) & (f5 < 0)
    temp_list = copy.deepcopy(trade_universe_filter)
    if 5 in temp_list:
        f123 = f123.reindex(f5[f5['filter_5'] == True].index)
        temp_list.remove(5)
    filter_all_df = f123[[f'filter_{x}' for x in temp_list]].sum(axis = 1)
    filter_all_df = filter_all_df[filter_all_df >= 1]
    return filter_all_df.index.tolist()

# def get_risk_blacklist():
#     return list(set(pd.read_csv(risk_blacklist_path, header = None)[0].tolist()))

def get_risk_blacklist(str_date):
    if os.path.exists('/data/group/800466/trade/Arrow/para/Arrow_'+str_date+'/strategy_params/arrow_strategy/buy_'+str_date+'_SH.json'):
        with open('/data/group/800466/trade/Arrow/para/Arrow_'+str_date+'/strategy_params/arrow_strategy/buy_'+str_date+'_SH.json') as f:
            param_dict = json.load(f)
            risk_blacklist = param_dict['买入交易黑名单'].split(',')
            return risk_blacklist
    else:
        with open('/data/group/800466/trade/Arrow/para/Arrow_'+str_date+'/strategy_params/arrow_strategy/buy_SH_'+str_date+'.json') as f:
            param_dict = json.load(f)
            risk_blacklist = param_dict['买入交易黑名单'].split(',')
            return risk_blacklist

def executor(trade_date=None, max_workers=24, tag='factors', factor_mode = 't', kind = 'history'):
    # load factors
    ...
    subclass_list = FactorGenerator.__subclasses__()
    if factor_mode in ['t']:
        subclass_list = [x for x in subclass_list if x().data_mode == factor_mode] + [x for x in subclass_list if x().data_mode == 'all']

    # merge hot data
    inst = FactorGenerator()
    data_stime = time.time()
    inst.merge_hot_data(trade_date=trade_date, factor_mode = factor_mode, kind = kind)
    str_date = inst.__trade_date__.strftime('%Y%m%d')
    ...
    factor_all = inst.__data__['histfactor'].append(factor_score).sort_index().replace([-np.inf, np.inf], np.nan)
    factor_prod = factor_all[factor_final_list]

    if is_filte_open:
        factor_norm1 = norm_3std(factor_prod[factor_prod['factor_openPct'] < 0]).loc[inst.__trade_date__:]
        factor_norm2 = norm_3std(factor_score[factor_score['factor_openPct'] < 0][factor_final_list].replace([-np.inf, np.inf], np.nan)).add_suffix('_zd')
    else:
        factor_norm1 = norm_3std(factor_prod).loc[inst.__trade_date__:]
        factor_norm2 = norm_3std(factor_score[factor_final_list].replace([-np.inf, np.inf], np.nan)).add_suffix('_zd')

    flist = [factor_norm1, factor_norm2, inst.__data__['dummy']]

    factor_input = pd.concat(flist, axis = 1, join = 'inner').replace([np.inf, -np.inf], np.nan).fillna(0)

    with Pool(2) as pool:
        rlist = pool.starmap(model_predict, [(factor_input, 'long'), (factor_input, 'short')])
    mv_dict = {}
    for r in rlist:
        mv_dict.update(r)

    pred_raw_df = mv_dict['long'].reset_index(level = 0, drop = True).sort_values(by = 'stack', ascending = False)
    short_pred_raw_df = mv_dict['short'].reset_index(level = 0, drop = True).sort_values(by = 'stack', ascending = False)

    final_score = pred_raw_df['stack']
    short_final_score = short_pred_raw_df['stack']
    late_trade_list = short_final_score[short_final_score < late_trade_threshold].index.tolist()
    
    universe_filter_list = get_universe_filter_list(factor_score.reset_index(level = 0, drop = True)[['factor_openPct']], inst.__data__['rule_blacklist_df'][['filter_1', 'filter_2', 'filter_3']])

    print('universe_filter_list: ', universe_filter_list)
    final_score = final_score.loc[universe_filter_list].dropna()

    #rule blacklist
    rule_blacklist = get_arrow_black_list(factor_score.reset_index(level = 0, drop = True)[['factor_1', 'factor_openPct', 'factor_s1_high_to_limit', 'factor_s2_high_to_limit']], inst.__data__['rule_blacklist_df'])

    rule_filtered = [x for x in rule_blacklist if x in final_score[final_score > threshold].index]
    if len(rule_filtered) > 0:
        print('rule blacklist: ', rule_blacklist)
        send_link('rule blacklist: ' + str(rule_blacklist))
        pd.Series(rule_filtered).to_csv(os.path.join(factor_savepath, '%s_rule_blacklist.csv' % str_date), index = False)
    final_score = final_score.drop(list(set(rule_blacklist) & set(final_score.index.tolist())))

    risk_blacklist = get_risk_blacklist(str_date)
    risk_filtered = [x for x in risk_blacklist if x in final_score[final_score > threshold].index]
    if len(risk_filtered) > 0:
        print('risk blacklist: ', risk_filtered)
        send_link('risk blacklist: ' + str(risk_filtered))
        pd.Series(risk_filtered).to_csv(os.path.join(factor_savepath, '%s_risk_blacklist.csv' % str_date), index = False)
    final_score = final_score.drop(list(set(risk_blacklist) & set(final_score.index.tolist())))

    buy_list = final_score[final_score > threshold].nlargest(daily_max_num).reset_index().Ticker.tolist()
    late_buy_list = sorted(list(set(buy_list) & set(late_trade_list)))
    normal_buy_list = sorted(list(set(buy_list) - set(late_buy_list)))

    # pred_raw_df.to_pickle('./model_value.pkl')
    buy_plan = pd.DataFrame(buy_list, columns = ['Ticker']).set_index('Ticker')
    if len(buy_plan) > 0:
        buy_plan['buy_time'] = '930'
        if len(late_buy_list) > 0:
            buy_plan.loc[late_buy_list, 'buy_time'] = '931'
    buy_plan = buy_plan.reset_index()

    # save result
    for path in [plan_savepath, model_value_path, short_model_value_path, factor_savepath]:
        if not os.path.exists(path):
            os.makedirs(path)
    
    normal_buy_list = ' '.join(normal_buy_list)
    late_buy_list = ' '.join(late_buy_list)
    if len(buy_plan) > daily_min_num:
        ...
        send_link(f'{str_date} trade plan: {len(buy_plan)} stocks    {buy_list}   normal_buy_list  {normal_buy_list}   late_buy_list  {late_buy_list}')
        print(f'{str_date} trade plan: {len(buy_plan)} stocks    {buy_list}   normal_buy_list  {normal_buy_list}   late_buy_list  {late_buy_list}')
    else:
        buy_plan.to_csv(os.path.join(plan_savepath, '%s_no_plan.csv' % str_date), index = False)
        buy_list = ' '.join(buy_plan.Ticker.tolist())
        send_link(f'{str_date} no trade plan: {len(buy_plan)} stocks    {buy_list}   normal_buy_list  {normal_buy_list}   late_buy_list  {late_buy_list}')
        print(f'{str_date} no trade plan: {len(buy_plan)} stocks    {buy_list}   normal_buy_list  {normal_buy_list}   late_buy_list  {late_buy_list}')

    print('start save result')

    pd.Series(universe_filter_list).to_csv(os.path.join(factor_savepath, '%s_universe_filter_list.csv' % str_date), index = False)
    pd.Series(late_trade_list).to_csv(os.path.join(factor_savepath, '%s_late_trade_list.csv' % str_date), index = False)
    
    pred_raw_df.to_csv(os.path.join(model_value_path, '%s.csv' % str_date))
    short_pred_raw_df.to_csv(os.path.join(short_model_value_path, '%s.csv' % str_date))

    factor_score.reset_index(level = 0, drop = True).to_csv(os.path.join(factor_savepath, '%s.csv' % str_date))
    IO.pd_hdf5_writer(factor_score[factor_final_list], rawfactor_path, dataset = rawfactor_dataset, append = True, data_columns=['dt', 'Ticker'])
    ...
    del(inst)
    return #factor_score, factor_input, pred_raw_df, short_pred_raw_df, buy_plan