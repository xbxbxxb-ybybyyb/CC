...
def _parallel_evolve(n_programs, parents, X, y, sample_weight, seeds, params):
    ...
    need_consts = params['need_consts']
...
class BaseSymbolic(BaseEstimator, metaclass=ABCMeta):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        self._need_consts = {}
        for function in self._function_set:
            _need_const = function.need_const
            self._need_consts[_need_const] = self._need_consts.get(_need_const, [])
            self._need_consts[_need_const].append(function)
        ...
        if not ((isinstance(self.const_params_range, tuple) and
                 len(self.const_params_range) == 2) or self.const_range is None):
        ...
        params['need_consts'] = self._need_consts
        ...
        for gen in range(prior_generations, self.generations):
            ...
            population = Parallel(n_jobs=n_jobs, max_nbytes='1G',
            ...
            # population是个list，里面的元素是_Program类
            population = list(itertools.chain.from_iterable(population))
            # print(len(population))

            fitness = [program.raw_fitness_ for program in population]
            if self._metric.greater_is_better:
                pd.Series((np.array(fitness)*(-1)).argsort()[:self.n_components]).to_csv(os.path.join(
                    self.factor_save_path, 'fitness', self.time_now, str(gen)) + '.csv')
            else:
                pd.Series(np.array(fitness).argsort()[:self.n_components]).to_csv(os.path.join(
                    self.factor_save_path, 'fitness', self.time_now, str(gen)) + '.csv')
            ...
            if self._metric.greater_is_better:
                ...
                print(self.get_str(best_program.program))
                # to_h5_list = (np.array(fitness) * (-1)).argsort()[:self.n_components]
                # factors_exist_list = [i[:-3] for i in os.listdir(os.path.join(self.factor_save_path, 'factors'))]
                # for i in to_h5_list:
                #     program_formula = self._programs[-1][i].__str__()
                #     factor_value = self._programs[-1][i].execute(X)
                #     factor_value.name = program_formula
                #     factor_value = factor_value.to_frame()
                #     if program_formula not in factors_exist_list:
                #         factor_value.to_hdf(os.path.join(self.factor_save_path, 'factors', program_formula) + '.h5',
                #                             key=program_formula)
                ...
                #                         show_image=False, savepath=os.path.join(self.factor_save_path,
                #                                                                 'pictures')).draw_result()
            else:
                best_program = population[np.nanargmin(fitness)]
                print(best_program.program.__str__())
                # to_h5_list = np.array(fitness).argsort()[:self.n_components]
                # factors_exist_list = [i[:-3] for i in os.listdir(os.path.join(self.factor_save_path, 'factors'))]
                # for i in to_h5_list:
                #     program_formula = self._programs[-1][i].__str__()
                #     factor_value = self._programs[-1][i].execute(X)
                ...
                #     factor_value = factor_value.to_frame()
                #     if program_formula not in factors_exist_list:
                #         factor_value.to_hdf(os.path.join(self.factor_save_path, program_formula) + '.h5',
                #                             key=program_formula)
                #         factors_exist_list.append(program_formula)
                #         SIF_Factor_Test(df=factor_value, factor_name=factor_value.columns[0], save_image=True,
                #                         show_image=False, savepath=os.path.join(self.factor_save_path,
