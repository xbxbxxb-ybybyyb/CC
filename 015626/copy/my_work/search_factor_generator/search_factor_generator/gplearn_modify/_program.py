...
class _Program(object):
    ...
    need_consts : dict
        A dictionary of the form `{need_const: [functions]}`. The need_consts is the
        argument whether the function needs constant parameters, if need_const=True, the
        function needs constant parameters, if need_const=False, the function doesn't need
        constant parameters the functions must match those in the `function_set` parameter.
    ...
    def build_program(self, random_state):
        ...
        need_const_list = [function.need_const]
        ...
        while terminal_stack:
            ...
            if (depth < max_depth) and (method == 'full' or
                ...
                need_const_list.append(function.need_const)
            else:
                ...
                while terminal_stack[-1] == 0:
                    ...
                    if not terminal_stack:
                        const_params_list = [random_state.randint(*self.const_params_range)
                                             if i is True else 0 for i in need_const_list]
                        need_tuple = (program, const_params_list)
                        return need_tuple
                    terminal_stack[-1] -= 1
    ...
    def __str__(self):
        ...
        # print([i.name for i in self.program[0] if isinstance(i, _Function)])
        ...
        for i, i_num in enumerate(self.program[1]):
            if i_num != 0:
    ...
    def execute(self, X):
        ...
        apply_stack = []
        bracket_pair_list = [i[1] for i in _Program.get_y(node_str)]  # 根据公式表达式得到各个算子执行的先后顺序
        ...
        for i, node in enumerate(self.program[0]):
            ...
            while len(apply_stack[-1]) == apply_stack[-1][0].arity + 1:
                ...
                terminals = [pd.Series(t, index=X.index) if isinstance(t, float) else X.iloc[:, t] if isinstance(t, int)
                             else t for t in apply_stack[-1][1:]]
                if func_execute_order_list[count] != 0:
                    terminals.append(func_execute_order_list[count])
                count += 1
                ...
                if len(apply_stack) != 1:
                    ...
                    return rolling_norm(intermediate_result, window=self.rolling_window, method='bn_move_rank')
    ...
    def point_mutation(self, random_state):
        ...
        for node in mutate:
            if isinstance(program[0][node], _Function):
                ...
                need_const = program[0][node].need_const
                # Find a valid replacement with same arity
                replacement_func_list = [i for i in self.arities[arity] if i in self.need_consts[need_const]]
