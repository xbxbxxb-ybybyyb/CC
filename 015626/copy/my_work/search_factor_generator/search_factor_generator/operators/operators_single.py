...
from utils_wsc.help_functions import rolling_window
...
class Operator(object):
    ...
    def ts_sum(self, df1):
        output = pd.DataFrame(bn.move_sum(df1, window=self.d, min_count=self.min_periods, axis=0),
                              index=df1.index, columns=df1.columns)
        return output
    ...
    def pct_change(self, df1):
        # (A_i-A_(i-d))/A_(i-d)
        output = df1.pct_change(periods=self.d)
        return output
...
# 以下算子不能在第一层就使用
class OperatorMustZscore(object):
    def __init__(self, params):
        self.n1 = params['n1']
        self.n2 = params['n2']
        self.n3 = params['n3']

    def max_with_num(self, df1):
        # 把df中小于num的数变成num
        output = np.maximum(df1, self.n1)
        return output

    def min_with_num(self, df1):
        # 把df中大于num的数变成num
        output = np.minimum(df1, self.n1)
        return output

    def max_with_num_nan(self, df1):
        # 把df中小于num的数变成nan值
        output = df1[df1 > self.n1]
        return output

    def min_with_num_nan(self, df1):
        # 把df中小于num的数变成nan值
        output = df1[df1 < self.n1]
        return output
