...
import scipy.stats
from help_functions_wsc import rolling_window_upgrade, replace_zero, type_convertor

__all__ = ['add2', 'div2', 'inv1', 'log', 'mul2', 'max2', 'min2', 'neg1', 'pairwise_corr_np', 'rolling_norm',
           'section_rank_np', 'section_rank_bk', 'shift_np', 'sqrt', 'square', 'sub2', 'ts_argmax', 'ts_argmin',
           'ts_corr', 'ts_cov', 'ts_decay_linear', 'ts_delay', 'ts_delta', 'ts_ema', 'ts_ema_span', 'ts_max', 'ts_mean',
           'ts_median', 'ts_min', 'ts_pct_change', 'ts_position', 'ts_pred', 'ts_pred_delta', 'ts_rank', 'ts_reg_alpha',
           'ts_reg_beta', 'ts_reg_residual', 'ts_skew', 'ts_std', 'ts_sum', 'ts_truncated_ema', 'ts_truncated_ema_span',
           'ts_truncated_ema_1', 'ts_truncated_ema_span_1', 'ts_zscore']
...
def ts_position(data, d):
    # (data - ts_min(data, d)) / (ts_max(data, d) - ts_min(data, d))，我也忘记了为什么不把公式直接写成这么简洁的形式
    if not isinstance(data, np.ndarray):
        data = data.values
    data_expanding = rolling_window_upgrade(data, d)
...
def ts_truncated_ema_1(data, d, alpha):
    ...
    return ts_decay_linear(data, d, weight=weight)


def ts_truncated_ema_span_1(data, d=120, span=5):
    # truncated ema
    return ts_truncated_ema_1(data=data, d=d, alpha=2 / (span + 1))


@type_convertor
def ts_zscore(data, d):
    if d == 1:
        output = data
    else:
        temp_mean = bk.move_mean(data, window=d, min_count=int(d / 2), axis=0)
        temp_std = bk.move_std(data, window=d, min_count=int(d / 2), axis=0, ddof=1)
        output = (data - temp_mean) / temp_std
    return output
