...
ticker = 'IC.CFE'
fs = [f for f in os.listdir('./factors_hf/') if f.endswith('.py')]
for f in fs:
...
if __name__ == '__main__':
    ...
    flag_path_start = flag_root + str(end_date) + '_overnight_factors_hf.start'
    ...
    for i, subcls in enumerate(subclass_list_cfg):
        ...
        subcls(savepath='/data/group/800466/warehouse/prod/alpha/CHINA_FUTURES/OVERNIGHT/1449/').__callback__(start_date, end_date)
        
    flag_path_success = flag_root + str(end_date) + '_overnight_factors_hf.success'
