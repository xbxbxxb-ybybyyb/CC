...
if ticker == 'IC.CFE':
    fs = [f for f in os.listdir('/data/user/017024/overnight_factors/factors/overnight_prod_20210127_76/ic/') if f.endswith('.py') & ~(f.split('.')[0].endswith('_if')|f.split('.')[0].endswith('_IF'))]
elif ticker == 'IF.CFE':
...
if __name__ == '__main__':
    ...
    flag_path_start = flag_root + str(end_date) + '_overnight_factors_cfg_ic.start'
    ...
    flag_path_success = flag_root + str(end_date) + '_overnight_factors_cfg_ic.success'
