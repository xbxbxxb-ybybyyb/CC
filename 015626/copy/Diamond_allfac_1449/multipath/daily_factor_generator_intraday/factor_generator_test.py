...
class FactorGenerator:
    ...
    def prepare_hot_data(inst, start_date, end_date):
        ...
        today_data = pd.read_hdf(os.path.join('/data/user/012245/warehouse/prod/market/closure/', str(end_date)) + '.h5')
        ...
        for i in future_data.keys():
            future_data[i] = future_data[i][:'20210303']
        for i in index_data.keys():
            index_data[i] = index_data[i][:'20210303']
        for i in daily_future_data.keys():
            daily_future_data[i] = daily_future_data[i][:'20210303']
        for i in daily_index_data.keys():
            daily_index_data[i] = daily_index_data[i][:'20210303']
