...
def sql_parser(data):
    ...
    try:
        ...
        data1 = re.sub(r"'{3,}", '', data)
        data1 = re.sub(r'"{3,}', '', data1)
        with open('D:\\013160\\test_log1.txt','w',encoding='utf8') as file:
            file.write(data)
        try:
            _data = eval(data1)
        except Exception as e:
            data2 = re.sub(r"'{3,}", r"'", data)
            data2 = re.sub(r'"{3,}', r'"', data2)
            with open('D:\\013160\\test_log.txt','w',encoding='utf8') as file:
                file.write(data2)
            _data = eval(data2)


        # else:
        #     with open('D:\\013160\\test_log.txt','w') as file:
        #         file.weite(_exp.msg)
        #     raise SyntaxError
    try:
