...
class HS300IndexWareHousing:
    ...
    def find_index_stock_by_time(self, time):
        ...
        df = IO.read_data(tradeday, alt=r'Z:\warehouse\prod\UNIV\CHINA_STOCK\DAILY\OPTM\UNIV_CHINA_STOCK_DAILY_OPTM.h5')
        df = df.reset_index()
    ...
    def get_data(self):
        ...
        data1 = IO.read_data([20070101, self.end_time], columns=['amt', 'mkt_cap_ard', 'close'],
                             alt=r'Z:\warehouse\prod\MD\CHINA_STOCK\DAILY\WIND\MD_CHINA_STOCK_DAILY_WIND.h5')
        ...
        df3 = IO.read_data(self.getYesterday(),
                           ...
                           alt=r'Z:\warehouse\prod\DATABASE\WIND\AShareCapitalization\AShareCapitalization.h5')
        df3 = df3.reset_index()
    ...
    def delete_ST(self, data):
        ...
        stock_AShareST = IO.read_data(self.getYesterday(), alt=r'Z:\warehouse\prod\DATABASE\WIND\AShareST\AShareST.h5')
    ...
    def select_GEM_stock(self, data):
        ...
        stock_description = IO.read_data(self.getYesterday(),
                                         alt=r'Z:\warehouse\prod\DATABASE\WIND\AShareDescription\AShareDescription.h5')
    ...
    def sort_average_daily_turnover(self, data):
        # 先将停牌的日子都删掉
        ...
        return top50_and60_average_daily_turnover, top50_daily_turnover_list
    ...
    def sort_average_daily_mkt(self, top50_and60_average_daily_turnover, top50_daily_turnover_list):
        ...
        return new_average_daily_MarketCap, sort_average_daily_MarketCap

    # 剔除证监会处罚的公司
    def drop_illegality(self):
        asi = IO.read_data([self.start_time, self.end_time],
                           alt=r'Z:\warehouse\prod\DATABASE\WIND\AShareIllegality\AShareIllegality.h5')
        asi = asi.reset_index()
    ...
    def drop_loss_profit(self):
        if self.end_time % 1000 == 430:
            ascf = IO.read_data((self.end_time // 10000 - 1) * 10000 + 1231,
                                ...
                                alt=r'Z:\warehouse\prod\DATABASE\WIND\AShareIncome\AShareIncome.h5')
        else:
    ...
    def get_bring_and_drop_list(self, new_average_daily_MarketCap, delete_list, sort_average_daily_MarketCap):

        # 在新的排序里删掉这些股票
        new_average_daily_MarketCap_delete_bring = new_average_daily_MarketCap[
            ~new_average_daily_MarketCap.Ticker.isin(delete_list)]

        # 得到前300名作为新的指数成分股票
        new_top300_index = new_average_daily_MarketCap_delete_bring[:300]
        ...
        if (len(set(new_top300_list) - set(self.old_index_stock_list)) <= 30):
            ...
            new_stock_all_list = list(
                set(new_average_daily_MarketCap_delete_bring.Ticker.tolist()) - set(self.old_index_stock_list))
            delete_size = len(new_stock_all_list) - 30
            delete_stock_list = new_average_daily_MarketCap_delete_bring[
                new_average_daily_MarketCap_delete_bring.Ticker.isin(new_stock_all_list)].tail(
                delete_size).Ticker.tolist()
            new_average_daily_MarketCap_delete_bring_keep30 = new_average_daily_MarketCap_delete_bring[
                ~new_average_daily_MarketCap_delete_bring.Ticker.isin(delete_stock_list)]
            if (len(new_average_daily_MarketCap_delete_bring_keep30) < 300):
                add_size = 300 - len(new_average_daily_MarketCap_delete_bring_keep30)
                add_df = sort_average_daily_MarketCap[sort_average_daily_MarketCap.Ticker.isin(list(
                    set(self.old_index_stock_list) - set(
                        new_average_daily_MarketCap_delete_bring_keep30.Ticker.tolist())))].head(add_size)
                new_average_daily_MarketCap_delete_bring_keep30 = new_average_daily_MarketCap_delete_bring_keep30.append(
                    add_df)

            new_top300_index_keep30 = new_average_daily_MarketCap_delete_bring_keep30[:300]
    ...
    def get_result(self):
        ...
        top50_and60_average_daily_turnover, top50_daily_turnover_list = self.sort_average_daily_turnover(data)
        print('sort data by average_daily_MarketCap')
        new_average_daily_MarketCap, sort_average_daily_MarketCap = self.sort_average_daily_mkt(
            top50_and60_average_daily_turnover, top50_daily_turnover_list)

        print('drop stock which was illegal')
        punish_stock_list = self.drop_illegality()
        ...
        # 选出不符合规则的新股票要删掉
        delete_list = list((set(punish_stock_list) | set(loss_profit_stock_list) | set(delete_new_suspension_list)) - set(self.old_index_stock_list))

        print('get new bring and drop list..')
        new_bring_list, new_drop_list, new_top300_list = self.get_bring_and_drop_list(
            new_average_daily_MarketCap, delete_list, sort_average_daily_MarketCap)
...
for mytime in time_list:
    ...
    hs300 = HS300IndexWareHousing(mytime, True)
