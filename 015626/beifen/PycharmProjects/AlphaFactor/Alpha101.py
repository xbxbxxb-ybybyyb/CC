...
class Alphas(object):
    def __init__(self, df_data):
        ...
        self.cap = df_data['mkt_cap_ard'] * 10000
        self.vwap = (df_data['amt']*1000)/(df_data['volume']*100+1)
...
#                             columns=self.close.columns)
    ...
    def alpha065(self):
        ...
        return ((rank(correlation(((self.open * 0.00817205) + (self.vwap * (1 - 0.00817205))), ts_sum(adv60,9), 6)) < rank((self.open - ts_min(self.open, 14)))) * -1)
    ...
    def alpha074(self):
        ...
        return ((rank(correlation(self.close, ts_sum(adv30, 37), 15)) <rank(correlation(rank(((self.high * 0.0261661) + (self.vwap * (1 - 0.0261661)))), rank(self.volume), 11)))* -1)
    ...
    def alpha081(self):
        ...
        data = (rank(log(product(rank((rank(correlation(self.vwap, ts_sum(adv10, 50),8)).pow(4))), 15))) < rank(correlation(rank(self.vwap), rank(self.volume), 5))) * -1
        for u in data.columns:
            if data[u].dtype == bool:
                data[u] = data[u].astype('int')
        return data
    ...
    def alpha086(self):
        adv20 = sma(self.volume, 20)
        return ((ts_rank(correlation(self.close, ts_sum(adv20, 15), 6), 20) < rank(((self.open+ self.close) - (self.vwap +self.open)))) * -1)
    ...
    def alpha095(self):
        adv40 = sma(self.volume, 40)
        return (rank((self.open - ts_min(self.open, 12))) < ts_rank((rank(correlation(ts_sum(((self.high + self.low)/ 2), 19), ts_sum(adv40, 19), 13)).pow(5)), 12))
    ...
    def alpha098(self):
        ...
        adv15 = sma(self.volume, 15)
        return (rank(decay_linear(correlation(self.vwap, ts_sum(adv5, 26), 5).to_frame(), 7).CLOSE) -rank(decay_linear(ts_rank(ts_argmin(correlation(rank(self.open), rank(adv15), 21), 9),7).to_frame(), 8).CLOSE))
