...
def get_available_cols(mkttype=MktType.CHINA, dtype=DType.STOCK, ftype=FType.MD, dfreq=DFreq.DAILY, dsource=DSource.HTSC, alt=None, h5root='/data/group/800080/warehouse/prod'):
    abs_h5_path = path_assembler(mkttype=mkttype, dtype=dtype, ftype=ftype, dfreq=dfreq, dsource=dsource, alt=alt, h5root=h5root)
    with pd.HDFStore(abs_h5_path, 'r') as h5_store:
        h5_root_keys = get_root_keys(h5_store)
        if len(h5_root_keys) == 1:
...
def pd_hdf5_writer(pd_factor, hdf5, dataset=None, override=None, append=None, from_scratch=True,
                   override_overlap=True, date_column=None):
    """
    --- DESCRIPTION ---
    Dump DataFrame/Series into HDF5 dataset with override and append switch
    By default, override and append switch are both unset (None)
        ...
        if override:
            ...
            for date_ticker in dt_lst_to_process:
                ...
                if not exist_flag or override_overlap:
                    # Append new records
                    if date_column is not None:
                        h5_store.append(dataset, pd_factor.loc[pd_factor[date_column]==date_ticker], data_columns=True)
                    else:
                        h5_store.append(dataset, pd_factor.loc[date_ticker.strftime('%Y%m%d')], data_columns=True)
                    h5_store.get_storer(dataset).attrs.modification_date = ts_meta
