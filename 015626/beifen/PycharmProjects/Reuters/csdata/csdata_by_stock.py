...
TRE_tables = []#,'TREActGoFwd','TREActRst','TREDetAper','TREDetHzn','TREDetPer','TRERecDet']
...
for table in TRE_tables:
    ...
    for date in cdate_list:
        print(table,date)
        sql = 'SELECT * FROM [dbo].[' + table + '] where AnnounceDate like \'' + parse(str(date)).strftime('%Y-%m-%d') + '%\''
        print(sql)
        cursor.execute(sql)
        rs = cursor.fetchall()
        if(len(rs) == 0):
        ...
        df.to_csv(os.path.join(table_csv_path,str(date) + '.csv'), encoding='utf-8')
