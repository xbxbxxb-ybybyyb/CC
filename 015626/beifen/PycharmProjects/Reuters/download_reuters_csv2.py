...
cursor = conn.cursor()
#'TREDetPer',199605
#已经完成'TREDetAper', 'TREActGoFwd',
TRE_tables = [ 'TREDetHzn', 'TREActRpt', 'TREActRst', 'TRERecDet']
...
for table in TRE_tables:
    ...
    sql = "select min(announcedate) from " + table + " A join ChinaAShareIBESCodes C on A.EstPermID = C.EstPermID"
    cursor.execute(sql)
    rs = cursor.fetchall()
    if rs[0][0] == None:
    ...
    # monthdate_list = pd.date_range('2019-08', '2019-10', freq='M')
    for monthdate in monthdate_list:

        # sql = 'SELECT * FROM [dbo].[' + table + '] where AnnounceDate like \'' + date.strftime('%Y-%m-%d') + '%\''
        sql = 'select C.Ticker, C.DsQtName, C.IBESTicker, A.* from ' + table + ' A join ChinaAShareIBESCodes C on A.EstPermID = C.EstPermID where AnnounceDate like \'' + monthdate.strftime('%Y-%m') + '%\''
        ...
        df['newAnnounceDate'] = df['AnnounceDate'].apply(lambda x:datetime.datetime.strptime(x[:10],'%Y-%m-%d').strftime('%Y%m%d'))
        date_list = df.newAnnounceDate.unique().tolist()
        for date in date_list:
            print(table, date)
            newdf = df[df.newAnnounceDate == date]
            newdf = newdf.drop(['newAnnounceDate'], axis=1)
