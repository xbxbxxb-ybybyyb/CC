...
def load_md_data(cdate_list,data_length=600, align=True):
    ...
    h5_data = IO.read_data([start_date,end_date],columns=index_list,ftype=FType.MD,dsource=DSource.WIND,h5root='Z:/warehouse/prod',max_workers=1)
    ...
    risk_ind = IO.read_data([start_date,end_date],ftype=FType.UNIV,dsource=DSource.OPTM,columns=['risk_universe'],h5root='Z:/warehouse/prod')
    ...
    h5_index = IO.read_data([start_date,end_date],['close'],ftype=FType.MD,dtype=DType.INDEX,dsource=DSource.WIND,h5root='Z:/warehouse/prod',max_workers=1)
    ...
    # 'industry_citiccode'
    h5_ind = IO.read_data([start_date,end_date],ftype=FType.INDUSTRY,dsource=DSource.WIND,columns=['CITIC_I'],h5root='Z:/warehouse/prod',max_workers=1) #CITIC Level 1 = Industry 3
...
def risk_size_date(cdate_list,dat): 
    ...
    mkt_cap = IO.read_data(date_take,columns=['mkt_cap_ard'],ftype=FType.MD,dsource=DSource.WIND,h5root='Z:/warehouse/prod',max_workers=1)
...
#IO.read_data([20180130,20180131],ftype=FType.RISK,dsource=DSource.STYLEFACTOR,max_workers=1)
def risk_h5_checker(cdate_list):
    log_path = 'S:\\Quant\\backtest\\local_data\\StyleFactor\\log\\'
    risk_date = IO.read_data([cdate_list[-1],cdate_list[-1]],ftype=FType.RISK,dsource=DSource.STYLEFACTOR,max_workers=1)
    risk_check = np.isfinite(risk_date).sum()
    if sum(risk_check<1000) >0:
        print ('Risk Data Error')
    print (risk_check)
    risk_check = pd.DataFrame(risk_check,columns=['number'])
    risk_check['number'] = risk_check['number'].astype('str')
    """record fail"""
    status_dict = risk_check.to_dict()
    log_file = log_path+str(cdate_list[-1])+'.json'
...
def update_risk(sdate=None,edate=None):
    sdate_prev,edate,cdate_list = check_update_date(sdate,edate)
    print ('Get Risk MD')
    update_risk_factor_md(cdate_list)
    print ('Get Risk FDD')
    update_risk_factor_fdd(cdate_list)
    
    print ('Check Data')
    risk_h5_checker(cdate_list)
    return 

# sdate,edate = 20181106,20181106
# update_risk_factor_daily(sdate,edate)
# update_risk(20090101,20181023)

#cdate_list = [20180301,20180302,20180305,20180306,20180307,20180308,20180309]
#fdate_list_dt = IO.read_data([20090101,20200101],ftype=FType.CALENDAR).index.get_level_values(0)
#fdate_list = [int(dt.datetime.strftime(i,'%Y%m%d')) for i in fdate_list_dt]
#cdate_list = fdate_list[fdate_list.index(20090105):fdate_list.index(20180309)+1]
#cdate_list = [20180312,20180313,20180314,20180315,20180316]

#update_risk()

# IO.read_data([20180213,20180222],alt= 'S:\\Quant\\data\\risk\\CHINA_STOCK\\DAILY\\STYLEFACTOR\\risk_CHINA_STOCK_DAILY_STYLEFACTOR.h5',max_workers=1)

"""
#Bayesian shrinkage
We segment stocks into deciles based on their market capitalization.
Within each size bucket, we compute the mean and standard deviation of
the specific risk forecasts. We then pull or “shrink” the volatility forecast to the mean within the size
decile, with the shrinkage intensity increasing with the number of standard deviations away from the mean.

# Global Industry Classification Standard


In order to facilitate comparison across style factors, the factors are standardized to have a capweighted
mean of 0 and an equal-weighted standard deviation of 1. The cap-weighted estimation
universe, therefore, is style neutral.


# factor stability coefficient 
cross-sectional correlation of factor exposures from one month to the next
corr<0.8 -> instable / corr>0.9 stable

# VIF measures the degree of collinearity among the factors.
# VIF scores above 5 are generally considered problematic.
...
# regression weighting scheme (square root of market capitalization)

# Measure of risk model:(the mean bias statistic, the P5 and P95 bias statistics, and the MRAD.)- rolling 12 periods
bias statistic is an out-of-sample measure that represents the ratio of realized risk to predicted risk
Nevertheless, we may define a confidence interval that is expected to contain 95 percent of the observations under the hypothesis of
...
visualize the magnitude of the average biases and can judge whether they were persistent or regimedependent.

accuracy of risk forecasts is the mean rolling absolute deviation, or MRAD
