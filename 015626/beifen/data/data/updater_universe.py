...
def get_ST(date):
    h5_path2 = 'Z:\\warehouse\\prod\\DATABASE\\WIND\\'
    table_name = 'AShareST'
...
def save_index_component(cdate_list,index_list,save_path):  
    ...
    h5_path = 'Z:\\warehouse\\prod\\DATABASE\\WIND\\'
    table_path = h5_path + table_name + '\\' +  table_name + '.h5'
    df = IO.read_data([20090101,21000101],alt = table_path)
    df.reset_index('dt',inplace=True)
    df.drop('dt',axis=1,inplace=True)
...
def csv_date_checker(cdate_list,check_path):
    logger.info ('check csv file')
    cdate_list = [cdate_list] if type(cdate_list) !=list else cdate_list
    """check csv existence by date list"""
    current_list = [int(i[:-4]) for i in os.listdir(check_path) if i[-3:]=='csv']
    current_list.sort()
    diff_list = list(set(cdate_list) - set(current_list))
    logger.info ('Sql download list: '+str(diff_list))
    return diff_list



def h5_checker(cdate_list,date_path):
    """check h5 data status by date"""    
...
def create_universe(cdate_list,h5_path):
    print(cdate_list)
    logger.info ('-'*20+'\nForming Universe') 
    logger.info ('Loading data') 
    fdate_list_dt = IO.read_data([20090101,20990101],ftype=FType.CALENDAR).index.get_level_values(0)
    fdate_list = [int(dt.datetime.strftime(i,'%Y%m%d')) for i in fdate_list_dt]
    # print(fdate_list[252])
    # take data from updated h5
    data_length = 252 # date used for compute 
    end_date = cdate_list[-1]
    print(fdate_list.index(cdate_list[0]))
    print(end_date)
    start_date = fdate_list[max(0, fdate_list.index(cdate_list[0])-data_length)]

    data_MI = IO.read_data([start_date,end_date],alt = h5_path,max_workers=1)
    date_check = np.isfinite(data_MI.loc[str(end_date)]).sum(axis=0)
    if (date_check >= [1000,1000,1000,1000,1000,1000]).sum() < 6:
        logger.info ('Check data for universe raw')
        logger.info (date_check)
        raise AssertionError   
    
    mkt_cap_MI = IO.read_data([start_date,end_date],columns=['mkt_cap_ard'],alt = r'Z:\warehouse\prod\MD\CHINA_STOCK\DAILY\WIND\MD_CHINA_STOCK_DAILY_WIND.h5',max_workers=1)
    mkt_cap = mkt_cap_MI['mkt_cap_ard'].unstack()
    if (np.isfinite(mkt_cap).sum(axis=1)<500).sum()>0:
        logger.info ('No data')
        raise AssertionError 
    ...
    data_dict = {}
    data_dict['filter_suspend'] = data_MI['SUSPEND'].unstack()
    data_dict['filter_stpt'] = data_MI['STPT'].unstack()
    date_list = data_dict['filter_stpt'].index.tolist()

    date_list_int = [int(i.strftime('%Y%m%d')) for i in date_list]
    date_list_int.sort()
    stock_date = data_MI.loc[str(end_date)]['Listing_date']
    stock_all_code = stock_date.index.get_level_values(1).tolist()
    listing_date = stock_date.values.tolist()
    day_num_year = 242
    day_half_year = 121
    date_num = len(date_list_int)
    stock_num = len(stock_all_code)
    bt_startdate,bt_enddate = date_list_int[0],date_list_int[-1]
    date_list_master = [i for i in fdate_list if i<=bt_enddate]
    if 3 * day_num_year + date_num > len(date_list_master):
        date_num = len(date_list_master) - 3 * day_num_year
        date_list = date_list[-date_num : ]    
    print(date_num)
    print(stock_num)
    date_1yr = date_list_master[-1*day_num_year-date_num:-1*day_num_year]
    date_3yr = date_list_master[-3*day_num_year-date_num:-3*day_num_year]
    date_121d = date_list_master[-1*day_half_year-date_num:-1*day_half_year]
    listing_date_mat =  np.repeat(np.matrix(listing_date),date_num,axis=0)
    # logger.info(np.matrix)
    # logger.info(listing_date_mat)
    date_1yr_mat = (np.repeat(np.matrix(date_1yr),stock_num,axis=0).T > listing_date_mat)
    date_3yr_mat = (np.repeat(np.matrix(date_3yr),stock_num,axis=0).T > listing_date_mat)
    date_121d_mat = (np.repeat(np.matrix(date_121d),stock_num,axis=0).T > listing_date_mat)
    
    listing_dict = dict()
    listing_dict['listing_1yr'] = pd.DataFrame(date_1yr_mat,index=date_list,columns=stock_all_code)
    listing_dict['listing_3yr'] = pd.DataFrame(date_3yr_mat,index=date_list,columns=stock_all_code)
    listing_dict['listing_half_yr'] = pd.DataFrame(date_121d_mat,index=date_list,columns=stock_all_code)
    listing_dict['over_half_for_half_year'] = (data_dict['filter_suspend'].fillna(0).rolling(window=day_half_year).sum()>int(day_half_year/2))
    listing_dict['risk_universe'] = listing_dict['listing_1yr']*data_dict['filter_stpt']*listing_dict['over_half_for_half_year']*mk_cap_ind
    #listing_dict['alpha_universe'] = listing_dict['listing_1yr']*data_dict['filter_stpt']*data_dict['filter_suspend'].shift(-1)*listing_dict['over_half_for_half_year']*mk_cap_ind
    listing_dict['alpha_universe'] = listing_dict['listing_1yr']*data_dict['filter_stpt']*data_dict['filter_suspend']*listing_dict['over_half_for_half_year']*mk_cap_ind
    print(data_dict)


    MI_listing = dict()
    for index in listing_dict:
        MI_listing[index] = pd.DataFrame(listing_dict[index].stack(),columns=[index])
        
    data_MI_full = pd.concat([data_MI,MI_listing['risk_universe'],MI_listing['alpha_universe'],
                              MI_listing['listing_1yr'],MI_listing['listing_3yr'],MI_listing['over_half_for_half_year']],axis=1)
    
    data_MI_full['index_50'] = data_MI_full['SH50']>0
    data_MI_full['index_300'] = data_MI_full['HS300']>0
    data_MI_full['index_500'] = data_MI_full['ZZ500']>0
    ...
    data_config = { 'index_name': ['dt','Ticker'],
                    ...
                    'name_new':['index_weight_hs300', 'Listing_date', 'filter_opendownlimit', 'filter_openuplimit', 
                               ...
                               'filter_stpt', 'filter_suspend', 'index_weight_zz500', 'risk_universe', 'alpha_universe',
                               'listing_1yr', 'listing_3yr', 'over_half_for_half_year', 'index_50',
                               'index_300', 'index_500'],
                    'column_order': ['risk_universe', 'alpha_universe','index_50','index_300', 'index_500',
                               ...
                               'filter_opendownlimit', 'filter_openuplimit','filter_sso','filter_stpt', 'filter_suspend',  
                               'Listing_date','listing_1yr','listing_3yr','over_half_for_half_year']}
    dat = data_MI_formatter(data_MI_full,data_config)
    cut_list = [str(i) for i in cdate_list] if len(cdate_list)>1 else str(cdate_list[0])
    data_universe = dat.loc[cut_list].dropna()
    logger.info ('Universe Complete\n'+'-'*20)
    # logger.info(data_universe)
    return data_universe
...
class unv_factor(object):
    ...
    def csv_to_database(self):
        ...
        update_universe_raw(self.cdate_list,csv_path,h5_path_source,factor_list)
        """Create Universe"""
...
# updater_universe(20190820,20190820)