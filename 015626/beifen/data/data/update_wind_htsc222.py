...
class WIND_DATABASE:
    def __init__(self, table_name, sdate, edate, 
        ...
        name_mapping_dict = {'WIND_AShareAFIndicator': 'AShareANNFinancialIndicator', 'WIND_Ashareeoddindicator': 'AShareEODDerivativeIndicator',
                    ...
                    'WIND_AshareFinaccounts': 'AshareFinancialaccounts','WIND_AShareConRollingData':'AShareConsensusRollingData',
                    'WIND_AIndexConRollingData':'AIndexConsensusRollingData','WIND_CCommodityFuturesPos':'CCommodityFuturesPositions'}
        ...
        self.over_write_list = ['WIND_AShareIndClassCITICS', 'WIND_AShareDescription', 'WIND_AShareIndustriesCode','WIND_AShareST',
                        'WIND_AShareCapitalization', 'WIND_AShareFreeFloat', 'WIND_AShareIPO', 'WIND_AShareAgency',
                         ...
                         'WIND_AShareMainandnoteitems','WIND_AIndexMembers','WIND_AShareConseption','WIND_AIndexMembersCITICS3']    
    ...
    def csv2h5(self, update_list, min_size=0):
        ...
        dat_fig_dict = {'WIND_AShareProfitExpress':{'dt':'REPORT_PERIOD','Ticker':'S_INFO_WINDCODE'},
                        ...
                        'WIND_CCommodityFuturesPos':{'dt':'TRADE_DT','Ticker':'S_INFO_WINDCODE'},
                        ...
                        'WIND_AShareMgHoldReward': {'dt': 'ANN_DATE', 'Ticker': 'S_INFO_WINDCODE'},
                        ...
                        'WIND_AShareMonRepBrokers':{'dt': 'REPORT_PERIOD', 'Ticker': 'S_INFO_WINDCODE'},
                        ...
                        'WIND_AShareConRollingData':{'dt':'EST_DT','Ticker':'S_INFO_WINDCODE'},
                        'WIND_AIndexConRollingData':{'dt':'EST_DT','Ticker':'S_INFO_WINDCODE'},
                        'WIND_AIndexConsensusData':{'dt':'EST_DT','Ticker':'S_INFO_WINDCODE'},
                        'WIND_AShareConsensusData':{'dt':'EST_DT','Ticker':'S_INFO_WINDCODE'},
                        'WIND_AShareEarningEst':{'dt':'EST_DT','Ticker':'S_INFO_WINDCODE'},
                        'WIND_AShareIPOPricingForecast':{'dt':'EST_DT','Ticker':'S_INFO_WINDCODE'},
                        'WIND_AShareConsensusindex':{'dt':'ANN_DT','Ticker':'S_INFO_COMPCODE'},
                        'WIND_AShareStockRating':{'dt':'ANN_DT','Ticker':'S_INFO_WINDCODE'},
                        'WIND_AShareStockRatingConsus':{'dt':'RATING_DT','Ticker':'S_INFO_WINDCODE'},
                        'WIND_AIndexMembersCITICS3':{'dt':'date','Ticker':'S_CON_WINDCODE'},
                        'WIND_AShareIndusRating':{'dt':'COLLECT_DT','Ticker':'WIND_IND_CODE'}
                        }
        ...
        with pd.HDFStore(self.table_h5_file) as h5_store:
            ...
            for fname in update_list:
                ...
                if len(dat1)<=min_size:
                    ...
                    if self.table_name == 'WIND_AShareProfitNotice':
                    ...
                    elif self.table_name == 'WIND_AShareConRollingData':
                        h5_store.append(self.table_name,dat,data_columns=True, min_itemsize={'ROLLING_TYPE': 10})
                        ...
                        h5_store.append(self.table_name,dat,data_columns=True)
                    elif self.table_name == 'WIND_AShareMgHoldReward':
                    ...
                    elif self.table_name == 'WIND_AShareMonRepBrokers':
                        dat.reset_index('Ticker', inplace=True)
                        def help(df):
                            comp = df['S_INFO_COMPCODE']
                            with open('Z:\warehouse\prod\LOCAL_DATA\CSV\WIND\comp_dict.pickle', 'rb') as file:
                        ...
                        h5_store.append(self.table_name,dat, data_columns=True,min_itemsize={'HOLDER_NAME':500,'TRANSACT_STOCK_SOURCE':200,'TRANSACT_SOURCE_FUNDS':200,
                                                                                            'HOLDER_STATUS':200,'TRANSACT_PERIOD_DESCRIPTION':200,'TRANSACTION_MODE':200,
                        ...
                        h5_store.append(self.table_name,dat,data_columns=True,min_itemsize={'SEC_ID':20})
                    elif self.table_name == 'WIND_AShareIndusRating':
                        dat.drop(['RATING_MEMO'], axis=1, inplace=True)
                        h5_store.append(self.table_name,dat,data_columns=True,min_itemsize={'REPORT_IND':19,'RATING_ANALYST':15,'RATING':20,'RATING_INSTITUTE':20,
                                                                                            'REPORT_IND':50})
                    elif self.table_name == 'WIND_AIndexConsensusData':
                        h5_store.append(self.table_name,dat,data_columns=True,min_itemsize={'Ticker':30})
                    elif self.table_name == 'WIND_AIndexConRollingData':
                        h5_store.append(self.table_name, dat, data_columns=True, min_itemsize={'Ticker': 13})
                    elif self.table_name == 'WIND_AShareEarningEst':
                        dat['ANALYST_NAME'] = dat['ANALYST_NAME'].astype(str)
                        dat['FIRST_OPTIME'] = dat['FIRST_OPTIME'].astype(str)
                        dat['ANALYST_ID'] = dat['ANALYST_ID'].astype(str)
                        dat['COLLECT_TIME'] = dat['COLLECT_TIME'].astype(str)
                        dat = dat.drop(['REPORT_NAME','REPORT_SUMMARY'], axis = 1)
                        dat['OPDATE'] = dat['OPDATE'].astype(str)

                        h5_store.append(self.table_name, dat, data_columns=True, min_itemsize={'ANALYST_NAME': 30,'RESEARCH_INST_NAME':30,
                                                                                               'FIRST_OPTIME':25,'COLLECT_TIME':25})
                    elif self.table_name == 'WIND_AShareStockRating':
                        dat.drop(['S_EST_REPORT_TITLE'], axis=1, inplace=True)
                        dat['S_EST_PRERATING_INST'] = dat['S_EST_PRERATING_INST'].astype(str)
                        h5_store.append(self.table_name, dat, data_columns=True, min_itemsize={'S_EST_INSTITUTE': 30, 'S_EST_RATINGANALYST':50, 'S_EST_PRERATING_INST':20})
                    else:
                        h5_store.append(self.table_name,dat,data_columns=True)
    ...
    def retriever(self):
        ...
        if self.table_name in ['WIND_AShareFinancialIndicator', 'WIND_AShareProfitExpress', 'WIND_AShareTTMHis',
                        ...
                        'WIND_AShareMonRepBrokers','WIND_AShareAuditOpinion','WIND_AIndexFinancialderivative','WIND_AShareSalesSegment',
            ...
            sql_where = ' where S_ProfitNotice_Period='
                            ...
                            'WIND_AMSCIIndexEOD','WIND_MSCICAIncRMBEODPrices','WIND_CIndexFuturesPositions','WIND_CBondFuturesPositions',
                            'WIND_CCommodityFuturesPos']:
        ...
        elif self.table_name in ['WIND_AShareStockRating','WIND_AShareConsensusindex','WIND_CMFOtherPortfolio',  'WIND_AShareHolderNumber', 'WIND_AShareIllegality',
            ...
            sql_where = ' where S_EVENT_ANNCEDATE='
        elif self.table_name in self.over_write_list:
            sql_where = ''
        # elif self.table_name in ['WIND_htzqedbdzzbs']:
        #     sql_where = ' where TDATE='
        elif self.table_name in ['WIND_AShareMgHoldReward']:
            sql_where = ' where ANN_DATE='
        elif self.table_name in ['WIND_MSCICAInclusionRMBWeight']:
            sql_where = ' where TRADE_DATE='
        elif self.table_name in ['WIND_BOIndexWeightsWIND']:
            sql_where = ' where CHANGE_DT='
        elif self.table_name in ['WIND_AShareStyleCoefficient']:
            sql_where = ' where S_CHANGE_DATE='
        elif self.table_name in ['WIND_AShareTradingSuspension']:
            sql_where = ' where S_DQ_SUSPENDDATE='
        elif self.table_name in ['WIND_AShareIPOPricingForecast','WIND_AShareEarningEst','WIND_AShareConsensusData','WIND_AShareConRollingData','WIND_AIndexConRollingData','WIND_AIndexConsensusData']:
            sql_where = ' where EST_DT='
        elif self.table_name in ['WIND_AShareIndusRating']:
            sql_where = ' where COLLECT_dt='
        elif self.table_name in ['WIND_AShareStockRatingConsus']:
            sql_where = ' where RATING_DT='
        else:
        ...
        if self.table_name == 'WIND_AShareMonRepBrokers':
            ...
            for date in self.cdate_list:
                if self.table_name in self.over_write_list:
                    ...
                    if self.table_name == 'WIND_AShareAnnInf':
                        df = sql_parser(queryUserTableData('select count(*) from ' + self.table_name + ' where ANN_DT=to_date(' + str(date) + ",'YYYYMMDD')"))
                    else:
    ...
    def dumper(self):
        ...
        if self.dfreq =='QUARTERLY':
            ...
            elif self.table_name in ['WIND_AShareMonRepBrokers']:
                new_cdate_list = []
                update_list = []
                month_dict = {1:31, 2:28, 3:31, 4:30, 5:31, 6:30, 7:31, 8:31, 9:30, 10:31, 11:30, 12:31}
                for date in self.cdate_list:
...
def check_eod_valid(date):
    root = 'Z:\\warehouse\\prod\\LOCAL_DATA\\CSV\\WIND\\WIND_Ashareeoddindicator\\'
    path = root + str(date) + '.csv'
    df = pd.read_csv(path)
    df = df[['S_VAL_PE','S_VAL_PB_NEW','S_DQ_TURN','UP_DOWN_LIMIT_STATUS','LOWEST_HIGHEST_STATUS']]
    rst = pd.isna(df).sum()/len(df)
    rst = pd.DataFrame(rst)
    rst.columns = ['NAN_pct']
    for col in rst.index:
        score = float(rst.loc[col])
        if col in ['S_VAL_PE','S_VAL_PB_NEW']:
            if score >= 0.3:
                print(col + ' WARNING!!!!')
                return False
        else:
...
'''
'WIND_AShareConRollingData',WIND_AIndexConRollingData','WIND_AIndexConsensusData','WIND_AShareConsensusData','WIND_AShareConsensusindex',
'WIND_AShareStockRatingConsus'
'''
# CIndexFuturesPositions，CBondFuturesPositions,CCommodityFuturesPositions 
# 

def add_new_tables(sdate=None,edate=None):
    sdate,edate,cdate_list = check_update_date(sdate = sdate, edate = edate)
    qtr_list = []
    daily_list =  ['WIND_CIndexFuturesPositions','WIND_CBondFuturesPositions','WIND_CCommodityFuturesPos']


    table_dict = {'DAILY':['WIND_AShareTechIndicators', 'WIND_AshareintensitytrendADJ', 'WIND_AShareEnergyindexADJ','WIND_AShareswingRevADJ'] ,'QUARTERLY': []}
    # AShareswingRevADJ
    for table_type in table_dict:
        for table_name in table_dict[table_type]:
            print (table_name)
            instantiate(table_name, sdate, edate, base_path = 'Z:\\warehouse\\test\\', operation = 'append',table_dict=table_dict,update_h5_flag=False)
    
    logger.info('===============add_new_tables finished======================')


def AIndex_job(sdate=None, edate=None):
    sdate, edate, cdate_list = check_update_date(sdate=sdate, edate=edate)

    table_dict = {'DAILY': ['WIND_AIndexIndustriesEODCITICS'], 'QUARTERLY': []}
    # AShareswingRevADJ
    for table_type in table_dict:
        for table_name in table_dict[table_type]:
            print(table_name)
            instantiate(table_name, sdate, edate, base_path='Z:\\warehouse\\prod\\', operation='create',
                        table_dict=table_dict, update_h5_flag=True)

    logger.info('===============tech_daily_job finished======================')

def wind_concensus_job(sdate=None, edate=None):
    sdate, edate, cdate_list = check_update_date(sdate=sdate, edate=edate)
    # 333 工作 ,'WIND_AIndexConRollingData','WIND_AShareEarningEst',
    # 已经完成'WIND_AShareConRollingData','WIND_AShareStockRating','WIND_AShareStockRatingConsus','WIND_AShareEarningEst'

    table_dict = {'DAILY': [], 'QUARTERLY': []}
    # AShareswingRevADJ
    for table_type in table_dict:
        for table_name in table_dict[table_type]:
            print(table_name)
    ...
    logger.info('===============wind_concensus_job finished======================')





# AIndex_job(20040101,20191030)

wind_concensus_job(20040101,20191112)
# wind_concensus_job(20070320,20191112)
