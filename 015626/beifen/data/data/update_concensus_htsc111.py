...
def retrieve(table_name,cdate_list,root_folder):
    ...
    if table_name in ['block_confidence', 'block_report_number', 'block_report_number_organ', 'block_diversity',
                      'block_deviation', 'block_deviation2', 'block_deviation3', 'block_emotion',
                      'block_rating_strength', 'block_adjustment','con_per_organ','con_forecast_cb_zx',
                      'con_forecast_c3_cgb_zx','con_forecast_c3_zx','con_forecast_zx','con_forecast_c2_zx',
                      'con_forecast_schedule', 'con_forecast_stk','cmb_report_subtable',
            'cmb_report_research','der_report_subtable', 'der_report_research', 'cmb_report_adjust',
            'cmb_report_score_adjust', 'i_organ_score','report_author','researcher_info','con_hy','gg_org_list',
    ...
    elif table_name in ['con_hy','researcher_info', 'author_core', 'author_core_type', 'i_report_type', 'i_organ_score',
                         'gg_org_list',  't_great_author', 'author_pjhb','t_author_honor']:
        use_date = 3
        sql_where = ''
        print(use_date)
    elif table_name in ['con_forecast_cb_zx','con_forecast_c3_cgb_zx']:
        sql_where = ' where EntryDate='
    else:
    ...
    for date in cdate_list:
        ...
        if table_name in ['con_forecast_schedule','con_forecast_stk','con_stock_deviation',
        ...
        elif table_name in ['con_per_organ','con_forecast_c3_zx','con_forecast_zx','con_forecast_c2_zx']:
            dat_fig = {'dt': 'TDATE', 'Ticker': 'STOCK_CODE'}

        elif table_name in ['block_confidence', 'block_report_number', 'block_report_number_organ', 'block_diversity',
                            'block_deviation', 'block_deviation2', 'block_deviation3', 'block_emotion', 'block_rating_strength', 'block_adjustment']:
            dat_fig = {'dt': 'TDATE', 'Ticker': 'BLOCK_CODE'}


        elif table_name in ['con_forecast_cb_zx','con_forecast_c3_cgb_zx','cmb_report_subtable', 'der_report_subtable',
        ...
        elif table_name in ['researcher_info','con_hy']:
            dat_fig = {}
            override = True
        else:    
        ...
        elif table_name in 'con_hy':
            df['dt'] = '2001-01-01'
            df = df.set_index(['dt','ICODE'])
            df.index.names = ['dt', 'Ticker']
            df.to_csv(table_folder + 'con_hy.csv', sep=',', encoding='utf_8_sig')
...
def update_consensus_data(sdate=None,edate=None,operation='append'):
    ...
    # table_list = ['con_forecast_zx','con_forecast_c2_zx','con_hy'] 20191001
    #已经生成h5，'con_forecast_c3_zx', \'block_confidence','block_report_number', 'block_report_number_organ', 'block_diversity',
    #'block_deviation3', 'block_emotion', 'block_rating_strength','block_adjustment',

    #数据库里没有这些表'block_deviation','con_forecast_cb_zx','con_forecast_c3_cgb_zx'
    #待生成h5出错'block_deviation2'20160106,'con_per_organ'20140303出错
    table_list = ['block_confidence','block_report_number','block_report_number_organ','block_diversity','block_deviation3',
                  'block_emotion','block_rating_strength','block_adjustment']
    
    table_list3 = ['con_hy', 'researcher_info', 'author_core', 'author_core_type', 'i_report_type', 't_author_honor',
    ...
    for table_name in total_table_list:
        ...
        if table_name not in ['cmb_report_research','der_report_research','author_pj']:
            ...
            h5_folder_path = root_path+'DATABASE\\SUNTIME\\'+ table_name
...
if __name__ == '__main__':
    ...
    print('****************finished*****************')
    # flag_path = 'Z:\\warehouse\\test\\LOCAL_DATA\\SUNTIME_FLAG\\' + str(end_date) + '_' + 'concensus_csv.success'
    # with open(flag_path,'w') as file:
    #     pass

    # time.sleep(1800)
    # increment_checker(end_date)
