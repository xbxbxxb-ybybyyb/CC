...
def man(start_date, end_date):
    save_folder = 'Z:\\warehouse\\prod\\LOCAL_DATA\\CSV\\wind_data\\dividendyield2\\'
    universe_folder = config_reader.getConfig('root_path', 'wind_stock_path')
    sdate_dummy,edate_dummy = 20040101,20990101    
    dividend_data = IO.read_data([sdate_dummy,edate_dummy],alt=r'Z:\warehouse\prod\DATABASE\WIND\AShareDividend\AShareDividend.h5')
    dividend_data = dividend_data[dividend_data['S_DIV_PROGRESS']==3]
    start_date_old= get_old_day_helper(start_date,250)
    dat_uni = IO.read_data([start_date_old, end_date], columns=['mkt_cap_ard', 'total_shares'], alt = r'Z:\warehouse\prod\MD\CHINA_STOCK\DAILY\WIND\MD_CHINA_STOCK_DAILY_WIND.h5')
    print(dat_uni)
