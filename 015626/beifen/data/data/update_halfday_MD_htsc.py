...
def get_halfday_data(dfprices0,dfprices1,dfhalfday0,dfhalfday1,dfminute_preclose):
    ...
    dfhalfday0 = pd.merge(dfhalfday0, dfprices0, on='Ticker')
    dfhalfday0 = dfhalfday0.rename(columns={'S_DQ_ADJFACTOR': 'adjfactor0'})
    dfhalfday0 = pd.merge(dfhalfday0, dfprices1, on='Ticker')
    dfhalfday0['adjfac'] = dfhalfday0.adjfactor0 / dfhalfday0.S_DQ_ADJFACTOR

    dfminute_preclose = dfminute_preclose.rename(columns={'close': 'pre_close'})
    dfminute_preclose = dfminute_preclose[['Ticker', 'pre_close']]
    dfhalfday0 = pd.merge(dfhalfday0, dfminute_preclose, on='Ticker')

    indicators = ['open', 'high', 'low', 'close', 'pre_close']
    for indi in indicators:
        dfhalfday0.eval(indi + '=' + indi + '* adjfac', inplace=True)
    dfhalfday0['volume'] = dfhalfday0.volume / dfhalfday0.adjfac
...
def get_total_data(dfindicator1,dfprices1,dfhalfday,df_preclose,dfprices0,day):
    ...
    df_total = pd.merge(dfhalfday, df_indi_price, on='Ticker')
    ...
    df_total = pd.merge(df_total, df_preclose, on='Ticker')
...
def run(start = None, end = None):
    sdate, edate, cdate_list = check_update_date(20191106,end)
    ...
    df_total.to_csv('Z://warehouse//prod//LOCAL_DATA//CSV//WIND//HIGH_FREQ_MD//' + str(day0) + '.csv')

def csv2h5(date_list, operation = 'append'):

    root_path = 'Z://warehouse//prod//LOCAL_DATA//CSV//WIND//HIGH_FREQ_MD//'
    table_h5_file = 'Z://warehouse//prod//MD//CHINA_STOCK//DAILY//WIND//HIGH_FREQ_DAILY_MD.h5'
...
def flag_check(date):
    path1 = 'Z:\\warehouse\\prod\\LOCAL_DATA\\FLAG\\' + str(date) + '\\' + str(date) + '_' + 'MINUTE.success'
    path2 = 'Z:\\warehouse\\prod\\LOCAL_DATA\\FLAG\\' + str(date) + '\\' + str(date) + '_' + 'RDF.success'
    return os.path.exists(path1) and os.path.exists(path2)

if __name__ == '__main__':
    sdate, edate, cdate_list = check_update_date(20191106,None)
    while True:
        if flag_check(edate):
            break
    run()
