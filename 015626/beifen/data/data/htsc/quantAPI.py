...
def hfactor(stockList=[], factorList=[], dateList=[], factorPar=[]):
    ...
    data = data.read().decode('utf-8')
    if 0 == dataExtractType:
        return parse_json(data), dateList, stockList
    else:
        beginoffset = data.find(';')
        if 0 < beginoffset:
            data = ('[['+data[beginoffset+1:-1] +']]').replace(';','],[')
...
def parse_json(data):
    ...
    for item in title:
        dic_data[item] = []
    for item in data[1:]:
        for k,v in zip(title, item.split(',')):
            dic_data[k].append(v)
    pd_data = pd.DataFrame(dic_data)
    try:
        if len(pd_data['DATETIME'][0]) == 8:
            pd_data['DATETIME'] = pd.to_datetime(pd_data['DATETIME'], format='%Y%m%d')
        else:
            pd_data['DATETIME'] = pd.to_datetime(pd_data['DATETIME'], format='%Y%m%d%H%M%S')
        pd_data.replace('NaN', np.nan)
        return pd_data
    except:
        return np.nan
