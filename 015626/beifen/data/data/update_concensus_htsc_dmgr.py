...
def retrieve(table_name,cdate_list,root_folder):
    ...
    for date in cdate_list:
        ...
        if table_name in 'researcher_info':
            ...
            csv_path = 'Z:\\warehouse\\test\\LOCAL_DATA\\CSV\\gogoal_htsc\\' + table_name + '\\' + str(date) + '.csv'
            df.to_csv(csv_path, sep='|', encoding='gbk')

        elif table_name == 'der_report_subtable':
            csv_path = 'Z:\\warehouse\\test\\LOCAL_DATA\\CSV\\gogoal_htsc\\der_report_subtable\\' + str(date) + '.csv'
            df.to_csv(csv_path, sep='|', encoding='gbk')
...
def update_consensus_data(sdate=None,edate=None,operation='append'):
    ...
    table_list = ['der_report_research']
    ...
    root_folder = 'Z:\\warehouse\\test\\LOCAL_DATA\\CSV\\gogoal_htsc\\'
...
def convert_table():
    table_name = 'der_report_research'
    root = 'Z:\\warehouse\\prod\\LOCAL_DATA\\CSV\\gogoal_htsc\\' + table_name + '\\'
    root2 = 'Z:\\warehouse\\test\\LOCAL_DATA\\CSV\\gogoal_htsc\\' + table_name + '\\'
    ...
    for path in os.listdir(root):
        ...
        df = df[['ID','CODE','CODE_NAME']]
        df.to_csv(root2 + path, sep = ',', encoding='gbk')
        # print(df)
        # break
