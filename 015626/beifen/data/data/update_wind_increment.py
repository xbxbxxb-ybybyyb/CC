...
#     table_path = h5_path +  table_name + '\\' + table_name + '.h5'
#     for date in cdate_list:
#         df = IO.read_data([20090101, 20201231], columns=['CITICS_IND_CODE', 'CUR_SIGN'], alt = table_path)
...
#         df.drop(['CITICS_IND_CODE', 'CUR_SIGN', 'lv2_ind_code', 'lv2_ind_code'], axis=1, inplace=True)
#         df.reset_index('dt', inplace=True)
#         df.drop('dt', axis=1, inplace=True)
...
#     for date in cdate_list:
#         print(date)
#         df = IO.read_data(date,alt= source_path)
#         df.reset_index('dt', inplace=True)
#         df.drop('dt', axis=1, inplace=True)
...
def industry_retriever(cdate_list, save_path):
    ...
    table_path = h5_path +  table_name + '\\' + table_name + '.h5'
    for date in cdate_list:
        df = IO.read_data([20090101, 20201231], columns=['CITICS_IND_CODE', 'ENTRY_DT','REMOVE_DT','CUR_SIGN'], alt = table_path)
        # df = df[df['CUR_SIGN'] == 1.0]
...
def create_listing_delisting_filter(start_date, end_date, merged_mask=True,
    ...
    delist_date_pd.reindex(index=trading_dates)
    ...
    list_date_pd.reindex(index=trading_dates)
...
def backfill(factor_name, start_date, end_date, factor_qtr_pd, issuing_date_ps=None, trading_date_list=None,
...
def update_wind_qtr2daily(sdate,edate,h5_quarterly_path,h5_daily_path,operation='append',factor_list=None, max_process=5):
    test_fail = []
    ...
    if operation=='append':
        ...
        op_type = None
        qtr_list,stock_code = get_qtr_list(cdate_list,num_qtr=100)
        qtr_list = [i for i in qtr_list if i>20090000]
    ...
    issuing_date_ps = issuing_date_checker(issuing_date_ps)
    trading_date_list = tdt.get_trading_date_range(start_date, end_date)
    
    #issue_date_mat = get_issue_date_mat(qtr_list)
    dat = IO.read_data([qtr_list[0], qtr_list[-1]],alt=h5_quarterly_path)
    print(dat)
    factor_list = dat.columns.tolist()
    logger.info('Read Done')
    factor_exception = 'stm_issuingdate'
    if factor_exception in factor_list:
        factor_list.remove(factor_exception)
    fac_num = len(factor_list)
    data_final_list = {}
    with Pool(max_process) as executor:
        future_tasks = []
        for factor_name in factor_list:
            future_tasks.append(executor.submit(backfill, factor_name,  qtr_list[0], end_date, dat[[factor_name]], issuing_date_ps, trading_date_list,
                                                True, listing_delisting_filter))
        for f in as_completed(future_tasks):
            try:
                ret = f.done()
                if ret:
                    factor_name = f.result()[1]
                    data_final_list[factor_name] = f.result()[0]
            except Exception as e:
                f.cancel()
                logger.error(e)
    logger.info('multi end')

    for factor_name in data_final_list:
            try:
                data_final_MI = data_final_list[factor_name]
                data_final_MI[factor_name] = data_final_MI[factor_name].astype('float64')
                logger.info(factor_name + ' ,Saving to h5')
                IO.pd_hdf5_writer(data_final_MI,h5_daily_path,factor_name,append=op_type)
                logger.info('Save done')
            except Exception as e:
                logger.error(e)
                test_fail.append(factor_name)
    return test_fail
...
class Factor(object):
    def __init__(self, start_date, end_date, dtype, dfreq, dsource, mkttype, ftype, factor_list, operation='append', factor_scale = None, stock_list= None, max_process=5):
        ...
        self.h5_path = 'Z:\\warehouse\\prod\\INDUSTRY\\CHINA_STOCK\\DAILY\\WIND\\tmp.h5'
...
class DailyFactor(Factor):
    ...
    def csv_to_database(self, h5_checker = None):
        ...
        self.fail_dict_master[dict_name] = update_wind_daily_h5(self.cdate_list, self.factor_list, self.csv_path,
                                                            self.h5_path, operation='create', factor_scale=self.factor_scale)
...
class FDD_qtr(QuarterlyFactor):
    def __init__(self, start_date, end_date):
        ...
        factor_table = pd.read_excel('find_indicators1.xlsx',header=0)
        factor_name_dict = {}
...
class INDUSTRY(DailyFactor):
    def __init__(self, start_date, end_date):
        factor_list = ['industry_citiccode']
