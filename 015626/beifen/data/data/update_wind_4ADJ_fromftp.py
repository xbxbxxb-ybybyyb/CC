...
name_mapping_dict = {'WIND_AShareAFIndicator': 'AShareANNFinancialIndicator',
                     'WIND_Ashareeoddindicator': 'AShareEODDerivativeIndicator',
                     'WIND_ASharePledgepro': 'ASharePledgeproportion',
                     'WIND_AShareFFCalendar': 'AShareFreeFloatCalendar',
                     'WIND_CMFundAssetPortfolio': 'ChinaMutualFundAssetPortfolio',
                     'WIND_CMFundIndPortfolio': 'ChinaMutualFundIndPortfolio',
                     'WIND_CMFundStockPortfolio': 'ChinaMutualFundStockPortfolio',
                     'WIND_CMFundBondPortfolio': 'ChinaMutualFundBondPortfolio',
                     'WIND_AShareIndClassCITICS': 'AShareIndustriesClassCITICS',
                     'WIND_AShareMgHoldReward': 'AShareManagementHoldReward',
                     'WIND_AShareEXRightDivRecord': 'AShareEXRightDividendRecord',
                     'WIND_AShareswingRevADJ': 'AShareswingReversetrendADJ',
                     'WIND_AShareMonRepBrokers': 'AShareMonthlyReportsofBrokers',
                     'WIND_AShareFinExpense': 'AShareFinancialExpense',
                     'WIND_AshareOthreceivables': 'AshareOtherreceivables',
                     'WIND_Top5ByAccReceivable': 'Top5ByAccountsReceivable',
                     'WIND_AshareFinaccounts': 'AshareFinancialaccounts',
                     'WIND_AShareConRollingData': 'AShareConsensusRollingData',
                     'WIND_AIndexConRollingData': 'AIndexConsensusRollingData',
                     'WIND_CCommodityFuturesPos': 'CCommodityFuturesPositions'}
...
class WIND_DATABASE:
    def __init__(self, table_name, sdate, edate,
        ...
        if self.dfreq == 'DAILY':
            ...
            raise Exception
        self.over_write_list = ['WIND_AShareIndClassCITICS', 'WIND_AShareDescription', 'WIND_AShareIndustriesCode',
                                'WIND_AShareST',
        ...
        with pd.HDFStore(self.table_h5_file) as h5_store:
            ...
            for fname in update_list:
                ...
                if len(dat1) <= min_size:
                    ...
                    if self.table_name == 'WIND_AShareProfitNotice':
                        ...
                        h5_store.append(self.table_name, dat, data_columns=True,
                                        min_itemsize={'HOLDER_NAME': 500, 'TRANSACT_STOCK_SOURCE': 200,
                                                      ...
                                                      'HOLDER_STATUS': 200, 'TRANSACT_PERIOD_DESCRIPTION': 200,
                        ...
                        h5_store.append(self.table_name, dat, data_columns=True,
                                        min_itemsize={'REPORT_IND': 19, 'RATING_ANALYST': 15, 'RATING': 20,
                                                      'RATING_INSTITUTE': 20,
                                                      'REPORT_IND': 50})
                    ...
                    elif self.table_name == 'WIND_AShareTechIndicators':
                        dat.drop(['VMACD_EMA12D', 'VMACD_EMA26D', 'VRSI_6D', 'VROC_12D', 'SOBV', 'VR_26D'], axis=1, inplace=True)
                        h5_store.append(self.table_name, dat, data_columns=True)
    ...
    def retriever(self):
        ...
        if self.table_name in ['WIND_AShareFinancialIndicator', 'WIND_AShareProfitExpress', 'WIND_AShareTTMHis',
                               ...
                               'WIND_AShareAFIndicator',
                               'WIND_FinNotesDetail', 'WIND_AShareIBrokerIndicator', 'WIND_AShareInsuranceIndicator',
                               'WIND_AShareBankIndicator',
                               'WIND_AShareMonRepBrokers', 'WIND_AShareAuditOpinion', 'WIND_AIndexFinancialderivative',
                               'WIND_AShareSalesSegment',
            ...
            sql_where = ' where S_ProfitNotice_Period='
                                 ...
                                 'WIND_AMSCIIndexEOD', 'WIND_MSCICAIncRMBEODPrices', 'WIND_CIndexFuturesPositions',
                                 'WIND_CBondFuturesPositions',
        ...
        elif self.table_name in ['WIND_AShareConRollingData', 'WIND_AIndexConRollingData', 'WIND_AIndexConsensusData']:
                ...
                if self.table_name in self.over_write_list:
                    ...
                    if total_row_count < 100000:
                        ...
                        for i in range(group + 1):
                            ...
                            sql_use = '(select * from ' + self.table_name + ' where rownum <= ' + str(
                                end_line) + ') ' + 'minus(select * from ' + self.table_name + ' where rownum <= ' + str(
                                start_line) + ')'
                            print(sql_use)
                            df = sql_parser(queryUserTableData(sql_use))
                            df_list.append(df)
                    ...
                    if self.table_name == 'WIND_AShareAnnInf':
                        df = sql_parser(queryUserTableData(
                            'select count(*) from ' + self.table_name + ' where ANN_DT=to_date(' + str(
                                date) + ",'YYYYMMDD')"))
                    else:
                        df = sql_parser(
                            queryUserTableData('select count(*) from ' + self.table_name + sql_where + str(date)))
                    total_row_count = int(df['COUNT(*)'])
                    save_name = os.path.join(self.table_csv_path, str(date) + '.csv')
                    ...
                    if total_row_count < 100000:
                        ...
                        for i in range(group + 1):
                            ...
                            sql_use = '(select * from ' + self.table_name + sql_where + str(
                                date) + ' and rownum <= ' + str(
                                end_line) + ') ' + 'minus(select * from ' + self.table_name + sql_where + str(
                                date) + ' and rownum <= ' + str(start_line) + ')'
                            print(sql_use)
                            df = sql_parser(queryUserTableData(sql_use))
                            df_list.append(df)
                        df = pd.concat(df_list)
...
def downloadcsv(table_name, date):
    ...
    localpath = os.path.join('Z:\\warehouse\\prod\\LOCAL_DATA\\CSV\\WIND',table_name,str(date) + '.csv')
    remotepath = os.path.join('/XQuant/015626/ftp_data/WIND', table_name,str(date) + '.csv')
    if table_name == 'WIND_AShareswingRevADJ':
        remotepath = os.path.join('/XQuant/015626/ftp_data/WIND/WIND_AShareswingReversetrendADJ',  str(date) + '.csv')
    ftp = FTP()
...
def instantiate(table_name, sdate, edate, base_path, operation, table_dict, update_h5_flag=False):
    ...
    wind_db = WIND_DATABASE(table_name, sdate, edate, base_path,
                            ...
                            operation=operation)
    # wind_db.retriever()
    downloadcsv(table_name, sdate)
    # if not table_name in ['WIND_AShareEquityPledgeInfo']:
    if update_h5_flag:
        wind_db.dumper()
        flag_root = 'Z:\\warehouse\\prod\\LOCAL_DATA\\FLAG\\' + str(edate) + '\\RDF\\'
        if table_name in name_mapping_dict:
            table_name = name_mapping_dict[table_name]
        else:
...
def add_new_tables(sdate=None, edate=None):
    ...
    table_dict = {'DAILY': ['WIND_AShareTechIndicators', 'WIND_AshareintensitytrendADJ', 'WIND_AShareEnergyindexADJ',
                            'WIND_AShareswingRevADJ'], 'QUARTERLY': []}
    # AShareswingRevADJ
    for table_type in table_dict:
        for table_name in table_dict[table_type]:
            print(table_name)
            instantiate(table_name, sdate, edate, base_path='Z:\\warehouse\\test\\', operation='append',
                        table_dict=table_dict, update_h5_flag=False)
...
def tech_daily_job(sdate=None, edate=None):
    ...
    for table_type in table_dict:
        for table_name in table_dict[table_type]:
            print(table_name)
            instantiate(table_name, sdate, edate, base_path='Z:\\warehouse\\prod\\', operation='append',
...
flag_path = 'D:\\warehousetest\\FLAG\\' + str(end_date) + '_'
with open(flag_path + 'tech_daily.start', 'w') as file:
    pass

tech_daily_job()

with open(flag_path + 'tech_daily.success', 'w') as file:
    pass
# tech_daily_job()