...
def update_wind(sdate, edate):
    ...
    for _type in [MD, INDEX]:
        _type(sdate, edate).cronb()

    for _type in [INDUSTRY,CINDUSTRY,FDD]:
        _type(sdate, edate).cronb()
    
    flag_path = flag_root + str(edate) + '_' + 'MD.success'
...
def tmp(sdate, edate):
    ...
    for _type in [INDUSTRY,CINDUSTRY]:
        _type(sdate, edate).cronb()


# tmp(20190819,20190819)