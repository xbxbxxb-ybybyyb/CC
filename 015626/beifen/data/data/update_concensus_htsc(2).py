...
def retrieve_bak(table_name,cdate_list,root_folder):
    ...
    for date in cdate_list:
        ...
        elif table_name in ['change_event']:
            sql_use = sql_select + " where EntryDate>=to_date('" + str(date) +  " 0:00:00','YYYYMMDD HH24:mi:ss') and EntryDate<=to_date('" +  str(date) + " 23:59:59','YYYYMMDD HH24:mi:ss')"
...
def retriever(edate,root_folder):
    ...
    zip_root = 'Z:\\warehouse\\prod\\LOCAL_DATA\\CSV\\zip_suntime\\'
    ...
    # download index
    remotepath = '/XQuant/013160/zip_suntime/' + str(edate) + '/' + str(edate) + '.zip'
    localpath = zip_root + '\\' + str(edate) + '.zip'
    ...
    csv_path = 'Z:\\warehouse\\prod\\LOCAL_DATA\\CSV\\zip_suntime\\' + str(edate) + '\\'
    with zipfile.ZipFile(localpath, 'r') as zipObj:
        zipObj.extractall(csv_path)

    table_path = 'Z:\\warehouse\\prod\\LOCAL_DATA\\CSV\\zip_suntime\\' + str(self.edate) + '\\data\\user\\013160\\warehouse\\prod\\' + str(self.edate) + '\\LOCAL_DATA\\CSV\\gogoal_htsc\\' + table_name 

    table_path = 'Z:\\warehouse\\prod\\LOCAL_DATA\\CSV\\' + str(self.edate) + '\\data\\user\\013160\\warehouse\\prod\\LOCAL_DATA\\CSV\\WIND\\' + table_name 
    os.system('cp -r ' + table_path + ' Z:\\warehouse\\prod\\LOCAL_DATA\\CSV\\WIND')
    return
...
def update_consensus_data(sdate=None,edate=None,operation='append'):
    ...
    # total_table_list = ['con_forecast_schedule']
...
if __name__ == '__main__':
    ...
    flag_path = 'Z:\\warehouse\\prod\\LOCAL_DATA\\SUNTIME_FLAG\\' + str(end_date) + '_' + 'concensus_csv.success'
