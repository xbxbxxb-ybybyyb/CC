...
class IndexDividends(object):
    ...
    def __init__(self, date, futures = ['HS300', 'ZZ500', 'SH50'], lookback = 5, writepath = 'A:\\weiyc\\每日股指期货分红预测\\'):
        self.date = str(date)
    ...
    def Index_Info(self, aim):
        ...
        idx = IO.read_data([self.date], columns=['close'], dfreq=DFreq.DAILY, dtype=DType.INDEX, dsource=DSource.WIND, ftype=FType.MD)
        idx_close = idx.xs(tmp, level='Ticker').iloc[0][0]
    ...
    def Predict_Div_Exdate(self, tickers):
        ...
        tmp = []
        # 获取上年及上上年除权除息日
        asdpath = 'Z:/warehouse/prod/DATABASE/WIND/AShareDividend/AShareDividend.h5'
        t = pd.DataFrame({'Ticker': tickers})
        asd1 = IO.read_data([str(self.year - 2) + '1231'], alt = asdpath).reset_index()[['Ticker','EX_DT']]
        merge1 = pd.merge(t, asd1, on = 'Ticker', how = 'left')
        year1 = merge1.EX_DT.tolist()
        for i in range(len(tickers)):
            if year1[i] == year1[i]:
                year1[i] = np.nan if ((year1[i]//10000) < (self.year - 1)) else str(int(year1[i]))

        asd2 = IO.read_data([str(self.year - 3) + '1231'], alt = asdpath).reset_index()[['Ticker', 'EX_DT']]
        merge2 = pd.merge(t, asd2, on='Ticker', how='left')
        year2 = merge2.EX_DT.tolist()
        for i in range(len(tickers)):
            if year2[i] == year2[i]:
                year2[i] = np.nan if ((year2[i]//10000) < (self.year - 2)) else str(int(year2[i]))
    ...
    def Div_Info(self, predict_div_exdate):
        # 分红实施
        asdpath = 'Z:/warehouse/prod/DATABASE/WIND/AShareDividend/AShareDividend.h5'
        a = IO.read_data([(self.year - 1) * 10000 + 1231], alt = asdpath)
        if len(a) == 0:
    ...
    def Total_Shares_Info(self, tickers):
        sto = IO.read_data([self.date],columns='total_shares', dfreq=DFreq.DAILY, dtype=DType.STOCK, dsource=DSource.WIND, ftype=FType.MD)
        sto = sto[sto.index.get_level_values(1).isin(tickers)]
        sto = sto.reset_index().drop('dt', axis=1).set_index('Ticker')
        sto['total_shares'] = sto.total_shares * 10000  # total_shares 单位为万股
        return sto.to_dict()['total_shares']
            ...
            if ticker in list(div_info.index):
                ...
                if (predict_net_profit[ticker] < 0) | (int(self.date) > int(self.date[: 4] + '0430')):
                    ...
                    res = res.append(pd.DataFrame([[tmp, predict_div_exdate[ticker]]], index=[ticker],
                                                  columns=['dividendsper_share_pretax', 'exrights_exdividend_date']))
        sto_close = IO.read_data([self.date], columns='close', dfreq=DFreq.DAILY, dtype=DType.STOCK, dsource=DSource.WIND, ftype=FType.MD)
        sto_close = sto_close.reset_index()[['Ticker','close']]
        res = res.reset_index().rename(columns = {'index':'Ticker'})
        res = pd.merge(res,sto_close,on='Ticker',how='left').set_index('Ticker', drop = True)
        res['weights'] = weights
        # 对于指数点位变动的影响为 每股税前分红 / 收盘价 * 权重 * 指数收盘价
    ...
    def run(self):
        ...
        writer = pd.ExcelWriter(os.path.join(self.writepath, '股指期货分红预测_%s.xlsx') % self.now_trading_date)
