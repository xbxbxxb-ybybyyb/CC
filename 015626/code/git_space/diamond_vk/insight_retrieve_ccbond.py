...
def retrieve_ccbond_helper(start_time, stop_time, num_per_exec=2500, release_resource=True):
    ...
    newdf = ccbond_pd.reset_index(level = 1)
    newdf['isSH'] = newdf.Ticker.apply(lambda x:x.endswith('.SH'))
    newdf.loc[newdf.isSH == True, 'volume'] = newdf.loc[newdf.isSH == True, 'volume'] * 10
    ccbond_pd = newdf.reset_index().set_index(['dt','Ticker']).drop(['isSH'], axis= 1).sort_index()
