...
class strongFactorTest:
    def __init__(self, start_date, end_date, segment_number=20, style_list=['freeturn', 'BT_Time'], cal_mi = False,
                 time_interval=[93000000, 150000000], first_over_threshold=False, break_is_zt=False, last_is_zt=False,
                 file_path = '/data/user/013600/generalStrong/all_factor/sft_base_data/sft_base_data.h5'):
    ...
    def load_data(self, file_path):
        ...
        df = self_IO.read_data([self.start_date, self.end_date], alt=file_path)
        ...
        #样本筛选
        df = df[time_interval_filter & first_over_threshold_filter & last_is_zt_filter & break_is_zt_filter]
    ...
    def get_corr_sta(self, corr_month):
        ...
        if self.cal_mi:
            ...
            mi_value = mine.mic()
            sta_df['mi_tot'] = [np.nan, np.nan, mi_value]
    ...
    def get_factor_information(self):
        ...
        factor_information['Factor Info'] = pd.Series({'Factor Name': self.factor_name,
                                                       ...
                                                       '':''})
        factor_information[' '] = ['Segment Number', 'Time Interval', 'First Over Threshold', 'Last is Zt', 'Break is Zt', 'Style List']
        factor_information['Settings'] = [self.segment_number, str(self.time_interval), str(self.first_over_threshold),
                                          str(self.last_is_zt), str(self.break_is_zt) ,str(self.style_list)]
...
factor_df = self_IO.read_data([start_date, end_date], alt='/data/user/013600/generalStrong/all_factor/sft_base_data/sft_base_data.h5')
result_path = '/data/user/013600/generalStrong/test_factor/%s/' % ('all_factor_5_test')
