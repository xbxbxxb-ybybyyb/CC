...
class strongFactorTest:
    def __init__(self, start_date, end_date, segment_number=20, style_list=['freeturn', 'BT_Time'], cal_mi = False,
                 time_interval=[93000000, 150000000], last_is_zt=False, test_sample='bt',
    ...
    def load_data(self):
        if self.test_sample == 'bt':
            df = self_IO.read_data([self.start_date, self.end_date], alt='/data/user/013600/generalStrong/all_factor/sft_base_data/sft_base_data.h5')
        elif self.test_sample == 'zt':
            df = self_IO.read_data([self.start_date, self.end_date], alt='/data/user/013600/generalStrong/all_factor_zt/sft_base_data/sft_base_data.h5')

        last_is_zt_filter = df['last_is_zt'] == int(self.last_is_zt)
        if self.test_sample == 'bt':
            time_interval_filter = (df['BT_Time'] >= self.time_interval[0]) & (df['BT_Time'] <= self.time_interval[1])
            first_over_threshold_filter = df['first_over_threshold'] == self.first_over_threshold
            break_is_zt_filter = df['label_bk_tick_zt'] == self.break_is_zt
            all_filter = (time_interval_filter & last_is_zt_filter & first_over_threshold_filter & break_is_zt_filter)
        elif self.test_sample == 'zt':
            ...
            all_filter = (time_interval_filter & last_is_zt_filter & open_is_zt_filter & first_is_zt_filter)
        #样本筛选
        df = df[all_filter]
        df = df.rename(columns = {'label_T_is_zt':'class', 'label_TN_o2ul':'value'})
    ...
    def generate_pdf(self, result_dic, save_path):
        ...
        elements += [h_ds, p_dis, p_styles0, p_styles1, t_styles, p_double_sort, h_dis_month, p_dis_month]
    ...
    def get_corr_sta(self, corr_month):
        ...
        #if self.cal_mi:
            #mine = MINE()
            #mine.compute_score(self.basic_df['factor'], self.basic_df['mixed'])
            #mi_value = mine.mic()
            #sta_df['mic_tot'] = [np.nan, np.nan, mi_value]
...
'''
start_date, end_date = 20190101, 20190630
today = dt.datetime.now().strftime("%Y%m%d")
factor_type = 'PastMinute'
sft = strongFactorTest(start_date, end_date, style_list=['freeturn', 'BT_Time'], cal_mi=True)

factor_df = self_IO.read_data([start_date, end_date], alt='/data/user/013600/generalStrong/all_factor/all_factor_20160101_20190930_20191223.h5')#'/data/user/013600/generalStrong/%s/%s_%d_%d.h5'%(factor_type,factor_type,  20160101, 20191205))
result_path = '/data/user/013600/generalStrong/test_factor/%s/' % ('all_factor_out_of_sample')
if not os.path.exists(result_path):
    os.makedirs(result_path)
for factor_col in list(factor_df.columns)[120:]:
    print(factor_col)
    if factor_col not in ['bid_qty_chg_5_1']:
        sft.factor_test(factor_df[[factor_col]], result_path=result_path, factor_corr_test=True)
    break
corr_matrix = factor_df.corr()
'''

#style_factor默认为两个字符串元素的list, 可供选择的字段包括'/data/user/013600/generalStrong/all_factor/sft_base_data/sft_base_data.h5'文件中全部的列名
#对于因子库中高相关的因子，可以被选中为其中一个style_list,看一下被因子库的因子解释之后，新的因子是否存在增量信息

#cal_mi为是否计算互信息的参数，计算互信息需要用到minepy包，在个人docker中安装
#互信息计算速度很慢，且结果与样本总数量有较大关系

#test_sample有两个值可以选，一个是bt，即之前的突破的样本集；另一个为zt，是新的涨停样本集
'''
if __name__ == '__main__':
    start_date, end_date = 20160101, 20190314
    sft = strongFactorTest(start_date, end_date, style_list=['freeturn', 'ZT_Time'], cal_mi=False, test_sample='zt')
    factor_df = self_IO.read_data([start_date, end_date], alt='/data/user/013600/generalStrong/all_factor_zt/all_factor_zt_20160101_20190930_20191225.h5')
    result_path = '/data/user/013600/generalStrong/test_factor/%s/' % ('all_factor_zt')
    if not os.path.exists(result_path):
        os.makedirs(result_path)
    for factor in factor_df.columns:
        if factor=='Last_20_absolute_strength_zb':
            print(factor)
            #try:
            sft.factor_test(factor_df[[factor]], result_path=result_path, factor_corr_test=True)
