...
class FddAlp:
    def __init__(self,stDate,edDate,universe = 'alpha_universe', indexName = None):
        ...
        if indexName is None:
            self.index = IO.read_data([stDate,edDate], alt = '/data/group/800080/warehouse/prod/UNIV/CHINA_STOCK/DAILY/OPTM/UNIV_CHINA_STOCK_DAILY_OPTM.h5')
        else:
    ...
    def genalp(self):
        ...
        self.genprofitchg()
        return self
    def genfalp(self,alpName = 'falp', usefdays = False):
        stDate = self.stDate
        edDate = self.edDate
        ...
        for fd in fdays:
            ...
            # ipo data
            ipodata = pd.read_hdf(r'Z:\warehouse\prod\ETC\CHINA_STOCK\WIND\STOCK_LISTING_DELISTING_DATE.h5')
            # get secuCodes
            ...
            secucodevolsd = list(datavol[0:200].index)
	...
	def genfalpdaily(self,alpName = 'falp'):
        ...
        for td in tradingdays:
            ...
            data = IO.read_data(fdate,alt = os.path.join(self.fddqtrlypath,'falp.h5'))
            ...
            if os.path.exists(fileName):
                ...
                append = None
            IO.pd_hdf5_writer(data,fileName,dataset = 'falp',append = append)
        return None

    def gengrossmargin(self,alpName = 'grossmargin_ttm2_alpuniv'):
        h5_data = IO.read_data([self.stDate,self.edDate], columns = ['grossmargin_ttm2'],ftype=IO_enums.FType.FDD,h5root=r'W:\zhangf\data')
    ...
    def genoperatecashflow(self,alpName = 'operatecashflow_ttm2_alpuniv'):
        h5_data = IO.read_data([self.stDate,self.edDate], columns = ['operatecashflow_ttm2'],ftype=IO_enums.FType.FDD,h5root=r'W:\zhangf\data')
        data = h5_data.join(self.index)
        data = data[data[self.universe] == 1]
    ...
    def genoperateincome(self,alpName = 'operateincome_alpuniv'):
        ...
        savepath = self.fdddailypath
        h5_data = IO.read_data([stDate,edDate], columns = ['qfa_operateincome'],ftype=IO_enums.FType.FDD,h5root=r'W:\zhangf\data')
        index = self.index
    ...
    def genroa(self,alpName = 'roa_alpuniv'):
        h5_data = IO.read_data([self.stDate,self.edDate], columns = ['qfa_roa'],ftype=IO_enums.FType.FDD,h5root=r'W:\zhangf\data')
        data = h5_data.join(self.index)
        data = data[data[self.universe] == 1]
        roa = data['qfa_roa']
        # unstack
    ...
    def genroaroe(self,alpName = 'roaroe_alpuniv'):
        h5_data = IO.read_data([self.stDate,self.edDate], columns = ['qfa_roe','qfa_roa'],ftype=IO_enums.FType.FDD,h5root=r'W:\zhangf\data')
        data = h5_data.join(self.index)
        data = data[data[self.universe] == 1]
    ...
    def genroe(self,alpName = 'roe_alpuniv'):
        h5_data = IO.read_data([self.stDate,self.edDate], columns = ['qfa_roe'],ftype=IO_enums.FType.FDD,h5root=r'W:\zhangf\data')
        data = h5_data.join(self.index)
        data = data[data[self.universe] == 1]
        roe = data['qfa_roe']
        # unstack
    ...
    def genb2p(self,alpName = 'B2P_alpuniv'):
        h5_data = IO.read_data([self.stDate,self.edDate], columns = ['pe_ttm','pb_lf'],ftype=IO_enums.FType.FDD,h5root=r'W:\zhangf\data')
    ...
    def gene2p(self,alpName = 'E2P_alpuniv'):
        h5_data = IO.read_data([self.stDate,self.edDate], columns = ['pe_ttm'],ftype=IO_enums.FType.FDD,h5root=r'W:\zhangf\data')
        data = h5_data.join(self.index)
        data = data[data[self.universe] == 1]
        E2P = 1/data['pe_ttm']
        # unstack
