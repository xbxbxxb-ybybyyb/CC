...
class stk2idx_close_rank_short(FactorGeneratorComplex):
    def __init__(self):
        super(stk2idx_close_rank_short, self).__init__(required_columns=['close_zz500', 'amount_zz500'],
                                                     lookback_bars=2000)
    ...
    def on_bar(self, data):
        ...
        # factor logic
        rank_win = 30
        stk_rank = calc_ts_pct(stk_close,rank_win)
        stk2idx_rank_raw = stk_rank.mean(axis=1)
        ma_win = 30
        ts_pct_win = 2400
        min_pct = 0.9
        stk2idx_close_rank_short = calc_ma_helper(stk2idx_rank_raw,ma_win,ts_pct_win,min_pct)

        factor = stk2idx_close_rank_short.to_frame()
