...
class stk2idx_amt_jump_std(FactorGeneratorComplex):
    def __init__(self):
        super(stk2idx_amt_jump_std, self).__init__(required_columns=['close_zz500', 'amount_zz500'],
                                                     lookback_bars=2000)
    ...
    def on_bar(self, data):
        ...
        short_win = 5
        long_win = 30
        stk_amt_short = stk_amt.rolling(short_win,int(short_win*0.5)).mean()
        stk_amt_long = stk_amt.rolling(long_win,int(long_win*0.5)).mean()
        stk_amt_jump = stk_amt_short - stk_amt_long
        stk2idx_amt_jump_raw = stk_amt_jump.mean(axis=1)

        std_win = 30
        ts_pct_win = 1200
        stk2idx_amt_jump_std = calc_std_helper(stk2idx_amt_jump_raw,std_win,ts_pct_win)


        factor = stk2idx_amt_jump_std.to_frame()
