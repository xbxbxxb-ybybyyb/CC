...
class stk2idx_max_ret_a2p(FactorGeneratorComplex):
    def __init__(self):
        super(stk2idx_max_ret_a2p, self).__init__(required_columns=['close_zz500', 'amount_zz500'],
                                                     lookback_bars=2000)
    ...
    def on_bar(self, data):
        ...
        ts_pct_win = 1200
        min_pct = 0.9
        stk2idx_max_ret_a2p = calc_ma_helper(stk2idx_score_a2p_raw,ma_win,ts_pct_win,min_pct) 
        # ts_factor_quick(stk2idx_trade_strength, price, factor_name, layers=5)

        factor = stk2idx_max_ret_a2p.to_frame()
