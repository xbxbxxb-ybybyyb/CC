...
class wyc_ts20_icspot_if(FactorGenerator):
    ...
    def on_bar(self, df):
        ...
        factor = ts_sum(((df.close_spot - df.low_spot) - (df.high_spot - df.close_spot)) / (
                    df.high_spot - df.low_spot) * df.volume_spot, 20)
        # factor = ts_mean(factor, 5)
        factor = ts_rank_bk(factor, 4 * 242)
        factor = ts_mean(factor, 20)

        factor.columns = [columnname]
        factor = factor.fillna(method='ffill')
        factor = rolling_normalize(factor, 4 * 242)
        return factor