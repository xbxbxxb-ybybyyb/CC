"""The underlying data structure used in gplearn_super.

The :mod:`gplearn_super._program` module contains the underlying representation of a
computer program. It is used for creating and evolving programs used in the
:mod:`gplearn_super.genetic` module.
"""
...
from .functions import _Function, _ts_function_map, _fixed_function_map
...
class _Program(object):
    ...
    def __init__(self,
        ...
        self.d_ls = [5, 10, 20, 30, 50, 80, 100, 150, 200, 300, 400, 500]
        self.ts_function_set = [v for k, v in _ts_function_map.items()]
        self.fixed_function_set = []
        # self.fixed_function_set = [v for k, v in _fixed_function_map.items()]
    ...
    def build_program(self, random_state):
        ...
        while terminal_stack:
            ...
            # Determine if we are adding a function or terminal
            if (depth < max_depth) and (method == 'full' or choice < len(self.function_set) + len(self.ts_function_set)):
                function = random_state.randint(len(self.function_set))
                # function = self.function_set[function]
                ...
                # 新增
                    ...
                    terminal = make_ts_function(self.fixed_function_set[terminal - self.n_features], self.d_ls, random_state)
                program.append(terminal)
                terminal_stack[-1] -= 1
                while terminal_stack[-1] == 0:
                    terminal_stack.pop()
