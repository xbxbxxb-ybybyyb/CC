...
class wyc_icih(FactorGenerator):
    def __init__(self):
        required_columns=['close_ic', 'close_ih']
        lookback_bars=2000
        super(wyc_icih, self).__init__(required_columns=required_columns,
                                  lookback_bars=lookback_bars)

    def on_bar(self, df):
        columnname = self.__class__.__name__

        factor = df['close_ic'] - df['close_ih']
