# -*- coding: utf-8 -*-
"""
Created on Tue Aug 25 14:53:30 2020
...
class CFG22_CC(FactorGeneratorComplex):
    def __init__(self):
        required_columns=['low_zz500', 'close_zz500', 'weight_zz500']

        super(CFG22_CC, self).__init__(required_columns=required_columns
                                  )
    ...
    def on_bar(self, df):
        ...
        a = hlow-hclose.rolling(150, min_periods = 30).mean()
        htemp = ((a)*hweight).mean(axis = 1)

        htemp = self.normalization(htemp.to_frame(), 242*3)
        htemp = htemp.rolling(3, min_periods = 2).mean()
        htemp = self.normalization(htemp)
        
        htemp.columns = [columnname]
        htemp[htemp<=-0.5] = np.nan
        return htemp