...
import pandas as pd
import numpy as np
class wyc_ts53_spot(FactorGenerator):
    def __init__(self):

        required_columns=['close_spot','volume_spot','position']
        lookback_bars=2000
        super(wyc_ts53_spot, self).__init__(
    ...
    def on_bar(self, df):
        ...
        N = 10
        turnover = df.volume_spot / df.position
        returns = df.close_spot.pct_change(fill_method=None)
