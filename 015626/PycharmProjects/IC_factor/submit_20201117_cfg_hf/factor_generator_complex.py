...
class FactorGeneratorComplex:
    ...
    def __init__(self, factor_name='test', lookback_bars=5000, required_columns=None,
                 savepath='/data/user/015626/data/share/factor/1min/IC_factors/hfcfg_test_20201117'):
    ...
    def prepare_hot_data(inst, start_date, end_date):
        ...
        data_dict = pd.read_pickle(os.path.join(data_path, 'IC_cfg_hf_insample.pkl'))
        data_dict.update(pd.read_pickle(os.path.join(data_path, 'IF_cfg_hf_insample.pkl')))
    ...
    def __callback__(self, start_date, end_date):
        ...
        if factor.columns.tolist[0].endswith('if'):
            factor['Ticker'] = 'IF.CFE'
        else:
