...
class wsc7_future_kpz(FactorGenerator):
    def __init__(self):
        super(wsc7_future_kpz, self).__init__(required_columns=['amount_if'],
                                              lookback_bars=2000)

    def on_bar(self, df):
        factor = df['amount_if'].to_frame()
        factor = ts_max(factor, 45)

        columnname = self.__class__.__name__
        factor.columns = [columnname]
        factor[columnname] = ts_rank(factor, 120*5)
