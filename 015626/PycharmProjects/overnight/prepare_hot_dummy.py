...
def prepare_edb_dummy(ref_date, last_num_days=20):
    ...
    out_path = os.path.join(trade_root, 'hot', end_date.strftime('%Y%m%d'), 'edb.h5')
...
def prepare_mdconstant_dummy(ref_date):
    ...
    out_path = os.path.join(trade_root, 'hot', ref_date.strftime('%Y%m%d'), 'mdconstant.h5')
    if not os.path.exists(os.path.dirname(out_path)):
        os.makedirs(os.path.dirname(out_path))
...
def prepare_misc_minute_dummy(ref_date):
    ...
    # get futures minute
    recent_month_contract = get_trade_contract(ref_date, ref_date, prod_id='IC.CFE').loc[ref_date, 'contract']
    ...
    futures_data = futures_data.loc[futures_data.index.get_level_values(level=1).str.contains(recent_month_contract)]
    ...
    out_path = os.path.join(trade_root, 'hot', ref_date.strftime('%Y%m%d'), 'misc_minute.h5')
    if not os.path.exists(os.path.dirname(out_path)):
        os.makedirs(os.path.dirname(out_path))
...
def prepare_alla_kline_1min(ref_date):
    ...
    out_path = os.path.join(trade_root, 'hot', ref_date.strftime('%Y%m%d'))
