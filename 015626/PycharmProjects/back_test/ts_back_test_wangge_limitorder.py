...
class TS_BACK_TEST:
    ...
    def back_test(self):
        ...
        for i in tqdm(range(1, length)):
            ...
            if (((stop_loss_flag and (now_hold_num != 0)) \
                or (now_hold_num_state * pre_target_pos_state == -1) \
                or (now_hold_num_abs > target_open_num_max)) \
                and (((now_hold_num_state == -1) and (last_close > low)) or ((now_hold_num_state == 1) and (last_close < high)))) \
                or ((((nowtime.hour == 14) and (nowtime.minute == 57)) or (
                ...
                # 两点半后 止损 当日开仓金额达到上限不开仓 限价单不满足条件不开仓
                if (open_contract_num <= 0) \
                        or ((nowtime.hour == self.hour_t) and (nowtime.minute >= self.minute_t)) \
                        or stop_loss_flag \
                        or (open_value_intraday > self.max_open_value) \
                        or ((pre_target_pos_state == 1) and (last_close <= low))\
                        or ((pre_target_pos_state == -1) and (last_close >= high)):
                    pnl_dict[nowtime] = now_hold_num_profit
    ...
    def prepare_data(self):
        ...
        t_days_list = udt.get_trading_date_range(str(df.index[0].date()).replace('-', ''),
                                                 str(df.index[-1].date()).replace('-', ''))
