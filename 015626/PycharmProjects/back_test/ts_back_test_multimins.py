...
class TS_BACK_TEST:
    ...
    def __init__(self, signal_df, ticker='IC.CFE', price_kind='vwap', long_in=0.5, long_out=0.5, short_in=-100,
                 short_out=-0.5,
                 signal_down_t=0.2, profit_down_t=0.002, signal_inout_diff=0.1, min_holding_period=5, stop_profit=100,
                 stop_loss=-100, open_limit_intraday=5, open_num_permin = 10, close_num_permin = 10, max_pos_state = 10,
                 max_hold_num = 100, initial_cash=100000000, c_rate=2.5 / 100000, slippage=0.6,
        ...
        :param open_num_permin: 每分钟开仓数量
        :param close_num_permin: 每分钟平仓数量
        :param max_pos_state: 单次最多开仓次数，空仓时pos_state为0, 每开仓一次pos_state+=1, 每平仓一次pos_state-=1
        :param max_hold_num: 最多持仓数量
        ...
        self.max_pos_state = max_pos_state
        ...
        for i in tqdm(range(1, length)):
            ...
            if signal != 0:
                ...
                if open_num_intraday >= self.open_limit_intraday:  # 超出单日开仓上限不开仓
                    signal = None
                if now_hold_num == self.max_hold_num:
                    signal = None
            # 如果当前没有仓位，看上一个分钟是否给出开仓信号，或者持仓没有达到上限
            if pos_state == 0:
            if (abs(pos_state) <= self.max_pos_state):
                if pre_signal == None:
                    pnl_dict[nowtime] = 0 # 此分钟收益为0
                    pre_signal = signal
                    pre_date = now_date
                    continue
                ...
                if pre_signal != 0:
                    ...
                    if self.max_hold_num == now_hold_num:
                        continue
                    elif self.max_hold_num - now_hold_num >= self.open_num_permin:
                        contract_num = self.open_num_permin
                    else:
                        contract_num = self.max_hold_num - now_hold_num
                    if len(trade_dict) == 0:
                        contract_num = min(contract_num, np.floor(self.initial_cash * self.leverage_rate / (self.face_value * open_price)))
                    else:

                    open_value = open_price * self.face_value * contract_num
                    ...
                    trade_dict[deal_count] = {'pos': pos_state, 'open_time': open_time, 'open_price': open_price,
                                              ...
                                              'close_contract_num': None, 'net_value': None, 'out_threshold': None,
            ...
            # 如果当前有持仓，看上一个信号是否要平仓
                ...
                if pre_signal == None:  # 没信号，但是此时要看是否止盈止损
                    ...
                    pnl_dict[nowtime] = nowprofit - close_fee # 此分钟盈亏应为此分钟收益减去手续费
                    # 每一次平仓说明完成了一笔交易
                    pos_state = 0
                    deal_count += 1
    ...
    def get_signal(self, pre_raw, raw, pos_state, pre_signal):
        if (pre_raw <= self.long_in) and (raw > self.long_in):
            return 1
        if (sign(pos_state) == 1) and (raw < self.long_out):
            return 0
        if (pre_raw >= self.short_in) and (raw < self.short_in):
            return -1
        if (sign(pos_state) == -1) and (raw > self.short_out):
            return 0
...
# 示例
'''
save_root_path = '/data/user/015626/data/share/factor/back_test/IC_ts/20200812/'
zsj_path = '/data/user/012315/share/ts/strategy/minute/res_lowcorr/base/lr_cla2.h5'

factor = pd.read_hdf(zsj_path)
idx = factor.index
...
factor_name = 'lr_cla2'
name_prefix = factor_name + '_origin_vwap_'
price_kind = 'vwap'
long_in = 0.5
long_out = 0.4
short_in = -0.5
short_out = -0.4
stop_loss=-0.01
open_limit_intraday = 5
save_path = os.path.join(save_root_path,factor_name+'_long_in'+str(long_in)+'_long_out'+str(long_out)+'_short_in'+str(short_in)+'_short_out'+str(short_out)+'_stop_loss'+str(stop_loss)+'_open_limit_intraday'+str(open_limit_intraday))
ts = TS_BACK_TEST(factor,price_kind=price_kind, long_in=long_in, long_out=long_out, short_in=short_in, short_out=short_out,
              stop_loss = stop_loss, signal_down_t = 0, profit_down_t = 0, signal_inout_diff = 0, min_holding_period = 0, open_limit_intraday=open_limit_intraday, save_path = save_path, name_prefix = name_prefix)
c =  ts.back_test()
'''