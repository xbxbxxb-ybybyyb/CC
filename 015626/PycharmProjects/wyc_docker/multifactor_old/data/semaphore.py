...
def sub_run(python_file, python_location=None, process_type='subprocess', **kwargs):
    ...
    if process_type == 'subprocess':
        return subprocess.run([python_location, python_file], shell=False, cwd=file_path, **kwargs)
...
class ZooKeeper:
    ...
    def fire(self, job_name, ftypes, python_file, process_type='subprocess', exit_failed=True, python_location=private_python_path, **kwargs):
        self.logger.info('*' * 30)
        ...
        if self.smp.wait(ftypes, self.date):
            ...
            handler = sub_run(python_file, python_location=python_location, process_type=process_type, **kwargs)
            if process_type == 'subprocess':
...
if __name__ == '__main__':
    ...
    # print(smp.wait(['fdd', 'md'], '20200101'))
