...
class TS_BACK_TEST:
    ...
    def __init__(self, signal_df, ticker='IC.CFE', price_kind='vwap',
                 ...
                 stop_loss=-100, open_limit_intraday=5, open_num_permin=10, close_num_permin=10, deal_volume_ratio=0.1,
    ...
    def back_test(self):
        ...
        for i in tqdm(range(1, length)):
            ...
            #             if (nowtime.hour == 14) and (nowtime.minute == 57):
            #                 if now_hold_num != 0:
            #                     print(nowtime, now_hold_num)
            ...
            target_open_num_max = np.floor(
                self.initial_cash * max(pre_target_pos) * self.leverage_rate / (self.face_value * open_price))
            target_open_num_min = np.floor(
                self.initial_cash * min(pre_target_pos) * self.leverage_rate / (self.face_value * open_price))
        ...
        results, daily_return = self.strategy_evaluate(pnl_df.copy())
    ...
    # 计算策略评价指标
    def strategy_evaluate(self, pnl):
        """
        :param trade: 每笔交易的df
        :return:
        """
