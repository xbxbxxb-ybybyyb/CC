...
class Order:
    def __init__(self, symbol=None, action=None, submit_price=None, fill_price=None, quantity=0, status=None,
                 create_sig=None, create_time=None, create_ticktime=None, finish_sig=None, finish_time=None,
                 finish_ticktime=None,
...
def strategy_evaluate(pnl, initial_cash, total_order_counts, cancel_order_counts):
    ...
    # 计算最大回撤开始时间
    start_date = \
        sharpedailyreturn[sharpedailyreturn['date'] <= end_date].sort_values(by='equity_curve', ascending=False).iloc[
            0][
            'date']
    # 将无关的变量删除
...
class TS_BACK_TEST:
    ...
    def back_test_singleday(self, date):
        ...
        for i in range(1, date_signal_lenth):
            ...
            #             print(nowtime)
            need_trade_num, put_order_status, iscancel_openorder, iscancel_closeorder = self.get_trade_num(
                order_submitted_list, pre_sig,
            ...
            if need_trade_num == 0:
                ...
                if put_order_status == 'open_pos':  # 当需要挂单开仓时
                    ...
                    _order = Order(symbol=contract, action=None, submit_price=open_price, quantity=1, status='submit',
                                   create_sig=pre_sig, create_time=nowtime, create_ticktime=tickdt_time,
                                   create_tick_pxlist=tick_pxlist)
        ...
        col_list = ['symbol', 'action', 'submit_price', 'fill_price', 'quantity', 'status', 'create_time',
                    'create_ticktime', 'create_sig', 'finish_time', 'finish_ticktime', 'finish_sig',
                    'create_tick_pxlist', 'finish_tick_pxlist']
        finishdf = [
            [x.symbol, x.action, x.submit_price, x.fill_price, x.quantity, x.status, x.create_time, x.create_ticktime,
             x.create_sig,
             x.finish_time, x.finish_ticktime, x.finish_sig, x.create_tick_pxlist, x.finish_tick_pxlist] for x in
            order_finished_list]
    ...
    def back_test(self):
        ...
        results, daily_return = strategy_evaluate(totaltrade_df, self.initial_cash, total_order_counts,
                                                  cancel_order_counts)
        ...
        return {'total_trade_detail': totaltrade_df, 'results': results, 'order_detail': trade_df,
