...
class wsc_ma1_short(FactorGenerator):
    def __init__(self):
        super(wsc_ma1_short, self).__init__(required_columns=['close'],
                                            lookback_bars=2000)
    ...
    def on_bar(self, data):
        ...
        close = data['close']
        close_ma_long = close.rolling(60, min_periods=30).mean()
        close_ma_short = close.rolling(10, min_periods=5).mean()
        factor = (close_ma_short - close_ma_long).rolling(10, min_periods=5).sum()
        factor = abs(factor - factor.rolling(600, min_periods=300).median())
