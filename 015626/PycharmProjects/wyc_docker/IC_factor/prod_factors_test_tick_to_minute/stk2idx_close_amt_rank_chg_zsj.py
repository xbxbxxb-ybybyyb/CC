...
class stk2idx_close_amt_rank_chg_zsj(FactorGeneratorComplex):
    def __init__(self):
        super(stk2idx_close_amt_rank_chg_zsj, self).__init__(required_columns=['close_zz500', 'amount_zz500'],
                                                     lookback_bars=2000)
    ...
    def on_bar(self, data):
        ...
        # factor logic
        rank_win = 240
        stk_amt_rank = calc_ts_pct(stk_amt,rank_win)
        stk2idx_amt_rank_raw = stk_amt_rank.mean(axis=1)
        stk_close_rank = calc_ts_pct(stk_close,rank_win)
        stk2idx_close_rank_raw = stk_close_rank.mean(axis=1)
        stk2idx_close_amt_rank_raw = stk2idx_close_rank_raw + stk2idx_amt_rank_raw

        short_win = 20
        long_win = 90
        ts_pct_win = 1200
        stk2idx_close_amt_rank_chg = calc_change_helper(stk2idx_close_amt_rank_raw,short_win,long_win,ts_pct_win)   

        factor = stk2idx_close_amt_rank_chg.to_frame()
