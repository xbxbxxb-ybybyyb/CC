...
class FactorGeneratorComplex:
    ...
    def prepare_hot_data(inst, start_date, end_date):
        ...
        tick2minute_data = IO.read_data([start_date, end_date], alt=os.path.join('/data/user/015626/data/share/MD/CHINA_FUTURES/TICK_TO_MINUTE/','MD_SIF_TICK_TO_MINUTE_MAIN.h5')).xs('IC.CFE', level=1).sort_index()
        ...
        cfg_stocks_data_if = IO.read_data([start_date, end_date],
                                       alt=os.path.join(data_path, 'IF_STOCKS_MINUTE_DATA.h5')).unstack().sort_index()

        data = pd.concat([index_data, futures_data, tick2minute_data], axis=1).sort_index()
        cfg = cfg_stocks_data.reindex(data.index)
        cfg_if = cfg_stocks_data_if.reindex(data.index)

        plist = ['open', 'high', 'low', 'close',
                 'open_spot', 'high_spot', 'low_spot', 'close_spot',
                 'open_zz500', 'high_zz500', 'low_zz500', 'close_zz500',
                 'open_hs300', 'high_hs300', 'low_hs300', 'close_hs300']

        for d in [data, cfg, cfg_if]:
