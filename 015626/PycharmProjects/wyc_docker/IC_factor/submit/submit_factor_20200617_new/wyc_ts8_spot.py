...
import pandas as pd
import numpy as np
class wyc_ts8_spot(FactorGenerator):
    def __init__(self):
        required_columns=['close_spot']
        lookback_bars=2000
        super(wyc_ts8_spot, self).__init__(
    ...
    def on_bar(self, df):
        ...
        factor = pd.DataFrame(
            np.where(df['close_spot'] - delay(df['close_spot'], 1) < 0, abs(df['close_spot'] - delay(df['close_spot'], 1)), 0),
