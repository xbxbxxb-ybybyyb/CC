...
class wyc_stix(FactorGenerator):
    def __init__(self):
        required_columns=['upclose', 'downclose']
        lookback_bars=2000
        super(wyc_stix, self).__init__(required_columns=required_columns,
                                  lookback_bars=lookback_bars)

    def on_bar(self, df):
        columnname = self.__class__.__name__
        factor = df.upclose / (df.upclose + df.downclose)
        factor = factor.ewm(40, adjust=False).mean()
        factor = mean(factor, 20).to_frame()
