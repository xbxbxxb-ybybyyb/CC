...
import pandas as pd
import numpy as np
class wyc_ts36_future(FactorGenerator):
    def __init__(self):

        required_columns=['high','low','vwap']
        lookback_bars=2000
        super(wyc_ts36_future, self).__init__(
                                  required_columns=required_columns,
                                  lookback_bars=lookback_bars)

    def on_bar(self, df):

        factor = ts_rank(delta((df.high + df.low + df.vwap), 60), 60)
