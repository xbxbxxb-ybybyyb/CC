...
import pandas as pd
import numpy as np
class wyc_ts12_spot(FactorGenerator):
    def __init__(self):
        required_columns=['close_spot']
        lookback_bars=2000
        super(wyc_ts12_spot, self).__init__(
                                  required_columns=required_columns,
                                  lookback_bars=lookback_bars)

    def on_bar(self, df):
        factor = (mean(df['close_spot'], 3) + mean(df['close_spot'], 6) + mean(df['close_spot'], 12) + mean(df['close_spot'], 24)) / 4
