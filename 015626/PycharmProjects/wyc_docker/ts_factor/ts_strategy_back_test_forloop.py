...
def back_test(signal_df, ticker, price_kind='twap', long_in=0.5, long_out=0.5, short_in=-100, short_out=-0.5,
              stop_profit=100, stop_loss=-100, initial_cash=10000000, c_rate=2.5 / 100000, slippage=0.6,
              leverage_rate=1, hour_t=14, minute_t=30):
    # def back_test(signal_df, ticker, paralist, price_kind='vwap', short_in=-100, short_out=-0.5,
    #              stop_profit=100, initial_cash=10000000, c_rate=2.5 / 100000, slippage=0.6,
    #              leverage_rate=1, hour_t=14, minute_t=30):
    #     long_in = paralist[0]
    #     long_out = paralist[1]
    #     stop_loss = paralist[2]
    """
    ...
    :return: 一个字典：'results': 策略评价指标,
            ...
            'monthly_return': 月度收益
    """
    df = prepare_data(signal_df, ticker=ticker)

    face_value = face_value_dict[ticker]
    ...
    for i in tqdm(range(1, length)):
        ...
        pre_raw = raw
        raw = df.iloc[i]['raw']
        signal = get_signal(pre_raw, raw, long_in, long_out, short_in, short_out)
        if signal != 0:
            if (nowtime.hour == hour_t) & (nowtime.minute > minute_t):
                signal = None
        ...
        if pos_state == 0:
            ...
            if pre_signal != 0:
                open_time = nowtime
                open_price = df.iloc[i][price_kind] + slippage * pre_signal
                contract_num = np.floor(initial_cash * leverage_rate / (face_value * open_price))
                # 开仓扣除手续费后, 我的账户余额以及合约共价值多少钱
                open_fee = open_price * face_value * contract_num * c_rate
                value = initial_cash - open_fee
                ...
                nowprofit = face_value * contract_num * (close - open_price) * pos_state
        ...
        # 如果当前有持仓，看上一个信号是否要平仓
            ...
            if pre_signal == None:  # 没信号，但是此时要看是否止盈止损
                pnl_df.loc[nowtime, 'profit'] = face_value * contract_num * (close - last_close) * pos_state  # 此分钟收益
                nowprofit = face_value * contract_num * (close - open_price) * pos_state

                nowvalue = value + nowprofit
                ratio = nowvalue / initial_cash - 1
                if (ratio >= stop_profit) | (ratio <= stop_loss):
            ...
            close_price = df.iloc[i][price_kind] - slippage * pos_state
            close_fee = close_price * face_value * contract_num * c_rate
            deal_profit = face_value * contract_num * (close_price - open_price) * pos_state
            ...
            nowprofit = face_value * contract_num * (close_price - last_close) * pos_state
    ...
    trade_df['change'] = trade_df.net_value / initial_cash - 1
    trade_df['equity_curve'] = trade_df.change.cumsum() + 1
    trade_df['holding_time'] = trade_df.apply(lambda x: get_timediff_minutes(x.open_time, x.close_time), axis=1)

    pnl_df = pnl_df.reset_index()
    pnl_df.columns = ['dt', 'profit']
    pnl_df['change'] = pnl_df['profit'] / initial_cash
    pnl_df['equity_curve'] = (pnl_df['profit'].cumsum() + initial_cash) / initial_cash

    results, daily_return, monthly_return = strategy_evaluate(pnl_df.copy(), trade_df.copy())
...
def prepare_data(signal, ticker):
    ...
    if len(signal.columns.tolist()) == 1:
        signal.columns = ['raw']
        ...
        md_twap = md_twap.xs(ticker, level=1)
        df = signal.join(md).join(md_twap)
    else:
        clist = ['raw'] + signal.columns.tolist()[1:]
        signal.columns = clist
        df = signal.copy()
...
def get_signal(pre_raw, raw, long_in=0.5, long_out=0.5, short_in=-10000, short_out=-0.5):
    if (pre_raw <= long_in) and (raw > long_in):
        return 1
    if (pre_raw >= long_out) and (raw < long_out):
        return 0
    if (pre_raw >= short_in) and (raw < short_in):
        return -1
    if (pre_raw <= short_out) and (raw > short_out):
        return 0
    return None


def get_timediff_minutes(a, b):
...
# 计算策略评价指标
def strategy_evaluate(pnl, trade):
    """
    :param trade: 每笔交易的df
    :return:
    """
    ...
    # ===计算年化收益
    #     annual_return = (pnl['equity_curve'].iloc[-1] / pnl['equity_curve'].iloc[0]) ** (
    #             '1 days 00:00:00' / (pnl['dt'].iloc[-1] - pnl['dt'].iloc[0]) * 365) - 1
...
# idx = factor.index
# factor = factor.loc[idx > datetime.datetime(2017,1,1,0,0,0)]
# idx = factor.index
# factor = factor.loc[idx < datetime.datetime(2019,8,1,0,0,0)]

# factor = factor.sort_index().to_frame()

# # 以下为可以在函数外读取好数据，避免函数内再读数据
# # md = IO.read_data([20170101, 20190801], columns=['open', 'high', 'low', 'close', 'vwap'],
# #                           alt='/data/user/015626/data/share/MD/CHINA_FUTURES/MINUTE/MD_STOCK_INDEX_FUTURES_MINUTE_MAIN.h5')
# # md = md.xs('IC.CFE', level=1)
# # factor = factor.join(md)

# c =  back_test(factor,'IC.CFE')
