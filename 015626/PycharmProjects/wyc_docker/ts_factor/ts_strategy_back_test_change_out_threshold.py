...
class TS_BACK_TEST:
    ...
    def __init__(self,signal_df, ticker, price_kind='vwap', long_in=0.5, long_out=0.5, short_in=-100, short_out=-0.5,
             stop_profit=100, signal_down_t = 0.2, profit_down_t = 0.002, signal_inout_diff = 0.1, min_holding_period = 5, stop_loss=-100, initial_cash=10000000, c_rate=2.5 / 100000, slippage=0.6,
    ...
    def back_test(self):
        ...
        for i in tqdm(range(1, length)):
            ...
            if holding_period <= self.min_holding_period:
                self.long_out = self.long_out - self.signal_inout_diff
                self.short_out = self.short_out + self.signal_inout_diff
            else:
            ...
            # 如果当前有持仓，看上一个信号是否要平仓
                ...
                trade_df.loc[deal_count, 'out_threshold'] = self.long_out if pos_state == 1 else self.short_out
    ...
    def prepare_data(self):
        ...
        if len(self.signal_df.columns.tolist()) == 1:
            ...
            df = self.signal_df.join(md).join(md_twap)
        else:
