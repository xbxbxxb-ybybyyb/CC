...
class FactorGeneratorComplex:
    ...
    def pd_writer(self, sig, savepath):
        ...
        if os.path.exists(file_name):
            sigold = IO.read_data([20100101,20210909],alt=file_name)
    ...
    def prepare_hot_data(inst, start_date, end_date):
        ...
        data_dict = pd.read_pickle(os.path.join(data_path, 'IF_cfg_data_insample.pkl'))
    ...
    def __callback__(self, start_date, end_date):
        data = self.slicer()
        savepath = os.path.join(self.savepath, 'IF_prod')
