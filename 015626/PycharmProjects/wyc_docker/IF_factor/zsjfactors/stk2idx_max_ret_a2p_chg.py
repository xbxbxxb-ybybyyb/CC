...
class stk2idx_max_ret_a2p_chg(FactorGeneratorComplex):
    def __init__(self):
        super(stk2idx_max_ret_a2p_chg, self).__init__(required_columns=['close_zz500', 'amount_zz500'],
                                                     lookback_bars=2000)
    ...
    def on_bar(self, data):
        ...
        min_size = 0.8
        ret_win = 60
        get_top_mean_x = partial(get_top_mean, top_n = top_n, min_size = top_n)
        stk_max_ret = stk_ret.rolling(ret_win,int(ret_win*0.5)).apply(get_top_mean_x)

        # factor logic
        stk_score = stk_max_ret.copy()
        stk_score_active = stk_score[active_mask]
        stk_score_inactive = stk_score[inactive_mask]
        stk2idx_score_active = stk_score_active.mean(axis=1)
        stk2idx_score_inactive = stk_score_inactive.mean(axis=1)
        stk2idx_score_a2p_raw = stk2idx_score_active - stk2idx_score_inactive

        short_win = 20
        long_win = 30
        ts_pct_win = 1200
        min_pct = 0.9
        stk2idx_max_ret_a2p_chg = calc_change_helper(stk2idx_score_a2p_raw,short_win,long_win,ts_pct_win)
        # ts_factor_quick(stk2idx_trade_strength, price, factor_name, layers=5)

        factor = stk2idx_max_ret_a2p_chg.to_frame()
