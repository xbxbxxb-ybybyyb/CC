...
def order_planner(auction_amount, stock_price, tick_deal_ratio=0.2, target_amount=3E6, order_freq=100,
                  min_order_size=100):
    ...
    TICK_AUCTION_RATIO = {1: 0.5617, 2: 0.2053, 3: 0.1326, 4: 0.1178, 5: 0.0986, 6: 0.0778, 7: 0.0725, 8: 0.0729,
                          9: 0.0762, 10: 0.0772,
                          11: 0.078, 12: 0.0734, 13: 0.0719, 14: 0.0683, 15: 0.0634, 16: 0.0594, 17: 0.0598, 18: 0.0579,
                          19: 0.0575,
                          20: 0.0614}
...
while deal_vol < np.sum(order_list) and order_idx < len(order_list) and deal_vol < target_vol:
    ...
    for pv in t_px_vol:
        p = pv[0]
        v = pv[1]
        _deal_vol = min(res_vol, v * tran_deal_ratio)
    ...
    for j_submit in range(len(submit_list)-1, -1, -1):
        ...
        for i_deal in range(len(t_px_vol_for_submit_order)):
            if round(t_px_vol_for_submit_order[i_deal][0], 2) <= round(submit_list[j_submit][0], 2):
                _deal_vol = min(t_px_vol_for_submit_order[i_deal][1], submit_list[j_submit][1])
    ...
    # 撮合完毕，继续挂单
    if len(px_qty_list) > 0:
        submit_list += [[px_qty_list[-1][0] + i*0.01, order_list[0]] for i in price_index_list]

    order_idx += 1
    tran_idx += 1
result = pd.DataFrame([date, stock, deal_money, deal_vol, np.sum(order_list), submit_deal_money, submit_deal_vol], index = ['dt','Ticker','deal_money','deal_vol', 'target_vol', 'submit_deal_money', 'submit_deal_vol']).T