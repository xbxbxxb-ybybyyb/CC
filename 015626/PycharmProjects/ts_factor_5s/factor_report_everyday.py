...
from factorlib_test import FactorLibTest
...
def generate_plot(df, plot_name, x_label, y_label, plot_type='line', c_width=None, c_height=None):
    ...
    if c_width is not None and c_height is not None:
        df_plot = df.plot(kind=plot_type, stacked=stack, figsize=(c_width * 3, c_height * 3), legend=legend_on,
                          fontsize=14)
    else:
        df_plot = df.plot(kind=plot_type, stacked=stack, figsize=(fig_width * 3, fig_height * 3), legend=legend_on,
                          fontsize=14)
...
def generate_plot_with_secondary(df1, df2, plot_name, x_label, y_label1, y_label2, plot_type='line'):
    ...
    df2_plot = df2.plot(ax=ax, kind=plot_type, figsize=(fig_width * 3, fig_height * 3), fontsize=14, secondary_y=True,
                        style='--')
...
def generate_pdf(excel_name, output_folder):
    ...
    top_trail_max_q_cumsum1 = max_q_df[top_trail_max_q1.index].iloc[
                              -holding_period - show_max_trail1:-holding_period].fillna(0).cumsum()

    top_trail_ic1 = trail_ic['last %d days' % (show_max_trail1)].sort_values(ascending=False).iloc[:top_line_num]
    top_trail_ic_cumsum1 = ic_ts_df[top_trail_ic1.index].iloc[-holding_period - show_max_trail1:-holding_period].fillna(
        0).cumsum()

    top_trail_max_q = trail_max_q['last %d days' % (show_max_trail)].sort_values(ascending=False).iloc[:top_line_num]
    top_trail_max_q_cumsum = max_q_df[top_trail_max_q.index].iloc[
                             -holding_period - show_max_trail:-holding_period].fillna(0).cumsum()

    top_trail_ic = trail_ic['last %d days' % (show_max_trail)].sort_values(ascending=False).iloc[:top_line_num]
    top_trail_ic_cumsum = ic_ts_df[top_trail_ic.index].iloc[-holding_period - show_max_trail:-holding_period].fillna(
        0).cumsum()
    ...
    g_top_trail_max_q_cumsum = generate_plot(top_trail_max_q_cumsum,
                                             'Cum Excess Return - Top %d Factors (last %d days)' % (
                                             top_line_num, show_max_trail),
                                             x_label='', y_label='CumRet', plot_type='line', c_width=7, c_height=2)
    g_top_trail_ic_cumsum = generate_plot(top_trail_ic_cumsum,
                                          'Cum IC - Top %d Factors (last %d days)' % (top_line_num, show_max_trail),
                                          x_label='',
                                          y_label='Cum IC', plot_type='line', c_width=7, c_height=2)

    g_top_trail_max_q_cumsum1 = generate_plot(top_trail_max_q_cumsum1,
                                              'Cum Excess Return - Top %d Factors (last %d days)' % (
                                              top_line_num, show_max_trail1),
                                              x_label='', y_label='CumRet', plot_type='line', c_width=7, c_height=2)
    g_top_trail_ic_cumsum1 = generate_plot(top_trail_ic_cumsum1,
                                           'Cum IC - Top %d Factors (last %d days)' % (top_line_num, show_max_trail1),
                                           x_label='',
                                           y_label='Cum IC', plot_type='line', c_width=7, c_height=2)

    g_positive_excess_ret_roll = generate_plot(positive_excess_ret_roll,
                                               'Number of Factors with Positive Excess Return', x_label='',
...
def get_result(edate, output_folder):
    print('dfafda')
    rdict_IF_prod = {}
    rdict_IC_prod = {}
    for i in [0, 4, 19, 59, 119, 239]:
        r = FactorLibTest('IF_prod', ticker='IF.CFE',
                          start_date=int(str(udt.get_trading_day_offset(edate, -1 * i)[0])[:10].replace('-', '')),
                          end_date=edate).run()
        rdict_IF_prod[str(i + 1) + ' tdays'] = [len(r),
                                        round(r['IC-1min'].fillna(value=0).mean(), 3),
                                        round(r['sharpe_Q3-Q0'].fillna(value=0).mean(), 3),
                                        len(r[r['ret_per_deal'] > 0]), len(r[r['ret_per_deal'] < 0]),
                                        len(r[r['ret_per_deal'].isna()])]

        r = FactorLibTest('IC_prod', ticker='IC.CFE',
                          start_date=int(str(udt.get_trading_day_offset(edate, -1 * i)[0])[:10].replace('-', '')),
                          end_date=edate).run()
        rdict_IC_prod[str(i + 1) + ' tdays'] = [len(r),
                                                round(r['IC-1min'].fillna(value=0).mean(), 3),
                                                round(r['sharpe_Q3-Q0'].fillna(value=0).mean(), 3),
                                                len(r[r['ret_per_deal'] > 0]), len(r[r['ret_per_deal'] < 0]),
                                                len(r[r['ret_per_deal'].isna()])]

    result_IF_prod = pd.DataFrame(rdict_IF_prod, index=['count', 'IC', 'sharpe', 'profit count', 'loss count', 'no deal count'])
    result_IC_prod = pd.DataFrame(rdict_IC_prod, index=['count', 'IC', 'sharpe', 'profit count', 'loss count', 'no deal count'])
    ...
    summary_IC_prod_title = Paragraph('IC_prod Surveillance Report', heading_size)
    summary_IF_prod_title = Paragraph('IF_prod Surveillance Report', heading_size)


    pprint('Generating tables')

    col_type = ['dcm0', 'dcm3', 'dcm3', 'dcm0', 'dcm0']
    summary_IC_prod_df = generate_table(result_IC_prod, col_type, axis=0, reformat_type=True)
    summary_IF_prod_df = generate_table(result_IF_prod, col_type, axis=0, reformat_type=True)

    elements = []

    elements.append(summary_IC_prod_title)
    elements.append(summary_IC_prod_df)
    elements.append(summary_IF_prod_title)
    elements.append(summary_IF_prod_df)

    pprint('Generating pdf')
    pdf_path = os.path.join(output_folder, str(datetime.datetime.now())[:19].replace(':','') + '_factor_report.pdf')
    doc = SimpleDocTemplate(pdf_path, pagesize=letter, topMargin=0.8 * inch,
                            bottomMargin=0.8 * inch)
...
get_result(20201225, '/data/user/015626/data/share/alpha/CHINA_FUTURES/MINUTE/factor_report/factor_report_pdf/')
