...
public class Saturn_t931_wd_cs1_amt_d_sum
extends BaseFactor {
    private final Set<String> stocksFiltered;

    public Saturn_t931_wd_cs1_amt_d_sum(SaturnMarketDataManager marketDataManager, Map<String, Double> factorValueMap) {
        super(marketDataManager, factorValueMap);
        this.factorName = new String[]{"saturn_t931_wd_cs1_amt_d_sum"};
    ...
    public void calculate() {
        ...
        if (null != currentLastTick && (totalValueTradeSum = this.stocksFiltered.stream().mapToDouble(stock -> {
            Tick lastTick = (Tick)lastTickMap.get(stock);
            if (null == lastTick || lastTick.getLastPx() <= 0.0) {
                return 0.0;
            }
            return lastTick.getTotalValueTrade();
        }).sum()) != 0.0) {
            factorValue = currentLastTick.getTotalValueTrade() / totalValueTradeSum;
        }
