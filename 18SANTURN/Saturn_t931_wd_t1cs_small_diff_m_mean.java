...
public class Saturn_t931_wd_t1cs_small_diff_m_mean
extends BaseFactor {
    private Set<String> stockSet = new HashSet<String>();

    public Saturn_t931_wd_t1cs_small_diff_m_mean(SaturnMarketDataManager marketDataManager, Map<String, Double> factorValueMap) {
        super(marketDataManager, factorValueMap);
        this.factorName = new String[]{"saturn_t931_wd_t1cs_small_diff_m_mean"};
        ...
        for (String stock : this.stockSet) {
            ...
            if (buyMap != null && !buyMap.isEmpty()) {
                for (OrderInfo orderInfo : buyMap.values()) {
                    if (!(orderInfo.getAmt() < 100000.0)) continue;
            ...
            double pct = (amtBuySum - amtSellSum) / totalAmt;
            pctSum += pct;
            pctCnt += 1.0;
