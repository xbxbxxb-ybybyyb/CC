...
public class Saturn_t931_sss_t1mcs_1peramt_buy_min
extends BaseFactor {
    private Set<String> stockSet = new HashSet<String>();

    public Saturn_t931_sss_t1mcs_1peramt_buy_min(SaturnMarketDataManager marketDataManager, Map<String, Double> factorValueMap) {
        super(marketDataManager, factorValueMap);
        this.factorName = new String[]{"saturn_t931_sss_t1mcs_1peramt_buy_min"};
    ...
    public void calculate() {
        ...
        for (String stock : this.stockSet) {
            HashSet<Long> buyNo = new HashSet<Long>();
            double amtSum = 0.0;
            for (Trade trade : this.marketDataManager.getCsTradeMap().get(stock)) {
                if (!(trade.getTurnover() > 0.0) || !TimeUtil.UDateToLocalTime(trade.getTimestamp()).isAfter(localTime)) continue;
                buyNo.add(trade.getTradeBuyNo());
                amtSum += trade.getTurnover().doubleValue();
            }
            if (buyNo.isEmpty()) continue;
            double pct = amtSum / (double)buyNo.size();
            if (pct < min) {
                min = pct;
            }
