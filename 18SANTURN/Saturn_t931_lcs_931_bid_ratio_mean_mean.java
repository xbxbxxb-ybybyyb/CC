...
public class Saturn_t931_lcs_931_bid_ratio_mean_mean
extends BaseFactor {
    private final String currentSymbol;
    private final Set<String> stocksFiltered;

    public Saturn_t931_lcs_931_bid_ratio_mean_mean(SaturnMarketDataManager marketDataManager, Map<String, Double> factorValueMap) {
        super(marketDataManager, factorValueMap);
        this.factorName = new String[]{"saturn_t931_lcs_931_bid_ratio_mean_mean"};
    ...
    private double calcUlTotBidByTime(Map<String, TreeMap<Long, Double>> allBidAmtMap, Long time) {
        return allBidAmtMap.values().stream().mapToDouble(timeToBidAmt -> this.calcBidAmtByTime((TreeMap<Long, Double>)timeToBidAmt, time)).filter(x -> !Double.isNaN(x)).average().orElse(0.0);
    }
    ...
    private double calcBidAmtByTime(TreeMap<Long, Double> timeToBidAmt, Long time) {
        ...
        return null == entry ? Double.NaN : entry.getValue();
    }
}
